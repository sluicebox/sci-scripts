...
(instance useHammer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gGlobalSound3 number: 828 loop: 1 vol: 127 play: self)
			)
			(4
			...
			(7
				(gGlobalSound3 number: 828 loop: 1 play: self)
			)
			(8
				(gEgo cel: 0 setCycle: End self)
			)
...
(instance rescue of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(PrintDC 86 3 #at 10 140 #width 280 #dispose) ; "I told you I'd repay your kindness when you saved me from that horrible cat. Good luck, friend."
				(= seconds 11)
			)
			(11
				(cls)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (not local36)
						(PrintDC 86 8) ; "A rusty lock secures the door preventing Graham from leaving the cellar."
					else
						(HandsOff)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 86 11) ; "That won't help Graham unlock the door."
							(event claimed: 1)
						)
					)
				)
