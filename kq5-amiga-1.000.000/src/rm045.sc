...
(instance rm045 of KQ5Room
	...
	(method (doit &tmp temp0 temp1 temp2)
		...
		(cond
			...
			((and (not (IsFlag 89)) (== global361 45) (gEgo has: 18)) ; Beeswax
				(self setScript: boatHereGraham)
			)
			(
				(and
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 45 0) ; "Jagged rocks surround the waterfall at the base of the cliff and then continue on into the ocean."
					(event claimed: 1)
				)
			)
		)
...
(instance beach of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 45 1) ; "The narrow beach at the base of the steep, rocky cliff widens here somewhat, while the chilly ocean continues to beat relentlessly upon it."
					(event claimed: 1)
				)
			)
		)
...
(instance waterFall of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 45 2) ; "A cascading waterfall, though very beautiful, nevertheless cuts off further travel to the north."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 45 3) ; "There is little that Graham can do about the waterfall."
					(event claimed: 1)
				)
			)
		)
...
(instance boatHereGraham of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(SetFlag 89)
				(proc0_28 75 45 4 67 10 10 25 8) ; "Look Graham, there is a boat here! Maybe we can use it!"
				(= cycles 1)
			)
			(2
				(self dispose:)
