...
(instance leaveRoom of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(joeBob view: 370 loop: 0 cel: 0 setCycle: CT 6 1 self)
			)
			(6
				(if (OneOf gDay 4 5)
...
(instance csPlate of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			(if (gPqFlags test: 44)
				(gCurRoom setScript: lookPlateScr)
			else
				(gMessager say: 7 4 1 1) ; "First, you need to gain entry into the yard."
			)
		else
...
(instance lookPlateScr of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 14 1 0 1 self) ; "Looking at the license plate, you discover...."
			)
			(2
				(gGame handsOn:)
				(gCurRoom setInset: csPlateInset self)
			)
			(3
				(gGame handsOff:)
				(gCurRoom drawPic: -1)
...
(instance joeBob of Actor
	...
	(method (doVerb theVerb)
		(if (== theVerb 27) ; badge
			(cond
				((gPqFlags test: 44)
					(gMessager say: 3 27 17 0) ; "It's unnecessary to show him your badge. You already checked in with the front office."
				)
				(local25
