...
(instance rm745 of Room
	...
	(method (init)
		...
		(couch init:)
		(armchair init:)
		(theWindow init: approachVerbs: 1 4 2 45 61) ; Look, Do, Talk, matches, casePuttyKnife
		(drapes1 init:)
		(drapes2 init:)
		(pillows init:)
		(floor init:)
		...
		(if (== gPrevRoomNum 710)
			...
			(theWindow approachVerbs: 1 4 2 45 61) ; Look, Do, Talk, matches, casePuttyKnife
			(gMouseDownHandler addToFront: self)
			(gGame handsOn:)
		)
	)
			...
			(
				...
				(cond
					((InRect 0 4 4 127 (event x:) (event y:))
						(gMessager say: 13 49 0 0) ; "Proceed at your own risk, Detective..."
						(event claimed: 1)
						(return)
					)
					((InRect 305 0 319 72 (event x:) (event y:))
						(gMessager say: 14 49 0 0) ; "The coast is clear, Carey."
						(event claimed: 1)
						(return)
					)
					(else
...
(instance shrinkCode of Code ; UNUSED
	(properties)
		...
		(switch (= state newState)
			...
			(1
				(gEgo
					...
					cel: 0
					y: 126
					setScaler: Scaler 108 108 151 120
					setCycle: End self
				)
			)
			(2
...
(instance throwBallMid of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(Load rsVIEW 741)
				(dog view: 741 loop: 2 cel: 0 setCycle: End self)
				(ourBall setMotion: 0)
			)
			(12
				(Load rsVIEW 746)
				(dog view: 746 loop: 0 cel: 0 setCycle: End self)
			)
			(13
				(Load rsVIEW 740)
		...
		(switch theVerb
			(1 ; Look
				(cond
					...
					((gPqFlags test: 126)
						(gMessager say: noun theVerb 1 0) ; "A gruesome sight... The dog didn't survive the fall."
					)
					(else
						(super doVerb: theVerb)
					)
