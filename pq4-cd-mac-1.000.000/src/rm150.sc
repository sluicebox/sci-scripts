		...
		(switch theVerb
			(1 ; Look
				(if
					...
					(gMessager say: noun theVerb 6 0) ; "They're just empty freight containers. This field must be used to unpack stolen goods..."
				)
			)
			(4 ; Do
...
(instance boxScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(16
				(if (not register)
					(gMessager say: 6 4 0 0 self) ; "All that's left is packing material..."
				else
					(= cycles 2)
				)
			)
...
(instance drawOnRopeScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gMessager say: 13 22 0 0 self) ; "It's a snake! No... it's a rope. Put the gun away, Carey."
			)
			(3
				(= cycles 2)
			)
	...
	(method (doVerb theVerb)
		(if (== theVerb 4) ; Do
			(cond
				...
				((localproc_0)
					(gMessager say: noun theVerb 4) ; "You've already collected all the slugs that were here, Carey."
				)
				(else
		...
		(switch theVerb
			(4 ; Do
				(if (gPqFlags test: bulletFlag)
					(gMessager say: noun theVerb 2) ; "Careful, Detective, the slug could drop into the grass."
				else
					(super doVerb: theVerb &rest)
				)
			)
			(61 ; casePuttyKnife
				(if (gPqFlags test: bulletFlag)
					(gMessager say: noun theVerb 2) ; "The slug is already loose. Be careful, Carey, you could lose it in the grass below."
				else
					(gGame points: 1)
					(gMessager say: noun theVerb 3 0 gCurRoom) ; "You loosen the slug from the wall."
			...
			(13 ; caseBaggies
				(if (gPqFlags test: bulletFlag)
					(switch (++ local0)
						(1
							(sfx number: 1051 loop: 1 play:)
						)
						(2
							(sfx number: 1052 loop: 1 play:)
						)
						(else
							(sfx number: 1052 loop: 1 play:)
						)
					...
					(gMessager say: 3 13 3) ; "To collect the slugs, they first have be loosened from the wall."
				)
			)
			(else
				(super doVerb: theVerb &rest)
		...
		(switch theVerb
			(52 ; notebook
				(if (gPqPointFlags test: 33)
					(gMessager say: noun theVerb 1 0) ; "This information has already been notated, Detective."
				else
					(gGame points: 2 33)
					(super doVerb: theVerb)
				)
