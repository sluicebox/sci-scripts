...
(instance rm160 of LLRoom
	...
	(method (init)
		...
		(if gDebugging
			(gEgo get: 16 get: 14) ; ribbon, hammer
		)
		...
		(super init:)
		(if (!= (gTheMusic2 number:) 800)
			(gTheMusic2 number: 800 loop: -1 vol: 127 flags: 1 play:)
		)
		(cond
		...
		(eastWindow setCel: (and (IsFlag 0) 4) init:) ; fAlleyWindowBroken
...
(instance sFall of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo
					egoSpeed: (+ gLarrySpeed global101)
					normal: 0
					view: 164
					setLoop: 4
					setCel: 0
					...
					(switch invItem
						...
						(else
							(if (IsObject (Inv at: invItem))
								(gEgo put: invItem 0)
								(Printf ; "Believing %s will be of no further use, you toss it into the dumpster, never to be seen again!"
									160
									31
			...
			(switch theVerb
				(3 ; Do
					(cond
						...
						((< (gEgo y:) 100)
							(Print 160 34) ; "Try moving to the west end of the fire escape."
						)
						(else
							(Print 160 35) ; "There's no way to reach the ladder from down here. It's only accessible from above."
						)
					)
				)
				(4 ; Inventory
						...
						(16 ; ribbon
							(cond
								(
									...
									(Print 160 36) ; "You already ARE tied to the railing."
								)
								(
									(and
										(== (gEgo view:) 162)
								...
								(
									...
									(Print 160 37) ; "You'll be better off if you leave the ribbon exactly where it is."
								)
								((< (gEgo y:) 100)
									(Print 160 38) ; "Good idea. But first, tie the ribbon to yourself."
								)
								(else
									(Print 160 39) ; "It's not long enough to reach from here."
									(Print 160 40 #at -1 140) ; "(Of course, you've heard THAT line before!)"
								)
							)
						)
						(11 ; pocketKnife
...
(instance hookerWindowF of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (< (gEgo y:) 100)
					(Print 160 41) ; "Looking inside the left window, you see the room you just left."
				else
					(Print 160 42) ; "There are two windows high above you. The left window near the fire escape has a dim yellow glow; the other window is dark. There may be something on the sill of the dark window, but you can't quite make it out from way down here."
				)
			)
			(3 ; Do
				(if (< (gEgo y:) 100)
					(if
						...
						(Print 160 43) ; "You're currently all tied up!"
					else
						(HandsOff)
						(gCurRoom setScript: sToHooker)
					)
				else
					(Print 160 44) ; "There's just no way to reach the windows from here. They're only accessible from above."
				)
			)
			(4 ; Inventory
				(switch invItem
					(14 ; hammer
						(if (< (gEgo y:) 100)
							(Print 160 45) ; "The window is already open. There's no use in breaking it!"
						else
							(Print 160 46) ; "There's no use throwing the hammer up there."
							(Print 160 47 #at -1 140) ; "(Besides, you probably just miss the window any way!)"
						)
					)
					(else
						(super doVerb: theVerb invItem)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
				...
				(if (< (gEgo y:) 100)
					(if (ObjInRoom 15)
						(if (and (== (gEgo view:) 162) (> (gEgo loop:) 1))
							(Print 160 48) ; "Looking at the dark window, you notice a bottle of pills sitting on the window ledge."
						else
							(Print 160 49) ; "There may be something in the east window, but from here, you're not sure."
						)
					else
						(Print 160 50) ; "You broke it, didn't you?"
					)
				else
					(hookerWindowF doVerb: theVerb invItem)
				)
			...
			(3 ; Do
				(cond
					((not (< (gEgo y:) 100))
						(Print 160 51) ; "There's no way to reach the window from here."
					)
					...
					((not (IsFlag 0)) ; fAlleyWindowBroken
						(Print 160 52) ; "You consider sticking your hand through a solid glass window, but realize that if you did, you wouldn't be able to play the violin."
						(Print 160 53 #at -1 140) ; "(Of course, you can't play the violin now, but you get the idea.)"
					)
					((!= ((gInventory at: 15) owner:) 160) ; pills
						(Print 160 54) ; "You already took the pills. Why go through all this again?"
					)
					(else
						(HandsOff)
					...
					(14 ; hammer
						(if (and (== (gEgo view:) 162) (> (gEgo loop:) 1))
							(if (IsFlag 0) ; fAlleyWindowBroken
								(Print 160 55) ; "You already smashed the window. Why do it again?"
							else
							...
							(Print 160 56) ; "You might try reaching over to the window first."
						)
					)
					(11 ; pocketKnife
						(Print 160 57) ; "You could never pry that window open with your pocket knife."
					)
					(4 ; ring
						(Print 160 58) ; "That's an idea; it's just not a good idea!"
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance fenceF of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 160 59) ; "The fence is much too tall for you to climb over."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance lidF of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 160 60) ; "Unfortunately the lid on the trash bin just will not close."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
