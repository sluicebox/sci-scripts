...
(instance rm300 of LLRoom
	...
	(method (init)
		...
		(aDoorLeft init: stopUpd: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
		(aDoorRight init: stopUpd: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
		(switch gPrevRoomNum
			(400
				(self style: 40)
				(gEgo y: 143)
		...
		(gEgo init:)
		(tit init: cycleSpeed: global101 setCycle: Fwd)
		(knee init: cycleSpeed: global101 setCycle: Fwd)
		(hip init: cycleSpeed: global101 setCycle: Fwd)
		(fountain init: cycleSpeed: global101 setCycle: Fwd)
		(fPlant init:)
		(fSign init:)
		(fLights init:)
		(statue init:)
		...
		(if (or gDebugging (ObjInRoom 3))
			(if (IsFlag 100)
				...
				(Load rsSOUND 302)
				(aAppleMan cycleSpeed: global101 moveSpeed: global101 init:)
				(appleHead cycleSpeed: global101 init:)
			)
			(ToggleFlag 100)
		)
...
(instance sAppleMan of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Printf 300 1 10) ; "Hello, sir," says the skinny young man in the barrel. "Would you help out a poor unfortunate by purchasing an apple? The price is reasonable: only $%d.00."
				(= seconds 7)
			)
			(6
...
(instance sBuyApple of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				2
				(if (> (gEgo distanceTo: aAppleMan) 30)
					(gEgo
						setCycle: Walk
						setMotion:
							PolyPath
			...
			(4
				...
				(cond
					...
					((== ((gInventory at: 3) owner:) gEgo) ; apple
						(Printf 300 6 10) ; "You know," says the man, peering deep into his barrel, "I'm all out! But thank you for your continued support. And for the $%d.00!"
						(= state 7)
					)
					(else
						(Printf 300 7 10) ; "Thank you, sir," says the skinny young man in the barrel. "For helping out a poor unfortunate by purchasing an apple at a reasonable $%d.00."
						(Points 51 3)
					)
				)
				(= cycles 1)
		...
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					...
					(1 ; breathSpray
						(Print 300 14) ; "No, thanks," he says, "I'm broke, not odiferous!"
					)
					(3 ; apple
						(Print 300 15) ; "I don't want that thing back!"
					)
					(5 ; whiskey
						(Print 300 16) ; "No, thanks, I don't drink!"
					)
					(7 ; rose
						(Print 300 17) ; "Sweet, but I'm busted! What do I need with a rose?"
					)
					(8 ; lubber
						(Print 300 18) ; "I'm selling apples, not cucumbers!"
					)
					(10 ; discoPass
						(Print 300 19) ; "I can't afford to buy a drink in THAT place!"
					)
					(11 ; pocketKnife
						(Print 300 20) ; "What do you think? I want to slash my wrists?"
						(Print 300 21) ; "No, thanks, Mister. I deal in cash only."
					)
					(12 ; wine
						(Print 300 16) ; "No, thanks, I don't drink!"
					)
					(14 ; hammer
						(Print 300 22) ; "You take a swing at the apple salesman, but miss."
						(Print 300 23 #at -1 140) ; "(You never were any good at hack-and-slash games!)"
					)
					(else
						(Print 300 24) ; "Sorry," replies the apple salesman to your generous offer, "but I'm broke, not stupid!"
					)
				)
			)
			(2 ; Look
				(if lookedOnce
					...
					(Print 300 25) ; "He's yet another poor soul who donated his everything so the shrine of Lost Wages could continue to flourish."
					(= lookedOnce 1)
				)
			)
			(5 ; Talk
				(Print 300 26) ; "Hey, Buddy," you joke. "How's your Pippin?"
				(Print 300 27) ; "He mutters under his breath, "(10,000 comedians out of work in this town...)""
			)
			(3 ; Do
				(aAppleMan setScript: sLookInBarrel 0 1)
			)
			(10 ; Zipper
				(Print 300 28) ; "Sorry, buddy, but this is MY night in the barrel!"
			)
			(11 ; Taste/Smell
				(Print 300 29) ; "He smells of decomposed apple cores and fresh pork rind."
			)
			(else
...
(instance aDoorRight of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 30) ; "Isn't it funny how every time you reach out to touch these doors, they open?"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance fountain of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 31) ; "That water sure is deep."
			)
			(10 ; Zipper
				(Print 300 32) ; "Use the restroom at Lefty's instead."
			)
			(11 ; Taste/Smell
				(Print 300 33) ; "The water tastes rather metallic and a little acidic."
			)
			(else
...
(instance fPlant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 34) ; "The plant feels like someone spit apple juice on it earlier!"
			)
			(5 ; Talk
				(Print 300 35) ; "Do you think talking to artificial plants will make them grow?"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fSign of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 36) ; "You can't reach that sign from down here!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fLights of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 37) ; "You can't reach the lights."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance statue of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 300 38) ; "You and the sculpture have absolutely no common interests."
			)
			(3 ; Do
				(Print 300 39) ; "It feels like metal."
			)
			(10 ; Zipper
				(Print 300 40) ; "Jeez, Larry; take a cold shower or something!"
			)
			(11 ; Taste/Smell
				(Print 300 41) ; "It tastes rather metallic."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
