...
(local
	...
	[openPts 38] = [262 0 262 151 242 151 242 146 251 146 251 143 213 133 81 133 3 153 3 184 315 184 314 158 293 152 273 151 273 0 319 0 319 189 0 189 0 0]
...
(instance rm350 of LLRoom
	...
	(method (init)
		...
		(doors cycleSpeed: (+ global101 1) init: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
		(elevator
	...
	(method (changeState newState &tmp [string 100])
		(switch (= state newState)
			...
			(1
				...
				(Print
					(Format @string 350 11 (if register {open} else {close})) ; "The penthouse elevator doors glide %s."
					#dispose
					#time
					5
					#at
