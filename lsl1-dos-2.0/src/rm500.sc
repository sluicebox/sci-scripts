...
(instance rm500 of LLRoom
	...
	(method (init)
		...
		(if
			(or
				gDebugging
				(and (ObjInRoom 11) (> (gEgo x:) 50) (!= gPrevRoomNum 505))
			)
			(if (IsFlag 99)
		...
		(switch theVerb
			...
			(4 ; Inventory
				...
				(switch invItem
					...
					(3 ; apple
						(Printf 500 45 invItem) ; "useVerb hit, with invItem = %d"
					...
					(else
						(Printf 500 49 ((Inv at: invItem) description:)) ; "That bum has no need for %s."
					)
				)
			)
			(else
