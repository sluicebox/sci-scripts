	...
	(method (init)
		...
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 0 0 319 0 319 112 150 112 135 102 117 99 82 97 46 99 15 108 18 126 32 126 32 139 0 139
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
		...
		(if (== ((gInventory at: 10) owner:) 330) ; discoPass
			(discoPass init: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
		)
	)
...
(instance theWindow of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 330 16) ; "Carefully feeling the panes, you decide this window is really a painting."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
