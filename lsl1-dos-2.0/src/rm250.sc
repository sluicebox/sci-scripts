...
(procedure (BackToRoom)
	...
	(if brokeHouse
		(Print 250 56) ; "This machine only contains $10,000.00; trust us, we'll mail you the rest."
	)
	(Printf 250 57 machineDollars) ; "You pocket your $%d.00 from the tray and wish you had won more."
	(gEgo z: 0 hide:)
	(deck dispose:)
...
(procedure (AskInsurance)
	...
	(if
		...
		(Animate (gCast elements:) 0)
		(return (Print 250 58 #button {Yes} 1 #button {No} 0)) ; "Do you want insurance?"
...
(procedure (ChangeDollars amount)
	...
	(Display 250 44 dsRESTOREPIXELS status2)
	(= status2
...
(procedure (HandOver)
	...
	(if (>= bet machineDollars)
		...
		(Display 250 44 dsRESTOREPIXELS status1)
		(= status1
			(Display
							...
							(while (!= ((= event (Event new:)) type:) evMOUSERELEASE)
								...
								(cond
									...
									((< bet 0)
										...
										(Print 250 45) ; "You must bet more than that!"
									)
									((mod bet 2)
										(if gModelessDialog
											(gModelessDialog dispose:)
										)
										(Print 250 46) ; "Please bet an even number of dollars (2, 4, 6, etc.). This machine doesn't pay half-dollars."
										(-- bet)
										(Display 250 44 dsRESTOREPIXELS status1)
										(= status1
											(Display
		...
		(if (self onMe: event) ; UNINIT
			(cond
				...
				((== handOver 0)
					...
					(Print 250 47) ; "Wait until this hand is over."
					(event claimed: 1) ; UNINIT
					(return)
				)
				(else
						...
						(evMOUSEBUTTON
							(while (!= ((= event (Event new:)) type:) evMOUSERELEASE)
								...
								(if (> bet 11)
									...
									(Printf 250 48 10) ; "The minimum bet in this machine is $%d.00."
									(break)
								)
								(event dispose:)
							)
			...
			(3 ; Do
				(cond
					(
						...
						(Print 250 49) ; "You're a little short of funds at the moment, Larry."
					)
					((== handOver 1)
						(if gModelessDialog
							(gModelessDialog dispose:)
					...
					(splitting
						(Print 250 50) ; "There's a limit of one split per screen!"
					)
					...
					(else
						...
						(Print 250 51) ; "You may only double when your hand contains two cards totaling 10 or 11."
					)
				)
			)
			(else
			...
			(0
				(cond
					...
					((and (== egoCardInc 1) (not splitting))
						...
						(Print 250 52) ; "You surrender your cards and are refunded half of your bet."
						(ChangeDollars (- (/ bet 2)))
						(HandOver)
					)
					(else
						...
						(Print 250 53) ; "It's too late now!"
					)
				)
				(self dispose:)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				...
				(Print 250 54 #at 15 -1 #width 280 #title {HOUSE RULES}) ; "This machine has a $10.00 minimum bet and a $10,000.00 limit. Bets must be a multiple of $2.00. Dealer must hit 16, and must stand on 17, unless he has an ace. Blackjack pays 3 to 2 odds. Any six cards under 22 wins."
				(Print 250 55 #at 15 -1 #width 280 #title {HOUSE RULES}) ; "Insurance bets are offered whenever the dealer shows an ace. They are half your bet, and pay 2 to 1 odds. You may split a pair or surrender whenever you have only two cards. You may double down if your cards total 10 or 11 points."
			)
			(else
				(super doVerb: theVerb &rest)
			)
