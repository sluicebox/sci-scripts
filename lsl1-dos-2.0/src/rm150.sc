...
(instance rm150 of LLRoom
	...
	(method (init)
		...
		(if (IsFlag 35)
			...
			(theHooker stopUpd:)
			(hookerHead cycleSpeed: global101 init: approachVerbs: 3 4 10 11 5) ; Do, Inventory, Zipper, Taste/Smell, Talk
		)
...
(instance sGetDressed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo cycleSpeed: (* 2 global101) setCycle: Beg self)
			)
			(1
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((== (gEgo view:) 803)
						(Print 150 25) ; "NOW!? You've come this far, Larry; now's the time to do more than just look!"
					)
					(else
						(gEgo setMotion: 0)
						(gCast eachElementDo: #hide)
						(herMouth cycleSpeed: gLarrySpeed init: z: 0)
						(eyesProp cycleSpeed: (+ global101 1) init: z: 11)
						(CloseupFeats)
			...
			(5 ; Talk
				(if (IsFlag 35)
					...
					(Print 150 28) ; "So," she snarls, "you just gonna stand there, or what?"
				)
			)
			(11 ; Taste/Smell
				(Print 150 29) ; "From here, her perfume smells cheap and overpowering."
				(Print 150 30 #at -1 140) ; "(Also from the next county, you suppose!)"
			)
			(10 ; Zipper
				(cond
					((and (== (gEgo view:) 803) (== (gEgo loop:) 0))
			...
			(4 ; Inventory
				(switch invItem
					(0 ; wallet
						(Print 150 31) ; "I don' need dat, Honey. Didn't ju pay downstairs?"
					)
					(2 ; watch
						(Print 150 32) ; "Dat's right, Honey," she gums, "yore time's runnin' out."
					)
					(4 ; ring
						(Print 150 33) ; "No commitments, Honey. This is a biz'nus deal!"
					)
					(6 ; remoteControl
						(Print 150 34) ; "You jes' gonna stand there and play with your li'l remote?"
					)
					(8 ; lubber
						(Print 150 35) ; "Yeah, them's a good idea. Lemme see that'n. Well, how 'bout dat! I didn't know they came in `Extra Small!'"
					)
					(9 ; candy
						(Print 150 36) ; "Naw, Nuggie, none for me. But I jes' had me a john that gimme a box a' candy jus' like that'n."
					)
					(10 ; discoPass
						(Print 150 37) ; "Why waste yore time at the disco? We kin do the horizontal bop right here!"
					)
					(12 ; wine
						(Print 150 38) ; "Honey, dinner an' drinks ain' necessary here!"
					)
					(13 ; magazine
						(Print 150 39) ; "I've seen better!"
					)
					(15 ; pills
						(Print 150 40) ; "I hardly need extrinsic motivation!"
					)
					(16 ; ribbon
						(Print 150 41) ; "No S. and M. here, buddy!"
					)
					(11 ; pocketKnife
						(Print 150 42) ; "Honey, you don' need no knife. You can assault me with YORE deadly weapon!"
					)
					(17 ; graffiti
						(Print 150 43) ; "He did? Ain't he the sweetest? He's always sendin' me bizness!"
					)
					(else
						(Print 150 44) ; "Say, Honey-chil', whacha come up here for--bartering?"
					)
				)
			)
			(else
...
(instance eyesProp of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 150 46) ; "What smoldering eyes!"
			)
			(else
...
(instance herMouth of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 150 47) ; "Look out! It's the "Jaws of Life!""
			)
			(else
				(herFace doVerb: theVerb invItem)
			)
...
(instance table of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (not (ObjInRoom 9))
					(Print 150 48) ; "It's a really ugly table."
				else
					(Print 150 49) ; "There is a box of candy on the hooker's table. Probably a gift from some admirer, you suppose."
				)
			)
			(3 ; Do
				(cond
					((not (ObjInRoom 9))
						(Print 150 50) ; "You can't take her table. She's nailed it to the floor."
					)
					((== (gEgo view:) 803)
						(Print 150 45) ; "NOW!?"
					)
...
(instance underwear of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 51) ; "Although you would look precious in that little white lace number, you're just not that kind of guy!"
			)
			(11 ; Taste/Smell
				(Print 150 52) ; "The hooker's underwear are not of the edible variety."
			)
			(4 ; Inventory
				(switch invItem
					(11 ; pocketKnife
...
(instance bed of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(11 ; Taste/Smell
				(Print 150 53) ; "The bed smells of stale cigarette smoke."
			)
			(else
...
(instance clothesline of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 54) ; "The clothesline is barely strong enough to hold its present light load. Besides, if you take it, her underwear would fall down."
				(Print 150 55 #at -1 140) ; "(And you know how em-bare-assing THAT would be!)"
			)
			(4 ; Inventory
				(switch invItem
					(11 ; pocketKnife
						(Print 150 56) ; "Although the pocket knife is sharp enough, you have no need to hassle this poor girl."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance windowsill of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 57) ; "You have no use for cigarette butts."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance herBreast of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Zipper
				(Print 150 58) ; "Yeah, Larry! You should be so lucky!"
			)
			(11 ; Taste/Smell
				(Print 150 59) ; "To you, they taste slightly salty."
			)
			(3 ; Do
				(Print 150 60) ; "Is that all you came up those stairs for, Honey? To cop a little feel?"
			)
			(5 ; Talk
				(Print 150 61) ; ""Mmmmmbbbblllemmm," you mumble, imagining your face buried in that valley."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance herFace of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 62) ; ""Is that all you want to do, Honey?" she asks."
			)
			(5 ; Talk
				(Print 150 4) ; ""Nice mandibles," you offer."
				(Print 150 5) ; ""You don' think I'm saggy?" she replies, no Rhodes scholar."
			...
			(10 ; Zipper
				(Print 150 63) ; "Hey, baby," she groans, "none o' dat. `Straight ahead' is all you get!"
			)
			(11 ; Taste/Smell
				(Print 150 64) ; "You recognize that cologne: "Eau de Toilette Cie't!""
			)
			(4 ; Inventory
				(theHooker doVerb: theVerb invItem &rest)
			)
...
(instance herCrack of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 65) ; "Get your hand out of the crack!"
			)
			(5 ; Talk
				(Print 150 66) ; "You can't imagine putting your tongue in THAT crack!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance bedpost of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 150 67) ; "No, there's no chewing gum losing its flavor on THAT bedpost."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
