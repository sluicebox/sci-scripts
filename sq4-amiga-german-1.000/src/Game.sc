...
(class Game of Obj
	...
	(method (save &tmp [comment 20] num oldCur oldPause)
		...
		(ScriptID 990)
		(= oldCur (gGame setCursor: gNormalCursor))
		(Sound pause: 1)
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Save doit: @comment)) -1)
				(= parseLang oldPause)
				(= oldCur (gGame setCursor: gWaitCursor 1))
				(if (not (SaveGame name num @comment gVersion))
					(Print 994 0 #font 0 #button {OK} 1) ; "Your save game disk is full. You must either use anotherdisk or save over an existing saved game."
				)
				(gGame setCursor: oldCur (HaveMouse))
			)
	...
	(method (restore param1 &tmp [temp0 20] temp20 temp21 temp22)
		...
		(Load rsFONT gSmallFont)
		(ScriptID 990)
		(= temp21 (gGame setCursor: gNormalCursor))
		(Sound pause: 1)
		(if (PromptForDiskChange 1)
			(if gModelessDialog
				(gModelessDialog dispose:)
			...
			(if (!= (= temp20 (Restore doit: &rest)) -1)
				...
				(if (CheckSaveGame name temp20 gVersion)
					...
					(Print 994 1 #font 0 #button {OK} 1) ; "That game was saved under a different interpreter. Itcannot be restored."
					(gGame setCursor: temp21 (HaveMouse))
					(= parseLang temp22)
				)
			else
				(= parseLang temp22)
...
(class SL of Obj
	...
	(method (doit &tmp theLine)
		(if code
			(= theLine (Memory memALLOC_CRIT (if 49 240 else 82)))
			(code doit: theLine)
