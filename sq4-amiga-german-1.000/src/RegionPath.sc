...
(class RegionPath of MoveTo
	...
	(method (nextRoom)
		...
		(if (and (!= currentRoom gCurRoomNum) (< (client z:) 1000))
			(client z: (+ (client z:) 1000) illegalBits: 0 ignoreActors: 1)
		)
		(client posn: x y)
	)
	...
	(method (at)
		(Printf 806 0 name name) ; "%s needs an 'at:' method."
		(return 0)
	)
	...
	(method (dispose)
		(if theRegion
			...
			(Print 806 1 name) ; "%s theRegion: not defined."
		)
	)
)
