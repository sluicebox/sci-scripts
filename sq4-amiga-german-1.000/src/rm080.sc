...
(instance rm080 of SQRoom
	...
	(method (init &tmp temp0)
		...
		(self style: temp0)
		((ScriptID 702 5) init: setLoop: 2 posn: 113 62) ; theDrip1
		(if (>= (gGame detailLevel:) 2)
			((ScriptID 702 5) setScript: daDripScript) ; theDrip1
		else
			((ScriptID 702 5) stopUpd:) ; theDrip1
		)
		((ScriptID 702 6) init: setLoop: 3 posn: 250 45 cycleSpeed: 1) ; theDrip2
		(if (>= (gGame detailLevel:) 2)
			((ScriptID 702 6) setCycle: Fwd) ; theDrip2
		else
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(6
				...
				(sewer status: 3)
				((ScriptID 702 2) setScript: (ScriptID 702 4) moveSpeed: 0) ; theSlime, theSlimeScript
				(self dispose:)
			)
		)
	)
