...
(procedure (localproc_0 param1 &tmp temp0)
	...
	(if (= temp0 (MakeMessageText 0 0 79 1 14)) ; "Now reading file "%s.""
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 temp4)
		(switch (= state newState)
			(0
				...
				(TextDialog (MakeMessageText 0 0 75 1 14) (Str with: global288)) ; "You've placed a bitmap of yourself and/or recorded lines in the game directory. We're about to read them into memory to test them. If the game crashes, or you see any error messages, please re-read the BEASTAR file and make sure your files are in the proper format. 1.bmp should be a 256-color Windows RGB uncompressed bitmap, about 325x325 pixels. The wave files (1-12) should be PCM (uncompressed) format, and no greater than 16 bit, 22kHz, mono. Please press enter to continue."
				(if local4
					(localproc_0 {1.bmp})
					(if (not (= temp0 (proc64896_20 325 325)))
						(= temp3 (MakeMessageText 0 0 78 1 14)) ; "An error occurred reading "%s". Please make sure it is a Windows RGB bitmap, uncompressed, 8-bit (256 color), about 325x325 pixels."
				...
				(if local3
					(for ((= temp4 1)) (<= temp4 12) ((++ temp4))
						(= temp1 (Str format: {%d.wav} temp4))
						(if (proc64896_21 temp4)
							(localproc_0 (temp1 data:))
							(oSimpleSound playSound: temp4)
							(oSimpleSound stop:)
						else
							(= temp3 (MakeMessageText 0 0 77 1 14)) ; "The wave file "%s" is missing. This will not cause a problem, but you won't hear your voice reading this line."
