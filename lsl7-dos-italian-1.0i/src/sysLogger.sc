...
(instance sysLogger of Code
	...
	(method (doit &tmp i j l c firstNote theDrv commented saveInfont str cfgPath thePath theToken QAinitials kbdDrvEntry joyDrvEntry videoDrvEntry soundDrvEntry mouseDrvEntry audioDrvEntry)
		...
		(= gInputFont gUserFont)
		(if (= firstNote (not (gSysLogPath size:)))
			(while (not (< 0 (thePath size:) 19))
				(Print
					font: gInputFont
					fore: (Palette 3 0 0 0) ; PalMatch
					back: (Palette 3 255 255 255) ; PalMatch
					addText:
						{Enter drive letter, path, and your name\n(no extension, max 40 characters)}
					addEdit: thePath 40 0 45
					init:
				)
			)
			(gSysLogPath copy: (thePath data:))
		)
		(thePath format: {%s.mem} (KArray 9 gSysLogPath)) ; ArrayGetData
		...
		(thePath format: {%s.log} (KArray 9 gSysLogPath)) ; ArrayGetData
		(if
			(and
				firstNote
				(or
		...
		(if (== -1 logHandle)
			...
			(Log
				...
				(Print
					...
					addButton: {Moderate} {MODERATE} 50 12
					addButton: {Minor} {MINOR} 125 12
					saveCursor: 1
					init:
				)
			)
			(Log
				...
				(Print
					...
					addButton: {No} {NO} 55 12
					addButton: {Intermittent} {INTERMIT.} 110 12
					saveCursor: 1
					init:
				)
			)
			...
			(Log
				...
				(Print
					...
					addButton: {Programming} {PROG} 55 12
					addButton: {Music} {MUSIC} 110 12
					addButton: {Design} {DESIGN} 165 12
					saveCursor: 1
					init:
				)
			)
