	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(PrintDC 6 3 #at 10 35 #dispose) ; "Frantic squeakings alert Graham to a mangy cat chasing a terrified rat."
			...
			(9
				...
				(PrintDC 6 4 #at 20 120 #width 270) ; "Oh, thank you, thank you, good sir! You saved my life! My children and I will never forget your kindness. Maybe someday I will be able to return the favor. Oh! I hear my children calling! Good-bye...until we meet again."
				(self cue:)
			)
			(10
				(gGlobalSound4 fade:)
			...
			(switch (event message:)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(8
							(if local0
								(PrintDC 6 13 #at 10 10) ; "Too late! The poor rat is already in the clutches of the wily cat."
							else
								(= local1 1)
								(gEgo put: 8 6) ; Shoe
								(SetScore 4)
...
(instance rat of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 6 14 #at 10 10) ; "A scroungy cat catches a large, grey rat in its wicked claws."
					(event claimed: 1)
				)
			)
		)
