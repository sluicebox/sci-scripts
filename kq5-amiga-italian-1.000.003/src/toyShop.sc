		...
		(switch (= state newState)
			(0
				(if (== (gGame detailLevel:) 3)
					...
					(toyMaker stopUpd:)
					(lArm show: loop: 5 setCel: 2 stopUpd:)
					(rArm show: loop: 6 x: 160 stopUpd:)
					(self dispose:)
				)
			)
			(1
			...
			(switch (event message:)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(4
							(proc0_28 160 204 62 67 10 10 25 6) ; "Can I buy the sled with a silver coin?"
							(proc0_28 192 204 58 67 10 10 25 8) ; "No, I'm sorry. I'd love to sell it to you, but I'm afraid I need a bit more than that."
							(event claimed: 1)
						)
						(else
...
(instance door of Door
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 63 #at 10 10) ; "Apparently the toymaker's son, working in the back room, is learning the toy business from his father."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 204 64 #at 10 10) ; "The back door is for employees only."
					(event claimed: 1)
				)
			)
		)
...
(instance fishBowl of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 65 #at 10 10) ; "A pet goldfish swims endlessly around its fishbowl."
					(event claimed: 1)
				)
			)
		)
...
(instance girl of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 66 #at 10 10) ; "A pretty little girl, who must be the toymaker's granddaughter, plays with a doll while Grandfather looks on."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 204 67 #at 10 10) ; "The little girl seems happy enough playing with the doll. She needs nothing more."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 204 68 #at 10 10) ; "The little girl is too interested in her doll. She pays no attention to Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 204 69) ; "The little girl only seems to care about the doll she's playing with."
							(event claimed: 1)
						)
					)
				)
...
(instance shopRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 70 #at 10 10) ; "A child's wonderland of toys fills this cute little toyshop."
					(event claimed: 1)
				)
			)
		)
