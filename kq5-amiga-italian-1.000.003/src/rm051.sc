...
(instance sailBoat of View
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 51 12) ; "Oh, dear! Now what?! Poor Graham's boat has been dashed to pieces against the jagged rocks of Mordack's island and is now completely useless."
					(event claimed: 1)
				)
				(3 ; Do
					(if (and (not (IsFlag 55)) (not local0))
		...
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (and (not (IsFlag 55)) (not local0))
						(++ local0)
						(PrintDC 51 13) ; "Unfortunately nothing can be done about the wrecked boat. It appears that Graham is stuck here."
						(event claimed: 1)
					)
				)
			)
...
(instance Cedric of Actor
	...
	(method (handleEvent event &tmp temp0)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 51 14) ; "A bit shaken, Cedric anxiously takes stock of their unsure situation."
					(event claimed: 1)
				)
				(5 ; Talk
					(if (not local3)
						(proc0_28 75 51 15 67 10 10 25 8) ; "I hate to say this, Graham, but...I don't like this place at all!"
						(proc0_28 160 51 16 67 10 10 25 5) ; "I know what you mean, Cedric."
						(= local3 1)
						(event claimed: 1)
					else
						(event claimed: 0)
...
(instance fishy of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 51 17) ; "A dead fish lies on the rocky beach at the foot of the stairs."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
