		...
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (not local0)
						(++ local0)
						(PrintDC 42 10) ; "Graham finds himself in a very helpless position!"
						(event claimed: 1)
					)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 42 11) ; "That's not going to work with a baby roc!"
							(event claimed: 1)
						)
					)
				)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (>= (hatch state:) 4)
						(PrintDC 42 12) ; "Graham finds himself in a terrible situation: dropped painfully in the middle of a huge roc's nest set high on a rocky ledge with no way of climbing down. And now the baby roc looks very hungry!"
					else
						(PrintDC 42 13) ; "Graham finds himself in a terrible situation: dropped painfully in the middle of a huge roc's nest set high on a rocky ledge with no way of climbing down. And, even worse...the roc's egg is hatching!"
					)
					(event claimed: 1)
				)
			)
