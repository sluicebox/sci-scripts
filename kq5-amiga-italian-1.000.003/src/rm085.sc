...
(instance dog of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 85 10) ; "A flea-bitten old dog lies asleep in the corner."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 85 11) ; "The dog is just out of Graham's reach."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 85 12) ; "Graham has nothing to say to a dog."
					(event claimed: 1)
				)
			)
		)
...
(instance backRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 85 13) ; "Through an open doorway, Graham can see part of the inn's dining room."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 85 14) ; "The inn's dining room is just a few steps away."
					(event claimed: 1)
				)
			)
		)
...
(instance room of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 85 15) ; "The country inn's tavern looks a bit shabby and run-down. At the bar, Graham notices three men."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(10
							(PrintDC 85 16) ; "Playing the harp has attracted the attention of the men at the bar."
							(++ local0)
