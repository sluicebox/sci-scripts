	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(chair loop: 0 cel: 0 setPri: (- (body priority:) 1) stopUpd:)
				(client setScript: 0)
			)
		)
	)
...
(instance giveCoinStand of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if local55
					(proc0_28 103 13 7 67 10 10 25 10) ; "Vell, eet eez not a gold coin, but I guess eet veel be fine. Go on een and see Madame Mushka."
				else
					(proc0_28 103 13 6 67 10 10 25 5) ; "You may see Madame Mushka now."
				)
				(gypsy
					...
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(11
							(if (gEgo has: 27) ; Amulet
								(proc0_28 103 13 17 67 10 10 25 10) ; "Madame Mushka eez tired. No more for today."
							else
						...
						(4
							(proc0_28 103 13 18 67 10 10 25 10) ; "Eet costs one GOLD coin to see Madame Mushka, not one SILVER coin!"
							(event claimed: 1)
						)
						(28
							(event claimed: 0)
				...
				(4 ; Inventory
					(if (not (gCast contains: gypsy))
						(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
							...
							(3
								(if (gEgo has: 27) ; Amulet
									(proc0_28 103 13 20 67 10 10 25 10) ; "Madame Mushka eez tired. No more for today."
								else
									(= local55 1)
									(gEgo put: 3 13) ; Golden_Needle
									(HandsOff)
							...
							(4
								(proc0_28 103 13 21 67 10 10 25 10) ; "Eet costs one GOLD coin to see Madame Mushka, not one SILVER coin!"
								(event claimed: 1)
							)
							(else
			...
			(switch (event message:)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(4
							(proc0_28 103 13 24 67 10 10 25 10) ; "Eet costs one GOLD coin to see Madame Mushka, not one SILVER coin!"
							(event claimed: 1)
						)
						(else
							(proc0_28 103 13 25) ; "Zat eez NOT a gold coin!"
							(event claimed: 1)
						)
					)
				)
...
(instance coals of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 26) ; "A steaming pot of stew bubbles over a small campfire near the gypsy wagon."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 27) ; "Obviously, the man wouldn't tolerate Graham taking his supper."
					(event claimed: 1)
				)
			)
		)
...
(instance grassRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 28) ; "Graham notices a grassy clearing at the edge of the woods."
					(event claimed: 1)
				)
			)
		)
...
(instance tambourine of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 29) ; "A tambourine lies on the ground near the abandoned gypsy encampment."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo loop: 1)
				(RedrawCast)
				(proc0_28 75 13 30 67 10 10 25 10) ; "There's nothing but a hot, dry desert further west. Most people avoid it, because there are bandits out there! If you insist on going, I'll wait for you HERE!"
				(= cycles 1)
			)
			(1
				(HandsOn)
