...
(class FPInvItem of InvI
	...
	(method (doVerb theVerb)
		(if (not (and (OneOf nameCase 29) (OneOf name 3 5 10 11 12 9)))
			(Message msgGET 15 72 0 nameCase nameSeq @local5)
			(gNarrator name: @local5 showTitle: 1)
		)
		(cond
			((== theVerb 1) ; Look
			...
			((not (OneOf theVerb 3 4 2 7 8)) ; ???, Do, Talk, ???, icon10
				(if (Message msgSIZE (+ view 1000) noun theVerb 0 1)
					(gMessager say: noun theVerb 0 0 0 (+ view 1000))
				else
					(gMessager say: 0 6 0 0 0 15) ; "Those two just don't go together."
				)
			)
			(else
...
(instance Med_1 of FPInvItem
	...
	(method (doVerb theVerb)
		(switch theVerb
			(else
				(Message msgGET 15 72 0 30 26 @local5)
				(gNarrator name: @local5 showTitle: 1)
...
(instance Correct_Rx of FPInvItem
	...
	(method (doVerb theVerb)
		(Message msgGET 15 72 0 29 6 @local5)
		(gNarrator name: @local5 showTitle: 1)
		(switch theVerb
			(else
				(super doVerb: theVerb &rest)
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			...
			(26 ; Charcoal
				...
				(gMessager say: noun theVerb 0 0 self 15) ; "You drop the charcoal into tin can."
				(gGame points: 3)
			)
			(27 ; Leather_Strap
				(if (IsFlag 55)
					...
					(gInventory curIcon: 0)
					(gMessager say: noun theVerb 0 0 self 15) ; "You slip the leather strap through the holes in the tin can. You always wanted a feed bag."
					(gGame points: 5)
				else
					(gMessager say: noun theVerb 21 0 0 15) ; "The leather strap won't fit in the tin can..."
				)
			)
			(1 ; Look
				(cond
					...
					((and (IsFlag 57) (IsFlag 55))
						(gMessager say: noun 1 18 0 0 15) ; "The hole-ridden tin can is chock full of charcoal."
					)
					((IsFlag 55)
					...
					((IsFlag 57)
						(gMessager say: noun 1 20 0 0 15) ; "This tin can is fulla charcoal."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance Snails of FPInvItem
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(33 ; Open_Beer
				(gMessager say: noun theVerb 0 0 self 15) ; "You pour a small amount of beer on the snails."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(44 ; Knife
				...
				(gMessager say: noun theVerb 0 0 self 15) ; "With your newly-acquired skills, you whittle the candle wax into the shape of an ear, one that should theoretically attach snugly to the small knot of cartilage that remains of your original ear."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(51 ; Bullets
				...
				(SetFlag 54)
				(gInventory curIcon: 0)
				(gMessager say: noun theVerb 0 0 self 15) ; "You slip the bullets in the chamber. All the old memories come rushing back to you!"
			)
			(50 ; Cleaning_Kit
				...
				(gInventory curIcon: 0)
				(if (IsFlag 54)
					(gMessager say: noun theVerb 27 0 self 15) ; "You remove the bullets from your old pistols. Then, with the cleaning kit, you lovingly and polish the pistols 'till they shine like a Coarsegold kid with his first passing grade. You remove a fair amount of rust from the barrel, a damn good thing or you might've had a disaster, and you replace the bullets in the chamber."
				else
					(gMessager say: noun theVerb 0 0 self 15) ; "With the Sheriff's cleaning kit, you lovingly clean and polish your old pistols 'till they're like new. You remove a fair amount of rust from the barrel, a damn good thing or you might've had a disaster on your hands!"
				)
				(gGame points: 5)
			)
			(4 ; Do
			...
			(1 ; Look
				(cond
					((and (IsFlag 4) (IsFlag 54))
						(gMessager say: noun 1 22 0 0 15) ; "Your pistols, cleaned and loaded for bear! You're cookin' now!"
					)
					((IsFlag 54)
						(gMessager say: noun 1 23 0 0 15) ; "Your old pistols, loaded with the Sheriff's surplus ammo."
					)
					((IsFlag 4)
						(gMessager say: noun 1 15 0 0 15) ; "Your old pistols, now cleaned and restored to their original working condition!"
					)
					(else
						(super doVerb: theVerb &rest)
					)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				...
				(gMessager say: noun theVerb 0 0 self 15) ; "You scrape the clay off and discard it, leaving you with a gleaming silver ear! Beautifully done!"
				(gGame points: 20)
			)
			(else
				(super doVerb: theVerb &rest)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(52 ; Clay
				...
				(gMessager say: noun theVerb 0 0 self 15) ; "You carefully pack the clay in and around the wax ear, leaving some of it open so that you can pour the wax out later."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				...
				(gNarrator name: @local5 showTitle: 1)
				(++ local1)
				(gMessager say: 58 4 24 0 self 15) ; "Say, let me show you a little trick with a borrowed neckerchief..."
			)
			(else
				(super doVerb: theVerb &rest)
			)
