...
(procedure (PrintD args &tmp arg theDialog theItem right bottom top left ret type theObj xDialog yDialog theTitle firstItem firstItemNum) ; UNUSED
	...
	(= theTitle 0)
	(= firstItemNum 0)
	((= theDialog (DText new:)) window: gSystemWindow)
	(for ((= arg 0)) (< arg argc) ((++ arg))
		(switch (= type [args arg])
			...
			(else
				...
				(switch type
					(23
						((= theItem (DItem new:)) text: [args arg])
					)
					(78
						((= theItem (DIcon new:))
							text: [args arg]
				...
				(theItem check: playBed: (+ right 4) (+ top 4))
				(theDialog add: theItem)
				(= right (theItem nsRight:))
			)
		)
	)
	(theDialog check: cycle:)
	(theDialog
		playBed:
			(if (== -1 xDialog)
				(theDialog nsLeft:)
	...
	(= ret (theDialog setMark: (if theTitle 4 else 0) -1 doit: firstItem))
	(if (IsObject ret)
		(if (ret isKindOf: DIcon)
			(= ret (ret value:))
		else
			(= ret 1)
		)
