...
(local
	...
	local161
	local162
	[local163 9] = [1000 93 46 4 11 24 19 23 30]
	[local172 10] = [1 9106 0 7040 1 9107 0 7041 0 0]
	[local182 9] = [1003 140 100 4 11 25 23 31 31]
)
		...
		(if (and (== gPrevRoomNum 109) (not (GameIsRestarting))) ; intro10
			...
			(switch gPrevRoomNum
				...
				(west
					(= temp1 (gEgo y:))
					(= temp0 0)
					(gEgo setMotion: MoveTo (+ (gEgo x:) 3) (gEgo y:))
				)
				(76
					(= temp0 169)
					(= temp1 98)
...
(instance enterFromTree of Script ; UNUSED
	(properties)
...
(instance getDrink of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(Say 9070)
				(gEgo normal: 1 view: 2 setCycle: SyncWalk cycleSpeed: 0)
				((gEgo head:) show:)
				(HandsOn)
				(client setScript: 0)
			)
...
(instance introRoomScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(proc762_0 @local182 @local163 @local172 self)
			)
			(3
				(HandsOn)
				(self dispose:)
