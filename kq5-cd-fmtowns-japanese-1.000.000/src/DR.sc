...
(procedure (FakeKey theKey &tmp event) ; UNUSED
	(View addToFront: ((View notFacing:) type: 4 message: theKey showSelf:))
)

(procedure (FakeDir theDir &tmp event) ; UNUSED
	(View addToFront: ((View notFacing:) type: 64 message: theDir showSelf:))
)

(procedure (FakeMouse theType theX theY theModifiers &tmp event) ; UNUSED
	((= event (View notFacing:))
		type: theType
		x: theX
		y: theY
...
(instance DR of SRDialog
	...
	(method (flags)
		...
		(self script: (ScriptID (+ gCurRoomNum global55)))
		(Dialog time: global60)
	)

	(method (doit)
		((View notFacing:) type: 0 message: 0 modifiers: 0 y: 0 x: 0 claimed: 0)
		(super doit:)
	)
