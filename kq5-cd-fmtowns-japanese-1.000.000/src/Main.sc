...
(procedure (proc0_15 param1 param2 param3 param4 &tmp temp0 temp1 temp2)
	(if global156
		(Display 0 4 dsRESTOREPIXELS global157)
		(Display 0 4 dsRESTOREPIXELS global156)
		(= global156 0)
	)
	(if param1
...
(procedure (Say param1 param2 &tmp temp0 temp1 temp2)
	(cond
		((and (== argc 2) (not (== argc 3)))
			(speakTimer set60ths: param2 (DoAudio audPLAY param1))
		)
		((== argc 3)
			(DoAudio audPLAY param1)
		)
		(else
			(= temp0 (gGame setCursor: 456))
							...
							(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
								...
								(19
									(if (== (++ global316) 1)
										...
										(SetFlag 16)
										(Say 142)
										((gInventory at: 19) cel: 5 cursor: 52) ; Leg_of_Lamb
...
(instance KQ5 of Game
	...
	(method (init &tmp [temp0 270])
		...
		(= global330 159)
		(gGame egoMoveSpeed: 2 setSpeed: 1 masterVolume: 10)
		(= gPMouse kQPMouse)
		...
		(cls)
		((= gTheIconBar IconBar)
			add: icon0 icon1 icon2 icon3 icon4 icon5 icon8 icon9
			eachElementDo: #init
		...
		((ScriptID 755) init:) ; KQ5Controls
		((ScriptID 758) init:) ; KQInv
		(= global103 0)
		(= global106 (= global75 6))
		(self setCursor: global75 1 159 94)
		(= gUseObstacles 1)
		(HandsOn)
		(if (GameIsRestarting)
			(= global322 1)
	...
	(method (save &tmp [temp0 20] temp20 temp21 temp22)
		...
		(if (!= (= temp20 (IconSave show: @temp0)) -1)
			(= parseLang temp22)
			(= temp21 (self setCursor: gWaitCursor 1))
			(if (not (SaveGame name temp20 @temp0 gVersion))
				(Print 0 0 #font 0 #button {OK} 1) ; "Your save game disk is full. You must either use another disk or save over an existing saved game."
			)
	...
	(method (restore &tmp [temp0 20] temp20 temp21 temp22)
		...
		(if (!= (= temp20 (IconSave show: 0)) -1)
			(self setCursor: gWaitCursor 1)
			(if (CheckSaveGame name temp20 gVersion)
				(RestoreGame name temp20 gVersion)
			else
				(Print 0 1 #font 0 #button {OK} 1) ; "That game was saved under a different interpreter. It cannot be restored."
				(self setCursor: temp21 (HaveMouse))
				(= parseLang temp22)
			)
		else
	...
	(method (quitGame)
		...
		(super quitGame: (Print 0 2 #button {Yes} 1 #button {No} 0)) ; "Do you really want to quit?"
	)
	...
	(method (newRoom newRoomNumber)
		...
		(if (and gModelessDialog global380)
	...
	(method (startRoom roomNum &tmp temp0)
		(gGlobalSound3 stop:)
		(gGlobalSound4 stop:)
		(LoadMany 0 972 970 760 762 929 941 759 982 949 202 888 889 764)
		...
		(if
			(and
				...
				(Print 0 3 #button {Who cares} 0 #button {Debug} 1) ; "Memory fragmented."
			)
			(SetDebug)
		)
