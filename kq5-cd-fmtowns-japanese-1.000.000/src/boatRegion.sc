...
(local
	[local0 8] = [319 143 319 154 215 154 230 147]
...
(instance leave of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(if (gCast contains: global322)
					(proc762_1 @local42 7015)
				)
				(= cycles 1)
			)
			(12
				(if (gCast contains: global322)
					(proc762_1 @local24 3002)
				)
				(= cycles 1)
			)
			(13
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (not (IsFlag 94))
						...
						(PrintDC 220 0) ; "Graham is not strong enough to push the massive boat."
					)
					(event claimed: 1)
				)
				(4 ; Inventory
