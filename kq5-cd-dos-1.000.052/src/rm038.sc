...
(instance rm038 of KQ5Room
	...
	(method (doit &tmp temp0)
		(if (and (not (local18 mover:)) (== (gGame detailLevel:) 3))
			(local18 x: 295 y: 0 setMotion: MoveTo 295 111)
		)
		(if
			(and
				(> (local18 y:) 34)
			...
			(local19 x: 295 y: 0 setMotion: MoveTo 295 111 show:)
		)
		(if
			(and
				(> (local18 y:) 70)
			...
			(local20 x: 295 y: 0 setMotion: MoveTo 295 111 show:)
		)
		(cond
			(script
				(script doit:)
...
(instance hammerCrystal of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if local66
					(gEgo setLoop: 1 cycleSpeed: 1 setCycle: CT 4 1 self)
					(gGlobalAudio number: 8113 loop: 1 play:)
				else
			...
			(6
				...
				(gGlobalAudio number: 8085 loop: 1 play:)
			)
			(7
