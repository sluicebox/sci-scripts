...
(procedure (localproc_6)
	(if (and (== global384 local24) (== global385 local25))
		(hairPin init: posn: local22 local23 0 setCycle: 0 stopUpd:)
		(return 1)
	else
		(return 0)
	)
...
(procedure (localproc_8 &tmp temp0) ; UNUSED
	(if global159
		(Animate (gCast elements:) 1)
		(for ((= temp0 0)) (<= temp0 100) ((+= temp0 local2))
...
(instance mainDinkScript of Script
	...
	(method (doit &tmp temp0 temp1)
		(super doit: &rest)
		(= temp0 (GetAngle (client x:) (client y:) (gEgo x:) (gEgo y:)))
		(cond
			((<= 270 temp0 360)
				(= temp1 1)
			)
...
(instance hugScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (IsObject dinkTalk)
					(dinkTalk setCycle: 0 dispose:)
				)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(2
				...
				(gGlobalAudio number: 8050 loop: -1 play:)
			)
			(3
				(gGlobalAudio stop:)
			...
			(8
				...
				(UnLoad 132 890)
				(DoAudio audWPLAY 8889)
				(dink view: 1060 setLoop: 0 cel: 0 setCycle: End self)
			)
			(9
				(gGlobalAudio number: 8889 loop: 1 play:)
				(UnLoad 132 888)
				(hairPin init:)
...
(instance hairPinScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gGlobalAudio number: 8797 loop: 1 play:)
				(client cue:)
				(= seconds 3)
			)
			(3
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(gGlobalAudio number: 8150 loop: -1 play:)
				(mazeBug
	...
	(method (doit &tmp temp0 temp1)
		...
		(cond
			...
			((and (not temp0) (not temp1))
				(self x: (dink x:) y: (- (dink y:) 39) 0 loop: 0 cel: 0)
			)
			((and (not temp0) (== temp1 6))
				(self x: (- (dink x:) 4) y: (- (dink y:) 38) 0 loop: 0 cel: 1)
			)
			((and (== temp0 4) (not temp0))
				(self x: (- (dink x:) 6) y: (- (dink y:) 36) 0 loop: 1 cel: 0)
			)
			((== temp0 temp1 4)
				(self x: (- (dink x:) 10) y: (- (dink y:) 35) 0 loop: 1 cel: 1)
			)
			((and (== temp0 6) (== temp1 8))
				(self x: (- (dink x:) 4) y: (- (dink y:) 38) 0 loop: 2 cel: 0)
			)
			(else
				(self z: 1000)
			)
