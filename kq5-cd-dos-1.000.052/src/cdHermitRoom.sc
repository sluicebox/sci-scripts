...
(instance cdHermitRoom of KQ5Room
	...
	(method (init)
		(LoadMany rsVIEW 624 626 628 623 625 26 618 138 615 137 0)
		(Load rsSOUND 821)
		(Load rsSCRIPT 941)
		(Load rsSCRIPT 401)
		(Load rsSCRIPT 929)
		(Load rsSOUND 7)
		(LoadMany rsSYNC 1147 1148 1151)
...
(instance cartoon2 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(18
				(gGlobalAudio number: 7055 loop: -1 doNotStop: 1 play:)
				(if (and (IsFlag 55) (!= global361 46))
					(self dispose:)
					(gCurRoom setScript: goGetBoatScript)
				else
					(= cycles 1)
				)
			)
			...
			(28
				(gCurRoom newRoom: 663) ; cdHermit4
			)
		)
	)
)
