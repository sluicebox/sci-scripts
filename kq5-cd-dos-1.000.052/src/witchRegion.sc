...
(procedure (localproc_0)
	...
	(= [local29 1] (Min (Max 0 (- (gEgo x:) 40)) 239))
)
		...
		(switch (= state newState)
			(0
				...
				(if (IsFlag 90)
					...
					(gGlobalAudio number: 8095 loop: 1 play:)
				)
			)
			(1
				(if (IsFlag 90)
...
(instance zapHim of Script
	...
	(method (doit)
		...
		(if
			(and
				(!= (gGlobalAudio number:) 8096)
				(!= (gGlobalSound3 number:) 96)
				(== state 2)
				(== (witch cel:) 2)
			)
			(fireball
			...
			(gGlobalAudio number: 8096 loop: 1 play:)
		)
	)
		...
		(switch (= state newState)
			...
			(4
				(if (and (gEgo has: 27) (IsFlag 84)) ; Amulet
					...
					(gGlobalAudio number: 8082 loop: 1 play:)
					(+= state 4)
				else
					(fireball dispose:)
					((gEgo head:) hide:)
			...
			(6
				(gGlobalAudio number: 9117 loop: 1 play: self)
			)
			(7
				(if (not register)
					(-= state 2)
			...
			(12
				(witch setCycle: End self)
				(gGlobalAudio number: 8083 loop: 1 play:)
			)
			(13
				(switch register
					(2
