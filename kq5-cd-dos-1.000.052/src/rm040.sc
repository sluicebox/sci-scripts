...
(instance rm040 of KQ5Room
	...
	(method (init)
		...
		(gGlobalAudio number: 7054 loop: -1 doNotStop: 1 play:)
		(waterfall init:)
		(= global320 221)
		(= global321 91)
		(= global325 3044)
	...
	(method (doit &tmp temp0 temp1)
		(cond
			...
			((& (= temp1 (gEgo onControl: 0)) $0040)
				(gEgo priority: 0 signal: (| (gEgo signal:) $0010))
			)
			(
				(and
					(gEgo edgeHit:)
...
(instance shadow of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(pond2 cel: 0 loop: local0 cycleSpeed: 10 setCycle: End self)
				(= cycles 1)
				(++ local0)
			)
			(1
