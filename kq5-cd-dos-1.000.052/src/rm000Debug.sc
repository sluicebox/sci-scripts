...
(procedure (localproc_0 param1 &tmp [temp0 20])
	(= temp0 0)
...
(instance rm000Debug of Code
	...
	(method (doit param1 &tmp temp0 temp1 temp2 temp3 temp4 [temp5 255])
		(switch (param1 message:)
			...
			(4864
				(Printf 889 1 gCurRoomNum) ; "This is room %d."
			)
			(7936
				(repeat
					(breakif (!= (= temp0 (GetNumber {Set Flag#: })) -1))
				)
				(SetFlag temp0)
			)
			(5120
			...
			(17152
				(= temp1 (Sound pause: 1))
				(if (Print 889 2 #font 0 #button {__Restart__} 1 #button { Continue } 0) ; "Do you wish to start over?"
					(gGame restart:)
				)
				(Sound pause: temp1)
			)
			...
			(5376
				(repeat
					(breakif
						(!=
							(= temp0
								(localproc_0 (Format @temp5 {Audio number#:}))
							)
							-1
						)
					)
			...
			(11264
				(= gQuit
					(Print 889 3 #font 0 #button {____Quit____} 1 #button { Continue } 0) ; "Do you really want to quit?"
				)
			)
			(else
				(param1 claimed: 0)
