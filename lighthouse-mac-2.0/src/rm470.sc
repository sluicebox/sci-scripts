		...
		(switch param1
			...
			(461
				...
				(if (IsFlag 68)
					...
					(if (not (IsFlag 78))
						(birdMan init: 161 0 206 78 500 start: 1 15 1)
					)
				else
			...
			(465
				...
				(if
					...
					(if
						...
						(birdMan init: 465 0 -295 -13 -1 start: 1 15)
						(gMySoundFX number: 44002 setLoop: -1 play:)
					)
				)
			...
			(467
				...
				(if
					...
					(if
						...
						(birdMan init: 163 0 161 -15 -1 start: 1 15)
						(gMySoundFX number: 44002 setLoop: -1 play:)
					)
				)
			)
...
(instance rockOnBirdMan of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_1)
				(birdMan stop: init: 155 0 179 12 500 caller: self start: 1 15)
		...
		(switch (= state newState)
			(0
				...
				(if (and (not (IsFlag 68)) (IsFlag 79))
					...
					(birdMan init: 150 20 204 78 caller: self start: 1 15)
				)
			)
			(1
				(SetFlag 79)
			...
			(3
				(birdProp dispose:)
				(birdMan caller: 0 init: 161 0 206 78 start: 1 15 1)
				(SetFlag 68)
				(gGame handsOn:)
				(self dispose:)
			)
...
(instance bottomGate of Actor
	...
	(method (init)
		(self setStep: 1 10 cycleSpeed: 1 setLoop: 2 moveSpeed: 0 setPri: 600)
		(super init: &rest)
		(approachX hotVerbs: 0)
	)
)
