...
(class Lite of Game
	...
	(method (getDisc param1 &tmp temp0 temp1 temp2 temp3 temp4)
		(= gNumCD param1)
		(= temp3 (= temp2 0))
		(if (and (!= -1 (= temp0 (CD 0))) (!= temp0 param1)) ; Check
			(gGame hideCursor:)
	...
	(method (save &tmp temp0 [temp1 3])
		(Sound pause: 1)
		(if gNumCD
			(self getDisc: gNumCD)
		)
		(self setCursor: gTheControlWaitCursor 1)
		(Platform 0 3 1 temp0 (KArray 9 gVersion)) ; UNINIT, ArrayGetData
		(self setCursor: gTheControlNormalCursor 1)
		(Sound pause: 0)
	)

	(method (restore &tmp temp0 temp1 [temp2 3] temp5 temp6 temp7 temp8 temp9 temp10 temp11)
		(Sound pause: 1)
		(for
		...
		(if (Platform 0 3 -1 temp0 (KArray 9 gVersion)) ; UNINIT, ArrayGetData
			(Sound pause: 1)
			(self getDisc: (CD 1)) ; GetSaveCD
			(Platform 0 3 0 temp0 (KArray 9 gVersion)) ; UNINIT, ArrayGetData
		else
			(for
				((= temp9 (KList 3 (gPlanes elements:)))) ; FirstNode
				temp9
	...
	(method (restart &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		...
		(if (Save 3 {rst} 0 (KArray 9 gVersion) 0) ; CheckSaveGame, ArrayGetData
			(gGame hideCursor:)
			(self getDisc: 1)
			(Sound pause: 1)
			(temp7 dispose:)
			(Save 1 {rst} 0 (KArray 9 gVersion) 0) ; RestoreGame, ArrayGetData
		else
			(for
				((= temp0 (KList 3 (gPlanes elements:)))) ; FirstNode
				temp0
