...
(local
	...
	local26
	local27
	[local28 9] = [4 5 6 7 8 9 10 11 12]
)

(procedure (localproc_0 &tmp [temp0 30]) ; UNUSED
	(Format @temp0 {%d %d} gMouseX gMouseY)
	(if local23
		(Display {} 108)
	)
	(= local23 (Display @temp0 dsCOORD 0 70 dsFONT gUserFont dsSAVEPIXELS))
)
...
(procedure (localproc_4 param1 &tmp [temp0 3])
...
(class Asteroid of FloatObj
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gMessager say: [local28 (= local27 (mod (++ local27) 9))] 1 0 0)
			)
			(else
		...
		(cond
			(
				(and
					(not script)
					(gUser canControl:)
					(or (not (IsObject event)) (& (event type:) $0040)) ; direction
				)
				(if (IsObject event)
					(= temp0 (event message:))
					(event claimed: 1)
...
(instance target of Prop
	...
	(method (init)
		(super init: &rest)
		(self setCycle: Fwd)
		(Display {TARGET IN RANGE} dsFONT gUserFont dsCOORD 65 25 dsCOLOR global153)
	)

	(method (dispose)
		(Display {TARGET IN RANGE} dsFONT gUserFont dsCOORD 65 25 dsCOLOR global151)
		(super dispose: &rest)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gMessager say: 2 0 2 0 self) ; "Cliffy has been successfully grappled. Now all that remains it to make it back to the Eureka... if you can find it."
			)
			(2
				(cliffy addToPic:)
				(claw addToPic:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(switch local22
					(0
						(gMessager say: 26 0 3 0 self) ; "The tightness in your lungs and throat along with the ''zero'' reading on the oxygen gauge leads to the inescapable conclusion that you are a fool and have spent too much time screwing around."
					)
					(1
						(gMessager say: 26 0 5 0 self) ; "It might help to bring along a fully recharged oxygen tank."
					)
					(2
						(gMessager say: 26 0 4 0 self) ; "While the last wisps of oxygen filter through the pods life support system you try to yell for help, but the only sound that escapes your throat is a loud burping sound. As the world fades to black you know you're doomed: In space, no one can hear you belch."
					)
				)
			)
			(3
...
(instance warning of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 29 0 7 0 self) ; "Aren't you forgetting one tiny little detail like oh maybe..., lets say..., hmmm..., maybe... CLIFFY!"
				(= local20 1)
			)
			(1
				(self dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(SetScore 193 50)
				(gMessager say: 29 0 6 0 self) ; "Good flying there ace! You've returned to the safety of the Eureka."
				(gSq5Music1 fade: 0 10 5 1)
			)
			(1
				((gTheIconBar at: 2) cursor: 982)
...
(instance alien1Tkr of Narrator
	...
	(method (init)
		...
		(= font gUserFont)
		(gSystemWindow tailX: 80 xOffset: -20 tailY: 60 isBottom: 1)
		(super init: &rest)
	)
