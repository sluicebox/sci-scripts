...
(procedure (localproc_5 &tmp temp0 temp1 temp2 temp3 temp4 temp5)
	(cond
		...
		(
			...
			(= local39 ap)
			(if (and (not local35) (== local26 2))
				(gMessager say: 10 2 0 (+ (= local33 (mod (++ local33) 8)) 1))
			)
		)
		((not (< 5 (gEgo x:) 315))
			(= local39 2)
	...
	(if (OneOf local39 cone1 cone2 cone3)
		(local39 setLoop: 0 setCycle: ForwardCounter 3 local39)
	)
	(= temp1 (GetAngle (local39 x:) (local39 y:) (gEgo x:) (gEgo y:)))
...
(instance rm119 of Rm
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(cond
					((and (not local27) (gCast contains: disk) (== local26 2))
						(gCurRoom setScript: getOffScrubber)
					)
					((and local27 (gCast contains: disk))
						(gMessager say: 5 4 8 0) ; "You might have better luck if you used some cleaning supplies."
					)
					((IsFlag 6)
...
(instance quirkYells of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(3
				(gEgo setCycle: 0 setLoop: SpecialLooper)
				(quirk
					init:
					posn: 250 -1
					setCycle: Walk
...
(instance sMeetQuirk of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(8
				(gMessager say: 6 0 0 3 self) ; "It's nice to see our tax buckazoids aren't going COMPLETELY to waste, Captain Quirk."
			)
			(9
			...
			(22
				(gMessager say: 6 0 0 7 self) ; "Omigosh, It's HER! The woman from the Holodisk in SQ IV. Now's your big chance Roger! Say something clever and romantic!"
			)
			(23
		...
		(if (== local26 2)
			...
			(if
				(and
					(not (IsFlag 6))
					(&
						(= local36 (OnControl CONTROL nsLeft nsTop nsRight nsBottom))
						$001e
					)
...
(instance ap of Actor
	...
	(method (cue)
		(if
			...
			(self init: (= local38 (mod (++ local38) 8)))
		)
	)
	...
	(method (init param1 &tmp temp0 temp1)
		...
		(= temp0 param1)
		(self setLoop: param1 setCycle: Walk setAvoider: PAvoider)
		(if loop
			(= noun 8)
		else
			(= noun 7)
		...
		(if local27
			(+= param1 (* (Random 0 1) 4))
			(self
				posn: [local41 param1] [local41 (+ param1 1)]
				setMotion:
					PolyPath
					[local41 (+ param1 2)]
					[local41 (+ param1 3)]
			...
			(+= param1 8)
			(= temp1 (Random [local41 param1] [local41 (+ param1 1)]))
			(if (< temp0 2)
				(self posn: -22 temp1 setMotion: PolyPath 320 temp1 self)
			else
				(self
					posn: temp1 [local41 (+ param1 3)]
					setMotion:
						PolyPath
						(+ temp1 [local41 (+ param1 2)])
						[local41 (+ param1 4)]
						self
				)
			)
...
(instance quirkTalker of Talker
	...
	(method (init)
		...
		(switch local28
			...
			(0
				(gSystemWindow tailX: 132 tailY: 35 xOffset: 40 isBottom: 1)
			)
			(2
				(self talkWidth: 200 textX: -40)
				(gSystemWindow tailX: 202 tailY: 65 xOffset: -30 isBottom: 1)
			)
			(-1
...
(instance beaTalker of Talker
	...
	(method (init)
		...
		(= gSystemWindow gSpeakWindow)
		(if (== local28 2)
			(gSystemWindow tailX: 83 tailY: 33 xOffset: 30 isBottom: 1)
			(self cel: 0 loop: 14 talkWidth: 0)
			(super init: 0 0 0 &rest)
		else
			(gSystemWindow tailX: 224 tailY: 55 xOffset: -50 isBottom: 1)
			(= talkWidth 200)
		...
		(gSystemWindow
			...
			tailY:
				(-
					(-
						(quirk y:)
						(CelHigh (quirk view:) (quirk loop:) (quirk cel:))
					)
					5
				)
		)
...
(instance crestFeature of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 6)
					(gMessager say: noun theVerb 5 0) ; "The StarCon crest is bright, shiny, and has a fresh pine scent."
				else
