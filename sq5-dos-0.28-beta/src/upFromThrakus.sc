...
(instance useAntAcids of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(gMessager say: 22 29 0 1 self) ; "Pour little fella must have eaten something that didn't agree with him. Those antacids should reduce the acidity of his metabolism."
			)
			(10
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				((ScriptID 240 15) normal: 0) ; tkrRoger
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(proc240_25 1)
				(proc240_26 1)
				(gMessager say: 1 2 1 1 10 self) ; "Ugh. Sorry, I tried to kick your butt back there on the planet, Captain Wilco. I thought you were a pukoid..."
			)
			(7
				(beaArm init:)
				(= seconds 5)
...
(instance goliathConv of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (& global169 $0008)
					((ScriptID 240 11) normal: 0 talkWidth: 75) ; tkrCliffy
					(gGame handsOn:)
					(gGame setCursor: 999 1)
			...
			(3
				(gGame handsOff:)
				((ScriptID 240 11) normal: 1 talkWidth: 100) ; tkrCliffy
...
(instance breakBea of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(EgoDead 47) ; "Beatrice really went to pieces on you..."
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(2
				...
				(cond
					...
					(else
						(gMessager say: 26 24 54 0 self) ; "The Goliath's anti-teleportation shields must be up. Looks like you'll have to find another way..."
					)
				)
			)
			(3
...
(instance specialBeam of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				(gMessager say: 28 0 40 4 5 self) ; "I was putting the Oakhurst queen back together and I had this piece left over. Thought you might want it."
			)
			(14
