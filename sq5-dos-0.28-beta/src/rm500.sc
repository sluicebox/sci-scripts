			...
			(switch theVerb
				(1 ; Look
					(if (not (IsFlag 48))
						...
						(gMessager say: 20 1 1 0) ; "The atmosphere of the spacebar is growing thick with the pungent smelling bodies of Space Monkeys."
					)
				)
			)
		)
...
(instance sGuyTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 3 0 0 0 self) ; "Who's the moron who let all these damn space monkey's loose!"
			)
			(2
				((ScriptID 501 25) setScript: (ScriptID 501 24)) ; guy1Mouth, sGuys
				(self dispose:)
...
(instance sFromDetention of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if (IsFlag 48)
					(gMessager say: 9 0 0 0 self) ; "The space monkeys are reproducing at an alarmingly high rate."
				else
					(= cycles 1)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(14
				...
				(cliffy setLoop: 9 cel: 0 setCycle: End self)
				(guard2 view: 507 setLoop: 13 cel: 0 setCycle: End self)
				(victim view: 507 setLoop: 12 cel: 0 setPri: 13)
			)
			(15 0)
			(16 0)
			(17
