		...
		(switch (event type:)
			(evKEYBOARD
				...
				(switch (event message:)
					...
					(KEY_ALT_t
						...
						(if (and temp0 (> (= temp162 (ReadNumber @temp0)) 0))
					...
					($0016
						(Print
							addText: {Reviewers copy of SQ5 10-22-92}
							addText: {Version number:} 0 14
							addText: gVersion 0 28
							init:
						)
					)
					(KEY_ALT_y
			...
			(evMOUSEBUTTON
				(switch (event modifiers:)
					...
					($000c ; emALT | emCTRL
						(event claimed: 1)
						(Format @temp0 10 4 (event x:) (event y:)) ; "%d/%d"
						(= temp160
							(Print
								posn: 160 10
								font: 999
