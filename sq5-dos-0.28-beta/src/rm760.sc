	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(17
				(= local10 0)
				(if local9
					(EgoDead 26) ; "You aren't going to ''bug'' anyone now."
				else
					(mwog dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gSq5Music2 number: 600 loop: -1 play:)
				(rogCan view: 606)
				(gMessager say: 1 0 0 0 self) ; "The probability of an insect flying into the transporter field at the exact moment of beamout is approximately 1,365,500,012 to 1 against."
			)
			(3
				(gGame handsOn:)
		...
		(switch (= state newState)
			...
			(6
				...
				(if (IsFlag 64)
					...
					(gMessager say: 6 0 2 0 self) ; "Help me Flo, I've been turned into a fly!"
				)
			)
			(7
