		...
		(switch theVerb
			...
			(4 ; Do
				(if (Message msgSIZE modNum noun 4 0 1)
					...
					(= temp0 (GetPort))
					(Print addText: 0 4 0 0 0 0 modNum init:) ; "That item isn't operated in that way."
					(SetPort temp0)
				)
			)
			(else
...
(instance sq5Inv of ScrollableInventory
	...
	(method (init)
		...
		(self
			...
			add: invLook invHand invSelect invHelp invUp invDown ok
		)
		(self
			state: 2048
			upIcon: invUp
...
(instance Business_Card of Sq5InvItem
	...
	(method (doVerb theVerb &tmp temp0)
		(if (== theVerb 33) ; Hole_Punch
			(if (OneOf gCurRoomNum 201 200 228 226)
				(= temp0 (GetPort))
				(Print addText: noun theVerb 1 1 0 0 15 init:) ; "This is niether the time nor the place for that."
				(SetPort temp0)
			else
				(gInventory hide:)
