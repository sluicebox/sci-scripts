...
(instance sGiveCommBack of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(gEgo view: 6000 setLoop: 4 cel: 0 x: 51 y: 134)
			...
			(13
				(gMessager say: 13 0 0 0 self) ; "Here is your communicator, Captain. I located it during my scans of the area."
			)
			(14
...
(instance sHuman790 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(cliffyTalker normal: 0 keepWindow: 1)
				(= cycles 1)
			)
			(5
				(gGame handsOn:)
			...
			(3
				...
				(if (!= global129 341)
					(if (< global165 5)
						(cond
							((& global129 $feaa)
								(Print
									...
									addButton: 0 8 0 0 0 120 44 ; "Reset Puzzle"
									init:
								)
								(= global129 0)
								(++ global165)
						...
						(EgoDead 42) ; "Five times and you still couldn't get it right. Better go buy the hint book."
					)
					(gGame handsOn:)
					(self dispose:)
				else
...
(instance rogTalker of Narrator
	...
	(method (init)
		...
		(gSystemWindow tailX: 91 tailY: 65 xOffset: 10 isBottom: 0)
		(super init: &rest)
	)
