...
(instance sGlob2 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				((ScriptID 500 4) setMotion: MoveTo 207 86 self) ; lavaglob2
			)
			(2
				(= seconds (Random 1 4))
			)
			...
			(4
				((ScriptID 500 4) setMotion: MoveTo 218 1 self) ; lavaglob2
			)
			(5
				(= state -1)
				(= cycles 2)
...
(instance theStairs of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(gMessager say: 22 theVerb 0 0 500) ; "This stairwell leads up to Scumsoft's latest and greatest new arcade game ''Star Cruiser.''"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance thePatrons1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (not (IsFlag 54))
					(gMessager say: 11 theVerb 0 0 500) ; "Looking around you the bar you see a mixture of humanoid and alien forms, most of them seriously occupied in the time honored military tradition of 'tying one on'."
				)
			)
			(else
				(super doVerb: theVerb &rest)
