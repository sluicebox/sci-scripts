...
(instance rm1020 of Rm
	...
	(method (init)
		...
		(eView1
			x: [local39 (eView1 cel:)]
			y: [local51 (eView1 cel:)]
			init:
			setPri: 8
			ignoreActors:
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo
					view: 675
					loop: [local63 (+ register 1 (* 4 local37))]
					cel: [local63 (+ register 2 (* 4 local37))]
					x: [local63 (+ register 3 (* 4 local37))]
					y: [local63 (+ register 4 (* 4 local37))]
				)
				(if (< (++ local37) [local63 register])
					(-- state)
				)
				(= cycles 3)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(eView2
					cel: (mod global135 6)
					x: [local39 (mod global135 6)]
					y: [local51 (mod global135 6)]
					init:
					setPri: 8
					ignoreActors:
				)
				(eView3
					cel: (+ 6 (/ global135 6))
					x: [local39 (+ 6 (/ global135 6))]
					y: [local51 (+ 6 (/ global135 6))]
					init:
					setPri: 8
					ignoreActors:
				)
				...
				(if (localproc_0 [local0 global135] 4)
					...
					(if (or (== gPrevRoomNum 1030) (== gPrevRoomNum 1035))
				...
				(if (and (== gPrevRoomNum 1025) (== local38 0))
					(self dispose:)
				)
				(= cycles 1)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if
					(or
						(and (== gGoliathFloorNum 1) (== global135 13) (not local36))
						(and (== gGoliathFloorNum 4) (== global135 21) (not local36))
						(and (== gGoliathFloorNum 8) (== global135 7) (not local36))
					)
					(= next sPacman)
					(self dispose:)
				else
...
(instance arrowBase of View
	...
	(method (handleEvent event)
		(cond
			...
			((and (gUser canControl:) (== (event message:) JOY_RIGHT))
				(event claimed: 1)
				(return 0)
			)
			(else
