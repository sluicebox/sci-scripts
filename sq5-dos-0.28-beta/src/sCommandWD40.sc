		...
		(switch (= state newState)
			(0
				(switch gEurekaLocation
					...
					(5 ; clorox2
						(if (IsFlag 30)
							(gMessager say: 33 0 27 1 self) ; "Atmosphere: Nitrogen, oxygen, carbon dioxide.  Gravity: .097 normal  Average temperature: 27 degrees Celsius.  Life Forms: Indeterminate readings"
						else
							(gMessager say: 33 0 27 2 self) ; "The colony shield is up sir. No scan is possible at this time."
						)
					)
					(3 ; ku
						(gMessager say: 33 0 56 1 self) ; "Kiz Urazgubi is a class M planet capable of sustaining humanoid lifeforms."
					)
					(6 ; thrakus
			...
			(0
				(switch gEurekaLocation
					(6 ; thrakus
						(if (IsFlag 42)
							(if (eureka damaged:)
								(gMessager say: 34 0 20 1 self) ; "The Goliath is coming about for another pass captain. Her weapons are fully charged."
							else
