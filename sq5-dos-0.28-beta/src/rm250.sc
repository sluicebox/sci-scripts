...
(local
	...
	local2
	local3
	[local4 5] = [25 64 77 94 111]
	[local9 5] = [25 75 90 107 127]
)

(procedure (localproc_0 param1)
	(cond
		((InRect 83 [local4 1] 110 [local9 1] param1)
			(return 1)
		)
		((InRect 83 [local4 2] 110 [local9 2] param1)
			(return 2)
		)
		((InRect 83 [local4 3] 110 [local9 3] param1)
			(return 3)
		)
		((InRect 83 [local4 4] 110 [local9 4] param1)
			(return 4)
		)
		(else
			(return -1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gGame setCursor: 982 1 [local4 1] [local9 1])
			)
			(2
				(= seconds 2)
			)
				...
				(switch register
					(1
						(if (gCast contains: cliffy)
							(gMessager say: 11 0 0 0) ; "Captain, are you crazy?! We'll get sucked out into the deep void of space!"
						else
							(= next sDoRedButton)
						)
					)
...
(instance sRogPushButton of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo hide:)
				(rogThink init: loop: 4 setPri: 12 posn: 240 170)
				(rogArm init: setPri: 13 cycleSpeed: 20 setCycle: End self)
			)
			(2
				(rogArm setCycle: Beg self)
			)
			(3
				(rogArm dispose:)
				(rogThink loop: 2 setPri: 12 posn: 240 170 setCycle: End self)
			)
			(4
				(self dispose:)
			)
...
(instance sDoButton of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(theKey show: register)
				(gGame handsOff:)
				(= seconds 2)
			)
			(1
				(theKey dispose:)
				(sPopUpConsole register: register)
...
(instance sOpenPodDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(podDoor init: setCycle: End self)
...
(instance sDoRedButton of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(podRmDoors init: cycleSpeed: 20 setCycle: End self)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(pod view: 251 loop: 1 cel: 4 stopUpd:)
				(self setScript: sOpenPodDoor self)
			)
			(6
				(gEgo
...
(instance theKey of View
	...
	(method (show param1)
		(self posn: 83 (localproc_1 param1) setLoop: param1 setCel: 0)
		(super show: &rest)
	)
)
...
(instance popUpConsole of Inset
	...
	(method (doit)
		(if local2
			...
			(= local2 0)
			(self setScript: sDoButton 0 local3)
		)
		(super doit: &rest)
	)
			...
			(cond
				((and (= temp0 (event message:)) (& (event type:) evKEYBOARD))
					(switch temp0
						...
						(JOY_UP
							(= local3 (if (<= local3 1) 4 else (-- local3)))
							(event claimed: 1)
							(return)
						)
						(JOY_DOWN
							(= local3 (if (>= local3 4) 1 else (++ local3)))
							(event claimed: 1)
							(return)
						)
						(else
