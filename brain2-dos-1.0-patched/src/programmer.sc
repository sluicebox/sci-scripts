...
(instance programmer of PuzzleBar
	...
	(method (doit)
		(if (== [local333 0] [local333 1] [local333 2] -1)
			(gMessager say: 58 4 10 0) ; "You have completed the puzzle!"
			(= local365 2)
	...
	(method (buyClue)
		(cond
			(local407
				(gMessager say: 58 4 16) ; "Position indicator already turned on."
			)
			...
			(else
				...
				(gMessager say: 58 4 17) ; "Robot position indicator turned on."
			)
		)
	)
)
	...
	(method (select)
		(if (super select: &rest)
			(if (< local363 99)
				...
				(Say 0 58 4 13) ; "Program terminated: You have reached the end of the command module length."
			)
		)
	)
)
	...
	(method (highlight param1)
		(cond
			(param1
				((Print new:)
					modeless: 1
					addText: noun 1 0 value 0 0 gCurRoomNum
					init:
				)
			)
...
(instance robot of Actor
	...
	(method (cue &tmp temp0 temp1 temp2 [temp3 2])
		(super cue:)
		(switch local391
			(1
