...
(procedure (localproc_0 param1 param2)
	(return
		(if (< param2 param1)
			...
			(+ 1 (localproc_0 param1 (/ param2 param1)))
		)
	)
)
...
(class HedgeRow of Rm
	...
	(method (doit)
		(gEgo
			priority: (localproc_0 2 (gEgo onControl: 1))
	...
	(method (doVerb theVerb invItem)
		(return
			(if (and (== theVerb 4) (== invItem 12)) ; Inventory, puzzleBox
				(proc13_1 490 0) ; "I'll wait for a safer place to work on the puzzle box."
				1
			)
		)
	)
	...
	(method (doVerb theVerb invItem)
		(return
			(and
				...
				(switch invItem
					(12 ; puzzleBox
						(proc13_1 490 1) ; "I've no time for the puzzle box now!"
						1
					)
					(1 ; horn
						(Print 490 2 1) ; "That would be extemely stupid here in the middle of town."
					)
				)
			)
		)
	...
	(method (doVerb theVerb invItem)
		(return
			(if (and (== theVerb 4) (== invItem 1)) ; Inventory, horn
				(Print 490 2) ; "That would be extemely stupid here in the middle of town."
				1
			)
		)
	)
