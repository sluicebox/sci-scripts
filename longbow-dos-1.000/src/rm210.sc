...
(local
	...
	[local473 7] = [1211 16 1 2 1 1 0]
	[local480 4] = [1211 20 1 0]
	[local484 4] = [1211 21 1 0]
	[local488 4] = [1211 22 1 0]
	[local492 6] = [1211 23 2 1 1 0]
	[local498 4] = [1317 12 1 0]
	[local502 4] = [1210 169 1 0]
	[local506 3] = [-1 0 0]
	[local509 3] = [-1 {Willows} 5]
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (IsFlag 179)
					(ClearFlag 179)
					(Converse @local419 @local506 0 self) ; "Thank the Blessed Virgin! The sprite's spell has worn off!"
				else
					(self cue:)
				)
			)
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					...
					(17 ; handScroll
						(if (IsFlag 34)
							(Converse 1 @local502 22 3) ; "You must give it to the Queen's spy."
						else
...
(instance willowTalk of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local445 @local509 999 self 0) ; "What would you ask of us?"
			)
			(2
				(Converse register @local509 999 self)
			)
			(3
				(self dispose:)
				(localproc_0)
			...
			(4
				(-= state 2)
				(Converse 1 register @local509 0 self)
			)
			(5
				(-= state 3)
...
(instance spillBeans of TScript
	...
	(method (changeState newState &tmp [temp0 30])
		(switch (= state newState)
			(0
				(Converse @local492 @local509 999 self 0) ; "Did Marian whisper a password to you?"
			)
			(1
				(= [local498 1] (+ -10 global140))
				(Converse @local498 @local509 999 self) ; "LIONS."
				(SetFlag 51)
			)
			(2
				(self dispose:)
