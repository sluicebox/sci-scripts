...
(local
	...
	[local45 4] = [1530 112 1 0]
	[local49 4] = [1530 104 1 0]
	[local53 4] = [1530 129 1 0]
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(cond
					((== invItem 0) ; bucks
						(Converse @local53 12) ; "That would be a waste of good silver."
					)
					(local96
						(gEgo setScript: die)
					)
...
(instance man1 of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Converse 1 @local45 @local12 1) ; "That would be utterly foolish."
			)
			(3 ; Do
				(Converse 1 @local49 @local12 1) ; "Don't try to force your way past me, outlaw. I fought in the Crusades and I've no fear of any man that faces me fairly."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance wench1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Say 1530 107 1) ; "Nor would I want him to."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance wench2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(5 ; Talk
				(Say 1530 113 1) ; "Keep moving, Monk. I don't like having you around right now."
			)
			(3 ; Do
				(Say 1530 105 1) ; "Don't you lay hands on me, Monk. I've no love for your kind."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance vbKeeper of SpecialDoVerb
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(if (== invItem 0) ; bucks
					(Say 1530 130 1) ; "Keeper your silver, Brother. I'm feeling generoush tonight."
				else
					(return 1)
				)
			)
