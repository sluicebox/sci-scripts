...
(instance rm15 of Rm
	...
	(method (init)
		...
		(aView1 loop: 2 cel: 0 setPri: 8 addToPic:)
		...
		(if (== gPrevRoomNum 115)
			(aDoorEast setLoop: 1 setCel: 255 cycleSpeed: 2 init: setCycle: Beg)
		else
			(aView3 ignoreActors: setLoop: 1 addToPic:)
		)
		(if thisIsIt
			(= gLAhenchAfterEgo 1)
			(= gCurrentStatus 1)
			...
			(Load rsVIEW 223)
			(gCurRoom east: 0)
			(aView4 loop: 3 cel: 0 setPri: 15 addToPic:)
			(aCar setLoop: 4 setPri: 8 cycleSpeed: 0 init:)
			(aHench loop: 0 setCycle: Walk stopUpd: init:)
			(gEgo posn: 253 88 init:)
			(HandsOff)
		else
			(aSignal
				loop: 3
