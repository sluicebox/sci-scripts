...
(instance rm50 of Rm
	...
	(method (init)
		...
		(Load rsSCRIPT 985)
		(super init:)
		(gAddToPics add: aSign aEatSign aShitSign doit:)
		(aRadar setPri: 0 cycleSpeed: 1 setCycle: Fwd isExtra: 1 init:)
		(= currentHench 0)
		(if (== gForceAtest 1)
			(= gCurrentEgoView 151)
		)
		...
		(if (== gCurrentEgoView 151)
			...
			(gAddToPics add: aCop1 aCop2 doit:)
...
(instance rm50Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(cls)
				(Print 50 26 #at -1 130 #draw) ; "(Nice going, Larry. You've succeeded in bypassing the KGBers again!)"
				(aHench1 dispose:)
				(aHench2 dispose:)
...
(instance aBigFace of Prop
	...
	(method (cue)
		...
		(Print 50 33 #at -1 130) ; "(But what is this?!)"
		(self posn: 160 1054)
		(HandsOn)
	)
)
