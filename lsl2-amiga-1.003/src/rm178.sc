...
(instance rm178 of Rm
	...
	(method (init)
		...
		(Load rsPIC 178)
		(super init:)
		(aThoughtBalloon setCel: 0 setPri: 14 init:)
		(aHandle init:)
		(aThought setPri: 15 init:)
		(aChair setPri: 8 stopUpd: init:)
		(aBarber setCycle: Walk illegalBits: 0 init:)
...
(instance rm178Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo loop: 3)
				(Print 178 0 #at -1 130 #draw) ; "(Don't all witch doctor huts look alike?)"
				(= seconds 3)
			)
			(2
