...
(instance rm138 of Rm
	...
	(method (init)
		...
		(Load rsVIEW 124)
		(super init:)
		(aBigEgo setPri: 3 stopUpd: init:)
		(if (== gWearingWig 1)
			(aWig setPri: 5 stopUpd: init:)
		)
		(aCalendar setLoop: 1 setPri: 7 posn: 220 39 setCycle: Fwd init: hide:)
		(aDate setLoop: 2 setPri: 9 posn: 228 71 init: hide:)
		(aPage
			setLoop: 3
			setPri: 11
			posn: 220 39
		...
		(aFlame setLoop: 2 setPri: 5 setCycle: Fwd init: hide:)
		(HandsOff)
		(= gCurrentStatus 21)
		...
		(switch (= state newState)
			...
			(10
				...
				(if (== gWearingSunscreen 2)
					(Print 138 15 #at -1 130) ; "(Looks like that sunscreen is not waterproof!)"
				)
				(= state 24)
				(= seconds 3)
			)
