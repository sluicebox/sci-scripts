	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			...
			(if (Said '[/airport,hill]')
				...
				(Print 75 5 #at -1 130) ; "(You wonder why it isn't covered with lots of cheap motels?!)"
			)
		)
		...
		(if (Said 'get/beach')
			(cond
				...
				(else
					...
					(if (> gFilthLevel 10)
						(Print 75 9 #at -1 130) ; "(At last your pants are bulging!)"
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				...
				(Print 75 14 #at -1 130) ; "(And also, second sight!)"
			)
			(9
				(gEgo setLoop: 0)
				(= cycles 10)
			...
			(31
				...
				(Print 75 29 #at -1 130) ; "(You're no fool, Larry. That sounds like a cue!)"
				(= seconds 3)
			)
			(32
				(theSound dispose:)
