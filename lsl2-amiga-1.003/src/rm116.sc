...
(instance rm116 of Rm
	...
	(method (init)
		...
		(aBigHand setPri: 15 setCycle: Fwd cycleSpeed: 2 init:)
		(aClerk posn: 174 105 setPri: 9 setCycle: Walk init:)
		(NormalEgo)
		(gEgo posn: 161 161 init:)
	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			...
			(if (Said '/art,ceiling')
				...
				(Print 116 9 #at -1 130) ; "(And also, no help!)"
			)
			(if (Said '/finger')
	...
	(method (changeState newState &tmp work)
		(switch (= state newState)
			...
			(3
				...
				(if (> gMachineSpeed 30)
					(aBigHand setCel: 0 setCycle: Fwd posn: 29 187)
				)
				(= seconds (Random 5 10))
			)
			(4
			...
			(7
				...
				(aClerk
					setLoop: -1
					setCycle: Walk
					setMotion: MoveTo work 105 self
				)
			)
			(8
