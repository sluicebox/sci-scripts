	...
	(method (handleEvent event &tmp msg i [string2 100])
		(switch (= msg (super handleEvent: event))
			(257
				...
				(Print
					(Format @gString 997 1 gVersion) ; "Sierra's Creative Interpreter developed by Jeff Stephenson, Bob Heitman, Pablo Ghenis, and Stuart Goldstein Music composed and performed by Al Lowe Atari ST Translation by Corey Cole Amiga Translation by Robert Lindsley and Dan Foy AmigaSound by: Mark Siebert Version %s, October 16, 1989"
					#font
					gSmallFont
					#mode
					1
			...
			(258
				...
				(Print 997 5 #font gSmallFont) ; "If you really need help, pick up a copy of the "Looking for Love" hint book at your software dealer, or call Sierra's hint line at (209) 683-6858."
			)
			(513
				(gGame save:)
			)
			...
			(770
				(if (not (HaveMem 1024))
					(Print 997 9) ; "Inventory cannot be displayed at this time."
				else
				...
				(if (< (MemoryInfo 1) 2048) ; FreeHeap
					...
					(cond
						(
							...
							(Print 997 10) ; "Perhaps you should buy "Mother Goose!""
						)
						((<= gFilthLevel 4)
			...
			(1025
				(if (not (HaveMem 2048))
					(Print 997 14) ; "Speed cannot be changed at this time."
				else
					(= i
						((Gauge new:)
							description:
			...
			(1281
				(if (not (HaveMem 2048))
					(Print 997 15) ; "Volume cannot be changed at this time."
				else
					(= i
						((Gauge new:)
							description: {Use the mouse to adjust the volume.}
							text: {Sound Volume}
							normal: 15
							higher: {Louder}
							lower: {Softer}
			...
			(1282
				...
				(if i
					...
					(Print 997 16) ; "Sound is off."
				else
					(SetMenu 1282 110 {Turn Off})
					(Print 997 17) ; "Sound is on."
				)
			)
			(else
				(if gDebugMenu
