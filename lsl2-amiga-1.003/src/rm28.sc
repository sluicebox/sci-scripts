...
(instance rm28 of Rm
	...
	(method (init)
		...
		(theSound init:)
		(super init:)
		(aSparkle setLoop: 0 setPri: 15 init: hide: setScript: sparkleScript)
		(aLA setLoop: 1 setPri: 1 setStep: 1 1 moveSpeed: 2 init:)
		(aBird setLoop: 0 setPri: 4 ignoreActors: init: setCycle: Fwd)
		(HandsOff)
		(= gCurrentStatus 4)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 28 2 #at -1 130) ; "(Or just keep looking in all the wrong places?)"
				(= seconds 60)
			)
			(2
				(= seconds 2)
