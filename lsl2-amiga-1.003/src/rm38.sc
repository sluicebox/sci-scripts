...
(instance rm38Script of Script
	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			(if (Said '/lagoon,water,lagoon')
				...
				(Print 38 7 #at -1 130) ; "(Where "something" is defined as your lunch!)"
			)
			(if (Said '/chain')
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(Print 38 19 #at -1 130) ; "(Gee. This is exciting.)"
				(= seconds 7)
			)
			(9
				(Print 38 20 #at -1 130) ; "(Are we having fun yet?)"
			)
			(10
				(User canInput: 0)
				(= cycles (= seconds 0))
				(gEgo setCel: 255 setCycle: CT 3 969 self)
			)
			(11
