...
(instance rm63 of Rm
	...
	(method (init)
		...
		(aSmoker2 setPri: 8 cycleSpeed: 2 setCycle: Fwd stopUpd: init:)
		(aSmoker3 setPri: 7 cycleSpeed: 3 setCycle: Fwd stopUpd: init:)
		(aJohnLight setPri: 3 stopUpd: init:)
		(aJohnDoor setPri: 5 stopUpd: init: setScript: johnScript)
		(aJohnUser1 setPri: 0 illegalBits: 0 setCycle: Walk init: hide:)
		(aJohnUser2 setPri: 0 illegalBits: 0 setCycle: Walk init: hide:)
		(aEmergencyExit illegalBits: 0 setPri: 15 setStep: 5 5 stopUpd: init:)
		(NormalEgo 0)
		(gEgo posn: 37 104 init: observeControl: 16384)
...
(instance rm63Script of Script
	...
	(method (handleEvent event)
		...
		(if (Said 'bang/door')
			...
			(Print 63 6 #at -1 130) ; "(You never were good at jokes!)"
		)
		(if (Said 'talk')
		...
		(if (Said 'drain,apply,drop/rejuvenator/bolt,door,cord')
			(cond
				...
				(else
					(Print 63 23) ; "Ok. You carefully pour your bottle of highly flammable hair rejuvenator on the emergency exit's padlock. It makes the lock look really new!"
		...
		(if
			...
			(cond
				...
				((not (gEgo has: 25)) ; Bobby_Pin
					...
					(Print 63 26 #at -1 130) ; "(You've heard that line before!)"
				)
				(else
					(gGame changeScore: 5)
		...
		(if (Said 'man,(ask<for)/cigarette')
			...
			(Print 63 35 #at -1 130) ; "(Why, Larry! You don't even smoke!)"
		)
	)
