...
(instance rm32 of Rm
	...
	(method (init)
		...
		(aPorthole setCycle: Fwd cycleSpeed: 5 isExtra: 1 init:)
		(if gTimesInRm33
			(Load rsVIEW 309)
			(Load rsSOUND 6)
...
(instance rm32Script of Script
	...
	(method (handleEvent event)
		...
		(if (Said 'look<on,over,above/cabinet[<lid]')
			...
			(Print 32 2 #at -1 130) ; "(But remember this if you ever play "King's Quest III.")"
		)
		...
		(if (Said 'look>')
			(if (Said '/bed')
				...
				(if (> gFilthLevel 10)
					(Print 32 6 #at -1 130) ; "(A condition not unknown to you!)"
				)
			)
		...
		(if (Said '(get,climb<in),board,board/cabinet')
			...
			(Print 32 16 #at -1 130) ; "(Bet you were expecting another cheap "out of the closet" joke!)"
		)
		(if (Said 'get/dirt')
