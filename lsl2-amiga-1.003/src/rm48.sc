	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			...
			(if (Said 'dive,hop')
				...
				(Print 48 3 #at -1 130) ; "(The idea is to NOT do that!)"
			)
		...
		(if
			...
			(if (!= state 13)
				...
				(Print 48 8 #at -1 130) ; "(At least you still have your beautiful, long blond hair!)"
				(= gCurrentEgoView 149)
				(= gBraContents 0)
		...
		(switch (= state newState)
			...
			(14
				...
				(if (!= gScore oldScore)
					(= gScore oldScore)
					(SL doit:)
					(Print 48 13 #at -1 130) ; "(By the way, you didn't think you were going to keep all those free points, did you?)"
				)
				(gCurRoom newRoom: 50)
			)
			(15
