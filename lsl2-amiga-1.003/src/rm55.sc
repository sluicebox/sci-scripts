...
(instance rm55 of Rm
	...
	(method (init)
		...
		(Load rsVIEW 520)
		(super init:)
		(gAddToPics add: aMachine1 aMachine2 aSign doit:)
		(aPlate init:)
		(aDoor setPri: 11 cycleSpeed: 1 stopUpd: init:)
		(aParachute setCel: 0 setPri: 11 cycleSpeed: 1 setCycle: Fwd init:)
		(aPlane startX: 306 startY: 22 endX: 22 endY: 22 init:)
		(aSidewalkNorth
			setLoop: 1
			setPri: 0
			setStep: 1 1
	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			...
			(if (Said '/dispenser')
				...
				(Print 55 5 #at -1 130) ; "(It's a little large for an insurance machine, isn't it?)"
			)
			...
			(if (Said '/woman,agent')
				...
				(Print 55 8 #at -1 130) ; "(Trust me on this one!)"
			)
			(if (Said '/menu,sign')
		...
		(if (Said 'talk/woman,agent')
			(cond
				...
				((== orderedSpecial 2)
					...
					(Print 55 17 #at -1 130) ; "(She does not respond.)"
				)
				((and orderedSpecial (< orderedSpecial 2))
					(Print 55 18) ; ""Hey, mama, hurry up with that chow," you order."
					(Print 55 19 #at -1 130) ; "(It affects her speed not in the least!)"
				)
				(else
		...
		(if (Said 'buy,buy/special,appetizer')
			(cond
				((or orderedSpecial (not ((gInventory at: 25) ownedBy: gCurRoomNum))) ; Bobby_Pin
					...
					(Print 55 23 #at -1 130) ; "(And you don't want another!)"
				)
				((!= gCurrentStatus 0)
					(NotNow) ; "Not now!"
				)
		...
		(if
			...
			(cond
				...
				((not ((gInventory at: 24) ownedBy: gCurRoomNum)) ; Parachute
					...
					(Print 55 33 #at -1 130) ; "(But it did swallow another of your hundred dollar bills!)"
				)
				(else
					(self changeState: 5)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(16
				...
				(Print 55 45 #at -1 130) ; "(What a shame the waitress never heard of Dr. Heimlich!)"
				(= gCurrentStatus 1001)
			)
		)
	)
