		...
		(if (and gHenchOnScreen (< (henchScript state:) 5) (Said '/woman>'))
			(cond
				...
				(else
					...
					(Print 35 6 #at -1 130) ; "(And vice versa.)"
				)
			)
		)
		(if (Said 'look>')
			...
			(if (Said '/buffet,man,woman,children')
				...
				(Print 35 11 #at -1 130) ; "(And they don't want to be bothered by you!)"
			)
			(if (Said '/cup,craft')
		...
		(if (Said 'talk/agent')
			(cond
				...
				(else
					...
					(Print 35 25 #at -1 130) ; "(Well, that didn't work Larry!)"
				)
			)
		)
		(if (Said 'talk/woman')
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(3
				...
				(Print 35 41 #at -1 130) ; "(Geez, Larry. A real woman! Talking to you!!)"
				(= seconds 5)
			)
			(4
