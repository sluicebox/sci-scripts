...
(instance rm103 of Rm
	...
	(method (init)
		...
		(super init:)
		(theSound init:)
		(aSignWest ignoreActors: init: stopUpd:)
		(aSignEast ignoreActors: init: stopUpd:)
		(aApplause init: stopUpd:)
		(aTV ignoreActors: setCycle: Fwd init:)
		(aCameraMonitor ignoreActors: isExtra: 1 init: setCycle: Fwd)
		(aView1 ignoreActors: addToPic:)
		(aMC ignoreActors: stopUpd: init:)
		(aView2 ignoreActors: addToPic:)
		(aGirl ignoreActors: setCycle: Fwd stopUpd: init:)
		(aView3 ignoreActors: addToPic:)
		(aBoy1 ignoreActors: init: stopUpd:)
		(aStoolLarry ignoreActors: init: stopUpd:)
		(aLarry ignoreActors: init: hide:)
		(aView4 ignoreActors: addToPic:)
		(aBoy3 ignoreActors: init: stopUpd:)
		(aCameraman
			setCycle: Walk
			moveSpeed: 2
			cycleSpeed: 2
		...
		(gEgo view: 100 loop: 1 posn: 291 92 setCycle: Walk setPri: 5 init:)
		(HandsOff)
		(= gCurrentStatus 18)
...
(instance rm103Script of Script
	...
	(method (handleEvent event)
		...
		(if
			...
			(= oldPrompt (User prompt:))
			(User prompt: {Give her your best line...} getInput: event)
			(User prompt: oldPrompt)
			(Format @userLine (User inputLineAddr:))
			(if (< (StrLen @userLine) 7)
				(Format @userLine {Uh, I dunno.})
			)
			(if (== state 15)
				(self changeState: 17)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gEgo setMotion: MoveTo 205 92 self)
			...
			(12
				...
				(Print 103 21 #at -1 130) ; "("Like you know" it seems obvious her elevator doesn't reach her penthouse!)"
				(StopTalk aGirl)
				(StartTalk aBoy1 0)
				(= seconds 4)
			)
			(13
				...
				(Print 103 24 #at -1 130) ; "(You're sure not even this mental midget would fall for a cornball line like that!)"
				(StopTalk aBoy1)
				(StartTalk aGirl 3)
				(= seconds 4)
			)
			(14
				...
				(Print 103 27 #at -1 130) ; "(So much for your theory, Larry!)"
				(minicamScript changeState: 7)
				(= seconds 4)
			)
			(15
			...
			(20
				...
				(Print 103 36 #at -1 130) ; "(How's that for a way with words?)"
				(StopTalk aBoy3)
				(StartTalk aGirl 3)
				(= seconds 4)
			)
			(21
				...
				(Print 103 39 #at -1 130) ; "(Do you think anyone's still watching this tripe?)"
				(StopTalk aGirl)
				(StartTalk aMC 5)
				(= seconds 4)
			)
			...
			(24
				...
				(Print 103 46 #at -1 130) ; "(At least his garden would be well-fertilized!)"
				(StartTalk aGirl 3)
				(StopTalk aBoy1)
				(= seconds 4)
			)
			...
			(31
				...
				(Print 103 57 #at -1 130) ; "(Does this show supply antacids?)"
				(StartTalk aGirl 3)
				(StopTalk aBoy3)
				(= seconds 4)
			)
			...
			(34
				...
				(Print 103 61 #at -1 130) ; "(Something makes you feel the "both" doesn't include you!)"
			...
			(35
				...
				(Print 103 65 #draw #at -1 130) ; "(Did he say bachelor number TWO?)"
			...
			(38
				...
				(Print 103 71 #at -1 130) ; "(audience goes oooooh)"
			...
			(42
				...
				(Print 103 78 #at -1 130) ; "(a smattering of applause)"
			...
			(45
				(gEgo setMotion: MoveTo 291 92 self)
			)
			(46
