...
(procedure (insertSortedByPlane oList oNew &tmp oListItem curNode)
	(if (oList isDuplicate: oNew)
		(oList delete: oNew)
	)
	(cond
		((oList isEmpty:)
			(oList addToFront: oNew)
		)
		((> (oNew priority:) ((oList at: 0) priority:))
			(oList addToFront: oNew)
		)
		(else
			(for
				((= curNode (oList next: (oList first:))))
				curNode
				((= curNode (oList next: curNode)))
				
				(= oListItem (KList 8 curNode)) ; NodeValue
				(if (> (oNew priority:) (oListItem priority:))
					(oList addAfter: (KList 8 (oList prev: curNode)) oNew) ; NodeValue
					(return)
				)
			)
			(if (== 0 curNode)
...
(class Plane of Obj
	...
	(method (init l t r b il it_ ir ib &tmp [temp0 2])
		(insertSortedByPlane gPlanes self)
		(if (== resX -1)
			(= resX gScreenWidth)
		)
		(if (== resY -1)
	...
	(method (moveTo x y &tmp nWidth nHeight)
		(= nWidth (- right left))
		(= nHeight (- bottom top))
		(self setRect: x y (+ x nWidth) (+ y nHeight))
	)

	(method (findCastMember who &tmp i aCast node aObj)
		(for ((= i 0)) (< i (casts size:)) ((++ i))
			(= aCast (casts at: i))
			(for
				((= node (aCast first:)))
				node
				((= node (aCast next: node)))
				
				(= aObj (KList 8 node)) ; NodeValue
				(if (and (aObj name:) (who compare: (aObj name:)))
					(return aObj)
				)
			)
		)
	...
	(method (initItems oItem &tmp oTmp i)
		(for ((= i 0)) (< i argc) ((++ i))
			(= oTmp [oItem i])
			(cond
				((oTmp isKindOf: View)
					(oTmp init: (self getMainCast:))
				)
				((oTmp isKindOf: Feature)
					(oTmp init: self)
				)
			)
		)
	...
	(method (disable)
		(insertSortedByPlane gDisabledPlanes self)
		(gPlanes delete: self)
	)
