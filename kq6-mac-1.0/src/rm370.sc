...
(instance rm370 of KQ6Room
	...
	(method (init)
		...
		(rHand init: stopUpd:)
		(myHead init:)
		(gEgo view: 374 normal: 0 loop: 0 cel: 0 posn: 158 147 init:)
		(kingarm init: stopUpd:)
		(queenHand init:)
		(alexHead2 init:)
		(candle1 init: setCycle: Fwd)
		(candle2 init: setCycle: Fwd)
		(if (and (IsFlag 1) (not (IsFlag 3)))
...
(instance caughtAtGateTXT of CartoonScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(15
				(kingarm setLoop: 2 setCycle: End)
				(AzurePrint addText: 1 0 1 8 init:) ; "(FINDING HIMSELF CLEVER) A dilemma then--whom shall I obey in regards to your fate? The Oracle or the Crown?"
			)
			(16
				(kingarm setLoop: 2 setCycle: End)
				(AzurePrint addText: 1 0 1 9 init:) ; "But since Alhazred did not dictate HOW I was to dispose of intruders, and since you cannot possibly survive the catacombs, your imprisonment there should serve both purposes quite admirably."
			)
			(17
				(kingarm setCycle: Beg)
				(= cycles 2)
			...
			(18
				(AlexPrint addText: 1 0 1 10 init:) ; "(FIRM, CALM) I will not resist you in this, my lord. I shall do my best to save your daughter."
			)
			(19
				(= cycles 2)
			)
			(20
				(kingarm setLoop: 2 setCycle: End)
				(AzurePrint addText: 1 0 1 11 init:) ; "(CONDESCENDING) Hmph. First I must tell you that the catacombs are a labyrinth of rooms, a place of exceeding danger. You will need many tools and clear wits to survive it."
			)
			(21
				(kingarm setCycle: Beg)
				(SetFlag 2)
