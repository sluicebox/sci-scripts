...
(class Kq6 of Game
	...
	(method (init &tmp [temp0 20])
		...
		(Platform 0 4 0 7 icon0 icon1 icon2 icon3 icon4 icon5 icon6)
	...
	(method (save)
		(if (and (not (IsFlag 49)) (u>= (MemoryInfo 0) 1500)) ; LargestPtr
			(super save: &rest)
			(self
				setCursor:
	...
	(method (restore)
		(if (or (not (IsFlag 49)) (u>= (MemoryInfo 0) 1500)) ; LargestPtr
			(super restore: &rest)
		else
			(gMessager say: 7 0 15 0 0 0) ; "Sorry! There's not enough memory to do that right now. Try it a little later and/or on another screen."
		)
	...
	(method (quitGame &tmp temp0)
		...
		(if (and (== gCurRoomNum 740) global83)
			(DoAudio audCD 3)
			(= gQuit 1)
		else
			(gTheIconBar disable:)
				...
				(not
					(= gQuit
						(KQ6Print
							posn: 59 85
							font: 4
							addButton: 1 4 0 12 0 115 36 0 ; "Yes"
							addButton: 0 4 0 11 0 65 36 0 ; "NO"
							font: 1
							say: 4 0 0 0 0 0 0 ; "There's no reason to use that object there."
							init:
						)
					)
				)
		...
		(if (not argc)
			...
			(= temp0
				(KQ6Print
					posn: 59 85
					font: 4
					addButton: 1 5 0 13 0 115 36 0 ; "Yes"
					addButton: 0 5 0 14 0 65 36 0 ; "No"
					font: 1
					say: 5 0 0 0 0 0 0 ; "There's no reason to use that object there."
					init:
				)
			)
			(Platform 0 4 4 5 -1)
...
(instance icon6 of Kq6IconItem
	...
	(method (doit)
		(if (u>= (MemoryInfo 0) 1500) ; LargestPtr
...
(instance swordCursor of Cursor ; UNUSED
	(properties
		view 990
		loop 3
		cel 14
