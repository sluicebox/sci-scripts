...
(instance rm740 of CastleRoom
	...
	(method (handleEvent event)
		(if
			...
			(gGame handsOff:)
			((ScriptID 740 7) add: -1 2 2 21 0) ; roomConv, MISSING MESSAGE
...
(instance sCredits of Script
	...
	(method (init)
		(if (not register)
			...
			(if global83
				...
				(gGlobalSound number: 10 loop: 1 play: self)
			)
			(++ register)
		)
		(super init: &rest)
	...
	(method (changeState newState &tmp [temp0 50] [temp50 500] temp550 temp551)
		(switch (= state newState)
			(0
				(if local7
					(Message msgGET 194 3 0 0 local6 @temp0)
					(Message msgGET 194 3 0 0 (++ local6) @temp50)
				else
					(Message msgGET 194 0 0 0 local6 @temp0)
					(Message msgGET 194 0 0 0 (++ local6) @temp50)
				)
				(if (>= (++ local6) 28)
					(= local7 1)
					(= local6 1)
			...
			(1
				(Display 740 0 dsRESTOREPIXELS local11)
				(Display 740 0 dsRESTOREPIXELS local10)
				(Display 740 0 dsRESTOREPIXELS local9)
				(Display 740 0 dsRESTOREPIXELS local8)
				(if (and (not local7) (== local6 15))
			...
			(5
				...
				(Display @temp50 dsCOLOR 7 dsCOORD 64 60 dsWIDTH 200 dsFONT 3110 dsALIGN alCENTER)
				(= seconds 4)
			)
			(6
				(DrawPic 99)
