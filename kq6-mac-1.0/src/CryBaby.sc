			...
			(switch theVerb
				...
				(44 ; teaCup
					(cond
						...
						(else
							(gMessager say: 9 56 22 1 0 480) ; "The tears might be useful for the "Make Rain Spell," but the spell book calls for a teapot. That lamp looks nothing like a teapot."
						)
					)
				)
				(24 ; sacredWater
		...
		(switch (= state newState)
			...
			(1
				(gEgo
					...
					cel: 0
					posn:
						(+ ((ScriptID 480 6) x:) 26) ; myBottle
						(+ ((ScriptID 480 6) y:) 16) ; myBottle
					normal: 0
					cycleSpeed: 12
					setCycle: End self
				)
