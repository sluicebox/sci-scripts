...
(instance drinkPotionGenieScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				(gGlobalSound4 number: 925 loop: 1 play:)
				(= cycles 1)
			)
			(14
				(gEgo setCycle: End self)
...
(instance genieBadgerOwnerScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if (== gMsgType 2)
					(gMessager say: 1 0 3 1 self 280) ; "(ANGRY) Your candy dish is empty!"
				else
					(KQ6Print
						font: gUserFont
						posn: 120 40
						width: 175
						addText: 1 0 3 1 ; "(ANGRY) Your candy dish is empty!"
						init: self
					)
				)
			)
			...
			(6
				(if (== gMsgType 2)
					(gMessager say: 1 0 3 3 self 280) ; "(IMPATIENT) Well, get some more then!"
				else
					(KQ6Print
						font: gUserFont
						posn: 120 40
						width: 175
						addText: 1 0 3 3 ; "(IMPATIENT) Well, get some more then!"
						init: self
					)
				)
			)
			...
			(8
				(if (== gMsgType 2)
					(gMessager say: 1 0 3 5 self 280) ; "(TEMPER TANTRUM) OOOH! I HATE not getting what I want!"
				else
					(KQ6Print
						font: gUserFont
						posn: 120 40
						width: 175
						addText: 1 0 3 5 ; "(TEMPER TANTRUM) OOOH! I HATE not getting what I want!"
						init: self
					)
				)
			)
