	...
	(method (changeState newState &tmp temp0 [temp1 200])
		(switch (= state newState)
			...
			(9
				...
				(repeat
					(Display {Restore} dsCOORD 74 120 dsWIDTH 70 dsCOLOR 47 dsFONT gUserFont)
					(Display {Restart} dsCOORD 144 120 dsWIDTH 70 dsCOLOR 47 dsFONT gUserFont)
					(Display {Quit} dsCOORD 214 120 dsWIDTH 70 dsCOLOR 47 dsFONT gUserFont)
					(switch
						(= temp0
							(Print
								window: DeathWindow
								posn: 70 134
								addButton: 1 {Restore} 0 0
								addButton: 2 {Restart} 70 0
								addButton: 3 {Quit} 140 0
								init:
							)
						)
						(1
