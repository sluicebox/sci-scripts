...
(class Inv of IconBar
	...
	(procedure (DrawInvWindow whom selection &tmp numOwned tallestInv widestInv numIcons tallestIcon iconBarWidth cWide cHigh node obj invW invH iTop iLeft iBottom iRight numRows atX atY firstX i)
		...
		(if (not numOwned)
			(if 1
				(= local1 (Memory memALLOC_CRIT (StrLen normalHeading)))
				(= local2 (Memory memALLOC_CRIT (StrLen normalHeading)))
				(= local3 (Memory memALLOC_CRIT (StrLen empty)))
				(= local4 (Memory memALLOC_CRIT (StrLen empty)))
				(StrSplitInTwo local1 local2 normalHeading)
				(StrSplitInTwo local3 local4 empty)
				(Printf 995 1 local1 local3 local2 local4) ; "%s %s"
				(Memory memFREE local1)
				(Memory memFREE local3)
				(Memory memFREE local2)
				(Memory memFREE local4)
			)
			(return 0)
		)
		...
		(= local0 (/ numOwned numRows))
		(if (< (* numRows local0) numOwned)
			(++ local0)
		)
		(= invW (Max (+ 4 iconBarWidth) (* local0 (+ 4 widestInv))))
