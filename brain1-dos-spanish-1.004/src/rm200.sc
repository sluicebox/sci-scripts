			...
			((& [local3 local0] (= temp0 [local402 (mod (+ local131 2) 4)]))
				(cond
					((== temp0 64)
						...
						(Wait 0)
						(Wait 45)
						(Print 200 9) ; "You leave the maze through the north exit."
						(gCurRoom newRoom: local147)
					)
					((== temp0 128)
						(gCMusic2 number: 937 setLoop: 1 play:)
						(Wait 0)
						(Wait 45)
						(Print 200 10) ; "You leave the maze through the south exit."
						(gCurRoom newRoom: local148)
					)
				)
			)
...
(instance downMover of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 200 11) ; "Pushing this button moves the elevator car down."
			)
			((!= theVerb 3)
				(super doVerb: theVerb &rest)
			)
			...
			(else
				...
				(Print 200 12 #time 5 #dispose #at -1 12) ; "Try as you might, you just can't bump through the floor."
			)
		)
		(self cel: 1)
	)
...
(instance upMover of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 200 13) ; "Pushing this button moves the elevator car up."
			)
			((!= theVerb 3)
				(super doVerb: theVerb &rest)
			)
			...
			(else
				...
				(gCMusic2 number: 920 setLoop: 1 play:)
				(ShakeScreen 2 ssUPDOWN)
				(Print 200 14 #time 5 #dispose #at -1 12) ; "Try as you might, you just can't bump through the ceiling."
			)
		)
		(self cel: 1)
	)
...
(instance northMazeDoor of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 15) ; "This is the north door out of the maze."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance southMazeDoor of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 16) ; "This is the south door out of the maze."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance flasher of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 20) ; "The lights on the control panel show that the elevator is working -- always a comfort."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance railing1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 21) ; "You can hold on to these if you tend to become seasick."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance mach of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 22) ; "The machinery allows the elevator to turn in any direction and to move through the maze."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bulb of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 200 23) ; "This powerful bulb provides enough light to let you see some of the maze outside the elevator car. It's just one more of Dr. Brain's brilliant ideas."
			)
			(else
				(super doVerb: theVerb &rest)
			)
