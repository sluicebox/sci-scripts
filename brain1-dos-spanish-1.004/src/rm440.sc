...
(instance rm440 of Rm
	...
	(method (init &tmp temp0)
		(= picture (proc0_3 440 441 440 440 440))
		(if (not (IsFlag 0))
		...
		(LoadMany rsVIEW (proc0_3 451 452 450 450 450))
		(bookShelf init:)
		(patch init:)
		(phone init:)
		(teletype init:)
		...
		(cond
			...
			((== theVerb 2)
				...
				(if 1
					...
					(proc5_14 440 1 global408 global409) ; "This note says "%s.""
				...
				(switch local21
					...
					(else
						...
						(if 1
							...
							(proc5_14 440 5 global408 global409) ; "One of the books is called "%s"."
							(Memory memFREE global408)
							(Memory memFREE global409)
						)
					)
		...
		(cond
			...
			((== theVerb 2)
				(if cel
					...
					(proc5_9 440 11) ; "This patch doesn't look very secure."
				)
			)
			(else
				(super doVerb: theVerb &rest)
		...
		(cond
			...
			((== theVerb 3)
				(if (not local31)
					...
					(proc5_9 440 13) ; "You just hear a dial tone."
					(gCMusic2 stop:)
				else
					(phoneS stop:)
					...
					(switch local22
						(0
							(proc5_14 440 14) ; ""Hello? Hello?" It's someone from the pizza parlor asking why Dr. Brain hasn't paid last month's pizza bills."
						)
						(1
							(proc5_14 440 15) ; ""Hello? Hello?" It's someone from the rent office asking about Dr. Brain's rent payment."
						)
						(2
							(proc5_14 440 16) ; ""Hello? Hello?" It's Dr. Brain's publisher asking why the manuscript for his latest book is three months late."
						)
						(3
							(proc5_14 440 17) ; "You're beginning to get an idea as to why Dr. Brain never answers the phone."
						)
						(else
							(proc5_14 440 18) ; "Better leave the phone alone; they're starting to sound mad."
						)
					)
					(++ local22)
...
(instance teletype of Prop
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 3)
				(proc5_9 440 20) ; "Don't bother -- it'll all be in the paper or Junior Scientist magazine tomorrow."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(cond
			...
			((== theVerb 3)
				(if local23
					...
					(proc5_9 440 21) ; "The drawer is locked."
				)
			)
			((== theVerb 2)
				(cond
					((not cel)
						(proc5_9 440 22) ; "This drawer has an unusual red lock."
					)
					((gEgo has: 20)
						(proc5_9 440 23) ; "There is nothing else of interest in the drawer."
					)
					(else
						(proc5_9 440 24) ; "The only thing of interest in the drawer is the ring. It looks as though the face of the ring can be rotated."
					)
				)
			)
			(else
		...
		(switch (= state newState)
			...
			(1
				(if (not (gEgo has: 20))
					...
					(proc5_9 440 25) ; "The drawer opens to reveal a cheap-looking ring."
				)
				(self dispose:)
			)
		)
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 3)
				(cond
					((IsFlag 38)
						(proc5_9 440 27) ; "You have already proven your job skills. Now you need to meet with Dr. Brain."
					)
					((jobPuzzle show:)
						(jobPuzzle dispose:)
					)
...
(instance desk of Feature
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 3)
				(proc5_9 440 29) ; "It feels like good, solid oak to you."
			)
			(else
...
(instance wasteBasket of Feature
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 3)
				(proc5_9 440 32) ; "You can take out the trash later, once you get the lab assistant job. Nice of you to offer, though."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance jobPuzzle of PuzzleBar
	...
	(method (show)
		...
		(if local62
			(self solvePuzzle: 2)
			(proc5_9 440 34) ; "You hear a voice from the next room."
			(proc5_9 440 35) ; "Very good. You seem to have mastered the skills you will need. Please meet me in the Control Room behind the bookshelf as soon as you can."
		)
		(phoneS vol: 127)
		(teletypeS vol: 127)
		(return local62)
	...
	(method (showHelp)
		(proc5_9 440 36) ; "During your journey through the Castle of Dr. Brain, you've demonstrated a number of skills that should qualify you for the lab assistant job. Now prove it by placing the job skill plaques on the board and matching them with the puzzle plaques you've earned along the way."
		(proc5_9 440 37) ; "Some of your puzzles might apply to two or more skills, so make sure you choose the one which will let you use all of your puzzle plaques. If you get stuck, you could always buy an answer with one of your hint coins (but Dr. Brain would probably not be impressed!)"
	)

	(method (buyClue &tmp temp0 temp1)
		(cond
			(local60
				(proc5_9 440 38) ; "Just click on the skill names."
			)
			((super buyClue: &rest)
				(if local59
					(= temp0 (localproc_0 0))
...
(instance choice of CodeIcon
	...
	(method (show)
		(= view (proc0_3 451 452 450 450 450))
		(super show: &rest)
	)
		...
		(cond
			...
			(else
				(if 1
					...
					(proc5_14 440 39 global408 global409) ; "Nice try, but solving that puzzle doesn't prove you are skilled in %s."
					(Memory memFREE global408)
					(Memory memFREE global409)
				)
				(localproc_1)
...
(instance jobWindow of SysWindow
	...
	(method (open)
		(super open: &rest)
		(DrawCel (proc0_3 451 452 450 450 450) 0 0 0 local126 15)
	)
)
...
(instance bookPuzzle of PuzzleBar
				...
				(cond
					(
						...
						(cond
							((not temp7)
								(gCMusic2 number: 58 setLoop: 1 play:)
								(proc5_9 440 40) ; "The book won't fit on the shelf there."
							)
							([local137 temp7]
								(gCMusic2 number: 58 setLoop: 1 play:)
								(proc5_9 440 40) ; "The book won't fit on the shelf there."
					...
					(else
						(proc5_9 440 41) ; "You can't put the book here."
					)
				)
				(event dispose:)
				(return 0)
	...
	(method (showHelp)
		(proc5_9 440 42) ; "You're sure that this bookshelf blocks the secret passage to Dr. Brain's secret basement lab. (He's into secrets.) But you'll have to move the books in exactly the right order to open the bookshelf."
		(proc5_9 440 43) ; "The clue sheet you got from the robot room should give you some help -- if you can get it to make sense. If you're totally stuck, a hint coin will suggest how you can do that."
	)

	(method (buyClue)
		(cond
			((IsFlag 51)
				(proc5_9 440 44) ; "You already have all the clues you need for this puzzle. If you think you've done something wrong, click on Exit and come back to start over."
			)
			((super buyClue: &rest)
				(proc5_9 440 45) ; "That clue sheet doesn't seem to make much sense, does it? Maybe there is something you can do to it which will make it more readable. Check out the other things you're carrying."
				(bookPuzzle goAway:)
			)
		)
	)
...
(instance chicken of CodeIcon
	...
	(method (select)
		...
		(if (== [local195 local205] 69)
			...
			(proc5_9 440 46) ; "Hey! Leave that poor bird alone! (For now.)"
		)
	)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				...
				(cond
					((< local205 2)
						(proc5_9 440 47) ; "You'd better stop and think about what you're doing. Random guesses won't cut it here."
					)
					((< local205 4)
						(proc5_9 440 48) ; "Maybe you should recheck the code sheet. You took a wrong turn a while ago."
					)
					((< local205 6)
						(proc5_9 440 49) ; "So close, and yet so far. You just don't seem to be getting the last few steps."
					)
					(else
						(proc5_9 440 50) ; "Sorry, you almost got it, but messed up a few steps back."
					)
				)
				(localproc_5)
				(= local206 0)
...
(instance TTDBoard of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 51) ; "This board is labelled "Things to Do Now!"."
			)
			((== theVerb 3)
				(proc5_9 440 52) ; "You can start doing these once you become Dr. Brain's lab assistant. You'd better wait until then."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance otherDeskDrawers of Feature
	...
	(method (doVerb theVerb)
		(if (or (== theVerb 3) (== theVerb 2))
			(proc5_9 440 53) ; "This "drawer" is actually a false front -- the desk was cheaper that way."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance blotter of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 54) ; "The desk blotter is covered with esoteric scribblings. From them, you learn that E=mc squared, and that Dr. Brain's favorite food is a thick crust sausage and artichoke pizza."
			)
			((== theVerb 3)
				(proc5_9 440 55) ; "There's a little bit of left-over pizza on the blotter. You decide to leave it there."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance stapler of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 56) ; "It's a desk stapler."
			)
			((== theVerb 3)
				(proc5_9 440 57) ; "This looks like the only way Dr. Brain can hold things together around here."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance papersOnFloor of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 58) ; "It appears that Dr. Brain could use a housekeeper as well as a lab assistant. Busy scientists go through a lot of paper."
			)
			((== theVerb 3)
				(proc5_9 440 59) ; "There are too many papers here to clean up right now."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pictureWindow of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 60) ; "You admire the landscape beyond the window for a few moments. Then you remember that you're in the basement."
			)
			((== theVerb 3)
				(proc5_9 440 61) ; "It feels like a TV screen."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance mirror of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 62) ; "It's a mirror frame, minus the mirror (a mere frame?)."
			)
			((== theVerb 3)
				(proc5_9 440 63) ; "You know, this really looks like one of those things you'd find in an old house in a movie. You really think you ought to be able to move it and have the bookshelf slide into the wall, revealing a secret passage to Dr. Brain's secret basement lab."
				(proc5_9 440 64) ; "But nothing happens when you push the mirror frame. It does get you thinking about the bookshelf, though. What if there really was a way to move it?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance dart of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 440 65) ; "Dr. Brain uses a scientific system of pseudo-random determination to decide which job to tackle next. In other words, he throws darts."
			)
			((== theVerb 3)
				(proc5_9 440 66) ; "You'd better leave the dart alone for now. It could be part of an important scientific experiment."
			)
			(else
				(super doVerb: theVerb)
			)
