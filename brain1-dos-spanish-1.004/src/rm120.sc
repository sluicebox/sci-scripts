...
(procedure (localproc_13 &tmp [temp0 5])
	...
	(proc5_8 (Format @temp0 120 43 (+ local588 1)) [local0 16] 21 38 -1 gUserFont) ; "%d"
				...
				(cond
					...
					(else
						(proc5_14 ; "The Math Marvel is one of Dr. Brain's inventions. You need to construct correct addition, subtraction, multiplication, and division problems to get what's in the treasure chest.%s"
							...
							(if (< local124 2)
								{ Cada problema requiere un n\a3mero de cada fila.}
							else
								{}
							)
						)
		...
		(if (or local113 (super buyClue: &rest))
			(if (== local125 2)
				...
				(if (== (gGame printLang:) 1)
					...
					(proc5_9 120 18) ; "Empieza por la fila superior y trabaja hacia abajo. Una t\82cnica que te ser\a0 \a3til es la de "deslizar" los azulejos hasta que encajen en su posici\a2n. Supongamos que quieres situar los dos azulejos iniciales. Mueve el azulejo "1" hasta colocarlo debajo de la posici\a2n "2", y coloca el azulejo "2" debajo de \82ste. Ahora mueve los azulejos procurando que la casilla libre se mantenga en la posici\a2n "2", y seguidamente desliza ambos azulejos hacia arriba, coloca la casilla libre en la posici\a2n "1", desliza el azulejo "1", y seguidamente desliza el azulejo "2" hacia arriba. Resuelve las dem\a0s casillas de arriba abajo usando este mismo m\82todo."
				)
				(if (!= local125 0)
					(if (== (gGame printLang:) 1)
						...
						(proc5_9 120 20) ; "Las dos filas inferiores son m\a0s dif\a1ciles. Imagina que estas casillas son puntos alrededor de un c\a1rculo. Supongamos que casi has resuelto la tercera fila, pero el "10" a\a3n no sigue el orden (supongamos que el orden es 9-11-10-12). Mueve las piezas de la fila inferior hasta que la casilla libre est\82 debajo del "10", desliza el "10" hacia abajo, y gira ambas filas hasta que la casilla libre coincida entre el "9" y el "11", asegur\a0ndote de que el "10" est\82 situado debajo de \82stos. Desliza el "10" hasta colocarlo en su posici\a2n, y gira las piezas hasta conseguir que encajen en sus filas correspondientes."
					)
				)
			)
			(= local113 1)
