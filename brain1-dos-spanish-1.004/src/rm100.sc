...
(procedure (localproc_0 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
	(= temp5 (+ 6 local64 local64))
	(= temp6 0)
	(while (not temp6)
		(= temp6 1)
		(for ((= temp0 0)) (< temp0 local21) ((++ temp0))
			(= [local32 temp0] (= temp3 (Random 0 (- temp5 1))))
			(= temp2 1)
...
(instance grillScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(grill stopUpd: setScript: grillScript)
			)
		)
	)
)
...
(instance doorbell of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 100 4) ; "This looks like every other castle doorbell you've seen."
			)
			(3
				(= local64 global114)
				(switch global114
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(toneSound number: [local42 register] setLoop: 1 play:)
				(= ticks 20)
			)
			(2
			...
			(6
				(if (not (gEgo has: 27))
					(proc5_9 100 5) ; "Congratulations! You have successfully unlocked the front door. You are awarded the "Memory Puzzle Plaque". (You keep it to help you remember this feat.)"
				)
				(gEgo get: 27)
				(gCurRoom newRoom: 120)
			)
