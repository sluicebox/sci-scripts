...
(procedure (localproc_5 &tmp temp0)
	(if local20
		...
		(DrawCel (proc0_3 125 126 125 125 125) 0 0 (- 48 local22) 20 15)
	)
	(localproc_3)
)
...
(instance timeClock of PuzzleBar
	...
	(method (showHelp)
		(proc5_9 190 5) ; "Visitors to the castle need to punch in on this time clock. Dr. Brain is very big on punctuality, and expects you to punch in at exactly the right time.."
		(proc5_9 190 6) ; "You will need to punch each of your time cards in at the correct time according to the time sequence on the card."
	)
	...
	(method (select)
		(if (and (== gTheCursor 111) (super select: &rest))
			...
			(if (localproc_0)
				...
				(proc5_11 190 8) ; "It's not the right time to clock in yet."
			)
		...
		(if param1
			...
			(if (not local48)
				...
				(if 1
					...
					(Format @temp2 190 9 global408 temp0 global409 temp0) ; "The %s card has previously been used at: %s"
					(Memory memFREE global408)
					(Memory memFREE global409)
				)
				(Print @temp2 #at 166 130 #width 140 #time 3 #dispose)
			)
		else
			(if (IsObject gModelessDialog)
				(gModelessDialog dispose:)
