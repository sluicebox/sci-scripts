...
(local
	...
	[local53 10] = [{} {ADELANTE} {IZQUIERDA} {ATRAS} {DERECHA} {ACTIVAR} {DESACTIVAR} {INICIAR} {FIN} {}]
	[local63 3] = [1 3 2]
	[local66 3] = [24 25 26]
	...
	[local211 6] = [{CABEZA:} {BRAZO:} {ESTADO PROGRAMA:} {SISTEMA VERIFICADO} {ABORTA OPERACION} {ERROR PROGRAMA}]
...
(procedure (localproc_0)
	(Graph grFILL_BOX 92 4 124 109 1 [local0 2] -1 -1)
	(Graph grUPDATE_BOX 92 4 124 109 1)
)
...
(procedure (localproc_7 param1)
	...
	(if param1
		...
		(localproc_12)
		(localproc_15
			(LangSwitch {ITEM DELIVERED} {ARTICULO DADO})
			8
			[local0 4]
			5
		)
		...
		(localproc_0)
		(localproc_15
			(LangSwitch {INVALID DROP SPOT} {LUGAR NO VALIDO})
			7
			[local0 4]
			4
		)
		(localproc_15
			(LangSwitch {PROGRAM ABORTED} {PROGRAMA ABORTADO})
			8
			[local0 4]
			4
		)
...
(procedure (localproc_14 &tmp temp0 temp1)
	...
	(if global355
		(if global363
			(= temp0 (LangSwitch {BROKEN} {ROTO}))
			(= temp1 0)
		else
			(= temp0 (LangSwitch {PRESENT} {PRESENTE}))
		)
	else
		(= temp0 (LangSwitch {MISSING} {FALTA}))
		(= temp1 0)
	)
	...
	(if (== [global301 3] 8)
		(= temp0 (LangSwitch {NO ENTRIES} {NO HAY ENTRADAS}))
		(= temp1 0)
	else
		(= temp0 (LangSwitch {READY} {LISTO}))
	)
	...
	(method (animateOnce &tmp temp0 temp1 [temp2 3] temp5 temp6 temp7 [temp8 20])
		(cond
			((and (not local36) (!= (= temp1 [global301 local37]) 9))
				(Graph grFILL_BOX 131 8 143 106 1 [local0 2] -1 -1)
				(Graph grUPDATE_BOX 131 8 143 106 1)
				(if temp1
				...
				(cond
					((== temp1 5)
						(cond
							...
							((== local38 [local386 global365])
								(localproc_0)
								(localproc_15
									(LangSwitch {HAND TURNED ON} {MANO ACTIVADA})
									7
									[local0 4]
									5
								)
					...
					((== temp1 6)
						(cond
							...
							(else
								(localproc_0)
								(localproc_15
									(LangSwitch {INVALID TURNOFF} {INVALIDA})
									7
									[local0 4]
									5
								)
					...
					((== temp1 8)
						(localproc_0)
						(localproc_15
							(LangSwitch {END OF PROGRAM} {FIN DEL PROGRAMA})
							7
							[local0 4]
							5
						)
						(localproc_15
							(LangSwitch {ENCOUNTERED} {ENCONTRADO})
							8
							[local0 4]
							5
						)
						...
						(if local35
							(localproc_0)
							(localproc_15
								(LangSwitch {ITEM IN HAND} {ARTICULO EN MANO})
								7
								[local0 4]
								5
							)
							(localproc_15
								(LangSwitch {RETURN ITEM} {DEVOLVER ARTICULO})
								8
								[local0 4]
								5
							)
			...
			(local36
				...
				(localproc_15 (localproc_17 5) 7 [local0 4] 5)
				(localproc_15
					(LangSwitch {INVALID DIRECTION} {DIRECCION INVALIDA})
					8
					[local0 4]
					5
				)
			...
			((== local37 2)
				(localproc_0)
				(localproc_15
					(LangSwitch {PROGRAM REQUIRED} {REQUIERE PROGRAMA})
					7
					[local0 4]
					5
				)
