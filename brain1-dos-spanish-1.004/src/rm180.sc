...
(instance rm180 of Rm
	...
	(method (init &tmp temp0)
		...
		(LoadMany
			rsVIEW
			(proc0_3 125 126 125 125 125)
			180
			185
			(proc0_3 195 197 195 195 195)
			196
		)
...
(instance clockRadioFace of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 180 15) ; "The controls don't seem to do anything. This must be a radio-controlled radio."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance sundial of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 180 16) ; "Dr. Brain wanted to make sure he could still tell time if the power went out, so he installed this sundial. Of course, he forgot about the minor problem of having two floors of the castle between this room and the sun. That triangular thing in the center is called a "gnomon"."
			)
			(3
				(proc5_9 180 17) ; "The gnomon is firmly attached to the sundial. (If you really need a gnomon, try the sundial in "Trinity".)"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance hourGlasses of Feature
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(Print 180 18) ; "This desk features a state-of-the-art Acme 40-second time lock. Of course, Acme is in a very small state (it makes Rhode Island look like a continent). A pair of hourglasses has been provided to help open the lock."
			)
			((and (IsFlag 58) (== theVerb 3))
				(if (IsFlag 53)
					(proc5_9 180 19) ; "There is nothing more for you to do with the hour glasses."
				else
					(if local133
						(= local133 0)
...
(instance buttonPad of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 180 20) ; "Now that you've disarmed the alarm system, these buttons no longer do anything."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance switch1 of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(proc5_9 180 21) ; "This button turns the radio on and off."
			)
...
(instance switch2 of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(proc5_9 180 22) ; "This button starts or stops the clock. It also sets off the alarm."
			)
			((and (== theVerb 3) (not (IsFlag 58)))
				(gCMusic2 number: 61 setLoop: 1 play:)
				(self setCel: 1)
...
(instance switch3 of Prop
	...
	(method (doVerb theVerb &tmp temp0)
		(cond
			((== theVerb 2)
				(proc5_9 180 23) ; "This button makes the cuckoo enter or leave the clock. It also turns the clock on."
			)
			((and (== theVerb 3) (not (IsFlag 58)))
				(gCMusic2 number: 61 setLoop: 1 play:)
				(self setCel: 1)
...
(instance switch4 of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2)
				(proc5_9 180 24) ; "This button turns off the alarm (but only if the cuckoo is quiet and the radio is playing)."
			)
			((and (== theVerb 3) (not (IsFlag 58)))
				(gCMusic2 number: 61 setLoop: 1 play:)
				(self setCel: 1)
...
(instance canElf of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 180 25) ; "It's one of the famous Dancing Elves. Turn on the radio, and the Elf will dance for you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cuckoo of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 180 26) ; "It's a cuckoo, Dr. Brain's favorite bird."
			)
			(3
				(proc5_9 180 27) ; "You'd better not mess with it; that's Dr. Brain's favorite cuckoo."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		(cond
			((and (== theVerb 2) (or (gEgo has: 7) (gEgo has: 22)))
				(proc5_9 180 28) ; "It's an empty drawer."
			)
			((and (IsFlag 53) (not (gEgo has: 7)))
				(Print 180 29) ; "There is a set of three time cards in the drawer."
			)
			((!= theVerb 3)
				(super doVerb: theVerb &rest)
			)
			((not (IsFlag 58))
				(proc5_9 180 30) ; "The drawer won't open until things quiet down in here."
			)
			((not (IsFlag 53))
				(if local133
					(= local133 0)
			...
			((and (not (gEgo has: 7)) (not (gEgo has: 22)))
				(proc5_9 180 31) ; "The time cards may come in useful."
			)
			(else
				(proc5_9 180 32) ; "There is nothing more for you to do with the drawer."
			)
		)
	)
)
...
(instance clock of Prop
	...
	(method (doVerb theVerb invItem)
		(cond
			((== theVerb 2)
				(Print 180 33) ; "Visitors to the castle need to punch in on this time clock. Dr. Brain is very big on punctuality, and expects you to punch in at exactly the right time."
			)
			(
				(and
					(IsFlag 58)
				...
				(cond
					...
					((and (not (gEgo has: 7)) (not (IsFlag 1)))
						(proc5_9 180 34) ; "You'll have to get a time card before you can punch in. There are a few in the desk drawer."
					)
					((IsFlag 1)
						(proc5_9 180 35) ; "You've already punched in. Dr. Brain might get suspicious if you punch in again during the same visit."
					)
					((not (IsFlag 58))
						(proc5_9 180 36) ; "The time clock won't open until things quiet down in here."
					)
				)
			)
			(else
...
(instance wallClock of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 180 37) ; "Another of Dr. Brain's famous garage sale artifacts, this is a clock that runs backwards... sometimes. It doesn't keep very good time even when it's running (but might make a pretty good fan if you attached some fan blades)."
			)
			(3
				(proc5_9 180 38) ; "No thanks, the clock already has a perfectly good pair of hands!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
			...
			(cond
				...
				((== value 3)
					...
					(if (not local118)
						...
						(proc5_9 180 39) ; "WARNING! SECURITY ALERT! ATTEMPT TO OPEN LOCK AT WRONG TIME!"
					else
...
(instance hourGlass of PuzzleBar
	...
	(method (show &tmp temp0 temp1)
		...
		(if local118
			(self solvePuzzle:)
			(proc5_9 180 40) ; "As the last sands fall into place, and you press the "Open" button, the drawer opens to reveal three time cards."
		)
		(return local118)
	)
	...
	(method (buyClue)
		(if (super buyClue:)
			(if global114
				(proc5_9 180 41) ; "Both hourglasses start running when you press "Start". The trick is to measure 40 seconds with a 35- and a 15-second hourglass. Figure out how to end up with just five seconds of sand in the 15-second glass when the 35-second glass runs out."
				(proc5_9 180 42) ; "(Remember, you can flip EITHER glass once one runs out, so the five seconds worth of sand can be in either the top or the bottom of the 15-second glass when the 35-second glass expires.)"
			else
				(proc5_9 180 43) ; "Both hourglasses start running when you press "Start". The trick is to measure 40 seconds with a 25- and a 15-second hourglass. Since 25+15 equals 40, wait until the 25-second hourglass runs out, then turn over the 15-second hourglass. When it runs out, press the "Open" button."
			)
			(Animate (gCast elements:) 1)
		)
	)
	...
	(method (showHelp)
		(proc5_9 180 44) ; "This is one of Acme's famous "time locks". To open it, you need to press the "Open" button exactly 40 seconds after pressing "Start". You can flip either hourglass immediately after either runs out."
		(proc5_14 180 45 (if global114 35 else 25) (if global114 35 else 25)) ; "Of course, the lock is really controlled by the hourglasses, not by time, so you have a few seconds to make each flip. But you will have to flip the hourglasses in the right order to measure out 40 seconds to open the drawer. The large hourglass will run for %d seconds when flipped, and the small one for 15 seconds. Spend a Hint Coin to get some tips."
		(Animate (gCast elements:) 1)
	)
...
(instance hourGlassWindow of SysWindow
	...
	(method (open)
		(super open: &rest)
		(DrawCel (proc0_3 195 197 195 195 195) 0 0 0 22 15)
	)
)
...
(instance elevatorDoor of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 180 46) ; "This looks like an elevator door."
			)
			(3
				(if (IsFlag 1)
					(gCurRoom setScript: elevatorScript)
				else
					(proc5_9 180 47) ; "A special security feature prevents the elevator doors from being opened until you've punched in."
				)
			)
			(else
				(super doVerb: theVerb &rest)
