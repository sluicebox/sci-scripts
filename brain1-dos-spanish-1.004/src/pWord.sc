...
(local
	...
	[local16 20]
	local36
	[local37 11] = [1 17 18 6 13 22 0 24 10 7 8]
	local48 = -1
	[local49 44] = [26 24 114 35 37 35 103 46 15 46 70 57 37 57 103 68 15 68 81 79 26 79 92 90 4 101 114 112 4 112 103 123 26 123 103 134 26 134 114 145 15 145 70 156]
	local93 = -1
	[local94 11] = [{S} {I} {E} {R} {R} {A} {G} {A} {M} {E} {S}]
	local105
	local106
	local107
)
...
(procedure (localproc_1 &tmp temp0 temp1 temp2 [temp3 60] temp63 temp64 temp65)
	(= temp64 (sWordSet size:))
	(for ((= temp0 0)) (< temp0 11) ((++ temp0))
		(= temp63 (sWordSet at: temp0))
		(localproc_0 (pWord at: [local37 temp0]) @temp3)
		(if temp64
			(if (temp63 filled:)
				(localproc_0 (pWord at: (temp63 curGuest:)) @temp3)
				(temp63 upDate: 0 [local0 3])
				(temp63 showWord: @temp3)
				(if (temp63 correct:)
					(= temp65 1)
				else
					(= temp65 0)
				)
				(temp63 placeWord: @temp3 temp65 (temp63 curGuest:))
			)
		else
			(= temp2 (SWord new:))
			(= temp1 (* temp0 4))
...
(procedure (localproc_2 &tmp temp0 [temp1 250])
...
(class SWord of Obj
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4 [temp5 60] temp65 temp66)
		(= temp1 (event x:))
		(= temp2 (event y:))
		(= temp65 0)
		(= temp3 (event type:))
		...
		(if (self onMe: event)
			...
			(switch temp3
				...
				(evMOUSEBUTTON
					(if
						(or
							(= temp65 (== local48 [local37 value]))
							(== length (StrLen @local16))
						)
						(if (!= (= temp66 (localproc_2)) -1)
							((sWordSet at: temp66)
								curGuest: -1
								filled: 0
								correct: 0
						...
						(self placeWord: @local16 temp65 0)
					else
	...
	(method (flashSecretWord param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 [temp6 60])
		(Wait 0)
		(for ((= temp0 0)) (<= temp0 10) ((++ temp0))
...
(instance theWord of PuzzleBar
	...
	(method (buyClue &tmp temp0 temp1 [temp2 60])
		(if (super buyClue:)
			(for
				((= temp1 0))
