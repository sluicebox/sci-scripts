...
(procedure (proc240_30)
	(cond
		...
		(else
			(gSq5Music1 number: 101 loop: -1 play: 90)
		)
	)
)
		...
		(cond
			((== theVerb 2) ; Talk
				(cond
					...
					((IsFlag 61)
						(gMessager say: 5 2 52 0) ; "What in tarnation is going on sir?! Who locked a tractor beam onto us?"
					)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(2 ; Talk
				(if (== noun 4)
					(if (IsFlag 72)
						(gMessager say: noun 2 46 0) ; "Are we there yet, Reggie?"
					else
						(gMessager say: noun 2 58 0) ; "C'mon Ronald, let's get going; I'm late for my nail appointment."
					)
				else
					(super doVerb: theVerb &rest)
				)
							...
							(cond
								((== (eureka puke:) 7)
									(if (== gBeaState 9)
										...
										(EgoDead 37) ; "Bea is dead. In an alternate future she would've borne your son. In the future past of Space Quest 4, your son would've saved your life. But she didn't, so he couldn't--therefore you aren't."
									)
								)
								((< 0 (eureka puke:) 7)
...
(instance tkrRoger of ChoiceTalker
	...
	(method (init)
		...
		(if local79
			...
			(= gSystemWindow gSpeakWindow)
			(gSystemWindow tailX: 160 tailY: 40 isBottom: 1 xOffset: 1)
...
(instance tank of Feature
	...
	(method (doVerb theVerb)
		(cond
			...
			((IsFlag 57)
				(gMessager say: 23 0 0 0) ; "You'll have to wait until Cliffy fixes the tank before you can use it again."
			)
			(local81
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb 3 4) ; Walk, Do
			(cond
				...
				((== (chamber view:) 280)
					(gMessager say: noun 4 57 0) ; "You can't leave the cryo-chamber open like that. Remember what your mom said about leaving the refrigerator door open!"
				)
				(
					(and
						(== gEurekaLocation 6) ; thrakus
