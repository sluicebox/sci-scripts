		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'look>')
						(cond
							((or (Said '<under/table') (Said '<under/desk'))
								(if local0
							...
							((Said '/mirror')
								(if local0
									(Print 26 17 #at -1 140) ; "You don't have time for self-admiration."
								else
									(Print 26 1 #at -1 20) ; "You are not in the bathroom."
								)
							)
							((Said '/crapper')
								(if local0
									(Print 26 18 #at -1 140) ; "Nothing special. It's just a normal motel toilet."
								else
									(Print 26 1 #at -1 20) ; "You are not in the bathroom."
								)
							)
							...
							((Said '/painting')
								(if local0
									...
									(Print 26 24 #at -1 140) ; "You look at the painting and think: "Not bad, for a print.""
								)
							)
							((Said '/table,desk')
								(if local0
							...
							((Said '/rope,cable,(trap[<booby])')
								(Print 26 26 #at -1 140) ; "The booby trap consisted of a shotgun, cable, and pulleys. It was nearly a deadly combination."
								(Print 26 27 #at -1 140) ; "As you look at the mechanism that nearly cost you your life, you think... "BAINS, YOU HEARTLESS RAT!""
							)
							((Said '/wall')
								(Print 26 28 #at -1 20) ; "The walls are in desperate need of repainting."
							)
							((Said '/hanger')
								(Print 26 29 #at -1 140) ; "You see nothing of significance."
							)
							...
							((or (Said '/floor,carpet,carpet') (Said '<down'))
								(if local0
									...
									(Print 26 32 #at -1 140) ; "You look at the floor and see what appears to be a substantial amount of blood."
								)
							)
							((or (Said '/nightstand') (Said '/stand[<night]'))
								(if local0
									...
									(Print 26 33 #at -1 140) ; "It's a common type of motel nightstand, with a single drawer."
								)
							)
							((Said '/envelope')
								(if (gEgo has: 25) ; envelope
							...
							((Said '/name,address')
								(if (not (gEgo has: 25)) ; envelope
									...
									(Print 26 37 #at -1 140) ; "The letter is addressed to Woody Roberts."
								)
							)
							((Said '/television')
								(if local0
									...
									(Print 26 38 #at -1 140) ; "Nothing special. Just a normal TV, chained to the wall."
								)
							)
							((Said '/bench')
								(if local0
									...
									(Print 26 39 #at -1 140) ; "Bains must have used this chair for the booby trap....."
								)
							)
							((Said '/drawer')
								(cond
									...
									((gEgo inRect: 55 115 80 125)
										(if (== local2 1)
											(if (gEgo has: 25) ; envelope
												...
												(Print 26 42 #at -1 140) ; "You look inside the drawer ..."
												(Print 26 43 #icon 125) ; "and you find an envelope with the corner torn off."
											)
										else
											(Print 26 44 #at -1 140) ; "The drawer is closed..."
										)
									)
									(else
							...
							((Said '/blood,blood')
								(Print 26 46 #at -1 140) ; "There is a dark stain on the carpet."
							)
							((Said '/bath')
								(if local0
							...
							((Said '[<at,around][/chamber,bathroom]')
								(if local0
									(Print 26 49 #at -1 140) ; "You look around the empty bathroom. You see typical bathroom fixtures: a sink, a toilet, a shower and a mirror."
								else
									(Print 26 50 #at -1 140) ; "You see a dark stain on the carpet."
								)
							)
						)
					)
					...
					((Said 'move/blanket')
						(if (gEgo inRect: 67 85 139 122)
							(Print 26 55 #at -1 140) ; "You move it, look under it, but find nothing."
						else
							(Print 26 45 #at -1 20) ; "You are not close enough."
						)
					)
					...
					((Said 'open/drawer')
						(if (gEgo inRect: 55 115 80 125)
							(if (== local2 0)
								(Print 26 59 #at -1 140) ; "You open the drawer and look inside..."
								(if (gEgo has: 25) ; envelope
									(Print 26 60 #at -1 140) ; "You find nothing."
								else
									(= local2 1)
					...
					((Said 'close/drawer')
						(if (gEgo inRect: 55 115 80 125)
							(if (== local2 1)
								(Print 26 63 #at -1 140) ; "You close the drawer."
								(= local2 0)
							else
								(Print 26 64 #at -1 140) ; "The drawer is already closed."
							)
						else
							(Print 26 45 #at -1 20) ; "You are not close enough."
						)
							...
							(if (gEgo inRect: 55 115 80 125)
								(if (gEgo has: 25) ; envelope
									...
									(if (gEgo has: 24) ; envelope_corner
										(Print 26 68 #at -1 140) ; "As you examine the envelope, you compare it with the piece of paper found on Roberts, and..."
										(Print 26 69 #at -1 140) ; ""They match!" You also note that the envelope is addressed to Woody Roberts."
									else
										(Print 26 70) ; "Ok."
									)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(if global112
					(global112 setMotion: MoveTo 187 122 self)
				else
					(HandsOn)
					(client setScript: 0)
				)
			...
			(3
				...
				(local1 dispose:)
				(SetFlag 0)
				(Print 26 74 #draw #at -1 140) ; "Here's the little culprit, Sonny. I'll take it out to the car and run the numbers."
				(global112
					setAvoider: Avoid
					ignoreActors:
...
(instance keithBack of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 26 75 #at -1 140 #width 280 #draw) ; "Keith returns and says... "That shotgun has wants on it from a local burglary." He continues..."We can contact Burglary division for more information.""
