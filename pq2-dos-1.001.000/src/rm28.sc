			...
			(evSAID
				(cond
					...
					(
						...
						(if (localproc_0)
							(Print 28 11) ; "You see no foot prints worth casting."
						)
					)
					((or (Said 'use,get,remove/baggie') (Said 'get/hair,dirt'))
						(if (localproc_0)
							(Print 28 12) ; "Looking around carefully, you decide there's nothing worth using your only plastic baggy on."
						)
					)
					((Said 'look/blood')
						(Print 28 13) ; "You look at the blood, and feel a little squeamish."
					)
					((Said 'look,read/note,threat<187')
