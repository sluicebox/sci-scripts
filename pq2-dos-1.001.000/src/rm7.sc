...
(procedure (localproc_1)
	...
	(switch local1
		(local2
			...
			(Display 7 14 dsCOORD 20 7 dsWIDTH 300) ; "SUMMARY:   After a lengthy investigation, Officer Sonny Bonds successfully infiltrated the illegal gambling operation at the Hotel Delphoria. A confrontation with Bains ended in a shootout and his subsequent arrest. Suspect was convicted of first degree murder. Further: testimony provided by Woody Roberts, Marie Wilkans, and Don Colby was instrumental in convicting subject Bains on additional counts of narcotics trafficking and illegal gambling activities.   CONVICTED: 10/25/1983.  INCARCERATED: State Pen.  LOCATION: Wixsom.  SENTENCE: 97 years.  PAROLE DATE: None.  CASE STATUS: Closed END FILE:              (2)"
		)
		(local3
			(mugShot posn: 60 -30)
			(Animate (gCast elements:) 0)
			(Display 7 15 dsCOORD 20 10 dsWIDTH 300) ; "CONVICTED: 4/25/1986. INCARCERATED: State Pen. LOCATION: Wixsom. SENTENCE: 34 years. PAROLE DATE: None. CASE STATUS: Closed. END FILE:                   <2>"
		)
		(local4
			(mugShot posn: 60 -30)
			(Animate (gCast elements:) 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(local1
					startUpd:
					setMotion: MoveTo (local1 x:) (+ (local1 y:) 20)
				)
			)
		)
	)
						...
						(cond
							...
							(
								...
								(if local12
									...
									(Print 7 25 #at -1 150) ; "This is the end of the file!"
								)
							)
							(
								(or
									(Said '[read,turn,enter,look]/1,1[<page]')
									(Said
										'[read,turn,enter,look]/page<1st,preceding'
									)
									(Said 'turn,enter<back[/1,1<page]')
									(Said
										'turn,enter<back[/page<1st,preceding]'
									)
									(Said '/back')
								)
								(if local13
									...
									(Print 7 26 #at -1 150) ; "This is the beginning of the file!"
								)
							)
							(
								(or
								...
								(Print 7 27 #at -1 150) ; "There is no third page in this file."
							)
							(
								(Said
									'[read,see,look,get,open,pull]/file,bains,loofin,botts,martinez,julia,jason,lonny'
								)
								(Print 7 28 #at -1 150) ; "You can only look at one file at a time."
							)
							(
								(Said
									'exit,close,exit,close/(drawer,cabinet)[<file]'
								)
								(Print 7 29 #at -1 150) ; "If you value your hide in Homicide, you won't leave files lying around loose. Put it back!"
							)
							(
								(or
