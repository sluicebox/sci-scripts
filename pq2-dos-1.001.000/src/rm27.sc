			...
			(13
				(cond
					((not (gEgo inRect: 268 152 290 161))
						(Print 27 101) ; "You're not close enough to your car's door."
					)
					((== global133 1)
						(Print 27 102) ; "Your car door is locked"
						(return 0)
					)
					(global135
						(Print 27 103) ; "Close the trunk first."
						(return 0)
					)
					((gEgo has: 10) ; field_kit
						(Print 27 104) ; "You should put your field kit in the trunk first."
						(return 0)
					)
					(else
		...
		(switch (event type:)
			(evSAID
						...
						(cond
							...
							(
								(or
									...
									(Said
										'use,make,cast,get/plaster,cast,footprint,footprint'
									)
									(Said 'cast/print,footprint')
								)
								(if (localproc_0)
									(Print 27 5) ; "You see no foot prints worth casting."
								)
							)
							...
							(
								...
								(Print 27 7) ; "Looking around carefully, you decide there's nothing worth using your only plastic baggy on."
							)
						)
					)
					(cond
						...
						((Said 'look,frisk>')
							(cond
								...
								((Said 'look/auto<police,marked')
									(Print 27 26) ; "It's just your standard police cruiser."
								)
								((Said 'look/auto<blue,unmarked')
				...
				(cond
					...
					((Said 'climb,enter<through/hole,fence')
					...
					((Said 'enter,walk,enter,crawl/hole')
					...
					((Said 'remove,get,move,hoist/body,man')
						(cond
							...
							((and (== local9 1) (== global182 0))
								(Print 27 62) ; "That's a job for the coroner. You'd better wait."
							)
							((== local9 1)
								(Print 27 63) ; "The body's already been removed from the trunk."
							)
							(else
								(Print 27 64) ; "What body?"
							)
						)
					)
						...
						(if (gEgo inRect: 268 152 290 161)
							(cond
								((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
									(if (== global133 1)
										(= global133 0)
										(Print 27 65) ; "OK. It's unlocked."
									else
										(Print 27 66) ; "The door is already unlocked."
									)
								)
								((== global131 13)
									(Print 27 67) ; "You need a key to unlock this door."
								)
							)
							(cond
					...
					((Said 'lock/door')
						(if (gEgo inRect: 268 152 290 161)
							(if (== global131 13)
								(if (== global133 0)
									(= global133 1)
									(Print 27 68) ; "OK. It's locked."
								else
									(Print 27 69) ; "The door is already locked."
								)
							)
							(if (== global131 33)
								(if (== global136 0)
					...
					((Said 'open,unlock/trunk')
						(cond
							((gEgo inRect: 170 133 196 149)
								(Print 27 70) ; "The trunk of the green car is already open."
							)
							((== global131 13)
								(if (gEgo inRect: 207 156 258 178)
									(cond
										(global135
											(Print 27 71) ; "The trunk's already open."
										)
										...
										(else
											(Print 27 72) ; "You need a key to open this trunk."
										)
									)
								else
									(proc0_7) ; "You're not close enough."
							...
							(else
								(Print 27 73) ; "Your car's "hatch-back" hasn't worked since its warranty expired."
							)
						)
					)
					((Said 'close,lock/trunk')
						(cond
							((gEgo inRect: 170 133 184 149)
								(Print 27 74) ; "After carefull consideration, you wisely decide not to close the trunk. It would make your investigation more difficult."
							)
							((== global131 13)
								(if (gEgo inRect: 207 156 258 178)
									(if global135
										...
										(Print 27 75) ; "It's already closed."
									)
								else
									(proc0_7) ; "You're not close enough."
								)
...
(instance rm27Script of Script
	...
	(method (doit)
		...
		(if (and (not (gEgo onControl:)) (not local6))
			(Print 27 76) ; "You will need your car in order to leave the area."
			(cond
				((<= (gEgo x:) 0)
		...
		(switch (= state newState)
			(0
				...
				(if (and (== global131 13) local8)
					(global112 setPri: 9 setLoop: 2 posn: 239 156)
				)
				(if (not local8)
					(= global135 0)
					(stopScript init:)
				...
				(if (and local8 (== global182 0))
					(= global182 1)
					(if (IsFlag 153)
						(Print 27 77 #draw #at -1 50) ; ""Here comes the 'meat wagon', Sonny," Keith anounces from inside the car."
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (== global131 13)
					(global112 posn: 291 170 loop: 0 cel: 1 startUpd:)
					((= local5 (Prop new:))
						view: 51
						loop: 3
						cel: 0
			...
			(5
				(switch local9
					(1
						(Print 27 78 #at -1 22) ; ""Ugh!" grunts Keith, as he walks past the body."
						(Print 27 79 #at -1 22) ; "I'll search the car, Sonny. YOU can examine the trunk."
						(global112
							ignoreActors:
							illegalBits: 0
					...
					(2
						(Print 27 80 #at -1 22) ; "What are we doing back here, Sonny?"
					)
					(3
						(Print 27 81 #at -1 22) ; "Keith looks at you accusingly; "They've already cleaned up the murder scene! I told you we should've come here right away. We're going to catch it from Fletch now!""
					)
					(0
						(Print 27 82 #at -1 160 #draw) ; "What are we doing here?" Keith asks. "Are we going to meet someone?"
					)
				)
			)
			(6
				(global112 setPri: 9 setMotion: MoveTo 239 156)
			)
			(7
				(HandsOff)
				(gEgo stopUpd: ignoreBlocks: ourCarBlock)
				(if (and (== global131 13) (< (global112 y:) 158))
					(Print 27 83) ; "Keith yells: "Wait up, Sonny! Don't leave me here!""
					(global112
						ignoreActors:
						illegalBits: 0
						setLoop: -1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					(local8
						(cond
							((IsFlag 9)
								(Print 27 84 #at 40 24) ; ""Did'a you find something, Sonny?" Mario asks."
								(= local13 1000)
							)
							((not (IsFlag 44))
								(Print 27 85 #at 40 24) ; "Pretty gruesome, eh, Sonny?" groans Mario. "How'a one person can'a do that to another is really... you know'a... sick!"
								(SetFlag 44)
							)
						)
					)
					((IsFlag 42)
						(Print 27 86) ; ""Hello again, guys," shouts Officer Gelepsi."
					)
					(else
						(SetFlag 42)
			...
			(3
				(gEgo loop: 1)
				(HandsOff)
				(Print 27 87 #at -1 24 #draw) ; "In his heavy Italian accent, Officer Gelepsi says... "Well! It'sa bout'a time, Sonny!""
				(= seconds 2)
			)
			(4
				(Print 27 88 #at -1 24) ; ""I found'a this car with'a blood dripping from'a the trunk" he says..."
				(= seconds 2)
			)
			(5
			...
			(6
				(Print 27 89 #at -1 24) ; "Continuing, he says "The door was un'alocked, with'a the keys innaside.""
				(carScript changeState: 2)
				(= seconds 1)
			)
			(7
				(Print 27 90 #at -1 14) ; "You listen intently as he explains, "I took'a the keys and opened the trunk. That'sa when I found the man with the hole'a inna his head.""
				(= seconds 1)
			)
			(8
				(local0 setMotion: MoveTo 88 150 self)
			)
			(9
				(local0 setMotion: MoveTo 90 152)
				(Print 27 91 #at -1 60) ; "Continuing..."Thisa car isa registered to Woody Roberts.""
				(Print 27 92 #at 88 144 #draw) ; "Gelepsi finishes, "I called'a the coroner. Now YOU canna take over this'a mess, Sonny. ""
				(HandsOn)
			)
		)
	)
		...
		(switch (= state newState)
			(0
				...
				(if (gEgo inRect: 168 151 206 250)
					(Print 27 93) ; "Remembering the coroner's poor eyesight, you decide to move out of his car's way."
			...
			(1
				(if (== global182 1)
					(= global182 2)
					(gEgo observeBlocks: quincyCarBlock)
					(local2 startUpd: setMotion: MoveTo 188 187 self)
				)
			)
			(2
				(local2 stopUpd:)
			...
			(5
				...
				(Print 27 94 #at -1 60 #draw) ; "Where is this body?" asks the coroner. "I don't see no body! (haw haw)"
				(= seconds 2)
			)
			(6
				(Print 27 95 #at -1 50) ; "Let me guess: the trunk! Of course, where else WOULD it be?"
			...
			(8
				(Print 27 96 #at -1 50) ; "Yes-sir-ree-Bob!" remarks the coroner "Definitely looks like an execution."
				(HandsOn)
			)
			(9
				(HandsOff)
				(SetScore 2)
				(Print 27 97) ; "Would you help me, Officer Gelepsi?" asks the coroner. "This fella is pretty heavy. He's on a diet now, though (haw haw)."
			...
			(12
				(Print 27 98 #at -1 67 #width 118) ; "Officer Gelepsi and the coroner struggle to carry the victim into the back of the coroner's vehicle."
				(= local10 1)
			...
			(14
				(Print 27 99) ; ""Bye, Sonny! I gotta dig up a date for tonight (haw!)." the cheerful coroner yells over his shoulder."
				(= global182 3)
				(= local10 0)
				(qDoor
					view: 162
			...
			(15
				...
				(if (gEgo inRect: 162 187 212 240)
					(Print 27 100) ; "The coroner has work to do - and since you don't want to end up on his table next to Woody, you decide to MOVE!"
					(HandsOff)
