...
(instance rm15 of Rm
	...
	(method (doit)
		...
		(cond
			((and (!= (mod (gEgo view:) 2) 0) (<= (gEgo y:) 124))
				(gEgo view: (- (gEgo view:) 1))
			)
			((and (!= (mod (gEgo view:) 2) 1) (> (gEgo y:) 124))
				(gEgo view: (+ (gEgo view:) 1))
			)
		)
		(if
...
(instance rm15Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 15 2 #width 26 #at 221 66 #time 3) ; "My, My!"
				(Print 15 3 #width 50 #at 252 65 #time 4) ; "Didja see that?"
				(if local7
					(EgoDead
				...
				(cond
					...
					(
						(or
							...
							(Said 'are/flower')
							(Said 'give,talk,ask[/me,woman,wanda]/cost')
							(Said 'give,talk,ask/cost')
						)
						(if local10
