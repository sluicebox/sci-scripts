		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(or
							(Said '(stop,police,stop)')
							(Said 'say/(stop,police,stop)')
						)
						(if (not local112)
...
(instance rm133Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo loop: 0 cel: 0 posn: 190 115)
			...
			(26
				...
				(= local5 (localproc_0 133 48 86)) ; "The barrage is over. Bains lies motionless on the control room floor."
				(= seconds 8)
			)
			(27
				(clr)
				(= local5 (localproc_0 133 49 86)) ; "It appears that all the firing range practice has paid off."
				(gEgo
					view: 1
			...
			(29
				(= local5 (localproc_0 133 50 86)) ; "Checking Bains, you find all vital signs have ceased."
				(= seconds 7)
			)
			(30
				(clr)
				(= local5 (localproc_0 133 51 86)) ; "You view Bains in his deceased state. Deep down inside, you kind of feel sorry for him. The reality, however, is that...."
				(= seconds 13)
			)
			(31
				(clr)
				(= local5 (localproc_0 133 52 86)) ; "the destructive vengeance of "The Death Angel" has been silenced."
				(= seconds 8)
			)
			(32
				(clr)
			...
			(34
				(RedrawCast)
				(localproc_0 133 53 86) ; "Oh Sonny!" Marie says..."You're my hero! I don't want to leave you ever again!"
				(= seconds 5)
			)
			(35
