...
(instance Bathroom of Feature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'open/door')
				(Print 42 8) ; "Just walk to it and it will open automatically."
			)
			((Said '(look,frisk)>')
				(cond
					((Said '/wall')
...
(instance rm42 of Rm
	...
	(method (init)
		...
		(= local127 0)
		(HandsOn)
		(bombTimer setReal: doBlowUp 140)
	)
...
(instance egoSearch of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (and (== local104 2) (< (gEgo x:) 274))
					(gEgo
						illegalBits: 0
						setMotion: MoveTo 274 (gEgo y:) self
					)
				else
					(self cue:)
				)
...
(instance TowelDispenser of Script
	...
	(method (doit)
		...
		(if (== local123 6)
			(localproc_2 42 51) ; "The timer stops. The bomb is disarmed!"
			(bombTimer seconds: -1)
			(gContinuousMusic stop:)
				...
				(cond
					((Said '(get,move,remove)/bomb')
						(cond
							...
							((> local123 5)
								(localproc_2 42 52) ; "You don't need the disarmed bomb."
							)
							(else
								(localproc_2 42 53) ; "Ok."
								(localproc_2 42 54) ; "As you take the bomb..."
								(= local110 1)
							)
						)
					)
					((Said 'disarm')
						(cond
							...
							((> local123 5)
								(localproc_2 42 55) ; "The bomb is already disarmed."
							)
							(else
								(localproc_2 42 56) ; "How do you propose to do that?"
							)
						)
					)
					((Said 'look>')
						(cond
							((Said '/wires')
								(if (not local121)
									...
									(localproc_2 42 57) ; "There is a blue, white, yellow, and purple wire attached to the bomb."
								)
							)
							((Said '/cable>')
								(cond
									...
									((Said '/!*')
										(localproc_2 42 58) ; "There are four wires: a blue, white, yellow, and purple one."
									)
									...
									(else
										(localproc_2 42 63) ; "Please specify either the blue, white, yellow, or purple wire."
									)
								)
							)
							((Said '/timer,(device[<timing])')
								(cond
									((not local121)
										(localproc_2 42 64) ; "You don't see a timing device here."
									)
									((> local123 5)
										(localproc_2 42 65) ; "The device is off."
									)
									(else
										(localproc_2 42 66) ; "The timing device quickly ticks down."
									)
								)
							)
							((Said '/bomb')
								(if local121
									(localproc_2 42 67) ; "The bomb has four wires and a timing device."
								else
									(localproc_2 42 68) ; "You don't see a bomb?"
								)
							)
							((Said '/bracket')
								(if local121
									(localproc_2 42 69) ; "In the towel dispenser is a bomb!"
								else
									(localproc_2 42 70) ; "You look at the paper towel dispenser, but you see nothing out of the ordinary."
								)
							)
							((Said '/dynamite')
								(localproc_2 42 71) ; "It looks like it could easily blow up this plane!"
							)
							((Said '[<at,around][/(!*,chamber,bathroom)]')
								(self changeState: 10)
							)
					...
					((Said 'open[/bracket,lid]')
						(if (not local121)
							...
							(localproc_2 42 72) ; "It's already open."
						)
					)
					((Said 'close[/bracket,lid]')
						(if local121
							...
							(localproc_2 42 73) ; "It is not yet open."
						)
					)
					((or (Said 'get/towel') (Said 'use/bracket'))
						(localproc_2 42 74) ; "The towels seem to be stuck."
					)
					((Said 'pull,bite/cable,wires')
						(localproc_2 42 75) ; "That would not be a good idea."
					)
					((Said '(cut,clip,disconnect)/>')
						(cond
							((not (gEgo has: 9)) ; wire_clippers
								(localproc_2 42 76) ; "You'll need something to cut the wires apart."
								(event claimed: 1)
							)
							((> local123 5)
								(localproc_2 42 77) ; "Leave the bomb alone...it is disarmed."
								(event claimed: 1)
							)
							((not local121)
								(localproc_2 42 78) ; "You can't do that now."
								(event claimed: 1)
							)
							((Said '/blue,(cable<blue)')
								(if (== local124 1)
									(localproc_2 42 79) ; "It already is."
								else
									(localproc_2 42 80) ; "Snip."
									(local113 cel: 1)
									(= local124 1)
									(switch local123
										(2
					...
					((Said 'attach,(drop<together)/>')
						(cond
							((not local121)
								(event claimed: 1)
								(Print 42 78) ; "You can't do that now."
							)
							((> local123 5)
								(localproc_2 42 77) ; "Leave the bomb alone...it is disarmed."
								(event claimed: 1)
							...
							((Said '/blue,(cable<blue)')
								(if (== local124 0)
									(localproc_2 42 81) ; "It is already connected."
								else
									(localproc_2 42 82) ; "You put the blue wire together."
									(local113 cel: 0)
									(= local110 1)
								)
							)
							((Said '/white,(cable<white)')
								(if (== local125 0)
									...
									(localproc_2 42 83) ; "You put the white wire together."
									(local114 cel: 0)
									(= local110 1)
								)
							)
							((Said '/yellow,(cable<yellow)')
								(if (== local126 0)
									...
									(localproc_2 42 84) ; "You put the yellow wire together."
									(local115 cel: 0)
									(if (!= local123 3)
										(= local110 1)
									else
							...
							((Said '/purple,(cable<purple)')
								(if (== local127 0)
									...
									(localproc_2 42 85) ; "You put the purple wire together."
									(local116 cel: 0)
									(= local110 1)
								)
							)
