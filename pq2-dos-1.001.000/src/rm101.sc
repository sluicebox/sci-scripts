...
(instance rm101 of Rm
	...
	(method (doit)
		...
		(if (and (gEgo inRect: 135 135 153 139) (not local14))
			(theDoor changeState: 1)
		)
		(if (and (not local14) (gEgo inRect: 110 129 130 139))
			(if (not local5)
				(gEgo illegalBits: 0)
					...
					(cond
						((Said '/woman,woman')
							(cond
								...
								((and (== local7 2) (< (gEgo x:) 137))
									(localproc_0 101 23) ; "Have a nice day sir."
								)
								((and local5 (< (gEgo x:) 137))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(localproc_0 101 46 83) ; "Then Wild Willy says..."
				(localproc_0 101 47 83) ; "Well! you dudes are chasing one fine, upstanding citizen." Continuing, he says..."It seems this punk Bains has already started his handiwork here, too."
				(localproc_0 101 48 83) ; ""You know the 'phone tap' subject Don Colby?" He asks. Without waiting for a reply, he goes on to say...."
				(localproc_0 101 49 83) ; "I hate to tell you boys this, but after putting a tap on Colby's phone, I sent an unmarked unit over to check on him, and..."
				(localproc_0 101 50 83) ; "My man found him deader than a doornail with a .32 cal hole in his head."
				(localproc_0 101 51 83) ; "We did manage to connect", Willy tells you, "on one threatening call from Bains. I don't think it will help but..."
				(localproc_0 101 52 83) ; ""The call was traced to a pay phone in our local "Burt Park"."
				(localproc_0 101 53 83) ; "I sent a man over there to have a look around," he says, "but nothing was found."
				(localproc_0 101 54 83) ; "You boys will probably want to go over there and look around." Willy grunts.."But I think you'll be spinning your wheels."
				(localproc_0 101 55 83) ; "Wild Willy says..."I think Officer Pitman is waiting in the hall. He can drive you over to the park.""
				(localproc_0 101 56 83) ; "Good luck, boys!"
				(= local5 1)
				(self cue:)
			)
			(7
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				...
				(HandsOff)
				(global112 stopUpd:)
				(localproc_0 101 57) ; "Keith says, "Did you forget something, Sonny? I'll wait here for you.""
				(self cue:)
			)
			(2
				(gEgo
			...
			(4
				...
				(if (gEgo has: 30) ; walkie_talkie
					(localproc_0 101 58) ; "The Lieutenant says "What's up, Bonds...did you forget something?""
				else
