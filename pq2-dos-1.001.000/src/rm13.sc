...
(procedure (proc13_1)
	(if (and (== local109 local107) (== local110 local108))
		...
		(if (and (>= global100 12) (== local103 25))
			(= local103 225)
		)
		(NormalEgo)
...
(instance keithBud of View
	(properties)
)

(instance sShoulder of View
	(properties)
)

(instance kShoulder of View
	(properties)
)
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					((Said 'look>')
						(cond
							...
							((Said 'look/pane')
								...
								(switch local109
									(0
										(Print 13 22) ; "... the newly built Lytton Airport parking lot and terminal building."
									)
									(40
										(Print 13 23) ; "Snuggler's Inn. Sleazy and cheap, it's a motel for the down-and-out."
									)
									(80
										(Print 13 24) ; "Arnie's restaurant. Their food is good, and the service is adequate."
									)
									(100
										(Print 13 25) ; "Oak Tree Mall. This is Lytton's major shopping area."
									)
									(122
										(Print 13 26) ; "Cotton Cove. It's a poorly-kept-up park. It holds many fond memories from your youth."
									)
									(90
										(Print 13 27) ; "... the underground parking structure of the new Lytton Justice Building. the city jail's entrance is nearby."
									)
									(30
										(Print 13 28) ; "... a typical American neighborhood. You're parked in front of Marie Wilkan's house."
									)
									(62
										(Print 13 29) ; "... the warehouse district. A lot goes down around here, free from prying eyes."
									)
									(110
										(Print 13 30) ; "... the Lytton Police Station and parking lot. The detective's entrance is nearby."
									)
									(else
										(Print 13 31) ; "... the streets of Lytton as they fly by."
									)
								)
							)
							((Said '/mirror')
								(Print 13 32) ; "That's good driving practice. Tail-gaters can be dangerous."
							)
							((Said '/console')
								(Print 13 33) ; "The dashboard is cluttered with various necessary items: police radio, note pad, reading light, and Keith's pack of cigarettes."
							)
							((Said '/extender,scanner')
								(Print 13 34) ; "The police radio scans the emergency frequencies, constantly in search of new developments."
							)
							((Said '/note,clipboard')
								(Print 13 35) ; "The note on the dashboard is from three days ago. It's an old grocery list."
							)
							((Said '/cigarette,pack')
								(Print 13 36) ; "You read: "SURGEON GENERAL'S WARNING: Smoking Causes Lung Cancer, Heart Disease, Emphysema, And May Complicate Pregnancy.""
							)
							(
								(Said
									'/building,ave,air,cloud,sun,barn,home,airplane,airport,lot,fence,cove,inn,bush,tree,garage,sign,mall,store,cafe,warehouse'
								)
								(Print 13 37) ; "You can't see much from inside the car."
							)
							(
								(Said
									'[<at,around][/!*,auto,up,door,bonds,down,ceiling,floor,bench]'
								)
								(Print 13 38) ; "You're behind the wheel of an unmarked police cruiser. Your partner, Keith, is puffing away on a cigarette."
							)
						)
					)
					(
						...
						(Print 13 39) ; "Good idea. Safety first."
					)
					((Said 'turn/extender,scanner')
						(Print 13 40) ; "Keith keeps the police scanner on at all times."
					)
					((Said 'listen/extender,scanner')
						(Print 13 41) ; "You listen intently to the radio traffic. It takes practice to pick out the important calls."
					)
					(
						(or
					...
					((Said 'drive/auto')
						(Print 13 46) ; "To drive somewhere, type: "drive to (destination)""
					)
					((Said 'drive,enter>')
						(if (not (gEgo has: 3)) ; unmarked_car_keys
							(Print 13 47) ; "You need the correct key to operate this car."
							(event claimed: 1)
						else
							...
							(cond
								(
									...
									(if (!= local103 31)
										...
										(if local102
											(Print 13 48) ; "You're already going there."
										else
											(Print 13 49) ; "You're already there."
										)
										(User canInput: 1)
									)
								)
								...
								((Said '/ave<10th<5556')
									(Print 13 51) ; "You have hotter leads to follow."
									(User canInput: 1)
								)
								(
									(or
										...
										(Said '/willie<bitch')
										(Said '/chamber<blue')
										(Said '/delphoria[<hotel]')
									)
									(User canInput: 1)
									(Print 13 52) ; "Lytton has seen many businesses fail in the last year... and that was one of them."
								)
								((Said '/steelton,houston,coarsegold')
									(User canInput: 1)
									(Print 13 53) ; "That's a great place, but it's too far to drive there right now."
								)
								((Said '/[*]')
									(User canInput: 1)
									(Print 13 54) ; "You don't need to drive there."
								)
								(else
									(User canInput: 1)
									(event claimed: 1)
									(Print 13 55) ; "Where to?"
								)
							)
						)
					...
					(
						(or
							(Said 'stop<bath/bains')
							(Said 'call<off/bath,pursuit')
							(Said 'stop/bath,pursuit')
						)
						(if local114
							(= local103 61)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				...
				(SetFlag 19)
				(localproc_2 1 13 64)
				(if (Print 13 65 #at -1 138 #button {You're ON!} 1 #button {No way.} 0)
					(= local112 0)
					(SetFlag 114)
			...
			(43
				(localproc_2 0 13 100)
			)
			(44
				(localproc_2 3 13 101)
			)
			(45
				(localproc_2 1 13 102)
				(= local113 1)
			)
			(46
				(= local113 0)
			...
			(50
				(= local113 0)
				(localproc_2 1 13 107)
			)
			(51
				(localproc_2 0 13 108)
			)
			(52
				(localproc_2 1 13 109)
				(= local113 1)
			)
			(53
				(SetFlag 22)
			...
			(59
				...
				(localproc_2 0 13 116)
				(localproc_0)
			)
			(60
				(localproc_2 1 13 117)
			)
			(61
			...
			(66
				(localproc_2 0 13 123)
			)
			(67
				(localproc_2 1 13 124)
				(= local113 1)
				(User canInput: 1)
			)
			(68
			...
			(74
				(= local113 0)
				(SetFlag 16)
				(localproc_2 0 13 130)
			)
			(75
				(localproc_2 1 13 131)
			)
			(76
				(localproc_2 0 13 132)
			)
			(77
				(localproc_2 0 13 133)
			)
			(78
				(localproc_2 1 13 134)
				(= local113 1)
			)
			(79
				(= local113 0)
				(localproc_2 0 13 135)
				(= local100 1)
			)
			(80
				(localproc_2 1 13 136)
			)
			(81
				(localproc_2 0 13 137)
			)
			(82
				(= global100 5)
				(= global158 300)
			...
			(85
				...
				(if (and (== gPrevRoomNum 27) (== global100 10))
					(= global100 11)
				)
				(cond
					(
			...
			(93
				...
				(ClearFlag 50)
				(SetScore 1)
				(localproc_2 1 13 154)
			)
			(94
				(localproc_2 0 13 155)
			)
			(95
			...
			(96
				...
				(SetScore 2)
				(ClearFlag 51)
				(localproc_2 1 13 157)
			)
			(97
				(localproc_2 0 13 158)
			)
			(98
			...
			(99
				(= local113 0)
				(localproc_2 1 13 160)
			)
			(100
				(localproc_2 0 13 161)
				(= local113 1)
			)
			(101
				(= local113 0)
				(localproc_2 1 13 162)
			)
			(102
				(localproc_2 0 13 163)
				(= local113 1)
			)
			(103
				(= local113 0)
				(localproc_2 1 13 164)
			)
			(104
				(localproc_2 0 13 165)
				(= local113 1)
			)
			(105
				(= seconds 3)
			...
			(114
				...
				(if (Print 13 65 #button {YES!!} 0 #button {no} 1 #at -1 138)
			...
			(115
				(switch (Random 0 15)
					...
					(13
						(Format @local0 13 193) ; "I don't know what you're complaining about! I filter the smoke with my lungs first, you know."
					)
					(14
						(Format @local0 13 194) ; "How many times have I saved your life, huh partner? And now... now, when I need a little consideration from you, you turn on me."
					)
					(15
						(Format @local0 13 195) ; "Well, if I didn't smoke, we'd BOTH be nervous wrecks! One of us has to keep his mental faculties sharp."
					)
				)
				(= local113 1)
			)
