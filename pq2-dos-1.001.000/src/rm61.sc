	...
	(switch global131
		(13
			(if (not (gEgo inRect: 144 188 166 197))
				(Print 61 114) ; "You're not close enough to your car's door."
			else
				(if (and (== global100 5) (not global183))
					(gEgo setMotion: MoveTo 125 188)
					(return (Print 61 115)) ; "Hey, Sonny!" Keith shouts, "We can't leave yet. We haven't checked everything out."
				)
				(cond
					(global133
						(Print 61 116) ; "Your car door is locked"
						(return 0)
					)
					(global135
						(Print 61 117) ; "Close the trunk first."
						(return 0)
					)
					((gEgo has: 10) ; field_kit
						(Print 61 118) ; "You should put your field kit in the trunk first."
						(return 0)
					)
					(else
						(carScript changeState: 6)
						(return 1)
					)
				)
			)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(Said
							'enter,walk,enter,crawl[<through]/hole,fence[/hole,fence]'
						)
					...
					((Said 'talk>')
						(cond
										...
										(if (== global131 13)
											(cond
												(
													...
													(if global183
														(Print 61 59) ; "Officer Gelepsi and Keith are busy discussing the details of the shoot-out."
													else
														(Print 61 60) ; "Officer Gelepsi is busy talking to Keith. Listening, you hear some good gossip, but nothing relating to the task at hand."
													)
												)
												(
													(and
											...
											(Print 61 63) ; ""Hey, Sonny, where's your partner? And what are you doing driving your personal car?" Officer Gelepsi asks."
										)
									else
										(Print 61 64) ; "Officer Gelepsi is a little hard of hearing. Better get closer."
									)
								else
									(Print 61 31) ; "There are no uniformed officers in the area."
								)
							...
							((Said 'close/door[<booth]')
								(Print 61 65) ; "The phone booth's door is broken and can't be closed."
							)
							((Said '/friend')
						...
						(cond
							((Said '[say]/hello')
								(if (> (gEgo distanceTo: barbie) 38)
									(Print 61 73) ; "You can't seem to attract her attention from this distance."
								else
							...
							((Said 'interrogate,ask,get>')
								(cond
									((> (gEgo distanceTo: barbie) 38)
										(event claimed: 1)
										(Print 61 74) ; "You're too far away for her to hear you."
									)
									...
									(
										...
										(Print 61 75) ; "You're kinda cute, but I don't date cops."
									)
									((Said '/name[<woman]')
									...
									(
										...
										(if local6
											(Print 61 76) ; ""I've told you all I know, Officer," she stammers."
										else
											(= local6 1)
											(SetScore 2)
											(barbieScript changeState: 5)
											(localproc_0 61 77) ; "The girl tells you the location..."Just walk upriver about a hundred yards or so. It's right next to the water!""
										)
									)
									(else
										(Print 61 78) ; ""I saw blood... and drag marks..." she cries."
										(event claimed: 1)
									)
								)
							)
					...
					((Said 'ask/')
						(Print 61 79) ; "You listen carefully, but nobody answers you."
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(switch local4
					(1
						(Print 61 80 #draw) ; "What are we doing back here, Sonny?"
					)
					(2
						(Print 61 81 #at -1 60 #draw) ; "Keith looks at you accusingly; "We're too late, Sonny! I told you we should've come here right away. Captain Fletcher is gonna chew us out now!""
					)
					(0
						(Print 61 82 #at -1 60 #draw) ; "What are we doing here?" Keith asks. "Are we going to meet someone?"
					)
				)
			)
			(5
				(global112 setMotion: MoveTo 124 148)
			)
			(6
				(HandsOff)
				...
				(if (== global131 13)
					(if global186
						(Print 61 83) ; "Keith says..."I don't think we can catch him Sonny! He's probably long gone.""
					else
						(Print 61 84) ; "Keith yells: "Wait up, Sonny! What's the rush, anyway?""
					)
			...
			(8
				(if (gCast contains: gelepsi)
					(cond
						(global186
							(Print 61 85) ; "Keith tells Officer Gelepsi..."Mario, stand by and secure this area until we return!.""
						)
						(global184
							(Print 61 86) ; "I'lla stick 'round here an'a clean things up," Mario tells you. "See you'a later."
						)
						(else
							(Print 61 86) ; "I'lla stick 'round here an'a clean things up," Mario tells you. "See you'a later."
						)
		...
		(switch (= state newState)
			...
			(1
				(if local3
					(if (and global183 (not (IsFlag 30)))
						...
						(Print 61 87) ; ""M-my gosh'a, S-S-Sonny! I heard'a the GUNSHOTS!" stammers Officer Gelepsi."
					else
						(Print 61 88 #at 40 24) ; ""What's up, Sonny? I think'a you'a doin' fine." says Officer Gelepsi."
					)
				else
			...
			(2
				...
				(cond
					((== local4 3)
						(if (IsFlag 53)
							(Print 61 89) ; ""What'sa up, guys?" Mario asks."
						else
							(if (IsFlag 114)
								(++ global107)
								(localproc_0 61 90) ; "As the car pulls up, Keith winces..."Oh no! I can see from here it's a gal... You got me again, partner. Here's your buck.""
							else
								(localproc_0 61 91) ; "As the car pulls up, Keith says..."Well Sonny, you should have taken the bet. I can see from here it's a gal.""
							)
							(Print 61 92 #at -1 24 #draw) ; "Hi'ya Sonny," Officer Gelepsi says, "The lady over there isa pretty shook up over what she'sa found. You'da better have a talk with her."
							(SetFlag 53)
							(= seconds 2)
						)
					)
					((and (== local4 4) (not local1))
						(= global186 0)
						(localproc_0 61 93) ; "I wasa listening ona the radio, Sonny," Officer Gelepsi grunts. "Too bad you lost that cocka-de-roacha."
					)
					((== local4 4)
						(localproc_0 61 94) ; "Officer Gelepsi says, "Donn'a worry, Sonny. They'lla catch that cocka-de-roacha.""
					)
				)
			)
			(3
				(localproc_0 61 95) ; "Officer Gelepsi continues... "I'lla stick around for awhile, just in case you need me.""
			)
			(4
...
(instance barbieScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if local7
					(localproc_0 61 96) ; "Hello AGAIN! You sure are friendly."
				else
					(localproc_0 61 97) ; "Hello"
					(= local7 1)
				)
			)
			(2
				(if local8
					(localproc_0 61 98) ; "You have a short memory. Maybe you'd better write this down."
				else
					(= local8 1)
				)
				(localproc_0 61 99) ; "Shelly Wingate"
			)
			(3
				(if local9
					(localproc_0 61 98) ; "You have a short memory. Maybe you'd better write this down."
				...
				(localproc_0 61 100) ; "108 Tenth Street"
			)
			(4
				(if local10
					(Print 61 101) ; "All right, buster, THAT does it! I'm leaving. YOU find that stuff WITHOUT ME!"
					(self changeState: 7)
				else
					(= local10 1)
					(switch (Random 1 4)
						(1
							(Print 61 102) ; "I've always heard such nice things about policemen. But now I see where the nick-name 'pig' comes from."
						)
						(2
							(Print 61 103) ; "Why you filthy little sniveling creep!"
						)
						(3
							(Print 61 104) ; "If I weren't a LADY, I'd...!"
						)
						(4
							(Print 61 105) ; "Why you slimy little SQUID!"
						)
					)
				)
			)
			(5
				(localproc_0 61 106) ; "In an excited voice the woman says... "I was jogging along the river's edge, and...and I came across something that looked like blood...""
				(localproc_0 61 107) ; "Continuing, she says, "I stopped to look, and right next to the blood, I saw marks that looked like something had been dragged into the river. And... and footprints -- I saw footprints!""
				(localproc_0 61 108) ; "You attempt to interrupt, but she keeps rolling it out as she says... "I got scared when I heard the brush crack, so I ran down here and called the Police.""
				(localproc_0 61 109) ; "The jogger says... "Officer, please go look! I hope nothing bad has happened.""
			)
			(6
				(localproc_0 61 110) ; "The girl answers..."Not me! I'm not going back there!" she says, "I'll tell you where it is, but I'm not going!""
				(localproc_0 61 77) ; "The girl tells you the location..."Just walk upriver about a hundred yards or so. It's right next to the water!""
				(= local6 1)
			)
			(7
			...
			(9
				(if global183
					(localproc_0 61 111) ; "The jogger, frightened by the gunfire, runs away."
				)
				(User canControl: 1)
		...
		(switch (= state newState)
			...
			(1
				...
				(if (and (not local1) (== gPrevRoomNum 13))
					(Print 61 112 #draw) ; "Keith says... "I'm right behind you partner.""
				)
			)
			(2
			...
			(3
				(if (< global189 3)
					(Print 61 113) ; ""We've radioed for the diver, Sonny, and he'll be here shortly," Keith volunteers."
				)
			)
		)
	)
