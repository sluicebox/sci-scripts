		...
		(13
			(cond
				(
					...
					(Print 25 68) ; "You're not close enough to your car's door."
				)
				((== global133 1)
					(Print 25 69) ; "Your car door is locked"
					(return 0)
				)
				(global135
					(Print 25 70) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(Print 25 71) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
		...
		(cond
				...
				(cond
					...
					((Said '/pane')
						(cond
							...
							(else
								(Print 25 12) ; "Someone's gonna call the cops if you keep looking into the windows like that!"
							)
						)
					)
					((Said '/door')
			...
			((or (Said 'press,unlock,open/door') (Said 'use/key'))
				(cond
					...
					((!= (gEgo onControl:) 256)
						(Print 25 24) ; "You don't have a key to this door."
					)
					((== global100 12)
						(Print 25 25) ; "The door is already open."
					)
					((not (gEgo has: 27)) ; motel_key
						(Print 25 26) ; "You don't have the key."
					)
					((== local41 256)
			...
			((Said 'extender,ask,get,call/warrant[<frisk]')
				(cond
					((IsFlag 13)
						(Print 25 27) ; "You already have it."
					)
					((> global170 1)
						(Print 25 28) ; "Your requested warrant has not arrived yet."
					)
					((IsFlag 29)
					...
					(else
						(Print 25 29) ; "You should call dispatch."
					)
				)
			)
			((Said 'extender,call/dispatch')
				(Print 25 30) ; "Use your car's radio."
			)
			((Said 'look,look/warrant[<frisk]')
				(cond
					((IsFlag 13)
						(Print 25 31) ; "The search warrant authorizes you to legally search room 108 at the Snugglers Inn."
					)
					((> global170 1)
						(Print 25 28) ; "Your requested warrant has not arrived yet."
					)
					(else
						(Print 25 32) ; "You should call dispatch and request a warrant."
					)
				)
			)
			((Said 'extender,ask,call,get/backup,swat,team')
				(cond
					...
					((> global171 1)
						(Print 25 33) ; "Your requested backup has not arrived yet."
					)
					(else
						(Print 25 29) ; "You should call dispatch."
					)
			...
			((Said 'lock/door')
				(if
					...
					(if (== global131 13)
						(if (== global133 0)
							(= global133 1)
							(Print 25 35) ; "OK. It's locked."
						else
							(Print 25 36) ; "The door is already locked."
						)
					)
					(if (== global131 33)
						(if (== global136 0)
					...
					(Print 25 37) ; "You're not close enough."
				)
			)
			((Said 'knock[/door]')
				(if local41
			...
			((Said '[open]/police')
				(if local41
					...
					(Print 25 41) ; "You aren't close enough to the door."
				)
			)
				...
				(if (== global131 13)
					(if
						...
						(cond
							...
							(else
								(Print 25 43) ; "You need a key to open this trunk."
							)
						)
					else
						(proc0_7) ; "You're not close enough."
				...
				(if (== global131 13)
					(if
						...
						(if global135
							...
							(Print 25 44) ; "It's already closed."
						)
					else
						(proc0_7) ; "You're not close enough."
					)
			...
			((Said 'talk,ask/friend')
				(if (== global131 13)
					...
					(Print 25 45) ; ""I'm right behind you, partner!" Keith says..."
				else
					(Print 25 46) ; "You came here alone. Keith is not with you."
				)
			)
			((Said 'talk>')
				(event claimed: 1)
				(cond
					((and (IsFlag 14) (<= (gEgo distanceTo: local24) 30))
						(cond
							((IsFlag 0)
								(Print 25 47) ; "You tell Chuck that the room is secure."
								(Print 25 48) ; "Chuck responds, "Ok, Sonny. We'll wrap things up here.""
							)
							((== global100 12)
								(Print 25 49) ; "Chuck says, "You go ahead and do your thing. We'll stay here.""
							)
							(else
			...
			((or (Said 'break/door') (Said 'hit/door'))
				(cond
					...
					(
						...
						(Print 25 55) ; "Being a macho type, you kick in the door, making a fool out of yourself."
						(Print 25 56) ; "You quickly close the door."
					)
					((== global100 12)
						(Print 25 57) ; "What do you think you are doing? The door is already open!"
					)
					(else
						(= local25 1)
			...
			((Said 'drop,place/briefcase')
				(if
					...
					(if global135
						(if (gEgo has: 10) ; field_kit
							(Print 25 58) ; "You place your field kit inside the trunk."
							...
							(Print 25 59) ; "You don't have the field kit."
						)
					else
			...
			((Said 'get/browning')
				(if
					...
					(if 1
						(Print 25 64) ; "You don't need to take it out of the trunk."
					else
						(Print 25 65) ; "There is no shotgun."
					)
				else
					(Print 25 66) ; "You aren't close enough."
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(HandsOff)
				(if (== global131 13)
					(Print 25 67) ; "Keith yells: "Geez, Sonny! Hold on a second, will ya?""
					(cond
						((> (global112 y:) 189)
							(global112
								ignoreActors:
...
(instance warrantArrives of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(local27 setMotion: 0)
				(Print 25 72) ; "The officer says... "Here's the search warrant, Sonny.""
				(HandsOn)
				(client setScript: 0)
			)
		)
	...
	(method (changeState newState &tmp [temp0 100])
		(switch (= state newState)
			(0
				...
				(cond
					((< global100 11)
						(Print 25 74) ; "The boy at the the window says, "Yes sir, what can I do for you?""
					)
					((== global100 11)
						(Print 25 75) ; "The man at the window says, "Yes sir, what can I do for you?""
					)
					(else
						(Print 25 76) ; "The motel manager is fuming when he says... "You dudes aren't gonna get away with this! You're gonna pay for that damage!!""
					)
				)
			)
		)
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look/pane,man,boy')
				(if (< global100 11)
					(Print 25 77) ; "You see a small boy inside. Probably the manager's son."
				else
					(Print 25 78) ; "You see a man inside. You assume he is the manager."
				)
			)
			((Said 'give/cash')
				(if (< global100 11)
					(Print 25 79) ; "The boy says, "I don't think I should take your money.""
				else
					(Print 25 80) ; "The man says, "I don't need your money, buster.""
				)
			)
			((Said 'talk/man,boy')
				(cond
					((< global100 11)
						(Print 25 81) ; "Hello sir. My dad's not here right now, but maybe I can help you."
					)
					((== global100 12)
						(Print 25 82) ; "You guys did a bang-up job of ruining my day!" the manager says. "Now how about getting lost?"
					)
					(else
						(Print 25 83) ; "Yes sir, what can I do for you?"
					)
				)
			)
			((Said 'show/badge,badge')
				(if (gEgo has: 7) ; wallet
					(Print 25 84) ; "Yes officer, how can I help you?"
					(= global175 1)
				else
					(Print 25 85) ; "You don't have your wallet."
				)
			)
			((Said 'rent/chamber')
				(if (== global100 12)
					(switch (Random 0 2)
						(0
							(Print 25 86) ; "GET SERIOUS, MAC!" says the manager... "After the mess you made, you guys ain't rentin' nothing!"
						)
						(1
							(Print 25 87) ; "Slightly upset the manager says..."I only have four words for you jerks!...#$%* the b%& o&%!!!""
						)
						(2
							(Print 25 88) ; "The manager quickly says... "Why? So you can blow up another one?""
						)
					)
				else
					(Print 25 89) ; "I'm sorry, sir" he says, "but we don't have any vacancies."
				)
			)
			((Said 'show/painting,(shot<mug),mugshot')
				(cond
					((< global100 11)
						(Print 25 90) ; "Well," the boy says, "I don't know... Maybe you'd better come back later. My dad isn't here right now."
					)
					((gEgo has: 12) ; new_mug_shot
						(Print 25 91 #icon 112) ; "Well," the manager says, "the photograph is vaguely familiar... I believe his name was William Cole. He has room 108."
						(if global112
							(Print 25 92) ; "Ah ha!" you think to yourself, "Bains is using the name `Cole'."
						)
						(if (not (IsFlag 88))
							(SetFlag 1)
							(SetFlag 2)
					...
					((gEgo has: 23) ; old_mug_shot
						(Print 25 93 #icon 123) ; "Well," the manager says, "I don't remember seeing him."
					)
					(else
						(Print 25 17) ; "You don't have it."
					)
			...
			((Said 'look,get,give,show,ask/list,guest,register')
				(cond
					...
					((not global175)
						(Print 25 94) ; "The manager says, "Who do you think you are? My customers expect a little privacy when they stay here.""
					)
					(else
						(Print 25 95) ; "The manager says, "Who are you looking for?""
					)
				)
			)
			((or (Said '[ask]/bains') (Said '[ask]/man,boy/bains'))
				(if (< global100 11)
					(Print 25 96) ; "The boy says, "I don't know.""
				else
					(Print 25 97) ; "Scanning his guest book, he shakes his head and says... "I'm sorry. I don't have anyone listed by that name.""
				)
			)
			(
				(or
				...
				(if (< global100 11)
					...
					(= global178 1)
					(Print 25 98) ; "Scanning his guest book, he nods his head and says... "Yes, here we are: William Cole...Room 108.""
					(if (not (IsFlag 88))
						(SetFlag 1)
						(SetFlag 2)
						(SetScore 3 88)
			...
			((Said 'show/warrant[<frisk]')
				(cond
					...
					((IsFlag 13)
						(Print 25 99) ; "You show the search warrant to the manager, and he says... "Ok, now what?""
						(= global174 1)
					)
					(else
						(Print 25 17) ; "You don't have it."
			...
			((or (Said 'replace/key') (Said 'give/key/manager,boy'))
				(if (gEgo has: 27) ; motel_key
					...
					(Print 25 101) ; "You don't have the key to any room here."
				)
			)
			((Said '[ask,get]/key')
				(cond
					...
					((not global175)
						(Print 25 102) ; "You ask the manager for the key, and he answers... "Get serious, Mac! You rent a room...you get a key. Unfortunately for you," he says, "we don't have a vacancy.""
					)
					((not global174)
						(Print 25 103) ; "I don't care if you're the Queen of England!" he blurts out. "I have an obligation to my customers, and I know the law and there ain't nobody gettin' any key!"
					)
					((not global178)
						(Print 25 104) ; "The manager sounds like he'd really like to help as he says, "I don't know what room's key you want! Besides, I've got paying customers in all of my rooms.""
					)
					(else
						(Print 25 105) ; ""Anything you say, officer." the manager says, as he gives you the key to Room 108."
			...
			((Said 'arrest/man,boy')
				(Print 25 106) ; "You could, but it wouldn't get you anywhere."
			)
		)
	)
)
...
(instance kickInDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 25 107) ; "Your last act in life is kicking the door in as you're blasted into an unknown dimension."
				(Print 25 108) ; "Because of your improper positioning, you have been blasted into oblivion."
				(EgoDead
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= global100 12)
				(if global112
					(Print 25 109 #draw) ; "Keith says... "Sonny! Be careful now.""
					(global112
						view: 53
			...
			(1
				(Print 25 110) ; "As the door opens, you are startled by a sudden explosion."
			...
			(2
				(if (not local42)
					(Print 25 111) ; "Trained reflexes throw you back, slamming your gun hand against the wall. "That was TOO close for comfort!" you say to yourself."
			...
			(3
				(Print 25 113 #at -1 20 #width 280) ; "You watch as the "SWAT team" unloads tear gas into the room."
				(local24
					view: 268
					setLoop: 2
					posn: 73 164
		...
		(switch (= state newState)
			(0
				...
				(if (and global112 local3)
					(Print 25 114) ; "Just before you enter the gas filled room, Keith says: "Better stay put, Sonny.""
				)
				(gEgo
					ignoreControl: -1
					setLoop: -1
			...
			(1
				(if local3
					(Print 25 115) ; "You enter into the motel room, but in a short time, you are overcome by the gas and fall unconscious to the floor."
					(= seconds 2)
				else
					(HandsOn)
					(if local44
