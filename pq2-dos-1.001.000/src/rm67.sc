		...
		(13
			(cond
				(
					...
					(Print 67 65) ; "You're not close enough to your car's door."
				)
				((== global133 1)
					(Print 67 66) ; "Your car door is locked."
					(return 0)
				)
				(global135
					(Print 67 67) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(Print 67 68) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
		...
		(cond
			...
			((Said 'look>')
				(cond
					...
					((or (Said '/license[<license]') (Said '/license'))
						(cond
			...
			((or (Said 'read/license[<license]') (Said 'read/license'))
				(cond
					((gEgo inRect: 151 176 206 210)
						(switch (Random 0 2)
							(0
					...
					((gEgo inRect: 252 173 304 210)
						(switch (Random 0 2)
							...
							(2
								(Print 67 48) ; "You find a personalized front plate that reads... "WOW" you think to yourself..." Quite fitting for such a sharp Vette!""
							)
						)
					)
					(else
			...
			((or (Said 'enter/auto') (Said 'open/door'))
				(cond
					...
					(
						...
						(Print 67 49) ; "You don't need to open this door."
					)
					(
						(and
							(== (gEgo loop:) 2)
						...
						(Print 67 50) ; "This side is locked. Try the other side."
					)
					(
						(and
							(== (gEgo loop:) 3)
						...
						(Print 67 51) ; "You don't need anything from the police car."
					)
					((gEgo inRect: 220 170 240 180)
						(Print 67 52) ; "Which car?"
					)
					(else
						(localproc_0)
					)
				...
				(cond
					(
						...
						(cond
							((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
								(if (== global133 1)
									(= global133 0)
									(Print 67 53) ; "OK. It's locked"
								else
									(Print 67 54) ; "The door is already unlocked."
								)
							)
							((== global131 13)
								(Print 67 55) ; "You need a key to unlock this door."
							)
						)
						(cond
					...
					((== global131 13)
						(if (gEgo inRect: 22 148 40 152)
							(Print 67 57) ; "You don't need to do that."
						else
							(Print 67 58) ; "You're not close enough."
						)
					)
					...
					(
						...
						(cond
							((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
								(if (== global133 0)
									(= global133 1)
									(Print 67 59) ; "OK. It's locked."
								else
									(Print 67 60) ; "The door is already locked."
								)
							)
							((== global131 13)
								(Print 67 61) ; "You need a key to lock this door."
							)
						)
						(cond
							((and (== global131 33) (gEgo has: 2)) ; key_ring
			...
			(0
				...
				(cond
					...
					((not (global112 inRect: -10 0 330 240))
						...
						(Print 67 62) ; "Keith yells: "I'm coming, Sonny!""
					)
					(else
						(Print 67 63) ; "Keith says: "Wait up, Sonny!""
						(if (> (global112 y:) 158)
							(if (< (global112 x:) 102)
								(global112
			...
			(4
				(if (== global131 13)
					...
					(if
						...
						(SetFlag 156)
						(Print 67 64) ; "As you get into the car, you fill Keith in on what you've discovered."
					)
				)
		...
		(switch (= state newState)
			(0
				...
				(if global112
					...
					(Print 67 69) ; "Keith says, "Look Sonny, there's Haines.""
				else
					(Print 67 70 #at -1 40) ; "You see Officer Haines approaching."
				)
				(gEgo setMotion: MoveTo 106 163)
				(haines setMotion: MoveTo 152 178 self)
			)
			(1
				(if global112
					(global112 setAvoider: Avoid setMotion: MoveTo 110 158)
				)
				(haines setAvoider: Avoid setMotion: MoveTo 134 164 self)
			)
			(2
				(haines loop: 1)
				(Print 67 71 #draw #at -1 40) ; ""This is definitely the Correctional Officer's car." Haines says..."
				(Print 67 72 #at -1 40) ; "It's been sitting here a while, though," he continues. "The engine is cold."
				(if global112
					(Print 67 73 #at -1 40) ; "Sonny, I'm going to take a look around," Keith says. "You go ahead and check out the car."
					(global112
						illegalBits: $8000
			...
			(3
				...
				(Print 67 74 #at -1 40) ; "Haines says, "I think I'll go check around a little. There may be some witnesses.""
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(jailerdoor stopUpd:)
				(haines loop: 2)
				(Print 67 75 #at -1 20 #draw) ; "Hey Sonny!", Haines shouts. "Looks like Bains may have taken another car. This lady says her car has been stolen."
				(self cue:)
			)
			(2
			...
			(4
				(gEgo loop: 1)
				(haines loop: 0)
				(Print 67 76 #at -1 20 #draw) ; ""I figure you'll have some questions to ask her," says Haines."
				(User canInput: 1)
				(= seconds 60)
			)
			(5
				(HandsOff)
				(Print 67 77 #at -1 20 #draw) ; "I'll take it from here Sonny.", Haines assures you. "You've got your work cut out for you."
