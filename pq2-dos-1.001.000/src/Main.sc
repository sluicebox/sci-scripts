...
(procedure (proc0_7)
	(Print 0 30) ; "You're not close enough."
)

(procedure (proc0_8)
	(Print 0 31) ; "You already took it."
)

(procedure (proc0_10)
	(Print 0 32) ; "You can't do that now."
)

(procedure (proc0_9)
	(Print 0 33) ; "You don't have it."
)

(procedure (proc0_13)
	(Print 0 34) ; "You don't have your gun."
)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((or (Said '[say]/hello') (Said 'laugh,wave'))
						(Print 0 11) ; "Hi."
					)
					((Said '[say]/bye')
						(Print 0 12) ; "`Bye."
...
(instance money_clip of InvI
	...
	(method (showSelf &tmp [temp0 40])
		(Print (Format @temp0 0 35 global107) #title name #icon view 0 0) ; "You have $%u cash in your money clip."
	)
)
...
(instance plane_ticket of InvI
	...
	(method (showSelf &tmp [temp0 40])
		(Print
			(Format @temp0 0 37 (if (== global195 1) {Steelton} else {Houston})) ; "An airplane ticket to %s."
			#title
			name
			#icon
			view
...
(instance finger_print of Iitem
	(properties
		name {finger print}
