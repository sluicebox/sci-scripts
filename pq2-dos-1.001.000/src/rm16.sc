					...
					((Said 'look>')
						(cond
							...
							((Said '[<at,down][/floor,dirt,tile]')
								(localproc_1 16 20) ; "The floor tiles' shine is marred only by the smeared trail of some child's lost chocolate bar."
							)
							((Said '/pane')
								(cond
									((== (gEgo loop:) 2)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if (IsFlag 36)
					(cond
						((not (gEgo has: 16)) ; plane_ticket
							(localproc_1 16 99) ; "Yes, sir," the agent says, "we have been given authorization by the Police Department to get both of you onto the connecting flight for Houston."
							(localproc_1 16 100) ; "Here are your tickets, gentlemen. The flight will be boarding at Gate 12, up the escalator and to your right."
						)
						((== global195 0)
							(localproc_1 16 101) ; "You already have your ticket to Houston."
						)
						((== global195 1)
							(localproc_1 16 102) ; "You trade in your unused ticket to Steelton for a ticket to Houston."
						)
					)
					(= local9 0)
					...
					(localproc_1 16 103) ; "Your ticket to Houston will be $195.00, please."
					(localproc_1 16 104) ; "You look at the folding cash in your money clip, and you know beyond the shadow of a doubt that you'll never board that flight for Houston if you have to pay for it yourself."
					(HandsOn)
					(if
						(and
			...
			(6
				(if (IsFlag 37)
					(cond
						((not (gEgo has: 16)) ; plane_ticket
							(localproc_1 16 105) ; "Yes, sir," the agent says, "we have been given authorization by the Police Department to get both of you onto the next flight for Steelton."
							(localproc_1 16 100) ; "Here are your tickets, gentlemen. The flight will be boarding at Gate 12, up the escalator and to your right."
							(if (gEgo has: 10) ; field_kit
								(localproc_1 16 106) ; ""You check your field kit onto the Steelton flight."
							)
						)
						((== global195 1)
							(localproc_1 16 107) ; "You already have your ticket to Steelton."
						)
						((== global195 0)
							(localproc_1 16 108) ; "You trade in your unused ticket to Houston for a ticket to Steelton."
						)
					)
					(= local10 0)
					(SetScore 3 155)
					...
					(localproc_1 16 109) ; "Your ticket to Steelton will be $160.00, please."
					(localproc_1 16 110) ; "You look at the folding cash in your money clip, and you know beyond the shadow of a doubt that you'll never board that flight for Steelton if you have to pay for it yourself."
					(HandsOn)
					(if
						(and
							(gCast contains: global112)
			...
			(7
				(localproc_1 16 111) ; "You dig into your money clip, but you can't come up with enough for the ticket. "Oh, well..." you think..." It's not the right season to visit Coarsegold, anyway!""
			)
		)
	)
)
...
(instance keithJoinsEgoScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_1 16 112 83 25 3) ; "Here comes Keith."
			...
			(4
				...
				(switch global169
					(1
						(= global169 3)
						(localproc_1 16 113 83) ; "Keith says, "Well, old buddy...while you were in here wondering what to do, I managed to find Bains stolen car out there in the parking lot.""
						(localproc_1 16 114) ; "I've already notified dispatch, and they've towed it on down to the impound lot."
						(localproc_1 16 115) ; "Anyway, what have you found out?"
					)
					(2
						(localproc_1 16 116 83) ; "Keith says, "Well, old buddy...while you were in here wondering what to do, I managed to notify dispatch, and they've towed that stolen car on down to the impound lot.""
						(localproc_1 16 115) ; "Anyway, what have you found out?"
					)
					(3
						(localproc_1 16 117 83) ; "Keith says, "Well old buddy...I couldn't find anything else out there. They've towed the stolen car away. Anything happening in here?""
					)
				)
				(= global169 0)
				(self cue:)
			...
			(5
				(localproc_1 16 118 83) ; "You tell Keith that you found Luis Pate's name on a list of passengers for a Houston flight."
				(localproc_1 16 119) ; "It HAS to be Bains that bought that ticket!" you tell Keith. "He must've used Pate's identification."
...
(instance getTheTicketScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(localproc_1 16 120) ; "Keith jumps in, saying "Hey, Sonny, put your wallet away! Let me call Captain Hall for authorization to purchase tickets.""
				(= local15 1)
			...
			(4
				(localproc_1 16 121 25 7 83) ; "Keith shows his badge to the ticket agent and requests permission to use the desk phone."
				(self cue:)
			)
			(5
				(global112 hide:)
			...
			(6
				(localproc_1 16 122 25 10 83) ; "As Keith calls to arrange for the purchase of your tickets, your mind works furiously as you try to put together all the pieces of information you've gathered in the course of your investigation."
				(self cue:)
			)
			(7
			...
			(9
				...
				(if (or local14 local13)
					(localproc_1 16 123 83) ; "Hey, man," Keith tells you, "it's all set. Fletch has a purchase order for our tickets, and he's getting it cleared with the ticket agent right now."
					(cond
						(local9
							(SetFlag 36)
							(= local9 0)
						...
						(local10
							...
							(= local10 0)
							(if (not (IsFlag 94))
								(localproc_1 16 124) ; "And Sonny," Keith says, "the Captain suggests that you call Steelton Police and have them warn Colby that Bains is loose."
							)
						)
					)
				else
					(localproc_1 16 125) ; "Hey, man," Keith tells you, "Fletch said that we don't have near enough evidence booked to justify authorization from him to go flying off anywhere."
					(localproc_1 16 126) ; "I guess we'll just have to continue our investigation right here in Lytton."
				)
				(HandsOn)
				(self cue:)
			)
...
(instance egoOnThePhoneScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(localproc_1 16 127 25 3 83) ; "You pick up the phone."
				(self cue:)
			)
			(4
				(HandsOff)
