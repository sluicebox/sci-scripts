				...
				(cond
					...
					((Said 'sat')
						(if (not global201)
							...
							(proc154_1 41 14 67 20 20 33 gSmallFont)
						)
					)
					((Said 'look>')
						(cond
		...
		(switch (= state newState)
			...
			(1
				...
				(if local103
					(cond
						((== local0 3)
							(Format @local1 41 26) ; "You fire at the hijacker but unfortunately, hit the stewardess was in the way and you her, and the hijacker kills you."
						)
						(global197
							(Format @local1 41 27) ; "You fire at the hijacker but miss because of your intoxicated condition. Unfortunately he didn't miss you!"
						)
						(else
							(Format @local1 41 28) ; "You fire at the hijacker but miss because your gun is not sighted."
						)
					)
				else
					(Format @local1 41 29) ; "Because of your furtive movement, the hijacker unloads on you."
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(Format @local1 41 37) ; "The masked hijacker kills you, avenging his companions death."
				(if global201
					(gEgo
						view: 83
						loop: 6
