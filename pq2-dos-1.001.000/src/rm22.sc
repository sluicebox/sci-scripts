...
(procedure (localproc_1)
	(switch global131
		(13
			(cond
				((not (gEgo inRect: 230 118 250 127))
					(localproc_0 22 102) ; "You're not close enough to the door."
				)
				((== global133 1)
					(localproc_0 22 103) ; "Your car door is locked."
					(return 0)
				)
				(global135
					(localproc_0 22 104) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(localproc_0 22 105) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
		...
		(33
			(cond
				((not (gEgo inRect: 230 118 250 127))
					(localproc_0 22 106) ; "You're not close enough to the door"
				)
				(global136
					(localproc_0 22 103) ; "Your car door is locked."
				)
			...
			(evSAID
				(cond
					((Said 'look>')
						(cond
							...
							((Said '/friend')
								(if
									...
									(if (gEgo inRect: 235 102 300 158)
										...
										(localproc_0 22 3) ; "You can't see him from where you're standing. The garage is not well lighted."
									)
								else
									(event claimed: 0)
								)
							...
							((Said '[<at,around][/building,garage,chamber]')
								(localproc_0 22 4 70 260) ; "This is the underground parking garage of the new Lytton City Jail. You are filled with pride as the memories of the dedication ceremonies return."
								(if (>= global100 1)
									(localproc_0 22 5 70 280) ; "Following your heroic capture of Jessie Bains (aka: the Death Angel) you became an instant celebrity. But now, unless you recapture Bains, only the bitter memories of failure will remain..."
								)
							)
					...
					((Said 'get,remove/9mm')
						(cond
							...
							((== ((gInventory at: 0) owner:) 22) ; hand_gun
								(localproc_0 22 65) ; "You take your gun, holster it, and close and lock the locker."
								(gEgo get: 0) ; hand_gun
								(= global165 0)
							)
							(else
						...
						(if (gEgo inRect: 135 99 163 108)
							(if global165
								(if (gEgo has: 0) ; hand_gun
									(localproc_0 22 67) ; "You place your gun in the locker."
									(gEgo put: 0 22) ; hand_gun
								else
									(localproc_0 22 68) ; "Whaddaya mean? You don't have your gun!"
								)
							else
								(localproc_0 22 63) ; "You should open the locker first."
							)
					...
					((Said 'drop,place,replace,throw/key[<locker]')
						(if local18
							(localproc_0 22 69) ; "Don't do that."
						else
							(localproc_0 22 34) ; "You don't have it."
						)
					)
					((Said 'get/key<locker')
						(if local18
							...
							(localproc_0 22 70) ; "You must close and lock the locker in order to get the key."
						)
					)
					((Said 'get/key')
						(if (and (gEgo inRect: 135 99 163 108) local18)
							(localproc_0 22 71) ; "You have the locker key."
						else
							(localproc_0 22 72) ; "You have the car keys."
						)
					)
					(
						(or
						...
						(if (gEgo inRect: 135 99 163 108)
							(if global165
								(= global165 0)
								(localproc_0 22 73) ; "You close and lock the locker."
								(if (== ((gInventory at: 0) owner:) 22) ; hand_gun
									(SetScore 3 115)
								)
							else
								(localproc_0 22 74) ; "The locker is already closed."
							)
						else
							(localproc_0 22 62) ; "You're too far away."
						)
					...
					((Said 'open,move/gate,fence')
						(localproc_0 22 75) ; "This area is under construction. There is no need to go past the gate."
					)
					((Said 'climb/gate,fence')
						(localproc_0 22 76) ; "You're wasting your time."
					)
					((Said 'unlock/door')
						(cond
							((gEgo inRect: 230 118 250 127)
								(cond
									((== global131 13)
										(if (gEgo has: 3) ; unmarked_car_keys
											(if global133
												(= global133 0)
												(localproc_0 22 77) ; "OK. It's unlocked."
											else
												(localproc_0 22 78) ; "The door is already unlocked."
											)
										else
											(localproc_0 22 79) ; "You need a key to unlock this door."
										)
									)
									((== global131 33)
							...
							((gEgo inRect: 230 129 260 138)
								(localproc_0 22 80) ; "If you're getting into your car, go around to the driver's side."
							)
							(else
								(localproc_0 22 45) ; "You're not close enough."
							)
						...
						(cond
							((gEgo inRect: 230 118 250 127)
								(if (== global131 13)
									(if (not global133)
										(= global133 1)
										(localproc_0 22 81) ; "OK. It's locked."
									else
										(localproc_0 22 82) ; "The door is already locked."
									)
								)
								(if (== global131 33)
									(if (not global136)
							...
							((gEgo inRect: 230 129 260 138)
								(localproc_0 22 83) ; "Go around to the driver's side."
							)
							(else
								(localproc_0 22 45) ; "You're not close enough."
							)
						...
						(cond
							((== global131 13)
								(if (gEgo inRect: 176 123 206 135)
									(cond
										(global135
											(Print 22 84) ; "It's already open."
										)
										((gEgo has: 3) ; unmarked_car_keys
											(carScript changeState: 14)
										)
										(else
											(localproc_0 22 85) ; "You need a key to open this trunk."
										)
									)
								else
									(localproc_0 22 86) ; "You're not close enough to your trunk, and you shouldn't open anybody else's trunk!"
								)
							)
							((gEgo inRect: 176 123 206 135)
								(localproc_0 22 87) ; "Your car's "hatch-back" hasn't worked since its warranty expired."
							)
							(else
								(localproc_0 22 86) ; "You're not close enough to your trunk, and you shouldn't open anybody else's trunk!"
							)
						...
						(if (== global131 13)
							(if (gEgo inRect: 176 123 206 135)
								(if global135
									...
									(Print 22 88) ; "It's already closed."
								)
							else
					...
					((Said 'close/door')
						(localproc_0 22 89) ; "The door is already closed."
					)
					((Said 'drop,place/briefcase')
						(if (gEgo inRect: 176 123 206 135)
							(if global135
								(if (gEgo has: 10) ; field_kit
									(localproc_0 22 90) ; "You place your field kit inside the trunk."
									...
									(localproc_0 22 91) ; "You don't have the field kit."
								)
							else
								(localproc_0 22 92) ; "Open the trunk first."
							)
						else
							(localproc_0 22 93) ; "You're not close enough to your car's trunk."
						)
					)
						...
						(if (gEgo inRect: 176 123 206 135)
							(if global135
								(if (== ((gInventory at: 10) owner:) 13) ; field_kit
									(localproc_0 22 94) ; "You take your field kit from the trunk."
									(gEgo get: 10) ; field_kit
								else
									(localproc_0 22 95) ; "The field kit is not in the trunk."
								)
							else
								(localproc_0 22 92) ; "Open the trunk first."
							)
...
(instance doorScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(localproc_0 22 96) ; "After showing your ID, you hear a loud metal "clank" as the jailer releases the door locks."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (== global131 13)
					(if local7
						(localproc_0 22 97) ; "Gee, Sonny." Keith says. "I thought we were finished here."
						(localproc_0 22 98) ; "I'll just kick back in the car for a while if you have some business to take care of."
						(SetFlag 142)
					else
						(local3 dispose:)
						(global112
							illegalBits: 0
							setMotion: MoveTo 200 136 self
						)
					)
				)
			)
			...
			(5
				(global112 setMotion: MoveTo 133 158 self)
			)
			(6
				(global112 setMotion: MoveTo -5 158)
			)
			(7
				(global112 stopUpd:)
			)
			(8
				...
				(if (== global131 13)
					(if (IsFlag 142)
						(localproc_0 22 100) ; "Keith wakes up and says "Hi, Sonny.""
						(self changeState: 12)
					else
						(localproc_0 22 101) ; "Keith yells: "Wait up, Sonny! What's the rush, anyway?""
						(self cue:)
					)
				else
					(self changeState: 12)
			...
			(10
				(global112 setMotion: MoveTo 250 136 self)
			)
			(11
				((= local3 (Prop new:))
					view: 51
