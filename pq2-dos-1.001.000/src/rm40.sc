...
(instance initScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: MoveTo 290 65 self)
				(global112 setMotion: MoveTo 285 60)
				(global198 loop: 0)
			)
			(1
				(proc154_1 40 2)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				...
				(local11 dispose:)
				(EgoDead
					{Sonny, you really must learn not to be such a nuisance. It's not that hard to find your seat or fasten your seatbelt!}
				)
			)
		)
	)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said '/no')
						(if (and (== state 4) (not local9))
							(proc154_1 40 35 67 20 40 33 gSmallFont)
							(= local9 1)
							(self changeState: 5)
						else
							(event claimed: 0)
					...
					((Said 'call/attendant')
						(if (== state 4)
							...
							(Print 40 18 #at 20 40 #font gSmallFont) ; "Ok."
							(= local7 1)
						)
					)
					((Said 'drink')
					...
					((Said 'use,enter/bathroom,bathroom,(chamber<(bath,rest))')
