	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(17
				...
				(vanDoor setMotion: MoveTo 290 175)
				(local13 setLoop: -1 setMotion: MoveTo 224 180 self)
			)
			(18
				(local13 setMotion: MoveTo 220 139 self)
			)
			(19
				(if local17
					(HandsOn)
				...
				(cond
					...
					((Said 'find,get,show,frisk,look/card,certificate')
						(if local15
							...
							(Print 60 30) ; "It wouldn't do any good right now."
						)
					)
					((Said 'talk/cop,man,diver,diver')
						(if
							...
							(Print 60 31) ; "Officer Moore says, "Hi, Sonny.""
						else
							(Print 60 32) ; "He can't hear you."
						)
					)
					((Said 'apprehend,bath/bains')
						(if (and (== global111 3) global183 global188)
							(Print 60 33) ; "I wouldn't advise it, but it's up to you."
						else
							(Print 60 34) ; "That's a fine idea... but how?"
						)
					)
					((or (Said '/police,stop') (Said 'stop'))
						(if (and (== global111 3) global183 global188)
							(Print 60 35) ; ""HALT! POLICE!!" you shout."
						else
							(Print 60 36) ; "Who are you talking to?"
						)
					)
					((Said 'look,hit,get,talk/bains,suspect')
						(cond
							(local1
								(Print 60 37) ; "Bains' face is a mask of hatred."
							)
							((and (== global111 3) global183 global188)
								(Print 60 38) ; "Are you serious??? This man is trying to kill you!"
							)
							(else
								(Print 60 34) ; "That's a fine idea... but how?"
							)
					...
					((Said 'look,read/heart,initials')
						(if (gEgo inRect: 0 114 60 145)
							(Print 60 39) ; "The memories return as you read, "M.W + S.B""
							(Print 60 40) ; "You carved those initials one night in high school. Marie wanted to watch the submarine races, but they were called off due to low tide."
						else
							(Print 60 41) ; "You can't read the initials from way over here."
						)
					)
					((or (Said 'open/door,van') (Said 'enter,(get<in)/van,bus'))
						(if (and (> global189 2) (>= global111 3))
							(if (== global189 8)
								(Print 60 42) ; "Just walk up to it."
							else
								(Print 60 43) ; "You have no reason to do that."
							)
						else
							(Print 60 44) ; "That's nowhere to be seen."
						)
					)
					((Said 'get,drive,rob/van,bus')
						(Print 60 45) ; "The van's not yours."
					)
					(
						(Said
							'find,get,frisk,look/clue,blood,mark,footprint,(print<feet)'
						)
						(Print 60 46) ; "You look, but can't find any evidence here."
					)
					((Said 'look>')
						(cond
							((Said '/diver,diver,cop,man')
								(cond
									(
										...
										(Print 60 47) ; "Officer Moore is in top physical condition. He's an energetic and happy kind of guy."
									)
									((Said '/man')
										(event claimed: 0)
									)
									(else
										(Print 60 48) ; "He's not here."
									)
								)
							)
							((Said '/auto')
								(if local1
									(Print 60 49) ; "Bains has jumped into his getaway car, and is speeding off."
								else
									(Print 60 50) ; "There are no cars here."
								)
							)
							((Said '/license')
								(cond
									(local1
										(Print 60 51) ; "All you can make out, as the car whizzes by you, is "C03"."
									)
									((and (>= global189 5) (== global100 5))
										(Print 60 52) ; "The van's license plate is of no interest to you."
									)
									(else
										(Print 60 53) ; "There are no license plates here."
									)
								)
							)
							((Said '/van,bus,pane,door')
								(if (and (> global189 2) (>= global111 3))
									(Print 60 54) ; "This is a 'Special Forces' van, used just for unusual circumstances."
								else
									(Print 60 44) ; "That's nowhere to be seen."
								)
							)
							((Said '<behind/tree')
								(Print 60 55) ; "You look, but find nothing behind the tree."
							)
							((Said '<under/bush')
								(Print 60 56) ; "You look, but find nothing under the bush."
							)
							((Said '/tree')
								(if (gEgo inRect: 0 117 62 147)
									(Print 60 57) ; "You look and see a heart, with initials, carved into the trunk of the tree."
								else
									(Print 60 58) ; "From this distance, it looks like any other oak tree."
								)
							)
							((Said '[<at,around][/(!*,cove,area)]')
								(Print 60 59) ; "This is Cotton Cove. It brings back many memories of your youth."
							)
						)
					)
					((Said '/clue')
						(Print 60 60) ; "You look all around, but find nothing."
					)
					((Said '/bullet')
						(Print 60 61) ; "You look, but don't see any expended shells."
					)
					(
						(or
							...
							(Said '/swim,dive,help')
							(Said 'enter,jump/lake,clearwater,water')
							(Said 'let//dive,swim,find,frisk,look,get')
							(Said 'let/look,frisk,find,dive,get,enter')
						)
						(if (> global189 5)
							(if (< (gEgo distanceTo: local13) 40)
								...
								(Print 60 62) ; "Officer Moore can't hear you. You'd better get closer."
							)
						else
							(Print 60 63) ; "That's a fine idea. But how?"
						)
					)
				)
			)
...
(instance bainsScript of Script
	...
	(method (doit)
		...
		(if global205
			(if (== local10 0)
				(if local7
					(Print 60 64 #at -1 24) ; "Although shaking from surprise, confusion, and fear, you somehow manage to return fire!"
				)
				(if (and global183 (or (not global166) (!= global205 4)))
					(Print 60 65 #at -1 20) ; "Unfortunately, however, the flight of your bullet misses its target, and the killer continues his ambush!"
					(= local10 1)
				else
					(= local10 2)
					(= global208
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(cond
					(local7
						(Print 60 66 #at -1 20) ; "Although the flight of your bullet misses its target, it comes close enough to cause the suspect to take leg bail."
					)
					(global183
						(Print 60 67) ; "As Bains comes running towards you, you notice the gun in his hand, analyze the situation quickly, and take action. Although your bullet misses its mark, it comes close enough for the suspect to take leg bail."
					)
					(else
						(EgoDead 60 68) ; "Your nerves on edge, you fired wildly into the bushes. Was it Bains, or a little boy playing? Without knowing, you should not have discharged your weapon. Next time, be sure you are in danger before firing."
					)
				)
				(SetScore 4)
			...
			(7
				(Print 60 69 #at -1 20) ; "Just before taking a hot round in the chest, you hear Bains yell... "This one's for you, PIG!!!""
				(self changeState: 4)
			)
			(8
				(= seconds 2)
			...
			(9
				(switch local10
					(1
						(EgoDead 60 70) ; "Too bad, so sad, you've been had. But wait! All is not lost. Just restore your game and begin again. Next time though, make sure your gun is properly sighted in and pointed in the right direction."
					)
					(else
						(EgoDead 60 71) ; "Too bad, so sad, you've been had. But wait! All is not lost. Just restore your game and begin again. Next time though, you might try returning gun fire before it's too late."
					)
				)
			)
		)
...
(instance carScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (== global131 13)
					(Print 60 72) ; ""That was BAINS!" shouts Keith."
			...
			(8
				(if (== global131 13)
					(global112 loop: 1)
					(Print 60 73 #draw) ; "Gee, Sonny, did YOU see THAT?!" gasps Keith. "Sonny. Sonny? SONNY!?"
				)
				(= seconds 4)
			)
			(9
				(if (== global131 13)
					(EgoDead 60 74) ; "Don't worry; Keith will explain in his report how he heroically 'watched your back'."
				)
			)
		)
	)
