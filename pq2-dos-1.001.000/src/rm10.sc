	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(attendant view: 72 loop: 3 cel: 0 posn: 161 156 stopUpd: init:)
				(= local1 0)
				(gEgo
					posn: 20 145
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(or
							(Said 'ask,use/range[<ambush]')
							(Said 'use/booth[<ambush]')
...
(instance shooterScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				([local5 local4] setCycle: Beg self startUpd:)
			)
			(1
		...
		(switch (= state newState)
			(0
				...
				(if (not local71)
					...
					(rightArm init: signal: 26640)
					(leftArm init: signal: 26640)
					((= local14 (Prop new:))
						view: 70
						posn: 159 59
						setLoop: 2
			...
			(2
				...
				(if (and (== [global215 global207] 6) (== local73 0))
			...
			(3
				(if (HandsOff)
					(gEgo yStep: 4 setCel: 0 setMotion: MoveTo 160 150 self)
				)
			)
			(4
				(gEgo setStep: 1 1)
			...
			(5
				(if (HandsOff)
					(gEgo yStep: 4 setMotion: MoveTo 160 245 self setCel: 0)
				)
			)
			(6
				(gEgo setStep: 1 1)
			...
			(evSAID
				(cond
					...
					((Said 'look/bullseye,hole')
						(cond
							...
							((and (<= -6 global103 6) (<= -4 global104 4))
								(Print 10 52) ; "Your sights are properly aligned."
							)
							((< global104 -4)
								(Print
					...
					((Said 'look/button,wall')
						(Print 10 56) ; "There are two buttons on the wall of the booth. One is labeled 'VIEW', and the other is labeled 'BACK'."
					)
					((or (Said 'press/view,(button<view)') (Said 'view'))
						(cond
							(global204
								(Print 10 57) ; "With what? The barrel of your gun? Lower your weapon first."
							)
							(local76
								(Print 10 47) ; "Wait a while."
							)
							...
							(else
								(Print 10 58) ; "This is as close as the target gets."
							)
						)
					)
					(
						...
						(cond
							((== (local14 cel:) 4)
								(Print 10 59) ; "The target is as far back as it will go."
							)
							(global204
								(Print 10 60) ; "Lower your weapon first."
							)
							((and (> local16 0) local15)
								(Print 10 61) ; "You had better replace the old target with a new one first."
							)
							(local76
								(Print 10 47) ; "Wait a while."
							)
					...
					((Said 'press/button')
						(Print 10 62) ; "Which button do you want to push?"
					)
					(
						(or
						...
						(cond
							((== (local14 cel:) 4)
								(Print 10 63) ; "You can't change the target while it is back there."
							)
							(global204
								(Print 10 60) ; "Lower your weapon first."
							)
					...
					((Said 'adjust/bullseye')
						(Print 10 64) ; "That would make it too easy."
					)
					((Said 'adjust/sight,9mm,windage,elevation')
						(if (not global204)
							...
							(Print 10 65) ; "Lower your gun first."
						)
					)
					...
					(
						...
						(cond
							...
							(local73
								(Print 10 66) ; "You are wearing them."
							)
							(global204
								(Print 10 60) ; "Lower your weapon first."
							)
					...
					((Said '(get<off),remove/ep[<ear]')
						(cond
							((not local73)
								(Print 10 67) ; "You are not wearing ear protectors."
							)
							((<= (gEgo y:) 236)
								(Print 10 60) ; "Lower your weapon first."
							)
			...
			($0040 ; direction
				...
				(switch (= local24 (event message:))
					...
					(JOY_RIGHT
						(local69
							cel:
								(if (== (local69 cel:) (local69 lastCel:))
									0
								else
									(+ (local69 cel:) 1)
								)
					...
					(JOY_DOWN
						(local70
							cel:
								(if (== (local70 cel:) (- (NumCels local70) 1))
									0
								else
									(+ (local70 cel:) 1)
								)
