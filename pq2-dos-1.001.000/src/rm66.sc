		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					((Said 'look>')
						(cond
							((Said '[<around,in,in][/van,equipment]')
							...
							(
								...
								(Print 66 5 #at -1 155) ; "The tanks contain compressed air used for scuba diving."
								(Print 66 6) ; "You may choose one of the yellow tanks. Which would you like; number 1, 2, or 3?"
							)
							((Said '/belt[<weight]')
								(Print 66 7 #at -1 155) ; "It is made of sturdy material and filled with lead weights. It is used to overcome bouyancy."
							)
							(
								(or
									(Said '/bcv[<buoyancy]')
								...
								(Print 66 8 #at -1 155) ; "You can inflate it (from your tank) or deflate it to control your depth."
							)
							((Said '/fin')
								(Print 66 9 #at -1 155) ; "They help you to swim like a fish."
							)
							((Said '/mask')
					...
					((Said 'get/bottle[<yellow,air]')
						(if local112
							(Print 66 13 #at -1 155) ; "Which one?"
						else
							(Print 66 6 #at -1 155) ; "You may choose one of the yellow tanks. Which would you like; number 1, 2, or 3?"
							(= local112 1)
						)
					)
					((or (Said '[get]/1') (Said '[get]/bottle<1'))
						(if (IsFlag 3)
							(if (== local102 1)
								(Print 66 14 #at -1 155) ; "You already have that tank."
							else
								(Print 66 15 #at -1 155) ; "You can only use one tank at a time."
							)
						else
							(Print 66 16 #at -1 155) ; "You remove the air tank from the back of the van."
							(SetFlag 3)
							(local108 setCel: 4)
							(= local102 1)
						)
					...
					((Said 'get/belt[<weight]')
						(if (IsFlag 6)
							(Print 66 18 #at -1 155) ; "You already took that item."
						else
							(Print 66 19 #at -1 155) ; "You take the weight belt."
							(local103 hide:)
							(SetFlag 6)
						)
					)
					((Said 'get/mask')
						(if (IsFlag 8)
							...
							(Print 66 20 #at -1 155) ; "You take the diving mask."
							(local106 hide:)
							(SetFlag 8)
						)
					)
					((Said 'get/fin>')
						(cond
							...
							(else
								(Print 66 22 #at -1 155) ; "You take the pair of yellow fins."
								(local107 hide:)
								(SetFlag 5)
							)
						)
					...
					(
						...
						(if (IsFlag 4)
							(Print 66 23 #at -1 155) ; "You've already changed into the wet suit."
						else
							(Print 66 24 #at -1 155) ; "You take the wet suit, and change into it."
							(local105 hide:)
							(SetFlag 4)
						)
					)
					(
						...
						(if (IsFlag 7)
							...
							(Print 66 25 #at -1 155) ; "You take one buoyancy control vest."
							(local104 hide:)
							(SetFlag 7)
						)
					)
					...
					((Said 'replace,drop/bottle,1,2,3')
						(if local102
							...
							(Print 66 28 #at -1 155) ; "You put the tank back into its wall brackets."
							(= local102 0)
							(ClearFlag 3)
							(= local113 1)
						else
							(Print 66 29 #at -1 155) ; "You don't have a tank to put back."
						)
					)
