			...
			(evSAID
				(cond
					...
					((Said 'look/mirror')
						(switch (Random 0 2)
							(0
								(Print 33 24) ; "You're getting paranoid. Don't worry... nobody's following you."
							)
							(1
								(Print 33 25) ; "Don't be so vain!"
							)
							(2
								(Print 33 26) ; "You look in the mirror and think to yourself... "You good looking devil you!""
							)
						)
					)
					((Said 'look/dash')
						(Print 33 27) ; "Your car's dashboard looks like any other economy car's."
						(if (IsItemAt 2 0) ; key_ring
							(Print 33 28) ; "In the ignition, you see keys on a ring."
						)
					)
					((Said 'look/key,ring')
						(if (IsItemAt 2 0) ; key_ring
					...
					((Said 'look/wheel[<steering]')
						(Print 33 29) ; "The brown "rubber" of the steering wheel is very stylishly molded."
					)
					((Said 'look/back,(bench<back)')
						(Print 33 30) ; "You see an adventure gamer looking over your shoulder."
					)
					((Said 'look/air,tree,home,cloud')
						(Print 33 31) ; "You look, but the view isn't that good from here."
					)
					((Said 'adjust/')
						(Print 33 0) ; "OK."
					)
					((Said '/extender')
						(Print 33 32) ; "Click..."
						(Print 33 33) ; "Nothing. The radio's warranty has expired. But that's ok - it sounded terrible anyway."
					)
					(
						(or
							(Said 'wear,fasten,use,buckle,(drop<on)/belt,belt')
							(Said 'buckle')
						)
						(Print 33 34) ; "Good idea. Safety first."
					)
					((Said 'turn,look<on/heat,air,conditioner,fan')
						(Print 33 35) ; "The car's "automatic climate control system" is on the fritz."
					)
					((Said '/tray,ashtray')
						(Print 33 36) ; "Since you don't smoke, you use the ashtray for change. However, it's empty at the moment."
					)
					((Said '/visor')
						(Print 33 37) ; "The visors are made of cheap plastic. Fortunately, the sun isn't in your eyes right now."
					)
					(
						(or
							...
							(Said 'drop/key/ignition')
						)
						(PutItem 2 0) ; key_ring
						(Print 33 38) ; "The little gutless-wonder of an engine is running."
					)
					(
						(or
						...
						(if (== local4 local6)
							(Print 33 39) ; "The car's engine is off."
						else
							(Print 33 1) ; "It would probably be a good idea to wait until you've arrived."
						)
					)
					((Said 'open/box,compartment')
						(if (== global137 1)
							(Print 33 40) ; "The glove box is already open."
						else
							(= global137 1)
							(gloveBoxScript changeState: 0)
						)
					...
					((Said 'close/box,compartment')
						(if (== global137 0)
							(Print 33 41) ; "The glove box is already closed."
						else
							(= global137 0)
					...
					((Said 'look/box,compartment')
						(if (gCast contains: gloveBox)
							...
							(Print 33 42) ; "Open the glove box first."
						)
					)
					((Said 'get,remove/key,ring')
						(cond
							((not (IsItemAt 2 0)) ; key_ring
								(Print 33 43) ; "There are no keys here."
							)
							((not local0)
								(gEgo get: 2) ; key_ring
								(Print 33 44) ; "You take the keys from the ignition."
								(SetScore 1 102)
							)
							(else
								(Print 33 1) ; "It would probably be a good idea to wait until you've arrived."
					...
					((Said 'get,remove/card')
						(cond
							...
							((and global137 (IsItemAt 37)) ; your_LPD_business_card
								...
								(businessCard startUpd: hide:)
								(Print 33 45 #draw) ; "OK"
								(SetScore 1 103)
							)
							(else
								(Print 33 46) ; "You see no card here."
							)
						)
					)
					((Said 'drop,replace/card')
						(cond
							...
							(else
								...
								(businessCard show: stopUpd:)
							)
						)
					)
					((Said 'look,get/newspaper,registration')
						(if (== global137 0)
							(Print 33 47) ; "There is no paper here."
						else
							(registration startUpd: hide:)
							(Print 33 48 #mode 1 #draw) ; "This is the vehicle's registration. Careful examination reveals that this car belongs to Sonny Bonds. Why... that's YOU!"
							(Print 33 49) ; "The registration should stay in the car, so you return it to the glove box."
					...
					(
						...
						(Print 33 50) ; "You can't see much from inside the car."
					)
					((Said 'look[<at,around,in][/!*,auto]')
						(Print 33 51) ; "You're behind the wheel of your personal car. Marie talked you into trading in your sports car for this... "sensible" compact model."
					)
					((Said 'beep,press/horn')
						(Print 33 52) ; "beeep beeep"
					)
					((Said 'drive/auto')
						(Print 33 53) ; "To drive somewhere, type: "drive to (destination)""
					)
					((Said 'drive,enter>')
						(PutItem 2 0) ; key_ring
						(cond
							(
								...
								(cond
									...
									(local0
										(Print 33 54) ; "You're already going there."
									)
									(else
										(Print 33 55) ; "You're already there."
									)
								)
							)
							((Said '/jail[<lytton]')
							...
							(
								...
								(cond
									(
										...
										(Print 33 56) ; "Murder? What murder?"
									)
									((!= local1 27)
										(= local1 27)
										(= local4 62)
							...
							((Said '/ave<10th<5556')
								(Print 33 57) ; "You have hotter leads to follow."
								(User canInput: 1)
							)
							(
								(or
								...
								(Print 33 58) ; "Lytton has seen many businesses fail in the last year... and that was one of them."
							)
							((Said '/steelton,houston,coarsegold')
								(User canInput: 1)
								(Print 33 59) ; "That's a great place, but it's too far to drive there right now."
							)
							((Said '/[*]')
								(User canInput: 1)
								(Print 33 60) ; "You don't need to drive there."
							)
							(else
								(event claimed: 1)
								(Print 33 61) ; "A good detective always knows where he's going."
							)
						)
					)
					(
