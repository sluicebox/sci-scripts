...
(instance Room654 of Rm
	...
	(method (init)
		...
		(= local63 0)
		(gEgo posn: 150 125 view: 4 xStep: 4 yStep: 1 init: setCycle: Walk)
		((= [local51 0] (View new:))
			view: 677
			posn: 78 103
			loop: 3
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= local58 0)
				(= local60
					(Print 654 15 #font gSmallFont #at 200 20 #width 110 #dispose) ; "You watch quietly as the dwarfs file in one by one, get a bowl of soup, and take a seat at the table."
				)
				(dwarfMusic play:)
			...
			(3
				([local51 0] cel: (+ ([local51 0] cel:) 1) startUpd:)
				([local1 local58]
			...
			(16
				([local1 local58]
					...
					moveSpeed: 0
					setCycle: Walk
					setMotion: MoveTo 140 ([local1 (+ local58 6)] y:) self
				)
