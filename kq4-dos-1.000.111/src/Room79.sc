		...
		(if (and (== (gCurRoom script:) 0) (== global105 0))
			(cond
				((& (gEgo onControl: 0) $0002)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(gCurRoom setScript: fallBlue)
					(stopHench cue:)
				)
				((& (gEgo onControl: 0) $0008)
					...
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallCyan)
				)
				((& (gEgo onControl: 0) $0004)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallGreen)
				)
				((& (gEgo onControl: 0) $0080)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallLgrey)
				)
				((& (gEgo onControl: 0) $0100)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallGrey)
				)
				((& (gEgo onControl: 0) $0200)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallLblue)
				)
				((& (gEgo onControl: 0) $0400)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallLgreen)
				)
				((& (gEgo onControl: 0) $0800)
					(gSounds eachElementDo: #dispose)
					(sFalling loop: 1 number: 51 play:)
					(stopHench cue:)
					(gCurRoom setScript: fallLcyan)
				)
...
(instance h1Actions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(henchTheme number: 29 loop: -1 play:)
				(local0
					ignoreHorizon:
					posn: 57 7
					view: 142
