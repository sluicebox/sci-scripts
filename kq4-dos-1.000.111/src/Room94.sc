		...
		(cond
			...
			((Said '/unicorn,horse>')
				(cond
					...
					((Said 'free')
						(Print 94 13) ; "Try opening the gate."
					)
					(else
						(Print 94 14) ; "That would not be a good idea."
						(event claimed: 1)
					)
				)
			)
			((Said 'look>')
				(cond
					((Said '/gate')
						(Print 94 15) ; "You see two stable gates."
					)
					((Said '/wall')
						(Print 94 16) ; "You see nothing of importance on the walls."
					)
					((Said '[<around][/room,barn]')
						(Print 94 17) ; "You have entered Lolotte's stable... and, it looks like an ordinary stable at that."
						(if (== gUnicornState 99)
							(Print 94 18) ; "The poor unicorn has been imprisoned here!!"
						)
					)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(Print 94 19 #at -1 15 #width 290) ; "As soon as you open the gate, the splendid wild unicorn impatiently leaves its hated prison. It quickly trots out the open stable door to freedom."
			)
			(6
				(local0
					setLoop: 2
