		...
		(switch (= state newState)
			...
			(6
				(if (== ((ScriptID 520 1) state:) 3) ; KQ4MUSIC
					...
					(openMusic number: 105 loop: 1 play:)
					(self cue:)
				)
			)
			(7
			...
			(10
				...
				(local13 view: 766 loop: 3 cel: 0 posn: 230 72 setPri: 2)
				(= local26
			...
			(19
				(if
					(or
						(not (gSounds contains: openMusic))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(local6 show:)
				(local6 setLoop: 1 setCycle: End self)
			)
			(5
				(local6 setLoop: 3 hide:)
				(= local4 ((Act new:) view: 765 setLoop: 4 init: yourself:))
				(local4
					cel: 2
					setCel:
					posn: (+ (local6 x:) 9) (+ (local6 y:) 6)
					setPri: 15
					ignoreActors:
					setStep: 0 1
					setMotion: MoveTo (+ (local6 x:) 9) (+ (local6 y:) 30) self
				)
			)
			(6
				(local4 dispose:)
