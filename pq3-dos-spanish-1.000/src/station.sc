	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(cond
					((IsFlag 207)
						(EgoDead 19) ; "Your failure to respond to the mall resulted in a suspension.  An officer who doesn't report to assigned callsputs the public in danger!"
					)
					((IsFlag 54)
			...
			(0
				(cond
					...
					((and (IsFlag 32) (== gDay 5))
						...
						(if (IsFlag 2)
							(Print 891 14) ; "You describe the vehicle as being a gold General Motors sedan with possible white paint transfers."
						else
							(Print 891 15) ; "You describe the vehicle as being a light red General Motors sedan with possible white paint transfers."
						)
						(Print 891 16) ; "You also advise that the occupants may be armed and dangerous and to approach with caution."
						(Print 891 17) ; "I've got it all," she says. "I'll put this out on the air immediately!"
						(SetFlag 33)
						(SetScore 162 5)
					)
					((and (IsFlag 54) (not (IsFlag 55)))
						...
						(SetFlag 6)
						(SetFlag 58)
						(Print 891 18) ; "Dispatch: "Sonny, we need you to drive out to Aspen Falls to respond to a call. All other units are occupied.""
						(= global112 111)
					)
					((and (IsFlag 207) (not (IsFlag 206)))
						(SetFlag 206)
						(Print 891 19) ; "Respond to an assault at the 300 block of East Rose,... the Oak Tree Mall."
					)
					(else
						(Print 891 20) ; "If you don't have a reason to call, don't."
						(phoneSfx stop:)
					)
				)
				(= cycles 1)
...
(instance phoneInset of View
	...
	(method (init param1)
		(= view (proc0_20 235 233 235 235 235))
...
(class myKeyPadBut of keyPadBut
	(properties)
