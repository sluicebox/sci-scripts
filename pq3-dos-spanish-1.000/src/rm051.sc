	...
	(method (init)
		...
		(if (!= gPrevRoomNum 52)
			...
			(if (> (gGame detailLevel:) 0)
				...
				(fire3 init: setCycle: Fwd)
			)
			(embers1 init: setCycle: Fwd)
			(embers2 init: setCycle: Fwd)
			(embers3 init: setCycle: Fwd)
			(embers4 init: setCycle: Fwd)
			(embers5 init: setCycle: Fwd)
...
(instance theRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== local0 0)
					(Print 51 29) ; "You see the fire gutted shell of a house still ablaze. It looks like the fire fighters have it under control."
				else
					(Print 51 28) ; "You see the fire gutted shell of a house."
				)
			)
...
(instance firetruck of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 51 30) ; "It's a Lytton Fire Department pumper."
			)
			(3 ; Do
				(Print 51 31) ; "The firemen have the fire well in hand and don't need your assistance."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theCar of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 51 32) ; "It's the trunk of your car."
			)
			(3 ; Do
				(cond
					((localproc_0)
						(Print 51 33) ; "You need to put the field equipment back into the trunk first."
					)
					((gEgo has: 6) ; photo
						(proc0_17 5)
						(SetFlag 202)
