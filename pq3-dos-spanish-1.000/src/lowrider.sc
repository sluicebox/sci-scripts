...
(instance lowrider of Rgn
	...
	(method (init)
		...
		(thePlate init:)
		(theCar approachVerbs: 5 4 init: setScript: lowRide) ; Talk, Inventory
		(lWindow init:)
		(rWheel init:)
		(fWheel init:)
...
(instance insetSuspect of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 886 19) ; "A mean looking dude ain't he."
			)
			(5 ; Talk
				(HandsOff)
				(cond
					...
					(local2
						(Print 886 20) ; "He doesn't look too pleased. Better get on with it."
						(HandsOn)
					)
					((gEgo has: 23) ; license
...
(instance theCar of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 886 21) ; "It's a lowered 58 Ford Fairlane. Nice flame job!"
			)
			(5 ; Talk
				(HandsOff)
				(cond
			...
			(3 ; Do
				(Print 886 22) ; "What are you going to do? Kick the tires?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance thePlate of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 886 23) ; "The plate reads - 22776"
			)
			(else
