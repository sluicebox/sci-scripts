		...
		(self
			...
			addObstacle:
				...
				((Polygon new:)
					...
					init: 215 81 132 56 26 98 223 161 308 111 242 76 263 61 213 37 246 0 234 0 191 39 257 62
					yourself:
				)
		)
		(super init:)
...
(instance locker of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 1) ; "That's Pat Morales' locker."
			)
			(3 ; Do
				(if (IsFlag 34)
					(HandsOff)
					...
					(Print 17 2) ; "You can't force open the locker. You'll need the combination."
				)
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 17 3) ; "That would only attract attention, don't you think."
					)
					(else
						(Print 17 4) ; "That won't help you open the locker."
					)
				)
			)
			(else
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(28 ; noteBook
						(SetScore 174 10)
						(Print 17 5) ; "You take notes on what you've found in Morales' locker."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance stallDoor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 6) ; "If you've seen one stall, you've seen them all."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance cocaine of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 7) ; "Unbelievable! It looks like Morales has been stealing packets of cocaine."
			)
			(3 ; Do
				(Print 17 8) ; "You'd better leave the cocaine here. If it were missing, Morales would get suspicious."
			)
			(4 ; Inventory
				(switch invItem
					(28 ; noteBook
						(SetScore 174 10)
						(Print 17 9) ; "You take notes on what you have found."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance theWomansRoom of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 10) ; "This is the women's locker room. Hence, the lockers."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance bench of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 11) ; "The benches provide a place to sit while getting dressed."
			)
			(3 ; Do
				(Print 17 12) ; "You need to get out of here quickly. You don't have time to lounge!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance stalls of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 17 13) ; "The toilet stalls lend a pleasant ambiance to the locker room."
			)
			(3 ; Do
				(Print 17 14) ; "It's bad enough that you're even in here!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance wrongLockers of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 17 15) ; "That's not Morales' locker!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
