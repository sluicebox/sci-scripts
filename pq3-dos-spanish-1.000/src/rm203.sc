...
(procedure (localproc_2 param1 param2 param3 param4 param5 &tmp [temp0 250])
	(if (u< param1 1000)
		(GetFarText param1 param2 @temp0)
	else
		(StrCpy @temp0 param1)
...
(instance rm203 of Rm
	...
	(method (init)
		...
		(reviewBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 51 11 48 48 48)
			init:
		)
		(newBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 144 92 151 151 151)
			init:
		)
		(numBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 202 209 204 204 204)
			init:
		)
		(gTheIconBar disable:)
		(gGame setCursor: 999)
		...
		(switch (= state newState)
			...
			(1
				(if local8
					...
					(localproc_0 504 local6 50 20 (LangSwitch 240 270) global125)
					...
					(localproc_0 504 (+ 6 local6) 10 55 215 global125)
					...
					(localproc_0 504 (+ 7 local6) 10 70 300 global125)
					...
					(localproc_0 504 (+ 10 local6) 10 145 300 global125)
					(localproc_0 ; "MOTIVE -"
						203
						14
						10
					...
					(localproc_0 504 (+ 11 local6) 10 170 250 global125)
				)
				(if (StrCmp @local1 {199137})
					(conBut
						x: (proc0_20 307 293 293 293 293)
						view: (proc0_20 1971 199 197 197 197)
						init:
					)
				)
		...
		(switch (= state newState)
			...
			(1
				(if local8
					...
					(localproc_0 504 (+ 6 local6) 10 55 300 global125)
					(localproc_0 ; "WEAPON USED -"
						203
						12
						10
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (proc0_20 110 102 110 110 110))
						(== (event message:) (proc0_20 78 70 78 78 78))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
					(self loop: 1 forceUpd:)
					(Animate (gCast elements:) 0)
					...
					(Animate (gCast elements:) 0)
					(if (IsFlag 87)
						(localproc_2 203 17 20 20 200) ; "New Case Number: 199146"
					else
						(localproc_2 203 18 20 20 200) ; "New Case Number: 199145"
						(Animate (gCast elements:) 0)
						(Wait 60)
						(if (IsFlag 80)
							(Print 203 19) ; "Based on the information you obtained at the murder scene, you open up a new file on the Dent murder."
			...
			(evMOUSEBUTTON
				(if (and (MousedOn self event) (not (gCurRoom script:)))
					...
					(if (IsFlag 87)
						...
						(localproc_2 203 20 20 20 200) ; "New Case Number: 199145"
						(Animate (gCast elements:) 0)
						(Wait 60)
						(if (IsFlag 80)
							(Print 203 19) ; "Based on the information you obtained at the murder scene, you open up a new file on the Dent murder."
		...
		(if (not (gCurRoom script:))
			(switch (event type:)
				(evKEYBOARD
					(if
						(or
							(== (event message:) (proc0_20 97 114 114 114 114))
							(== (event message:) (proc0_20 65 82 82 82 82))
							(and
				...
				(evKEYBOARD
					(if
						(or
							(==
								(event message:)
								(proc0_20 115 110 115 115 115)
							)
							(== (event message:) (proc0_20 83 78 83 83 83))
							(and
								(== (event message:) KEY_RETURN)
								(MousedOn self event)
							)
	...
	(method (changeState newState &tmp [temp0 20])
		(switch (= state newState)
			...
			(1
				...
				(StrCpy @local1 {})
				(self register: -1)
				(localproc_2 203 21 30 40 200) ; "Enter serial number:"
				(= cycles 1)
				(= local5 0)
			)
			(2
				(cond
					...
					((< register 10)
						...
						(localproc_3 (proc0_20 160 170 160 160 125) 40)
						(self register: -1)
						(= cycles (= state 1))
					)
				)
			...
			(5
				(cond
					...
					((not (StrCmp @local1 {98765}))
						(localproc_1)
						(mug2 init:)
						(localproc_2 203 22 100 20 70) ; "Steve Rocklin"
						(localproc_2 203 23 100 30 200) ; "M/W 6'2" 210 red/hzl"
						(localproc_2 203 24 100 40 200) ; "d.o.b. 1/10/50."
						(localproc_2 203 25 100 50 200) ; "Address Jan/89"
						(localproc_2 203 26 100 60 100) ; "214 Pigeon Ct."
						(localproc_2 203 27 100 70 100) ; "Gibbon U.S.A."
						(localproc_2 203 28 100 90 100) ; "SUMMARY"
						(localproc_2 ; "Convicted on two counts of burglary 6/23/85. Sentenced to 5 years in state penitentiary. Paroled 6/25/88 and is currently serving 3 years probation."
							203
							29
							(LangSwitch 10 5)
							110
					...
					(else
						(localproc_2 203 32 20 170 200) ; "No Match"
						(= cycles 1)
					)
				)
			)
...
(instance displayStar of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 203 33 10 20 300 global125) ; "U.S. Army Bronze Star"
				(localproc_0 203 34 10 30 300 global125) ; "Awarded to Samuel Britt, Infantry"
				(localproc_0 203 35 10 40 300 global125) ; "For meritorious Service during WWII."
				(= seconds 8)
			)
			(2
				(localproc_0 203 36 10 100 300 global125) ; "Samuel Britt murder victim"
				(localproc_0 203 37 10 110 300 global125) ; "See case number #199124"
				(SetScore 130 5)
				(self dispose:)
			)
		)
						...
						(or
							(and
								(or
									(==
										(event message:)
										(proc0_20 119 99 99 99 99)
									)
									(==
										(event message:)
										(proc0_20 87 67 67 67 67)
									)
								)
								(== local8 0)
							)
							(and
								(or
									(==
										(event message:)
										(proc0_20 122 112 112 112 112)
									)
									(==
										(event message:)
										(proc0_20 90 80 80 80 80)
									)
								)
								(== local8 1)
							)
