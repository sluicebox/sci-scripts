...
(instance rmMap of Rm
	...
	(method (init)
		...
		(quitBut
			...
			x: (proc0_20 307 252 289 289 289)
			init:
		)
		(pointBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 54 9 50 50 50)
			init:
		)
		(drawBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 167 107 156 156 156)
			init:
		)
		(cleanBut
			view: (proc0_20 1971 199 197 197 197)
			x: (proc0_20 238 207 237 237 237)
			init:
		)
		(gTheIconBar disable:)
	)
	...
	(method (handleEvent event &tmp [temp0 2] temp2)
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (proc0_20 101 7 113 113 113))
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (proc0_20 102 116 112 112 112))
						(== (event message:) (proc0_20 70 84 80 80 80))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
					(self loop: 1 forceUpd:)
					(Animate (gCast elements:) 0)
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (proc0_20 112 100 99 99 99))
						(== (event message:) (proc0_20 80 68 67 67 67))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (proc0_20 108 98 101 101 101))
						(== (event message:) (proc0_20 76 66 69 69 69))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
					(self loop: 1 forceUpd:)
					(Animate (gCast elements:) 0)
		...
		(switch (event type:)
			...
			(evMOUSEBUTTON
				(if
					...
					(if (== local56 19)
						(Print 202 6) ; "Come now. There aren't that many homicides you're working on Sonny."
					else
						((= [local36 local56] (Clone star)) init:)
						(++ local56)
					)
