...
(instance florist of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 37 27) ; "If you want something from the florist, you must pay for it."
			)
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(cond
							((gEgo has: 36) ; rose
								(Print 37 28) ; "You already have a rose."
							)
					...
					(2 ; handcuff
						(Print 37 29) ; "Selling flowers isn't a criminal offense!"
					)
					(1 ; gun
						(Print 37 29) ; "Selling flowers isn't a criminal offense!"
					)
					(36 ; rose
						(Print 37 30) ; "The florist already has PLENTY of roses."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance rose of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(cond
							((gEgo has: 36) ; rose
								(Print 37 31) ; "You already have a rose."
							)
							((== ((gInventory at: 36) state:) 0) ; rose
								(HandsOff)
								(gCurRoom setScript: takeRose)
			...
			(3 ; Do
				(Print 37 32) ; "If you want something from the florist, you must pay for it."
			)
			(else
...
(instance chitChat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 37 33) ; "Hi, Sargeant Bonds. Your wife's in room 307."
				(= cycles 1)
			)
			(2
				(HandsOn)
...
(instance receptionist of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 37 34) ; "If you want something from the receptionist, just ask her."
			)
			(5 ; Talk
				(HandsOff)
				(if (> gDay 3)
			...
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(if (IsFlag 195)
							(Print 37 35) ; "The receptionist stonily ignores you. You gather that she's used to ignoring people."
						else
							(Print 37 3) ; "May I help you, Sir?"
						)
					)
					(1 ; gun
						(Print 37 36) ; "That's right! Torture Marie's room number out of her!"
					)
					(2 ; handcuff
						(Print 37 36) ; "That's right! Torture Marie's room number out of her!"
					)
					(else
						(Print 37 37) ; "The receptionist isn't interested in that."
					)
				)
			)
			(else
...
(instance elevatorUpper of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 37 38) ; "Just press the button. Trying to pry those doors open with your hands will only give you a hernia."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance elevatorLower of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 37 39) ; "Just press the button. Trying to pry those doors open with your hands will only give you a hernia."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 37 41) ; "If you want something, why don't you ask the receptionist?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance frontDesk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 37 42) ; "If you want something, why don't you ask the receptionist?"
			)
			(else
				(super doVerb: theVerb)
			)
