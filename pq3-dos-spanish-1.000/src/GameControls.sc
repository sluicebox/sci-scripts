...
(class Slider of IconI
	...
	(method (move amount doSend &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
		(= temp6 (or (not argc) doSend))
		(= temp2 (* (sign amount) yStep))
		(for ((= temp4 (Abs amount))) (<= yStep temp4) ((-= temp4 yStep))
			(= sTop
				(cond
					((< (= temp0 (- sTop temp2)) minY) minY)
					((> temp0 maxY) maxY)
					(else temp0)
				)
			)
			...
			(Graph
				...
				(- sTop yStep)
				sLeft
				(+ sTop (CelHigh sliderView sliderLoop sliderCel) yStep)
				sRight
				1
			)
			(PicNotValid temp1)
