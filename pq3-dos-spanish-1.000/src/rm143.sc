...
(instance wall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 143 6) ; "An aisle runs down the middle of the spectator area to the council area."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance chairs of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 143 7) ; "Theater chairs line the spectator area."
			)
			(3 ; Do
				(if (== (gEgo view:) 460)
					(Print 143 8) ; "You're happy with the seat you're in."
				else
					(Print 143 9) ; "You'll need to sit up front at the prosecutor's table."
				)
			)
			(else
				(super doVerb: theVerb)
