		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((== local0 1)
						(Print 47 19) ; "You can't take a paint sample without the proper field equipment."
					)
					(else
						(Print 47 9) ; "You can't search the inside of the car without a warrant."
					)
			...
			(2 ; Look
				(cond
					...
					((!= local0 1)
						(= local0 1)
						(roomSFX2 number: 471 flags: 1 play:)
						(Print 47 20) ; "You look at the parked car and notice that the license plates are missing. The car matches the description of the suspect vehicle."
						(Print 47 21) ; "Giving the car a closer look, you see what appears to be a fresh white paint transfer."
						(Print 47 12) ; "Bingo", you mutter, "I got you now, you S.O.B..."
					)
					(else
						(Print 47 13) ; "You inspect the suspect car once more, and find no evidence other than the paint transfer."
					...
					(switch invItem
						...
						(42 ; sampleEnv
							(if (gEgo has: 30) ; whitePaint
								...
								(Print 47 22) ; "You need to get the paint sample before you can store it in an envelope."
							)
						)
						(else
							(super doVerb: theVerb invItem &rest)
		...
		(switch theVerb
			(4 ; Inventory
				(if (trunk showing:)
					...
					(switch invItem
						(39 ; scraper
							(Print 47 23) ; "Rubber's just rubber. If you want some there's easier ways to get it than scraping it off the street."
						)
					)
				)
			)
		...
		(switch theVerb
			(3 ; Do
				(if (== local0 1)
					...
					(Print 47 24) ; "There is no apparent need for anything in the trunk right now."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance carDoor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 25) ; "Convinced that the serial killer will strike here next, you decide to stick around."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance nugget of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 26) ; "Just use the front door."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance mug of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 27) ; "If you want a drink, you'd have better luck inside."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
