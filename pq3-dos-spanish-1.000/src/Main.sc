...
(procedure (Say param1 &tmp [temp0 4] temp4 [temp5 250])
	(if gModelessDialog
...
(procedure (proc0_21)
	(= global198 (gGame parseLang:))
	(gGame parseLang: 1)
)

(procedure (proc0_22)
	(gGame parseLang: global198)
)
...
(instance pq3 of Game
	...
	(method (quitGame)
		(super quitGame: (Print 0 2 #button {QUIT} 1 #button {PLAY} 0)) ; "Going off duty?"
	)
		...
		(switch theVerb
			(2 ; Look
				(Printf ; "You have %s"
					...
					(cond
						((& state $0002) {las llaves del loco.})
						((& state $0008)
							(cond
								((& state $0001) {las llaves de Pat y la copia de la llave.})
								((& state $0004) {las llaves de Rocklin y la copia de la llave.})
								((& state $0008) {la copia de la llave.})
							)
						)
						((& state $0001) {las llaves de Pat})
						((& state $0004) {las llaves de Rocklin})
					)
				)
			)
			(else
...
(instance icon6 of PQ3IconItem
	...
	(method (show &tmp [temp0 15] [temp15 4] temp19)
		(= loop (proc0_20 8 11 8 8 8))
		(= maskLoop (proc0_20 8 11 8 8 8))
		(super show: &rest)
		(= temp19 30)
		(Format @temp0 0 26 gScore gPossibleScore) ; "%d of %d"
		(TextSize @temp15 @temp0 temp19 0)
		(Display
			@temp0
			dsCOLOR
			global137
