		...
		(switch theVerb
			...
			(3 ; Do
				(if (== local9 1)
					...
					(Print 48 24) ; "If you want something from the man why don't you just ask."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance sTalkToPlayer of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 25) ; "Is that your car out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say poolPlayer 48 26) ; "*Sniff* Anybody else in here smell bacon?"
				(= seconds 3)
			)
			(2
				(HandsOn)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 48 27) ; "Why on earth would you want to do that?!"
					)
				)
			)
			(5 ; Talk
				(switch (++ local7)
					...
					(else
						(Print 48 28) ; "He seems harmless enough. You decide to leave him alone."
					)
				)
			)
			(3 ; Do
				(Print 48 29) ; "The guy hasn't done anything wrong."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sPatron1Talks of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 30) ; "Excuse me, Sir. is that your car parked right out front?"
				(= seconds 5)
			)
			(1
				(= save1 1)
				(Say patron1 48 31) ; "Naw! My car's been in the shop all week. Why?"
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 32) ; "Nothing. thanks."
				(= seconds 2)
			)
			(3
				(HandsOn)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 48 33) ; "Why on earth would you want to do that?!"
					)
				)
			)
			(5 ; Talk
				(switch (++ local8)
					...
					(else
						(Print 48 34) ; "The guy's busy drowning his sorrows and doesn't pay any attention to you."
					)
				)
			)
			(3 ; Do
				(Print 48 35) ; "He hasn't done anything wrong. Leave him alone."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sPatron2Talks of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 36) ; "Excuse me sir, is that your car parked right out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say patron2 48 37) ; "Not me. I drive a Harley man!"
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 38) ; "Sorry to bother you."
				(= seconds 4)
			)
			(3
				(HandsOn)
...
(instance poolTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 39) ; "Touching the pool table while someone is playing isn't a good idea."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boose of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 40) ; "If you want a drink just order one."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tables of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 41) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poolCues of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 42) ; "Ahh... if you only had time!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tvFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 43) ; "You don't have time to watch television."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance candles of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 44) ; "You wouldn't take one of those tacky candles if somebody paid you."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance ashtrays of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 45) ; "No thanks, you don't smoke."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
