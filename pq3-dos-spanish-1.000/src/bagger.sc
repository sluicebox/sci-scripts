...
(instance lunch of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 876 28) ; "There's a brown bag sitting on the desk. It's probably someone's lunch."
			)
			(3 ; Do
				(if (gCast contains: bagLady)
					(gCurRoom setScript: getLunch)
				else
					(Print 876 29) ; "You have no need for that now."
				)
			)
			(else
				(super doVerb: theVerb)
