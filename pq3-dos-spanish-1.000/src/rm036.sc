...
(instance doctor of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 36 49 #at 40 115) ; "One moment, Sir."
			)
			(3 ; Do
				(Print 36 50) ; "That won't help Marie come back. They're doing all they can."
			)
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(Print 36 51) ; "You've already introduced yourself to the hospital staff."
					)
					(1 ; gun
						(Print 36 50) ; "That won't help Marie come back. They're doing all they can."
					)
		...
		(switch theVerb
			(3 ; Do
				(if (> gDay 1)
					...
					(Print 36 52) ; "Marie is resting gently. You don't want to disturb her."
				)
			)
			(5 ; Talk
				(localproc_3)
				...
				(switch invItem
					...
					(36 ; rose
						(if (not (> ((gInventory at: 36) state:) 0)) ; rose
							...
							(Print 36 53) ; "You gently place a fresh rose in Marie's hand.  However, there is no response. Nice try Sonny."
							(gEgo put: 36) ; rose
						)
					)
					(37 ; locket
					...
					(1 ; gun
						(Print 36 54) ; "Marie's condition might yet be reversible. Don't give up without a fight."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance ivStand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 55) ; "You can't change the medication by yourself."
			)
			(2 ; Look
				(HandsOff)
...
(instance hospitalWindow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 56) ; "The room temperature is monitored carefully by the staff.  You don't need to open the window."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bed of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (>= (mariesEyes cel:) 1)
					(Print 36 57 #at 40 115) ; "Marie lies awake in the bed."
				else
					(Print 36 58 #at 40 115) ; "Marie lies motionless on the hospital bed."
				)
			)
			(3 ; Do
				(Print 36 59) ; "The hospital bed does not need adjustment."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance ekgMonitor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (>= (mariesEyes cel:) 1)
					(Print 36 60) ; "Words cannot describe the joy you feel seeing Marie awake from her coma."
				else
					(Print 36 61 #at 40 115) ; "The EKG monitor registers your wife's vital signs.  Its steady beep in response to her pulse gives you some small comfort."
				)
			)
			(3 ; Do
				(if (>= (mariesEyes cel:) 1)
					(Print 36 62) ; "The Monitor's beep reminds you that although Marie is awake, she is not out of the woods yet."
				else
					(Print 36 63) ; "Leave the monitor alone. It will alert the staff if Marie goes into distress."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance metalStand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 64) ; "You do not need the metal stand."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cabinet of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 65) ; "There's nothing in the cabinets that would be of use to you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(switch theVerb
			(3 ; Do
				(if
					...
					(Print 36 66) ; "You're too upset by the sight of Marie to even think about sitting down."
				else
					(Print 36 67) ; "You don't feel like sitting right now."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance curtains of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 68) ; "There's no reason to draw the curtains right now."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(switch theVerb
			...
			(5 ; Talk
				(cond
					...
					(else
						(Print 36 69 #at 40 115) ; "You talk gently to your wife, hoping that some part of her hears you."
					)
				)
			)
			(2 ; Look
				(if (>= (mariesEyes cel:) 1)
					(Print 36 70 #at 40 115) ; "You are overjoyed to see your wife recovered."
				else
					(Print 36 71 #at 40 115) ; "Your beloved wife lies motionless in a coma. You wish there was some way to know what she's thinking... if she's thinking."
				)
			)
			(4 ; Inventory
				(switch invItem
					...
					(36 ; rose
						(if (not (> ((gInventory at: 36) state:) 0)) ; rose
							...
							(Print 36 72 #at 40 115) ; "You gently place a fresh rose in Marie's hand.  However, there is no response."
							(gEgo put: 36) ; rose
						)
					)
					(37 ; locket
					...
					(19 ; wallet
						(Print 36 73 #at 40 115) ; "Marie's proud of your career as a police officer, but she's not interested in your badge right now."
					)
					(1 ; gun
						(Print 36 74 #at 40 115) ; "Marie's condition might yet be reversable. Don't give up without a fight."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance ekg of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (>= (mariesEyes cel:) 1)
					(Print 36 75 #at 40 115) ; "Words cannot describe the joy you feel seeing Marie awake from her coma."
				else
					(Print 36 76 #at 40 115) ; "The EKG monitor registers your wife's vital signs.  Its steady beep in response to her pulse gives you some small comfort."
				)
			)
			(3 ; Do
				(if (>= (mariesEyes cel:) 1)
					(Print 36 77) ; "You shouldn't mess with things you know nothing about."
				else
					(Print 36 78) ; "Leave the monitor alone. It will alert the staff if Marie goes into distress."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance light of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 79) ; "There's no reason to turn off the light."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance nurseButton of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(if (IsFlag 41)
					(Print 36 80) ; "You don't need the nurse now."
				else
					(Print 36 81 #at 40 115) ; "You press the call button and hope that the nurse is on the way."
...
(instance roseInset of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 82) ; "Let Marie keep her rose."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance musicboxInset of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 83) ; "Let Marie keep her music box."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance locketInset of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 36 84) ; "Let Marie keep her locket."
			)
			(else
				(super doVerb: theVerb &rest)
			)
