					...
					(4 ; flashlight
						(cond
							...
							((== ((gInventory at: 4) state:) 1) ; flashlight
								(gEgo view: 380)
								(spot init:)
								(Print 30 27) ; "You turn your flashlight on and begin a search of the scene."
							)
							(else
								(Print 30 21) ; "Your batteries appear to be dead."
							)
...
(instance otherStores of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 28) ; "Some of the mall's stores are undergoing renovation."
				else
					(Print 30 25) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 29) ; "That store is closed."
			)
			(4 ; Inventory
				(switch invItem
					(4 ; flashlight
...
(instance sidewalk of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 30) ; "Sidewalks connect the store fronts."
				else
					(Print 30 25) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(4 ; Inventory
				(switch invItem
					(4 ; flashlight
						(cond
							((== (gEgo view:) 380)
								(Print 30 31) ; "The flashlight is already on."
							)
							((== ((gInventory at: 4) state:) 1) ; flashlight
								(gEgo view: 380)
								(spot init:)
...
(instance armyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 32) ; "That's the Army Recruiter's office."
				else
					(Print 30 25) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 33) ; "The Army Recruiter's office is closed for the night."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance keyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 34) ; "That's the shop of "Zak" the keymaker."
				else
					(Print 30 25) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 35) ; "Zak's key shop is closed for the night."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance mariesCar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 30 36) ; "Marie's Trans Am still sits in the poorly lit parking lot of the mall. You scan the car for clues... but find nothing."
			)
			(else
				(super doVerb: theVerb)
			)
