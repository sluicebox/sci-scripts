...
(instance rm046 of PQRoom
	...
	(method (init)
		(LoadMany rsVIEW 495 (proc0_20 496 498 496 496 496))
...
(instance id of View
	...
	(method (init)
		(= view (proc0_20 496 498 496 496 496))
...
(instance lowerbody of Feature
	(properties
		onMeCheck 16384
		lookStr {You examine the body. By its skin color and state of rigor mortis, you'd guess that the victim's been dead for at least a few hours.}
	)
		...
		(switch theVerb
			(3 ; Do
				(if
					...
					(Print 46 12) ; "You carefully search the body and remove the victim's identification."
					(id init:)
					(gEgo get: 23) ; license
					...
					(Print 46 13) ; "There's nothing more in the victim's pockets."
				)
			)
			(1 ; Walk
				(OutSide doVerb: theVerb)
...
(instance upperbody of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 46 14) ; "You examine the knife wounds in the upper chest.  Leon will be able to tell you more about the wounds in the Coroner's report."
			)
			(2 ; Look
				(Print 46 15) ; "You look carefully at the knife wounds in the upper chest. Not a pretty sight. But it's the pentagram carved into the flesh that really hits you between the eyes."
			)
			(1 ; Walk
				(OutSide doVerb: theVerb)
			)
			...
			(4 ; Inventory
				(if (== invItem 40) ; toothpick
					(cond
						...
						(else
							(Print 46 16) ; "That's a good idea, but you'll need to use the proper field equipment."
						)
					)
				)
			)
...
(instance face of Feature
	(properties
		onMeCheck 4096
		lookStr {His face looks anything but peaceful.}
	)
...
(instance dumpster of Feature
	(properties
		onMeCheck 4
		lookStr {What an awful smelly place to die.}
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 46 19) ; "You have plenty of trash at home."
			)
			(1 ; Walk
				(OutSide doVerb: theVerb)
			)
