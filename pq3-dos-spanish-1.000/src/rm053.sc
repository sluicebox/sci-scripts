...
(instance rm053 of PQRoom
	...
	(method (init)
		...
		(walls init:)
		(Blood init:)
		(glow init: setCycle: Fwd self)
		(glow2 init: setCycle: Fwd self)
		(drip init: setCycle: Fwd self)
		(backwalldrip init: setCycle: Fwd self)
		(pipe init:)
		(candles init:)
		(floor init:)
...
(instance coals of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print 53 10) ; "You don't want to touch that. Some of this stuff is still hot."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance pipe of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 53 11) ; "The pipe doesn't interest you."
			)
			(2 ; Look
				(Print 53 12) ; "Exposed water pipes and charred studs are all that remain of some of the interior walls."
			)
			(else
				(super doVerb: theVerb &rest)
			)
