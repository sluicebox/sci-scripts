	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				...
				(button1 view: (proc0_20 121 1210 121 121 121) init:)
				(button2 view: (proc0_20 121 1210 121 121 121) init:)
				(button3 view: (proc0_20 121 1210 121 121 121) init:)
				(buttonG view: (proc0_20 121 1210 121 121 121) init:)
				(plate view: (proc0_20 121 1210 121 121 121) init:)
			)
			(8
				(if (localproc_0 [local46 local62] [local50 local62])
					(= cycles 1)
				else
					(gEgo
						setMotion:
...
(instance elevBut of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 894 1) ; "That's the elevator button."
			)
			(3 ; Do
				(cond
					((and (IsObject gTrunk) (gTrunk showing:)) 0)
					((IsFlag 16)
						(Print 894 2) ; "Prisoners are not allowed upstairs."
					)
					((IsFlag 5)
						(Print 894 3) ; "Prisoners aren't allowed upstairs - especially half-naked ones!"
					)
					((& (gEgo onControl: 0) $2000)
						(Print 894 4) ; "You already have the elevator's complete attention."
					)
					(else
						(HandsOff)
			...
			(4 ; Inventory
				(Print 894 5) ; "Its amazing how buttons - like that one - are so ideally suited to the human HAND."
			)
			(else
...
(instance elevDoor of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 894 6) ; "The elevator provides access to the other floors of the police station."
			)
			(3 ; Do
				(Print 894 7) ; "Just press the button. Trying to pry those doors open with your hands will only give you a hernia."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance elevDoor2 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 894 8) ; "That's the elevator door."
			)
			(3 ; Do
				(Print 894 7) ; "Just press the button. Trying to pry those doors open with your hands will only give you a hernia."
			)
