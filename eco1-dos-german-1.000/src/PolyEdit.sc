	...
	(method (setCur node displayCursor)
		(if (= curNode node)
			...
			(if (or (< argc 2) displayCursor)
				(SetCursor gNormalCursor 1)
				(MoveCursor (curPt x:) (curPt y:))
			)
		)
	)
			...
			(evKEYBOARD
				...
				(switch (event message:)
					...
					(KEY_h
						(switch state
							(0
								(DoPrint 943 3 30 1) ; "CREATING POLYGON  Click to create each corner of the polygon, then choose Done from the menu to finish. You can also press Esc or Ctrl-click to finish.  To UNDO a corner, choose Undo.  To change MAP displayed (visual or control), choose Map.  To EXIT the Polygon Editor, choose eXit or press Ctrl-S."
							)
							(1
					...
					(KEY_a
						(DoPrint 943 5 30 1) ; "by  Mark Wilden  Original program by Chad Bye"
					)
					(KEY_r
						(if (== state 1)
							(self draw:)
