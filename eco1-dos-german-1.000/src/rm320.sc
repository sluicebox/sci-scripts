...
(instance rm320 of EcoRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 7) ; "This isn't the place to dive."
			)
			(else
	...
	(method (notify)
		(if (== gPrevRoomNum south)
			((ScriptID 2 0) init: 10 1) ; Delphineus, "You lead the way for a while, Adam, I'm still trying to get my sea legs back! Haha!"
		)
	)
)
...
(instance easternCliffs of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 4) ; "Adam doesn't want to get out of the water."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sky of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 6) ; "Adam doesn't have wings."
			)
			(else
				(super doVerb: theVerb &rest)
			)
