...
(instance sGetBra of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 435 0) ; "You take the double-barreled brassiere from the workbench. It smells strongly of fresh gunpowder."
				(= seconds 3)
			)
			(4
...
(instance door of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 2) ; "You decide that it wouldn't be a very good idea to try to open your new boss' private office--especially when he's locked the door."
			)
			(5 ; Talk
...
(instance dartboard of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 5) ; "It is easy to see that F.B.I. employees don't always score!"
			)
			(3 ; Do
...
(instance northBank of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 8) ; "You really shouldn't mess around with the F.B.I.'s equipment."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance monitors of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 9) ; "You try to turn on the monitor, but are unable to do so."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance plug of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 10) ; "What? And get electrocuted?!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance outlet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 11) ; "You attempt to communicate through the intercom, but you find you're suffering from insufficient clearance."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 12) ; "Don't move it. You don't need anything that's past the chair."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance joystick of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 13) ; "You briefly picture yourself playing "Zeliard.""
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance circuitboard of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 435 14) ; "Be careful. You'll get shocked!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
