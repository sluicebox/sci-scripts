	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(10 ; Zipper
				...
				(TPrint 880 9 #at -1 185) ; "(Although he IS kind of cute!)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				...
				(TPrint 880 12 #at -1 185) ; "(Especially one that must weigh a ton!)"
			)
			(10 ; Zipper
				(TPrint 880 13) ; "Patti!"
				(TPrint 880 14 #at -1 185) ; "(Besides, wouldn't that steel be cold!?)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
