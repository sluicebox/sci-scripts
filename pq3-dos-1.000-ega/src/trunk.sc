...
(class trunk of Prop
	(properties
		lookStr {The trunk contains items you may need in the field.}
		view 205
		priority 15
		signal 16401
		cycleSpeed 0
...
(class Case of Prop
	(properties
		lookStr {The briefcase contains tools and supplies for evidence collection.}
		view 205
		cel 1
		priority 15
		signal 16
...
(instance thetoothpicks of View
	(properties
		lookStr {The toothpicks are useful for removing evidence from small places.}
		view 205
		cel 2
		priority 15
		signal 16
...
(instance thebags of View
	(properties
		lookStr {The envelopes are used for protecting and storing collected evidence.}
		view 205
		cel 3
		priority 15
		signal 16
...
(instance thecamera of View
	(properties
		lookStr {The camera is used to visually record a crime scene or a piece of evidence.}
		view 205
		cel 4
		priority 15
		signal 16
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 881 0) ; "You don't need the camera."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance thescraper of View
	(properties
		lookStr {The scraping tool is used to scrape dried evidence such as blood or paint off of other surfaces.}
		view 205
		cel 5
		priority 15
		signal 16
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(cond
					((== (self loop:) 0)
						(Print 881 1) ; "You have a full flare box."
					)
					((== (self loop:) 1)
						(Print 881 2) ; "There are no flares in the flare box."
					)
					(else
						(Print 881 3) ; "OOPS we shouldn't have gotten here with the flares."
					)
				)
			)
			(3 ; Do
				(if (trunk marked:)
					(Print 881 4) ; "You have no use for the flares right now."
				else
					(gEgo get: 15) ; flares
					(self loop: 1 cel: 1)
				)
			...
			(4 ; Inventory
				(switch invItem
					(15 ; flares
						(if (trunk marked:)
							(Print 881 5) ; "Your flare box is already full!"
						else
...
(instance caseLid of View
	(properties
		lookStr {There's a metal briefcase in the trunk.}
		view 205
		cel 7
		priority 15
		signal 16
