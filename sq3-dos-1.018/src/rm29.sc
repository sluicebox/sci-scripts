			...
			(evSAID
				(cond
					((Said 'look>')
						(cond
							((or (Said '/pane') (Said '<out'))
								(Print 29 1) ; "Outside, the cold vacuum of space pulls at the fragile steel membrane that surrounds you.  Apprehension grips you as you realize your insignificance in the vastness of the universe.  The whirling cosmic display seems to... you know, the usual science fiction kinda stuff."
							)
							...
							(
								(Said
									'/alien,being,man,bystander,folk,animal,customer'
								)
							...
							(
								(or
									(Said
										'/astro,astro,grafitti,game,device,comp[<grafitti]'
									)
									(Said '/astro<astro')
								)
							...
							((Said '[<around,at,in][/area,cafe]')
								(Print 29 15) ; "The decor, like the food, is the same in Monolith Burgers all over the universe. Generic counter clerks are eagerly waiting to help you. Diverse life-forms are crowded around the counter and sitting in booths consuming what can only loosely be termed food."
							)
						)
					)
					...
					(
						(or
							(Said
								'play/astro,astro,grafitti,game,device[<grafitti]'
							)
					...
					((Said 'hit,tilt/grafitti,game,device[<grafitti]')
						(if (gEgo inRect: 158 94 200 124)
							(Print 29 17) ; "The sophisticated electronics resist your feeble tamperings."
						else
							(Print 29 12) ; "You'll have to get a little closer."
						)
					)
					(
						(or
							(Said
								'order,buy,get,ask[/dinner,dinner,burger,drink]'
							)
							(Said 'talk/clerk')
						)
						(cond
							((not (gEgo inRect: 282 125 319 135))
								(Print 29 18) ; "If you walk over to the clerk, you won't have to shout across the room."
							)
							((gEgo has: 17) ; Bag_of_Fast_Food
								(Print 29 19) ; "Why not eat what you already have before ordering more?"
							)
							((not global248)
								(gCurRoom newRoom: 25)
							)
					...
					(
						...
						(Print 29 20) ; "The customers have better things to do than talk to a small-brained biped from a hick planet."
					)
					((Said 'talk')
						(Print 29 21) ; "You mumble to yourself. The other customers eye you suspiciously."
					)
					(
						(Said
							'kiss/alien,being,man,bystander,folk,animal,customer'
						)
						(Print 29 22) ; "Inter-species relations are frowned upon around here.  You could get yourself killed for trying something like that."
					)
					(
						(Said
							'smell[/man,alien,being,him,bystander,animal,customer]'
						)
						(Print 29 23) ; "The only thing you can smell is the aroma of home-cooked food. Obviously a piped-in smell to mask the odor of the real food."
					)
					((Said 'down,sit[<down]')
						(cond
							((== local6 0)
								(Print 29 24) ; "You're already sitting."
							)
							((gEgo inRect: 173 123 232 156)
								(gCurRoom setScript: SitDown)
							)
							(else
								(Print 29 25) ; "You'll have to get closer to an empty booth first."
							)
						)
					)
					((Said 'up,(get<up),stand[/up]')
						(if (== local6 1)
							(Print 29 26) ; "You're already standing."
						else
					...
					((Said 'eat[<dinner]')
						(cond
							((!= local6 0)
								(Print 29 27) ; "You have to sit down to eat."
							)
							((gEgo has: 17) ; Bag_of_Fast_Food
								(gCurRoom setScript: EgoEating)
							)
							(else
								(Print 29 28) ; "You don't have any food."
							)
						)
					)
					((Said 'get[<up]/bag')
						(cond
							((gEgo has: 17) ; Bag_of_Fast_Food
								(Print 29 29) ; "You already have it."
							)
							((== local7 0)
								(Print 29 30) ; "Nah, let Mr. Employee of the Week clean it up."
							)
							(else
								(Print 29 31) ; "There's no bag around here."
							)
						)
					)
					(
						...
						(cond
							((gEgo has: 17) ; Bag_of_Fast_Food
								(Print 29 32) ; "It's not trash yet."
							)
							((== local7 0)
								(Print 29 30) ; "Nah, let Mr. Employee of the Week clean it up."
							)
							(else
								(Print 29 33) ; "There's no trash around here."
							)
						)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(balloon dispose:)
				(bag dispose:)
				(Print 29 34) ; "You gingerly pick up the greasy bag. You can hardly wait to have a seat and dig in!"
...
(instance EgoEating of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(33
				(Print 29 35) ; "OWWW!!! Hey, what's this in my burger? Oh, it must be my Fun Meal prize! Hey, it's a swell decoder ring!"
			...
			(40
				(Print 29 36) ; "Mmmmmm, that was mighty tasty. Well, maybe mildly tasty. Well, maybe not tasty at all. In fact, it reminded you of the slick skin of a Vorlian mucusworm."
				(gEgo setCycle: 0)
				(User canInput: 1)
				(= global159 0)
				(= global249 0)
...
(instance balloon of View
	...
	(method (init)
		(super init:)
		(self view: 59 setLoop: 5 setCel: 0 posn: 297 85 ignoreActors: 1)
	)
)
...
(instance bully of Act
	...
	(method (init)
		(super init:)
		(self view: 57 loop: 0 posn: 122 89 ignoreActors: 1 setStep: 3 2)
	)
)
...
(instance pow of Prop
	...
	(method (init)
		(super init:)
		(self view: 57 setLoop: 3 setCel: 0 posn: 145 81 setPri: 15)
	)
)
