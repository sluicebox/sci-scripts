		...
		(if (== (event type:) evSAID)
			(cond
				...
				((Said 'disembark,disembark/')
					(switch
						(= local0 (Print 81 14 #button {Ship} 1 #button {ScumSoft} 2)) ; "Where to Buddy?"
						(1
							(gCurRoom newRoom: 80)
						)
						(2
...
(instance Actions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 81 15) ; "You make your way through the forest of strange trees to this clearing where you discover the entrance to some large underground complex. This must be ScumSoft. Suddenly the door to the complex begins to open."
				(= global129 1)
				(door
					illegalBits: 0
					ignoreActors:
