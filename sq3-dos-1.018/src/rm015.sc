...
(instance rm015 of Rm
	...
	(method (handleEvent event)
		...
		(cond
					...
					((or (Said '/hal<cable') (Said '/cable<hal'))
						(if (gEgo inRect: 0 125 38 155)
							(if (IsItemAt 3) ; Reactor
								...
								(Print 15 28) ; "They would not be helpful."
							)
						else
							(Print 15 28) ; "They would not be helpful."
						)
					)
					((Said '/artifact,support,cover')
						(Print 15 29) ; "This stuff has no value to you."
					)
					((Said '/generator')
						(if (IsItemAt 3) ; Reactor
							(if (gEgo inRect: 0 125 38 155)
								(gGame changeScore: 15)
								(Print 15 30) ; "You unhook the reactor from the cheap wires and take it with you."
								(reactorScript init:)
							else
								(Print 15 31) ; "You aren't near it."
							)
						else
							(Print 15 32) ; "There isn't one here to get."
						)
					)
					((Said '/cable')
						(if (IsItemAt 1) ; Wire
			...
			((Said 'climb,crawl[<through,up,on]/partition,cavity,system')
