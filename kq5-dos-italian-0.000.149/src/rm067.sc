...
(instance hole of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 67 22) ; "A fallen stone has revealed a large hole in the cell wall."
					(event claimed: 1)
				)
				(3 ; Do
					(proc550_17)
...
(instance dieScumScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(PrintDC 67 23) ; "After observing no discernible escape route from the small cell, Graham sinks to the floor in despair, knowing he will never see the light of day again."
				(= cycles 1)
			)
			(3
				((gEgo head:) hide:)
...
(instance grate of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 67 24) ; "Above him, Graham can see a damp, rusty grate leading to...who knows where?"
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 67 25) ; "The grate is too high up.  Graham can do nothing with it."
					(event claimed: 1)
				)
			)
		)
