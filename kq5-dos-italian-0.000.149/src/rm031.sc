...
(instance log of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 6) ; "An old log has fallen precariously across a washed-out portion of the narrow trail."
					(event claimed: 1)
				)
			)
		)
...
(instance waterFall of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 7) ; "As with the path below, the now-frozen waterfall has also washed away parts of this upper trail which skirts the mountain edge and then heads off to the east."
					(event claimed: 1)
				)
			)
		)
...
(instance rope of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 8) ; "Graham's rope dangles from a rocky overhang down to the path below."
					(event claimed: 1)
				)
				(3 ; Do
					(if (== local0 0)
						...
						(PrintDC 31 9) ; "Graham can't reach the rope from here."
					)
					(event claimed: 1)
				)
			)
