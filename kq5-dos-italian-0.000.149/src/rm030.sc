...
(local
	...
	[local8 18] = [0 0 319 0 319 167 180 167 135 153 112 159 75 159 52 164 0 163]
	local26
	local27
	local28
)
...
(instance rm030 of KQ5Room
	...
	(method (doit &tmp temp0)
		(cond
			...
			((and (not (IsFlag 15)) (> (gEgo x:) 60))
				(HandsOff)
				(self setScript: freeze)
			)
			((and (not local26) (not (IsFlag 15)) (> (gEgo x:) 20))
				(++ local26)
			...
			((and (not (IsFlag 14)) (not (IsFlag 16)) (> (gEgo x:) 80))
				(SetFlag 14)
			...
			((& (gEgo onControl: 0) $2000)
				(HandsOff)
				(proc0_28 75 30 2 67 10 10 25 5) ; "Graham! Don't get too close...!"
...
(instance ropeOnBranch of Script
	...
	(method (doit)
		(super doit:)
		(if (and (not (IsFlag 15)) (> (gEgo x:) 45))
			(HandsOff)
			(client setScript: 0)
...
(instance ropeOnRock of Script
	...
	(method (doit)
		(super doit:)
		(if (and (not (IsFlag 15)) (> (gEgo x:) 60))
			(HandsOff)
			(client setScript: 0)
			(gCurRoom setScript: freeze)
		)
		(if (and (not (IsFlag 14)) (not (IsFlag 16)) (> (gEgo x:) 80))
			(SetFlag 14)
			(PrintDC 30 5) ; "Graham notices his stomach beginning to rumble with hunger from the exertion of the mountain climb."
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if (not (IsFlag 88))
					(proc0_28 75 30 6 67 10 10 25 5) ; "Oh...be careful, Graham!"
				)
				(if (== ((gInventory at: 20) owner:) 31) ; Rope
					(branch setCycle: End)
				)
...
(instance path30 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 30 7) ; "A frozen waterfall hinders Graham's progress as in warmer seasons it has washed away part of the path on which he is travelling."
					(event claimed: 1)
				)
			)
		)
...
(instance cliff of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 30 8) ; "Looking upwards, Graham can see a snowy ledge above the icy cliff next to him."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 30 9) ; "The icy cliff is much too steep for Graham to climb without help."
					(event claimed: 1)
				)
			)
		)
...
(instance ledge of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 30 10) ; "Graham sees another possible route through the mountains from an upper ledge."
					(PrintDC 30 11) ; "If only he could reach it...!"
					(event claimed: 1)
				)
			)
		)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (== ((gInventory at: 20) owner:) 30) ; Rope
						(PrintDC 30 12) ; "Graham notices a rock overhang near an upper ledge. The rope hangs from it."
					else
						(PrintDC 30 13) ; "Graham notices a rock overhang near an upper ledge."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 30 14) ; "The rocky overhang catches Graham's interest..."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (== ((gInventory at: 20) owner:) 31) ; Rope
						(PrintDC 30 15) ; "The remains of an old tree poke out of the mountainside near an upper ledge. Graham's rope dangles from it."
					else
						(PrintDC 30 16) ; "The remains of an old tree poke out of the mountainside near an upper ledge."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 30 17) ; "The tree branch looks interesting..."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (== ((gInventory at: 20) owner:) 30) ; Rope
						(PrintDC 30 18) ; "Graham's rope hangs dangerously from a rocky overhang."
					else
						(PrintDC 30 19) ; "The rope dangles precariously from a protruding tree branch."
					)
					(event claimed: 1)
				)
				(3 ; Do
