...
(instance cliffs of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 29 6) ; "Sheer, vertical cliffs plummet dizzyingly downward from the edge of the frozen path.#Un dirupo cade a strapiombo e vertiginosamente verso il basso dal bordo del sentiero ghiacciato."
					(event claimed: 1)
				)
			)
		)
