			...
			(switch (event message:)
				(2 ; Look
					(cond
						...
						((!= ((gInventory at: 2) owner:) 36) ; Pie
							(PrintDC 35 8) ; "The large grey wolf stubbornly blocks Graham's passage back to the Ice Queen's palace. Graham's only option is to go toward a distant cave...where the dreaded yeti can be found!"
						)
						(else
							(PrintDC 35 9) ; "The wolf waits anxiously for Graham, having observed the yeti's demise."
						)
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (!= ((gInventory at: 2) owner:) 36) ; Pie
						(PrintDC 35 10) ; "As the huge wolf keeps a sharp eye on Graham there is little he can do to escape his unfair sentence."
						(event claimed: 1)
					)
				)
				(5 ; Talk
					(cond
						((!= ((gInventory at: 2) owner:) 36) ; Pie
							(if (IsFlag 27)
								(PrintDC 35 11) ; "As the wolf looks very angry at Graham's hesitation, Graham wisely decides to hold his tongue."
							else
								(SetFlag 27)
								(HandsOff)
						...
						((IsFlag 57)
							(PrintDC 35 12) ; "Graham's duties done, the large grey wolf sits in silence blocking the passage back to the Ice Queen's palace."
						)
						(else
							(HandsOff)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 35 13) ; "That won't change the wolf's mind."
							(event claimed: 1)
						)
					)
				)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (IsFlag 57)
						(PrintDC 35 14) ; "Off in the distance, Graham can see the crystal cave."
					else
						(PrintDC 35 15) ; "Off in the distance, a large cave can be seen. Graham can only assume it to be the Ice Queen's precious crystal cave."
					)
					(event claimed: 1)
				)
				(3 ; Do
...
(instance path35 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 35 16) ; "A dangerous, snowy path skirts the edge of a vertical mountainside toward a cave which can be seen in the distance."
					(event claimed: 1)
				)
			)
		)
