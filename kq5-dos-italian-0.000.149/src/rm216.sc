...
(instance killEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gGlobalSound number: 129 loop: 1 play:)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(PrintDC 216 11) ; "Disturbing the horses would be very unwise at this time."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
...
(instance camel of RPicView
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 216 12) ; "A lone camel waits near the large tent while his owner celebrates inside."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 216 13) ; "The bandits would notice if Graham disturbed the camel."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 216 14) ; "It would be unwise to disturb the camel right now."
							(event claimed: 1)
						)
					)
				)
			...
			(switch (event message:)
				(2 ; Look
					(if (IsFlag 32)
						...
						(PrintDC 216 15) ; "Two tents sit silently in the desert apparently deserted for the moment."
					)
					(event claimed: 1)
				)
			)
...
(instance littleTent of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 216 16) ; "There doesn't seem to be any activity going on within the smaller tent."
					(event claimed: 1)
				)
			)
		)
...
(instance dancer of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 216 17) ; "A beautiful harem girl belly dances for the rowdy bandits."
					(event claimed: 1)
				)
			)
		)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (self script:)
						(PrintDC 216 18) ; "*** This guy really knows how to party!"
					else
						(PrintDC 216 19) ; "With disgust, Graham looks at a drunken bandit lying face down in the desert sand...completely passed out."
					)
					(event claimed: 1)
				)
				(3 ; Do
