...
(instance sGetAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gMessager say: 2 0 0 0 self) ; "Captain, the Eureka has broken up. The Blob is attacking |f3|us|f| now..."
			)
			(4
				(blob
					view: 697
			...
			(8
				(EgoDead 44) ; "The tremendous force exerted by the blob finally breached the Eureka's hull, leaving the Quirk/Pukoid Blob Creature free to wreak havoc upon the universe. But look on the bright side, you won't be around to face a court martial."
			)
		)
	)
)
...
(instance sRogCaught of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(if (not (IsFlag 234))
					(gMessager say: 9 0 0 0 self) ; "You're the one who's toast this time, Roger. Forgetting that little item of vital importance back on Genetix has come back to haunt you. We hate to say we told you so, but we did."
				else
			...
			(10
				(if (not (IsFlag 234))
					(EgoDead 49) ; "Well... this new look is going to be all the rage in the Quadrant, now that you won't be around to stop the mutants."
				else
...
(instance sBlobOutside of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(17
				(gGame handsOff:)
				(tkrFlo normal: 1 keepWindow: 0 textY: 10)
			...
			(24
				(if (== (tkrFlo whichSelect:) 2)
					(EgoDead 35) ; "Some space hero you are! Sheesh, you couldn't adventure yourself out of a damp cellulose container without third-party intervention!"
				else
...
(instance tkrRoger of Talker
	...
	(method (init)
		...
		(= talkWidth (Localize 220 220 220 220 150))
		(gSystemWindow tailX: 80 tailY: 160 xOffset: (Localize 105 105 105 105 65))
