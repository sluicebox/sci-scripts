...
(instance fallInHole of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(gMessager say: 1 0 4 0 self) ; "Watch that first step... It's a doozey."
			)
			(8
				(gGame handsOn:)
				(self dispose:)
...
(instance sPukeAttacks of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3 0)
			(4
				(gMessager say: 2 0 12 0 self) ; "Yeeuuck! The giant, pus-oozing blob has burst through the trash containment hatchway. You're up to your knees in it now--literally!"
...
(instance light1 of Actor
	...
	(method (init)
		(self setLoop: 3 setPri: 10 moveSpeed: 0 setScript: sGreenLights)
		(super init:)
	)
)
