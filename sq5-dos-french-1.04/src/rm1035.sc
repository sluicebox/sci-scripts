...
(instance sSquishRog of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(EgoDead 34) ; "Looks like you've been spreading yourself a bit thin lately. Perhaps you'd like to try one of our less challenging games like ''Mixed up Mother Goose?''"
				(self dispose:)
			)
		)
	)
			...
			(switch (event message:)
				...
				(JOY_DOWNRIGHT
					(if (gEgo mover:)
						...
						(gEgo setMotion: PolyPath 94 167)
					)
				)
				(JOY_DOWN
					(if (gEgo mover:)
