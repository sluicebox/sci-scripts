...
(instance rm300 of LLRoom
	...
	(method (init)
		...
		(gEgo init:)
		(tit cycleSpeed: global101 setCycle: Fwd --UNKNOWN-PROPERTY--)
		(knee cycleSpeed: global101 setCycle: Fwd --UNKNOWN-PROPERTY--)
		(hip cycleSpeed: global101 setCycle: Fwd --UNKNOWN-PROPERTY--)
		(fountain cycleSpeed: global101 setCycle: Fwd --UNKNOWN-PROPERTY--)
		(fPlant init:)
		(fSign init:)
		(fLights init:)
		(statue init:)
		...
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					...
					(14 ; hammer
						(Print 300 23) ; "You take a swing at the apple salesman, but miss."
						(Print 300 24 #at -1 140) ; "(You never were any good at hack-and-slash games!)"
					)
					(else
						(Print 300 25) ; "Sorry," replies the apple salesman to your generous offer, "but I'm broke, not stupid!"
					)
				)
			)
			(2 ; Look
				(if lookedOnce
					...
					(Print 300 26) ; "He's yet another poor soul who donated his everything so the shrine of Lost Wages could continue to flourish."
					(= lookedOnce 1)
				)
			)
			(5 ; Talk
				(Print 300 27) ; "Hey, Buddy," you joke. "How's your Pippin?"
				(Print 300 28) ; "He mutters under his breath, "(10,000 comedians out of work in this town...)""
			)
			(3 ; Do
				(aAppleMan setScript: sLookInBarrel 0 1)
			)
			(10 ; Zipper
				(Print 300 29) ; "Sorry, buddy, but this is MY night in the barrel!"
			)
			(11 ; Taste/Smell
				(Print 300 30) ; "He smells of decomposed apple cores and fresh pork rind."
			)
			(else
...
(instance aDoorRight of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 31) ; "Isn't it funny how every time you reach out to touch these doors, they open?"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance fountain of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 32) ; "That water sure is deep."
			)
			(10 ; Zipper
				(Print 300 33) ; "Use the restroom at Lefty's instead."
			)
			(11 ; Taste/Smell
				(Print 300 34) ; "The water tastes rather metallic and a little acidic."
			)
			(else
...
(instance fPlant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 35) ; "The plant feels like someone spit apple juice on it earlier!"
			)
			(5 ; Talk
				(Print 300 36) ; "Do you think talking to artificial plants will make them grow?"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fSign of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 37) ; "You can't reach that sign from down here!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fLights of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 300 38) ; "You can't reach the lights."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance statue of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 300 39) ; "You and the sculpture have absolutely no common interests."
			)
			(3 ; Do
				(Print 300 40) ; "It feels like metal."
			)
			(10 ; Zipper
				(Print 300 41) ; "Jeez, Larry; take a cold shower or something!"
			)
			(11 ; Taste/Smell
				(Print 300 42) ; "It tastes rather metallic."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
