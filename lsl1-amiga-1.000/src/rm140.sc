...
(instance rm140 of LLRoom
	...
	(method (init)
		...
		(pimp cycleSpeed: global101 approachVerbs: 3 4 10 11 5) ; Do, Inventory, Zipper, Taste/Smell, Talk
		(if (!= (pimp loop:) 7)
			(pimp setCycle: Fwd)
		)
		(tv init: approachVerbs: 3 4 10 11 2) ; Do, Inventory, Zipper, Taste/Smell, Look
		(mooseTail cycleSpeed: global101 init: setScript: sTail)
		(moose init:)
		(boxes init:)
		...
		(cond
			...
			((not (IsFlag 27)) ; fPimpStandingAside
				(if (and (not (pimp script:)) (== (gGame detailLevel:) 3))
					(pimp setScript: sPimp checkDetail:)
				)
				(gCurRoom
					addObstacle:
						((Polygon new:)
	...
	(method (doit)
		...
		(cond
			...
			(
				...
				(ClearFlag 27) ; fPimpStandingAside
				(pimp loop: 7 cel: 4 setCycle: Beg pimp)
		...
		(switch (= state newState)
			...
			(2
				...
				(if (not (IsFlag 42)) ; fTVOn
					...
					(Points 93 3)
					(SetFlag 42) ; fTVOn
					(tvLights cycleSpeed: global101 init: setCycle: End)
				else
			...
			(4
				(cond
					...
					(else
						(Printf 140 6 141 (+ 1 (* 2 gTvChannel)) 67 -1 20) ; "Another boring %s"
					)
				)
				(= seconds 3)
			)
			...
			(switch theVerb
				...
				(4 ; Inventory
					(switch invItem
						...
						(else
							(Printf 140 28 ((Inv at: invItem) description:)) ; "The pimp isn't interested in %s. He prefers money."
						)
					)
				)
				(else
