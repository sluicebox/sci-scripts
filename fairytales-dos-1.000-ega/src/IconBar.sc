...
(class IconBar of Set
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		...
		(cond
			...
			(
				...
				(self doit:)
				(= temp4
					(if (and (== temp3 curIcon) (== temp5 curInvIcon))
						temp2
					else
						(curIcon cursor:)
					)
				...
				(if temp0
					(SetPort -1)
					(gGame setCursor: temp4 1 oldMouseX oldMouseY)
					(SetPort 0)
				else
					(gGame
						setCursor:
	...
	(method (dispatchEvent event &tmp evtY evtX evtType evtMsg thisIcon evtClaimed thePort)
		...
		(if (& evtType $0040) ; direction
			...
			(switch evtType
				(evNULL
					(cond
						((not (and (<= 0 evtX (+ y height)) (<= 0 evtY 320)))
							(if
								(and
									(& state $0400)
									(or
	...
	(method (swapCurIcon param1 &tmp temp0)
		(cond
			((& state $0004)
				(return)
			)
			...
			(else
				(= curIcon (= prevIcon (self at: 1)))
			)
		)
		(gGame setCursor: (curIcon cursor:) 1)
		(param1 type: (curIcon type:) message: (curIcon message:))
	)
)
