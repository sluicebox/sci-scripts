...
(class ego of Ego
			...
			(switch [param1 temp0]
				(1
					(if (not local1)
						...
						(local1 offset: 10000 init: self)
					)
				)
				(2
					(if (not local2)
						(= local2 (SyncedView new:))
						(local2 offset: 10100 init: self)
					)
				)
				(4
					(if (not local3)
						(= local3 (SyncedView new:))
						(local3 offset: 10200 init: self)
					)
				)
				(8
					(if (not local4)
						(= local4 (SyncedView new:))
						(local4 offset: 10300 init: self)
					)
				)
			)
		)
	...
	(method (showInv)
		(if (gInventory firstTrue: #ownedBy gEgo)
			...
			(gGame handsOff:)
			(if (== gDay 6)
				(gMessager say: 48 0 10 1 self 11) ; "Everything has been removed from your pockets!"
			else
				(gMessager say: 48 0 0 1 self 11) ; "Your pockets are empty!"
			)
		)
	)
