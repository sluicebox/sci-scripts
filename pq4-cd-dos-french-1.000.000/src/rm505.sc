...
(instance soundScript of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(if (or local4 (gTalkers size:) (Print dialog:))
					(-- state)
				else
					(= local5 1)
				)
			...
			(1
				...
				(sfx number: temp0 loop: 1 flags: 5 play: self)
			)
			(2
				(sfx number: 0 stop:)
				(= local5 0)
...
(instance moveRight of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3)
			(4
				(if (or local5 (gTalkers size:) (Print dialog:))
					(-- state)
				else
					(sfx number: 0 stop:)
					(= local4 1)
...
(instance walkOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo setScaler: 0 setCycle: 0 view: 7 loop: 0 cel: 0)
