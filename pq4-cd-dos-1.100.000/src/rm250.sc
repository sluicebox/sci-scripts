...
(instance rm250 of Room
	...
	(method (cue)
		(gGlobalSound0 pause: 0 fade: 127 15 5 0)
	)
	...
	(method (init &tmp temp0)
		...
		(gGame handsOff:)
		(super init: &rest)
		(for ((= local9 1)) (Message msgSIZE gCurRoomNum 3 2 2 local9) ((++ local9))
		)
		(-= local9 2)
		(= local8 (IntArray new: (+ local9 1)))
		(local8 name: {dumbMsg})
		(= local7 local9)
		(for ((= temp0 0)) (<= temp0 local7) ((++ temp0))
			(local8 at: temp0 (+ temp0 1))
		)
		(if (IsHiRes)
			(hiResBox init:)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(local0 show: cel: 0 setCycle: End self)
			...
			(7
				(sherryTalker modeless: 2)
				(sherry setCycle: End self)
				(gMessager say: 3 2 3 2 self) ; "Show me your badge."
			)
			(8)
			(9
				(sherry setLoop: 1 1 cel: 0 setCycle: End self)
			)
			(10
				(sherryTalker modeless: 0)
				(gGame handsOn:)
				(self dispose:)
			)
		)
...
(instance wagFinger of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(LoadMany rsVIEW 254)
				(sherry view: 254 loop: 0 cel: 0 posn: 211 100 setCycle: End)
				(sherryTalker modeless: 2)
				(gMessager say: 3 2 3 2 self) ; "Show me your badge."
			)
			(3
		...
		(switch theVerb
			...
			(2 ; Talk
				(cond
					...
					(local1
						...
						(= temp2 (local8 at: (= temp1 (Random 0 local7))))
						(for ((= temp0 temp1)) (< temp0 local7) ((++ temp0))
							(local8 at: temp0 (local8 at: (+ temp0 1)))
						)
						(if (< (-- local7) 0)
							(= local7 local9)
							(for ((= temp0 0)) (<= temp0 local7) ((++ temp0))
			...
			(22 ; handgun
				(switch local5
					(0
						(++ local5)
						(gMessager say: 3 22 17 0) ; "She might not be a brain surgeon but in this world that's not a death sentence. Put the gun away."
					)
					(1
						(++ local5)
...
(instance inBoxes of Feature
	...
	(method (doVerb theVerb)
		(cond
			...
			(else
				(gMessager say: noun 6 0 0) ; "There are probably a lot more productive things you could be doing right now... Like working!"
			)
		)
	)
)
