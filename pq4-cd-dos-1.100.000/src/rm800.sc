		...
		(switch theVerb
			(65 ; caseGlassJar
				(if (gEgo has: 20) ; shoe
					...
					(gMessager say: noun theVerb 2) ; "If you suspect hidden evidence, you'll need to look closer to confirm and collect."
				)
			)
			(52 ; notebook
				(if (gEgo has: 20) ; shoe
	...
	(method (doVerb theVerb)
		(if (== theVerb 52) ; notebook
			(if (gPqPointFlags test: 36)
				(gMessager say: noun theVerb 3 0) ; "The information has already been notated, Detective."
			else
...
(instance tape of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 52) ; notebook
			(if (gPqPointFlags test: 36)
				(gMessager say: 2 theVerb 3 0) ; "The information has already been notated, Detective."
			else
				(gCurRoom setScript: noteSceneScr)
			)
		else
...
(instance noteSceneScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 2 52 0 0 self) ; "Notating the crime scene's relationship to the house could prove important later."
			)
			(2
