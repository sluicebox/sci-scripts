...
(instance rm180 of Room
	...
	(method (init)
		...
		(if (gCast contains: laSondra)
			(gGlobalSound1 number: 0 stop:)
			(gGlobalSound0 number: 380 loop: 1 play: self setVol: 127)
		else
			(gGlobalSound1 number: 0 stop:)
	...
	(method (doit)
		...
		(cond
			...
			((gEgo inRect: 0 0 44 86)
				(gCurRoom setScript: exitTo100Scr)
			)
			((> (gEgo x:) 245)
				(gCurRoom setScript: exitTo190Scr)
			)
		)
	)
)
		...
		(switch theVerb
			...
			(27 ; badge
				(if (gPqPointFlags test: 14)
					(gMessager say: 2 27 3 0) ; "She has a pretty good memory for a little girl, Carey. There's no need to identify yourself again."
				else
...
(instance girlJamsScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(laSondra view: 187 loop: 0 cel: 0 setCycle: End self)
			)
			(3
				(laSondra
					view: 187
			...
			(4
				...
				(laSondra view: 187 loop: 2 cel: 0 setCycle: End self)
			)
			(5
				(laSondra dispose:)
...
(instance exitTo100Scr of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				...
				(gMessager say: 2 19 3 2 self) ; "Yes... thank you."
			)
			(8
				(= cycles 2)
			)
...
(instance exitTo190Scr of Script
	(properties)
