...
(procedure (PickAJoke)
	(switch (++ jokeNumber)
		(1
			(Print 110 101 #at 5 5 #mode 0 #dispose) ; "...'cause Thursday's your night in the barrel!"
		)
		(2
			(Print 110 102 #at 5 5 #mode 0 #dispose) ; "...just think of the money he saved!"
		)
		(3
			(Print 110 103 #at 5 5 #mode 0 #dispose) ; "...and there stood the pig and the cow!"
		)
		(4
			(Print 110 104 #at 5 5 #mode 0 #dispose) ; "...dammit Liza, doncha know ya own reflection?"
		)
		(5
			(Print 110 105 #at 5 5 #mode 0 #dispose) ; "...what's the matter, Honey? Ain't you got a vase?"
		)
		(6
			(Print 110 106 #at 5 5 #mode 0 #dispose) ; "...You idiot! I said POSSE!!"
		)
		(7
			(Print 110 107 #at 5 5 #mode 0 #dispose) ; "...nobody can eat THAT much ice cream!"
		)
		(8
			(Print 110 108 #at 5 5 #mode 0 #dispose) ; "...25 bucks, same as in town!"
		)
		(else
			(Print 110 109 #at 5 5 #mode 0 #dispose) ; "...a lotta guys just leave her hanging in a tree somewhere!"
			(= jokeNumber 0)
		)
	)
)
...
(instance rm110 of LLRoom
	...
	(method (init)
		...
		(super init:)
		(jukebox cycleSpeed: gHowFast init: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
		(if (!= (gTheMusic number:) 110)
		...
		(blondHead cycleSpeed: (+ gHowFast 2) init: setScript: sGuyScript)
		(babe
		...
		(babeTop cycleSpeed: (+ gHowFast 1) init: approachVerbs: 3 4 10 11 5) ; Do, Inventory, Zipper, Taste/Smell, Talk
		(ken cycleSpeed: (+ gHowFast 1) init: stopUpd:)
		(kenHead cycleSpeed: (+ gHowFast 1) init: setScript: sKenTalksGirl)
		(dude init: stopUpd:)
		(skinnyMan init: cycleSpeed: gHowFast setScript: sDudeScript)
		(fatso cycleSpeed: (+ gHowFast 1) init: setScript: sFatsoScript)
		(lefty cycleSpeed: gHowFast init: stopUpd:)
		(fan cycleSpeed: gHowFast setCycle: Fwd init:)
		(mooseEyes cycleSpeed: gHowFast init: setScript: (sDudeScript new:))
		(door
			cycleSpeed: (+ gHowFast 1)
			init:
			stopUpd:
		...
		(peephole cycleSpeed: gHowFast stopUpd: init: approachVerbs: 3 4 10 11) ; Do, Inventory, Zipper, Taste/Smell
...
(instance ken of Person
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(10 ; Zipper
				(Print 110 50) ; "Larry! That's a good way to get yourself killed in a place like this!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance skinnyMan of Person
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 110 51) ; "It's not a good idea to pick a fight in a place like this, Larry!"
			)
			(10 ; Zipper
				(Print 110 52) ; "Larry! Put that thing away!"
				(Print 110 53 #at -1 140) ; "(Besides, he already has a date!)"
			)
			(5 ; Talk
				(Print 110 54) ; "Excuse me," you say to the skinny man, "is by any chance your last name Sprat?"
				(Print 110 55) ; "Buzz off, Jerk!"
				(Print 110 56) ; "He continues to concentrate on his overweight buddy, leaving you with nothing to do but find something else to do."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance fatso of Person
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 110 57) ; "Excuse me, buddy," you say to the overweight man, "but how many stools are under there?"
				(Print 110 58) ; "Jer, er, buzz off, Jerk!"
				(Print 110 59) ; "He continues to concentrate on his underweight buddy, leaving you with nothing to do but find something else to do."
			)
			(10 ; Zipper
				(Print 110 52) ; "Larry! Put that thing away!"
				(Print 110 60 #at -1 140) ; "(Besides, it looks like he's already found a date!)"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance lefty of Person
	...
	(method (cue)
		(super cue:)
		(Print 110 61) ; "He appears to be perverted, twisted and sick!"
		(Print 110 62 #at -1 140) ; "(You instantly take a liking to him.)"
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(10 ; Zipper
				(Print 110 63) ; "Larry! Put that thing away! Lefty isn't interested in playing "match me for a drink" with you!"
			)
			(3 ; Do
				(self doVerb: 5)
			)
			(5 ; Talk
				(cond
					((not (== (gEgo view:) 801))
						(Print 110 64) ; "Why not sit down at the bar first?"
					)
					((or ordered (== (lefty script:) sLeftyServes))
						(Print 110 65) ; "Hold on; you haven't got your last order yet!"
					)
					(else
						(if (not saidHello)
							(= saidHello 1)
							(Print 110 66) ; "You greet the bartender with a friendly "Hullo.""
						)
						(gGame setCursor: 999 1)
						(Animate (gCast elements:) 0)
						(= ordered
						...
						(switch ordered
							(0
								(Print 110 67) ; ""I'll have a glass of your fine, well whiskey!" you tell Lefty."
								(lefty setScript: sLeftyServes)
							)
							(1
								(Print 110 68) ; ""May I please have a glass of your delicate white zinfandel, sir?" you ask Lefty."
								(Print 110 69) ; "Hey, was that your voice?"
								(lefty setScript: sLeftyServes)
							)
							(2
								(Print 110 70) ; ""Gimme a mug o' beer!" you order Lefty."
								(lefty setScript: sLeftyServes)
							)
							(3
								(Print 110 71 #at 73 42) ; "I ain't sellin' no sissy drinks here," says Lefty. "Try again."
								(= ordered 0)
							)
							(4
								(Print 110 72 #at 73 42) ; "You mean to say you're playing "Leisure Suit Larry," and all you want is LIGHT beer??"
								(= ordered 0)
							)
							(5
								(= ordered 0)
								(cond
									((not beenToldPrice)
										(= beenToldPrice 1)
										(Print 110 73 #at 73 42) ; "Are you sure?" asks Lefty. "It'll cost ya $90.00!"
									)
									((< gLarryDollars 90)
										(Print 110 74) ; "I really want to, Mr. Lefty," you say, "but I just don't have 90 bucks!"
									)
									(else
										(Print 110 75 #at 73 42) ; "Hey, everybody," cries Lefty, "Here comes a round from the last of the big time spenders!"
										(Print 110 76) ; "You pop $90.00 out on the bar in another unsuccessful attempt to buy friendship."
										(-= gLarryDollars 90)
									)
								)
							)
			...
			(4 ; Inventory
				(switch invItem
					(0 ; wallet
						(cond
							((not (== (gEgo view:) 801))
								(Print 110 77) ; "Try sitting at the bar; that usually gets Lefty's attention."
							)
							((< gLarryDollars 10)
								(Print 110 78) ; "You don't have enough money!"
							)
							(else
								(-= gLarryDollars 10)
								(switch (++ tipCount)
									(1
										(Print 110 79 #at 73 42) ; "Double down on eleven."
									)
									(2
										(Print 110 80 #at 73 42) ; "If you're short of money, you could always try gambling at the casino."
									)
									(3
										(Print 110 81 #at 73 42) ; "If you're bored hanging around my place, try hailing a cab outside."
									)
									(4
										(Print 110 82 #at 73 42) ; "Have you telephoned Sierra On-Line lately?"
									)
									(else
										(Print 110 83 #at 73 42) ; "Guys tell me I need to clean my rest room walls."
									)
								)
							)
						)
					...
					(5 ; whiskey
						(if (not (== (gEgo view:) 801))
							...
							(Print 110 84 #at 73 42) ; "No, thanks," Lefty replies. "I don't drink."
						)
					)
					(else
						(super doVerb: theVerb invItem)
...
(instance fan of Prop
	...
	(method (cue)
		(super cue:)
		(Print 110 85) ; "It's just going around in circles..."
		(Print 110 86) ; "...like your life."
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Zipper
				(Print 110 87) ; "Good idea! Sliced meat loaf, anyone?"
			)
			(2 ; Look
				(if (not (== (gEgo view:) 801))
					(gEgo
			...
			(3 ; Do
				(Print 110 88) ; "When you decided to become a swinging guy, this wasn't exactly what you had in mind."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance peephole of Prop
	...
	(method (cue)
		(super cue:)
		(if cel
			(Print 110 89) ; "An eye peers back at you through the peephole in the door."
		else
			(Print 110 90) ; "The peephole is tightly shut."
		)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 110 91) ; "The peephole is locked from the inside."
			)
			(else
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((self script:)
						(Print 110 92) ; "Patience, Larry; patience!"
					)
					(else
			...
			(4 ; Inventory
				(switch invItem
					(17 ; graffiti
						(if (== (sDoor state:) 2)
							(HandsOff)
							(Print 110 93) ; ""The password is: `Ken Sent Me'" you whisper through the open peephole."
							(sDoor seconds: 3 state: 3 cue:)
						else
							(Print 110 94) ; "You carefully whisper "Ken Sent Me" to the closed door. Why, you do not know."
						)
					)
					(14 ; hammer
						(Print 110 95) ; "The Naugahyde bounces the hammer back, grazing your scalp but slightly."
					)
					(else
						(super doVerb: theVerb invItem)
					)
			...
			(5 ; Talk
				(if (== (sDoor state:) 2)
					(Print 110 96) ; ""Does the password have two syllables?" you ask the man on the other side of the peephole."
					(Print 110 97 #at 180 55) ; ""At least," replies the voice through the door."
					(sDoor seconds: 0 cue:)
				else
					(Print 110 98) ; "No one can hear you through the door."
				)
			)
			(else
				(super doVerb: theVerb invItem)
...
(instance moose of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(if (== (gEgo view:) 801)
					(Print 110 100) ; "Geez, Larry, you MUST be drunk! Talking to a moose, indeed?!"
					(= tookADrink 4)
				else
					(super doVerb: theVerb invItem)
				)
