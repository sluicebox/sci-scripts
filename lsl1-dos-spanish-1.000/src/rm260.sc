...
(instance spinEm of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(wheelLeft setCycle: CT (* 2 (Random 0 3)) 1 wheelLeft)
				(= seconds 2)
			)
			(2
				(wheelCenter setCycle: CT (* 2 (Random 0 3)) 1 wheelCenter)
				(= seconds 2)
			)
			(3
				(wheelRight setCycle: CT (* 2 (Random 0 3)) 1 self)
			)
			(4
				(= blewIt 0)
				...
				(switch (event type:) ; UNINIT
					...
					(evKEYBOARD
						(if (== (event message:) KEY_RETURN) ; UNINIT
							(cond
								((== bet 10000)
									(Print 260 23) ; "This machine has a $10,000.00 limit."
								)
								((< machineDollars (+ (BetSize) bet 1))
									(if (mod machineDollars 2)
										(= bet (- machineDollars 1))
					...
					(evMOUSEBUTTON
						(while (!= ((= event (Event new:)) type:) evMOUSERELEASE)
							...
							(if (> bet 10)
								...
								(Printf 260 24 10 10) ; "This machine's minimum bet is $%d.00."
								(break)
							)
							(event dispose:)
						)
					...
					(evKEYBOARD
						(if (== (event message:) KEY_RETURN) ; UNINIT
							(if (> bet 10)
								...
								(Printf 260 25 10 10) ; "You can't bet less than $%d.00."
							)
							(event claimed: 1) ; UNINIT
							(return)
						)
...
(instance oddsButton of Prop
	...
	(method (cue)
		(super cue:)
		(Print 260 26 #width 200 #at -1 30) ; "PAYOFF  RESULTS  1 Cherry  2 Cherry Cherry  4 Cherry Cherry Cherry  2 Bell  Bell  4 Bell  Bell  Bell  3 Seven  Seven  5 Seven  Seven  Seven  7 Bar   Bar  Bar"
	)
