...
(local
	...
	[sierra2 10] = [8 0 0 3 2 6 6 6 5 4]
	index
	dialTimer
	[phoneEntry 11]
	touchTone
	...
	(if (not (and (== number 1) (not index)))
		...
		(if (or (== (++ index) 7) (== index 10))
			(cond
				...
				((and (== (CheckSierra2) 8) (<= index 7)) 0)
				(else
					(ResetPhone)
					(sfxWrongNumber play:)
					(Print 505 38) ; "I'm sorry, but you have reached a number that is disconnected or no longer in service. Please hang up and dial again."
					(Print 505 39) ; "This is a recording."
...
(instance sCallSexline of Script
	...
	(method (changeState newState &tmp [tmpStr 300])
		(switch (= state newState)
			(0
				(ResetPhone)
			...
			(6
				(Printf 505 10 @gYourPart @gYourPart) ; "Oh, I so love a man with a manly, yet urbane, %s!"
				(= seconds 3)
			)
			(7
				(while (> 2 (StrLen @global183))
					(PrintD
						26
			...
			(10
				...
				(while (> 2 (StrLen @global233))
					(PrintD 26 @tmpStr 103 41 @global233 24)
				)
				(= seconds 3)
			)
			(11
				...
				(while (> 2 (StrLen @global246))
					(PrintD 26 @tmpStr 103 41 @global246 24)
				)
				(= seconds 3)
			)
			(12
				(Format
					@tmpStr
					{"What's %s's favorite object?"}
					@global223
					@global223
				)
				(while (> 2 (StrLen @global259))
					(PrintD 26 @tmpStr 103 41 @global259 24)
				)
				(= seconds 3)
			)
			(13
				(while (> 2 (StrLen @global272))
					(PrintD
						26
			...
			(14
				...
				(while (> 2 (StrLen @global285))
					(PrintD 26 @tmpStr 103 41 @global285 50)
				)
				(DEdit font: tmpFont)
				(= seconds 3)
			)
...
(instance sSurveyResponse of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Printf ; "Hello, Larry! This is %s. I was just sitting here in the %s, wearing your %s and thinking about you (if you know what I mean!)."
					505
					19
					@global223
					@global272
			...
			(3
				(Printf ; "Why don't you forget about this silly game, and come over to my place? I'll slip into my %s, and we'll curl up in front of the fireplace and I'll stick in our copy of "%s!""
					505
					20
					@global246
					@global183
			...
			(4
				(Printf ; "You know your %s has always turned me on! So bring along a %s and come play with my %s!"
					505
					21
					@gYourPart
					@global259
			...
			(5
				(Printf 505 22 @global285 @global285) ; "Pretty soon, we'll both get excited and we'll %s like we always do!"
				(= seconds 3)
			)
			(6
...
(instance sCallSierra of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 505 33) ; "We're not open at night, but if you'd call during business hours (and stop using this pretend telephone), we would be happy to sell you a copy of one of Al Lowe's other fine games!"
				(= seconds 2)
			)
			(3
				(Print 505 34) ; "We also have many lines of GOOD games you should consider. Just dial Sierra's Order Desk at 800 326-6654 (International customers call 209 683-4468)."
				(= seconds 2)
			)
			(4
				(Print 505 35) ; "At the same time, be sure to inquire about our full line of Leisure Suit Larry novelty items."
				(= seconds 2)
			)
			(5
				(Print 505 36) ; "Thank you for calling Sierra On-Line, and for your purchase of `Leisure Suit Larry in the Land of the Lounge Lizards.' Goodbye."
				(sfxPhoneHangUp play:)
				(= seconds 2)
			)
			(6
				(sfxDialTone play: hold: 10)
				(Print 505 37) ; "Such blatant commercialism turns you on. You make a mental note to purchase one of those great, Al Lowe games!"
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance one of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 505 40) ; "Please dial the party to whom you wish to speak with."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance pound of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 505 41) ; "Please dial the party to whom you wish to speak with."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
