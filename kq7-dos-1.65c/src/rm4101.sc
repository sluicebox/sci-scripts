				...
				(cond
					((and (gCurRoom script:) ((ScriptID 4001 0) script:)) ; OogaBooga
						...
						(if
							...
							((ScriptID 4001 6) ; firecrackerTimer
								setCycle: (ScriptID 4001 6) 12000 ; firecrackerTimer
							)
						)
						(horseman hide:)
						(self dispose:)
...
(instance runOverVal of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(sBlows stop:)
				(= local9 ((ScriptID 13 0) hide_mouth:)) ; aEgoTalker
				((ScriptID 13 0) hide_mouth: 1) ; aEgoTalker
				(= cycles 1)
			)
			(2
			...
			(3
				((ScriptID 13 0) hide_mouth: local9) ; aEgoTalker
				(if (not (IsFlag 219))
					(WIBWeeps play:)
				)
		...
		(if (not (IsFlag 539))
			(switch (= state newState)
				...
				(21
					...
					(if (not (gCast contains: womanInBlack))
						(womanInBlack init:)
					)
					(womanInBlack
						view: 4119
				...
				(25
					(elspethTalker hide_mouth: 1 clientCel: -2)
					(countTalker hide_mouth: 1 modeless: 1 clientCel: -1)
...
(instance countAndWifeUnite of Script
	...
	(method (changeState newState)
		(if (not (IsFlag 539))
			(switch (= state newState)
				(0
					(horseman setLoop: 1 1 setCel: 0 setCycle: CT 7 1 self)
				)
				(1
					(self setScript: dismountHorse)
					(womanInBlack
						setLoop: 0 1
						setCel: 6
						cycleSpeed: 9
				...
				(2
					...
					(womanInBlack setLoop: 0 1 setCel: 19 setCycle: End)
				)
				(3
					(horseSound stop:)
					(womanInBlack hide:)
...
(instance dismountHorse of Script
	...
	(method (changeState newState)
		(if (not (IsFlag 539))
			(switch (= state newState)
				(0
					(horseman cycleSpeed: 9)
					(= cycles 1)
				)
				(1
					(horseman setCycle: CT 12 1)
					(= ticks 120)
				)
				(2
					(dog
