	...
	(method (changeState newState &tmp temp0 [temp1 30])
		(switch (= state newState)
			...
			(1
				...
				(= local0
					(Display @temp1 dsFONT 2510 dsCOORD 60 9 dsWIDTH 200 dsALIGN alCENTER dsCOLOR 25 dsSAVEPIXELS)
				)
				(= local1
					(Display @temp1 dsFONT 2510 dsCOORD 59 8 dsWIDTH 200 dsALIGN alCENTER dsCOLOR 30 dsSAVEPIXELS)
				)
				(= seconds 5)
			)
			(2
...
(instance titleSign of View
	...
	(method (init)
		(= x (proc0_21 160 160 160 31 31))
		(= y (proc0_21 30 30 30 42 42))
		(super init: &rest)
	)
)
