				...
				(cond
					...
					((Said '[/area]')
						(Printf ; "You are in Fat City's Aerobics Studio, which is presently filled with audio/video equipment. %s%"
							...
							(cond
								((gCast contains: aBambi) {Une superbe nana blonde se tient sur la sc\8ane, avec une expression soucieuse sur le visage.})
								((TestFlag 53) {On dirait que quelqu'un \82tait ici, il n'y a pas longtemps, en train de faire un vid\82o.})
								(else {On dirait que l'\82quipement a surchauff\82!})
							)
						)
					)
				)
