...
(instance RoomScript of Script
			...
			((Said 'look,look>')
				(cond
					...
					((Said '/cloth,hat,bra,panties')
						(if (== gEgoState 11)
							(Print 440 6) ; "There they are! Your clothes are still here, right where you left them."
						else
							(Print 440 7) ; "You are so happy to once again be back in your white leisure suit."
							(Print 440 8 #at -1 144) ; "(Although you are becoming more and more fond of cross-dressing!)"
						)
					)
					((Said '/prop,drop,drop')
						(Print 440 9) ; "Say! What the heck are they doing there?"
					)
					((Said '/bottle')
						(Print 440 10) ; "The set pieces here are very strange."
					)
					((Said '[/area,backstage]')
						(Print 440 11) ; "Backstage is an interesting sight. Those set pieces always look so real from out front, but from here you can tell they are merely a facade."
						(Print 440 12 #at -1 144) ; "(Much like your life, Larry!)"
					)
				)
			)
		)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(2
				(PrintCherri 440 13) ; "Larry! You're so wonderful. You've given me what I've always wanted."
				(aCherri view: 442 setLoop: 0 setMotion: MoveTo 94 151 self)
			)
			(3
				(PrintLarry 440 14) ; "I think you know what I've always wanted!"
			)
			(4
			...
			(5
				(PrintCherri 440 15) ; "Cherri smiles at you in the darkness, "At last I'll be able to remove this costume, never to wear it again!""
			...
			(13
				(PrintCherri 440 16) ; "What are you? Modest? Hurry up, Larry, the second show's about to start."
				(= seconds 3)
			)
			(14
				(Print 440 17) ; "You carefully consider her offer..."
				(Print 440 18) ; "...but not for long!"
				(= gEgoState 1)
				(gEgo
					viewer: 0
					view: 442
			...
			(19
				(PrintCherri 440 19) ; "Oh, Larry!" cries Cherri. "Don't you just love the real estate business?!"
				(= seconds 3)
			)
			(20
				(if (< gFilthLevel 1)
			...
			(24
				(PrintCherri 440 20) ; "Oh, Larry! I could go on like this all night!"
				(= seconds 3)
			)
			(25
				(PrintLarry 440 21) ; "You're the only one!"
				(= seconds 3)
			)
			(26
				...
				(PrintLarry 440 22) ; "Who turned out that light?"
				(= cycles 11)
			)
			(27
				(PrintLarry 440 23) ; "What's that drum roll!?"
				(= seconds 3)
			)
			(28
				(PrintCherri 440 24) ; "Oh, no!!" she cries, "Quick, Larry! Get dressed! The second show is starting!!"
				(= seconds 3)
			)
			(29
				(Print 440 25) ; "You both scramble to find, sort, and wear whatever articles of clothing you can!"
				(= seconds 3)
			)
			(30
				(PrintLarry 440 26) ; "Geez, it's dark, Cherri," you shout, "I can't tell what's what!"
				(= seconds 3)
			)
			(31
				(Print 440 27 #at 10 5 #width 290) ; "The public address system booms, "Lady and gentlemen, welcome to `Nontoonyt For You,' our spectacular tribute to the thing we all love most.""
				(= seconds 2)
			)
			(32
				(PrintLarry 440 28) ; "Where's my pants?"
			...
			(40
				(Print 440 29) ; "Ah! Now that feels like home."
			)
		)
	)
)
