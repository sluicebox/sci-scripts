		...
		(cond
			...
			(
				...
				(cond
					...
					((& (gEgo onControl:) $0040)
						(Print 253 9) ; "This cubicle has no door. You are far too modest to change clothes here."
					)
					((not (& (gEgo onControl:) $0010))
						(Print 253 1) ; "Not here! Can't you find someplace more modest?"
					)
					((!= gEgoState 10)
						(Print 253 10) ; "How can you do that?"
					)
					((and (< gFilthLevel 3) (aDoor cel:))
						(Print 253 8) ; "Close the door!"
					)
			...
			((Said 'clean/face,eye,body,me,bracelet')
				(if (or (TestFlag 8) (TestFlag 10))
					(Print 253 11) ; "You need more than just a hand basin to get rid of that workout aroma!"
				else
					(Print 253 12) ; "You would, but why bother?"
				)
			)
			((Said 'look,look>')
				(cond
					((Said '/bathroom,building')
						(Print 253 13) ; "The cabana has three cubicles (some of which have doors), and a sink with running water at its left end."
					)
					((Said '/sink,basin,fountain,faucet,handle')
						(Printf ; "A public sink is mounted on the side of the cabana.%s%"
							...
							(if (InRoom 5)
								{ Une savonnette pend au-dessus du lavabo, au bout d'une ficelle accroch\82e \85 un clou.}
							else
								{}
							)
						)
					...
					((and (InRoom 5) (Said '/soap,lasso,hemp'))
						(Print 253 15) ; "There's a bar of soap that's hanging from a rope that's looped over a nail that's driven into a board that's attached to the wall that's at the end of the cabana."
					)
					((and (not (gEgo has: 13)) (Said '/water')) ; Bottle_of_Wine
						(Print 253 16) ; "There is a water faucet on the sink. It may work."
					)
					((Said '/wall,clovis')
						(if (& (gEgo onControl:) $0010)
							(Print 253 17) ; "On the wall is scrawled the word "ZAX" which you think you've heard before, you're just not sure where."
						else
							(Print 253 18) ; "Two Guys from Clovis"
						)
					)
					((Said '/clovis')
						(Print 253 19) ; "Graffiti is not important in this game."
					)
					((Said '/nail,board,board,lasso,hemp')
						(Print 253 20) ; "Hey, come on. That was a JOKE!"
					)
					((Said '/barstool,hole')
						(if (& (gEgo onControl:) $0010)
							(Print 253 21) ; "YUCK!"
					...
					((Said '/barrel,barrel,can,can')
						(if (& (gEgo onControl:) $0100)
							(Print 253 22) ; "Believing yourself to be playing a previous Leisure Suit Larry game, you dig through all the garbage in the can, only to discover..."
							(Print 253 23) ; "...a postcard saying, "The weather is here; wish you were beautiful." You return it to the trash."
						else
							(NotClose) ; "You're not close enough."
						)
					)
					((Said '/backstage,casino')
						(Print 253 24) ; "The casino is located just to the south of this clearing."
					)
					((Said '/door')
						(cond
							((& (gEgo onControl:) $0010)
								(Print 253 25) ; "The door closed behind you."
							)
							((& (gEgo onControl:) $0002)
								(Print 253 26) ; "The left cubicle is unoccupied at the moment. Just walk inside."
							)
							((& (gEgo onControl:) $0004)
								(Print 253 27) ; "This door is stuck. You cannot get inside."
							)
							((& (gEgo onControl:) $0008)
								(Print 253 28) ; "This cubicle has no door. You have no desire to use it!"
							)
							(else
								(NotClose) ; "You're not close enough."
							)
					...
					((Said '[/area]')
						(Print 253 29) ; "The casino's changing cabana (which bears an amazing resemblance to an outhouse) is nestled in a grove of trees."
					)
				)
			)
			((Said '/door')
				(Print 253 30) ; "The left door is the only one that is available to you. The center door seems stuck and the right door is missing."
			)
			((Said '/barrel,barrel,can,can')
				(Print 253 31) ; "Stop thinking about garbage and get your mind back in the gutter!"
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(Print 253 32) ; "You grab the unusually-shaped "Soap-On-A-Rope" that hangs above the sink."
			)
			(5
				(HandsOff)
				(Ok) ; "O.K."
			...
			(11
				...
				(Print 253 34 #icon 13 0 0) ; "You place your bottle under the tap, while carefully considering the level of carcinogenic compounds therein."
			)
			(12
			...
			(14
				...
				(Format ((Inv at: 13) name:) 253 35) ; "Bottle of Water%", Bottle_of_Wine
				(if (not (TestFlag 42))
					(SetFlag 42)
			...
			(16
				...
				(= gEgoState 10)
				(NormalEgo 3)
				(Print 253 36 #icon 23 0 0) ; "You remove your leisure suit and hide it inside the stall while slipping into the grass skirt. Boy, do you look like a native now!"
			)
			(17
				(Ok) ; "O.K."
				(HandsOff)
			...
			(18
				...
				(if (gEgo has: 6) ; A_Twenty_Dollar_Bill
					(PutInRoom 4 -1)
					(Print 253 37) ; "Since you have no further use for the silly grass skirt, you toss it through a hole conveniently placed in the floor (you presume for just this purpose)."
				)
			)
		)
	)
...
(instance BillScript of Script
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'talk,talk/bill,man')
				(Print 253 38) ; "Bill ignores you (as usual)."
			)
			((Said 'talk,talk/al')
				(Print 253 39) ; "Al is too busy working on something else to talk to you."
			)
			((Said 'look,look/al')
				(Print 253 40) ; "You couldn't bear the thought!"
			)
			((Said 'look,look/bill')
				(Print 253 41) ; "He's giving Al more verbal abuse."
			)
			((Said 'look,look/man')
				(Print 253 42) ; "It's William Skirvin, world-famous artist talking to Al Lowe, a ne'r-do-well game designer."
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 253 43) ; "Wow!" says Bill. "Take a look at that bikini!"
				(Print 253 44) ; ""Open the door a little, will you, Bill?" responds Al."
				(= seconds 3)
			)
			(4
...
(instance JodiScript of Script
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said '/body')
				(Print 253 45 #at -1 144) ; "(Pant, pant, pant.)"
			)
			((Said '/woman,blond,blond')
				(cond
					...
					((and (> state 3) (< state 6))
						(Print 253 47) ; "She's undressing!"
					)
					(else
						(Print 253 48) ; "What a body!"
					)
				)
			)
		)
		...
		(switch (= state newState)
			...
			(5
				...
				(if
					...
					(Print 253 49) ; "Are you enjoying the view, buddy?"
				)
				(= seconds 3)
			)
			(6
