...
(instance RoomScript of Script
	...
	(method (handleEvent event)
			...
			(cond
				...
				((Said 'talk,talk/dale,man,dale')
					(switch DaleState
						...
						(1
							(Print 330 15) ; "He can't hear you for the roar of the crowd."
						)
						(6
							(Print 330 16) ; "You've got him sitting here with you; why don't you try looking into his eyes?"
						)
						(else
							(Print 330 17) ; "He's gone for good, Patti; you've missed him!"
						)
					)
				)
				((Said 'talk,talk')
					(Print 330 18) ; "The people at the tables are uninterested in conversing with you Patti. They're here to see the show."
				)
				(
					(and
						(== DaleState 6)
		...
		(cond
			((and clothesOnTable (Said 'get,get,(up<pick)/cloth'))
				(Print 330 20) ; "Just leave them there. You have no interest in cross-dressing."
			)
			((Said 'get,get,buy,order/drink,drink,cup')
				(Print 330 21) ; "You didn't come in here to drink, Patti!"
			)
			(
				(or
				...
				(Print 330 22) ; "You're too sophisticated to lower yourself like that!"
			)
			((Said 'lie')
				(cond
					((& (gEgo onControl:) $0004)
						(Print 330 23) ; "This chair not only faces the wrong way, but has a "Reserved" sign on it. However, the chair beside it is unoccupied."
					)
			...
			(
				...
				(cond
					...
					((== DaleState 6)
						(Print 330 24) ; "It would be rude for you to ignore your guest, Patti!"
					)
					(else
						(self changeState: 9)
					)
			...
			((Said 'throw/buck,bribe,bribe,buck')
				(Print 330 25) ; "That's not what you should throw in this place."
			)
			((Said 'look,look,caress<below/buffet,barstool')
				(Print 330 26) ; "You wouldn't want to!"
			)
			((Said 'look,look>')
				(cond
					((and clothesOnTable (Said '/cloth,buffet'))
						(Print 330 27) ; "Perhaps Dale finds you interesting. Maybe his clothes are a clue..."
					)
					((Said '/backstage')
						(Print 330 28) ; "That's why people come here -- to look at the stage!"
					)
					((Said '/barstool')
						(Print 330 29) ; "There are only two chairs available here."
						(if (== gEgoState 1004)
							(Print 330 30) ; "You're sitting in one of them."
						)
						(if (== DaleState 6)
							(Print 330 31) ; "Dale is sitting beside you!"
							(Print 330 32 #at -1 144) ; "(And you're looking at his chair?)"
						)
					)
					((Said '/burn,burn')
						(Print 330 33) ; "The spotlight is aimed directly at the stage."
					)
					((Said '/curtain')
						(if (aCurtain cel:)
							(Print 330 34) ; "Through that curtain walks someone who may be helpful to your quest, Patti!"
						else
							(Print 330 35) ; "It successfully prevents you from looking backstage."
						)
					)
					((Said '/buffet,woman,man,dale,dale,cup,drink,drink,couple')
						(Print 330 36) ; "Unlike you, the other people here are happy and enjoying themselves; not worried about finding their lover like you."
					)
					((Said '[/area]')
						(Print 330 37) ; "What a nasty establishment this is!"
						(Print 330 38 #at -1 144) ; "(You love it!)"
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(12
				(Ok) ; "O.K."
				(Print 330 39) ; "You slip off your "Saturday Night Specials" and readjust your pantyhose -- all in one smooth movement!"
				(= cycles (= seconds 0))
				(HandsOff)
				(gEgo
					view: 336
			...
			(16
				(Print 330 40 #icon 14 0 0 #at -1 10) ; "Hey, Dale," you cry, "these are for you!"
			...
			(17
				(Print 330 41) ; "You shriek, "Take me, big boy. I'm yours!""
				(= cycles (= seconds 0))
				(aPanties stopUpd:)
			...
			(18
				...
				(= gEgoState 14)
				(if (not (TestFlag 56))
					(Print 330 42) ; "Hello, handsome," you tell the male stripper. "My name is Patti, but you may call me Passionate!"
					(= seconds 3)
				else
					(self cue:)
				)
			...
			(19
				(if (not (TestFlag 56))
					(SetFlag 56) ; seenDale
					(gGame changeScore: 1)
					(Print 330 43) ; "Thanks, Patti," he responds, "and you may call me Dale."
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Print 330 44) ; "Ladies and ladies," says the voiceover announcer, "Chip 'n' Dale's is proud to present, in person, tonight only, for one show only, the first, the original, the greatest, (the owner) ...DALE!!!"
				(Print 330 45 #at -1 144) ; "(a smattering of applause)"
			...
			(14
				(Print 330 46 #at -1 10) ; "Ok, ladies! Let's really hear for him."
				(Print 330 47 #at -1 10) ; "Isn't he wonderful?!"
				(= cycles (Random 11 44))
			)
			(15
			...
			(21
				(Print 330 48) ; "Patti, look! It's him!! Dale!!! He's here, walking through the audience!"
			)
			(22
				(if (or sitWithPatti (InRoom 14 335))
					...
					(if (InRoom 14 335)
						(Print 330 49) ; "Hey, that was you that threw that lovely little pair of panties at me, wasn't it?" asks Dale. "You look lonely. Would you like a little company?"
					else
						(Print 330 50) ; "Say beautiful, aren't you that fabulous babe who's entertaining over at the casino?" Dale asks you. "It's a privilege for me even to sit with you!"
					)
				else
					(self changeState: 26)
				)
			...
			(25
				(if (!= gEgoState 1004)
					...
					(Print 330 51) ; ""Well, babe, it's been my pleasure," says Dale, standing to leave."
					(= DaleState 5)
