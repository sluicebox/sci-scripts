...
(instance RoomScript of Script
	...
	(method (doit)
		...
		(if (& (gEgo onControl:) $0100)
			(if (not seenMsg)
				(= seenMsg 1)
				(Printf 260 0 gEgoName gEgoName) ; "You have no desire to swim in a lagoon as polluted as this, %s!%"
			)
		else
			(= seenMsg 0)
		)
	...
	(method (handleEvent event)
		...
		(cond
			...
			(
				(or
					(Said '(get,get<on),throw,cast,use,lie,fuck,fuck/towel')
					(Said '(bath,bath<air,air),sunbath')
					(Said 'get,get/tan,booth')
					(Said '/(bath,bath<air,air),sunbath')
					(Said 'drain/cloth')
					(Said 'lie,fuck,fuck/down[<on]/towel')
				)
				(cond
					((== gEgoState 1005)
						(YouAre) ; "You are."
			...
			(
				(or
					(Said
						'rise,(rise,nightstand<up),(get,get<off),(get,get<up)'
					)
					(Said 'sunbath<stop')
					(Said 'exit/towel')
				)
			...
			((Said '/towel>')
				(cond
					...
					(
						...
						(Print 260 9) ; "But... you can order an official "Leisure Suit Larry 2" beach towel just by calling Sierra-on- Line!!"
						(event claimed: 1)
					)
					((Said 'get,get,grab,rob,(pick<up)')
						(cond
							((== gEgoState 10)
								(Print 260 3) ; "Change clothes first."
							)
		...
		(switch (= state newState)
			...
			(8
				...
				((ScriptID 40) ; DyingScript
					caller: (+ 4 (gEgo view:))
					register: (Format @string 260 24 gEgoName gEgoName) ; "Next time, %s, stay out of that polluted lagoon!%"
			...
			(36
				(PrintSplit 260 39) ; "Most assuredly so," says the peddler, "it's made with hand-rubbed hickory handles, and a blade of the finest Sheffield carbonized steel, drawn from the hottest 100-year-old oak charcoal fires, honed to perfection by small Oriental virgins, and guaranteed for life!"
			...
			(46
				(Print (Format @string 260 55 global304 global305) #at -1 10) ; ""%s" you cry. "Tawni, I've got a terrible case of crabs!!"%"
					...
					((== 707 (gEgo view:))
						...
						(cond
							...
							((== theObj (gInventory at: 5)) ; Soap-On-A-Rope
								...
								(Print 260 65) ; "Souvenir soap!"
								(Print 260 66) ; "Soap?! You're trying to sell me soap? What a lousy souvenir. Get out of here, pudgy nature boy!"
							)
							((== theObj (gInventory at: 11)) ; some_Orchids
								(Print 260 67) ; "Wildflowers? Why would anyone buy flowers from a beach vendor when they grow wild everywhere on this island?"
							)
							(else
								(Print 260 68) ; "No thanks," says Tawni, "I'm not interested."
							)
						)
						(Print 260 69) ; "At least she didn't recognize you in your clever disguise. Now if you only had an authentic Nontoonyt souvenir!"
					)
					((== gBeachState 4)
						(Print 260 70) ; "Oh, wow!" exclaims Tawni, without looking up. "SOUVENIRS! Whacha selling?"
						(Print 260 71) ; "She turns her head and recognizes you. "Oh, it's you, again! Leave me alone, you creep!""
						(Print 260 72 #at -1 144) ; "(Obviously she still remembers you, Larry. If only you had some sort of disguise...)"
					)
					(else
						(Print 260 73) ; "You might be able to, if you would look at her first!"
					)
				)
				(event claimed: 1)
			)
			((or (Said '//(maller,woman)>') (Said '/(maller,woman)>'))
				(cond
					((> gBeachState 4)
						(event claimed: 1)
						(Print 260 74) ; "Where?"
					...
					((Said 'fuck,fuck')
						(Print 260 75) ; "Yeah, yeah, that's the idea; but as usual, the question is: how?!"
					)
					((Said 'talk,talk')
						(cond
							((not (& (gEgo onControl:) $1000))
							...
							((== 707 (gEgo view:))
								(Print 260 76) ; "Oh, wow!" exclaims Tawni, without looking up. "SOUVENIRS! Whacha selling?"
								(Print 260 77) ; "Larry! She doesn't recognize you in your clever disguise. Now if you only had something to sell her!"
							)
							((== gBeachState 4)
								(Print 260 76) ; "Oh, wow!" exclaims Tawni, without looking up. "SOUVENIRS! Whacha selling?"
								(Print 260 78) ; "She turns her head and recognizes you. "Oh, it's you, again! Leave me alone, you creep!""
								(Print 260 72 #at -1 144) ; "(Obviously she still remembers you, Larry. If only you had some sort of disguise...)"
							)
							(else
								(Print 260 79) ; "Before speaking to a woman, it is advisable to look at her, Larry."
							)
						)
					)
					((Said 'look,look')
						(cond
							((== 707 (gEgo view:))
								(Print 260 80) ; "She doesn't seem to recognize you in your clever disguise."
							)
							((== gBeachState 4)
								(Print 260 81) ; "You wonder if she still has that twenty dollars she got in change from the souvenir bead vendor."
							)
							((== tawniBusy 1)
								(Print 260 82) ; "Now? She seems to be interested in that beach vendor."
							)
							((not (& (gEgo onControl:) $1000))
								(NotClose) ; "You're not close enough."
							)
...
(instance VendorScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look,look/man,man')
				(if (or (< state 1) (> state 10))
					(Print 260 74) ; "Where?"
				else
					(Print 260 85) ; "Don't you just love these colorful native types?"
				)
			)
			((Said 'talk,talk/man,man')
				(if (or (< state 1) (> state 10))
					...
					(Print 260 86) ; "Uh, excuse me, sir," you ask. "Do you have a permit to do this?"
					(Print 260 87) ; "Beat it, bub!" he whispers under his breath, carefully preventing the woman from hearing him. "Can't you see I'm just trying to make a quick buck off the Americano here?"
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 260 89) ; "Get your genuine Nontoonyt souvenirs!"
			)
			(5
				(if (and (!= gBeachState 3) (>= (TawniScript state:) 5))
					(return)
				)
				(Print 260 90) ; "Oh, wow!" exclaims Tawni, "SOUVENIRS!"
				...
				(Print 260 91) ; ""Whacha selling?" she asks the peddler."
				(switch gCurVendor
					(267
						(Print 260 92) ; "Genuine plastic souvenirs, handmade in the city of rsubfqsdf."
					)
					(268
						(Print 260 93) ; "Special souvenir seashells, mounted on a free-swinging necklace made of real Nontoonyt string."
					)
					(269
						(Print 260 94) ; "Today, only, I have a very special deal on these wonderful, souvenir beads."
					)
					(707
						(Print 260 95) ; "Genuine native-islander, designer-labeled erotic carvings for only twenty dollars!" you cry. "But you'd better hurry; I only have ONE left!"
					)
				)
				(TawniScript changeState: 5)
				(= cycles 30)
			...
			(6
				(switch gCurVendor
					(267
						(Print 260 96) ; ""What was that?" asks the girl."
					)
					(268
						(Print 260 97) ; "Seashells?" she surmises, "I see you sell seashells at this slimy seashore!"
						(Print 260 98) ; "Exactly how are your shells different from the ones lying all over Sunaffa Beach?"
					)
					(269
						(Print 260 99) ; "Ooooh, I just love jewelry," says the girl. "Would you allow me to purchase some?"
					)
					(707
						(Print 260 100) ; "Well," says Tawni, "I might be interested. Does it have any special features?"
						(Print 260 101 #at -1 144) ; "(Your disguise seems to be working, Larry; Tawni doesn't recognize you!)"
					)
				)
				(= cycles 30)
			)
			(7
				(switch gCurVendor
					(267
						(Print 260 102) ; ""(Hong Kong.)" the vendor mumbles under his breath."
					)
					(268
						(Print 260 103) ; "These are no longer contaminated with that crude oil/sewage mixture we call ocean water."
					)
					(269
						(Print 260 104) ; "Sure, if you're nice to me. And, give me lots of American money!"
					)
					(707
						(if (== ((Inv at: 3) view:) 22) ; Granadilla_Wood
							(Print 260 105 #icon 22 0 0) ; "I'm glad you asked," you respond, "there is one small part that might attract your eye!"
						else
							(Print 260 106 #icon 34 0 0) ; "No, but it is hand-made!"
						)
					)
				)
				(= cycles 30)
			...
			(8
				(switch gCurVendor
					(267
						(Print 260 107) ; "Oh, no matter. As long as they're genuine, give me several. You never know when I might get back this way and I just love buying souvenirs!"
					)
					(268
						(Print 260 108) ; "Oh. Well, better let me have a few," she says, "here's fifty bucks. Can I get lots?"
					)
					(269
						(Print 260 109) ; ""Ok. Here. Take all I've got," she says."
						(Print 260 110 #at -1 144) ; "(Boy, this gal will buy anything!)"
					)
					(707
						(Printf 260 111 global304 global305) ; ""%s It's a deal," she says, "I don't know much about art... but I know what I like!"%"
					)
				)
				(= cycles 30)
			)
			(9
				...
				(switch gCurVendor
					(267
						(Print 260 112) ; "All together, that comes to..." the peddler pauses, considering what the market will bear, "...three hundred dollars, American. And, I'll cover the taxes for you!"
						(Print 260 113) ; "Really? That expensive? Well, ok, if you say so."
						(Print 260 114) ; "Hey, thanks a lot," says the vendor. "Have a nice day!"
					)
					(268
						(Print 260 115) ; "Sure, here's a bag full," says the vendor. "Have a nice day!"
					)
					(269
						(Print 260 116) ; "Hey, thanks a lot," says the vendor, "But why don't you keep this twenty? You might need it."
						(Print 260 117) ; "Have a nice day!"
					)
					(707
						(Print 260 118) ; "You're lucky. I'm down to my last twenty bucks!" says Tawni. "Here you are."
						(gGame changeScore: 35)
						(gEgo get: 6 put: 3 -1) ; A_Twenty_Dollar_Bill, Granadilla_Wood
						(= gDollars 20)
						(Print 260 119) ; "Thank you very much, Miss. And... use it in good health."
					)
				)
				(TawniScript cue:)
				(= cycles 30)
...
(instance LizardScript of Script
	...
	(method (handleEvent event)
		...
		(if (and state (Said '/lizard>'))
			(cond
				((Said 'get,get,grab/')
					(Print 260 120) ; "He's too fast for you; you'd never catch him."
				)
				((Said 'look,look/')
					(cond
						((== state 0)
						...
						((and (== state 5) (== gEgoState 1005))
							(Print 260 121) ; "What a nasty looking little devil he is!"
						)
						(else
							(Print 260 122) ; "A bright red lizard scampers across the rocks near Sunaffa Beach."
						)
					)
				)
				(else
					(Print 260 123) ; "Leave the lizard alone. He's of no use to you."
					(event claimed: 1)
				)
			)
		)
