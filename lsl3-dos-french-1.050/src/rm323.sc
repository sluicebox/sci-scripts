		...
		(cond
			...
			((or (Said '/deed,land') (Said '//deed,land'))
				(cond
					...
					(else
						(Print 323 19) ; ""Nice day, isn't it, Ms. Cheatem?" you ask."
						(Format @local0 323 20) ; ""What? You want me to make small talk at $500.00 an hour?"%"
						(SuziScript changeState: 2)
					)
				)
			)
			((or (Said '/decree') (Said '//decree'))
				(cond
					((!= gEgoState 1004)
						(Print 323 21) ; "Please have a seat, Mr. Laffer!"
					)
					((!= local501 0)
						(Print 323 14) ; "She seems to be busy right now."
					)
					((or (== gLawyerState 1) (== gLawyerState 2))
						(Print 323 22) ; "How exactly do I go about getting a divorce, Ms. Cheatem?"
						(Format @local0 323 23) ; ""It's simple, Mr. Laffer. As long as you don't expect to take your case through the local court system (which is owned lock, stock and barrel by Kalalau's father) you can just let me take care of everything. My standard divorce runs $500.00, and of course, I expect to be paid in cash."%"
						(SuziScript changeState: 2)
					)
					((== gLawyerState 5)
						(++ gLawyerState)
						(gGame changeScore: 40)
						(Print 323 24) ; "Isn't it time we finalized my divorce, Ms. Cheatem?"
						(Format @local0 323 25) ; ""Absolutely, Larry. Since you paid your $500.00 to Roger, I'll get started on it right away. The next time you're in the neighborhood, stop by his desk. It's a fairly straightforward agreement; it shouldn't take long to prepare."%"
						(SuziScript changeState: 2)
					)
					((== gLawyerState 6)
						(Print 323 26) ; "I was too stupid to listen the first time you told me about the divorce. Would you explain it all again, Ms. Cheatem?"
						(Format @local0 323 27) ; ""Absolutely not, Larry. What do you expect for only $500.00?"%"
						(SuziScript changeState: 2)
					)
					(else
						(Format @local0 323 28) ; ""Thank you very much for stopping by, Mr. Laffer. Your free consultation is now over."%"
						(SuziScript changeState: 2)
					)
				)
			)
			((Said '/equipment')
				(Print 323 29) ; "It's for internal use only."
				(Print 323 30 #at -1 144) ; "(Corporate internals, that is!)"
			)
			((Said '/call,call')
				(Print 323 31) ; "There is a telephone on the desk, but don't get any cute ideas about using it!"
			)
			((Said '/buffet,calf')
				(Print 323 32) ; "Why look! You can see right through her desktop as if it were glass!"
			)
			((Said '/barstool')
				(Print 323 33) ; "It looks comfortable."
			)
			((Said '/art')
				(Print 323 34) ; "The picture is beautiful, but you're more interested in other vistas hereabouts."
			)
			((Said '/door')
				(Print 323 35) ; "The doors are open and Ms. Cheatem expects them to stay that way!"
			)
			((Said '/cup')
				(Print 323 36) ; "Ms. Cheatem's glass-topped desk sits before her mini-blinded window overlooking the lovely toxic-waste and hyphen-removal plant."
			)
			((Said '/bookcase,bookcase,bookcase,cabinet,book')
				(Print 323 37) ; "Her shelves are filled with leather-bound legal tomes and collections of "TV Guide.""
			)
			((Said 'look,look>')
				(cond
					((Said '/woman,face,eye,attorney')
						(switch local501
							(1
								(Print 323 38) ; "Ms. Cheatem must feel whatever it is she's writing is more important than talking to you."
							)
							(3
								(Print 323 39) ; "Ms. Cheatem is busy right now, talking to a bigger name on line 3."
							)
							(5
								(Print 323 40) ; "How rude! She got up and walked away from her desk right in the middle of your stimulating conversation."
							)
							(6
								(Print 323 41) ; "Ms. Cheatem must have had an important incoming fax, since she had to interrupt your meeting to pick it up from the fax machine."
							)
							(else
								(Ok) ; "O.K."
								(= gPrevRoomX (gEgo x:))
					...
					((Said '[/office,area]')
						(Print 323 42) ; "Ms. Cheatem has a nice office with a large picture window, much nicer than that cheap cubicle Chairman Kenneth gave you at Natives, Inc!"
					)
				)
			)
		)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(3
				(if (== local0 0)
					(if (== gLawyerState 1)
						(Format @local0 323 45) ; ""Well, I've certainly enjoyed our little visit together. When you're ready to do business, please come back and see me again."%"
					else
						(Format @local0 323 46) ; ""It was so nice of you to stop by again, Mr. Laffer. Please come see us again, whenever you have legal needs to be fulfilled!"%"
					)
				)
