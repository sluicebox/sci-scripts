...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'get,get/bribe,bribe,tips,jar,cup,buck,jar')
				(cond
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
			...
			(
				(Said
					'get,get,buy,order,call,call/attendant,attendant,drink,drink'
				)
			...
			(
				(or
					(Said 'give,drop/bribe,bribe,tips,jar,cup,buck,jar')
					(Said 'bribe,bribe')
				)
				(cond
					(gEgoIsPatti
			...
			((Said 'look,look>')
				(cond
					(
						(or
							(Said
								'/blackboard,blackboard,awning,board,board,special'
							)
					...
					((Said '/buffet,buffet,booth')
						(Print 450 32) ; "The tables here are usually crowded with Patti's fans."
						(Print 450 33 #at -1 144) ; "(Perhaps they heard you coming!)"
					)
					((Said '/barstool')
						(Print 450 34) ; "The stools here are near the piano bar."
					)
					((Said '/drink,drink')
						(Print 450 35) ; "You haven't ordered a drink yet."
					)
					(
						(and
							(TestFlag 67)
							(Said '/bribe,bribe,tips,jar,cup,buck,jar')
						)
						(Print 450 36) ; "On the end of the piano sits a large brandy snifter filled with money; tips donated by enthusiastic fans."
					)
					((Said '/door')
						(Print 450 37) ; "There is a door to the north marked "Employees Only" in addition to the way you entered."
					)
					((Said '/keyboard,entertainer')
						(cond
							((< gShowroomState 6)
								(Print 450 38) ; "The piano awaits the arrival of its new performer -- Passionate Patti!"
							)
							((InRoom 12)
								(Print 450 39) ; "Look, Larry! It's that woman you've been waiting for -- Passionate Patti herself!! She looks even better than you expected."
								(Print 450 40 #at -1 144) ; "(You wonder why she selected "Passionate" as her stage name.)"
							)
							((not gEgoIsPatti)
								(Print 450 41) ; "She's not here, Larry. She's waiting for you upstairs."
							)
							((TestFlag 67)
								(Print 450 42) ; "It's your tip glass! Of course! You forgot all about it."
							)
							(else
								(Print 450 43) ; "This piano brings back fond memories for you."
							)
						)
					)
					((Said '[/bar,area]')
						(if gEgoIsPatti
							(Print 450 44) ; "This piano bar has been quite good to you, but then, you deserve it!"
						else
							(Print 450 45) ; "Unusual walls rise to a sculptured ceiling that descends upon the stool before a grand piano."
							(if pattiAtPiano
								(Print 450 46) ; "Seated before you is none other than the queen of the keyboard, the irresistable instrumentalist of the ivories, the sultress of the Steinway, the babe of the Bosendorfer... Passionate Patti!"
							else
								(Print 450 47) ; "This place is sure dead!"
							)
						)
					)
				)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(13
				...
				(= gEgoState 14)
				(if (not (TestFlag 9))
					(Printf 450 48 global386 global387) ; ""Hello, beautiful," you say to the pianist. %s%"
					(= seconds 3)
				else
					(self cue:)
				)
...
(instance ElvisScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look,look/man,elvis')
				(Print 450 50) ; "Why, he looks just like The King!"
			)
			((Said 'talk,talk/elvis')
				(Print 450 51) ; "He looks like he doesn't want to be disturbed."
			)
			((Said '/elvis')
				(Print 450 52) ; "So this is where he's been hiding!"
				(self changeState: 3)
			)
		)
	)
...
(instance RogerScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look,look/man,yes,hardy')
				(Print 450 53) ; "Hey, look! It's Roger Wilco."
			)
			((Said 'talk,talk/yes,hardy,man')
				(Print 450 54) ; "He looks like he doesn't want to be disturbed."
			)
			((Said '/yes,hardy')
				(Print 450 55) ; ""So what are you doing here without those Two Guys?" you ask him."
				(Print 450 56) ; ""What two guys?" he replies."
			)
		)
	)
...
(instance PattiScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'get,make,(ask<about),gamble/music,buy,order')
				(gMusic fade:)
				(= request
					(switch (Random 1 16)
						(1 110)
				...
				(Printf ; ""Ok," says Patti, "how about a selection from that hot new Sierra adventure game `Leisure Suit Larry 3?' Here's the hauntingly-beautiful LSL3 `%s Theme.'"%"
					...
					(switch request
						...
						(206 {Jumelles})
						(265 {de Bronzette})
						(323 {D\82pouillonlais, Coku & Humili\82})
						(330 {Pollux Danse Exotique})
						(335 {de Pollux})
						(395 {de Bambi})
						(399 {Fat City})
						(431 {Danse de Ch\82rie})
						(435 {de Ch\82rie})
						(500 {Bambou})
						(560 {Rafting})
						(599 {Jungle de Pacesoar})
						(540 {Cochon Sauvage})
						(else {Larry Attrape des Bestioles})
					)
				)
				(self changeState: 1)
			)
			((Said 'talk,talk[/entertainer,woman,entertainer]')
				(if gEgoIsPatti
					(Print 450 58) ; "No one can hear you, Patti."
				else
					(Print 450 59) ; "Before speaking to a woman, it is advisable to look at her, Larry."
				)
			)
			((Said 'look,look/entertainer,woman,entertainer')
				(if (!= gEgoState 1004)
					(Print 450 60) ; "Why not have a seat first, Larry?"
				else
					(RoomScript changeState: 13)
				)
			)
			((Said '/entertainer,woman,entertainer')
				(Print 450 61) ; "Perhaps you should establish eye contact."
			)
		)
	)
	...
	(method (changeState newState &tmp howMany)
		...
		(switch (= state newState)
			...
			(4
				(Print 450 62) ; ""Here's another song you might enjoy," says Patti."
				(self changeState: 1)
			)
			(5)
			(6
			...
			(8
				(Print 450 63) ; "Here, Larry, take the key to my suite," says Patti. "I'll slip out the back way and use the service elevator. Just wait a few minutes before you come upstairs, as I'd like to slip into something more comfortable..."
				(Print 450 64) ; "...like your flowers!"
			...
			(9
				(Print 450 65) ; "Oh, and remember Larry: I refuse to make love to a man without something to drink first!" she says over her shoulder. "No offense, of course."
				(aPatti setMotion: MoveTo 168 46 self)
			)
			(10
				(Print 450 66 #at -1 144) ; "(None taken, you suppose.)"
				(aPatti dispose:)
				(User canInput: 1)
				(= gEgoState 1004)
			)
