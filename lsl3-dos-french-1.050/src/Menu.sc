...
(class TheMenuBar of MenuBar
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 [temp4 560] temp564 temp565 temp566)
		(if (and (== (event type:) evKEYBOARD) (== (event message:) KEY_ALT_l))
			(gGame
				parseLang:
					(if (== (gGame parseLang:) 33)
						(Print 997 0) ; "Parse English"
						1
					else
						(Print 997 1) ; "Parse French"
						33
					)
			)
			(event claimed: 1)
		...
		(switch (= temp0 (super handleEvent: event))
			(257
				...
				(Format @temp4 997 3 gVersion gVersion) ; "Sierra's Creative Interpreter by Jeff Stephenson, Bob Heitman, Pablo Ghenis, and Stuart Goldstein Music Composed and Performed by Mike Dana Sound Effects by Rob Atesalp Atari ST Translation by Corey Cole Amiga Translation by Robert Lindsley Quality Assurance led by Robin Bradley Version %s 1/15/91 %"
				(PrintSplit
					@temp4
					33
					gSmallFont
				...
				(PrintSplit ; "French Edition: Produced by Pablo Ghenis Translated by: Claudine Kroll and Danielle Penzer, The Wordmill Program conversion by: Gary Kamigawachi and Sol Ackerman Graphics conversion by Cheri Lloyd Quality Assurance by: UBISOFT Bilingual System by Pablo Ghenis"
					997
					4
					33
					gSmallFont
					...
					(switch
						...
						(1
							(if temp566
								...
								(gGame
									printLang: (if (!= temp565 1) 1 else 33)
								)
							)
						)
						(2
						...
						(3
							(gGame
								parseLang:
									(if (== (gGame parseLang:) 1) 33 else 1)
							)
						)
					)
					(= temp565 (gGame printLang:))
