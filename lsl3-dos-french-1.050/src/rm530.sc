...
(instance RoomScript of Script
			...
			((Said 'look,look>')
				(cond
					((Said '/boulder')
						(cond
							((== potState 531)
								(Print 530 46) ; "Your rope is hanging from the rock, but at such an angle and so near the edge of the cliff that you dare not take it."
							)
							((== potState 4)
								(Print 530 47) ; "It looks secure enough."
								(Print 530 48 #at -1 144) ; "(But is it?)"
							)
							(else
								(Print 530 49) ; "Across the canyon is a rock that rises to a point. Get the point?"
							)
						)
					)
					((Said '/carpet')
						(Print 530 50) ; "Two small depressions indicate your landing point."
					)
					((Said '/bureau,palm')
						(cond
							(onOtherSide
								(Print 530 51) ; "A path to the northwest leads through the trees and away from here."
							)
							((== gEgoState 536)
								(Print 530 52) ; "Since your legs are wrapped around it, the tree is two inches from your nose and quite easy to see!"
							)
							(else
								(Print 530 53) ; "Looking up under the fronds, you can see that these are coconut palms."
							)
						)
					)
					((Said '/coconut')
						(if (>= gFilthLevel 2)
							(Print 530 54 #at -1 144) ; "(You now realize you've been waiting for a chance to type that this entire game!)"
						)
						(cond
							((gEgo has: 19) ; Coconuts
								(event claimed: 0)
							...
							(onOtherSide
								(Print 530 55) ; "From over here, you think you may be able to make out a faint outline of some coconuts hanging beneath the leaves of the palm tree to which you tied your rope."
							)
							(else
								(Print 530 56) ; "Peeking beneath the palm trees' fronds, you can see that these trees have large, brown nuts."
								(Print 530 57) ; "You surmise they are coconut palms."
							)
						)
					)
					(
						...
						(if (== gEgoState 536)
							(Print 530 58) ; "Carefully peering beneath the overhanging fronds, you observe this is a male of the species."
						else
							(Print 530 59) ; "Dr. Nonookee, the evil genius that your Leisure Suit Larry defeated in "Looking for Love (in Several Wrong Places)," planted this garden many years ago. He must have used a helicopter for harvesting!"
						)
					)
					((Said '/plant,bush,exit')
						(cond
							((== gEgoState 536)
								(Print 530 60) ; "Carefully peering beneath the overhanging fronds, you observe this is a male of the species."
							)
							(onOtherSide
								(Print 530 61) ; "There is a small opening in the brush towards the northwest."
							)
							(else
								(Print 530 62) ; "Wait a minute! You've seen those leaves in pamphlets from the Surgeon General's office. Why, that stuff's marijuana! You must have tumbled into what's left of Dr. Nonookee's private stash!"
							)
						)
					)
					((Said '[/ledge,area]')
						(cond
							((== gEgoState 536)
								(Print 530 63) ; "From up here, you can see a long way. Across the canyon you catch a glimpse of running water and flashes of pink through the jungle growth."
							)
							(onOtherSide
								(Print 530 64) ; "There is a small opening in the brush towards the northwest. There's no reason to mess around here, Patti. Your destination lies before you."
							)
							(else
								(Print 530 65) ; "Your three-point landing has left you upon a small ledge projecting from the face of the cliff. All around you are plants with unusually shaped leaves."
							)
						)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 530 66) ; "You slowly recover from your near-tragic pantyhosing, and try to remember why you are here."
				(= seconds 4)
			)
			(4
				(Print 530 67) ; "Of course! It's Larry you're seeking. You bring yourself back to your feet, ready to again resume your quest for the man of your dreams!"
				(= seconds 4)
			)
			(5
			...
			(7
				...
				(Ok) ; "O.K."
				(Print 530 68) ; "You bend over and gather a few choice products from Dr. Nonookee's garden of unearthly delights."
				(HandsOff)
				(= moveCounter 0)
				(gEgo
					view: 531
			...
			(11
				...
				(Print 530 69) ; "Are you sure this is something you should be doing?"
			)
			(12
				(HandsOff)
				(= gEgoState 1001)
				(Print 530 70) ; "You feel your precious bodily fluids turn to dust!"
				(gEgo loop: 2)
				(gMusic fade:)
				(= seconds 3)
			)
			...
			(14
				...
				((ScriptID 40) ; DyingScript
					caller: 537
					register: (Format @string 530 71) ; "You were so near... and yet, so far!%"
					next: (Format @string2 530 72) ; "A Little "Dry" Humor%"
				)
			)
			(15
				(Ok) ; "O.K."
			...
			(17
				...
				(if (== gEgoState 535)
					(Print 530 73 #at -1 10 #icon 20 0 0) ; "You fill your mouth with marijuana leaves and slowly chew them..."
					(= state 24)
				else
					(Print 530 74 #at -1 10 #icon 20 0 0) ; "You quickly dry a few leaves in the sun, pull a pack of rolling papers from your bra and place the leaves in the paper while professionally executing a one-handed roll."
				)
				(= seconds 3)
			)
			(18
			...
			(19
				(Print 530 75 #dispose #at -1 10) ; "Grabbing a couple of rocks, you slap them together until they make a spark that ignites your joint."
				(= moveCounter 0)
			...
			(25
				(Print 530 76 #dispose #at -1 10) ; "You begin to get a little buzz on."
				(gMusic number: 531 loop: -1 play:)
				(= seconds (LangSwitch 3 6))
			)
			(26
				(gGame setSpeed: 6)
				(gEgo
			...
			(28
				(Print 530 77 #dispose #at -1 10) ; "Whoa! What's happening?"
				(= cycles (LangSwitch 11 22))
			)
			(29
				(Print 530 78 #dispose #at -1 10) ; "This stuff makes you feel like you're floating."
			)
			(30
				(Print 530 79 #dispose #at -1 10) ; "Looking down, you notice the ground seems far below you."
				(gEgo setPri: 2 setMotion: MoveTo 49 91 self)
				(= cycles 22)
			)
			(31
				(Print 530 80 #dispose #at -1 10) ; "Hey, cool, dude!"
				(= cycles 22)
			)
			(32
				(Print 530 81 #dispose #at -1 144) ; "(You see visions of Daventry!)"
			)
			(33
				(gMusic fade:)
				(Print 530 82 #dispose #at -1 10) ; "Aw, bogus! Is this stuff wearing off already?"
				(= seconds (LangSwitch 3 6))
			)
			(34
				(= seconds (= cycles 0))
				(Print
					(Format @string 530 83 global304 global305) ; "%s%"
					#dispose
					#at
					-1
					10
			...
			(35
				(Ok) ; "O.K."
				(HandsOff)
				(Print 530 84) ; "You wrap your slender thighs tightly around the long, cylindrical trunk."
				(if (>= gFilthLevel 3)
			...
			(41
				...
				(Print 530 86) ; "Reaching beneath the palm tree's frond kilt, you grab a pair of the large brown hairy nuts."
				(if (>= gFilthLevel 3)
					(Print 530 87 #at -1 144) ; "...an experience not wholly unfamiliar to you!"
				)
				(User canInput: 1)
			)
			(42
			...
			(50
				(Print 530 91 #at -1 10) ; "Giving it everything you've got, you hurl the rope as hard as you can."
			...
			(52
				...
				(Print 530 92 #at 10 5 #width 290) ; "You did it! You've lassoed that rock on the far side of the canyon."
				(= potState 3)
				(gSoundFX stop:)
				(gMusic play:)
				(HandsOn)
			...
			(55
				(Print 530 93 #at 10 5 #width 290) ; "Although your previous experience with knots has been limited to fashion accessories (plus a few weekends spent on yachts in ports around the world serving as deck hand au naturel), you tie the rope to the palm tree as securely as you can."
				(NormalEgo 1)
			...
			(57
				(gGame changeScore: 50)
				(Print 530 94 #at 10 5 #width 290) ; "Good idea, Patti! You discreetly remove approximately eight inches from the hem of your dress; just enough so when you climb on your rope you'll have a safety harness for security!"
				(= gNormalEgoView 803)
			...
			(59
				...
				((ScriptID 40) ; DyingScript
					caller: 27
					register: (Format @string 530 95) ; "Oh, Patti. Without that rope, you'll never make it across the canyon!%"
					next: (Format @string2 530 96) ; "Hey! Hold still, willya?%"
				)
			)
			(60
				(Ok) ; "O.K."
			...
			(61
				(if (!= gNormalEgoView 803)
					...
					(Print 530 97 #at -1 10) ; "Before climbing onto the rope, you slip into your safety harness. Will it be strong enough to hold you?"
					(gEgo
						view: 534
						setLoop: 5
						cel: 0
			...
			(63
				(Print 530 98) ; "You wrap your long, delicate, pianist's fingers and your long, delicate, pianist's legs around your handmade rope. Squeezing your legs tightly together, (an experience wholly unfamiliar to you), you move out over the canyon."
				(gMusic fade:)
				(= seconds 3)
			)
			(64
			...
			(67
				...
				(if (or (== gEgoState 534) (== gEgoState 535))
					(Print 530 99) ; "Remember, Patti: next time "Just Say NO!""
				else
					(Print 530 100) ; "Way to go, Newton!"
				)
				(if gDebugging
					(NormalEgo)
					(= gEgoState 0)
					...
					((ScriptID 40) ; DyingScript
						caller: 814
						register: (Format @string 530 101) ; "Who told you the law of gravity had been repealed?%"
					)
				)
			)
		)
...
(instance RopeScript of Script
	...
	(method (cue)
		(aRope stopUpd:)
		(Print 530 102 #at 10 5 #width 290) ; "Patti! Your knot around the palm tree came loose. Now there's no way for you to turn back!"
	)
)
