...
(procedure (StartExercising onWhat)
	...
	(switch (++ messageNum)
		(1
			(Print 380 31) ; "Use the cursor keys until you work up a sweat."
		)
		(2
			(Print 380 32) ; "No pain, no gain!"
		)
		(3
			(Print 380 33) ; "You can do it!"
		)
		(4
			(Print 380 34) ; "Smile, this is supposed to be fun."
		)
		(5
			(Print 380 35) ; "Who booked this gig?"
			(= messageNum 0)
		)
	)
...
(procedure (UpdateCounters &tmp [string 33])
	(= touchedBottom 0)
	(Display
		(Format @string 380 36 gBarPullCount gPullupsCount gBenchPressCount gLegCurlsCount) ; "%-2d %-2d %-2d %-2d"
		dsCOORD
		122
		82
		dsFONT
		...
		(cond
			(
				(or
					...
					(Said '(get,get<off,up),exit,stop,done')
					(Said 'exit/barstool')
				)
				(switch curMachine
					(0
			...
			(
				(or
					(Said 'use,do,exercise,class/equipment<pull,pull,bar')
					(Said 'use,do,exercise,class/bar,pull,pull')
					(Said 'use,do,exercise,class/pull,pull<bar')
					(Said 'pull,pull/bar')
				)
				(cond
					(curMachine
						(Print 380 7) ; "You are already exercising!"
					)
					((>= gBarPullCount BarPullMax)
						(Print 380 8) ; "You've pulled that bar enough! How about something else?"
					)
					((& (gEgo onControl:) $0020)
						(self changeState: 28)
					)
					(else
						(Print 380 9) ; "Move until you are near the bench on the left side of the exercise machine."
					)
				)
			)
			(
				(or
					(Said 'use,do,exercise,class/equipment<curl,curl')
					(Said 'curl,curl')
					(Said 'use,do,exercise,class/curl,curl')
				)
				(cond
					(curMachine
						(Print 380 0) ; "You are already exercising!"
					...
					((>= gLegCurlsCount LegCurlsMax)
						(Print 380 10) ; "You've done enough leg curls for any hunk! Why not try another station?"
					)
					((& (gEgo onControl:) $0008)
						(self changeState: 1)
					)
					(else
						(Print 380 11) ; "Move until you are at the far side of the bench on the right side of the exercise machine."
					)
				)
			)
			(
				(or
					(Said 'use,do,exercise,class/equipment<pull,barstool')
					(Said 'pull,bell')
					(Said 'barstool<pull/')
					(Said 'get,get<on/barstool')
					(Said 'increase,use,do,exercise,class/pull,bell,bell')
				)
				(cond
					(curMachine
						(Print 380 0) ; "You are already exercising!"
					...
					((>= gBenchPressCount BenchPressMax)
						(Print 380 12) ; "You've lifted enough iron for any man. Why not try another station?"
					)
					((& (gEgo onControl:) $0004)
						(self changeState: 10)
					)
					(else
						(Print 380 13) ; "Move until you are on the near side of the bench at the right side of the exercise machine."
					)
				)
			)
			(
				(or
					(Said 'use,do,exercise,class/equipment<up,pullup,pull,pull')
					(Said 'use,do,exercise,class/up<pull,pull')
					(Said 'pullup')
					(Said '(up<pull,pull)')
					(Said 'use,do,exercise,class/pullup')
				)
				(cond
					(curMachine
						(Print 380 0) ; "You are already exercising!"
					...
					((>= gPullupsCount PullupsMax)
						(Print 380 14) ; "You've worked out enough on this machine! Why not try another station?"
					)
					((& (gEgo onControl:) $0010)
						(self changeState: 19)
					)
					(else
						(Print 380 15) ; "Move until you are at the front center of the exercise machine."
					)
				)
			)
			((Said 'perspire,perspiration')
				(if curMachine
					(YouAre) ; "You are."
				else
					(Print 380 16) ; "Ok. Use one of the machines."
				)
			)
			((and (Said 'look,look>') (Said '[/equipment,pos,pos,area]'))
				(Print 380 17) ; "There are four separate workout stations on Fat City's exercise equipment: the "Leg Curl" bench, the "Bench Press" weight-lifting dumbbell bench, the "Pullup" station, and the "Pull Bar.""
			)
		)
	)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(4
				...
				(if touchedBottom
					(if (== (++ gLegCurlsCount) LegCurlsMax)
						(Printf 380 18 gLegCurlsCount gLegCurlsCount) ; "Congratulations! You've done %d leg curls.%"
					)
					(UpdateCounters)
				)
			)
			...
			(14
				...
				(if touchedBottom
					(if (== (++ gBenchPressCount) BenchPressMax)
						(Printf 380 19 (* 100 gBenchPressCount) (* 100 gBenchPressCount)) ; "Congratulations! You've pumped over %d pounds.%"
					)
					(UpdateCounters)
				)
			)
			...
			(24
				...
				(if touchedBottom
					(if (== (++ gPullupsCount) PullupsMax)
						(Printf 380 20 gPullupsCount gPullupsCount) ; "Congratulations! You've done %d pull ups.%"
					)
					(UpdateCounters)
				)
			)
			...
			(32
				...
				(if touchedBottom
					(if (== (++ gBarPullCount) BarPullMax)
						(Printf 380 21 gBarPullCount gBarPullCount) ; "Congratulations! You've done %d bar pulls.%"
					)
					(UpdateCounters)
				)
			)
			...
			(40
				(Print 380 22) ; "Boy, oh boy! This exercise stuff really works! Why, you look and feel like a new man!"
			...
			(42
				(cond
					((== 0 loopNum)
						(Print 380 23) ; "Hey! What's happening!?"
					)
					((== 3 loopNum)
						(Print 380 24) ; "Whoa!"
					)
			...
			(43
				(Print 380 25) ; "Well, well! Look at this. It's "The Incredible Dork!""
			...
			(45
				(Print 380 26) ; "There they are! The "Pulsating Pectorals!!""
				(= cycles 33)
			)
			(46
			...
			(47
				(Print 380 27) ; "You could learn to like this!"
				(gEgo setCycle: Beg)
				(= cycles 11)
			)
			(48
			...
			(50
				(Print 380 28) ; "Maybe you could enter one of those contests!"
			...
			(57
				(Print 380 29) ; "Oops. Your new body has sprung a leak!"
			...
			(59
				...
				(Print 380 30) ; "Oh, well. Who would want to be that "pumped up" anyway? At least you're no longer overweight; why, just take a look at yourself. That gut of yours is gone and just look at those muscles! What a hunk!!"
