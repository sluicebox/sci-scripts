		...
		(cond
			...
			((Said 'wear,get,get,(alter,alter<in),(drop<on)/towel')
				(cond
					...
					((TestFlag 51)
						(Print 370 30) ; "Your locker is empty! You didn't leave it open when you left before, did you?"
					)
					(else
						(Format @string 370 31) ; "You carefully tie the beach towel around your waist, leaving your leisure suit in the locker with the sweatsuit.%"
						(= gEgoState 8)
						(self changeState: 1)
					)
				)
			...
			(
				...
				(cond
					((== gEgoState 9)
						(Print 370 32) ; "You're already wearing the sweatsuit."
					)
					((TestFlag 50)
						(Print 370 33) ; "You can't change into the sweatsuit with water all over your body!"
					)
					((not (& (gEgo onControl:) $0040))
						(Print 370 25) ; "You can't reach your clothes from here!"
					)
					...
					(else
						(Format @string 370 34) ; "Gosh! Don't you look sweet in a pair of (newly-larger) women's sweats!%"
						(= gEgoState 9)
						(if (not (TestFlag 41))
							(SetFlag 41)
							(gGame changeScore: 4)
			...
			(
				...
				(cond
					(
						...
						(Print 370 35) ; "You're freezing; not even close."
					)
					((> howFar 80)
						(Print 370 36) ; "You're cold."
					)
					((& (gEgo onControl:) $0200)
						(Print 370 37) ; "You're warm."
					)
					((& (gEgo onControl:) $0100)
						(Print 370 38) ; "You're getting warmer."
					)
					((& (gEgo onControl:) $0080)
						(Print 370 39) ; "You're hot; very hot! You're burning up!!"
					)
					((& (gEgo onControl:) $0040)
						(Print 370 40) ; "HEY! This is it! You've found good ol' number 69!"
					)
					(else
						(Print 370 41) ; "It must be in here somewhere."
					)
				)
			)
			((Said 'pick/lock,bolt,bolt,locker,69')
				(Print 370 42) ; "You feel fortunate when you can successfully pick your nose!"
			)
			((Said 'caress/locker,top,bay')
				(Print 370 43) ; "It feels rusty."
			)
			((Said '(look,look<in),explore,(look,look<in)/locker,(door<locker)')
				(cond
					((not (& (gEgo onControl:) $0040))
						(Print 370 44) ; "It's difficult to see through those narrow slots."
					)
					((not (< (aLocker y:) 999))
						(Print 370 45) ; "It's difficult to see through the narrow slots of your closed locker door."
					)
					((TestFlag 51)
						(Print 370 46) ; "It's empty!"
						(Print 370 47) ; "Where's your stuff?"
						(Print 370 48) ; "You didn't leave the area without locking your locker, did you?"
					)
					((== gEgoState 7)
						(Print 370 49) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, and a set of woman's sweats."
					)
					((== gEgoState 8)
						(Print 370 50) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, your leisure suit, and a set of woman's sweats."
					)
					((== gEgoState 9)
						(Print 370 51) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, and your leisure suit."
					)
					((or (== gEgoState 5) (== gEgoState 6))
						(Print 370 52) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, your leisure suit, and a set of woman's sweats."
					)
				)
			)
			...
			((Said 'look,look>')
				(cond
					...
					((Said '/locker,bay')
						(Print 370 76) ; "The lockers are made of steel, painted in a lovely cyan hue."
					)
					((Said '/sweatpants')
						(if (!= gEgoState 9)
							(DontHave) ; "You don't have it."
						else
							(Print 370 77) ; "These sweats are so bulky, you can't tell what shape your body's in!"
						)
					)
					((Said '/deodorant,can,can,spray,spray')
						(Print 370 78) ; "Perhaps you should use it!"
					)
					((Said '[/area]')
						(Print 370 79) ; "You are in the "Fat City" spa's locker room. On the far wall are two doorways. There is another door that leads to the lobby. Identical lockers are everywhere. Which one should you use?"
					)
				)
			)
			((or (Said '/69') (Said '//69'))
				(Print 370 80) ; "There you go again!"
			)
		)
	)
