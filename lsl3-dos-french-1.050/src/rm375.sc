		...
		(cond
			...
			(
				...
				(cond
					...
					((not (& (gEgo onControl:) $0002))
						(Print 375 4) ; "The faucet is near the top of the screen."
					)
					(else
						(Ok) ; "O.K."
						(gEgo view: 377)
			...
			((Said 'rinse')
				(cond
					((not (TestFlag 49))
						(Print 375 5) ; "You may want to turn on the shower first."
					)
					((not (& (gEgo onControl:) $0004))
						(Print 375 6) ; "Shouldn't you be in the water?"
					)
					(else
						(Ok) ; "O.K."
						(Print 375 7) ; "You rinse your body in the sensually-warm running water."
					)
				)
			)
			((or (Said 'clean') (Said 'clean,use/soap'))
				(cond
					((not (TestFlag 49))
						(Print 375 8) ; "You may want to turn on the shower first."
					)
					((not (& (gEgo onControl:) $0004))
						(Print 375 6) ; "Shouldn't you be in the water?"
					)
					((not (gEgo has: 5)) ; Soap-On-A-Rope
						(Print 375 9) ; "You rub your hands all over yourself in an excellent imitation of a bar of soap. Too bad you don't have one."
						(Print 375 10 #at -1 144) ; "(Bar of soap, that is!)"
					)
					(else
						(Ok) ; "O.K."
						...
						(Print 375 11 #icon 5 0 0) ; "You were so wise to bring your "Soap-On-A-Rope" into the shower with you."
						(Print 375 12) ; "You carefully rub it over every part of your body."
						(if (>= gFilthLevel 3)
							(Print 375 13) ; "...some parts longer than others!"
						)
						(Print 375 14) ; "Soon, it's all gone!"
					)
				)
			)
			((and (not (gEgo has: 5)) (Said 'get,get/soap')) ; Soap-On-A-Rope
				(Print 375 15) ; "Well, look at that! Somebody used up all the soap and left none for you!"
				(Print 375 16 #at -1 144) ; "(Where could you find soap on this island?)"
			)
			((Said 'look,look>')
				(cond
					((Said '/faucet,handle,channel')
						(if (TestFlag 49)
							(Print 375 17) ; "It's "on.""
						else
							(Print 375 18) ; "It's "off.""
						)
					)
					((Said '/drain,drain')
						(if (TestFlag 49)
							(Printf ; "Look! The water is going down the drain in a %sclockwise direction!%"
								...
								(if temp0 {counter-} else {})
							)
						else
							(Print 375 20) ; "It looks grate!"
						)
					)
					((Said '[/carpet,bath,bath,area]')
						(Print 375 21) ; "The shower room has a slippery tile floor."
					)
				)
			)
		)
