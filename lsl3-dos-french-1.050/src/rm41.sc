	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print (Format @string 41 0 global304 global305) #at -1 10 #dispose) ; "%s%"
				(= gEgoState 2)
				(gEgo
			...
			(3
				...
				(if gDebugging
					...
					((ScriptID 40) ; DyingScript
						caller: (+ 1 (gEgo view:))
						register: (Format @string 41 1 gEgoName gEgoName) ; "Well, %s, are you now fully convinced that gravity really sucks?%"
