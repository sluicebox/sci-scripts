...
(instance rm40 of SQRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 40 0) ; "Nothing to do there."
			)
			(5 ; Talk
				(if (IsFlag 27)
					(Print 40 1) ; "That's funny. Tiny is nowhere to be seen."
				else
					(Print 40 2) ; "No response from that."
				)
			)
			(12 ; Smell
				(Print 40 3) ; "You detect a bit of sulfur in the air here as well. You're getting quite tired of that smell, aren't you? I'm a friend. We can talk."
			)
			(11 ; Taste
				(Print 40 4) ; "Don't lick the air. You're libel to get more more than you bargained for."
			)
			(else
...
(instance getIntoShip of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 40 5) ; "You proudly climb into your new used spacecraft. There is definitely something unique about it."
				(gEgo
					ignoreActors: 1
					illegalBits: 0
					posn: 216 137
			...
			(4
				...
				(Print 40 6) ; "You turn the key excitedly."
				(= seconds 3)
			)
			(5
			...
			(12
				(Print 40 7) ; "Drat! Where did mom and dad go wrong? You be the judge."
				(= seconds 2)
			)
			(13
...
(instance store of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Print 40 8) ; "You can't do anything to this stucco-style ship house."
			)
			(5 ; Talk
				(Print 40 9) ; "It responds not."
			)
			(12 ; Smell
				(Print 40 10) ; "It's just like all the other buildings in this place, rank!"
			)
			(11 ; Taste
				(Print 40 10) ; "It's just like all the other buildings in this place, rank!"
			)
...
(instance banners of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Print 40 11) ; "You can't do anything to them. You're not in the proper union."
			)
			(5 ; Talk
				(Print 40 12) ; "In response, they hang limp."
			)
			(12 ; Smell
				(Print {You can't.})
			)
...
(instance pinkShip of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Print 40 13) ; "Parts is parts and you don't have any need for them."
			)
			(5 ; Talk
				(Print 40 14) ; "Bad idea."
			)
			(12 ; Smell
				(Print 40 15) ; "It smells shippy."
			)
			(11 ; Taste
				(Print 40 16) ; "No thanks, I have to drive."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance greenShip of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Print 40 17) ; "You really don't want to get handprints all over a valuable antique like this one."
			)
			(5 ; Talk
				(Print 40 18) ; "Your shout produces no response. The ship is empty."
			)
			(12 ; Smell
				(Print 40 19) ; "You're a nosy little beggar, aren't you?"
			)
			(11 ; Taste
				(Print 40 20) ; "Didn't your mother tell you not to go licking other people's spaceships?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance distantShips of Feature
	...
	(method (doVerb theVerb)
		...
		(if (OneOf theVerb 3 5 12 11) ; Do, Talk, Smell, Taste
			(Print 40 21) ; "They're way up there. You can't do anything from this distance."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance radar of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 40 22) ; "I wouldn't mess with them if I were you."
			)
			(5 ; Talk
				(Print 40 23) ; "No answer is forthcoming."
			)
			(12 ; Smell
				(Print 40 22) ; "I wouldn't mess with them if I were you."
			)
...
(instance tiny of Actor
	...
	(method (doVerb theVerb invItem &tmp [temp0 40])
		(switch theVerb
				...
				(if (IsFlag 19)
					...
					(tinyTalker
						init:
							...
							tinyEye
							tinyMouth
							(Format @temp0 40 24 99) ; "I'll tell ya, ol' buddy, %u buckazoids is as low as I can go on this baby."
							0
							0
							1
					)
			...
			(3 ; Do
				(Print 40 25) ; "Touching Tiny would be rude. However, you surmise from his scaly skin that he's probably about as slimy as his salespitches."
			)
			(12 ; Smell
				(Print 40 26) ; "Something smells fishy about this salesbeing."
			)
			(11 ; Taste
				(Print 40 27) ; "Sushi, anyone? Na, Tiny looks like a tough cookie."
			)
			(4 ; Inventory
				(switch invItem
					(10 ; buckazoid
						(cond
							((IsFlag 18)
								(Print 40 28) ; "There's nothing here to buy."
							)
							((IsFlag 19)
								(tinyTalker
									init:
							...
							(else
								(Print 40 29) ; "Maybe you could get Tiny to sell you something if you talked to him."
							)
						)
					)
					(1 ; keyCard
...
(instance hole of View
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb 2 3 1) ; Look, Do, ???
			...
			(switch theVerb
				(3 ; Do
					(Print 40 30) ; "It's a hole. In this game there's nothing you can do to one."
				)
				(12 ; Smell
					(Print 40 31) ; "The hole smells like freshly dug dirt."
				)
				(11 ; Taste
					(Print 40 32) ; "It's dirt, man!"
				)
				(5 ; Talk
					(Print 40 33) ; "The void does not answer."
				)
				(4 ; Inventory
					(Print 40 34) ; "After carefully considering the hole matter, you decide not to throw anything down it."
				)
				(else
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					(else
						(Print 40 35) ; "There's an old saying in these parts: "You don't touch a ship that isn't yours, you don't die.""
					)
				)
			)
			(12 ; Smell
				(Print 40 36) ; "For some reason, this ship smells like freshly-dug dirt."
			)
			(11 ; Taste
				(Print 40 37) ; "No wonder it smells like dirt. It also tastes like it."
			)
			(5 ; Talk
				(Print 40 38) ; "The ship offers no response."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance ladder of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 40 39) ; "There's no need for that."
			)
			(12 ; Smell
				(Print 40 40) ; "It smells like boot bottoms."
			)
			(11 ; Taste
				(Print 40 41) ; "UGGH! It tastes like boot bottoms!"
			)
			(5 ; Talk
				(Print 40 42) ; "It doesn't acknowledge your inquiry."
			)
			(else
				(super doVerb: theVerb)
			)
