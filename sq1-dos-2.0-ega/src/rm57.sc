	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(Print 57 43) ; "That was a rough and nearly nauseating ride. You almost lost those Keronian Ales you downed back at Ulence Flats. Your head is definitely not clear."
			)
			(5
			...
			(19
				(Print 57 44) ; "Whoa... what a ride! Your stomach feels a bit better now and the room doesn't seem to be moving by itself any longer."
				(gEgo
					view: 32
					setLoop: 5
					posn: 112 158
...
(instance getInWasher of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(6
				(Print 57 45) ; "Uh, oh! You hear someone coming."
				(sarien
					show:
					startUpd:
					x: 278
			...
			(23
				(Print 57 46) ; "Darn static cling! Hey, look at that! By the most amazing stroke of luck, you've traded in your extremely conspicuous Xenon space suit for a Sarien officer's uniform complete with helmet."
				(SetFlag 65)
				(if (gEgo has: 0) ; Cartridge
					(Print 57 47) ; "Searching the pockets of your newly found disguise, you find the number of possessions you were packing has been greatly reduced. In fact, lost in that limbo void to where socks and baseballs disappear is everything but the data cartridge."
				else
					(Print 57 48) ; "Searching the pockets of your newly found disguise, you find the number of possessions you were packing has been greatly reduced. In fact, everything was lost in that limbo void to where socks and baseballs disappear except one lucky buckazoid."
				)
...
(instance killEgoTimeOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 57 49) ; "It sounds like someone is approaching this room!"
				(= seconds 20)
			)
			(2
				(Print 57 50) ; "You hear footsteps approaching. You only have seconds to do something!"
				(= seconds 10)
			)
			(3
				(= local212 1)
...
(instance egoTestVent of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 57 51) ; "This vent is shut tight. You don't think anything short of kicking it from the other side could get it open."
				(gEgo
...
(instance cantReachIt of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 57 52) ; "You can't seem to reach the vent from here"
				(gEgo setCycle: Beg self)
			)
			(4
				(proc703_1)
