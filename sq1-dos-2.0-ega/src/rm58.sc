		...
		(switch theVerb
			(3 ; Do
				(if local0
					...
					(Print 58 0) ; "You can't get that from here."
				)
			)
			(2 ; Look
				(Print 58 1) ; "It looks like a gas grenade."
			)
			(else
...
(instance panel of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(Print 58 2) ; "It's a counter panel capable of opening to allow passage behind the counter."
			)
			(11 ; Taste
				(Print 58 3) ; "Naw. Bad idea."
			)
			(12 ; Smell
				(Print 58 3) ; "Naw. Bad idea."
			)
...
(instance robot of Actor
	(properties
		x 135
		y 164
		lookStr {A robot serves behind the counter. Its appearance is that of a plain and unintelligent droid. But then, looks can be deceiving as in your case.}
		yStep 4
		view 460
		loop 4
		cel 4
		...
		(switch theVerb
			(5 ; Talk
				(if (not script)
					...
					(Print 58 4) ; "The droid is to busy to chitchat."
				)
			)
			(4 ; Inventory
				(cond
					((not script)
						(switch invItem
							...
							(0 ; Cartridge
								(Print 58 5) ; "I really have no idea why you feel I'd want to waste my enormous brain power contemplating a common data cartridge. You really are a dim-witted lifeform, aren't you?"
							)
							(else
								(super doVerb: theVerb invItem &rest)
							)
					...
					(else
						(Print 58 6) ; "The droid is too busy."
					)
				)
			)
			(else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(switch (DeltaurRegion numGrenades:)
					(0
						(Print 58 7) ; "This is a storeroom where a great deal of the Deltaur's weaponry is kept. Various other weapons are firmly secured to the exterior of the small structure."
					)
					(1
						(Print 58 8) ; "This is a storeroom where a great deal of the Deltaur's weaponry is kept. Various other weapons are firmly secured to the exterior of the small structure. On the counter, in front of the storeroom, is a loose gas grenade."
					)
					(2
						(Print 58 9) ; "This is a store room where a great deal of the Deltaur's weapons are kept. Various other weapons are firmly secured to the exterior of the small structure. On the counter, in front of the storeroom, are two loose gas grenades."
					)
				)
			)
			(4 ; Inventory
...
(instance counterTop of RegionFeature
	(properties
		...
		onMeCheck 1024
		lookStr {This is a long S-shaped counter top that runs from one end of the armory to the other. At one end there is an opening, probably used to allow access to the other side of the counter.}
		level 2
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 58 10) ; "That yields no result."
			)
			(2 ; Look
				(Print 58 11) ; "This is a long S-shaped counter top that runs from one end of the armory to the other. At one end there is an opening, probably used to allow access to the other side of the counter."
			)
			(11 ; Taste
				(Print 58 12) ; "It tastes very mica-like."
			)
			(12 ; Smell
				(Print 58 13) ; "It smells like you'd expect a counter to smell."
			)
			(4 ; Inventory
				(if (== invItem 12) ; Pulseray_Laser_Pistol
					(gCurRoom setScript: warnEgo)
				else
					(Print 58 14) ; "That doesn't help."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance openning of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(11 ; Taste
				(Print 58 15) ; "Funny, no taste."
			)
			(12 ; Smell
				(Print 58 16) ; "It smells void."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance shadows of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 58 17) ; "It's just a shadow."
			)
			(2 ; Look
				(Print 58 18) ; "Various pipes, ducts and other innocuous pieces of equipment form pronounced shadows throughout the room."
			)
			(11 ; Taste
				(Print 58 17) ; "It's just a shadow."
			)
			...
			(4 ; Inventory
				(if (== invItem 12) ; Pulseray_Laser_Pistol
					...
					(Print 58 19) ; "You can't do anything to or with a shadow."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
		...
		(switch (= state newState)
			(0
				...
				(if (gEgo has: 13) ; Grenade
					(Print 58 20) ; "You can carry only one grenade at a time."
					(HandsOn)
					(self dispose:)
				else
					(DeltaurRegion
					...
					(switch (DeltaurRegion numGrenades:)
						...
						(else
							(Print 58 21) ; "Bad grenade number"
						)
					)
				)
			)
...
(instance robotHead of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 58 22) ; "I wouldn't mess with him if I were you."
			)
			(2 ; Look
				(Print 58 23) ; "It's a pretty stupid-looking robot, but, then, looks can be deceiving (except in your case)."
			)
			(11 ; Taste
				(Print 58 24) ; "I don't think it would take kindly to that action."
			)
			(12 ; Smell
				(Print 58 24) ; "I don't think it would take kindly to that action."
			)
