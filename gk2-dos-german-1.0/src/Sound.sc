...
(class Sound of Obj
	...
	(method (fade newVol fTicks fSteps fEnd whoCares &tmp temp0)
		(if argc
			(if (> argc 4)
				(= client whoCares)
			)
			(= temp0 (if (> newVol gLastVolume) gLastVolume else newVol))
			(DoSound sndFADE self temp0 fTicks fSteps fEnd)
		else
