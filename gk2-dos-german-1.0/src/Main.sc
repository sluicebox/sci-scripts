...
(class TopMessage of PanelText
	...
	(method (toggleText &tmp temp0)
		(if (vmdText text:)
			...
			(vmdText newText: {Mausklick, um Szene zu \81berspringen.})
		)
	)
...
(class GK2 of Game
	...
	(method (getDisc param1 &tmp temp0)
		(= gNumCD param1)
		(while (and (!= -1 (= temp0 (CD 0 param1))) (!= temp0 param1)) ; Check
			(Printf
				{Bitte, legen Sie die CD Nr. %d ein, und dr\81cken Sie eine beliebige Taste.}
				param1
			)
		)
		(return temp0)
	...
	(method (changeScore param1 &tmp [temp0 2])
		...
		(return
			(scoreText newText: (Str format: {Punkte: %d von 678} gScore))
		)
	)
	...
	(method (quitGame)
		...
		(= gQuit
			(Print
				...
				fore: 14
				addTitle:
					{Danke, und ich hoffe, Sie hatten Spa\e1 beim Spielen!}
				addText: {M\94chten Sie wirklich aufh\94ren?} 0 3
				addButton: 1 { Ja } 55 20
				addButton: 0 { Nein } 95 20
				mode: 1
				init:
			)
		)
