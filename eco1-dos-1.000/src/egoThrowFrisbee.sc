		...
		(switch (= state newState)
			...
			(1
				...
				(if (<= 11 global247 14)
					...
					((ScriptID 2 0) init: 31 1) ; Delphineus, "Well, maybe just a couple more tosses, then I gotta go!"
					((ScriptID gCurRoomNum 3)
						setCycle: 0
						setScript: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(SetScore 1 234)
				(LoadMany rsVIEW 383 397 390 391 386 385)
			...
			(6
				(gSoundEffects number: 379 loop: 1 play:)
				((ScriptID 380 1) view: 396 loop: 2 cel: 0 init:) ; junk
				(= seconds 3)
			)
			(7
				((ScriptID 380 1) ; junk
			...
			(10
				...
				(gEgo view: 383 loop: 6 cel: 0 posn: 64 104)
				((ScriptID gCurRoomNum 3)
					view: 397
					loop: 3
					cel: 0
			...
			(15
				(proc380_8)
				(gEgo hide: posn: 71 101 get: 6) ; frisbee
				(SetFlag 113)
				(proc380_5)
				((ScriptID gCurRoomNum 3)
					ignoreActors: 1
			...
			(18
				(gEgo hide:)
				((ScriptID gCurRoomNum 3) view: 381 loop: 9 cel: 0 posn: 94 102)
				(= seconds 2)
			)
			(19
				((ScriptID 2 1) init: 1 1 self) ; Adam, "Whuh? You talked!"
			)
			(20
				((ScriptID 2 0) init: 2 1 self) ; Delphineus, "Did I? Oh, uh, I guess I did...Er, I guess the hermit's out of the crab, so to speak! Allow me to introduce myself, Adam--"
			)
			(21
				((ScriptID 2 1) init: 2 1 self) ; Adam, "And you know my name?"
			)
			(22
				((ScriptID 2 0) init: 3 0 self) ; Delphineus, "Nothin' wrong with my hearing - above or under the water!"
			)
			(23
				((ScriptID 2 0) init: 4 1 self) ; Delphineus, "As I was trying to say, my name's Delphineus!"
			)
			(24
				((ScriptID 2 1) init: 3 1 self) ; Adam, "Del- er, what?"
			)
			(25
				((ScriptID 2 0) init: 5 1 self) ; Delphineus, "DEL- FIN- EEE- US. Got it?"
			)
			(26
				((ScriptID 2 1) init: 4 1 self) ; Adam, "Del-phin-e-us, got it! Complicated name."
			)
			(27
				((ScriptID 2 0) init: 6 0 self) ; Delphineus, "Cetus, our Great Whale King named me. Of course it sounds different in our language, sort of like this --#&%--++|(***."
			)
			(28
				((ScriptID 2 0) init: 7 0 self) ; Delphineus, "I believe your human scientists call that my 'signature whistle.'"
			)
			(29
				((ScriptID 2 0) init: 8 1 self) ; Delphineus, "I also speak tuna and some of the more obscure coral dialects! Those are too high-pitched for you to hear. Anyway, they're only useful in my kingdom."
			)
			(30
				((ScriptID 2 1) init: 5 1 self) ; Adam, "Cool! Your kingdom?"
			)
			(31
				((ScriptID 2 0) init: 9 1 self) ; Delphineus, "Right...My kingdom, it's...Oh no! My kingdom! Cetus!!! I remember now. I don't know what I've been thinking! How long have I been here?"
			)
			(32
				((ScriptID 2 1) init: 6 1 self) ; Adam, "About a week. You were in pretty bad shape--"
			)
			(33
				((ScriptID 2 0) init: 10 1 self) ; Delphineus, "A week! What a disaster! I've got to get out of here right away!"
			)
			(34
				((ScriptID 2 1) init: 7 1 self) ; Adam, "Hold on. I'm not getting this. What are you talking about? What's the matter?"
			)
			(35
				((ScriptID 2 0) init: 11 0 self) ; Delphineus, "That's me all over. Just rushing straight ahead, not looking where I'm going. That's how I got into this mess in the first place."
			)
			(36
				((ScriptID 2 0) init: 12 0 self) ; Delphineus, "When I ran into that net, it's because I was searching for Cetus. He's missing and I was sent to find him. Maybe he's in trouble, or hurt, or worse. I have to find out before it's too late."
			)
			(37
				((ScriptID 2 0) init: 13 1 self) ; Delphineus, "Adam, please help me get out of here!"
			)
			(38
			...
			(40
				((ScriptID 2 1) init: 114 1 self) ; Adam, "Of course I'll help you, Delphineus!"
			)
			(41
