	...
	(method (init)
		...
		(gCurRoom
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: -100 114 45 126 70 151 92 175 288 175 319 165 380 165 380 300 -100 300
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
				...
				((Polygon new:)
					type: PBarredAccess
					init: 286 125 204 125 209 99 277 99
					yourself:
				)
		)
		(switch gPrevRoomNum
		...
		(switch gPrevRoomNum
			(440
				(if (not (IsFlag 73))
					(gDelph
						posn: -60 (gEgo y:)
						heading: 90
						setMotion: DPath 60 30 140 140 self
					)
				)
			...
			(343
				(gEgo
					posn: 274 -20
					heading: 180
					setMotion: PolyPath 274 110 self
				)
				(if (not (IsFlag 73))
					(gDelph
						posn: (gEgo x:) -80
...
(instance egoGetsCable of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator posn: -1 20 init: 29) ; "Adam reaches down and pulls out the cable."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(cond
					((IsFlag 136)
						(Narrator posn: -1 30 init: 32) ; "Adam does his trick with the mirror again!"
					)
					((IsFlag 139)
						(Narrator posn: -1 30 init: 24) ; "Adam shines the mirror toward the octopus."
					)
					(else
						(= local9 1)
					)
...
(instance swimThruBrainCoral of Script ; UNUSED
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: PolyPath 182 146 self)
			)
			(1
				(gEgo setMotion: PolyPath 192 156 self)
			)
			(2
				(gEgo
					setCycle: 0
			...
			(3
				(gSoundEffects number: 484 loop: 1 play:)
				(gEgo cel: 8 setCycle: CT 11 1 self)
			)
			(4
				(gSoundEffects number: 485 loop: 1 play:)
				(gEgo cel: 12)
				(= cycles (gEgo cycleSpeed:))
			)
			(5
				(gEgo loop: 1 posn: 212 166 setCycle: End self)
			)
			(6
				(= cycles 5)
			)
