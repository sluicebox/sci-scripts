...
(instance rm680 of EcoRoom
	...
	(method (doit)
		...
		(MoveCursor 0 200)
		(if local12
			(Palette palSET_INTENSITY 0 255 (+= local13 local14))
			(if (>= local13 100)
				(= local12 0)
			)
		)
	...
	(method (changeState newState &tmp [temp0 30] [temp30 30] [temp60 30] [temp90 30])
		(switch (= state newState)
			...
			(10
				...
				(SetCursor 957 0 7)
				(GetMessage 98 0 35 @temp0)
				(GetMessage 98 0 36 @temp30)
				(GetMessage 98 0 37 @temp60)
				(GetMessage 98 0 7 @temp90)
				(repeat
					(switch
						(PrintD
							@temp0
...
(instance endingCredits of Actor
	...
	(method (doit)
		...
		(if (< y -10)
			(self dispose:)
			(if (and (== loop local10) (== cel local11))
				(sEndIt cue:)
			)
		)
	)
