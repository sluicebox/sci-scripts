...
(procedure (GetDirectory where &tmp result newDir str buf1 len p theText ret)
	...
	(= buf1 (Str new:))
	(repeat
		(= len (Min 120 (+ (Max 29 (+ (where size:) 6)) 11)))
		((= p (Print new:))
			font: 999
			fore: 0
			back: saveColGray
		...
		((p dialog:)
			add:
				((DText new:)
					text: (KArray 8 (theText data:)) ; ArrayDup
					font: 999
					fore: 0
					back: saveColGray
					skip: 254
...
(class SRDialog of Dialog
	...
	(method (init theComment names nums &tmp db)
		...
		(cancelI
			text: (KArray 8 (butbuf4 data:)) ; ArrayDup
			setSize:
	...
	(method (update theComment names nums &tmp db buf)
		...
		(textI
			text: (KArray 8 (buf data:)) ; ArrayDup
			x: 0
			y: 0
			setSize: 240
			moveTo: 4 4
