...
(class CueObj of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(if (and (client sightAngle:) (!= (client sightAngle:) 26505))
					(gEgo
						setHeading:
							(GetAngle
								(gEgo x:)
...
(class Feature of Obj
	...
	(method (approachVerbs args &tmp i theBits)
		(= _approachVerbs 0)
		(if (and argc gLarryApproachCode [args 0])
			(for ((= i 0)) (< i argc) ((++ i))
				(= theBits (gLarryApproachCode doit: [args i]))
			...
			(
				...
				(if
					(and
						...
						gLarryApproachCode
						(& _approachVerbs (gLarryApproachCode doit: (event message:)))
					)
					(gEgo
						setMotion:
							PolyPath
