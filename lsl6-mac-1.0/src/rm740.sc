...
(instance modifyPaletteScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					((and (== local41 -1) (> register local44))
						(-- register)
						(-- state)
					)
	...
	(method (cue &tmp [temp0 110])
		(if (== (gGlobalSound1 prevSignal:) -1)
			(if (not register)
				...
				(= register 1)
				(if (and local51 local52)
					(Display {} dsRESTOREPIXELS local52)
					(= local52 0)
				)
			else
				(Message msgGET 740 0 0 22 (+ local46 2) @temp0)
				(= local52
					(Display
						@temp0
						dsALIGN
...
(instance unknown_740_14 of Script
	(properties)

	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(for ((= temp0 100)) (> temp0 local44) ((-= temp0 1))
...
(instance unknown_740_15 of Script
	(properties)
