...
(procedure (proc0_1) ; UNUSED
	(larryWindow brBottom: 139)
)

(procedure (proc0_2) ; UNUSED
...
(class LSL6 of Game
	...
	(method (init &tmp [temp0 33] temp33)
		...
		(Platform 0 4 0 7 icon0 icon1 icon2 icon3 icon4 icon6 icon7)
		(Platform 0 6 5 7 1)
		(= gMenuBar (ScriptID 93 6)) ; menuBar
	...
	(method (save param1 &tmp [temp0 150])
		(= global195 gTheCursor)
		(Platform 0 4 3 -1)
		(cond
			((IsFlag 86)
				(gMessager say: 0 0 15 0 0 0) ; "There's not enough free memory to save at this time. Please try again later."
			)
			((== gCurRoomNum 740)
	...
	(method (restore &tmp temp0)
		(if (IsFlag 86)
			(gMessager say: 0 0 19 0 0 0) ; "There's not enough free memory to restore at this time. Please try again later."
		else
			(= temp0 gTheCursor)
			(SetPort 0 0 190 320 10 0)
			(super restore: &rest)
