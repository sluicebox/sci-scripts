...
(instance rm300 of LarryRoom
	...
	(method (init &tmp temp0 temp1)
		...
		(windowFrame init:)
		(carpet init:)
		(doorBackdrop init: addToPic:)
		(door init: stopUpd: approachVerbs: 4) ; Do
		(super init: &rest)
	)
...
(instance gammieScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gammie show: ignoreActors: 0 setMotion: MoveTo 227 116 self)
			)
			(3
				(= global171 4)
...
(instance visibleSculpture of Feature
	...
	(method (init)
		(super init: &rest)
		(self setOnMeCheck: 2 (picturePoly2 init: yourself:))
	)
...
(instance hiddenSculpture of Feature
	...
	(method (init)
		(super init: &rest)
		(self setOnMeCheck: 2 (picturePoly1 init: yourself:))
	)
