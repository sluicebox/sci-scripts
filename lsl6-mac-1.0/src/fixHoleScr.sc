...
(instance fixHoleScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				((ScriptID 330 9) view: 330 loop: 3 cel: 0) ; hoseWithHole
				(= cycles 2)
			)
			(5
				(gEgo put: 4 normalize: 900 8 cel: 6) ; belt
				((ScriptID 330 9) stopUpd:) ; hoseWithHole
				(gGame handsOn:)
				(self dispose:)
			)
		)
...
(instance greasePistonScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(gEgo setSpeed: 6 setMotion: MoveTo 43 119 self)
				(= local0 register)
				(= register 3)
			)
			(1
		...
		(switch (= state newState)
			...
			(2
				(if register
					...
					((ScriptID 330 10) init: stopUpd:) ; filter
				)
				(= ticks 20)
			)
			(3
