...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] str [buf1 40] [buf2 10] [buf3 5]) ; UNUSED
...
(class Game of Obj
	...
	(method (replay &tmp temp0 temp1 temp2 temp3)
		(Platform 0 4 2 -1)
		(for
			...
			(if (& (temp2 signal:) $0004)
				(Platform 0 4 3 (temp2 iconIndex:))
			else
				(Platform 0 4 2 (temp2 iconIndex:))
			)
		)
		(if gLastEvent
			(gLastEvent dispose:)
		...
		(DrawPic (gCurRoom curPic:) temp0 1)
		(if (!= gOverlays -1)
	...
	(method (save &tmp [temp0 50] temp50 [temp51 206])
		(SaveGame name temp50 @temp0 gVersion) ; UNINIT
	)

	(method (restore param1 &tmp [temp0 227])
		(RestoreGame (if argc [param1 0] else 0) gVersion)
	)
	...
	(method (myInterface)
		(self setScript: (ScriptID 93 1)) ; unknown_93_2
	)

	(method (myCustServ)
		(self setScript: (ScriptID 93 2)) ; unknown_93_3
	)

	(method (myHint)
		(self setScript: (ScriptID 93 3)) ; unknown_93_4
	)

	(method (myEnjoy)
		(self setScript: (ScriptID 93 4)) ; unknown_93_5
	)

	(method (myAbout)
		(self setScript: (ScriptID 93 5)) ; unknown_93_6
	)

	(method (myAuto)
		(self setScript: (ScriptID 93 7)) ; unknown_93_7
	)

	(method (mySave)
		(self setScript: (ScriptID 93 8)) ; unknown_93_8
	)

	(method (myKey)
		(self setScript: (ScriptID 93 9)) ; unknown_93_9
	)
)
