...
(class WalkieTalkie of Obj
	...
	(method (pause param1)
		(if param1
			...
			(Robot 10) ; Pause
			(self started: 0 paused: 1)
		else
			(self started: 1 paused: 0)
			(Robot 4) ; Play
		)
	)
	...
	(method (showFrame theNum theFrame theX theY pri whichPlane &tmp showX showY thatOldPlane thatOldPri)
		(= showX (if (> argc 2) theX else 0))
		(= showY (if (> argc 3) theY else 0))
		(= thatOldPri (if (> argc 4) pri else -1))
		(= thatOldPlane (if (> argc 5) whichPlane else gThePlane))
		(Robot 0 theNum thatOldPlane thatOldPri showX showY) ; Open
