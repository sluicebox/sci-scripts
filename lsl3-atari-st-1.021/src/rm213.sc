...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((or (Said '//camera,drop>') (Said '/camera,drop>'))
				(cond
					((Said 'watch')
						(if (< (aTv y:) 200)
			...
			((Said 'get/buffet')
				(Printf 213 19 gEgoName) ; "You're after things with TWO legs, %s."
			)
			((Said 'get/burn')
			...
			((Said 'look>')
				(cond
					((Said '/burn')
						(Printf 213 23 gEgoName) ; "They just hang around -- rather like you, %s!"
					)
					((Said '/buffet')
						(cond
							(havePaper
