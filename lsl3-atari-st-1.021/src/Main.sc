...
(procedure (NotNow)
	(Print 0 173) ; "Good idea. You might try that again later."
)

(procedure (NotClose)
	(Print 0 174) ; "You're not close enough."
)

(procedure (AlreadyTook)
	(Print 0 175) ; "You already took it."
)

(procedure (DontHave)
	(Print 0 176) ; "You don't have it."
)
...
(procedure (HaveMem howMuch)
	(if (> (MemoryInfo 1) howMuch) ; FreeHeap
		...
		(Print 0 57) ; "You can't do that here; at least, not now."
		(return 0)
	)
)
		...
		(cond
			...
			((or (Said 'caress/ginsu') (Said 'sharpen/ginsu'))
				(cond
					...
					((!= gCurRoomNum 250)
						(Print 0 16) ; "There's nothing here suitable for sharpening your knife."
					)
				)
			)
			((or (Said 'drop/*/bottle') (Said 'fill/bottle'))
				(Print 0 17) ; "This is not the correct place to do that."
			)
			((Said '(drain<out),drain/beer,bottle')
				(cond
					...
					((not gEgoIsPatti)
						(Print 0 18) ; "That's not a good idea, Larry. Patti wanted that wine!"
					)
					((== (Bottle_of_Wine_ view:) 28)
						(Print 0 19) ; "The bottle is already empty."
					)
					(else
						(Bottle_of_Wine_ view: 28)
						(Format (Bottle_of_Wine_ name:) 0 20) ; "Empty Bottle"
						(Print 0 21) ; "Ok. It's empty now."
					)
				)
			)
			((Said 'cut,cut>')
				(cond
					((not (gEgo has: 2)) ; Ginsu_Knife
						(Print 0 22) ; "You have nothing with which to cut anything."
					)
					((== (Ginsu_Knife view:) 2)
						(Print 0 23) ; "Your knife is too dull to cut anything."
					)
					((Said '[/!*]')
						(Print 0 24) ; "What do you want to cut?"
					)
					((Said '/blade')
						(Print 0 25) ; "This grass isn't really suitable. Keep looking."
					)
					((not (Said '/carving,granadilla'))
						(Print 0 26) ; "No; find something else to carve."
					)
					((not (gEgo has: 3)) ; Granadilla_Wood
						(Print 0 27) ; "You have nothing to carve."
					)
					(
						(or
						...
						(Print 0 28) ; "You've already done the best you can."
					)
			...
			((or (Said 'use/flower/lei<make') (Said 'weave,make/flower,lei'))
				(cond
					((not (gEgo has: 11)) ; some_Orchids
						(Print 0 29) ; "You have no flowers with which to make a lei."
					)
					((== (some_Orchids view:) 26)
						(Print 0 30) ; "You've already made a lei from the orchids."
					)
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
			...
			((Said 'wear,(alter<in),(drop<on)/flower,lei')
				(cond
					((not (gEgo has: 11)) ; some_Orchids
						(Print 0 31) ; "An interesting thought -- but how?"
					)
					((!= (some_Orchids view:) 26)
						(Print 0 32) ; "Right now, they're just a beautiful bunch of orchids."
					)
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
					(else
						(Print 0 33) ; "You're close, but you are not the person for whom they are intended."
					)
				)
			)
			((or (Said 'use/blade/skirt<make') (Said 'weave,make/blade,skirt'))
				(cond
					((not (gEgo has: 4)) ; Native_Grass
						(Print 0 34) ; "You have no grass."
					)
					((== (Native_Grass view:) 23)
						(Print 0 35) ; "You've already made a skirt from the grass."
					)
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
			...
			((or (Said 'get/naked') (Said '(get<off),drain/cloth,cloth'))
				(Print 0 36) ; "Please! Not here."
			)
			((Said 'alter/cloth,cloth')
				(Print 0 37) ; "Be more specific. Say what you want to wear."
			)
			((Said 'wear,(alter<in),(drop<on)/blade,skirt')
				(cond
					...
					((not (== (Native_Grass view:) 23))
						(Print 0 38) ; "It would be difficult to wear a clump of grass."
					)
					(else
						(Print 0 39) ; "A good idea, but you're far too modest to change clothes here."
					)
				)
			)
			((and (gEgo has: 5) (Said 'use/soap')) ; Soap-On-A-Rope
				(Print 0 40) ; "A nice shower WOULD feel good!"
			)
			(
				(or
					(Said 'unknownnumber/')
				...
				(Print 0 41) ; "That's may do something somewhere but not here!"
			)
			(
				(and
				...
				(Print 0 42) ; "Well, well. Look at this. Somehow Suzi Cheatem's Fat City Membership Card got mixed up in these papers. What a sleuth you are!"
			)
			(
				(and
			...
			((Said 'count>')
				...
				(cond
					((Said '[/!*]')
						(Print 0 43) ; "What do you want to count?"
					)
					((not temp2)
						(event claimed: 1)
						(Printf 0 44 (Random 10 999)) ; "You quickly count to %d. Such fun, eh?"
					)
					((!= (gInventory indexOf: temp2) 6)
						(Print 0 45) ; "How can you count that?"
					)
					((not (temp2 ownedBy: gEgo))
						(Print 0 46) ; "You're busted!"
					)
					(else
						(Printf 0 47 gDollars) ; "You have %d dollars."
					)
				)
			)
			((or (Said 'give/bill,buck') (Said 'bribe'))
				(cond
					((not (gEgo has: 6)) ; A_Twenty_Dollar_Bill
						(Print 0 48) ; "You have no money."
					)
					((== (A_Twenty_Dollar_Bill view:) 24)
						(Print 0 49 #icon (A_Twenty_Dollar_Bill view:) 0 0) ; ""How about a few of these ones?" you ask."
						(Print 0 50 #at -1 144) ; "(Evidently no one is interested.)"
					)
					((== (A_Twenty_Dollar_Bill view:) 25)
						(Print 0 51 #icon (A_Twenty_Dollar_Bill view:) 0 0) ; ""Hey! Can I give you a tip?" you say."
						(Print 0 52) ; ""Guess not," you think."
					)
					(else
						(Print 0 53 #icon (A_Twenty_Dollar_Bill view:) 0 0) ; "I've got twenty bucks on me," you announce. "Anybody interested?"
						(Print 0 54) ; "There is no response."
					)
				)
			)
			(
				...
				(Print 0 55) ; "A good idea, but you can't change here!"
			)
			((and (gEgo has: 16) (Said 'drop//(bra)>')) ; Bra
				(= temp2 (gInventory saidMe:))
				(cond
					((Said '[/!*]')
						(Print 0 56) ; "What did you want to put inside your bra?"
					)
					((not temp2)
						(event claimed: 1)
						(Print 0 57) ; "You can't do that here; at least, not now."
					...
					(else
						(Print 0 58 #icon 16 0 0) ; "Ok. You put it in. It fits. You remove it."
						(Print 0 59 #at -1 144) ; "(Pretty much fun, eh?)"
					)
				)
			)
			((and (gEgo has: 2) (or (Said 'use/ginsu') (Said 'hit'))) ; Ginsu_Knife
				(Print 0 60) ; "Hey! We want our R rating for sex, not violence!"
			)
			((Said 'open,(look<in)>')
				(= temp2 (gInventory saidMe:))
				(cond
					((Said '[/!*]')
						(Print 0 61) ; "What do you want to inspect?"
					)
					((not temp2)
						(event claimed: 1)
						(Print 0 57) ; "You can't do that here; at least, not now."
					...
					(else
						(switch (gInventory indexOf: temp2)
							(11
								(Print 0 63 #icon 11 0 0) ; "The orchids look perfectly safe."
							)
							(13
								(switch (Bottle_of_Wine_ view:)
									(28
										(Print 0 64 #icon 28 0 0) ; "The bottle is empty. Not even a note!"
									)
									(29
										(Print 0 65 #icon 29 0 0) ; "The water is still inside the bottle."
									)
									(else
										(Print 0 66 #icon 13 0 0) ; "The bottle is full of wine."
									)
								)
							)
							(14
								(Print 0 67 #icon 14 0 0) ; "100% Pure Silk"
							)
							(15
								(Print 0 68 #icon 15 0 0) ; "Extra Sheer"
							)
							(16
								(Print 0 69 #icon 16 0 0) ; "36C"
							)
							(17
								(Print 0 70 #icon 17 0 0) ; "You look pretty good, if you do think so yourself!"
							)
							(19
								(Print 0 71 #icon 19 0 0) ; "You can not open the coconuts."
							)
							(else
								(Print 0 62) ; "You see nothing special."
								(LameResponse)
			...
			((Said 'thank')
				(Print 0 73) ; "You're welcome."
			)
			((Said 'knock')
				(Print 0 74) ; "Knock, knock."
				(Print (Format @temp6 0 75 gEgoName) #at -1 144) ; "(Nobody wants to play that game, %s!)"
			)
			((Said 'hit')
				(Print 0 60) ; "Hey! We want our R rating for sex, not violence!"
			)
			((or (Said 'go/bathroom') (Said 'leak') (Said 'get/leak'))
				(if gEgoIsPatti
					(Print 0 76) ; "Patti!"
					(Print 0 77 #at -1 144) ; "(Really!)"
				else
					(Print 0 78) ; "A warm feeling spreads down your leg."
				)
			)
			((Said 'climb>')
				(cond
					((Said '/wall,building')
						(Print 0 79) ; "Climbing the walls already?"
					)
					(gEgoIsPatti
						(Print 0 80) ; "You'd better not. You might rip your dress."
					)
					(else
						(Print 0 81) ; "That would be unbecoming a man of leisure."
					)
				)
				(event claimed: 1)
			)
			((or (Said '//larry') (Said '/larry'))
				(cond
					((not gEgoIsPatti)
						(Print 0 82) ; "Talking to yourself again, Larry?"
					)
					((< gCurRoomNum 590)
						(Print 0 83) ; "You really miss him, don't you, Patti?"
					)
					(else
						(Print 0 84) ; "You are so glad you two are back together!"
					)
				)
			)
			((Said 'jump,dance')
			...
			((Said 'sing')
				(Print 0 86) ; "FEELINGS! NOTHING MORE THAN FEELINGS!!"
			)
			((Said 'delay')
				(Print 0 87) ; "Isn't that what you are doing?"
			)
			((Said 'pick')
				(Print 0 88) ; "If you pick it, it won't heal!"
			)
			((Said 'yell')
				(Print 0 89) ; "HELP!"
			)
			((Said 'rob')
				(Print 0 90) ; "That's against the law!"
			)
			((or (Said 'no') (Said 'yes'))
				(Ok) ; "O.K."
			)
			((Said 'borrow')
				(Print 0 91) ; "Who would loan you anything?"
			)
			((Said 'cheat')
				(Print 0 92) ; "Ok, you win.\05"
				(Print 0 93 #at -1 144) ; "(Game over.)"
				(= gQuit 1)
			)
			((Said '(drop<on),wear>')
				(cond
					((Said '[/!*]')
						(Print 0 94) ; "What do you want to wear?"
					)
					((= temp2 (gInventory saidMe:))
						(cond
							...
							(else
								(Print 0 95) ; "You'd look better if you didn't!"
							)
						)
					)
					(else
						(Print 0 96) ; "You can't wear what is not yours."
						(event claimed: 1)
					)
				)
			)
			((Said 'drop>')
				(cond
					((Said '[/!*]')
						(Print 0 97) ; "What do you want to drop?"
					)
					((= temp2 (gInventory saidMe:))
						(if (not (gEgo has: (gInventory indexOf: temp2)))
							(DontHave) ; "You don't have it."
						else
							(Print 0 98) ; "Nah. Better not!"
						)
					)
					(else
						(Print 0 99) ; "It's not yours to drop."
						(event claimed: 1)
					)
				)
			)
			((Said 'throw>')
				(cond
					((Said '[/!*]')
						(Print 0 100) ; "What do you want to throw?"
					)
					((= temp2 (gInventory saidMe:))
						(if (not (gEgo has: (gInventory indexOf: temp2)))
							(DontHave) ; "You don't have it."
						else
							(Print 0 101) ; "Nah. You might need it later!"
						)
					)
					(else
						(Print 0 102) ; "That doesn't belong to you."
						(event claimed: 1)
					)
				)
			)
			((Said 'smell')
				(Print 0 103) ; "Sniff, sniff."
				(cond
					((TestFlag 8)
						(Print 0 104) ; "You should have showered after that workout!"
					)
					((TestFlag 10)
						(Print 0 105) ; "You should have used soap during your shower!"
					)
					((TestFlag 62)
						(Print 0 106) ; "Your deodorant has failed!"
					)
				)
			)
			((Said 'whistle')
				(Print 0 107) ; "You whistle a happy tune."
			)
			((Said 'ask/job')
				(Print 0 108) ; ""Is there any chance I could get a job around here?" you say."
				(Print 0 109 #at -1 144) ; "(There is no response.)"
			)
			((and gEgoIsPatti (or (Said '/arnold') (Said '//arnold')))
				(Print 0 110) ; "Forget about that creep, Patti!"
			)
			((Said 'laugh')
				(Print 0 111) ; "Ha, ha!"
			)
			((Said 'eat')
				(Print 0 112) ; "Once you tasted it, you wouldn't want it!"
			)
			((Said 'lie,lie')
				(Print 0 113) ; "There's no time!"
			)
			((Said 'aid/me,self')
				(Print 0 114) ; "Stop your whining!"
			)
			((Said 'aid')
				(Print 0 115) ; "How would you help?"
			)
			((Said 'explore>')
				(if (Said '/cloth,panties,entertainer,larry')
					(Print 0 116) ; "Searching through your clothes you find..."
					(gInventory showSelf: gEgo)
				else
					(event claimed: 1)
					(Print 0 117) ; "You find nothing."
				)
			)
			((or (Said '/fuck/you') (Said 'fuck/you'))
				(Print 0 118) ; "Same to you, buddy!"
			)
			((Said 'fuck/*')
				(Print 0 119) ; "Yeah, you probably would, too!"
			)
			((or (Said 'caress/me,larry,self') (Said 'jack'))
				(Print 0 120) ; "Aren't your palms hairy enough?"
			)
			((Said 'caress,caress,embrace,look/clit')
				(Printf 0 121 gEgoName) ; "Obviously, restraint is no problem for you, %s."
			)
			((Said 'eat/woman')
				(Print 0 122) ; "That comes later."
			)
			((or (Said 'look<in/blouse') (Said 'look<up/skirt'))
				(Print 0 123) ; "You see something special."
			)
			((Said 'drink')
				(Print 0 124) ; "Nobody's THAT thirsty!"
			)
			((or (Said 'use/key') (Said 'unbolt'))
				(Print 0 125) ; "It's not that easy!"
			)
			((or (Said '/book') (Said '//book'))
				(Print 0 126) ; "It's in the box your disks came in. Have you read it?"
			)
			((Said 'listen')
				(Print 0 127) ; "If you are very quiet, you can hear your keyboard click."
			)
			((Said 'embrace')
				(Print 0 128) ; "Wouldn't you rather just remain friends?"
			)
			(
				(or
					(Said 'eat,fuck/me')
			...
			((or (Said '/hell') (Said '//hell'))
				(Print 0 130) ; "That's probably where you'll end up!"
			)
			((Said 'look>')
				(cond
					((Said '/self,larry,entertainer,me,blouse,cloth,panties')
						(if (not gEgoIsPatti)
							(Print 0 131) ; "You look pretty good, if you do say so yourself!"
						else
							(Print 0 132) ; "You look great, no matter what you wear!"
						)
					)
					((and (== (gEgo view:) 718) (Said '/sunglass'))
						(Print 0 133) ; "You're too cool!"
					)
					((Said '/ankle,sandal')
						(if (== (gEgo view:) 718)
							(Print 0 134) ; "Your toes are so ugly!"
						else
							(Print 0 135) ; "Your feet should hurt, what with all this walking!"
						)
					)
					((Said '/bush,palm')
						(Print 0 136) ; "You see many leaves."
					)
					((Said '/man,woman,couple')
						(Print 0 137) ; "Where?"
					)
					((Said '/wall,building')
						(Print 0 138) ; "Staring at the walls already?"
					)
					((Said '/carpet,down')
						(Print 0 139) ; "It just lies there, under your feet."
					)
					((Said '/air,ceiling')
						(Print 0 140) ; "It's still up there!"
					)
					((Said '/boob')
						(if gEgoIsPatti
							(Print 0 141) ; "Hey! That's Larry's line!"
						else
							(Print 0 142) ; "You really love this, don't you!"
						)
					)
					((Said '/ass')
						(if gEgoIsPatti
							(Print 0 143) ; "Tight butts drive you nuts!"
						else
							(Print 0 144) ; "Has it been 30 seconds already?"
						)
					)
					((Said '<in/cup')
						(Print 0 145) ; "This game does not encourage voyeurism!"
					)
					((= temp2 (gInventory saidMe:))
					...
					(else
						(switch (Random 42 44)
							(42
								(Print 0 146) ; "It's just as it appears."
							)
							(43
								(Print 0 147) ; "It doesn't look interesting."
							)
							(44
								(Print 0 62) ; "You see nothing special."
							)
			...
			((or (Said 'use,buy/bill,tips,buck') (Said 'buy'))
				(if (gEgo has: 6) ; A_Twenty_Dollar_Bill
					(Print 0 148) ; "There's nothing worth buying here."
				else
					(Print 0 149) ; "You're broke!"
				)
			)
			((Said 'use>')
				(= temp2 (gInventory saidMe:))
				...
				(cond
					((Said '[/!*]')
						(Print 0 150) ; "What do you want to use?"
					)
					((not temp2)
						(Print 0 57) ; "You can't do that here; at least, not now."
					)
					...
					(else
						(Print 0 151) ; "There's no need to use it here."
						(LameResponse)
					)
				)
				(event claimed: 1)
			...
			((Said 'give>')
				...
				(cond
					...
					((Said '[/!*]')
						(Print 0 153) ; "What do you want to give?"
					)
					((not temp2)
						(Print 0 154) ; "It's not yours to give."
					)
					((not (temp2 ownedBy: gEgo))
						(DontHave) ; "You don't have it."
					)
					(else
						(Print 0 155) ; "You have no reason to do that."
						(LameResponse)
					)
				)
				(event claimed: 1)
			...
			((Said 'get<down')
				(Print 0 156) ; "You get down from a duck, but not here!"
			)
			((Said 'get<up')
				(Print 0 157) ; "You already are "up for the game!""
			)
			((Said 'get>')
				(cond
					((Said '[/!*]')
					...
					((and (= temp2 (gInventory saidMe:)) (temp2 ownedBy: gEgo))
						(Print 0 159) ; "You already have it."
					)
					((== temp2 3)
						(Print 0 160) ; "There is no wood here that strikes your fancy."
					)
					(else
						(switch (Random 33 35)
							(33
								(Print 0 161) ; "It's of no use."
							)
							(34
								(Print 0 162) ; "You don't need it."
							)
							(35
								(Print 0 163) ; "There's no reason to take it."
							)
						)
						(LameResponse)
					)
			...
			((= temp2 (gInventory saidMe:))
				(if (not (temp2 ownedBy: gEgo))
					...
					(Print 0 164) ; "You can't do that now."
				)
			)
			((Said 'talk>')
				(if (Said '[/!*]')
			...
			(
				...
				(Print 0 166) ; "Ok, but I don't usually do that!"
			)
			((or (Said '//woman') (Said '/woman/') (Said '/woman'))
				(Print 0 167) ; "She doesn't seem interested in that!"
			)
		)
	)
)
