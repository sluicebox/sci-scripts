		...
		(cond
			...
			(else
				(if 1
					...
					(proc5_14 440 29 global408 global409) ; "Nice try, but solving that puzzle doesn't prove you are skilled in %s."
					(Memory memFREE global408)
					(Memory memFREE global409)
				)
				(localproc_1)
...
(instance TTDBoard of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 41) ; "You can start doing these once you become Dr. Brain's lab assistant. You'd better wait until then."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance blotter of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 42) ; "There's a little bit of left-over pizza on the blotter. You decide to leave it there."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance stapler of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 43) ; "This looks like the only way Dr. Brain can hold things together around here."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance papersOnFloor of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 44) ; "There are too many papers here to clean up right now."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance pictureWindow of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 45) ; "It feels like a TV screen."
		else
			(super doVerb: theVerb)
		)
	)
...
(instance mirror of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 46) ; "You know, this really looks like one of those things you'd find in an old house in a movie. You really think you ought to be able to move it and have the bookshelf slide into the wall, revealing a secret passage to Dr. Brain's secret basement lab."
			(proc5_9 440 47) ; "But nothing happens when you push the mirror frame. It does get you thinking about the bookshelf, though. What if there really was a way to move it?"
		else
			(super doVerb: theVerb)
		)
	)
...
(instance dart of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 440 48) ; "You'd better leave the dart alone for now. It could be part of an important scientific experiment."
		else
			(super doVerb: theVerb)
		)
	)
