...
(instance rm360 of Rm
		...
		(if
			(and
				(or
					(event claimed:)
					(!= (event type:) evVERB)
					(not ((local417 placedList:) handleEvent: event))
				)
				(== (event type:) evVERB)
			)
			(= temp1 1)
			(if (and (== (event message:) JOY_UPRIGHT) (not (IsFlag 7)))
	...
	(method (doit)
		(super doit: &rest)
		(if (GameIsRestarting)
			((local417 placedList:) eachElementDo: #show)
		)
	)
)
...
(class VObject of Obj
	...
	(method (hide &tmp temp0 temp1)
		(= temp0 (self width:))
		(= temp1 (self height:))
		(Graph grFILL_BOX y x (+ y temp1) (+ x temp0) 1 (proc0_1 198 1))
		(Graph grUPDATE_BOX y x (+ y temp1) (+ x temp0) 1)
	)
...
(class PuzPiece of VObject
	...
	(method (posn param1 param2)
		...
		(if (== self (local417 curPiece:))
			((local417 pView:) posn: x y)
		)
	)

	(method (doit param1 param2 &tmp temp0 temp1)
		(if (not ((local417 dragList:) size:))
			(-= param1 (/ (self width:) 2))
			(-= param2 (/ (self height:) 2))
		)
		(if (== self (= temp1 (local417 curPiece:)))
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		(cond
			...
			((== (event message:) JOY_RIGHT)
				...
				(local417 pickPiece: self)
				(local417 getDragList: temp4 temp5 temp6 temp7)
				(event claimed: 1)
			)
...
(class RPuzzle of Obj
	...
	(method (isAlreadySaved param1 &tmp temp0 temp1 temp2 temp3)
		...
		(= temp2 (mod temp3 16))
		(>>= temp0 temp2)
		(return (or (IsFlag 7) (& [global210 temp1] temp0)))
	)
...
(instance puzzle of RPuzzle
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 [temp8 253] temp261 [temp262 4])
		(super init: &rest)
		(if (== global170 -1)
			(= global170 global114)
		)
		...
		(= numPieces 36)
		(pView client: self)
		(= temp261 (= temp0 (= temp7 0)))
		(= temp4 0)
		(for ((= temp1 0)) (< temp1 5) ((++ temp1))
			(for ((= temp2 0)) (< temp2 8) ((++ temp2))
				(if (!= [local5 (* (= temp3 (+ (* 8 temp1) temp2)) 2)] -1)
					(= temp6 (+ temp3 temp3))
					((= temp5 (PuzPiece new:))
						x: -100
						y: -100
						...
						loop: (/ temp3 16)
						cel: (/ temp2 2)
						trueXPos: [local5 temp6]
						trueYPos: [local5 (+ temp6 1)]
						client: self
						value: temp4 species
					)
					(if (IsFlag 7)
...
(instance hardPuzzle of RPuzzle
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11 [temp12 253] temp265 [temp266 4])
		(super init: &rest)
		(if (== global170 -1)
			(= global170 global114)
		)
				...
				(for ((= temp5 0)) (< temp5 8) ((++ temp5))
					...
					(if
						...
						((= temp9 (PuzPiece new:))
							...
							loop: temp4
							cel: temp5
							trueXPos: [local93 temp10]
							trueYPos: [local93 (+ temp10 1)]
							client: self
							init:
							value: temp8
						)
...
(instance backFeat of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 7) ; "This door is labelled with the mysterious phrase "Doce Omor"."
			)
			(3
		...
		(cond
			...
			(local1
				(switch (event message:)
					(JOY_UPRIGHT
						(proc5_9 360 8) ; "You look in vain, but find nothing except a 1981 Prince Charles and Lady Diana picture calendar with popups. You decide you can do without that."
					)
					(JOY_RIGHT
						(proc5_9 360 9) ; "Sorry, but the chest has been nailed to the floor by one of Dr. Brain's paranoid former assistants."
					)
				)
			)
			((== (event message:) JOY_UPRIGHT)
				...
				(cond
					...
					((== local3 1)
						(proc5_9 360 10) ; "This chest contains a stack of puzzle pieces and your long-missing car keys."
					)
					((== local3 2)
						(proc5_9 360 11) ; "Oh, sorry, my mistake. Your car keys weren't in the box after all. You must have left them somewhere else."
					)
					(else
						(proc5_9 360 1) ; "You can get puzzle pieces by clicking the Hand on the chest. Click a piece anywhere on the screen to place it. Pick up a piece by clicking on it, or several pieces by dragging the mouse across them. You can put pieces aside by clicking them on the chest opening."
					)
			...
			((!= (event message:) JOY_RIGHT)
				(proc5_9 360 12) ; "That won't work here."
			)
			((client curPiece:)
...
(instance guitar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 13) ; "It's a beautifully hand-crafted Spanish guitar."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance unicycle of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 14) ; "You can't be a clown without a unicycle."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bowArrow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 15) ; "These would be useful on a visit to Sherwood Forest."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance roundTable of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 16) ; "This is an elemental table, borrowed from Aziza in Quest for Glory II: Trial By Fire, (Actually, this is a cheap plug for another of Corey's computer games.)"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bowlingBall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 17) ; "Dr. Brain's experiment with magnetizing his bowling ball and the headpin almost worked. But he got caught (or, rather, the pin did!)"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sword of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 18) ; "Dr. Brain bought these in Spielburg, but decided that he was more interested in being a scientist than a Hero. Of course, you can do both. (Watch out, another cheap plug coming up!). All you have to do is buy "Quest for Glory I: So You Want To Be A Hero!""
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance magTable of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 19) ; "This table features a number of false compartments and other tricks useful to the aspiring amateur stage magician."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance magHat of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 20) ; ""Here, watch me pull a rabbit out of my hat!" On the other hand, maybe you'd better leave that for the professionals."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cards of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 21) ; "On closer inspection, you discover that these cards have been marked. But why would anyone want to know which cards are "8"'s?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance ship of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 22) ; "Dr. Brain couldn't figure out how to fit this ship in a bottle."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance helmet of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 23) ; "Dr. Brain likes to wear this helmet on walks, especially when he's just Roman around."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theMask of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 24) ; "This is a porcelain theater mask."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance caneStand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 25) ; "Dr. Brain keeps his favorite walking sticks, a sword cane and a sugar cane, in this stand."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance block of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 26) ; "Dr. Brain much prefers alphabet blocks to mental blocks."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance boxingGloves of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 27) ; "In his youth, Dr. Brain contended for the heavy-weight title as "Bear-Belly Brain". He lost."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bear of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 28) ; "It's unbearably cute."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance fan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 29) ; "This is one of Dr. Brain's greatest fans."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance clown of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 30) ; "This clown doesn't seem to know which end is up."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance wand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 31) ; "Magicians use these wands to improve their spelling."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance wig of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 32) ; "This head looks wigged-out."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance scarf of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 360 33) ; "It's a rabbit. No, it's a scarf! Saves on cleaning bills."
			)
			(else
				(super doVerb: theVerb &rest)
			)
