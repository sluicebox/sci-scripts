...
(class LSL6 of Game
	...
	(method (init &tmp [temp0 3])
		...
		(= gVersion (Str new: 20))
		(version open: 1 read: gVersion 11 dispose:)
	...
	(method (play)
		...
		(if (or (IsHiRes) (Platform 2))
			...
			(= global182 (Str new: 511))
			(cond
				((not (IsFlag 97))
					(if global110
						(Message msgGET 900 7 0 0 (Random 1 15) (global182 data:))
					else
						(Message msgGET 0 7 0 0 (Random 1 15) (global182 data:))
					)
				)
				(global110
					(Message msgGET 900 7 0 0 16 (global182 data:))
				)
				(else
			...
			(if global110
				(Message msgGET 900 13 0 0 1 (global182 data:))
			else
	...
	(method (restart &tmp temp0)
		...
		(if
			...
			(UpdatePlane (gThePlane setRect: 0 0 320 200 drawPic: -1 yourself:))
	...
	(method (save param1 &tmp temp0 temp1 temp2)
		...
		(cond
				...
				(if (or (gCurRoom script:) isHandsOff)
					...
					(switch
						(Print
							...
							addButton: 0 8 0 3 1 (proc79_15 90 85 95 85 85) 33 0 ; "Screw It"
			...
			((and argc (== param1 0) (!= global176 -1))
				(gCurSaveDir copy: global190)
				(if (not (ValidPath (KString 9 gCurSaveDir))) ; StrGetData
					(= temp0 (Str new:))
					(= temp1 (Str new:))
					(Message msgGET 0 12 0 16 1 (temp1 data:)) ; "Invalid save game directory:"
					(temp0 format: temp1 global190)
					(Print addText: temp0 addButtonBM: -1 0 0 1 {OK} 0 30 init:)
					(temp0 dispose:)
					(temp1 dispose:)
					(return)
				)
				(gGame setCursor: gWaitCursor)
				(if (= temp2 (gTextScroller isVisible:))
					(gTextScroller hide:)
					(= global210 1)
				)
				...
				(if
					...
					(gTextScroller
						kWindow:
							(ScrollWindow 0 gTextScroller (gTextScroller maxItems:)) ; ScrollCreate
					)
					(ScrollWindow 19 (gTextScroller kWindow:) global258) ; ScrollReconstruct
					(ScrollWindow 8 (gTextScroller kWindow:)) ; ScrollEnd
					(if temp2
					...
					(= temp0 (Str new:))
					(Message msgGET 0 12 0 0 1 (temp0 data:)) ; "Your game was saved successfully as "%s.""
