	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gMessager sayRange: 1 25 0 1 4 self) ; "Here's your batteries, Char. But, I might mention I had to go through a lot of trouble just to find them."
			)
			(2
				(gGame changeScore: 15 204)
				(if (IsFlag 203)
					(gMessager sayRange: 1 25 6 1 3 self) ; "Good idea. I've already opened the door for us. Come on, Char. Let's go right now."
				else
					(gMessager sayRange: 1 25 0 5 6 self) ; "But, don't they keep that door locked?"
				)
			)
			(3
				(gCurRoom newRoom: 400)
