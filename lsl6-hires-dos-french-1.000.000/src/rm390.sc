...
(instance rm390 of LarryRoom
	...
	(method (init)
		...
		(if (or (!= gPrevRoomNum 320) (not (gGlobalSound2 handle:)))
			(gGlobalSound2 number: 320 loop: -1 play: setVol: 90)
		)
		(roseEyes init:)
		(breasts init:)
		(hair init:)
		...
		((ScriptID 0 11) init: rmCue) ; iconExit
		(fan init:)
		(lubricant init:)
		(pipe init:)
...
(instance fan of Feature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(cond
			((Message msgGET 390 noun theVerb 0 1 (temp0 data:))
				(gMessager say: 11 theVerb)
			)
			((Message msgGET 320 2 theVerb 0 1 (temp0 data:))
...
(instance lubricant of Feature
	...
	(method (doVerb theVerb &tmp temp0 temp1)
		(if (and (== theVerb 4) (not (-- local2))) ; Do
			(gEgo get: 16) ; flowers
			(SetFlag 99)
			...
			(if (Message msgGET 320 noun theVerb 0 1 (temp1 data:))
