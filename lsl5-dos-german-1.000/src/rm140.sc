	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(24
				...
				(TPrint 140 11 67 0 147 80 {A Yes Man}) ; "Isn't that what we wanted, Mr. Scruemall?"
				(TPrint 140 12 67 68 91 80 {A Yes Man}) ; "Bravo, Mr. Scruemall!"
				(= seconds 3)
			)
			(25
				(Say Silas_Scruemall 140 14 108 139 self) ; "I've changed my mind! Now I think we should hold a blind audition, where the women don't know they're being auditioned!"
			)
			(26
				(= ticks 60)
			)
			(27
				...
				(TPrint 140 11 67 180 113 80 {A Yes Man}) ; "Isn't that what we wanted, Mr. Scruemall?"
				(TPrint 140 12 67 180 147 80 {A Yes Man}) ; "Bravo, Mr. Scruemall!"
				(= seconds 3)
			)
			(28
				(Say Scooter 140 15 108 139 self) ; "But (with all due respects, Mr. Scruemall) that's impossible. We need videotapes of their auditions for our own, uh, personal review purposes. There's no way we could follow the guy around without her seeing us!"
			)
			(29
				(fScruemall setCel: 1)
				(= seconds 3)
			...
			(30
				(Say Silas_Scruemall 140 16 108 139 self) ; "Well, yes, we do need those tapes to review. But what if we could find an auditioner so inept, so sexless, so lame, so unappealing, that it would take the sexiest woman in America to find him attractive?"
			)
			(31
				(= ticks 60)
			)
			(32
				(TPrint 140 17 67 0 147 80 {A Yes Man}) ; "Great idea, Mr. Scruemall!"
				(TPrint 140 18 67 68 91 80 {A Yes Man}) ; "Of course, Mr. Scruemall!"
				(TPrint 140 19 67 105 137 80 {A Yes Man}) ; "Masterful stroke, Mr. Scruemall!"
				(fScruemall setCel: 0)
				(= seconds 4)
			)
			(33
				(Say Scooter 140 20 108 139 self) ; "But, Sil--, er, Mr. Scruemall! How will we ever find somebody so lame?"
			)
			(34
				(fScruemall setCel: 0)
				(= ticks 150)
...
(instance sLarryCartoon of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Say Silas_Scruemall 140 22 108 139 self) ; "You may begin with me, boy."
			)
			(3
				(TPrint 140 23 67 -1 185) ; "(So you decide to do exactly that!)"
