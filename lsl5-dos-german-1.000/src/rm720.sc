			...
			(3 ; Do
				(if dealOK
					(cond
						...
						(else
							(TPrint 720 9) ; "You must bet something!"
						)
					)
				)
			)
			...
			(3 ; Do
				...
				(if (and dealOK (not (& ((gTheIconBar at: 8) signal:) $0004)))
					...
					(if credits
						...
						(TPrint 720 10) ; "You take your money from the tray."
					...
					(TPrint 720 11) ; "First, finish this hand."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
			...
			(3 ; Do
				(if dealOK
					...
					(if theBet
						...
						(TPrint 720 12) ; "You have no money and pretend to play the game, assuming that the other gamblers standing around you won't notice. You win zero dollars and have zero fun!"
					)
				)
			)
			(else
	...
	(method (changeState newState &tmp i [str 50])
		(switch (= state newState)
			...
			(8
				...
				(TallyHand)
				(if (== theHand 9)
					(Format @str 720 13 gPokerJackpot gPokerJackpot) ; "P R O G R E S S I V E J A C K P O T  ! !  You win %d dollars!"
					(TPrint
						@str
						#width
						200
			...
			(10
				...
				(if credits
					...
					(TPrint 720 14) ; "You sadly walk away, your turned-out pockets telling everyone of your gambling "problem.""
					(gTheIconBar enable: 8)
					(SetupExit 0)
					(gCurRoom newRoom: 710)
				)
