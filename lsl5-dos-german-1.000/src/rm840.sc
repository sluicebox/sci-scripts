...
(instance opener of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 31) ; "There's a needle-sharp letter opener lying on Krapper's desk."
			)
			(3 ; Do
				(TPrint 840 5) ; "Never knowing when you might get some mail, you decide to take Krapper's letter opener from his desk."
				(opener hide:)
...
(instance peekScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 32) ; "Opening the doorway to carefully peek outside, you notice the receptionist has returned. You decide you cannot exit the room that way!"
				(= seconds 3)
			)
			(3
				(lobbyDoor setCel: 0)
...
(instance getKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 10 148)
				(TPrint 840 33 #at -1 28) ; "Bending over at the base of the tree, you dig into the humus and find the metallic object that you saw earlier. It looks like a desk key."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance sPawThruPlant of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 840 34) ; "Pawing through the humus in the tree's pot, you see a glint of metal."
				(SetFlag 23) ; fSeenGlint
				(HandsOn)
				(self dispose:)
			)
...
(instance replaceKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 35 #at -1 28) ; "You carefully return the key to its hiding place in the humus."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance sOpenDesk2ndTime of Script
	...
	(method (changeState newState &tmp [str 80])
		(switch (= state newState)
			...
			(4
				(Format @str 840 37 gRoomBNumber gRoomBNumber) ; "Where's that little blue stickum? Oh, here it is! %u"
				(TPrint @str)
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
...
(instance pickLockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 5 147)
				(TPrint 840 38) ; "Prying away at the desk's inefficient lock with Krapper's letter opener, you find a weak spot and carefully pry the drawer open."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance unlockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 13 147)
				(TPrint 840 39) ; "Trying the key you found buried in the tree's pot, you insert into the desk and turn it carefully. It opens."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance lockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 40) ; "You lock the desk."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance getPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Points 5 145)
				(TPrint 840 41) ; "Searching the desk drawer, you find the folders marked "Personal.""
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance lookPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 840 42) ; "Carefully searching through the many drawers of Krapper's desk, you find a folder marked "Personal.""
				(= ticks 60)
			)
			(2
				(TPrint 840 43) ; "For a moment, you wonder if you should take it."
				(= ticks 60)
			)
			(3
				(gEgo
			...
			(4
				(Points 5 145)
				(TPrint 840 44) ; "But of course you do!"
				(= ticks 123)
			)
			(5
				(gEgo
...
(instance putPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(TPrint 840 45) ; "You return the folder of evidence to Krapper's desk, and close his desk drawer, being careful to leave everything exactly as you found it."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance runCopierScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 12 151)
				(TPrint 840 46) ; "You place John Krapper's personal papers in his copier, and press the "Start" button."
				(= ticks 123)
			)
			(3
			...
			(8
				(Say gEgo 840 47) ; "Hey, what's the matter with this thing?" you think. "Why did it stop working? What did I do, jam it?"
				(= seconds 3)
			)
			(9
			...
			(10
				(paper dispose:)
				(copier setCel: 0)
				(Say gEgo 840 48) ; ""Oh well, at least I got the most important evidence copied before it jammed," you think, grabbing your copies from the machine."
				(= seconds 3)
			)
			(11
			...
			(12
				(Say gEgo 840 49) ; ""What does this switch here do?" you wonder, pulling on something that doesn't belong to you."
				(= ticks 123)
			)
			(13
				(TPrint 840 50 #at -1 185) ; "(An experience not wholly unfamiliar to you!)"
				(= ticks 60)
			)
			(14
				(gEgo loop: 4 setCel: 255 setCycle: Beg self)
			...
			(20
				(Say gEgo 840 51) ; "GREAT!" you think. "Now just look at me! I'm covered with toner from head to toe!"
				(copier stopUpd:)
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 842)
					(TPrint 840 52) ; "Oh no," you think, "how am I ever going to get this stuff off?"
					(return 1)
				else
					(TPrint 840 53) ; "You look yourself over. You like what you see. "Although, I could stand to lose a few pounds," you think."
					(return 1)
				)
			)
			(else
