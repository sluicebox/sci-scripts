...
(class RTRandCycle of RandCycle
	...
	(method (doit &tmp theTime)
		(if (> (- count (= theTime gGameTime)) 0)
			(if (> (Abs (- theTime cycleCnt)) (client cycleSpeed:))
...
(class Talker of Prop
	...
	(method (startText param1 param2 &tmp [temp0 350] temp350 temp351)
		(if viewInPrint
			(= temp350
				(if useFrame
					loop
			...
			(Print @temp0 #at x y #dispose #title name #width temp351)
		)
	)
