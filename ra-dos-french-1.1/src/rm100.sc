...
(procedure (localproc_0 param1 param2 &tmp temp0 [temp1 4] temp5)
	(= temp5 (if param2 global157 else global162))
	(switch param1
		(fIntro
			(TextSize @[temp1 0] @local5 61 0 0)
			(= temp0 (- 240 (/ (- [temp1 3] [temp1 1]) 2)))
			(Display @local5 dsCOORD temp0 103 dsFONT 61 dsCOLOR global151)
			(Display @local5 dsCOORD temp0 103 dsFONT 60 dsCOLOR temp5)
		)
		(fPlay
			(TextSize @[temp1 0] @local30 61 0 0)
			(= temp0 (- 240 (/ (- [temp1 3] [temp1 1]) 2)))
			(Display @local30 dsCOORD temp0 113 dsFONT 61 dsCOLOR global151)
			(Display @local30 dsCOORD temp0 113 dsFONT 60 dsCOLOR temp5)
		)
		(fContinue
			(TextSize @[temp1 0] @local55 61 0 0)
			(= temp0 (- 240 (/ (- [temp1 3] [temp1 1]) 2)))
			(Display @local55 dsCOORD temp0 123 dsFONT 61 dsCOLOR global151)
			(Display @local55 dsCOORD temp0 123 dsFONT 60 dsCOLOR temp5)
		)
		(fQuit
			(TextSize @[temp1 0] @local80 61 0 0)
			(= temp0 (- 240 (/ (- [temp1 3] [temp1 1]) 2)))
			(Display @local80 dsCOORD temp0 133 dsFONT 61 dsCOLOR global151)
			(Display @local80 dsCOORD temp0 133 dsFONT 60 dsCOLOR temp5)
		)
	)
)
...
(instance rm100 of LBRoom
	...
	(method (init)
		...
		(super init:)
		(Message msgGET 100 1 0 0 1 @local5) ; "INTRODUCTION"
		(Message msgGET 100 2 0 0 1 @local30) ; "PLAY GAME"
		(Message msgGET 100 4 0 0 1 @local80) ; "QUIT"
		(Message msgGET 100 3 0 0 1 @local55) ; "CONTINUE SAVED GAME"
		(gTheIconBar disable:)
