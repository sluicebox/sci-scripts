...
(instance carSound of Sound ; UNUSED
	(properties)
)
		...
		(switch (= state newState)
			...
			(19
				(if local17
					...
					(Print 60 12 #at -1 50) ; "Don't leave me here." says the diver, "Just standby 'till I come out of the river."
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					((Said 'change,japanesechange[/cloth]')
					...
					(
						(and
							local15
							(Said
								'find,get,show,frisk,look/card,certificate,licence'
							)
						)
					...
					((Said 'apprehend,chase/bains')
						(if (and (== global111 3) global183 local18)
					...
					((Said 'look,hit,get,talk/bains,suspect')
						(cond
							...
							((and (== global111 3) global183 local18)
					...
					(
						(or
							(Said 'open/door,japanesedoor,van')
					...
					(
						...
						(if (> global189 5)
							(if (< (gEgo distanceTo: local13) 40)
								...
								(Print 60 63) ; "Officer Moore can't hear you. You'd better get closer."
							)
						else
							(Print 60 64) ; "That's a fine idea. But how?"
						)
					)
				)
			)
...
(instance bainsScript of Script
	...
	(method (doit)
		...
		(if global205
			(if (== local10 0)
				(if local7
					(Print 60 65 #at -1 24) ; "Although shaking from surprise, confusion, and fear, you somehow manage to return fire!"
				)
				(if (and global183 (or (not global166) (!= global205 4)))
					(Print 60 66 #at -1 20) ; "Unfortunately, however, the flight of your bullet misses its target, and the killer continues his ambush!"
					(= local10 1)
				else
					(= local10 2)
					(= global208
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(bainsShot priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 14) play:)
			...
			(5
				...
				(cond
					(local7
						(Print 60 67 #at -1 20) ; "Although the flight of your bullet misses its target, it comes close enough to cause the suspect to take leg bail."
					)
					(global183
			...
			(7
				(Print 60 70 #at -1 20) ; "Just before taking a hot round in the chest, you hear Bains yell... "This one's for you, PIG!!!""
				(self changeState: 4)
			)
			(8
				(= seconds 2)
