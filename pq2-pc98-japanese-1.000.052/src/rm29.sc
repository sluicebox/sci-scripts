		...
		(13
			(cond
				(
					...
					(localproc_0 29 49) ; "You're not close enough to your car's door."
				)
				(global133
					(localproc_0 29 50) ; "Your car door is locked"
					(return 0)
				)
				(global135
					(localproc_0 29 51) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(localproc_0 29 52) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
...
(instance rm29Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo view: 0 posn: 0 -100 init: setMotion: 0)
				(if (== global131 13)
				...
				(cond
					...
					((Said 'read/sign')
						(if (gEgo inRect: 160 105 215 128)
							(localproc_0 29 9) ; "Visa. Mastercard."
							(if local16
								(localproc_0 29 25) ; "Closed."
							else
								(localproc_0 29 26) ; "Open."
							)
						else
							(localproc_0 29 12) ; "The restaurant is called "Arnie's"."
						)
						...
						(if (gEgo inRect: 79 174 116 187)
							(if global135
								(if (gEgo has: 10) ; field_kit
									(localproc_0 29 27) ; "You place your field kit inside the trunk."
									...
									(localproc_0 29 28) ; "You don't have the field kit."
								)
							else
								(localproc_0 29 29) ; "Open the trunk first."
							)
						else
							(localproc_0 29 30) ; "You're not close enough to your car's trunk."
						)
					)
						...
						(if (gEgo inRect: 79 174 116 187)
							(if global135
								(if (IsItemAt 10 13) ; field_kit
									(localproc_0 29 31) ; "You take your field kit from the trunk."
									(gEgo get: 10) ; field_kit
								else
									(localproc_0 29 32) ; "The field kit is not in the trunk."
								)
							else
								(localproc_0 29 29) ; "Open the trunk first."
							)
					...
					((or (Said 'open/door,japanesedoor') (Said 'get<in'))
						(cond
							...
							(local16
								(localproc_0 29 33) ; "Can't you read? The restaurant's closed!"
							)
							(else
								(localproc_0 29 34) ; "Just walk on in!"
							)
						)
					)
						...
						(if
							...
							(cond
								((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
									(if global133
										(= global133 0)
										(localproc_0 29 35) ; "OK. It's unlocked."
									else
										(localproc_0 29 36) ; "The door is already unlocked."
									)
								)
								((== global131 13)
									(localproc_0 29 37) ; "You need a key to unlock this door."
								)
							)
							(cond
								((and (== global131 33) (gEgo has: 2)) ; key_ring
									(if global136
										(= global136 0)
										(localproc_0 29 38) ; "OK. It's unlocked."
									else
										(localproc_0 29 36) ; "The door is already unlocked."
									)
								)
							...
							(localproc_0 29 39) ; "You're not close enough."
						)
					)
					((Said 'lock/door,japanesedoor,auto')
						(if
							...
							(if (== global131 13)
								(if (not global133)
									(= global133 1)
									(localproc_0 29 40) ; "OK. It's locked."
								else
									(localproc_0 29 41) ; "The door is already locked."
								)
							)
							(if (== global131 33)
								(if (not global136)
							...
							(localproc_0 29 42) ; "You're not close enough."
						)
					)
					((Said 'open,unlock/trunk')
						(if (== global131 13)
							(if (gEgo inRect: 79 174 116 187)
								(cond
									(global135
										(Print 29 43) ; "It's already open."
									)
									((gEgo has: 3) ; unmarked_car_keys
										(carScript changeState: 7)
									)
									(else
										(localproc_0 29 44) ; "You need a key to open this trunk."
									)
								)
							else
							...
							(localproc_0 29 45) ; "Your car's "hatch-back" hasn't worked since its warranty expired."
						)
					)
						...
						(if (== global131 13)
							(if (gEgo inRect: 79 174 116 187)
								(if global135
									...
									(Print 29 46) ; "It's already closed."
								)
							else
								(proc0_7) ; "You're not close enough."
							)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if (== global131 13)
					(if local16
						(localproc_0 29 47 83) ; "Gee, Sonny," says Keith. "I guess the restaurant's closed."
					else
						(localproc_0 29 48 83) ; "Go on and eat," says Keith. "Sleep sounds better than food, so I think I'll catch a quick snooze. Don't tell the Captain!"
					)
