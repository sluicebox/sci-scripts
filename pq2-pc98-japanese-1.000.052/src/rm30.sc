	...
	(method (doit)
		...
		(cond
			...
			((and (gEgo inRect: 170 111 223 120) (not local22))
				(= local22 1)
				(mariesTheme priority: (if (== (DoSound sndCHECK_DRIVER) 6) 0 else 5) play:)
				...
				(cond
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							...
							((Said '/japanesecounter')
								(localproc_0 30 5) ; "The elegant tables are made of solid oak."
							)
							((Said '/wall')
								(Print 30 6 #at -1 130) ; "There are some limited edition prints on the wall. There is a partition which separates the rear of the restaurant from the front."
							)
							(
								(or
									(Said 'japaneselookup[/!*]')
							...
							((Said '/man,crowd,customer,cafe,customer')
								(cond
									((> (gEgo x:) 105)
										(switch (Random 0 5)
											(0
												(Print 30 41 #at -1 130) ; "Thinking you're the waiter, the diner says..."I'll take the #3 medium well, with baked potato and... oh yes, a side order of mushrooms, please.""
											)
											(1
					...
					((Said 'eat,blow,fuck/couple,customer,cafe,customer,man')
						(Print 30 61 #at -1 130) ; "Golly gee!" the customer declares. "It must be one of them there preeverts that you read about in the papers. Go on, buddy! Get away! Shoo! Shoo!"
					)
					...
					(
						(or
							...
							(Said 'look,give,feed,show,get/menu[/me]')
					...
					(
						(and
							(Said 'give,feed,japanesehand>')
							(Said
								'/flower,japaneseflower,rose,plant,bouquet,carnation'
							)
						)
						(cond
		...
		(switch (= state newState)
			...
			(11
				...
				(if local37
					...
					(Print 30 106 #at 59 150 #time 6) ; "Good day sir," says the waiter..."Our specials of the day are meatloaf, prime rib, and lobster."
					(Print 30 107 #at 59 160) ; "Continuing he says, "Which would you prefer?""
					(= local35 1)
				)
			)
			(12
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(local1 setCycle: Beg)
				(local2 setCycle: Beg)
				(local10 posn: -100 0 stopUpd:)
				(local11 posn: -90 0 stopUpd:)
				(local12 posn: -80 0 stopUpd:)
				(= seconds 4)
				(cond
					((== local45 2)
						(SetScore 2)
