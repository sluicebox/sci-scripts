...
(instance sewer of Rgn
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look,japaneselookup,japaneselookdown>')
				(cond
					((Said '[<at,around][/(!*,chamber)]')
						(Print (Format @local0 205 6)) ; "You see walkways and grates that lead around the dark and smelly sewer. Pipes line the walls, and lights barely illuminate the darkness.%"
						(if
							(and
								(!= gCurRoomNum 122)
								(!= gCurRoomNum 123)
							...
							(Print 205 7) ; "Rats scurry about the sewer pipes."
						)
						(if
							(or
								(== gCurRoomNum 121)
							...
							(Print 205 8) ; "You see a drain pipe on the wall."
						)
						(if
							(or
								(== gCurRoomNum 120)
							...
							(Print 205 9) ; "A brightly-colored ladder leads to the surface."
						)
					)
					((Said '/slime')
						(Print 205 10) ; "The slime slowly seeps through the sewer."
					)
					((Said '/bridge')
						(Print 205 11) ; "The metal grate spans the sickening sewer water."
					)
					((Said '/outlet<drain')
						(if (or (== gCurRoomNum 121) (== gCurRoomNum 126))
							(Print 205 12) ; "The drain pipe looks kind of lethal."
						else
							(Print 205 13) ; "What drain pipe?"
						)
					)
					((Said '/drain')
						(if (or (== gCurRoomNum 121) (== gCurRoomNum 126))
							(Print 205 14) ; "Sewer water rushes beneath the drain into the main sewer."
						else
							(Print 205 15) ; "What drain?"
						)
					)
					((Said '/faucet')
						(if
							...
							(Print 205 16) ; "The faucet can only be turned on with a special wrench."
						else
							(Print 205 17) ; "What faucet?"
						)
					)
					((Said '/wall')
						(Print 205 18) ; "The slime and dampness on the walls make you cringe."
					)
					((Said '/bug')
						(Print 205 19) ; "You don't see any bugs here."
					)
					((Said '/outlet')
						(Print 205 20) ; "On the pipe, you see a rat scurrying about its business."
					)
					(
						(or
						...
						(Print 205 21) ; "The slimy brick walkway makes the footing difficult."
					)
					((or (Said 'japaneselookup') (Said '[<at,up][/ceiling]'))
						(Print 205 22) ; "The ceiling of the sewer doesn't look any better than the ground - slime, slime, slime."
					)
					((Said '/sewer,water')
						(Print 205 23) ; "As you look into the murky fluid, you think to yourself... "Mercy sakes, how nasty!""
					)
					((Said '/light')
						(if (== (gLightObj cel:) 3)
							(Print 205 24) ; "One light has been shot out."
						else
							(Print 205 25) ; "One light is on."
						)
						(cond
							((not (gCast contains: global115)) 0)
							((== (global115 cel:) 3)
								(Print 205 26) ; "The other light has been shot out."
							)
							(else
								(Print 205 27) ; "The other light is on."
							)
						)
					)
				)
			...
			((Said 'listen,ask')
				(Print 205 28) ; "A distant echo and the occasional sound of dripping water is all that you hear."
			)
			((Said 'smell[/*]')
				(Print 205 29) ; "The pungent odor stifles your nostrils."
			)
			((or (Said 'extender') (Said 'use/extender,extender,extender'))
				(if (gEgo has: 30) ; walkie_talkie
					(Print 205 30) ; "Your walkie talkie does not seem to be working in here."
				else
			...
			((Said 'turn,use/faucet')
				(if
					...
					(Print 205 31) ; "You can't do anything with the faucet."
				else
					(Print 205 17) ; "What faucet?"
				)
			)
			((Said 'japaneseopen/mask,japanesemask')
				(cond
					((not (gEgo has: 32)) ; gas_mask
						(Print 205 32) ; "What mask?"
					)
					((not global140)
						(Print 205 33) ; "You don't have the gas mask on."
					)
					(else
						(= global140 0)
						(if (== (gEgo view:) 296)
			...
			((Said 'wear,drop,japaneseput,japaneseturnon,use/mask,japanesemask')
				(cond
					((not (gEgo has: 32)) ; gas_mask
						(Print 205 34) ; "You don't have a mask."
					)
					((== global156 -1)
						(Print 205 35) ; "You don't need to wear the mask now."
					)
					(global140
						(Print 205 36) ; "You already are."
					)
					(else
						(= global140 1)
						(if (== (gEgo view:) 0)
	...
	(method (doit)
		...
		(if (and (> global156 0) (not global140) (not global139))
			(switch (-- global156)
				...
				(20
					(Print 205 37) ; "You feel weak and light-headed."
				)
				(40
					(Print 205 38) ; "Your breathing becomes labored, and you can't seem to get enough air."
				)
				(60
					(Print 205 39) ; "You feel a burning sensation in your eyes."
...
(instance asphyxiation of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 205 40) ; "Just before losing consciousness, while slumping to the floor, you realize..."
				(Print 205 41) ; "Oh no! It's..It's..methane gas."
				(EgoDead
