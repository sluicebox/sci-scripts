...
(procedure (localproc_1)
	(switch global131
		(13
			(cond
				((not (gEgo inRect: 230 118 250 127))
					(localproc_0 22 104) ; "You're not close enough to the door."
				)
				((== global133 1)
					(localproc_0 22 105) ; "Your car door is locked."
					(return 0)
				)
				(global135
					(localproc_0 22 106) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(localproc_0 22 107) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
		...
		(33
			(cond
				((not (gEgo inRect: 230 118 250 127))
					(localproc_0 22 108) ; "You're not close enough to the door"
				)
				(global136
					(localproc_0 22 105) ; "Your car door is locked."
				)
				...
				(cond
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							...
							((Said '[<at,around][/building,garage,chamber]')
								(localproc_0 22 4) ; "This is the underground parking garage of the new Lytton City Jail. You are filled with pride as the memories of the dedication ceremonies return."
								(if (>= global100 1)
									(localproc_0 22 5) ; "Following your heroic capture of Jessie Bains (aka: the Death Angel) you became an instant celebrity. But now, unless you recapture Bains, only the bitter memories of failure will remain..."
								)
							)
							(
								(or
							...
							((Said '/wall')
								...
								(localproc_0 22 21) ; "There appears to be a built-in video camera surveillance system and a speaker on the wall above the door. There is a button to the right of the door."
							)
							((Said '<in/locker')
								(if global165
					...
					(
						(or
							...
							(Said 'unlock,open/(door,japanesedoor<locker)')
						)
						(cond
							(global165
					...
					((Said 'show/badge,card,badge,billfold,japaneseshot')
						(if local12
							(if (gEgo inRect: 145 99 205 108)
					...
					(
						(Said
							'drop,japaneseput,drop,drop,replace,japanesereplace,throw,exit/9mm'
						)
						(cond
							((not (gEgo inRect: 135 99 163 108))
								(localproc_0 22 62) ; "You're too far away."
					...
					(
						(Said
							'drop,japaneseput,drop,drop,replace,japanesereplace,throw/key[<locker]'
						)
						(if local18
					...
					(
						(or
							...
							(Said 'close,lock/(door,japanesedoor<locker)')
						)
						(if (gEgo inRect: 135 99 163 108)
							(if global165
								(= global165 0)
					...
					((Said 'unlock/door,japanesedoor')
						(cond
								...
								(cond
									...
									((== global131 33)
										(if (gEgo has: 2) ; key_ring
											...
											(localproc_0 22 81) ; "You need a key to unlock this door."
										)
									)
								)
							)
							...
							((gEgo inRect: 230 129 260 138)
								(localproc_0 22 82) ; "If you're getting into your car, go around to the driver's side."
							)
							(else
								(localproc_0 22 45) ; "You're not close enough."
							)
						...
						(cond
							((gEgo inRect: 230 118 250 127)
								(if (== global131 13)
									(if (not global133)
										(= global133 1)
										(localproc_0 22 83) ; "OK. It's locked."
									else
										(localproc_0 22 84) ; "The door is already locked."
									)
								)
								(if (== global131 33)
									(if (not global136)
							...
							((gEgo inRect: 230 129 260 138)
								(localproc_0 22 85) ; "Go around to the driver's side."
							)
							(else
								(localproc_0 22 45) ; "You're not close enough."
							)
						...
						(cond
							((== global131 13)
								(if (gEgo inRect: 176 123 206 135)
									(cond
										(global135
											(Print 22 86) ; "It's already open."
										)
										((gEgo has: 3) ; unmarked_car_keys
											(carScript changeState: 14)
										)
										(else
											(localproc_0 22 87) ; "You need a key to open this trunk."
										)
									)
								else
									(localproc_0 22 88) ; "You're not close enough to your trunk, and you shouldn't open anybody else's trunk!"
								)
							)
							((gEgo inRect: 176 123 206 135)
								(localproc_0 22 89) ; "Your car's "hatch-back" hasn't worked since its warranty expired."
							)
							(else
								(localproc_0 22 88) ; "You're not close enough to your trunk, and you shouldn't open anybody else's trunk!"
							)
						...
						(if (== global131 13)
							(if (gEgo inRect: 176 123 206 135)
								(if global135
									...
									(Print 22 90) ; "It's already closed."
								)
							else
					...
					(
						(or
							(Said 'press,open/door,japanesedoor')
							(Said 'let/me')
							(Said 'get<in')
						)
						(cond
					...
					((Said 'close/door,japanesedoor')
						(localproc_0 22 91) ; "The door is already closed."
					)
					((Said 'drop,japaneseput,drop,drop/briefcase')
						(if (gEgo inRect: 176 123 206 135)
							(if global135
								(if (gEgo has: 10) ; field_kit
									(localproc_0 22 92) ; "You place your field kit inside the trunk."
									...
									(localproc_0 22 93) ; "You don't have the field kit."
								)
							else
								(localproc_0 22 94) ; "Open the trunk first."
							)
						else
							(localproc_0 22 95) ; "You're not close enough to your car's trunk."
						)
					)
						...
						(if (gEgo inRect: 176 123 206 135)
							(if global135
								(if (== ((gInventory at: 10) owner:) 13) ; field_kit
									(localproc_0 22 96) ; "You take your field kit from the trunk."
									(gEgo get: 10) ; field_kit
								else
									(localproc_0 22 97) ; "The field kit is not in the trunk."
								)
							else
								(localproc_0 22 94) ; "Open the trunk first."
							)
...
(instance doorScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(localproc_0 22 98) ; "After showing your ID, you hear a loud metal "clank" as the jailer releases the door locks."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (== global131 13)
					(if local7
						(localproc_0 22 99) ; "Gee, Sonny." Keith says. "I thought we were finished here."
						(localproc_0 22 100) ; "I'll just kick back in the car for a while if you have some business to take care of."
						(SetFlag 142)
					else
						(local3 dispose:)
						(global112
			...
			(8
				...
				(if (== global131 13)
					(if (IsFlag 142)
						(localproc_0 22 102) ; "Keith wakes up and says "Hi, Sonny.""
						(self changeState: 12)
					else
						(localproc_0 22 103) ; "Keith yells: "Wait up, Sonny! What's the rush, anyway?""
						(self cue:)
					)
				else
					(self changeState: 12)
