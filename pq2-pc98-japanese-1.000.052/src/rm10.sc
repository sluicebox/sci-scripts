	...
	(method (doit)
		...
		(cond
			((and (== (gEgo onControl: 1) 4096) (< state 3))
				(= state 3)
				(local0 setMotion: MoveTo 270 138 self)
			)
			((and (!= (gEgo onControl: 1) 4096) (== state 4))
				(= state 1)
				(local0 setMotion: MoveTo 242 138 self)
			)
		)
	)
				...
				(cond
					((Said 'look,check,frisk>')
						(cond
							...
							((Said '/counter,japanesecounter,bookcase,coatrack')
					...
					((or (Said 'give,feed/9mm') (Said 'show/9mm'))
						(if (== (gEgo onControl: 1) 8192)
					...
					((Said '(hand<in),book,exit,replace,give,feed/ep[<ear]')
						(cond
					...
					(
						(and
							(Said 'get,replace,japanesereplace,exchange,ask>')
							(or
								(Said '/bullet,ammo,clip,japaneseammoclip')
								(Said '//bullet,ammo,clip,japaneseammoclip')
							)
						)
						(cond
							...
							(else
								(Print 10 30) ; "Ken says, "Use up your old ammo first.""
							)
						)
					)
					(
						(or
							(Said
								'use,wear,japaneseturnon,drop,japaneseput,japaneseturnon,use/ep'
							)
							(Said 'drop,japaneseput,japaneseturnon,use[<in]/ep')
						)
						(if (gEgo has: 15) ; ear_protectors
							(Print 10 31) ; "You should put on the ear protectors when you enter the booth."
						else
							(Print 10 32) ; "You don't have them."
						)
					)
					((Said 'get,japaneseopen/ep')
						(if (== local75 0)
							(Print 10 33) ; "You're not wearing ear protectors."
						else
							(Print 10 34) ; "OK"
							(= local75 0)
						)
...
(instance boothScript of Script
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(9
				(Print 10 40 #draw) ; "You replace the old target with a new one from the shelf below the counter."
				...
				(cond
					((== temp0 KEY_F10)
						(cond
							...
							((!= (gEgo yStep:) 1)
								(Print 10 41) ; "Practice your quick-draw later, Sonny; this is TARGET practice!"
							)
							((not [global215 global207])
								(Print 10 42 #time 2) ; "CLICK!"
							)
							((> local16 6)
								(Print 10 43) ; "You had better bring the target up before it's so full of holes you can't tell them apart."
							)
							((and (!= (local14 cel:) 4) (<= (gEgo y:) 208))
								(Print 10 44) ; "No fair! ANYBODY could hit it at this distance."
							)
							((not local78)
			...
			(evSAID
				(cond
					((Said 'look[<at,around][/booth,chamber]')
						(Print 10 45) ; "A shelf beneath the table holds extra targets. The target is positioned by the buttons on the left wall."
					)
					((Said 'look/bookcase')
						(Print 10 46) ; "The shelf has extra targets on it."
					)
					((Said 'draw,give/9mm[/!*]')
						(cond
							((not (gEgo has: 0)) ; hand_gun
							...
							(global204
								(Print 10 47) ; "Your gun is raised."
							)
							(local78
								(Print 10 48) ; "Wait a while."
							)
							(else
								(= global204 1)
								(self changeState: 3)
					...
					((Said 'lower,unload/9mm')
						(cond
							...
							(else
								(Print 10 49) ; "Your gun is lowered."
							)
						)
					)
					((Said 'look/bullseye,hole')
						(cond
							((== (local14 cel:) 4)
								(Print 10 50) ; "The target is too far away."
							)
							((not local16)
								(Print 10 51) ; "The target shows the accepted "kill" areas of the torso."
							)
							(local76
								(Print 10 52) ; "You should replace the target and shoot again."
							)
							((not local15)
								(Print 10 53) ; "You missed the target completely, so you cannot tell how good your gun is sighted."
							)
							((and (<= -6 global103 6) (<= -4 global104 4))
								(Print 10 54) ; "Your sights are properly aligned."
							)
							((< global104 -4)
								(cond
									((> global103 6)
										(Print 10 55) ; "Your gun is shooting high and to the right."
									)
									((< global103 -6)
										(Print 10 56) ; "Your gun is shooting high and to the left."
									)
									(else
										(Print 10 57) ; "Your gun is shooting high."
									)
								)
							)
							((> global104 4)
								(cond
									((> global103 6)
										(Print 10 58) ; "Your gun is shooting low and to the right."
									)
									((< global103 -6)
										(Print 10 59) ; "Your gun is shooting low and to the left."
									)
									(else
										(Print 10 60) ; "Your gun is shooting low."
									)
								)
							)
							((> global103 6)
								(Print 10 61) ; "Your gun is shooting to the right."
							)
							((< global103 -6)
								(Print 10 62) ; "Your gun is shooting to the left."
							)
						)
					)
					((Said 'look/button,wall')
						(Print 10 63) ; "There are two buttons on the wall of the booth. One is labeled 'VIEW', and the other is labeled 'BACK'."
					)
					((or (Said 'press/view,(button<view)') (Said 'view[/!*]'))
						(cond
							(global204
								(Print 10 64) ; "With what? The barrel of your gun? Lower your weapon first."
							)
							(local78
								(Print 10 48) ; "Wait a while."
							)
							...
							(else
								(Print 10 65) ; "This is as close as the target gets."
							)
						)
					)
					(
						(or
							...
							(Said 'press/back,(button<back)')
							(Said 'back[/!*]')
						)
						(cond
							((== (local14 cel:) 4)
								(Print 10 66) ; "The target is as far back as it will go."
							)
							(global204
								(Print 10 67) ; "Lower your weapon first."
							)
							((and (> local16 0) local15)
								(Print 10 68) ; "You had better replace the old target with a new one first."
							)
							(local78
								(Print 10 48) ; "Wait a while."
							)
					...
					((Said 'press/button')
						(Print 10 69) ; "Which button do you want to push?"
					)
					(
						(or
							(Said
								'get,change,japanesechange,replace,japanesereplace/bullseye'
							)
							(Said '/bullseye<new')
						)
						(cond
							((== (local14 cel:) 4)
								(Print 10 70) ; "You can't change the target while it is back there."
							)
							(global204
								(Print 10 67) ; "Lower your weapon first."
							)
					...
					((Said 'adjust/bullseye')
						(Print 10 71) ; "That would make it too easy."
					)
					((Said 'adjust/sight,9mm,windage,elevation')
						(if (not global204)
							...
							(Print 10 72) ; "Lower your gun first."
						)
					)
					(
						(Said
							'exit,japaneseclimbdown,(walk<out)[/booth,chamber,range]'
						)
						(if local78
							(Print 10 48) ; "Wait a while."
						else
					...
					(
						(or
							(Said
								'(drop,japaneseput<on),wear,japaneseturnon,use/ep[<ear]'
							)
							(Said 'drop,japaneseput[<in]/ep')
						)
						(cond
							((not (gEgo has: 15)) ; ear_protectors
								(proc0_9) ; "You don't have it."
							...
							(local75
								(Print 10 73) ; "You are wearing them."
							)
							(global204
								(Print 10 67) ; "Lower your weapon first."
							)
					...
					((Said '(get<off),japaneseopen/ep[<ear]')
						(cond
							((not local75)
								(Print 10 74) ; "You are not wearing ear protectors."
							)
							((<= (gEgo y:) 236)
								(Print 10 67) ; "Lower your weapon first."
							)
