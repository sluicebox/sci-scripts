...
(instance rm225Script of Script
	...
	(method (doit)
		...
		(if (and (== local2 0) (not (gEgo onControl: 1)))
			...
			(evSAID
				(if (== (gEgo onControl: 1) 4096)
					(cond
						...
						(
							(Said
								'show[/japaneseshot,japanesepict,mugshot,shot,japaneseshot]'
							)
							(cond
				...
				(cond
					...
					(
						(or
							(Said 'break/door,japanesedoor')
							(Said 'hit/door,japanesedoor')
						)
						(if
							(or
								(== (gEgo onControl: 1) 1024)
							...
							(Print 225 33) ; "You aren't close enough to the door."
						)
					)
					((or (Said '/hello') (Said 'talk[/man,cop]'))
						(if (gEgo inRect: 180 148 288 171)
							(Print 225 34) ; "Sonny says... " How are you doing ? ""
							(if (== global131 13)
								(Print 225 35) ; "The officer says... "Hi, guys. Sorry, you can't go inside now.""
							else
								(Print 225 36) ; "The officer says... "Hi, Sonny. Sorry, you can't go inside now.""
							)
						else
							(Print 225 37) ; "You aren't close enough to talk."
						)
					)
					((or (Said 'use/key') (Said 'open/door,japanesedoor'))
						(cond
							(
								(or
									(== (gEgo onControl: 1) 1024)
								...
								(Print 225 38) ; "You can't do that. You don't have a key to this room"
							)
							(
								(gEgo
									inRect:
							...
							((gEgo inRect: 180 187 200 197)
								(Print 225 39) ; "You don't need to open this door."
							)
							(else
								(Print 225 33) ; "You aren't close enough to the door."
							)
					...
					(
						(or
							(Said 'get/out,exit')
							(Said 'exit,japaneseclimbdown/chamber')
						)
						(if (== local10 1)
							(rm225Script changeState: 6)
							(= local10 0)
						else
							(Print 225 40) ; "What do you mean "get out"? You aren't in a motel room."
						)
					)
					((Said 'enter/auto')
						(localproc_0)
						...
						(if
							...
							(cond
								((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
									(if (== global133 1)
										(= global133 0)
										(Print 225 41) ; "OK. It's unlocked."
									else
										(Print 225 42) ; "The door is already unlocked."
									)
								)
								((== global131 13)
									(Print 225 43) ; "You need a key to unlock this door."
								)
							)
							(cond
							...
							(Print 225 44) ; "You're not close enough."
						)
					)
					((Said 'lock/door,japanesedoor')
						(if
							...
							(if (== global131 13)
								(if (== global133 0)
									(= global133 1)
									(Print 225 45) ; "OK. It's locked."
								else
									(Print 225 46) ; "The door is already locked."
								)
							)
							(if (== global131 33)
								(if (== global136 0)
					...
					((or (Said 'knock') (Said 'knock/door,japanesedoor'))
						(if
							(or
								(== (gEgo onControl: 1) 1024)
								(== (gEgo onControl: 1) 512)
							...
							(Print 225 47) ; "You knock, but no one responds."
						else
							(Print 225 48) ; "You aren't close enough."
						)
					)
					((Said 'open/police')
						(if
							...
							(Print 225 49) ; "In a commanding voice you yell, "Open up in the name of the police!""
							(Print 225 50) ; "You listen carefully but are unable to hear anything."
						else
							(Print 225 28) ; "You aren't close enough to the door."
						)
					)
					((Said 'show[/badge,badge]')
						(if (gEgo inRect: 180 148 288 171)
							(if (gEgo has: 7) ; wallet
								(Print 225 51 #icon 999 0 0) ; "Yes, what can I do for you ?"
							else
								(Print 225 52) ; "You don't have your identification."
							)
						else
							(Print 225 53) ; "You aren't close enough to anybody to show your identification."
						)
					)
					((Said 'open,unlock/trunk')
						(if (== global131 13)
							(if
								...
								(cond
									(global135
										(Print 225 54) ; "It's already open."
									)
									((gEgo has: 3) ; unmarked_car_keys
										(carScript changeState: 9)
									)
									(else
										(Print 225 55) ; "You need a key to open this trunk."
									)
								)
							else
						...
						(if (== global131 13)
							(if
								...
								(if global135
									...
									(Print 225 56) ; "It's already closed."
								)
							else
								(proc0_7) ; "You're not close enough."
							)
					...
					((Said 'drop,japaneseput,location,drop/briefcase')
						(if
							...
							(if global135
								(if (gEgo has: 10) ; field_kit
									(Print 225 57) ; "You place your field kit inside the trunk."
									(PutItem 10 13) ; field_kit
								else
									(Print 225 58) ; "You don't have the field kit."
								)
							else
								(Print 225 59) ; "Open the trunk first."
							)
						else
							(Print 225 60) ; "You're not close enough to your car's trunk."
						)
					)
						...
						(if
							...
							(if global135
								(if (IsItemAt 10 13) ; field_kit
									(Print 225 61) ; "You take your field kit from the trunk."
									(gEgo get: 10) ; field_kit
								else
									(Print 225 62) ; "The field kit is not in the trunk."
								)
							else
								(Print 225 59) ; "Open the trunk first."
							)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(Print 225 63 #draw) ; "What're we doing here, Sonny?"
			)
			(4
				(HandsOff)
				(if (== global131 13)
					(Print 225 64) ; "Keith yells: "Geez, Sonny! Hold on a second, will ya?""
					(cond
						((> (global112 y:) 189)
							(global112
								ignoreActors:
