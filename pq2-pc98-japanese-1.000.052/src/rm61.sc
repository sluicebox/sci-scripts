	...
	(switch global131
		(13
			(if (not (gEgo inRect: 144 188 166 197))
				(Print 61 118) ; "You're not close enough to your car's door."
			else
				(if (and (== global100 5) (not global183))
					(gEgo setMotion: MoveTo 125 188)
					(return (Print 61 119)) ; "Hey, Sonny!" Keith shouts, "We can't leave yet. We haven't checked everything out."
				)
				(cond
					(global133
						(Print 61 120) ; "Your car door is locked"
						(return 0)
					)
					(global135
						(Print 61 121) ; "Close the trunk first."
						(return 0)
					)
					((gEgo has: 10) ; field_kit
						(Print 61 122) ; "You should put your field kit in the trunk first."
						(return 0)
					)
					(else
...
(instance rm61 of Rm
	...
	(method (init)
		...
		(self setLocales: 155)
		(gAddToPics add: grass1 grass2 grass3 grass4)
		(if (== gPrevRoomNum 60)
			(gEgo observeBlocks: ourCarBlock)
		)
		(if (>= local4 3)
			(gEgo observeBlocks: bwCarBlock)
			(gAddToPics add: policeCruiser)
			(gelepsi
				view: 48
				loop: 2
				posn: 190 120
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'japaneselookthru,(look<through)/fence,hole')
					...
					((Said 'smell/garbage,shit,garbage')
						(Print 61 35) ; "Yes, you can smell the swamp."
					)
					((Said 'drop,japaneseput,location,drop,replace/briefcase')
					...
					(
						(or
							(Said 'use,dial/phone')
							(Said 'drop,japaneseput/dime,cash[/phone]')
							(Said 'hang')
						)
					...
					((Said 'draw,move,get/board,plank,wood')
						(if (gEgo inRect: 120 145 196 170)
							(Print 61 22) ; "Dead grass and bugs; believe me - those are the only things under there."
						else
							(Print 61 49) ; "You're too far away to move it."
						)
					)
					(
						(or
					...
					((Said 'unlock/door,japanesedoor')
						(if (gEgo inRect: 144 188 166 197)
							(cond
								((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
									(if (== global133 1)
										(= global133 0)
										(Print 61 50) ; "OK. It's unlocked."
									else
										(Print 61 51) ; "The door is already unlocked."
									)
								)
								((== global131 13)
									(Print 61 52) ; "You need a key to unlock this door."
								)
							)
							(cond
					...
					((Said 'lock/door,japanesedoor')
						(if (gEgo inRect: 144 188 166 197)
							(if (== global131 13)
								(if (== global133 0)
									(= global133 1)
									(Print 61 53) ; "OK. It's locked."
								else
									(Print 61 54) ; "The door is already locked."
								)
							)
							(if (== global131 33)
								(if (== global136 0)
					...
					((Said 'open,unlock/trunk')
						(if (== global131 13)
							(if (gEgo inRect: 93 193 129 208)
								(cond
									(global135
										(Print 61 55) ; "It's already open."
									)
									((gEgo has: 3) ; unmarked_car_keys
										(carScript changeState: 13)
									)
									(else
										(Print 61 56) ; "You need a key to open this trunk."
									)
								)
							else
								(proc0_7) ; "You're not close enough."
						...
						(if (== global131 13)
							(if (gEgo inRect: 93 193 129 208)
								(if global135
									...
									(Print 61 57) ; "It's already closed."
								)
							else
								(proc0_7) ; "You're not close enough."
							)
					...
					((Said 'calm,calm[/woman]')
						(if (gCast contains: barbie)
							(Print 61 58) ; "You try to calm the witness, but she's just too excited."
						else
							(Print 61 59) ; "She's no where around."
						)
					)
					((Said 'call/coroner')
						(cond
							((not (gCast contains: gelepsi))
								(Print 61 60) ; "There is no reason to call the coroner."
							)
							((and global184 (not (IsFlag 54)))
								(Print 61 61) ; ""Ok'a, Sonny, I'lla call the coroner for you," responds officer Gelepsi."
								(SetScore 3 54)
							)
							((< (gEgo distanceTo: gelepsi) 35)
								(Print 61 62) ; "I like talkin', Sonny," grunts Officer Gelepsi, "but shouldn't you be getting on with the investigation?"
							)
							(else
								(Print 61 63) ; "Officer Gelepsi is a little hard of hearing. Better get closer."
							)
						)
					)
					((Said 'talk>')
										...
										(if (== global131 13)
											(cond
												(
													...
													(if global183
														(Print 61 64) ; "Officer Gelepsi and Keith are busy discussing the details of the shoot-out."
													else
														(Print 61 65) ; "Officer Gelepsi is busy talking to Keith. Listening, you hear some good gossip, but nothing relating to the task at hand."
													)
												)
												(
													(and
													...
													(Print 61 66) ; ""Ok'a, Sonny, I'lla call the coroner for you," responds officer Gelepsi."
													(SetScore 3 54)
												)
												(else
													(Print 61 62) ; "I like talkin', Sonny," grunts Officer Gelepsi, "but shouldn't you be getting on with the investigation?"
											...
											(Print 61 67) ; ""Hey, Sonny, where's your partner? And what are you doing driving your personal car?" Officer Gelepsi asks."
										)
									else
										(Print 61 63) ; "Officer Gelepsi is a little hard of hearing. Better get closer."
									)
							...
							((Said '/friend')
								(if (gCast contains: global112)
									(if (< (gEgo distanceTo: global112) 45)
										(Print 61 68) ; "Keith says "I'm with ya, Sonny.""
									else
										(Print 61 69) ; "Get closer."
									)
								else
									(Print 61 70) ; "Keith is not here."
								)
							)
							((Said '/woman')
								(if
									...
									(if local5
										(localproc_1 61 71) ; "(You could talk all day, but she's a witness - so ASK her something!)"
									else
										(= local5 1)
										(Print 61 72) ; "Oh, Officer! It was terrible. There was... oh... BLOOD and... and... drag marks to the river!"
									)
								else
									(Print 61 73) ; "She can't hear you."
								)
							)
							(else
								(Print 61 74) ; "Who are you talking to?"
								(event claimed: 1)
							)
						)
					)
					((Said 'close/door,japanesedoor[<booth]')
						(Print 61 75) ; "The phone booth's door is broken and can't be closed."
					)
					((Said '/briefcase')
						(Print 61 76) ; "You don't need to use your field kit here."
					)
					((gCast contains: barbie)
						(cond
							((Said '[talk]/hello')
								(if (> (gEgo distanceTo: barbie) 38)
									(Print 61 77) ; "You can't seem to attract her attention from this distance."
								else
							...
							((Said 'interrogate,ask,get>')
								(cond
									((Said '/japaneseshot,phone,booth,tree')
										(proc0_10) ; "You can't do that now."
									)
									((> (gEgo distanceTo: barbie) 38)
										(event claimed: 1)
										(Print 61 78) ; "You're too far away for her to hear you."
									)
									((Said '/badge,licence[<woman]')
									...
									(
										...
										(Print 61 79) ; "You're kinda cute, but I don't date cops."
									)
									((Said '/name[<woman]')
									...
									(
										...
										(if local6
											(Print 61 80) ; ""I've told you all I know, Officer," she stammers."
										else
											(= local6 1)
											(SetScore 2)
											(barbieScript changeState: 5)
											(localproc_0 61 81) ; "The girl tells you the location..."Just walk upriver about a hundred yards or so. It's right next to the water!""
										)
									)
									(else
										(Print 61 82) ; ""I saw blood... and drag marks..." she cries."
										(event claimed: 1)
									)
								)
							)
					...
					((Said 'ask/')
						(Print 61 83) ; "You listen carefully, but nobody answers you."
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(switch local4
					(1
						(Print 61 84 #draw) ; "What are we doing back here, Sonny?"
					)
					(2
						(Print 61 85 #at -1 60 #draw) ; "Keith looks at you accusingly; "We're too late, Sonny! I told you we should've come here right away. Captain Fletcher is gonna chew us out now!""
					)
					(0
						(Print 61 86 #at -1 60 #draw) ; "What are we doing here?" Keith asks. "Are we going to meet someone?"
					)
				)
			)
			(5
			...
			(7
				...
				(if (== global131 13)
					(if global186
						(Print 61 87) ; "Keith says..."I don't think we can catch him Sonny! He's probably long gone.""
					else
						(Print 61 88) ; "Keith yells: "Wait up, Sonny! What's the rush, anyway?""
					)
			...
			(9
				(if (gCast contains: gelepsi)
					(cond
						(global186
							(Print 61 89) ; "Keith tells Officer Gelepsi..."Mario, stand by and secure this area until we return!.""
						)
						(global184
							(Print 61 90) ; "I'lla stick 'round here an'a clean things up," Mario tells you. "See you'a later."
						)
						(else
							(Print 61 90) ; "I'lla stick 'round here an'a clean things up," Mario tells you. "See you'a later."
						)
		...
		(switch (= state newState)
			...
			(2
				...
				(if local3
					(if (and global183 (not (IsFlag 30)))
						...
						(Print 61 91) ; ""M-my gosh'a, S-S-Sonny! I heard'a the GUNSHOTS!" stammers Officer Gelepsi."
					else
						(Print 61 92 #at 40 24) ; ""What's up, Sonny? I think'a you'a doin' fine." says Officer Gelepsi."
					)
				else
			...
			(3
				...
				(cond
					((== local4 3)
						(if (IsFlag 53)
							(Print 61 93) ; ""What'sa up, guys?" Mario asks."
						else
							(if (IsFlag 114)
								(++ global107)
								(localproc_0 61 94) ; "As the car pulls up, Keith winces..."Oh no! I can see from here it's a gal... You got me again, partner. Here's your buck.""
							else
								(localproc_0 61 95) ; "As the car pulls up, Keith says..."Well Sonny, you should have taken the bet. I can see from here it's a gal.""
							)
							(Print 61 96 #at -1 24 #draw) ; "Hi'ya Sonny," Officer Gelepsi says, "The lady over there isa pretty shook up over what she'sa found. You'da better have a talk with her."
							(SetFlag 53)
							(= seconds 2)
						)
					)
					((and (== local4 4) (not local1))
						(= global186 0)
						(localproc_0 61 97) ; "I wasa listening ona the radio, Sonny," Officer Gelepsi grunts. "Too bad you lost that cocka-de-roacha."
					)
					((== local4 4)
						(localproc_0 61 98) ; "Officer Gelepsi says, "Donn'a worry, Sonny. They'lla catch that cocka-de-roacha.""
					)
				)
			)
			(4
				(localproc_0 61 99) ; "Officer Gelepsi continues... "I'lla stick around for awhile, just in case you need me.""
			)
			(5
...
(instance barbieScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if local7
					(localproc_0 61 100) ; "Hello AGAIN! You sure are friendly."
				else
					(localproc_0 61 101) ; "Hello"
					(= local7 1)
				)
			)
			(2
				(if local8
					(localproc_0 61 102) ; "You have a short memory. Maybe you'd better write this down."
				else
					(= local8 1)
				)
				(localproc_0 61 103) ; "Shelly Wingate"
			)
			(3
				(if local9
					(localproc_0 61 102) ; "You have a short memory. Maybe you'd better write this down."
				...
				(localproc_0 61 104) ; "108 Tenth Street"
			)
			(4
				(if local10
					(Print 61 105) ; "All right, buster, THAT does it! I'm leaving. YOU find that stuff WITHOUT ME!"
					(self changeState: 7)
				else
					(= local10 1)
					(switch (Random 1 4)
						(1
							(Print 61 106) ; "I've always heard such nice things about policemen. But now I see where the nick-name 'pig' comes from."
						)
						(2
							(Print 61 107) ; "Why you filthy little sniveling creep!"
						)
						(3
							(Print 61 108) ; "If I weren't a LADY, I'd...!"
						)
						(4
							(Print 61 109) ; "Why you slimy little SQUID!"
						)
					)
				)
			)
			(5
				(localproc_0 61 110) ; "In an excited voice the woman says... "I was jogging along the river's edge, and...and I came across something that looked like blood...""
				(localproc_0 61 111) ; "Continuing, she says, "I stopped to look, and right next to the blood, I saw marks that looked like something had been dragged into the river. And... and footprints -- I saw footprints!""
				(localproc_0 61 112) ; "You attempt to interrupt, but she keeps rolling it out as she says... "I got scared when I heard the brush crack, so I ran down here and called the Police.""
				(localproc_0 61 113) ; "The jogger says... "Officer, please go look! I hope nothing bad has happened.""
			)
			(6
				(localproc_0 61 114) ; "The girl answers..."Not me! I'm not going back there!" she says, "I'll tell you where it is, but I'm not going!""
				(localproc_0 61 81) ; "The girl tells you the location..."Just walk upriver about a hundred yards or so. It's right next to the water!""
				(= local6 1)
			)
			(7
			...
			(9
				(if global183
					(localproc_0 61 115) ; "The jogger, frightened by the gunfire, runs away."
				)
		...
		(switch (= state newState)
			...
			(1
				...
				(if (and (not local1) (== gPrevRoomNum 13))
					(Print 61 116 #draw) ; "Keith says... "I'm right behind you partner.""
				)
			)
			(2
			...
			(3
				(if (< global189 3)
					(Print 61 117) ; ""We've radioed for the diver, Sonny, and he'll be here shortly," Keith volunteers."
				)
			)
		)
	)
