		...
		(switch (event type:)
			...
			(evSAID
				(cond
					(
						(or
							(Said 'open[/booth,bathroom,door,japanesedoor]')
							(and (not local11) (Said 'enter[/booth,bathroom]'))
							(and
								local11
								(or
									(Said 'exit[/booth,bathroom]')
									(Said 'get<out')
									(Said 'exit')
								)
							)
						...
						(cond
							((and (not local11) global204)
								(Print 19 4) ; "Put away your gun first."
							)
							...
							(else
								(localproc_0 19 5) ; "You aren't close enough."
							)
						)
					)
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							((Said '/dirt')
								(localproc_0 19 6) ; "The ground is outside. In the new, modern Lytton Airport, they've put in floors."
							)
							((Said '/draw')
								(if local11
									(localproc_0 19 7) ; "If this was an "R" rated program like "Leisure Suit Larry", we might show it to you, but....!"
								else
									(localproc_0 19 8) ; "There are no bathroom drawings out here."
								)
							)
							((Said '/wall')
								(if local11
									(localproc_0 19 9) ; "The stall is furnished with a stool for your sitting pleasure and toilet paper for your comfort."
									(localproc_0 19 10) ; "There seems to be some graffiti scrawled on the wall of the stall... You can just barely read it."
								else
									(localproc_0 19 11) ; "On the walls, you see a paper towel dispenser, an electric hand dryer, a mirror, and a vent mounted above the stalls."
								)
							)
							((Said '/water')
								(if local11
									(localproc_0 19 12) ; "The toilet water is unappetizing."
								else
									(localproc_0 19 13) ; "Find some, first."
								)
							)
							((Said '/vent,fan')
								(localproc_0 19 14) ; "This vent is equipped with an electric fan, designed to remove offensive odors."
							)
							((Said '/drain')
								(if (== (floodScript state:) 2)
									(localproc_0 19 15) ; "It appears to be doing its job well."
								else
									(localproc_0 19 16) ; "You look in the drain and see nothing important."
								)
							)
							((Said '/man,gentleman,marshal')
								(if (gCast contains: local0)
									(localproc_0 19 17) ; "He appears to be a rather tall gentleman."
								else
									(localproc_0 19 18) ; "Huh?"
								)
							)
							((Said '<below/basin')
								(localproc_0 19 19) ; "There is a cabinet which hides the pipes."
							)
							((Said '/cabinet')
								(localproc_0 19 20) ; "It's mostly for decoration."
							)
							((Said '/outlet')
								(localproc_0 19 21) ; "They're hidden."
							)
							((Said '/door,japanesedoor')
								(localproc_0 19 22) ; "The stall doors are uniformly uninteresting."
							)
							((Said '/basin')
								(if local11
									(localproc_0 19 23) ; "You'll have to leave the stall to get a good look at it."
								else
									(localproc_0 19 24) ; "It's handy for washing the hands. The soap dispensers are empty, however."
								)
							)
							((Said '/bracket<soap')
								(localproc_0 19 25) ; "Empty."
							)
							(
								(or
									(Said '/bracket')
									(Said '/bracket,machine[<towel]')
									(Said '/bracket,machine<towel<japanesehair')
								)
								(if local11
									(localproc_0 19 23) ; "You'll have to leave the stall to get a good look at it."
								else
									(localproc_0 19 26) ; "It's shiny, and it DOES have paper towels in it."
								)
							)
							((Said '/dryer')
								(if local11
									...
									(localproc_0 19 27) ; "It's shiny. You wonder who invented the concept."
								)
							)
							(
								(or
								...
								(localproc_0 19 28) ; "You look up and see the usual bathroom ceiling with flourescent lighting fixtures."
							)
							(
								(or
								...
								(if local11
									(localproc_0 19 29) ; "You see a toilet mounted to the tile floor."
								else
									(localproc_0 19 30) ; "You see a clean tile floor with a water drain in the middle."
								)
							)
							((Said '/9mm')
								(if (gEgo has: 31) ; jailer_s_revolver
									(localproc_0 19 31) ; "It's a 4" Smith and Wesson .38 caliber K-38 revolver, Serial# 5557763."
								else
									(event claimed: 0)
								)
							)
							((Said '/japanesemiller')
								(cond
									...
									((< (gEgo y:) 135)
										(if (== (gEgo loop:) 0)
											(localproc_0 19 32) ; "Quit being so vain."
										else
											(localproc_0 19 33) ; "You'll have to turn around to see the mirror."
										)
									)
									(else
										(localproc_0 19 34) ; "You're not close enough."
									)
								)
							)
							((Said '/booth,bathroom')
								(if local11
									...
									(localproc_0 19 35) ; "You can't see much from the outside. Better open one first."
								)
							)
							((Said '/crapper,crapper,bathroom,tank')
								(if local11
									(gCurRoom setScript: stallScript)
								else
									(localproc_0 19 36) ; "The toilets are in the stalls, and you're not."
								)
							)
							((Said '[<at,around][/chamber,bathroom,bathroom]')
								(if local11
									...
									(localproc_0 19 37) ; "You look around the public rest room but the only unusual thing you notice is that it's cleaner than most. Good old Lytton!"
								)
							)
						)
					)
					((Said 'open,unlock/cabinet')
						(localproc_0 19 38) ; "It's locked, you don't have the key, and you don't need anything out of the cabinet."
					)
					((Said 'smell/odor')
						(localproc_0 19 39) ; "There aren't any. The fan works."
					)
					(
						(or
							(Said
								'move,japaneseopen,draw,open/cover,cover,cover'
							)
							(Said 'open/tank,crapper,bathroom')
							(Said 'frisk/tank')
						)
						(if local11
							(= [local5 local11] 1)
							...
							(localproc_0 19 40) ; "You're not close enough."
						)
					)
					...
					(
						(or
							(Said 'wipe,cloth/ass')
							(Said 'use/japanesehair[<crapper,bathroom]')
						)
						(if local11
					...
					((Said 'frisk,check,frisk[<out]/bathroom,booth')
						(if local11
					...
					((Said 'frisk,check,frisk[<out]/crapper,bathroom,tank')
						(if local11
							(gCurRoom setScript: stallScript)
						else
							(localproc_0 19 63) ; "Go ahead and enter one of the stalls."
					...
					((Said 'show/mugshot,shot,japaneseshot,japaneseshot')
						(if (gCast contains: local0)
					...
					(
						(Said
							'use,go/crapper,bathroom,bathroom,bathroom,(chamber<pause,bath)'
						)
						(if local11
							(switch (Random 0 3)
								(0
					...
					(
						(or
							(Said 'start,start,use,japaneseturnon>')
							(Said 'turn<on>')
						)
						(cond
							((Said '/dryer,(switch<dryer)')
								(if (gEgo inRect: 155 116 182 122)
									(if (> local12 0)
					...
					(
						(or
							(Said 'stop,stop,japanesestop,japaneseturnoff>')
							(Said 'turn,close<off>')
						)
						(cond
							((Said '/dryer')
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					((Said 'replace,japanesereplace,close,drop,japaneseput>')
						(cond
							((Said '/cover,cover,cover')
								(if (== [local5 local11] 1)
									(= [local5 local11] 0)
									(local18 posn: 154 75)
									(local19 posn: 155 137)
					...
					((Said 'draw,get,japaneseopen/9mm,revolver')
						(cond
					...
					(
						(or
							(Said
								'exit,japaneseclimbdown[/booth,bathroom,crapper,bathroom]'
							)
							(Said '(get<out),exit')
					...
					((Said 'taste,drink,drink[/water]')
						(localproc_0 19 101) ; "You've GOT to be kidding!"
					)
					((Said 'sat')
						(localproc_0 19 102) ; "Now's no time to relax, Sonny."
					)
					((Said 'frisk/crapper,bathroom,bathroom,booth')
						(localproc_0 19 103) ; "You look all around the toilet and the stall, but you find nothing out of the ordinary."
					)
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							(
								(or
									(Said
										'[<in,in]/crapper,bathroom,tank,water'
									)
									(Said
										'<in,in[/crapper,bathroom,tank,water]'
									)
								...
								(if [local5 local11]
									...
									(if
										...
										(localproc_0 19 104 83) ; "You see what looks like a police revolver hidden in the bottom of the toilet tank."
									else
										(localproc_0 19 105 83) ; "Full of water."
									)
								else
									(localproc_0 19 106) ; "Remove the lid first."
								)
								(if
									(and
										global128
									...
									(localproc_0 19 107) ; "The toilet is overflowing. It's not a pretty sight!"
								)
							)
							((Said '/handle')
								(localproc_0 19 108) ; "It's used to flush the toilet."
							)
							(
								(or
									(Said 'japaneselookdown')
							...
							((Said '/9mm')
								(cond
									((gEgo has: 31) ; jailer_s_revolver
										(localproc_0 19 109) ; "It's a 4" Smith and Wesson .38 caliber K-38, Serial# 5557763"
									)
									(
										(and
											(== (local19 y:) 0)
										...
										(localproc_0 19 104) ; "You see what looks like a police revolver hidden in the bottom of the toilet tank."
									)
									(else
										(localproc_0 19 110) ; "You can't see it."
									)
								)
							)
							((Said '/basin')
							...
							((Said '[<at,around][/booth,bathroom,wall]')
								(localproc_0 19 9) ; "The stall is furnished with a stool for your sitting pleasure and toilet paper for your comfort."
								(localproc_0 19 10) ; "There seems to be some graffiti scrawled on the wall of the stall... You can just barely read it."
							)
							(
							...
							((Said '/japanesehair[<crapper,bathroom]')
								(localproc_0 19 111) ; "Just your ordinary, white, thin, scratchy toilet paper."
							)
						)
					)
				)
...
(instance marshallScript of Script
	...
	(method (doit)
		(if (and (<= (local0 distanceTo: gEgo) 10) (not local16))
			(= local16 1)
			(localproc_0 19 112) ; "Excuse me, please. You're in my way."
