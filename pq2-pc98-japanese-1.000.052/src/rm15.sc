...
(instance rm15 of Rm
	...
	(method (doit)
		...
		(if (gCast contains: global112)
			(global112 setMotion: Follow gEgo 35)
		)
		(cond
			(local19 0)
			((> (gEgo x:) 320)
		...
		(if
			...
			(cond
				...
				(local18
					(= local18 0)
					(flowerGirl setMotion: Follow gEgo 40)
				)
			)
		)
		(if
...
(instance rm15Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 15 2 #at 221 66 #time 3) ; "My, My!"
				(Print 15 3 #at 252 65 #time 4) ; "Didja see that?"
				(if local7
					(EgoDead
		...
		(switch (event type:)
			(evSAID
				(cond
					(
						(or
							(Said 'give,feed/japanesehair')
							(Said 'ask,get/japanesehair')
						)
					...
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							...
							((Said '/flower,japaneseflower,basket')
								(cond
									(local10
					...
					((Said 'show/shot,japaneseshot,mugshot,japaneseshot')
					...
					(
						(or
							(Said
								'show,see,ask,talk/flower,japaneseflower,rose,bouquet,plant'
							)
							(Said
								'show,ask,talk[/woman,me]/flower,japaneseflower,rose,bouquet,plant'
							)
							(Said '(had<did,feed)<what')
						)
						(cond
							((not local8)
					...
					(
						(Said
							'give,feed/flower,japaneseflower,rose,bouquet,carnation,plant'
						)
						(cond
							(local8
					...
					(
						(Said
							'smell/flower,japaneseflower,rose,carnation,bouquet,plant'
						)
					...
					((Said 'open/door,japanesedoor[<auto]')
						(if (< (gEgo x:) 100)
							(localproc_0 15 50) ; "What kind of cop are you?? That's NOT your car!!"
						else
					...
					((Said 'get,buy/flower,japaneseflower')
						(if local10
							(if (< (gEgo distanceTo: flowerGirl) 50)
					...
					(
						(or
							(Said '[buy,get]/yellow')
							(Said '/(1,flower,japaneseflower)<yellow')
						)
						(if local10
							(if (< (gEgo distanceTo: flowerGirl) 50)
					...
					(
						(or
							(Said '[buy,get]/red')
							(Said '/(1,flower,japaneseflower)<red')
						)
						(if local10
							(if (< (gEgo distanceTo: flowerGirl) 50)
					...
					((Said 'talk/woman,woman,wanda[<flower,japaneseflower]')
						(cond
							((not local8)
							...
							((not (< (gEgo distanceTo: flowerGirl) 50))
								(proc0_7) ; "You're not close enough."
							)
							(else
								(Print 15 27 #at -1 15) ; "I have some lovely flowers...." the flower girl says. "Single roses, a bouquet of carnations....even some easy-to-care-for potted plants. You'll get a big kiss if you give these to your girl!"
					...
					(
						(or
							(Said 'more<how[/flower,japaneseflower]')
							(Said 'am/flower,japaneseflower')
							(Said 'give,feed,talk,ask[/me,woman,wanda]/cost')
							(Said 'give,feed,talk,ask/cost')
						)
						(if local10
							(if (< (gEgo distanceTo: flowerGirl) 50)
