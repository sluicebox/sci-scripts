				...
				(cond
					...
					((Said 'look,japaneselookup,japaneselookdown>')
						(cond
							...
							((Said '/cover,cover,manhole,hole,sewer')
								(if (gEgo inRect: 0 160 132 200)
									(if (not global196)
										(if (not local16)
											((= local11 (View new:))
					...
					(
						(or
							...
							(Said '(key<up),use/extender,(extender<extender)')
						)
					...
					(
						(Said
							'open,draw,japaneseopen,move,press,get/cover,cover,manhole,sewer'
						)
						(if (gEgo inRect: 40 170 102 189)
							...
							(localproc_0 81 57) ; "You're too far away."
						)
					)
					(
						(Said
							'drop,japaneseput,close,replace,japanesereplace/cover,manhole'
						)
						(if (gEgo inRect: 40 170 102 189)
							(if (not global196)
								(localproc_0 81 58) ; "The cover's on pretty tight already."
							else
								(localproc_0 81 59) ; "What for?"
							)
						else
							(localproc_0 81 57) ; "You're too far away."
						)
					...
					(
						...
						(cond
							...
							(local9
								(not global196)
								(localproc_0 81 60) ; "You have to get to it first."
							)
							(else
								(localproc_0 81 61) ; "What? Where?"
							)
						)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo illegalBits: 0)
				(localproc_0 81 62 25 8) ; "You step onto the ladder and begin the climb down."
				(self cue:)
			)
			(1
		...
		(switch (= state newState)
			(0
				(if (gEgo inRect: 54 180 83 189)
					(localproc_0 81 63) ; "How can you do that when you're standing on it?"
				else
					(self cue:)
				)
			)
			...
			(2
				(localproc_0 81 64 83) ; "Straining, you manage to slide the heavy cover off of the manhole."
				(self cue:)
			)
			(3
				(= global196 1)
