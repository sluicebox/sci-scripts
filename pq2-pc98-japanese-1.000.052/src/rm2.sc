...
(procedure (localproc_1)
	(if (gEgo has: 28) ; vial_of_blood
		(if (IsFlag 144)
			(ClearFlag 144)
			(localproc_2 28 146 2 90)
			(gEgo get: 28) ; vial_of_blood
		)
		(if (IsFlag 143)
			(ClearFlag 143)
			(localproc_2 28 147 2 91)
			(gEgo get: 28) ; vial_of_blood
		)
		(if (IsFlag 113)
			(ClearFlag 113)
			(localproc_2 28 148 2 92)
			(gEgo get: 28) ; vial_of_blood
		)
		(if (gEgo has: 28) ; vial_of_blood
				...
				(cond
					((Said 'look>')
						(cond
							...
							((Said '/counter,japanesecounter,bin,locker')
								(cond
							...
							((Said '/japaneseshot,japanesepict')
					...
					((Said 'drop,japaneseput/briefcase')
						(cond
							((not (gEgo inRect: 250 105 280 122))
								(event claimed: 1)
								(proc0_7) ; "You're not close enough."
...
(instance bigJonScript of Script
	...
	(method (handleEvent event &tmp temp0)
		...
		(cond
			((Said 'give,feed,book,(turn<in),japanesehand,book,book>')
				(= local7 0)
				(cond
					((gCast contains: local1)
						(event claimed: 1)
						(localproc_0 2 61) ; "Wait your turn, Bonds."
					)
					((< state 4)
						(event claimed: 1)
						(Print 2 62) ; "Wait until he's at the window."
					)
					((Said '[/!*]')
						(Print 2 63) ; "Book what?"
					)
					((Said '/clue')
						(localproc_0 2 64) ; "OK," says John, "What do you have?"
						(self changeState: 9)
					)
					...
					(else
						(switch (gInventory indexOf: temp0)
							...
							(else
								(Print 2 65) ; "That is not something you need to book."
							)
						)
					)
				)
			...
			((or (Said '/hello') (Said 'talk/john,man,man'))
				(cond
					...
					((not (gEgo inRect: 165 105 200 112))
						(Print 2 66) ; "Get closer to talk to him."
					)
					((<= (bigJonScript state:) 1)
						(Print 2 12) ; "He is not here."
					)
					((<= (bigJonScript state:) 3)
						(Print 2 67) ; "Wait until he is at the window."
					)
			...
			(
				...
				(cond
					...
					(else
						(localproc_0 2 68) ; "Big John looks surprised. "They keep the field kits in the bins right there behind the counter, where they've always been, Bonds," he says."
					)
				)
			)
			(
				...
				(cond
					...
					((not (gEgo inRect: 165 105 200 112))
						(Print 2 69) ; "Get closer to talk to him."
					)
					((<= (bigJonScript state:) 1)
						(Print 2 70) ; "He is not here."
					)
					((== (bigJonScript state:) 3)
						(Print 2 67) ; "Wait until he is at the window."
					)
					(else
						(localproc_0 2 71) ; "Big John says, "The results will be placed in your message basket tomorrow.""
					)
				)
			)
			((Said 'yes,ash')
				(switch local6
					(1
						(localproc_0 2 72) ; "What do you want to book?"
						(= local6 2)
					)
					(2
						(localproc_0 2 73) ; "Give me the evidence you want to book."
					)
					(3
						(localproc_0 2 73) ; "Give me the evidence you want to book."
					)
					(5
						(localproc_0 2 74) ; "I would be too."
					)
					(6
						(localproc_0 2 75) ; "That's good. I hope he's found and put away for a long time."
					)
					(else
						(Print 2 76) ; "Huh?"
					)
			...
			((Said 'no')
				(switch local6
					(1
						(localproc_0 2 77) ; "Ok, then leave me alone."
					)
					(2
						(localproc_0 2 78) ; "What are you talking about Sonny?"
					)
					(3
						(localproc_0 2 77) ; "Ok, then leave me alone."
					)
					(5
						(localproc_0 2 79) ; "Boy, Sonny, you must be really brave."
					)
					(6
						(localproc_0 2 80) ; "Well, I hope you find something on Bains because who knows what he'll do."
					)
					(else
						(Print 2 76) ; "Huh?"
					)
			...
			((Said '/none')
				(switch local6
					(2
						(localproc_0 2 81) ; "Ok, then quit bothering me!"
					)
					(else
						(Print 2 76) ; "Huh?"
					)
...
(instance mrGScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 2 82 #at -1 118) ; "In his heavy Italian accent, Officer Gelepsi says..."
				(switch (Random 0 2)
					(0
						(Print 2 83 #at -1 118) ; "I'da like'a to talk, but I'ma... how do you say... booke' de evidence'"
					)
					(1
						(Print 2 84 #at -1 118) ; "Sonny, I jus' don'a hav'a time to talk."
					)
					(2
						(Print 2 85 #at -1 118) ; "Cant'a you see I'ma busy?"
					)
				)
			)
			(2
				(Print 2 82) ; "In his heavy Italian accent, Officer Gelepsi says..."
				(switch (Random 0 2)
					(0
						(Print 2 86 #at -1 118) ; "Look, Mista detective: GET LOST!"
					)
					(1
						(Print 2 87 #at -1 118) ; "Stop buggin' me, hot-shot! I MEAN it!!"
					)
					(2
						(Print 2 88 #at -1 118) ; "GO 'WAY! I don' wanna talk to you no more!"
					)
				)
			)
			(3
				(Print 2 89 #at -1 118) ; "In a heated tone of voice Gelepsi says... "Hey, you cock'a de roach'a! Don'a you unnastand'a de english???""
			)
		)
	)
