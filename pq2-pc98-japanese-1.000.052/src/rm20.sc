		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'move,get/japaneseshot')
						(localproc_0 20 46) ; "Sorry! It's bolted to the wall."
					)
					((Said 'move,japaneseopen,break,cut,wear,draw,get/rope')
						(if (and (< (gEgo x:) 150) local5)
					...
					((Said 'show,flash/badge')
						(if (gEgo has: 7) ; wallet
							...
							(localproc_0 20 55) ; "You don't have your identification with you."
						)
					)
					(
						(Said
							'show/mugshot,japaneseshot,japanesepict,[shot,japaneseshot<japaneseface]'
						)
						(if
							(and
							...
							(localproc_0 20 56) ; "You don't have it."
						else
							(localproc_0 20 57) ; "Nobody recognizes Bains' picture."
						)
					)
					((or (Said 'draw,pull,use,show/9mm') (Said 'fire,kill'))
						(cond
							...
							(
								...
								(localproc_0 20 59) ; "Not now while he's frisking you."
							)
							(else
								(localproc_0 20 60 25 4) ; "Uh-oh! Perhaps now is not a good time (and here is not a good place) to be taking your gun out."
								(guardScript changeState: 21)
							)
						)
					)
						...
						(cond
							...
							((Said '/guard,cop')
								(if (gEgo inRect: 108 128 163 147)
									(localproc_0 20 61) ; "He doesn't seem real talkative."
								else
									(localproc_0 20 62) ; "He can't hear you at this distance."
								)
							)
							((Said '/man')
								(cond
									((gEgo inRect: 160 124 200 135)
										(localproc_0 20 63) ; "He doesn't say anything, and he's not Larry."
									)
									((gEgo inRect: 200 124 222 135)
										(localproc_0 20 64) ; "Airports are such a drag, especially sitting next to this guy here (referring to the man in white)."
									)
									((gEgo inRect: 83 128 115 136)
										(localproc_0 20 65) ; "The long-haired blonde says, "What's your trip, man?""
									)
									((gEgo inRect: 60 140 120 165)
										(localproc_0 20 66) ; "You hear a high-pitched voice say "Huh?""
									)
									((gEgo inRect: 108 128 163 147)
										(localproc_0 20 67) ; "Hello. Have a nice day."
									)
									(else
										(localproc_0 20 68) ; "There is no one close enough to talk to."
									)
								)
							)
						)
...
(instance manScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 20 69) ; "The guy turns around and says, "What's the matter with you, bud?""
				(= local0 20)
			)
			(2
...
(instance guardScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(localproc_0 20 70) ; "You show your badge, and the security guard says... "Pass right on through, Officer. Just remember...no partaking of the spirits.""
				(User canControl: 1)
			)
			(3
				(= local10 1)
				...
				(if (gEgo has: 0) ; hand_gun
					(localproc_0 20 71 25 4) ; "As you walk into the magnetic field, your gun sets off an alarm."
				else
					(localproc_0 20 72 25 4) ; "As you walk into the magnetic field, something sets off an alarm."
				)
			...
			(6
				(localproc_0 20 73 25 5 83) ; "One minute, sir!" the portly security guard says. "Please empty your pockets and walk through the detector again."
			...
			(9
				(HandsOn)
				(= local11 1)
				(localproc_0 20 74 25 4 83) ; "You remove everything from your pockets and try it again."
			)
			(10
				(HandsOff)
				(= local11 0)
				(User canControl: 0)
				(localproc_0 20 75 25 4) ; "Walking through the detector, your gun again sets off the alarm."
				(localproc_0 20 76 25 4) ; "Getting serious, the security guard says..."Hey, Mac! Are you packing a gun or something?""
				(self cue:)
			)
			(11
			...
			(13
				(localproc_0 20 77 25 4 83) ; ""Freeze right where you are, fella!" orders the guard."
				(self cue:)
			)
			(14
				(guard
			...
			(17
				(localproc_0 20 78 25 4 83) ; "The guard pats you down, finds and removes your gun, and says to you..."
				(if local9
					(localproc_0 20 79) ; "The next time you're packin' iron, and you want to pass through a metal detector, think about identifying yourself."
					(HandsOn)
					(= local13 1)
				else
					(localproc_0 20 80) ; "Okay, you pistol-packin' dude. You're under arrest!"
					(EgoDead
			...
			(20
				(guard setLoop: 0)
				(User canControl: 1)
				(localproc_0 20 81 25 4 83) ; "OK, Mr. Homicide Detective Bonds...go ahead with your business."
			)
			(21
				(= local15 1)
			...
			(24
				...
				(Print 20 82 #at -1 116 #draw) ; "You draw your weapon just as the guard goes for his. Being in fear of his life, the security guard blows you into eternity."
				(self cue:)
			)
			(25
				(EgoDead
...
(instance boardingScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(localproc_0 20 83 25 5 83) ; "The flight attendant at the entrance to the airplane asks you for your ticket."
				(global112
					illegalBits: 0
					ignoreActors:
					startUpd:
			...
			(1
				(localproc_0 20 84 83) ; "You hand your ticket to the attendant as you board the plane."
				(global112 setLoop: 1)
				(self cue:)
			)
			(2
				(localproc_0 20 85 83) ; "Keith flashes his badge at the security guard and hollers: "Hey, Sonny! I'm right behind you!""
			...
			(5
				(localproc_0 20 86) ; "Have a nice day!"
				(gEgo setMotion: MoveTo 110 124)
			)
		)
	)
