...
(procedure (proc0_7)
	(Print 0 33) ; "You're not close enough."
)

(procedure (proc0_8)
	(Print 0 34) ; "You already took it."
)

(procedure (proc0_10)
	(Print 0 35) ; "You can't do that now."
)

(procedure (proc0_9)
	(Print 0 36) ; "You don't have it."
)

(procedure (proc0_13)
	(Print 0 37) ; "You don't have your gun."
)
...
(procedure (SetFlag param1)
	(|= [global245 (/ param1 16)] (>> $8000 (mod param1 16)))
)
...
(procedure (IsFlag param1)
	(return (& [global245 (/ param1 16)] (>> $8000 (mod param1 16))))
)
...
(procedure (SetScore param1 param2)
	...
	(if (IsFlag 125)
		...
		(music number: 6 priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 10) play:)
	)
	(if (== argc 2)
		(SetFlag param2)
	)
...
(instance statusCode of Code
	...
	(method (doit param1)
		(Format ; "Score: %d of %d  %s  Police Quest \06"
			param1
			0
			0
			gScore
...
(instance PQ of Game
	...
	(method (replay)
		(MenuBar draw:)
		(SL enable:)
		(DoSound sndSET_SOUND (GetMenu 1282 113))
		(super replay:)
	)

	(method (wordFail param1 &tmp [temp0 80])
		(if (== (gGame parseLang:) 1)
			(Printf 0 1 param1) ; "You will not need to use the word "%s" in this game."
		else
			(Printf 0 2 param1) ; "%s \82\cd\82\b1\82\cc\83Q\81[\83\80\82\c5\82\cd\8eg\97p\82\b5\82\c4\82\a2\82\dc\82\b9\82\f1\81B"
		)
	)

	(method (syntaxFail)
		(Print 0 3 #icon 555 1 0) ; "Please try a different way of saying that."
	)

	(method (pragmaFail &tmp [temp0 200])
		(Print 0 4) ; "I'm not sure what you want me to do."
	)
	...
	(method (startRoom roomNum &tmp temp0 temp1 [temp2 100])
		(= global112 0)
		...
		(music priority: 10)
		(continuousMusic priority: 2)
		(if
			(and
				global101
				(u> (MemoryInfo 1) (+ 20 (MemoryInfo 0))) ; FreeHeap, LargestPtr
				(Print 0 5 #button {Debug} 1) ; "Memory fragmented."
			)
			(SetDebug)
		)
		(= global211 1)
	...
	(method (handleEvent event &tmp temp0 [temp1 103])
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(Said
							'check,frisk,japaneselookthru,(look<in,through)/billfold'
						)
					...
					((Said 'read/instruction,japanesehair')
						(if (gEgo has: 33) ; bomb_instructions
							(Print 800 53) ; "KUDOFI'S DO-IT-YOURSELF BOMB Connect yellow wire for timer. Connect white for backup battery. Disconnect yellow wire. Connect purple wire and blue wire for main battery."
							(Print 800 54) ; "Set timer. Connect yellow wire. Bomb is now armed. Do not move or touch wires! WARNING: Only connect same color wires. Connecting wires in any other order may result in immediate detonation."
						else
							(proc0_9) ; "You don't have it."
						)
					)
					(
						(Said
							'look,compare,check,frisk/handwriting,memo,handwriting[<hand,japanesehand]'
						)
						(if (gEgo has: 36) ; note_from_Marie_s_door
							(SetScore 3 90)
							(Print 0 6 #at -1 15 #icon 136 0 0) ; "Looking closer at the note, you think..."This is definitely NOT Marie's handwriting!!""
						else
							(proc0_9) ; "You don't have it."
						)
					)
					((Said 'yell[/!*]')
						(Print 0 7) ; "Talk in a normal voice, please."
					)
					((Said 'talk>')
						(if (Said '/friend')
							(cond
								...
								(
									(or
										...
										(not
											(global112 inRect: -10 -10 340 240)
										)
									)
									(Print 0 8) ; "Keith is not here."
								)
								((> (gEgo distanceTo: global112) 30)
									(Print 0 9) ; "Keith is not close enough to talk to."
								)
								(else
									(Print 800 (Random 10 15))
								)
					...
					((or (Said '[talk]/hello') (Said 'laugh,wave'))
						(Print 0 11) ; "Hi."
					)
					((Said '[talk]/bye')
						(Print 0 12) ; "`Bye."
					...
					(
						(or
							(Said 'use,japaneseopen/powder,brush')
							(Said 'drop,japaneseput,apply,use/powder,dust')
							(Said 'dust,powder')
							(Said
					...
					((Said 'look,read>')
						(cond
							...
							((Said '/(certificate,licence)<scuba')
								(if (IsFlag 33)
							...
							(
								...
								(Print 0 29) ; "You don't see him here."
							)
							((Said '/kim,holt,gomez,cheeks,cheeks,holt')
								(Print 0 30) ; "You don't see her here."
							)
							((Said '/boob')
								(if
									(or
									...
									(Print 0 32) ; "You don't see that here."
								)
							)
...
(class Iitem of InvI
	...
	(method (showSelf)
		(Print 899 (- view 100) #icon view 0 cel)
	)
)
...
(instance extra_ammo_clips of Iitem
	(properties
		...
		said '/ammo,japaneseammoclip,(clip[<ammo]),bullet'
		owner 5
		view 101
	)
...
(instance money_clip of InvI
	(properties
		name {money clip}
		said '/coat,(pocket<coat),cash,japanesemoneyclip,(clip[<cash])'
		view 104
	)

	(method (showSelf &tmp [temp0 20])
		(Print 0 38 #icon view 0 (if (== global107 0) 1 else 0)) ; "The amount of cash in your money clip is:"
		(Printf 0 39 global107) ; "$%u"
	)
)
...
(instance thank_you_letter of Iitem
	(properties
		...
		said '/handwriting,memo,letter[<you<thank]'
		owner 12
		view 105
	)
)
...
(instance potted_plant of Iitem
	(properties
		name {potted plant}
		said '/flower,japaneseflower,rose,plant,bouquet,carnation'
		owner 15
		view 111
	)
	...
	(method (showSelf &tmp [temp0 80])
		(switch cel
			(0
				(Print 0 40 #icon view loop cel) ; "A potted plant you bought at Lytton airport."
			)
			(1
				(Print 0 41 #icon view loop cel) ; "A single long-stemmed rose you bought at Lytton airport."
			)
			(2
				(Print 0 42 #icon view loop cel) ; "A bouquet you bought at Lytton airport."
			)
		)
	)
)
...
(instance new_mug_shot of Iitem
	(properties
		name {new mug shot}
		said '/mugshot,japaneseshot,(shot,japaneseshot<japaneseface)'
		owner 23
		view 112
	)
	...
	(method (saidMe param1)
		(cond
			...
			(
				(or
					(Said '/mugshot,japaneseshot<old')
					(Said '/(shot,japaneseshot)<japaneseface<old')
				)
				(param1 claimed: 0)
				(return 0)
			)
...
(instance plane_ticket of InvI
	...
	(method (showSelf &tmp [temp0 80])
		(if (== global195 1)
			(Print 0 43 #icon view 0 0) ; "An airplane ticket to Steelton."
		else
			(Print 0 44 #icon view 0 0) ; "An airplane ticket to Houston."
		)
	)
)
...
(instance old_mug_shot of Iitem
	(properties
		name {old mug shot}
		said '/mugshot,(shot,japaneseshot<japaneseface)'
		owner 7
		view 123
	)
)
...
(instance walkie_talkie of Iitem
	(properties
		name {walkie talkie}
		said '/(extender[<extender]),extender'
		owner 101
		view 130
	)
)
