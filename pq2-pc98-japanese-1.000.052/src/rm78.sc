				...
				(cond
					...
					(
						(or
							...
							(Said 'use/extender,(extender<extender)')
							(Said 'extender<use')
						)
					...
					((or (Said 'dial,use,pick[<up]/phone') (Said 'hang'))
					...
					((Said 'open,close/booth,door,japanesedoor')
						(if (gEgo inRect: 185 170 240 189)
					...
					((Said 'pick/flower,japaneseflower')
					...
					((Said 'dust')
						(if (& (gEgo onControl: 1) $0002)
							(localproc_0 78 38) ; "It wouldn't do you any good. At least a million fingers have punched those buttons at one time or another."
						else
							(localproc_0 78 39) ; "There's no future in doing that."
						)
					)
				)
			)
...
(instance startScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 78 40 83) ; "As you approach the park entrance, the patrol car's radio crackles: "Unit S56...we have a burglary in progress at 10th and Violet. Request you respond immediately. Over.""
				(self cue:)
			)
			(2
			...
			(3
				(local0 loop: 1 cel: 0)
				(localproc_0 78 41 83) ; "Well, boys..." says Officer Pitman, "I gotta go. I doubt you'll find anything, but go ahead and look around the park."
				(localproc_0 78 42 83) ; "Right here's the telephone booth Bains used to make that call."
				(localproc_0 78 43 83) ; "Call us when you need to be picked up, or if you need any kind of assistance. Good luck!"
				(self cue:)
			)
			(4
			...
			(5
				(if (gEgo has: 30) ; walkie_talkie
					(localproc_0 78 44) ; "Keith looks around and says..."I think we should split up Sonny. Call me on the two-way if you need me.""
				else
					(localproc_0 78 45) ; "Keith looks around and says..."I think we should split up Sonny. I'll check the park perimeter. Holler if you need me.""
				)
				(self cue:)
			)
			(6
