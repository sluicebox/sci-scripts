		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'open/cabinet,compartment,door,door')
						(cond
							(local8
					...
					((Said 'look[<around][/(!*,chamber)]')
						(Print 126 8) ; "You see walkways and grates that lead around the dark and smelly sewer."
						(cond
							((not local8)
								(Print 126 9) ; "On the wall, you see a cabinet that is closed."
							)
							((IsItemAt 32) ; gas_mask
								(Print 126 10) ; "On the wall, you see a cabinet that is open with a gas mask in it."
							)
							(else
								(Print 126 11) ; "On the wall, you see a cabinet that is open and empty."
							)
						)
					)
					((Said 'look/wall')
						(Print 126 12) ; "On one wall is a cabinet."
					)
					((Said 'close/cabinet,compartment,door,door')
						(cond
							((not local8)
								(Print 126 3) ; "It already is."
							)
					...
					((Said '(drop<back),japaneseput,replace/mask,japanesemask')
						(if (gEgo has: 32) ; gas_mask
							(Print 126 13) ; "You will find the mask useful."
						else
...
(instance downTheDrain of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 126 14) ; "You step into the path of high pressure water being released from the surface..."
				(Print 126 15) ; "And you're swept into the sewer . Your head hits hard during the fall, rendering you unconscious..."
				(Print 126 16) ; "Unfortunately for you, the sewer does the rest."
				(gEgo dispose:)
				(EgoDead 126 17) ; "You drown in the sewer because of an unfortunate accident. Next time, try to minimize the chance of it happening again."
			)
		)
	)
)
