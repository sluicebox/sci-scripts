...
(instance rm27 of Rm
			...
			(evSAID
				(if (== local9 1)
					...
					(cond
						...
						((Said 'read,look/memo,threat,letter')
							((gInventory at: 6) showSelf:) ; death_threat
						)
						((Said 'look,frisk,check>')
							(cond
								((Said '/man')
								...
								((Said '/auto<coroner,japanesecastle')
									(if (> 3 global182 0)
				...
				(cond
					...
					((Said 'japaneseopen,get,move,draw/body,man')
						(cond
							((!= local9 1)
	...
	(method (init)
		...
		(gAddToPics doit:)
		(if (== local9 1)
			(gAddToPics add: polcar woodycar)
			(gAddToPics doit:)
...
(instance quincyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(12
				(Print 27 100 #at -1 67) ; "Officer Gelepsi and the coroner struggle to carry the victim into the back of the coroner's vehicle."
				(= local10 1)
