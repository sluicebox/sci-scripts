		...
		(13
			(cond
				(
					...
					(Print 67 71) ; "You're not close enough to your car's door."
				)
				((== global133 1)
					(Print 67 72) ; "Your car door is locked."
					(return 0)
				)
				(global135
					(Print 67 73) ; "Close the trunk first."
					(return 0)
				)
				((gEgo has: 10) ; field_kit
					(Print 67 74) ; "You should put your field kit in the trunk first."
					(return 0)
				)
				(else
...
(instance rm67 of Rm
	...
	(method (init)
		...
		(jBlock top: 178 bottom: 193 left: 194 right: 320)
		(ourCar
			view: 54
			setStep: 4 1
			setLoop: 0
		...
		(gAddToPics add: bench brownCar yellowCar whiteWagon brownWagon)
		(if (== global100 3)
			(gAddToPics add: policeCar blueCar)
			(plight
				view: 54
				loop: 3
				cel: 0
		...
		(cond
			...
			(
				(or
					...
					(Said 'ask,interrogate,get/name,address,number,licence')
					(Said
				...
				(cond
					...
					((not (IsFlag 51))
						(Print 67 6 #at -1 20) ; "You ask for the description of her car and she says... "Oh, yes! It's a 1986 compact Chevy station wagon. It's black, and the license number is...is... C035... Oh! That's all I can remember, Officer.""
						(SetScore 3)
						(SetFlag 51)
						(script seconds: 3)
					)
			...
			((Said 'talk/cop,calvin')
				(cond
					...
					(else
						(Print 67 12 #at -1 40) ; "You know, Bonds," says Haines, " it's a good thing I had to pick up toothpaste for my wife, or we might not have found the jailer's car for a while."
					)
				)
			)
			...
			((Said 'drop,japaneseput,location,drop/briefcase[/trunk]')
				(cond
			...
			((Said 'get,japaneseopen/briefcase[/trunk]')
				(cond
					((gEgo has: 10) ; field_kit
			...
			((Said 'look>')
				(cond
					((Said '[<around][/!*,building,mall,chamber]')
					...
					((or (Said '/licence[<licence]') (Said '/licence'))
						(cond
			...
			((or (Said 'read/licence[<licence]') (Said 'read/licence'))
				(cond
					((gEgo inRect: 151 176 206 210)
						(switch (Random 0 2)
							(0
								(Print 67 48) ; "You look and find the rear plate missing."
							)
							(1
								(Print 67 49) ; "You notice that the rear plate is missing."
							)
							(2
								(Print 67 50) ; "The rear plate is missing."
							)
						)
					)
					((gEgo inRect: 252 173 304 210)
						(switch (Random 0 2)
							(0
								(Print 67 51) ; "You make a mental note of the front plate... personalized letters...."WOW""
							)
							(1
								(Print 67 52) ; "You look at the front plate which reads... "WOW""
							)
							(2
								(Print 67 53) ; "You find a personalized front plate that reads... "WOW" you think to yourself..." Quite fitting for such a sharp Vette!""
							)
						)
					)
					(else
			...
			((or (Said 'enter/auto') (Said 'open/door,japanesedoor'))
				(cond
					...
					(
						...
						(Print 67 54) ; "You don't need to open this door."
					)
					(
						(and
							(== (gEgo loop:) 2)
						...
						(Print 67 55) ; "This side is locked. Try the other side."
					)
					(
						(and
							(== (gEgo loop:) 3)
						...
						(Print 67 56) ; "You don't need anything from the police car."
					)
					((gEgo inRect: 220 170 240 180)
						(Print 67 57) ; "Which car?"
					)
					(else
						(localproc_0)
					)
				...
				(cond
						...
						(cond
							((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
								(if (== global133 1)
									...
									(Print 67 59) ; "The door is already unlocked."
								)
							)
							((== global131 13)
								(Print 67 60) ; "You need a key to unlock this door."
							)
						)
						(cond
					...
					((== global131 13)
						(if (gEgo inRect: 22 148 40 152)
							(Print 67 62) ; "You don't need to do that."
						else
							(Print 67 63) ; "You're not close enough."
						)
					)
					...
					(
						...
						(cond
							((and (== global131 13) (gEgo has: 3)) ; unmarked_car_keys
								(if (== global133 0)
									(= global133 1)
									(Print 67 64) ; "OK. It's locked."
								else
									(Print 67 65) ; "The door is already locked."
								)
							)
							((== global131 13)
								(Print 67 66) ; "You need a key to lock this door."
							)
						)
						(cond
							((and (== global131 33) (gEgo has: 2)) ; key_ring
							...
							((== global131 33)
								(Print 67 67) ; "You need a key to lock this door."
							)
						)
					)
					((== global131 13)
			...
			(0
				...
				(cond
					...
					((not (global112 inRect: -10 0 330 240))
						...
						(Print 67 68) ; "Keith yells: "I'm coming, Sonny!""
					)
					(else
						(Print 67 69) ; "Keith says: "Wait up, Sonny!""
						(if (> (global112 y:) 158)
							(if (< (global112 x:) 102)
								(global112
			...
			(4
				(if (== global131 13)
					...
					(if
						...
						(SetFlag 156)
						(Print 67 70) ; "As you get into the car, you fill Keith in on what you've discovered."
					)
				)
		...
		(switch (= state newState)
			(0
				...
				(if global112
					...
					(Print 67 75) ; "Keith says, "Look Sonny, there's Haines.""
				else
					(Print 67 76 #at -1 40) ; "You see Officer Haines approaching."
				)
			...
			(2
				(gEgo stopUpd:)
				(haines loop: 1)
				(Print 67 77 #draw #at -1 40) ; ""This is definitely the Correctional Officer's car," Haines says..."
				(Print 67 78 #at -1 40) ; "It's been sitting here a while, though," he continues. "The engine is cold."
				(if global112
			...
			(3
				...
				(Print 67 80 #at -1 40) ; "Haines says, "I think I'll go check around a little. There may be some witnesses.""
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(jailerdoor stopUpd:)
				(haines loop: 2)
				(Print 67 81 #at -1 20 #draw) ; "Hey Sonny!" Haines shouts. "Looks like Bains may have taken another car. This lady says her car has been stolen."
				(self cue:)
			)
			(2
				(haines stopUpd:)
			...
			(4
				...
				(Print 67 82 #at -1 20 #draw) ; ""I figure you'll have some questions to ask her," says Haines."
				(User canInput: 1)
				(= seconds 60)
			)
			(5
				(HandsOff)
				(Print 67 83 #at -1 20 #draw) ; "I'll take it from here Sonny," Haines assures you. "You've got your work cut out for you."
