		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'open,japaneseopen,press,move/cover[<manhole]')
						(if (not local0)
					...
					((Said 'climb[/ladder]>')
						(cond
							((or (Said 'japaneseclimbdown') (Said '<down'))
								(if (not local0)
...
(instance ladderScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gEgo view: 395 loop: 0 cel: 0 setCycle: CT 7 1 self)
				(User canInput: 0)
			)
			(4
				(HandsOn)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(bainsGunFire priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 9) play:)
			)
			(3
