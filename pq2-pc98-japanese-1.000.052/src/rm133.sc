		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(or
							(Said 'japaneseopen/(cheeks,rope)')
							(Said 'japaneseopen/rope')
						)
						(cond
							((not local111)
					...
					(
						(or
							(Said '(keep,am,japanesebe)<calm,calm')
							(Said '(close<up),japaneseshutup')
							(Said 'calm/cheeks')
							(Said 'talk/cheeks/am<calm')
							(Said 'calm')
						)
						(cond
							(local109
					...
					(
						(or
							(Said '(stop,police,stop,japanesestop)')
							(Said 'talk/(stop,police,stop)')
						)
						(if (not local112)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(bainsGunShot priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 8) play:)
			)
			(6
			...
			(22
				(if (<= local107 1)
					...
					(bainsGunShot
						priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 8)
						play:
					)
					(= cycles 6)
				else
			...
			(26
				...
				(= local5 (localproc_0 133 48)) ; "The barrage is over. Bains lies motionless on the control room floor."
				(= seconds 8)
			)
			(27
				(clr)
				(= local5 (localproc_0 133 49)) ; "It appears that all the firing range practice has paid off."
				(gEgo
					view: 1
			...
			(29
				(= local5 (localproc_0 133 50)) ; "Checking Bains, you find all vital signs have ceased."
				(= seconds 7)
			)
			(30
				(clr)
				(= local5 (localproc_0 133 51)) ; "You view Bains in his deceased state. Deep down inside, you kind of feel sorry for him. The reality, however, is that...."
				(= seconds 13)
			)
			(31
				(clr)
				(= local5 (localproc_0 133 52)) ; "the destructive vengeance of "The Death Angel" has been silenced."
				(= seconds 8)
			)
			(32
				(clr)
			...
			(34
				(RedrawCast)
				(localproc_0 133 53) ; "Oh Sonny!" Marie says..."You're my hero! I never want to be apart from you ever again!"
				(= seconds 5)
			)
			(35
