...
(instance rm4 of Rm
	...
	(method (init)
		...
		(switch gPrevRoomNum
			(2
				(gEgo posn: 120 162 init: setMotion: MoveTo 120 155)
				(User prevDir: 1)
			)
			(7
		...
		(switch (= state newState)
			(0
				...
				(if (<= local2 3)
					...
					(blab setCycle: Fwd init:)
				else
					(captain loop: 4 setCycle: Fwd cycleSpeed: 7)
				)
				((= global112 (View new:)) view: 65 cel: 0 init:)
				(switch local2
					(1
						(global112 view: 62 posn: 213 134 loop: 0)
					)
					(2
						(global112 posn: 210 136 loop: 6)
					)
					(3
						(global112 posn: 133 130 loop: 6)
					)
					(4
						(global112 posn: 163 148 loop: 6)
					)
					(5
						(global112 posn: 120 140 loop: 7)
					)
					(6
						(global112 posn: 214 148 loop: 7)
					)
				)
				(global112 stopUpd:)
				(if (!= local2 1)
				...
				(rambo cel: (if (== (mod local2 2) 0) 0 else 1))
				(carKey
					view: 65
					posn:
				...
				(gAddToPics add: rambo chair doit:)
				(switch local4
					(2
						(cond
							((== global138 1)
			...
			(evSAID
				(cond
					(
						(or
							(Said
								'show,give,feed,talk/clue,list,memo,threat,card,clue'
							)
							(Said
								'show,give,feed,talk/hall/clue,list,memo,threat,card,clue'
							)
						)
						(cond
					...
					((Said 'look,read/japanesehair,flyer')
						(cond
					...
					(
						(or
							(Said '/shot,japaneseshot<japaneseface')
							(Said '/mugshot')
						)
						(Print 4 27) ; "There are no mugshots here."
					)
					((Said '(turn<on),japaneseturnon/computer')
				...
				(cond
					(
						(and
							(Said 'look,get,read,check,frisk>')
							(Said
								'/basket,subpoena,memo,envelope,finding,japanesehair'
							)
						)
						(event claimed: 0)
							...
							(cond
								(
									(and
										(< global100 6)
										(Said
											'/basket,subpoena,envelope,japanesehair'
										)
									)
									(SetScore 1 64)
									(localproc_0 4 42) ; "In the basket you see a subpoena."
									(localproc_0 4 43) ; "You pick up and read the subpoena."
									(localproc_0 4 44) ; "You are commanded to appear in Division 4, Superior Court of the City of Lytton, as a witness for the People in the retrial matter against the defendant, Jessie Bains. Notification of time and date will follow."
								...
								(
									(and
										(>= global100 8)
										(Said
											'/basket,envelope,finding,japanesehair'
										)
									)
									(if
										(or
								...
								(
									(and
										global161
										(Said
											'/basket,memo,letter,japanesehair'
										)
									)
					...
					((Said 'look/hall')
						(Print 4 68) ; "Captain Fletcher Hall is a very large man with an over-powering presence."
					)
					((Said 'look/james')
					...
					((Said 'look/desk')
						(cond
							((gEgo inRect: 71 131 122 156)
								(Print 4 69) ; "This is Captain Fletcher Hall's desk. On it, you see some folders and papers."
							)
							((gEgo inRect: 90 117 140 132)
								(Print 4 70) ; "This is Detective Jim Pierson's desk."
							)
							((gEgo inRect: 189 117 244 139)
								(Print 4 71) ; "This is your partner, Keith Robinson's desk."
							)
							((gEgo inRect: 154 140 235 200)
								(Print 4 72) ; "This desk is used for the computer, which is shared by everyone in the office."
							)
							(else
								(Print 4 73) ; "Your desk is equipped with drawers, a phone, a message basket, and a lamp."
							)
						)
					)
					((Said 'get,japaneseopen/subpoena')
						(if (< global100 6)
							(Print 4 74) ; "Leave it in the basket. The station's clerical staff is very good at filing these things away for you."
						else
							(Print 4 75) ; "The subpoena has been filed away."
						)
					)
					((Said 'get,japaneseopen/memo')
						(if (== global100 6)
							(Print 4 76) ; "As you reach for the note, you think... "Nah, just one more piece of paper to carry around."
						else
							(Print 4 77) ; "The note is gone. Clerks periodically clean out your basket, so that important papers aren't allowed to sit."
						)
					)
					((Said 'get,japaneseopen/clue,finding,envelope')
						(if (>= global100 8)
							(Print 4 74) ; "Leave it in the basket. The station's clerical staff is very good at filing these things away for you."
						else
							(Print 4 78) ; "What? You see nothing like that here."
						)
					)
					...
					((Said 'open,close/locker')
						(Print 4 79) ; "The Captain keeps it closed and locked."
					)
					(
						(or
					...
					((Said 'close/(cabinet[<file]),file')
						(if (== gPrevRoomNum 7)
							(Print 4 80) ; "You closed it yourself."
						else
							(Print 4 81) ; "It's already closed."
						)
					)
					((Said 'get,japaneseopen/key')
						(if (gEgo inRect: 122 117 168 124)
							(if (gEgo has: 3) ; unmarked_car_keys
								(Print 4 82) ; "You already have them."
							else
								(gEgo get: 3) ; unmarked_car_keys
								(carKey posn: -100 0)
								(Print 4 83) ; "You remove the keys from the board."
								(SetScore 1 104)
							)
						else
							(proc0_7) ; "You're not close enough."
					...
					((Said 'drop,japaneseput,replace,japanesereplace/key')
						(if (gEgo has: 3) ; unmarked_car_keys
							(if (gEgo inRect: 122 117 168 124)
								(PutItem 3) ; unmarked_car_keys
								(carKey posn: 141 110)
					...
					((Said 'sat[/!*]')
						(cond
							(local3
								(Print 4 85) ; "You're already sitting."
							)
					...
					((or (Said 'stand[<up][/!*]') (Said 'get<up[/!*]'))
						(if local3
							...
							(Print 4 86) ; "You're standing already."
						)
					)
					((Said 'unlock,open/drawer,desk,key')
						(cond
							((not local3)
							...
							(else
								(Print 4 88) ; "You don't have the key to the drawer."
							)
						)
					)
					((Said 'lock/drawer,desk')
						(if local3
							(cond
								((IsFlag 12)
									(Print 4 89) ; "It's already locked."
								)
								((gEgo has: 2) ; key_ring
									(Print 4 84) ; "OK."
									(IsFlag 12)
							...
							(Print 4 90) ; "You must sit down first."
						)
					)
					((or (Said 'lock,close/drawer') (Said 'stand[/!*]'))
						(if local3
							(if (== (rm4 script:) drawerScript)
								...
								(Print 4 91) ; "The drawer's already closed."
							)
						else
							(Print 4 92) ; "You should sit down first."
						)
					)
					((or (Said 'use,get,dial,(pick<up)/phone') (Said 'hang'))
						(if local3
							(self changeState: 5)
							(= local5 1)
						else
							(Print 4 93) ; "Sit at your desk to use the phone."
						)
					)
					(
						(or
							(Said 'drop,japaneseput[<down]/phone')
							(Said 'replace,japanesereplace/phone')
							(Said 'hang<up[/phone]')
						)
					...
					((and (Said 'call,phone/') (not local5))
						(Print 4 94) ; "Better use the phone."
					)
				)
			)
		)
...
(instance drawerScript of Script
	...
	(method (handleEvent event)
		(switch (event type:)
			(evSAID
				(cond
					((Said 'get,japaneseopen/letter,card')
						(if (IsItemAt 5 12) ; thank_you_letter
							(local0 dispose:)
							(Print 4 84 #draw) ; "OK."
							...
							(Print 4 95) ; "That isn't in the drawer."
						)
					)
					((Said 'get/billfold')
						(if (IsItemAt 7 12) ; wallet
					...
					(
						(Said
							'drop,japaneseput,replace,japanesereplace/memo,letter,card'
						)
						(if (gEgo has: 5) ; thank_you_letter
							...
							(Print 4 96) ; "You don't have that."
						)
					)
					((Said 'drop,japaneseput,replace,japanesereplace/billfold')
					...
					((Said 'look,frisk[/drawer]')
						(gInventory
							carrying:
								{Your desk drawer contains: }
							empty:
								{Your desk drawer is empty. }
							showSelf: 12
						)
					)
					(
						...
						(if (IsItemAt 5 12) ; thank_you_letter
							(Print 4 97) ; "You see it in the drawer."
						else
							(event claimed: 0)
						)
					)
					(
						...
						(Print 4 98) ; "There are no mugshots here."
					)
					((Said 'close[/drawer]')
...
(instance captainScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(localproc_1 4 103) ; "Bonds, we have bad news. Jessie Bains has escaped Lytton City Jail!"
				(aTimer setCycle: self 18)
				(bainsTheme play:)
			)
			(8
			...
			(17
				(gEgo loop: 1)
				(localproc_1 4 113) ; "Hey Bonds!" the Captain blurts. "We have a traffic cop standing by down in the old warehouse district with a 187 victim."
			...
			(20
				(gEgo loop: 1)
				(localproc_1 4 116) ; ""Keith just told me about the motel incident!" the Captain says in an excited voice."
			...
			(25
				...
				(gEgo loop: 1)
				(if (not (IsFlag 27))
					(localproc_1 4 123) ; "Barely able to contain your emotions, you quickly tell Captain Hall about the signs of a struggle at Marie's."
				)
				(captain setCycle: End)
				(aTimer setCycle: self 18)
			)
			(26
				(gEgo loop: 1)
				(localproc_1 4 124) ; ""Sorry to hear about Marie, Sonny," Captain Fletcher offers."
			...
			(31
				(cond
					...
					((IsFlag 136)
						(localproc_0 4 134) ; "I've looked over the evidence and see that Bains now knows where Colby is."
						(localproc_0 4 132) ; "This chump Bains," he continues, "most likely has kidnapped Marie to use her as a hostage. He's trying to sucker you into another set-up, Bonds."
						(localproc_0 4 133) ; "A maniac like Bains is hard to read, boys," Captain Hall muses. "You've got a blank check to proceed however you feel is best."
					)
					(else
						(localproc_0 4 135) ; "Colby's where-abouts are unknown to Bains," Captain Hall thinks aloud, "so Bains will probably stick around to take another crack at you, Sonny."
					)
				)
				(aTimer setCycle: self 18)
			)
			(32
				...
				(if (or (gEgo has: 35) (gEgo has: 13)) ; Colby_s_business_card, hit_list
					...
					(localproc_0 4 136) ; "I'll book this evidence for you. You'd better decide what to do next... and FAST!"
				)
				(captain setCycle: Beg)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= global163 1)
				(Print 4 137 #at 90 40 #draw) ; "Your partner Keith takes a deep puff and says:"
				(Print 4 138 #at 90 40) ; "Say, Sonny...I heard that dirtbag you sent up the river last year is back in town for retrial. Isn't his name Bains, or something like that?"
				(Print 4 139 #at 90 40) ; "Maybe this time you can put him away for the duration."
				(Print 4 140 #at 90 40) ; "It just seems like the garbage of this world gets every chance known to man."
