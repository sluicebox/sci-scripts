		...
		(cond
			((Said 'look,open,use/briefcase')
				(if local1
					...
					(Print 153 1 #at 165 11 #draw #mode 1) ; "Your Detective's Field Kit is used to carry various things you will need in the course of your investigations: Plastic Baggie Small Camera  Casting Plaster Glass Vial  Eye Dropper Fingerprint Powder Fingerprint Brush Fingerprint Tape"
				)
			)
			...
			(
				(or
					...
					(Said 'dust,powder')
					(Said 'get,japaneseopen,draw/fingerprint,print[<finger]')
				)
				(if local1
			...
			(
				(or
					(Said 'use,japaneseopen/camera')
					(Said 'get/japaneseshot,japanesepict')
				)
				(if local1
					(Print 153 14) ; "You don't have your field kit."
				else
					(Print 153 15) ; ""Click!" You take a picture, and return the camera to your field kit."
				)
			)
			((or (Said 'use,japaneseopen/tape') (Said 'tape'))
				(Print 153 16) ; "The fingerprint tape is used only after finding a useful print. By picking up the print with the tape, it can be transferred to a card."
			)
			(
				(or
					(Said 'use,japaneseopen/dropper,vial,baggie')
					(Said 'get/sample')
				)
				(Print 153 17) ; "There's nothing here worth the trouble."
			)
			((Said 'make,use/plaster,cast,footprint,(print<feet)')
				(Print 153 18) ; "You see nothing worth wasting casting plaster on."
			)
		)
	)
)
