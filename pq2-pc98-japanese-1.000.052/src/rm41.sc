				...
				(cond
					...
					((Said 'sat')
						(if (not global201)
							...
							(proc154_1 41 17 67 20 20 33 gSmallFont)
						)
					)
					((Said 'look>')
						(cond
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(hijackMusic stop:)
				(hijackGunFire
					priority: (if (== (DoSound sndCHECK_DRIVER) 6) 1 else 5)
					play:
				)
				(if local4
					(cond
...
(instance intoCockpit of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(pilotDoor setPri: 0 setCycle: End self)
			)
			(8
				(pilotDoor stopUpd:)
