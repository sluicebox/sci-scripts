				...
				(cond
						...
						(cond
							((Said '<below>')
								(cond
									(
										(Said
											'/fridge,oven,cabinet,counter,japanesecounter,basin'
										)
							...
							((Said '/plant,flower,japaneseflower')
								(cond
									((gEgo inRect: 0 0 172 115)
					...
					((Said 'frisk>')
						(cond
							...
							(
								(Said
									'/drawer,counter,japanesecounter,basin,oven,fridge'
								)
								(if (gEgo inRect: 0 0 172 115)
					...
					((Said 'move>')
						(cond
							...
							(
								(Said
									'/couch,bench,television,ashtray,japanesecounter'
								)
								(localproc_0 32 98) ; "There's no need to do that."
							)
						)
					)
					((Said 'read/japanesehair,list')
						(if (gEgo has: 13) ; hit_list
							(localproc_1 32 62 82 113) ; "You read over a list of names..."Roberts, Wilkans, Colby, Bonds.""
							(if (not local5)
								(= local5 1)
					...
					(
						(Said
							'(turn[<on,off]),japaneseturnon,japaneseturnoff/light,lamp'
						)
					...
					((or (Said 'dust>') (Said 'powder>') (Said 'apply/powder>'))
						(cond
							(
								(Said
									'/japanesecounter,couch,bench,counter,japanesecounter,cabinet,phone,ashtray'
								)
								(if (gEgo has: 10) ; field_kit
									(global122 setPri: 0)
									(global120 setPri: 0)
					...
					((Said 'clock,use,(turn<on),japaneseturnon/television,set')
