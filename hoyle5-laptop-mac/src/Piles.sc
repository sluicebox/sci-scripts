...
(class Hand of EventHandler
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4)
		(if
			...
			(if (or (== (Dealer curPlayer:) self) (== owner -1))
				...
				(if
					(and
						(== (= temp4 (temp0 processEvent: event global399)) 1)
		...
		(return
			(cond
				...
				((or (== handDirection 1) (== handDirection 3))
					(= temp1 (+ (* (- (Dealer cardsToDeal:) 1) global396) 67))
					(= temp2 0)
					(if (!= temp1 247)
						(= temp2 (+ x (/ (- 247 temp1) 2)))
					else
	...
	(method (calcNextY &tmp temp0 temp1 temp2)
		(return
			(cond
				...
				((== handDirection 2)
					(= temp1 (+ (* (- (Dealer cardsToDeal:) 1) global397) 95))
					(= temp2 0)
					(if (!= temp1 263)
						(= temp2 (+ y (/ (- 263 temp1) 2)))
					else
	...
	(method (add param1 param2 param3 param4 &tmp temp0)
		(if (and (not (== self gTray)) (proc0_11 param1))
			(param1 setPri: 2)
		)
		(= local7 0)
		(if argc
	...
	(method (center &tmp temp0 temp1 temp2 temp3 temp4 temp5)
		...
		(cond
			...
			((== handDirection 1)
				(if (!= (= temp2 (+ (* global396 (- size 1)) 67)) 247)
					(= temp4 (/ (- 247 temp2) 2))
				)
			)
			(
	...
	(method (sort param1)
		(if (and (not (Dealer script:)) (or faceUp (and argc param1)))
			(switch gSortMode
				(1
					(KList 22 (self elements:) 768 0) ; Sort
				)
				(2
					(KList 22 (self elements:) 768 1) ; Sort
				)
				(3
					(self convertSuit: 3)
					(KList 22 (self elements:) 768 0) ; Sort
					(KList 22 (self elements:) 767 0) ; Sort
					(self revertSuit: 3)
				)
				(4
					(self convertSuit: 4)
					(KList 22 (self elements:) 768 1) ; Sort
					(KList 22 (self elements:) 767 1) ; Sort
					(self revertSuit: 4)
				)
				(5
