...
(procedure (CantDo)
	(Print 0 135) ; "You can't do that here; at least, not now."
)
	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 3])
		...
		(cond
			...
			((and (gEgo has: 17) (or (Said 'apply/gun') (Said 'hit'))) ; Knife
				(Print 0 22) ; "Hey! We want our R rating for sex, not violence!"
			)
			((and (gEgo has: 19) (Said 'apply,burn/match')) ; Matches
				(if (gEgo has: 21) ; Hair_Rejuvenator
					...
					(Print 0 23) ; "You light the book of matches and watch it go up in flames. Whee!"
			...
			(
				...
				(cond
					...
					((Said '/me,head,hair')
						(Print 0 25) ; "You've had enough problems with your hair. You don't need to wear explosives, too!"
					)
					(else
						(event claimed: 1)
						(Print 0 26) ; "Ok. It's gone now."
			...
			(
				...
				(Print 0 27) ; "Good idea. But this is not the place."
			)
			((and (gEgo has: 21) (Said 'burn,burn/bottle,rejuvenator')) ; Hair_Rejuvenator
				(if (gEgo has: 19) ; Matches
					...
					(Print 0 28) ; "Fortunately, you have no matches with which to immolate yourself!"
				)
			)
			((Said 'open,(look<in)>')
				...
				(cond
					((Said '[/!]')
						(Print 0 29) ; "What do you want to inspect?"
					)
					((not temp1)
						(event claimed: 1)
					...
					(else
						(switch (gInventory indexOf: temp1)
							(3
								(Print 0 30) ; "If it's a good cruise, it's a Wonder!"
							)
							(5
								(if (== gCurrentEgoView 132)
									(Print 0 31) ; "Pervert!"
								else
									(Print 0 32) ; "Hand wash only."
								)
							)
							(6
								(Print 0 33) ; "You only find more money."
							)
							(7
								(Print 0 34 #icon 7 1 0) ; "Nice photo, Larry!"
							)
							(10
								(Print 0 35) ; "You can't find the microfiche in there."
								(Print 0 36 #at -1 152) ; "(It's a secret and you don't know about it!)"
							)
							(12
								(Print 0 37) ; "Nothing in the sewing kit is of use to you."
							)
							(14
								(Print 0 38) ; "Made in Taiwan"
							)
							(15
								(Print 0 39) ; "40D"
							)
							...
							(23
								(Print 0 42) ; "You have no rights. (Just ask us!)"
							)
							(24
								(Print 0 43) ; "You open the parachute and it spills out everywhere. Since you know nothing about parachute folding, you throw it away."
								(gEgo put: 24 -1) ; Parachute
								(gGame changeScore: -3)
							)
							(26
								(Print 0 44) ; "Repent!"
								(Print 0 45) ; "And send money."
							)
							(27
								(Print 0 46) ; "You look inside the bag and see nothing."
							)
							(else
			...
			((Said 'thank')
				(Print 0 49) ; "You're welcome."
			)
			((Said 'bang')
				(Print 0 50) ; "Knock, knock."
				(Print 0 51 #at -1 152) ; "(Nobody wants to play now, Larry!)"
			)
			((Said 'hit')
				(Print 0 22) ; "Hey! We want our R rating for sex, not violence!"
			)
			((or (Said 'board/bathroom') (Said 'leak') (Said 'get/leak'))
				(if (== 100 (gEgo view:))
					(Print 0 52) ; "A warm feeling spreads down your leg. Your suit is not as white as it was!"
				else
					(NotNow) ; "Not now!"
				)
			)
			((Said 'climb>')
				(if (Said '/brick,building')
					(Print 0 53) ; "Climbing the walls already?"
				else
					(Print 0 54) ; "That would be unbecoming a man of leisure."
					(event claimed: 1)
				)
			)
			((Said 'hop')
			...
			((Said 'rob')
				(Print 0 58) ; "That's against the law!"
			)
			((Said 'cheat')
				(Print 0 59) ; "Ok, you win.\05"
				(Print 0 60 #at -1 152) ; "(Game over.)"
				(= gQuit 1)
			)
			((Said '(drop<on),wear>')
				(cond
					((Said '[/!]')
						(Print 0 61) ; "What do you want to wear?"
					)
					((= temp1 (gInventory saidMe:))
						(if (not (gEgo has: (gInventory indexOf: temp1)))
							(DontHave) ; "You don't have it."
						else
							(Print 0 62) ; "You'd look better if you didn't!"
						)
					)
					(else
						(Print 0 63) ; "You can't wear what is not yours."
						(event claimed: 1)
					)
				)
			)
			((Said 'drop>')
				(cond
					((Said '[/!]')
						(Print 0 64) ; "What do you want to drop?"
					)
					((= temp1 (gInventory saidMe:))
						(if (not (gEgo has: (gInventory indexOf: temp1)))
							(DontHave) ; "You don't have it."
						else
							(Print 0 65) ; "Nah. Better not!"
						)
					)
					(else
						(Print 0 66) ; "It's not yours to drop."
						(event claimed: 1)
					)
				)
			)
			((Said 'throw>')
				(cond
					((Said '[/!]')
						(Print 0 67) ; "What do you want to throw?"
					)
					((= temp1 (gInventory saidMe:))
						(if (not (gEgo has: (gInventory indexOf: temp1)))
							(DontHave) ; "You don't have it."
					...
					(else
						(Print 0 68) ; "That doesn't belong to you."
						(event claimed: 1)
					)
				)
			)
			((Said 'smell')
				(Print 0 69) ; "Sniff, sniff."
			)
			((Said 'wear/crown')
				(Print 0 70) ; "You'll have to buy "King's Quest IV" to do that!"
			)
			((Said '/microfiche')
				(Print 0 71) ; "Shh! You're not supposed to know about that."
			)
			((Said 'whistle')
				(Print 0 72) ; "You whistle a happy tune."
				(Print 0 73 #at -1 152) ; "(You envision Opie on a dirt road.)"
			)
			((Said 'laugh')
				(Print 0 74) ; "Ha, ha!"
			)
			((Said 'eat')
				(Print 0 75) ; "Once you tasted it, you wouldn't do that any more!"
			)
			((Said 'lie,bath,nap')
				(Print 0 76) ; "There's no time!"
			)
			((Said 'aid')
				(Print 0 77) ; "Don't whine!"
			)
			((Said 'explore>')
				(if (Said '/bra,me')
					(Print 0 78) ; "Searching through your pockets you find..."
					(gInventory showSelf: gEgo)
				else
					(event claimed: 1)
					(Print 0 79) ; "You find nothing."
				)
			)
			((or (Said '/key') (Said 'unbolt,bolt'))
				(Print 0 80) ; "You can't. There's no key in this game!"
			)
			((or (Said '/fuck/you') (Said 'fuck/you'))
				(Print 0 81) ; "Same to you, buddy!"
			)
			((Said 'fuck/*')
				(Print 0 82) ; "Yeah, you probably would, too!"
			)
			((Said 'jack')
				(Print 0 83) ; "Aren't your palms hairy enough?"
			)
			((Said 'caress,caress,embrace,look/clit')
				(Print 0 84) ; "Obviously, restraint is no problem for you, Larry."
			)
			((Said 'eat/woman')
				(Print 0 85) ; "That comes later."
			)
			((Said 'drink')
				(Print 0 86) ; "Nobody's THAT thirsty!"
			)
			((Said 'embrace')
				(Print 0 87) ; "Wouldn't you rather just remain friends?"
			)
			(
				(or
					(Said 'eat,fuck/me')
			...
			((Said '/hell')
				(Print 0 89) ; "That's probably where you'll end up!"
			)
			((Said '/heck')
				(Print 0 90) ; "Aren't we the little prissy one!"
			)
			((Said 'look>')
				(cond
					((Said '/*/man,woman,children>')
						(cond
							((not (= temp1 (gInventory saidMe:)))
								(event claimed: 1)
								(Print 0 91) ; "It's not yours."
							)
							((not (temp1 ownedBy: gEgo))
								(DontHave) ; "You don't have it."
							)
							(else
								(Print 0 92) ; "You have no reason to do that."
								(event claimed: 1)
							)
						)
					)
					((Said '/bra,me')
						(if (== 100 (gEgo view:))
							(Print 0 93) ; "You're wearing the latest in fashion."
							(Print 0 94 #at -1 152) ; "(If you consider 1971 "late")!"
						)
						(Print 0 78) ; "Searching through your pockets you find..."
						(gInventory showSelf: gEgo)
					)
					(
						...
						(Print 0 95) ; "Oops, wrong game! If you want to see that, you'll have to buy "Lounge Lizards.""
					)
					((Said '/bush,palm')
						(Print 0 96) ; "You see many leaves."
					)
					((Said '/man,woman,children')
						(Print 0 97) ; "Where?"
					)
					((Said '/brick,building')
						(Print 0 98) ; "Staring at the walls already?"
					)
					((Said '/carpet,down')
						(Print 0 99) ; "It just lies there, under your feet."
					)
					((Said '/cloud,ceiling')
						(Print 0 100) ; "It's still up there!"
					)
					((Said '<in/cup')
						(Print 0 101) ; "This game does not encourage voyeurs!"
					)
					((Said '/bikini')
					...
					(else
						(switch (Random 42 44)
							(42
								(Print 0 102) ; "It's just as it appears."
							)
							(43
								(Print 0 103) ; "It doesn't look interesting."
							)
							(44
								(SeeNothing) ; "You see nothing special."
							)
			...
			((or (Said 'apply,buy/bill,million,buck,wad') (Said 'buy'))
				(if (or (gEgo has: 1) (gEgo has: 6) (gEgo has: 4)) ; Dollar_Bill, Wad_O__Dough, Million_Dollar_Bill
					(Print 0 104) ; "There's nothing worth buying here."
				else
					(Print 0 105) ; "You're broke!"
				)
			)
			((Said 'apply>')
				(= temp1 (gInventory saidMe:))
				...
				(cond
					((Said '/ticket')
						(if
							...
							(Print 0 106) ; "There's no need to use the ticket here."
						else
							(Print 0 107) ; "What ticket?"
						)
					)
					((Said '[/!]')
						(Print 0 108) ; "What do you want to use?"
					)
					((not temp1)
						(event claimed: 1)
						(CantDo) ; "You can't do that here; at least, not now."
					...
					(else
						(Print 0 109) ; "There's no need to use it here."
						(event claimed: 1)
					)
				)
			)
			((Said 'give>')
				...
				(cond
					...
					((Said '[/!]')
						(Print 0 111) ; "What do you want to give?"
					)
					((not temp1)
						(event claimed: 1)
						(Print 0 112) ; "It's not yours to give."
					)
					((not (temp1 ownedBy: gEgo))
						(DontHave) ; "You don't have it."
					)
			...
			((Said 'get>')
				(cond
					((Said '[/!]')
						(Print 0 113) ; "What do you want to take?"
					)
					((and (= temp1 (gInventory saidMe:)) (temp1 ownedBy: gEgo))
						(Print 0 114) ; "You already have it."
					)
					(else
						(switch (Random 33 35)
							(33
								(Print 0 115) ; "It's of no use."
							)
							(34
								(Print 0 116) ; "You don't need it."
							)
							(35
								(Print 0 117) ; "There's no reason to take it."
							)
						)
						(event claimed: 1)
					)
			...
			((= temp1 (gInventory saidMe:))
				(if (not (temp1 ownedBy: gEgo))
					...
					(Print 0 118) ; "You can't do that now."
				)
			)
			((Said 'talk>')
				(if (Said '[/!]')
					...
					(Print 0 119) ; ""Hello," you say."
					(Print 0 120 #at -1 152) ; "(There is no response.)"
					(event claimed: 1)
				)
			)
			(
			...
			((or (Said '//woman') (Said '/woman/') (Said '/woman'))
				(Print 0 122) ; "She's not interested in that!"
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(1
				...
				(if
					...
					(Print 0 124) ; "Dial (209) 683-6858 from 8-5 Pacific Time. Have your credit card handy."
				)
				(repeat
					(switch
...
(instance statusCode of Code
	...
	(method (doit str)
		(Format str 0 126 gScore gPossibleScore 0 127 gRank 0 127) ; "Score: %d of %d%15s Rank: %s%10s"
	)
)
