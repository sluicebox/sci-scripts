	...
	(method (getDisc param1 param2 &tmp temp0 temp1)
		...
		(while (!= temp0 param1)
			(if (and (> argc 1) param2)
				(= temp0 (CD 0 param1 1)) ; Check
			else
				(= temp0 (CD 0 param1 0)) ; Check
			)
			(if (== temp0 99)
				(break)
			)
...
(class PasswordFile of File
	...
	(method (init)
		(= scratch (Str newWith: 128 {}))
		(KString 5 (scratch data:) 0 128 0) ; StrFill
		(scratch cat: {BUTTON})
		(= name (scratch data:))
	...
	(method (hasPassword &tmp temp0 temp1 temp2 temp3)
		(if (self readPassword:)
			(= temp0 (Str newWith: 30 {}))
			(KString 5 (temp0 data:) 0 30 0) ; StrFill
			(temp0 cat: password)
			(temp0 upper:)
			(if scratch
				(= temp3 3)
	...
	(method (readPassword &tmp temp0 temp1 temp2 temp3)
		(if (self open: 1)
			(= temp0 (Str newWith: 21 {}))
			(KString 5 (temp0 data:) 0 21 0) ; StrFill
	...
	(method (newPassword param1 &tmp temp0 temp1 temp2 temp3)
		...
		(if argc
			...
			(= temp1 (Str newWith: 30 {}))
			(KString 5 (temp1 data:) 0 30 0) ; StrFill
			(temp1 cat: password)
			(temp0 cat: temp1)
			(temp1 dispose:)
		)
