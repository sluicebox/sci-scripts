...
(instance Room49 of Rm
	...
	(method (doit)
		...
		(if (and (& (gEgo onControl: 0) $0010) (not local10))
			(HandsOn)
			(gCurRoom newRoom: 51)
		)
		...
		(if (and (& (gEgo onControl: 0) $0020) (not local10))
			(HandsOn)
			(gCurRoom newRoom: 50)
		)
		(if (and (& (gEgo onControl: 0) $0400) (not local9) (not local10))
			(= local9 1)
			(HandsOff)
			...
			((== (event type:) evSAID)
				(cond
					...
					((Said 'get,rob/chicken')
						(cond
							((not local3)
								(Print 49 33) ; "Do you see one here?"
							)
							((gEgo has: 33) ; Magic_Hen
					...
					((Said 'kill/giant')
						(Print 49 34) ; "You have no way of killing a giant ogre!"
					)
					(
						(or
							(Said 'kiss/giant')
						...
						(Print 49 35) ; "He wouldn't like it, and neither would you!"
					)
					((Said 'throw,give,feed/bone')
						(if (gEgo has: 23) ; Bone
							(if (> (gEgo distanceTo: local4) 40)
								...
								(Print 49 36) ; "It's too late for that, Rosella!"
							)
						else
							(Print 49 37) ; "Sorry, you don't have it to throw!"
						)
					)
					((Said 'get,rob/bone')
						(if (== ((gInventory at: 23) owner:) 49) ; Bone
							(Print 49 38) ; "That's REALLY not a good idea!"
						else
							(Print 49 39) ; "What bone?"
						)
					)
					((Said 'get,capture/giant')
						(Print 49 40) ; "That's ridiculous."
					)
					((and (Said 'give>') (= temp0 (gInventory saidMe:)))
						(if (and temp0 (gEgo has: (gInventory indexOf: temp0)))
							(if (gCast contains: local0)
								(Print 49 41) ; "You don't want to get that close to the ogre."
							else
								(Print 49 42) ; "The dog would not be interested in it."
							)
							(event claimed: 1)
						else
							(Print 49 43) ; "You don't have that."
							(event claimed: 1)
						)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				...
				(Print 49 44 #at -1 30 #draw) ; "Got ya!"
				(theMusic number: 6 loop: 1 play:)
				(Print 49 45 #at -1 20) ; "Oh, no! You're caught! The terrible ogre grabs you by the braids and drags you off to an untimely end. Dinner will definitely be on YOU tonight!"
			)
			(14
				(local0
					view: 79
			...
			(20
				(Print 49 46 #at -1 20) ; "You see the ogre sitting at the table."
				(ogreAwake state: 0)
				((= local0 (Act new:))
					illegalBits: 0
					view: 256
			...
			(30
				(if (== gPrevRoomNum 51)
					(Print 49 47 #at -1 20) ; "You see the ogre sleeping at the table. You had better be real quiet..."
				)
			)
		)
	)
		...
		(switch (= state newState)
			...
			(2
				...
				(cond
					((gEgo has: 33) ; Magic_Hen
						(if (not local12)
							(Print 49 48 #time 4) ; ""SQUAWK! SQUAWK!" Oh, oh! The hen has woken the ogre!"
						)
					)
					((== global165 3)
						(Print 49 49) ; "With a grunt, the ogre suddenly wakes up! Noticing you, he quickly jumps from his chair and heads your way."
					)
				)
				(theMusic number: 5 loop: -1 play:)
			...
			(5
				...
				(if (gEgo has: 33) ; Magic_Hen
					(Print 49 50 #draw) ; "Now you've gone and done it! The vicious ogre grabs you and drags you toward the kitchen. As to what happens in the kitchen, let's just say that you are cordially invited to BE dinner."
				else
					(Print 49 45 #draw) ; "Oh, no! You're caught! The terrible ogre grabs you by the braids and drags you off to an untimely end. Dinner will definitely be on YOU tonight!"
				)
				(theMusic number: 6 loop: 1 play:)
				(gEgo dispose:)
				(local0 view: 78 setCycle: End self)
...
(instance ogressChase of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 49 51 #at -1 20 #draw) ; "Poor Rosella! Looks like you'll join the deer in the stew pot tonight!!"
				(= seconds 4)
			)
			(3
				(= gDeathFlag 1)
...
(instance chaseEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 49 52 #at -1 20 #draw) ; "Watch out! A VERY unfriendly bulldog rushes toward you!"
				(local4
					setAvoider: (Avoid new:)
					setCycle: Fwd
					ignoreActors: 1
			...
			(5
				(local8 dispose:)
				(Print 49 53 #draw #at -1 20) ; "You weren't fast enough, Rosella! You were nothing but a bite-sized morsel for that big dog!"
				(= gDeathFlag 1)
			)
		)
	)
...
(instance catchBone of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 49 54 #width 290 #at -1 20 #dispose) ; "Quickly, you toss the bone to the big dog who catches it in mid-air. Happy with his surprise gift, he takes it to his favorite rug, lies down, and begins to gnaw at it. He seems to have completely forgotten you."
