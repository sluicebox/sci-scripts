...
(instance cells of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(switch local31
				(local44
					(Print 450 0) ; "Oh, pardon me, Brothers! I didn't mean to interrupt such a tender scene of..ahem...brotherly love."
				)
				(local45
					(Print 450 1) ; "Larry? Patty? What are YOU doing here?! No, this ISN'T the Nottingham Swinging Singles Health Spa."
				)
				(local46
					(Print 450 2) ; "Ah, in this cell a monk is counting the number of angels dancing upon the head of a pin."
				)
				(else
					(Print 450 3 #icon 450 0 0) ; "These Monks don't lead such a hard life with quarters like these. But they've nothing I want."
				)
			)
		else
			(super doVerb: theVerb &rest)
