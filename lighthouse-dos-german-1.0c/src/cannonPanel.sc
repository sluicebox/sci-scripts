	...
	(method (drag param1 &tmp temp0 temp1 temp2 temp3)
		...
		(while (not (& (param1 type:) $0002))
			...
			(if (or (!= temp0 (param1 x:)) (!= temp1 (param1 y:)))
				(= temp0 x)
				(= temp1 y)
				(PrintDebug {xOff=%d yOff=%d} temp2 temp3)
				(self x: (- (param1 x:) temp2) y: (- (param1 y:) temp3))
				(UpdateScreenItem self)
				(FrameOut)
			)
			(GetEvent evALL_EVENTS param1)
		)
		(self setPri: -1)
		(gTheCursor show:)
	)
...
(class SelectPart of PanelView
	...
	(method (handleEvent event &tmp temp0 temp1)
		...
		(cond
			...
			((not (& (event type:) $0fff)) ; extMouse | speechEvent | joyEvent | direction | evKEYUP | evMOUSEKEYBOARD | evMOUSERELEASE | $0c80
				(if (self onMe: event)
					(self highlight: 1)
				else
					(self highlight: 0)
