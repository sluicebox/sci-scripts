...
(class MenuBar of MenuBar
	...
	(method (handleEvent event &tmp [temp0 3] temp3 [temp4 320] temp324)
		(switch (super handleEvent: event)
			(257
				(Print
					(Format @temp4 997 0 gVersion) ; "DESIGNED BY: Jim Walls ILLUSTRATED BY: Vu Nguyen, Cheryl Cornelius PROGRAMMED BY: Jerry Shaw, Bob Fischbach, David Slayback, Bob Heitman, Mickie Lee, Chris Hoyt MUSIC BY: Mark Seibert Version %s"
					#title
				...
				(Print
					(Format @temp4 997 1) ; "DEVELOPMENT SYSTEM BY: Jeff Stephenson, Robert E. Heitman, Pablo Ghenis, Stuart Goldstein QUALITY ASSURANCE BY: Jennifer Cobb, Dennis Jonathan, Bridget Wight, Randy MacNeil, Kathy Sands, Jodi Shirk, Kim Woods, Chad Bye, Robin Bradley, Kevin Smith"
					#title
			...
			(516
				(if (Print 997 3 #button {Restart} 1 #button {Continue} 0 #icon 999 0 0) ; "Do you really want to  restart your game?"
					(gGame restart:)
				)
			)
			(517
				(= gQuit
					(Print 997 4 #button {Quit} 1 #button {Continue} 0 #icon 999 0 0) ; "Are you really turning   in your badge?"
				)
			)
			(769
				(= temp324 (Sound pause: 1))
				(Print 997 5 #title {Game Paused} #icon 999 1 0 #button {Let's roll.} 1) ; "Every cop needs a 10-7 (break) now and then. Keith will smoke a cigarette until you return."
				(Sound pause: temp324)
			)
			(770
				(gInventory
			...
			(1025
				(if
					(!=
						(= temp3
							(GetNumber
								(Format @temp4 {Current speed: %d} gSpeed)
							)
						)
						-1
					)
