...
(instance napoleon of Room
	...
	(method (init)
		(if (and (DoSound sndGET_AUDIO_CAPABILITY) (== (gGame detailLevel:) 3))
			(Load 140 461) ; WAVE
		)
		(DoAudio 13 local0) ; AudChannels
		(if (== (gGame detailLevel:) 3)
			(gGkSound2 setLoop: -1 number: 461 flags: 5 play:)
		)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(sam setSpeed: 9)
				(gMessager say: 15 46 local5 1 self)
			)
			(1
				(sam view: 4622 loop: 0 cel: 0 setCycle: End self)
				(samBottle show:)
				(gMessager say: 15 46 local5 2 self)
			)
			(2 1)
			(3
				(if (not (IsFlag 258))
					(sam loop: 1 cel: 0 setCycle: End)
					(gMessager sayRange: 15 46 local5 3 4 self)
				else
					(= cycles 1)
				)
			)
			...
			(9
				...
				(gMessager sayRange: 15 46 local5 (- temp0 1) temp0 self)
			)
			(10
				(= local7 (= local8 (= local9 0)))
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(43
				...
				(sam view: 466 loop: 0 cel: 0 approachX: 158)
				(gGame handsOn:)
				(self dispose:)
			)
		)
...
(instance getBracelet of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo setMotion: PolyPath 158 127 self)
...
(instance markus of Actor
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 10) ; Ask
				(gMessager say: 16 10 (if local1 10 else 9) 0)
				(++ local1)
			)
		...
		(switch theVerb
			...
			(46 ; oil
				(if (and (& [global182 6] $0001) (& [global182 7] $4000))
					(self setCycle: 0)
					(self setScript: 0)
					(= local5 (if (not (IsFlag 258)) 26 else 30))
					(if (not (IsFlag 258))
						(gEgo getPoints: -999 1)
					)
...
(instance oilTeller of GKTeller
	...
	(method (cue)
		(switch iconValue
			...
			(28 ; Ever wonder why Markus wins every time?
				(++ local8)
				(= iconValue (if local7 44 else 43))
				(self sayMessage:)
			)
			...
			(25 ; This Voodoo oil could make a NUN get lucky.
				(++ local7)
				(= iconValue (if local8 47 else 46))
				(self sayMessage:)
			)
			...
			(36 ; Never mind. It probably wouldn't work anyway.
				(= iconValue (if local8 32 else 31))
				(self sayMessage:)
			)
