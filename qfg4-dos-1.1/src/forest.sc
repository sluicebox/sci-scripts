...
(procedure (localproc_1)
	(for
		...
		((= global475 (local15 at: (Random 0 3))))
	)
	(return global475)
)
	...
	(method (keepChasing)
		...
		(cond
			((and global462 (== self local10))
				(+= x (Random -20 20))
				(= cel (Random 0 5))
				(local10 setCycle: Walk setScript: (sChaseHim new:))
			)
			((and global463 (== self local11))
				(+= x (Random -20 20))
				(= cel (Random 0 5))
	...
	(method (getHurt param1 param2)
		(if
			(or
				...
				(and (== self local10) (not (local10 dead:)))
			...
			(switch self
				(local10
					(if (and (> global462 0) (< (-= global462 param2) 1))
						(= global462 0)
						(local10 setScript: monsterDies2)
					)
				)
				(local11
					(if (and (> global463 0) (< (-= global463 param2) 1))
						(= global463 0)
						(local11 setScript: monsterDies3)
					)
				)
				(local12
					(if (and (> global464 0) (< (-= global464 param2) 1))
						(= global464 0)
						(local12 setScript: monsterDies4)
					)
				)
				(it
					(if (and (> global156 0) (< (-= global156 param2) 1))
						(= global156 0)
...
(instance forest of Rgn
	...
	(method (init &tmp temp0 temp1)
		(= local14 (IntArray with: 0 6 4 2))
		(= local15 (IntArray with: 0 0 0 0))
		(= local17 (IntArray with: 0 60 63 80))
		(= local16
			(IntArray
				with:
					551
		...
		(= local13 (local17 at: (/ (- (gCurRoom picture:) 400) 10)))
		(local15 at: 0 (gCurRoom north:))
		(local15 at: 1 (gCurRoom south:))
		(local15 at: 2 (gCurRoom east:))
		(local15 at: 3 (gCurRoom west:))
		(if (OneOf gCurRoomNum 575 593 588 551)
			(= global475 -1)
		else
			(= global475 (localproc_1))
		)
		(gEgo init: normalize: setScaler: Scaler 100 40 147 61)
		(super init: &rest)
		(if (!= gPrevRoomNum 810) ; combat
			(cond
				((IsFlag 35)
				...
				(gCombatMonsterNum
					(if (and (!= gCombatMonsterNum 870) (> (++ global470) (Random 3 6))) ; nectar
						...
						(if (> global462 0)
							((= local10 (it new:)) init: keepChasing:)
						)
						(if (> global463 0)
				...
				(else
					...
					(if
						(and
							(> global366 (local14 at: global439))
							(<
								(Random 0 100)
								(*
									(if gNight 10 else 5)
						...
						(if
							...
							(if (== gCombatMonsterNum 825) ; badder
								((= local10 (it new:)) init: startChasing:)
		...
		(switch gPrevRoomNum
			((gCurRoom west:)
				...
				(= local0 (+ 7 (if gCombatMonsterNum 1 else 31)))
				(= local1 136)
				(gCurRoom setScript: enterRoomScr)
			)
			((gCurRoom east:)
				(gEgo posn: 335 136 setHeading: 270)
				(= local0 (- 313 (if gCombatMonsterNum 1 else 31)))
				(= local1 136)
				(gCurRoom setScript: enterRoomScr)
			)
			((gCurRoom north:)
				...
				(= local0 160)
				(= local1 (+ (gCurRoom horizon:) (if gCombatMonsterNum 1 else 21)))
			...
			((gCurRoom south:)
				...
				(= local0 160)
				(= local1 (- 182 (if gCombatMonsterNum 1 else 21)))
				(gCurRoom setScript: enterRoomScr)
			)
			(810 ; combat
				(if (!= gCombatMonsterNum 850) ; wraith
					...
					(switch gCombatResult
						...
						(3
							...
							(if (== gCombatMonsterNum 825) ; badder
								(if (> global462 0)
									((= local10 (it new:)) init: runChasing:)
								)
								(if (> global463 0)
...
(instance enterRoomScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (and gCombatMonsterNum (not local2))
					(switch gPrevRoomNum
						((gCurRoom west:)
							(if (gCurRoom east:)
								(gEgo
		...
		(switch (= state newState)
			...
			(1
				(switch gCombatMonsterNum
					...
					(870 ; nectar
						(EgoDead 13 50) ; "And all this time, you though Necrotaurs were vegetarians. Your final battle certainly ruled out THAT theory!"
					)
					(820
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(3
				...
				(gEgo setSpeed: local20 normalize: 2 addHonor: 5)
				(gGlory handsOn:)
				(self dispose:)
			)
		)
...
(instance bird of TargActor
	...
	(method (init &tmp temp0)
		(for ((= temp0 0)) (<= temp0 88) ((+= temp0 4))
			(if (== gCurRoomNum (local16 at: temp0))
				(= local3 (local16 at: (+ temp0 1)))
				(= local4
					(+ (local16 at: (+ temp0 2)) (local16 at: (+ temp0 3)))
				)
				(= local5 (local16 at: (+ temp0 3)))
			)
		)
		(if local3
			(super init: &rest)
			(self x: -100 y: local4 z: local5 setStep: 8 2 setScript: sBird)
		)
	)
...
(instance sBird of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				1
				(client y: (- (+ local4 25) (Random 0 50)) setScaler: gEgo)
				(= seconds (Random 1 12))
			)
			(2
				2
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(= seconds (+ 2 (/ [gEgoStats 25] 40))) ; calmSpell
			)
			(2
				(self changeState: 0)
			)
			...
			(1
				(if local19
					...
					(switch (it noun:)
						(34
							(= local18 (Str new:))
							(Message msgGET 50 (it noun:) 4 4 1 (local18 data:))
							(= temp0 (+ 3 (Random 0 5)))
							(= temp1 (+ 10 (Random 0 15)))
							(local18 format: (local18 data:) temp0 temp1)
							(+= global395 temp1)
							(gEgo get: 0 temp0) ; thePurse
							(gNarrator say: local18 self)
						)
						(32
							(= local18 (Str new:))
							(Message msgGET 50 (it noun:) 4 4 1 (local18 data:))
							(= temp1 (+ 15 (Random 25)))
							(local18 format: (local18 data:) temp1)
							(+= global395 temp1)
							(gNarrator say: local18 self)
						)
						(else
