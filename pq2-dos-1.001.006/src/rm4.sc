...
(instance drawerScript of Script
	...
	(method (handleEvent event)
		(switch (event type:)
			(evSAID
				(cond
					((Said 'get,remove/letter,note,card')
						(if (IsItemAt 5 12) ; thank_you_letter
							(local0 dispose:)
							(Print 4 83 #draw) ; "OK."
					...
					((Said 'drop,replace/note,letter,card')
					...
					(
						...
						(if (IsItemAt 5 12) ; thank_you_letter
							(Print 4 99) ; "You see it in the drawer."
						else
							(event claimed: 0)
						)
					)
...
(instance captainScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 4 100) ; "The Captain speaks up and says..."
				(aTimer setCycle: self 18)
			)
			(2
				(localproc_0 4 101) ; "Well Bonds, no doubt you heard about Jessie Bains by now."
				(aTimer setCycle: self 18)
			)
			(3
				(localproc_0 4 102) ; "The District Attorney told me that the reason this jackass won a retrial was some technical BS about improper jury instruction."
				(aTimer setCycle: self 10)
			)
			(4
				(localproc_0 4 103) ; "It's a real shame, but that's life."
				(if local3
					(User canInput: 1)
				else
					(HandsOn)
			...
			(7
				(Print 4 104 #at -1 140 #draw) ; "Bonds, we have bad news. Jessie Bains has escaped Lytton City Jail!"
				(aTimer setCycle: self 18)
				(bainsTheme play:)
			)
			(8
				(localproc_0 4 105) ; "Not only has he escaped," the Captain says,"but he took the jailer as hostage!"
				(aTimer setCycle: self 18)
			)
			(9
				(localproc_0 4 106) ; ""Bonds, I've named you and your partner Keith, to a special task force with the code name of 'Target.'"
				(aTimer setCycle: self 18)
			)
			(10
				(localproc_0 4 107) ; "The Captain continues:"I want you boys to answer any and all calls that might be even remotely connected to this dirtbag!""
				(aTimer setCycle: self 18)
			)
			(11
				(localproc_0 4 108) ; "The Captain orders you: "Find a mug shot of that punk, and get on this immediately!""
				(if local3
					(User canInput: 1)
				else
					(HandsOn)
			...
			(13
				(switch (Random 0 2)
					(0
						(localproc_0 4 109) ; "You have your orders, Bonds!"
					)
					(1
						(localproc_0 4 110) ; "You're wasting valuable time, Bonds!"
					)
					(2
						(localproc_0 4 111) ; "You're a detective. Start earning your money!"
					)
				)
				(if local3
					(User canInput: 1)
			...
			(15
				(switch (Random 0 1)
					(0
						(localproc_0 4 112) ; "Bonds," says the Captain, "go ahead and take off. You've earned your daily dollar."
					)
					(1
						(localproc_0 4 113) ; "You're on your own time boy....Go somewhere!"
					)
				)
				(if local3
					(User canInput: 1)
			...
			(17
				(gEgo loop: 1)
				(Print 4 114 #at -1 140 #draw) ; "Hey Bonds!" the Captain blurts. "We have a traffic cop standing by down in the old warehouse district with a 187 victim."
				(localproc_0 4 115) ; "Continuing, the Captain says: "The victim took it in the back of the head. Looks like it could be a professional hit.""
				(gEgo startUpd:)
				(aTimer setCycle: self 18)
			)
			(18
				(localproc_0 4 116) ; "Sonny! The address is 160 West Rose. Take Keith and get over there!"
				(captain setCycle: Beg)
				(if local3
					(User canInput: 1)
				else
			...
			(20
				(gEgo loop: 1)
				(Print 4 117 #at -1 140 #draw) ; ""Keith just told me about the motel incident!" the Captain says in an excited voice."
				(localproc_0 4 118) ; "Good thing Keith was there with you, keeping a cool head and following procedures."
				(aTimer setCycle: self 18)
			)
			(21
				(localproc_0 4 119) ; "The Captain continues... "I've been thinking....I don't like what I see.""
				(localproc_0 4 120) ; "Point #1:" says Fletch, "The murder victim, Woody Roberts, testified against Bains at his trial last year."
				(aTimer setCycle: self 18)
			)
			(22
				(localproc_0 4 121) ; "Point #2:" he continues, "Bonds, you also testified against Bains last year."
				(aTimer setCycle: self 18)
			)
			(23
				(localproc_0 4 122) ; "Point #3: You were led into a booby trap at the motel. If you ask me," he says, "this whole thing is shaping up as a vendetta!"
				(aTimer setCycle: self 18)
			)
			(24
				(localproc_0 4 123) ; "In a worried tone of voice, the Captain says,"You boys better start digging. We appear to have a vengeful maniac on our hands!""
				(blab dispose:)
			...
			(25
				...
				(gEgo loop: 1)
				(if (not (IsFlag 27))
					(Print 4 124 #at -1 140 #draw) ; "Barely able to contain your emotions, you quickly tell Captain Hall about the signs of a struggle at Marie's."
				)
				(captain setCycle: End)
				(aTimer setCycle: self 18)
			)
			(26
				(gEgo loop: 1)
				(Print 4 125 #at -1 140 #draw) ; ""Sorry to hear about Marie, Sonny," Captain Fletcher offers."
				(aTimer setCycle: self 8)
			)
			(27
				(localproc_0 4 126) ; "If you boys have a solid lead, now's the time to bring it up."
				(blab hide:)
				(HandsOn)
				(= local7 1)
			)
			...
			(29
				(if (gEgo has: 13) ; hit_list
					(localproc_0 4 127) ; "The Captain takes the victim list from you and begins to read."
				else
					(localproc_0 4 128) ; "Captain Fletcher Hall says, "While you were out, I dug around in the files and came up with a list of folks who testified against Bains.""
					(localproc_0 4 129) ; "They are..." he says, reading the list, "Woody Roberts, Don Colby, Marie Wilkans, and Sonny Bonds."
				)
				(aTimer setCycle: self 12)
			)
			(30
				(Print 4 130 #at -1 120) ; "Huummm," the Captain mumbles. "Everyone on this list testified against Bains, and are now either dead or missing. Except you and Colby."
				(localproc_0 4 131) ; "The question is, Sonny: will he go after you or Colby next?"
				(aTimer setCycle: self 18)
			)
			(31
				(cond
					((gEgo has: 35) ; Colby_s_business_card
						(localproc_0 4 132) ; "Captain Hall takes the card from you. "So, Bains knows where Colby is!""
						(localproc_0 4 133) ; "This chump Bains," he continues, "most likely has kidnapped Marie to use her as a hostage. He's trying to sucker you into another set-up, Bonds."
						(localproc_0 4 134) ; "A maniac like Bains is hard to read, boys," Captain Hall muses. "You've got a blank check to proceed however you feel is best."
					)
					((IsFlag 136)
						(localproc_0 4 135) ; "I've looked over the evidence and see that Bains now knows where Colby is."
						(localproc_0 4 133) ; "This chump Bains," he continues, "most likely has kidnapped Marie to use her as a hostage. He's trying to sucker you into another set-up, Bonds."
						(localproc_0 4 134) ; "A maniac like Bains is hard to read, boys," Captain Hall muses. "You've got a blank check to proceed however you feel is best."
					)
					(else
						(localproc_0 4 136) ; "Colby's where-abouts are unknown to Bains," Captain Hall thinks aloud, "so Bains will probably stick around to take another crack at you, Sonny."
					)
				)
				(aTimer setCycle: self 18)
			)
			(32
				...
				(if (or (gEgo has: 35) (gEgo has: 13)) ; Colby_s_business_card, hit_list
					...
					(localproc_0 4 137) ; "I'll book this evidence for you. You'd better decide what to do next... and FAST!"
				)
				(captain setCycle: Beg)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= global163 1)
				(Print 4 138 #at 110 40 #draw) ; "Your partner Keith takes a deep puff and says:"
				(Print 4 139 #at 110 40) ; "Say, Sonny...I heard that dirtbag you sent up the river last year is back in town for retrial. Isn't his name Bains, or something like that?"
				(Print 4 140 #at 110 40) ; "Maybe this time you can put him away for the duration."
				(Print 4 141 #at 110 40) ; "It just seems like the garbage of this world gets every chance known to man."
