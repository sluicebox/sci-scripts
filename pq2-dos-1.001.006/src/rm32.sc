		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'enter,enter[<in]/bedroom,(chamber<bed)')
						(if (gEgo inRect: 215 95 275 111)
							(cond
								((gCast contains: global112)
									(localproc_0 32 9) ; "Keith is taking care of it."
					...
					((or (Said 'dust>') (Said 'powder>') (Said 'apply/powder>'))
						(cond
							((Said '/ashtray')
								(if (gEgo inRect: 214 110 290 138)
									(if (not local3)
										(global122 setPri: 0)
										(global120 setPri: 0)
										(localproc_0 32 109 83) ; "Carefully, you apply the dust and ... mumbling to yourself ... "Here's one worth taking!""
										(= local2 2)
										(= local3 1)
										(global122 setPri: 15)
										(global120 setPri: 14)
									else
										(localproc_0 32 110) ; "You already did."
										(= local2 2)
										(= local3 1)
									)
								)
							)
							((Said '/table,couch,bench,counter,cabinet,phone')
								(if (gEgo has: 10) ; field_kit
									(global122 setPri: 0)
									(global120 setPri: 0)
									(localproc_0 32 111 83) ; "Carefully, you apply the fingerprint powder ... you examine the powder and think to yourself... "Not worth processing.""
									(global122 setPri: 15)
									(global120 setPri: 14)
									(= local2 1)
								else
							...
							((Said '/*')
								(localproc_0 32 112) ; "A wasted effort."
							)
						)
					)
					(
						...
						(if (gEgo has: 10) ; field_kit
							(cond
								((== local2 2)
									(global123 setPri: 0)
									(localproc_0 32 113 83) ; "Using the tape, you carefully transfer the print to a 3x5 card."
									(global123 setPri: 15)
									(gEgo get: 22) ; fingerprint
								)
								(local2
									(localproc_0 32 114) ; "There aren't any prints."
								)
								(else
									(localproc_0 32 115) ; "Don't you know the correct way to get fingerprints?"
								)
							)
							(global122 setPri: 15)
							(global120 setPri: 14)
					...
					((or (Said 'use,dial,pick[<up]/phone') (Said 'make/call'))
						(if (< global100 8)
							...
							(localproc_0 32 116) ; "It's been torn from the wall. It's inoperative."
						)
					)
					((Said 'use,enter/bathroom,(chamber<bath)')
						(if local1
							(localproc_0 32 117) ; "You can hold it."
						else
							(localproc_0 32 118) ; "First, go into the bathroom."
						)
					)
					((Said 'use/basin,oven,fridge')
						(localproc_0 32 119) ; "You don't need to do that now."
					)
					((Said 'clock,use,(turn<on)/television,set')
						(localproc_0 32 120) ; "You don't have time to watch TV."
					)
					((Said 'sat[<down]')
						(if (>= global100 8)
							(localproc_0 32 121) ; "You're too upset to sit and relax."
						else
							(localproc_0 32 122) ; "You have business to take care of, Bonds."
						)
					)
				)
			)
		...
		(switch (= state newState)
			...
			(2
				(if (not global127)
					...
					(localproc_0 32 1) ; "Oh, Marie!" you think..."What has that maniac done to you??!"
					(localproc_0 32 123) ; "Trying to keep your composure, you say to Keith..."
					(localproc_0 32 124) ; "OK, Keith, why don't you go check out the back of the house? I'll look around here."
				)
				(= cycles 5)
				(User canControl: 1)
				(= global127 1)
			...
			(6
				(localproc_0 32 125) ; "Hearing you open the door, Keith hollers: "Hey, Sonny! Wait up!""
				(gEgo setLoop: 3)
...
(instance bathroomScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (< global100 8)
					(localproc_0 32 126 83) ; "This is Marie's bathroom. As usual, it is neat and tidy. Marie is not here. You close the door."
				else
					(localproc_0 32 127 83) ; "There are signs of a struggle in Marie's bathroom. Marie is not here. You close the door."
				)
				(= seconds 2)
			)
			(2
