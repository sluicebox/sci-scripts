...
(instance ginRummy of HoyleRoom
	...
	(method (doit)
		...
		(if global290
			(if (not local7)
				...
				(Message msgGET 400 19 0 0 1 gStr) ; "You can't sort now."
			)
		)
	)
	...
	(method (dispose)
		...
		(Piles dispose:)
		(Dealer dispose:)
		(gTheHands eachElementDo: #dispose dispose:)
		(gKnownCards dispose:)
...
(class GinHand of Hand
		...
		(if bestDeadWood
			(if (<= (- bestDeadWood highestDeadWood) local22)
				...
				(if (= temp3 (- bestDeadWood highestDeadWood))
					(gChar1 say: 1000 1 0 (+ temp3 11) 16 711 1)
					(DisableCursor)
					(= local11 1)
				else
					(gChar1 say: 1000 1 0 22 16 711 1)
					(DisableCursor)
					(= local11 1)
				)
	...
	(method (ginSort &tmp temp0 temp1 temp2)
		(for ((= temp0 1)) (<= temp0 10) ((++ temp0))
			(for ((= temp1 2)) (<= temp1 10) ((++ temp1))
				(if (> ((self at: temp0) rank:) ((self at: temp1) rank:))
					(= temp2 (self at: temp0))
				)
			)
		)
			...
			(cond
				((> (local31 bestDeadWood:) local21)
					(if ((local31 opponent:) shouldIAcceptKnock:)
						(gChar1 say: 400 0 77 0 1 711 1)
						(= local32 (if (== local31 hand1) hand2 else hand1))
						(return 1)
					else
						(gChar1 say: 400 0 76 0 1 711 1)
						(for
							((= temp22 0))
							(< temp22 (hand1 size:))
							((++ temp22))
