...
(procedure (localproc_0)
	...
	((= gTheIconBar IconBar)
		...
		add:
			...
			((ScriptID 12 5) theObj: (ScriptID 12 5) selector: #doit yourself:) ; deckIcon, deckIcon
			((ScriptID 12 4) theObj: (ScriptID 12 4) selector: #doit yourself:) ; helpIcon, helpIcon
...
(procedure (proc0_8 param1)
	(if (OneOf gCurRoomNum 100 200 300 400 500 600 700 800) ; crazy8s, oldMaid, heartsGame, ginRummy, cribbage, klondike
		(if (and argc param1)
			(gSong stop:)
...
(class HoyleRoom of Room
	...
	(method (drawNewPic)
		(self drawPic: picture 13)
	)

	(method (doit &tmp temp0 temp1 temp2)
		(super doit: &rest)
		(if (!= (+ global385 global747) picture)
			(IconBar disable:)
			(= temp1 picture)
			(= picture (+ global385 global747))
			(DisableCursor)
...
(instance hoyle4 of Game
	...
	(method (startRoom roomNum)
		...
		(if global538
			(= global538 0)
			(gGame setCursor: 997 1 (mouseCursor x:) (mouseCursor y:))
		else
			(gGame setCursor: 997 1)
		)
	)
	...
	(method (doit param1 &tmp temp0 temp1 temp2)
		...
		(cond
			...
			((self open: 1)
				...
				(= temp2 ((Str with: temp1) asInteger:))
				(gGame masterVolume: temp2)
				(self readString: temp1 3)
				(= global384 ((Str with: temp1) asInteger:))
				(self readString: temp1 3)
				(= global385 ((Str with: temp1) asInteger:))
				(self readString: temp1 3)
				(= global387 ((Str with: temp1) asInteger:))
				(self readString: temp1 3)
				(= global386 ((Str with: temp1) asInteger:))
				(self readString: temp1 2)
				(= global462 ((Str with: temp1) asInteger:))
				(self readString: temp1 2)
				(= global121 ((Str with: temp1) asInteger:))
				(self close:)
				(temp0 dispose:)
				(temp1 dispose:)
				(return 0)
