...
(instance ginRummyScoreWindow of InvisibleWindow
	...
	(method (init &tmp temp0 temp1 temp2 temp3)
		...
		(cond
			(global111
				(proc0_10 480 3 0 34 7 15 ginRummyScoreWindow)
				(if
					(or
						(== (gChar1 view:) 1)
						(== (gChar1 view:) 5)
					...
					(proc0_10 480 1 3 178 8 15 ginRummyScoreWindow)
				else
					(proc0_10 480 1 4 178 8 15 ginRummyScoreWindow)
				)
				(= local8 4)
				...
				(if local3
					...
					(= temp3 [local37 0])
					(+= temp2 [local28 0])
					(proc0_10 480 1 0 103 8 15 ginRummyScoreWindow)
					(if (not [local37 0])
						(*= temp2 2)
					...
					(if
						...
						(proc0_10 480 1 1 103 8 15 ginRummyScoreWindow)
					else
						(proc0_10 480 1 2 103 8 15 ginRummyScoreWindow)
					)
					(if (not [local28 0])
						(*= temp3 2)
			...
			(global244
				(drawScore doit:)
				(= local8 8)
				(proc0_10 480 2 0 6 6 15 ginRummyScoreWindow)
				(if (and (== gGinRummyOptionIsVariant 1) local9)
					(proc0_10 480 2 3 5 28 15 ginRummyScoreWindow)
				)
				(cond
					(local3
						(proc0_10 480 1 0 103 8 15 ginRummyScoreWindow)
				...
				(cond
					(local5
						...
						(proc0_10 480 2 1 6 40 15 ginRummyScoreWindow)
					...
					((not local4)
						...
						(proc0_10 480 2 2 6 40 15 ginRummyScoreWindow)
						(localproc_0 local6 80 57 1)
					)
					(else
...
(instance scoreCode of Code
	...
	(method (doit &tmp temp0 temp1 temp2 [temp3 20])
		(= temp0 ((gTheHands at: 0) knockHand:))
		(= local6 0)
