		...
		(switch (= state newState)
			...
			(12
				(if
					...
					(proc0_28 160 220 3 67 50 10 25 4) ; "Come on, Cedric. Get in the boat!"
				)
				(= cycles 1)
			)
			(13
				(if
					...
					(proc0_28 75 220 4 67 10 10 25 4) ; "Aye, aye, Captain!"
				)
				(= cycles 1)
			)
			(14
...
(instance fixBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(PrintDC 220 5) ; "Graham firmly wedges the softened piece of beeswax into the small hole in the boat's hull. Hopefully, the wax will hold and make her seaworthy."
				(HandsOn)
				(= cycles 1)
			)
			(7
			...
			(switch (event message:)
				(2 ; Look
					(cond
						((and (not (IsFlag 55)) (IsFlag 54) (IsFlag 71))
							(PrintDC 220 6) ; "Having found help for Cedric from the hermit, Graham decides it would be best to sail there rather than risk hurting him further by carrying him."
						)
						((and (not (IsFlag 55)) (IsFlag 54))
							(PrintDC 220 7) ; "With Cedric quietly moaning from the bottom of the boat, Graham decides it would be best to leave him there and go find help."
						)
						(
							(or
							...
							(PrintDC 220 8) ; "An old, cast-off sailboat sits forlornly on the narrow, sandy beach."
						)
						(else
							(PrintDC 220 9) ; "Graham notices a small hole in the bottom of the old sailboat."
						)
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (not (IsFlag 94))
						...
						(PrintDC 220 10) ; "Graham is not strong enough to push the massive boat."
					)
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 220 11) ; "That has no effect on the boat."
							(event claimed: 1)
						)
					)
				)
		...
		(if
			...
			(switch (event message:)
				(2 ; Look
					(cond
						((and (not (IsFlag 55)) (IsFlag 54))
							(PrintDC 220 13) ; "Cedric quietly moans from the bottom of the boat."
						)
						(
							(or
								(> (gEgo distanceTo: self) 40)
