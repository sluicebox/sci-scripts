	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 2) ; "A crude, misshapen structure, which seems to be some sort of house, sits forlornly; surrounded by a hot, deep crevasse."
					(event claimed: 1)
				)
			)
		)
...
(instance bridge of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 3) ; "A grotesque bridge of rib-like bones crosses the hot crevasse where a path continues to the front door of the odd little house."
					(event claimed: 1)
				)
			)
		)
...
(instance pathWay of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 4) ; "The path seems to dwindle down to nothing as it nears the strange structure."
					(event claimed: 1)
				)
			)
		)
...
(instance crevasse of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 5) ; "Spewing hot steam and flames, a deep crevasse surrounds the strange little house."
					(event claimed: 1)
				)
			)
		)
...
(instance door of Door
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 6) ; "A small door fronts the odd structure."
					(event claimed: 1)
				)
				(3 ; Do
					(if (== ((gInventory at: 6) owner:) 200) ; Brass_Bottle
...
(instance thisRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 22 7) ; "The dark forest seems to encase the witch's bizarre little house like a claustrophobic shroud."
					(event claimed: 1)
				)
			)
		)
