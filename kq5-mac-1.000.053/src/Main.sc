...
(procedure (proc0_15 param1 param2 param3 param4 &tmp temp0 temp1 temp2)
	(if global156
		(Display 0 31 dsRESTOREPIXELS global157)
		(Display 0 31 dsRESTOREPIXELS global156)
		(= global156 0)
	)
	(if param1
...
(instance KQ5 of Game
	...
	(method (restart param1)
		(if (if argc param1 else 1)
			(if (Print 0 25 #button {Yes} 1 #button {No} 0) ; "Do you really want to restart?"
				(super restart:)
			)
		else
			(super restart:)
	...
	(method (quitGame)
		(super quitGame: (Print 0 26 #button {Yes} 1 #button {No} 0)) ; "Do you really want to quit?"
		(DoAudio audSTOP)
	)
	...
	(method (startRoom roomNum &tmp temp0)
		...
		(if
			(and
				...
				(Print 0 27 #button {Who cares} 0 #button {Debug} 1) ; "Memory fragmented."
			)
			(SetDebug)
		)
			...
			(4 ; Inventory
				(cond
					((== (gInventory indexOf: (gTheIconBar curInvIcon:)) 28)
						(if (IsFlag 60)
							(PrintDC 0 28) ; "Graham waves Crispin's wand which now pulsates with renewed power, but nothing happens."
						else
							(PrintDC 0 29) ; "Graham gives the wand a good shake, but it only fizzles and dies."
						)
						(event claimed: 1)
					)
					((== (gInventory indexOf: (gTheIconBar curInvIcon:)) 35)
						(PrintDC 0 30) ; "Graham waves Mordack's glowing wand...but nothing happens. Could it be that Crispin was right? That wands only respond to their owner...like a pet?"
						(event claimed: 1)
					)
				)
			)
