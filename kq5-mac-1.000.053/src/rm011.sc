...
(instance rm011 of KQ5Room
	...
	(method (init)
		...
		(= global320 141)
		(= global321 66)
		(= global325 {"We've got things to do, Graham! Come on!"})
			...
			((not local9)
				...
				(cond
					...
					((and (not (IsFlag 86)) (not (IsFlag 36)))
						(SetFlag 86)
						(proc0_28 75 11 0 67 20 20 25 6) ; "Oh, oh! Stay away from those bees!"
					)
				)
			)
			(
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 1) ; "The swift river turns sharply to the south here."
					(event claimed: 1)
				)
				(3 ; Do
					(gEgo
...
(instance relievedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(proc0_28 75 11 2 67 20 100 25 7) ; "Well, there you are! I was starting to get concerned!"
				(proc0_28 160 11 3 67 20 20 25 8) ; "Don't worry about me, Cedric. I'm used to this kind of thing."
				(= cycles 1)
			)
			(3
				(HandsOn)
...
(instance drownHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(proc0_28 75 11 4 67 20 20 25 3) ; "No, don't...!"
				((gEgo head:) hide:)
				(gEgo
					normal: 0
					view: 69
...
(instance bearScript of Script
	...
	(method (doit &tmp temp0)
		...
		(cond
			((and (== state 6) (== -1 (gGlobalSound prevSignal:)))
				(self cue:)
			)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(4
				(gGlobalSound number: 17 loop: 1 vol: 127 play:)
				(bear
					view: 304
					posn: 101 142
					loop: 0
			...
			(7
				(if
					...
					(= global330 {Shouldn't feed the bears, Graham!})
					(EgoDead 262)
				)
			)
			(8
			...
			(14
				...
				(PrintDC 11 5 #width 150 #at 150 10 #dispose) ; "I am Queen Beetrice, kind sir. I wish to thank you ever so much for saving our hive from the claws of that horrible bear."
				(= seconds 10)
			)
			(15
				(cls)
				(PrintDC 11 6 #width 150 #at 150 10 #dispose) ; "In return, I offer you a luscious honeycomb from our hive. Please feel free to retrieve one. I promise my bees won't harm you. It may come in handy on your travels."
				(= seconds 10)
			)
			(16
				(cls)
...
(instance getWax of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(PrintDC 11 7) ; "Graham reaches a hand into the beehive and retrieves a very sticky chunk of honeycomb. Wrapping it in a protective piece of cloth, he then pockets it."
...
(instance deathByBees of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(= global330 {Mustn't mess around with bees!})
				(EgoDead 263)
			)
		)
	)
...
(instance getStick of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 11 8) ; "Graham bends down and picks up the large stick from the ground."
				((gEgo head:) hide:)
...
(instance tree of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 9) ; "A swarm of bees buzz around a hole in a rotted old tree."
					(event claimed: 1)
				)
			)
		)
...
(instance hive of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 10) ; "It appears the bees have a hive inside the hole of the old dead tree."
					(event claimed: 1)
				)
				(3 ; Do
					(if (!= ((gInventory at: 17) owner:) 11) ; Honeycomb
						(PrintDC 11 11) ; "Graham has already taken a honeycomb. It wouldn't be wise to antagonize the now-friendly bees."
					else
						(HandsOff)
...
(instance anthill of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 12) ; "Graham notices a huge anthill in the distance to the north."
					(event claimed: 1)
				)
			)
		)
...
(instance bear of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 13) ; "A large bear seems to be mighty interested in the honey inside the old tree."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 11 14) ; "Graham should be very careful around a big old bear!"
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 11 15) ; "The bear is too intent on the honey to listen to Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 11 16) ; "The bear is more interested in the honey."
							(event claimed: 1)
						)
					)
				)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(5 ; Talk
					(PrintDC 11 17) ; "Bees don't have much to say."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 11 18) ; "It would be wise if Graham didn't bother the bees."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 11 19) ; "The bees don't care about it."
							(event claimed: 1)
						)
					)
				)
...
(instance stick of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 20) ; "Graham notices a large stick lying on the ground near the old tree."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
...
(instance warnAboutBear of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(proc0_28 75 11 21 67 20 20 25 6) ; "Watch out for the bear, Graham!"
				(= cycles 1)
			)
			(1
				(if (== (gGlobalSound3 prevSignal:) -1)
					(gGlobalSound3 number: 851 loop: 1 vol: 127 play: self)
				else
					(= cycles 1)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo loop: 1)
				(cls)
				(proc0_28 75 11 22 67 10 10 25 10) ; "There's nothing but a hot, dry desert further west. Most people avoid it, because there are bandits out there! If you insist on going, I'll wait for you HERE!"
				(= cycles 1)
			)
			(1
				(HandsOn)
