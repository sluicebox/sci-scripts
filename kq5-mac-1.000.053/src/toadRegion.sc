...
(instance toad of Actor
	...
	(method (doit)
		...
		(if (== cel 1)
			(cond
				((!= gCurRoomNum 22)
					(gGlobalSound4 number: (Random 99 100) loop: 1 play:)
				)
				((or (== script hopAbout) (== ((gInventory at: 6) owner:) 200)) ; Brass_Bottle
					(gGlobalSound4 number: (Random 99 100) loop: 1 play: 40)
				)
			)
		)
		(return
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 551 0) ; "It seems to Graham that there is an unusually large number of toads in this forest."
					(event claimed: 1)
				)
				(3 ; Do
		...
		(switch (= state newState)
			...
			(1
				(toad
					...
					stopUpd:
					setCel: 0
					setScript: (if (> (gGame detailLevel:) 1) hopAbout else 0)
				)
			)
		)
	)
