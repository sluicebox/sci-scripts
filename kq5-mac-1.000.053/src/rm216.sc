		...
		(cond
			...
			(else
				(switch (event message:)
					(5 ; Talk
						(PrintDC 216 1) ; "This is no time for talking!"
						(event claimed: 1)
					)
				)
			)
...
(instance searchDrunk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(PrintDC 216 2) ; "Whew! Holding his nose against the drunken bandit's pungent odor, Graham searches him but doesn't find anything of importance."
				(gEgo
					normal: 1
					view: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 216 3 #at 20 20 #dispose) ; "An intruder!"
		...
		(switch (= state newState)
			...
			(1
				(if (== ((gTheIconBar curIcon:) message:) 3)
					...
					(Print 216 4 #at 110 120 #dispose) ; "Ah! Life-giving water! Nectar of the gods! Graham can now feel renewal flowing through him."
					(= global322 0)
					(= local110 1)
					(= cycles 1)
				else
			...
			(2
				(if local110
					...
					(PrintDC 216 5) ; "A large clay jar full of water stands by the campfire."
					(= cycles 1)
				)
			)
			(3
		...
		(switch theVerb
			...
			(3
				(if local109
					...
					(PrintDC 216 6) ; "Graham has had all the water he wants for now."
				)
			)
			(else
				(super doVerb: &rest)
...
(instance crowd1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(PrintDC 216 7) ; "Overhearing loud music and laughter from within the larger tent, Graham guesses the bandits must be celebrating their latest plunder."
			)
			(else
				(super doVerb: &rest)
			)
...
(instance horse1 of RPicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(PrintDC 216 8) ; "The bandits' horses rest in the desert sun while their unsavory owners revel within the large tent."
			)
			(3
				(if (not local0)
					(++ local0)
					(PrintDC 216 9) ; "Disturbing the horses would be very unwise at this time."
				)
			)
			(4
				(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
					(28 0)
					(else
						(PrintDC 216 10) ; "It would be unwise to disturb the horses right now."
					)
				)
			)
			(else
...
(instance camel of RPicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(PrintDC 216 11) ; "A lone camel waits near the large tent while his owner celebrates inside."
			)
			(3
				(PrintDC 216 12) ; "The bandits would notice if Graham disturbed the camel."
			)
			(4
				(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
					(28 0)
					(else
						(PrintDC 216 13) ; "It would be unwise to disturb the camel right now."
					)
				)
			)
			(else
		...
		(switch theVerb
			(2
				(if (IsFlag 32)
					...
					(PrintDC 216 14) ; "Two tents sit silently in the desert apparently deserted for the moment."
				)
			)
			(else
				(super doVerb: &rest)
...
(instance littleTent of RFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(PrintDC 216 15) ; "There doesn't seem to be any activity going on within the smaller tent."
			)
			(else
				(super doVerb: &rest)
			)
...
(instance dancer of RFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(PrintDC 216 16) ; "A beautiful harem girl belly dances for the rowdy bandits."
			)
			(else
				(super doVerb: &rest)
			)
...
(instance drunkGuy of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(if (self script:)
					(PrintDC 216 17) ; "*** This guy really knows how to party!"
				else
					(PrintDC 216 18) ; "With disgust, Graham looks at a drunken bandit lying face down in the desert sand...completely passed out."
				)
			)
			(3
				(if (not local2)
