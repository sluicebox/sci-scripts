...
(instance iCS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 359 2) ; "Crawling on the floor in an attempt to regain her balance, Lisa protests, "Hey, come on now! I ain't drunk, I'm just drinking!""
				((ScriptID 11 1) ; lush
					cycleSpeed: 1
					moveSpeed: 1
					setLoop: 0
				...
				(iC setMotion: theFollow (ScriptID 11 1) 30) ; lush
			)
			(6
				((ScriptID 11 1) ; lush
					loop: 1
			...
			(8
				...
				(if (and (== (gEgo view:) 206) (not (gEgo script:)))
					(HandsOn)
				)
