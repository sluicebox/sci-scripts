...
(class EditablePolygon of Polygon
	...
	(method (showSelf &tmp temp0 temp1 temp2)
		...
		(for ((= temp0 0)) (< temp0 size) ((++ temp0))
			(self indexToCoor: temp0)
			(temp2 format: {%d } currentX)
			(temp1 cat: temp2)
			(temp2 format: {%d } currentY)
			(temp1 cat: temp2)
		)
		(Printf {%s} (temp1 data:))
		(temp1 dispose:)
	...
	(method (writeToFile param1 param2 &tmp temp0 temp1 temp2)
		(if (or (and (> argc 1) param2) scratch)
			(param1
				writeString: {\r\n\t\t\t; *** }
...
(class PEditor of Set
	...
	(method (init)
		...
		((= local2 (DText new:))
			text: (KArray 8 (local1 data:)) ; ArrayDup
			fore: 255
			posn: 1 1
			setSize: 240
			setPri: 255
	...
	(method (dropAnchor param1 param2 &tmp temp0 temp1 [temp2 2])
		(= temp1
	...
	(method (constructPolygon param1 param2 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		(= temp0 0)
		(= currentX param1)
		(= currentY param2)
		(= depressType 0)
		...
		(if scratch
			(= temp5 (Str new: 13))
			(= temp6 (Str newWith: 13 {default}))
			(Print addTitle: {Feature Name:} addEdit: temp5 12 0 0 temp6 init:)
			(FrameOut)
			(currentPolygon name: (temp5 data:) type: 0 scratch: 1)
			(temp6 dispose:)
		)
			...
			(if
				(= temp7
					(Print
						...
						addTextF: {Number of Points: %d} (currentPolygon size:)
						addButton: 1 {Total} 0 20
						addButton: 2 { Near } 60 20
						addButton: 3 { Barred } 120 20
						addButton: 4 { Container } 195 20
						init:
					)
				)
				(currentPolygon type: (- temp7 1))
			)
		else
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4)
		(if (& (event type:) evKEYBOARD)
			(switch (event message:)
				...
				(KEY_c
					(if (< lineColor 255)
						(self lineColor: (+ lineColor 1) show:)
					)
				)
				(KEY_ALT_c
					(if (> lineColor 0)
			...
			(0
				(if (& (event type:) evMOUSEBUTTON)
					...
					(if (not (event modifiers:))
						...
						(while (!= ((= event ((User curEvent:) new:)) type:) evMOUSEBUTTON)
							(event localize: plane)
							(self scrollPlane: event)
							(FrameOut)
						)
						(event claimed: 1)
					)
				...
				(if (and (& (event type:) evMOUSEBUTTON) (self size:))
					(cond
						...
						((& (event modifiers:) emCTRL)
							...
							(self findClosestPoint: (event x:) (event y:))
							(= temp0 currentIndex)
							(if (< (= temp1 (- currentIndex 1)) 0)
								(= temp1 (- (currentPolygon size:) 1))
							)
							(if
								(>=
									(= temp2 (+ currentIndex 1))
									(currentPolygon size:)
								)
								(= temp2 0)
							)
							...
							(currentPolygon
								insertPoint:
									(if (> temp3 temp4) temp1 else currentIndex)
									(event x:)
									(event y:)
									1
							)
	...
	(method (scrollPlane param1 &tmp temp0 temp1)
		(= temp0 (= temp1 0))
		(if
			(and
				(not (<= (Abs (plane left:)) 30))
				(<= (param1 x:) (+ (Abs (plane left:)) 30))
			)
			(= temp0 1)
		)
		(if
			(and
				(not (<= (Abs (plane top:)) 40))
				(<= (param1 y:) (+ (Abs (plane top:)) 30))
			)
			(= temp1 1)
		)
		(if
			(and
				(> (plane right:) gScreenWidth)
				(>= (param1 x:) (- (+ (Abs (plane left:)) gScreenWidth) 30))
			)
			(= temp0 -1)
		)
		(if
			(and
				(> (plane bottom:) gScreenHeight)
				(>= (param1 y:) (- (+ (Abs (plane top:)) gScreenHeight) 30))
			)
			(= temp1 -1)
		)
		(if (or (!= temp0 0) (!= temp1 0))
			(*= temp0 20)
			(*= temp1 20)
			(gCurRoom doVerb: 30 temp0 temp1)
			(UpdatePlane plane)
		)
	)
