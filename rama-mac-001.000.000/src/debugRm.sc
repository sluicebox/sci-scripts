...
(procedure (localproc_3 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
	...
	(if (Print addTitle: {Output File Name:} addEdit: temp0 12 0 0 temp1 init:)
		...
		(if
			...
			(switch temp7
				(1
					((global112 plane:) priority: -1)
					(UpdatePlane (global112 plane:))
				)
				(2
					(= temp6 0)
					(gEgo hide:)
...
(procedure (localproc_4 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
	(= temp7 (gGame setCursor: gNormalCursor))
	(= temp0 (= temp1 0))
	(repeat
		(Print font: gSmallFont)
		(= temp3 (= temp4 (= temp5 0)))
		(if (> temp1 0)
			(Print addButton: 198 {SCROLL UP} temp4 temp3)
			((Print dialog:) setSize:)
			(if (> temp3 130)
				(= temp5 temp3)
				(= temp3 0)
				(= temp4
					(- ((Print dialog:) nsRight:) ((Print dialog:) nsLeft:))
				)
			else
				(+= temp3 15)
			)
		...
		(= temp1 temp0)
		(= temp2 0)
		(while (and (<= temp1 124) (< temp2 15))
			(cond
				((== temp1 124)
					(++ temp2)
					(Print addButton: temp1 {MannaMelon} temp4 temp3)
					((Print dialog:) setSize:)
					(if (> temp3 250)
						(= temp5 temp3)
						(= temp3 0)
				...
				((not (OneOf ((proc70_6 temp1) owner:) -3 -2))
					(++ temp2)
					(Print
						addButton: temp1 ((proc70_6 temp1) name:) temp4 temp3
					)
					((Print dialog:) setSize:)
					(if (> temp3 250)
						(= temp5 temp3)
		...
		(if (< temp1 124)
			(Print addButton: 199 {SCROLL DOWN} temp4 temp3)
			((Print dialog:) setSize:)
			(if (> temp3 250)
				(= temp5 temp3)
				(= temp3 0)
		...
		(Print addButton: 200 {Outta Here} temp4 (if temp5 temp5 else temp3))
		((Print dialog:) setSize:)
		(switch (= temp6 (Print init:))
			(198
				(= temp0 (Max (- temp0 15) 0))
			)
			(199
				(= temp0 (Min (+ temp1 1) 124))
			)
			(200
				(break)
			)
...
(instance debugRm of Feature
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 [temp10 2] temp12 temp13 temp14 temp15 temp16 temp17 temp18 temp19 temp20 temp21 temp22 temp23 temp24 temp25 temp26 temp27 temp28)
		(if (event claimed:)
			(return)
		)
		(switch (event type:)
				...
				(switch (event message:)
					...
					(KEY_ALT_a
						(if (global117 size:)
							...
							(for
								((= temp14 (global117 first:)))
								temp14
								((= temp14 (global117 next: temp14)))
					...
					(KEY_ALT_d
						(repeat
							...
							(proc70_3 (proc70_7 (gInventory at: temp2)))
						)
						(Print font: gUserFont)
					)
					(KEY_ALT_e
						(= temp12
							(cond
								(global151 global151)
								((global117 contains: (gUser alterEgo:))
									(gUser alterEgo:)
								)
								(else
									(Prints {no ego!})
					...
					(KEY_ALT_o
						...
						(if (PEditor size:)
							...
							(for
								((= temp17 (KList 3 (PEditor elements:)))) ; FirstNode
								temp17
								((= temp17 temp18))
					...
					(KEY_ALT_q
						...
						(switch
							...
							(1
								...
								(temp24 cat: {robaud})
								((= temp25 (File new:))
									name: (KArray 8 (temp24 data:)) ; ArrayDup
									open: 0
								)
								(= temp15 (Str newWith: 75 {}))
								(temp15
					...
					(KEY_ALT_r
						(Printf
							...
							(gCurRoom name:)
							gCurRoomNum
							((gCurRoom picObj:) name:)
							((gCurRoom picObj:) picture:)
							((gCurRoom picObj:) heading:)
							(gCurRoom style:)
							(gCurRoom horizon:)
							(gCurRoom north:)
							(gCurRoom south:)
					...
					(KEY_ALT_t
						(if
							...
							(if gCurRoom
								...
								(gRegions eachElementDo: #newRoom temp9)
								(= gNewRoomNum temp9)
							)
						)
					)
					...
					(JOY_RIGHT
						(if (gTheCursor invItem:)
							((gTheCursor invItem:) doVerb: 41)
						)
					)
					(JOY_DOWNLEFT
						(= global141 (not global141))
					)
					(KEY_PAUSE
			...
			(evMOUSEBUTTON
				(switch (event modifiers:)
					...
					($000c ; emALT | emCTRL
						...
						(= temp7 (event x:))
						(= temp8 (event y:))
						(event localize: (global117 plane:))
						(Printf
							{global: %d/%d\n_local: %d/%d}
							temp7
							temp8
...
(instance featureCheck of Code
	...
	(method (doit &tmp temp0 temp1)
		((gUser curEvent:) localize: (global117 plane:))
		...
		(cond
			((= local5 (gFeatures firstTrue: #onMe temp0 temp1))
				(if (!= local6 local5)
					...
					((= local4 (DText new:))
						text: (KArray 8 (local5 name:)) ; ArrayDup
						fore: 24
						back: 0
						setSize: 240
						setPri: 255
