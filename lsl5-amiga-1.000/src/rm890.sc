	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(if jammed
					(if volumeUp
						(gCurRoom setScript: useMikeScript)
					else
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if jammed
					(if volumeUp
						(TPrint 890 44) ; "Since you've cranked it up to the point of feedback, you can't go any louder! Besides, it's almost loud enough to break glass."
					else
						(TPrint 890 45) ; "You crank up the volume on your control room's monitor speakers to an ear-piercing level."
						(= volumeUp 1)
					)
				else
					(switch mixerDoCounter
						(1)
						(2
							(TPrint 890 46) ; "You bring up the volume control labeled "Control Room C.""
							(Wait 60)
							(TPrint 890 47) ; "You hear nothing but hiss."
						)
						(3
							(TPrint 890 48) ; "You bring up the potentiometer labeled "Studio C.""
							(Wait 60)
							(TPrint 890 49) ; "You hear nothing but hiss and a little air conditioner bleed-through."
						)
						(4
						...
						(5
							...
							(Wait 60)
							(TPrint 890 53) ; "You hear nothing."
						)
						(6
							(TPrint 890 54) ; "You carefully open the slider on "Control Room A," as if P. C. Hammer could hear you doing so."
							(Wait 60)
							(TPrint 890 55) ; "He's recording his "live drive-time" show for tomorrow morning. Who cares?"
						)
						(else
							(if (not (IsFlag 154))
