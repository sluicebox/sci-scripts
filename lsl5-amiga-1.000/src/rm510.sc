...
(instance sBribe of Script
	...
	(method (changeState newState &tmp bribe [str 30])
		(switch (= state newState)
			(0
				(Say gEgo 510 5) ; "Say, good fellow," you say to the maitre d', "would you be willing to accept a little bribe?"
				(= ticks 60)
			)
			(1
				(cond
			...
			(2
				(cond
					((== ((Inv at: 14) state:) 0) ; Membership_Tape
						(switch (++ talkCounter)
							(1
								(Say gEgo 510 12) ; "Hello," you say to the maitre d', "do you have a table for a swinging guy like me?"
							)
							(2
								(Say gEgo 510 13) ; "But sir," you protest, "perhaps you've heard of me... my name is Larry; Larry Laffer."
							)
							(3
								(Say gEgo 510 14) ; "I'd really like to get something to eat," you say to the maitre d'. "May I have a table now?"
							)
							(4
								(Say gEgo 510 15) ; "I'd really like to get something to drink," you say to the maitre d'. "May I please have a table now?"
							)
							(5
								(Say gEgo 510 16) ; "You know, it's amazing that a place like this could even stay in business," you say, beginning to lose your temper. "I've never been to a place where the customer is always WRONG!"
							)
							(6
								(Say gEgo 510 17) ; "Let me into that cafe, you ninny," you shout at the maitre d'. "You better let me in, I think!"
							)
							(7
								(Say gEgo 510 18) ; "So when can I get in, eh?" you whine, "I've got to meet somebody here!"
							)
							(8
								(Say gEgo 510 19) ; "She's really quite lovely," you offer, "in fact, I bet you know her."
							)
							(9
								(Say gEgo 510 20) ; "Say, since I'm just going to stand here anyway, why don't I tell you a little story? A story about how a unassuming, traveling software salesman, born near a log cabin in Gumbo, Missouri, made it into the big time..."
							)
							(else
								(Say gEgo 510 21) ; ""Want to hear a little more about me?" you offer."
							)
						)
					)
					((== ((Inv at: 14) owner:) 510) ; Membership_Tape
						(Say gEgo 510 22) ; "Thanks for making that tape for me. Could you give it to me now?"
					)
					((== ((Inv at: 14) state:) 4) ; Membership_Tape
						(Say gEgo 510 23) ; "Uhh, I seemed to have misplaced my membership tape," you tell the maitre d'. "Could you cut me a new one?"
					)
					((IsFlag 2)
						(Say gEgo 510 24) ; "Excuse me," you say to the maitre d', "How do I get into that inner door?"
					)
					((== ((Inv at: 14) state:) 1) ; Membership_Tape
						(Say gEgo 510 25) ; "Thanks for the membership tape!" you say to the maitre d'. "But where do I go?"
					)
				)
				(= cycles 3)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Points 4 109)
				(TPrint 510 39) ; "You grab your tape from the tape reader."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance phone of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 16)
					(TPrint 510 40) ; "A telephone rests near the maitre d's podium--and it's finally available for your use!"
				else
					(TPrint 510 41) ; "That woman has tied up the telephone for as long as you've been here."
				)
			)
			(5 ; Talk
				(self doVerb: 3 &rest)
			...
			(3 ; Do
				(cond
					((IsFlag 1)
						(TPrint 510 42) ; "There's no need to use the telephone again, since you've already contacted the limo company. Shouldn't you walk outside? Your limo may be waiting for you."
					)
					((IsFlag 16)
						(gCurRoom setScript: sPhone)
					)
					(else
						(Say gEgo 510 43) ; "Get off that phone," you shout. "There are others here that want to make a call."
						(TPrint 510 44) ; "She ignores you completely."
					)
				)
			)
			(else
...
(instance woman of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Say gEgo 510 43) ; "Get off that phone," you shout. "There are others here that want to make a call."
				(TPrint 510 45) ; "Bertha ignores you completely."
			)
			(5 ; Talk
				(Say gEgo 510 46) ; ""Hey, I'd like to make a call someday, myself," you say to the large woman."
				(TPrint 510 45) ; "Bertha ignores you completely."
			)
			(else
				...
				(switch invItem
					(14 ; Membership_Tape
						(if (not (> ((Inv at: 14) state:) 1)) ; Membership_Tape
							...
							(TPrint 510 52) ; "You wonder if this will ruin your only means of admission to the Cafe!"
							(gCurRoom setScript: sMusicBox)
						else
							(TPrint 510 53) ; "If you punch any more holes in that piece of tape, there will be nothing left!"
						)
					)
					(else
						(super doVerb: theVerb invItem &rest)
...
(instance door of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 510 66) ; "These doors operate by means of a Membership Tape inserted and read by that machine to their left."
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 67) ; "Insert your tape into the machine just to the left of these doors."
					)
					(7 ; AeroDork_Gold_Card
						(TPrint 510 68) ; "You might try using that on the maitre d'."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance tapeReader of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 510 69) ; "An ancient mainframe computer's paper tape reader sits against the far wall near the door."
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
					(TPrint 510 70) ; "Your membership tape is still hanging from the side of the machine."
				)
			)
			(3 ; Do
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
			...
			(5 ; Talk
				(TPrint 510 71) ; "This paper tape reader grants access to the "Hard Disk Cafe" proper, which is located behind those closed doors. To enter, just insert your membership tape in the slot."
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
...
(instance diskDrive of Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= theVerb 2) ; Look
			(TPrint 510 72) ; "This disk drive is for display purposes only."
		else
...
(instance pet of Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= theVerb 2) ; Look
			(TPrint 510 73) ; "This computer is for display purposes only."
		else
			(super doVerb: theVerb invItem)
		)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 74) ; "This machine once used magnetic tape, not paper tape."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance pianoRoll of Feature
	...
	(method (doVerb)
		(TPrint 510 75) ; "The historic old piano roll is hermetically sealed in that glass case."
	)
)
...
(instance punchCardMachine of Feature
	...
	(method (doVerb)
		(TPrint 510 76) ; "A sign on the punch card reader requests that you keep your hands off."
	)
)
...
(instance cactus1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Say gEgo 510 77) ; "OUCH!"
			)
			(5 ; Talk
				(TPrint 510 78) ; "Who do you think you are, Snoopy's Uncle Spike?"
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 79) ; "Punch holes in your membership tape? A good idea, but this is not the way to do it."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
