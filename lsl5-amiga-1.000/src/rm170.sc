...
(instance rm170 of LLRoom
	...
	(method (init)
		...
		(if (not (or (gEgo has: 2) (gEgo has: 3) (gEgo has: 4))) ; A_Blank_Videotape_a, A_Blank_Videotape_b, A_Blank_Videotape_c
			(camcorderTapes init: stopUpd: approachVerbs: 3) ; Do
		)
		...
		(lightEast init:)
		(dice init:)
		(eightTrack init: stopUpd: approachVerbs: 3) ; Do
		(gCurRoom
			addObstacle:
				((Polygon new:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(switch register
					(3
						(TPrint 170 0 67 -1 150) ; "Nope, nothing of interest in this drawer!"
					)
					(2
						(TPrint 170 1 67 -1 150) ; "This drawer only contains a few old donut crumbs."
					)
					(1
						(if (IsFlag 85)
							(TPrint 170 2 67 -1 150) ; "Without the battery recharger, this drawer seems so empty!"
						else
							(TPrint 170 3 67 -1 150) ; "Well, well! What's this? So that's where you left your battery recharger!"
							(Points 8 85)
							(TPrint 170 4 67 -1 150) ; "You take it from the drawer and pop it into your pocket."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(TPrint 170 5) ; "You always wondered what these tiny little tapes were. These miniature videotapes are a perfect fit for your new camcorder."
				(TPrint 170 6 67 -1 185) ; "(And you thought they were DAT tapes!)"
				(egoBody dispose:)
...
(instance sDegaussTape of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not register)
					(TPrint 170 7) ; "The tape has already been degaussed."
					(self changeState: 3)
				else
					(gTheMusic2 number: 172 setLoop: -1 play:)
					(Points 2)
					(if (not gTapesDegaussed)
						(TPrint 170 8) ; "Sliding the switch to send rapidly alternating, pulsating currents through your degausser, you sensually slide the girl's dedicated magnetic particles in slow concentric circles over the degausser, gradually increasing in speed and intensity, until they (and you) are randomly aligned. A shudder passes through your body!!"
						(TPrint 170 9 67 -1 185) ; "(By gawd, that's degaussing!)"
						(= cycles 1)
					else
						(= ticks 120)
					)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(0 ; Camcorder
						(TPrint 170 10) ; "You try in vain to insert your camcorder into those tiny tapes."
					)
					(else
...
(instance degausser of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 11) ; "Degaussers work on tapes, not hands. They return the magnetic particles to a random distribution."
				(TPrint 170 12 67 -1 185) ; "(They erase tapes!)"
			)
			(4 ; Inventory
...
(instance cabinet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 13) ; "You've always wanted to use these upper cabinets, but your arms are too short, so you've never seen inside them."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sterileBarrel of Feature
	...
	(method (doVerb theVerb invItem &tmp [str 100])
		(switch theVerb
			(3 ; Do
				(Points 1 83)
				(TPrint 170 14) ; "You quickly rinse your hands with the sterilizing solution. You rather enjoy it, although for the life of you, you don't know why."
			)
			(5 ; Talk
				(TPrint 170 15) ; "Hey, Sterilizer!" you yell into the metal drum. "Every night's your night in the barrel!"
			)
			(4 ; Inventory
				(Format @str 170 16 ((Inv at: invItem) description:) 37) ; "You rinse %s in a stream of sterilizer, effectively removing over 90%c of the bacteria from its environmental surfaces."
				(TPrint @str)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance videoMonitor1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 17) ; "You've seen enough of this monitor over the last few months. You don't want to see any more."
			)
			(4 ; Inventory
				(if (OneOf invItem 2 3 4) ; A_Blank_Videotape_a, A_Blank_Videotape_b, A_Blank_Videotape_c
					(TPrint 170 18) ; "These tapes only fit in your Pocket Protector Camcorder."
				else
					(super doVerb: theVerb invItem &rest)
				)
			)
...
(instance drain of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 19) ; "Cleverly surmising the humble floor drain is really a secret passage to another part of the building, you yank and tug on the grate, only to find it is firmly screwed."
				(TPrint 170 20 67 -1 185) ; "(You can relate to that!)"
			)
			(5 ; Talk
				(TPrint 170 21) ; ""Hello!" you shout down the drain."
				(TPrint 170 22 67 -1 28) ; "Heeeellllooooo."
				(TPrint 170 22) ; "Heeeellllooooo."
				(TPrint 170 22 67 -1 185) ; "Heeeellllooooo."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boxes of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 23) ; "Nah," you think, "who needs a box?"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance videotapes1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 170 24) ; "These tapes are of no use to you."
			)
			(2 ; Look
				(TPrint 170 25) ; "These are the tapes of the women who didn't make the cut. They won't fit in your camcorder."
				(TPrint 170 26 67 -1 185) ; "(And neither do their tapes!)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
		...
		(switch theVerb
			(3 ; Do
				...
				(switch num
					...
					(else
						(gTheMusic number: num setLoop: -1 play: send: 8 78 1)
					)
				)
				(if
					(and
