	...
	(method (doVerb theVerb)
		(cond
			...
			((and (IsFlag 53) (not (gEgo has: 7)))
				(Print 180 28) ; "There is a set of three time cards in the drawer."
			)
			((!= theVerb 3)
				(super doVerb: theVerb &rest)
			)
			((not (IsFlag 58))
				(proc5_9 180 29) ; "The drawer won't open until things quiet down in here."
			)
			((not (IsFlag 53))
				(if local133
					(= local133 0)
			...
			((and (not (gEgo has: 7)) (not (gEgo has: 22)))
				(proc5_9 180 30) ; "The time cards may come in useful."
			)
			(else
				(proc5_9 180 31) ; "There is nothing more for you to do with the drawer."
			)
		)
	)
)
