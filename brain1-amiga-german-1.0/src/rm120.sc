...
(procedure (localproc_10 &tmp temp0 temp1 temp2 temp3 temp4)
	(if
		...
		(if temp1
			...
			(Wait 0)
			(Wait 30)
			(proc5_9 120 28) ; "Try it again. This number sentence is not an equation."
			(localproc_11)
		)
		(if (== local142 4)
			(= local40 1)
			(proc5_9 120 29) ; "You solved the Math Marvel puzzle!"
			(DrawCel 140 6 2 (mathBox x:) (mathBox y:) 15)
			...
			(proc5_9 120 30) ; "In the chest, you find a key. You take it and put it away for later use."
			(gEgo get: 4)
			(gCMusic2 number: 58 setLoop: 1 play:)
			(DrawCel 140 6 1 (mathBox x:) (mathBox y:) 15)
			(Wait 0)
...
(instance bar15 of PuzzleBar
	...
	(method (showHelp)
		(proc5_9 120 14) ; "Inside the drawer, you find a sliding-tile puzzle. The object is to arrange the tiles correctly by sliding tiles into the empty space."
		(switch local125
			(0
				(proc5_9 120 15) ; "When you have correctly arranged the tiles, you will have the numbers 1-3 across the top row, 4-6 across the middle row, and 7-8 on the left side of the bottom row."
			)
			(1
				(proc5_9 120 16) ; "When you have correctly arranged the tiles, you will have the numbers 1-4 across the top row, 5-8 across the second row, 9-12 across the third row, and 13-15 on the left side of the bottom row."
			)
			(else
				(if (IsFlag 0)
					(proc5_9 120 17) ; "When you have correctly arranged the tiles, you will have created a picture of Dr. Brain's castle. (The empty tile will end up in the bottom right corner.)"
				else
					(proc5_9 120 18) ; "When you have correctly arranged the tiles, you will have created a picture of Dr. Brain. (The empty tile will end up in the bottom right corner.)"
				)
			)
		)
		(proc5_9 120 19) ; "Click on any tile in the same row or column as the empty square to slide it (and its neighbors) into the empty square. You can spend a Hint Coin to get some suggestions on how to solve this puzzle."
	)
		...
		(if (and (not local38) (= temp2 (super select: &rest)))
			...
			(cond
				...
				(else
					...
					(proc5_9 120 20) ; "You can't move that tile -- it isn't in the same row or column as the blank square."
					(= temp1 0)
				)
			)
			(if temp1
...
(instance barMagic of PuzzleBar
	...
	(method (buyClue)
		(if (super buyClue: &rest)
			(switch local123
				(0
					(proc5_9 120 21) ; "Each row, column, and diagonal will add up to "15". The center square is used four times (one row, one column, and both diagonals), so you'll want a very "average" number there -- put "5" in the center."
					(proc5_9 120 22) ; "The corners are used three times each, while the sides are each used just twice. So put the "extreme" numbers 1 and 9 opposite each other on the sides. Now work in the rest of the numbers around them to get the correct totals."
				)
				(1
					(proc5_9 120 23) ; "Each row, column, and diagonal will add up to "34". Try to keep the middle-sized numbers near the center of the square. One way to solve this type of magic square is to arrange all the numbers in order, then swap each number on the diagonals with its "complement" (1 with 16, 4 with 13, and so on -- subtract the number from 17 to get the number with which you should swap it)."
				)
				(2
					(proc5_9 120 24) ; "Each row, column, and diagonal will add up to "64". Try to keep the middle-sized numbers near the center of the square. One way to solve this type of magic square is to arrange all the numbers in order, then swap each number on the diagonals with its "complement" (1 with 31, 7 with 25, and so on -- subtract the number from 32 to get the number with which you should swap it)."
				)
			)
		)
	)
	...
	(method (showHelp)
		(proc5_9 120 25) ; "This is a "magic square". Place numbers in the squares such that they add to the same totals across each row, down each column, and along both diagonals. If you get stuck, spend a Hint Coin for some clues on how to solve this type of magic square."
	)
		...
		(if (super select: &rest)
			...
			(if
				...
				(if (= local573 local586)
					...
					(proc5_9 120 31) ; "Great! You unlocked the door!"
					(SetFlag 32)
				else
					(proc5_9 120 32) ; "Nothing happens. That must not have been the correct combination."
					(= local586 1)
				)
				(backDoorLock goAway:)
			else
				(gCMusic2 number: 51 setLoop: 1 play:)
				(proc5_9 120 33) ; "You hear a "Click!". You figured out part of the combination. Good going!"
				(localproc_13)
			)
		)
	)
...
(instance backDoorLock of PuzzleBar
	...
	(method (buyClue)
		(cond
			((>= local126 2)
				(proc5_9 120 34) ; "It appears that the previous hint coin got stuck in the slot."
			)
			((super buyClue: &rest)
				(theMusic3 number: 56 setLoop: 1 play:)
	...
	(method (showHelp)
		(proc5_9 120 35) ; "This is the security lock for the castle. Each time you solve one of the hallway puzzles, you will learn another code word from the combination. Using your Ultra Top Secret Decoder Grid, find the symbols which represent each code word, then key in the symbols in the correct order to open the lock. Click on ENTER (or press E) after each set of symbols."
	)
