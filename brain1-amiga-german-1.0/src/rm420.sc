...
(instance rm420 of Rm
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 0) ; "Dr. Brain is more interested in looking at the stars than being one."
			)
			(else
...
(instance monolith of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 2) ; "At the dawn of mankind, some mysterious power conferred intelligence on our ancestors. Now the monolith is the gateway to even greater knowledge -- the Office of Dr. Brain."
			)
			(3
				(cond
					((IsFlag 35)
...
(instance medSmallBluePlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 3) ; "This is either an Earth-like planet, or a hard-boiled egg."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance RingedPlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 4) ; "You think this planet might be Saturn, at least that has the right ring to it."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance medSmallBrownPlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 5) ; "At last you know what happens to the hole after they cut it out of a donut."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance larLightBrownPlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 6) ; "Dr. Brain has been missing this baked potato for some time."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance larPockMarkedPlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 7) ; "Nobody seemed to be using this manhole cover, so Dr. Brain picked it up to use as a planet."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance smallPockMarkedPlan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 8) ; "It's another one of those shirt buttons that keep getting lost in the wash."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance spiralNebula of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 9) ; "Dr. Brain picked up this spiral nebula at a great discount (using his revolving charge, of course)."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance Globespinning of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 420 14) ; "This spinning globe accurately depicts all of the Earth's major geophysical and political features. Oops, there go the Baltic states!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance consFeat of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2
				(Print 420 20) ; "Dr. Brain has arranged for several of his favorite constellations to appear on the planetarium dome. Your gaze moves across the majestic heavens, just above the smog line."
			)
			(3
				(if (not (IsFlag 31))
					(sun setCycle: 0 stopUpd:)
		...
		(switch theVerb
			(3
				(cond
					...
					((not global173)
						(Print 420 23) ; "The cabinet is securely locked."
					)
				)
			)
			(2
...
(instance sun of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2
				(Print 420 26) ; "It's a good thing this is just a painted image of the sun, or you wouldn't be able to see the stars at all!"
			)
			(3
				(Print 420 27) ; "Don't touch the sun -- nuclear fusion in progress!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance monoAppears of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(proc5_9 420 2) ; "At the dawn of mankind, some mysterious power conferred intelligence on our ancestors. Now the monolith is the gateway to even greater knowledge -- the Office of Dr. Brain."
				(proc5_9 420 28) ; "But to pass through the monolith, you must show that you know the cradle of mankind -- your own star system. Tag each of the planets with its name -- if you can catch it!"
				(= cycles 1)
			)
			(6
				(proc427_1)
