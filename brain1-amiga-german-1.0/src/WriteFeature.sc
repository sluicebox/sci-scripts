...
(procedure (localproc_0 param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
	(Print 948 12) ; "Click left mouse button on top left corner"
	...
	(= temp4 (temp0 x:))
	(temp0 dispose:)
	(Print 948 13) ; "Click left mouse button on bottom right corner"
	(while (!= ((= temp0 (Event new:)) type:) evMOUSEBUTTON)
		(temp0 dispose:)
	)
	(GlobalToLocal temp0)
	...
	(if local409
		(Graph grDRAW_LINE temp3 temp4 temp3 temp6 1 31)
		(Graph grDRAW_LINE temp5 temp4 temp5 temp6 1 31)
		(Graph grDRAW_LINE temp3 temp4 temp5 temp4 1 31)
		(Graph grDRAW_LINE temp3 temp6 temp5 temp6 1 31)
...
(procedure (localproc_3 param1)
	(param1
		...
		lookStr: (GetInput @local300 40 {look string?})
		sightAngle: (= local412 (GetNumber {sight angle?} local412))
		_approachVerbs: (aprchD init: local400)
	)
	(if (param1 description:)
		(param1 description: @local200)
	)
	(if (param1 lookStr:)
		(param1 lookStr: @local300)
	)
)

(procedure (localproc_4 param1 &tmp temp0)
	(Print 948 14) ; "Click left mouse button on approach point"
	(while (!= ((= temp0 (Event new:)) type:) evMOUSEBUTTON)
		(temp0 dispose:)
	)
	(GlobalToLocal temp0)
...
(class WriteCode
	...
	(method (doit theObj &tmp [temp0 400] [temp400 40] [temp440 20] [temp460 40] [temp500 60])
		...
		(if (theObj lookStr:)
			(Format @temp500 948 4 (theObj lookStr:)) ; "lookStr "%s""
		else
			(= temp500 0)
		)
		(Format ; "(instance %s of %s  (properties  x %d   y %d  %s %s  description "%s"   sightAngle %d  %s %s ) )"
			@temp0
			948
			5
			@local0
...
(class CreateObject
	...
	(method (doit &tmp [temp0 15] temp15 temp16 temp17 temp18)
		(= temp17 gSystemWindow)
		(= gSystemWindow SysWindow)
		(if (not local410)
			(= temp0 0)
			(Format @temp0 948 6 (gCurRoom curPic:)) ; "%d.fea"
			(if (not (GetInput @temp0 30 {Enter path and filename}))
				(return)
			)
			(Format @gSysLogPath @temp0)
			(switch
				(Print 948 7 #title {Feature Writer V1.0} #button {YES} 1 #button {NO} 2) ; "Outline Features?"
				(0
					(return)
				)
				(1
			...
			(switch
				(Print 948 8 #title {Feature Writer V1.0} #button {YES} 1 #button {NO} 2) ; "Display code to screen?"
				(0
					(return)
				)
				(1
		...
		(= temp18 (gGame setCursor: gNormalCursor (HaveMouse)))
		(if
			(not
				(= local411
					(Print ; "Class?"
		...
		(if (Print 948 10 #title {Feature Writer V1.0} #button {NO} 0 #button {YES} 1) ; "Z property"
			(Print 948 11) ; "Please click the mouse on the objects projection onto the ground"
		...
		(gGame setCursor: temp18 (HaveMouse))
	)
)
