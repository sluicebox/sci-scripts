...
(class Feature of Obj
	...
	(method (approachVerbs args &tmp i theBits)
		(= _approachVerbs 0)
		(if (and argc gPq1ApproachCode [args 0])
			(for ((= i 0)) (< i argc) ((++ i))
				(= theBits (gPq1ApproachCode doit: [args i]))
			...
			(
				...
				(if
					(and
						...
						gPq1ApproachCode
						(& _approachVerbs (gPq1ApproachCode doit: (event message:)))
					)
					(gEgo
						setMotion:
							PolyPath
	...
	(method (facingMe act &tmp theActor theAngle)
		(if (not (OneOf (gEgo view:) 0 1 2 124))
			(return 1)
		else
