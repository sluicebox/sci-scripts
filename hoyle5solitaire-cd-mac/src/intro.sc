...
(procedure (localproc_1 param1)
	(if (== param1 951)
		(tempView view: param1 posn: 95 0 setCycle: End (gCurRoom script:))
	else
		(tempView view: param1 posn: 0 0 setCycle: End (gCurRoom script:))
	)
)
	...
	(method (handleEvent event)
		...
		(if (not (event claimed:))
			(cond
				...
				(local7
					(gotoClassic handleEvent: event)
					(gotoBlackJack handleEvent: event)
...
(instance introScript of Script
	...
	(method (changeState newState &tmp [temp0 2] temp2 temp3)
		(switch (= state newState)
			(0
				(tempView view: 950 posn: 158 8 init: setPri: 50)
				(if global119
					(tempView view: 950)
					(= state 3)
					(= ticks 1)
			...
			(12
				(= gNewRoomNum (if (== gCardGameScriptNumber 6005) 6005 else 975))
			)
			(13
				(= state 20)
				(= cycles 1)
			...
			(20
				(switch local0
					...
					(3
						(PlaySong play: 2 0 1)
						(proc821_0 910 18 0 0)
						(PlaySong play: 1 0 1)
					)
					(4
						(proc0_10 800 0 0 0 0 255 (gCurRoom plane:))
						(= temp3 (GetTime))
						(while (< (Abs (- (GetTime) temp3)) 200)
						)
						(= gQuit 1)
					)
					(5
						(PlaySong play: 2 0 1)
						(aboutHoyleView init: setPri: 120)
					)
					(6
						(PlaySong play: 2 0 1)
						(proc821_0 5 7 1 0)
...
(instance panel of View
	...
	(method (init)
		(self setLoop: loop posn: 236 102 setPri: priority)
		(super init:)
	)
)
...
(instance gotoClassic of TrackingView
	(properties
		x 250
		y 188
		priority 121
	...
	(method (handleEvent event &tmp temp0)
		(if (super handleEvent: event)
			(event claimed: 1)
			(proc0_10 802 0 0 0 0 200 (intro plane:) {classic})
			(= local7 0)
			(= temp0 (GetTime))
			(while (< (Abs (- (GetTime) temp0)) 460)
			)
			(localproc_3 {classic})
			(= local7 1)
		)
...
(instance gotoBlackJack of TrackingView
	(properties
		x 250
		y 152
		priority 121
	...
	(method (handleEvent event &tmp temp0)
		(if (super handleEvent: event)
			(event claimed: 1)
			(proc0_10 800 0 0 0 0 200 (intro plane:) {bj})
			(= local7 0)
			(= temp0 (GetTime))
			(while (< (Abs (- (GetTime) temp0)) 460)
			)
...
(instance Cancel of TrackingView
	(properties
		x 404
		y 336
		priority 121
	...
	(method (handleEvent event)
		(if (super handleEvent: event)
			(event claimed: 1)
			(gotoClassic hide: dispose:)
			(gotoBlackJack hide: dispose:)
			(aboutHoyleView hide: dispose:)
			(= local7 0)
			(PlaySong play: 1 0 1)
			(self hide: dispose:)
		)
...
(instance aboutHoyleView of View
	(properties
		x 158
		y 109
		priority 120
	...
	(method (init)
		(= priority 120)
		(= local7 1)
		(gotoClassic init: setPri: 121)
		(gotoBlackJack init: setPri: 121)
		(Cancel init: setPri: 121)
		(super init: &rest)
	)
)
