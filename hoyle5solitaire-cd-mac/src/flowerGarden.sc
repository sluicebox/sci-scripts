...
(procedure (localproc_0 param1 &tmp temp0 temp1)
	(if (> ((param1 owner:) size:) 1)
		(= temp1 ((param1 owner:) indexOf: param1))
		(if
			(<
				(= temp0 (- (((param1 owner:) at: (+ temp1 1)) x:) (param1 x:)))
				54
			)
			(-= temp0 3)
			(param1 nsLeft: 0 nsRight: temp0 nsTop: 0 nsBottom: 77)
		else
			(param1 nsLeft: 0 nsRight: 54 nsTop: 0 nsBottom: 77)
		)
	)
)

(instance flowerGarden of HoyleRoom
	(properties
		style 14
	)
		...
		(switch (= state newState)
			...
			(2
				...
				(if (== global927 0)
					...
					(hand5 cardsUp: 6)
					(hand6 cardsUp: 6)
					(reserveHand cardsUp: 16)
				)
				(dealList add: hand1 hand2 hand3 hand4 hand5 hand6 reserveHand)
			...
			(5
				...
				(for ((= temp1 0)) (< temp1 (reserveHand size:)) ((++ temp1))
					(origReserve add: (reserveHand at: temp1))
				)
				(= local6 1)
				(EnableCursor)
				...
				(handleEventList add: reserveHand)
				(for ((= temp1 0)) (< temp1 (gTheHands size:)) ((++ temp1))
...
(instance undoButton of TrackingView
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3)
		(if (and (not global226) (!= global928 52) (super handleEvent: event))
			(event claimed: 1)
			(if (== global225 0)
				(Print
					ticks: 400
				...
				(for ((= temp0 0)) (< temp0 (Deck size:)) ((++ temp0))
					(if
						...
						(if
							...
							(temp3 wasInReserve: 0)
							(reserveHand release:)
							(for
								((= temp1 0))
								(< temp1 (origReserve size:))
								((++ temp1))
								
								(if
									(not
										((origReserve at: temp1) wasInReserve:)
									)
									(reserveHand add: (origReserve at: temp1))
								)
							)
						else
							(((temp3 previousHand:)
						...
						(if (> ((temp3 previousHand:) size:) 1)
							(for
								((= temp1 0))
								(< temp1 (- ((temp3 previousHand:) size:) 1))
								((++ temp1))
