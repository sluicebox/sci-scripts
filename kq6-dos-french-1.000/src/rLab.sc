...
(instance theTorch of Prop
	...
	(method (show)
		(if (mod (rLab labCoords:) 2)
			...
			(theFlame x: (+ x 4) y: y z: 106 setLoop: 3 show: setCycle: Fwd)
			(if (!= gCurRoomNum 406)
				(torchCycler x: 233 y: 48 loop: 7 show: setCycle: RandCycle)
			)
		else
			...
			(if (!= gCurRoomNum 406)
				(torchCycler x: 82 y: 50 loop: 6 show: setCycle: RandCycle)
			)
		)
		(super show:)
	)
	...
	(method (init)
		(if (mod (rLab labCoords:) 2)
			(self view: 400 x: 228 y: 141 z: 81 setLoop: 8 cel: 1 stopUpd:)
			(theFlame x: 232 y: 141 z: 106 setLoop: 3 init: setCycle: Fwd)
			(if (!= gCurRoomNum 410)
				(torchCycler x: 233 y: 48 setLoop: 7 init: setCycle: RandCycle)
			)
		else
			(self
				view: 400
