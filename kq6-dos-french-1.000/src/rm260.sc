...
(instance rm260 of KQ6Room
	...
	(method (init)
		...
		(gEgo init: reset: 2 actions: egoDoVerb setScale: 145)
		((ScriptID 10 4) onMeCheck: 128 init:) ; rocks
		(hatch init:)
		(sail1 init:)
		(sail2 init:)
		(sail3 init:)
...
(instance genieScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(28
				(= local2 0)
				(genieBoy view: 262 loop: 11 cel: 0 noun: 4 setCycle: End self)
...
(instance egoDoVerb of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(else
				(gEgo setMotion: PolyPath (gEgo x:) (gEgo y:))
				(return 0)
			)
		)
	)
