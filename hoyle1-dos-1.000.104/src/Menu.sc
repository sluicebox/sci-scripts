...
(class MenuBar of MenuBar
	...
	(method (handleEvent event &tmp temp0 temp1 temp2)
		...
		(switch (= temp1 (super handleEvent: event))
			(257
				(Print
					(Format @global520 997 0 gVersion 30 1 80 {HOYLE's Volume 1}) ; "DESIGNER AND PROGRAMMER:   Warren Schwader ARTWORK BY:   Jerry Moore   Eileen Fisher   Warren Schwader MUSIC AND SOUND EFFECTS BY:   Rob Atesalp ALSO, THANKS TO:   Guruka Singh Khalsa   David Slayback   J. Mark Hood   Ellen Guon   David Skinner   Version %s"
				)
			)
			(258
				(Print (Format @global520 997 1 30 1 80 {HOYLE's HELP})) ; "MOVING THE ARROW CURSOR MOUSE: Point with the Mouse, select with the Left Button. Rearrange cards in your hand by Dragging them. KEYBOARD: Use the Keyboard Arrows to move the cursor, select with the Enter key, use the Insert key to move cards around in your hand, and to play a dimmed card."
				(Print (Format @global520 997 2 30 1 80 {HOYLE's HELP})) ; "USING THE MENU BAR Point at the Menu Bar with the Mouse or Esc. Select an item by dragging the Mouse or by pressing arrow keys. When using the keyboard, press Enter after selecting an item. Esc will also put the menu bar away. Shortcuts are given after each item."
			)
			(513
				(if
					(= temp2
					...
					(cond
						...
						((and (!= gCurRoomNum 2) (<= 1 gCurRoomNum 7))
							(Print (Format @global520 997 4)) ; "Reset of scores will take effect beginning next hand."
							(= global508 1)
						)
					)
				)
			...
			(515
				(= gQuit (Print 997 5 #button {YES} 1 #button {NO} 0)) ; "Quitting HOYLE?"
			)
			(517
				(if global774
					(= global774 0)
					(Print 997 6) ; "Conversation OFF"
				else
					(= global774 1)
					(Print 997 7) ; "Conversation ON"
				)
				(proc997_0)
				(proc0_8)
			)
