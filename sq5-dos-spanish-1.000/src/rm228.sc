	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(1
				(if (fuseList size:)
					(if (socketList size:)
						(gMessager say: 0 0 0 1 self) ; "Realizing it's not a good idea to interrupt the power to critically important circuits, you replace the fuses before leaving."
					else
						(= cycles 1)
					)
				else
...
(instance sPowerFailure of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gCurRoom drawPic: 0)
				(EgoDead 4) ; "Guess that wasn't the right fuse to pull, Rog."
				(self dispose:)
			)
		)
	)
...
(instance sLifeSupport of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame setCursor: 996)
				(EgoDead 5) ; "Warning! Deactivating life support may be hazardous to your health."
				(self dispose:)
			)
		)
	)
