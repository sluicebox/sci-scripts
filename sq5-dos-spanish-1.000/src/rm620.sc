...
(instance sYourDead of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(EgoDead 29) ; "They homed right in on you. Better try that one again, ace."
			)
		)
	)
)
...
(instance sNoMask of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(EgoDead 28) ; "4 out of 5 Doctors agree that beaming into a toxic atmosphere without the proper equipment can be hazardous to your health."
			)
		)
	)
)
		...
		(switch theVerb
			(4 ; Do
				(if (< (gEgo y:) 120)
					...
					(gMessager say: 4 4 0 0) ; "You'll need to get a little closer before you can touch it."
				)
			)
			(1 ; Look
