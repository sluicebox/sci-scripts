...
(local
	...
	local821
	local822
	[local823 15] = [2 2 2 2 2 2 2 2 3 2 2 2 2 2 2]
	[local838 15] = [2 2 2 2 2 2 2 0 0 0 1 1 1 1 1]
	[local853 15] = [146 152 161 170 186 213 230 240 258 253 229 196 154 114 75]
	[local868 15] = [88 82 77 68 55 32 22 22 20 28 24 45 75 115 134]
	[local883 15] = [33 43 38 43 53 78 83 128 128 128 60 80 91 103 128]
)
...
(instance sGo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(theComet
					setLoop: [local823 local821]
					setCel: [local838 local821]
					x: [local853 local821]
					y: [local868 local821]
					scaleX: [local883 local821]
					scaleY: [local883 local821]
				)
				(if (== (++ local821) 8)
					(theComet setPri: 15)
				)
...
(instance sCaptainsLog of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Message msgGET 104 1 0 0 local815 @local13)
				(= local813 (StrLen @local13))
				(= local814 0)
				(= cycles 1)
			)
			(1
				(repeat
					(StrAt @local413 local814 (StrAt @local13 local814))
					(StrAt @local413 (++ local814) 0)
					(if
						(or
							(== (StrAt @local13 (- local814 1)) 32)
							(== (StrAt @local13 (- local814 1)) 0)
						)
						(break)
					)
				)
				(Display @local413 dsCOORD 40 10 dsCOLOR 254 dsFONT 1307 dsWIDTH 250)
				(if (< local814 local813)
					(-- state)
					(= ticks 15)
				)
			)
			(2
				(= seconds 0)
				(Display @local413 dsCOORD 40 10 dsCOLOR 0 dsFONT 1307 dsWIDTH 250)
				(if (> (++ local815) local816)
					(= cycles 1)
				else
					(= state -1)
					(= seconds 1)
...
(instance quirkTalker of PriorityTalker
	...
	(method (init)
		...
		(gSystemWindow tailX: 171 tailY: 38 xOffset: 0)
