...
(instance Room4 of Rm
	...
	(method (init)
		...
		(super init:)
		(if gNight
			(gCurRoom overlay: 104)
		)
		(self setRegions: 508 517) ; Forest_Region, Ogre_s_Region
		(if (and (== gAct 2) (== gSeenOgressDeerFlag 0))
			(Load rsVIEW 245)
			(Load rsVIEW 246)
			(Load rsVIEW 57)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(12
				...
				(Print 4 27 #at -1 10 #draw) ; "Poor Rosella! It looks like you'll join the deer in the stew pot tonight!"
			)
			(13
				(= seconds 2)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(Print 4 28 #at -1 10 #draw) ; "Oh, no! You're caught! The terrible ogre grabs you by the braids and drags you off to an untimely end. Dinner will definitely be on YOU tonight!"
				(cond
