...
(instance swimIn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gEgo view: 2 setAvoider: 0 setCycle: Walk)
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(HandsOff)
				(LookAt gEgo local0)
				(gEgo view: 21 cel: 255 setMotion: 0 setCycle: End self)
			)
			(2
				(= global182 1)
				(local0 dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 70 22 #at -1 10 #dispose) ; "You kneel down and take a drink of the cool river water. Mmmmm. Not too bad."
			)
			(2
				(gEgo setCycle: Beg self)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo view: 10 cel: 255 loop: 0 setCycle: End self)
			)
			(1
				(gEgo hide:)
				(gCurRoom newRoom: 24)
