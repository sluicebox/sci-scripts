...
(instance drown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gEgo cel: 255 setCycle: End self)
			)
			(4
				(gEgo cel: 255 setCycle: End self)
			)
			(5
				(gEgo hide:)
				(if gNight
					(Print 31 18 #at -1 10) ; "The ocean confuses you at night! Panicking, you loose control...and drown."
				else
					(Print 31 19 #at -1 10) ; "You're too tired to swim anymore. Your body gives out as you prepare to join Davey Jones in his locker."
				)
				(= gDeathFlag 1)
			)
		)
...
(instance whaleActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (gCast contains: local8)
					(local8 loop: 2 cel: 255 setCycle: End self)
				)
			)
			(4
				(if (gCast contains: local8)
...
(instance sharkActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(= global204 0)
				(Print 31 21 #at -1 10) ; "You make a tasty morsel for that hungry shark!"
				(= gDeathFlag 1)
			)
		)
	)
