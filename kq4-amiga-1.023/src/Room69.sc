...
(instance Room69 of Rm
	...
	(method (init)
		...
		(= local6 0)
		(= gIndoors 1)
		((View new:) view: 522 loop: 3 cel: 0 posn: 253 71 addToPic:)
		((Prop new:) view: 522 loop: 2 cel: 0 posn: 252 57 init: setCycle: Fwd)
		((= local1 (View new:))
			view: 520
			loop: 2
			cel: 0
		...
		(cond
			...
			((== (event type:) evSAID)
				(cond
					...
					((Said 'get,pull,lower/hemp,ladder,heap')
						(cond
							(global170
					...
					((Said 'bang[<on]/casket')
						(if (< (gEgo distanceTo: local1) 15)
							...
							(Print 69 22 #at -1 10) ; "You'll have to get a little closer, Rosella."
						)
					)
					((Said 'open/casket,lid')
						(cond
							...
							((local2 cel:)
								(Print 69 23 #at -1 10) ; "It's already closed!"
							)
							(else
								(Print 69 2) ; "You don't want to do that!"
							)
...
(instance doMummy of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (not (gEgo inRect: 138 110 192 125))
					(Print 69 32 #at -1 10 #time 3) ; "Look out for the mummy!!"
					(gEgo observeBlocks: mummyBlock)
					(local2 priority: 8)
					(= local8 1)
					(local3 show:)
			...
			(2
				...
				(mummyMusic init: play:)
			)
			(3
				(local2 ignoreActors: 0 stopUpd:)
				(if (gEgo has: 7) ; Obsidian_Scarab
					(Print 69 33 #at -1 10 #time 4) ; "The mummy runs from your powerful scarab."
					(self changeState: 5)
				else
					(Print 69 34 #at -1 10 #time 5) ; "How did you get here without the scarab? You will be the next victim of the mummy!!"
					(self changeState: 20)
				)
			)
			(5
...
(instance DownLadder of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gEgo loop: 0 posn: 104 149 setCycle: Beg self)
			)
			(5
				(gEgo
					view: 4
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(local0 stopUpd:)
				(= global170 1)
				(gEgo view: 41 loop: 0 cel: 255 setCycle: End self)
			)
			(4
...
(instance cryptFall of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 69 36 #width 100 #at -1 10) ; "Oops! You slipped."
				(= seconds 4)
			)
			(5
				(client setScript: 0)
...
(instance stoop of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo view: 40 cel: 255 setCycle: End self)
				(if (> (gEgo x:) (local4 x:))
					(gEgo loop: 1)
				else
					(gEgo loop: 0)
