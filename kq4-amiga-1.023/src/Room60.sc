...
(instance Room60 of Rm
	...
	(method (init)
		...
		(super init:)
		(if gNight
			((View new:) view: 648 loop: 1 posn: 163 90 setPri: 5 addToPic:)
			((Prop new:)
				view: 536
				loop: 2
				posn: 267 75
		...
		(if (== gPrevRoomNum 68)
			(gEgo posn: 68 159 view: 4 setStep: 4 2 init:)
		else
			(gEgo posn: 161 59 view: 77 setStep: 4 2 init:)
			...
			((== (event type:) evSAID)
				(cond
					(
						...
						(if (== global134 3)
							(Print 60 1) ; "You see the sad ghost of a beautiful young woman. She sits in her rocking chair and weeps as if her heart will break."
						)
					)
					((Said 'look>')
						(cond
							((Said '<under/bed')
								(Print 60 2) ; "There is nothing under the bed."
							)
							((Said '/bed')
								(Print 60 3) ; "The old bed still looks comfortable."
							)
							((Said '/window')
								(if (gEgo inRect: 137 114 191 124)
									(Print 60 4) ; "You see the cemetery out the window."
								else
									(Print 800 1) ; "You're not close enough."
								)
							)
							((Said '/fireplace')
								(Print 60 5) ; "The fireplace has been cold for years. There's nothing there."
							)
							((Said '/carpet,carpet')
								(Print 60 6) ; "A worn rug lies beneath the bed."
							)
							((Said '<in/chest,dresser,drawer')
								(Print 60 7) ; "The chest of drawers is empty."
							)
							((Said '/chandelier')
								(Print 60 8) ; "The lamp is old and dusty."
							)
							((Said '/chest,dresser')
								(Print 60 9) ; "A large, old chest of drawers rests against the wall."
							)
							((Said '/wall')
								(Print 60 10) ; "There is nothing of importance on the walls."
							)
							((or (Said '/dirt') (Said '<down'))
								(Print 60 11) ; "There is nothing of interest on the floor."
							)
							((Said '/ceiling')
								(if local1
									(Print 60 12) ; "You see a ladder descending into the room from an open trapdoor in the ceiling."
								else
									(Print 60 13) ; "You look up at the ceiling and notice a closed trapdoor."
								)
							)
							((Said '<in,up/trapdoor')
								(if local1
									(Print 60 14) ; "You look up through the open trapdoor, but can't see anything as it's too dark up there."
								else
									(Print 60 15) ; "The trapdoor is closed."
								)
							)
							((Said '<up')
								(if local1
									(Print 60 16) ; "You look up through the open trapdoor, but can't make out anything as it's too dark up there."
								else
				...
				(if
					...
					(cond
						...
						(
							...
							(Print 60 31) ; "You try to comfort the lady ghost, but she doesn't respond. It seems as if she doesn't even know you're here as she continues her deep sobbing."
						)
						((Said 'get/ghost,woman,woman,woman')
							(Print 60 32) ; "You can't get a ghost!"
						)
						((Said 'get,capture/ghost,woman,woman,woman')
							(Print 60 33) ; "You can't catch a ghost!"
						)
						((Said 'kill/ghost,woman,woman,woman')
							(Print 60 34) ; "How can you kill something that is already dead!"
						)
						(
							(or
							...
							(Print 60 35) ; "You can't, as the ghost has no substance."
						)
						((Said 'help/ghost,woman,woman,woman')
							(Print 60 36) ; "You'd like to help the sad lady ghost, but how can you do that?"
						)
						((Said 'give>')
							(if (gEgo inRect: 60 118 105 152)
								(cond
									(
										(and
										...
										(if (== (gInventory indexOf: temp0) 28)
											...
											(gGame changeScore: 2)
											(Print 60 37) ; "You hand the lovely locket to the distraught ghost. The locket seems to make her very happy as she vanishes into thin air."
											(Notify 603 -1)
										else
											(Print 60 38) ; "The lady ghost doesn't even acknowledge your presence."
										)
									)
									(temp0
										(event claimed: 0)
