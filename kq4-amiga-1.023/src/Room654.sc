	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= local58 0)
				(= local60 (Print 654 15 #font gSmallFont #at 200 10 #width 110 #dispose)) ; "You watch quietly as the dwarfs file in one by one, get a bowl of soup, and take a seat at the table."
				(dwarfMusic play:)
			...
			(4
				...
				(if (== ([local51 0] cel:) 7)
					(cls)
					(= state 2)
					(= local60 (Print 654 16 #font gSmallFont #at -1 10 #dispose)) ; "This dwarf must really be hungry; he's getting two bowls of soup!"
					(= seconds 1)
				else
			...
			(9
				...
				(if (> local58 48)
					(cls)
					(= local60
						(Print 654 17 #font gSmallFont #at 7 5 #width 290 #draw #dispose) ; "The Seven Dwarfs seem very pleased that you tidied their messy home. One politely asks you your name. You tell him, and he cordially invites you to sit down with them and eat the bowl of soup he got for you."
					)
				)
				(= seconds 15)
			)
			...
			(11
				(User canInput: 1)
				(cls)
				(= local60 (Print 654 18 #font gSmallFont #at -1 10 #dispose)) ; "You seat yourself at the table and begin to eat the surprisingly delicious soup."
				(gEgo view: 76 loop: 5)
				(dwarfMusic dispose:)
				(= global176 1)
			...
			(13
				(cls)
				(= local60 (Print 654 19 #font gSmallFont #at -1 10 #dispose)) ; "It's time for the little men to go back to the mine."
				(= global176 2)
				(User canInput: 0)
			...
			(14
				(dwarfMusic play:)
				(cls)
				(= local60 (Print 654 20 #font gSmallFont #at -1 10 #dispose)) ; ""Good-bye, and thanks for the soup!" you call, as they take their leave."
				(self cue:)
			)
			(15
				(-= local58 7)
		...
		(cond
			...
			((== (event type:) evSAID)
				(cond
					...
					((or (Said 'kiss[/!*]') (Said 'kiss/dwarf,man'))
