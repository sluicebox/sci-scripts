	...
	(method (init)
		...
		(if ((Inv at: 0) ownedBy: 201) ; Silver_Flute
			...
			(if (and (<= (Random 1 100) 40) (>= local5 2))
				...
				(panTheme number: 7 init: loop: -1 play:)
			else
				(= global172 0)
			)
		else
...
(instance playLute of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 514 21 #at -1 10 #draw) ; "Pan has ceased his dancing and now looks at you, and the lute, curiously."
