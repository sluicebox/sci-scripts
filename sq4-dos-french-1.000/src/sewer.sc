...
(instance theSlime of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
				...
				(cond
					...
					(else
						...
						(switch (sewer status:)
								...
								(switch gCurRoomNum
									...
									(95
										(cond
											(
												...
												(cond
													((< (theSlime y:) 60)
														(Print 702 11) ; "Not quite yet... wait until it's in the right position!"
													)
													((> (theSlime y:) 110)
														(Print 702 12) ; "Too late! You'd need longer arms to reach it from here."
													)
													(else
														(gCurRoom
															setScript:
											...
											((> (sewer distance:) 10)
												(Print 702 13) ; "Either you or the slime will have to be in a different position for that. Try waiting until it stops moving."
											)
											(else
												(switch (Random 0 1)
													(0
														(Print 702 14) ; "You are not quite close enough!"
													)
													(1
														(Print 702 15) ; "Get just a little bit closer!"
													)
												)
											)
										)
									...
									(100
										(cond
											(
												...
												(cond
													((< (theSlime y:) 75)
														(Print 702 16) ; "Not quite yet... wait until it's in the right position!"
													)
													((> (theSlime y:) 95)
														(Print 702 17) ; "Too late! You'd need longer arms to reach it from here."
													)
													(else
														(gCurRoom
															setScript:
											...
											((> (sewer distance:) 10)
												(Print 702 18) ; "Either you or the slime will have to be in a different position for that. Try waiting until it stops moving."
											)
											(else
												(switch (Random 0 1)
													(0
													...
													(1
														(Print 702 19) ; "Get just a little bit closer!"
													)
												)
											)
										)
									...
									(115
										(if
											...
											(Print 702 20) ; "The slime's already too far away! Perhaps it's a personal hygiene problem on your part."
										)
									)
									(105
										(if
											...
											(Print 702 21) ; "The slime's already too far away! Perhaps it's a personal hygiene problem on your part."
										)
									)
									(else
										(switch (Random 1 4)
											(1
												(Print 702 22) ; "You are not in a good position for that."
											)
											(2
												(Print 702 23) ; "You can't do that from where you stand."
											)
											(3
												(Print 702 24) ; "Get closer."
											)
											(4
												(Print 702 25) ; "You're too far away to do that."
											)
										)
									)
								)
							...
							(else
								(Print 702 26) ; "That's not possible now."
							)
						)
					)
				)
			...
			(10 ; Taste
				(Print 702 27) ; "You taste a tiny bit of the green slime. Instantly, several recent fillings dissolve into a puddle of silver amalgam."
			)
			(11 ; Smell
				(Print 702 28) ; "What were you expecting, Lime Slime? You notice a burning sensation when you get a strong whiff."
			)
			(else
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(= local66 0)
				(HandsOn)
				(Print 702 30) ; "You scoop up the slimy secretion. Better get away now before it scoops you up!"
...
(instance thePipes of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 702 31) ; "You really don't want to touch the slime-laden pipes."
			)
			(10 ; Taste
				(switch (Random 0 1)
					(0
						(Print 702 32) ; "The pipes have the tangy taste of 100% real steel, basted with slime and rusted to a delicate crunch."
					)
					(1
						(Print 702 33) ; "The corrosive effect of the slimy pipes reduces your tongue to a stump. Now you truly are tasteless!"
					)
				)
			)
			(11 ; Smell
				(Print 702 34) ; "Your nostrils flare as you inhale the fragrant aroma of cold steel with just a hint of sewage."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theConduit of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 702 35) ; "This rough area tastes strangely like blood. Oh, that IS blood. You shredded your tongue. Your mother should have warned you about licking strange areas!"
			)
			(11 ; Smell
				(switch (Random 0 1)
					(0
						(Print 702 36) ; "It smells like cotton candy, roses, and and fresh roasted peanuts. Yeah, right!"
					)
					(1
						(Print 702 37) ; "The stench lingering in this sewer reminds you of your clothes hamper!"
					)
				)
			)
			(else
...
(instance theVent of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 702 38) ; "Your tongue won't reach up there. Perhaps if you'd done those exercises you read about in TIME STUD magazine..."
			)
			(11 ; Smell
				(Print 702 39) ; "Your nose isn't long enough to reach the vent. That may seem like an inconvenience at this point, but you should be thankful."
			)
			(3 ; Do
				(Print 702 40) ; "You can't quite reach the vent."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theDrip1 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 702 41) ; "You see some dripping, oozing stuff."
			)
			(3 ; Do
				(Print 702 42) ; "There's nothing you'd want to do with the dripping liquid."
			)
			(4 ; Inventory
				(Print 702 43) ; "You don't need to do that."
			)
			(10 ; Taste
				(Print 702 44) ; "You don't really want to do that."
			)
			(11 ; Smell
				(Print 702 45) ; "It smells just like you thought it would."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theDrip2 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 702 46) ; "You see some dripping, oozing stuff."
			)
			(3 ; Do
				(Print 702 47) ; "There's nothing you'd want to do with the dripping liquid."
			)
			(4 ; Inventory
				(Print 702 48) ; "You don't need to do that."
			)
			(10 ; Taste
				(Print 702 49) ; "You don't really want to do that."
			)
			(11 ; Smell
				(Print 702 45) ; "It smells just like you thought it would."
			)
