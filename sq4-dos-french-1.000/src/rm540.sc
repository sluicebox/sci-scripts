...
(instance doorBottom of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 7)
					(Print 540 3) ; "Thanks to your janitorial ingenuity, the doorlock is now destroyed and you may enter and leave at will."
				else
					(Print 540 4) ; "Your inate sense of curiosity makes you wish you knew the way to open this thing."
				)
			)
		)
	)
		...
		(switch (= state newState)
			...
			(2
				(if (IsFlag 7)
					...
					(Print 540 6) ; "Try as you might, none of the codes you punch in seem to have any effect on the door."
					(HandsOn)
					(self dispose:)
				)
			)
...
(instance acidScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 540 7) ; "Well, I don't think there's much left of the lock now."
				(SetScore 7 10)
