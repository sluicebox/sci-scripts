...
(instance theMatches of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 615 14) ; "You see a book of matches on the bar countertop."
			)
			(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(++ local0)
				(if (not (IsFlag 15))
					(Print 615 17) ; "You step over to talk to the bartender. As you do, you attract the attention of the crude fellows at the bar."
					(gCurRoom setScript: lookMonoGuys)
				else
					(switch local0
						(1
						...
						(else
							(Print 615 18) ; "You don't want to get into that mess again."
						)
					)
				)
			)
...
(instance band of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 615 19) ; "You see two guys singing."
			)
			(3 ; Do
				(Print 615 20) ; "The band members have nothing of interest."
			)
			(5 ; Talk
				(Print 615 21) ; "They don't have time to talk right now."
			)
			(11 ; Smell
				(Print 615 22) ; "Fortunately, they play much better than they smell."
			)
			(4 ; Inventory
				(if (!= invItem 0) ; buckazoid
					(Print 615 23) ; "They are not interested in that."
				else
					(Print 615 24) ; "You don't have enough to afford their services."
				)
			)
			(else
				(super doVerb: theVerb &rest)
