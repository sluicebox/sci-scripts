...
(instance arm of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 386 6) ; "It's the gruff, unpersonable manager of this particular Monolith Burger franchise."
			)
			(5 ; Talk
				(if (not (gCurRoom script:))
					(HandsOff)
			...
			(4 ; Inventory
				(switch invItem
					(0 ; buckazoid
						(Print 386 7) ; "No, Roger, the idea is to earn money, not give it away!"
					)
					(else
						(super doVerb: theVerb invItem)
					)
			...
			(10 ; Taste
				(Print 386 8) ; "Maybe it's not wise to order a bacon burger..."
			)
			(11 ; Smell
				(Print 386 5) ; "Woof! This dude smells like a pig sty."
			)
				...
				(switch
					...
					(0
						(if
							...
							(SetScore 52 -3)
							(Print 386 9) ; "Oh, by the way, you are about to be fired."
						else
							(Print 386 10) ; "Decided to WIMP out eh? Well, we decided not to give you the cash."
						)
						(= cycles 1)
					)
					(1
...
(instance redStuff of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 386 11) ; "Mmmm! Famous Mono sauce!"
			)
			(10 ; Taste
				(Print 386 12) ; "Sucking on a bottle of ketchup doesn't seem to satisfy your hunger no matter what the government thinks."
			)
			(11 ; Smell
				(Print 386 13) ; "Smells like a synthesized version of an ancient vegetable sauce."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance yellowStuff of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 386 14) ; "Mmmm! Famous Mono sauce No. 2!"
			)
			(10 ; Taste
				(Print 386 15) ; "Sucking on a bottle of mustard doesn't seem to satisfy your hunger."
			)
			(11 ; Smell
				(Print 386 16) ; "Smells like a synthesized version of an ancient sauce."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance microwave of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 386 17) ; "It's a microwave."
			)
			(3 ; Do
				(Print 386 18) ; "The microwave quit a long time ago."
			)
			(11 ; Smell
				(Print 386 19) ; "It smells dusty."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bottomCigar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 386 20) ; "It's a cigar."
			)
			(3 ; Do
				(Print 386 21) ; "Whatever would you want tho use that for?"
			)
			(11 ; Smell
				(Print 386 22) ; "Sniffing the cigar makes you lose what little appetite you had after smelling the air in this place."
			)
			(10 ; Taste
				(Print 386 23) ; "Yuck! You rapidly remove the ashes from your tongue."
			)
			(else
				(super doVerb: theVerb)
			)
