...
(instance dummy3 of Mannequin
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(5 ; Talk
				(Print 371 45) ; "Although just an animated mannequin, it's not intelligent enough to converse with even you."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance dummy4 of Mannequin
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 46) ; "Fringe! Would that be too daring?"
			)
			(3 ; Do
				(if
					(and
					...
					(Print 371 47) ; "Keep your hands to yourself!"
				)
			)
			(10 ; Taste
				(Print 371 48) ; "This doesn't look like your favorite flavor mannequin."
			)
			(11 ; Smell
				(Print 371 49) ; "Yep. Smells robotic."
			)
			(5 ; Talk
				(Print 371 50) ; "Don't bother. You don't need dummies making you look stupid, too!"
			)
			(else
				(super doVerb: theVerb)
			)
						...
						(cond
							((and (< gBuckazoidCount local2) local1 local0)
								(self
									save1:
										(proc0_12
											(Format @local4 371 51 local2 local2) ; "Are you in need of an auditory exam or what? I said it will be %3d buckazoids please."
											67
											1
											1
											28
...
(instance clerk of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 371 53) ; "You check out the clerk. She's a burnished silver beauty if you ever saw one. You've always had a thing for women with antennae."
			)
			(4 ; Inventory
				(switch invItem
					(0 ; buckazoid
			...
			(5 ; Talk
				(cond
					((TestMallFlag (ScriptID 700 0) #rFlag3 4) ; mall
						(Print 371 54) ; "Having already purchased the lovely dress, you decide to stick with men's clothes from now on... even though it was quite enjoyable... almost too enjoyable."
					)
					(
						(and
							(not (clerk script:))
			...
			(3 ; Do
				(Print 371 55) ; "Although it seems darn tempting, you realize that duty calls and this will have to wait. Maybe after the game is over and all these players have gone to bed, you could cruise back by."
			)
			(10 ; Taste
				(Print 371 56) ; "Augh! It tastes like Tarnex(tm)! Well, at least you can rest assured knowing that your tongue is tarnish free."
			)
			(11 ; Smell
				(Print 371 57) ; "Uhmm! The rich smell of robotic lubricants wafts through your smell buds."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance wig of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 58) ; "It's a deep pile wig."
			)
			(3 ; Do
				(Print 371 59) ; "Nah. I'd rather not. You never know where it's been."
			)
			(11 ; Smell
				(Print 371 60) ; "Whew!! From what type of beast was this rendered?"
			)
			(10 ; Taste
				(Print 371 61) ; "You lick the wig and almost immediately cough up a fur ball."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theRack of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 62) ; "The latest in swank fashion displayed in a number of horrifying colors."
			)
			(3 ; Do
				(Print 371 63) ; "While enjoying this banquet o' fashion you wonder what you might look like in one of these cute little frocks."
				(Print 371 64) ; "Hey! What kind of thing is that for a studly guy like you to be thinking? Get ahold of yourself, fella!"
			)
			(11 ; Smell
				(Print 371 65) ; "AAAhh! The smell of brand new simulated fabric!"
			)
			(10 ; Taste
				(Print 371 66) ; "That's not a very clean habit."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theRack1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 67) ; "The latest in swank fashion displayed in a number of horrifying colors."
			)
			(3 ; Do
				(Print 371 68) ; "While enjoying this banquet o' fashion you wonder what you might look like in one of these cute little frocks."
				(Print 371 69) ; "Hey! What kind of thing is that for a studly guy like you to be thinking! Get ahold of yourself, fella!"
			)
			(11 ; Smell
				(Print 371 70) ; "AAAhh! The smell of brand new simulated fabric!"
			)
			(10 ; Taste
				(Print 371 71) ; "That's not a very clean habit."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theRack2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print 371 72) ; "While enjoying this banquet o' fashion you wonder what you might look like in one of these cute little frocks."
				(Print 371 73) ; "Hey! What kind of thing is that for a studly guy like you to be thinking! Get ahold of yourself, fella!"
			)
			(11 ; Smell
				(Print 371 74) ; "AAAhh! The smell of brand new simulated fabric!"
			)
			(10 ; Taste
				(Print 371 71) ; "That's not a very clean habit."
			)
...
(instance theArea of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 75) ; "All the best-dressed bipedal female fashion slaves shop here at Sack's. The dazzling decor is just eye-numbing enough to make the price tags hard to read. "Wow", you think to yourself, "this place is fancier than Frederick's of Uranus!""
			)
			(11 ; Smell
				(Print 371 76) ; "The air is thick with the smell of synthetics."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance model1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 77) ; "It's a nice outfit, but you're not sure you have the midriff for it."
			)
			(3 ; Do
				(Print 371 78) ; "Oooooh! Real plastic!"
			)
			(11 ; Smell
				(Print 371 79) ; "It doesn't smell great, but it at least smells better than you."
			)
			(10 ; Taste
				(Print 371 80) ; "You tongue the mannequin and find that it doesn't please your palate."
			)
			(5 ; Talk
				(Print 371 81) ; "Although just a mannequin, it's not intelligent enough to converse with even you."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance model2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 82) ; "The skin color and cape make a statement - the kind taken in police stations!"
			)
			(3 ; Do
				(Print 371 83) ; "Wow! Genuine Zynthion leather."
			)
			(11 ; Smell
				(Print 371 84) ; "Whoaa, baby! Where have you been? Eau de Water Buffalo went out of style epochs ago."
			)
			(10 ; Taste
				(Print 371 85) ; "Yechh! Seriously gross!"
			)
			(5 ; Talk
				(Print 371 86) ; "Sorry. No brain."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance rogerDance of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 371 87) ; "Let's boogie, girls!"
				(roger init:)
				(gEgo y: 1000)
				(arm setCycle: Osc)
				(= cycles 1)
			...
			(5
				(Print 371 88) ; "I sure know how to bust a move!"
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance wigs of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 89) ; "These wigs are the latest fashion if you have a conehead or like the purple haze look."
			)
			(3 ; Do
				(Print 371 90) ; "Yep. It's synthetic hair."
			)
			(10 ; Taste
				(Print 371 91) ; "Dragging your tongue across the fine strands of of synthetic hair is almost as enjoyable as running it through a french fry slicer."
			)
			(11 ; Smell
				(Print 371 92) ; "No smell."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance changeRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 94) ; "It's the dressing room door."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance wigs2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 95) ; "These are some darn attractive wigs. If you want a head like an ax, or you've wanted to capture that Ed Grimly look, they're just the thing."
			)
			(3 ; Do
				(Print 371 96) ; "Yep. It's synthetic hair all right."
			)
			(10 ; Taste
				(Print 371 97) ; "Dragging your tongue across the fine strands of of synthetic hair is almost as enjoyable as running it through a french fry slicer."
			)
			(11 ; Smell
				(Print 371 92) ; "No smell."
			)
