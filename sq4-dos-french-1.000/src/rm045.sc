	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(1 ; rope
						(Print 45 7) ; "You examine your puny piece of rope and realize that it wouldn't be strong enough to hold this puppy to the ground."
					)
					(else
						(super doVerb: theVerb)
					)
			...
			(11 ; Smell
				(Print 45 8) ; "The dust kicked up by the retros, the heavy stench of burning fuel, and the sharp smell of electricity make for a unique olfactory extravaganza."
			)
			(10 ; Taste
				(Print 45 9) ; "You sear your tongue on the hot metal. Apparently it's only safe to touch the now-exposed area beneath the landing gear housing flap."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance compartmentFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(Print 45 10) ; "You don't want to put that in there. You might need it later."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theStreet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(11 ; Smell
				(Print 45 11) ; "It doesn't exactly smell April-fresh."
			)
			(10 ; Taste
				(Print 45 12) ; "It's not as big a taste sensation as other parts of the city."
			)
			(4 ; Inventory
				((ScriptID 705 4) doVerb: theVerb invItem) ; theRoom
			)
