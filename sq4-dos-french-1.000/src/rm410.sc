...
(instance skater1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 1) ; "The Zero-G skaters look to be having fun. But I wouldn't get in their way if I were you. They enjoy playing "Spin the Dweeb" with any non-skaters who happen to wander into their air space."
			)
			(else
...
(instance skater2 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 2) ; "The Zero-G skaters look to be having fun. But I wouldn't get in their way if I were you. They enjoy playing "Spin the Dweeb" with any non-skaters who happen to wander into their air space."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(4
				...
				(EgoDead 8 18) ; "Well... maybe. It's worth a try."
			)
		)
	)
)
...
(instance skateorama of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 3) ; "It's a Zero-G Skate-O-Rama. In here, the mall's gravity generators have no effect, providing an ideal and unique entertainment opportunity for the entire family."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance light of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 4) ; "The Skate-O-Rama is illuminated by a nearby sun whose glow is reflected through a mirror set into the floor. With this latest innovation, you can now enjoy Zero-G while tuning up your tan."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance steps of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 5) ; "These steps lead up to the rest of the Galaxy Galleria Mall."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance wall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 6) ; "These walls protect shoppers from errant skaters."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance restOfMall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 7) ; "The rest of the Galaxy Galleria awaits the unwary and uninformed shopper."
			)
			(else
				(super doVerb: theVerb &rest)
			)
