		...
		(1
			(StrCat
				...
				(LangSwitch
					...
					{\1b - La Rencontre avec le Sarien}
				)
			)
		)
		(3
		...
		(4
			(StrCat
				...
				(LangSwitch
					...
					{\1a - Roger Wilco et les Egorgeurs du Temps}
				)
			)
		)
		(10
			(StrCat
				...
				(LangSwitch
					...
					{\1e - Les Poup\82es en latex d'Estros}
				)
			)
		)
		(12
...
(instance sq4 of Game
	...
	(method (newRoom)
		(if (== ((gInventory at: 12) state:) 1) ; cigar
			(Print 0 9) ; "The cigar goes, as cigars often do, out."
...
(instance jar of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== cel 2)
					(Print 0 11) ; "You have a jar of green acid."
				else
					(Print 0 12) ; "You have an empty jar."
				)
			)
		)
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(cond
					((gCurRoom script:)
						(Print 0 13) ; "You're far too busy for that now."
					)
					((and (OneOf gCurRoomNum 397 398) (not (IsFlag 31)))
						(Print 0 14) ; "Hey! This isn't a free hint service. You need to pay for it before you read it!"
					)
					((not (HaveMem 6144))
						(Print 0 15) ; "Sorry, you don't have enough memory to view the hint book."
					)
					(else
		...
		(switch theVerb
			(4 ; Inventory
				(cond
					...
					(state
						(Print 0 16) ; "The horrid-smelling smoke eminating from its tip would indicate that the cigar is already lit."
					)
					(else
						(Print 0 17) ; "You slide the conveniently pre-moistened stogie between your lips, apply fire via a match, and proceed to nearly hack up a lung."
						(= state 1)
					)
				)
			)
...
(instance diskette of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 0 18) ; "The label on the diskette reads:    Roger Jr.   Brain Tools   Stunt Flyer"
			)
		)
	)
)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (== view 700)
					...
					(SetScore 67 3)
					(Print 0 20) ; "You extract the portable power pellet from the rear confines of the hare's polyplastoid torso."
					(Inv hide:)
					(gEgo get: 4) ; battery
					(self view: 700 loop: 0 cel: 11)
					(Inv show: gEgo)
			...
			(4 ; Inventory
				(switch invItem
					(4 ; battery
						...
						(Print 0 21) ; "You clumsily insert the battery into the furry burrow inhabitant's hollow trunk."
						(Inv hide:)
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((== ((gInventory at: 4) owner:) 1) ; battery
						(Print 0 22) ; "Your PocketPal now has power. Now all you need is a place to use it."
					)
					(else
						(Inv hide:)
			...
			(3 ; Do
				(if (== ((gInventory at: 4) owner:) 1) ; battery
					(Print 0 23) ; "You liberate the battery from the PocketPal."
					(Inv hide:)
					(gEgo get: 4) ; battery
					(Inv show: gEgo)
				else
					(Print 0 24) ; "This is just a dumb terminal. It won't do anything by itself."
				)
			)
			(4 ; Inventory
				(switch invItem
					(4 ; battery
						...
						(SetScore 72 3)
						(Print 0 25) ; "You plug the mobile power cylinder into the laptop."
						(Inv hide:)
						(self view: 700 loop: 0 cel: 15)
						(Inv curIcon: 0 show: gEgo)
						(gGame setCursor: 999 1)
					...
					(11 ; plug
						(gEgo put: 11 1) ; plug
						(SetScore 71 3)
						(Print 0 26) ; "You attach the plug to the PocketPal."
						(Inv hide:)
						(Inv curIcon: 0 show: gEgo)
						(gGame setCursor: 999 1)
						(gTheIconBar curIcon: icon0)
...
(instance paper_with_gum of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if cel
					(Print 0 27) ; "These symbols look vaguely familiar. Now, where have you seen them?"
				else
					(Print 0 28) ; "Ummm! This lumpy paper-wrapped wad looks mighty interesting."
				)
			)
			(3 ; Do
				(if (not cel)
...
(instance gcWin of BorderWindow
	...
	(method (open &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11 temp12 temp13 [temp14 40] [temp54 4])
		...
		(Format @temp14 0 30 gScore gPossibleScore gScore gPossibleScore) ; "Score: %d of %d"
