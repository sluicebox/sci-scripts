				...
				(if (or (Said 'stand') (Said 'get<up'))
					(cond
						(
							...
							(self changeState: 8)
							((ScriptID 301 2) setScript: (ScriptID 169 0) self) ; shema, goingIn
						)
						((or (== global340 1) (== global341 1))
							(HighPrint 170 0) ; "You should wait for Shema to bring you what you ordered."
						)
						((== global340 2)
							(HighPrint 170 1) ; "Eat your meal first."
						)
						((== global341 2)
							(HighPrint 170 2) ; "Finish your drink."
						)
						(else
							(self changeState: 9)
						)
