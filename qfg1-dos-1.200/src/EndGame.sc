...
(instance EndGame of Rm
	...
	(method (init)
		...
		(LoadMany rsVIEW 139 101 148 925 920 906 907)
		(SolvePuzzle 734 25)
...
(instance post2 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (IsFlag 325)
					(= local85 (localproc_2 14 9 600 3 67 224 70 70 80 25 14)) ; "The brigand band has been dispersed Their treasure has been reimbursed."
				else
					(= local85 (localproc_2 14 9 600 4 67 224 70 70 80 25 10)) ; "So with the Kattas and the Merchant You board a magic carpet to the land of Shapeir."
				)
				(= gModelessDialog 0)
				(= seconds 5)
			)
...
(instance cornyCredits of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(localproc_2 13 1 600 10 67 178 132 70 110 25 5) ; "Next Generation Graphics: Jerry Moore"
				(= seconds 5)
			)
			(5
				(localproc_2 0 7 600 11 67 80 130 70 130 25 6) ; "Marathon Coding and Rude Puns: Bob Fischbach"
				(= seconds 7)
			)
			(6
				(localproc_2 10 2 600 12 67 30 100 70 130 25 5) ; "Algorithmic Gyrations: Larry Scott"
				(= seconds 5)
			)
			(7
			...
			(8
				(localproc_2 1 14 600 14 67 40 30 70 110 25 5) ; "Insidious System Software: Corey Cole"
				(= seconds 6)
			)
			(9
				(localproc_2 10 2 600 15 67 80 50 70 130 25 6) ; "Magical Mystical Music: Mark Seibert"
				(= seconds 7)
			)
			(10
