...
(instance rm150 of SQRoom
	...
	(method (init)
		(LoadMany rsVIEW 522 2002)
		(LoadMany rsVIEW (proc0_19 150 1100 150 150 150) 550)
		(Load rsSCRIPT 151)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(bridge dispose:)
				(bridgeSound stop:)
				(RogerJr view: 2002 setLoop: 2 setCel: 0 setCycle: End self)
			...
			(8
				(Print 150 0) ; "Hee, hee, hee! Hello again, Roger. It's me - your old friend Sludge Vohaul. I've taken the liberty of borrowing your son's body. I had to remove him first to make room for my mental self. His is on this disk. I must say, it is most enjoyable to be in a young, healthy body - even if it is from YOUR blood line."
				(Print 150 1) ; "In fact, I like it so much I think I'll keep it. I guess we'll have no use for this anymore."
				(= seconds 2)
			)
			(9
			...
			(12)
			(13
				(Print 150 2) ; "Now it's time to settle things once and for all. You'd better be careful, though. Keep this in mind - if I die while in this body, that disk will be useless. Your son will NEVER draw another breath and, if you don't defend yourself you will never live to buff another helmet!"
				(Print 150 3) ; "Come on - show me what you've got, mop jockey!"
		...
		(switch (= state newState)
			(0
				...
				(if (OneOf ((Inv at: 14) owner:) gEgo gCurRoomNum diskDrive) ; diskette
					(Print 150 4) ; "You don't really want to go down there AGAIN..."
					(= register 1)
...
(instance jrFreeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 150 6) ; "Thanks for saving me. I thought I was a goner."
				(Print 150 7) ; "Follow me. There are many things we need to talk about."
...
(instance diskDrive of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(Print 150 8) ; "It's a disk drive unit."
			)
			(else
...
(instance pedestal of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(10 ; Taste
				(Print 150 9) ; "Even standing on your tiptoes, you'd never be able to reach that with your tongue."
			)
			(11 ; Smell
				(Print 150 10) ; "It smells like a desperate, sweaty maintenance engineer has been walking around here."
			)
			(else
				(super doVerb: theVerb &rest)
			)
