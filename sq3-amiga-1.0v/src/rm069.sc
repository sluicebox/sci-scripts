		...
		(cond
			((Said 'look>')
				(cond
					((Said '[/around,area]')
						(if global253
							(Print 69 0) ; "The planet Ortega is truly a lava-lovers paradise.  Volcanic activity constantly reshapes its surface, so if you have any maps older than last week, throw 'em out. A few pieces of research equipment are laying around."
						else
							(Print 69 1) ; "Two ScumSoft lackeys are working at a make-shift research station. They appear to be monitoring the seismic activity of this area."
						)
					)
					((Said '/craft')
						(if local2
							(Print 69 2) ; "The speeding ship is only a blur."
						else
							(event claimed: 0)
						)
					)
					(
						...
						(if (gEgo has: 9) ; Metal_Pole
							(Print 69 3) ; "There's a telescope, some seismic equipment, and a crate of some sort."
						else
							(Print 69 4) ; "There's a telescope, an anemometer on a pole, some seismic equipment, and a crate of some sort."
						)
					)
					((Said '/pole')
						(if (IsItemAt 9) ; Metal_Pole
							(Print 69 5) ; "A handy metal pole with a thousand and one uses."
						else
							(event claimed: 0)
						)
					)
					((Said '/anemometer')
						(Print 69 6) ; "It looks like a couple of tennis balls cut in half, and spins when the wind blows."
					)
					((Said '/antenna')
						(Print 69 7) ; "It's used for sending and receiving data."
					)
					((Said '/box')
						(if (gEgo inRect: 220 141 254 155)
							(Print 69 8) ; "It's full of thermal detonators."
						else
							(Print 69 9) ; "From here it just looks like an old crate."
						)
					)
					((Said '<in,in,through/scope')
						(if (gEgo inRect: 56 139 76 159)
							...
							(Print 69 10) ; "You need to get real close and look in the little hole first."
						)
					)
					((Said '/scope')
						(Print 69 11) ; "It looks like it's probably used for surveying and stuff like that."
					)
					((Said '/gun,weapons')
						(if global253
							(Print 69 12) ; "There aren't any weapons here."
						else
							(Print 69 13) ; "The pirates are packing jello pistols."
						)
					)
					((Said '/rock,boulder')
