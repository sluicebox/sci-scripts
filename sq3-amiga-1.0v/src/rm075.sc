		...
		(cond
			((Said 'look>')
				(cond
					((Said '[/area,around]')
						(if global170
							(Print 75 0) ; "While trying to catch your breath you take in the panoramic view. Below, you can see your ship off in the distance, and volcanoes stretching out over the horizon. There is a large circular opening here at the top of the generator. This must be where the beam originates. Watch your step up here."
						else
							(Print 75 1) ; "While trying to catch your breath you take in the panoramic view. Below, you can see your ship off in the distance, and volcanoes stretching out over the horizon. There is a large circular opening here at the top of this building. This must be where the beam originates. Watch your step up here."
						)
					)
					((Said '/cavity')
			...
			((Said 'jump,enter,go/generator,cavity')
			...
			((Said 'down,descend,(climb[<down])[/ladder]')
				(if (gEgo inRect: 157 98 183 106)
					...
					(Print 75 16) ; "You're not close enough to the ladder."
				)
			)
			((Said 'throw,drop/detonator,bomb')
				(cond
					...
					((not (gEgo has: 10)) ; Thermal_Detonator
						(Print 75 17) ; "You don't have one."
					)
					(
						(or
							(== (gEgo onControl:) 64)
					...
					(else
						(Print 75 18) ; "It wouldn't do any good to do that here. Besides, you'll probably just blow your own foot off."
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(if global170
					(Print 75 19) ; "The explosion disables the force field generator. You may now travel safely to Pestulon."
				else
					(Print 75 20) ; "The explosion has shut off the beam that was shooting out from this thing. Someone's gonna be really sore at you for this."
				)
