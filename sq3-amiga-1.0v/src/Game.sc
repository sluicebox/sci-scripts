...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] [str 40])
	...
	(if (and (DeviceInfo diPATHS_EQUAL @saveDevice @curDevice) (DeviceInfo diIS_FLOPPY @curDevice))
		...
		(Format @str 994 7 (if saveDisk {SAVE GAME} else {GAME}) @curDevice) ; "Please insert your %s disk in drive %s."
		(if
			(==
				(= ret
					(if saveDisk
...
(class Game of Obj
	...
	(method (save &tmp [comment 20] num oldCur oldPause)
		...
		(if (or (== gCurRoomNum 120) (== gCurRoomNum 95))
			(Print 994 0) ; "Not Now"
			(return)
		)
		...
		(if (PromptForDiskChange 1)
			...
			(if
				(and
					(!= (= num (Save doit: @comment)) -1)
					(not (SaveGame name num @comment gVersion))
				)
				(Print 994 1 #font 0 #button {OK} 1) ; "Your save game disk is full. You must either use another disk or save over an existing saved game."
			)
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Restore doit: &rest)) -1)
				(if (CheckSaveGame name num gVersion)
					...
					(Print 994 2 #font 0 #button {OK} 1) ; "That game was saved under a different interpreter. It cannot be restored."
				)
			)
			(PromptForDiskChange 0)
		)
	...
	(method (wordFail word &tmp [str 100])
		(Printf 994 3 word) ; "I don't understand "%s"."
		(return 0)
	)

	(method (syntaxFail)
		(Print 994 4) ; "That doesn't appear to be a proper sentence."
	)

	(method (semanticFail)
		(Print 994 5) ; "That sentence doesn't make sense."
	)

	(method (pragmaFail)
		(Print 994 6) ; "You've left me responseless."
	)
