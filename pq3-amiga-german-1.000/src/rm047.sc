...
(instance rm047 of PQRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 0) ; "You're standing outside the infamous Nugget saloon."
			)
			(else
	...
	(method (doit &tmp temp0)
		...
		(cond
			...
			((localproc_0 gEgo 2)
				(HandsOff)
				(Print 47 1) ; "The back entrance to the bar seems to be locked."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(ShowClock 0)
				(if (== local1 1)
					(Print 47 2) ; "Morales ignores your attempt at conversation."
					(HandsOn)
					(self dispose:)
				else
					(= save1 1)
					(sInset init:)
					(Say sInset 47 3 108) ; "This car looks like the suspect vehicle we've been looking for."
			...
			(2
				...
				(= save1 1)
				(pInset init:)
				(Say pInset 47 4 108) ; "Hmmmm... no tags, though."
				(= seconds 4)
			)
			(3
				(pInset dispose:)
...
(instance sEdgeHit of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 47 5) ; "You wander around for a moment and realize that there's nothing else of interest in this part of town."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance sGetInCar of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 47 6) ; "You curse as you watch the suspect getting away."
				(pat
					init:
					view: 20
					x: 134
			...
			(6
				(= save1 1)
				(Say pat 47 7) ; "What... What's going on?"
				(= seconds 3)
			)
			(7
				(= save1 1)
				(Say gEgo 47 8) ; "Just get in the car!"
				(= seconds 3)
			)
			(8
				(gEgo
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((== local0 1)
						(Print 47 9) ; "You can't take a paint sample without the proper field equipment."
					)
					(else
						(Print 47 10) ; "You can't search the inside of the car without a warrant."
					)
				)
			)
			(2 ; Look
				(cond
					...
					((!= local0 1)
						...
						(Print 47 11) ; "You look at the parked car and notice that the license plates are missing. The car matches the description of the suspect vehicle."
						(Print 47 12) ; "Giving the car a closer look, you see what appears to be a fresh white paint transfer."
						(Print 47 13) ; "Bingo", you mutter, "I got you now, you S.O.B..."
					)
					(else
						(Print 47 14) ; "You inspect the suspect car once more, and find no evidence other than the paint transfer."
					)
				)
			)
			(4 ; Inventory
				(if (trunk showing:)
					...
					(switch invItem
						(22 ; tracker
							(Print 47 15) ; "You plant the tracking device on the suspect vehicle."
							(HandsOff)
							(= local2 502)
						...
						(39 ; scraper
							(if (not (IsFlag 166))
								(Print 47 16) ; "You carefully scrape some of the white paint transfer off and put it in an envelope."
								(HandsOff)
								(SetScore 166 5)
								...
								(Print 47 17) ; "You don't need any more paint."
							)
						)
						(42 ; sampleEnv
							(if (gEgo has: 30) ; whitePaint
								(Print 47 18) ; "The car won't fit in the envelope."
							else
								(Print 47 19) ; "You need to get the paint sample before you can store it in an envelope."
							)
						)
						(else
							(super doVerb: theVerb invItem &rest)
...
(instance bugs of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 21) ; "The light attracts the bugs."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance barDoor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 22) ; "This place looks pretty rugged. Better be on your toes."
			)
			(3 ; Do
				(if (trunk showing:)
					(super doVerb: theVerb invItem &rest)
...
(instance unmarkedCar of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 23) ; "You look fondly at your car."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
			...
			(2 ; Look
				(cond
					...
					((!= local0 1)
						...
						(Print 47 11) ; "You look at the parked car and notice that the license plates are missing. The car matches the description of the suspect vehicle."
						(Print 47 12) ; "Giving the car a closer look, you see what appears to be a fresh white paint transfer."
						(Print 47 24) ; "Bingo", you mutter, "I got you now, you S.O.B..."
					)
					(else
						(Print 47 14) ; "You inspect the suspect car once more, and find no evidence other than the paint transfer."
					)
...
(instance tireMarks of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 25) ; "You can still smell the rubber left in those tire marks."
			)
			(4 ; Inventory
				(if (trunk showing:)
					(super doVerb: theVerb invItem &rest)
					...
					(switch invItem
						(39 ; scraper
							(Print 47 26) ; "Rubber's just rubber. If you want some there's easier ways to get it than scraping it off the street."
						)
					)
				)
			)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (== local0 1)
					...
					(Print 47 27) ; "There is no apparent need for anything in the trunk right now."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance carDoor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 47 28) ; "Convinced that the serial killer will strike here next, you decide to stick around."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance garageDoor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 29) ; "This part of town has numerous parking garages and small lots."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance windows of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 30) ; "The windows are tinted. You can't see inside from here."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance alley of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 31) ; "An alley runs alongside the Old Nugget."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance streetSign of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 32) ; "Billboards are not uncommon in this part of town. They make streets even uglier than they already are."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance nugget of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 33) ; "This is the infamous Old Nugget saloon."
			)
			(3 ; Do
				(Print 47 34) ; "Just use the front door."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance mug of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 35) ; "Based on Paul Bunyan's mug there, I'd say it's a safe bet that they serve beer here."
			)
			(3 ; Do
				(Print 47 36) ; "If you want a drink, you'd have better luck inside."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance neonSign of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 47 37) ; "The sign says 'Old Nugget'. It's neon glow attracts insects (including those of the human variety)."
			)
			(3 ; Do
				(Print 47 34) ; "Just use the front door."
			)
