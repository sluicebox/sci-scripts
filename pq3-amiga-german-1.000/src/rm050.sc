...
(instance coronerConversation of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(= save1 1)
				(Print 50 4 #at 15 10 #width 230) ; "With a sinking feeling, you recognize the old news article covering your promotion."
				(= cycles 1)
			)
			(9
				(= save1 1)
				(Print 50 5 #at 60 10) ; "A red marking pen pentagram mars your photograph."
				(= cycles 1)
			)
			(10
				(= save1 1)
...
(instance floor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 50 26) ; "The tiled floor is freshly cleaned and waxed."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance lamps of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 50 37) ; "The office lamps give off bright, clinical-looking light."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance vent of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 50 38) ; "The air vent circulates the air in the office. Nevertheless, it always smells like formaldehyde in here."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance skeleton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 39) ; "You'd better leave "Demi" alone. You know how Leon is about his bones."
			)
			(2 ; Look
				(Print 50 40) ; "That's Leon's 'girlfriend', Demi Bones."
			)
			(else
				(super doVerb: theVerb)
			)
					...
					(switch (local5 corpseType:)
						(0
							(if 1
								...
								(Printf 50 41 global186 global192) ; "The toe tag reads "%s"."
						...
						(1
							...
							(Printf 50 42) ; "Good old Leon. He has so much fun naming the John Does."
						)
						(2
							(if 1
								(= global186
								...
								(StrSplitInTwo global186 global192 (local5 toeStr:))
								(Printf 50 43 global186 global192) ; "The toe tag reads "%s" - the poor stiff you found in the alley."
								(Memory memFREE global186)
								(Memory memFREE global192)
							)
						)
						(3
							...
							(Printf 50 44) ; "Underneath the name is a note: "Bonds - Caught you peeking! Stepped out for a bite. Be back soon - Leon.""
							(= local6 1)
						)
					)
				else
...
(instance sink of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 45) ; "This place does have a way of making you want to wash your hands, doesn't it?"
			)
			(2 ; Look
				(Print 50 46) ; "The Coroner's office is equipped with a sink. You hate to think about what washes down those drains!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance embalmMachine of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 47) ; "I don't think Leon would appreciate your playing with his toys."
			)
			(2 ; Look
				(Print 50 48) ; "The embalming machine is a kind of mechanical vampire. Out with the old, in with the new..."
			)
			(else
				(super doVerb: theVerb)
			)
