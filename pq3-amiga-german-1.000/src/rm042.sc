...
(instance rm042 of PQRoom
	...
	(method (init)
		...
		(if (== gDay 1)
			(patrolCarBack init: stopUpd:)
		else
			(unMarkedCarBack init: stopUpd:)
		)
		(building init:)
		...
		(self
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 110 128 80 119 93 113 58 103 77 94 0 71 0 0 319 0 319 154 274 169 214 151 192 154 174 147 202 102 262 113 288 98 257 91 293 74 271 67 209 80 200 82 182 76 157 83 144 79 129 82 158 92
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
	...
	(method (doit &tmp temp0)
		...
		(cond
			(script 0)
			((and (& temp0 $0010) (== (gEgo view:) 5) (IsFlag 28))
				(ClearFlag 28)
				(HandsOff)
				(gCurRoom setScript: goInMore 0 43)
			)
			((and (IsFlag 185) (& temp0 $0010))
				(HandsOff)
...
(instance patrolCarFront of View
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 42 3) ; "You look fondly at your car."
		else
...
(instance patrolCarBack of View
	...
	(method (init)
		(patrolCarFront init: stopUpd:)
		(patrolCarMiddle init: stopUpd:)
		(patrolCarDoor init: stopUpd:)
		(super init:)
	)
)
...
(instance unMarkedCarBack of View
	...
	(method (init)
		...
		(unMarkedCarDoor init: stopUpd:)
		(unMarkedCarMiddle init: stopUpd:)
		(super init:)
	)
...
(instance building of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 42 4) ; "Lytton city courthouse... You hope Judge Simpson got up on the right side of the bed this morning"
			)
			(3 ; Do
				(Print 42 5) ; "Just use the front door."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance entrance of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 42 6) ; "The front entrance to the courthouse."
			)
			(3 ; Do
				(cond
...
(instance plantsLFT of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 42 7) ; "This site boasts typical Lytton city landscape."
			)
			(3 ; Do
				(Print 42 8) ; "You consider using the bushes, but decide that you can wait until you get to a restroom."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sidewalk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 42 9) ; "A nice, wide sidewalk runs in front of the courthouse."
			)
			(3 ; Do
				(Print 42 10) ; "Hey! Try to keep your mind on your work, Ok, bonds?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance asphalt of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 42 11) ; "The asphalt looks hot and dry."
			)
			(3 ; Do
				(Print 42 12) ; "Hey! Try to keep your mind on your work Ok bonds?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
