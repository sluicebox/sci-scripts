...
(instance dumpster of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 45 25) ; "The rusty dumpster contains the corpse of a murdered man. You pray to God that when YOUR time comes you end up someplace more dignified than the trash."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance body of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 45 26) ; "Ah! Would that the dead COULD speak!"
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 45 27) ; "You think about shooting the murder victim, but he's ALREADY DEAD!"
					)
					(19 ; wallet
						(Print 45 28) ; "You identify yourself to the stiff, then contemplate a long vacation."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance windshield of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 45 29) ; "The windshield is marked with the Vehicle Identification Number. It's 127633"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance abandoncar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 45 30) ; "The car's been abandoned for some time. There's nothing of interest in there."
			)
			(2 ; Look
				(if (not (trunk showing:))
					(HandsOff)
...
(instance carinset of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 2)
					(Print 45 31) ; "It's the abandoned white car. The dent contains gold paint."
				else
					(Print 45 32) ; "It's the abandoned white car. The dent contains light red paint."
				)
			)
			(4 ; Inventory
				(cond
					((gEgo has: 31) ; goldPaint
						(Print 45 33) ; "You don't need another sample."
					)
					((and (== invItem 39) (gEgo has: 42)) ; scraper, sampleEnv
						(sScraper play:)
						(if (IsFlag 2)
							(Print 45 34) ; "You carefully scrape some of the gold paint transfer from the side of the white car along with a little white paint into the envelope."
						else
							(Print 45 35) ; "You carefully scrape some of the light red paint transfer from the side of the white car along with a little white paint into the envelope."
						)
						(gEgo get: 31) ; goldPaint
						(SetScore 153 5)
					)
					((and (gEgo has: 39) (gEgo has: 42)) ; scraper, sampleEnv
						(Print 45 36) ; "You need to use the proper field equipment to take a clean paint sample."
					)
					(else
						(Print 45 37) ; "You don't have the field equipment necessary to take a clean paint sample."
					)
				)
			)
			(else
...
(instance backwall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 38) ; "The alley is a dead end - appropriate in this case."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance street of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 39) ; "The back alley asphalt is cracked and filthy. Weeds have struggled through in several places."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance buildings of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 40) ; "This part of town is full of typical American brick tenements."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance trash of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 41) ; "An alley is a lousy place to die."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance lamp of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 42) ; "It's a street lamp."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance garage of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 43) ; "The garage door is closed and locked."
			)
			(3 ; Do
				(Print 45 44) ; "You don't need to worry about the garage."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance yellowTape of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 45) ; "The yellow tape marks off the crime scene and prevents curiosity-seekers from getting too close."
			)
			(3 ; Do
				(Print 45 46) ; "The uniformed officer will remove the tape when the body has been removed and the crime lab is finished with the area."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance carfront of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 47) ; "You look fondly at your car."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance carback of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 45 48) ; "It's the trunk of your car."
			)
			(3 ; Do
				(trunk open:)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(cond
					((gEgo has: 23) ; license
						(Print 45 49) ; "You need to give the license to the coroner first."
					)
					((localproc_0)
						(Print 45 50) ; "You need to put the field equipment back into the trunk first."
					)
					((!= ((gInventory at: 23) owner:) 45) ; license
						(Print 45 51) ; "You're not finished with your investigation."
					)
					(else
						(ClearFlag 63)
