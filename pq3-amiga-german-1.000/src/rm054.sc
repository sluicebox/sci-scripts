...
(instance printer of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 31) ; "It's a computer printer."
			)
			(3 ; Do
				(if local1
					(HandsOff)
					...
					(Print 54 32) ; "The recruiter would no doubt flatten your face if you tried to walk out with his printer."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance recruiter of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 54 33) ; "This looks like the kind of man you want to take seriously."
			)
			(5 ; Talk
				(HandsOff)
			...
			(3 ; Do
				(Print 54 34) ; "I don't think that's a good idea."
			)
			(4 ; Inventory
				(switch invItem
					(6 ; photo
						(cond
							(local0
								(cond
									((or local1 (gEgo has: 26)) ; miltaryRecord
										(Print 54 35) ; "There's no need for that, he has already helped you."
									)
									((IsFlag 232)
										(= local3 3)
										(= local0 1)
									...
									(else
										(Print 54 36) ; "Just showing him the photo again won't do any good."
									)
								)
							)
							((IsFlag 232)
					...
					(19 ; wallet
						(cond
							((and (gEgo has: 6) (not (IsFlag 232))) ; photo
								(HandsOff)
								(gCurRoom setScript: IDScript)
							)
							((IsFlag 232)
								(Print 54 37) ; "You have already identified yourself."
							)
							(else
								(Print 54 38) ; "The recruiter isn't interested."
							)
						)
					)
					(1 ; gun
						(Print 54 39) ; "The recruiter is career army. He's not interested in seeing your puny weapons."
					)
					(2 ; handcuff
						(Print 54 39) ; "The recruiter is career army. He's not interested in seeing your puny weapons."
					)
					(16 ; nightStick
						(Print 54 40) ; "The recruiter is career army. He's not interested in seeing your puny weapons."
					)
					(else
						(Print 54 38) ; "The recruiter isn't interested."
					)
...
(instance desk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 42) ; "The desk has a military neatness."
			)
			(3 ; Do
				(Print 54 43) ; "The desk is not yours."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance poster of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 44) ; "Just looking at all those posters makes you feel patriotic."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 45) ; "Those computers are everywhere, aren't they?"
			)
			(3 ; Do
				(Print 54 46) ; "The recruiter would no doubt flatten your face if you tried to walk out with his computer."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance fileCab of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 47) ; "The filing cabinets probably hold countless forms in triplicate."
			)
			(3 ; Do
				(Print 54 48) ; "The recruiting officer might be upset if you starting rifling through his files uninvited."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance trash of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 49) ; "A trash can"
			)
			(3 ; Do
				(Print 54 50) ; "You're not a janitor."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance counterLt of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 51) ; "The counters are polished and bare."
			)
			(else
				(super doVerb: theVerb)
			)
