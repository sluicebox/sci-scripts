...
(instance givenTwo of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say iEgo 44 16) ; "Your Honor, I'm convinced that some sort of cult is operating a crack house at this location."
				(= seconds 9)
			)
			(2
				(if gModelessDialog
...
(instance judge of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 44 27) ; "Judge Simpson looks like a woman who likes to get right down to business."
			)
			(3 ; Do
				(Print 44 28) ; "The judge is not the kind of person you want to mess with."
			)
			(5 ; Talk
				(HandsOff)
				(cond
					...
					((gEgo has: 8) ; warrant
						(HandsOn)
						(Print 44 29) ; "The judge doesn't want to talk now."
					)
					((gEgo has: 9) ; judicialOrder
						(HandsOn)
						(Print 44 30) ; "You got what you wanted. What are you waiting for?"
					)
					(else
						(HandsOn)
						(Print 44 31) ; "The judge just wants to see some evidence."
					)
				)
			)
			(4 ; Inventory
				(switch invItem
					(37 ; locket
						(cond
							((IsFlag 230)
								(Print 44 32 #at 40 115) ; "You've already shown Marie's locket to Judge Simpson."
							)
							((not (gEgo has: 8)) ; warrant
								(HandsOff)
								(++ local1)
							...
							(else
								(Print 44 33) ; "The judge doesn't care about Marie's locket now."
							)
						)
					)
					(6 ; photo
						(if (== ((gInventory at: 6) state:) 1) ; photo
							(Print 44 34 #at 40 115) ; "You've already shown the photograph to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(35 ; article
						(if (== ((gInventory at: 35) state:) 1) ; article
							(Print 44 35 #at 40 115) ; "You've already shown the news clipping to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(1 ; gun
						(Print 44 36) ; "She'd rip you apart, Bonds."
					)
					(2 ; handcuff
						(Print 44 36) ; "She'd rip you apart, Bonds."
					)
					...
					(19 ; wallet
						(Print 44 37) ; "I remember you perfectly well, Officer Bonds!"
					)
					(else
						(Print 44 38) ; "The judge isn't interested in that."
					)
				)
			)
			(else
...
(instance warrant of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (gEgo has: 8) ; warrant
					(Print 44 39) ; "The judicial order lies on the desk."
				else
					(Print 44 40) ; "The search warrant lies on the desk."
				)
			)
			(3 ; Do
				(if (gEgo has: 8) ; warrant
					(SetScore 189 5)
					(Print 44 41) ; "You pick up the judicial order."
					(gEgo put: 8 0) ; warrant
					(gEgo get: 9) ; judicialOrder
					(= local3 0)
				else
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 42) ; "The bookcase is filled with legal tomes."
			)
			(3 ; Do
				(Print 44 43) ; "The judge would not appreciate you handling her things."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance desk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 44) ; "The judge's desk is neat and orderly."
			)
			(3 ; Do
				(Print 44 43) ; "The judge would not appreciate you handling her things."
			)
...
(instance table of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 45) ; "The small table holds a lamp and nothing else of interest."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance tableLamp of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 46) ; "A lamp is of no interest."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance windows of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 47) ; "The window looks over the park next door."
			)
			(3 ; Do
				(Print 44 48) ; "If the judge wants her windows open, she'll open them herself."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance paneling of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 49) ; "The judge's chambers has nice oak paneling."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance floor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 44 50) ; "The judge's chambers has nice, thick, blue carpeting."
			)
			(else
				(super doVerb: theVerb)
			)
