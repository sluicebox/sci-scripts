	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 133 25) ; "Dispatch: "64S2, Lytton. Go ahead...""
				(Print 133 26) ; "Sonny: "Requesting assistance in gaining access to a suspected crack house. Will need the ram unit and backup at 522 West Palm, over.""
				(Print 133 27) ; "Dispatch: "10-4, Sonny! They're on their way.""
			)
		)
	)
)
...
(instance warrantScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 133 28) ; "Sonny: "I think I've got enough evidence to obtain a search warrant.""
				(= seconds (if (not gHowFast) 600 else 300))
			)
			(2
				(Print 133 29) ; "Morales: "If you want that warrant, Bonds... you're going to half to see da judge.""
				(self start: 1 init:)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 133 30) ; "You notice Morales checking her watch. She suddenly says..."
				(Say geek 133 31) ; "Yo, Sonny. Drive me over to the mall real quick, would ya?  I gotta make a phone call."
				(= seconds 8)
			)
			(2
				(if gModelessDialog
			...
			(4
				(Print 133 32) ; "Grudgingly, you drive Morales to the mall."
			...
			(5
				(patInset init:)
				(Say patInset 133 33 108) ; "Sit tight, boss! I'll only be a minute."
				(= seconds 4)
			)
			(6
				(if gModelessDialog
			...
			(9
				(if (== gDay 4)
					(Print 133 34) ; "She seems in an awful hurry this time. She's even neglected to take her purse with her."
				else
					(Print 133 35) ; "You notice that she's careful to take her purse with her."
				)
			...
			(11
				(= global110 20)
				(= seconds
					(if (and (== gDay 4) (== gHowFast 0)) 15 else 10)
				)
			)
			(12
				(HandsOff)
				(patInset init:)
				(Say patInset 133 36 108) ; "See! I'm back! Let's go!"
				(= seconds 3)
			)
			(13
				(if gModelessDialog
...
(instance patPissedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(geek init: posn: 255 112)
				(Print 133 37) ; "Noticing her key missing, Pat says..."
				(Say geek 133 38) ; "Hey! What the.. where's my key? Whatcha doin' messing with my purse, Bonds?"
				(= seconds 8)
			)
			(2
				(if gModelessDialog
			...
			(3
				(EgoDead 0 133 39) ; "BUSTED! You die of embarrassment."
			)
		)
	)
)
...
(instance goHomeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 133 40) ; "So ends another shift. You drop Pat off at the station and head home."
				(gCurRoom newRoom: 38)
			)
		)
	)
...
(instance stationScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 133 41) ; "With nothing further to do, you head back to the station."
				(gCurRoom newRoom: 10)
			)
		)
	)
