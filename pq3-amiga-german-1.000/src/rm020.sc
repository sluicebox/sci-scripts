	...
	(method (doit &tmp temp0)
		(cond
			...
			((& (gEgo onControl: 0) $0002)
				(self x: 231 y: 189 setScript: (ScriptID 896 2) 0 18) ; sWO
			)
		)
		(super doit:)
	)
		...
		(switch theVerb
			(2 ; Look
				...
				(if
					...
					(Print 20 28) ; "The In/Out basket appears to have uomething in it."
				else
					(Print 20 29) ; "The In/Out basket is empty."
				)
			)
			(3 ; Do
				(cInset dispose:)
			...
			(4 ; Inventory
				(Print 20 30) ; "This is for memos, requests and files."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 20 31) ; "Your computer occupies the center of your desk. In this business, the computer is an indispensible tool."
			)
			(3 ; Do
				(cond
					((> gDay 1)
						(Print 20 32) ; "You are in homicide division now. Use the computer there."
					)
					((gCast contains: cInset)
						(Print 20 33) ; "Your computer is already turned on."
					)
					(else
						(HandsOff)
			...
			(4 ; Inventory
				(switch invItem
					(21 ; compCard
						(if (> gDay 1)
							(Print 20 34) ; "You are in homicide division now. Use the your computer there."
						else
							(HandsOff)
							(gCurRoom setScript: compScript)
						)
...
(instance thisRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 35) ; "This is the Sergeant's office."
			)
			(3 ; Do
				(if (not (gEgo normal:))
					(HandsOff)
...
(instance desk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 36) ; "Your desk is neat and orderly - probably because you don't spend a lot of time here."
			)
			(3 ; Do
				(if (== gDay 1)
					(HandsOff)
					...
					(Print 20 37) ; "You are in homicide division now. Use your desk there."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance thePhone of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 38) ; "Your phone sits silently on your desk."
			)
			(3 ; Do
				(if (> gDay 1)
					(Print 20 39) ; "You are in homicide division now. Use the phone there."
				else
					(HandsOff)
					(cInset dispose:)
...
(instance ltDesk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 40) ; "That desk belongs to the Lieutenant."
			)
			(3 ; Do
				(Print 20 41) ; "Use your own desk."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance ltStuff of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 42) ; "That's the Lieutenant's stuff."
			)
			(3 ; Do
				(Print 20 43) ; "Don't mess with the Lieutenant's stuff."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance garbageCan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 44) ; "It's a garbage can."
			)
			(3 ; Do
				(Print 20 45) ; "Now why would you want to take a garbage can?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bulletinBoard of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 46) ; "The bulletin board in here has information so old, there's a missing person bulletin about Jimmy Hoffa."
			)
			(3 ; Do
				(Print 20 47) ; "There's nothing even remotely interesting on that bulletin board."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cabinets of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 48) ; "Those are file drawers."
			)
			(3 ; Do
				(Print 20 49) ; "There's nothing you need in the file drawers."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance rm20picture of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 50) ; "Ah, some of the fine art displayed here at the Lytton County Police Department!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance chair of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 51) ; "It's your good ole, well-worn chair."
			)
			(3 ; Do
				(if (== gDay 1)
					(HandsOff)
					...
					(Print 20 52) ; "You are in homicide division now. Your new desk and chair are there."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance visitorChair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 53) ; "That chair's for your visitors."
			)
			(3 ; Do
				(Print 20 54) ; "Why would you want to sit on that hard visitor's chair when you can sit on your own comfy one?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance ltChair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 55) ; "That chair belongs to the Lieutenant."
			)
			(3 ; Do
				(Print 20 56) ; "You have your own chair. Leave the Lieutenant's alone."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 57) ; "The bookcase contains law reference materials."
			)
			(3 ; Do
				(Print 20 58) ; "You prefer to catch the criminals - let the lawyers read all that crap."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cInset of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 59) ; "You need a computer access card first."
			)
			(2 ; Look
				(Print 20 60) ; "It's a computer."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance computebutton of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 61) ; "That's the power switch."
			)
			(3 ; Do
				(cInset dispose:)
			)
...
(instance screen of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 20 62) ; "That's the computer monitor."
			)
			(3 ; Do
				(Print 20 59) ; "You need a computer access card first."
			)
...
(instance card of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 20 63) ; "That's the computer access card slot."
			)
			(3 ; Do
				(Print 20 59) ; "You need a computer access card first."
			)
			(4 ; Inventory
				(switch invItem
					(21 ; compCard
						(Print 20 64) ; "You insert your computer access card."
						(computerCard play:)
