...
(instance rmPersonel of Rm
	...
	(method (init)
		...
		(quitBut
			...
			x: (proc0_20 307 298 298 298 298)
			init:
		)
		(gTheIconBar disable:)
...
(instance showRecord of Script
	...
	(method (changeState newState &tmp [temp0 200] [temp200 40] [temp240 40] [temp280 40] [temp320 40] [temp360 40])
		(switch (= state newState)
			(0
				(theRoom dispose:)
			...
			(1
				...
				(GetFarText 502 1 @temp240) ; "BADGE"
				(GetFarText 502 2 @temp280) ; "D.O.H."
				(GetFarText 502 3 @temp320) ; "DIVISION"
				(GetFarText 502 4 @temp360) ; "STATUS"
				(if 1
					(= global186 (Memory memALLOC_CRIT (StrLen @temp200)))
					(= global187 (Memory memALLOC_CRIT (StrLen @temp240)))
					(= global188 (Memory memALLOC_CRIT (StrLen @temp280)))
					(= global189 (Memory memALLOC_CRIT (StrLen @temp320)))
					(= global190 (Memory memALLOC_CRIT (StrLen @temp360)))
					(= global192 (Memory memALLOC_CRIT (StrLen @temp200)))
					(= global193 (Memory memALLOC_CRIT (StrLen @temp240)))
					(= global194 (Memory memALLOC_CRIT (StrLen @temp280)))
					(= global195 (Memory memALLOC_CRIT (StrLen @temp320)))
					(= global196 (Memory memALLOC_CRIT (StrLen @temp360)))
					(StrSplitInTwo global186 global192 @temp200)
					(StrSplitInTwo global187 global193 @temp240)
					(StrSplitInTwo global188 global194 @temp280)
					(StrSplitInTwo global189 global195 @temp320)
					(StrSplitInTwo global190 global196 @temp360)
					(Format ; "%s %s %s %s %s"
						@local3
						205
						0
				...
				(GetFarText 502 (+ 1 local0) @temp240)
				(GetFarText 502 (+ 2 local0) @temp280)
				(GetFarText 502 (+ 3 local0) @temp320)
				(GetFarText 502 (+ 4 local0) @temp360)
				(if 1
					(= global186 (Memory memALLOC_CRIT (StrLen @temp200)))
					(= global187 (Memory memALLOC_CRIT (StrLen @temp240)))
					(= global188 (Memory memALLOC_CRIT (StrLen @temp280)))
				...
				(GetFarText 502 6 @temp240) ; "D.O.B."
				(GetFarText 502 7 @temp280) ; "HEIGHT"
				(GetFarText 502 8 @temp320) ; "WEIGHT"
				(if 1
					(= global186 (Memory memALLOC_CRIT (StrLen @temp200)))
					(= global187 (Memory memALLOC_CRIT (StrLen @temp240)))
					(= global188 (Memory memALLOC_CRIT (StrLen @temp280)))
				...
				(GetFarText 502 (+ 6 local0) @temp240)
				(GetFarText 502 (+ 7 local0) @temp280)
				(GetFarText 502 (+ 8 local0) @temp320)
				(if 1
					(= global186 (Memory memALLOC_CRIT (StrLen @temp200)))
					(= global187 (Memory memALLOC_CRIT (StrLen @temp240)))
					(= global188 (Memory memALLOC_CRIT (StrLen @temp280)))
