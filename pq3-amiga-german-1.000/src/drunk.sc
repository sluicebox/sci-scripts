...
(instance theCar of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 885 21) ; "The suspect's car."
			)
			(5 ; Talk
				(cond
					((== local2 0)
...
(instance thePlate of PicView
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 885 22) ; "The plate reads - 01923"
		else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(cond
					((== view 334)
						(Print 885 23) ; "He's sitting quietly in the patrol car."
					)
					((== view 331)
						(Print 885 24) ; "It's a good thing he has a car to lean on."
					)
					(else
						(Print 885 25) ; "He looks to be having a little trouble walking."
					)
				)
			)
			(3 ; Do
				(cond
					...
					((== ((gInventory at: 2) owner:) 13) ; handcuff
						(Print 885 26) ; "Damn" you think to yourself, "I left my cuffs back at the Booking Office."
					)
					(else
			...
			(5 ; Talk
				(if (== local5 4)
					...
					(Print 885 27) ; "Don't bother. He's too incoherent."
				)
			)
			(else
				(super doVerb: theVerb invItem)
...
(instance puke of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print 885 29) ; "You're one sick puppy, Bonds."
			)
			(else
				(super doVerb: theVerb)
			)
