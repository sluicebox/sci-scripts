...
(instance rm048 of PQRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 0) ; "The place has a lot of atmosphere... "cough, cough.""
			)
			(else
				...
				(switch invItem
					(1 ; gun
						(if (== local13 1)
							...
							(Print 48 1) ; "Better keep it holstered until you REALLY need it."
						)
					)
				)
			)
...
(instance sEgoWalkOffScreen of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 48 2) ; "Hmmm... nothing of interest back here."
				(= ticks 1)
			)
			(2
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(= save1 1)
				(pInset init:)
				(Say pInset 48 3) ; "I'm gonna check out the back for the suspect, Sonny."
				(= seconds 4)
			)
			(5
				(pInset dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(switch local11
					(1
						(Print 48 4) ; "Hey that guy kinda looks familiar."
					)
					(2
						(Print 48 5) ; "The punk's hateful glare burns right through you."
					)
					(3
						(Print 48 6) ; "You look carefully at the man. He closely matches the bag lady's physical description of the suspect in Marie's stabbing."
					)
				)
				(= ticks 1)
			)
...
(instance rocklin of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(if (== local11 1)
					(Print 48 7) ; "As you turn to ask the pool player about the vehicle..."
				)
				(Face gEgo rocklin)
				(= local10 1)
			)
			...
			(3 ; Do
				(if (<= local11 2)
					(Print 48 8) ; "If you want something from the man why don't you just ask."
				else
					(Print 48 9) ; "Approaching a potentially armed suspect, unprepared is not advisable."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance bartender of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 10) ; "The bartender is a tough-looking woman. She's sporting a " Peterbilt" T-shirt and a "don't mess with me" grimace. She looks as though she's perfectly capable of keeping her customers in line."
			)
			(5 ; Talk
				(= local1 1)
				(= local0 1000)
				(cond
					((!= (bartender mover:) 0)
						(Print 48 11) ; "Maybe I'd have better luck getting her attention when she isn't quite so busy."
					)
					((!= (bartender cycler:) 0)
						(Print 48 11) ; "Maybe I'd have better luck getting her attention when she isn't quite so busy."
					)
...
(instance sBartenderTalk1 of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 12) ; "You know anything about that car parked out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say bartender 48 13) ; "My customer's cars are their own business."
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 14) ; "Okay, thanks."
				(= seconds 3)
			)
			(3
				(= local1 0)
...
(instance sBartenderTalk2 of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say bartender 48 15) ; "What can I do for ya tonight?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say gEgo 48 16) ; "Nothing. Thanks."
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say bartender 48 17) ; "Hey pal! If yer not buyin' booze, yer just takin' up real estate!"
				(= seconds 3)
			)
			(3
				(= local1 0)
...
(instance sBartenderTalk3 of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say bartender 48 18) ; "If you wanna order a drink then order it."
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say bartender 48 19) ; "I ain't got time for idle chit chat right now!"
				(= seconds 4)
			)
			(2
				(= local1 0)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 48 20) ; "The man playing pool hasn't done anything wrong, except be rude."
					)
				)
			)
			(2 ; Look
				(if (< local18 0)
					(Print 48 21) ; "He's waiting for his pool partner to finish his turn."
				else
					(Print 48 22) ; "The man playing pool looks like a rough customer. You wonder where his pool partner is."
				)
			)
			(5 ; Talk
				(cond
					((== local9 0)
						(Print 48 23) ; "Maybe you should wait until he makes his shot first."
					)
					((== local9 1)
						(HandsOff)
					...
					((== local9 2)
						(Print 48 24) ; "The man ignores you. He seems to be waiting on someone to come finish the game."
					)
				)
			)
			(3 ; Do
				(if (== local9 1)
					(Print 48 25) ; "You think about busting the guy's head, but you restrain yourself."
				else
					(Print 48 26) ; "If you want something from the man why don't you just ask."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance sTalkToPlayer of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 27) ; "Is that your car out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say poolPlayer 48 28) ; "*Sniff* Anybody else in here smell bacon?"
				(= seconds 3)
			)
			(2
				(HandsOn)
...
(instance patron1 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 29) ; "Just a skinny lil' guy minding his own business."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 48 30) ; "Why on earth would you want to do that?!"
					)
				)
			)
			(5 ; Talk
				(switch (++ local7)
					...
					(else
						(Print 48 31) ; "He seems harmless enough. You decide to leave him alone."
					)
				)
			)
			(3 ; Do
				(Print 48 32) ; "The guy hasn't done anything wrong."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sPatron1Talks of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 33) ; "Excuse me, Sir. is that your car parked right out front?"
				(= seconds 5)
			)
			(1
				(= save1 1)
				(Say patron1 48 34) ; "Naw! My car's been in the shop all week. Why?"
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 35) ; "Nothing. thanks."
				(= seconds 2)
			)
			(3
				(HandsOn)
...
(instance patron2 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 36) ; "This guy looks like he sprouted out of that bar stool. Must be a regular here."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
			...
			(5 ; Talk
				(switch (++ local8)
					...
					(else
						(Print 48 37) ; "The guy's busy drowning his sorrows and doesn't pay any attention to you."
					)
				)
			)
			(3 ; Do
				(Print 48 38) ; "He hasn't done anything wrong. Leave him alone."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sPatron2Talks of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 39) ; "Excuse me sir, is that your car parked right out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say patron2 48 40) ; "Not me. I drive a Harley, man!"
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 41) ; "Sorry to bother you."
				(= seconds 4)
			)
			(3
				(HandsOn)
...
(instance rocklinInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 42) ; "Hmm... sure matches the description."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tv of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 43) ; "Hmm.. might be an interesting program if it would stop flipping like that."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poolBalls of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 44) ; "Whoa! Nice set of balls!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poolTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 46) ; "There's a pool table at one end of the bar room - a staple of American neighborhood juice joints."
			)
			(3 ; Do
				(Print 48 47) ; "Touching the pool table while someone is playing isn't a good idea."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance billiardsLight of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 48) ; "The lamp over the pool table gives off a sickly glare."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boose of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 49) ; "The bartender's arsenal of poisons is neatly arranged behind her."
			)
			(3 ; Do
				(Print 48 50) ; "If you want a drink just order one."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance bar of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 51) ; "The bar's worn and waxed wooden surface has obviously seen a lot of action."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tables of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 52) ; "Empty tables and chairs sit silently about, waiting for a crowd to arrive."
			)
			(3 ; Do
				(Print 48 53) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance greaseBoard of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 54) ; "You see various scores of past games."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poolCues of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 55) ; "Those cues look pretty beat up, probably get a sliver from one of them.."
			)
			(3 ; Do
				(Print 48 56) ; "Ahh... if you only had time!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tvFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 57) ; "Looks like someone used the screen for a spitting contest target."
			)
			(3 ; Do
				(Print 48 58) ; "You don't have time to watch television."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance barStools of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 59) ; "The barstool's leather tops have been worn smooth by countless rear ends."
			)
			(3 ; Do
				(Print 48 60) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance candles of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 61) ; "Seventies-style glass candle-holders attempt to soften the atmosphere."
			)
			(3 ; Do
				(Print 48 62) ; "You wouldn't take one of those tacky candles if somebody paid you."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance ashtrays of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 48 63) ; "As in most bars, ashtrays are everywhere. You try to ignore their contents."
			)
			(3 ; Do
				(Print 48 64) ; "No thanks, you don't smoke."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
