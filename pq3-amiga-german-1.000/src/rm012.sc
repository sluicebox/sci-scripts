...
(instance rm012 of PQRoom
		...
		(switch gPrevRoomNum
			...
			(25
				...
				(if
					...
					(gEgo normal: 0 view: 16 setLoop: 4 cel: 0 posn: 94 147)
					((= local1 uCarDoor) setPri: 15)
				else
					(gEgo
						normal: 0
	...
	(method (doit &tmp temp0)
		(if
			...
			(self setScript: (ScriptID 891 3)) ; goToHosScript
		)
...
(instance suspect of Actor
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 12 3) ; "He has a wild look in his eye."
		else
...
(instance drunk of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 4) ; "He is still wobbling."
			)
			(3 ; Do
				(Print 12 5) ; "This is not the appropiate place to search a suspect."
			)
			(else
...
(instance door1 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 6) ; "The automatic doors lead to Prisoner Booking and the Jail."
			)
			(3 ; Do
				(Print 12 7) ; "These doors are sensor activated."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance eviDoor of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 8) ; "That door leads to Evidence Lockup and Lab Analysis."
			)
			(3 ; Do
				(cond
					((trunk showing:) 0)
					((not (IsFlag 18))
						(Print 12 9) ; "The door is locked. I guess the Evidence Officer must be out to lunch."
					)
					((or (IsFlag 5) (IsFlag 16))
						(Print 12 10) ; "You really should do something with the prisoner first."
					)
					((localproc_0)
						(Print 12 11) ; "You need to put the field equipment back into the trunk first."
					)
					(else
						(HandsOff)
...
(instance gunLocker of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 12) ; "The lockers are for storing your weapons before entering the jail."
			)
			(3 ; Do
...
(instance gunLockerDoor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== ((gInventory at: 1) owner:) 12) ; gun
					(Print 12 13) ; "Your gun is in that locker."
				else
					(Print 12 14) ; "A locker door."
				)
			)
			(3 ; Do
				(switch (self cel:)
					(0
						(Print 12 15) ; "It is locked."
					)
					(1
						(self cel: 2 forceUpd:)
						(sfx1 number: 905 loop: 1 play:)
						(Animate (gCast elements:) 0)
						(Print 12 16) ; "You open the locker."
					)
					(2
						(sfx1 number: 906 loop: 1 play:)
						(self cel: 1 forceUpd:)
						(Print 12 17) ; "You close the locker."
					)
					(3
						(Print 12 18) ; "You take your gun."
			...
			(4 ; Inventory
				(switch invItem
					(13 ; gunKey
						(sfx1 number: 905 loop: 1 play:)
						(self cel: 3 forceUpd:)
						(Print 12 19) ; "You unlock the locker."
						(gEgo put: 13 12) ; gunKey
					)
					(1 ; gun
						(if (== (self cel:) 2)
							...
							(sfx1 number: 906 loop: 1 play:)
							(Animate (gCast elements:) 0)
							(Print 12 20) ; "You put your gun in the locker, lock the door, and take the key."
							(SetScore 109 2)
							(gunLockerProp hide:)
							(gunLockerDoor hide:)
							(= local0 0)
						else
							(Print 12 21) ; "Why don't open it first?"
						)
					)
					(16 ; nightStick
						(Print 12 22) ; "It's too long to fit in the gun locker."
					)
					(else
						(Print 12 23) ; "There's no regulation against carrying that into the jail."
					)
				)
			)
			(else
...
(instance gunLockerProp of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 12 24) ; "That locker is already taken."
			)
			(4 ; Inventory
				(switch invItem
					(13 ; gunKey
						(Print 12 25) ; "Your key doesn't open that locker."
					)
					(1 ; gun
						(Print 12 26) ; "Why don't you open it first?"
					)
				)
			)
			(else
...
(instance policeSign of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 12 27) ; "The sign reads 'Lytton County Police Department'."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance garageRoom of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 12 28) ; "You're in the underground garage at the Lytton County Police Department's new building."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance car1 of PicView
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 12 29) ; "The marked patrol car is used by uniformed officers on duty."
		else
			(super doVerb: theVerb &rest)
		)
	)
		...
		(switch theVerb
			...
			(3 ; Do
				...
				(cond
					...
					((localproc_0)
						(Print 12 30) ; "You need to put your equipment back in the trunk."
					)
					(
						(and
							(== gDay 1)
						...
						(Print 12 31) ; "Now where do you think you are going???"
					)
					(else
						(= local2 1)
						(HandsOff)
			...
			(3 ; Do
				(cond
					...
					(
						...
						(Print 12 32) ; "You're trying to be inconspicuous. Leave the marked car for uniformed officers."
					)
					(else
						(trunk marked: 1 open:)
					)
...
(instance uCar1 of PicView
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 12 33) ; "The unmarked department car is used by non-uniformed officers on duty."
		else
			(super doVerb: theVerb &rest)
		)
	)
		...
		(switch theVerb
			...
			(3 ; Do
				...
				(cond
					...
					((not (IsFlag 85))
						(Print 12 34) ; "Don't you think you should checkin at Homicide first?"
					)
					(else
						(HandsOff)
						(= local2 1)
			...
			(3 ; Do
				(cond
					...
					(
						...
						(Print 12 35) ; "The unmarked car is reserved for officers in plain clothes."
					)
					(else
						(trunk marked: 0)
						(trunk open:)
...
(instance pat of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 36) ; "Pat Morales, your partner."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance lightSwitch of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 12 37) ; "It's the light switch for the garage."
			)
			(3 ; Do
				(Print 12 38) ; "You need one of those funny-looking keys."
			)
			(else
				(super doVerb: theVerb)
			)
