...
(instance rm037 of PQRoom
	...
	(method (doit &tmp temp0)
		...
		(cond
			(script 0)
			((and (not (gCast contains: florist)) (& temp0 $0010))
				(HandsOff)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(florMouth setCycle: RandCycle)
				(= seconds 3)
			)
			(2
				(florMouth setCycle: 0 cel: 0)
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance partner of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 37 23) ; "It's Officer Pat Morales, your partner."
			)
			(3 ; Do
				(Print 37 24) ; "Not now Bonds, I need to make a phone call."
			)
			(5 ; Talk
				(Print 37 24) ; "Not now Bonds, I need to make a phone call."
			)
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(Print 37 25) ; "Blow it out your ear, Bonds."
					)
					(else
						(Print 37 26) ; "You hang on to that stuff. I'm not interested in it!"
					)
				)
			)
			(else
...
(instance florist of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 37 27) ; "The florist is a chipper sort, eager to make a sale."
			)
			(3 ; Do
				(Print 37 28) ; "If you want something from the florist, you must pay for it."
			)
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(cond
							((gEgo has: 36) ; rose
								(Print 37 29) ; "You already have a rose."
							)
					...
					(2 ; handcuff
						(Print 37 30) ; "Selling flowers isn't a criminal offense!"
					)
					(1 ; gun
						(Print 37 30) ; "Selling flowers isn't a criminal offense!"
					)
					(36 ; rose
						(Print 37 31) ; "The florist already has PLENTY of roses."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance chitChat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 37 33) ; "Hi, Sergeant Bonds. Your wife's in room 307."
				(= cycles 1)
			)
			(2
				(HandsOn)
...
(instance receptionist of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 37 34) ; "The receptionist looks friendly enough."
			)
			(3 ; Do
				(Print 37 35) ; "If you want something from the receptionist, just ask her."
			)
			(5 ; Talk
				(HandsOff)
				(if (> gDay 3)
			...
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(if (IsFlag 195)
							(Print 37 36) ; "The receptionist stonily ignores you. You gather that she's used to ignoring people."
						else
							(Print 37 37) ; "May I help you, Sir?"
						)
					)
					(1 ; gun
						(Print 37 38) ; "That's right! Torture Marie's room number out of her!"
					)
					(2 ; handcuff
						(Print 37 38) ; "That's right! Torture Marie's room number out of her!"
					)
					(else
						(Print 37 39) ; "The receptionist isn't interested in that."
					)
				)
			)
			(else
...
(instance elevatorUpper of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 40) ; "It looks like an elevator."
			)
			(3 ; Do
				(Print 37 41) ; "Just press the button. Trying to pry those doors open with your hands will only give you a hernia."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance frontEntrance of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 42) ; "That's the door you came in from."
			)
			(3 ; Do
				(Print 37 43) ; "Just walk there."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance flowerShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 44) ; "You notice a flower shop off the lobby."
			)
			(3 ; Do
				(Print 37 43) ; "Just walk there."
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 45) ; "It's a computer."
			)
			(3 ; Do
				(Print 37 46) ; "If you want something, why don't you ask the receptionist?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance frontDesk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 47) ; "It's the front desk."
			)
			(3 ; Do
				(Print 37 48) ; "If you want something, why don't you ask the receptionist?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance carpet of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 49) ; "The hospital floor is kept spotless."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance aisle of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 50) ; "The hospital floor is waxed and gleaming."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance insideHospital of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 37 51) ; "The hospital's architecture is stark and modern - a far cry from the dank corridors of the old hospital."
			)
			(else
				(super doVerb: theVerb)
			)
