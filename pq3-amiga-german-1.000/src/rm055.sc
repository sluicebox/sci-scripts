...
(instance pCar of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 12) ; "Your back-up's patrol car is parked next to yours in the yard."
			)
			(5 ; Talk
				(Print 55 13) ; "If you want to leave, take your own car."
			)
			(else
...
(instance officer of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 14) ; "Your back-up officer is in position."
			)
			(5 ; Talk
				(cond
					((IsFlag 52)
						(Print 55 15) ; "Better find out what the dog's found instead of wasting time out here."
					)
					(local0
						(Print 55 16) ; "Things are too precarious right now to chit chat with your fellow officers."
					)
					(else
						(HandsOff)
...
(instance handler of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 17) ; "Officer Parks is with the K9 unit."
			)
			(5 ; Talk
				(Print 55 18) ; "I wonder what Spike's all upset about?"
			)
			(3 ; Do
				(Print 55 19) ; "This is a slightly higher species than Officer Parks is used to handling."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance halfTrack of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 20) ; "The ram is a fortified military vehicle that's been converted for police work and fitted with a steel hydraulic boom. It can get through just about anything."
			)
			(3 ; Do
				(Print 55 21) ; "If you want the officers to use the ram, you'll need to give the order."
			)
			(5 ; Talk
				(HandsOff)
				(Print 55 22) ; "You give the order for the ram to proceed."
				(gCurRoom setScript: ramScript)
			)
			(else
				(super doVerb: theVerb)
...
(instance frontDoor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if cel
					(Print 55 23) ; "The ram really did a number on that steel door."
				else
					(Print 55 24) ; "The front door is made of steel. It would look more at home on a military installation than on this run-down shack."
				)
			)
			(3 ; Do
				(HandsOff)
			...
			(4 ; Inventory
				(switch invItem
					(16 ; nightStick
						(Print 55 25) ; "That nightstick wouldn't make a dent in the door."
					)
					(1 ; gun
						(Print 55 26) ; "That door could take more bullets than you have."
					)
					(else
						(super doVerb: theVerb)
					)
...
(instance dog of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 55 27) ; "Looks like a happy little mutt."
			)
			(3 ; Do
				(Print 55 28) ; "You've got more important animals to worry about today."
			)
			(4 ; Inventory
				(switch invItem
					(16 ; nightStick
						(Print 55 29) ; "The dog isn't any threat to you."
					)
					(else
						(super doVerb: theVerb)
					)
...
(instance shadowWindow of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 30) ; "The windows have been partially boarded over. Underneath the boards you can see thick metal bars."
			)
			(3 ; Do
				(Print 55 31) ; "Those metal bars aren't going to budge."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pat of Actor
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 55 32) ; "Your partner, Pat Morales."
		else
...
(instance yourDoor of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 55 33) ; "That's the passenger car door."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance yourCar of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 34) ; "You look fondly at your car."
			)
			(3 ; Do
				(if (gEgo has: 9) ; judicialOrder
					(Print 55 35) ; "Your job here is not done yet, Bonds!"
				else
					(HandsOff)
...
(instance theStair of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 36) ; "A rickety flight of stairs lead to the second floor."
			)
			(3 ; Do
				(Print 55 37) ; "Carpentry never was your strong point."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 38) ; "The front lawn is unkempt and neglected. Obviously, the people that live here aren't gardeners."
			)
			(3 ; Do
				(Print 55 39) ; "There's nothing you can do with the lawn."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theHouse of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 40) ; "The house looks like a shack but, judging by the barred windows and heavy front door, it's well fortified."
			)
			(3 ; Do
				(Print 55 41) ; "You can't force your way in with your bare hands."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance wreak of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 42) ; "The old car reminds you of the car you had while you were in college - except this one is in better condition."
			)
			(3 ; Do
				(Print 55 43) ; "Not even the auto club could get this heap started."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance stairs of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 44) ; "A rickety flight of stairs leads to the second floor."
			)
			(3 ; Do
				(Print 55 45) ; "Carpentry was never your strong point."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance barrel of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 46) ; "Discarded junk in the yard adds to the house's air of neglect."
			)
			(3 ; Do
				(Print 55 47) ; "That old junk doesn't interest you."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance porch of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 48) ; "The porch looks like it's about ready to collapse."
			)
			(3 ; Do
				(Print 55 49) ; "Carpentry was never your strong suit."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance meter of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 55 50) ; "There's an electric meter on the side of the house."
			)
			(3 ; Do
				(Print 55 51) ; "The occupant's electric usage does not interest you."
			)
			(else
				(super doVerb: theVerb)
			)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 55 52) ; "Shooting at the bars won't help."
					)
					(16 ; nightStick
						(Print 55 53) ; "That nightstick is not quite up to prying apart steel."
					)
					(else
						(super doVerb: theVerb)
					)
