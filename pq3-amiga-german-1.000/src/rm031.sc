...
(local
	...
	local1
	local2
	[local3 12] = [64 176 82 158 114 161 122 170 116 179 106 181]
	[local15 18] = [181 180 189 160 174 145 180 137 201 136 218 99 272 110 239 189 206 189]
	[local33 26] = [-100 0 319 0 319 108 305 105 291 42 288 36 109 23 6 122 6 182 45 182 74 189 74 289 -100 289]
	[local59 12] = [0 143 112 27 288 43 290 48 113 32 0 153]
)
...
(instance rm031 of PQRoom
	...
	(method (init)
		...
		(poly3 points: @local33 size: 13)
		(poly4 points: @local59 size: 6)
		...
		(switch gPrevRoomNum
			...
			(else
				...
				(cond
					((== global110 10)
						(patWalkToBooth start: 8)
						(= local2 1)
						(pat
							view: 384
							posn: 297 40
							setScript: patWalkToBooth
							init:
						)
					)
					((== global110 20)
	...
	(method (doit &tmp temp0 temp1 temp2)
		...
		(cond
			...
			(
				(and
					(not (== (gCurRoom script:) goEast))
					(gEgo mover:)
					(not (> ((gEgo mover:) x:) (gEgo x:)))
...
(instance patWalkToBooth of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(= seconds
					(if (and (== gDay 4) (== gHowFast 0)) 75 else 60)
				)
			)
			(9
				(= local2 0)
		...
		(switch (= state newState)
			(0
				...
				(gEgo
					...
					cel: 0
					setCycle: End self
					setMotion: PolyPath register (+ (gEgo y:) 35) self
				)
			)
			(1
...
(instance keyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 12) ; "That's the shop of "Zak" the keymaker."
			)
			(3 ; Do
				(HandsOff)
...
(instance armyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 13) ; "That's the Army Recruiter's office."
			)
			(3 ; Do
				(HandsOff)
...
(instance zakInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 31 14) ; "It's Zak the keymaker."
			)
			(4 ; Inventory
				(switch invItem
					(3 ; theKeys
					...
					(19 ; wallet
						(if local0
							...
							(Print 31 15) ; "Just giving away money, Sonny?"
						)
					)
					(13 ; gunKey
						(HandsOff)
...
(instance phone of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 16) ; "It's a public telephone."
			)
			(3 ; Do
				(Print 31 17) ; "You don't have the correct change."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance lamp of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 18) ; "It's a very LARGE streetlamp."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance marieStall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 19) ; "Marie's car was parked here during her attack."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance blacktop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 20) ; "The parking lot blacktop."
			)
			(else
				(super doVerb: theVerb)
			)
	...
	(method (handleEvent event)
		(if (and (== ((gTheIconBar curIcon:) message:) 1) (& (event type:) $0040)) ; direction
			(if (== (gEgo view:) 361)
				(gCurRoom obstacles: polyList0)
			)
...
(instance otherStores of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 21) ; "Some of the mall's stores are undergoing renovation."
			)
			(3 ; Do
				(Print 31 22) ; "That store is closed."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance sidewalk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 23) ; "Sidewalks connect the store fronts."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pat of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if local2
					(Print 31 24) ; "Morales is making a phone call."
				else
					(Print 31 25) ; "That's Officer Pat Morales, your partner."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance theCar of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 26) ; "You look fondly at your car."
			)
			(3 ; Do
				(HandsOff)
...
(instance star of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 31 27) ; "You spot an object laying on the ground."
			)
			(3 ; Do
				(Print 31 28) ; "You retrieve the bronze star."
