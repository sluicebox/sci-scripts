...
(instance lights of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 27) ; "The fire engine's lights are on."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance embers1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== local0 0)
					(Print 51 28) ; "You see the fire gutted shell of a house still ablaze. It looks like the fire fighters have it under control."
				else
					(Print 51 29) ; "You see the fire gutted shell of a house."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance hoseWater1 of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 30) ; "The hose is squirting water."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance grass of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 31) ; "The lawn is green and closely clipped."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance sidewalk of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 32) ; "The sidewalk is wet with water from the firemen's hoses."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance windows of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 33) ; "The windows have been blown to pieces by the heat of the fire."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance hydrant of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 51 34) ; "You see a fire hydrant."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance firetruck of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 51 35) ; "It's a Lytton Fire Department pumper."
			)
			(3 ; Do
				(Print 51 36) ; "The firemen have the fire well in hand and don't need your assistance."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance theCar of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 51 37) ; "It's the trunk of your car."
			)
			(3 ; Do
				(cond
					((localproc_0)
						(Print 51 38) ; "You need to put the field equipment back into the trunk first."
					)
					((gEgo has: 6) ; photo
						(proc0_17 5)
						(SetFlag 202)
...
(instance theTrunk of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 51 39) ; "It's The trunk of your car."
			)
			(3 ; Do
				(trunk open:)
			)
