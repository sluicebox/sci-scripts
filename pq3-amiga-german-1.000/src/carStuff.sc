...
(instance carStuff of Rgn
	...
	(method (newRoom newRoomNumber)
		...
		(if (and (OneOf newRoomNumber 204 890) (gCast contains: trackInset)) ; rmDmv, dilema
			(SetFlag 213)
		else
			(ClearFlag 213)
		)
...
(instance ones of Prop
	...
	(method (doit)
		(super doit:)
		(if (and (!= cel 10) (not ((ScriptID 25 1) engineOn:))) ; mainInset
			(= cel 10)
		)
	)
)
...
(instance chart of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 130 6) ; "It's a speedometer calibration chart - a service record for this car's speedometer. Departmental policy requires that all speedometers be calibrated every 90 days."
			)
			(3 ; Do
				(if (or (not (IsFlag 28)) (IsFlag 29) (IsFlag 146))
					(Print 130 7) ; "You should leave that with the vehicle unless you have a specific need for it."
				else
					(self dispose:)
					(gEgo get: 10) ; calibration
					(SetScore 146 5)
					(Print 130 8) ; "Thinking about your upcoming court appearance, you wisely decide to take the speedometer calibration chart with you."
				)
			)
			(else
				(super doVerb: theVerb invItem)
...
(instance theTicket of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 130 9) ; "It's a traffic citation."
			)
			(3 ; Do
				(Print 130 10) ; "You pull the ticket and the license out of the computer."
				(gEgo get: 24) ; ticket
				(self dispose:)
			)
			(else
			...
			(1
				(if (> (++ register) 25)
					(if (gCast contains: trackInset)
						(Print 130 12) ; "The blip has stopped on the freeway."
					)
					(gLongSong fade:)
					(SetFlag 88)
...
(instance crashTimerScript of Script
	...
	(method (doit)
		(if
			...
			(if (== ((ScriptID 25 1) heading:) 270) ; mainInset
				(Print 130 13) ; "Up ahead you see a car overturned in the middle of the freeway.  You slow to a stop."
			else
				(Print 130 14) ; "Up ahead you see a car overturned on the other side of the freeway. You turn around and slow to a stop."
			)
			(self start: 4 init:)
		)
		(super doit:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(ClearFlag 88)
				(Print 130 15) ; "The car radio interrupts the silence:"
				(Print 130 16) ; "Dispatch says..."64 David.""
				(Print 130 17) ; "Morales responds..."This is 64 David, go ahead with your traffic.""
				(Print 130 18) ; "Respond to an 11-44, west bound on the freeway just east of the Lilly off ramp."
				(Print 130 19) ; "Continuing she says..."Be advised this is your possible suspect vehicle.""
				(Print 130 20) ; "Your partner acknowledges the call and says... "Get this puppy rolling Bonds!""
				(= seconds (if (not gHowFast) 600 else 300))
			)
			(3
				(Print 130 21) ; "You have failed to respond to the Freeway accident in a reasonable amount of time."
				(EgoDead 0 130 22) ; "Another case of dereliction of duty!"
			)
			(4
...
(instance trackButt of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 130 24) ; "This button activates the tracker."
			)
			(3 ; Do
				(if (not (gCast contains: trackInset))
					(if ((ScriptID 25 1) engineOn:) ; mainInset
						...
						(Print 130 25) ; "The engine must be running to use the tracker."
					)
				else
					(trackInset dispose:)
					(trackLight cue:)
...
(instance trackLight of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 130 26) ; "The small light indicates that the tracker is activated."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance lightFeat of Feature
	...
	(method (doVerb theVerb invItem &tmp temp0)
		(switch theVerb
			(2 ; Look
				(Print 130 27) ; "This button controls the emergency lights and siren."
			)
			(3 ; Do
				(if (= local26 (not local26))
					(if ((ScriptID 25 1) engineOn:) ; mainInset
						...
						(if (and (!= gDay 1) (not (IsFlag 209)))
							(Print 130 28) ; "Pat places the portable red light on the dash."
						)
						(siren play:)
						(SetFlag 209)
					else
...
(instance keyFeat of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 130 29) ; "An ignition key."
			)
			(3 ; Do
				(cond
					(((ScriptID 25 1) mySpeed:) ; mainInset
						(Print 130 30) ; "Turning the engine off while you're moving is not a good idea, Sonny."
					)
					(
						(and
							(gEgo has: 23) ; license
						...
						(Print 130 31) ; "You need to return the driver's license first."
					)
					((and (== ((gInventory at: 2) owner:) 885) (not (IsFlag 16))) ; handcuff
						(Print 130 32) ; "You left the drunk standing by his car in your handcuffs!"
					)
					((IsFlag 225)
						(Print 130 33) ; "Better wait for your partner."
					)
					((== gPrevRoomNum 12)
						(if (== gDay 1)
							(Print 130 34) ; "You must have forgotten about being temporarily assigned to traffic division. You'll have to drive the black and white today, Sonny."
						else
							(Print 130 35) ; "You're a homicide detective now. You don't drive a black and white anymore."
						)
					)
...
(instance compFeat of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 130 36) ; "It's a computer. Lytton sure is going high-tech."
			)
			(3 ; Do
				(if (== gPrevRoomNum 12)
					(super doVerb: theVerb invItem)
					...
					(23 ; license
						(cond
							(
								...
								(Print 130 37) ; "You already have the ticket."
							)
							((== ((gInventory at: 23) state:) 3) ; license
								(Print 130 38) ; "A ticket isn't going to keep this guy from killing someone on the road. You're going to have to take him downtown."
							)
							(else
								(Print 130 39) ; "You insert the driver's license into the slot.  This activates the computer."
		...
		(switch theVerb
			(2 ; Look
				(switch global134
					...
					(1
						(Print 130 40) ; "It's your partner, Pat Morales."
					)
					(2
						(Print 130 41) ; "It's your buddy, Mr. Loony Tunes!"
					)
					(3
						(Print 130 42) ; "It's your buddy, Mr."But I only had one beer!""
					)
				)
			)
			(5 ; Talk
				(switch global134
					...
					(1
						(switch (Random 0 4)
							(0
								(Print 130 43) ; "Morales says..."Some day, eh, Sonny?""
							)
							(1
								(Print 130 44) ; "Morales teasingly says..."Ya drive like my Grandma, Bonds.""
							)
							(2
								(Print 130 45) ; "Morales says..."Ya tryin' ta rack up points for pedestrians, or what, Bonds?""
							)
							(3
								(Print 130 46) ; "Morales says..."If ya want me to drive, jus' holler.""
							)
							(4
								(Print 130 47) ; "Excitedly, Morales says..."Did ya see that gorgeous hunka man back there?""
							)
						)
					)
					(2
						(switch (Random 0 2)
							(0
								(Print 130 48) ; "Out of his head, the prisoner says..."Cold!  I'm so f@!#ing cold! Who let the fire go out, for C#$%@ sake?""
							)
							(1
								(Print 130 49) ; "Deliriously, the prisoner says..."I wasn't doin' nothin'. I wasn't the one! Oh, no, don't look at me!""
							)
							(2
								(Print 130 50) ; "The prisoner rambles incoherently..."Don't put that on me. I ain't got nuthin', I told yas.""
							)
						)
					)
					(3
						(if (not ((ScriptID 25 1) mySpeed:)) ; mainInset
							(Print 130 51) ; "The driver says..."This isn't nessethary, Occifer. I'm perfectedly fine.""
						else
							(switch (Random 0 2)
								(0
									(Print 130 52) ; "The driver says..."This isn't nessethary occifer. I'm perfectedly fine.""
								)
								(1
									(Print 130 53) ; "The driver says..."Could you schlow it down, pleasse, Doc? My head is schpinning.""
								)
								(2
									(Print 130 54) ; "The driver says..."We juss passsed my place there. Maybe you could juss drop me off...?""
								)
							)
						)
					)
...
(instance purse of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (proc882_2 1)
					(Print 130 55) ; "Pat's purse rests between the seats."
				else
					(Print 130 56) ; "Pat's purse rests between the seats. She keeps a key attached to the handle."
				)
			)
			(3 ; Do
				(cond
					((== global134 1)
						(Print 130 57) ; "You can't just take the key with Pat sitting there."
					)
					((proc882_2 1)
						(Print 130 58) ; "You already have the key. Don't mess with her purse anymore."
					)
					(else
						(proc882_0 1)
						(SetScore 147 5)
						(Print 130 59) ; "You take the key, knowing that it'll have to be returned before Morales notices that it's gone."
						(self setCel: 1)
					)
				)
			)
			(4 ; Inventory
				(switch invItem
					(3 ; theKeys
						(if (proc882_2 1)
							(Print 130 60) ; "In the nick of time, you replace Morales' key and hope she doesn't notice anything funny."
							(proc882_1 1)
							(SetScore 149 1)
							(self setCel: 0)
...
(instance trackFeat of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 130 61) ; "It's a tracking screen. This device allows you to track a person, vehicle, or any other moving object that a tracking device has been attached to."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance radioFeat of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 130 62) ; "It's a police radio."
			)
			(else
				(super doVerb: theVerb &rest)
			)
