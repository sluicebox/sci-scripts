...
(instance reporter of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 30 17) ; "Uh-oh. It looks like one of those ambulance-chasing reporters."
			)
			(5 ; Talk
				(if (not (gEgo has: 38)) ; busCard
					...
					(Print 30 18) ; "You feel a moment of remorse for your outburst, and almost say 'thanks', but fortunately the moment passes."
				)
			)
			(else
...
(instance officer of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 30 19) ; "One of the uniformed officers is busy searching the scene of the crime."
			)
			(5 ; Talk
				(Print 30 20) ; "The officer is intent on his search and ignores you."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pCar of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 30 21) ; "It's a Lytton P.D. patrol car."
			)
			(3 ; Do
				(HandsOff)
					...
					(4 ; flashlight
						(cond
							((== (gEgo view:) 380)
								(gEgo view: 1)
								(spot dispose:)
								(Print 30 22) ; "You turn your flashlight off."
							)
							((== ((gInventory at: 4) state:) 1) ; flashlight
								(gEgo view: 380)
								(spot init:)
								(Print 30 23) ; "You turn your flashlight on and begin a search of the scene."
							)
							(else
								(Print 30 24) ; "Your batteries appear to be dead."
							)
						)
					)
					(else
...
(instance star of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if local0
					(Print 30 25) ; "You catch the glint of an object lying just beneath Marie's car."
				else
					(super doVerb: theVerb)
				)
			)
			(3 ; Do
				(if local0
					(Print 30 26) ; "You pick up what appears to be a medallion. Upon closer inspection you recognize it to be a "Bronze Star" war medal."
...
(instance lamp of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 27) ; "It's a very LARGE streetlamp."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance blacktop of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 29) ; "The parking lot blacktop."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
				...
				(switch invItem
					(4 ; flashlight
						(cond
							...
							(else
								(Print 30 30) ; "Your batteries appear to be dead."
							)
						)
					)
					(else
...
(instance otherStores of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 31) ; "Some of the mall's stores are undergoing renovation."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 32) ; "That store is closed."
			)
			(4 ; Inventory
				(switch invItem
					(4 ; flashlight
...
(instance sidewalk of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 33) ; "Sidewalks connect the store fronts."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(4 ; Inventory
				(switch invItem
					(4 ; flashlight
						(cond
							((== (gEgo view:) 380)
								(Print 30 34) ; "The flashlight is already on."
							)
							((== ((gInventory at: 4) state:) 1) ; flashlight
								(gEgo view: 380)
								(spot init:)
...
(instance armyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 35) ; "That's the Army Recruiter's office."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 36) ; "The Army Recruiter's office is closed for the night."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance keyShop of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 380)
					(Print 30 37) ; "That's the shop of "Zak" the keymaker."
				else
					(Print 30 28) ; "You try to search the area, but it's just too dark to see anything. Perhaps you should come back when it's light."
				)
			)
			(3 ; Do
				(Print 30 38) ; "Zak's key shop is closed for the night."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance mariesCar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 30 39) ; "Marie's Trans Am still sits in the poorly lit parking lot of the mall. You scan the car for clues... but find nothing."
			)
			(else
				(super doVerb: theVerb)
			)
