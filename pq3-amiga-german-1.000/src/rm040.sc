...
(instance rm040 of PQRoom
	...
	(method (init)
		...
		(if (== gDay 1)
			...
			(carMid init: stopUpd:)
			(carBack init: stopUpd:)
			(carDoor setPri: 15 init: stopUpd:)
		)
		(super init:)
		(dresser init:)
		(bed init:)
...
(instance bed of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 12) ; "It's your bed. It looks empty without Marie."
			)
			(3 ; Do
				(Print 40 13) ; "Anything you could 'do' with the bed wouldn't be any fun without Marie."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance phone of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 14) ; "You glance at the phone and hope that nobody bothers you at home for once."
			)
			(3 ; Do
				(Print 40 15) ; "There's no one you need to call."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance nightStand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 16) ; "It's a night stand."
			)
			(3 ; Do
				(Print 40 17) ; "The night stand doesn't interest you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance rm40window of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 18) ; "Outside the Lytton suburbs look peaceful."
			)
			(3 ; Do
				(Print 40 19) ; "The temperature in the room is fine. You decide not to open the window."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance readingArea of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 20) ; "Marie used to curl up in that chair for hours. This was her favorite little reading area."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance furnishings of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 21) ; "Marie was so excited about buying this house. She loved decorating this room and hand-picking each accessory."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print 40 22) ; "You don't have time to sit down and read. You're late for work."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance musicBox of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 40 23) ; "The music box plays Marie's favorite song. She liked to listen to it while you were on duty because it reminded her of you."
			)
			(3 ; Do
				(HandsOff)
