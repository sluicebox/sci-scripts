...
(instance rm041 of PQRoom
	...
	(method (init)
		...
		(if (== ((gInventory at: 2) owner:) 41) ; handcuff
			(groceryCart setPri: 6 posn: 138 116 init: stopUpd:)
		)
		(if (== gPrevRoomNum 10)
			(newsPaper init:)
			(= global105 16)
			(= global106 30)
			(bagLady view: 449 setCycle: Walk posn: -15 126 init: stopUpd:)
			(HandsOff)
			(gCurRoom setScript: returnToHome)
		)
		(if (IsFlag 4)
			(HandsOn)
			(groceryCart init: stopUpd:)
...
(instance blwalkUpStairs of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(bagLady setLoop: -1 setPri: -1 stopUpd:)
				(self dispose:)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(insetBagLady dispose:)
				(insetEgo init:)
				(Say insetEgo 41 12) ; "You called and said you saw something at the Oak Tree Mall last night."
				(= seconds 15)
			)
			(6
				(= save1 1)
				(Say insetEgo 41 13) ; "I'd like you to come downtown with me and help us out, maybe give us a description."
				(= seconds 15)
			)
			(7
				(= save1 1)
				(insetEgo dispose:)
				(insetBagLady init:)
				(Say insetBagLady 41 14) ; "Ooh! I'd love ta go ridin' in one of them real life squad cars. Okay! Just help me put my stuff in the car."
				(= seconds 10)
			)
			(8
				(= save1 1)
				(insetEgo init:)
				(insetBagLady dispose:)
				(Say insetEgo 41 15) ; "Ma'am, I'm sorry but there's not enough room in the trunk for your cart. You'll have to leave it here."
				(= seconds 10)
			)
			(9
				(= save1 1)
				(insetEgo dispose:)
				(insetBagLady init:)
				(Say insetBagLady 41 16) ; "What?! Leave my STUFF? Ooh, nooo! I can't very well just up and leave my STUFF."
				(= seconds 10)
			)
			(10
				(insetBagLady dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(insetBagLadyStand init:)
				(if (== ((gInventory at: 2) owner:) 41) ; handcuff
					(Say insetBagLadyStand 41 17) ; "Well, bless your heart. My cart is still safe."
				else
					(Say insetBagLadyStand 41 18) ; "Thanks for the sandwich."
				)
				(= seconds 7)
			)
			(6
...
(instance getCuffs of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(NormalEgo 5 1)
				(Print 41 19) ; "You retrieve your cuffs."
				(gEgo get: 2) ; handcuff
				(SetScore 143 2)
				(HandsOn)
				(self dispose:)
		...
		(switch theVerb
			(3 ; Do
				(switch local0
					...
					(2
						(Print 41 20) ; "You move to place a hand on the bag lady's arm and she draws away in horror. She obviously hates being touched."
					)
					(3
						(Print 41 21) ; "Manhandling the bag lady is not going to solve anything. She's not likely to help you if she's angry."
					)
					(else
			...
			(2 ; Look
				(switch local0
					(0
						(Print 41 22) ; "The shape of that pile of newspapers looks vaguely familiar."
					)
					(else
						(Print 41 23) ; "As you look closely at the bag lady, you realize that she's not nearly as old as you first thought. The streets have a way of making all of the homeless look old and used up."
						(= local2 1)
					)
				)
			)
				...
				(switch invItem
					(19 ; wallet
						(switch local0
							...
							(0
								(if local2
									(Print 41 24) ; "She's asleep. Why don't you try it when she's awake?"
								else
									(Print 41 25) ; "Showing your badge to a bundle of newspapers is pretty useless."
								)
							)
							(1
								(Print 41 26) ; "She's still waking up, she doesn't see your ID."
							)
							(else
								(Print 41 27) ; "Showing her your badge again doesn't make her any more impressed."
							)
						)
					)
					(2 ; handcuff
						(Print 41 28) ; "You only need handcuffs to restrain a suspect, not a witness."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance newsPaper of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 29) ; "The pile of grungy newspapers is empty now."
			)
			(3 ; Do
				(Print 41 30) ; "You ought not to disturb the old woman's... uh... bed."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(switch local0
					(0
						(Print 41 31) ; "The rusty shopping cart is loaded down with junk."
					)
					(else
						(if local83
							(Print 41 32) ; "Carla's shopping cart is loaded down with her treasures and memories."
						else
							(Print 41 33) ; "The rusty shopping cart is loaded down with the old woman's treasures and memories."
						)
					)
				)
			)
			(3 ; Do
				(if (== ((gInventory at: 2) owner:) 41) ; handcuff
					...
					(switch local0
						(3
							(Print 41 34) ; "It wouldn't fit in your car."
						)
						(0
							(if local83
								(Print 41 35) ; "Carla needs her cart more than you do."
							else
								(Print 41 36) ; "The cart probably belongs to a homeless person.  They need that stuff more than you do."
							)
						)
						(else
							(Print 41 37) ; "I think the bag lady would be irritated if you took off with her cart."
						)
					)
				)
			)
			(4 ; Inventory
				(if (== invItem 2) ; handcuff
					(if (== local0 3)
						...
						(Print 41 38) ; "You have no reason to do that right now."
					)
				else
					(super doVerb: theVerb &rest)
				)
...
(instance car of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 39) ; "You look fondly at your car."
			)
			(3 ; Do
				(HandsOff)
...
(instance warehouse of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 40) ; "This is the loading docks of a warehouse."
			)
			(3 ; Do
				(Print 41 41) ; "Use your car if you want to leave."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance truck of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 42) ; "There's a company truck parked here."
			)
			(3 ; Do
				(Print 41 41) ; "Use your car if you want to leave."
			)
...
(instance pipe of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 43) ; "There's a large, sturdy water pipe here."
			)
			(3 ; Do
				(Print 41 44) ; "What do you want to do with the pipe?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance trash of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 45) ; "The loading docks is littered with trash."
			)
			(3 ; Do
				(Print 41 46) ; "Nice thought, but you don't have time to clean up the place."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance trashBin of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 47) ; "It's a large trash bin for the warehouse."
			)
			(3 ; Do
				(Print 41 48) ; "There's nothing you need in the trash bin. Leave that stuff for the garbage collector."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance light of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 49) ; "There's a lamp on the loading docks, probably to discourage break-ins."
			)
			(else
				(super doVerb: theVerb &rest)
			)
