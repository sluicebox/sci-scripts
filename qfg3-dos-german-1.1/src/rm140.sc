...
(local
	...
	[local3 11]
	[local14 19]
	[local33 15] = [28 32 49 64 83 100 32 45 57 71 84 96 110 116 123]
	[local48 15] = [28 33 50 67 83 100 31 44 56 70 83 95 109 117 122]
	[local63 15] = [28 34 50 66 82 98 35 47 59 71 83 95 107 114 119]
	[local78 15] = [100 12 12 12 12 12 206 206 206 206 206 206 206 12 206]
	[local93 15] = [100 11 11 11 11 11 207 207 207 207 207 207 207 11 207]
	[local108 15] = [100 11 12 12 13 13 207 207 207 207 207 207 209 12 209]
	[local123 15] = [0 1 2 3 4 5 7 8 9 10 11 12 13 6 5]
	[local138 15] = [0 1 2 3 4 3 6 7 8 1 1 9 10 5 1]
	[local153 15] = [0 1 2 3 4 5 7 9 10 1 9 11 12 6 12]
	[local168 14] = [1 2 3 4 5 7 8 9 10 11 12 13 6 14]
	[local182 15] = [0 1 2 3 4 5 13 6 7 8 9 10 11 12 14]
	[local197 51] = [0 150 80 130 150 100 150 130 100 0 0 100 0 0 90 0 80 150 100 100 100 80 0 100 0 0 0 0 150 120 0 100 100 150 100 100 100 0 100 100 100 100 100 0 100 67 78 68 20 23 27]
	[local248 11] = [0 50 0 0 50 0 50 0 50 0 0]
	[local259 11] = [100 100 50 100 100 100 100 100 100 100 50]
	local270 = 1
	[local271 39]
	local310
	local311 = 50
	...
	local340 = 1
	local341
	[local342 6] = [71 159 245 71 159 245]
	[local348 6] = [42 44 43 44 46 45]
	local354
	[local355 10]
	[local365 10]
	[local375 5]
	...
	(cond
		((or (== gHeroType 3) (== gHeroType 0)) ; Paladin, Fighter
			(for ((= temp0 1)) (< temp0 15) ((++ temp0))
				(= [local197 temp0] [gEgoStats (- temp0 1)])
			)
		...
		((== gHeroType 1) ; Magic User
			...
			(while (< temp0 30) ; UNINIT
				(= [local197 temp0] [gEgoStats (- temp0 16)]) ; UNINIT
				(++ temp0) ; UNINIT
			)
			(= local270 1)
		)
		((== gHeroType 2) ; Thief
			...
			(while (< temp0 45) ; UNINIT
				(= [local197 temp0] [gEgoStats (- temp0 31)]) ; UNINIT
				(++ temp0) ; UNINIT
			)
			(= local270 2)
		)
	)
	(StrCpy @local271 @global157)
	(= local310 (StrLen @local271))
)
...
(procedure (localproc_2 &tmp [temp0 4] temp4)
	(for ((= temp4 1)) (< temp4 15) ((++ temp4))
		(= [gEgoStats (- temp4 1)] [local312 temp4])
	)
	...
	(Message msgGET 140 8 0 5 1 @local375) ; "%d"
	(Format @temp0 @local375 local311)
	...
	(localproc_1
		170
		86
		(Format @temp0 @local375 (= [gEgoStats 16] (gEgo maxHealth:))) ; health
	)
	(DrawCel 145 6 0 287 159 15)
	(localproc_1
		157
		284
		(Format @temp0 @local375 (= [gEgoStats 17] (gEgo maxStamina:))) ; stamina
	)
	(DrawCel 145 6 0 287 172 15)
	(localproc_1
		170
		284
		(Format @temp0 @local375 (= [gEgoStats 18] (gEgo maxMana:))) ; mana
	)
	(DrawCel 145 6 0 285 (proc0_21 135 132 136 132 132) 15)
	(localproc_1
		(proc0_21 133 130 134 130 130)
		283
		(Format @temp0 @local375 [gEgoStats 14]) ; honor
	)
)
	...
	(switch local270
			...
			(if [gEgoStats 12] ; magic use
				(for ((= temp0 19)) (< temp0 30) ((++ temp0))
					(if (not [gEgoStats temp0])
						(= [gEgoStats temp0] [local248 (- temp0 19)])
					)
				)
			)
		)
		...
		(1
			...
			(if [gEgoStats 12] ; magic use
				(for ((= temp0 19)) (< temp0 30) ((++ temp0))
					(if (not [gEgoStats temp0])
						(= [gEgoStats temp0] [local259 (- temp0 19)])
					)
				)
			)
		)
...
(procedure (localproc_5 param1 &tmp temp0 temp1)
	...
	(if
		(or
			...
			(and (not (HaveMouse)) (not local341))
		)
		(gGame
			setCursor:
				999
...
(procedure (localproc_6 &tmp [temp0 4])
	(Message msgGET 140 1 6 2 1 @local355) ; "Yes"
	(Message msgGET 140 1 6 3 1 @local365) ; "No"
...
(instance rm140 of Rm
	...
	(method (handleEvent event)
		(if local340
			...
			(cond
				((or (event claimed:) (not local341)) 0)
				((& (event type:) $0040) ; direction
					(switch (event message:)
						(JOY_LEFT
							(event claimed: 1)
...
(instance selectChar of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(= gBaseHeroType (= gHeroType local270))
				(gCurRoom picture: 145)
				(DrawPic 145)
				(= global157 0)
				(= cycles 2)
...
(instance startControls of GameControls
	...
	(method (init &tmp temp0)
			...
			(self
				add:
					((selectionIcon new:)
						...
						state: [local168 (- temp0 1)]
						yourself:
					)
			)
			(= [local326 temp0] [local197 (+ temp0 (* local270 15))])
			(= [local312 temp0] [local197 (+ temp0 (* local270 15))])
		)
		(self
			add:
				((selectionIcon new:)
		...
		(= [local326 13] [local197 (+ 13 (* local270 15))])
		(= [local312 13] [local197 (+ 13 (* local270 15))])
		...
		(= [local326 14] [local197 (+ 14 (* local270 15))])
		(= [local312 14] [local197 (+ 14 (* local270 15))])
...
(instance selectionIcon of IconI
			...
			(0
				(cond
					...
					(
						(and
							(== [local312 [local182 state]] 10)
							(not [local326 [local182 state]])
						)
						(= [local312 [local182 state]] 0)
						(+= local311 30)
					)
					(
						(and
						...
						(+= local311 (+ 20 [local312 [local182 state]]))
						(= [local312 [local182 state]] 0)
					)
					(
						(<
	...
	(method (highlight param1 &tmp temp0 [temp1 4] temp5 temp6)
		...
		(Message msgGET 140 8 0 5 1 @local375) ; "%d"
		(Display
			(Format @temp1 @local375 [local312 [local182 state]])
			dsCOORD
			(+ temp0 1)
			nsTop
			dsCOLOR
...
(instance startIcon of IconI
	...
	(method (select &tmp temp0 temp1)
		(if (& (startIcon signal:) $0004)
			...
			(= local1 0)
			(if (and local311 (localproc_6))
				(return 0)
			else
				(HandsOff)
				(localproc_4)
				(for ((= temp0 1)) (< temp0 15) ((++ temp0))
					(= [global211 (- temp0 1)] [local312 temp0])
				)
...
(instance namePlate of IconI
	...
	(method (select param1 &tmp [temp0 4])
		(TextSize @[temp0 0] @local271 3 0)
		(cond
			((== param1 8)
				(if local310
					(StrAt @local271 (-- local310) 0)
					(DrawCel
						maskView
						maskLoop
						maskCel
			...
			((<= [temp0 3] 150)
				(StrAt @local271 local310 param1)
				(StrAt @local271 (++ local310) 0)
				(self highlight: 1)
			)
			(else
	...
	(method (highlight param1 &tmp temp0 temp1 temp2)
		(= nsLeft (proc0_21 62 63 62 63 63))
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(3
				(if (== (theTitle x:) 71)
					(Message msgGET 140 2 0 0 1 @local3) ; "Fighter"
					(Display @local3 dsFONT 123 dsCOORD 24 29 dsWIDTH 100 dsALIGN alCENTER dsCOLOR 0)
					(Display @local3 dsFONT 123 dsCOORD 23 28 dsCOLOR 30 dsWIDTH 100 dsALIGN alCENTER)
				)
				(= seconds 3)
			)
			(4
			...
			(6
				(if (== (theTitle x:) 159)
					...
					(Display @local3 dsFONT 123 dsCOORD 108 29 dsCOLOR 0 dsWIDTH 100 dsALIGN alCENTER)
					(Display @local3 dsFONT 123 dsCOORD 107 28 dsCOLOR 30 dsWIDTH 100 dsALIGN alCENTER)
				)
			...
			(10
				(if (== (theTitle x:) 245)
					(Message msgGET 140 4 0 0 1 @local3) ; "Thief"
					(Display @local3 dsFONT 123 dsCOORD 191 30 dsCOLOR 0 dsWIDTH 100 dsALIGN alCENTER)
					(Display @local3 dsFONT 123 dsCOORD 190 29 dsCOLOR 30 dsWIDTH 100 dsALIGN alCENTER)
				)
...
(instance yesIcon of IconI
	...
	(method (show)
		...
		(Display @local355 dsCOORD 20 (+ nsTop 3) dsCOLOR 17)
		(if (& signal $0004)
			(self mask:)
		)
	...
	(method (highlight param1 &tmp temp0)
		...
		(Display @local355 dsCOORD 20 (+ nsTop 3) dsCOLOR temp0)
	)
)
...
(instance noIcon of IconI
	...
	(method (show &tmp [temp0 15])
		...
		(= nsBottom (+ nsTop 15))
		(DrawCel view loop cel nsLeft nsTop -1)
		(Display @local365 dsCOORD 20 (+ nsTop 3) dsCOLOR 17)
		(if (& signal $0004)
			(self mask:)
		)
		(if (and gPMouse (gPMouse respondsTo: #stop))
	...
	(method (highlight param1 &tmp temp0)
		...
		(Display @local365 dsCOORD 20 (+ nsTop 3) dsCOLOR temp0)
	)
)
