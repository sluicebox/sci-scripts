...
(procedure (NotClose)
	(Print 0 34) ; "It's not quite within reach."
)

(procedure (AlreadyTook)
	(Print 0 35) ; "You've accomplished that task."
)

(procedure (SeeNothing) ; UNUSED
	(Print 0 36) ; "You see nothing special."
)

(procedure (CantDo)
	(Print 0 37) ; "That's not possible now."
)

(procedure (DontHave)
	(Print 0 38) ; "You don't have it."
)
...
(procedure (proc0_18 param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8)
	(if (== argc 1)
		(Display 0 39 dsRESTOREPIXELS [param1 0])
	else
		(= temp4 (= temp5 -1))
		(= temp0 0)
		(= temp1 68)
...
(instance statusCode of Code
	...
	(method (doit param1)
		(Format param1 0 0 gScore gPossibleScore 0 1 {Space Quest III} 0 1) ; "Score: %d of %d%13s%s%1s"
	)
)
...
(instance SQ3 of Game
	...
	(method (replay)
		(TheMenuBar draw:)
		(SL enable:)
		(SetMenu 1538 110 (if (DoSound sndSET_SOUND) {Turn Off} else {Turn On}))
		(super replay:)
	)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'wait')
						(Print 0 3) ; "This isn't a text adventure!"
					)
					((or (Said 'wear/belt') (Said 'drop<on/belt'))
						(if (gEgo has: 16) ; Invisibility_Belt
							(if (not global126)
								(Print 0 4) ; "OK, you wear the belt."
								(= global126 1)
							else
								(Print 0 5) ; "You're already wearing it."
							)
						else
					...
					(
						...
						(cond
							...
							((not global126)
								(Print 0 6) ; "First, you need to wear it."
							)
							((== global120 2)
								(Print 0 7) ; "The belt is now completely out of power."
							)
							(else
								(Print 0 8) ; "Looking at the belt, you notice that the power supply is very low. You decide to hold off until you really need it."
							)
						)
					)
					((Said 'wear,(drop<on)/panties')
						(cond
							...
							(global182
								(Print 0 9) ; "You're already wearing them."
							)
							(else
								(Print 0 10) ; "Not a good idea, considering your location."
							)
						)
					)
					((Said 'wear,(drop<on)/decoder')
						(if (not (gEgo has: 7)) ; Monolith_Decoder_Ring
							...
							(Print 0 11) ; "You decide to leave the ring in the relative safety of your pocket."
						)
					)
					(
						(or
						...
						(Print 0 12) ; "I'll get naked if you get naked. You go first."
					)
					((Said 'wear/cap')
						(if (gEgo has: 6) ; Astro_Chicken_Flight_Hat
							(Print 0 13) ; "On second thought, it looks pretty silly. Let's not."
						else
							(DontHave) ; "You don't have it."
						)
					)
					((Said 'look/anemometer')
						(if (gEgo has: 9) ; Metal_Pole
							(Print 0 14) ; "It's on the end of your pole."
						else
							(DontHave) ; "You don't have it."
						)
					)
					((Said 'look/belt')
						(cond
							...
							((!= global120 2)
								(Print 0 15) ; "Look out, it's low on power."
							)
							(else
								(Print 0 16) ; "The terminator's invisibility belt is now completely out of power."
							)
						)
					)
					((and (Said 'look>') (= temp0 (gInventory firstTrue: #saidMe)))
						(if (temp0 ownedBy: gEgo)
							...
							(Print 0 17) ; "You don't have one of those."
						)
					)
					((Said 'eat,drop,use')
						(Print 0 18) ; "Forget it. Think of something else."
					)
					((Said 'get')
						(cond
							(
								...
								(Print 0 19) ; "That doesn't seem to be available."
							)
							((temp0 ownedBy: gEgo)
								(Print 0 20) ; "You already have it."
							)
							(else
								(event claimed: 0)
							)
					...
					((Said 'smell')
						(Print 0 21) ; "You smell nothing of note."
					)
					((Said 'throw')
						(Print 0 22) ; "Keep it to yourself."
					)
					((Said 'press')
						(Print 0 23) ; "You can't"
					)
					((Said 'jump')
						(Print 0 24) ; "Not now."
					)
					((Said 'jog')
						(Print 0 25) ; "Sorry. We don't do running."
					)
					((Said 'fuck')
						(if (> (++ global185) 25)
							(Print 0 26) ; "Hey, trashmouth! Don't be saying that shit!"
						else
							(Print 0 27) ; "Hey! What kind of talk is that?"
						)
					)
				...
				(if global200
					...
					(cond
						...
						((& temp7 emSHIFT)
							...
							(= temp1
								(Print
									(Format @temp9 0 28 temp5 temp6) ; "%d/%d"
									#at
									(cond
										((< temp5 20) temp5)
										((< 300 temp5)
			...
			(evKEYBOARD
				...
				(switch (event message:)
					...
					(KEY_ALT_r
						(Print (Format @temp9 0 29 gCurRoomNum)) ; "curRoomNum = %d"
					)
					(KEY_ALT_v
						(Show 1)
					)
	...
	(method (wordFail param1 &tmp [temp0 100])
		(Print (Format @temp0 0 30 param1)) ; "Try another word. Apparently, "%s" isn't in the Andromedan dictionary."
	)

	(method (syntaxFail)
		(Print 0 31) ; "Do me a favor and try rephrasing that thought."
	)

	(method (pragmaFail &tmp [temp0 100])
		(Print 0 32) ; "Pardon me. This program is too stupid to glean your desire from such a wonderfully crafted sentence. Please try something else."
	)
)
...
(instance Buckazoids of InvI
	...
	(method (showSelf)
		(Print (Format @global402 0 33 global154) #icon view loop cel) ; "You possess %d of these nifty monetary units known as `Buckazoids'."
	)
)
