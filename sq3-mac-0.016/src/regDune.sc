			...
			(switch (event type:)
				...
				($0040 ; direction
					(if (and (gCurRoom controls:) (IsObject gEgo))
						...
						(if
							(and
								(or (== temp0 evKEYBOARD) (== temp0 $0040)) ; direction
								(== global106 temp5)
								(IsObject (gEgo mover:))
							)
							(= temp5 JOY_NULL)
					...
					(cond
						((Said 'look>')
							(cond
								...
								((Said '/dirt,dirt')
									(Print 501 1) ; "The ground beneath your feet is composed of an unusually colored sand. Other than its bizzare coloration, you have no other interest in it. However... as you expectorate a mouthful of the wind blown sand you briefly reminisce about your adventure on Kerona (SQ I )."
								)
								((Said '/dune')
								...
								((or (Said '/dirt') (Said '<down'))
									(Print 501 3) ; "The ground beneath your feet is composed of an unusually colored sand. Other than its bizarre coloration, you have no other interest in it. However... as you expectorate a mouthful of the wind blown sand you briefly reminisce about your adventure on Kerona (SQ I )."
								)
								((Said '/lightning')
						...
						((Said 'hide')
							(Print 501 18) ; "You can run, but you can't hide."
						)
						((Said 'dig')
							(Print 501 19) ; "You might chap your delicate hands since you don't have a shovel."
						)
						((or (Said 'kill/bug') (Said 'stair/bug'))
							(if (gCast contains: local6)
								(Print 501 20) ; "You need to get close enough."
							)
						)
						((Said 'climb,sit,crawl,lie')
							(Print 501 21) ; "That would serve no purpose."
						)
						((Said 'talk/android')
							(if (gCast contains: global111)
								(Print 501 22) ; "I don't think he's interested in talking."
							else
								(Print 501 23) ; "You don't see anyone here to talk to."
							)
						)
					)
				)
