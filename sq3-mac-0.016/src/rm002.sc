		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'look>')
						(cond
							((Said '<in,through/craft,leech,pane[<escape]')
					...
					((Said 'fix,open,pry/craft,leech,door[<escape]')
					...
					((Said 'hoist,manipulate,press,pull/craft,leech[<escape]')
					...
					(
						(or
							(Said 'climb,get<in,in/craft,leech[<escape]')
							(Said 'enter,board[/craft,leech[<escape]]')
						)
...
(instance doorScript of Script
	...
	(method (doit)
		(if (and (== (self state:) 19) (== (doorSound state:) 0))
			(self cue:)
		)
		(super doit:)
	)
...
(instance motivator of Act
	...
	(method (handleEvent event)
		...
		(cond
				...
				(cond
					((Said '/motivator,artifact,device')
						(if global186
							...
							(Print 2 21) ; "A closer look at the object on the floor reveals that it is a warp motivator. It looks a bit more high-tech than the other junk strewn about this locale. Sturdily constructed, its only protrusion is a modular plug near its base."
						)
					)
					(
						(or
					...
					(
						...
						(Print 2 23) ; "The floor is composed of a mosaic of overlapping welded steel plates. The melange of used metal plates appear to have been cut from a variety of sources. There is a small round object lying nearby."
					)
				)
			)
			...
			(
				(Said
					'turn,get,manipulate,press,roll,pull/device,motivator,artifact'
				)
