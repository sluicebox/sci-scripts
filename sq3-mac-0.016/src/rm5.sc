		...
		(switch (event type:)
			(evSAID
				(cond
					((Said 'look>')
						(cond
							...
							(
								(or
									(Said
										'[<at]/bone,android,menace[<metallic]'
									)
									(Said
										'[<at]/appendage,head[<bone,android,menace[<metallic]]'
									)
								)
								(Print 5 2) ; "It looks to be another metallic menace whose time has come and gone. Hopefully, fate will be kinder to you."
							)
							((Said '/skull,finger,head,appendage[<android,man]')
					...
					(
						(Said
							'get,explore,climb[<on,onto,up]/bone,android,menace[<metallic]'
						)
			...
			((Said 'look>')
				(cond
					...
					((Said '[<down,below,at][/dirt,deck]')
						(Print 5 13) ; "The floor is composed of a mosaic of overlapping welded steel plates. The melange of used metal plates appear to have been cut from a variety of sources. There is a small round object lying nearby."
					)
				)
			)
			(
