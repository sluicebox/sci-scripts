...
(procedure (localproc_1 param1 param2)
	(if param1
		(Display 17 7 dsFONT 600 dsWIDTH 280 dsCOLOR 15 dsCOORD 20 param2 dsBACKGROUND 0)
	)
)
...
(procedure (localproc_2)
	...
	(cruiseBut
		cel:
			(cond
				((== global209 2) 1)
				((or (!= global208 2) (not global217) (== global206 0)) 2)
				(else 0)
			)
		draw:
	)
...
(instance rm017 of Rm
	...
	(method (init &tmp [temp0 50])
		...
		(localproc_2)
		(if (> global598 4)
			(Display 17 0 dsCOORD 133 15 dsFONT 612 dsCOLOR 10 dsBACKGROUND 0 dsALIGN alLEFT) ; "FLIGHT MODE"
			(Display 17 1 dsCOORD 244 15 dsFONT 612 dsCOLOR 10 dsBACKGROUND 0 dsALIGN alLEFT) ; "DEFENSE"
		)
		...
		(= local0
			(localproc_0
				(if global217
					(Format @local8 {DESTINATION: SECTOR %d} global217)
				else
					{NO COURSE SELECTED}
				)
				154
		...
		(Display 17 2 dsCOORD 20 144 dsFONT 600 dsWIDTH 250 dsCOLOR 10) ; "CURRENT SECTOR:"
		(Display 17 3 dsCOORD 97 67 dsFONT 600 dsWIDTH 180 dsCOLOR 15) ; "CLICK HERE FOR COCKPIT VIEW"
	...
	(method (doit)
		...
		(Display
			(Format @local3 17 4 global214) ; "%d"
			dsCOORD
			100
			144
			dsWIDTH
...
(instance LightWarning of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= global181 1)
				(localproc_0 {** LIGHT SPEED NON-FUNCTIONAL **} 174 14)
				(beeper play: self)
			)
			(1
				(Print 17 5) ; "Those jokers back on Pestulon must have tampered with the lightspeed thingamajig!"
				(localproc_1 {** LIGHT SPEED NON-FUNCTIONAL **} 174)
				(client setScript: 0)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(= local83 {WEAPONS SYSTEM INOPERABLE WHILE NOT IN FLIGHT})
				(= local1 114)
			...
			(7
				...
				(= local83 {NAVIGATION SYSTEM INOPERABLE WHILE NOT IN FLIGHT})
				(= local1 124)
			...
			(8
				...
				(= local83 {** ERROR DETECTED: CONSULT DIAGNOSTIC COMPUTER **})
				(= local1 164)
				(localproc_0 local83 164 12)
				(self changeState: 13)
			)
	...
	(method (doit)
		...
		(switch (self cel:)
			...
			(1
				(if global206
					(badBlip play:)
					(Print 17 8) ; "LEAVE RADAR SYSTEM ON WHILE IN FLIGHT"
				else
					(goodBlip play:)
					(self cel: 0 draw:)
