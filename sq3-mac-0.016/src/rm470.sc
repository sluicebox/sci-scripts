...
(procedure (localproc_3 param1)
	(switch
		(Print
			(Format @global402 470 46 param1) ; "I'll take it off your hands for %d buckazoids. Whaddya say?"
			#icon
			81
			0
			2
				...
				(cond
					...
					(
						...
						(if (gEgo has: 0) ; Glowing_Gem
							...
							(Print 470 32) ; "You don't have one."
						)
					)
				)
			)
...
(instance Howdy of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0)
				(= local0 (Print 470 33 #at -1 1 #width 280 #font 600 #dispose)) ; "Howdy, stranger! The name's Blatz, Fester Blatz. Welcome to World o' Wonders. Go ahead, have a look at some of the trendiest items in the known universe. Make the most of your vacation buckazoid!"
				(= seconds 15)
			)
			(2
				(localproc_2)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(RedrawCast)
				(localproc_0)
				(= local0 (Print 470 34 #at -1 1 #width 280 #font 600 #dispose)) ; "Pestulon, Pestulon. Isn't that a deodorant? No, no, that's PestBeGone. Hmmmm, I think Pestulon's just south of the Lungfish Nebula. Yeah, I'm sure that's where it is."
				(= local2 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(RedrawCast)
				(localproc_0)
				(= local0 (Print 470 35 #at -1 1 #width 280 #font 600 #dispose)) ; "They make Astro Chicken, don't they? Kind of a hush hush operation from what I hear. I've heard rumors that they're involved in the Two Guys From Andromeda kidnapping."
				(= local2 0)
				(EarWax changeState: 2)
				(= seconds 15)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(RedrawCast)
				(localproc_0)
				(= local0 (Print 470 36 #at -1 1 #width 280 #font 600 #dispose)) ; "Thanks for stop'n by! Hey, looks like that lightning is gettin' close, better be careful out there!"
				(= seconds 10)
			)
			(1
				(localproc_2)
		...
		(switch (= state newState)
			...
			(1
				(if ((gInventory at: 4) ownedBy: 470) ; Orat_on_a_Stick
					...
					(= local0 (Print 470 37 #at -1 1 #width 280 #font 600 #dispose)) ; "How about a nice Orat on a Stick! The kids'll just love this! We're talkin' hours of fun for the whole family! Just look how cute this little guy is!"
					(= seconds 15)
				else
					(= cycles 2)
				)
			...
			(3
				(if ((gInventory at: 6) ownedBy: 470) ; Astro_Chicken_Flight_Hat
					...
					(= local0 (Print 470 38 #at -1 1 #width 280 #font 600 #dispose)) ; "Now here's a red-hot item, the Official Astro Chicken Flight Hat! You'll really turn some heads in this sporty little number! It's modeled after the hot new arcade game that's sweeping the galaxy."
					(= seconds 15)
				else
					(= cycles 2)
				)
			...
			(5
				(if ((gInventory at: 5) ownedBy: 470) ; ThermoWeave_Underwear
					...
					(= local0 (Print 470 39 #at -1 1 #width 280 #font 600 #dispose)) ; "Goin' to any high-temp planets? This nice pair of ThermoWeave underwear'll keep your internal environment pleasant on even the sweatiest worlds."
					(= seconds 15)
				else
					(= cycles 2)
				)
			...
			(7
				(localproc_0)
				(= local0 (Print 470 40 #at -1 1 #width 280 #font 600 #dispose)) ; "You just take your time, ya hear? Don't feel pressured to buy anything."
				(= seconds 10)
			)
			(8
				(localproc_2)
			...
			(9
				(if (== local1 1)
					(localproc_0)
					(= local0 (Print 470 41 #at -1 1 #width 280 #font 600 #dispose)) ; "I seen ya eyein' the display case. You'll notice a very nice collection of orium crystals. I think you'll agree that they're some of the highest quality gems on this hemisphere. And priced to sell, too!"
					(= seconds 15)
				else
					(= cycles 2)
				)
	...
	(method (changeState newState &tmp [temp0 50])
		(switch (= state newState)
			...
			(2
				...
				(localproc_2)
				(localproc_0)
				(= local0 (Print 470 42 #at -1 1 #width 280 #font 600 #dispose)) ; "My, my, that certainly is one fine hunk o' orium!"
				(= seconds 8)
			)
			(3
				(= global103 1)
			...
			(6
				(localproc_0)
				(= local0 (Print 470 43 #at -1 1 #width 280 #font 600 #dispose)) ; "Well, 'fraid I can't go any higher. Maybe some other time, eh?"
				(= seconds 8)
			)
			(7
				(localproc_2)
	...
	(method (changeState newState &tmp [temp0 50])
		(switch (= state newState)
			...
			(1
				...
				(localproc_0)
				(= local0 (Print 470 44 #at -1 1 #width 280 #font 600 #dispose)) ; "Yessiree, that's one fine hunk o' orium! Course, the orium market's a little slow today..."
				(= seconds 8)
			)
			(2
				(localproc_2)
			...
			(3
				(localproc_0)
				(= local0 (Print 470 45 #at -1 1 #width 280 #font 600 #dispose)) ; "Well, OK. Ya drive a hard bargain, stranger. Maybe some other time, eh?"
				(= seconds 8)
			)
			(4
				(localproc_2)
