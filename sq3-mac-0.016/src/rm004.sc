				...
				(cond
					((Said 'look>')
						(cond
							(
								(Said
									'/bucket,conveyer,device,elevator,hoist[/belt]'
								)
							...
							(
								(Said
									'[<in,in,through,at]/craft,pane,port,jup,2,leech'
								)
					...
					(
						(Said
							'stop,(turn<off),(close<down)/conveyer,elevator,hoist'
						)
						(Print 4 9) ; "That's not within your power."
					)
					(
						(Said
							'break,open/pane,glass,dome,port,(cavity<port),finger,appendage,appendage'
						)
					...
					((Said 'climb[<on,up]/craft,leech,jup,2')
						(Print 4 12) ; "You are not skilled in ship climbing."
					)
					((Said 'hoist,manipulate,press,pull/craft,leech,jup,2')
					...
					(
						(Said
							'get,manipulate,pull,press,hoist[<up]/box,block,toaster,pole'
						)
					...
					(
						(Said
							'get,manipulate,pull,press,hoist,get,climb/appendage,appendage,finger[<android,big]'
						)
						(Print 4 16) ; "No."
					)
					((Said 'get>')
			...
			((Said 'look>')
				(cond
					...
					((Said '[<down,below,at][/dirt,deck]')
						(Print 4 21) ; "The floor is composed of a mosaic of overlapping welded steel plates. The melange of used metal plates appear to have been cut from a variety of sources. There is a small round object lying nearby."
					)
				)
			)
			((Said 'pull,press,remove,get/plug[<modular,8,eightprong]')
