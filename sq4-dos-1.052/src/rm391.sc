		...
		(switch (= state newState)
			...
			(4
				...
				(switch
					...
					(0
						(gLongSong number: (Random 392 395) play:)
						(Print 391 2 100 60 144) ; "You do not possess the necessary number of Buckazoids."
						(= cycles 1)
					)
					(1
						(gLongSong number: (Random 392 395) play:)
...
(instance eyes of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Do
				(Print 391 3) ; "Be reasonable. What would you do with robot eyes, anyway?"
			)
			(11 ; Smell
...
(instance lEar of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(10 ; Taste
				(Print 391 4) ; "I know you're lonely, but licking a robot's ear?"
			)
			(11 ; Smell
				(Print {The robot has your typical mechanical aromas.})
			)
			...
			(2 ; Do
				(Print 391 5) ; "Don't touch the robot's ear. You don't know where it's been."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance mouth of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(10 ; Taste
				(Print 391 6) ; "Everyone knows that licking a salesbot on the mouth is like plugging your tongue into a wall socket."
			)
			(11 ; Smell
				(Print 391 7) ; "Ozone oozes from his frantically flashing lips."
			)
			(2 ; Do
				(Print 391 8) ; "You should stay out of the mouths of others."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance roboHead of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(11 ; Smell
				(Print 391 9) ; "Smells like he put too much "Lube-Ina-Tube" on this morning."
			)
			(2 ; Do
				(Print 391 10) ; "Hey! Leave the head alone!"
			)
			(4 ; Talk
				(Print 391 11) ; "This model is not programmed to talk with the customers. You must interact with him through his touch-screen chest monitor."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance chest of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(10 ; Taste
				(Print 391 12) ; "The friendly salesbot smiles at you and thanks you for licking his chest monitor clean."
			)
			(11 ; Smell
				(Print 391 13) ; "It smells dusty - like old electronics."
			)
			(else
