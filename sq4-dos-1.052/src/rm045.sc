...
(instance deathScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 45 0) ; "The young shuttle pilot, his seat suddenly humidified by your surprise entry, fires an errant pulseray shot."
			...
			(9
				(gLongSong2 stop:)
				(Print 45 1) ; "The shot just misses you (lucky stiff) and then caroms off the reflective surfaces of the cabin interior. During the course of this event it does eventually manage to take you both out (perforated stiff)."
				(EgoDead 8 28)
			)
		)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Do
				(if (and (IsFlag 0) (not (gCurRoom script:)))
					(if (not script)
						...
						(Print 45 2) ; "You're starting to show your age."
					)
				else
			...
			(3 ; Inventory
				(switch invItem
					(1 ; rope
						(Print 45 3) ; "You examine your puny piece of rope and realize that it wouldn't be strong enough to hold this puppy to the ground."
					)
				)
			)
			(else
...
(instance gear2 of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Do
				(Print 45 4) ; "You can't budge this."
			)
			(3 ; Inventory
				(switch invItem
					(1 ; rope
...
(instance shipFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Do
				(Print 45 5) ; "Your pockets aren't big enough."
			)
			(3 ; Inventory
				(switch invItem
					(1 ; rope
			...
			(11 ; Smell
				(Print 45 6) ; "The dust kicked up by the retros, the heavy stench of burning fuel, and the sharp smell of electricity make for a unique olfactory extravaganza."
			)
			(10 ; Taste
				(Print 45 7) ; "You sear your tongue on the hot metal. Apparently it's only safe to touch the now-exposed area beneath the landing gear housing flap."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance compartmentFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Inventory
				(Print 45 8) ; "You don't want to put that in there. You might need it later."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cockpitFeature of Feature
	...
	(method (doVerb theVerb)
		(if (or (== theVerb 2) (== theVerb 3)) ; Do, Inventory
			(HandsOff)
...
(instance theStreet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(11 ; Smell
				(Print 45 9) ; "It doesn't exactly smell April-fresh."
			)
			(10 ; Taste
				(Print 45 10) ; "It's not as big a taste sensation as other parts of the city."
			)
			(3 ; Inventory
				((ScriptID 705 4) doVerb: theVerb invItem) ; theRoom
			)
