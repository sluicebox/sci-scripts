...
(instance rm397 of SQRoom
	(properties
		lookStr {The software selection seems to be pretty slim, except for the bargain bin near the entrance.}
		picture 397
		style 30
		south 395
	)
				...
				(if (!= gPrevRoomNum 398)
					...
					(self
						save1:
							(proc0_12
								(switch (Random 0 2)
									(0 {"So, are ya looking for a good data base? We're got one coming in next week."})
									(1 {"Hey, have you seen the newest time travel simulator? Neither have I. I'm sure we're getting some in soon."})
									(3 {"If you're here to return something, forget it. We have a strict 'No Returns' policy. I'd like to help you out, guy. But you know how corporations are."})
								)
								67
								2
								5
...
(instance clerk of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Inventory
				(switch invItem
					(0 ; buckazoid
						(cond
							((not global160)
								(Print 397 0) ; "You should pick something out before you give money to the clerk."
							)
							((IsFlag 31)
								(Print 397 1) ; "You have already paid the clerk."
							)
							((< gBuckazoidCount 5)
								(gCurRoom setScript: buyBox)
							)
			...
			(1 ; Look
				(Print 397 2) ; "It looks like the clerk is keeping his eye on you."
			)
			(11 ; Smell
				(Print 397 3) ; "Ah! The distinctive aroma of Centauri Moon No. 5."
			)
			(else
