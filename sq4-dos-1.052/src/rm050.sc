	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(EgoDead 0 8) ; "Oh, well. Just think proudly of your accomplishmen... On second thought, just think - it happens to even IMPORTANT people!"
			)
		)
	)
)
...
(instance theGrate of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(11 ; Smell
				(Print 50 2) ; "The cool, rank smell of a sewer wafts up from underground."
			)
			(10 ; Taste
				(Print 50 3) ; "Okay, you taste grate and find it less filling."
			)
			(else
...
(instance otherGrate of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Do
				(Print 50 4) ; "The bolted down grate won't budge."
			)
			(11 ; Smell
				(Print 50 2) ; "The cool, rank smell of a sewer wafts up from underground."
			)
...
(instance theRedBldg of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Do
				(Print 50 5) ; "The building is closed up with welded metal panels enhancing non-access."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theHole of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 50 6) ; "As is so often the case with voids there is nothing to smell."
			)
			(10 ; Taste
				(Print 50 7) ; "As is so often the case with voids there is nothing to taste."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theLiquid of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Do
				(Print 50 8) ; "It would serve no purpose."
			)
			(11 ; Smell
				(Print 50 9) ; "It's probably the most nauseating thing to smell in the whole non-liquid liquid family of smells."
			)
			(10 ; Taste
				(Print 50 10) ; "It's definitely the most nauseating thing to taste in the whole non-liquid liquid family of tastes."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance street1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(1 ; Look
				(Print 50 11) ; "I'll bet you're getting bored with these street descriptions so, in an effort to decrease the message tedium to some degree, I won't go into some long description about the grates or the sizeable void to the east. Happy adventuring!"
			)
			(11 ; Smell
				(Print 50 12) ; "After being away for so long, you'd forgotten how rancid the streets of Xenon smell."
			)
			(10 ; Taste
				(Print 50 13) ; "The artificial taste of the Xenon road makes you long for the all-natural flavor of unpaved country lanes."
			)
			(3 ; Inventory
...
(instance sidewalk1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(1 ; Look
				(Print 50 14) ; "You are at the eastern boundary of a clearing in this ruined city. Passage to the east is stopped by a great void in the landscape."
			)
			(11 ; Smell
				(Print 50 15) ; "You smell the sidewalk, and it smells back."
			)
			(10 ; Taste
				(Print 50 16) ; "You give the sidewalk a lick and accidentally step on a crack. Somewhere, your mother screams out in agony, her spinal cord snapped cleanly in half. Of course, she probably deserved it for the way she spazzed-out and threw away those magazines she found in your room that time back in junior high. Those things cost you a lot of good lunch money!"
			)
			(3 ; Inventory
				((ScriptID 705 4) doVerb: theVerb invItem) ; theRoom
			)
