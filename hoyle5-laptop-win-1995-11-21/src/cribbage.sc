			...
			(if global486
				...
				(switch global486
					(11
						...
						(++ local595)
						(= [local606 (+ [local606 0] 1)] 11)
						(= [local606 (+ [local606 0] 2)] 2)
						(+= [local606 0] 2)
					)
					(1
						(= [local596 local595] 27)
						(++ local595)
						(= [local606 (+ [local606 0] 1)] 1)
						(= [local606 (+ [local606 0] 2)] 2)
						(+= [local606 0] 2)
					)
				)
			...
			(if global487
				...
				(switch global487
					...
					(3
						...
						(++ local595)
						(= temp4 3)
						(= [local606 (+ [local606 0] 1)] 3)
						(= [local606 (+ [local606 0] 2)] 6)
						(+= [local606 0] 2)
					)
					(4
						...
						(++ local595)
						(= temp4 4)
						(= [local606 (+ [local606 0] 1)] 4)
						(= [local606 (+ [local606 0] 2)] 1)
						(+= [local606 0] 2)
					)
				)
			)
			(if global488
				...
				(switch global488
					(5
						...
						(++ local595)
						(= [local606 (+ [local606 0] 1)] 5)
						(= [local606 (+ [local606 0] 2)] 3)
						(+= [local606 0] 2)
					)
					(6
						(= [local596 local595] 170)
						(++ local595)
						(= [local606 (+ [local606 0] 1)] 6)
						(= [local606 (+ [local606 0] 2)] 4)
						(+= [local606 0] 2)
					)
					(7
						(= [local596 local595] 170)
						(++ local595)
						(= [local606 (+ [local606 0] 1)] 7)
						(= [local606 (+ [local606 0] 2)] 5)
						(+= [local606 0] 2)
					)
					(13
						(= [local596 local595] 170)
					...
					(14
						...
						(++ local595)
						(= temp4 14)
						(= [local606 (+ [local606 0] 1)] 9)
						(= [local606 (+ [local606 0] 2)] 7)
						(+= [local606 0] 2)
					)
				)
			)
...
(instance pegScorePanel of Code
	...
	(method (init &tmp temp0 temp1)
		...
		(= temp1 [local606 0])
		(if (== [local606 (= temp0 1)] 4)
			(proc0_10 505 [local606 temp0] 0 539 199 14 0 {score})
			(proc0_10 505 15 1 546 199 14 0 {score})
			(proc0_10 505 15 2 551 199 14 0 {score})
		else
			(proc0_10 505 [local606 temp0] 0 539 199 14 0 {score})
			(proc0_10 505 15 [local606 (+ temp0 1)] 546 199 14 0 {score})
		)
		(if (> temp1 2)
			(if (== [local606 (= temp0 3)] 4)
				(proc0_10 505 [local606 temp0] 0 539 214 14 0 {score})
				(proc0_10 505 15 1 546 214 14 0 {score})
				(proc0_10 505 15 2 551 214 14 0 {score})
			else
				(proc0_10 505 [local606 temp0] 0 539 214 14 0 {score})
				(proc0_10 505 15 [local606 (+ temp0 1)] 546 214 14 0 {score})
			)
			(if (> temp1 4)
				(if (== [local606 (= temp0 5)] 4)
					(proc0_10 505 [local606 temp0] 0 539 232 14 0 {score})
...
(instance roomScript of Script
	...
	(method (changeState newState &tmp [temp0 2] temp2 [temp3 3] temp6 temp7 temp8 [temp9 100])
		(switch (= state newState)
			(0
				(DisableCursor)
				(= ticks 1)
			...
			(12
				...
				(if
					...
					(if (!= global491 local14)
						...
						(if gAttitudeSetting
							(gChar1
								sayReg: (+ 1000 (gChar1 view:)) 2 29 0 1
							)
						else
							(gChar1 say: 1000 2 29 0 2)
						)
						(= local16 1)
					...
					(if (!= (Dealer curPlayer:) hand1)
						(if (== ((Dealer curPlayer:) type:) 1)
							...
							(for
								((= temp8 (Event new:)))
								(and
									(!= (temp8 type:) evMOUSEBUTTON)
									(or
										(!= (temp8 type:) evKEYBOARD)
										(and
											(== (temp8 type:) evKEYBOARD)
											(<= 58 (/ (temp8 message:) 256) 70)
										)
									)
									(< (GetTime) local605)
								)
