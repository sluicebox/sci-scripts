...
(instance sysLogger of Code
	...
	(method (doit &tmp i j l c firstNote theDrv commented saveInfont [str 40] [cfgPath 30] [thePath 30] [theToken 30] [QAinitials 5] [kbdDrvEntry 40] [joyDrvEntry 40] [videoDrvEntry 40] [soundDrvEntry 40] [mouseDrvEntry 40] [audioDrvEntry 40])
		...
		(if (not (CheckFreeSpace @gSysLogPath))
			(Print
				font: 999
		...
		(if firstNote
			(Print
				font: 999
				addText: {Enter your initials\n(max 3 characters):}
				addEdit: @QAinitials 5 0 20
				init:
			)
		...
		(if (== -1 logHandle)
			(Print
				font: 999
				addTextF: @str {Error: Couldn't open %s} @thePath
				init:
			)
		else
		...
		(if
			...
			(Print
				font: 999
				addTextF: @str {Error: Couldn't open memory file %s!} @thePath
				init:
			)
		else
