...
(local
	...
	local1
	local2
	[local3 30] = [319 0 319 137 134 127 143 112 196 107 220 82 246 90 303 72 295 10 216 9 206 14 189 10 110 24 85 18 63 0]
...
(instance rm040 of Rm
	...
	(method (init)
		...
		(= global320 221)
		(= global321 91)
		(= global325 {"Let's get a move-on, shall we, Graham!"})
...
(instance climb of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Say 75 40 0 67 142 8 70 160 25 4) ; "Graham! Look out!"
				(SetScore 2)
				(gCurRoom newRoom: 41)
			)
		)
...
(instance fallWater of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say 75 40 1 67 142 8 70 160 25 4) ; "Graham! Watch out!"
				(gEgo
		...
		(if
			...
			(switch (event message:)
				...
				(4 ; Talk
					(if (not local2)
						(++ local2)
						(Say 75 40 7 67 142 8 70 160 25 8) ; "Will we EVER find our way out of these mountains, Graham?"
						(Say 165 40 8 67 8 8 25 8) ; "I don't know, Cedric. I'm beginning to wonder!"
					else
						(switch (Random 0 3)
							(0
