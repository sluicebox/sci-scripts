...
(local
	[local0 52] = [0 165 24 154 72 155 65 163 64 174 81 172 84 157 97 149 116 146 95 156 95 172 238 176 268 172 280 159 276 154 269 150 281 152 288 161 263 180 306 180 306 186 0 186 0 189 319 189 319 0 0 0]
	local52
	local53
	local54
)
		...
		(switch (= state newState)
			...
			(1
				...
				(gEgo
					normal: 0
					view: 745
					put: 35 gCurRoomNum ; Mordack_s_Wand
					setLoop: 0
					cel: 0
					cycleSpeed: 1
					setCycle: End self
...
(instance machineScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(proc0_24 132 865 866 867 869 870 871 872 873 874 875 876 877)
			...
			(3
				(SetScore 5)
				(PrintDC 66 0) ; "Graham tosses the moldy cheese into the machine's bubbling liquid."
				(if (IsObject (gEgo head:))
					((gEgo head:) show:)
				)
				(gEgo
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (IsFlag 61)
						(PrintDC 66 5) ; "Mordack's wand looks dim and powerless. Graham doesn't want it."
						(event claimed: 1)
					else
						(HandsOff)
