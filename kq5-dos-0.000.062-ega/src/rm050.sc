...
(local
	...
	[local28 12] = [319 80 283 86 279 114 212 141 269 142 319 148]
)
...
(instance rm050 of Rm
	...
	(method (init)
		...
		(gEgo view: (if (IsFlag 75) 656 else 0) init: setStep: 3 2)
		(if (IsFlag 56)
			(cedric init:)
...
(instance getCedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say 75 50 0 67 -1 10 25 5) ; "Graham! (cough, cough) Help... me."
				(= cycles 1)
			)
			(1
				(gEgo
			...
			(4
				(Say 160 50 1 25 4 67 -1 10) ; "Cedric, where do you hurt?"
				(= cycles 1)
			)
			(5
				(Say 75 50 2 25 3 67 -1 10) ; "Everywhere... (cough, cough)."
				(= cycles 1)
				(gGlobalSound fade:)
			)
			(6
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(4 ; Talk
					(Say 75 50 6 67 30 100) ; "Mooaaannn!"
					(event claimed: 1)
				)
				(5 ; Inventory
					(if (!= global69 28) ; Wand
