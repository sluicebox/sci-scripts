		...
		(for ((= temp0 0)) (< temp0 temp4) ((++ temp0))
			(for ((= temp1 0)) (< temp1 temp5) ((++ temp1))
				...
				(if
					...
					(PrintDebug {row,col:%d,%d} temp2 temp3)
					(PrintDebug {board:%d} (Board get: temp2 temp3))
				)
			)
		)
	)
				...
				(if (& (temp0 type:) evMOUSERELEASE)
					...
					(if (== temp7 1)
						...
						(if
							...
							(PrintDebug {pre:(%d,%d)} x y)
							(+= x 3)
							(= x (* (/ x 9) 9))
							(= y (/ (* (/ (* y 12) 90) 90) 12))
							(+= x 1)
							(+= y 4)
							(PrintDebug {post:(%d,%d)} x y)
							(self setPri: -1)
							(UpdateScreenItem self)
							(FrameOut)
							(if (Board canPlace: self)
								...
								(PrintDebug {snapback:%d,%d} temp1 temp2)
								(= x temp1)
								(= y temp2)
								(if onBoard
									(Board place: self)
						...
						(PrintDebug {back to:%d,%d} temp1 temp2)
						(= x temp1)
						(= y temp2)
						(if onBoard
							(Board place: self)
		...
		(if (or (== gPrevRoomNum 990) (== gPrevRoomNum 992)) ; shiversOptions, viewingInv
			...
			(for ((= temp0 1)) (<= temp0 28) ((++ temp0))
				...
				(if (Board canPlace: temp1)
					(Board place: temp1)
					(PrintDebug {placing %d} (temp1 pieceNum:))
				)
			)
			(temp8 close:)
			(temp8 delete:)
