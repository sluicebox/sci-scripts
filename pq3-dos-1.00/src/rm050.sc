...
(instance coronerConversation of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(= save1 1)
				(Print 50 4 #at 60 160) ; "With a sinking feeling, you recognize the old news article covering your promotion."
				(= cycles 1)
			)
			(9
				(= save1 1)
				(Print 50 5 #at 60 160) ; "A red marking pen pentagram mars your photograph."
				(= cycles 1)
			)
			(10
				(= save1 1)
			...
			(2 ; Look
				(if open
					(switch corpseType
						...
						(2
							(Print 50 14) ; "You notice a toe tag."
						)
						(3
							(Print 50 15) ; "This corpse's feet look particularly unattractive. You notice a toe tag."
						)
					)
				else
					(Print 50 16) ; "The drawers store bodies. You wonder if there's anyone you know in there."
				)
			)
			(5 ; Talk
				(if open
					(Print 50 17) ; "I'm starting to worry about you Bonds."
				)
			)
			(3 ; Do
				(if (and open local4)
...
(instance coroner of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 50 18) ; "Leon Mandel is the County Coroner. It's rare to see him without something or other in his mouth. He's a little strange, but you've come to appreciate his sense of humor."
			)
			(5) ; Talk
			(3 ; Do
				(Print 50 19) ; "Leon's not about to share his sandwich."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 50 20) ; "Nah. He might enjoy it."
					)
					(7 ; camera
						(Print 50 21) ; "Hey, you want a picture? I'll send you an autographed one. Me in my polka dot swim trunks. No problem."
					)
					(33 ; skinHair
						(Print 50 22) ; "Whaddaya want from me, a medal? Your evidence boys down at the station have the time and equipment for all that lab analysis stuff. Me, I'm busy enough with just the stiffs."
					)
					(11 ; envelope
						(Print 50 23) ; "Better hang on to it!"
					)
					(2 ; handcuff
						(Print 50 20) ; "Nah. He might enjoy it."
					)
...
(instance envelope of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(2 ; Look
				(Print 50 24) ; "There's a large manilla envelope on the counter."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cabinets of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 25) ; "Knowing what a strange individual Leon is, you decide to stay out of his cabinets."
			)
			(2 ; Look
				(Print 50 26) ; "Leon certainly has a lot of storage space in here."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance chart of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 27) ; "Even if Leon GAVE you that chart, you wouldn't take it."
			)
			(2 ; Look
				(Print 50 28) ; "You'd rather not look at that chart on the wall. You see enough of that stuff at accident scenes."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bottles of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 29) ; "You don't have much call for embalming fluid, Sergeant."
			)
			(2 ; Look
				(Print 50 30) ; "The bottles contain embalming fluid."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance table of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 31) ; "There'll be time enough for you to be on the table when you're dead."
			)
			(2 ; Look
				(Print 50 32) ; "The sight of the cold, stainless steel autopsy table sends a chill through the spine as you recall the many times you've come close to ending up on one."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance instruments of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 33) ; "This isn't "Life & Death". There's a REASON you didn't go into forensics. Just looking at those instruments makes you queasy."
			)
			(2 ; Look
				(Print 50 34) ; "A number of cruel-looking stainless steel instruments lie on a towel on the counter."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance skeleton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 35) ; "You'd better leave "Demi" alone. You know how Leon is about his bones."
			)
			(2 ; Look
				(Print 50 36) ; "That's Leon's 'girlfriend', Demi Bones."
			)
			(else
				(super doVerb: theVerb)
			)
			...
			(2 ; Look
				(if local4
					(switch (local5 corpseType:)
						(0
							(Printf 50 37 (local5 toeStr:)) ; "The toe tag reads "%s"."
						)
						(1
							(Printf 50 37 (local5 toeStr:)) ; "The toe tag reads "%s"."
							(Printf 50 38) ; "Good old Leon. He has so much fun naming the John Does."
						)
						(2
							(Printf 50 39 (local5 toeStr:)) ; "The toe tag reads "%s" - the poor stiff you found in the alley."
						)
						(3
							(Printf 50 37 (local5 toeStr:)) ; "The toe tag reads "%s"."
							(mCoroner stop:)
							(roomSFX number: 504 loop: 1 flags: 1 play:)
							(Printf 50 40) ; "Underneath the name is a note: "Bonds - Caught you peeking! Stepped out for a bite. Be back soon - Leon.""
							(= local6 1)
						)
					)
				else
...
(instance sink of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 41) ; "This place does have a way of making you want to wash your hands, doesn't it?"
			)
			(2 ; Look
				(Print 50 42) ; "The Coroner's office is equipped with a sink. You hate to think about what washes down those drains!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance embalmMachine of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 43) ; "I don't think Leon would appreciate your playing with his toys."
			)
			(2 ; Look
				(Print 50 44) ; "The embalming machine is a kind of mechanical vampire. Out with the old, in with the new..."
			)
			(else
				(super doVerb: theVerb)
			)
