		...
		(cond
			...
			(global135
				(switch global135
					...
					(10
						(= temp0 (ScriptID 129 11)) ; setUpScript
				...
				(switch global127
					(2
						((ScriptID 129 3) ; suspCar
							init:
							y: 999
							setScript: (ScriptID 129 11) 0 (ScriptID 129 0) ; setUpScript, speederScript
						)
					)
					(3
						((ScriptID 129 3) ; suspCar
							...
							setScript: (ScriptID 129 11) 0 (ScriptID 129 1) ; setUpScript, lowRiderScript
						)
					)
					(4
						((ScriptID 129 3) ; suspCar
							init:
							y: 999
							setScript: (ScriptID 129 11) 0 (ScriptID 129 13) ; setUpScript, alliedScript
						)
					)
					(5
						((ScriptID 129 3) ; suspCar
							...
							setScript: (ScriptID 129 11) 0 (ScriptID 129 2) ; setUpScript, drunkScript
						)
					)
				)
			)
			...
			(0
				(if
					(and
						...
						(!= ((ScriptID 129 3) script:) (ScriptID 129 11)) ; suspCar, setUpScript
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				((ScriptID 129 3) ; suspCar
					y: local0
					setScript: (ScriptID 129 11) 0 local1 ; setUpScript
				)
...
(instance noNutScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 22) ; "Your failure to secure the derranged man resulted in a few civilian injuries when he attacked a nearby family. You were suspended.  If you can't handle an unarmed man, how can you expect to function when things REALLY get tough?"
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(Print 132 17) ; "DISPATCH: "64 S2 Lytton.""
				(Print 132 18) ; "SONNY: "Lytton, 64 S2 go ahead.""
				(Print 132 19) ; "DISPATCH: "Respond to an assault at the Oak Tree Mall, S2.""
				(Print 132 15) ; "SONNY: "10-4.""
				(= seconds (if (not gHowFast) 600 else 300))
			)
			(4
				(HandsOff)
				(Print 132 20) ; "You have failed to respond to Oak Tree Mall in a reasonable amount of time."
