...
(instance rm045 of PQRoom
		...
		(gCurRoom
			addObstacle:
				...
				((Polygon new:)
					...
					init: 241 64 224 61 203 69 165 49 179 32 143 14 136 19 95 3 51 3 49 13 47 33 35 56 0 50 0 189 113 189 174 162 184 170 145 189 282 189 210 161 253 140 319 158 319 133 276 127 241 141 196 165 183 158 239 120 177 91
					yourself:
				)
		)
	)
	...
	(method (doit &tmp temp0)
		(cond
			(script 0)
			((and (& (gEgo onControl: 1) $0020) (== (gEgo script:) 0))
				(self setScript: ribbonRight)
			)
			((and (& (gEgo onControl: 1) $2000) (== (gEgo script:) 0))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(switch (Random 0 3)
					(0
						(Say coroner 45 15) ; "Did you hear the one about the Orthodox Jewish Coroner and the hotdog salesman?"
						(= seconds 5)
					)
					(1
						(Say coroner 45 16) ; "So I says to her, Aunt Sophie, could we just this once NOT turn this into a funeral already?"
						(= seconds 6)
					)
					(2
						(Say coroner 45 17) ; "So yesterday, I'm eatin' one of my wife's bagels (no lox, 'cause she forgot ta go to the deli), and I'm usin' this napkin. I THINK it's clean, right?..."
						(= seconds 8)
					)
					(3
...
(instance coroner of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 45 19) ; "That's Leon. He's a great Coroner, but the guy has a sick sense of humor."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 45 20) ; "One murder's enough for this dark alley, don't you think?"
					)
					(19 ; wallet
						(Print 45 21) ; "You really like to flash those credentials, don't you?"
					)
					(23 ; license
						(cond
							((IsFlag 82)
							...
							(local0
								(Print 45 22) ; "Don't you want to log the information about Andrew Dent in your notebook first?"
								(= local0 0)
							)
							(else
								(gCurRoom setScript: giveLicenseBack)
					...
					(33 ; skinHair
						(Print 45 23) ; "That's nice, Bonds, but I only care for that stuff when it's still on the body. Save it for L.P.D evidence."
					)
					(else
...
(instance body of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 45 24) ; "Ah! Would that the dead COULD speak!"
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 45 25) ; "You think about shooting the murder victim, but he's ALREADY DEAD!"
					)
					(19 ; wallet
						(Print 45 26) ; "You identify yourself to the stiff, then contemplate a long vacation."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance abandoncar of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 45 27) ; "The car's been abandoned for some time. There's nothing of interest in there."
			)
			(2 ; Look
				(if (not (trunk showing:))
					(HandsOff)
...
(instance carinset of View
	(properties
		...
		y 93
		sightAngle 90
		lookStr {It's the abandoned white car. The dent contains gold paint.}
		view 486
		priority 15
		signal 16400
	)
			...
			(4 ; Inventory
				(cond
					((gEgo has: 31) ; goldPaint
						(Print 45 28) ; "You don't need another sample."
					)
					((and (== invItem 39) (gEgo has: 42)) ; scraper, sampleEnv
						(sScraper play:)
						(Print 45 29) ; "You carefully scrape some of the gold paint transfer from the side of the white car along with a little white paint into the paper envelope."
						(gEgo get: 31) ; goldPaint
						(SetScore 153 5)
					)
					((and (gEgo has: 39) (gEgo has: 42)) ; scraper, sampleEnv
						(Print 45 30) ; "You need to use the proper field equipment to take a clean paint sample."
					)
					(else
						(Print 45 31) ; "You don't have the field equipment necessary to take a clean paint sample."
					)
				)
			)
			(else
...
(instance garage of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 45 32) ; "You don't need to worry about the garage."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance yellowTape of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 45 33) ; "The uniformed officer will remove the tape when the body has been removed and the crime lab is finished with the area."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(cond
					((gEgo has: 23) ; license
						(Print 45 34) ; "You need to give the license to the coroner first."
					)
					((localproc_0)
						(Print 45 35) ; "You need to put the field equipment back into the trunk first."
					)
					(else
