...
(instance egoAfterWarrant of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(self setScript: judgeSigning self)
			)
			(12
				(ijudge init:)
				(= cycles 1)
		...
		(switch (= state newState)
			...
			(1
				...
				(if (IsFlag 177)
					...
					(Say iEgo 44 23) ; "***I have this fix-it ticket that I was hoping you could clear for me."
					(= seconds 8)
				)
			)
			(2
			...
			(4
				(= save1 1)
				(if (IsFlag 177)
					(Say ijudge 44 24) ; "Well Then... how bout I just write one up for you..."
					(= seconds 6)
				else
					(= ticks 1)
				)
			...
			(5
				(= save1 1)
				(if (IsFlag 177)
					(Say ijudge 44 25) ; "Come on Officer! You know the rules. I need some evidence before I can issue you a warrant."
					(= seconds 6)
				else
					(Say ijudge 44 26) ; "You know the procedures. Sorry, I can't help you right now."
					(= seconds 6)
				)
			)
			(6
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(37 ; locket
						(if local2
							(Print 44 28 #at 40 115) ; "You've already shown Marie's locket to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(6 ; photo
						(if (== ((gInventory at: 6) state:) 1) ; photo
							(Print 44 29 #at 40 115) ; "You've already shown the photograph to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(35 ; article
						(if (== ((gInventory at: 35) state:) 1) ; article
							(Print 44 30 #at 40 115) ; "You've already shown the news clipping to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(1 ; gun
						(Print 44 31) ; "She'd rip you apart, Bonds."
					)
					(2 ; handcuff
						(Print 44 31) ; "She'd rip you apart, Bonds."
					)
					...
					(19 ; wallet
						(Print 44 32) ; "I remember you perfectly well, Officer Bonds!"
					)
					(else
						(Print 44 33) ; "The judge isn't interested in that."
					)
				)
			)
			(else
...
(instance warrant of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (gEgo has: 8) ; warrant
					(Print 44 34) ; "The judicial order lies on the desk."
				else
					(Print 44 35) ; "The search warrant lies on the desk."
				)
			)
			(3 ; Do
				(if (gEgo has: 8) ; warrant
					(SetScore 189 5)
					(Print 44 36) ; "You pick up the judicial order."
					(gEgo put: 8 0) ; warrant
					(gEgo get: 9) ; judicialOrder
				else
					(SetScore 187 5)
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 44 37) ; "The judge would not appreciate you handling her things."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance windows of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 44 38) ; "If the judge wants her windows open, she'll open them herself."
			)
			(else
				(super doVerb: theVerb)
			)
