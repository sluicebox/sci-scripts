...
(local
	...
	local9
	local10
	[local11 7] = [272 272 272 272 272 272 272]
	[local18 7] = [39 63 87 111 135 159 183]
	[local25 7] = [34 58 82 106 130 154 178]
	[local32 14] = [96 137 124 93 123 108 123 122 123 88 123 65 122 106]
)
...
(procedure (localproc_0 param1 param2)
	...
	(beard stopUpd:)
	(= local10 param1)
	(redBox x: [local11 local10] y: [local18 local10] forceUpd:)
	(if (== param2 cFace)
		(rFace startUpd:)
		(lFace startUpd:)
	else
	...
	(hControl y: (param2 y:))
	(wControl x: (param2 x:))
	(if (!= [local25 local10] 0)
		(rArrow y: [local25 local10] forceUpd:)
		(lArrow y: [local25 local10] forceUpd:)
	)
)
...
(procedure (localproc_2 &tmp temp0 [temp1 200])
	...
	(cond
		(
			(and
				(<= (Abs (- [local32 (* 2 local10)] (temp0 x:))) 2)
				(<= (Abs (- [local32 (+ (* 2 local10) 1)] (temp0 y:))) 2)
			)
			(Format @temp1 201 0 (temp0 description:)) ; "The %s looks ok."
			(= local0 (<< $0001 (mod local10 16)))
			(|= local1 local0)
		)
		((> (- [local32 (+ (* 2 local10) 1)] (temp0 y:)) 6)
			(Format @temp1 201 1 (temp0 description:)) ; "The %s is way too high."
		)
		((> (- (temp0 y:) [local32 (+ (* 2 local10) 1)]) 6)
			(Format @temp1 201 2 (temp0 description:)) ; "The %s is way too low"
		)
		((> (- [local32 (* 2 local10)] (temp0 x:)) 6)
			(Format @temp1 201 3 (temp0 description:)) ; "The %s is way too left"
		)
		((> (- (temp0 y:) [local32 (+ (* 2 local10) 1)]) 6)
			(Format @temp1 201 4 (temp0 description:)) ; "The %s is way too right"
		)
		((> (- [local32 (+ (* 2 local10) 1)] (temp0 y:)) 2)
			(Format @temp1 201 5 (temp0 description:)) ; "The %s is a little too high."
		)
		((> (- (temp0 y:) [local32 (+ (* 2 local10) 1)]) 2)
			(Format @temp1 201 6 (temp0 description:)) ; "The %s is a little too low"
		)
		((> (- [local32 (* 2 local10)] (temp0 x:)) 2)
			(Format @temp1 201 7 (temp0 description:)) ; "The %s is a little too left"
		)
		((> (- (temp0 y:) [local32 (+ (* 2 local10) 1)]) 2)
			(Format @temp1 201 8 (temp0 description:)) ; "The %s is a little too right"
		)
	)
	(Say dummy @temp1)
)
...
(procedure (localproc_3 &tmp temp0 temp1)
	(for ((= temp0 0)) (<= temp0 6) ((++ temp0))
		...
		(if
			(and
				(<= (Abs (- [local32 (* 2 temp0)] (temp1 x:))) 2)
				(<= (Abs (- [local32 (+ 1 (* 2 temp0))] (temp1 y:))) 2)
			)
...
(procedure (localproc_4) ; UNUSED
	(switch (local7 type:)
		(0
			(if (and (> gMouseY (local7 minV:)) (< gMouseY (local7 maxV:)))
				(local7 y: (- gMouseY 10))
			)
		)
		(1
			(if (and (> gMouseX (local7 minV:)) (< gMouseX (local7 maxV:)))
				(local7 x: gMouseX)
			)
		)
	)
...
(procedure (localproc_5)
	(Print 201 28) ; "MOUSE Click mouse on menu items, feature buttons, and arrows to cycle through each feature. Press and hold to move sliders. KEYS Use TAB key to select features, arrow keys to move sliders, and alpha keys to select menu items. Use the N and B keys to select the next and previous feature. JOYSTICK Move cursor to menus, arrows, selections, sliders, and press a button. Hold the button down to move sliders"
)
...
(instance searchScript of rmnScript
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(localproc_1 201 9 20 170 70) ; "searching..."
				(= seconds 1)
			)
			(1
				(Graph grFILL_BOX 170 20 180 70 1 0 -1 -1)
				(Graph grUPDATE_BOX 170 20 180 70 1)
				(if (< local5 3)
					(= state -1)
					(++ local5)
				)
			...
			(2
				...
				(if (and (== local1 127) (or (IsFlag 81) (IsFlag 142)))
					...
					(= save1 1)
					(localproc_1 201 11 20 170 70) ; "No Match"
					(= seconds 3)
				)
			)
			(3
			...
			(5
				...
				(localproc_1 201 13 100 30 100) ; "M/W 6'2" 210 red/hzl"
