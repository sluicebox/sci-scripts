...
(local
	...
	[local6 6] = [319 158 319 189 238 189]
...
(instance runOver of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2 0)
			(3
				(EgoDead 24) ; "A smart cop knows that you never approach a detained vehicle from the drivers side. Better crack open that officers Manual and bone up on correct procedures."
			)
		)
	)
)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((== global127 5)
						(Print 29 0) ; "You need to restrain him first."
					)
				)
			)
			(else
...
(instance yourCar of Prop
	(properties
		...
		y 203
		description {car}
		lookStr {Your car is parked where you left it.}
		view 208
		cel 1
		signal 16385
	)
		...
		(switch theVerb
			(3 ; Do
				(if (== global113 599)
					...
					(Print 29 1) ; "This is not the time to turn your back."
				else
					(HandsOff)
	...
	(method (init)
		(yourDoor posn: (+ (self x:) 33) (+ (self y:) 68) 100 init:)
		(super init:)
		(if (!= global127 1)
			(topLights
...
(instance theRoom of Feature
	(properties
		lookStr {Highway 41 runs all the way through Lytton.}
	)
)
...
(instance theRail of Feature
	(properties
		onMeCheck 64
		lookStr {Guardrails protect out-of-control vehicles from plunging off the road.}
	)
)
...
(instance theShoulder of Feature
	(properties
		onMeCheck 16384
		lookStr {The shoulder of the road is for emergency stops only.}
	)
)
