...
(local
	...
	local17 = 1000
	local18
	[local19 14] = [0 0 124 90 117 92 110 94 103 96 97 97 -1 -1]
	[local33 26] = [0 0 109 107 115 106 119 103 124 99 129 95 134 91 138 87 131 86 124 85 116 84 109 83 -1 -1]
	[local59 20] = [0 0 106 105 111 104 117 103 123 102 133 100 138 99 143 98 148 97 -1 -1]
)
...
(instance rm048 of PQRoom
	...
	(method (doit)
		...
		(cond
			...
			((and (!= (rocklin script:) sRocklinPlayPool) (== local17 0))
				(= local17 -1)
				(rocklin init: setScript: sRocklinPlayPool)
			)
		)
		(cond
			((and (<= (gEgo x:) (+ (rocklin x:) 15)) (!= local15 0))
				(= local15 0)
			)
			((and (> (gEgo x:) (+ (rocklin x:) 15)) (!= local15 1))
				(= local15 1)
			)
		)
		(cond
			((and (localproc_0 gEgo 128) (not script))
				(HandsOff)
				(gCurRoom setScript: sLeaveBar)
			)
			((and (localproc_0 gEgo 2) (not script))
				(HandsOff)
...
(instance sShootPool of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(if (== [local19 local18] 0)
					(poolPlayer setCycle: End)
					(roomSFX number: 485 play:)
				)
				(if (!= [local19 (+= local18 2)] -1)
					(-- state)
					(cueBall_a x: [local19 local18] y: [local19 (+ local18 1)])
				)
				(= cycles 2)
			)
			(11
...
(instance sRocklinPlayPool of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(if (== [local33 local18] 0)
					(roomSFX number: 485 play:)
					(rocklin setCycle: Beg)
				)
				(if (!= [local33 (+= local18 2)] -1)
					(-- state)
					(ball2 x: [local33 local18] y: [local33 (+ local18 1)])
				)
				(= cycles 2)
			)
			(8
			...
			(14
				(if (== [local59 local18] 0)
					(roomSFX number: 485 play:)
					(rocklin setCycle: Beg)
				)
				(if (!= [local59 (+= local18 2)] -1)
					(-- state)
					(ball3 x: [local59 local18] y: [local59 (+ local18 1)])
				)
				(= cycles 2)
			)
			(15
...
(instance poolCues of Feature
	(properties
		...
		onMeCheck 8
		lookStr {Those cues look pretty beat up, probably get a sliver from it.}
	)
