		...
		(self
			addObstacle:
				((Polygon new:)
					...
					init: 140 161 140 189 166 189 166 162 254 162 254 135 238 128 236 126 184 143 131 118 193 94 178 88 171 83 151 77 140 77 121 83 118 89 107 101 60 115 60 162 80 149 138 149
					yourself:
				)
		)
		(++ gDay)
...
(instance musicTimer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(phoneRing dispose:)
				(Print 40 6) ; "A slightly irratated Captain Tate says... "Come on, Bonds. Time to roll out! Gotta go catch the badguys, remember? Get it in gear!""
		...
		(switch theVerb
			...
			(2 ; Look
				...
				(if (not (gEgo has: 5)) ; musicBox
					(Print 40 11) ; "On the shelf there's several of her favorite momentos including a music box."
				)
			)
			(else
...
(instance readingArea of Feature
	(properties
		...
		onMeCheck 4096
		lookStr {Marie used to curl up in that chair for hours. This was her favorite little reading area.}
	)
)
