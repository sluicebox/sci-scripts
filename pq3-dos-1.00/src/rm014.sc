		...
		(switch (= state newState)
			...
			(9
				...
				(switch local0
					...
					(4
						(Say sonnyInset 14 12) ; "Skin and hair samples."
					)
					(5
						(Say sonnyInset 14 13) ; "Personal photo."
					)
					(6
						(Say sonnyInset 14 14) ; "Blood and hair samples."
					)
					(7
						(Say sonnyInset 14 15) ; "Insignia Ring."
					)
					(8
						(Say sonnyInset 14 16) ; "Cult book."
					)
					(9
						(Say sonnyInset 14 17) ; "White paint sample."
					)
				)
				(= seconds 3)
			)
			(10
				...
				(mouthO setCycle: RandCycle show:)
				(= save1 1)
				(Say insetO 14 18) ; "Location found?"
				(= seconds 3)
			)
			(11
				(if gModelessDialog
				...
				(switch local0
					(1
						(Say sonnyInset 14 19) ; "Clutched in victim's right hand."
					)
					(2
						(Say sonnyInset 14 20) ; "Scene of attack - mall parking lot."
					)
					(3
						(Say sonnyInset 14 21) ; "Left rear quarter panel of abandoned 1973 Ford Pinto."
					)
					(4
						(Say sonnyInset 14 22) ; "Under victim's fingernails."
					)
					(5
						(Say sonnyInset 14 23) ; "Burned-out house, 500 block west Peach."
					)
					(6
						(Say sonnyInset 14 23) ; "Burned-out house, 500 block west Peach."
					)
					(7
						(Say sonnyInset 14 24) ; "Left hand, murder suspect Steve Rocklin."
					)
					(8
						(Say sonnyInset 14 25) ; "On body of murder suspect Steve Rocklin."
					)
					(9
						(Say sonnyInset 14 26) ; "Left rear quarter panel of 1976 GTO."
					)
				)
				(= seconds 4)
			)
			(12
				...
				(mouthO setCycle: RandCycle show:)
				(= save1 1)
				(Say insetO 14 27) ; "And disposition?"
				(= seconds 3)
			)
			(13
				(if gModelessDialog
				...
				(cond
					((== 9 local0)
						(Say sonnyInset 14 28) ; "Analyze for possible match to with paint transfer in case #199145."
					)
					((== 3 local0)
						(Say sonnyInset 14 29) ; "Analyze for possible vehicle type."
					)
					((or (== 4 local0) (== 6 local0))
						(Say sonnyInset 14 30) ; "Analyze for DNA profile."
					)
					(else
						(Say sonnyInset 14 31) ; "Secure as evidence."
					)
				)
				(= seconds 3)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(sonnyInset init:)
				(= save1 1)
				(Say sonnyInset 14 32) ; "This is a new case Gibbs. I need a new number."
				(= seconds 5)
			)
			(1
				(sonnyInset dispose:)
				...
				(mouthO setCycle: RandCycle show:)
				(= save1 1)
				(Say insetO 14 33) ; "Ok Sonny, the next number is 199145."
				(= seconds 4)
			)
			(2
				(self dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(switch (Random 0 4)
					(0
						(Say insetO 14 34) ; "You got something to drop off or are you just here for the show?"
					)
					(1
						(Say insetO 14 35) ; "Yes, Sergeant? Can I do something for you?"
					)
					(2
						(Say insetO 14 36) ; "If you haven't got something for me, Bonds, do you mind clearing the path for officers that do?"
					)
					(3
						(Say insetO 14 37) ; "Are you here to book some evidence or are you just lost?"
					)
					(else
						(Say insetO 14 38) ; "If you don't want to make a 'deposit', Bonds, why the hell are you here?"
					)
				)
				(= seconds 6)
			)
		...
		(switch (= state newState)
			...
			(1
				(cond
					...
					((< register 11)
						(++ local1)
						(StrCat @local4 (Format @temp0 14 39 register)) ; "%d"
						(= register -1)
						(= state 0)
						(= cycles 1)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(mouthO setCycle: RandCycle show:)
				(= save1 1)
				(Say insetO 14 40) ; "I KNOW who you are, Sergeant Bonds. Quit being a wiseass."
				(= seconds 5)
			)
			(2
				(if gModelessDialog
...
(instance clerk of Actor
	(properties
		...
		lookStr {Officer Martha Gibbs has been the Evidence Officer for two years.}
		view 212
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 14 41) ; "You could try it, but it would only get you suspended REAL fast."
					)
					(19 ; wallet
						(HandsOff)
			...
			(3 ; Do
				(Print 14 42) ; "If you want to give Officer Gibbs something, use the evidence deposit slot."
			)
			(5 ; Talk
				(HandsOff)
...
(instance desk of Feature
	(properties
		...
		y 73
		onMeCheck 2048
		lookStr {That stuff belongs to Officer Gibbs' and is of no concern to you.}
	)
)
...
(instance bookingRoom of Feature
	(properties
		x 166
		y 73
		lookStr {This is Evidence Lockup. Physical evidence is deposited here for analysis and safekeeping.}
	)
)
...
(instance inBasket of Feature
	(properties
		...
		y 73
		onMeCheck 16384
		lookStr {This slot is where you put items you wish to check into Evidence.}
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 14 43) ; "Just put something in it."
			)
			(4 ; Inventory
				(if
					(or
					...
					(Print 14 44) ; "Evidence doesn't want that."
				)
			)
			(else
				(super doVerb: theVerb)
