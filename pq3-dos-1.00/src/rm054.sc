...
(instance rm054 of PQRoom
	...
	(method (init)
		...
		(counterLt init:)
		(trash init:)
		(printer approachVerbs: 3 init:) ; Do
		...
		(self
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: 83 159 40 129 72 112 116 141
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
						...
						(cond
							(local0
								(if local1
									...
									(Print 54 32) ; "Just showing him the photo again won't do any good."
								)
							)
							(local1
								(= local4 2)
					...
					(19 ; wallet
						(if local1
							(Print 54 33) ; "You have already identified yourself."
						else
							(= local1 1)
							(HandsOff)
					...
					(1 ; gun
						(Print 54 34) ; "The recruiter is career army. He's not interested in seeing your puny weapons."
					)
					(2 ; handcuff
						(Print 54 34) ; "The recruiter is career army. He's not interested in seeing your puny weapons."
					)
					...
					(else
						(Print 54 35) ; "The recruiter isn't interested."
					)
				)
			)
			(else
...
(instance desk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 54 36) ; "The desk is not yours."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 54 37) ; "The recruiter would no doubt flatten your face if you tried to walk out with his computer."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance fileCab of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 54 38) ; "The recruiting officer might be upset if you starting rifling through his files uninvited."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance trash of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 54 39) ; "You're not a janitor."
			)
			(else
				(super doVerb: theVerb)
			)
