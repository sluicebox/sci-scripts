	...
	(method (init)
		...
		(self
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: 0 163 48 147 71 154 88 168 74 179 46 189 0 189
					yourself:
				)
		)
		(if (IsFlag 6)
		...
		(picnicker1
			cycleSpeed: 9
			setScript:
				(if (> (gGame detailLevel:) (picnicker1 detailLevel:))
					picnicker1Script
				else
					0
				)
		...
		(picnicker2
			cycleSpeed: 9
			setScript:
				(if (> (gGame detailLevel:) (picnicker2 detailLevel:))
					picnicker2Script
				else
					0
				)
		...
		(picnicker3
			cycleSpeed: 9
			setScript:
				(if (> (gGame detailLevel:) (picnicker3 detailLevel:))
					picnicker3Script
				else
					0
				)
		...
		(picnicker4
			cycleSpeed: 9
			setScript:
				(if (> (gGame detailLevel:) (picnicker4 detailLevel:))
					picnicker4Script
				else
					0
				)
		...
		(if (gCast contains: woman)
			...
			(self
				addObstacle:
					...
					((Polygon new:)
						type: PBarredAccess
						init: 69 144 5 139 29 118 88 123
						yourself:
					)
			)
		)
...
(instance suspectOutCar of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(carDoor setPri: 13 setCycle: Beg self)
			)
			(5
				(soundEffects number: 901 loop: 1 play:)
				(= cycles 1)
...
(instance woman of Actor
	(properties
		x 88
		y 140
		lookStr {The woman seems close to hysteria over something.}
		view 280
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 27 19) ; "Smacking her in the face won't help. She's not that hysterical."
			)
			(5 ; Talk
				(if (== local3 0)
					(HandsOff)
					...
					(Print 27 20) ; "The woman is hysterical and won't calm down until you take care of the alleged crazy man."
				)
			)
			(4 ; Inventory
				(switch invItem
					(19 ; wallet
						(Print 27 21) ; "We can SEE that you're a police officer. Now why don't you DO something?"
					)
					(1 ; gun
						(Print 27 22) ; "She obviously ISN'T the perpetrator you were called here to subdue!"
					)
					(else
...
(instance car of PicView
	(properties
		...
		y 195
		description {Car}
		lookStr {You look at your car fondly.}
		view 208
		cel 1
		priority 13
		signal 16384
			...
			(3 ; Do
				(if (IsFlag 5)
					(if local0
						...
						(Print 27 23) ; "So what are you going to do with him?"
					)
				else
					(gCurRoom setScript: enterCar)
				)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					(else
						(Print 27 24) ; "It is easier to drive from the other side."
					)
				)
			)
			(else
...
(instance nut of Actor
	(properties
		x 325
		y 169
		lookStr {This guy is definitely unstable.}
		view 310
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(Print 27 25) ; "This guy's not saying anything that makes sense."
			)
			(3 ; Do
				(cond
					((gEgo has: 17) ; knife
						(Print 27 26) ; "He has already been frisked."
					)
					((== local0 1)
						(Print 27 27) ; "You can't frisk him from out there."
					)
					(else
						(HandsOff)
...
(instance picnicker1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 5)
					(Print 27 28) ; "These folks look relieved now that you have this nut under control."
				else
					(Print 27 29) ; "Something has disturbed the picnickers."
				)
			)
			(5 ; Talk
				(HandsOff)
			...
			(3 ; Do
				(Print 27 30) ; "The picnickers haven't done anything wrong."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance picnicker2 of Prop
	(properties
		...
		y 106
		description {the picnickers}
		lookStr {Something has disturbed the picnickers.}
		view 282
		loop 1
		priority 10
		signal 16400
...
(instance river of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 27 31) ; "You don't have your suit. Besides, you're on duty."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance tables of Feature
	(properties
		...
		onMeCheck 64
		lookStr {You've been picnicking at Aspen Falls many times before. One of those tables probably still has your initials on it.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 27 32) ; "You don't have time for a picnic."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance trees of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 27 33) ; "You don't have time to climb trees."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance dirt of Feature
	(properties
		...
		onMeCheck 256
		lookStr {The bare area disrupts the grasses.}
	)
)
