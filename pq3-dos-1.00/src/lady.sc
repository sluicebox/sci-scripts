	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(if (Print 888 10 #button {INCARCERATE} 1 #button {SIGNATURE} 0) ; "INCARCERATE-Take her to jail, Pat. SIGNATURE-I'll witness her failure to sign."
					(+= state 3)
				)
				(= cycles 1)
			)
...
(instance pat of PicView
	(properties
		x 312
		y 124
		lookStr {The Officer Morales' black eyes challenge you. She impresses you as being both arrogant and stubborn.}
		view 318
		loop 1
		signal 16384
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(cond
					((IsFlag 11)
						(Print 888 29) ; "You've already taken care of the situation. You had better get back on patrol."
					)
					(local0
						(HandsOff)
					...
					(local1
						(Print 888 30) ; "The officer is waiting."
					)
					(else
						(HandsOff)
						(= local1 1)
			...
			(3 ; Do
				(Print 888 31) ; "I don't think the officer would appreciate you touching her."
			)
			(4 ; Inventory
				(Print 888 32) ; "I sympathize, but Sonny wouldn't do that."
			)
			(else
...
(instance patCar of PicView
	(properties
		...
		y 117
		description {car}
		lookStr {That must be Officer Morales' patrol car.}
		view 208
		signal 16385
	)
)
...
(instance insetSuspect of Prop
	(properties
		x 244
		y 126
		lookStr {The woman looks furious and on the verge of tears. She's VERY pregnant.}
		view 322
		priority 15
		signal 16401
	)
...
(instance theCar of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Print 888 33) ; "Using force on the violator will only get you in some very hot water."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
					...
					(else
						(Print 888 34) ; "Do you think she really cares about that?"
					)
				)
			)
			(else
