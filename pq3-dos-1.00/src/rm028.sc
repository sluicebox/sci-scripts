...
(instance rm028 of PQRoom
	...
	(method (doit)
		(cond
			(script 0)
			((and (& (gEgo onControl: 0) $2000) (== global126 0))
				(HandsOff)
				(= global126 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(= save1 1)
				(iEgo init:)
				(Say iEgo 28 12) ; "There! Now your keys and my badge are neighbors!"
				(= seconds 5)
			)
			(6
				(if gModelessDialog
...
(instance goAfterEgo of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(= save1 1)
				(Say iNutWater 28 13) ; "I'll kill you for that - you flat-headed, flat-footed, ignoramous!"
				(= seconds 10)
			)
			(4
				(if gModelessDialog
...
(instance useStickOnNut of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(= save1 1)
				(Print 28 14 #at 10 10 #dispose) ; "Managing to subdue the deranged man with the nightstick, you check for injuries and see that he's more shaken-up than hurt."
				(= seconds 7)
			)
			(5
		...
		(switch (= state newState)
			...
			(2
				(if (== global126 6)
					...
					(= save1 1)
					(Print 28 15 #at 140 150) ; "Suddenly...the perpetrator regains consciousness."
					(nut
						view: 297
						setPri: -1
						setStep: 3 2
...
(instance waterScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 28 16) ; "This is no time to go swimming."
...
(instance cuffHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 28 17 #at 10 10 #dispose) ; "You cuff the perpetrator while he's down, insuring that he won't have another chance to attack."
				(gEgo
					view: 309
...
(instance shootHim of Script
	...
	(method (doit)
		(if
			(and
				(not (& (nut onControl: 0) $0080))
				(not (& (nut onControl: 0) $1000))
				(== state 1)
			)
			(self cue:)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (not local85)
					(iEgo init:)
					(Say iEgo 28 18) ; "Come out of there! Right now!"
					(= local85 1)
					(= seconds 4)
				else
					(iNutWater init:)
					(switch (Random 0 2)
						(0
							(Say iNutWater 28 19) ; "The Bathonians need our water! They will suck this planet dry!"
						)
						(1
							(Say iNutWater 28 20) ; "If you're here when they come, you'll be crushed like a pimple under the weight of the glorious mothership."
						)
						(2
							(Say iNutWater 28 21) ; "I'm warning you, mortal! You must leave now or DIE."
						)
					)
					(= seconds 8)
					(++ state)
			...
			(1
				...
				(iEgo dispose:)
				(iNutWater init:)
				(Say iNutWater 28 22) ; "You MUST leave now! If you're here when they come, you'll be toast!"
				(= seconds 8)
			)
			(2
				(iNutWater dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(iNutLand init:)
				(Say iNutLand 28 23) ; "You there! Leave this place! I've got to secure it before the Bathonians come!"
				(= seconds 7)
			)
			(2
				(= save1 1)
				(Say iNutLand 28 24) ; "Clear out! Get away! The mothership approaches!"
				(= seconds 6)
			)
			(3
				(if gModelessDialog
...
(instance useKeyOnNut of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say iNutWater 28 25) ; "Release those keys, mortal. I am the chosen one, not You!."
				(= seconds 10)
			)
			(2
				(iNutWater dispose:)
...
(instance clothes of Prop
	(properties
		x 208
		y 175
		lookStr {A pile of clothes, probably belonging to the half-naked perpetrator, is sitting by the rocks unattended.}
		view 295
		priority 2
		signal 16401
	)
		...
		(switch theVerb
			(3 ; Do
				(if (or (gEgo has: 23) (proc882_2 2)) ; license
					(Print 28 26) ; "There's nothing else of interest in the clothes."
				else
					(HandsOff)
			...
			(4 ; Inventory
				(Print 28 27) ; "So,... Your a tailor now!?"
			)
			(else
...
(instance nut of Actor
	(properties
		x 195
		y 169
		lookStr {The man's clearly in a deranged state of mind - though whether because of drug use or some mental instability is unclear.}
		view 302
		cycleSpeed 7
		moveSpeed 1
	)
		...
		(switch theVerb
			(5 ; Talk
				...
				(cond
					...
					(else
						(HandsOn)
						(Print 28 28) ; "The man isn't listening to you."
					)
				)
			)
			(3 ; Do
				(switch global126
					(1
						(Print 28 29) ; "You can't subdue him from here."
					)
					(2
						(Print 28 30) ; "What are you going to do, kiss him?"
					)
					(4
						(Print 28 31) ; "He's already down. Police brutality wasn't part of your training."
					)
					(else
			...
			(4 ; Inventory
				(switch invItem
					...
					(1 ; gun
						...
						(cond
							((== global126 4)
								(Print 28 32) ; "You aren't going to shoot an unconscious man in the back are you?"
							)
					...
					(2 ; handcuff
						(switch global126
							...
							(2
								(Print 28 33) ; "You need to subdue him first."
							)
							(3
								(Print 28 34) ; "Too late now..."
							)
							(else
								(Print 28 35) ; "You are not close enough to handcuff him."
							)
						)
					)
					(16 ; nightStick
						(switch global126
							...
							(else
								(Print 28 36) ; "Using a nightstick on someone who hasn't threatened you is against the law."
							)
						)
					)
					(19 ; wallet
						(Print 28 37) ; "That guy wouldn't know or care if you were President of the United States."
					)
					(else
						(Print 28 38) ; "The lunatic is not impressed."
					)
				)
			)
			(else
...
(instance river of Feature
	(properties
		onMeCheck 16384
		lookStr {The river's considerable undertow churns the waters into a muddy green.}
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 28 39) ; "The water babbles back."
			)
			(4 ; Inventory
				(switch invItem
					(3 ; theKeys
						(if (proc882_2 2)
							(Print 28 40) ; "Calling on your old softball arm, you haul back and throw the keys as far as you can into the river."
							(proc882_1 2)
							(if (gCast contains: nut)
								(HandsOff)
								(gCurRoom setScript: throwKeys)
							)
						else
							(super doVerb: theVerb invItem)
						)
...
(instance trees of Feature
	(properties
		...
		onMeCheck 12
		lookStr {The graceful willow trees make Aspen Falls the prettiest park for miles around.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 28 41) ; "You don't have time to climb trees."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance sky of Feature
	(properties
		...
		description {the sky}
		onMeCheck 2
		lookStr {The azure summer sky is accented by wispy clouds.}
	)
)
...
(instance grass of Feature
	(properties
		...
		onMeCheck 5280
		lookStr {The grass stays lush and green on the banks of the river.}
	)
)
...
(instance rocks of Feature
	(properties
		...
		onMeCheck 256
		lookStr {A few large rocks mark a favorite spot for wading.}
	)
)
