	...
	(method (doit &tmp temp0)
		(cond
			...
			((& (gEgo onControl: 0) $0002)
				(self posn: 258 189 setScript: (ScriptID 896 2) 0 19) ; sWO
			)
		)
		(super doit:)
	)
...
(instance psycho of Prop
	(properties
		...
		approachY 123
		_approachVerbs 5
		lookStr {Dr. Aimes is one of those intellectual types. Not a bad guy, but a little patronizing.}
		view 250
		signal 16384
	)
		...
		(switch theVerb
			(5 ; Talk
				...
				(cond
					(local4
						(HandsOn)
						(Print 23 21) ; "Well, Bonds? You've got your information, now go and catch him."
					)
					(local1
			...
			(3 ; Do
				(Print 23 22) ; "Now that IS Freudian."
			)
			(4 ; Inventory
				(switch invItem
					(26 ; miltaryRecord
						(if local4
							(Print 23 23) ; "He's already seen the military record."
						else
...
(instance clackersB of Prop
	(properties
		...
		lookStr {It's one of those mindless excutive toys.}
		view 251
		loop 8
		cel 5
		priority 15
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 24) ; "But... you're not a mindless executive."
			)
			(else
...
(instance clackersS of Prop
	(properties
		...
		y 108
		description {clackers}
		lookStr {It's one of those mindless executive toys.}
		view 250
		loop 3
		priority 9
		signal 16400
...
(instance chair of PicView
	(properties
		...
		y 106
		description {chair}
		lookStr {Dr. Aimes' chair is made of soft Corinthian leather.}
		view 250
		loop 2
		signal 16384
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 25) ; "If you want to play psychologist, you bought the wrong game."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance desk of Feature
	(properties
		...
		approachX 146
		approachY 130
		lookStr {That's Doctor Aimes' desk. There's a file on the desk.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 26) ; "Doctor Aimes prefers to keep his personal property personal."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pFile of Prop
	(properties
		...
		approachY 122
		_approachVerbs 3
		lookStr {It is a personnel file.}
		view 253
		priority 7
		signal 16401
	)
			...
			(3 ; Do
				(cond
					(local0
						(SetScore 164 5)
						(proc0_17 20)
						(Print 23 27) ; "No one seems to be around so you pick up the file. You notice that it's a personnel file of Pat Morales."
						(fileSound init: play:)
						(Print 23 28) ; "Afraid of being caught snooping, you read only the doctor's summary."
						(Print 23 29) ; "1. Displays chronic insubordination."
						(Print 23 30) ; "2. Irrational hatred of authority figures."
						(Print 23 31) ; "3. Low self esteem."
						(Print 23 32) ; "4. Borderline sociopath."
						(Print 23 33) ; "SUMMARY."
						(Print 23 34) ; "Evaluation was suggested by Internal Affairs."
						(Print 23 35) ; "Officer Morales was involved in the destruction of evidence on three different occasions."
						(Print 23 36) ; "Internal Affairs concluded the incidents were "coincidental" based on insufficient evidence for disciplinary action."
						(Print 23 37) ; "You return the file to the desk."
						(fileSound play:)
					)
...
(instance couch of Feature
	(properties
		...
		y 115
		onMeCheck 8192
		lookStr {The proverbial 'confessing couch'.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 38) ; "You prefer a cold beer to Freudian analysis!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance rm23Window of Feature
	(properties
		...
		y 67
		onMeCheck 4096
		lookStr {A window overlooks the parking lot. The blinds are drawn.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 39) ; "The parking lot isn't all that exciting a sight."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance wasteBasket of Feature
	(properties
		...
		y 127
		onMeCheck 2048
		lookStr {It's a theory receptacle.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 40) ; "Nah. You have enough trash at home."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance smallChair of Feature
	(properties
		...
		y 93
		onMeCheck 1024
		lookStr {Dr. Aimes provides a chair for those who feel silly lying down.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 41) ; "You don't feel like sitting down."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance fileCabinet of Feature
	(properties
		...
		y 75
		onMeCheck 32
		lookStr {The filing cabinets have a lot of dirt on a lot of people.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 42) ; "You can't get in there."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bookcase of Feature
	(properties
		...
		y 45
		onMeCheck 8
		lookStr {Just looking at the titles of Dr. Aimes' books puts you to sleep.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 23 43) ; "Just looking at the titles of Dr. Aimes' books puts you to sleep."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance plant of Feature
	(properties
		...
		y 149
		onMeCheck 4
		lookStr {A bit of green is always nice. This guy probably talks to his plants.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(Print 23 44) ; "Don't you have something better to do?"
			)
			(3 ; Do
				(Print 23 45) ; "You get enough vitamin 'B'."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance aroundRoom of Feature
	(properties
		...
		y 74
		onMeCheck 128
		lookStr {This is the office of the department's criminal psychologist, Dr. Sidney Aimes.}
	)
)
