...
(instance crash of PQRoom
	...
	(method (doit &tmp temp0)
		(cond
			((or (& (= temp0 (gEgo onControl: 0)) $2000) (& temp0 $0040))
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(trunk dispose:)
				(= local0 1)
				(gEgo setMotion: MoveTo -40 135 self)
			)
			(1
				(= seconds 5)
			)
			(2
				(if (!= (pat x:) 158)
					(pat setScript: patTrunk)
				)
				(gEgo setMotion: MoveTo 6 128 self)
			)
			(3
				(gEgo
					normal: 0
	...
	(method (changeState newState &tmp [temp0 200])
		(switch (= state newState)
			(0
				...
				(gEgo setMotion: PolyPath 223 61 self)
			)
			(1
				(Face gEgo body)
				(bodyInset init:)
...
(instance bodyInset of Prop
	(properties
		x 162
		y 115
		lookStr {It's the punk who tried to blow you away at the Nugget. He fits the bag lady's description to a tee. You regret that you didn't get your hands on him before he died.}
		view 356
		priority 14
		signal 16401
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 880 21) ; "You search the battered body but find no identification."
				(self dispose:)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(proc882_0 4)
				(self dispose:)
				(Print 880 22) ; "You remove the keys from the ignition."
			)
			(else
				(super doVerb: theVerb)
			)
					...
					(3 ; theKeys
						(if (proc882_2 4)
							(if (not local2)
								...
								(Print 880 23) ; "It's already open."
							)
						else
							(Print 880 24) ; "Your keys don't fit."
						)
					)
					(else
						(Print 880 25) ; "That won't do any good."
					)
				)
			)
			(else
...
(instance cocaine of Prop
	(properties
		...
		approachX 152
		approachY 98
		lookStr {The cocaine is packaged in five heavy-duty plastic bags. Apparently, murder wasn't this guy's only pastime.}
		view 342
		loop 1
		cel 1
		priority 2
...
(instance yourTrunk of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(if local0
					(Print 880 26) ; "Pat has already gotten the rest of the equipment."
				else
					(UnLoad 128 700)
					(UnLoad 128 900)
...
(instance yourDoor of Prop
	(properties
		description {car door}
		lookStr {The passenger car door.}
		view 353
		loop 1
		priority 15
		signal 16401
...
(instance yourCar of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 880 27) ; "You haven't finished up here. Don't leave yet."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance pat of Actor
	...
	(method (doit)
		(cond
			((or (& (self onControl: 0) $2000) (& (self onControl: 0) $0040))
				(self setPri: 13)
			)
			((& (self onControl: 0) $4000)
				(self setPri: 14)
			)
			((& (self onControl: 0) $0001)
				(self setPri: -1)
			)
			((> (self y:) 200)
				(self setPri: 15)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 880 28) ; "Morales ignores you."
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 880 29) ; "You can't shoot your partner in cold blood, Sonny."
					)
				)
			)
			(else
...
(instance ambulence of Actor
	(properties
		x -66
		y 206
		lookStr {The ambulance will remove any injured accident victims.}
		yStep 20
		view 357
		signal 18432
		xStep 20
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 880 30) ; "You've had one ambulance ride too many this week!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance attendent of Actor
	(properties
		...
		y 70
		description {Leon}
		lookStr {The ambulance attendant is used to this kind of scene.}
		view 342
		loop 1
		cel 5
		signal 1
	...
	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(Print 880 31) ; "The attendant is busy looking over the scene."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance coroner of Actor
	(properties
		...
		approachX 241
		approachY 63
		lookStr {That's Leon, the county coroner. You see him all too frequently at accident scenes.}
		view 342
		loop 1
		cel 6
		signal 16385
	...
	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(if local1
					(Print 880 32) ; "Leon just looks at you and shakes his head."
				else
					(++ local1)
					(HandsOff)
...
(instance flares1 of Prop
	(properties
		x 7
		y 125
		lookStr {The traffic on this road has been rerouted until the accident can be cleaned up.}
		view 342
		loop 2
		signal 16384
		cycleSpeed 5
...
(instance theRail of Feature ; UNUSED
	(properties
		onMeCheck 64
		lookStr {The guardrail was smashed when the vehicle collided with it.}
	)
...
(instance theCar of Feature
	(properties
		onMeCheck 128
		lookStr {The car is overturned, smashed up, and generally mutilated. A body slumps from the driver's side. So much for questioning the suspect.}
	)
)
