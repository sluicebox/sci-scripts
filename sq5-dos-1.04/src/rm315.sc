...
(instance rm315 of Rm
	...
	(method (init)
		...
		(super init:)
		(theMusic3 number: 403 owner: self flags: 1 init: setLoop: -1 play: 50)
		(if (not (IsFlag 13))
			(rock init:)
...
(instance sRogDiesInTunnel of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 18) ; "After having the interior of the cave redecorated with your entrails, you conclude that entering a cavern known to contain a killer android can be hazardous to one's health."
				(self dispose:)
			)
		)
	)
...
(instance sWD40Explodes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gSq5Music2 fade:)
				(gSq5Music1 number: 17 setLoop: -1 play:)
				(SetFlag 16)
				(ClearFlag 15)
				(gGame handsOn:)
				(gCurRoom newRoom: 305)
