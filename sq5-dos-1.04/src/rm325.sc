		...
		(switch (= state newState)
			...
			(3
				...
				(if (and (gEgo has: 12) (not (& global169 $0020))) ; Cloaking_Device
					(gMessager say: 6 0 5 0 self) ; "|f8|i got it! let's move before this thing blows!"
				else
					(= cycles 1)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(gMessager say: 12 0 0 0 self) ; "Hey--why'd we beam in here? I wonder if this little device had anything to do with it..."
			)
			(4
				(gGame handsOn:)
				(self dispose:)
		...
		(switch theVerb
			(1 ; Look
				(if (== (gCurRoom picture:) 65)
					(gMessager say: 8 1 4 0) ; "You can't see WD40's ship, but your razor sharp intellect tells you it must be around here somewhere."
				else
