...
(instance rm880 of LLRoom
	...
	(method (notify codeNumber)
		(if (and (OneOf codeNumber gRoomBNumber -413) (== correctDoor 1))
			(doorB locked: 0)
			(= correctDoor 0)
...
(instance doorA of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 880 4) ; "You really don't want to open this door. Your mission is to NOT get caught!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance PChammer of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 880 5) ; "P. C. Hammer is the man Desmond says is linked to much of the evil doings here at K-RAP radio!"
				(TPrint 880 6) ; "It's a good thing your disguise is working out so well. You certainly wouldn't want him to notice you."
			)
			(5 ; Talk
				(TPrint 880 7) ; "You may wish to talk with P. C. Hammer later, but now is not the time."
			)
			(3 ; Do
				(TPrint 880 8) ; "Stay away from the window, in case P. C. realizes you're not the woman you seem to be."
			)
			(10 ; Zipper
...
(instance mikeStand of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 880 11) ; "A large, heavy microphone stand waits for a gig."
			)
			(3 ; Do
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(TPrint 880 16) ; "You attempt to hike up those Hammerpants you're wearing, but it does no good!"
				(return 1)
			)
			(10 ; Zipper
				(TPrint 880 17) ; "Maintaining your cool, you surreptitiously check the zipper on your giant baggy pants. It's fine."
				(return 1)
			)
			(else
				(return 0)
