...
(instance elevatorKeypad of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 870 8) ; "Since you have no way of knowing John Krapper's personal elevator's access code, it appears you are trapped down here forever!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance clothes of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 870 9) ; "A set of brightly-colored, over-sized rapper's clothes hangs on the clothes rack, conveniently placed near the elevator."
			)
			(3 ; Do
				(if (== (gEgo view:) 873)
...
(instance dude1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 870 10) ; "Inside Studio A, the members of "2 Live 2 Screw" are carrying on a lively discussion. If only you could hear their conversation."
			)
			(3 ; Do
				(TPrint 870 11) ; "You are unable to hear those dudes through the soundproof glass."
			)
			(10 ; Zipper
				(TPrint 870 12) ; "Patti! You have no interest whatsoever in these guys!"
			)
			(4 ; Inventory
				(TPrint 870 13) ; "They can't hear your offer, and wouldn't be willing to deal with you anyway!"
			)
			(else
				(TPrint 870 14) ; "If only you could find some way to overhear their conversation."
			)
		)
	)
)
...
(instance elevator of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 870 15) ; "You are far too familiar with the "showervator" already! You hope that somehow, some way, you'll be able to find your way back up to Krapper's office and to your own clothes!"
			)
			(3 ; Do
				(TPrint 870 16) ; "From down here, Mr. Krapper's private elevator requires his personal access number through the keypad to the left."
			)
			(10 ; Zipper
				(TPrint 870 17) ; "While you might want to undress and re-enter the shower, you can't, as the elevator door is now firmly locked."
			)
			(4 ; Inventory
				(TPrint 870 18) ; "That doesn't seem to unlock the shower door."
			)
			(else
				(TPrint 870 19) ; "The showervator is off-limits to you at the moment."
			)
		)
	)
)
...
(instance getClothesScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 870 21) ; "Checking as much of the downstairs as you can see, you make sure that no one is coming. Cautiously leaning out of the showervator, you grab the set of clothes conveniently hanging on the nearby clothes rack."
				(= seconds 3)
			)
			(2
			...
			(6
				(TPrint 870 22) ; "Wow! Look at you! It's the all-new, all-what's-happenin' "Rappin' Patti P!!""
				(TPrint 870 23) ; "With that outfit and your impromptu copier make-up job, you're all set to blend right in down here!"
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(if (== (gEgo view:) 873)
					(TPrint 870 24) ; "You briefly consider doing what you often do when you're in the shower, but soon decide this is neither the time nor the place."
					(return 1)
				else
					(TPrint 870 25) ; "You attempt to hike up those Hammerpants you're wearing, but it does no good!"
					(return 1)
				)
			)
			(10 ; Zipper
				(if (== (gEgo view:) 873)
					...
					(TPrint 870 26) ; "Maintaining your cool, you surreptitiously check the zipper on your giant pants. It's fine."
					(return 1)
				)
			)
			(else
