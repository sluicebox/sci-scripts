...
(procedure (TalkFromBooth)
	...
	(Say Reverse_Biaz 640 59 #dispose) ; "Hello, out there! You must be Passionate Patti! Everything's all ready for your cut-in session. Just come on into the studio, I'm waiting for you."
)
		...
		(switch (= state newState)
			...
			(2
				(if (turnTableWRecord cycler:)
					...
					(TPrint 640 20 #at -1 185) ; "(But only when it's ON!)"
				)
...
(instance sEnterElevator of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Say Reverse_Biaz 640 21 #dispose #caller self) ; "Patti, I promise you: you'll never work in this town again!"
			)
			(5
				(elevatorDing number: 622 play:)
				(elevator setCycle: Beg self)
		...
		(switch theVerb
			...
			(3 ; Do
				(cond
					...
					(askedInToStudio
						(SetFlag 21) ; fWarned
						(Say Reverse_Biaz 640 26 #dispose) ; "Where are you going, Patti? There are a lot of people in this town who would love a chance to blow synth on a session here. I was happy to give you the gig. But, if you leave me now, I promise you I'll be on the phone to the Union before you're downstairs!"
					)
					(else
						(TalkFromBooth)
					)
