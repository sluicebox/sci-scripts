...
(procedure (SetupExit turnOn)
	(if turnOn
		(icon0
			view: (proc0_28 990 990 990 9901 990)
			loop: 15
			cursor: (proc0_28 6 6 6 61 6)
		)
	else
		(icon0 view: (proc0_28 990 990 990 9901 990) loop: 0 cursor: 0)
	)
...
(procedure (proc0_29)
	(proc0_28 1026 1040 1051 1050 995)
)
			...
			(if
				...
				(if (== gFfRoom 1000)
					(if (IsObject gFfCueObj)
						...
						(Print 0 25) ; "ERROR: Object passed to SetFFRoom -1 ain't."
					)
				else
					(if (== gCurRoomNum 535)
...
(instance ll5DoVerbCode of Code
	...
	(method (doit theVerb theObj &tmp objDesc theItem [str 150])
		...
		(switch theVerb
			...
			(2
				(if (theObj lookStr:)
					...
					(if 1
						...
						(Format @str 0 26 global200 global203) ; "Why, look! It's %s."
						(Memory memFREE global200)
						(Memory memFREE global203)
					)
					(TPrint @str)
			...
			(5
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 27 global200 global203) ; "You engage in a short, but personally meaningful, conversation with %s."
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
			...
			(3
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 28 global200 global203) ; "That feels exactly like %s."
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
			...
			(4
				...
				(if 1
					...
					(Format @str 0 29 global201 global200 global204 global203) ; "It seems %s just doesn't work with %s."
					(Memory memFREE global200)
			...
			(10
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 30 global200 global203) ; "Don't do that to %s!"
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
...
(instance gcWin of SysWindow
	...
	(method (open &tmp [temp0 2] temp2 temp3 temp4 temp5 temp6 temp7 [temp8 4] temp12 temp13 [temp14 20] [temp34 4])
		...
		(DrawCel
			...
			0
			2
			(- (- 147 (proc0_28 12 1 5 1 1)) (proc0_28 0 0 0 5))
			(- 37 (+ (CelHigh (proc0_29) 0 4) 3))
			temp12
		)
		(DrawCel
		...
		(Format @temp14 0 31 gScore gPossibleScore gScore gPossibleScore) ; "Score: %d of %d"
