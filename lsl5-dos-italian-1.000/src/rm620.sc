...
(instance sGuardWakes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= guardStatus 1)
				(Say The_Guard_ILa_Guardia 620 2 108) ; "Yes?" mumbles the sleepy guard, "You need somethin'?"
				(HandsOn)
				(Delay 6 2 self)
			)
			(2
...
(instance sGuardSleeps of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say The_Guard_ILa_Guardia 620 5 108) ; "Honey, if you don't know where you're going, how am I supposed to know?"
				(guard setLoop: 0 setCel: 0 setCycle: End self)
			)
			(2
				(guard setScript: sSleeping)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					((IsFlag 45)
						(Say The_Guard_ILa_Guardia 620 11 108) ; "Lady, I already told you: Reverse Biaz says, `you blew the gig when you split'--whatever that means!"
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					)
					(datamanSolution
						(Say The_Guard_ILa_Guardia 620 12 108) ; "Hey, pretty cool! I was thinkin' about gettin' one of these for the little missus! Now, let's see... `Biaz?' `Suite 900?' Sure, no problem. I'll summon the elevator for you right now!"
						(= datamanSolution 0)
						(Points 6 118)
					)
					(wrongDataman
						(Say The_Guard_ILa_Guardia 620 13 108) ; "Hey, pretty cool! I was thinkin' about gettin' one of these for the little missus! Now, let's see... `P. C. Hammer?' I'm sorry, but there's nobody by that name in this building."
						(= wrongDataman 0)
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					...
					(faxSolution
						(Say The_Guard_ILa_Guardia 620 14 108) ; "Kinda flimsy paper, ain't it? Well, let's see here... You know, it's hard to read this stuff; the text is all fuzzy-looking. Oh, here it is... `Biaz?' Why, he's in Suite 900. Sure, no problem. I'll summon the elevator for you right now!"
						(= faxSolution 0)
						(Points 6 119)
					)
					(wrongFax
						(Say The_Guard_ILa_Guardia 620 15 108) ; "Kinda flimsy paper, ain't it? Well, let's see here... You know, it's hard to read this stuff; the text is all fuzzy-looking. Hmm... `P. C. Hammer?' Nah, they only let us use screwdrivers and pliers on the computers here!"
						(= wrongFax 0)
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					...
					(else
						(Say The_Guard_ILa_Guardia 620 16 108) ; ""Huh? Oh, right. Give me just a second while I make sure he's in," sighs the overworked guard."
						(SetFlag 48)
					)
				)
				(self cue:)
			...
			(8
				(if (IsFlag 21)
					(Say The_Guard_ILa_Guardia 620 17 108) ; "I'm sorry, but he says he's found someone else. Take a hike!"
					(SetFlag 45)
					(HandsOn)
					(guard setScript: sBackToSleep)
					(self dispose:)
				else
					(Say The_Guard_ILa_Guardia 620 18 108) ; "Okay, honey. He's waiting for you upstairs. I'm calling for the elevator for you right now."
					(guard
						cycleSpeed: 18
						setLoop: 0
						setCel: 0
...
(instance guard of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
					...
					(switch invItem
						(0 ; Camcorder
							(cond
								...
								(else
									(Say The_Guard_ILa_Guardia 620 27 108) ; "I'm sorry, lady, but that don't mean nothing to me."
									(guard setScript: sBackToSleep)
								)
							)
						)
			...
			(3 ; Do
				(if (== guardStatus 1)
					(Say The_Guard_ILa_Guardia 620 31 108) ; "Oh, Ma'am," sighs the guard, "that feels good!"
				else
...
(instance businessSeven of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 42 67 -1 28) ; "Big Buttons, Inc. supplies buttons of all sizes for every occasion: political campaigns, birthday parties, fund raisers, software company identification, etc."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance The_Guard_ILa_Guardia of Talker
	(properties
		name {The Guard}
		nsTop 15
		nsLeft 35
