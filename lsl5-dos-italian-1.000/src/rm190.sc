...
(instance rm190 of LLRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 190 0) ; "You are outside PPC's general headquarters, nestled in the smog beneath the Hollywood hills. Gawd, how you love being in show biz!"
			)
			(else
...
(instance fountain of Prop
	...
	(method (doVerb theVerb invItem &tmp [str 60])
		(switch theVerb
			(3 ; Do
				(TPrint 190 1) ; "Carefully checking to make sure that no one can see you, you take full advantage of the PPC Memorial Fountain."
			)
			(4 ; Inventory
				(if 1
					(= global200
					...
					(Format @str 190 2 global200 global203) ; "Now %s is wet!"
...
(instance statue of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 1 90)
				(TPrint 190 3) ; "You remember her! She's PornProdCorp's founding mother: Chesty Turbo!"
			)
			(3 ; Do
				(TPrint 190 4) ; "You can't really do what you're thinking!"
			)
			(5 ; Talk
				(TPrint 190 5) ; ""I'm so lonely," you moan to the statue."
			)
			(10 ; Zipper
				(TPrint 190 6) ; "Larry! She's not that good!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tree of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(TPrint 190 10) ; ""I Talk to the Trees," you sing."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance windows1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 190 11) ; "Keep your hands off the glass."
			)
			(5 ; Talk
