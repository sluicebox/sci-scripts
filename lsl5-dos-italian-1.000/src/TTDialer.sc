...
(instance TTDialer of Prop
	...
	(method (init which who)
		...
		(if (!= (= local67 ((gTheIconBar at: 0) cursor:)) (proc0_28 6 6 6 61 6))
			(SetupExit 1)
		)
		(HandsOff)
	...
	(method (cue &tmp i)
		(switch (++ local14)
			...
			(255
				...
				(if (!= local67 (proc0_28 6 6 6 61 6))
					(SetupExit 0)
				)
				(RestoreIB)
			...
			(1
				(Format ; "You have reached Directory Assistance for the Greater %s Area. We're sorry, but we're unable to come to the phone right now. We're presently busy (yeah, busy %s!). Let us assure you that your call IS important; it's just not important to us!"
					...
					(if (== gCurRoom 200)
						...
						(switch gLarryLoc
							(1 {New York City Metropolitan})
							(2 {Atlantic City non Infestata dai Barboni})
							(3 {Dade County})
							(else {'Inserisci Qui la Tua Citt\85'})
						)
					)
					(if (== gCurRoom 200)
						{galleggiare sul Potomac}
					else
						(switch gLarryLoc
							(1 {farci derubare})
							(2 {riempire gli slots})
							(3 {spaparanzarci sulla spiaggia})
							(else {stare seduti sul bidone!})
						)
					)
				)
				(TPrint @string)
