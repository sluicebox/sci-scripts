...
(instance chiChi of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(10 ; Zipper
				(Say Chi_Chi_Lambada 910 18 #dispose) ; "Well, I might be interested, Larry," says Chi Chi. "But what about a little conversation first?"
			)
			(1 ; Walk
				(gCurRoom doVerb: 1)
			)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(2
				...
				(Points 40 165)
				(Say Chi_Chi_Lambada 910 19 #dispose #caller self) ; "Why don't we go downstairs to my apartment behind the gymnastics studio where I'll be able to express my appreciation in a more acceptable manner?"
			)
			(3
				(= ticks 90)
			)
			(4
				(TPrint 910 20 #at -1 185 #width 280) ; "(All right, Larry! It sounds like you're going to get a home-cooked meal for a change!)"
				(= ticks 123)
			)
			(5
			...
			(6
				(gTheMusic fade:)
				(CenterDisplay 3 gColWhite 910 21)
				(= seconds 3)
			)
			(7
