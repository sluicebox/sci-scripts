...
(instance johnDoor of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 16) ; "A sign on the door says "Krapper's Crapper."A smaller sign below it says "John's John"."
			)
			(3 ; Do
				(cond
					((gEgo has: 10) ; Some_Change
						(TPrint 840 17) ; "Mr. Krapper will surely miss his file folder, Patti! You'd better return it."
					)
					((gEgo has: 9) ; AeroDork_s_In-Flight_Magazine
						(TPrint 840 18) ; "Mr. Krapper will surely miss his desk key, Patti! You'd better return it."
					)
					((gEgo has: 8) ; Boarding_Pass
						(TPrint 840 19) ; "Mr. Krapper will surely miss his letter opener, Patti! You'd better return it."
					)
					(else
...
(instance lobbyDoor of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 20) ; "This is the door through which you entered. If you walked out it, you would return to the lobby."
			)
			(3 ; Do
...
(instance table of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 21) ; "Krapper has a beautiful glass coffee table."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 22) ; "This is the kind of furniture you would buy if you ever settled down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sofa of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 23) ; "What an unusual sofa! You so love modern furniture."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance telephone of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 24) ; "John Krapper's telephone has very few buttons. He must be REALLY important!"
			)
			(3 ; Do
				(TPrint 840 25) ; "You'd better not use this telephone, Patti. You might get caught by the switchboard operator."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 26) ; "Look at the dust on that keyboard! He must never use his computer!"
			)
			(3 ; Do
				(TPrint 840 27) ; "You briefly consider booting up John Krapper's computer in an attempt to contact Inspector Desmond, but realize there's no way his computer would be connected to FBI-Net!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance myWindow of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 28) ; "Krapper's office has a giant window looking out over beautiful downtown Philadelphia."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance stuff of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 29) ; "Krapper's bookcase is filled with items, all of them useless to your mission."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance lamp1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 30) ; "The small halogen spotlights cast a pure white light."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance lamp3 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 31) ; "The small halogen spotlights cast a pure white light."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance opener of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 32) ; "There's a needle-sharp letter opener lying on Krapper's desk."
			)
			(3 ; Do
				(TPrint 840 5) ; "Never knowing when you might get some mail, you decide to take Krapper's letter opener from his desk."
				(opener hide:)
...
(instance peekScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 33) ; "Opening the doorway to carefully peek outside, you notice the receptionist has returned. You decide you cannot exit the room that way!"
				(= seconds 3)
			)
			(3
				(lobbyDoor setCel: 0)
...
(instance getKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 10 148)
				(TPrint 840 34 67 -1 28) ; "Bending over at the base of the tree, you dig into the humus and find the metallic object that you saw earlier. It looks like a desk key."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance sPawThruPlant of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 840 35) ; "Pawing through the humus in the tree's pot, you see a glint of metal."
				(SetFlag 23)
				(HandsOn)
				(self dispose:)
			)
...
(instance replaceKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 36 67 -1 28) ; "You carefully return the key to its hiding place in the humus."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance sOpenDesk2ndTime of Script
	...
	(method (changeState newState &tmp [str 80])
		(switch (= state newState)
			...
			(4
				(Format @str 840 38 gRoomBNumber gRoomBNumber) ; "Where's that little blue stickum? Oh, here it is! %u"
				(TPrint @str)
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
...
(instance pickLockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 5 147)
				(TPrint 840 39) ; "Prying away at the desk's inefficient lock with Krapper's letter opener, you find a weak spot and carefully pry the drawer open."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
					loop: 2
...
(instance unlockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 13 147)
				(TPrint 840 40) ; "Trying the key you found buried in the tree's pot, you insert into the desk and turn it carefully. It opens."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
					loop: 2
...
(instance lockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 41) ; "You lock the desk."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
					loop: 2
...
(instance getPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Points 5 145)
				(TPrint 840 42) ; "Searching the desk drawer, you find the folders marked "Personal.""
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
					loop: 2
...
(instance lookPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 840 43) ; "Carefully searching through the many drawers of Krapper's desk, you find a folder marked "Personal.""
				(= ticks 60)
			)
			(2
				(TPrint 840 44) ; "For a moment, you wonder if you should take it."
				(= ticks 60)
			)
			(3
				(gEgo
			...
			(4
				(Points 5 145)
				(TPrint 840 45) ; "But of course you do!"
				(= ticks 123)
			)
			(5
				(gEgo
...
(instance putPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(TPrint 840 46) ; "You return the folder of evidence to Krapper's desk, and close his desk drawer, being careful to leave everything exactly as you found it."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570)
					observeControl: 2 4
					loop: 2
...
(instance runCopierScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 12 151)
				(TPrint 840 47) ; "You place John Krapper's personal papers in his copier, and press the "Start" button."
				(= ticks 123)
			)
			(3
			...
			(8
				(Say gEgo 840 48) ; "Hey, what's the matter with this thing?" you think. "Why did it stop working? What did I do, jam it?"
				(= seconds 3)
			)
			(9
			...
			(10
				(paper dispose:)
				(copier setCel: 0)
				(Say gEgo 840 49) ; ""Oh well, at least I got the most important evidence copied before it jammed," you think, grabbing your copies from the machine."
				(= seconds 3)
			)
			(11
			...
			(12
				(Say gEgo 840 50) ; ""What does this switch here do?" you wonder, pulling on something that doesn't belong to you."
				(= ticks 123)
			)
			(13
				(TPrint 840 51 67 -1 185) ; "(An experience not wholly unfamiliar to you!)"
				(= ticks 60)
			)
			(14
				(gEgo loop: 4 setCel: 255 setCycle: Beg self)
			...
			(20
				(Say gEgo 840 52) ; "GREAT!" you think. "Now just look at me! I'm covered with toner from head to toe!"
				(copier stopUpd:)
				(SetFlag 27)
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 842)
					(TPrint 840 53) ; "Oh no," you think, "how am I ever going to get this stuff off?"
					(return 1)
				else
					(TPrint 840 54) ; "You look yourself over. You like what you see. "Although, I could stand to lose a few pounds," you think."
					(return 1)
				)
			)
			(else
