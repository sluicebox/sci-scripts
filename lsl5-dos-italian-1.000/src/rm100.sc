...
(procedure (TypeIt &tmp [ltr 6] char theUnders)
	...
	(if (< (proc0_28 21 21 24 22 21) letter (proc0_28 29 29 32 30 29))
		(= [underBs (- letter (proc0_28 22 22 25 23 22))] theUnders)
	else
	...
	(method (init)
		...
		(StrCpy
			...
			(LangSwitch
				...
				{Passionate Patty fa un PittsbuLavoretto di Spionaggio}
			)
		)
		(super init:)
		(HandsOff)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(15
				(TypeIt)
				(typewriter play:)
				(if (< (++ letter) (proc0_28 29 29 32 30 29))
					(= cycles (Random 3 5))
					(-- state)
				else
					(= cycles 10)
			...
			(16
				(Display
					{}
					dsRESTOREPIXELS
					[underBs (- (-- letter) (proc0_28 22 22 25 23 22))]
				)
				(backSpace play:)
				(if (> letter (proc0_28 22 22 25 23 22))
					(= cycles 4)
					(-- state)
				else
					(= letterX (proc0_28 153 153 169 153 153))
					(= letter (proc0_28 29 29 32 30 29))
					(= cycles 10)
				)
			)
			(17
