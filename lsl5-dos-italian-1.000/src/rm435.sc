...
(instance monitors of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 14) ; "These monitors are presently disconnected. When they were heavily used many years ago, it was only to play "Jawbreaker!""
			)
			(3 ; Do
				(TPrint 435 15) ; "You try to turn on the monitor, but are unable to do so."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance plug of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 16) ; "You wonder exactly what that giant plug plugs into."
			)
			(3 ; Do
				(TPrint 435 17) ; "What? And get electrocuted?!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance outlet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 18) ; "The lab technicians use this intercom to communicate with the front office and Commander Twit."
			)
			(3 ; Do
				(TPrint 435 19) ; "You attempt to communicate through the intercom, but you find you're suffering from insufficient clearance."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 20) ; "This office chair prevents you from accessing the intricate equipment below it."
			)
			(3 ; Do
				(TPrint 435 21) ; "Don't move it. You don't need anything that's past the chair."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance joystick of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 22) ; "Wow! What a joystick!"
			)
			(3 ; Do
				(TPrint 435 23) ; "You briefly picture yourself playing "Zeliard.""
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance circuitboard of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 435 24) ; "This looks like a giant circuit board."
			)
			(3 ; Do
				(TPrint 435 25) ; "Be careful. You'll get shocked!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
