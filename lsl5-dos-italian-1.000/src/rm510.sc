		...
		(switch (= state newState)
			...
			(3
				(cond
					...
					((== ((Inv at: 14) state:) 4) ; Membership_Tape
						(Say the_maitre_d 510 35 #dispose) ; ""I suggest you find transportation to another location, Mr. (heh, heh) Laffer!" he smirks."
					)
					((IsFlag 2) ; fSeenMM
						(gCurRoom setScript: sDoubleTalk)
					)
					((== ((Inv at: 14) state:) 1) ; Membership_Tape
						(Say the_maitre_d 510 36 #dispose) ; "Just find a place to sit down. You have an assigned table. Find it!"
					)
				)
				(if (not okToMakeTape)
					(HandsOn)
			...
			(4
				...
				(= cycles 2)
				(Points 3 106)
				(TPrint 510 37 #at -1 185) ; "(Nice going, Larry!)"
			)
			(20
				(maitreD
					init:
			...
			(25
				(Say the_maitre_d 510 38 #dispose #caller self) ; "There you are, Sir!" barks the maitre d'. "Enjoy your visit here at the lovely Hard Disk Cafe!"
			)
			(26
				(maitreD
					view: 515
...
(instance sDoubleTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say the_maitre_d 510 39 #dispose #caller self) ; "I'm sorry, sir," he replies, "but admittance to the exclusive `Herman Hollerith Room' is limited to only our very finest customers."
			)
			(1
				(Say the_maitre_d 510 40 #dispose) ; ""(...which you are not!)" he concludes under his breath."
				(HandsOn)
				(self dispose:)
			)
		)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				...
				(if (gEgo has: 14) ; Membership_Tape
					(TPrint 510 51 #at -1 185) ; "(How interesting: the music box's cylinder appears to be the same width as your membership tape.)"
				else
			...
			(3 ; Do
				...
				(TPrint 510 53 #at 100 10) ; "The music box is either very old and very tired, or some practical joker has altered the pin placement on its cylinder. It sounds terrible!"
			)
			(5 ; Talk
...
(instance maitreD of Actor
	...
	(method (cue)
		(super cue:)
		(TPrint 510 58 #at -1 185) ; "(You wonder if his French accent could be fake?)"
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				...
				(if (gEgo has: 14) ; Membership_Tape
					(Say the_maitre_d 510 61 #dispose) ; ""Just use the membership tape I created for you," he replies."
				else
					(Say the_maitre_d 510 62 #dispose) ; "Please keep your hands off me," says the maitre d'. "I'm for the use of members only!"
				)
			)
			(5 ; Talk
				(gCurRoom setScript: sMD)
			...
			(4 ; Inventory
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
					(Say the_maitre_d 510 63 #dispose) ; "Why don't you get your tape out of the machine?"
				else
					(switch invItem
						(12 ; Money
							(HandsOff)
						...
						(13 ; Credit_Cards
							(if (== ((Inv at: 14) state:) 1) ; Membership_Tape
								...
								(Say the_maitre_d 510 65 #dispose) ; "But, of course, Sir!" he replies, snapping to attention. "I always have a soft spot for a fine fellow like you!"
								(Points 5 108)
								...
								(Say the_maitre_d 510 66 #dispose) ; "You might want to save these, Sir," he replies. "You may need them later."
							)
						)
						(6 ; Hard_Disk_Cafe_Napkin
							...
							(Say the_maitre_d 510 68 #dispose #caller self) ; ""Wee, wee, moisture!" he replies."
						...
						(11 ; DayTrotter
							...
							(Say the_maitre_d 510 70 #dispose) ; ""Hardly," he sniffs."
						)
						(else
							(super doVerb: theVerb invItem &rest)
						)
