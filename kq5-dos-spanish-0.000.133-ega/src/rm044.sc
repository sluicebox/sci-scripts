...
(local
	...
	local4
	[local5 2]
	[local7 34] = [235 0 198 73 236 93 230 124 170 152 89 136 87 126 179 109 179 96 96 76 147 65 121 65 92 58 81 52 188 42 173 40 174 0]
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(gEgo view: 0 normal: 1 setCycle: SyncWalk cycleSpeed: 0)
				((gEgo head:) show:)
				(SetScore 2)
				(= cycles 1)
			)
...
(instance beach of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 44 4) ; "A narrow strand of beach skirts the bottom of the steep cliffs while the cold water of the ocean seems to cunningly steal what little is left."
					(event claimed: 1)
				)
			)
		)
...
(instance waterFall of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 44 5) ; "Further up the beach, Graham can see a lovely waterfall cascading down the cliffs and falling into the cold, blue ocean."
					(event claimed: 1)
				)
			)
		)
...
(instance ironBar of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 44 6) ; "Graham notices a rusty iron bar lying on the sand near the bottom of the winding path."
					(event claimed: 1)
				)
				(3 ; Do
					(gEgo setScript: getBar)
...
(instance smallBoat of View
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 44 7) ; "Graham notices a boat in the distance."
					(event claimed: 1)
				)
			)
		)
