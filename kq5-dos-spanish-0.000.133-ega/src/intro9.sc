	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(PrintDC 108 0 #at 10 10 #dispose) ; "Parece que el polvo m\a0gico ha dejado de funcionar."
				(= seconds 3)
			)
			(6
			...
			(8
				...
				(PrintDC 108 1 #at 100 10 #dispose) ; "\adCedric! \a8D\a2nde estabas? \adTe he estado llamando!"
			)
			(9
			...
			(14
				(wizHead setCycle: Fwd)
				(PrintDC 108 2 #at 100 10 #dispose) ; "Bueno, bueno \a8qu\82 tenemos aqu\a1?"
				(= seconds 5)
			)
			(15
				(cls)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(PrintDC 108 3 #at 10 110 #dispose) ; "\a8Te has mojado un poco, no?. Bueno, entra en casa y s\82cate. No tiene sentido estar sentado por ah\a1 como un perro mojado."
				(= seconds 10)
			)
			(2
				(cls)
	...
	(method (changeState newState &tmp [temp0 100])
		(switch (= state newState)
			...
			(2
				...
				(wizHead setCycle: Fwd)
				(PrintDC 108 4 #at 100 10 #dispose) ; "Cedric, entra en la casa y preparanos una taza de t\82 caliente."
				(= seconds 5)
			)
			(3
				(cls)
				...
				(crispin setCycle: 0)
				(owlMouth setCycle: Fwd)
				(PrintDC 108 5 #at 10 120 #dispose) ; "Voy, voy, Crispin."
				(= seconds 4)
			)
			(4
				(cls)
