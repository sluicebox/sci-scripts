...
(instance rm031 of KQ5Room
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(else
				(gEgo view: 10 posn: 312 156 illegalBits: 0 setPri: 11 init:)
				(= local0 8)
...
(instance step1 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 5) ; "Graham can see tempting rock outcroppings protruding from the frozen waterfall."
					(event claimed: 1)
				)
				(3 ; Do
...
(instance step2 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 6) ; "Graham can see tempting rock outcroppings protruding from the frozen waterfall."
					(event claimed: 1)
				)
				(3 ; Do
					(if
...
(instance log of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 7) ; "An old log has fallen precariously across a washed-out portion of the narrow trail."
					(event claimed: 1)
				)
			)
		)
...
(instance waterFall of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 8) ; "As with the path below, the now-frozen waterfall has also washed away parts of this upper trail which skirts the mountain edge and then heads off to the east."
					(event claimed: 1)
				)
			)
		)
...
(instance rope of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 31 9) ; "Graham's rope dangles from a rocky overhang down to the path below."
					(event claimed: 1)
				)
				(3 ; Do
					(if (== local0 0)
						...
						(PrintDC 31 10) ; "Graham can't reach the rope from here."
					)
					(event claimed: 1)
				)
			)
