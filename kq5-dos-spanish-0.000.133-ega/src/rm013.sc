...
(instance getTambourine of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(PrintDC 13 9) ; "Not seeing the tambourine's owner, Graham bends down and rescues it from the ground."
				(gEgo
					normal: 1
					view: 0
					setCycle: Walk
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 10) ; "A gypsy wagon is encamped here at the edge of the woods. On the side of the wagon, the words "Fortune Teller" have been painted."
					(event claimed: 1)
				)
			)
		)
...
(instance door of Prop ; UNUSED
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 11) ; "The wagon door looks inviting to those who wish to have their fortunes told."
					(event claimed: 1)
				)
				(3 ; Do
...
(instance ox of Prop
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2
					(PrintDC 13 12) ; "A large ox, tied to the gypsies' wagon, peacefully munches on grass."
				)
				(3
					(PrintDC 13 13) ; "Graham finds the huge beast completely uninteresting."
				)
				(5
					(PrintDC 13 14) ; "The ox's mouth is too full of grass to even attempt a conversation with Graham!"
				)
				(4
					(switch invItem
						(28 1)
						(else
							(PrintDC 13 15) ; "The ox is not interested in any of Graham's possessions."
						)
					)
				)
			)
...
(instance body of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 16) ; "Within the gypsy encampment, Graham notices a dark, burly man who keeps a suspicious eye on him and Cedric."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 17) ; "The burly man keeps a watchful eye on Graham and Cedric."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(11
							(if (gEgo has: 27) ; Amulet
								(proc0_28 103 13 18 67 10 10 25 10) ; "Madame Mushka eez tired. No more for today."
							else
						...
						(4
							(proc0_28 103 13 19 67 10 10 25 10) ; "Eet costs one GOLD coin to see Madame Mushka, not one SILVER coin!"
							(event claimed: 1)
						)
						(28
							(event claimed: 0)
				...
				(5 ; Talk
					(PrintDC 13 20) ; "The sullen man doesn't look like the conversational sort."
					(event claimed: 1)
				)
			)
		)
				...
				(4 ; Inventory
					(if (not (gCast contains: gypsy))
						(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
							...
							(else
								(proc0_28 103 13 21) ; "Zat eez NOT a gold coin!"
								(event claimed: 1)
							)
						)
					)
...
(instance gypsy of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 22) ; "The dark gypsy man bars Graham's entry into the fortune teller's wagon without proper payment."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 17) ; "The burly man keeps a watchful eye on Graham and Cedric."
...
(instance coals of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 23) ; "A steaming pot of stew bubbles over a small campfire near the gypsy wagon."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 24) ; "Obviously, the man wouldn't tolerate Graham taking his supper."
					(event claimed: 1)
				)
			)
		)
...
(instance grassRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 25) ; "Graham notices a grassy clearing at the edge of the woods."
					(event claimed: 1)
				)
			)
		)
...
(instance tambourine of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 26) ; "A tambourine lies on the ground near the abandoned gypsy encampment."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
