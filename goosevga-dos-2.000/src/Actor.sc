...
(class Actor of Prop
	...
	(method (say param1 &tmp temp0 temp1)
		...
		(if global119
			(DoAudio audWPLAY [param1 0])
			(= temp1 (if head head else self))
			(temp1 setCycle: MouthSync [param1 0])
			(= global171 [param1 0])
			...
			(= temp1 (if head head else self))
			(temp1 setCycle: MouthSync [param1 0])
			(gSystemWindow speakingObj: self)
			(Print 900 param1 #dispose self)
			(= temp0 900)
		)
		(if (and (< global170 90) (== global169 1) global119)
			(+= global248 temp0)
			(temp1 doit:)
		)
		(return temp0)
		...
		(cond
			...
			((or (== (event type:) evVERB) (== (event type:) evMOUSEBUTTON))
				(if
					...
					(if (and (< 1 gTheCursor 6) (IsObject global345))
						(global345 doit:)
					else
