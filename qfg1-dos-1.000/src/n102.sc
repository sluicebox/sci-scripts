...
(procedure (ThrowRock atWhat &tmp temp0 temp1)
...
(instance rockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(client hide:)
				(if (and register (< -15 local1 15) (< -15 local2 15))
					(= local10 1)
					(local9 play: self)
				else
					(= cycles 1)
			...
			(3
				(NormalEgo)
				(gEgo priority: local6 illegalBits: local7 signal: local5)
				(client dispose:)
			)
		)
	)
