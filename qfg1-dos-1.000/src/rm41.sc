			...
			(evSAID
				(cond
					...
					((or (Said 'nap') (Said 'go[<to]/nap'))
						(HighPrint 41 12) ; "You barely get to sleep when the guard on night patrol kicks you out."
						(if (< 750 gClock 2550)
							(FixTime 21)
						)
						(gCurRoom newRoom: 37)
		...
		(switch (event type:)
			(evMOUSEBUTTON
				(if (MouseClaimed lGuard event 3)
					(HighPrint 41 13) ; "Plays a mean horn. A member in good standing of the musicians guild."
				)
			)
			(evSAID
				(cond
					((super handleEvent: event))
					((Said 'climb[/castle,wall]')
						(HighPrint 41 14) ; "You don't think the guards would allow you to do so."
					)
					...
					((Said 'look/horn')
						(HighPrint 41 16) ; "The horns are tarnished and starting to corrode."
					)
					((Said 'look/guard')
						(HighPrint 41 17) ; "The guards look to be middle-aged, but in good physical condition. Their clothing is faded and patched. Even the horns look dull."
					)
					((Said 'talk[<to]/guard,man,gatekeeper')
						(HighPrint 41 18) ; "Go ahead."
					)
					((Said 'talk,ask/moustache')
						(= local4 11)
					...
					(local2
						(cond
							...
							((Said 'talk,ask>')
								(event claimed: 1)
								(HighPrint 41 19) ; "The guards are ignoring you."
							)
							(else
								(event claimed: 0)
							)
		...
		(switch (event type:)
			(evMOUSEBUTTON
				(if (MouseClaimed rGuard event 3)
					(HighPrint 41 20) ; "He's very proud of his moustache, but don't mention his bald spot"
				)
			)
		)
	)
		...
		(switch (event type:)
			(evMOUSEBUTTON
				(if
					...
					(HighPrint 41 21) ; "Doorway to castle great hall."
				)
			)
		)
	)
		...
		(switch (event type:)
			(evMOUSEBUTTON
				(if (and (not local0) (MouseClaimed torch event 3))
					(HighPrint 41 22) ; "The torch lights the way to the Baron's Hall."
				)
			)
		)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((or (MouseClaimed onTrees1 event 3) (MouseClaimed onTrees2 event 3))
				(HighPrint 41 23) ; "You can't reach the windows from these trees."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(HighPrint 41 24) ; "Weeds grow profusely, but nobody notices."
			)
			(else
				(event claimed: 0)
			)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onCastle event 3)
				(HighPrint 41 25) ; "This is the castle of the Baron Stefan von Spielburg."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(switch local1
					(0
						(HighPrint 41 26) ; "These flagstones are from a rock quarry in eastern Germany."
						(++ local1)
					)
					(1
						(HighPrint 41 27) ; "Well...maybe the flagstones are from western Germany."
						(++ local1)
					)
					(2
						(HighPrint 41 28) ; "Flagstones from Europe?"
						(++ local1)
					)
					(3
						(HighPrint 41 29) ; "Granite from our very own mountains?"
						(++ local1)
					)
					(else
						(HighPrint 41 30) ; "I don't know!"
						(= local1 0)
					)
				)
			)
...
(instance rGuardTrumpets of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TimePrint 6 41 31) ; "The Baron von Spielburg and Baronet await you in the great hall."
				(= cycles 30)
			)
			(4
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(switch local3
					(1
						(HighPrint 41 32) ; "No one enters the castle without the Baron's permission."
					)
					(2
						(HighPrint 41 33) ; "Hay is for horses."
					)
					(3
						(HighPrint 41 34) ; "I'm here! What do you want?"
					)
					(4
						(HighPrint 41 35) ; "Says you."
					)
					(5
						(HighPrint 41 36) ; "This is the castle of the Baron Stefan von Spielburg."
					)
					(6
						(HighPrint 41 37) ; "The Baron's daughter has been gone for years."
					)
					(7
						(HighPrint 41 38) ; "I'm Frederick."
						(= local4 7)
						(self setScript: rGuardTalks)
					)
					(8
						(HighPrint 41 39) ; "The barracks are off to my right, but you have no business going there."
					)
					(9
						(HighPrint 41 40) ; "I told you not to mention it. Now you've hurt his feelings, and he won't talk until he gets over it."
					)
					(10
						(HighPrint 41 41) ; "I can't help you on that."
						(= local4 10)
						(self setScript: rGuardTalks)
					)
					(11
						(HighPrint 41 42) ; "Looks like meathead is sulking. If I talk to you anymore he'll get even with me."
						(= local4 12)
						(self setScript: rGuardTalks)
					)
					(12
					...
					(13
						(HighPrint 41 43) ; "Karl is the gatekeeper. He is a good one to answer questions."
					)
				)
				(lGuardHead dispose:)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(switch local4
					(1
						(HighPrint 41 44) ; "The Baron sees no one."
					)
					(2
						(HighPrint 41 45) ; "We cannot leave our post."
					)
					(3
						(HighPrint 41 46) ; "You will have to see the Baron to get his permission."
					)
					(4
						(if (IsFlag 170)
							(HighPrint 41 47) ; "The Baron is busy and will see no one."
						else
							(HighPrint 41 44) ; "The Baron sees no one."
						)
					)
					(5
						(if (IsFlag 170)
							(HighPrint 41 48) ; "The Baronet is in the castle recovering from his ordeal and will see no one."
						else
							(HighPrint 41 49) ; "The Baronet Barnard von Spielburg is missing."
						)
					)
					(6
						(HighPrint 41 50) ; "The stable is off to my left."
					)
					(7
						(HighPrint 41 51) ; "My name is Pierre. I'm one of the Baron's personal guards."
						(= local5 0)
					)
					(8
						(HighPrint 41 52) ; "It is not our place to answer questions."
					)
					(9
						(HighPrint 41 53) ; "Spot? What spot?"
					)
					(10
						(HighPrint 41 54) ; "Why don't you ask Karl?"
						(= local5 0)
					)
					(11
						(HighPrint 41 55) ; "Why thank you. Your manners have improved."
					)
					(12
						(HighPrint 41 56) ; "Don't get me mad."
						(= local5 0)
					)
					(13
						(HighPrint 41 44) ; "The Baron sees no one."
