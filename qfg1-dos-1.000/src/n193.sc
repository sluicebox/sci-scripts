			...
			(cond
				...
				((Said '/cabinet,box,table')
					(cond
						...
						((gEgo inRect: 276 143 320 177)
							(if ((ScriptID 94 0) notify: 1) ; rm94
								(HighPrint 193 28) ; "You don't have much time to look at toys."
							else
								(HighPrint 193 29) ; "The cabinet seems to have some kind of hasp."
							)
						)
						(else
							(HighPrint 193 30) ; "Get close for a good look."
						)
					)
				)
			)
		...
		((Said 'open/jack,box,toy,hasp,cabinet')
			(cond
				(((ScriptID 94 0) notify: 1) ; rm94
					(HighPrint 193 36) ; "It's already open."
				)
				((gEgo inRect: 276 143 320 177)
					((ScriptID 94 15) setScript: (ScriptID 192 0)) ; jackCycle, jackJumps
				)
				(else
					(HighPrint 193 37) ; "You're not close enough"
				)
			)
		)
