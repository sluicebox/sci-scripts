...
(instance rm83 of Rm
	...
	(method (init)
		...
		(if (IsFlag 73)
			(Say hermitTalk 83 6) ; "Come in, come in. Good to see you again.  Get's kind o' tiresome sitten here an' thinken by meself all the time."
		else
			...
			(evSAID
				(cond
					...
					((Said 'look[<around,at][/room]')
						(HighPrint 83 7) ; "The cave is cold and clammy and it smells like mildew and wet dog hair in here!"
					)
					((Said 'look>')
						(cond
							((Said '/cave,wall')
								(HighPrint 83 8) ; "The walls are covered with phosphorescent fungus."
							)
							((Said '/fungus')
								(HighPrint 83 9) ; "The phosphorescent fungus glows eerily in the cave's dim light."
							)
							((Said '/scroll,spell')
								(if (gCast contains: scroll)
									(HighPrint 83 10) ; "It's on the table."
								else
									(HighPrint 83 11) ; "There's nothing like that here."
								)
							)
						)
					)
					...
					((Said 'ask>')
						...
						(cond
							...
							((or (Said '/trigger') (Said 'ask/spell<trigger'))
					...
					((Said 'give,give/ration')
						(if (gEgo has: 3) ; food
							...
							(HighPrint 83 14) ; "You can't share what you don't have."
						)
					)
					(
						(or
						...
						(cond
							(local0
								(HighPrint 83 15) ; "You can't sleep in this old rickety chair. Get up and sleep on the bed of straw."
							)
							((> (gEgo x:) 165)
								(HighPrint 83 16) ; "Get closer to the bed of straw."
							)
							((or (IsFlag 338) (>= [gInvNum 3] 1)) ; food
								(if (CanSleep)
									(if (IsFlag 338)
										(ClearFlag 338)
									else
					...
					((Said 'sat/')
						(if (not local0)
							(if (gEgo inRect: 140 150 174 174)
								...
								(HighPrint 83 17) ; "You need to get closer to the chair."
							)
						else
							(HighPrint 83 18) ; "You are already sitting... on your brain, perhaps?"
						)
					)
					((or (Said 'stand,get<up/') (Said 'stand/'))
						(if local0
							...
							(HighPrint 83 19) ; "You're not sitting."
						)
					)
					((Said 'put,extinguish/candle,flame')
						(HighPrint 83 20) ; "The 'ermit wouldn't like that."
					)
					((Said 'search/')
						(HighPrint 83 21) ; "You should wait until the hermit is asleep to check his place out."
					)
					((Said 'play/cribbage')
						(cond
							((== gDay global590)
								(HighPrint 83 22) ; "Despite your intentions, you can't bring yourself to spend any more time playing with the talkative 'Enry."
							)
							(local0
								(= global590 gDay)
								(EgoRests 60 0)
								(HighPrint 83 23) ; "You spend an hour at the game while 'Enry talks. You've obviously made his day."
							)
							(else
								(HighPrint 83 24) ; "You should sit down first."
							)
						)
					)
					((Said 'get/scroll')
						(if (gCast contains: scroll)
							(scroll dispose:)
							(gEgo get: 10) ; note
							(HighPrint 83 25) ; "The scroll vanishes even as you read the magical runes upon it.  You now have the knowledge to cast a "Trigger" spell."
							(SetFlag 262)
							(gEgo learn: 19 10) ; triggerSpell
							(SolvePuzzle 619 4 1)
						else
							(HighPrint 83 26) ; "You see no scroll."
						)
					)
					((or (Said 'cast/detect') (Said 'cast/spell<detect'))
						(if (CastSpell 18)
							(HighPrint 83 27) ; "You can detect a faint aura of magic here."
						)
					)
					((or (Said 'cast/open') (Said 'cast/spell<open'))
						(if (CastSpell 17)
							(HighPrint 83 28) ; "There is no need to use magic to open anything here."
						)
					)
					((Said 'get,grab/candle,cot,hay,bed,table,chair')
						(HighPrint 83 29) ; "It's not polite to steal from your 'airy little 'ost."
						(HighPrint 83 30) ; "You seem to have made the 'ermit un'appy."
						(SetFlag 240)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(HandsOff)
				(Say hermitTalk 83 31) ; "Me guest bed is the straw in the corner over there. You're welcome to stay if you share some rations and play some cribbage."
			)
			(1
				(HandsOn)
			)
			...
			(switch (= state newState)
				(0
					...
					(if [gEgoStats 12] ; magic
						(Say hermitTalk 83 32) ; "If you be wanting to learn the spell, I'll be pleased to share it to you. It's just getting molding in me 'iding place."
					else
						(Say hermitTalk 83 33) ; "I think you be 'avin too little magic for it to be any use to you."
						(HandsOn)
						(client setScript: 0)
					)
				)
				(1
					(Say hermitTalk 83 34) ; "Do you want it?"
				)
				(2
					(User canInput: 1)
					(= seconds 6)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(Say hermitTalk 83 35) ; "Oh well, maybe sumone else can use it sumtime."
			)
			(1
				(HandsOn)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(HandsOff)
				(if (IsFlag 262)
					(Say hermitTalk 83 36) ; "You already 'ave me only scroll."
				else
					(Say hermitTalk 83 37) ; "Yes, I still have me magic scroll, but you didn't seem to want it. 'Ave you changed your mind?"
				)
			)
			(1
				(User canInput: 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(hermitTalk caller: self)
				(Say hermitTalk 83 38) ; "Let's see. 'Ow does Erasmus do it? Oh, yes."
			)
			(1
				(Say hermitTalk 83 39) ; "'E summons it with a Trigger. 'Ere goes!"
			)
			(2
				(scroll
					loop: 7
			...
			(3
				...
				(Say hermitTalk 83 40) ; "There you 'as it."
			)
			(4
				(HandsOn)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(Say hermitTalk 83 41) ; "I don't 'ave much, but wot I 'ave, I 'ave, an' t'isn't for sale."
			)
			(1
				(HandsOn)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(Say hermitTalk 83 42) ; "Well, see'en as I don't get out much, I don't know much."
			)
			(1
				(HandsOn)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(hermitTalk caller: self)
				(HandsOff)
				(Say hermitTalk 83 43) ; "If you be ever needen a place to stay the night and you're in the area, I could be willen to put you up for the night for sum rations and a couple o' games o' cribbage."
			)
			(1
				(gCurRoom newRoom: 82)
			)
...
(instance sGoSleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(HighPrint 83 44) ; "After having traded a ration, which the 'ermit promptly swallowed, and spending a rather boring hour of cribbage and listening to 'Enry chatter, you hit the hay."
			...
			(3
				(HighPrint 83 45) ; "Unfortunately, since the fungus on the walls shines at night so the room doesn't get dark, and the fact the hermit talks most of the night, you only get five hours rest."
				(EgoSleeps 5)
				(SetFlag 73)
				(gCurRoom newRoom: 82)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(hermitTalk caller: self)
				(Say hermitTalk 83 46) ; "You can't sleep here, you got's no rations to give me."
			)
			(1
				(SetFlag 241)
