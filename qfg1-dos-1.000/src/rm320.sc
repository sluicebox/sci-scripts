	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look>')
				...
				(cond
					((Said '[<at,around][/!*,room,street,hamlet]')
...
(instance centaur of Prop
	...
	(method (handleEvent event)
		(cond
			((Said 'talk')
				(if (gEgo inRect: 210 130 319 185)
					(HighPrint 297 (Random 15 19))
				)
			)
			((Said 'yes')
				(cond
			...
			((Said 'ask>')
				...
				(cond
					((Said '/name')
						(HighPrint 320 25) ; "I am Hilde, daughter of Heinrich Pferdefedern, the farmer."
					)
					((Said '/centaur')
						(HighPrint 320 26) ; "My father and I are the only centaurs in this valley."
					)
					((Said '/carrot,food')
						(HighPrint 320 27) ; "We have many kinds of fresh vegetables for sale today. There are lettuce, carrots, potatoes, and turnips right from the ground at five for a silver. They are very good for you!"
					)
					((Said '/beet')
						(HighPrint 320 28) ; "It is much too early in the season for those. Sorry."
					)
					((Said '/beet')
						(HighPrint 320 29) ; "I'm sorry, but the cherry harvest is not yet in. But we will have very nice cherries two months from now."
					)
					((Said '/apple,apple')
					...
					((Said '/bandit,robbery')
						(HighPrint 320 31) ; "They have twice robbed my Father of his money, and they have tried to steal our food this winter. That is because we farm outside of town. They would not dare to try that in town. The Sheriff would stop them fast!"
						(HighPrint 320 32) ; "Many robbers have run up to Father as he trotted back to town. My Father tried to fight, but they hurt him badly. They always ran away to the west."
					)
					((Said '/farm,farmer')
						(HighPrint 320 33) ; "We have some land to the north of town. It isn't very big, but we grow many things. You should be here during the harvest time. Then you would see many fruits and vegetables."
					)
					((Said '/harvest')
						(HighPrint 320 34) ; "You know...in the autumn! But then, perhaps you have never been a farmer."
					)
					((Said '/healer')
						(HighPrint 320 35) ; "The Healer lives in a nice little cottage on the North road, near my Father's fields. She is a very nice woman."
					)
					((Said '/date,kiss')
						(HighPrint 320 36) ; "Thank you, but my Father thinks I am too young. He would not permit it. But perhaps next year..."
					)
					((Said '/dad,heinrich')
						(HighPrint 320 37) ; "My Father is Heinrich Pferdefedern, who works our farm just north of town."
					)
					((Said '/*')
						(= local7 0)
						(HighPrint 320 38) ; "I am sorry. I do not get about very much, and I do not really know about many things."
					)
				)
				(if local7
					(SolvePuzzle 729 1)
			...
			((Said 'buy,get>')
				(cond
					((not (gEgo inRect: 210 130 319 185))
						(event claimed: 1)
						(HighPrint 320 39) ; "If you want to buy something, go over to the vendor."
					)
					((Said '/carrot,food')
						(if (Purchase 1)
							(HighPrint 320 40) ; "You pick out the freshest-looking vegetables you can find, and pay the lady a silver piece."
					...
					((Said '/apple,apple')
						(if (Purchase 1)
							(HighPrint 320 41) ; "You carefully select ten of the best apples from the barrel, and pay the lady a silver piece."
					...
					((Said '/beet')
						(HighPrint 320 42) ; "As I told you, sir, it is too early in the season. You would not want to buy any."
					)
					((Said '/*')
						(HighPrint 320 43) ; "I'm afraid I can't help you with that, sir."
					)
				)
			)
			((Said 'pay')
				(HighPrint 320 44) ; "Just pick out what you would like to purchase, then you can pay me."
			)
			((MouseClaimed onCentaur event 3)
				(HighPrint 320 45) ; "The creature in charge of the Farmer's Mart is a rather lovely and shy-looking young Centaur."
			)
			((MouseClaimed onBarrel1 event 3)
				(HighPrint 320 46) ; "The barrel contains potent looking potatoes more suited to throwing than eating."
			)
			((MouseClaimed onBarrel2 event 3)
				(HighPrint 320 47) ; "The small apples give off a sweet but pungent odor."
			)
			((MouseClaimed onCarrots event 3)
				(HighPrint 320 48) ; "Carrots are good for night vision and lower one's cholesterol."
			)
			((MouseClaimed onTurnips event 3)
				(HighPrint 320 49) ; "The turnips seem to blush with pride. You wonder if the farmer has been using Alar."
			)
			((or (MouseClaimed onGreens1 event 3) (MouseClaimed onGreens2 event 3))
				(HighPrint 320 50) ; "Lettuce and other leafy plants."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not (IsFlag 94))
					(User canControl: 0)
					(HighPrint 320 51) ; "You can smell apples as you approach this corner."
					(= cycles 12)
				)
			)
			(2
				(HighPrint 320 52) ; "The pretty young centaur looks at you and says, "Good day, and welcome to Spielburg! Do you wish to buy some nice fresh fruits or vegetables?""
				(User canControl: 1)
				(= local3 1)
			)
		)
