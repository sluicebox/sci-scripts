		...
		(cond
			...
			((== (event type:) evSAID)
				(cond
					...
					((Said 'ask>')
						(cond
							((Said '/man,fighter,giant,brauggi')
								(localproc_1)
							)
							(
								(Said
							...
							(
								(Said
									'/fight,ax,weapon,blade,(attack[<blade,about])'
								)
				...
				(cond
					...
					((Said 'look>')
						(cond
							...
							((Said '/gem')
								(if (or (not (IsFlag 208)) (gEgo has: 13)) ; magic gem
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(HandsOn)
				(if (not (IsFlag 48))
					(localproc_0 58 19) ; "Far from the frost field, fares forth this fighter, Hunger has hurled me hither from home. My name it is known in the Northlands as Brauggi, Barter with blade's clash, or bargain with me."
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(localproc_0 58 20) ; "Fruit you have found, to fill all my food stores, Thus filled the bargain, my gem you have bought. Brauggi has bartered, and all has been answered; The mead it may mellow, and now I head home."
				(SolvePuzzle 679 8)
				(= seconds 2)
			)
			(1
...
(instance Challenge of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 58 21) ; "Fiercer than frost's bite, my fury is fearless, My axe it is edged as is the ice. This warrior is willing to wield now the war drum, The battleaxe blazes and bellows for blood."
				(client setScript: giantBlocks)
			)
		)
	)
...
(instance giantFights of Script
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		(switch (= state newState)
			...
			(9
				(localproc_0 58 23) ; "Courage you carry, but skill it is scanty; Face not this foe before fiercer you fight."
				(HandsOn)
				(client setScript: giantBlocks)
			)
		)
			...
			(1
				(if (gEgo has: 11) ; apple
					(cond
						...
						((>= global329 40)
							(LowPrint 58 24) ; "Your fruit definitely makes a generous pile in the Giant's cupped hands, but they're not full yet."
						)
						((== global329 0)
							(HighPrint 58 25) ; "Oops!" you say. "I guess I don't have any fruit for you. Sorry!"
						)
						(else
							(LowPrint 58 26) ; "Your meagre selection of fruit makes a paltry pile at the bottom of the Giant's cupped hands. You'll need to get quite a few more apples to fill THOSE hands!"
						)
					)
				else
					(HighPrint 58 25) ; "Oops!" you say. "I guess I don't have any fruit for you. Sorry!"
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if local1
					(TimePrint 4 58 27) ; "As you start to throw..."
				else
					(TimePrint 4 58 28) ; "As you begin to cast your spell..."
				)
				(= seconds 2)
			)
			(1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(HandsOn)
				(NormalEgo)
				(HighPrint 58 30) ; "Running on the ice is apparently not a very good idea."
				(self dispose:)
			)
		)
	)
