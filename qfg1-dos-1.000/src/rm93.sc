...
(instance rm93 of Rm
	...
	(method (init)
		...
		(LoadMany rsSOUND 23 (SoundFX 35) (SoundFX 39) (SoundFX 31))
		(LoadMany rsSCRIPT 103 102 101)
	...
	(method (handleEvent event)
		(switch (event type:)
			(evMOUSEBUTTON
				(cond
					((super handleEvent: event))
					((MouseClaimed minotaur event 3)
						(HighPrint 93 39) ; "The Minotaur seems to have very sensitive hearing. He looks around every time the bush rustles."
					)
				)
			)
			(evSAID
				(cond
					((Said 'search[/bull,body,flail]')
						(cond
							(local6
								(HighPrint 93 40) ; "As you attempt the search, the sleeping Minotaur awakes, and..."
								(gCurRoom newRoom: 425) ; minotaurArena
							)
							((IsFlag 237)
								(if local10
									(HighPrint 93 41) ; "You've got all the loot you're going to find."
								else
									(= local10 1)
									(HighPrint 93 42) ; "You quickly search the dead Minotaur and find nothing."
									(HighPrint 93 43) ; "Wait a minute."
									(HighPrint 93 44) ; "The Minotaur's flail looks peculiar. You pick it up and examine it closely."
									(HighPrint 93 45) ; "Cleverly concealed in the flail are 50 silvers."
									(HighPrint 93 46) ; ""A just reward for defeating such a valiant fighter," you tell yourself as you pocket the silvers."
									(gEgo get: 1 50) ; silver
								)
							)
							(else
								(HighPrint 93 47) ; "That's not a good idea."
							)
						)
					)
					((Said 'look/bull,monster,creature')
						(if (IsFlag 237)
							(HighPrint 93 48) ; "If you've seen one, you've seen them all."
						else
							(HighPrint 93 49) ; "Good ears but very poor peripheral vision."
						)
					)
					((Said 'kill,cut,hit/bull,monster,creature')
						(if (IsFlag 237)
							(HighPrint 93 50) ; "Why bother?"
						else
							(HighPrint 93 51) ; "OK. Here's your chance."
						)
						(gCurRoom newRoom: 425) ; minotaurArena
					)
				)
		...
		(switch (= state newState)
			...
			(3
				(if
					...
					(switch local11
						(0
							(HighPrint 93 52) ; "I've been out here too long. My imagination is taking over."
						)
						(1
							(HighPrint 93 53) ; "What was that noise?"
						)
						(2
							(HighPrint 93 54) ; "Must be those Brigands playing games."
						)
						(3
							(HighPrint 93 55) ; "I guess I'm just getting jumpy."
						)
						(4
							(HighPrint 93 56) ; "Is anyone there?"
						)
						(5
							(HighPrint 93 57) ; "This place is spooky."
						)
					)
...
(instance jerkBush of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (and (not (IsFlag 237)) (not local6))
					(minotaur setScript: minotaurLooks)
				)
				(= seconds 17)
			)
...
(instance openMess of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(= local9 1)
				(HighPrint 93 58) ; "You hear a snick as the hasp on the gate is opened."
