...
(instance rm22 of Rm
	...
	(method (handleEvent event &tmp [temp0 50])
		(cond
			...
			((proc802_2 event {hut of brown})
				(cond
					((IsFlag 172)
						(HighPrint 22 6) ; "Nothing happens."
					)
					((proc802_1 (User inputLineAddr:) {now sit down})
						(SolvePuzzle 657 7)
						(if (not (hut script:))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(14
				...
				(hut setScript: 0)
				(if local3
					(EgoDead 22 69 80 { Bad strategy! } 82 516 3 7) ; "You really brought down the house! Too bad you were under it at the time."
				)
			)
		)
	)
