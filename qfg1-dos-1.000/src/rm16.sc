	...
	(method (handleEvent event &tmp temp0)
		(switch (event type:)
			...
			(evSAID
				(cond
					((Said 'run,sneak')
						(HighPrint 16 4) ; "That would not be appropriate here."
					)
					...
					((Said 'look>')
						(cond
							((Said '/north,west')
								(HighPrint 16 5) ; "To the north and west are steep cliffs.  There are some unusual plants growing out of the side of the cliffs."
							)
							((Said '/south,east')
								(HighPrint 16 6) ; "To the south and east is the forest from which you came."
							)
							((Said '[<at,around][/!*,forest]')
								(HighPrint 16 7) ; "To the south and east is the forest from which you came. To the north and west are steep cliffs. There are some unusual plants growing out of the side of the cliffs."
								(if (not (IsFlag 192))
									(HighPrint 16 8) ; "They are spitting what seems to be some kind of seed."
								)
							)
							((Said '/cliff,boulder')
								(HighPrint 16 9) ; "The cliff walls are very steep, and they look difficult to climb."
							)
							((Said '[<down][/ground,needle,moss,grass]')
								(HighPrint 16 10) ; "The meadow clearing in which you stand is grassy and littered with blown pine needles and small mossy rocks."
							)
							((Said '[<up][/sky,cloud,star]')
								(if gNight
									(HighPrint 16 11) ; "The evening is clear and the stars are bright. Dark clouds pass over the moon."
								else
									(HighPrint 16 12) ; "The sky is a piercing blue with scudding white clouds."
								)
							)
							((Said '/birch,tree')
								(HighPrint 16 13) ; "You can see pines, cedars, birches and other trees frequently associated with mountain forests."
							)
							((Said '/seed')
								(cond
									((gEgo has: 20) ; seed
									...
									((IsFlag 192)
										(HighPrint 16 14) ; "You can't see it."
									)
									(else
										(HighPrint 16 15) ; "The large seed spins as it travels through the air."
									)
								)
							)
							((Said '/leaf')
								(HighPrint 16 16) ; "The leaves look tough and fibrous."
							)
						)
					)
					(
						...
						(switch global326
							(1
								(HighPrint 16 17) ; "As you attempt to draw your weapon, you lose your balance momentarily."
								(gEgo setScript: (ScriptID 291 2)) ; fallDown
							)
							(3
								(HighPrint 16 3) ; "Not now!"
							...
							(0
								(cond
									...
									(else
										(HighPrint 16 18) ; "If you had a sword, you might be able to do that."
									)
								)
							)
						)
					...
					((Said 'climb[<up][/cliff,boulder]')
						(cond
							((>= global326 1)
								(HighPrint 16 19) ; "You don't think you can climb any further up the cliff."
							)
							((IsFlag 136)
								(localproc_1)
							)
					...
					((Said 'climb,get<down')
						(cond
							((and (> global326 1) (== global327 0))
								(HighPrint 16 20) ; "Wait a second!"
							)
							((>= global326 1)
								(gEgo setScript: (ScriptID 292 3)) ; climbDown
							)
							(else
								(HighPrint 16 21) ; "You're already on the ground."
							)
						)
					)
					((Said 'capture[/seed]')
						(cond
							((IsFlag 192)
								(HighPrint 16 22) ; "There's not much point to that now."
							)
							((IsFlag 136)
								(localproc_1)
							)
							...
							((or (== global326 2) (== global326 3))
								(HighPrint 16 23) ; "Why do you think you're standing like this. Trying to catch Flying Water?"
							)
							(else
								(HighPrint 16 24) ; "You can't catch the seed from down on the ground."
							)
						)
					)
						...
						(cond
							...
							((Said '/seed')
								(if (IsFlag 136)
									...
									(HighPrint 16 25) ; "It's not that easy."
								)
							)
						)
					)
						...
						(cond
							((Said '/boulder,brick')
								(cond
									...
									((IsFlag 192)
										(HighPrint 16 26) ; "You might have tried to throw a rock at the seed, but it's too late now."
									)
									((== global326 0)
										(if (gEgo has: 21) ; boulder
											(if (not local10)
												(HighPrint 16 27) ; "You get in position for a good throw."
											)
											(gEgo setScript: throwIt)
										else
											(HighPrint 16 28) ; "You need a good rock to throw."
										)
									)
									(else
										(HighPrint 16 3) ; "Not now!"
							...
							((Said '/dagger')
								(HighPrint 16 29) ; "Perhaps it would be better to find something else to throw."
							)
							((Said '/*')
								(HighPrint 16 30) ; "That's really not a good idea."
							)
						)
					)
					((Said 'cast>')
						(switch (= temp0 (SaidSpell event))
							(17
								(cond
									...
									((!= global326 0)
										(HighPrint 16 31) ; "You lose your concentration."
										(gEgo setScript: (ScriptID 291 2)) ; fallDown
									)
									((< [gEgoStats 17] 35) ; openSpell
										(HighPrint 16 32) ; "Your skill with the Open spell is not great enough to affect the plants."
									)
									(else
										(SetFlag 192)
							...
							(24
								(cond
									...
									((IsFlag 192)
										(HighPrint 16 33) ; "You might have tried to cast a spell at the seed, but it's too late now."
									)
									((== global326 0)
										(if (not local11)
											(HighPrint 16 34) ; "You get into a good position."
										)
										(= local9 1)
										(gEgo setScript: throwIt)
									)
							...
							(23
								(if (not (IsFlag 136))
									(HighPrint 16 35) ; "That spell would damage the seed. You don't want to do that."
								else
									(HighPrint 16 36) ; "There is no need to be so destructive."
								)
							)
							(else
								(event claimed: 0)
						...
						(cond
							((Said '/seed')
								(cond
									...
									((IsFlag 192)
										(HighPrint 16 37) ; "Since the flowers have stopped spitting, the seed might be a little harder to get to."
									)
									(else
										(HighPrint 16 38) ; "You'd like to do that, wouldn't you?"
									)
								)
							)
							((Said '/plant,flower,pod,leaf')
								(HighPrint 16 39) ; "You have no need to take the flowers."
							)
							((Said '/boulder')
								(localproc_2)
							)
...
(instance smashIt of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(HighPrint 16 40) ; "You see the seed floating above the shattered remains of the plant."
			...
			(11
				(HighPrint 16 41) ; "You take the seed."
				(SolvePuzzle 651 8)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(flyingSeed setStep: 3 15 setMotion: MoveTo (flyingSeed x:) 150)
			)
			(3
				(= global327 2)
				(= global328 2)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			(0
				(HandsOff)
				(SetFlag 192)
				(= temp0 (- ([local0 global327] x:) (magicLasso x:)))
				(= temp1 (- ([local0 global327] y:) (magicLasso y:)))
				(magicLasso
					setMotion:
						MoveTo
						(+ (magicLasso x:) temp0)
			...
			(2
				...
				(NormalEgo)
				(HandsOn)
				(HighPrint 16 42) ; "You lose concentration, and the spell fades. You will have to practice some more."
				(self dispose:)
			)
		)
	)
...
(instance lassoSeed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(HighPrint 16 43) ; "You place the seed into your pack."
				(SetFlag 136)
				(SolvePuzzle 651 8)
...
(instance openUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 521 cycleSpeed: 1 setCycle: End)
				(= cycles 25)
			)
			(2
				(localproc_3)
...
(instance flower3 of Prop
	...
	(method (handleEvent event)
		(if
			...
			(event claimed: 1)
			(HighPrint 16 44) ; "You've never seen anything quite like them. They're pretty, in a grotesque way."
			(if (or (IsFlag 137) (IsFlag 138) (IsFlag 139))
				(HighPrint 16 45) ; "At least, they're pretty when they're not dead."
			)
		)
	)
)
