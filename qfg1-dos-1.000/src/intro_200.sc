...
(instance intro of Rm
	...
	(method (init)
		...
		(LoadMany rsVIEW 906 907 908 909 913 5 7 430 460 462)
		(super init:)
		(HandsOff)
		(cond
			((== global212 1)
...
(instance page1Script of Script
	...
	(method (doit &tmp temp0 [temp1 10])
		...
		(if local11
			(Print (Format @temp1 200 0 state) #at 300 20 #width 15) ; "%d"
		)
		(super doit:)
	)
...
(instance page2Script of Script
	...
	(method (doit)
		(if (and (== -1 (introMusic prevSignal:)) (< state 14))
			(= seconds (= cycles 0))
			(self changeState: 14)
		)
		(super doit:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1)
			(2
				(heroText setLoop: 1 cel: 0 setCycle: Fwd)
				(questText setLoop: 3 cel: 0 setCycle: Fwd)
			...
			(13
				(dragonHead stopUpd:)
				(dragonTail setScript: 0 stopUpd:)
				(hero2Text setCycle: Fwd)
