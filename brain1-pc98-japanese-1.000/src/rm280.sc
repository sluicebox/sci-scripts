...
(local
	...
	[local104 10] = [{} {\cf\b4\cd \b3\ba\de\b8} {\cb\c0\de\d8\cd \b3\ba\de\b8} {\b3\bc\db\cd \b3\ba\de\b8} {\d0\b7\de\cd \b3\ba\de\b8} {\b1\b2\c3\d1\a6 \c4\d9} {\b1\b2\c3\d1\a6 \ca\c5\bd} {\ca\bc\de\d2\d9} {\d4\d2\d9} {}]
...
(procedure (localproc_3 &tmp temp0 temp1 temp2 temp3 temp4 [temp5 10] temp15)
	...
	(for ((= temp15 (- local18 2))) (< temp2 5) ((++ temp15))
		...
		(if (< 0 temp15 (+ local19 1))
			(Format @temp5 280 58 temp15 (localproc_5 temp1)) ; "%d : %s"
...
(instance hand3 of Prop
	...
	(method (doVerb theVerb &tmp temp0)
		(switch theVerb
			(2
				(Print 280 30) ; "This robot hand features a dart gun which fires a dart with a soft rubber tip. The dart should be able to grab and hold a lightweight, smooth-surfaced object."
			)
			(3
				(gCMusic2 number: 59 setLoop: 1 play:)
				(gEgo get: 13)
...
(instance robotBody of Feature
	...
	(method (doVerb theVerb invItem)
		(cond
			((== theVerb 2)
				(Print 280 33) ; "The robot torso is labelled "IMA Robot". (You've heard of Dr. Brain's Installable Modular Accessory Robots. You can attach one of several types of robot heads and hands for different uses.)"
			)
			((and (== theVerb 4) (== invItem 8))
				(if (not (and (IsFlag 24) (IsFlag 26) (IsFlag 25)))
					(= local20 1)
...
(instance robotSlot of Feature
	...
	(method (doVerb theVerb invItem)
		(cond
			((== theVerb 2)
				(Print 280 35) ; "This card slot looks like it will accept a data card."
			)
			((and (== theVerb 4) (== invItem 8))
				(if (not (and (IsFlag 24) (IsFlag 26) (IsFlag 25)))
					(= local20 1)
	...
	(method (dispatchEvent event &tmp temp0 temp1 temp2)
		(if (== (event type:) evKEYBOARD)
			...
			(switch (event message:)
				...
				(KEY_q
					(Print 280 53 #at 68 96 #width 200) ; "KEYBOARD COMMANDS: f - Forward...o - turn On b - Backward..t - Turn off l - Left......d - Delete r - Right.....i - Insert s - Save......p - Previous command c - Clear.....n - Next command q - Quick look at this list"
				)
				(else
					(= temp1 0)
				)
