...
(procedure (localproc_3 param1 &tmp temp0 temp1 temp2 temp3 [temp4 6])
	...
	(if (!= local51 2)
		(if local52
			(Display 240 19 dsRESTOREPIXELS [local39 (+ param1 5)])
		)
		(Format @temp4 240 24 temp1) ; "%d"
		(= [local39 (+ param1 5)]
...
(procedure (localproc_6 &tmp temp0 temp1 temp2 temp3 [temp4 3])
	...
	(for ((= temp0 0)) (<= temp0 4) ((++ temp0))
		(if local52
			(Display 240 19 dsRESTOREPIXELS [local39 temp0])
		)
		(Format @temp4 240 24 [local34 temp0]) ; "%d"
		(= [local39 temp0]
...
(instance circuit of PuzzleBar
	...
	(method (init &tmp temp0)
		...
		(for ((= temp0 0)) (< temp0 5) ((++ temp0))
			(self
				add:
					((= [local53 temp0] (responder_J______ new:))
	...
	(method (buyClue &tmp [temp0 2])
		(if (super buyClue:)
			(proc5_9 240 1) ; "Place each of the components so that they form a complete circuit from the lower left to the lower right. The correct order is: Battery, Switch, Resistor, Coil, Capacitor. (All will be placed horizontally except the Switch, which is vertical on the left side.)"
		)
	)

	(method (showHelp)
		(proc5_9 240 2) ; "This is the central computer control circuit. The object is to place the five components in the correct positions to complete the circuit. Instructions are on the maintenance card on the other panel on this console."
		(proc5_9 240 3) ; "If you get really stuck, insert a hint coin in the coin slot, and we'll tell you where each component goes. But you'll have more fun figuring it out for yourself."
	)
						...
						((and (<= 34 temp2 148) (<= 28 temp3 92))
							(if
								...
								(if (localproc_1)
									...
									(reward1Sound play:)
									(proc5_9 240 4) ; "Congratulations! You have successfully assembled the central control circuit. Now you can pick up Easy Listening 107, "All Music All the Time." Either that, or you've managed to repair the binary sequencing computer."
									(proc5_9 240 5) ; "You are awarded a memento copy of the control circuit, proving that you know how to follow directions."
									(gEgo get: 29)
									(= local49 1)
									(electricalArc play:)
...
(instance responder_J______ of CodeIcon
	(properties
		name {responder}
	)
...
(instance tapeFeat of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 9) ; "Dr. Brain built this tape drive out of an old washing machine. It's totally obsolete, but he insisted on keeping it in the system out of a sense of history."
			)
			(3
				(proc5_9 240 10) ; "Don't take the laundry out yet; the tape drive is still on its spin cycle."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance binaryFeat of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 11) ; "This is a once-state-of-the-art manually-motivated binary sequencing computer. (It's not even as up-to-date as the tape drive. But it could use your help.)"
			)
			(3
				(if (not local50)
					(sequencer init:)
				else
					(proc5_9 240 12) ; "The binary sequences are now correctly set."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance c1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 13) ; "It's a happy red disc capacitor."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance c2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 14) ; "It's a shy yellow disc capacitor."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance c3 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 15) ; "It's one of the famous Dancing Electrolytic Capacitors (the name may have something to do with the fact that they never made it big)."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance c4 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 16) ; "This is a resistor (as opposed to a draft resister)."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance c5 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2
				(Print 240 17) ; "This is a resistor. It is used to control the flow of current through a circuit. (As opposed to a draft resister, which probably controls the flow of air through a drafty castle.)"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sequencer of PuzzleBar
	...
	(method (show &tmp temp0)
		...
		(cond
			((not local49)
				(proc5_11 240 18) ; "Sorry, the binary sequencer is not currently functional. Please repair the central control circuit to achieve full sequencer operational status."
			)
			(local50
	...
	(method (solvePuzzle)
		...
		(reward2Sound play:)
		(proc5_11 240 20) ; "Congratulations! You have solved the binary conversion puzzle and fixed Dr. Brain's stereo system. A magnetic data card pops out of a slot on the console, and you pick it up for later use. Now, there's this nuclear reactor..."
		(gEgo get: 8)
		(= local50 1)
	)
	...
	(method (showHelp)
		(proc5_11 240 21) ; "This is the Binary Sequencing Computer. A decimal number is shown in the box at the left of each line. Turn the switches on and off to form the number's binary equivalent. When you have the correct number, the bulb at the right side of the line will light up."
		(if (!= global114 2)
			(proc5_11 240 22) ; "The second box shows the decimal equivalent of the number currently formed by the switches. Watch how this number changes as you push the switches. It will help you get the right settings."
		)
		(proc5_11 240 23) ; "If you get really stuck, insert a hint coin into the slot and we will solve the next row of the conversion for you."
	)
