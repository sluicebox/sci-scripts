...
(local
	[aMoney 6]
	[aVeggie 6]
	[spotlightX 9] = [999 113 127 141 155 169 183 197 999]
	[string 40]
	[string2 22]
)
...
(instance RoomScript of Script
	...
	(method (doit &tmp i)
		...
		(if (== gEgoState 431)
			...
			(aSpotlight posn: [spotlightX i] 52 setCel: i)
			(aFloor posn: [spotlightX i] 97 setCel: i)
		)
	)
	...
	(method (handleEvent event)
		...
		(cond
			...
			((and (Said 'look>') (Said '[/area,backstage,couple,man,woman]'))
	...
	(method (changeState newState &tmp i)
		...
		(switch (= state newState)
			...
			(3
				(aCurtain stopUpd:)
				(Print (Format @string 431 5 gExpletive) #font 7) ; "%s"
			...
			(14
				(cls)
				(for ((= i 1)) (<= i 5) ((++ i))
					(([aVeggie i] script:) changeState: 2)
				)
				(gGame setScript: (ScriptID 40)) ; DyingScript
				((ScriptID 40) ; DyingScript
					caller: 437
					register: (Format @string 431 22) ; "In fact, just for purposes of argument, you ARE dead of embarrassment!"
					next: (Format @string2 431 23) ; "Feets don't fail me now!"
				)
			)
			(15
				(= gEgoState 431)
				...
				(gEgo setMotion: MoveTo [spotlightX 6] 95 self)
				(= cycles 4)
			)
			(16
			...
			(17
				(gEgo setMotion: MoveTo [spotlightX 1] 95 self)
			)
			(18
				...
				([aMoney 1] setScript: (MoneyScript new:))
				(gEgo setMotion: MoveTo [spotlightX 3] 95 self)
			)
			(19
				([aMoney 2] setScript: (MoneyScript new:))
				([aMoney 3] setScript: (MoneyScript new:))
				(gEgo setStep: 1 1 setMotion: MoveTo [spotlightX 6] 95 self)
			)
			(20
				(cls)
				(Print 431 26 #at 10 5 #width 290 #time 6 #dispose) ; "Look, Larry! They're all throwing money at you. They like you! They REALLY like you!!"
				([aMoney 4] setScript: (MoneyScript new:))
				([aMoney 5] setScript: (MoneyScript new:))
				(= register 0)
				(self cue:)
			)
			(21
				(gEgo
					setMotion: MoveTo (Random [spotlightX 1] [spotlightX 6]) 95 self
				)
