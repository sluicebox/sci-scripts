...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'stop,(pull<off)/bath,faucet,channel,handle,water')
				(cond
					((not (TestFlag 49))
						(ItIs) ; "It is."
					)
			...
			(
				(or
					(Said 'bath,bath')
					(Said 'get/bath')
					(Said 'use,(pull<on)/bath,faucet,channel,handle,water')
				)
				(cond
					((TestFlag 49)
						(ItIs) ; "It is."
			...
			((or (Said 'clean') (Said 'clean,use/soap'))
				(cond
					...
					((not (gEgo has: 5)) ; Soap-On-A-Rope
						(Print 375 7) ; "You rub your hands all over yourself in an excellent imitation of a bar of soap. Too bad you don't have one."
						(Print 375 8 #at -1 144) ; "(Bar of soap, that is!)"
					)
					(else
						(Ok) ; "O.K."
						(ClearFlag 8)
						...
						(Print 375 10) ; "You carefully rub it over every part of your body."
						(if (>= gFilthLevel 3)
							(Print 375 11) ; "...some parts longer than others!"
						)
						(Print 375 12) ; "Soon, it's all gone!"
					)
				)
			)
			((and (not (gEgo has: 5)) (Said 'get/soap')) ; Soap-On-A-Rope
				(Print 375 13) ; "Well, look at that! Somebody used up all the soap and left none for you!"
				(Print 375 14 #at -1 144) ; "(Where could you find soap on this island?)"
			)
			((Said 'look>')
				(cond
					((Said '/faucet,handle,channel')
						(if (TestFlag 49)
							(Print 375 15) ; "It's "on.""
						else
							(Print 375 16) ; "It's "off.""
						)
					)
					((Said '/drain')
						(if (TestFlag 49)
							...
							(Print 375 18) ; "It looks grate!"
						)
					)
					((Said '[/carpet,bath,area]')
						(Print 375 19) ; "The shower room has a slippery tile floor."
					)
				)
			)
		)
