	...
	(method (handleEvent event &tmp theObj)
		...
		(cond
			...
			((Said 'give>')
				...
				(cond
					((not (& (gEgo onControl:) $0010))
						(Print 420 55) ; "Move closer to the maitre d'."
					)
					((Said '[/!*]')
						(Print 420 56) ; "What did you want to give him?"
					)
					((not theObj)
						(Print 420 57) ; "It's not yours to give."
					)
					((not (theObj ownedBy: gEgo))
						(DontHave) ; "You don't have it."
					)
					((== theObj (gInventory at: 2)) ; Ginsu_Knife
						(Print 420 58) ; "Hey!" shouts the maitre d'. "Put that knife away before I call security!!"
					)
					((== theObj (gInventory at: 4)) ; Native_Grass
						(Print 420 59) ; ""Get those weeds out of here!" shouts the maitre d'."
					)
					((== theObj (gInventory at: 3)) ; Granadilla_Wood
						(Print 420 60) ; "Isn't that precious?" he snarls. "A hunk of wood. Indeed!"
					)
					((== theObj (gInventory at: 11)) ; some_Orchids
						(Print 420 61) ; "No, I'm not going to be your date for the prom!"
					)
					((== theObj (gInventory at: 1)) ; Credit_Card
						(Print 420 62) ; "I'm sorry, sir, but we don't accept credit cards here. Especially not from deadbeats!"
					)
					(else
						(Print 420 63) ; "The maitre d' replies, "And just exactly what am I supposed to do with that?""
					)
				)
				(event claimed: 1)
			)
	...
	(method (changeState newState &tmp [theLine 5])
		...
		(switch (= state newState)
			...
			(1
				(Ok) ; "O.K."
				(HandsOff)
				(Print 420 64) ; "You say, "Excuse me, sir, but I'd like to see tonight's show. I have my copy of `Nontoonyt Tonight' right here.""
			...
			(2
				...
				(Format @string 420 65 pageNum) ; "Oh, really? Well, for tonight's show I am allowed to accept free passes only from our ad on page %d. What is the number of the pass on that page?"
				(PrintMD 422 3 0)
				(= theLine 0)
				...
				(if gDebugging
					(Printf 420 66 passNum [passNumbers pageNum] pageNum) ; "You typed %d; I wanted %d from page %d."
				)
				(Format @string 420 67) ; "Let me look through this list of passes and numbers..."
				(PrintMD 422 3 0)
				(= seconds 3)
			)
			(3
				...
				(if (not (TestFlag 47))
					(Format @string 420 68) ; "Well, well. Very good, sir," says the maitre d', his tone and mannerisms changing entirely. "But, I must have made a mistake. It appears there are no more seats available for this show! I'm so, so sorry. But, perhaps something will open up later."
					...
					(Format @string 420 69) ; "Say," he exclaims, "I remember you. Why look here; I must have overlooked this seat right here in the front of the house. Please, walk this way!"
					(PrintMD 422 3 6)
					(self changeState: 8)
				)
			)
			...
			(5
				(aMaitreD setCel: 0 stopUpd:)
				(if (not string)
					(Format @string 420 70) ; "Yes? May I help you sir?"
				)
				(PrintMD 422 3 0)
				(HandsOn)
			)
			...
			(7
				...
				(cond
					((== ((Inv at: 6) view:) 24) ; A_Twenty_Dollar_Bill
						(Print 420 71) ; "I don't want your stupid one-dollar bills!"
						(PrintMD 422 3 0)
					)
					((not (TestFlag 46))
						(Format @string 420 72) ; "Why thank you very much, sir! I'll try to keep your generosity in mind. Unfortunately, you must have a ticket or one of the casino's complimentary passes in order to gain admittance here. Do you have such a pass? If so, now is the time to show it to me."
						(PrintMD 422 3 0)
						(PutInRoom 6)
					...
					(else
						(HandsOff)
						(Format @string 420 73) ; "Why, thank you very much, sir, Well, well, look at this! I seem to have found a nice, front row seat available here. Allow me to show you to your seat!"
						(PrintMD 422 3 1)
						(PutInRoom 6)
						(SetFlag 47) ; tippedMaitreD
						(gGame changeScore: 50)
			...
			(13
				(aMaitreD setCel: 0 stopUpd:)
				(Format @string 420 74) ; "I'm sorry, sir, but that's not the right number. Next time, obtain a proper pass in advance! Wait right there while I summon security and have you arrested for attempting to pass a counterfeit showroom pass."
				(PrintMD 422 3 0)
				(= seconds 2)
			)
			(14
				(Format @string 420 75) ; "By the way, have you saved your game recently?"
				(PrintMD 422 3 0)
				(gGame setScript: (ScriptID 40)) ; DyingScript
				((ScriptID 40) ; DyingScript
					caller: 56
					register: (Format @string 420 76) ; "Next time, better have a copy of "Nontoonyt Tonight" before playing this game."
					next: (Format @string2 420 77) ; "Al says, "Read Your Manual!""
				)
			)
		)
	)
