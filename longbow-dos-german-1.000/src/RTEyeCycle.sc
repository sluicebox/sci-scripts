	...
	(method (cue param1 &tmp temp0)
		...
		(if (and (= temp0 (WordAt local0 (++ register))) notKilled)
			(if (!= (WordAt [local0 temp0] 0) -1)
				(GetFarText local11 local13 @local237)
				(if (> (StrLen @local237) 310)
					(= gUserFont 60)
				else
					(= gUserFont 300)
				)
				...
				(switch temp0
					(1
						(talker1 say: @local237 0 0 0 self)
					)
					(2
						(talker2 say: @local237 0 0 0 self)
					)
				)
			else
				(if gModelessDialog
