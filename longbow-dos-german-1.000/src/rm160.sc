...
(instance series of Script
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 [temp4 400] [temp404 400] [temp804 400])
		(if (<= (= state newState) start)
			(gCurRoom drawPic: 803 9)
			(StrSplitInTwo @temp404 @temp804 register state)
			(if (== (gGame printLang:) 1)
				(StrCpy @temp4 @temp404)
			else
				(StrCpy @temp4 @temp804)
			)
			(= temp2 @temp4)
			(= temp3 0)
			(= temp1 1)
