...
(class GiveOrTakeIcon of DIcon
	...
	(method (draw &tmp temp0 temp1 temp2 temp3 [temp4 20] [temp24 20] [temp44 20])
		(super draw: &rest)
		...
		(if temp0
			...
			(TextSize @temp44 @temp4 temp1)
			(= temp3 (+ nsRight 5))
		else
			(Format @temp4 {%d} (client number:))
			(TextSize @temp44 @temp4 temp1)
			(= temp3 (- (- nsLeft [temp44 2]) 5))
		)
		(Format @temp24 {%d} theLastVal)
		(Display
			@temp24
			dsCOLOR
			((dialog window:) back:)
		...
		(Display
			...
			dsCOORD
			(= theLastX temp3)
			(= theLastY (/ (- (+ nsBottom nsTop) [temp44 3]) 2))
		)
		(= theLastVal
			(if temp0
				(client taken:)
