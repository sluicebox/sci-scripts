...
(instance converse of Script
	...
	(method (changeState newState &tmp temp0 temp1 [temp2 125] [temp127 100] [temp227 150])
		(switch (= state newState)
			(0
				(HandsOff)
			...
			(3
				...
				(GetFarText 1317 global123 @temp127)
				(if 1
					(= global216 (Memory memALLOC_CRIT (StrLen @temp2)))
					(= global218 (Memory memALLOC_CRIT (StrLen @temp127)))
					(= global221 (Memory memALLOC_CRIT (StrLen @temp2)))
					(= global223 (Memory memALLOC_CRIT (StrLen @temp127)))
					(StrSplitInTwo global216 global221 @temp2)
					(StrSplitInTwo global218 global223 @temp127)
					(if (== (gGame printLang:) 49)
						(Format @temp227 314 0 global221 global223) ; "%s%s"
					else
						(Format @temp227 314 0 global216 global218) ; "%s%s"
					)
				...
				((ScriptID 851 2) show: say: @temp227 0 0 0 self) ; talker1
			)
			(4
