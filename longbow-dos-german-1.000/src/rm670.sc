...
(local
	...
	[local47 9] = [1670 26 2 1 2 1 2 2 0]
	[local56 6] = [1670 32 2 1 2 0]
	[local62 4] = [1670 35 1 0]
	[local66 4] = [1670 46 1 0]
)
...
(instance oRat of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Say 1670 51) ; "This place is overrun with rats."
			)
			(3 ; Do
				(Say 1670 52) ; "They'd make good rat stew, but I've no time for that now."
			)
			(5 ; Talk
				(SetMessageColor 32)
				(Say 1670 53 80 {Rat}) ; "Yeah, whaddayawant?"
				(SetMessageColor 0)
			)
			(4 ; Inventory
				(SetMessageColor 32)
				(Say 1670 54 80 {Rat}) ; "If it ain't cheese, I don't want it."
				(SetMessageColor 0)
			)
			(else
...
(instance bucket of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Say 1670 44) ; "A bucket of water for reviving tortured prisoners."
			)
			(3 ; Do
				(Say 1670 45) ; "It smells of the fens. I'll leave it be."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance walls of Feature
	(properties
		...
		nsBottom 150
		nsRight 268
		lookStr 41 ; "Mist from the fens slimes the dank walls."
	)
)
...
(instance floor of Feature
	(properties
		...
		nsBottom 189
		nsRight 268
		lookStr 43 ; "The straw strewn upon the floor does nothing to hide the stench of blood and fear."
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Say 1670 47) ; "The filthy straw on this floor is more than I care to smell, let alone touch."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance highWindow of Feature
	(properties
		...
		nsRight 207
		sightAngle 90
		lookStr 42 ; "It's too high and small to afford any means of escape."
	)
)
...
(instance rack of Feature
	(properties
		y 20
		lookStr 38 ; "I've never seen such a thing, but I can guess its purpose is to cause physical torment."
	)
...
(instance strappado of Feature
	(properties
		y 20
		lookStr 36 ; "It's a hideous device known as the strappado."
	)
...
(instance wheel of Feature
	(properties
		y 20
		lookStr 39 ; "Whatever it is, it must surely have an evil purpose."
	)
...
(instance leftRope of Feature
	(properties
		y 20
		lookStr 37 ; "The ropes of the strappado are designed to slowly tear a victim's limbs from their sockets by the weight of his own body."
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Say 1670 48) ; "They're beyond my reach and of no use to me."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(switch theVerb
			(3 ; Do
				(if (== (Monastery fulkStatus:) 0)
					...
					(Say 1670 50) ; "I've no more interest in those wretched ropes."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance rightTopRope of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if (== (Monastery fulkStatus:) 0)
					(Say 1670 49) ; "I cannot reach that part of the ropes, but they run through a pulley attached to the wall and made fast to a post lower down."
				else
					(Say 1670 50) ; "I've no more interest in those wretched ropes."
				)
			)
...
(instance wallChain of Feature
	(properties
		y 20
		lookStr 40 ; "Who knows how many victims have hung in torment from these chains?"
	)
...
(instance meetTheMonk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse @local62 11 0 self) ; "You may not enter, Brother. Prior's orders."
			)
			(3
			...
			(2
				(if (== (Monastery fulkStatus:) 2)
					(if (gEgo has: 17) ; handScroll
						(Converse @local56 8 (Monastery tFULK:) self) ; "Thank God! I'd begun to despair of you. Have you found the scroll you need?"
					else
...
(instance burnedByFire of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Converse @local66 11 0 self) ; "That ring! You're the thief the Abbot wants!"
			)
			(5
