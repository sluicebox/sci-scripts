...
(local
	...
	local133 = { CS BLHFTNDGQPZ__OU EIRMKVJXAWY}
	local134
	local135
	local136
	local137
...
(procedure (localproc_2 &tmp temp0 temp1 [temp2 30] [temp32 30] [temp62 30] [temp92 30] [temp122 30] [temp152 30])
	(for ((= temp0 0)) (< temp0 53) ((++ temp0))
		(StrCpy @temp2 [local42 temp0])
		(StrCpy @temp92 local15)
		(StrSplitInTwo @temp32 @temp62 @temp2)
		(if (== (gGame printLang:) 49)
			(StrCpy @temp2 @temp62)
		else
			(StrCpy @temp2 @temp32)
		)
		(if (== (StrCmp @local22 @temp2) 0)
			(= global124 temp0)
			(= temp1 1)
			(break)
		)
	)
	(StrSplitInTwo @temp122 @temp152 @temp92)
	(if (== (gGame printLang:) 49)
		(StrCpy @temp92 @temp152)
	else
		(StrCpy @temp92 @temp122)
	)
	(if (== (StrCmp @local22 @temp92) 0)
		(SetFlag 67)
		(= temp1 1)
	)
	(if (== temp1 0)
