...
(procedure (localproc_1)
	(if (GameIsRestarting)
		(= local16 (Display local17 dsCOORD 20 10 dsWIDTH 280 dsCOLOR 255 dsBACKGROUND 0 dsSAVEPIXELS))
	)
)
...
(instance doTheRiddles of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= local16
					(Display local17 dsCOORD 20 10 dsWIDTH 280 dsCOLOR 255 dsBACKGROUND 0 dsSAVEPIXELS)
				)
			)
		)
	)
