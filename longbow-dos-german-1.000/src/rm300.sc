		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(0 ; bucks
						(SetMessageColor 21)
						(Say ; "I'll not part with any blades for that price."
							1300
							53
							80
							{Garrett of York}
		...
		(switch theVerb
			...
			(5 ; Talk
				(if (> (gEgo y:) 100)
					...
					(SetMessageColor 21)
					(Say ; "I would dearly love some raw fish and rice."
						1300
						122
						80
						{Mistress Hayasher}
...
(instance sRandom of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(60
				(markArm startUpd: cycleSpeed: (Random 5 10) setCycle: End self)
			)
			(61
				(= cycles (Random 5 20))
			)
