	...
	(method (handleEvent event)
		...
		(cond
			...
			((and gDebugging (Said 'increase/backstage'))
				(Printf 417 2 (++ gShowroomState) (++ gShowroomState)) ; "Ok, Al. Showroom state is now %d, where 0 = SRshowIsOn 1 = SRshowDone 2 = SRcherriOnPhone 3 = SRknowsAboutDeed 4 = SRcherriBackstage 5 = SRstageDoorUnlocked 6 = SRdone %"
			)
			((and gDebugging (Said 'increase/document'))
				(Printf 417 3 (++ gNewspaperState) (++ gNewspaperState)) ; "Ok, Al. Newspaper state is now %d, where 0 = NSnotYet 1 = NSpComing 2 = NSshowroom 3 = NSpHere 4 = NSlMissing %"
			)
			((and gDebugging (Said 'increase/attorney'))
				(Printf 417 4 (++ gLawyerState) (++ gLawyerState)) ; "Ok, Al. Lawyer state is now %d, where 0 = LSbusy 1 = LSfree 2 = LSwaiting4deed 3 = LSdeedReady 4 = LSneeds500 5 = LSdivorce 6 = LSwaiting4divorce 7 = LSdivorceReady 8 = LSdone %"
			)
			((Said 'look,look>')
				(cond
