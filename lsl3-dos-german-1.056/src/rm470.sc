...
(instance RoomScript of Script
	...
	(method (handleEvent event &tmp [string 150])
		...
		(cond
			...
			(
				...
				(cond
					((not (gEgo has: 12)) ; Penthouse_Key
						...
						(Print
							(Format @string 470 2 gEgoName gEgoName) ; "(You must have a room key to operate this elevator, %s.)%"
							#at
							-1
							144
						)
					...
					(destination
						(Print 470 3) ; "The elevator is already underway!"
					)
					((> lightCel 7)
			...
			(
				(or
					(Said '/carpet,0,1,area')
					(Said '//carpet,0,1,(carpet<first),area')
				)
				(cond
					((not (gEgo has: 12)) ; Penthouse_Key
						(Print 470 1) ; "You attempt to pick the lock with your fingernail, but are unsuccessful!"
						(Print
							(Format @string 470 5 gEgoName gEgoName) ; "(You must have a room key to operate this elevator, %s.)%"
							#at
							-1
							144
						)
			...
			((Said 'up,down,open,close/door,carpet,elevator')
				(Print 470 6) ; "Press the floor number, please."
			)
			((Said 'pull,go/button,door,carpet')
				(Print 470 7) ; "You have no business travelling to any floor except one and nine."
			)
			((Said 'listen')
				(if gEgoIsPatti
					(Print 470 8) ; "Gawd, do you hate musical tripe!"
				else
					(Print 470 9) ; "Your favorite style of music is playing."
				)
			)
			((Said 'exit,go,exit,done,stop')
				(Ok) ; "O.K."
				(Print 470 10) ; "Wait until the door opens."
			)
			((and (gEgo has: 9) (Said '/keycard')) ; Spa_Keycard
				(Print 470 11) ; "The keycard would work better if you were in Fat City!"
			)
			((Said 'pick,hit,break/lock,bolt,bolt')
				(Print 470 12) ; "The locks here will only respond to an official hotel room key."
			)
			((Said 'look,look>')
				(cond
					((Said '/burn,burn')
						(Printf ; "The row of lights above the door indicate you are currently at floor %d.%"
							470
							13
							(+ 1 (aLight cel:))
							(+ 1 (aLight cel:))
					...
					((Said '/switch,handle,button')
						(Print 470 14) ; "On the wall to the right of the doors is a panel with a keyhole beside a vertical column of buttons numbered from one to nine. "Could those indicate potential floor destinations?" you wonder."
					)
					((Said '/door')
						(Printf ; "The elevator's door is presently %s.%"
							...
							(if (aDoor cel:) {ge\94ffnet} else {geschlossen})
						)
					)
					((Said '/(hole<key),bolt,bolt')
						(Print 470 16) ; "The keyhole is a hole into which you insert a key."
					)
					((Said '[/area,elevator]')
						(Print 470 17) ; "It's an elevator."
						(Print 470 18 #at -1 144) ; "(What did you expect?)"
					)
				)
			)
		)
	...
	(method (changeState newState &tmp destY)
		(switch (= state newState)
			...
			(1
				(Print 470 19) ; "Evidently, you don't want to change floors!"
				(self changeState: 13)
			)
			(2
				(Ok) ; "O.K."
			...
			(4
				(if (not (TestFlag 63))
					...
					(gGame changeScore: 4)
					(Print 470 20 #icon 12 0 0) ; "You insert Patti's penthouse key into the ninth floor penthouse access lock and turn it one-quarter turn to the right. Both you and the elevator begin to rise."
				)
