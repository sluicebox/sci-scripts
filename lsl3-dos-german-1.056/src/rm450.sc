...
(instance RogerScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said '/yes,hardy')
				(Print 450 54) ; ""So what are you doing here without those Two Guys?" you ask him."
				(Print 450 55) ; ""What two guys?" he replies."
			)
		)
	)
...
(instance PattiScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'get,make,(ask<about),gamble/music,buy,order')
				...
				(Printf ; ""Ok," says Patti, "how about a selection from that hot new Sierra adventure game `Leisure Suit Larry 3?' Here's the hauntingly-beautiful LSL3 `%s Theme.'"%"
					...
					(switch request
						...
						(500 {Bambus-})
						(560 {Wildwasserfahrt-})
						(599 {Nontoonyter Dschungel-})
						(540 {Inselwildschwein-})
						(else {Larry-erwischt-etwas-})
					)
				)
				(self changeState: 1)
			)
			((Said 'talk,talk[/entertainer,woman,entertainer]')
				(if gEgoIsPatti
					(Print 450 57) ; "No one can hear you, Patti."
				else
					(Print 450 58) ; "Before speaking to a woman, it is advisable to look at her, Larry."
				)
			)
			((Said 'look,look/entertainer,woman,entertainer')
				(if (!= gEgoState 1004)
					(Print 450 59) ; "Why not have a seat first, Larry?"
				else
					(RoomScript changeState: 13)
				)
			)
			((Said '/entertainer,woman,entertainer')
				(Print 450 60) ; "Perhaps you should establish eye contact."
			)
		)
	)
	...
	(method (changeState newState &tmp howMany)
		...
		(switch (= state newState)
			...
			(4
				(Print 450 61) ; ""Here's another song you might enjoy," says Patti."
				(self changeState: 1)
			)
			(5)
			(6
			...
			(8
				(Print 450 62) ; "Here, Larry, take the key to my suite," says Patti. "I'll slip out the back way and use the service elevator. Just wait a few minutes before you come upstairs, as I'd like to slip into something more comfortable..."
				(Print 450 63) ; "...like your flowers!"
			...
			(9
				(Print 450 64) ; "Oh, and remember Larry: I refuse to make love to a man without something to drink first!" she says over her shoulder. "No offense, of course."
				(aPatti setMotion: MoveTo 168 46 self)
			)
			(10
				(Print 450 65 #at -1 144) ; "(None taken, you suppose.)"
				(aPatti dispose:)
				(User canInput: 1)
				(= gEgoState 1004)
			)
