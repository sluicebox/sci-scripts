...
(procedure (GameOver &tmp [str 160])
	(Format @str &rest)
	(gSounds eachElementDo: #stop)
	(gTheMusic number: 190 vol: 127 loop: 1 flags: 1 play:)
	(deathIcon view: gDeathView loop: gDeathLoop cycleSpeed: (* (+ gHowFast 1) 4))
...
(procedure (Babble theView msgS msgO &tmp [buffer 500]) ; UNUSED
	...
	(babbleIcon view: theView cycleSpeed: (* (+ gHowFast 1) 4))
	(if (u< msgS 1000)
...
(instance LSL1 of Game
	...
	(method (init &tmp temp0 temp1 [temp2 10] [temp12 20])
		(StrSplit @temp2 @temp12 0)
		(Format @temp12 0 32 800) ; "%d.scr"
	...
	(method (pragmaFail &tmp theVerb theItem)
		(if (User canInput:)
			...
			(switch theVerb
				(2
					(Print 0 33) ; "It's not much to look at."
				)
				(5
					(Print 0 34) ; "To whom were you trying to speak?"
				)
				(3
					(Print 0 35) ; "There's nothing to do there."
				)
				(4
					(switch theItem
						(((Inv at: 16) description:) ; ribbon
							(Print 0 36) ; "You gaily display your bright pink ribbon for all to see."
						)
						(((Inv at: 11) description:) ; pocketKnife
							(Print 0 37) ; "Kill! Kill! Kill!"
							(Print 0 38 #at -1 140) ; "(Doesn't anybody ---- anymore?)"
						)
						(else
							(if 1
								(= global314 (Memory memALLOC_CRIT (StrLen theItem)))
								(= global323 (Memory memALLOC_CRIT (StrLen theItem)))
								(StrSplitInTwo global314 global323 theItem)
								(Printf 0 39 global314 global323) ; "Try finding some place else to place %s."
				...
				(10
					(cond
						...
						(else
							(Print 0 40) ; "You coyly turn your back and relieve yourself. You quickly feel much better. The hint of a smile crosses your lips."
						)
					)
				)
				(11
					(Print 0 41) ; "You really don't want to put your face near that."
				)
			)
		)
	)
	...
	(method (startRoom n)
		...
		(if
			(and
				...
				(Print 0 43 #button {Who cares} 0 #button {Debug} 1) ; "Memory fragmented."
			)
			(SetDebug)
		)
		(if gDebugOn
	...
	(method (handleEvent event &tmp temp0 [temp1 80])
			...
			(switch (event type:)
				(evKEYBOARD
					(switch (event message:)
						...
						(KEY_ALT_l
							(Format @temp1 0 32 952) ; "%d.scr"
							(if (and gDebugging (FileIO fiEXISTS @temp1))
								(if (HaveMem 1536)
									((ScriptID 952) doit: @gSysLogPath 0) ; MISSING SCRIPT
								else
									(Print 0 44) ; "Not Enough Memory!!"
								)
							)
						)
						(else
							(Format @temp1 0 32 800) ; "%d.scr"
							(if (and gDebugging (FileIO fiEXISTS @temp1))
		...
		(if (!= gOldSysTime (GetTime 1)) ; SysTime12
			...
			(if
				...
				(if (not (IsFlag 5))
					...
					(switch (Random 1 5)
						(1
							(Print 0 45) ; "Your mouth tastes like the inside of a motorman's glove!"
						)
						(2
							(Print 0 46) ; "Your breath smells like floor sweepings from a rendering plant."
						)
						(3
							(Print 0 47) ; "Your breath is enough to take your breath away."
						)
						(4
							(Print 0 48) ; "Your mouth tastes like drippings from a marathon runner's sock."
						)
						(5
							(Print 0 49) ; "Now you know where old elephants go to die--your mouth!"
						)
					)
				)
			)
			(if (and (< gLarryDollars 1) (not (IsFlag 1)))
				...
				(Print 0 50) ; "Oh, no! You are now penniless. You hock your leisure suit only to spend the rest of your life wandering from town to town, wondering about what "might have been.""
				(Death 807 1)
				(Format @gYourPart 0 51) ; "POOR Larry!"
				(GameOver 0 52)
			)
		)
		(super doit: &rest)
			...
			(switch theVerb
				(2
					(cond
						...
						(1
							(= global314 (Memory memALLOC_CRIT (StrLen objDesc)))
							(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
							(StrSplitInTwo global314 global323 objDesc)
							(Printf 0 53 global314 global323) ; "Why, look! It's %s."
							(Memory memFREE global314)
							(Memory memFREE global323)
						)
					)
				...
				(5
					(if 1
						...
						(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
						(StrSplitInTwo global314 global323 objDesc)
						(Printf 0 54 global314 global323) ; "You engage in a short, but personally meaningful conversation with %s."
						(Memory memFREE global314)
						(Memory memFREE global323)
					)
				)
				(3
					(if 1
						...
						(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
						(StrSplitInTwo global314 global323 objDesc)
						(Printf 0 55 global314 global323) ; "That feels exactly like %s."
						(Memory memFREE global314)
						(Memory memFREE global323)
					)
				)
					...
					(switch itemDesc
						(((gInventory at: 16) description:) ; ribbon
							(if 1
								...
								(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
								(StrSplitInTwo global314 global323 objDesc)
								(Printf 0 56 global314 global323) ; "Perhaps %s is not into velveteen S & M."
								(Memory memFREE global314)
								(Memory memFREE global323)
							)
						)
						(((gInventory at: 11) description:) ; pocketKnife
							(Print 0 57) ; "You're a lover, not a fighter!"
						)
						(else
							(if 1
								(= global314 (Memory memALLOC_CRIT (StrLen itemDesc)))
								(= global323 (Memory memALLOC_CRIT (StrLen itemDesc)))
								(= global315 (Memory memALLOC_CRIT (StrLen objDesc)))
								(= global324 (Memory memALLOC_CRIT (StrLen objDesc)))
								(StrSplitInTwo global314 global323 itemDesc)
								(StrSplitInTwo global315 global324 objDesc)
								...
								(Memory memFREE global314)
								(Memory memFREE global315)
								(Memory memFREE global323)
								(Memory memFREE global324)
							)
						)
					)
				)
				(10
					(if 1
						...
						(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
						(StrSplitInTwo global314 global323 objDesc)
						(Printf 0 59 global314 global323) ; "Larry! Don't do that to %s!"
						(Memory memFREE global314)
						(Memory memFREE global323)
					)
				)
				(11
					(if 1
						...
						(= global323 (Memory memALLOC_CRIT (StrLen objDesc)))
						(StrSplitInTwo global314 global323 objDesc)
						(Printf 0 60 global314 global323) ; "To you, %s has no distinctive smell or taste."
						(Memory memFREE global314)
						(Memory memFREE global323)
					)
				)
...
(class ll1Win of Window
	...
	(method (open)
		(if (not (HaveMouse))
			(= theOldCursor (gGame setCursor: 69 1))
		)
		(super open: &rest)
	)
	...
	(method (dispose)
		(super dispose: &rest)
		(if (not (HaveMouse))
			(gGame setCursor: theOldCursor 1)
		)
	)
)
...
(instance gcWin of Window
	...
	(method (open &tmp [temp0 14] [temp14 25] [temp39 4])
		...
		(Format @temp14 0 61 gScore gPossibleScore gScore gPossibleScore) ; "Score: %d of %d"
		...
		(cond
			((IsFlag 40)
				(if (< (self distanceTo: gEgo) 30)
					...
					(Print 0 62) ; "Hey, dork! You've got toilet paper stuck to your shoe!"
					(Print 0 63) ; "You nonchalantly scrape it off."
				)
			)
			((IsFlag 37)
				(if (< (self distanceTo: gEgo) 30)
					...
					(Print 0 64) ; "Phew! You smell like a used fire hydrant!"
				)
			)
			(
				(and
				...
				(Print 0 65) ; "Eeeyow! Do something about that breath of yours!"
			)
		)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				...
				(Print 0 66) ; "You'd better not. You might get your face slapped!"
			)
			(4 ; Inventory
				(if 1
					(= global314
					...
					(= global315 (Memory memALLOC_CRIT (StrLen description)))
					(= global324 (Memory memALLOC_CRIT (StrLen description)))
					(StrSplitInTwo
						global314
						global323
						((Inv at: invItem) description:)
					)
					(StrSplitInTwo global315 global324 description)
					(Printf 0 67 global314 global315 global323 global324) ; ""I have no use for %s!" says %s."
					(Memory memFREE global314)
					(Memory memFREE global315)
					(Memory memFREE global323)
					(Memory memFREE global324)
