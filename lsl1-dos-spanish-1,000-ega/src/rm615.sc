...
(instance rm615 of Rm
	...
	(method (init)
		...
		(herMouth cycleSpeed: (+ 2 gHowFast) init:)
		(= gSpraySeconds 600)
		(rightEye cycleSpeed: (+ 2 gHowFast) init:)
		(leftEye cycleSpeed: (+ 2 gHowFast) init:)
		(angryEyes cycleSpeed: (+ 2 gHowFast) init:)
		(fawnHead init:)
		(fawnBody init:)
		(fawnNeck init:)
		(super init:)
...
(instance sFrown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(angryEyes init: cycleSpeed: gHowFast setCel: 0 setCycle: End)
				...
				(switch invItem
					(0 ; wallet
						(cond
							...
							((and (IsFlag 10) (<= gLarryDollars 200)) ; fFawnAskedForBucks
								(= moneyTimer 0)
								(Printf 615 31 gLarryDollars gLarryDollars) ; "Uh, gosh, Fawn," you mumble. "I've only got $%d.00 bucks on me!"
					...
					(else
						(if 1
							...
							(Printf 615 51 global314 global323) ; "Oh, %s is sweet, but I'd like something special... from a guy like you!"
