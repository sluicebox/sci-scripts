...
(procedure (PrintJoke &tmp joke)
	...
	(switch joke
		...
		(23
			(Print 320 81) ; "My son has a problem with compulsive masturbation. His psychiatrist says he's gone completely whacky."
		)
		(24
...
(instance rm320 of LLRoom
	...
	(method (init &tmp i)
		...
		(if (IsFlag 98)
			...
			(= mcOnStage 1)
			(comedian cycleSpeed: gHowFast init: setScript: sComedian)
			(head cycleSpeed: gHowFast init:)
			(drummer cycleSpeed: (+ gHowFast 1) init:)
			(for ((= i 1)) (<= i 38) ((++ i))
				(jokes addToEnd: i)
			)
		else
		...
		(super init:)
		(light1 cycleSpeed: gHowFast setCycle: Fwd init:)
		(light2 cycleSpeed: gHowFast setCycle: Fwd init:)
		(light3 cycleSpeed: gHowFast setCycle: Fwd init:)
		(light4 cycleSpeed: gHowFast setCycle: Fwd init:)
		(light5 cycleSpeed: gHowFast setCycle: Fwd init:)
		(light6 cycleSpeed: gHowFast setCycle: Fwd init:)
		(table1 init:)
		(table2 init:)
		(table3 init:)
		(table4 init:)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(drummer setLoop: 1)
				(drummerHead cycleSpeed: gHowFast init: setCycle: Fwd)
			...
			(2
				...
				(Printf 320 17 gLarryDollars gLarryDollars 67 15 80 105) ; "I've GOT IT!" he exclaims. "If you open your billfold you'll discover some wrinkled business cards, lots of notes, some credit cards (which won't work in this game), and exactly $%d.00 in cash!!"
				(= seconds 5)
			)
			(3
				(if gModelessDialog
...
(instance sSit of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 320 20) ; "At least the comic's whoopie cushion doesn't smell!"
				(Points 89 1)
...
(instance drummer of Prop
	...
	(method (doVerb)
		(if (!= (comedian script:) sMagicTrick)
			(drummerHead cycleSpeed: gHowFast init: setCycle: Beg self)
		)
	)
)
