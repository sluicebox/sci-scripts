...
(instance rm510 of LLRoom
	...
	(method (dispose)
		(if (and lubberFlavor (== notPaid 0))
			...
			(if 1
				(= global314 (Memory memALLOC_CRIT (StrLen lubberWeight)))
				(= global323 (Memory memALLOC_CRIT (StrLen lubberWeight)))
				(= global315 (Memory memALLOC_CRIT (StrLen lubberThickness)))
				(= global324 (Memory memALLOC_CRIT (StrLen lubberThickness)))
				(= global316 (Memory memALLOC_CRIT (StrLen lubberFlavor)))
				(= global325 (Memory memALLOC_CRIT (StrLen lubberFlavor)))
				(= global317 (Memory memALLOC_CRIT (StrLen lubberPattern)))
				(= global326 (Memory memALLOC_CRIT (StrLen lubberPattern)))
				(= global318 (Memory memALLOC_CRIT (StrLen lubberLubrication)))
				(= global327 (Memory memALLOC_CRIT (StrLen lubberLubrication)))
				(= global319 (Memory memALLOC_CRIT (StrLen lubberColor)))
				(= global328 (Memory memALLOC_CRIT (StrLen lubberColor)))
				(= global320 (Memory memALLOC_CRIT (StrLen lubberTexture)))
				(= global329 (Memory memALLOC_CRIT (StrLen lubberTexture)))
				(= global321 (Memory memALLOC_CRIT (StrLen lubberMaterial)))
				(= global330 (Memory memALLOC_CRIT (StrLen lubberMaterial)))
				(= global322 (Memory memALLOC_CRIT (StrLen lubberCoating)))
				(= global331 (Memory memALLOC_CRIT (StrLen lubberCoating)))
				(StrSplitInTwo global314 global323 lubberWeight)
				(StrSplitInTwo global315 global324 lubberThickness)
				(StrSplitInTwo global316 global325 lubberFlavor)
				(StrSplitInTwo global317 global326 lubberPattern)
				(StrSplitInTwo global318 global327 lubberLubrication)
				(StrSplitInTwo global319 global328 lubberColor)
				(StrSplitInTwo global320 global329 lubberTexture)
				(StrSplitInTwo global321 global330 lubberMaterial)
				(StrSplitInTwo global322 global331 lubberCoating)
				...
				(Memory memFREE global316)
				(Memory memFREE global317)
				(Memory memFREE global318)
				(Memory memFREE global319)
				(Memory memFREE global320)
				(Memory memFREE global321)
				(Memory memFREE global322)
				(Memory memFREE global323)
				(Memory memFREE global324)
				(Memory memFREE global325)
				(Memory memFREE global326)
				(Memory memFREE global327)
				(Memory memFREE global328)
				(Memory memFREE global329)
				(Memory memFREE global330)
				(Memory memFREE global331)
			)
		)
		(gTheMusic fade:)
		(super dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(cam11 setCel: 0)
				(cam12 setCel: 4)
				(pClerk cycleSpeed: (+ 1 gHowFast) setCycle: RandCycle 20 self)
			)
			(6
				(if gModelessDialog
					(gModelessDialog dispose:)
			...
			(16
				(switch
					(Print ; ""What size you want?" he asks."
						510
						18
						#button
						{Large}
			...
			(20
				(sfxPervert play:)
				(Print 510 21 #font global312 #mode 1 #at -1 4 #width 250) ; "WHAT A PERVERT!!"
			...
			(21
				(Points 69 4)
				(Printf 510 22 moneyOwed moneyOwed) ; "He hands you the package, saying "All together, that will be $%d.00, prease.""
				(= askedForBucks 1)
				(gTheMusic play:)
				(HandsOn)
				(NormalEgo 1)
					...
					(0 ; wallet
						(cond
							((and readyForMoney notPaid (>= gLarryDollars moneyOwed))
								(if (== askedForBucks 0)
									(Printf 510 26 moneyOwed moneyOwed) ; "Oakie, doakie Meester! That'll be $%d.00."
								)
							...
							((< gLarryDollars moneyOwed)
								(Printf 510 29 gLarryDollars gLarryDollars) ; "Unfortunately, you only have $%d.00."
			...
			(5 ; Talk
				(if readSign
					...
					(switch clerkTalk
						...
						(1
							(Print 510 33 #dispose) ; "Poolskd asd woefpo lkjfq feps de foeps."
						)
						(2
