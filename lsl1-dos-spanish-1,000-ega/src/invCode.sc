...
(class LLinvItem of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(if 1
					(= global314 (Memory memALLOC_CRIT (StrLen (self description:))))
					(= global323 (Memory memALLOC_CRIT (StrLen (self description:))))
					(StrSplitInTwo global314 global323 (self description:))
					(Printf 814 0 global314 global323) ; "You might try using %s on yourself."
		...
		(switch theVerb
			(2 ; Look
				(if (or (== gCurRoomNum 250) (== gCurRoomNum 260))
					...
					(Print
						(Format @str 814 3 gLarryDollars gLarryDollars) ; "Your wallet contains some wrinkled business cards, lots of notes, some credit cards (which won't work in this game), and $%d.00 in cash."
						#icon
						view
						loop
						cel
