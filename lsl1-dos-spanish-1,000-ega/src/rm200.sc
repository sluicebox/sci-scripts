...
(instance rm200 of LLRoom
	...
	(method (init)
		...
		(windshield cycleSpeed: (+ gHowFast 5) init: cel: (Random 0 4))
		(driver cycleSpeed: gHowFast init:)
		(larry init: stopUpd:)
		...
		(meter3 init:)
		(meter4 init:)
		(rightTurn cycleSpeed: (+ gHowFast 3) setCycle: Fwd init:)
		(trunk init:)
		(license init:)
		(meter init:)
		(if (gEgo has: 12) ; wine
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				2
				(HandsOn)
				(leftTurn cycleSpeed: (+ gHowFast 3) setCycle: Fwd init:)
				(rightLight setCel: 0 stopUpd:)
				(leftLight setCel: 0 stopUpd:)
				(windshield setCycle: Fwd)
				(leftMirror cycleSpeed: (+ gHowFast 5) setCycle: Fwd init:)
				(rightMirror setCycle: Fwd cycleSpeed: (+ gHowFast 5) init:)
				(= cycles 60)
			)
			(3
				3
			...
			(9
				...
				(Printf 200 8 cabFare cabFare) ; "Well, here we are, buddy. That'll be $%d.00."
			)
			(10
				10
				(= badgerTimer 400)
	...
	(method (changeState newState &tmp carSpeed)
		(switch (= state newState)
			...
			(1
				...
				(windshield setCycle: Fwd)
				(leftMirror cycleSpeed: (+ gHowFast 5) setCycle: Fwd init:)
				(rightMirror cycleSpeed: (+ gHowFast 5) setCycle: Fwd init:)
				(meter4 setCycle: Fwd)
				(= cycles 20)
			)
			(2
