...
(procedure (HighLightCopy i c &tmp [tmpString 200])
	...
	(Format @string 720 12 (+ i 96)) ; "%c."
	(Display @string dsCOORD 50 atY dsCOLOR c dsFONT gUserFont)
	(GetFarText 719 (+ (* question 5) i) @tmpString)
	(Display @tmpString dsCOORD 65 atY dsCOLOR c dsFONT gUserFont dsWIDTH 205)
)
...
(procedure (SaveSetup &tmp [str 40])
	(if (!= (LARRY_DRV open: 2) 0)
		(Format @str 720 20 [filesSeen 0] [filesSeen 1] [filesSeen 2]) ; "%8d%8d%8d"
...
(instance RoomScript of Script
	...
	(method (changeState newState &tmp age i [tmpString 200])
		(switch (= state newState)
			(0
				(Print ; ""Leisure Suit Larry in the Land of the Lounge Lizards" contains some elements of plot which may not be considered appropriate for some children."
					720
					2
					#title
			...
			(5
				(gTheMusic2 play:)
				(Print 720 17) ; "And now a question that won't tax your memory. This question refers to the documentation that came with your copy of this game."
				(= question (Random 0 39))
				(GetFarText 719 (* question 5) @string)
				(= theAnswer (- (StrAt @string 0) 48))
				(for ((= i 0)) (<= i (StrLen @string)) ((++ i))
					(StrAt @tmpString i (StrAt @string (+ i 1)))
				)
				(= thisColor gColVLGreen)
				(= dQuestion
					(Display
						@tmpString
						dsCOORD
			...
			(6
				(if (== hisAnswer theAnswer)
					(HighLightCopy hisAnswer gColLGreen)
					(gTheMusic3 number: 711 play: 127)
					(Print 720 13 #at 200 34 #time 3) ; "Correct"
					(++ state)
				else
					(HighLightCopy hisAnswer gColVLRed)
					(gTheMusic3 number: 712 play:)
				)
				(= cycles 22)
			)
			(7
				(Print ; "Oh, oh! You couldn't possibly miss that question and have a legal copy of the game!"
					720
					18
					#title
					{Yo, ho. Yo, ho!}
			...
			(8
				...
				(Format @string 720 19) ; "Thank you. And now, slip into your leisure suit and prepare to enter the "Land of the Lounge Lizards" with "Leisure Suit Larry!""
