	...
	(method (init &tmp foo)
		...
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 235 96 85 96 85 109 157 109 157 128 137 138 57 156 38 177 70 182 210 182 210 171 221 162 254 154 240 136 214 114 241 114 248 95 248 0 319 0 319 189 0 189 0 0 235 0
					yourself:
				)
		)
		...
		(super init:)
		(door cycleSpeed: gHowFast approachVerbs: 3 4 10 11 init:) ; Do, Inventory, Zipper, Taste/Smell
		(fawn
			cycleSpeed: gHowFast
			moveSpeed: gHowFast
		...
		(if (IsFlag 41) ; fTurnedOnRadio
			...
			(if (not (IsFlag 24)) ; fOrderedWine
				(= radioTimer
					(Random (* (+ 1 gHowFast) 200) (* (+ 1 gHowFast) 300))
				)
			)
		)
		(if (IsFlag 24) ; fOrderedWine
			...
			(wine init: stopUpd: approachVerbs: 2 3 4 10 11) ; Look, Do, Inventory, Zipper, Taste/Smell
		)
		(theBed init:)
		(theWindow init:)
		...
		(cond
			...
			(radioTimer
				...
				(if (and (== radioTimer 1) (not (gCurRoom script:)))
					(= radioTimer
						(Random (* (+ gHowFast 1) 600) (* (+ gHowFast 2) 600))
					)
					(HandsOff)
	...
	(method (doVerb theVerb invItem)
		(if (not (== (gEgo view:) 391))
			(switch theVerb
				(4 ; Inventory
					(switch invItem
						(8 ; lubber
							(if wearingRubber
								(Print 390 10) ; "You already are wearing your "lubber!""
							else
								(Print 390 11) ; "O.K. Good idea."
								(= wearingRubber 1)
							)
						)
					)
				...
				(10 ; Zipper
					(cond
						((not (IsFlag 24)) ; fOrderedWine
							(Print 390 12) ; "Hey, don't rush it, buster! Can't a gal even have a drink first?" she "wines." "Geez, what a romantic!"
						)
						((not pouredWine)
							(Print 390 13) ; "Aren't you going to pour the wine?" she says, her eyes flashing."
						)
						((not doneForeplay)
							(Print 390 14) ; "Ain'cha never heard of `foreplay,' Larry?" says Fawn. "I ain' no rabbit, you know!"
						)
			...
			(return
				(switch theVerb
					(3 ; Do
						(Print 390 15) ; "You can't. She fastened you quite securely!"
					)
					(5 ; Talk
						(Print 390 16) ; "FAWN!! You scum-sucking gutterslug, get back in here!"
						(Print 390 8) ; "Unfortunately, your touching plea goes unheeded."
					)
					(4 ; Inventory
						(switch invItem
		...
		(switch (= state newState)
			...
			(5
				(if (and (IsFlag 24) (not (IsFlag 36))) ; fOrderedWine, fSeenDeliveryBoyGag
					...
					(Print 390 17 #at -1 20) ; "Oh, Larry, it's you!" Fawn says. "I thought it was that cute, young delivery boy again."
					(Print 390 18) ; "Hmm. What does she mean by that? Oh, well. You decide to let it pass. After all, this IS your wedding night!"
				else
					(Print 390 19 #at -1 20) ; "Oh, Larry!" oozes Fawn, "I've been waiting patiently right here on the bed just for you."
				)
...
(instance sGetHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= radioTimer 0)
				(Print 390 20) ; "She says, "Lie down, Larry. I have a special little surprise for you!""
			...
			(2
				(Print 390 21 #at -1 19 #width 280) ; "Then she says, "Close your eyes," and you do because you're sure that, finally, success is at hand."
			...
			(3
				(Print 390 22 #at -1 20) ; ""Oh, Fawn!" you moan."
				(= seconds 4)
			)
			(4
				(Print 390 23 #at -1 20) ; "Ooh, Baby! That feels so good! Ooooooooooh!!"
				(= seconds 4)
			)
			(5
				(Print 390 24 #at -1 20) ; "Say, Fawn, when are you going to get undressed?"
				(= seconds 4)
			)
			(6
				(Print 390 25 #at -1 20) ; "Hey, what are you doing with the ribbon?"
				(= seconds 4)
			)
			(7
				(Print 390 26 #at -1 20) ; "Oh, I see! We're gonna get kinky our first time?"
				(= seconds 2)
			)
			(8
			...
			(10
				(Print 390 27) ; "When Fawn has you securely tied to the bed, she leaps from the bed, grabs your wallet, and says, "You forgot to pay me for the wine!""
				(if (not (<= gLarryDollars 1))
					(Printf 390 28 fawnDollars fawnDollars) ; "It came to $%d.00, including the tip."
				)
				(= cycles (* (+ 1 gHowFast) 5))
			)
			(11
			...
			(12
				(Print 390 29) ; "Wait, Fawn!" you shout, "Come back! This is our wedding night!"
				(door setCycle: End self)
				(gTheMusic3 number: 801 setLoop: 1 play:)
			)
			(13
				(Print 390 30) ; "So long, sucker!"
				(= seconds 3)
			)
			(14
				(Print 390 31) ; "FAWN! Come back!" you shout. "This isn't funny!!"
			...
			(17
				(fawn dispose:)
				(Print 390 32) ; "Hmmm. Does it count if a guy "consummates" by himself?"
				(= seconds 2)
			)
			(18
				(Print 390 33) ; "Larry, it seems "the honeymoon's over.""
				(if (not (gEgo has: 11)) ; pocketKnife
					(= knifeTimer (* (+ 1 gHowFast) 300))
				)
				(HandsOn)
				(User canControl: 0)
...
(instance sCutLoose of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 390 34) ; "Right! Using the pocket knife you got from the old bum by the convenience store, you saw through the ribbon holding you to the bed."
			...
			(8
				...
				(if wearingRubber
					(Print 390 35) ; "You carefully remove your "lubber.""
				)
			)
			(9
			...
			(11
				(Printf ; "You grab your wallet and look inside. Oh, well. At least she didn't find the %d buck%s you hid in that secret compartment. With your consummate gambling skills, you should be back on your feet in no time!"
					390
					36
					gLarryDollars
					(if (== gLarryDollars 1) {} else {s})
					gLarryDollars
					(if (== gLarryDollars 1) {} else {s})
				)
				(ribbon approachX: 89)
...
(instance sGetRibbon of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 390 37) ; "Good idea. You never know when a nice piece of ribbon will come in handy."
				(gEgo get: 16) ; ribbon
				(ribbon dispose:)
				(Points 76 3)
		...
		(switch (= state newState)
			...
			(1
				(if register
					...
					(Print 390 38) ; "Sounds like a nice mellow station..."
					(Print 390 39) ; "The tuning knob on the radio is stuck. Looks like you're stuck with the Easy Listening station."
					(if (not (IsFlag 24)) ; fOrderedWine
						(= radioTimer
							(Random
								(* (+ 1 gHowFast) 200)
...
(instance sPourWine of Script
	...
	(method (changeState newState &tmp foo)
		(switch (= state newState)
			...
			(5
				(Print 390 41 #at -1 19 #width 280) ; "You carefully remove the bottle from the ice bucket, unscrew the cap, and pour two glasses. She downs her glass immediately, you refill it, and she does it again. Her eyes meet yours, and you're sure luck is with you this time!"
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(Print 390 42 #at -1 20 #dispose) ; "The radio interrupts the lovely Frank Sinatra medley with an obnoxious singing commercial..."
				(= seconds 6)
			)
			(1
				(gTheMusic2 play:)
				...
				(Print 390 43 #at -1 20 #dispose) ; "We're Ajax Liquor, And we're proud to say, We deliver 'cross The U.S.A. You're out of luck If you want to dine, But if you just need booze, Dial 555-8039!"
			)
			(2
				(if gModelessDialog
					(gModelessDialog dispose:)
				...
				(Print 390 44 #at -1 20 #dispose) ; "And now, back to our music."
				(= seconds 4)
				(Points 80 1)
			)
			(3
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(cond
					((== cel 1)
						(Print 390 45) ; "The remains of the ribbon are lying on the bed."
					)
					((== (gEgo view:) 391)
						(Print 390 46) ; "Isn't it amazing how a simple ribbon can hold you so securely?"
					)
					(else
						(Print 390 47) ; "Isn't that cute? The honeymoon suite bed is decorated like a giant box of candy--complete with a cute red ribbon."
					)
				)
			)
			(3 ; Do
				(cond
					((== (gEgo view:) 391)
						(Print 390 48) ; "You can't do anything. Fawn has you fastened quite securely!"
					)
					((self cel:)
						(HandsOff)
			...
			(5 ; Talk
				(if (== (gEgo view:) 391)
					(Print 390 7) ; "FAWN!! You scum-sucking gutterslug, get back in here!"
					(Print 390 49) ; "Unfortunately, your pleas go unheeded."
				else
					(super doVerb: theVerb invItem)
				)
			)
			(11 ; Taste/Smell
				(Print 390 50) ; "The ribbon smells Fawny."
			)
			(else
				(if (== (gEgo view:) 391)
					(Print 390 9) ; "You can't. You're tied to a bed!"
		...
		(if (== (gEgo view:) 391)
			...
			(switch theVerb
				...
				(4 ; Inventory
					(switch invItem
						(14 ; hammer
							(Print 390 53) ; "You can't. It's the same brand that people can't break when they want to christen a ship!"
						)
						(else
							(super doVerb: theVerb invItem)
						)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(cond
					(pouredWine
						(Print 390 54) ; "She looks ready, Larry."
					)
					((IsFlag 24) ; fOrderedWine
						(Print 390 55) ; "She looks like she could use some of that wine."
					)
					(else
						(Print 390 56) ; "She is so beautiful you want to do wonderful things to, er, ah, FOR her."
					)
				)
			)
			(5 ; Talk
				(cond
					((not (IsFlag 24)) ; fOrderedWine
						(if (Random 0 1)
							(Print 390 57) ; "You know, a little wine would help me get in the mood, Larry."
						else
							(Print 390 58) ; "I'd think the least a gentleman would do is to buy a lady a bottle of wine."
						)
					)
					((not pouredWine)
						(Print 390 59) ; ""Aren't you going to pour the wine?" she says, her eyes flashing."
					)
					(else
						(Print 390 60) ; "I'm ready when you are, honey!"
						(Print 390 61 #at -1 140) ; "(Come on, Larry; what are you waiting for?)"
					)
				)
			)
			(3 ; Do
				(cond
					((not (IsFlag 24)) ; fOrderedWine
						(Print 390 62) ; "She doesn't appear to be in the mood yet, Larry."
					)
					((not pouredWine)
						(Print 390 59) ; ""Aren't you going to pour the wine?" she says, her eyes flashing."
					)
					(else
						(Print 390 63) ; ""Ahh, that feels nice!" sighs Fawn."
						(Print 390 64 #at -1 140) ; "(You presume a little foreplay never hurts.)"
						(= doneForeplay 1)
					)
				)
			)
			(10 ; Zipper
				(cond
					((not (IsFlag 24)) ; fOrderedWine
						(Print 390 65) ; "Hey, don't rush it, buster! Can't a gal even have a drink first?" she "wines." "Geez, what a romantic!"
					)
					((not pouredWine)
						(Print 390 59) ; ""Aren't you going to pour the wine?" she says, her eyes flashing."
					)
					((not doneForeplay)
						(Print 390 66) ; "Ain'cha never heard of `foreplay,' Larry?" "I ain' no rabbit, you know!"
					)
					(else
						(HandsOff)
						(gCurRoom setScript: sGetHim)
			...
			(11 ; Taste/Smell
				(Print 390 67) ; "Fawn smells like a dream."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance theBed of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 390 68) ; "You have more important things on your mind than sitting."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance theWindow of Feature
	...
	(method (doVerb theVerb invItem)
		(if (== (gEgo view:) 391)
			...
			(switch theVerb
				(3 ; Do
					(Print 390 69) ; "This window doesn't open."
				)
				(4 ; Inventory
					(Print 390 70) ; "You are unable to break the window."
				)
				(else
					(super doVerb: theVerb invItem &rest)
				)
...
(instance painting of Feature
	(properties
		...
		sightAngle 40
		lookStr {To you, it looks like dozens of spermatazoa attacking a Whitman's sampler!}
	)
)
		...
		(switch theVerb
			(2 ; Look
				(if (gCast contains: wine)
					(Print 390 71) ; "The ice bucket holds your fancy bottle of wine."
				else
					(Print 390 72) ; "An ice bucket waits beside the bed for that bottle of wine you didn't bring!"
				)
			)
			(3 ; Do
				(if (gCast contains: wine)
					...
					(Print 390 73) ; "You can't. Room service glued it to the table."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance vase of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 390 74) ; "Oh, just leave the rose in its vase."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance theChair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 390 75) ; "You're in the Honeymoon Suite and all you want to do is sit on a chair?!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance flower of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 12) ; fFawnInRoom
					(Print 390 76) ; "Why, Fawn," you say, "you placed my rose in a different vase."
					(Print 390 77) ; "But, of course, my dear," she replies, "but let's not talk about flora at a time like this!"
				else
					(Print 390 78) ; "You hope at least the rose pricked Fawn."
				)
			)
			(3 ; Do
				(if (IsFlag 12) ; fFawnInRoom
					(Print 390 79) ; "You only gave it to her a little while ago. Don't take it now."
				else
					(Print 390 80) ; "Why take it? You don't want to remember Fawn (that scum-sucking gutterslug!)."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
