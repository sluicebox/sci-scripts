...
(instance rm100 of LLRoom
	...
	(method (init)
		...
		(door cycleSpeed: gHowFast init: approachVerbs: 3 stopUpd:) ; Do
		(if (< (LarryHours) 4)
			(leftySign cycleSpeed: (+ 3 gHowFast) setCycle: Fwd init:)
			(hotelSign cycleSpeed: 30 setCycle: Fwd init:)
			(glass cycleSpeed: (+ 2 gHowFast) setCycle: Fwd init:)
		)
		(taxiSign
			approachY: 159
			approachX: (gEgo x:)
...
(instance building of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (> 2 (LarryHours) 10)
					(Print 100 26) ; "Lefty's closes at 3:00 a.m."
				else
					(Print 100 27) ; "Considering the looks of this place, you decide that next time you'd be better off to spend a little more money on your computer games!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance aCop of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 100 28) ; ""STOP, PERVERT!!!" shouts the cop. "I wonder who he's yelling at?" you think."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance doormat of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 100 29) ; "Amazed by your own cleverness, you lift the door mat and discover..."
				(Print 100 30) ; "...absolutely nothing!"
				(Print 100 31 #at -1 140) ; "(What did you expect? A key?)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance cans of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 100 32) ; "Now that you're here, you decide you don't really want to look inside."
			)
			(11 ; Taste/Smell
				(Print 100 33) ; "Lefty's wife, Linda, must have made her famous tuna casserole again!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
