...
(local
	...
	larrySplash
	palDelay
	[INTO_TUB_PATH 41] = [0 0 159 136 0 1 159 136 0 2 159 136 0 3 154 114 0 4 150 105 0 5 144 94 0 6 138 92 0 6 129 111 0 7 141 133 0 8 133 134 -32768]
	[EXIT_TUB_PATH 29] = [4 1 117 153 4 2 121 150 4 3 125 147 4 4 129 144 4 5 133 141 4 6 137 138 4 7 144 134 -32768]
	[EXIT_TUB_AND_DRESS_PATH 33] = [6 0 118 154 6 1 124 151 6 2 130 149 6 3 136 146 6 4 142 144 6 5 148 141 6 6 154 139 6 7 159 136 -32768]
)
...
(procedure (JetsOn)
	...
	(rimJet1 init: cycleSpeed: gHowFast setCycle: Fwd)
	(rimJet2 init: cycleSpeed: gHowFast setCycle: Fwd)
	(rimJet3 init: cycleSpeed: gHowFast setCycle: RandCycle)
	(aJet init: cycleSpeed: gHowFast setCycle: RandCycle)
	(bJet init: cycleSpeed: gHowFast setCycle: RandCycle)
	(cJet init: cycleSpeed: gHowFast setCycle: RandCycle)
	(dJet init: cycleSpeed: gHowFast setCycle: RandCycle)
	(eJet init: cycleSpeed: gHowFast setCycle: RandCycle)
)
	...
	(method (init)
		...
		(if (== gPrevRoomNum 385)
			...
			(if (IsFlag 4) ; fDollFlies
				(aDoll cycleSpeed: gHowFast moveSpeed: gHowFast init:)
				(gEgo posn: 231 113)
...
(instance sUndress of Script
	...
	(method (doit)
		(super doit: &rest)
		(if (and (== larrySplash 1) (== (gEgo cel:) 7))
			(= larrySplash 0)
			(sfxSplash play:)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo illegalBits: 0 setMotion: PolyPath 159 136 self)
			)
			(1
			...
			(3
				...
				(gEgo
					...
					setPri: 10
					setCel: 0
					setCycle: MCyc @INTO_TUB_PATH self
				)
			)
			(4
		...
		(switch (= state newState)
			...
			(1
				(gEgo
					egoSpeed: (+ 1 gHowFast)
					view: 381
					setCycle: MCyc @EXIT_TUB_AND_DRESS_PATH self
				)
			)
			(2
		...
		(switch (= state newState)
			...
			(4
				(gEgo
					...
					setCel: 0
					posn: 117 153
					setCycle: MCyc @EXIT_TUB_PATH self
				)
			)
			(5 0)
			(6
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo egoSpeed: gHowFast setMotion: PolyPath 170 131 self)
			)
			(1
			...
			(5 ; Talk
				(cond
					...
					(else
						...
						(Print 380 22 #at -1 140) ; "(Was she talking to you, Larry?)"
						(SetFlag 7) ; fEveKnowsLarry
					)
				)
			)
			(3 ; Do
				(Print 380 23) ; "There's not much you can do from out here."
			)
			(else
				(Print 380 24) ; "Perhaps if you establish eye contact first..."
			)
		)
	)
)
...
(instance aJet of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 380 25) ; "You wonder if there's any way to turn off those bubbles!"
			)
			(else
				(Print 380 26) ; "Doesn't that water look inviting?"
			)
		)
	)
)
		...
		(switch theVerb
			(3 ; Do
				(if (IsFlag 17) ; fJetsOff
					...
					(JetsOn)
					(Print 380 27) ; "Because you are such a wimp, you turn the bubblers back on."
				else
					(SetFlag 17) ; fJetsOff
					(JetsOff)
					(Print 380 28) ; "You coyly "happen" to step on the spa bubbler control switch."
				)
			)
			(else
...
(instance fHotels of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 380 29) ; "Looking over at the next hotel, you see a window with its drapes wide open."
				(Print 380 30) ; "Say, look at that. There's an ugly girl getting undressed."
				(Print 380 31) ; "She's not really very pretty."
				(Print 380 32) ; "Well, she's not bad."
				(Print 380 33) ; "You know, she's looking better."
				(Print 380 34) ; "Say, she's pretty good."
				(Print 380 35) ; "Perhaps you could ignore some of her faults."
				(Print 380 36) ; "You may be in love!"
				(Print 380 37) ; "You ARE in love!"
				(Print 380 38) ; "Well, at least you're in lust!"
				(Print 380 39) ; "Pant, pant, pant, pant, pant."
			)
			(else
				(Print 380 40) ; "Say, what's that in that hotel window across the way?"
			)
		)
	)
)
