...
(instance rm120 of LLRoom
	...
	(method (init)
		...
		(switch gPrevRoomNum
			(110
				(if (Random 0 1)
					(aRat cycleSpeed: gHowFast init: setScript: sRatScript)
				)
			)
			(130
				(aDoor setCel: 255)
		...
		(aDoor cycleSpeed: gHowFast init: stopUpd: approachVerbs: 3) ; Do
		(aDrunk cycleSpeed: gHowFast init: stopUpd: approachVerbs: 3 5 4) ; Do, Talk, Inventory
		(drunkHead cycleSpeed: gHowFast init: approachVerbs: 3 5 4) ; Do, Talk, Inventory
		(drunkLeg init: stopUpd: approachVerbs: 3 5 4) ; Do, Talk, Inventory
		(drunkArm cycleSpeed: gHowFast init: approachVerbs: 3 5 4) ; Do, Talk, Inventory
		(boxes init:)
		(barrels init:)
...
(instance sDrunkDrinks of Script
	...
	(method (doit)
		(super doit: &rest)
		(if (and (== state 5) (== (gTheMusic3 prevSignal:) -1))
			(= cycles 1)
		)
	)
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					...
					(12 ; wine
						(Print 120 19) ; "Hey, waddaya think, I gotno class?" mumbles the drunk. "I'm a whiskey drinker, pure and simple!"
					)
					(2 ; watch
...
(instance theFan of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 120 29) ; "You can't reach that high."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
