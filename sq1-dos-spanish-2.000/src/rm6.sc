...
(instance rm6 of SQRoom
	...
	(method (init)
		...
		(alertLight view: (proc0_19 106 1106 106 106 106) setCycle: Fwd init:)
		(if (OneOf gPrevRoomNum west east)
			(HandsOn)
		)
	)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(gEgo setLoop: 2)
				(if (and (>= gHowFast 1) (not (proc700_1 554 2)))
					(= local104 1)
...
(instance searchBody of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if register
					(Printf ; "Your search of %s reveals nothing even you'd carry."
						6
						0
						((CueObj client:) description:)
						((CueObj client:) description:)
					)
					(= state 2)
				)
