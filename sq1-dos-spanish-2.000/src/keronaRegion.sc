...
(instance mountains of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 2) ; "Mountains generously dappled with spires of unknown formational origin loom at your visual terminus."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance keronaSky of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 3) ; "The lovely crisp green of the Keronian atmosphere reminds you that you'll need to do some laundry when (or if) you get back home."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance moon of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 4) ; "Loitering about the horizon is the second and closest moon of Kerona. It is much less hospitable than the sphere you presently roam."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance farSand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 5) ; "Spread before you is a seemingly endless expanse of sand."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance nearSand of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 6) ; "The sand of Kerona has a reddish-orange color. Were you a geologist instead of a janitor, you might find this fact interesting."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance dune of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 7) ; "Nomads of the desert, dunes are spread about in nearly every direction."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance bones of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 704 8) ; "A wide selection of skeletal accessories awaits your perusal. Vertebrae, ribs, and who knows what else rest upon the Keronian soil. The average height of this structure looks to be at least 7 meters. Good thing you didn't meet this thing in its heyday."
			)
			(3 ; Do
				(Print 704 9) ; "The bones, though utterly dehydrated, still bear too much mass for the puny likes of you to displace."
			)
			(11 ; Taste
				(Print 704 10) ; "While it might make one heck of a great soup bone, it bears no other appeal to your palate."
			)
			(5 ; Talk
				(Print 704 11) ; "Your words hitch a ride on the Keronian breeze never to be heard again."
			)
			(12 ; Smell
				(Print 704 12) ; "These bones have had the smell baked right out of them by centuries of relentless, Keronian sun. Any soft tissue was long ago ingested by the local parasites."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance base of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 704 13) ; "Upon a very narrow width of Keronian soil, able to resist erosion through the eons, rests the skeletal remains of this mammoth has-been."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(cond
					((gEgo has: 16) ; Plant
						(Print 704 14) ; "You already have your allocation of adhesive herbs."
					)
			...
			(12 ; Smell
				(Print 704 15) ; "You take a whiff of the plant and find it to be a regrettable experience. However, it does have an aroma reminiscent of a smell-a-vision movie production of 'Our Friend The Polymoss Shrub - Nature's Bondo'. You vaguely remember something about it having odd adhesive properties"
			)
			(11 ; Taste
				(Print 704 16) ; "Do that and you'll never utter another stupid syllable in your life. On second thought..."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance pickPlant of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 704 18) ; "You snag a small cluster of leaves from the gooey plant."
			...
			(3
				(if (== (gCurRoom script:) self)
					...
					(SetScore 2 142)
					(gEgo get: 16) ; Plant
					(Print 704 19) ; "That stuff sticks worse than fortnight-old undergarments. It's a good thing your gloves are tefloid-coated."
					(HandsOn)
				)
				(self dispose:)
			)
...
(instance egoDiesOfThirst of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 704 20) ; "Holy antiperspirant, dehydroman! The searing heat and dryness of this environment sucks the vital fluids from your body. Your mind begins to drift uncontrollably."
				(Print 704 21) ; "Mommy! Is that you? Mommy, how come all the other guys in class get new mops and I don't?"
				(Print 704 22) ; "Mercifully (for all of us), you die before the hallucinations get too hokey. It won't be long before you look like a scaled-down version of the skeletal structure occupying several hectares nearby."
				(= cycles 12)
			)
			(2
			...
			(4
				(EgoDead 948 0 0 704 23) ; "Within a few hours, your gaunt corpse will have dried to a crisp, your powdered remains evenly distributed across the parched terrain by a searing gust."
			)
		)
	)
)
...
(instance drinkScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 704 24) ; "Mmmmm...That dehydrated water really hit the spot! That should keep you going a bit longer."
				(gEgo
					setLoop: (- (gEgo loop:) 2)
					cel: 9
					setCycle: Beg self
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: 0)
				(Print 704 25) ; "Out of the corner of your eye, you spot an object hurtling from the greenish atmosphere toward the parched surface you currently occupy."
			...
			(7
				(Print 704 26) ; "Rats! If the eyes don't deceive, that's a Sarien Spider Droid. They must have detected the escape pod leaving the Arcada. The Spider Droid must have been sent along to settle any unfinished business."
				(Print 704 27) ; "After the jarring impact, small panels open through which legs sprout. You recall reading in an old issue of Space Piston magazine that this droid was designed to seek out organic life forms and self-destruct when close proximity to the target has been achieved."
				(spider
					view: 422
					setLoop: -1
					setLoop: spiderGroop
...
(instance bigBang of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(EgoDead 948 0 0 704 28) ; "How convenient! You've been blown into handy, bite-sized chunks. I guess that Space Piston article wasn't fiction. There's nothing quite like stretching out and enjoying the wide-open spaces."
			)
		)
	)
)
...
(instance spider of spiderRP
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 704 29) ; "Get closer."
			)
			(5 ; Talk
				(Print 704 30) ; "You greet the droid with a pleasant "Hello.""
				(Print 704 31) ; ""Say, I've got this annoying itch on the back of my sphere. Do you think you could scratch it for me? I'd be forever grateful." it responds."
			)
			(11 ; Taste
				(Print 704 32) ; "If you've got a taste for chrome, step on up and give it a try."
			)
			(12 ; Smell
				(Print 704 33) ; "You're not close enough to give it a good whiff if that's what you'd really like to do."
			)
			(4 ; Inventory
				(switch invItem
					(10 ; buckazoid
						(Print 704 34) ; "He seems unlikely to be willing to accept a bribe."
					)
					(0 ; Cartridge
						(Print 704 35) ; "If you let him eat the data cartridge, this is going to be a real short game."
					)
					(15 ; Widget
						(Print 704 36) ; "The good news is: the widget, being magnetic, will stick to the Spider Droid like ugly on an ape. The bad news is: The Spider Droid will have your widget and you won't."
					)
					(2 ; Gadget
						(Print 704 37) ; "Your attempts to translate the Spider Droid's clacking and whirring prove futile."
					)
					(3 ; Survival_Kit
						(Print 704 38) ; "Dropping your survival kit on the Spider Droid might prove hazardous to your survival."
					)
					(5 ; Dehydrated_Water
						(Print 704 39) ; "What a great idea! You could get him wet, then wait around for the next fifty years for him to rust out. On second thought, you decide to skip it."
					)
					(4 ; Knife
						(Print 704 40) ; "You couldn't even make a scratch on this guy."
					)
					(16 ; Plant
						(Print 704 41) ; "You attemp to drop the piece of plant down onto the Spider Droid, but it sticks to your fingers. Better try something else."
					)
					(8 ; Orat_Part
						(Print 704 42) ; "You find yourself reluctant to part with this dainty souvenir of the plant Kerona."
					)
					(else
						(Print 704 43) ; "If it could survive atmospheric entry and the jarring landing, you surely can't upset it."
					)
				)
			)
			(else
