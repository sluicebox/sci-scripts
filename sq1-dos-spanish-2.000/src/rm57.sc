...
(instance washerDoor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (== (DeltaurRegion egoStatus:) 0)
					(Print 57 9) ; "Inside the machine, you can see a load of dirty Sarien uniform components."
				else
...
(instance sarien of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(11 ; Taste
				(Print 57 24) ; "You can't do that from in here."
			)
			(12 ; Smell
				(Print 57 24) ; "You can't do that from in here."
			)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					(else
						(Print 57 25) ; "There's no need to go through that again!"
					)
				)
			)
			(2 ; Look
				(Print 57 26) ; "Another helmet sits in the washer."
			)
			(11 ; Taste
				(Print 57 27) ; "Ugh! It tastes like detergent."
			)
			(12 ; Smell
				(Print 57 28) ; "You see a hint of scaley dandruff."
			)
			(4 ; Inventory
				(Print 57 29) ; "That's not helpful."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance ventShaft of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 57 30) ; "You can't do anything to it."
			)
			(2 ; Look
				(Print 57 31) ; "A large vent shaft protrudes from the left wall of the laundry room."
			)
			(11 ; Taste
				(Print 57 32) ; "That's not a good idea."
			)
			(12 ; Smell
				(Print 57 32) ; "That's not a good idea."
			)
...
(instance shadows of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 57 33) ; "It's just a shadow."
			)
			(2 ; Look
				(Print 57 34) ; "Various pipes, ducts and other innocuous pieces of equipment form pronounced shadows throughout the room."
			)
			(11 ; Taste
				(Print 57 33) ; "It's just a shadow."
			)
...
(instance floor2 of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 57 35) ; "The floor's nice and shiny. Even if it wasn't, you don't have the spare to work on it."
			)
			(2 ; Look
				(Print 57 36) ; "The floor's nice and shiny."
			)
			(11 ; Taste
				(Print 57 37) ; "Just what you thought - waxey build-up. You'd be ashamed if you were in charge of this floor."
			)
			(12 ; Smell
				(Print 57 38) ; "It smells waxey."
			)
			(4 ; Inventory
				(Print 57 39) ; "No result is forthcoming."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					(else
						(Print 57 40) ; "There's no need to go through that again!"
					)
				)
			)
			(2 ; Look
				(Print 57 41) ; "This is almost certainly a cheap Sarien knock-off of the genuine Cleanse-O-Matic Rinse 'n Dip used to launder the crew uniforms aboard the Arcada."
			)
			(11 ; Taste
				(Print 57 42) ; "Quit goofing off and do something useful!"
			)
			(12 ; Smell
				(Print 57 43) ; "A faint miasma of Hijax liquid detergent hovers in the air near the machine."
			)
			(4 ; Inventory
				(Print 57 44) ; "That's of no help."
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(Print 57 45) ; "That was a rough and nearly nauseating ride. You almost lost those Keronian Ales you downed back at Ulence Flats. Your head is definitely not clear."
			)
			(5
			...
			(19
				(Print 57 46) ; "Whoa... what a ride! Your stomach feels a bit better now and the room doesn't seem to be moving by itself any longer."
				(gEgo
					view: 32
					setLoop: 5
					posn: 112 158
...
(instance getInWasher of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(6
				(Print 57 47) ; "Uh, oh! You hear someone coming."
				(sarien
					show:
					startUpd:
					x: 278
			...
			(23
				(Print 57 48) ; "Darn static cling! Hey, look at that! By the most amazing stroke of luck, you've traded in your extremely conspicuous Xenon space suit for a Sarien officer's uniform complete with helmet."
				(SetFlag 65)
				(if (gEgo has: 0) ; Cartridge
					(Print 57 49) ; "Searching the pockets of your newly found disguise, you find the number of possessions you were packing has been greatly reduced. In fact, lost in that limbo void to where socks and baseballs disappear is everything but the data cartridge."
				else
					(Print 57 50) ; "Searching the pockets of your newly found disguise, you find the number of possessions you were packing has been greatly reduced. In fact, everything was lost in that limbo void to where socks and baseballs disappear except one lucky buckazoid."
				)
...
(instance killEgoTimeOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 57 51) ; "It sounds like someone is approaching this room!"
				(= seconds 20)
			)
			(2
				(Print 57 52) ; "You hear footsteps approaching. You only have seconds to do something!"
				(= seconds 10)
			)
			(3
				(= local212 1)
...
(instance egoTestVent of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 57 53) ; "This vent is shut tight. You don't think anything short of kicking it from the other side could get it open."
				(gEgo
...
(instance cantReachIt of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 57 54) ; "You can't seem to reach the vent from here"
				(gEgo setCycle: Beg self)
			)
			(4
				(proc703_1)
