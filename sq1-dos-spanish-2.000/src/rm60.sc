...
(instance iris of Prop
	...
	(method (doVerb theVerb)
		(if (== global166 1)
			(switch theVerb
				(2 ; Look
					(Print 60 1) ; "This utilitarian-looking door probably leads to the trash disposal facility."
				)
				(3 ; Do
					(gEgo setScript: shreadEgo)
				)
				(12 ; Smell
					(Print 60 2) ; "Smells like garbage, doesn't it?"
				)
				(11 ; Taste
					(Print 60 3) ; "Oh, come on!"
				)
				(5 ; Talk
					(Print 60 4) ; "You holler down the chute, but no one answers. You're relieved, somehow."
				)
				(else
					(super doVerb: theVerb &rest)
				)
			...
			(Print 60 5) ; "Your on the wrong level to do that."
		)
	)
)
...
(instance pe1 of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 60 6) ; "Vast amounts of energy."
		)
		(if (== global166 2)
...
(instance beam of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 60 7) ; "An intense beam of energy."
		)
		(if (== global166 2)
			(if (and (== theVerb 4) (== invItem 12)) ; Inventory, Pulseray_Laser_Pistol
				(gCurRoom setScript: shipBlowsUp)
...
(instance lamp of Prop
	...
	(method (doVerb theVerb)
		(if (== global166 1)
			(switch theVerb
				(3 ; Do
					(Print 60 8) ; "You're not at home with the idea of touching this thing."
				)
				(12 ; Smell
					(Print 60 9) ; "The electric charge around the globe tickles your nose hairs."
				)
				(11 ; Taste
					(Print 60 10) ; "You really don't want to lick this."
				)
				(else
					(super doVerb: theVerb &rest)
				)
...
(instance shreadEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 160 8 0 60 11) ; "Yikes! I guess that was a bad idea, eh! I guess you'll be staying out of the garbage disposal in the future."
				(self dispose:)
			)
		)
	)
...
(instance toastEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 80 0 5 60 12) ; "Gee, that must have been a pretty powerful energy beam, seeing as how it reduced you to a pile of carbonized matter."
				(self dispose:)
			)
		)
	)
...
(instance shipBlowsUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(EgoDead 945 0 0 60 13) ; "Good shootin', Tex. You really did it now, Wilco. Don't you know that macromat field integrators are extremely fragile pieces of equipment and indiscriminatly shooting at them with a pulseray gun can be hazardous to your health?"
				(self dispose:)
			)
		)
	)
...
(instance balls of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 14) ; "From the looks of them, these globes contain high-power Wally coils, each one capable of discharging jillions of volts of deadly electricity."
			)
			(12 ; Smell
				(Print 60 15) ; "This close to the Wally globes, all your nose hairs are standing on end."
			)
			(11 ; Taste
				(Print 60 16) ; "I don't think you want to do that."
			)
			(5 ; Talk
				(Print 60 17) ; "Get real."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance mosl of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(12 ; Smell
				(Print 60 18) ; "Smells sort of blue.#Huele como a azul."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance hb of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 19) ; "The egg-shaped globe pulses with blue-white light. It appears to be some exotic form of bottled energy, or maybe a 60,000 watt light bulb."
			)
			(3 ; Do
				(Print 60 8) ; "You're not at home with the idea of touching this thing."
			)
			(12 ; Smell
				(Print 60 20) ; "The electric charge around the globe tickles your nose hairs."
			)
			(11 ; Taste
				(Print 60 10) ; "You really don't want to lick this."
			)
...
(instance mdofl of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 21) ; "Looks like a great place to curl up and take a nap. Too bad you're in such a hurry."
			)
			(3 ; Do
				(Print 60 22) ; "Hmmmm. Soft."
			)
			(11 ; Taste
				(Print 60 23) ; "Fuzzy. Provocative. Well-seasoned."
			)
			(5 ; Talk
				(Print 60 24) ; "There's nobody in there."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance fllym of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 25) ; "It's an expensive-looking machine that goes `ping.'"
			)
			(3 ; Do
				(Print 60 26) ; "You are reluctant to tarnish the shiny gold casing."
			)
			(12 ; Smell
				(Print 60 27) ; "It smells expensive."
			)
			(11 ; Taste
				(Print 60 28) ; "Not bad."
			)
...
(instance flum of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 29) ; "It does sort of look like a speaker, doesn't it. But it isn't."
			)
			(5 ; Talk
				(Print 60 29) ; "It does sort of look like a speaker, doesn't it. But it isn't."
			)
...
(instance hole of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 60 30) ; "Your engineering expertise informs you that this is a carelessly discarded Williamson coil."
				(Print 60 31) ; "Of course it could also be a humongous escargot."
			)
			(3 ; Do
				(Print 60 32) ; "Not knowing whether it's alive or not, you're not eager to touch it."
			)
			(12 ; Smell
				(Print 60 33) ; "Garlic - chives - butter..."
			)
			(11 ; Taste
				(Print 60 3) ; "Oh, come on!"
			)
			(5 ; Talk
				(Print 60 34) ; "No time for chit-chat. You're here on a mission, remember?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance robot of PicView
	...
	(method (doVerb theVerb)
		(if (== global166 1)
			(switch theVerb
				(2 ; Look
					(Print 60 35) ; "Broom at the ready, the cleaning droid waits for something useful to do."
				)
				(3 ; Do
					(Print 60 36) ; "You give the cleaning droid a shove. It appears to be in standby mode and doesn't budge."
				)
				(12 ; Smell
					(Print 60 37) ; "The heady scents of petroleum distillates and Sarien refuse titillates your nostrils."
				)
				(11 ; Taste
					(Print 60 38) ; "You can't be serious!"
				)
				(5 ; Talk
					(Print 60 39) ; "The droid does not speak to you. It's possible you've been snubbed."
				)
				(else
					(super doVerb: theVerb &rest)
				)
