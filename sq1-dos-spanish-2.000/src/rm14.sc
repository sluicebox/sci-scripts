		...
		(switch theVerb
			(2 ; Look
				(Printf ; "Dials galore populate the instrument console of the pod cabin.%s"
					...
					(if local1
						{ L\a0stima que ninguno de ellos funcione ya.}
					else
						{ }
					)
				)
		...
		(switch theVerb
			(2 ; Look
				(Printf ; "This is the lucky escapee's seat.%s"
					...
					(if local1
						{ Es posible que se haya mojado un poco debido al aterrizaje.}
					else
						{ }
					)
				)
		...
		(switch theVerb
			(2 ; Look
				(Printf ; "This is the power button for the escape pod.%s"
					...
					(if local1
						{ Su vida operativa ha terminado debido al abrupto aterrizaje.}
					else
						{ }
					)
				)
