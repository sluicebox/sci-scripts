...
(procedure (localproc_0)
	(switch (Random 0 1)
		(0
			(Print 3 38) ; "You seem to be on the wrong level to do that."
		)
		(1
			(Print 3 39) ; "You need to go to the top level."
		)
	)
)
...
(instance rm3 of SQRoom
	...
	(method (newRoom newRoomNumber)
		...
		(if (!= (gLongSong number:) 355)
			...
			(if (and (== newRoomNumber 103) (not (proc700_1 554 16)))
			...
			(3
				(Format ; "Just before his systems cease all functions short of decay, he looks over toward the shelves full of cartridges and utters "%s". With one last gasp, his lifeless form slumps to the floor."
					...
					(switch ((ScriptID 700 0) cartNumber:) ; arcadaRegion
						...
						(8 {Orbitas})
						(9 {Galaxias})
						(10 {Erupciones Solares})
						(11 {Lunas})
						(12 {Formaciones Planetarias})
						(13 {Constelaciones})
						(14 {Cuadrantes})
						(15 {Asteroides})
						(16 {Pulsares})
...
(instance theDummy of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== global166 1)
			(switch theVerb
				(2 ; Look
					(Print 3 6) ; "The old scientist doesn't appear healthy. In fact, his small intestine is about to unwind onto the floor."
				)
				(4 ; Inventory
					(switch invItem
						(10 ; buckazoid
							(Print 3 7) ; "No amount of money will put this man's lungs back in his chest."
						)
						(else
...
(instance leftDoor of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 8) ; "This is a door. Perhaps it leads elsewhere."
			)
		)
		(if (== global166 1)
			(super doVerb: theVerb &rest)
...
(instance rightDoor of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 9) ; "It's an automatic door."
			)
		)
		(if (== global166 1)
			(super doVerb: theVerb &rest)
...
(instance rightPanelLights of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 10) ; "These panel lights are here just for decoration and don't actually have anything to do with the Data Archive Unit."
			)
		)
		(if (== global166 1)
			(super doVerb: theVerb &rest)
...
(instance leftPanelLights of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 11) ; "These lights have something to do with the Data Archive Unit, though you don't really know what. They do seem to indicate that the unit is still operational."
			)
		)
		(if (== global166 1)
			(super doVerb: theVerb &rest)
		...
		(if (== global166 1)
			(switch theVerb
				(2 ; Look
					(cond
						((IsFlag 8)
							(Print 3 12) ; "It's a grisly sight. You can't tell where the toasted synthetic fabrics end and the fused organic material begins. There is nothing you can do for him now."
						)
						(local1
							(Print 3 13) ; "You see a man dressed in a lab coat. He appears to be ailing in a major way."
						)
						(else
							(Print 3 14) ; "A large laser hole has been burned in his uniform, giving you a nice view of previously unexposed tissue. Struggling painfully, he props himself up on one elbow."
							(= local1 1)
						)
					)
				)
				(3 ; Do
					(if (IsFlag 8)
						(Print 3 15) ; "You can do nothing more for the fused flesh-heap of a man."
					else
				...
				(4 ; Inventory
					(switch invItem
						...
						(0 ; Cartridge
							(Print 3 16) ; "In his present condition (dead) he can't make any use of the data cartridge."
						)
						(1 ; keyCard
							(Print 3 17) ; "If you give the keycard to the dead scientist, you won't be able to use the special elevators."
						)
						(15 ; Widget
							(Print 3 18) ; "The widget apparently has no power to raise the dead."
						)
						(2 ; Gadget
							(Print 3 19) ; "The handy dandy translation gadget will not enable you to communicate with the dead."
						)
						(else
							(super doVerb: theVerb invItem)
						)
				...
				(5 ; Talk
					(if (IsFlag 8)
						(Print 3 20) ; "His audible forms of communication will be limited to what he can convey through the venting of bodily gases."
					else
						(gCurRoom setScript: talkScientist)
					)
				)
				(12 ; Smell
					(if (IsFlag 8)
						(Print 3 21) ; "He didn't smell all that great when he was alive."
					else
						(Print 3 22) ; "The smell of freshly sheared copper fills the air around the damaged scientist."
					)
				)
				(11 ; Taste
					(Print 3 23) ; "You wouldn't want to do that. Trust me."
				)
				(else
			...
			(switch theVerb
				(2 ; Look
					(Printf ; "This is a Model DX Cartridge Retrieval Unit. Its function is to retrieve and return cartridges from and to the storage unit. It is currently %s."
						...
						(if local4 {sosteniendo un cartucho} else {vac\a1a})
					)
				)
				(3 ; Do
					(Print 3 25) ; "You can't reach it. Even if you did, the pinchers look quite nasty."
				)
				(5 ; Talk
					(Print 3 26) ; "It doesn't respond to human voice commands. It only works through the data archive console."
				)
				(12 ; Smell
					(Print 3 27) ; "I wouldn't be violating his airspace if I were you."
				)
				(11 ; Taste
					(Print 3 27) ; "I wouldn't be violating his airspace if I were you."
				)
				(4 ; Inventory
					(switch invItem
						(10 ; buckazoid
							(Print 3 28) ; "You figure the robot probably works for batteries."
						)
						(0 ; Cartridge
							(Print 3 29) ; "You decide that the cartridge is too important to give back to the robot."
						)
						(1 ; keyCard
							(Print 3 30) ; "You see no slot on the robot that would fit the keycard."
						)
						(15 ; Widget
							(Print 3 31) ; "The widget, being magnetic, sticks to the robot's outer casing. You remove it again with some difficulty."
						)
						(2 ; Gadget
							(Print 3 32) ; "Since the robot does not speak, the translator has nothing to translate."
						)
						(else
							(super doVerb: theVerb &rest)
						)
...
(instance Computer of Feature
	...
	(method (doVerb theVerb)
		(if (== global166 1)
			(switch theVerb
				(2 ; Look
					(Print 3 33) ; "This is the operation console of the Data Archive Unit. There is a CRT and chair."
				)
				(3 ; Do
...
(instance cartridgeSlots of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 34) ; "The data cartridges are stored in these secure storage modules."
			)
		)
		(if (== global166 1)
...
(instance mainComputer of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 35) ; "The Data Archive System secures its sophisticated electronic information in cartridges housed in these storage modules."
			)
		)
		(if (== global166 1)
			(self x: ((User curEvent:) x:) y: ((User curEvent:) y:))
...
(instance lights of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 3 36) ; "These things hanging from the ceiling, which look like blowfish-on-a-rope, are actually lighting fixtures for the operation console."
			)
		)
		(if (== global166 1)
			(self x: ((User curEvent:) x:) y: ((User curEvent:) y:))
		...
		(switch theVerb
			(2 ; Look
				(if (== global166 1)
					...
					(Print 3 37) ; "You can see a well-lit room above."
				)
			)
			(else
				(super doVerb: theVerb &rest)
