...
(instance rm13 of Rm
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 2) ; "The inside of the Arcada's escape pod is not exactly packed with luxurious appointments. However, when it comes to saving one's posterior, the pod's as good as a Rolls."
			)
		)
		(localproc_1)
		...
		(switch (= state newState)
			(0
				(if (not (gCast contains: padInset))
					...
					(Print 13 3) ; "Please press the desired button on the inset."
				)
				(= cycles 1)
			)
			(1
		...
		(switch (= state newState)
			...
			(4
				(switch register
					(1
						(if (not local0)
							(Print 13 4) ; "This option works better with the power on."
						else
							(Print 13 5) ; "You will need to leave the Launch Bay first."
						)
					)
					(2
						(if (not local0)
					...
					(else
						(if (not local0)
							...
							(Print 13 6) ; "This option works only when clear of the Arcada."
						)
					)
				)
				(= register 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(cond
					((not local0)
						(Print 13 7) ; "Nothing much will happen without power."
						(= cycles 3)
					)
					((not (IsFlag 10))
						((ScriptID 700 0) inGame: 0) ; arcadaRegion
					...
					((not local2)
						(EgoDead 948 0 0 13 8) ; "Buckle up for safety, buckle up."
					)
					(else
						(= state 1)
						(UnLoad 132 328)
			...
			(2
				(Print 13 9) ; "As you slide the throttle forward, you can feel the Arcada start to shake."
				(SetScore 15 138)
...
(instance crashThruDoors of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(EgoDead 945 0 0 13 10) ; "Guess those doors are as strong as they look."
				(self dispose:)
			)
		)
	)
...
(instance joystick of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 13 11) ; "This lever causes forward propulsion of the pod when powered-up."
			)
			(3 ; Do
...
(instance egoSuit of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 12) ; "There doesn't seem to be enough room."
			)
		)
		(localproc_1)
		(super doVerb: theVerb &rest)
...
(instance seatBeltFeat of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 13 13) ; "The seatbelt is dummy-tested, which ought to suit you just fine."
			)
			(3 ; Do
				(cond
					(local2
...
(instance Dials of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 14) ; "Dials galore populate the instrument console of the pod cabin."
			)
		)
		(localproc_1)
		(super doVerb: theVerb &rest)
...
(instance ControlPanel of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 15) ; "This is the pod's instrument console. Dials, gauges, and a couple of controls fill out the board."
			)
		)
		(localproc_1)
		(super doVerb: theVerb &rest)
...
(instance Chair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 16) ; "This is the lucky escapee's seat."
			)
		)
		(localproc_1)
		(super doVerb: theVerb &rest)
...
(instance PodWindow of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(if (IsFlag 10)
					(Print 13 17) ; "Through the open bay doors you can see a plethora of stars."
				else
					(Print 13 18) ; "Your view is limited by the closed bay doors."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance survivalKit of View
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 13 20) ; "The survival kit contains the basics for deep space survival."
			)
			(3 ; Do
				(Print 13 21) ; "You don't need that right now."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance screen of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 24) ; "This is radar for the Autonav system."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance head of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 26) ; "This is your head. This is your head in a fishbowl."
			)
		)
		(localproc_1)
		(super doVerb: theVerb &rest)
...
(instance star1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 27) ; "Stars glow from afar."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance arcada of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 28) ; "It's your former ship, the Arcada."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance escapePod of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 29) ; "This is the escape pod. It is currently occupied by you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance AutoNavButton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 30) ; "This is the Autonav button. When operative it allows the pod to navigate to the closest habitable planet."
			)
			(3 ; Do
...
(instance DontPressButton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 31) ; "THIS BUTTON IS NOT TO BE PUSHED AT ANYTIME."
			)
			(3 ; Do
...
(instance PowerButton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 13 32) ; "This is the power button for the escape pod."
			)
			(3 ; Do
