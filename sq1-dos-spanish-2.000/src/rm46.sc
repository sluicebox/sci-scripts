...
(instance rm46 of SQRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 46 0) ; "This is the pickup area for Droids-B-Us."
			)
			(else
...
(instance blowUpBox of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Print 46 1) ; "Well, okay. If you really feel competent enough to assemble this thing, just do it. Start with the smallest box."
...
(instance radar of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 46 3) ; "The settlement of Ulence Flats is surrounded by these force field generators. They repel such undesirables such as the grell which thrive below the sands. It will allow only airborne vehicles in or out."
			)
			(3 ; Do
				(Print 46 4) ; "I wouldn't mess with them if I were you."
			)
			(5 ; Talk
				(Print 46 5) ; "No answer is forthcoming."
			)
			(12 ; Smell
				(Print 46 4) ; "I wouldn't mess with them if I were you."
			)
...
(instance robbie of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 46 6) ; "This robot is a very old model. Somehow, he looks tired and depressed."
			)
			(5 ; Talk
				(gLongSong2 number: 622 loop: -1 flags: 1 play:)
				(switch (++ local2)
			...
			(12 ; Smell
				(Print 46 7) ; "It smells like a fine grade of light machine oil. You were expecting Hai Karate, perhaps?"
			)
			(11 ; Taste
				(Print 46 8) ; "Why on earth - or Kerona for that matter - would you want to do that?"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance suxRobot of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 46 9) ; "What a fine robot you've purchased!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance box1 of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Print 46 10) ; "Great. Three boxes of robot parts, sporting large signs which read: "Caution: This product contains explosive parts. Not intended for children under three.""
		)
		(if (OneOf theVerb 3 4 11) ; Do, Inventory, Taste
			(switch local1
				(0
					(Print 46 11) ; "Unfortunately, you AREN'T smarter than you look, so you'd best leave the incredibly complex robot parts alone before you hurt yourself. Too bad they don't give rebates to retrobates."
				)
				(1
					(Print 46 12) ; "You can tell this is definitely out of your league. You can't even open the boxes, much less assemble what's inside."
				)
				(2
...
(instance pickupArea of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 46 13) ; "This portion of the building is the warehouse where customers come to pick up their purchases."
			)
			(else
				(store doVerb: theVerb &rest)
			)
		)
	)
)
...
(instance store of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 46 14) ; "That's the Droids-B-Us storefront. Droids-B-Us is known throughout this end of the galaxy for the best deals on new and used psionic and cybernetic systems for home, school, office, and spaceship."
			)
			(3 ; Do
				(Print 46 15) ; "Like many buildings on these backwater planets, this one is a quickie Construct-O-Foam job that won't last more than a couple of thousand years at the outside. They just don't make `em like they used to."
			)
			(5 ; Talk
				(Print 46 16) ; "Talking to buildings is likely to get you arrested on this planet."
			)
			(12 ; Smell
				(Print 46 17) ; "Why would you possibly care what this building smells like?"
			)
			(11 ; Taste
				(Print 46 18) ; "Oh, blecchhh!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance pickupSign of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 46 19) ; "The badly weathered sign says "Droids-B-Us Pickup"."
			)
			(3 ; Do
				(Print 46 20) ; "You can't reach it."
			)
			(5 ; Talk
				(Print 46 21) ; "No matter how you try to convince it otherwise, the sign will only say "Droids-B-Us Pickup.""
			)
			(12 ; Smell
				(Print 46 22) ; "From here, you can discern no particular smell on the sign."
			)
			(11 ; Taste
				(Print 46 23) ; "If your neck was as long as a giraffazoid's, then maybe you could do that. But why would you want to?"
			)
			(else
				(super doVerb: &rest)
			)
...
(instance giraffe of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 46 24) ; "This cheerful fellow is Gerry Giraffazoid, logo, spokesbeing, and C.E.O. of Droids-B-Us, Inc."
			)
			(3 ; Do
				(Print 46 25) ; "You can't reach it."
			)
			(5 ; Talk
				(Print 46 26) ; "This isn't the real Gerry Giraffazoid, Roger - it's just a picture."
			)
			(12 ; Smell
				(Print 46 27) ; "Gerry Giraffazoid's picture doesn't smell like much of anything."
			)
			(11 ; Taste
				(Print 46 28) ; "Trust me - that logo lacks any taste."
			)
			(else
				(super doVerb: theVerb &rest)
			)
