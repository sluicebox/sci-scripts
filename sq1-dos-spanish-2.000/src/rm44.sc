...
(procedure (localproc_1)
	(Display 44 22 dsRESTOREPIXELS local1)
	(= local1 0)
)
...
(instance rm44 of Rm
	...
	(method (init &tmp [temp0 30])
		...
		(HandsOn)
		(ClearFlag 12)
		(tmp1 view: (proc0_19 144 1144 144 144 144) init: stopUpd:)
		(tmp2 view: (proc0_19 144 1144 144 144 144) init: stopUpd:)
		(tmp3 view: (proc0_19 144 1144 144 144 144) init: stopUpd:)
		(butn init:)
		(exitBtn view: (proc0_19 244 1244 244 244 244) init: stopUpd:)
		(coinSlot init:)
		(info view: (proc0_19 144 1144 144 144 144) init: cel: 1 stopUpd:)
		(pic1
			view: (proc0_19 144 1144 144 144 144)
			cel: (& global174 $000f)
			init:
		)
		(pic2
			view: (proc0_19 144 1144 144 144 144)
			cel: (& (>> global174 $0004) $000f)
			init:
		)
		(pic3
			view: (proc0_19 144 1144 144 144 144)
			cel: (& (>> global174 $0008) $000f)
			init:
		)
		(gLongSong setVol: 70)
		(if global175
				...
				(= local0
					(cond
						...
						(else
							(switch (Random 0 6)
								(0
									(localproc_0 44 3) ; "Eat Sand"
								)
								(1
									(localproc_0 44 4) ; "No Way"
								)
								(2
									(localproc_0 44 5) ; "Not A Chance"
								)
								(3
									(localproc_0 44 6) ; "It's Hopeless"
								)
								(4
									(localproc_0 44 7) ; "Tough Luck"
								)
								(5
									(localproc_0 44 8) ; "Suck Methane"
								)
								(6
									(localproc_0 44 9) ; "So Sorry"
								)
							)
							0
						)
			...
			(3
				(if (!= local0 1000)
					...
					(if local0
						...
						(= temp30 (* local0 global175))
						(localproc_0
							(Format ; "You Win $%u You Have $%u"
								@temp0
								44
								10
								temp30
					...
					(SetFlag 39)
					(localproc_0 44 12) ; "YOU LOSE, HOMEBOY!"
					(gCurRoom newRoom: 43)
				)
			)
			(4
...
(instance pic1 of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 44 13) ; "The pictures are protected by unbreakable windows."
			)
			(11 ; Taste
				(Print 44 14) ; "Very good! You got all that gunk off the glass. Licking the glass windows makes the pictures easier to see."
			)
			(12 ; Smell
				(Print 44 15) ; "A slight smell of ozone and charred meat permeates the machine."
			)
			(4 ; Inventory
				(switch invItem
					(15 ; Widget
					...
					(10 ; buckazoid
						(Print 44 16) ; "You're supposed to put your money in the money slot."
					)
					(else
				...
				(switch invItem
					(10 ; buckazoid
						(cond
							...
							(else
								(Print 44 20) ; "The "Slots-O-Death" machine will let you insert only three buckazoids per spin."
							)
						)
					)
					(15 ; Widget
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(localproc_0 44 21) ; "Insert a buckazoid  into the slot."
				(= seconds 3)
			)
			(1
				(localproc_1)
