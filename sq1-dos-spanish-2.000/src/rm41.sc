...
(instance mugger of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 41 16) ; "This being is about the size of a humanoid but has some odd features. He seems to have a friendly-looking mug."
			)
			(3 ; Do
				(Print 41 17) ; "You sure know how to push things."
			)
			(5 ; Talk
				(Print 41 18) ; "He wants cash, not conversation."
			)
			(12 ; Smell
				(Print 41 19) ; "He won't take kindly to that."
			)
			(11 ; Taste
				(Print 41 19) ; "He won't take kindly to that."
			)
...
(instance skimmer of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if (IsFlag 15)
					(Print 41 20) ; "You already have the keys."
				else
			...
			(2 ; Look
				(if (IsFlag 15)
					(Print 41 21) ; "The sand skimmer is a one-passenger mode of transportation and it looks like it has skimmed quite a bit of sand in its time. On its control panel is a small readout, currently dark, and a keyhole without a key in it."
				else
					(Print 41 22) ; "The sand skimmer is a one-passenger mode of transportation and it looks like it has skimmed quite a bit of sand in its time. On its control panel is a small readout, currently dark, and a keyhole with a key in it."
				)
			)
			(4 ; Inventory
				(switch invItem
			...
			(5 ; Talk
				(Print 41 23) ; "Alas, it does not respond."
			)
			(12 ; Smell
				(Print 41 24) ; "It smells warm after the long ride."
			)
			(11 ; Taste
				(Print 41 25) ; "You take a quick look, notice it hasn't been cleaned in awhile, and change your mind."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance radar of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 41 26) ; "The settlement of Ulence Flats is surrounded by these force field generators. They repel undesirables, such as the Grell, which thrive below the sands. It will allow only airborne vehicles in or out."
			)
			(3 ; Do
				(Print 41 27) ; "I wouldn't mess with them if I were you."
			)
			(5 ; Talk
				(Print 41 28) ; "No answer is forthcoming."
			)
			(12 ; Smell
				(Print 41 27) ; "I wouldn't mess with them if I were you."
			)
...
(instance shuttle of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 41 29) ; "This appears to be a long-range shuttle, perhaps from a large starship. It looks vaguely familiar somehow."
			)
			(3 ; Do
				(Print 41 30) ; "You decide it would be better not to tamper with it."
			)
			(5 ; Talk
				(Print 41 31) ; "Like most spaceships in your experience, this one won't talk to you."
			)
			(12 ; Smell
				(Print 41 32) ; "Smelling this ship seems like a giant waste of time."
			)
			(11 ; Taste
				(Print 41 33) ; "There's not much point in licking the ship - it's already quite clean."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance pickleShip of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 41 34) ; "This ship is a real classic - a WalWood WarpBlaster ZX with the original paint job. You've only seen these on old postcards."
			)
			(3 ; Do
				(Print 41 35) ; "You really don't want to get handprints all over a valuable antique like this one."
			)
			(5 ; Talk
				(Print 41 36) ; "Your shout produces no response. The ship is empty."
			)
			(12 ; Smell
				(Print 41 37) ; "You're a nosy little beggar, aren't you?"
			)
			(11 ; Taste
				(Print 41 38) ; "Didn't your mother tell you not to go licking other people's spaceships?"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance tallRocket of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 41 39) ; "This one's a genuine antique from the twenty-fourth-and-a-half century."
			)
			(3 ; Do
				(Print 41 40) ; "Better not - it'd probably fall over."
			)
			(5 ; Talk
				(Print 41 41) ; "You shout in the direction of the ship, but no one answers."
			)
			(12 ; Smell
				(Print 41 42) ; "It mainly smells like the dust that's been accumulating on it for the past fifty years."
			)
			(11 ; Taste
				(Print 41 43) ; "What a revolting idea!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance barFront of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 41 44) ; "There is a rounded structure here with a door on one side. It is typical of some of the prefab structures constructed in frontier areas years ago."
			)
		)
		(= x ((User curEvent:) x:))
		(= y ((User curEvent:) y:))
...
(instance barSigns of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Print 41 45) ; "The sign is very unusual. It looks as though the whole building has been constructed around the wreckage of a crashed spaceship."
				(Print 41 46) ; "Or maybe the traffic around here is just really awful."
			)
			(3 ; Do
				(Print 41 47) ; "The naked wiring you can see from here makes touching the sign seem unwise."
			)
			(5 ; Talk
				(Print 41 48) ; "The Rocket Bar sign has nothing to say to you."
			)
			(12 ; Smell
				(Print 41 49) ; "You detect the scent of neon and zapped bugs."
			)
			(11 ; Taste
				(Print 41 50) ; "That doesn't seem either intelligent or appetizing."
			)
			(else
				(super doVerb: theVerb &rest)
			)
