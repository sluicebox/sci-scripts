...
(instance rm64 of SQRoom
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 0) ; "This is where the Sariens placed the Star Generator after lifting it from the Arcada."
		)
			...
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
					(if
						...
						(Print 64 1) ; "You can't shoot that from here."
					else
...
(instance leftShieldEmitter of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 2) ; "The left hand force field emitter, much like the right, radiates an unknown form of energy that keeps you from approaching the Star Generator."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
					(if
						...
						(Print 64 3) ; "You can't shoot that from here."
					else
						(super doVerb: theVerb invItem &rest)
					)
				)
...
(instance rightShieldEmitter of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 4) ; "The right hand force field emitter, in a fashion altogether similar to the left hand one, beams an inexplicable power into the vicinity of the Star Generator that makes it impossible to get near."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
...
(instance shield of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 5) ; "Cowabunga, Wilco - this is IT! It's the unimaginably powerful Star Generator, which must not, at any cost, be allowed to remain in the hands of the evil Sariens! (but you knew that, right?)"
		)
		(if (and (== theVerb 4) (== global166 2)) ; Inventory
			(if (== invItem 14) ; Remote
				(if (not (IsFlag 50))
					...
					(Print 64 6) ; "There's no need to use the remote now, the force field is already off."
				)
			)
		else
			(super doVerb: theVerb invItem &rest)
...
(instance generatorOn of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 64 7) ; "The pulsing energy surging from the Star Generator tells you that it has been activated and you had better quit hanging around staring at the pretty colors."
			)
			(else
...
(instance remote of Prop
	...
	(method (doVerb theVerb invItem)
		(if (== global166 2)
			(switch theVerb
				(2 ; Look
					(Print 64 8) ; "A receptor for somekind of remote control signal."
				)
				(3 ; Do
					(cond
						((not (IsFlag 50))
							(Print 64 9) ; "There is a force field around the Star Generator. You will need to turn if off first."
						)
						((IsFlag 53)
							(Print 64 10) ; "The Star Generator is already set to blow, Roger! Maybe you oughtta do the same!"
						)
						(else
...
(instance starGenerator of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(cond
			((== global166 2)
				(switch theVerb
					...
					(2 ; Look
						...
						(Print 64 11) ; "Cowabunga, Wilco - this is IT! It's the unimaginably powerful Star Generator, which must not, at any cost, be allowed to remain in the hands of the evil Sariens! (but you knew that, right?)"
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
			...
			(else
				(Print 64 12) ; "You can't do that from up here."
			)
		)
	)
)
...
(instance grenade of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 64 13) ; "It's the used gas grenade."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(if (== global166 2)
			(switch theVerb
				(4 ; Inventory
					(switch invItem
						(13 ; Grenade
							(Print 64 14) ; "Don't you think you've gassed this guy enough?"
						)
						(0 ; Cartridge
							(Print 64 15) ; "You could drop the cartridge on his big ugly head, but you might damage the data. Better find something more suited to the task."
						)
						(19 ; Sarien_ID_Card
							(if (IsFlag 47)
								(Print 64 16) ; "You think he might be more interested in it if he were alive."
							else
					...
					(cond
						((not (IsFlag 47))
							...
							(if
								...
								(Print 64 18) ; "He's also wondering what your doing back there."
							)
						)
						((and (IsFlag 47) (not (gEgo has: 14))) ; Remote
							(Print 64 19) ; "You notice a remote control device on the guards belt."
						)
						(else
							(Print 64 20) ; "The guard lies in a red heap on the floor, overcome by the toxic gas from the grenade."
						)
					)
				)
				(5 ; Talk
					(if (not (IsFlag 47))
						(if (gEgo has: 0) ; Cartridge
							(Print 64 21) ; "This is a guy of few words, none of them for you."
						else
							(Print 64 22) ; "I can not answer any questions without proper ID."
						)
					else
						(Print 64 23) ; "This guy is more talkative dead than alive. He's dead silent."
					)
				)
				(3 ; Do
					(cond
						((not (IsFlag 47))
							(Print 64 24) ; "If you touch him, he might touch you back."
						)
						((and (IsFlag 47) (not (gEgo has: 14))) ; Remote
							(gCurRoom setScript: getRemote)
						)
						(else
							(Print 64 25) ; "The guard has nothing else of interest."
						)
					)
				)
				(else
			...
			(switch theVerb
				(2 ; Look
					(cond
						...
						((and (IsFlag 47) (not (gEgo has: 14))) ; Remote
							(Print 64 26) ; "You notice a remote control device on the guards belt"
						)
						(else
							(Print 64 27) ; "The guard lies in a red heap on the floor, overcome by the toxic gas from the grenade. Some kind of electronic gadget is hanging from his belt."
						)
					)
				)
				(4 ; Inventory
					(if (== invItem 13) ; Grenade
						(if (not (IsFlag 47))
							...
							(Print 64 28) ; "Don't you think you've gassed this guy enough?"
						)
					else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance killEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 64 29) ; "Oh no Roger! The guard has seen you and you're not wearing the helmet! You're in for it now Wilco!"
			...
			(12
				(EgoDead 940 2 0 64 30) ; "Boy, you're no Marshall Dillon, that's for sure. And you were so close. You'll need to react a little faster and a little sharper than that. Well, throw a little bondo into that hole and get back in there."
				(self dispose:)
			)
		)
	)
		...
		(switch (= state newState)
			...
			(5
				(if (not (< (guard nsLeft:) local0 (guard nsRight:)))
					(grenade dispose:)
					(if (not (IsFlag 47))
						(Print 64 31) ; "The Sarien warrior is guarding the stolen Star Generator so intensly, he didn't notice the gas grenade exploding."
						(Print 64 32) ; "Actually, his walkman is cranked up too high."
					else
						(Print 64 33) ; "The dead Sarien guard might have noticed that, if he were alive."
					)
				)
				(gEgo
					setLoop: (+ (gEgo loop:) 2)
...
(instance getRemote of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 64 34) ; "You remove the device from the guard's belt. It looks like you have found the remote control that turns off the force field around the Star Generator."
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(leftShieldEmitter dispose:)
				(Print 64 35) ; "You press the stud on the remote and the force field around the Star Generator begins to deplete."
...
(instance toastEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 945 0 0 64 36) ; "Ouch! That stings just a tad. You'll obviously need to disarm that baby before trying that again."
				(self dispose:)
			)
		)
	)
...
(instance showCard of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Printf 64 37 global184 global184) ; "Sir, the destruct code is %d."
				(gEgo setCycle: Beg self)
			)
			(4
				(proc703_1)
...
(instance bumpOffShield of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Print 64 38) ; "It appears the shield is preventing you from getting to the Star Generator's control panel and boy is it hot. You'd better not try to get close to the Star Generator again."
...
(instance setToBlow of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Print 64 39) ; "The Star Generator is already set to blow Roger! You had better find someway to get a million miles from the Deltaur and quick!"
				(gEgo setMotion: PolyPath 163 172 self)
			)
			(1
				(HandsOn)
...
(instance shootTheEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 64 40) ; "Oh No! Your rotten aim has attracted the attention of one of the patrol guards. You're in for it now."
				(if (> (gEgo x:) 120)
					(= cycles 10)
				else
					(gEgo
			...
			(5
				(EgoDead 940 2 0 64 41) ; "Boy, you're no Marshall Dillon, that's for sure. And you were so close. You'll need to react a little faster be little sharper than that. Well, throw a little bondo into that whole and get back in there."
				(self dispose:)
			)
		)
	)
...
(instance upperLanding of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 42) ; "It's the walkway above the Star Generator."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
...
(instance emitter of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 43) ; "This is one of the force field's emitters."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
...
(instance tubes of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 44) ; "The Wally Tubes look strangely empty without Thunder Agents in them."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
...
(instance space of RegionFeature
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(Print 64 45) ; "Looking into the vast reaches of space gives you the curious illusion of being tiny, unimportant, insignificant, negligible, and totally irrelevant in the scheme of things. Then you realize it's not an illusion."
		)
		(if (== theVerb 4) ; Inventory
			(switch invItem
				(12 ; Pulseray_Laser_Pistol
