...
(local
	...
	[local63 36] = [84 147 50 159 73 175 108 160 131 176 174 178 221 162 268 189 0 189 0 0 319 0 319 189 129 95 110 100 132 110 91 121 74 113 38 121]
)
		...
		(switch theVerb
			...
			(3 ; Do
				(cond
					...
					((or (IsFlag 29) (IsFlag 62))
						(Print 54 7) ; "You can't reach the vent and besides, you don't need to climb through the vent now that you have a Sarien uniform on."
					)
					((IsFlag 34)
						(Print 54 6) ; "Standing in the open trunk won't give you any height advantage."
					)
...
(instance trunk of Prop
	...
	(method (doVerb theVerb invItem)
		(if (not (IsFlag 29))
			(switch theVerb
				...
				(3 ; Do
					(cond
						...
						(else
							(Print 54 8) ; "It's already moved."
						)
					)
				)
				(2 ; Look
					(if cel
						(Print 54 9) ; "The trunk on the floor appears to be open. It looks big enough to hold a body."
					else
						(Print 54 10) ; "The trunk is unremarkable in every way."
					)
				)
				(else
					(super doVerb: theVerb invItem &rest)
			...
			(Print 54 11) ; "You don't need to mess with the trunk now."
			(super doVerb: theVerb invItem &rest)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(Print 54 12) ; "Fearing discovery by one of the ship's residents, you cleverly decide to hide in the trunk. As you close the lid, you hear the sounds of someone, or thing, entering the room."
				(vent hide:)
				(gCurRoom drawPic: 99 7)
				(Print 54 13) ; "Suddenly the trunk begins to move!"
...
(instance egoTestVent of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 54 14) ; "Your hands by themselves are incapable of opening it."
				(gEgo
...
(instance cantReachIt of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 54 15) ; "You feel a fresh breeze coming through the grill of the vent. If you were a lot taller, you could touch the grill itself."
				(gEgo setCycle: Beg self)
			)
			(4
				(proc703_1)
...
(instance killEgoTimeOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 54 16) ; "It sounds like someone (or something) is approaching."
				(= seconds 20)
			)
			(2
				(Print 54 17) ; "You hear footsteps approaching and they're getting close. You might only have seconds to do something!"
				(= seconds 10)
			)
			(3
				(= local0 1)
...
(instance killEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 945 0 0 54 18) ; "Bingo! He got you. You might consider not being around when these guys are shooting. Those things hurt."
				(self dispose:)
			)
		)
	)
...
(instance ventShaft of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 19) ; "A large vent shaft protrudes from the left wall of the storage room."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance shadows of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 20) ; "Various pipes, ducts and other innocuous pieces of equipment form pronounced shadows throughout the room."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance airLock of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 21) ; "That is the way back to the airlock, which is completely sealed and inaccessable to you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance vaults of RegionFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 54 22) ; "Two enormous storage vaults take up one whole wall of the room."
			)
			(else
				(super doVerb: theVerb &rest)
			)
