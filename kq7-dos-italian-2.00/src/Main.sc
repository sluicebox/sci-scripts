...
(class KQ7CD of Game
	...
	(method (showOptions &tmp temp0 temp1)
		(switch (gGame printLang:)
			(34
				((= temp1 (Print new:))
					...
					fore: 255
					back: 0
					addText: {BOOKMARK OPTIONS} 30 13
					addButton:
						1
						{<Vuelve a la ultima marca y sigue jugando>}
						0
						26
					addButton:
						2
						{<Haz una nueva marca y abandona el juego>}
						0
						40
					addButton:
						3
						{<Manten ultima marca y abandona el juego>}
						0
						54
					addButton: 4 {<Haz una nueva marca y sigue jugando>} 0 68
				)
			)
			(39
				((= temp1 (Print new:))
					...
					fore: 255
					back: 0
					addText: {OPZIONI SEGNALIBRO} 35 13
					addButton:
						1
						{<Torna al vecchio segnalibro e continua a giccare>}
						0
						26
					addButton:
						2
						{<Crea un nuovo segnalibro ed esci dal gioco>}
						0
						40
					addButton:
						3
						{<Mantieni il vecchio segnalibro ed esci dal gioco>}
						0
						54
					addButton:
						4
						{<Crea un nuovo segnalibro e continua a giocare>}
						0
						68
				)
			)
			(else
				((= temp1 (Print new:))
					...
					fore: 255
					back: 0
					addText: {BOOKMARK OPTIONS} 13 13
					addButton:
						1
						{<Return to old bookmark and keep playing>}
						0
						26
					addButton: 2 {<Make a new bookmark and quit the game>} 0 40
					addButton: 3 {<Keep old bookmark and quit the game>} 0 54
					addButton: 4 {<Make a new bookmark and keep playing>} 0 68
				)
			)
		)
		(switch (= temp0 (temp1 init:))
			(1
				(self smallRestore: global305)
			)
			(2
				(self smallSave:)
				(= gQuit 1)
		...
		(if (== (event type:) evKEYBOARD)
			(cond
				(
					(and
						(or
							(== (event message:) KEY_F5)
							(== (event message:) KEY_F7)
							(== (event message:) KEY_ESCAPE)
						)
						(not (OneOf gCurRoomNum 30 960 34 15)) ; endRoom
						(not (IsFlag 571))
						(not (IsFlag 572))
						(== isHandsOn 1)
						(User canInput:)
				...
				((== (event message:) KEY_ADD)
					(if (gUser controls:)
						(= global115 (Max 2 (-- global115)))
	...
	(method (smallSave param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
		...
		(= temp4 (Save 0 name global305 (param1 data:) (KString 9 gVersion))) ; SaveGame, StrGetData
		(temp6 dispose:)
		(Sound pause: 0)
		(temp3 dispose:)
		(return temp4)
	...
	(method (smallRestore param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10)
		...
		(if (!= param1 -1)
			...
			(if (Save 3 name param1 (KString 9 gVersion)) ; CheckSaveGame, StrGetData
