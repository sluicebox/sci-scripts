			...
			(7
				...
				(cond
					((FileIO fiEXISTS (temp1 data:))
						...
						(if (== (Platform) 2)
							(aboutBut
								x: (proc11_8 119 130 119 119 119)
								y: 103
								init:
...
(instance startBut of View
	...
	(method (doVerb &tmp temp0 temp1 temp2 [temp3 2])
		(if (localproc_0 self 1)
			(if (>= (= temp0 (proc21_0)) 10)
				(SetFlag 3)
		...
		(if (localproc_0 self 1)
			(if (== (Platform) 2)
				...
				(switch (gGame printLang:)
					...
					(39
						(Prints
							{Leggi il file KQ7GAME.TXT sul CD per saperne di pi\97 su King's Quest VII.}
						)
					)
					(else
						(Prints
