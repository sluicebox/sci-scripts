...
(instance Room36 of Rm
	...
	(method (handleEvent event)
		...
		(if (== (event type:) evSAID)
			...
			(if (and global208 local8 (IsFlag 52))
			...
			(cond
				...
				((Said 'look>')
					(cond
						((Said '[<around,at][/room]')
							(if (and (== gAct 3) (not (& global141 $0004)))
						...
						(
							(and
								(or (!= gAct 3) (& global141 $0004))
								(Said '/mud,spot,bootprint')
							)
							(Print 36 9) ; "You don't see any mud here"
						)
						((or (Said '/dirt') (Said '<down'))
							(if (== gAct 3)
								(cond
									...
									((not (& global141 $0004))
										(Print 36 10) ; "There are pieces of a broken record, a small pile of pink feathers, and some mud spots on the floor."
									)
									(else
										(event claimed: 0)
									)
						...
						((Said '/door')
							(Print 36 11) ; "The French doors lead outside."
						)
						((Said '/record')
							(cond
								((gEgo has: 9) ; broken_record
								...
								((< (gEgo distanceTo: phono) 20)
									(Print 36 12) ; "Oh! One of your favorites!"
								)
								(else
									(NotClose) ; "You're not close enough."
								)
				...
				((Said 'get/ball')
					(Print 36 13) ; "You don't need any billiard balls."
				)
			)
		)
	)
...
(instance enterPanel of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HandsOn)
				(Print 36 14) ; "The secret panel closes behind you and leaves no trace!"
				(= global204 0)
				(= local5 0)
				(client setScript: 0)
			)
...
(instance phono of RPicView
	...
	(method (handleEvent event)
		(cond
			((Said 'get/record')
				(Print 36 15) ; "The records don't belong to you."
			)
			(
				(or
					(Said
				...
				(if (not local0)
					(if (gEgo has: 9) ; broken_record
						(Print 36 16) ; "You don't have a record that will work."
					else
						(Print 36 17) ; "You don't have a record."
					)
				else
					(Print 36 18) ; "There is already a record playing. Oh! One of your favorites."
				)
			)
			((Said 'open,(look<in)/gramophone,armoire,(player<record)')
				(if (< (gEgo distanceTo: record) 30)
					(Print 36 19) ; "You look inside the Victrola cabinet and see several records. But since they don't belong to you, you decide to leave them alone."
				else
					(NotClose) ; "You're not close enough."
				)
			)
			(
				...
				(if (== gAct 3)
					(Print 36 20) ; "You see an old-fashioned Victrola and cabinet in the corner of the room. However, the record that is playing on the phonograph is current."
				else
					(Print 36 21) ; "You see an old-fashioned Victrola and cabinet in the corner of the room."
				)
				(event claimed: 1)
			)
		)
	...
	(method (handleEvent event)
		(cond
			((or (MousedOn self event 3) (Said 'look/guitar'))
				(event claimed: 1)
				(Print 36 22) ; "You see several musical instruments hanging on the wall."
			)
			((Said 'get/guitar')
				(Print 36 23) ; "You were never musically inclined."
			)
			((Said 'play/guitar')
				(Print 36 23) ; "You were never musically inclined."
			)
	...
	(method (handleEvent event)
		(cond
			((or (MousedOn self event 3) (Said 'look/rack,cue'))
				(event claimed: 1)
				(Print 36 24) ; "There are billiard cues in the rack on the wall."
			)
			((Said 'get/cue[<billiard]')
				(Print 36 25) ; "Since you don't play billiards, you don't need a billiard cue."
			)
		)
	)
)
...
(instance bookcase of RPicView
	...
	(method (handleEvent event)
		(if (or (MousedOn self event 3) (Said 'look/bookcase'))
			(event claimed: 1)
			(Print 36 26) ; "The bookcases are crammed full of books."
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			((or (MousedOn self event 3) (Said 'look/boat'))
				(event claimed: 1)
				(Print 36 27) ; "You notice a model of an old ship on the table."
			)
			((Said 'get/boat')
				(Print 36 28) ; "It's not yours. There's nothing special about it."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(if
			...
			(if (and (== gAct 1) (== gClarenceWilburState 4))
				(Print 36 29) ; "Currently, Gloria and Clarence are playing a game of billiards."
			else
				(Print 36 30) ; "This is an old billiard table. As it's quite dirty and dusty, it doesn't appear as if billiards has been played much. Upon it, you see three balls; two red and one white."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			((Said 'look<behind,below/painting')
				(Print 36 31) ; "You can't see anything behind the picture."
			)
			((Said 'get/painting')
				(Print 36 32) ; "The picture is firmly attached to the wall."
			)
			((Said 'open/painting')
				(Print 36 33) ; "It doesn't open."
			)
			(
				(or
				...
				(Print 36 34) ; "The eyes of the picture have a haunted, hollow look to them."
			)
			(
				(or
					(MousedOn self event 3)
				...
				(event claimed: 1)
				(Print 36 35) ; "You see a picture of a lovely dark-haired girl above the player piano. Strange...her eyes have a haunted, hollow look to them."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			(
				...
				(event claimed: 1)
				(Print 36 36) ; "You can turn the winding mechanism to "wind up" the player piano."
			)
			(
				(Said
				...
				(if (& (gEgo onControl: 1) $0010)
					(if (== local0 0)
						(if (== local1 0)
							...
							(Print 36 37) ; "It is already playing ."
						)
					else
						(Print 36 38) ; "Not now. There is already music playing from a record on the Victrola."
					)
				else
					(NotClose) ; "You're not close enough."
				)
	...
	(method (handleEvent event)
		(cond
			((Said 'open,(look<in)>')
				(cond
					((Said '/piano')
						(Print 36 39) ; "There is nothing that would interest you in the player piano."
					)
					((Said '/bench')
						(Print 36 40) ; "The piano bench doesn't open."
					)
				)
			)
			((or (MousedOn self event 3) (Said 'look/piano'))
				(event claimed: 1)
				(Print 36 41) ; "A lovely old player piano sits against the back wall."
			)
			((Said 'play/piano')
				(Print 36 42) ; "You never learned to play the piano. But, there is a winding mechanism on the side of the player piano."
			)
		)
	)
)
