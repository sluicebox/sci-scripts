	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3)
		...
		(if
			(or
				(event claimed:)
				(== temp1 evNULL)
				(and (!= evMOUSEBUTTON temp1) (not temp3) (!= $0040 temp1)) ; direction
			)
		...
		(if (= temp0 (self firstTrue: #handleEvent event))
			...
			(cond
				...
				(
					(or
						(and
							...
							(or (and temp3 (== KEY_RETURN temp2)) (== evMOUSEBUTTON temp1))
						)
				...
				(
					(or
						(and temp3 (== KEY_SHIFTTAB temp2))
						(and (== (event type:) $0040) (== (event message:) JOY_LEFT)) ; direction
						(and (== (event type:) $0040) (== (event message:) JOY_UP)) ; direction
					)
					(event claimed: 1)
					(self retreat:)
				)
