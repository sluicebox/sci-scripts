...
(class CardList of WL
	...
	(method (cue param1 &tmp [temp0 2])
		...
		(if (and (proc0_6 124) (proc304_0 self))
			(= underBits (Graph grSAVE_BOX (- y 2) x (+ y 43) (+ x 35) 1))
		)
		(self showUpdate:)
		(if foundationWithin
			(proc318_0 self)
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 [temp3 2] temp5)
		(if
			(and
				(not (event claimed:))
				...
				(if
					...
					(if (== dimRule 1)
						...
						(if
							(and
								(self numDimmed:)
								(!= (self firstDimmed: 1) temp5)
							)
							((self firstDimmed:) whiten:)
						)
					else
						(= temp1 (self lastCard:))
					)
