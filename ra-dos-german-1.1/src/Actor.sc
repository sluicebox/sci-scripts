...
(class Prop of View
	...
	(method (setScale theObjOrMatchOrY whoToMatch &tmp [temp0 40])
		(if scaler
			(scaler dispose:)
			(= scaler 0)
		)
...
(class Actor of Prop
		...
		(cond
			...
			(mover
				...
				(if (& scaleSignal $0001)
					(= temp5 (>> origStep $0008))
					(= temp6 (& origStep $00ff))
					(= temp3 (/ (+ (/ (* temp5 scaleX 10) 128) 5) 10))
					(= temp4 (/ (+ (/ (* temp6 scaleY 10) 128) 5) 10))
	...
	(method (setDirection dir &tmp vx vy xIncr yIncr ang maxCoord pathPts obs)
		...
		(cond
			((or xIncr yIncr)
				(self setMotion: PolyPath (+ x xIncr) (+ y yIncr))
			)
			(dir
