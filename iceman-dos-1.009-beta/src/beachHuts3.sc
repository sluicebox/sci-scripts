...
(instance beachHuts3 of Rm
	...
	(method (init)
		...
		(gAddToPics
			add: hutDoor beachLayer1 beachLayer2
			eachElementDo: #init
			doit:
		)
		(self
		...
		(cond
			...
			((Said '[/door]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 12 3) ; "This is the door to hut #1."
					)
					((GoToIfSaid self event self 20 0 12 4))
					((Said 'open')
		...
		(cond
			...
			((Said '[/man]>')
				(cond
					((TurnIfSaid self event 'look/*'))
					((Said 'look[<at]')
						(Print 12 13) ; "He's just lying there."
					)
					((Said 'talk')
						(Print 12 14) ; "He doesn't seem to hear you."
					)
					((Said 'wake,kick')
						(Print 12 15) ; "Why would you do that?"
					)
				)
			)
		)
...
(instance wave of SoundSyncWave
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/wave]')
				(Print 12 16) ; "These are gentle waves."
			)
		)
	)
		...
		(cond
			...
			((Said '[/window]>')
				(cond
					((TurnIfSaid self event 'look/*'))
					((Said 'look[<at]')
						(Print 12 17) ; "A hut window."
					)
					((Said 'look<in')
						(Print 12 18) ; "The reflection on the window prevents you from looking in."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/bush,palm]')
				(Print 12 19) ; "You see a dense jungle."
			)
		)
	)
)
...
(instance palmTree of RFeature
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/palm]')
				(Print 12 20) ; "The palm tree is of no interest to you."
			)
		)
	)
)
