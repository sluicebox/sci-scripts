...
(procedure (localproc_0 &tmp temp0)
	(= temp0 (CosMult (Submarine _absHeading:) (Submarine hSpeed:)))
	(if (< -10000 local12)
		(+= local12 temp0)
	)
)
		...
		(switch (= state newState)
			...
			(1
				(if (== register 0)
					...
					(Print 364 2) ; "Having ripped a gaping hole in the bow, you send the once proud USS Black Hawk to the bottom."
...
(instance slider of Act
	...
	(method (doit &tmp temp0)
		...
		(if (and (< 350 local12) (not (bergsOverScript client:)))
