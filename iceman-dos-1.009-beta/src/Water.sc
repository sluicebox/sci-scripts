		...
		(cond
			...
			((Said 'look>')
				(cond
					...
					((Said '[<at,down][/beach,sand,floor]')
						(switch (Random 1 3)
							(1
								(Print 301 7) ; "The white sandy beaches of Tahiti are some of the most beautiful in the world."
							)
							(2
								(Print 301 8) ; "You look down at the warm white sand."
							)
							(3
								(Print 301 9) ; "You look at the sand and think, "How beautiful.""
							)
						)
					)
					((Said '[<at,up][/sky]')
						(switch (Random 1 2)
							(1
								(Print 301 10) ; "You look up and see... a deep blue sky."
							)
							(2
								(Print 301 11) ; "You look up and are amazed at the beauty."
							)
						)
					)
				)
...
(instance bikini3Lady of Act
	...
	(method (handleEvent event)
		(cond
			((IsOffScreen self))
			((Said 'look[<at][/woman[<native]]')
				(closeUpView show: view)
				(Animate (gCast elements:) 0)
				(Print 301 13) ; "Whew, what a hot lookin' native girl!"
				(closeUpView hide:)
				(event claimed: 1)
			)
			((Said 'whistle')
				(Print 301 14) ; "She ignores your ancient ploy."
			)
			((Said '[/bitch,cunt,boob,brunette,woman[<native]]>')
				(cond
					((Said 'chase')
						(Print 301 15) ; "You can do that."
					)
					((Said 'talk')
						(Print 301 16) ; "She's trying to ignore you."
					)
					((Said 'kiss,suck,fuck')
						(if (> (gEgo distanceTo: self) 50)
							(Print 301 17) ; "You'll have to get closer than this."
						else
							(QueScript self 352)
						)
					)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/blond,woman[<blond]]')
				(closeUpView show: view)
				(Animate (gCast elements:) 0)
				(Print 301 18) ; "This blonde is definitely a babe."
				(closeUpView hide:)
				(event claimed: 1)
			)
			((Said 'whistle')
