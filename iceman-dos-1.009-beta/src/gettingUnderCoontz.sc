...
(instance gettingUnderCoontz of Script
	...
	(method (doit &tmp temp0)
		...
		(if (and (not (-- local0)) (not script))
			(self setScript: didntGetUnderCoontz)
		)
		(super doit:)
	)
...
(instance didntGetUnderCoontz of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 7 0 0 341 5) ; "Having missed your rendevous with the USS Coontz, you lost your protection. Mission aborted."
			)
		)
	)
)
...
(instance showedSelf of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 7 0 0 341 6) ; "The Coontz, having seen your sub surface, has assumed mission aborted."
			)
		)
	)
)
...
(instance firedOnCoontz of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 926 1 0 341 7) ; "The United States rules of engagement prohibit our use of weapons unless fired upon. You're a shame to the service, Johnny."
			)
		)
	)
)
...
(instance diedUnderCoontz of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(proc396_0 4 341 8) ; "SIR, WE HAVE HIT THE COONTZ."
			)
			(1
				(EgoDead 7 0 0 341 9) ; "Hitting the Coontz has ripped off your conning tower, down, down we go."
			)
		)
	)
)
