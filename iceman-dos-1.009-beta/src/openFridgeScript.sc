...
(instance cheezActionScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (== local1 1)
					(Print 362 3) ; "Loving cheese like you do, you eat the entire block."
				else
				...
				(cond
					((Said 'remove/lid')
						...
						(if (gEgo has: 14) ; Sub: Code_Book | Tunisia: Note
							...
							(Print 362 8) ; "A liplock bag that appears to have a piece of paper folded up inside."
						)
					)
					((Said 'look/liplock,bag[<liplock]')
						(if (gEgo has: 14) ; Sub: Code_Book | Tunisia: Note
							(event claimed: 1)
						else
					...
					(
						(or
							(Said 'get,open/liplock,bag[<liplock]')
							(Said 'get,remove/newspaper,note,letter')
						)
						(if (gEgo has: 14) ; Sub: Code_Book | Tunisia: Note
							(event claimed: 1)
						else
							(Print 362 11) ; "You get the liplock bag and remove the folded piece of paper."
			...
			((Said '[/(dish<butter),butter,butterdish]>')
				(cond
					((Said 'examine,look[<at]')
						(Print 362 17) ; "Looks like a normal butter dish with a lid."
					)
					((Said 'get')
						(if (gEgo script:)
							(Print 362 18) ; "You already have it."
						else
					...
					((Said 'place,drop')
						(if (gEgo script:)
							...
							(Print 362 19) ; "You don't have it."
						)
					)
				)
			)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/catsup]>')
				(cond
					((Said 'look[<at]')
						(Print 362 20) ; "A bottle of ketchup."
					)
					((Said 'get')
						(if (gEgo script:)
							(localproc_0)
						else
							(Print 362 21) ; "Picking the ketchup bottle up you think, "Yep, it's a bottle of ketchup alright." You then return it to the icebox."
						)
					)
					((Said 'open')
						(Print 362 22) ; "Don't bother, it's just ketchup."
					)
				)
			)
		)
