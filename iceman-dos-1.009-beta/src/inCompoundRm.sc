		...
		(cond
			...
			((Said 'shoot,fire[/guard,gun]')
				(cond
					...
					(else
						(Print 87 5) ; "You don't have the weapon."
					)
				)
			)
...
(instance messageScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 87 7 #at 10 10) ; "After entering the room, the second guard stands near the seated Ambassador."
				(Print 87 8 #at 10 10) ; "You start to perspire as you think to yourself, "I'm gonna have to move fast for sure!""
				(self dispose:)
			)
		)
	)
				...
				(cond
					((Said 'talk')
						(if (== (backGuard view:) 487)
							(Print 87 9) ; "After taking a look at the guards, you decide against it."
						else
							(Print 87 10) ; "You calm the ambassador and tell him who you are."
						)
					)
					((Said 'clear,untie')
						(if (== (backGuard view:) 487)
							(Print 87 11) ; "Not with the guards watching you."
						else
...
(instance westGuardScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HandsOff)
				(Print 87 12 #at 10 10) ; "In a stern voice the guard says, "Just give the man his food and get on with your business!""
				(if (not (gEgo script:))
			...
			(6
				(EgoDead 970 1 0 87 13) ; "Your hesitation has brought this mission to a premature end."
			)
		)
	)
)
...
(instance forcedOutScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not global55)
					(Print 87 14 #at 10 10) ; "You carefully place the food on the table."
				)
				(gEgo
					view: 187
					loop: 1
...
(instance setDownFoodScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 87 15 #at 10 10) ; "Quickly you grab the gun and point it at the nearest guard."
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/lid,cover]>')
				(if (Said 'remove,(get<off)')
					(if (gCast contains: foodLidView)
						(Print 87 16) ; "You already did that."
					else
						(self cue:)
					)
				)
			...
			((Said 'get/gun')
				(if (== state 4)
					...
					(Print 87 17) ; "You'll have to remove the lid first."
				)
			)
		)
	)
...
(instance westShootEgoScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 970 1 0 87 18) ; "Because of your slow reaction, the guard fires his automatic weapon at you and fills you full of holes."
			)
		)
	)
)
...
(instance backGetsShotScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 87 19 #at 10 10) ; "You fire a second tranquilizer dart at the remaining guard."
				(Print 87 20 #at 10 10) ; "The fast acting drug takes affect as he clutches his chest and slumps to the floor."
				(gGame changeScore: 5)
				(HandsOn)
				(gEgo
					view: 684
...
(instance freeAmbassadorScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 87 21 #at 10 10) ; "You calm the Ambassador as you explain to him who you are."
				(Print 87 22 #at 10 10) ; "Oh my God man! You don't know how happy I am to see you."
				(gEgo
					view: 684
					loop: 2
					setCycle: Walk
			...
			(7
				...
				(Print 87 23 #at 10 10) ; "He continues, "I was beginning to think this day would never come!""
				(Print 87 24 #at 10 10) ; "Truly" he says, "You are a real American hero!"
				(= seconds 4)
			)
			(8
				(Print 87 25 #at 10 10) ; "The Ambassador says, "I'll be so glad to see my family again.""
				(= seconds 4)
			)
			(9
				(Print 87 26 #at 10 10) ; "John my boy," he continues, "I am personally going see to it that you are decorated for bravery!"
				(= seconds 4)
			)
			(10
				(Print 87 27 #at 10 10) ; "That was ssssome tricky shooting Mr. Westland!"
				(= seconds 3)
			)
			(11
...
(instance changeClothesScript of Script
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'exit[/room]') (Said 'go<out'))
				(if (< state 5)
					(Print 87 28) ; "Hold on," the ambassador says, "I'm not done."
				else
...
(instance leaveRoomScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 87 29 #at 10 10) ; ""We'd better get out of here quick !" you tell the ambassador."
				(HandsOff)
...
(instance tookTooLongScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 87 30 #at 10 10) ; "The Ambassador screams, "WATCH OUT FOR THE GUARD!""
			...
			(3
				(EgoDead 970 1 0 87 31) ; "Wasting too much time listening to the Ambassador has cost you your life."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at]/platter,food')
				(if (gCast contains: foodLidView)
					(Print 87 32) ; "There is your gun laying on the platter."
				else
					(Print 87 33) ; "The platter has it's lid on."
				)
			)
			((Said 'get/gun')
				(if (not (gCast contains: foodLidView))
					(Print 87 34) ; "You'll have to remove the lid first"
				else
...
(instance westGuard of Act
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look/man,guard')
				(if (== view 787)
					(Print 87 35) ; "He looks pretty ruthless."
				else
					(Print 87 36) ; "Even unconcious, he looks pretty tough."
					(Print 87 37 #icon 387 0 0) ; "You notice he's wearing a ring with a KGB symbol.  "So," you think, "our friends the Russians are in on this!""
...
(instance table of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'give,place/tray,food[/table]')
				(if (gCast contains: foodView)
					(Print 87 38) ; "It's already on the table."
				else
					(gEgo setScript: setDownFoodScript)
				)
			)
			((Said 'look[<at][/table]')
				(Print 87 39) ; "It's just a wooden table."
			)
		)
	)
)
...
(instance mosaicPic of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((or (Said 'look[<at][/wall,picture]') (MousedOn self event))
				(Print 87 40) ; "You see a mosaic on the wall."
			)
		)
	)
)
