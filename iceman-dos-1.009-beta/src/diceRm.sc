...
(procedure (proc39_1)
	(if
		(or
			(> (oldSaltDice diceScore:) 500)
			(< 300 (oldSaltDice diceScore:) 400)
		)
...
(instance diceRm of Rm
	...
	(method (init)
		...
		(saltLArmP stopUpd: init: setScript: smokeCigar)
		(saltMouth setPri: 8 init: cel: 1)
		(saltEyes setPri: 9 init: cel: 1 setScript: blinkerScript)
		(johnnyLArmP init: setScript: johnTwitch)
		(johnnyRArmP stopUpd: init:)
		(johnnyMouth setPri: 6 init: hide:)
		(johnnyEyes setPri: 7 init: cel: 1 setScript: (Clone blinkerScript))
	...
	(method (replay)
		(Print 39 0) ; "The old salt looks you in the eye and says, "Now lad I won't be playin with you restoring every time you lose. That be cheatin'""
		...
		(cond
			...
			((Said 'bet/bottle')
				(if (gEgo has: 4) ; Tahiti: Change | Sub: Rum_Bottle | Tunisia: Capsule
					(Print 39 1) ; "You need the bottle."
				else
					(Print 39 2) ; "You have to win the bottle first."
				)
			)
			((Said 'drink/rum')
				(if (gEgo has: 4) ; Tahiti: Change | Sub: Rum_Bottle | Tunisia: Capsule
					(Print 39 3) ; "A good naval officer would never drink while on an important mission."
				else
					(Print 39 2) ; "You have to win the bottle first."
				)
			)
			((Said 'look>')
				(cond
					((Said '[<around][/room]')
						(Print 39 4) ; "You look around the small cafeteria called a "galley"."
						(Print 39 5) ; "This is the place where the crewmen of the Blackhawk stuff themselves with tender morsels."
					)
					((Said '/table')
						(Print 39 6) ; "You see the galley table that you are playing Boss Dice on!."
					)
				)
			)
			((Said 'talk')
				(Print 39 7) ; "Just play."
			)
		)
	)
...
(instance oldSaltV of View
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[/man]')
				(Print 39 8) ; "Sitting across from you is Jacob Flanagan."
			)
		)
	)
...
(instance rumBottle of View
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/bottle]')
				(Print 39 9) ; "You see a bottle of smooth Alonzo rum on the table."
			)
		)
	)
)
