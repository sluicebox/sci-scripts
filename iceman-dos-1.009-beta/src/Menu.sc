...
(class MenuBar of MenuBar
	...
	(method (init)
		(AddMenu { \01 } { About Iceman `^a: Help`#1: Debug Help`#4 })
		(AddMenu
			{ File }
	...
	(method (handleEvent event &tmp temp0 [temp1 100] temp101)
		(switch (super handleEvent: event)
			(257
				(Print
					(Format @temp1 997 0) ; "DESIGNED BY: Jim Walls"
					#title
					{__Iceman Credits__}
					#mode
					1
				...
				(Print
					(Format @temp1 997 1) ; "ILLUSTRATED BY: Cheryl Loyd Jim Larsen Jerry Moore Cindy Walker Jeff Crowe Kenn Nishiuye"
					#title
					{__Iceman Credits__}
					#mode
					1
				...
				(Print
					(Format @temp1 997 2 gVersion) ; "PROGRAMMED BY: J.Mark Hood Doug Oldfield Larry Scott Pablo Ghenis Carlos Escobar Robert Lindsley David Skinner version: %s"
					#title
					{__Iceman Credits__}
					#mode
					1
			...
			(259
				(Print 997 4 #font gSmallFont) ; "Debug items available in   rooms with sufficient heap Alt F   Show Memory Alt G   Set Global Variable Alt V   Show Visual Map Alt P   Show Priority Map Alt C   Show Control Map Alt A   Show Cast Alt T   Teleport Alt N   Make Note (Bug List) Alt R   Show Current Room Number Alt D   Toggle Debug at Room Change Alt Y   Vanishing Point"
			)
			(513
				(gGame save:)
			)
			...
			(516
				(if (Print 997 5 #button {Restart} 1 #button {Continue} 0) ; "Do you really want to  restart your game?"
					(gGame restart:)
				)
			)
			(517
				(= gQuit
					(Print 997 6 #button {Quit} 1 #button {Continue} 0 #icon 7 0 0) ; "Giving up already?"
				)
			)
			(769
				(= temp101 (Sound pause: 1))
				(Print 997 7 #title {Game Paused} #button {Let's roll.} 1) ; "Every spy needs a 10-7 (break) now and then."
				(Sound pause: temp101)
			)
			(770
				(if (proc0_4 1024)
					...
					(Print 997 8) ; "Sorry, not enough memory."
				)
			)
			(771
