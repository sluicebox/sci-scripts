...
(procedure (localproc_3 param1 param2 param3 param4 &tmp temp0)
	(= temp0 (localproc_4 param1 param2 param3 param4))
	(cond
		((< (- param3 param1) 0)
			(if (<= (- param4 param2) 0)
				(+= temp0 180)
...
(procedure (localproc_4 param1 param2 param3 param4 &tmp temp0 temp1 temp2 temp3)
	...
	(if (<= (= temp1 (Abs (- param4 param2))) temp0)
		...
		(if (< (= temp2 (/ (* 1000 temp1) temp0)) 100)
			...
			(= temp3 (localproc_5 temp2))
		)
	else
		(= temp3 (- 90 (localproc_4 param2 param1 param4 param3)))
	)
	(return temp3)
)
...
(instance scubaCycle of Rm
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'look[<at]/timer')
				(if (not (gEgo has: 3)) ; Tahiti: Black_Book | Sub: Explosive | Tunisia: Fish
					(Print 51 3) ; "You don't have a timer."
				else
					(Print 51 4) ; "A small digital timer for triggering explosives."
				)
			)
			((Said 'look[<at]/target')
				(cond
					(local25
						(Printf ; "Harbor = %d DEGREES ,%d YARDS"
							...
							(localproc_2 local17 local18)
						)
					)
					(local29
						(Print 51 6) ; "Your inertia device has been reset to zero once you have reached you destination."
					)
					(else
						(Printf ; "Oil rig = %d DEGREES ,%d YARDS"
							...
							(localproc_2 local15 local16)
						)
					)
				)
			)
			((Said 'look[<at]/instrument,pod')
				(Print 51 8) ; "This is the only information you have."
			)
		)
	)
...
(instance outOfAir of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (gEgo has: 6) ; Tahiti: Earring | Sub: Diver | Tunisia: Key
					(EgoDead 157 0 1 51 9) ; "Forced to the surface because your air ran out, you were captured."
				else
					(EgoDead 157 0 1 51 10) ; "Forced to the surface because your air ran out, you should have taken the dive vehicle."
				)
			)
		)
	)
...
(instance headingGauge of Code
	...
	(method (doit &tmp [temp0 3])
		(cond
			(local25
				(= local22 (localproc_1 local17 local18))
				(= local23 (mod (localproc_2 local17 local18) 10000))
			)
			(local29
				(= local22 0)
				(= local23 0)
			...
			(else
				(= local22 (localproc_1 local15 local16))
				(= local23 (mod (localproc_2 local15 local16) 10000))
			)
		)
		(Display 51 11 dsCOORD 219 17 dsCOLOR 8 dsALIGN alRIGHT dsWIDTH 25 dsFONT 30) ; "888"
		(Display
			(Format @temp0 51 12 local22) ; "%d"
			dsCOORD
			219
			17
			dsCOLOR
		...
		(Display 51 11 dsCOORD 267 17 dsCOLOR 8 dsALIGN alRIGHT dsWIDTH 25 dsFONT 30) ; "888"
		(Display
			(Format @temp0 51 12 local23) ; "%d"
			dsCOORD
			267
			17
			dsCOLOR
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond
					((localproc_7)
						(Print 51 13) ; "You look for a good spot to place the explosives."
						(gEgo
							illegalBits: 0
					...
					(else
						(= local26 0)
						(Print 51 14) ; "You're not near enough to the oil rig."
						(HandsOn)
						(self dispose:)
					)
				)
			...
			(6
				(Print 51 15) ; "You start the timer."
...
(instance blowUp of Script
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			(0
				(soundExplosion number: (proc0_5 73) init: play: blowUp)
			)
			(1
				(cond
					(
...
(instance blowUp1 of Script
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(2
				(soundExplosion dispose:)
				(EgoDead 926 1 0 51 16) ; "Your next assignment is as a crash test dummy for GM."
			)
		)
	)
)
			...
			(3
				...
				(cond
					((not (& (subMarine roomFlags:) $0020))
						(Print 51 17) ; "You should have checked the DV-3X. A missing washer has caused the diver to malfunction."
					)
					((not (& (subMarine roomFlags:) $0040))
						(Print 51 18) ; "Although you suppiled a missing washer you forgot to put a nut on the washer."
					)
					(else
						(Print 51 19) ; "You did not tighten the nut properly and now you are without the diver."
					)
				)
				(self dispose:)
			)
