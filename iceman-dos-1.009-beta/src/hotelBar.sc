...
(instance hotelBar of Rm
	...
	(method (init &tmp temp0)
		...
		(gAddToPics
			add:
				...
				((Clone cPPV) x: 264 y: 155 name: {cP3} priority: 13 yourself:)
			eachElementDo: #init
			doit:
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'drink')
				(Print 11 1) ; "Order a drink first."
			)
			((Said 'look[<at,around][/room]')
				(Print 11 2) ; "This is the hotel bar."
			)
			((Said 'dance')
				(Print 11 3) ; "You don't want to dance alone."
			)
			((Said 'open/door')
				(Print 11 4) ; "The door will open when you walk over to it."
			)
		)
	)
)
...
(instance husband of Act
	...
	(method (init)
		...
		(if (& (tahiti flags:) $0001)
			(self view: 11 setLoop: 5 posn: 169 146 setCel: 16)
		else
	...
	(method (handleEvent event)
		(cond
			...
			((IsOffScreen self))
			((Said 'look[<at][/man]')
				(Print 11 5) ; "You see a big Samoan man with a bad attitude."
			)
			((Said 'talk[/man]')
				(if (< local4 91)
					(= local4 0)
...
(instance aS of Script
	...
	(method (handleEvent event)
		(cond
			...
			(
				(or
					(Said 'buy,order/(mai<tai)[<woman][/woman]')
					(Said 'buy,order/drink[<woman][/woman]')
				)
				(cond
					((& (tahiti flags:) $0800)
						(Print 11 6) ; "I still have the one you bought me, thank you."
					)
					((gEgo script:)
						(Print 11 7) ; "Wait a bit."
					)
					(else
			...
			((or (Said 'dance') (Said 'ask/woman/dance'))
				(self setScript: wTDS self)
			...
			((Said 'sit')
				(cond
					((not (& (tahiti flags:) $0800))
						(Print 11 8) ; ""Aren't you at least going to buy me a drink first? " the beautiful young lady asks."
					)
					((not (& (tahiti flags:) $1000))
						(Print 11 9) ; "Just a little bold don't you think?" she says, " And we haven't even danced yet!"
					)
					(else
			...
			(0
				...
				(cond
					...
					((gEgo inRect: 248 129 293 167)
						(gEgo setMotion: MoveTo 248 167 self)
					)
					(else
						(= cycles 1)
					)
...
(instance eSitAS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 11 10) ; ""Would you like to come back to my place?" the girl asks."
			)
		)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'buy,order/drink[<woman][/woman]')
				(if (== (local2 view:) 9)
					(Print 11 11) ; ""I still have the one you bought me, thank you." the girl says."
				else
			...
			((Said 'yes')
				(if (== state 3)
					...
					(Print 11 12) ; "Yes what?"
				)
			)
			((Said 'no')
				(if (== state 3)
					(Print 11 13) ; ""Well alright then, maybe some other time." she says, disappointed."
					(local2 setScript: agentHuffScript)
				else
					(Print 11 14) ; "No what?"
				)
...
(instance gADS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 11 15) ; "Oh," she says, "I wouldn't mind sharing a bottle of champagne with you."
				(proc11_2 0)
				(HandsOff)
			...
			(2
				(Print 11 16) ; "Hey bartender, can I have a bottle of champagne and two glasses."
			...
			(4
				(Print 11 17) ; "Oh thank you."
				(Print 11 18) ; "Would you pour me a glass please?"
...
(instance lush of Act
	...
	(method (init)
		(super init:)
		(lAV init:)
		(self stopUpd: setScript: aES ignoreControl: -32768)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'talk[/woman,woman,lisa]')
				(cond
					((> (lDS state:) 8)
						(Print 11 19) ; "She's in no shape to talk."
					)
					((== (gEgo view:) 206)
						(self cue:)
						(if (not (cOS client:))
			...
			((or (Said 'buy,order/drink<woman') (Said 'buy,order/drink/woman'))
				(++ local0)
				(if (!= (bDS next:) bDS)
					(QueScript bartender bDS 0 lush)
				)
			)
			((or (Said 'dance') (Said '/woman,woman'))
...
(instance cOS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 11 20) ; "Her face appearing slightly flushed, the young lady says..."
				(Print 11 21) ; "HI big fella, sit and join me. I'm drinking Mai Tai."
				(= seconds 10)
			)
			(1
				(Print 11 22) ; "Oh HI!" she says, " Please sit down."
				(= seconds 10)
			)
			(2
				(Print 11 23) ; "Coyly she speaks, "Would you like to join me? I'm having a Mai Tai.""
				(= seconds 10)
			)
			(3
				(self init:)
...
(instance lDS of Script
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'help[/woman]')
				(if (and (>= local1 4) (OneOf state 9 10))
					(gEgo setScript: eTDS)
				else
					(Print 11 24) ; "At this point she is beyond your help."
				)
			)
			((or (Said '(get<up),stand[<up]') (Said 'exit/stool'))
				(cond
					((== (lush loop:) 7)
						(Print 11 25) ; "Keep your seat, the bartender has already called someone for this imbibed lady."
					)
					(local1
						(Print 11 26) ; "The lady says, "Buy us another round first big boy.""
						(++ local0)
			...
			((Said 'look[<at][/woman]')
				(if (OneOf state 9 10)
					(Print 11 27) ; "You see a pitifully drunk young lady."
				else
					(Print 11 28) ; "You see a lady who seems more interested in drinking than talking."
				)
			)
		)
	)
...
(instance stars of Prop
	...
	(method (init param1)
		(super init:)
		(self
			posn: (- (param1 x:) 15) (- (param1 y:) 10)
			setCycle: ForwardCounter 20 self
		)
	)
...
(instance eDS of Script
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'drink')
				(if (>= register 5)
					(Print 11 29) ; "Your glass is empty."
				else
					(self init:)
				)
			)
...
(instance eTDS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 907 1 0 11 30) ; "Passed out, Huh?"
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(if (== (gEgo view:) 206)
					(Print 11 31) ; "You'll have to find a stool first."
				else
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/woman]>')
				(cond
					((Said 'look[<at]')
						(Print 11 32) ; "A woman having a drink."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'talk')
						(Print 11 33) ; "She doesn't answer."
					)
				)
			)
		)
			...
			((Said '[/woman]>')
				(cond
					...
					((Said 'buy,order[/woman]/drink')
						(Print 11 34) ; "I already have one, thank you."
					)
				)
			)
			((Said 'ask/woman,name/name')
				(Print 11 35) ; "I'm sorry." she tells you, "But I don't divulge that information!"
			)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/man]>')
				(cond
					((Said 'look[<at]')
						(Print 11 36) ; "A man having a drink."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'talk')
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/man,local]')
				(Print 11 37) ; "A local having a drink."
			)
			((> (gEgo distanceTo: self) 40))
			((Said 'talk[/man,local]')
				(localproc_1 611 95 107)
		...
		(cond
			...
			((Said 'sit')
				(if (== (gEgo view:) 206)
					...
					(Print 11 38) ; "You already are seated."
				)
			)
			((Said 'stand,(get<up)')
				(cond
					((== (gEgo view:) 206)
						(Print 11 39) ; "You already are standing."
					)
					((== (lush loop:) 7)
						(event claimed: 0)
					)
...
(instance lAB of Prop
	...
	(method (handleEvent event)
		(cond
			...
			((!= (gEgo view:) 206))
			((Said 'look[<at][/woman]')
				(Print 11 42) ; "A woman sitting at the bar."
			)
			((> (gEgo distanceTo: self) 40))
			(
				(and
					(< (gEgo x:) x)
					(or (Said '/woman,woman') (Said 'talk,dance[/woman,woman]'))
				)
...
(instance mAB of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 11 43) ; "A man sitting at the bar."
			)
			((> (gEgo distanceTo: self) 40))
			((Said 'talk[/man]')
				(localproc_1 611 95 107)
...
(instance pPV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/plant]')
				(Print 11 44) ; "You see a nice fern in the corner of the bar."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/flower,centerpiece]>')
				(cond
					((Said 'look[<at]')
						(Print 11 45) ; "There is a small vase with fresh plumaria blossoms in the center of the table."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'smell')
						(Print 11 46) ; "The beautiful fragrance of plumaria fills the room."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/mirror]>')
				(cond
					((Said 'look[<at,in]')
						(Print 11 47) ; "You look into the mirror and see your happy reflection smiling back at you."
					)
					((Said 'break')
