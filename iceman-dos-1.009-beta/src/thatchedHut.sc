...
(instance thatchedHut of Rm
	...
	(method (init)
		...
		(if (== (tahiti script:) (ScriptID 300 1)) ; tookTooLongScript
			...
			(self setFeatures: noSinkyDinghy)
		)
		...
		(switch gPrevRoomNum
			...
			(else
				(HandsOn)
				(gEgo view: 200 posn: 133 105 setCycle: Walk)
			)
		)
		(shirt init:)
...
(instance noSinkyDinghy of PV
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/boat,dinghy]')
				(Print 2 16) ; "This is your transportation off of the Island."
			)
			((Said 'talk[/man]')
				(Print 2 17) ; "You all ready to go, bub?"
			)
			((Said 'board,(get<on)[/boat,dinghy]')
...
(instance dinghyRamp of View
	...
	(method (doit)
		...
		(if (== (gEgo onControl: 1) 256)
			(HandsOff)
			(gEgo viewer: 0 setScript: finallyOffThisStupidIsland)
			(|= signal $0100)
		)
	)
)
...
(instance finallyOffThisStupidIsland of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(tahiti setScript: 0)
				(Print 2 19) ; "Next stop: Tahiti airport."
				(gEgo
					illegalBits: 0
					ignoreActors: 1
		...
		(if (Said '/shirt>')
			(cond
				((== (gEgo view:) 206)
					(cond
						((Said 'look[<at]')
							(Print 2 20) ; "It's a nice floral aloha shirt."
						)
						((Said '(get<!*),wear,(place<on)')
							(Print 2 21) ; "You're already wearing your shirt."
						)
						((TurnIfSaid self event))
						((GoToIfSaid self event (- x 5) (- y 5) 0 2 13))
						((Said 'remove,(get<off)')
				...
				((Said 'remove,(get<off)')
					(Print 2 22) ; "You're not wearing it."
				)
				((TurnIfSaid self event 'look[<at]/*'))
				((Said 'look[<at]')
					(Print 2 20) ; "It's a nice floral aloha shirt."
...
(instance readPaperS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 2 23) ; "You read an article featuring the North African country of Tunisia. Because of the global oil shortage, this small third world country finds itself in a powerful political position."
				(Print 2 24) ; "Because of Tunisia's neutral stance, and her overabundance of high grade crude, the United States and Russia find themselves in a tug of war as they negotiate for the oil."
				(Print 2 25) ; "After reading the article you begin to think, "Hmm, what a dangerous, explosive situation this could be.""
