...
(local
	...
	local64 = 285
	local65 = 77
	[local66 28] = [0 15 30 45 60 75 90 105 120 130 135 140 145 150 155 141 129 116 104 94 83 69 55 41 37 28 16 4]
	[local94 42] = [0 15 30 45 60 75 90 105 120 135 150 165 180 195 210 225 235 285 315 316 298 282 270 260 250 235 223 209 195 180 165 155 143 128 118 106 67 37 0 0 0 0]
	local136
	local137
	local138
	local139
	local140
	local141
	[local142 12] = [151 83 151 39 226 12 301 19 274 48 285 77]
	[local154 14]
	local168
	local169
)
...
(procedure (localproc_0 param1 &tmp temp0 temp1)
	(= temp0 (= temp1 0))
	(while (< [local94 37] param1 [local94 (+ 19 temp1)])
		(= temp0 temp1)
		(++ temp1)
	)
	(if (== temp0 temp1)
	...
	(= temp0
		(+
			...
			(/
				(*
					(- param1 [local94 (+ temp0 19)])
					(- [local94 temp1] [local94 temp0])
				)
				(- [local94 (+ temp1 19)] [local94 (+ temp0 19)])
			)
		)
	)
	(if (< 195 temp0)
...
(procedure (localproc_1 param1 &tmp temp0 temp1)
	(= temp0 (= temp1 0))
	(while (< [local94 temp1] param1 [local94 18])
		(= temp0 temp1)
		(++ temp1)
	)
	(if (== temp0 temp1)
...
(procedure (localproc_2 param1 &tmp temp0 temp1)
	(= temp0 (= temp1 0))
	(while (<= [local66 27] param1 [local66 (+ 14 temp1)])
		(= temp0 temp1)
		(++ temp1)
	)
	(if (== temp0 temp1)
		...
		(-
			(= temp0
				(+
					...
					(/
						(*
							(- param1 [local66 (+ temp0 14)])
							(- [local66 temp1] [local66 temp0])
						)
						(- [local66 (+ temp1 14)] [local66 (+ temp0 14)])
					)
				)
			)
			60
...
(procedure (localproc_3 param1 &tmp temp0 temp1)
	(= temp0 (= temp1 0))
	(while (< [local66 temp1] param1 [local66 13])
		(= temp0 temp1)
		(++ temp1)
	)
	(if (== temp0 temp1)
...
(procedure (localproc_10 &tmp temp0 temp1)
	(localproc_15)
	(= local168 0)
	(for ((= temp0 0)) (and (< temp0 14) (!= [local154 temp0] 0)) ((++ temp0))
		(if
			(<
				local168
				(= temp1 (localproc_11 [local154 temp0] [local154 (++ temp0)]))
			)
			(= local168 temp1)
		)
	)
	(Printf 40 22 local168) ; "Your course has a maximum deviation of %d nautical miles from an optimal course."
	(if (< 12 local168)
		(return 0)
	else
		(return 1)
...
(procedure (localproc_15 &tmp temp0 temp1 temp2)
	...
	(if (IsObject (= temp2 (NodeValue (= temp1 (markerList first:)))))
		(while (< temp0 14)
			(= [local154 temp0] (temp2 x:))
			(++ temp0)
			(if (< 90 (= [local154 temp0] (temp2 y:)))
				(= local169 1)
			)
			(if
				(IsObject
...
(instance chartRm of Rm
	...
	(method (init &tmp temp0)
		...
		(= local62 (localproc_1 185))
		(= local63 (localproc_3 90))
		(= local60 (localproc_1 (+ (Submarine longitude:) 15)))
		(= local61 (localproc_3 (+ (Submarine latitude:) 60)))
		(gEgo
			view: 40
			setLoop: 4
			cel: 0
			xStep: 1
			yStep: 1
			posn: local62 local63
			ignoreActors: 1
			init:
			setScript: stealMouse
		)
		(subMarker x: local60 y: local61 init: setCycle: Fwd)
		(destMarker x: local64 y: local65 init: setCycle: Fwd)
		(markerList
			addToEnd:
				((locationMarker new:)
					view: 40
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look')
				...
				(Print 40 1) ; "You must plot an acceptable course before embarking on your mission. The command "Plot Course" will allow you to plot or replot a course to your destination (just off the coast of Portugal). Once ploting a course you may change the last waypoint with the command "Change Point"."
				(Print 40 2) ; "A course is a series of waypoints that the sub can travel in straight lines to arrive at the set destination.  The number of waypoints is limited to five. To set a waypoint use the direction arrows to set the desired longitude and latitude, then press the ENTER key. When your course is complete except for the last leg to your destination, use the ENTER key twice."
				(Print 40 3) ; "Shift along with a direction key will alter the rate of change of the latitude and longitude."
				(Print 40 4) ; "The course will then be reviewed by the on board navigation computer and either rejected or accepted.  Use the map enclosed with your game to aid in establishing the proper longitude and latitude for each waypoint. Good luck on your mission."
			)
			((Said 'exit,exit,stop')
				(if
					(or
						local141
						(!= local60 (localproc_1 185))
						(!= local61 (localproc_3 90))
					)
					(localproc_7 374)
					(localproc_8 375)
					(localproc_7 374)
...
(instance mapArea of RFeature
	...
	(method (handleEvent event &tmp temp0)
		(cond
			...
			(
				(or
					(Said 'draw,draw,place,draw,chart,enter/course,coordinate')
					(and
						(not local54)
				...
				(if
					...
					(Print 40 10) ; "You can not replot or change course once underway."
				)
			)
		)
	)
	...
	(method (cue &tmp temp0)
		(if
			(and
				...
				(> (courseMarker distanceTo: temp0) 1)
			)
...
(instance drawLastLine of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(= local138 (NodeValue (markerList last:)))
				(= temp0 (NodeValue (markerList prev: (markerList last:))))
				(courseMarker posn: (temp0 x:) (temp0 y:))
				(courseMarker
					penColor: 7
					setMotion: MoveTo (local138 x:) (local138 y:) self
				)
			)
			(1
				(localproc_6)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if
					(IsObject
						(= local139 (markerList at: (- (markerList size:) 2)))
					)
					(= local138 (NodeValue (markerList last:)))
					(courseMarker
						posn: (local139 x:) (local139 y:)
						penColor: 0
						setMotion: MoveTo (local138 x:) (local138 y:) self
					)
				else
			...
			(1
				(gEgo posn: (local139 x:) (local139 y:))
				(markerList delete: local138)
				(local138 dispose:)
				(localproc_6)
				(self dispose:)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(= local54 0)
				(User canControl: 0)
				(if (and (== local136 (gEgo x:)) (== local137 (gEgo y:)))
					(self changeState: 6)
				else
					(= local136 (gEgo x:))
					(= local137 (gEgo y:))
					(if
						(or
							(& (OnControl CONTROL local136 local137) $2000)
							(& (localproc_9 local136 local137 1) $2000)
						)
