		...
		(cond
			...
			((Said '[/chef,man,cook]>')
				(cond
					...
					((Said 'talk')
						(Print 32 11) ; "The chef says to you, "I'm blowing too much smoke right now to chew the fat.""
					)
				)
			)
		)
...
(instance serveItScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 32 12 #at 30 20 #time 8 #dispose) ; "The cook slides a burger out through the window."
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(if (localproc_0)
					(if register
						(Print 32 13) ; "One per customer, please."
					else
						(client setScript: serveItScript)
						(= register 1)
					)
				else
					(Print 32 14) ; "The meal isn't ready yet."
				)
			)
			((Said 'eat[/food,sandwich,burger]')
				(if (gCast contains: burger)
...
(instance oldSalt of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((TurnIfSaid self event 'look/man[<old]'))
			...
			((Said 'ask[/name]')
				...
				(Print 32 20) ; "Continuing he says, "My title is Chief Petty Officer. I be the top sea dog of the torpedo room.""
				(Print 32 21) ; "Continuing to talk, he says, "Fer thirty long yeers now I've been ramblin' the high seas!""
			)
		)
	)
)
...
(instance oldSaltScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 32 23 #at 30 20) ; "Aye, Commander Westland!" says the old seaman, "You appear to be eye-ballin' me bottle of rum?"
				(Print 32 24 #at 30 20) ; "Continuing he says, "Would ye be interested in wagering a ten dollar bill for this fine bottle, sir?""
				(= seconds 5)
			)
			(3
				(Print 32 25 #at 30 20) ; "Being yer not a wagering man Commander," he says, "I'll be taking me bottle of rum to me bunk."
...
(instance oldSaltStandScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(oldSalt heading: 90 setCycle: Beg self)
			)
			(1
				(oldSaltChair show:)
				(oldSalt
