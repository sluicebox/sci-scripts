...
(instance antiSubNetRm of Rm
		...
		(switch gPrevRoomNum
			...
			(55 ; shore2Rm
				(if (< 50 (gEgo y:))
					...
					(gEgo x: 10 loop: 0 setMotion: MoveTo 325 70)
				)
			)
			(68 ; caveExitRm
				(HandsOff)
	...
	(method (doit)
		(if (and (< 45 (gEgo y:) 55) (not (gEgo script:)) (not local0))
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look<through/net')
				(Print 53 3) ; "Looking through the net you see what appears to be the entrance to a cave."
				(Print 53 4) ; "You think to yourself, "I wonder where the other end of that cave might be?""
			)
		)
	)
)
...
(instance rightNet of Feature ; UNUSED
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look<through/net')
				(Print 53 5) ; "You look through the net to the other side but can't see anything of significance."
			)
		)
	)
)
...
(instance hitNet of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 53 6) ; "You feel a tug on your scuba vehicle and realize that you have been detected by some magnetic field across the harbor entrance."
				(EgoDead 157 0 0 53 7) ; "Forced to the surface by enemy scuba divers, you are captured."
			)
		)
	)
)
