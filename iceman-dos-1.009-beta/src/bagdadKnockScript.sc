...
(instance bagdadInScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 365 4) ; "You open the door letting the caterer inside."
				((ScriptID 310 4) setCycle: Walk setMotion: MoveTo 126 150 self) ; bagdad
			)
			(2
				(Print 365 5) ; "Here is your order." He says, "Hot and ready to eat."
			...
			(7
				(Print 365 6) ; "After putting the food on the counter the caterer says, "That will be $15.00 please.""
				(= cycles 1)
			)
			(8
			...
			(10
				(Print 365 7) ; "The caterer says to you again, "That will be $15.00 please.""
				(= seconds 8)
			)
			(11
				(Print 365 8) ; "Impatiently, the caterer says, "Sir, I must be going, please pay me!""
				(= seconds 6)
			)
			(12
				(HandsOff)
				(Print 365 9) ; "Disgusted, the caterer decides he has more important things to do than wait for you."
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'shoot,draw,use/man,gun,tranquilizer,(gun<tranquilizer)')
				(cond
					((not (gEgo has: 8)) ; Sub: Device | Tunisia: Tranquilizer_Gun
						(proc0_38) ; "You don't have that."
					)
					((== (gEgo view:) 584)
						(Print 365 10) ; "You almost use the weapon to subdue the caterer, but decide to save your limited ammo for the compound guards."
					)
					(else
			...
			((or (Said 'remove/clothes') (Said 'undress'))
				(Print 365 11) ; ""You must be crazy!" he says to you in disgust."
				(client setScript: bagdadOutScript)
			)
			((Said '//food,platter<about')
				(Print 365 12) ; "It is piping hot." He says, "And ready to eat."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(Print 365 13) ; ""FREEZE." you command."
...
(instance handsUpScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 365 14) ; "Nervously, the caterer puts his hands over his head and says, "Please show mercy! I only have $50.00.""
				(= register 100)
			)
			(3
				(= register 0)
			...
			(17
				(Print 365 16) ; "You securely bind the hands and feet of the caterer. You then tape his mouth to prevent him yelling for help."
				(gGame changeScore: 3)
				(if local0
					(HandsOn)
					(= register 2)
			...
			(24
				(Print 365 17) ; "You put on the caterer's clothes as a disguise."
				(tunisia bagBound: 1)
				(HandsOn)
				(gEgo
					view: 684
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'remove/clothes') (Said 'undress'))
				(if (> state 4)
					(Print 365 18) ; ""I am already undressed." he says to you with a puzzled look."
				else
					(Print 365 19) ; "This is not a robbery." you say, "What I need is your caterers uniform."
					(Print 365 20) ; "Continuing, you say, "I must ask you to remove the uniform!""
					(self cue:)
				)
			)
			(
				(or
					(Said 'bind,bind[<up]/bagdad,man[<up]')
					(Said 'use/tape')
					(Said 'get/clothes[/man]')
					(Said '//clothes<for')
				)
				(cond
					((not (gEgo has: 7)) ; Sub: Flare | Tunisia: Duct_Tape
						(Print 365 21) ; "You need something to tie him up with."
						(tunisia flags: (| (tunisia flags:) $0004))
						(client cue:)
					)
					((< state 3)
