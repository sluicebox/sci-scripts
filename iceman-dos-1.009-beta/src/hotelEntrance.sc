	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (not local1)
					(lobbyDoor startUpd: setCycle: End)
		...
		(cond
				...
				(cond
					...
					((Said 'look[<at,in]')
						(if (> (gEgo distanceTo: self) 35)
							(Print 9 7) ; "You see a newspaper stand."
						else
							(Print 9 8) ; "Through the plastic window you see the latest edition of the Tahitian Times."
						)
					)
				)
			)
			((Said '[/newspaper,news,times,newspaper]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 9 9) ; "There are newspapers in the newspaper stand."
					)
					((GoToIfSaid self event self 25 0 9 5))
					((Said 'get,get')
						(Print 9 10) ; "You need to deposit fifty cents."
					)
					((Said 'buy')
						(if (gEgo has: 4) ; Tahiti: Change | Sub: Rum_Bottle | Tunisia: Capsule
							(Print 9 11) ; "You deposit fifty cents and remove a copy of the paper."
							(gEgo setScript: readPaperScript)
						else
							(Print 9 12) ; "You don't have the change."
						)
					)
				)
			)
...
(instance readPaperScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 9 13 #at 65 120) ; "Reading the front page, you learn that the cold war between the USSR and the United States is beginning to heat up."
				(Print 9 14 #at 65 120) ; "The Russian supported radical terrorist group led by Jawharah Khommini Sieb Abdual, has taken the United States Ambassador to the Middle East as hostage."
			...
			(3
				(Print 9 15 #at 65 120) ; "You further learn the terrorist group is demanding ten million dollars ransom in order to purchase arms."
				(Print 9 16 #at 65 120) ; "If the demand is not met within 30 days, the ambassador will be shot through the head and his body dumped in front of the US Embassy."
				(Print 9 17 #at 65 120) ; "In response to the Soviet naval build up in the Mediterranian, the United States has ordered its seventh fleet into the area."
				(Print 9 18 #at 30 120 #width 260) ; "The President has vowed that if the ambassador is not released, unharmed, within a reasonable amount of time, the United States will not hesitate to take an aggressive military stance against the cowardly perpetrators."
				(gEgo setCycle: End self)
			)
			(4
				(gEgo
			...
			(5
				(Print 9 19 #at 65 120) ; "After reading the paper and discarding it, you think to yourself, "This is really something. After just a short time on military leave, the whole world goes to pot.""
				(gGame changeScore: 2)
				(gEgo setScript: 0)
				(HandsOn)
			)
		...
		(cond
			...
			((Said '[/planter,flower,box]>')
				(cond
					((TurnIfSaid self event 'look[<at]'))
					((Said 'look[<at]')
						(Print 9 20) ; "You see a planter box full of flowers."
					)
				)
			)
		)
				...
				(cond
					...
					((Said 'look[<at,in]')
						(if (> (gEgo distanceTo: self) 20)
							(Print 9 21) ; "You see the glare of the hotel lobby windows."
						else
							(Print 9 22) ; "You see the hotel lobby inside."
						)
					)
				)
			)
