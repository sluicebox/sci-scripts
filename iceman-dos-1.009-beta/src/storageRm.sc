...
(instance storageRm of Rm
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(else
				(gEgo posn: 245 102 loop: 1 heading: 90 init:)
			)
		)
	)
		...
		(cond
			...
			((Said 'look>')
				(cond
					...
					((Said 'box,provision')
						(Print 35 1) ; "The crates are full of anything from soup to nuts."
					)
					((Said 'soup,nut,food')
						(if local0
							(Print 35 2) ; "You don't need anything from the crates!"
						else
							(Print 35 3) ; "That's best left to the supply people who are responsible for keeping things stocked."
							(= local0 1)
						)
					)
				)
...
(instance fryToDeathScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 35 4) ; "As you emerge from the reactor room, you begin to feel a little funny."
			...
			(6
				(Print 35 5) ; "You realize that entering the reactor area without protective clothing was a fatal mistake."
				(Print 35 6) ; "The searing radiation has already begun to turn your cells to jelly."
				(gEgo
					setLoop: 1
					setCel: 0
					cycleSpeed: 2
			...
			(9
				(EgoDead 337 3 0 35 7) ; "Next time pay attention to radiation warnings."
			)
		)
	)
)
		...
		(cond
			...
			((Said '[/juice]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 35 15) ; "Frozen grape juice."
					)
					((Said 'get')
						(proc0_39) ; "You don't need to do that."
					)
		...
		(cond
			...
			((Said '[/door,freezer]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 35 18) ; "This is the refrigerator where perishables are kept."
					)
					((GoToIfSaid self event 94 106 0 35 19))
					((Said 'open')
						(if (not cel)
		...
		(cond
			...
			((Said '[/door,freezer]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 35 20) ; "This is where frozen foods are kept."
					)
					((GoToIfSaid self event 221 106 0 35 19))
					((Said 'open')
						(if (not cel)
							(self setCycle: End self)
						else
...
(instance scubaDoorLeft of Prop
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said '[/door,closet,compartment]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(cond
							((not cel)
								(Print 35 21) ; "A compartment."
							)
							((== (subMarine suitRoom:) 38)
								(Print 35 22) ; "The compartment is empty."
							)
							(else
								(Print 35 23) ; "Inside the compartment you see scuba gear."
							)
						)
					)
					((GoToIfSaid self event 52 106 0 35 19))
					((Said 'open,unlock')
						(cond
							(cel
								(proc0_31) ; "It is."
							...
							((gEgo has: 5) ; Tahiti: Key | Sub: Storage_Compartment_Key | Tunisia: Map
								(Print 35 24) ; "Using the key, you unlock the compartment door."
								(self setCycle: End self)
							...
							(else
								(Print 35 25) ; "This compartment is locked. The key is obtainable from the Machinists mate."
							)
						)
					)
					((Said 'close')
			...
			((Said 'wear,get[/scuba,equipment,gear][/gear,equipment]')
				(cond
					((not cel)
						(Print 35 26) ; "The door is closed."
					)
					((== (subMarine suitRoom:) 38)
						(Print 35 27) ; "The scuba gear has been moved to the escape hatch."
					)
					(else
						(Print 35 28) ; "No use lugging that heavy gear all over the sub. One of the crewmen can help you when you're ready to disembark."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/door,reactor]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 35 29) ; "This is the door to the reactor room. A radiation symbol is emblazoned on it."
					)
					((GoToIfSaid self event 28 107 0 35 19))
					((Said 'open')
						(if (not cel)
							(self setCycle: End self)
						else
...
(instance willieScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 35 30) ; ""Let me take the scuba equipment to the escape hatch for you, sir." says Petty Officer Johnson."
		...
		(cond
			...
			((Said '[/sign,symbol]>')
				(cond
					...
					((Said 'read,look[<at]')
						(Print 35 31) ; "The universal radiation symbol."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/thermometer,meter,gauge]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 35 32) ; "The thermometers for the refrigerator and freezer."
					)
				)
			)
		)
