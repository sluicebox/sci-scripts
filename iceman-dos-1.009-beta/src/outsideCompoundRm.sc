	...
	(method (init)
		...
		(switch gPrevRoomNum
			(east
				...
				(guardRight view: 787 loop: 2 cel: 0 posn: 223 78 init:)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at,around][/room,town,scene]')
				(Print 81 0) ; "The is the only entrance to the compound where the ambassador is being held."
			)
			((Said 'load/gun')
...
(instance offLimitsScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(Print 81 3) ; "As you near the compound, one of the guards begins walking toward you."
				(guardRight
					setCycle: Walk
					setMotion: MoveTo (- (gEgo x:) 42) (gEgo y:) self
				)
			)
			(2
				(guardRight
					setMotion: MoveTo (+ (guardRight x:) 2) (guardRight y:) self
				)
			)
			(3
		...
		(cond
			...
			(
				(or
					...
					(Said 'open/door[<truck]')
					(Said 'get[<out][/truck]')
				)
				(self cue:)
			)
			((Said 'get/food,platter')
				(cond
			...
			((Said 'hide/gun')
				(cond
					((not (gEgo has: 8)) ; Sub: Device | Tunisia: Tranquilizer_Gun
						(Print 81 8) ; "You don't have the weapon."
					)
					((== local0 0)
						(Print 81 9) ; "You hide the weapon in the food."
						(= local0 1)
						(gGame changeScore: 2)
					)
					(else
						(Print 81 10) ; "You already hid the weapon."
					)
				)
			)
			((Said 'look[<at]/compound')
				(Print 81 11) ; "Looking through the vans window, you see two guards standing by the door."
			)
			((Said 'talk/guard')
				(Print 81 12) ; "You can't do that from the van."
			)
			((Said 'shoot,fire[/guard,gun]')
				(Print 81 13) ; "You don't have a clear shot from within the van."
			)
		)
	)
...
(instance orderedOutScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(guardRight setCycle: Walk setMotion: MoveTo 177 84 self)
			)
			(1
			...
			(3
				(Print 81 14) ; "The guard is uneasy about you sitting in the van so long.  He orders you out."
				(Print 81 15) ; "Nervously you open the door of the van and step out."
				(vanDoor setCycle: End self)
			)
			(4
				(vanDoor setLoop: 3 stopUpd:)
...
(instance outOfVanScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(Print 81 16) ; "Your behavior has made the guard suspicious. He begins walking toward you."
				(guardRight
					setCycle: Walk
					setMotion: MoveTo (+ (gEgo x:) 20) (gEgo y:) self
				)
		...
		(switch (= state newState)
			...
			(1
				(if (not (tunisia madeCall:))
					(Print 81 17) ; "The guard looks at you suspiciously and says, "You are not the regular caterer. Something is not right here. What is your name?""
				else
					(Print 81 18) ; "The guard speaks to you and says, "Your name!""
				)
				(Print 81 19) ; ""Shlinka Abdule." you say."
				(= cycles 1)
			)
			(2
				(guardRight
			...
			(5
				(cond
					((not (tunisia madeCall:))
						(Print 81 20) ; "Because the guard doesn't recognize you, he does a more thorough search than usual."
						(Print 81 21) ; "He grabs the platter out of your hand and opens it."
						(Print 81 22) ; "AH HA!" he exclaimes, "What does Abdule the caterer need with a weapon of this sort?"
						(Print 81 23) ; ""Get going PIG!" he says."
					)
					((== local0 0)
						(Print 81 24) ; "AH HA!" he exclaims, "What does Abdule the caterer need with a weapon of this sort?"
						(Print 81 23) ; ""Get going PIG!" he says."
					)
					(else
						(Print 81 25) ; ""Ok Abdule" He says, "Bring the food inside."
					)
				)
				(= cycles 1)
			)
			...
			(10
				(if (or (== local0 0) (not (tunisia madeCall:)))
					(EgoDead 970 0 0 81 26) ; "You have just joined the Ambassador as another hostage."
				else
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (< (gEgo y:) 98)
					(gEgo setMotion: MoveTo 194 104 self)
				else
					(= cycles 1)
				)
			)
			...
			(3
				(Print 81 27) ; "You take the platter out of the van."
				(gEgo view: 85)
				(gEgo get: 13) ; Sub: Vernier_Caliper | Tunisia: Food_Platter
				(= cycles 5)
			)
...
(instance goHideGunScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 81 28) ; "Outside the guards field of vision, you hide the weapon in the food."
				(= local0 1)
				(gGame changeScore: 2)
				(= cycles 5)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 81 29) ; "You move into position to fire."
			)
			(2
				(gEgo view: 687 loop: 0 cel: 0 setCycle: End self)
				(Print 81 30) ; "You quickly draw the weapon and shoot both guards with animal tranquilizer."
			)
			(3
			...
			(5
				(Print 81 31) ; "All of a sudden as both guards lay on the ground, the hair stands up on the back of your neck as you see..."
				(Print 81 32) ; "Another guard!"
				(guardNew
					view: 787
					loop: 2
					cel: 0
			...
			(7
				(Print 81 33) ; "Just as you turn to shoot the third guard...."
				(Print 81 34) ; "He beats you to the trigger and opens fire."
			...
			(10
				(EgoDead 970 0 1 81 35) ; "After being hit numerous times, you fall motionless to the ground."
			)
		)
	)
)
...
(instance shootOutScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 81 36) ; ""HALT!" the guard screams."
				(gEgo view: 281 loop: 1 setCycle: End)
				(ambass view: 281 loop: 0 setCycle: End)
				(= cycles 10)
			)
			(4
				(Print 81 37) ; ""I shall now kill you both!" the guard shouts."
				(stacy
					init:
					setStep: 8 2
					setCel: 0
			...
			(9
				(Print 81 38) ; "The agent reacts just in time, saving the lives of both you and the Ambassador."
				(Print 81 39) ; "STACY!" you shout, "WHAT DID I EVER DO WITHOUT YOU!"
		...
		(switch (= state newState)
			...
			(6
				(if (== client offLimitsScript)
					(Print 81 40) ; "After questioning, you are unable to explain your presence in this area."
				else
					(Print 81 41) ; "After questioning, you are unable to explain your behavior."
				)
				(EgoDead 970 0 0 81 42) ; "The guard places you under arrest and takes you away."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/guard,man]>')
				(cond
					((Said 'talk')
						(Print 81 43) ; "That's probably not a good idea. Try not to draw attention to yourself."
					)
					((Said 'look[<at]')
						(Print 81 44) ; "The guard looks a little sleepy. But, one false move on your part will probably bring a quick response."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/truck]>')
				(cond
					((Said 'look[<at]')
						(Print 81 45) ; "The van belonging to Bagdad's Fast Food catering."
					)
					((Said 'enter')
						(if (== local3 0)
							(Print 81 46) ; "You're already in the van."
						else
							(Print 81 47) ; "You've committed yourself, there's no backing out now."
						)
					)
				)
			)
		...
		(cond
			...
			((Said '[/building]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 81 48) ; "A typical example of Tunisian architecture."
					)
				)
			)
		)
