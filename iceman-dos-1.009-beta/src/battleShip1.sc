...
(instance battleShip1 of Script
					...
					(switch (event message:)
						(JOY_NULL
							(if (event modifiers:)
								(if (/ local45 300)
									...
									(proc396_0 5 339 3) ; "THE DESTROYER IS TOO CLOSE FOR A MISSLE"
								)
							else
								(++ local50)
								(proc396_0 5 339 2) ; "HARPOON MISSLE LAUNCHED SIR"
						...
						(JOY_UPRIGHT
							(if (event modifiers:)
								...
								(proc396_0 5 339 6) ; "SIR MISSLE NOT LOCKED ON TARGET AND IT'S COMMING AT US!!"
								(proc331_0 300 0)
							)
						)
					)
	...
	(method (changeState newState &tmp temp0 temp1 [temp2 15])
		(switch (= state newState)
			...
			(2
				(if local50
					(proc396_0 4 339 7) ; "THE HARPOON GOING OFF COURSE SIR"
					(-- local50)
				)
				(if (< 2 local46)
					(= start 3)
				...
				(while (< temp0 6)
					(if (< [local0 temp0] 0)
						(if
							...
							(self setScript: (ScriptID 329 0)) ; deadByTorpedo
						else
							(= [local0 temp0] 0)
