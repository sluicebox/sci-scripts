...
(instance pentagonLobby of Rm
	...
	(method (init)
		...
		(if (== gPrevRoomNum 20) ; pentagonHall
			(gEgo init: z: 1000 setScript: leavingPentagon)
			(= local0 1)
		else
		...
		(switch (= state newState)
			...
			(1
				(switch (Random 0 2)
					...
					(2
						(Print 19 7) ; ""Sir, you can not use the elevators without checking with me!"    the guard says."
					)
				)
		...
		(cond
			...
			((Said '[/guard,man]>')
				(cond
					...
					((GoToIfSaid self event 156 128 0 19 10))
					((Said 'talk')
						(cond
							...
							(local0
								(Print 19 11) ; "The guard looks at you and says, "My duties prohibit unnecessary conversation.""
							)
							((Random 0 1)
								(Print 19 12) ; "Sir!" the guard says, "I must see proper identification before you may pass."
							)
							(else
								(Print 19 13) ; ""Your ID please." The guard says."
							)
						)
					)
				)
			...
			((Said '/room<briefing')
				(if local0
					(Print 19 14) ; "The guard says.. "Just take the elevator, sir""
				else
					(Print 19 15) ; ""Your ID please," The guard says."
				)
			)
			((Said '/id,card>')
				(cond
					...
					((Said 'show')
						(cond
							((not (gEgo has: 2)) ; ID_Card
								(EgoDead 918 0 0 19 16) ; "Your ID... don't leave home without it!"
							)
							(local0
								(Print 19 17) ; ""I have already seen your ID sir." the guard replies"
							)
							((gEgo has: 2) ; ID_Card
								(Print 19 18) ; "You hand your identification to the guard."
								(Print 19 19) ; "The guard looks at your ID, then returns it and says, "Thank you very much commander, you've been expected. Please pass through.""
								(= local0 1)
								(gGame changeScore: 1)
								(gEgo setScript: 0)
							)
		...
		(cond
			...
			((Said '[/door,elevator]>')
				(cond
					...
					((Said 'look')
						(Print 19 20) ; "A government approved elevator door."
					)
					((Said 'open')
						(Print 19 21) ; "Press the button to operate the elevator."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/button]>')
				(cond
					((TurnIfSaid self event 'look[<at]'))
					((Said 'look[<at]')
						(Print 19 22) ; "A government approved elevator button."
					)
					((GoToIfSaid self event self 20 0 19 10))
					((Said 'push,press')
						(cond
							((== gPrevRoomNum 20) ; pentagonHall
								(Print 19 23) ; "Nothing happens as you push the button. You suspect that the guard has control over the elevators."
								(Print 19 24) ; "The guard says, "Sorry sir, my orders state that you are only allowed access to the elevators for your briefing.""
							)
							((not local0)
								(Print 19 25) ; "You push the elevator button."
							)
							(else
								(if local1
									(= local1 0)
...
(instance leavingPentagon of Script
	(properties)
