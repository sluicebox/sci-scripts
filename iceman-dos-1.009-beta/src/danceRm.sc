...
(instance barPerson1PV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/!*]')
				(Print 199 1) ; "A woman sitting at the bar blows a smoke ring."
			)
		)
	)
)
...
(instance barPerson2PV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/!*]')
				(Print 199 2) ; "A local having a drink."
			)
		)
	)
)
...
(instance barPerson3PV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/!*]')
				(Print 199 3) ; "A man sitting at the bar."
			)
		)
	)
)
...
(instance barPerson4PV of RPicView
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/!*]')
				(if (== cel 2)
					(Print 199 4) ; "A young lady with a flirtatious smile, winks at you."
				else
					(Print 199 5) ; "An empty bar stool."
				)
			)
		)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/keyboard]')
				(Print 199 6) ; "This electric piano is one hot keyboard."
			)
			((Said 'look[<at]/band')
				(Print 199 7) ; "The hottest little band going, "Reggie-Bob and the Gnats," performing for your dancing pleasure."
			)
			((Said 'talk/band')
				(switch (Random 0 3)
					(0
						(Print 199 8) ; "They're too busy picking and a grinning."
					)
					(1
						(Print 199 9) ; "They don't talk when they're hammin and jammin."
					)
					(2
						(Print 199 10) ; "Come on now, not in the middle of this good song."
					)
					(3
						(Print 199 11) ; "These dudes are gettin down! They're not going to stop and crank up a conversation."
					)
				)
			)
		)
...
(instance keyboardistPV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 199 12) ; "This is Reggie Bob on the keyboards."
			)
			((Said 'talk[/man]')
				(Print 199 13) ; "Reggie is so into it his fingers do his talking."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/man,player<guitar]')
				(Print 199 14) ; "This is Mark, playing his famous RD-Artist."
			)
			((Said 'talk[/man]')
				(Print 199 15) ; "The stuff he's puttin out right now doesn't allow him any time to talk."
			)
		)
	)
)
...
(instance manInChairPV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 199 16) ; "This is Chris Hoyt, programmer of KQ4 and now The Colonels Bequest."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/woman[<blond]]')
				(Print 199 17) ; "This girl is with a date."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/flower,centerpiece]')
				(Print 199 18) ; "There are flowers on the table."
			)
		)
	)
)
...
(instance tablePV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/table]')
				(Print 199 19) ; "This is the table."
			)
		)
	)
)
...
(instance guitarist2PV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 199 20) ; "This is Paul on guitar."
			)
		)
	)
)
...
(instance husbandPV of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 199 21) ; "This is a local having a drink with his wife."
			)
		)
	)
)
...
(instance topTablePV of RPicView
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/table]')
				(if (husbandPV cel:)
					(Print 199 22) ; "A big Samoan is at this table with his wife."
				else
					(Print 199 23) ; "This is the table with the blonde."
				)
			)
		)
	)
