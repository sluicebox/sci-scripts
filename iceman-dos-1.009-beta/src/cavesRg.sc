	...
	(method (handleEvent event)
		(cond
			...
			((and (gEgo has: 6) (Said 'press/lever,switch<left')) ; Tahiti: Earring | Sub: Diver | Tunisia: Key
				(if local87
					(Print 304 0) ; "You turn the light off."
					(= local87 0)
				else
					(Print 304 1) ; "You find the switch for the headlight and turn it on."
					(= local87 1)
				)
			)
			((Said 'light/flare')
				(if (and (not local88) (gEgo has: 7)) ; Sub: Flare | Tunisia: Duct_Tape
					...
					(Print 304 2) ; "You don't have one."
				)
			)
			((and (gEgo has: 6) (Said 'press/lever,switch<right')) ; Tahiti: Earring | Sub: Diver | Tunisia: Key
				(Print 304 3) ; "To activate the propeller, use the directional keys."
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(3
				(if (< (Random 0 19) 16)
					...
					(Print 304 4) ; "The flare is a dud!"
					(self changeState: 7)
				)
			)
			(4
			...
			(7
				...
				(gEgo setLoop: 1 setCel: 2 setCycle: Beg self)
...
(instance dyingEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(EgoDead 951 0 0 304 5) ; "Your air is gone and you never were any good at holding your breath."
			)
		)
	)
)
