	...
	(if (and (<= param1 ((subMarine script:) state:)) (not [local0 param2]))
		...
		(switch (Random 0 3)
			(0
				...
				(Print 385 6) ; "The message is encrypted using the CIA alternates."
			)
			(1
				(Print 385 7) ; "Also," he continues, "I'm holding a second coded message from CIA command headquarters."
				(Print 385 8) ; "Don't forget to use the CIA decoding offsets."
			)
			(2
				(Print 385 9) ; "The radioman hands you the message from CIA Command."
			)
			(3
				(Print 385 10) ; ""There's a message from the CIA Command, Sir."  the radioman replies."
			)
		)
		(Print 385 11) ; "The CIA code keys are:"
