...
(instance dontGo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				((ScriptID 2 0) init: 5 1 self) ; Delphineus, "Please don't turn back now, Adam. Eluria needs you."
			)
			(1
...
(instance delScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(HandsOff)
				((ScriptID 2 0) init: 4 1 self) ; Delphineus, "I'm counting on you to find our way through the weeds, Adam!"
			)
			(5
...
(instance firstKelpTalk of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				((ScriptID 2 0) init: 1 1 self) ; Delphineus, "Oh, my head! I feel kinda funny. I mean, funny peculiar."
			)
			(1
				((ScriptID 2 1) init: 1 1 self) ; Adam, "Maybe we ought to go back. You could still be hurt!"
			)
			(2
				(= seconds 3)
			)
			(3
				((ScriptID 2 0) init: 2 1 self) ; Delphineus, "No, it's not that. I think there was something in that garbage we swam through. It's interfered with my sonar - you know, my direction-finding senses."
			)
			(4
				(= seconds 3)
			)
			(5
				((ScriptID 2 1) init: 2 1 self) ; Adam, "What do you want to do? That's a lot of tangled seaweed up ahead."
			)
			(6
				((ScriptID 2 0) init: 3 1 self) ; Delphineus, "We have to go through. Cetus used to clear a path for us, but it's gotten overgrown. There's one way through. I can follow if you lead."
			)
			(7
				(= seconds 1)
			)
			(8
				((ScriptID 2 1) init: 3 1 self) ; Adam, "Okay, I'll try!"
			)
			(9
				(self dispose:)
			)
