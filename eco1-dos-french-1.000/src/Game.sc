...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] str [buf1 40] [buf2 10] [buf3 5])
	...
	(if
		...
		(proc932_8 98 0 63 @buf1)
		(proc932_8 98 0 64 @buf2)
		(proc932_8 98 0 65 @buf3)
		(Format str @buf1 (if saveDisk @buf2 else @buf3) @saveDevice)
		(Load rsFONT gUserFont)
		(DeviceInfo 4) ; CloseDevice
		(proc932_8 98 0 48 @buf1)
		(proc932_8 98 0 49 @buf2)
		(proc932_8 98 0 53 @buf3)
		(if
			(==
				(= ret
					(if saveDisk
...
(class Game of Obj
	...
	(method (save &tmp [comment 20] num oldCur buf1 [buf2 100] [str 5])
		(= buf1 parseLang)
		(= parseLang 1)
		...
		(if (PromptForDiskChange 1)
			...
			(= oldCur (self setCursor: gNormalCursor 1))
			(= num (Save doit: @comment))
			(self setCursor: oldCur 1)
			(if (!= num -1)
				(= parseLang buf1)
				(= oldCur (self setCursor: gWaitCursor 1))
				(if (not (SaveGame name num @comment gVersion))
					(proc932_8 98 0 60 @buf2)
					(proc932_8 98 0 48 @str)
					(Print @buf2 #font 0 #button @str 1)
				)
				(self setCursor: oldCur 1)
			)
	...
	(method (restore &tmp [comment 20] num oldCur buf1 [buf2 80] [str 5])
		(= buf1 parseLang)
		(= parseLang 1)
		(Load rsFONT gSmallFont)
		(ScriptID 990)
