...
(instance keyPadView of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator posn: -1 140 init: 92 0 400) ; "Oops. Try clicking on the keys."
			)
			(else
		...
		(if (User controls:)
			...
			(cond
				...
				(
					...
					(event claimed: 1)
					(Narrator posn: -1 140 init: lookStr)
				)
				(
					(and
						(not (event modifiers:))
		...
		(switch (= state newState)
			...
			(4
				(if (== local11 9731)
					...
					(Narrator posn: -1 140 init: 85 self 400) ; "That doesn't seem to be the combination."
				)
			)
			(5
				(SetFlag 14)
