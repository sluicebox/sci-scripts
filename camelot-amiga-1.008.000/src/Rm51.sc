		...
		(cond
			...
			((Said 'give/moneybag')
				(if (< (gEgo distanceTo: mercenary) 40)
					(if (or global122 global121 global120)
						(PutItem 3) ; purse
						(Talk 51 19) ; "I magnanimously accept your donation. Now move along."
						(mercenary setScript: standAside)
					else
						(Talk 51 20) ; "This is empty, dog! Give me money or be gone!"
					)
				else
					(NotClose) ; "Perhaps you should move closer."
				)
			...
			((Said 'give,pay[/man,thug,coin][/man,thug,coin]')
				(cond
					((not (DoPurse))
						(if (IsFlag 79)
							(Talk 51 21) ; "Do you mock us, infidel? We are not amused."
						else
							(Talk 51 22) ; "You had better pay in coin, infidel, or you will pay in blood."
						)
						(return)
					)
					((== (gEgo onControl: 1) 4096)
					...
					(else
						(Talk 51 23) ; "If you want to pass, you will come here and pay me."
						(proc0_18)
						(return)
					)
				)
			...
			((Said 'talk[/man,thug]')
				(if (and (< gPrevRoomNum 52) (> (gEgo x:) 170))
					(if (IsFlag 79)
						(Talk 51 24) ; "I do not waste speech with heathens. Move along, unless you wish to give us more money, heh heh."
					else
						(Talk 51 25 51 26) ; "The Zion Gate is closed. You cannot enter here. You must go to the Jaffa Gate."
					)
				else
					(Talk 51 27) ; "I do not waste speech with infidels. I would as soon talk to a cur in the street."
				)
			)
		)
	)
...
(instance Rm51 of eRoom
		...
		(if (and (not (self goingOut:)) (not (self comingIn:)))
			(cond
				...
				(
					...
					(if (< local5 2)
						(++ local5)
						(Talk 51 28) ; "Be on your way, infidel, before we tire of your ugly face."
					else
						(= local7 1)
						(Talk 51 4) ; "Now we shall see how well an infidel dies."
				...
				(
					...
					(if (== local5 2)
						(Talk 51 29) ; "You were warned, infidel!"
						(mercenary setScript: pauseBeforeKill)
					else
						(++ local5)
			...
			(cond
				((and (IsFlag 106) (not local1))
					(= local1 1)
					(Talk 51 30) ; "By Allah! Look, men, our infidel returns as bedraggled as an old sow that has been rolling in dust and muck."
				)
				...
				((and (< (gEgo y:) 186) (IsFlag 80))
					(ClearFlag 80)
					(Talk 51 32) ; "Hah! They must raise some clumsy fools where you come from."
				)
				((and (not local1) (< (gEgo y:) 183) (!= gPrevRoomNum 52))
					(= local1 1)
					(gEgo setMotion: 0)
					(if global124
						(Talk 51 33) ; "Sheathe your sword, infidel, or we will chop you to pieces."
					else
						(Talk 51 34) ; "Be warned, infidel! Keep your sword sheathed or we will chop you to pieces."
					)
					(= local7 1)
					(mercenary setScript: gangUp)
				)
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'open/door,gate')
				(Talk 51 35) ; "The Zion Gate is closed. Move along, infidel, and I mean NOW!"
			)
			(
				(or
					(event claimed:)
...
(instance payTheMan of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gEgo view: 0 setLoop: -1 ignoreActors: 0 cycler: local8)
				(gTObj talkCue: self)
				(localproc_0)
			)
			(6
