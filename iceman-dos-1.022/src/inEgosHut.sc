				...
				(cond
					...
					((not (StrCmp @local0 {555-6969}))
						(if
							...
							(Print 6 14) ; "The voice on the other end intones, "No Sinky Dinghy Transport, can I help you?""
							(User canInput: 1)
							(= register 1)
							(= seconds 10)
						else
		...
		(cond
			...
			(else
				(if
					...
					(Print 6 22) ; "After the mutual "hellos" and "how-have-you-been" greetings, the General says:"
					(if ((tahiti script:) state:)
						(Print 6 23) ; "Where on earth have you been, Johnny?"
						(Print 6 24) ; "We've got an explosive situation on our hands and you've been playing around on that island."
					else
		...
		(cond
			...
			((Said '[/dresser,drawer,dresser,lamp,light,flower]>')
				(cond
						...
						(if (not cel)
							...
							(cond
								(
									...
									(Print 6 31) ; "You see your ID card and some change."
								)
								((proc316_1 gCurRoomNum 2)
									(Print 6 32) ; "You see your ID card."
								)
								((proc316_1 gCurRoomNum 4)
									(Print 6 33) ; "You see your change."
								)
								(else
									(Print 6 34) ; "You see an empty drawer."
								)
							)
						)
					)
					...
					((Said 'close')
						(if (not cel)
							(Print 6 36) ; "It's already closed."
						else
							(proc0_30)
							(drawerView hide:)
					...
					((Said 'examine')
						(Print 6 37) ; "It's made of wood and has one drawer."
					)
				)
			)
			((Said 'adjust,return,drop[/id,card,change,money]>')
				(cond
					((TurnIfSaid self event 'adjust,return,drop/*'))
					((GoToIfSaid self event (+ 20 x) y 0 6 7))
					((not cel)
						(Print 6 38) ; "You must first open the drawer."
						(event claimed: 1)
						(return)
					)
					((Said '/change,money')
						(if (gEgo has: 4) ; Tahiti: Change | Sub: Rum_Bottle | Tunisia: Capsule
							(Change show:)
							(Print 6 39) ; "You return the change to the drawer."
					...
					((gEgo has: 2) ; ID_Card
						(Wallet show:)
						(Print 6 40) ; "You return the ID card to the drawer."
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(Print 6 41) ; "The drawer is empty."
				(event claimed: 1)
			)
		)
	)
		...
		(cond
			...
			((Said '[/closet,(door<closet)]>')
				(cond
					...
					((Said 'look[<at,in]')
						(if (localproc_0)
							(Print 6 42) ; "You see your clothes in the closet."
						else
							(Print 6 43) ; "This is where you keep your clothes."
						)
					)
			...
			((Said 'look[<at]/coat,coat,coat,clothes,pants')
				(if (not (localproc_0))
					(Print 6 44) ; "You can't see the inside of the closet too well from this distance."
				else
					(event claimed: 0)
				)
			)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(Print 6 45) ; "It's much too hot here for a suit."
			)
			...
			((Said 'examine,look[<at,in]>')
				(cond
					((Said '/coat,coat,coat,clothes')
						(Print 6 46) ; "It's a nice dark suit with 2 front pockets."
					)
					((Said '/pocket[/coat,coat]')
						(if (proc316_1 gCurRoomNum 3)
							(Print 6 47) ; "There is a little black book in the pocket."
						else
							(Print 6 48) ; "The pocket is empty."
						)
					)
					((Said '[/closet]')
						(Print 6 49) ; "Looking in the closet, you see you see extra clothes."
					)
				)
			)
		...
		(cond
			...
			((Said 'look[<at][/id,card]')
				(if (and (proc316_1 gCurRoomNum 2) (& (tahiti flags:) $0020))
					(Print 6 50) ; "You see your ID card in the drawer."
				else
					(event claimed: 0)
				)
			)
			((or (Said '(get[<!*])/id,card') (MousedOn self event))
				(cond
					...
					((proc316_1 gEgo 2)
						(if (Said '(get[<!*])/id,card')
							(Print 6 51) ; "You already have that."
						else
							(Print 6 52) ; "There's nothing there!"
						)
					)
					((not (User canControl:))
						(Print 6 53) ; "Stand up first."
					)
...
(instance Change of Prop
		...
		(cond
			...
			((Said 'look[<at][/change,coin,money]')
				(if (and (proc316_1 gCurRoomNum 4) (& (tahiti flags:) $0020))
					(Print 6 54) ; "You see loose change in the drawer."
				else
					(event claimed: 0)
				)
			)
	...
	(method (cue)
		(super cue:)
		(self hide: stopUpd:)
		(Print 6 55 #time 10) ; "You pick up the loose change in the drawer."
		...
		(cond
			...
			((Said '[/painting,wall]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 6 56) ; "You see a picture of fish on the wall."
					)
					((Said 'move/*')
						(proc0_39) ; "You don't need to do that."
					)
			...
			((Said '[/floor,floor]>')
				(cond
					...
					((Said 'look[<at,down,through]')
						(if (not (Random 0 5))
							(Print 6 57) ; "A dolphin!"
						else
							(Print 6 58) ; "Through the glass floor you see the beautiful Tahitian sea life."
						)
					)
				)
			)
		...
		(cond
			...
			((Said '[/table,flower]>')
				(cond
					...
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 6 59) ; "A mahogany table with ginger flowers."
					)
					((GoToIfSaid self event x (+ 25 y) 0 6 7))
					((Said 'smell')
						(Print 6 60) ; "The ginger fragrance fills the air."
					)
				)
			)
		)
...
(instance egoSitScript of Script
			...
			(1
				...
				(switch register
					...
					(1
						(switch (Random 1 3)
							(1
								(Print 6 65) ; "Clean sheets."
							)
							(2
								(Print 6 66) ; "Borax."
							)
							(3
								(Print 6 67) ; "Lemon Fresh."
							)
						)
						(HandsOn)
						(self dispose:)
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'sit')
				(Print 6 68) ; "You already are sitting."
			)
			((Said 'stand')
				(self cue:)
			)
		...
		(cond
			...
			((Said '[/carpet,carpet]>')
				(cond
					...
					((Said 'look[<at,below]')
						(Print 6 69) ; "A cheap rug obviously made of synthetic material."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/shutter]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 6 70) ; "An incredible view of azure sky which compliments the aqua blue of the ocean waves can is visible through the window."
					)
				)
			)
		)
...
(instance closetDoorFeature of RFeature
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((or (Said 'wear,(change<in)/coat') (Said 'change/clothes'))
				(proc0_39) ; "You don't need to do that."
			)
			((Said '[/closet]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 6 72) ; "You see the unopened left side of your closet."
					)
				)
			)
		)
