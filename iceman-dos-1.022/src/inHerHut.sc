	...
	(method (handleEvent event &tmp temp0)
		(cond
			...
			((Said 'look[<around,at][/room][/!*]')
				(Print 14 1) ; "Looking around the room you think to yourself, "Yep! she's flown the coop, all right!""
				(Print 14 2) ; "WOW, what a girl," you think, "So beautiful, yet so mysterious!"
			)
			((Said 'look[<at]//earring')
				(Print 14 3) ; "Stacy has already searched her room, and further searching by you would be pointless."
			)
			((Said 'call<use')
				(Print 14 4) ; "The only phone you can use is the one in your room."
			)
			((Said 'kiss')
				(if (gCast contains: local0)
					(Print 14 5) ; "Why don't you go sit down with her first?"
				else
					(Print 14 6) ; "You kiss yourself a couple times but soon tire of it."
				)
			)
			((Said '(stand[<up]),(get<up)')
				(Print 14 7) ; "You're already standing."
			)
			((Said 'detach/clothes,shirt')
				(if (gCast contains: local0)
					(Print 14 8) ; "You're being a little forward, aren't you?"
				else
					(Print 14 9) ; "You don't need to."
				)
			)
		)
	)
		...
		(cond
			...
			((Said '[/plant][/floor,corner]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 14 10) ; "There is a beautiful tropical plant in the corner of the hut."
					)
					((Said 'move')
		...
		(cond
			...
			((Said '[/lamp][/floor,table]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 14 11) ; "A lamp on a mahogany end table by the bed."
					)
					((Said 'turn[<on,off]')
						(proc0_39) ; "You don't need to do that."
					)
		...
		(cond
			...
			((Said '[/table,desk,flower][/table,desk]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<on,at]')
						(Print 14 12) ; "A mahogany table with ginger flowers."
						(if (and (gCast contains: note) (not (note z:)))
							(Print 14 13) ; "A folded note rests atop the table."
						)
					)
					((Said 'look<below')
						(Print 14 14) ; "All these tables seem to have four legs."
					)
					((GoToIfSaid self event self 25 'smell' 14 15))
					((Said 'smell')
						(Print 14 16) ; "The ginger fragrance fills the air."
					)
				)
			)
			((Said '[/drawer]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 14 17) ; "A mahogany table with two drawers."
					)
					((Said 'open')
						(Print 14 18) ; "You don't need to open the drawer."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/carpet,carpet][/floor]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at,on]')
						(Print 14 19) ; "A beautiful hand-woven rug."
					)
					((GoToIfSaid self event self 5 0 14 15))
					((Said 'look[<below]')
						(Print 14 20) ; "Theres nothing of interest under there."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/carpet,(table[<coffee])][/floor]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<on,at]')
						(Print 14 21) ; "You see a glass top coffee table sitting on a beautiful hand-woven rug."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/couch][/floor]>')
				(cond
					...
					((Said 'look<below')
						(Print 14 23) ; "Are you checking the maid's work?"
					)
					((GoToIfSaid self event 220 115 'sit[<on]' 14 15))
					((Said 'sit[<on]')
		...
		(cond
			...
			((Said '[/bed][/floor]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 14 24) ; "Satin sheets on a King Size bed, it makes me warm just to think about it."
					)
					((GoToIfSaid self event self 30 'look[<below]' 14 15))
					((Said 'look[<below]')
						(Print 14 25) ; "Just a dusty wooden floor under the bed."
					)
					((Said 'make/')
						(Print 14 26) ; "The maids take care of that."
					)
					((Said 'look[<in]')
						(Print 14 27) ; "You see nothing."
					)
					((Said 'sit[/*]')
						(if (== (gEgo view:) 206)
...
(instance agentScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HandsOn)
				(Print 14 28 #at 70 150) ; "She looks at you and says, "Come over and have a seat.""
			)
			(4)
			(5
				(switch (Random 0 1)
					(0
						(Print 14 29 #at 70 150) ; "The heat of passion begins to overcome you."
					)
					(1
						(Print 14 30 #at 70 150) ; "Your heart palpitates as Stacy plants her hot lips on yours."
					)
				)
			)
			(6
				(Print 14 31 #at 70 150) ; "As you embrace, your eyes begin to roll back into your head!"
			)
			(7
				(switch (Random 4 5)
					(4
						(Print 14 32 #at 70 150) ; "You thought you had been kissed by the best until this!"
					)
					(5
						(Print 14 33 #at 70 150) ; "The romantic tenderness of this kiss is overwhelming you."
					)
				)
			)
			(8
				(Print 14 34 #at 70 150) ; "You think to yourself, "MY, MY, MY!""
			)
			(9
				(gEgo
					observeControl: -32768
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'talk')
				(if (== (gEgo view:) 206)
					(Print 14 35) ; "Sit next to me," she says, "and we'll talk."
				else
			...
			((Said 'love<me/you')
				(switch (Random 0 2)
					(0
						(Print 14 36) ; "You can't love me," she says, "we just met."
					)
					(1
						(Print 14 37) ; "She looks at you and says, "Don't expect me to believe that one!""
					)
					(2
						(Print 14 38) ; "Love is such a splendid thing," she says, " But I don't think this relationship has reached that level yet."
					)
				)
			)
			((Said 'sit[<on][/bed]')
				(if (== (gEgo view:) 106)
					(Print 14 39) ; "You'll have to stand up first."
				else
					(self cue:)
					(gEgo setScript: egoSitScript)
				)
			...
			((or (Said 'look<around[/room]') (Said 'look/room'))
				(switch (Random 0 2)
					(0
						(Print 14 40) ; "She notices your snooping and says, " A little nosy, don't you think?""
					)
					(1
						(Print 14 41) ; "Stacy mentions to you, "I don't think you'll find anything of interest.""
					)
					(2
						(Print 14 42) ; "Say, bud," she protests, "Are you in here to see me or to search my bungalow?"
					)
				)
			)
			(
		...
		(cond
			...
			((Said '[/door][/room]>')
				(cond
					...
					((GoToIfSaid self event 42 146 'open' 14 15))
					((Said 'open')
						(Print 14 58) ; "Ok."
					)
				)
		...
		(cond
			...
			((Said '/note,letter,message>')
				(cond
					((Said 'look[<at]')
						(if z
							(Print 14 59) ; "It's just ordinary paper."
						else
							(Print 14 60) ; "On the table you see a note."
						)
					)
					((Said 'read')
						(if z
							(Print 14 61) ; "You read a note apparently written by Stacy."
							(Print 14 62) ; "Dear John, Thanks for making my vacation so wonderful. You were sleeping so peacefully I just couldn't bring myself to wake you. I have urgent business to tend to and must leave. I wish you the best and hope we meet again soon.         Love,         Stacy"
						else
							(Print 14 63) ; "You'll need to get it first."
						)
					)
					((TurnIfSaid self event 'look/*'))
					((GoToIfSaid self event self 30 0 14 15))
					((Said 'get')
						(if z
							(proc0_35) ; "You already took that."
						else
							(Print 14 64) ; "You pick up the note."
							(gGame changeScore: 1)
		...
		(cond
			...
			((Said '[/closet]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 14 65) ; "A closet."
					)
					((Said 'open,examine')
						(Print 14 66) ; "No need to do that. You won't find a thing!"
					)
				)
			)
		)
