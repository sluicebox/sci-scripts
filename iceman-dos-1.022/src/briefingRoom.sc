	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at]/coffee,cup')
				(Print 21 32) ; "It's just an ordinary coffee cup."
			)
		)
	)
)
		...
		(cond
			...
			((Said '/[envelope,order,instructions]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 21 33) ; "This envelope has your orders temporarily assigning you to the USS Blackhawk."
					)
					((Said 'open,look[<in]')
						(Print 21 34) ; "These orders are not to be opened until you're aboard the plane to Hawaii."
					)
					((GoToIfSaid self event envelope 40 0 21 35))
					((Said 'get')
						(if (gEgo has: 0) ; Envelope
							(Print 21 36) ; "You already have that."
						else
							(Print 21 37) ; "You pick the envelope up off of the desk."
							(envelope hide:)
							(gEgo get: 0) ; Envelope
							(gGame changeScore: 1)
						)
					)
					((Said 'drop/envelope,order,instructions')
						(if (gEgo has: 0) ; Envelope
							(Print 21 38) ; "That would be a futile gesture."
						else
							(Print 21 39) ; "You don't have that."
						)
					)
				)
			)
		...
		(cond
			...
			((Said '[/table]>')
				(cond
					...
					((Said 'look[<on,at]')
						(if (gEgo has: 0) ; Envelope
							(Print 21 40) ; "An Ordinary table."
						else
							(Print 21 41) ; "There is a manila envelope on the table."
						)
					)
				)
			)
