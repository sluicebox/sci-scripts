...
(procedure (localproc_0) ; UNUSED
	(Format @global100 212 0) ; "Welcome to the Pawn Shop. Nothing is too hot for us to handle."
	(Format @global100 212 1) ; "Welcome to the Pawn Shop. Down on your luck? Save the story for someone who hasn't heard it."
	(Format @global100 212 2) ; "Welcome to the Pawn Shop. I may look mean on the outside, but I've got a heart of stone."
	(Format @global100 212 3) ; "Welcome to the Pawn Shop. Why crawl to anybody else?"
	(Format @global100 212 4) ; "Welcome to the Pawn Shop, where we pardon your beg!"
	(Format @global100 212 5) ; "Welcome to the Pawn Shop. Your first stop on the way down the corporate ladder!"
	(Format @global100 212 6) ; "Welcome to the Pawn Shop. Where 'pawn' is just another word for nothing left to lose!"
	(Format @global100 212 7) ; "Welcome to the Pawn Shop. Please don't beg, it scuffs the carpet."
	(Format @global100 212 8) ; "Thank You very much."
	(Format @global100 212 9) ; "If ya ever need cash, I'm here."
	(Format @global100 212 10) ; "Y'all com'back now, y'hear."
	(Format @global100 212 11) ; "Have a nice day."
	(Format @global100 212 12) ; "You know a good deal when you see one."
	(Format @global100 212 13) ; "Come and see us again please."
	(Format @global100 212 14) ; "What else can I do for ya today?"
	(Format @global100 212 15) ; "Can I interest you in something else?"
)
...
(procedure (localproc_3 &tmp [temp0 31] temp31 temp32)
	(= temp32 (global302 durables:))
	(= local3 0)
	(for ((= temp31 0)) (< temp31 300) ((++ temp31))
		(= [local4 temp31] 0)
	)
	(Format @local4 212 17)
	(if (temp32 size:)
...
(procedure (localproc_10 &tmp temp0 temp1 temp2 temp3 temp4 temp5)
...
(procedure (localproc_12 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
...
(procedure (localproc_13 &tmp temp0 temp1)
	(buyMessage erase:)
	(doneBuying erase:)
	((pawnShop keyMouseList:) release:)
...
(procedure (localproc_14 &tmp temp0)) ; UNUSED
...
(instance pawnShop of Dialog
	...
	(method (init param1 &tmp [temp0 2] temp2)
		(if (not global518)
			(Load rsTEXT 212)
			(proc0_17 2)
			(= keyMouseList dialogKeyMouse)
			(= prevDialog global502)
			(global59 color: 0 back: (if global535 82 else 3))
			(= global502 self)
			(= global440 7)
			(= global441 110)
			(= global442 78)
...
(instance buyButton of DIcon
	...
	(method (doit &tmp temp0)
		...
		(if (localproc_1)
			...
			(global413 init: 16)
			(Print ; "No Pawned Items For Sale."
				212
				22
				310
				global413
...
(instance pawn of ErasableDIcon
	...
	(method (doit &tmp temp0 temp1)
		...
		(if (>= (localproc_5) 6)
			(global413 init: 16)
			(Print ; "Sorry, I really do not have room to buy anything else."
				212
				23
				310
				global413
		...
		(if (= local2 (localproc_4))
			(if (== local2 -1)
				(global413 init: 16)
				(Print
					(Format @global100 212 24) ; "Sorry, I don't need another one of those."
					310
					global413
					global440
					global441
			...
			(if
				(or
					...
					(Print
						(Format @global100 212 25 local1 700 (local2 indexNum:)) ; "I'll give you $%d for your %s, take it or leave it."
						81
						{Take It}
						1
						81
...
(instance aRedeemableItem of CostDItem
	...
	(method (doFormat param1)
		(if price
			(Format param1 212 26 text price) ; "%s $%d"
		else
