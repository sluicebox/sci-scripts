...
(instance lowcost of Dialog
	...
	(method (init param1 &tmp temp0)
		(if (not global518)
			...
			(= global413 0)
			(background view: (if (== (global302 livesAt:) 0) 700 else 699))
			(self add: background exitButton)
			(if (== (global302 livesAt:) 0)
				(self add: relaxButton)
				(if (== global414 0)
					(self add: theSaying theChair theStool)
				)
			)
			(self
				window: global59
