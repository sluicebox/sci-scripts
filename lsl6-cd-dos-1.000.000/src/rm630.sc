...
(instance rm630 of LarryRoom
	...
	(method (init)
		...
		(if (== gPrevRoomNum 860)
			(= style 7)
			(theDoor init: cel: 0 init: approachVerbs: 4) ; Do
			(gEgo
				show:
				x: (sink approachX:)
				y: (sink approachY:)
			...
			(theDoor init: approachVerbs: 4) ; Do
			...
			(switch theVerb
				(4 ; Do
					(cond
						...
						((and (> local59 1) (not local60) (not (IsFlag 79)))
							...
							(= local60 1)
							(SetFlag 79)
							(gMessager say: 12 4 24 0 flush_zipperCue) ; "Oh, no! The water's rising!"
						)
						((IsFlag 79)
							(gGame handsOff:)
						...
						(else
							...
							(= local59 0)
							(sfx number: 633 loop: 1 play:)
							(gMessager say: 12 4 0 1 flush_zipperCue) ; "You flush the toilet. Good boy!"
						)
					)
				)
				(61 ; toiletcover
				...
				(6 ; Zipper
					(cond
						((IsFlag 79)
							(gGame handsOff:)
							(gMessager say: 12 6 24 0 flush_zipperCue) ; "Even another drop would run this mess over the top and onto the floor!"
						)
						(local56
...
(instance flush_zipperCue of cObj
	(properties
		name {flush&zipperCue}
	)
