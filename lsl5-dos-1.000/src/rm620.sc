...
(instance businessOne of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 36 67 -1 20) ; "Halloween Supplies is Baltimore's largest local supplier of Halloween goods and party supplies."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessTwo of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 37 67 -1 20) ; "Hello Industries is one of the world's leading producers of greeting cards."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessThree of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 38 67 -1 20) ; "Duck Youth is a wonderful public service organization that specializes in familiarizing inner-city youngsters with all varieties of barnyard and aquatic fowl."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
		...
		(return
			(switch theVerb
				(2 ; Look
					...
					(Points 3 120)
					(TPrint 620 39 67 -1 20) ; "That's it, Patti! "des Rever Records" is located in Room 900."
				)
				(3 ; Do
					(SetFlag 17) ; fSeenRBOffice
					(Points 3 120)
...
(instance businessFive of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 40 67 -1 20) ; "Ship Heads, Inc. provides the finest commodes available for ocean-going vessels."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSix of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 41 67 -1 20) ; "AeroDork is a very high-class airline. So high-class their offices are forbidden to the public."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSeven of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 42 67 -1 20) ; "Big Buttons, Inc. supplies buttons of all sizes for every occasion: political campaigns, birthday parties, fund raisers, software company identification, etc."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessEight of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 43 67 -1 20) ; "Hard Disk Services can handle all of your computer hardware needs--from correcting corrupted database files to undeleting sensitive information, to recovering formatted hard drives."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance fStairs of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 620 44) ; "Since the staircase is presently being renovated, you are unable to use it. Besides, you know how much you hate climbing stairs in heels!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fAshtray of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 620 45) ; "Carefully examining the ashtray for a disco pass, for a brief moment you think you're playing "Leisure Suit Larry 1: In the Land of the Lounge Lizards." Of course, you're not."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance breasts of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 620 46) ; "You would, but you have no ladder!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
