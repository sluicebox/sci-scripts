	...
	(method (init)
		...
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 0 0 319 0 319 123 272 130 247 111 240 111 240 128 83 128 77 112 63 112 52 121 59 128 59 138 51 145 32 152 6 146 4 186 54 186 61 177 89 177 112 165 145 165 139 177 173 177 172 165 207 165 217 177 249 178 256 187 317 187 314 162 283 136 318 131 319 189 0 189
					yourself:
				)
		)
...
(instance toilet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 850 2) ; "You reach over and give the handle a solid push."
				(flushSound play:)
			)
			(10 ; Zipper
				(TPrint 850 2) ; "You reach over and give the handle a solid push."
			...
			(4 ; Inventory
				(switch invItem
					(11 ; DayTrotter
						(TPrint 850 3) ; "Wouldn't flushing the photocopies be defeating the whole purpose of your mission?"
					)
					(else
						(TPrint 850 4) ; "Hey! You might need that!"
					)
				)
			)
			(else
...
(instance sink1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 850 5) ; "Made of hand-carved, gold-flecked marble with gold-plated faucets, you bet this sink alone is worth more than your grand piano!"
			)
			(3 ; Do
				(if (IsFlag 27) ; fPattiBlackface
					(TPrint 850 6) ; "What you really need is a shower."
				else
					(TPrint 850 7) ; "Your hands aren't dirty."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
	...
	(method (changeState newState &tmp i invItem)
		(switch (= state newState)
			(0
				(HandsOff)
				(Points 7)
				(TPrint 850 8) ; "Now, that's a good idea, Patti. Use Krapper's shower to wash that toner from your hands and face."
			...
			(4
				(TPrint 850 9) ; "As you turn what appears to be a gold-plated faucet, you feel the "shower" begin to move!"
			...
			(5
				...
				(TPrint 850 10) ; "Where will you end up on this naked adventure?"
				(TPrint 850 11) ; "And without your inventory, too!"
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 842)
					(Say gEgo 850 12) ; "Oh no," you think. "How will I ever get this stuff off me?"
					(return 1)
				else
					(TPrint 850 13) ; "You look yourself over. You like what you see. "Although I could stand to lose a few pounds," you think."
					(return 1)
				)
			)
			(else
