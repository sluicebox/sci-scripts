...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 6) ; "You can't stand up; you have nowhere else to go."
			)
			(1 ; Walk
				(TPrint 530 7) ; "Why not talk to that beautiful woman instead. Go on, Larry; strike up a conversation."
			)
			(else
...
(instance table1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 9) ; "You can't sit with a strange couple!"
			)
			(5 ; Talk
...
(instance theTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 15) ; "You know what you want to do, Larry. Your problem is figuring out how to do it!"
			)
			(4 ; Inventory
				(TPrint 530 16) ; "Perhaps you should sit down with her first."
			)
			(10 ; Zipper
				(TPrint 530 17) ; "A little premature, aren't you, Larry? Maybe first a little dinner, then a show, perhaps a little dancing...."
			)
			(5 ; Talk
...
(instance cart of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 27) ; "You can't move the cart. Then you'd have no table!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sDoMichelle of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(gEgo normalize: setMotion: PolyPath 260 143 self)
			)
			(2
				(gEgo view: 531 setLoop: 1 setCel: 0 setCycle: End self)
			)
			(3
...
(instance drape1 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 530 29) ; "Every booth has curtains to assure your total privacy while dining."
			)
			(3 ; Do
				(TPrint 530 30) ; "There's nothing you can do with the curtains."
			)
			(5 ; Talk
				(if talkedToTable1
					(switch (Random 1 3)
						(1
							(TPrint 530 31) ; "They don't want to talk to you, Larry!"
						)
						(2
							(TPrint 530 32) ; "Just leave them alone!"
						)
						(3
							(TPrint 530 33) ; "Don't you get the hint? They don't want to talk to you!"
						)
					)
				else
					(table1 doVerb: 5)
...
(instance drape3 of Prop
	...
	(method (cue)
		(super cue:)
		(TPrint 530 35) ; ""Why doesn't that come as a surprise?" the woman replies snootily."
		(self setCycle: End)
		(table3 dispose:)
		(extra3 dispose:)
		(= talkedToTable3 1)
