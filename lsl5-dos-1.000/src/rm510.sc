...
(instance rm510 of LLRoom
	...
	(method (init)
		...
		(maitreD init: approachVerbs: 5 3 4) ; Talk, Do, Inventory
	...
	(method (notify what)
		(switch what
			...
			(else
				(TPrint 510 5) ; "I'm sorry, but that is not a working number. This is a recording."
			)
		)
	)
		...
		(switch (= state newState)
			...
			(1
				(cond
					((== ((Inv at: 14) state:) 0) ; Membership_Tape
						(Say the_maitre_d_ 510 7 #dispose #caller self) ; "But, of course, My Sewer," says the maitre d' snootily. "It worked in Larry 3, didn't it?"
					)
					((== ((Inv at: 14) state:) 4) ; Membership_Tape
						(Say the_maitre_d_ 510 8 #dispose) ; ""I suggest you find transportation to another location, Mr. (heh, heh) Laffer!" he smirks."
						(HandsOn)
						(self dispose:)
					)
					(else
						(Say the_maitre_d_ 510 9 #dispose) ; "You have your tape, Mr. Laffer. Stop bothering me!"
						(HandsOn)
						(self dispose:)
					)
				)
			...
			(3
				...
				(if (> bribe 99)
					...
					(Points 3 106)
					(gEgo put: 12) ; Money
					(Say the_maitre_d_ 510 10 #dispose) ; "Please, follow me, sir," says the maitre d', walking to the punch tape reader. "Allow me to prepare you for membership in our exclusive club."
					(= okToMakeTape 1)
					((Inv at: 14) state: 1) ; Membership_Tape
					(gCurRoom setScript: sMD)
				else
					(Say the_maitre_d_ 510 11 #dispose) ; "Thank you very much, sir," says the maitre d' greedily. "Have a nice day!"
					(HandsOn)
					(self dispose:)
				)
			)
...
(instance sMD of Script
	...
	(method (changeState newState)
		(switch (= state newState)
				...
				(cond
					((== ((Inv at: 14) state:) 0) ; Membership_Tape
						(switch talkCounter
							...
							(5
								(Say the_maitre_d_ 510 28 #dispose) ; "No?"
							)
							(6
								(Say the_maitre_d_ 510 29 #dispose) ; "Know."
							)
							(7
								(Say the_maitre_d_ 510 30 #dispose) ; "Ho."
							)
							(8
								(Say the_maitre_d_ 510 31 #dispose) ; "Who?"
							)
							(9
								(Say the_maitre_d_ 510 32 #dispose #caller self) ; ""OK! STOP!! I'll get you a damned ticket!" he shouts."
								(= okToMakeTape 1)
								((Inv at: 14) state: 1) ; Membership_Tape
							)
							(else
								(Say the_maitre_d_ 510 33 #dispose) ; "I'm sorry, sir, but my limit is one bore per customer!"
							)
						)
					)
					((== ((Inv at: 14) owner:) 510) ; Membership_Tape
						(Say the_maitre_d_ 510 34 #dispose) ; "You fool! Eeet's hanging there in plain sight!"
					)
					((== ((Inv at: 14) state:) 4) ; Membership_Tape
						(Say the_maitre_d_ 510 8 #dispose) ; ""I suggest you find transportation to another location, Mr. (heh, heh) Laffer!" he smirks."
					)
					...
					((== ((Inv at: 14) state:) 1) ; Membership_Tape
						(Say the_maitre_d_ 510 35 #dispose) ; "Just find a place to sit down. You have an assigned table. Find it!"
					)
				)
				(if (not okToMakeTape)
					(HandsOn)
			...
			(25
				(Say the_maitre_d_ 510 37 #dispose #caller self) ; "There you are, Sir!" barks the maitre d'. "Enjoy your visit here at the lovely Hard Disk Cafe!"
			)
			(26
				(maitreD
					view: 515
...
(instance sDoubleTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say the_maitre_d_ 510 38 #dispose #caller self) ; "I'm sorry, sir," he replies, "but admittance to the exclusive `Herman Hollerith Room' is limited to only our very finest customers."
			)
			(1
				(Say the_maitre_d_ 510 39 #dispose) ; ""(...which you are not!)" he concludes under his breath."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance woman of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Say gEgo 510 44) ; "Get off that phone," you shout. "There are others here that want to make a call."
				(TPrint 510 46) ; "Bertha ignores you completely."
			)
			(5 ; Talk
				(Say gEgo 510 47) ; ""Hey, I'd like to make a call someday, myself," you say to the large woman."
				(TPrint 510 46) ; "Bertha ignores you completely."
			)
			(else
...
(instance musicBox of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 510 48) ; "Music boxes show an early use of the principle that made the punchcard possible. It gave precisely aligned pins meaning as musical pitches."
				(if (gEgo has: 14) ; Membership_Tape
					(TPrint 510 49 #at -1 185) ; "(How interesting: the music box's cylinder appears to be the same width as your membership tape.)"
				else
					(TPrint 510 50) ; "Its cylinder is exactly 35 millimeters wide."
				)
			)
			(3 ; Do
				(musicBoxFX play: self)
				(gTheMusic fade: 80 10 10 0)
				(TPrint 510 51 #at 100 10) ; "The music box is either very old and very tired, or some practical joker has altered the pin placement on its cylinder. It sounds terrible!"
			)
			(5 ; Talk
				(TPrint 510 52) ; "It's a music box, not an Edison cylinder recorder."
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(if (not (> ((Inv at: 14) state:) 1)) ; Membership_Tape
							...
							(TPrint 510 53) ; "You wonder if this will ruin your only means of admission to the Cafe!"
							(gCurRoom setScript: sMusicBox)
						else
							(TPrint 510 54) ; "If you punch any more holes in that piece of tape, there will be nothing left!"
						)
					)
					(else
						(super doVerb: theVerb invItem &rest)
...
(instance maitreD of Actor
	...
	(method (cue)
		(super cue:)
		(TPrint 510 55 #at -1 185) ; "(You wonder if his French accent could be fake?)"
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Say gEgo 510 56) ; "Hello," you say to the maitre d', "do you have a table for a guy like me?"
				(if (gEgo has: 14) ; Membership_Tape
					(Say the_maitre_d_ 510 57 #dispose) ; ""Just use the membership tape I created for you," he replies."
				else
					(Say the_maitre_d_ 510 58 #dispose) ; "Please keep your hands off me," says the maitre d'. "I'm for the use of members only!"
				)
			)
			(5 ; Talk
				(gCurRoom setScript: sMD)
			...
			(4 ; Inventory
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
					(Say the_maitre_d_ 510 59 #dispose) ; "Why don't you get your tape out of the machine?"
				else
					(switch invItem
						(12 ; Money
							(HandsOff)
						...
						(13 ; Credit_Cards
							(if (== ((Inv at: 14) state:) 1) ; Membership_Tape
								(Say gEgo 510 60) ; "You know, I'd really love to be rid of all these troublesome credit cards," you tell the maitre d'. "Would you be willing to trade them for a table inside?"
								(Say the_maitre_d_ 510 61 #dispose) ; "But, of course, Sir!" he replies, snapping to attention. "I always have a soft spot for a fine fellow like you!"
								(Points 5 108)
								...
								(Say the_maitre_d_ 510 62 #dispose) ; ""You might want to save these, Sir," he replies. "You may need them later."
							)
						)
						(6 ; Hard_Disk_Cafe_Napkin
							(Say gEgo 510 63) ; "Would you mind throwing this away for me," you say, "Now that I know how to get here I don't need it any more."
							(Say the_maitre_d_ 510 64 #dispose #caller self) ; ""Wee, wee, moisture!" he replies."
							(gEgo put: 6 gCurRoomNum) ; Hard_Disk_Cafe_Napkin
							(gTheIconBar disable: 6 curIcon: (gTheIconBar at: 0))
							(gGame setCursor: ((gTheIconBar curIcon:) cursor:))
						)
						(11 ; DayTrotter
							(Say gEgo 510 65) ; ""Would you like to see my appointment calendar for the next year?" you ask the maitre d'."
							(Say the_maitre_d_ 510 66 #dispose) ; ""Hardly," he sniffs."
						)
						(else
							(super doVerb: theVerb invItem &rest)
						)
...
(instance door of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 510 67) ; "These doors operate by means of a Membership Tape inserted and read by that machine to their left."
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 68) ; "Insert your tape into the machine just to the left of these doors."
					)
					(7 ; AeroDork_Gold_Card
						(TPrint 510 69) ; "You might try using that on the maitre d'."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance tapeReader of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 510 70) ; "An ancient mainframe computer's paper tape reader sits against the far wall near the door."
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
					(TPrint 510 71) ; "Your membership tape is still hanging from the side of the machine."
				)
			)
			(3 ; Do
				(if (== ((Inv at: 14) owner:) 510) ; Membership_Tape
			...
			(5 ; Talk
				(TPrint 510 72) ; "This paper tape reader grants access to the "Hard Disk Cafe" proper, which is located behind those closed doors. To enter, just insert your membership tape in the slot."
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
...
(instance diskDrive of Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= theVerb 2) ; Look
			(TPrint 510 73) ; "This disk drive is for display purposes only."
		else
...
(instance pet of Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= theVerb 2) ; Look
			(TPrint 510 74) ; "This computer is for display purposes only."
		else
			(super doVerb: theVerb invItem)
		)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 75) ; "This machine once used magnetic tape, not paper tape."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance pianoRoll of Feature
	...
	(method (doVerb)
		(TPrint 510 76) ; "The historic old piano roll is hermetically sealed in that glass case."
	)
)
...
(instance punchCardMachine of Feature
	...
	(method (doVerb)
		(TPrint 510 77) ; "A sign on the punch card reader requests that you keep your hands off."
	)
)
...
(instance cactus1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Say gEgo 510 78) ; "OUCH!"
			)
			(5 ; Talk
				(TPrint 510 79) ; "Who do you think you are, Snoopy's Uncle Spike?"
			)
			(4 ; Inventory
				(switch invItem
					(14 ; Membership_Tape
						(TPrint 510 80) ; "Punch holes in your membership tape? A good idea, but this is not the way to do it."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
