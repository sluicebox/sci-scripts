...
(instance door of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 0) ; "It's locked, and you're glad that you can't get in!"
			)
			(5 ; Talk
				(TPrint 445 1) ; "You knock on Dr. Phil Hopian's office door. There is no answer because the good Doctor is off playing golf."
			)
			(else
...
(instance machine of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 2) ; "You have not a clue as to what this machine may do!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 3) ; "You are afraid to touch any computer keyboard that doesn't have MIDI built in!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance contraption of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 4) ; "You'll never be able to duplicate the F.B.I.'s "secret formula" of eleven natural herbs and spices."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance books of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 7) ; "You don't have time to read a book now, Patti!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance desk of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 445 8) ; "You open the drawer and look inside. There's nothing in there but a few charred slips of paper."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance duckF of Feature
	...
	(method (doVerb theVerb invItem)
		(if (== theVerb 2) ; Look
			(TPrint 445 9) ; "Aw! The technician took his duck with him!"
		else
