...
(procedure (DisplayStats &tmp [str 50])
	(Display 720 3 dsCOORD 60 25 dsFONT gSmallFont dsCOLOR gColWhite dsBACKGROUND gColBlack) ; "Win:$       Bet:$       Credits:$"
	(Display 720 4 dsRESTOREPIXELS wUnders)
	(Format @str 720 5 winnings) ; "%d"
	(= wUnders
		(Display @str dsCOORD 80 25 dsFONT gSmallFont dsCOLOR gColWhite dsBACKGROUND gColBlack)
	)
	(Display 720 4 dsRESTOREPIXELS bUnders)
	(Format @str 720 6 theBet) ; "%d"
	(= bUnders
		(Display @str dsCOORD 153 25 dsFONT gSmallFont dsCOLOR gColWhite dsBACKGROUND gColBlack)
	)
	(Display 720 4 dsRESTOREPIXELS cUnders)
	(Format @str 720 6 credits) ; "%d"
	(= cUnders
		(Display @str dsCOORD 242 25 dsFONT gSmallFont dsCOLOR gColWhite dsBACKGROUND gColBlack)
	)
)
...
(instance rm720 of LLRoom
	...
	(method (doVerb theVerb &tmp [str 200])
		(switch theVerb
			(2 ; Look
				(Format @str 720 0 gPokerJackpot) ; "Video Poker Odds  Progressive Royal Flush  $%d  Straight Flush      50 to 1  4 of a Kind        20 to 1  Full House        10 to 1  Flush           5 to 1  Straight          4 to 1  3 of a Kind        3 to 1  2 Pair           2 to 1  Pair, Jacks or Better   1 to 1"
				(TPrint
					@str
					#title
			...
			(3 ; Do
				(if dealOK
					(if (< theBet 100)
						(cond
							(paid
								(TPrint 720 7) ; "It's too late now to change your bet!"
							)
							((< theBet credits)
								(buttonSound play:)
								(++ theBet)
							...
							(else
								(TPrint 720 8) ; "You can't bet more than your credits."
							)
						)
					else
						(TPrint 720 9) ; "The Tramp Casino poker machines are limited to bets of $100."
					)
				)
			)
			(else
	...
	(method (changeState newState &tmp i [str 50])
		(switch (= state newState)
			...
			(8
				...
				(TallyHand)
				(if (== theHand 9)
					(Format @str 720 14 gPokerJackpot) ; "P R O G R E S S I V E J A C K P O T  ! !  You win %d dollars!"
					(TPrint
						@str
						#width
						200
