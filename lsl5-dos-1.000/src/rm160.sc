...
(instance rm160 of LLRoom
	...
	(method (init)
		...
		(presDoor init: approachVerbs: 3) ; Do
		(if (not (IsFlag 7))
		...
		(gCurRoom
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: 0 73 79 73 99 76 98 92 66 108 54 113 12 113 11 119 60 119 48 131 22 135 2 148 2 187 185 187 190 181 216 181 220 187 314 187 319 157 319 189 0 189
					yourself:
				)
		)
		(if (not (IsFlag 7))
...
(instance coffeeMaker of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 160 23) ; "You'd love to brew a fresh pot, but there's no pot left to brew in!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance bigSign of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 24) ; "Yes, of course you think the sign is lovely, but you really have no use for a big, ugly sign."
				(= usedHand 1)
			)
			(5 ; Talk
				(TPrint 160 25) ; "If that thing talks back, you vow to leave the building immediately!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance portrait of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(TPrint 160 29) ; "Try talking to people instead."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poster of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(TPrint 160 32) ; "You really ARE lonely, aren't you, Larry?"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance pictures of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 33) ; "You really enjoy playing with your hands, eh, Larry?"
				(= usedHand 1)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance onePicture of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 34) ; "You have so much fun feeling things!"
				(= usedHand 1)
			)
			(5 ; Talk
				(TPrint 160 35) ; "You attempt to throw your voice into the picture. You fail."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance plant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 38) ; "Ouch! Those plastic leaves are sharp!"
				(= usedHand 1)
			)
			(5 ; Talk
				(TPrint 160 39) ; "Does talking to plastic plants help them grow?"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance smallPlant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 40) ; "Ouch! Pricked again!"
				(= usedHand 1)
			)
			(5 ; Talk
				(TPrint 160 39) ; "Does talking to plastic plants help them grow?"
...
(instance largePlant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 41) ; ""SPROING!" says the leaf."
				(= usedHand 1)
			)
			(5 ; Talk
				(TPrint 160 39) ; "Does talking to plastic plants help them grow?"
...
(instance bronzeAward of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 160 44) ; "Is this the best thing you've found to do with your hand cursor?"
				(= usedHand 1)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 45) ; "You're afraid to sit down here since your boss may come out of his office at any moment. Shouldn't you continue with your mission?"
				(= usedHand 1)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance desk of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 46) ; "Rubbing your hand across the desk makes you appreciate its fine vinyl woodgrain surface."
				(= usedHand 1)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance cabinetDoors of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 160 47) ; "You have no need for coffee or filters."
				(= usedHand 1)
			)
			(else
				(super doVerb: theVerb invItem &rest)
		...
		(switch theVerb
			(2 ; Look
				(if (== ((Inv at: 1) owner:) gCurRoom) ; Battery_Charger
					(TPrint 160 48) ; "The coffee pot is plugged into the upper electrical outlet, and your battery charger is "currently" occupying the lower outlet."
				else
					(TPrint 160 49) ; "The coffee pot is plugged into the upper electrical outlet, but the lower one is "currently" available."
				)
				(= usedEyeball 1)
			)
			(3 ; Do
			...
			(4 ; Inventory
				(switch invItem
					(1 ; Battery_Charger
						(TPrint 160 50) ; "Good idea! You can never tell when a guy might need his pocket protector fully charged!"
						(gCurRoom setScript: (ScriptID 22 0)) ; sPlugCharger
					)
					(0 ; Camcorder
						(TPrint 160 51) ; "The camcorder can't be plugged into an outlet. You must use its recharger base."
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance doVerb160Code of Code
	...
	(method (doit theVerb theObj &tmp objDesc theItem [str 100])
		...
		(switch theVerb
			...
			(5
				(if (theObj facingMe: gEgo)
					(Format @str 160 52 objDesc) ; "You engage in a short, but personally meaningful, conversation with %s."
					(TPrint @str)
				)
			)
			(3
				(Format @str 160 53 objDesc) ; "That feels exactly like %s."
				(TPrint @str)
				(= usedHand 1)
			)
			(4
				(= theItem ((gTheIconBar curInvIcon:) description:))
				(Format @str 160 54 theItem objDesc) ; "It seems %s just doesn't work with %s."
				(TPrint @str)
			)
			(10
				(Format @str 160 55 objDesc) ; "Don't do that to %s!"
				(TPrint @str)
			)
			(else
				(NoResponse theObj theVerb)
