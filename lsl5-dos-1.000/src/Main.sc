...
(procedure (TPrint arg1 arg2 &tmp [temp0 300] temp300 temp301 temp302)
	(if gModelessDialog
	...
	(if (u< arg1 1000)
		(GetFarText arg1 arg2 @temp0)
		(= temp300 (Max 3 (/ (* global155 (StrLen @temp0)) 120)))
		(Print @temp0 #time temp300 &rest)
	else
		(= temp300 (Max 3 (/ (* global155 (StrLen arg1)) 120)))
		(Print arg1 25 temp300 arg2 &rest)
	)
	(if (not (HaveMouse))
		(gGame setCursor: gTheCursor 1 temp301 temp302)
	)
	(return 1)
)
...
(procedure (NoResponse obj verb &tmp [str 40] [fileName 10] [vstr 10])
	...
	(Format @fileName 0 23 gCurRoomNum) ; "%d.msg"
	(Format ; ";%s (%s) [%d,%d]:"
		@str
		0
		24
...
(instance LSL5 of Game
		...
		(return
			(and
				...
				(switch (= theVerb ((User curEvent:) message:))
					(2
						(TPrint 0 1) ; "It's not much to look at."
					)
					(5
						(TPrint 0 2) ; "To whom were you trying to speak?"
					)
					(3
						(TPrint 0 3) ; "There's nothing to do there."
					)
					(10
						(cond
							((gCurRoom script:)
							...
							(gEgoIsLarry
								(TPrint 0 5) ; "You coyly turn your back and relieve yourself. You quickly feel much better. The hint of a smile crosses your lips."
							)
							(else
								(TPrint 0 6) ; "Visions of cucumbers dance through your head."
							)
						)
					)
					(4
						...
						(if gEgoIsLarry
							(switch theItem
								...
								(7
									(TPrint 0 10) ; "That's one more place that won't accept your AeroDork Gold Card!"
								)
								(10
									(TPrint 0 11) ; "That doesn't look like a vending machine!"
								)
								(12
									(TPrint 0 12) ; "Stop flashing your money around, Larry!"
								)
								(16
									(TPrint 0 13) ; "That's not something you should set afire!"
								)
								(22
									(TPrint 0 14) ; "Your doily would look lovely there, but you'd better keep it instead."
								)
								(else
							...
							(switch theItem
								...
								(5
									(TPrint 0 19) ; "Your gold record would get scratched there."
								)
								(7
									(TPrint 0 16) ; "You wave the fax in the air. You have fun."
									(TPrint 0 17 #at -1 185) ; "(But not much!)"
	...
	(method (quitGame &tmp [str1 10] [str2 8] theFile)
		...
		(if
			...
			(if (theFile open: 2)
				(Format @str2 0 22 gPokerJackpot) ; "%d"
				(theFile
					writeString: @str1
					writeString: {\n}
					writeString: @str2
	...
	(method (startRoom roomNum)
		...
		(if (and gDebugging (not (OneOf roomNum 200 460)))
			((ScriptID 10 0) init:) ; debugHandler
			(= gDebugging 0)
		)
		(gTheIconBar enable:)
...
(instance icon5 of IconI
	...
	(method (select)
		(if (and gFfRoom (super select: &rest))
			(gTheIconBar hide:)
			(if (Print 0 25 #title {Fast Forward} #button {Yes} 1 #button {Oops} 0) ; "Do you really want to skip ahead?"
				(if (== gFfRoom 1000)
		...
		(switch theVerb
			...
			(2
				(if (theObj lookStr:)
					...
					(Format @str 0 27 objDesc) ; "Why, look! It's %s."
					(TPrint @str)
				)
			)
			(5
				(Format @str 0 28 objDesc) ; "You engage in a short, but personally meaningful, conversation with %s."
				(TPrint @str)
			)
			(3
				(Format @str 0 29 objDesc) ; "That feels exactly like %s."
				(TPrint @str)
			)
			(4
				(= theItem ((gTheIconBar curInvIcon:) description:))
				(Format @str 0 30 theItem objDesc) ; "It seems %s just doesn't work with %s."
				(TPrint @str)
			)
			(10
				(Format @str 0 31 objDesc) ; "Don't do that to %s!"
				(TPrint @str)
			)
			(else
