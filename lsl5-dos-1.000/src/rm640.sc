		...
		(switch (= state newState)
			...
			(1
				...
				(if (not (DoSound sndGET_AUDIO_CAPABILITY))
					(TPrint 640 4 67 -1 20 108 self) ; "Amidst the hiss and crackle of this old LP, you can just barely make out a husky female voice saying, "Porn is best at a quarter a whack!""
				else
			...
			(2
				...
				(Points 3 125)
				(TPrint 640 5 67 -1 20 108 self) ; "Of all the nerve! des Rever Records imprints subliminal messages encouraging young Americans to frequent adult bookstores!"
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(if (not (DoSound sndGET_AUDIO_CAPABILITY))
					(TPrint 640 7 67 -1 20 108 self) ; "Although the music sounds quite garbled backwards, you feel certain you can understand the words, "Just Say Yes!""
				else
					(gTheMusic2 setVol: 80)
					(message2 play: self)
				)
			...
			(2
				(gTheMusic2 setVol: 127)
				(Points 3 123)
				(TPrint 640 8 67 -1 20 108 self) ; ""Just Say Yes!" Why, that's just the opposite of what Nancy Reagan said! You now have the incontrovertible evidence Desmond sent you to find! And you finally understand what evil this place does!"
			)
			(3
				(TPrint 640 9 67 -1 20 108 self) ; "How vile! How corrupt! des Rever Records records backwards music urging innocent teenagers to violate the law! You never really believed this before--but you believe it now!"
			)
			(4
				(TPrint 640 10 67 -1 20 108 self) ; "If you could only obtain more evidence to prove your case."
			)
			(5
				(HandsOn)
				(User canControl: 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(if (not (DoSound sndGET_AUDIO_CAPABILITY))
					(TPrint 640 11 67 -1 20 108 self) ; "Although it seems improbable, you feel sure you can understand a message among the sped-up music: "Digital Audio Tape causes impotence." How odd."
				else
					(gTheMusic2 setVol: 80)
					(message3 play: self)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HandsOff)
				(Points 3 122)
				(TPrint 640 13 67 -1 20 108 self) ; "Well, that's interesting. You've always wondered if there was really a record underneath all that "gold.""
			)
			(2
				(HandsOn)
				(User canControl: 0)
