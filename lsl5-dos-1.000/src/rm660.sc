		...
		(switch (= state newState)
			...
			(2
				(if (and (== realTakes 1) (not (HaveMouse)))
					(TPrint 660 14 #at -1 15 #width 280 #title {A Tip from AL}) ; "Play the white synthesizer keys by typing the keys on the A S D F row. Play the black synthesizer keys by typing the corresponding keys on the W E T Y U row. Holding down the Shift key plays higher."
				)
				(if (> realTakes 1)
					(rewind play: self)
			...
			(5
				(Format @str 660 15 realTakes) ; "Take %d!"
...
(instance sEnterBooth of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(15
				(HandsOff)
				(Say Reverse_Biaz 660 20 #dispose) ; "Well, Patti," Reverse says, "I've got another session starting soon. You're going to have to go now; I've got to get to work!"
				(keyboard dispose:)
...
(instance door of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 660 56) ; "You would like to see the control room, and talk with Mr. Biaz, but first you want to complete this recording session."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance reverseBiaz of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if didSession
					(Say Reverse_Biaz 660 57 #dispose) ; "Oooh! Now that feels nice, Patti."
				else
					(TPrint 660 58) ; "You are unable to touch Reverse Biaz through that double-paned soundproof glass."
				)
			)
			(5 ; Talk
				(if didSession
					...
					(Say Reverse_Biaz 660 59 #dispose) ; "Patti, I can see your lips moving, but I don't have an open mike out there. Just go ahead and play, and we can talk later, okay?"
				)
			)
			(4 ; Inventory
				(switch invItem
					(4 ; A_Blank_Videotape_c
						(if didSession
							...
							(TPrint 660 60) ; "You don't pour THROUGH glass, Patti, you pour INTO glass!"
						)
					)
					(else
						(Say Reverse_Biaz 660 61 #dispose) ; "I don't know why you're offering me that, Patti. Did you come to play or come to bargain?"
					)
				)
			)
			(10 ; Zipper
				(if didSession
					...
					(TPrint 660 62) ; "You have no way to do that through that window!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance reelOne of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 660 63) ; "You assumed all modern studios recorded directly to hard disks!"
			)
			(3 ; Do
				(if didSession
					(TPrint 660 64) ; "Reverse would prefer you to leave his reels alone!"
				else
					(TPrint 660 65) ; "Those tapes are inaccessible to you--for now!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(class synthKey of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 660 66) ; "Wow! It's a Farelight 2000, the highest-tech high-tech synthesizer keyboard. You've always wanted one of these for its wonderful Boesendorfer patch!"
				(TPrint 660 67) ; "Gurn Blanston plays one with "Rex Famine and the Limestone Bug Legs.""
			)
			(3 ; Do
				(if didSession
					(TPrint 660 68) ; "You can't play the synthesizer keyboard from inside the control room!"
				else
					(super doVerb: theVerb invItem &rest)
				)
			)
...
(instance music of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if didSession
					(TPrint 660 69) ; "This is the song you recorded for Reverse."
				else
					(TPrint 660 70) ; "This is the song you are supposed to perform. You DO remember how to play keyboards, don't you, Patti?"
				)
			)
			(3 ; Do
				(if didSession
					(TPrint 660 71) ; "You can't reach the music from inside the control room."
				else
					(TPrint 660 72) ; "There's no need for you to turn the page, Patti!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance synthPanel of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if didSession
					(TPrint 660 73) ; "You can't reach the synthesizer from inside the booth!"
				else
					(TPrint 660 74) ; "As much as you hate to, you just can't resist tweaking a patch here and there!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance controlPanel of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 660 75) ; "The control room mixer is covered with a complicated array of sliders, knobs, buttons, patch bays and XLR connectors."
			)
			(3 ; Do
				(TPrint 660 76) ; "You'd love to get your hands on those potentiometers, but assume Reverse is as particular as most engineers!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
