...
(instance rm840 of LLRoom
	...
	(method (init)
		...
		(opener init: approachVerbs: 3) ; Do
		(table init:)
		(sofa init:)
		(chair init:)
		...
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 0 0 319 0 319 142 268 146 267 139 255 122 235 122 226 113 92 113 79 113 96 119 96 133 70 139 52 147 33 147 0 139
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
					init: 0 155 33 155 4 173 3 187 316 187 316 180 282 156 319 156 319 189 0 189
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
				...
				(switch invItem
					...
					(10 ; Some_Change
						(if (IsFlag 24) ; fDrawerOpen
							...
							(TPrint 840 7) ; "You would return the folder to the drawer, but the desk is locked."
						)
					)
					(else
		...
		(switch theVerb
			(3 ; Do
				(if (gEgo has: 11) ; DayTrotter
					(TPrint 840 11) ; "The copier is now broken! You should know; you broke it!"
				else
					(TPrint 840 12) ; "To use the `Zeroz 9000,' simply insert the pages you wish to copy."
				)
			)
			(4 ; Inventory
				(switch invItem
			...
			(3 ; Do
				(cond
					((gEgo has: 10) ; Some_Change
						(TPrint 840 13) ; "Mr. Krapper will surely miss his file folder, Patti! You'd better return it."
					)
					((gEgo has: 9) ; AeroDork_s_In-Flight_Magazine
						(TPrint 840 14) ; "Mr. Krapper will surely miss his desk key, Patti! You'd better return it."
					)
					((gEgo has: 8) ; Boarding_Pass
						(TPrint 840 15) ; "Mr. Krapper will surely miss his letter opener, Patti! You'd better return it."
					)
					(else
...
(instance telephone of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 840 16) ; "You'd better not use this telephone, Patti. You might get caught by the switchboard operator."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 840 17) ; "You briefly consider booting up John Krapper's computer in an attempt to contact Inspector Desmond, but realize there's no way his computer would be connected to FBI-Net!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance opener of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 18) ; "There's a needle-sharp letter opener lying on Krapper's desk."
			)
			(3 ; Do
				(TPrint 840 4) ; "Never knowing when you might get some mail, you decide to take Krapper's letter opener from his desk."
				(opener hide:)
...
(instance peekScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 19) ; "Opening the doorway to carefully peek outside, you notice the receptionist has returned. You decide you cannot exit the room that way!"
				(= seconds 3)
			)
			(3
				(lobbyDoor setCel: 0)
...
(instance getKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 10 148)
				(TPrint 840 20 67 -1 20) ; "Bending over at the base of the tree, you dig into the humus and find the metallic object that you saw earlier. It looks like a desk key."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance sPawThruPlant of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 840 21) ; "Pawing through the humus in the tree's pot, you see a glint of metal."
				(SetFlag 23) ; fSeenGlint
				(HandsOn)
				(self dispose:)
			)
...
(instance replaceKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 22 67 -1 20) ; "You carefully return the key to its hiding place in the humus."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance openDeskScript of Script
	...
	(method (changeState newState &tmp [str 100])
		(switch (= state newState)
			...
			(4
				(Format @str 840 23 (= gRoomBNumber (Random 10000 -536))) ; "Peering through John Krapper's personal (formerly-locked) file drawer, you notice a number written on a small blue stickum! Why in the world would he want to remember %u?"
				(Points 4 144)
				(TPrint @str)
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
...
(instance sOpenDesk2ndTime of Script
	...
	(method (changeState newState &tmp [str 100])
		(switch (= state newState)
			...
			(4
				(Format @str 840 24 gRoomBNumber) ; "Where's that little blue stickum? Oh, here it is! %u"
				(TPrint @str)
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
...
(instance pickLockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 5 147)
				(TPrint 840 25) ; "Prying away at the desk's inefficient lock with Krapper's letter opener, you find a weak spot and carefully pry the drawer open."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance unlockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 13 147)
				(TPrint 840 26) ; "Trying the key you found buried in the tree's pot, you insert into the desk and turn it carefully. It opens."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance lockDeskScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 27) ; "You lock the desk."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance getPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Points 5 145)
				(TPrint 840 28) ; "Before closing the desk drawer, you find some folders marked "Personal.""
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance lookPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 840 29) ; "Carefully searching through the many drawers of Krapper's desk, you find a folder marked "Personal.""
				(= ticks 60)
			)
			(2
				(TPrint 840 30) ; "For a moment, you wonder if you should take it."
				(= ticks 60)
			)
			(3
				(Points 5 145)
				(TPrint 840 31) ; "But of course you do!"
				(= ticks 123)
			)
			(4
				(gEgo
...
(instance putPapersScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(TPrint 840 32) ; "You return the folder of evidence to Krapper's desk, and close and lock his desk drawer, being careful to leave everything exactly as you found it."
				(gEgo
					normalize: (if (IsFlag 27) 842 else 570) ; fPattiBlackface
					observeControl: 2 4
					loop: 2
...
(instance runCopierScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 12 151)
				(TPrint 840 33) ; "You place John Krapper's personal papers in his copier, and press the "Start" button."
				(= ticks 123)
			)
			(3
			...
			(8
				(Say gEgo 840 34) ; "Hey, what's the matter with this thing?" you think. "Why did it stop working? What did I do, jam it?"
				(= seconds 3)
			)
			(9
			...
			(10
				(paper dispose:)
				(copier setCel: 0)
				(Say gEgo 840 35) ; ""Oh well, at least I got the most important evidence copied before it jammed," you think, grabbing your copies from the machine."
				(= seconds 3)
			)
			(11
			...
			(12
				(Say gEgo 840 36) ; ""What does this switch here do?" you wonder, pulling on something that doesn't belong to you."
				(= ticks 123)
			)
			(13
				(TPrint 840 37 67 -1 185) ; "(An experience not wholly unfamiliar to you!)"
				(= ticks 60)
			)
			(14
				(gEgo loop: 4 setCel: 255 setCycle: Beg self)
			...
			(15
				(gEgo normalize: 570 x: 69 y: 140 loop: 1 setHeading: 270)
				(= ticks 60)
			)
			(16
			...
			(20
				(Say gEgo 840 38) ; "GREAT!" you think. "Now just look at me! I'm covered with toner from head to toe!"
				(copier stopUpd:)
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== (gEgo view:) 842)
					(TPrint 840 39) ; "Oh no," you think, "how am I ever going to get this stuff off?"
					(return 1)
				else
					(TPrint 840 40) ; "You look yourself over. You like what you see. "Although, I could stand to lose a few pounds," you think."
					(return 1)
				)
			)
			(else
