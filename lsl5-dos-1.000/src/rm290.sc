...
(instance rm290 of LLRoom
	...
	(method (cue)
		(switch (++ local0)
			...
			(3
				(TPrint 290 1) ; "A voice comes over the public address system, "AeroDork Airlines is proud to announce the departure of its Chartreuse Service to msdffft, wqpaffw, and paovnnq.""
				(TPrint 290 2) ; "Hmm. That speaker is none too clear."
				(Delay 2 0 gCurRoom)
			)
			(4
				(HandsOn)
...
(instance chair of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 3) ; "How thoughtful! AeroDork has provided a comfortable metal folding chair for your sitting convenience!"
			)
			(3 ; Do
...
(instance door of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 4) ; "The door to the jetway is firmly locked, and without protrusions of any kind."
			)
			(3 ; Do
				(TPrint 290 5) ; "You can only open this door by using the ABM to the right."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boardingSign of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 32)
					(TPrint 290 6) ; "The Boarding Pass sign is currently flashing. Are you going to miss the plane?"
				else
					(TPrint 290 7) ; "It appears to be a sign."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
			...
			(4 ; Inventory
				(switch invItem
					(7 ; AeroDork_Gold_Card
						(TPrint 290 8) ; "Your AeroDork Gold Card will only work in the ATM outside the airport."
					)
					(8 ; Boarding_Pass
						(if (IsFlag 32)
							...
							(TPrint 290 9) ; "You're a little early. That flight isn't due to board for another few minutes."
						)
					)
					(else
						(super doVerb: theVerb invItem &rest)
...
(instance pot of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 13) ; "It's just another example of AeroDork's extravagant attitude towards its customers. Freshly brewed coffee just for you!"
			)
			(3 ; Do
				(TPrint 290 14) ; "YUCK! The pot has boiled dry! Nothing remains but the stench of burned coffee."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance cabinet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 15) ; "The cabinet holds up the coffee maker."
			)
			(3 ; Do
				(TPrint 290 16) ; "There's nothing in the cabinet that you need."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance light of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 17) ; "Wouldn't you think the local transportation authorities would worry about exposed wiring?"
			)
			(3 ; Do
				(TPrint 290 18) ; "Right! Of course you can reach the ceiling in here!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance leftPainting of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 290 19) ; "There's nothing hidden behind the painting."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
