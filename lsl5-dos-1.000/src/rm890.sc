	...
	(method (doit)
		(cond
			...
			((gEgo edgeHit:)
				(if jammed
					(if (not egoMove)
						(TPrint 890 0) ; "Someone has jammed the door shut with a large, immovable object!"
						(= egoMove 1)
					)
				else
...
(instance sCartoon of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 890 1) ; "Oh, no! P. C. Hammer has noticed the "Recording" sign over the window of Control Room B, realized you are not an engineer, and is heading for your room!"
				(hammer_a
					view: 883
					xStep: 3
					yStep: 2
			...
			(3
				(TPrint 890 2) ; "Blocking the only way out of the control room with that heavy microphone stand, he heads for the elevator!"
				(TPrint 890 3) ; "You're in trouble now, Patti!"
...
(instance fElectronics4 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 890 7) ; "This recorder will not operate without a reel of tape."
			)
			(2 ; Look
				(TPrint 890 8) ; "You haven't seen a 2", 24-track, 3"-hub reel-to-reel recorder in years. What antiquated equipment! Where's K-RAP's digital equipment?"
			)
			(4 ; Inventory
				(switch invItem
					(12 ; Money
						(TPrint 890 9) ; "You are unable to mount your reel's tiny hole onto this recorder's massive spindle!"
					)
					(else
		...
		(switch theVerb
			(3 ; Do
				(if (!= ((Inv at: 12) owner:) 0) ; Money
					(TPrint 890 10) ; "You have no use for another tape. Use the one you have!"
				else
		...
		(switch theVerb
			(2 ; Look
				(switch (++ mixerLookCounter)
					(1
						(TPrint 890 11) ; "Before you lies an elaborate microphone mixer and studio control panel console. The names of the various downstairs studios are engraved on the potentiometers."
					)
					(2
						(TPrint 890 12) ; "This volume control says, "Control Room C.""
					)
					(3
						(TPrint 890 13) ; "This potentiometer says, "Studio C.""
					)
					(4
						(TPrint 890 14) ; "This slider says, "Control Room B.""
					)
					(5
						(TPrint 890 15) ; "This pot is labeled, "Studio B.""
					)
					(6
						(TPrint 890 16) ; "This controls the volume coming from "Control Room A.""
					)
					(7
						(TPrint 890 17) ; "This fader is labeled, "Studio A.""
					)
					(else
						(TPrint 890 18) ; "The rest of these are labeled "Carts (A-F)," "Tables (A-C)," "CDs (A-R).""
					)
				)
			)
			(3 ; Do
				(if (== (++ mixerDoCounter) 1)
					(TPrint 890 19) ; "The console contains many knobs, sliders, meters and buttons. Which one will you choose?"
				else
...
(instance microphone of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 890 20) ; "You carefully aim the microphone so it's pointing just below your mouth."
			)
			(5 ; Talk
				(if jammed
					(if volumeUp
						...
						(TPrint 890 21) ; "You hear your voice echo through the room's monitor speakers. The speakers are amazingly loud; loud enough to break glass."
					)
				else
					(TPrint 890 22) ; "You carefully check the level of your voice on the mixer. It's okay."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
				...
				(cond
					...
					((== ((Inv at: 12) owner:) 3) ; Money
						(cond
							...
							(else
								(TPrint 890 23) ; "What's the use of recording silence? There no signal currently being fed to this deck."
							)
						)
					)
					(else
						(TPrint 890 24) ; "The recorder's automatic shut-off prevents it from operating without a properly mounted tape."
					)
				)
			)
			(2 ; Look
				(cond
					(recording
						(TPrint 890 25) ; "The recorder is capturing every nuance of "2 Live 2 Screw's" conversation."
					)
					((== ((Inv at: 12) owner:) 3) ; Money
						(cond
							(rewoundTape
								(TPrint 890 26) ; "The recorder contains a tape full of incriminating evidence."
							)
							(recorded
								(TPrint 890 27) ; "This tape is half on the take-up reel. It needs to be rewound before you can remove it from the recorder."
							)
							(else
								(TPrint 890 28) ; "There is a blank tape on the recorder."
							)
						)
					)
					(else
						(TPrint 890 29) ; "A large Studer console recorder is all warmed up and ready to record--except there's no tape upon which to record!"
					)
				)
			)
			(4 ; Inventory
				(switch invItem
					(12 ; Money
						(if recorded
							(TPrint 890 30) ; "Your tape is already recorded."
						else
...
(instance useMikeScript of Script
	...
	(method (changeState newState &tmp i invItem)
		(switch (= state newState)
			...
			(3
				(TPrint 890 31) ; "Taking your biggest possible diaphragmatic breath, you lean back and belt out your most powerful high C, directly into the microphone, through the powerful amplifiers and out through the control room monitor speakers!"
			...
			(6
				(TPrint 890 32) ; "Is it Patti..."
				(TPrint 890 33) ; "...or is it Memorex?"
				(= seconds 4)
			)
			(7
				(gEgo
			...
			(14
				(gTheIconBar enable:)
				(TPrint 890 34) ; "Punching in the access code you learned by watching P. C. Hammer, you are able to ride the showervator back to John Krapper's bathroom. Luckily, no one noticed your clothes lying on the floor there."
				(= seconds 3)
			)
			(15
				(TPrint 890 35) ; "Putting on your dress and gathering your possessions, you sneak back to the receptionist's lobby door and wait for her to take a break before making for the safety of your patiently waiting limo."
				(= seconds 3)
			)
			(16
...
(instance GetTape of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Points 4 158)
				(TPrint 890 36) ; "You grab a blank reel of tape from the shelf."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance turnRecorderOn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 4 155)
				(TPrint 890 37) ; "You turn on the reel-to-reel tape recorder."
				(= recording 1)
				(= seconds 7)
			)
			(3
				(TPrint 890 38) ; "As you record "2 Live 2 Screw," you giggle with delight as they thoroughly incriminate themselves."
				(= seconds 3)
			)
			(4
			...
			(5
				(TPrint 890 39) ; "What's that? What's P. C. Hammer doing? He looks like he's seen your "Recording" sign! And he doesn't look too happy about it."
				(= seconds 3)
			)
			(6
			...
			(7
				(TPrint 890 40) ; "Oh, oh! Where did Hammer go?"
				(glass startUpd:)
				(= cycles 5)
			)
			(8
...
(instance turnRecorderOff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 890 41) ; "You turn off the recorder."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance rewindTape of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(TPrint 890 42) ; "The tape is now fully rewound."
				(= rewoundTape 1)
				(HandsOn)
				(self dispose:)
			)
...
(instance getRecordedTape of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Points 7 156)
				(TPrint 890 43) ; "You remove the reel-to-reel tape."
				(= ticks 30)
			)
			(4
				(HandsOn)
		...
		(switch (= state newState)
			...
			(2
				(switch mixerDoCounter
					(1)
					(2
						(TPrint 890 44) ; "You bring up the volume control labeled "Control Room C.""
						(Wait 60)
						(TPrint 890 45) ; "You hear nothing but hiss."
					)
					(3
						(TPrint 890 46) ; "You bring up the potentiometer labeled "Studio C.""
						(Wait 60)
						(TPrint 890 47) ; "You hear nothing but hiss and a little air conditioner bleed-through."
					)
					(4
						(TPrint 890 48) ; "You pull up the slider labeled "Control Room B" and hear what sounds like a female breathing."
						(Wait 60)
						(TPrint 890 49 #at -1 185) ; "(Oh, wait! That's you! You're in Control Room B!)"
					)
					(5
						(TPrint 890 50) ; "You raise the volume on "Studio B,""
						(Wait 60)
						(TPrint 890 51) ; "You hear nothing."
					)
					(6
						(TPrint 890 52) ; "You carefully open the slider on "Control Room A," as if P. C. Hammer could hear you doing so."
						(Wait 60)
						(TPrint 890 53) ; "He's recording his "live drive-time" show for tomorrow morning. Who cares?"
					)
					(else
						(cond
							((not (IsFlag 154))
							...
							(else
								(TPrint 890 56) ; "You are eavesdropping on the conversations from Studio A and to you it sounds just like "2 Live 2 Screw!""
							)
						)
					)
				)
...
(instance sHearingScrew of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(TPrint 890 57) ; "You fade up the volume on "Studio A" and... listen to that!"
				(lights setCycle: Fwd startUpd:)
				(= seconds 3)
			)
			(1
				(TPrint 890 58) ; "Why, that's "2 Live 2 Screw!" Patti! You did it! Now you can overhear their conversation!"
				(= seconds 3)
			)
			(2
				(Points 8 154)
				(TPrint 890 59) ; "But how can you get some hard evidence to take back to Inspector Desmond to prove you really are hearing this!?"
				(= usedMixer 1)
				(HandsOn)
				(self dispose:)
			)
...
(instance mountTapeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 890 60) ; "You place your tape on the recorder, carefully threading it around the tension arms, over the heads and past the capstan, finishing by threading the leader onto the takeup reel with a professional flourish."
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance doorA of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 890 61) ; "You don't want to try that door, Patti! The direct approach won't work with P. C. Hammer."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance ActionsKRAP of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(TPrint 890 62) ; "You attempt to hike up those Hammerpants you're wearing, but it does no good!"
				(return 1)
			)
			(10 ; Zipper
				(TPrint 890 63) ; "Maintaining your cool, you surreptitiously check the zipper on your giant baggy pants. It's fine."
				(return 1)
			)
			(else
				(return 0)
