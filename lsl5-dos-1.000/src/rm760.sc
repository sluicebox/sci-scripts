...
(local
	...
	[growY 11] = [103 124 145 0 0 0 0 0 0 0 0]
			...
			((StepOn gEgo 8)
				(cond
					...
					((and (gEgo skating:) (not movingOffControl))
						(if (not seenWarning)
							(TPrint 760 0) ; "Be careful, Larry! You nearly skated right off the boardwalk."
							(= seenWarning 1)
						)
						(HandsOff)
						(gEgo
...
(instance lana of Skater
	...
	(method (doit)
		...
		(if
			...
			(TPrint 760 1) ; "There she is! It's Lana Luscious!! She looks just like that photo you saw in her resume!"
			(= seenMsg 1)
			(= freq 6)
		)
		(if
		...
		(switch theVerb
			...
			(5 ; Talk
				(if (or (< (lana x:) 25) (> (lana x:) 300))
					(TPrint 760 2) ; "She must not have heard you, Larry. Perhaps you should try to get closer next time."
				else
					(= caughtLana 1)
					(gEgo setScript: sMeetLana)
				)
			...
			(2 ; Look
				(switch loop
					(0
						(switch cel
							(0
								(TPrint 760 3) ; "This shanty reminds you of "The Shack," your favorite collegiate hangout!"
							)
							(1
								(TPrint 760 4) ; "What a strange location for a rocket fuel manufacturing plant."
							)
							(2
								(TPrint 760 5) ; "You haven't seen a slot-car raceway in years!"
							)
							(3
								(TPrint 760 6) ; "The "Salty Dog Saloon" is built upon the very same location as the old "Pepper Dog Saloon," which burnt down years ago in the Great Tramp Boardwalk Fire of '06."
							)
							(4
								(TPrint 760 7) ; "Wow! In this place you can watch movies for only 25 cents! What a bargain!"
							)
							(5
								(TPrint 760 8) ; "This is one of the last of the juke joints!"
							)
							(6
								(TPrint 760 9) ; "You wonder what they sell inside the "Love Shack.""
							)
							(7
								(TPrint 760 10) ; "This house was built here many years ago, before there even was a Tramp Boardwalk!"
							)
							(8
								(TPrint 760 11) ; "Recliners are a very popular item among any Atlantic City tourists that can still walk!"
							)
						)
					)
					(1
						(switch cel
							(0
								(TPrint 760 12) ; "A sign on the front door reads, "Shirts? No shoes. No service.""
							)
							(1
								(TPrint 760 13) ; "Carlos Escobar has been running this taco stand ever since he wrecked his van staring at sidewalk scenery!"
							)
							(2
								(TPrint 760 14) ; "You wonder if they have your favorite poster--the extra-large-size, full-color, "Wayne Newton Perspiring!""
							)
							(3
								(TPrint 760 15) ; "The owners of the "Live Girl" shop are faring much better financially since they changed their name from the "Dead Girl" shop!"
							)
							(4
								(TPrint 760 16) ; "If you wanna have fun, this is the place to do it."
							)
							(5
								(TPrint 760 17) ; "Contrary to popular rumor, this is NOT a jukebox recycling center."
							)
							(6
								(TPrint 760 18) ; "If you want a CD real bad, this is the place! They've got some CDs that are real bad!"
							)
							(7
								(TPrint 760 19) ; "No boardwalk is complete without the requisite salt-water taffy store."
							)
						)
					)
					(2
						(TPrint 760 20) ; "Look! It's the skate shop of the supermarket-famous Ivana Tramp! And--she's open!!"
					)
				)
			)
			(3 ; Do
				(if
					...
					(if (== loop 2)
						(if (gEgo skating:)
							(TPrint 760 21) ; "Sorry, Larry. A sign on the door clearly reads, "No shirt, no shoes, no service." You're going to have to remove your skates before entering."
						else
...
(instance sMeetLana of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Say gEgo 760 28) ; "It would be my pleasure."
				(= ticks 30)
			)
			(5
				(gEgo
...
(instance sLanaTalks of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say gEgo 760 29) ; ""Skate here often?" you ask."
				(= ticks 60)
			)
			(1
			...
			(2)
			(3
				(Say gEgo 760 31) ; ""So, does that happen often?" you ask."
				(= ticks 60)
			)
			(4
			...
			(5)
			(6
				(Say gEgo 760 33) ; ""I must tell you how much I love your skating technique," you tell Lana."
				(= ticks 60)
			)
			(7
			...
			(8)
			(9
				(Say gEgo 760 35) ; ""Where is the mud wrestling held?" you ask."
				(= ticks 60)
			)
			(10
			...
			(13
				(Points 2 140)
				(Say gEgo 760 38) ; "I'll be there, Lana!" you exclaim. "See you there!"
				(= ticks 60)
			)
			(14
...
(instance lanaFtr of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 760 41) ; "You are sitting on the boardwalk staring directly into the eyes of a gorgeous, oversexed blond! If only you could think of something to say!"
			)
			(5 ; Talk
				(if (gCurRoom script:)
					((gCurRoom script:) cue:)
...
(instance buildings of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 760 45) ; "Almost every building on the boardwalk is closed today in honor of a long celebrated local holiday--Donald Tramp's birthday."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
