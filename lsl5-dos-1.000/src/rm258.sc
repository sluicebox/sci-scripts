...
(instance rm258 of LLRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 258 0) ; "You are standing before one of AeroDork Airline's amazing new Automatic Ticket Machines. Just insert your exclusive AeroDork Gold Club membership card into the slot on the left."
			)
			(4 ; Inventory
				(if (== invItem 7) ; AeroDork_Gold_Card
					(TPrint 258 1) ; "Insert your exclusive AeroDork Gold Club membership card into the slot on the left."
				else
	...
	(method (newRoom)
		(if (gCast contains: card)
			(TPrint 258 2) ; "Hey! Don't forget your gold card!"
		else
			(SetupExit 0)
			(super newRoom: &rest)
		)
...
(instance boardPass of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 258 4) ; "Your newly obtained boarding pass is just hanging there, waiting for you to grab it."
			)
			(3 ; Do
				(Points 4)
...
(instance card of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 258 5) ; "Your AeroDork Gold Card is ready to return to your pocket."
			)
			(3 ; Do
				(HandsOff)
				(gCurRoom drawPic: (gCurRoom picture:) 100)
...
(instance slot of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 258 6) ; "Insert your AeroDork Gold card into this slot."
			)
			(4 ; Inventory
				(switch invItem
	...
	(method (doVerb theVerb invItem &tmp [str 80])
		(switch theVerb
			...
			(3 ; Do
				(= theBar 0)
				(gCurRoom drawPic: (gCurRoom picture:) 100)
				(Display 258 8 dsCOORD 144 75 dsCOLOR gColWhite dsWIDTH 110 dsFONT gSmallFont) ; "Please enter the five-character Destination Code from your AeroDork Airline Travel Brochure."
				(TPrint 258 9 #title {A hint from Al Lowe}) ; "You can use your number pad to enter your code. The keys match in position (i.e. the 7 key will press the top left button, etc.). Be sure to use only the numeric key pad, not the cursor keys."
	...
	(method (doVerb theVerb invItem &tmp [str 80])
		(switch theVerb
			...
			(3 ; Do
				(= theBar 2)
				(gCurRoom drawPic: (gCurRoom picture:) 100)
				(Display 258 10 dsCOORD 144 75 dsCOLOR gColWhite dsWIDTH 110 dsFONT gSmallFont) ; "Please enter the five character Destination Code from your AeroDork Airline Travel Brochure."
				(TPrint 258 9 #title {A hint from Al Lowe}) ; "You can use your number pad to enter your code. The keys match in position (i.e. the 7 key will press the top left button, etc.). Be sure to use only the numeric key pad, not the cursor keys."
				((ScriptID 20 0) init: 0) ; TTDialer
				(bar1 dispose:)
				(bar2 dispose:)
