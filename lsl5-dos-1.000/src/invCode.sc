...
(instance Camcorder of InvI
	...
	(method (doVerb theVerb invItem &tmp [temp0 50] temp50 temp51)
		...
		(switch theVerb
			...
			(3 ; Do
				(cond
					((>= state 100)
						(-= state 100)
						(TPrint 19 2) ; "Delicately exchanging your blue ballpoint with your yellow highlighter, you switch the camcorder off."
					...
					((and temp50 (gChargeTimer seconds:))
						(+= state 100)
						(Points 4 72)
						(TPrint 19 4) ; "Slipping your green felt-tip between the .5-mm lead mechanical pencil and the blue ink rolling ball pen, you switch the camcorder on."
						(Format ; "Charge: %d min %d sec"
							@temp0
							19
							3
					...
					(temp50
						(TPrint 19 5) ; "Your camcorder's batteries are exhausted."
					)
					(else
						(TPrint 19 6) ; "The camcorder will not operate without a videotape inside."
					)
				)
			)
			(4 ; Inventory
				(if (OneOf invItem 2 3 4) ; A_Blank_Videotape_a, A_Blank_Videotape_b, A_Blank_Videotape_c
					(if temp50
						(TPrint 19 7) ; "Before installing another videotape, you first remove the tape in the camcorder."
					...
					(if (!= ((Inv at: invItem) state:) 1)
						...
						(Points 4 73)
						(TPrint 19 10) ; "You insert the tape into "The Rob Lowe.""
						(= state invItem)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(0 ; Camcorder
						(TPrint 19 13) ; "The battery recharger must be plugged into a wall socket before the camcorder may be installed in it."
					)
					(else
						(localproc_0 (Inv indexOf: self) invItem)
					)
...
(instance A_Blank_Videotape_a of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 19) ; "You wiggle the tiny door on the videotape just to make sure it still works. It does."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance A_Blank_Videotape_b of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 20) ; "You shake the videotape violently, seeking to prevent premature shell stickage."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance A_Blank_Videotape_c of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 21) ; "You smooth down the label on the videotape with your hand."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Michelle_Milken_s_Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 25) ; "You open Michelle Milken's resume and look inside."
				(self doVerb: 2)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				...
				(Points 5 68)
				(TPrint 19 34) ; "You promise to remember this information, in case you ever run across a music box in a computer store."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Membership_Tape of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 47) ; "You briefly consider folding, spindling or mutilating this tape, but remembering your early training, decide not to do it--at least, not here."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(switch state
					(0
						(TPrint 19 51) ; "The DataMan has no on/off switch as it is always on. However, right now it is empty."
					)
					(1
						(= state 0)
						(TPrint 19 52) ; "You remove Reverse Biaz's DataPak from your DataMan."
					...
					(2
						(= state 0)
						(TPrint 19 53) ; "You remove P. C. Hammer's DataPak from your DataMan."
			...
			(4 ; Inventory
				(switch invItem
					(1 ; Battery_Charger
						(Points 7 75)
						(if (== (DataMan state:) 0)
							(TPrint 19 54) ; "You insert Reverse Biaz's DataPak into your DataMan."
							(= state 1)
							(Reverse_Biaz_DataPak owner: self)
							(Inv curIcon: 0 hide:)
							(gEgo showInv:)
						else
							(TPrint 19 55) ; "The DataMan already contains a DataPak."
						)
					)
					(2 ; A_Blank_Videotape_a
						(Points 7 75)
						(if (== (DataMan state:) 0)
							(TPrint 19 56) ; "You insert P. C. Hammer's DataPak into your DataMan."
							(= state 2)
...
(instance Lana_Luscious__Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 63) ; "You open Lana Luscious' resume and look inside."
				(self doVerb: 2)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
...
(instance Silver_Dollar of InvI
	...
	(method (doVerb theVerb invItem &tmp [temp0 200])
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 69) ; "You enjoy the clinking sound these silver dollars make."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (== gCurRoomNum 760)
					...
					(TPrint 19 71) ; "You can't skate here, Larry!"
				)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
...
(instance Gold_Record of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 77) ; "You have your doubts that this record is solid gold."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Chi_Chi_Lambada_s_Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 83) ; "You open Chi Chi Lambada's resume and look inside."
				(self doVerb: 2)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(cond
					((!= gCurRoomNum 905)
						(TPrint 19 92) ; "You'd be too embarrassed to wear the doily now!"
					)
					((gEgo sitting:)
						(TPrint 19 93) ; "Stand up first."
					)
					(else
						(if state
							(gEgo
							...
							(TPrint 19 94) ; "Sadly, you remove the doily from your head."
						else
							(gEgo
								view: 907
								setLoop: 4
							...
							(Points 8 159)
							(TPrint 19 95) ; "Feeling rather silly, you wrap the doily around your head in a classic "3 Stooges-toothache" style."
						)
						(= state (not state))
					)
				)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (gEgo has: 11) ; DayTrotter
					(TPrint 19 102) ; "There is nothing more to find in this folder. Besides, you already have photocopies of everything inside."
				else
					(TPrint 19 103) ; "Checking through these papers makes you realize you need some way to photocopy them."
				)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
...
(instance Reel_To_Reel_Tape of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(TPrint 19 109) ; "You really need a tape deck to use this, Patti."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
