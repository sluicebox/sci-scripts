...
(class SQ6 of Game
	...
	(method (init &tmp temp0 temp1 temp2)
		...
		(if 1
			(while (Message msgSIZE 0 18 0 0 temp2) ; UNINIT
				(++ temp2) ; UNINIT
			)
			(= temp1 (Str new:))
			(Message msgGET 0 18 0 0 (RandomNumber 1 (- temp2 1)) (temp1 data:)) ; UNINIT
			(SetQuitStr (temp1 data:))
			(temp1 dispose:)
		)
	)
		...
		(if
			(= temp0
				(switch param1
					...
					(10
						(ScriptID 280 1) ; paTalker
					)
					(8
						(ScriptID 300 1) ; bartenderTalker
					)
					(39
						(switch gCurRoomNum
							(360
								(ScriptID 360 1) ; starCon
							)
							(100
					...
					(28
						(switch gCurRoomNum
							...
							(520
								(ScriptID 520 1) ; sharpeiTalker
							)
							(770
								(ScriptID 770 1) ; sarpeiTalker
							)
						)
					)
					(40
						...
						(switch gCurRoomNum
							(100
								(if (== (gCurRoom curPic:) 80)
									(ScriptID 100 1) ; KielbasaCloseTalker
								else
							...
							(380
								(ScriptID 380 3) ; kielTalker
							)
							(490
								(ScriptID 490 1) ; tkrKielbasa
							)
							(510
								(ScriptID 510 2) ; kielbasaTalker
							)
							(460
								(ScriptID 460 2) ; kielbasa
							)
						)
					)
					(2
