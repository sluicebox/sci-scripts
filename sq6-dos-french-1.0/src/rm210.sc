...
(instance sAshTrayScript of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(= seconds (RandomNumber 5 30))
			)
			(1
				(client loop: local10 cel: 0 setCycle: End self)
			)
			(2
				(if (<= local10 3)
					(++ local10)
...
(instance sStartGame of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(stoogeSound number: 10000 play: self)
			)
			(9
				(stoogeSound number: 11000 play: self)
			...
			(11
				(localproc_2)
				(= local13 1)
				(ashtray init: setScript: sAshTrayScript)
				(self dispose:)
			)
		)
	)
...
(instance sResetTheGame of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if local14
					(if (== client playerOne)
						...
						(stoogeSound number: 19000 play: self)
					else
						(Message msgGET 210 11 0 0 2 (local1 data:)) ; "He moidered you!"
						(stoogeSound number: 11100 play: self)
					)
			...
			(10
				(stoogeSound number: 12000 play: self)
				(rumble init: setCycle: Fwd)
			)
			(11
				(rumble dispose:)
