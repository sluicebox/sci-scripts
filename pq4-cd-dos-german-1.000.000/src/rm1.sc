...
(instance myEgo of stroidsEgo
		...
		(if (gUser canControl:)
			(switch (event type:)
				(evMOUSEBUTTON
					...
					(event type: evKEYBOARD modifiers: $0000)
					(self handleEvent: event)
				)
				(evMOUSERELEASE
					(if (== (event modifiers:) emSHIFT)
						(event type: evKEYUP message: KEY_UP modifiers: $0000)
						(= local11 0)
						(self handleEvent: event)
					)
				)
				...
				(evKEYUP
					(if local1
						(local4 stop:)
						(= local1 0)
						(self setCycle: 0 view: 2010 setCel: 0)
						(event claimed: 1)
						(return)
					)
				)
	...
	(method (doit &tmp temp0)
		(cond
			...
			((and local1 (not (-- local15)))
				(= local15 5)
				(= temp0 (Clone Event))
				(temp0 type: 4 message: 18432 modifiers: 0)
