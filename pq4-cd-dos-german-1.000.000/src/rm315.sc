...
(instance rm315 of Room
	...
	(method (init)
		...
		(sFx init:)
		(sFx setLoop: -1 number: 923 play:)
		(gEgo setTalker: careyTalker x: 0 y: 0)
		(barFly
			view: 9315
			setLoop: 0
			cel: 0
			...
			(switch theVerb
				(27 ; badge
					(if (== noun 1)
						(gGame points: 2 52)
						(if local3
							(gMessager say: 1 27 0 0) ; "Ummm... so you're a real cop. Tell me, handsome, is everything you're packing real?"
							(return 1)
						else
							(= local3 1)
			...
			(4 ; Do
				...
				(cond
					...
					((not (gCast contains: stripper))
						...
						(gMessager say: 6 4 34) ; "Never knowing when you might need to light a lady's cigarette, you pocket the lighter."
					)
					(else
						(gMessager say: 6 4 26) ; "This lighter belongs to the lady. Not you."
					)
				)
			)
			(else
...
(instance lightCig of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(gMessager say: 6 4 6 0 self) ; "Behaving as a gentleman, you pick up the lighter and give the lady what she asked for."
			)
			(1
				(if (not (barFly cel:))
					(barFly setCycle: End self)
...
(instance showShoe of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(barFly view: 9322 loop: 0 cel: 0 setCycle: End self)
			)
			(4
				(barFly loop: 1 cel: 0 setCycle: End self)
			)
			(5
				(barFly loop: 2 cel: 0 setCycle: End self)
			)
			(6
				(if local3
					(if
...
(instance barbieTalker of Narrator
	(properties
		modeless 2
	)
)
...
(instance careyTalker of Narrator
	...
	(method (dispose)
		(if (gCast contains: careyTalkerHead)
			(barFly show:)
			(self modeless: 0)
			(careyTalkerHead dispose:)
		)
		(super dispose: &rest)
	)
)

(instance careyTalkerHead of Prop
	(properties
		x 122
		y 118
		view 9323
