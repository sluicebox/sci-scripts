...
(instance rm355 of Room
	...
	(method (init &tmp temp0)
		...
		(super init: &rest)
		(for ((= local3 1)) (Message msgSIZE gCurRoomNum 1 2 4 local3) ((++ local3))
		)
		(-= local3 2)
		(= local2 (IntArray new: (+ local3 1)))
		(local2 name: {dumbMsg})
		(= local1 local3)
		(for ((= temp0 0)) (<= temp0 local1) ((++ temp0))
			(local2 at: temp0 (+ temp0 1))
		)
		...
		(switch gPrevRoomNum
			(360
				(gEgo posn: 308 124 setHeading: 225)
			)
			(else
...
(instance comeFrom360 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(gEgo setMotion: MoveTo 200 144 self)
			)
			(1
				(gGame handsOn:)
				(self dispose:)
				...
				(cond
					...
					((gPqFlags test: 44)
						(if (not local0)
							...
							(= temp2 (local2 at: (= temp1 (Random 0 local1))))
							...
							(gMessager say: 1 2 4 temp2 self)
						)
					)
					((gPqFlags test: 39)
						(= temp2 (local2 at: (= temp1 (Random 0 local1))))
...
(instance playSounds of Script ; UNUSED
	(properties)
