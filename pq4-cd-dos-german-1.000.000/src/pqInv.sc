	...
	(method (doVerb theVerb)
		(cond
			...
			(else
				(gEgo put: 15) ; shells
				(= state 8)
				(gMessager say: 15 26 0 0 0 11) ; "You reload your shotgun."
			)
		)
	)
)
...
(instance shells of PQInvItem
	...
	(method (doVerb theVerb)
		(if (== theVerb 23) ; shotgun
			((gInventory at: 12) doVerb: 26) ; shotgun
		else
			(super doVerb: theVerb)
		)
	)
				...
				(cond
					...
					((gPqFlags test: 42)
						(if (& (actionReports value:) $0020)
							...
							(gMessager say: 78 52 0 0 0 11) ; "You fill out follow-up report form 3.14 with information concerning found arm bone."
						)
					)
					((gPqFlags test: 100)
...
(instance photo of PQInvItem
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			(if (or (gCurRoom inset:) (gPqFlags test: 29) (gPqFlags test: 27))
				(gGame points: 2 66)
				(super doVerb: theVerb &rest)
			else
				(gInventory hide:)
				(gGame points: 2 66 handsOff: setScript: (ScriptID 46 0)) ; photoScr
			)
		else
			(super doVerb: theVerb &rest)
		)
