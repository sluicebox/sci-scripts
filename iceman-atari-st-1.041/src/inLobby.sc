...
(instance girlRFeature of RFeature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at]/chair')
				(Print 10 13) ; "A typical island chair."
			)
		)
	)
)
...
(instance clerk of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'ask,get/time')
				(Print 10 14) ; "The only time we have around here is a good time."
			)
				...
				(cond
					...
					((Said 'look[<at]')
						(if (> (gEgo distanceTo: self) 30)
							(Print 10 15) ; "A beautiful native girl is standing behind the counter."
						else
					...
					((Said 'talk')
						(switch (Random 0 2)
							(0
								(Print 10 17) ; "What can I do for you, sir," she asks."
							)
							(1
								(Print 10 18) ; "Hello," the clerk chirps, "Can I help you?"
							)
							(2
								(Print 10 19) ; "Hi there," she says, "How can I help you?"
							)
						)
						(SetScore tahiti 413 32 1)
					)
					((Said 'thank')
						(Print 10 20) ; "You're welcome."
					)
					(
						(or
							(Said 'kiss')
						...
						(Print 10 21) ; "Please," she says, "I hardly know you."
					)
					((Said 'buy,order/drink[<woman,clerk][/woman,clerk]')
						(Print 10 22) ; ""I don't drink on duty," she replies."
					)
					(
						(or
						...
						(Print 10 23) ; "The girl responds, "There is a phone available in your hut, sir.""
					)
				)
			)
			((TurnIfSaid self event '[/headband,message,note]'))
			((Said 'look[<at]/headband')
				(Print 10 24) ; "A wreath made from fresh plumaria blossoms adorns her head."
			)
					...
					((Said 'get,get,read,look[<at]')
						(cond
							...
							(local0
								...
								(Print 10 26) ; "It reads: Call immediately! General Braxton."
								(Print 10 27) ; "You read and discard it."
								(gGame changeScore: 1)
			...
			((or (Said '/bar') (Said '//bar'))
				(Print 10 28) ; "The clerk says, "The bar is through the door to the right.  Check it out, you'll like it.""
			)
			((or (Said '/transport') (Said '//transport'))
				(Print 10 29) ; ""You will find an advertisement on the wall giving you information about our island transportation," She replies."
			)
			((Said 'get/information')
				(Print 10 30) ; ""Our lounge has a nice band playing." she replies."
			)
			((Said 'pay/bill,tab,room')
				(Print 10 31) ; ""That's all taken care of, sir," the clerk responds."
			)
			((Said '/key,room>')
				(cond
					(
	...
	(method (cue)
		...
		(if (and (not (& (tahiti flags:) $0100)) (& (tahiti flags:) $0008))
			(= local0 1)
			(Print 10 32 #time 10) ; "The clerk says to you, "Oh, Mr. Westland, I have an urgent message for you!""
			(Print 10 33) ; ""It's from a General Braxton," she informs you."
...
(instance keyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if register
					(Print 10 34) ; "Yes sir," the clerk says, "I'll have it right here when you need it!"
				)
			...
			(3
				(clerk loop: 0 cel: 0)
				(if (not register)
					(Print 10 35) ; "Yes sir!" she says, "Here you are."
				)
			)
		)
	)
		...
		(cond
			...
			((Said '[/flower,centerpiece,table]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 10 36) ; "There is a flower centerpiece on the table."
					)
					((GoToIfSaid self event self 50 0 10 7))
					((Said 'smell')
						(Print 10 37) ; "These flowers have no odor."
					)
					((Said 'get')
						(Print 10 38) ; "You don't need the flowers. They are for decoration."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/plant,fern]>')
				(cond
					((TurnIfSaid self event 'look/*'))
					((Said 'look[<at]')
						(Print 10 39) ; "There is a tropical fern in the corner of the room."
					)
					((GoToIfSaid self event self 40 0 10 7))
					((Said 'move,get')
						(proc0_39) ; "You don't need to do that."
					...
					((Said 'smell')
						(Print 10 40) ; "It smells like a fern."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/cubbyhole,box,(wall<center,medium,rear)]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'check,look[<at]')
						(Print 10 41) ; "They keep the keys in these boxes."
					)
				)
			)
		)
