...
(instance westGuardScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(EgoDead 918 0 0 87 15) ; "Your hesitation has brought this mission to a premature end."
			)
		)
	)
)
		...
		(cond
			...
			(
				...
				(cond
					...
					(else
						(Print 87 18) ; "You're not holding the gun."
					)
				)
				(event claimed: 1)
			)
			((Said '[/lid,cover]>')
				(if
					...
					(if (gCast contains: foodLidView)
						(Print 87 19) ; "You already did that."
					else
						(self cue:)
					)
				)
			...
			((Said 'get/gun')
				(cond
					...
					(local1
						(Print 87 20) ; "You already have a weapon."
					)
					(else
						(Print 87 21) ; "You'll have to remove the lid first."
					)
				)
			)
		)
...
(instance westShootEgoScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 970 1 0 87 22) ; "Due to of your slow reaction-time, the guard fires his automatic weapon at you and fills you full of holes."
			)
		)
	)
)
...
(instance backGetsShotScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 87 23 #at 10 10) ; "You fire a second tranquilizer dart at the remaining guard."
				(Print 87 24 #at 10 10) ; "The fast-acting drug takes effect as he clutches his chest and slumps to the floor."
				(gGame changeScore: 5)
				(HandsOn)
				(gEgo
					view: 684
...
(instance freeAmbassadorScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 87 25 #at 10 10) ; "You calm the Ambassador as you explain to him your identity."
				(Print 87 26 #at 10 10) ; "Oh my God, man! You don't know how happy I am to see you."
				(gEgo
					view: 684
					loop: 2
					setCycle: Walk
			...
			(7
				...
				(Print 87 27 #at 10 10) ; "He continues, "I was beginning to think this day would never come!""
				(Print 87 28 #at 10 10) ; "Truly," he says, "You are a real American hero!"
				(= seconds 4)
			)
			(8
				(Print 87 29 #at 10 10) ; "The Ambassador says, "I'll be so glad to see my family again.""
				(= seconds 4)
			)
			(9
				(Print 87 30 #at 10 10) ; "John, my boy," he continues, "I am personally going to see to it that you are decorated for bravery!"
				(= seconds 4)
			)
			(10
				(Print 87 31 #at 10 10) ; "That was ssssome tricky shooting, Mr. Westland!"
				(= seconds 3)
			)
			(11
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'bind/man,guard')
				(Print 87 32) ; "No need to do that. They're out cold."
			)
			((Said 'untie/guard')
				(Print 87 33) ; "They aren't tied."
			)
		)
	)
)
...
(instance changeClothesScript of Script
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'exit[/room]') (Said 'go<out'))
				(if (< state 5)
					(Print 87 34) ; "Hold on," the Ambassador says, "I'm not done."
				else
			...
			((Said 'talk/man,ambassador')
				(if (< state 5)
					(Print 87 35) ; "Hold on," the Ambassador says, "I'm almost done."
				else
					(Print 87 36) ; ""Shouldn't we be going," he asks with concern."
				)
			)
		)
	)
...
(instance leaveRoomScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 87 37 #at 10 10) ; ""We'd better get out of here quick!" you urge the Ambassador."
				(HandsOff)
...
(instance tookTooLongScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 87 38 #at 10 10) ; "The Ambassador screams, "WATCH OUT FOR THE GUARD!""
			...
			(3
				(EgoDead 970 1 0 87 39) ; "While you were wasting time, one of the guards revived and took your life."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at]/platter,food')
				(cond
					(local1
						(Print 87 40) ; "Cold food."
					)
					((gCast contains: foodLidView)
						(Print 87 41) ; "Your gun is lying on the platter."
					)
					(else
						(Print 87 42) ; "The platter has its lid on."
					)
				)
			)
			((Said 'get/gun')
				(cond
					...
					((not (gCast contains: foodLidView))
						(Print 87 43) ; "You'll have to remove the lid first"
					)
					(else
		...
		(cond
			...
			((Said '[/guard,man]>')
				(cond
					((Said 'look[<at]')
						(if (== view 787)
							(Print 87 44) ; "He looks pretty ruthless."
						else
							(Print 87 45) ; "Even unconcious, he looks pretty tough."
							(Print 87 46 #icon 387 0 0) ; "You notice he's wearing a ring with a KGB symbol.  "So," you think, "our friends the Russians are in on this!""
			...
			((Said 'look/ring')
				(if (== view 787)
					...
					(Print 87 47 #icon 387 0 0) ; "It has a KGB symbol on it."
				)
			)
			((Said 'get/ring')
	...
	(method (handleEvent event)
		(cond
			...
			((and (Said '[/ambassador,man]>') (Said 'look[<at]'))
				(Print 87 48) ; "He looks a little worn-out from his ordeal."
			)
		)
	)
)
		...
		(cond
			...
			((Said 'give,adjust,drop/tray,food[/table]')
				(if (gCast contains: foodView)
					(Print 87 49) ; "It's already on the table."
				else
					(gEgo setScript: setDownFoodScript)
				)
			)
			((Said 'look[<at][/table]')
				(Print 87 50) ; "It's just a wooden table."
			)
		)
	)
)
		...
		(cond
			...
			((Said '[/wall,painting,mosaic]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 87 51) ; "You see a mosaic on the wall."
					)
				)
			)
		)
