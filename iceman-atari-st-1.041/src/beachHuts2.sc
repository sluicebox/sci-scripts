		...
		(cond
			...
			((and (Said 'look>') (Said '[<around,at][/beach]'))
				(switch (Random 1 5)
					...
					(else
						(Print 5 1) ; "This is the beach with your hut."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/door]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 5 2) ; "This is the door to your hut, #6"
					)
					((Said 'open,unlock')
						(if (gEgo has: 5) ; Tahiti: Key | Sub: Storage_Compartment_Key | Tunisia: Map
							...
							(Print 5 3) ; "You don't have the key."
						)
					)
					((or (Said 'lock') (Said 'lock<to<key<use'))
						(Print 5 4) ; "It's already locked."
					)
					((Said 'knock')
						(Print 5 5) ; "There is no response."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/door]>')
				(cond
					((TurnIfSaid self event '*/*'))
					((Said 'look[<at]')
						(Print 5 6) ; "This is the door to Hut #7."
					)
					((Said 'open,unlock')
						(Print 5 7) ; "This isn't your hut."
					)
					((Said 'knock')
						(Print 5 8) ; "Don't bother."
					)
					((or (Said 'lock') (Said 'lock<to<key<use'))
						(Print 5 4) ; "It's already locked."
					)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/shutter]>')
				(cond
					((Said 'look[<at]')
						(Print 5 9) ; "A hut window."
					)
					((Said 'look<in,through')
						(Print 5 10) ; "The reflection on the window prevents you from looking in."
					)
				)
			)
		...
		(cond
			...
			((Said '[/bush]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 5 11) ; "You see the dense Tahitian jungle."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((and (Said 'look[<at]>') (Said '[/beach]'))
				(Print 5 12) ; "You see more beachfront bungalows further up the beach."
			)
		)
	)
)
		...
		(cond
			...
			((Said 'look[<at]>')
				(cond
					...
					((Said '[/water,bay]')
						(Print 5 13) ; "You see the beautiful Tahitian waters."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/horizon]')
				(Print 5 14) ; "You can see the long beach stretching off towards the horizon."
			)
		)
	)
)
