			...
			((Said '[/chair]>')
				(cond
						...
						(cond
							((== (gEgo view:) 2)
								(if (Said '/*')
									...
									(Print 2 10) ; "Next to it is a small table."
								)
							)
							((== (gEgo view:) 206)
								(Print 2 11) ; "You see a beach chair facing the water."
								(Print 2 10) ; "Next to it is a small table."
								(event claimed: 1)
							)
							(else
								(Print 2 12) ; "Draped across the back of the chair is your shirt."
								(Print 2 10) ; "Next to it is a small table."
								(event claimed: 1)
							)
						)
					...
					((Said 'sit>')
						(cond
							((localproc_1)
								(Print 2 13) ; "You're already sitting."
							)
		...
		(cond
			...
			((Said '[/table,magazine,news,newspaper]>')
				(cond
					...
					((Said 'look[<at,on]')
						(Print 2 15) ; "On the table you see a news magazine."
					)
				)
			)
		)
...
(instance wave of SoundSyncWave
	...
	(method (handleEvent)
		(if (Said 'look[<at][/wave]')
			(Print 2 16) ; "These are gentle waves."
		)
	)
)
		...
		(cond
			...
			((Said '[/boat,dinghy]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 2 17) ; "This is your transportation off the Island."
					)
					((Said 'board,(get<on)')
						(Print 2 18) ; "Just walk aboard."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/man,captain]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 2 19) ; "The captain of your boat."
					)
					((Said 'talk')
						(Print 2 20) ; "We really oughta shove off, bub."
					)
				)
			)
		)
...
(instance finallyOffThisStupidIsland of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(tahiti setScript: 0)
				(Print 2 21) ; "Next stop: Tahiti airport."
				(gEgo
					illegalBits: 0
					ignoreActors: 1
		...
		(if (Said '[/shirt][/!*]>')
			(cond
				((== (gEgo view:) 206)
					(cond
						((Said 'look[<at]')
							(Print 2 22) ; "It's a nice floral aloha shirt."
						)
						((Said '(get<!*),(wear[<!*]),(adjust<on)')
							(Print 2 23) ; "You're already wearing your shirt."
						)
						((TurnIfSaid self event 'look[<at]/*'))
						(
							(GoToIfSaid
				...
				((Said '(detach[<!*]),(get<off)')
					(Print 2 24) ; "You're not wearing it."
				)
				((TurnIfSaid self event 'look[<at]/*'))
				((Said 'look[<at]')
					(Print 2 22) ; "It's a nice floral aloha shirt."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(tahiti flags: (| (tahiti flags:) $4000))
				(self dispose:)
			)
		)
	)
...
(instance readPaperS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 2 25) ; "You read an article featuring the North African country of Tunisia. Because of the global oil shortage, this small third world country finds itself in a powerful political position."
				(Print 2 26) ; "Because of Tunisia's neutral stance, and her overabundance of high grade crude, the United States and Russia find themselves in a tug of war as they negotiate for the oil."
				(Print 2 27) ; "After reading the article you begin to think, "Hmm, what a dangerous, explosive situation this could be.""
