...
(instance firstCodedMsg of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				((ScriptID 27 4) posn: 46 54) ; positionBlip
	...
	(method (handleEvent event)
		(cond
			...
			((and (== state 4) (Said '/attain,acknowledge,confirm<depth'))
	...
	(method (dispose)
		(cond
			...
			((!= (Submarine throttle:) 1)
				(EgoDead 7 0 0 349 29) ; "Ahead slow means AHEAD SLOW."
			)
			((!= (Submarine pitch:) 0)
				(EgoDead 7 0 0 349 30) ; "You should have stabilized the submarine's pitch."
			)
			(else
				(gGame changeScore: 1)
				(subMarine cue:)
