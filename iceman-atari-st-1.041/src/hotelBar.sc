...
(procedure (localproc_0 param1 param2 param3 param4 param5)
	(Print
		...
		(switch argc
			...
			(4
				(+ param2 (Random 0 (- (/ (+ 1 (- param3 param2)) param4) 1)))
			)
			(5
		...
		(cond
			...
			((Said 'sit')
				(cond
					...
					((OneOf (gEgo view:) 511 11)
						(Print 11 1) ; "You are sitting."
					)
					(else
						(event claimed: 0)
					)
			...
			((Said 'stand')
				(if (== (gEgo view:) 206)
					(Print 11 2) ; "You're already standing."
				else
					(proc0_33) ; "Not now!"
				)
			)
			((Said 'drink')
				(if (== (gEgo view:) 206)
					(Print 11 3) ; "Sit down and order a drink first."
				else
					(Print 11 4) ; "Order a drink first."
				)
			)
			((Said 'look[<at,around][/room]')
				(Print 11 5) ; "This is the hotel bar."
			)
			((Said 'dance')
				(Print 11 6) ; "You don't want to dance alone."
			)
			((Said 'open/door')
				(Print 11 7) ; "The door will open when you walk over to it."
			)
			((Said 'ask/woman/date')
		...
		(switch (= state newState)
			...
			(6
				(if (not (& (tahiti flags:) $0010))
					(HandsOff)
					(client setPri: (- (hCPV priority:) 1) setCycle: Beg self)
				else
					(= start 5)
					(self init:)
				)
			...
			((Said 'sit')
				(cond
					...
					((not (gCast contains: local2))
						(Print 11 10) ; "You don't want to sit at this big table all alone."
					)
					((not (& (tahiti flags:) $0800))
						(Print 11 11) ; ""Aren't you at least going to buy me a drink first," the beautiful young lady asks."
					)
					((not (& (tahiti flags:) $1000))
						(Print 11 12) ; "Just a little bold don't you think," she says, " And we haven't even danced yet!"
					)
					(else
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(cond
					((& (tahiti flags:) $0800)
						(Print 11 13) ; ""I still have the one you bought me, thank you," the young lady replies."
					)
					((gEgo script:)
						(Print 11 14) ; "Wait a bit."
					)
					(else
			...
			((Said 'talk')
				(cond
					((not (& (tahiti flags:) $2000))
						(localproc_0 611 16 20)
						(tahiti flags: (| (tahiti flags:) $2000))
					)
					((& (tahiti flags:) $1000)
						(localproc_0 611 25 29)
					)
					(else
						(localproc_0 611 21 24)
					)
				)
			)
		)
...
(instance eSitAS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (gEgo inRect: 203 172 227 178)
					(gEgo setMotion: MoveTo 210 172 self)
				else
					(= cycles 1)
				)
			)
			...
			(2
				(gEgo setMotion: MoveTo (eChair x:) (- (eChair y:) 3) self)
			)
			(3
				(gEgo
					view: 11
			...
			(5
				(Print 11 15) ; ""Would you like to come back to my place," the girl asks."
			)
		)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'buy,order/drink[<woman,stacy][/woman,stacy]')
				(Print 11 16) ; ""I still have the one you bought me, thank you," the girl says."
			)
			((== state 5)
				(cond
					((Said 'yes')
					...
					((Said 'no')
						(Print 11 17) ; ""Well, all right then, maybe some other time," she says in a disappointed tone."
						(local2 setScript: agentHuffScript)
					)
					(else
						(Print 11 18) ; "Answer the question first."
						(event claimed: 1)
					)
				)
			)
...
(instance gADS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 11 19) ; "Oh," she says coyly, "I wouldn't mind sharing a bottle of champagne with you."
				(proc11_2 0)
				(HandsOff)
			...
			(2
				(Print 11 20) ; "Hey, bartender," you say," can I have a bottle of champagne and two glasses?"
			...
			(4
				(Print 11 21) ; ""Oh, thank you," she breathes."
				(Print 11 22) ; "Would you pour me a glass, please?"
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'talk[/woman,woman,lisa]')
				(cond
					((> (gEgo distanceTo: self) 49)
						(event claimed: 0)
					)
					((> (lDS state:) 8)
						(Print 11 23) ; "She's in no shape to talk."
					)
					((== (gEgo view:) 206)
						(self cue:)
						(if (not (cOS client:))
			...
			(
				...
				(cond
					((and (bartender script:) ((bartender script:) next:))
						(Print 11 24) ; "The bartender is busy right now."
					)
					((& (tahiti flags:) $0002)
						(Print 11 25) ; "Don't you think she's had enough?"
					)
...
(instance cOS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 11 26) ; "Her face slightly flushed, the young blonde lady says..."
				(Print 11 27) ; "HI, big fella, sit and join me. I'm drinking Mai Tais."
				(= seconds 10)
			)
			(1
				(Print 11 28) ; "Oh, HI," she says, " Please sit down."
				(= seconds 10)
			)
			(2
				(Print 11 29) ; "Coyly she asks, "Would you like to join me? I'm having a Mai Tai.""
				(= seconds 10)
			)
			(3
				(self init:)
		...
		(cond
			...
			((Said 'help[/woman,lisa]')
				(if (and (>= local1 4) (OneOf state 9 10 11))
					...
					(Print 11 30) ; "At this point she is beyond your help."
				)
			)
			((or (Said '(get<up),stand[<up]') (Said 'exit/stool'))
				(cond
					...
					((& (tahiti flags:) $0002)
						(Print 11 31) ; "Keep your seat, the bartender has already called someone for this inebriated lady."
					)
					(local1
						(Print 11 32) ; "The lady says, "Buy us another round first, big boy.""
						(++ local0)
						(HandsOff)
						(QueScript bartender bDS 0 gEgo)
					)
					((== (gEgo view:) 206)
						(Print 11 33) ; "You are standing."
					)
					(else
			...
			((Said 'look[<at][/woman,lisa]')
				(if (OneOf state 9 10 11)
					(Print 11 34) ; "You see a pathetically drunk young lady."
				else
					(Print 11 35) ; "You see a lady who seems more interested in drinking than talking."
				)
			)
		)
	)
...
(instance eDS of Script
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'drink')
				(if (>= register 5)
					(Print 11 36) ; "Your glass is empty."
				else
					(self init:)
				)
			)
...
(instance eTDS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead 907 1 0 11 37) ; "Driving the porcelain sub, Johnny?"
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(if (== (gEgo view:) 511)
					(HandsOff)
					(QueScript self (Clone bDS) 0 gEgo)
				else
					(Print 11 38) ; "You'll have to find a stool first."
				)
			)
			((> (gEgo distanceTo: self) 120))
			((Said 'ask,get/name[<bartender]')
				(Print 11 39) ; ""I don't socialize with the customers, sir," the bartender replies."
			)
			((Said 'talk/bartender')
				(localproc_0 611 4 8)
			)
			((or (Said 'pay/man,bartender') (Said 'pay/tab'))
				(localproc_0 611 9 10)
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/woman]>')
				(cond
					((Said 'look[<at]')
						(Print 11 40) ; "A woman having a drink."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'talk')
						(Print 11 41) ; "She doesn't answer."
					)
				)
			)
		)
				...
				(cond
					((Said 'look[<at]')
						(if (and (== (husband view:) 11) (== (husband loop:) 5))
							(localproc_0 611 66 67)
						else
							(localproc_0 611 64 65)
						)
					)
					((> (gEgo distanceTo: wife) 49))
					((Said 'talk')
						(localproc_0 611 68 70)
					)
					((or (Said 'dance') (Said 'ask/*/dance'))
						(localproc_0 611 71 76)
					)
					((Said 'kiss,fuck,suck')
						(localproc_0 611 77 79)
					)
					((Said 'buy,order[/woman]/drink')
						(Print 11 42) ; "I already have one, thank you."
					)
				)
			)
			((Said 'ask/woman,name/name[<woman]')
				(Print 11 43) ; "I'm sorry," she tells you, "But I don't divulge that information!"
			)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/man]>')
				(cond
					((Said 'look[<at]')
						(Print 11 44) ; "A man having a drink."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'talk')
						(localproc_0 611 95 107)
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/man,local]')
				(Print 11 45) ; "A local having a drink."
			)
			((Said 'talk[/man,local]')
				(localproc_0 611 95 107)
			)
		...
		(cond
			...
			((Said 'stand,(get<up)')
				(cond
					...
					((and (& (tahiti flags:) $0002) (gCast contains: lush))
						(event claimed: 0)
					)
					((and local0 local1 (gCast contains: lush))
						(event claimed: 0)
...
(instance hCPV of PV
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'sit')
				(if (== (husband view:) 11)
					(localproc_0 611 92 94)
				else
					(localproc_0 611 80 83)
				)
			)
		)
	)
...
(instance lAB of Prop
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/woman]')
				(Print 11 47) ; "A woman sitting at the bar."
			)
			((!= (gEgo view:) 206))
			((> (gEgo distanceTo: self) 40))
			(
...
(instance mAB of RPicView
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/man]')
				(Print 11 48) ; "A man sitting at the bar."
			)
			((Said 'hit/man')
				(Print 11 49) ; "That's not a good idea."
			)
			((> (gEgo distanceTo: self) 40))
			((Said 'talk[/man]')
				(localproc_0 611 95 107)
	...
	(method (handleEvent event)
		(cond
			...
			((Said '[/flower,centerpiece]>')
				(cond
					((Said 'look[<at]')
						(Print 11 50) ; "A small vase with fresh plumaria blossoms is in the center of the table."
					)
					((> (gEgo distanceTo: self) 40))
					((Said 'smell')
						(Print 11 51) ; "The beautiful fragrance of plumaria fills the room."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at,in][/mirror]')
				(Print 11 52) ; "You look into the mirror and see your happy reflection smiling back at you."
			)
