...
(class Inv of IconBar
	...
	(method (advanceCurIcon)
		(if (IsFlag 3)
			(if
				(or
					(== ((gGame script:) state:) 5)
					(== ((gGame script:) state:) 9)
				)
				(super advanceCurIcon:)
			else
				((gGame script:) report:)
			)
		else
		...
		(if (and (IsFlag 3) (gGame script:))
			(cond
				(
					(and
						(== ((gInventory curIcon:) message:) 4)
						(== ((gGame script:) state:) 5)
					)
					((gGame script:) cue:)
				)
	...
	(method (hide &tmp theCurs)
		...
		(if (and (IsFlag 3) (== ((gGame script:) state:) 13))
			((gGame script:) cue:)
		)
	)
)
