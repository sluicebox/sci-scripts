...
(procedure (Print args &tmp theDialog textI iconI editI ret i atX atY fixWidth keepIt default curPort [buttons 6] buttonWide buttonsUsed butAtX [buffer 500])
	(= curPort (GetPort))
	(= atX (= atY -1))
	(= keepIt (= fixWidth (= buttonWide (= iconI (= editI (= buttonsUsed 0))))))
	((= theDialog (Dialog new:)) window: gSpeakWindow name: {PrintD})
	(= textI (DText new:))
	(cond
		((u< [args 0] 1000)
	...
	(textI text: @buffer font: gUserFont setSize:)
	(theDialog add: textI)
	(for ((= i i)) (< i argc) ((++ i))
		(switch [args i]
			(30
	...
	(textI moveTo: ((theDialog window:) hMargin:) ((theDialog window:) vMargin:))
	(if iconI
		(iconI moveTo: ((theDialog window:) hMargin:) ((theDialog window:) vMargin:))
	...
	(for ((= i 0)) (< i buttonsUsed) ((++ i))
		([buttons i] moveTo: butAtX (+ 4 (theDialog nsBottom:)))
		(theDialog add: [buttons i])
		(= butAtX (+ 4 ([buttons i] nsRight:)))
	)
	(theDialog setSize:)
	(theDialog
		nsRight: (+ ((theDialog window:) hMargin:) (theDialog nsRight:))
		nsBottom: (+ ((theDialog window:) vMargin:) (theDialog nsBottom:))
		center:
	)
