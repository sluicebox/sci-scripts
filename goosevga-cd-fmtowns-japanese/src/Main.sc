...
(procedure (proc0_7 param1 param2 &tmp temp0 temp1 temp2 temp3)
	...
	(if (== gEgo param1)
		(cond
			((< temp2 -20)
				((gEgo head:) loop: (+ (* temp1 2) 4))
			)
			((> temp2 20)
				((gEgo head:) loop: (+ (* temp1 2) 6))
			)
			(else
				((gEgo head:) loop: (+ (* temp1 2) 5))
			)
		)
	)
	(if (== argc 3)
	...
	(method (changeScore param1 &tmp temp0 temp1)
		(if (<= gScore 17)
			...
			(for ((= temp0 0)) (< temp0 gScore) ((++ temp0))
				(DrawCel 147 0 8 [global260 temp0] [global280 temp0] 0 -1)
			)
...
(instance stopBut of idiotBut
	...
	(method (doit &tmp temp0 temp1)
		(= temp1 (DButton font:))
		(DButton font: 301)
		(if (Print 0 1 #mode 1 #button {\d4\d2\d9} 1 #button {\c2\c2\de\b9\d9!!} 0) ; "Thanks for playing!"
