...
(class MyLooper of Code
	...
	(method (doit param1 param2 &tmp temp0)
		(if (not (& (param1 signal:) $0800))
			(= temp0
				(if (>= (NumLoops param1) 4)
					4
				else
					(NumLoops param1)
				)
			)
			(= param2 (mod (+ (-= param2 (/ 360 (* temp0 2))) 360) 360))
...
(class Act of Prop
	...
	(method (setDirection dir &tmp temp0)
		(if (== dir 0)
			(self setMotion: 0)
			(return)
		)
	...
	(method (say param1 &tmp [temp0 300] temp300 [temp301 2])
		(= temp300 (DoAudio audPLAY [param1 0]))
		(Timer set60ths: [param1 1] temp300)
		(return temp300)
	)
)
...
(class CDActor of Act
	...
	(method (say param1 &tmp temp0 temp1)
		(= caller [param1 1])
		(if head
			(head syncMouth: 1)
		)
		(if gesture
			(= oldScript script)
			(self setScript: gesture)
		)
		(if global119
			(if head
				(head setCycle: MouthSync [param1 0])
			else
				(self setCycle: MouthSync [param1 0])
			)
			(= global171 [param1 0])
			(= temp0 (DoAudio audPLAY [param1 0]))
		else
			(= temp0 0)
		)
		(Timer set60ths: self temp0)
		(return temp0)
	)
