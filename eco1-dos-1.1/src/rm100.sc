...
(instance tryToGoSouth of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (IsFlag 73)
					(Narrator init: 3 0 0 34 self) ; "That way leads to the kelp maze. There's nothing Adam can do for Delphineus there."
				else
		...
		(switch (= state newState)
			(0
				...
				(if (not (IsFlag 73))
					...
					(Narrator init: 3 0 0 35 self) ; "There's nothing Adam can do to help Delphineus on the ocean's surface."
				)
			)
			(1
				(gEgo
...
(instance delTalkScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				((ScriptID 2 0) init: 1 0 0 38 1 self) ; Delphineus, "C'mon! Let's go east to the reef. We already done all we can for the city."
			)
			(1
				(HandsOn)
				(self dispose:)
...
(instance BigHead of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 2) ; "Perhaps Adam should swim closer to the head first."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 2) ; "Perhaps Adam should swim closer to the head first."
			)
			...
			(5 ; Talk
				(Narrator init: 3 0 0 5) ; "There is no reply from the giant head."
			)
			(else
...
(instance eluriaCity of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(10 ; Recycle
				(Narrator init: 3 0 0 4) ; "If Adam wants to clean up Eluria, he'll have to get closer."
			)
			(5 ; Talk
				(Narrator init: 3 0 0 6) ; "Adam can't talk to the city."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 7) ; "Adam can't do anything to the city from here."
			)
			(3 ; Do
				(Narrator init: 3 0 0 16) ; "Just swim there."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance eluriaOutskirts of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(5 ; Talk
				(Narrator init: 3 0 0 19) ; "Adam can't talk to the ruins surrounding Eluria."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 18) ; "Adam can't do anything with the ruins around Eluria."
			)
			(10 ; Recycle
				(Narrator init: 3 0 0 22) ; "Adam can't clean up the ruins or the spires of rock."
			)
			(3 ; Do
				(Narrator init: 3 0 0 17) ; "The ruins surrounding Eluria have been long abandoned and are of no interest to Adam or the Elurians."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance closeColumn of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 12) ; "The columns are merely silent sentinels and are of no use to Adam."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 12) ; "The columns are merely silent sentinels and are of no use to Adam."
			)
			(5 ; Talk
				(Narrator init: 3 0 0 13) ; "The columns stonily refuse to reply."
			)
			(10 ; Recycle
				(Narrator init: 3 0 0 14) ; "The columns do not need to be thrown away."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance leaves of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 9) ; "Adam can't take the kelp. It's a living plant."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 10) ; "There's nothing Adam can to do with the kelp."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance pillars of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 21) ; "Adam can't do anything with the rock spires that hold up Eluria - nor would he want to."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 21) ; "Adam can't do anything with the rock spires that hold up Eluria - nor would he want to."
			)
			...
			(5 ; Talk
				(Narrator init: 3 0 0 23) ; "Adam can't talk to the rock spires that hold up Eluria."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance coral of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 25) ; "The coral is sharp and would cut Adam's hands."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 26) ; "Adam doesn't want to disturb the natural coral."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance soldierMask of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 30) ; "The helmet is so overgrown with moss that it's part of the landscape now. Adam decides to leave it alone."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 31) ; "Adam doesn't need to do anything with the helmet."
			)
			(10 ; Recycle
				(Narrator init: 3 0 0 32) ; "The helmet is part of the ancient history of Eluria and doesn't need to be thrown away."
			)
			(5 ; Talk
				(Narrator init: 3 0 0 33) ; "The helmet has nothing to say."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance moss of Feature
	...
	(method (doVerb theVerb invItem)
		(if (OneOf theVerb 4 3) ; Inventory, Do
			(Narrator init: 3 0 0 28) ; "Adam can't do anything with the green moss."
		else
