	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(6
				...
				(gEgo get: 6 put: 6 360) ; frisbee, frisbee
				((ScriptID gCurRoomNum 3)
					view: 384
					setLoop: 0
					cel: 0
			...
			(8
				(if (> (++ register) 29)
					(if (-- local0)
						...
						(-= state 2)
						(if (== local0 1)
							(Narrator init: 6 0 0 14) ; "Never saw a dolphin do that before? That's because Adam's imagining it and so are you. Maybe you should get more sleep!"
						)
					)
				else
					(-= state 2)
