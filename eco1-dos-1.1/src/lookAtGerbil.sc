...
(instance cageView of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 69)
					(Narrator posn: 10 140 init: 3 0 0 65) ; "The gerbil is really getting that heart rate up!"
				else
					(Narrator posn: 10 140 init: 3 0 0 10) ; "Adam's gerbil lives in this cage."
				)
			)
			(3 ; Do
				(if (IsFlag 69)
					(Narrator posn: 10 140 init: 3 0 0 64) ; "Adam shouldn't interfere with the gerbil's aerobic workout."
				else
					(Narrator posn: 10 140 init: 3 0 0 56)
				)
			)
			(4 ; Inventory
				(cond
					...
					(else
						(Narrator posn: 10 140 init: 3 0 0 82)
					)
				)
			)
			(else
...
(instance waterGerbil of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator posn: 10 140 init: 3 0 0 93 self 400)
			)
			(2
