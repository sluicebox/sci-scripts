...
(instance easternCliffs of PicView
	...
	(method (doVerb theVerb)
		(if (== theVerb 2) ; Look
			(Narrator init: 1 0 0 3 0 320) ; "Rocky cliffs mark the coastline here."
		else
...
(instance ship of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 1 0 0 6) ; "Adam can't swim that fast."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sky of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 1 0 0 5) ; "Adam can't get there from here."
			)
			(else
				(super doVerb: theVerb &rest)
			)
