...
(instance RM561 of EcoRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 1 0 0 32) ; "The level of chemicals here make it unsafe to touch anything."
			)
			(10 ; Recycle
				(Narrator init: 1 0 0 43) ; "Garbage bagging this is way beyond Adam's skill."
			)
			(else
...
(instance fromAlgaeCaveScript of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 1 0 0 1 store: 36) ; "A group of metal drums lie rusting in this hidden spot! A familiar greenish glow seeps from rusting patches in the metal."
				(= cycles 1)
			)
			(2
				(self dispose:)
...
(instance attachCable of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 1 0 0 8 self) ; "Adam attaches the cable to the barrel. It drifts aimlessly in the current."
			)
			(2
...
(instance attachCableAndFloat of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 1 0 0 9 self) ; "Adam attaches the cable to the barrel. The float rises silently to the surface."
			)
			(2
				(= seconds 1)
			)
			(3
				(Narrator init: 1 0 0 10 self) ; "The float is bobbing on the surface of the water. However, it's unlikely to summon help on its own."
			)
			(4
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (IsFlag 331)
					(= register 1)
					(Narrator init: 1 0 0 11 self) ; "Adam retrieves the cable and float."
				else
					(= register 0)
					(Narrator init: 1 0 0 12 self) ; "Adam retrieves the cable."
				)
			)
			(3
				(gEgo
...
(instance attachCableAndXmitter of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 1 0 0 20 self) ; "Adam turns on the transmitter and attaches his improvised satellite buoy to the barrel! The float rises toward the surface. The transmitter is emitting a constant, powerful signal."
			)
			(2
				(= seconds 2)
			)
...
(instance cable of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 1 0 0 (if (IsFlag 331) 10 else 21))
			)
			(3 ; Do
			...
			(4 ; Inventory
				(switch invItem
					(32 ; transmitter
						(if (not (IsFlag 331))
							(Narrator init: 1 0 0 13) ; "Adam needs to retrieve the cable first."
						else
							(Narrator init: 1 0 0 16) ; "Adam needs to retrieve the float first."
						)
					)
					(27 ; float
						(Narrator init: 1 0 0 14) ; "Adam needs to retrieve the cable first."
					)
					(else
...
(instance deadFish of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Narrator init: 1 0 0 (if (Random 0 1) 6 else 29))
			)
			(3 ; Do
				(Narrator init: 1 0 0 (if (Random 0 1) 7 else 30))
			)
			(10 ; Recycle
				(gCurRoom doVerb: 10)
			)
...
(instance deadPlant of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 1 0 0 31) ; "The plants have absorbed a high level of toxin. It would be unwise to touch them."
			)
			(10 ; Recycle
				(gCurRoom doVerb: 10)
			)
...
(instance stalagmite of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(2 ; Look
				(Narrator init: 1 0 0 lookStr 0 560)
			)
			(10 ; Recycle
				(gCurRoom doVerb: 10)
			)
...
(instance barrels of Feature
	...
	(method (doVerb theVerb invItem)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 1 0 0 5) ; "The barrels are rusting. It would be unsafe to touch them."
			)
			(10 ; Recycle
				(gCurRoom doVerb: 10)
			)
			(4 ; Inventory
				(switch invItem
					...
					(32 ; transmitter
						(Narrator init: 1 0 0 37) ; "These barrels don't need any more leaks!"
					)
					(13 ; sharpShell
						(Narrator init: 1 0 0 37) ; "These barrels don't need any more leaks!"
					)
					(17 ; spine
						(Narrator init: 1 0 0 38) ; "The spine carries a natural poison - but it won't cancel out the chemical poisons found here."
					)
					(21 ; healingPotion
						(Narrator init: 1 0 0 39) ; "Demeter's healing potion is powerless against such strong toxins."
					)
					(27 ; float
						(Narrator init: 1 0 0 40) ; "That's a good idea, but Adam would need to attach it to something first."
					)
					(30 ; hackSaw
						(Narrator init: 1 0 0 41) ; "Liberating all those chemicals into the water wouldn't be helpful."
					)
					(12 ; trident
						(Narrator init: 1 0 0 37) ; "These barrels don't need any more leaks!"
					)
					(else
						(Narrator init: 1 0 0 42) ; "The barrels are hopelessly contaminated. That won't help."
					)
				)
			)
			(else
