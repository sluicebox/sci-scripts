...
(instance rm225 of ApartmentRoom
	...
	(method (notify)
		(cond
			...
			((IsFlag 43)
				(Narrator init: 3 0 0 96) ; "Gregarious isn't here. He's still waiting at the surface for Adam to take care of those propellers."
			)
...
(instance gregSwimToSurface of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				((ScriptID 2 1) init: 1 0 0 29 1 self) ; Adam, "How about if I go up with you? You can show me the boat while we're up there. Maybe I can do something about the propellers."
			)
			(9
		...
		(switch theVerb
			(2 ; Look
				(if (== ((Inv at: 10) owner:) 335) ; bikeCage
					(Narrator init: 3 0 0 78) ; "Gregarious looks a lot happier now that he doesn't have to worry about those propellers."
				else
					(Narrator init: 3 0 0 6) ; "The manatee is absolutely covered with bandages! He's also turning blue."
				)
			)
			(3 ; Do
				(Narrator init: 3 0 0 79) ; "With all of his cuts and bandages, Gregarious might not appreciate being touched."
			)
			(5 ; Talk
				(if (== ((Inv at: 10) owner:) 335) ; bikeCage
					(Gregarious init: 2 0 0 8 1) ; "There're still some citizens in the apartments, Adam. Check the ones with that lights <a-hyuh>."
				else
...
(instance tools of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 94) ; "Gregarious needs his tools in his job as handyman for the apartment. Adam doesn't want to take them."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 98) ; "Adam doesn't want to mess around with Gregarious' things."
			)
			(10 ; Recycle
				(Narrator init: 3 0 0 99) ; "Gregarious doesn't need his things cleaned up, thanks."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chest of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 80) ; "The toolbox contains Gregarious' tools for working around the apartments. There's nothing in there that Adam needs."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 98) ; "Adam doesn't want to mess around with Gregarious' things."
			)
...
(instance saw of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Narrator init: 3 0 0 97) ; "Some of Gregarious' tools are lying on the floor. They look rather dull."
			)
			(3 ; Do
				(Narrator init: 3 0 0 94) ; "Gregarious needs his tools in his job as handyman for the apartment. Adam doesn't want to take them."
			)
...
(instance portraits of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 92) ; "Adam doesn't want to take the pictures of Gregarious' parents. They have faces only a son could love."
			)
			(4 ; Inventory
				(Narrator init: 3 0 0 98) ; "Adam doesn't want to mess around with Gregarious' things."
			)
