...
(instance rm360 of Rm
	...
	(method (init)
		(LoadMany rsVIEW 360 364 40 361 362 956 363 342 365)
	...
	(method (doit &tmp [temp0 60] [temp60 20] [temp80 20] [temp100 20])
		...
		(return
			(if local19
				(Message msgGET 360 1 0 0 1 @temp0) ; "Do you wanna skip the beginning cartoon or watch the whole thing?"
				(Message msgGET 360 1 0 0 2 @temp60) ; "Skip it"
				(Message msgGET 360 1 0 0 3 @temp80) ; "Show it to me!"
				(Message msgGET 360 1 0 0 4 @temp100) ; "Restore a game"
				(Sound pause: 1)
		...
		(switch (= state newState)
			(0
				...
				(if (> global102 16)
					...
					(gCurRoom drawPic: 360 100)
				)
				(if (not (> global102 16))
					(PalVary pvINIT 361 0)
					(= local23 1)
				)
				(= cycles 1)
			)
...
(instance dolphinNet of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				(if (or (>= (net y:) 27) (< (gLongSong prevSignal:) 10))
