...
(instance enterRmScript of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					...
					((and (== gPrevRoomNum (gCurRoom south:)) (not (IsFlag 124)))
						(Narrator init: 1 0 0 1 self) ; "Following the taut cable up and up, towards the surface light..."
					)
					((== gPrevRoomNum (gCurRoom south:))
						(Narrator init: 1 0 0 4 self) ; "Adam follows the cable up to King Cetus."
					)
					(else
						(Narrator init: 1 0 0 2 self) ; "Adam follows the cable back down to the ship."
					)
				)
			)
			(2
			...
			(3
				(if (not (SetFlag 124))
					(Narrator init: 1 0 0 3 self) ; "Up and up, not looking where he's going..."
				else
					(= cycles 1)
				)
			)
