		...
		(switch theVerb
			(2
				(if (theObj lookStr:)
					...
					(Printf 950 0 objDesc objDesc) ; "The %s looks like any other %s."
				)
			)
			(4
				(if (= iItem (gInventory at: invNo))
					(Printf 950 1 (iItem description:) objDesc) ; "You clicked inv item %s on %s."
				)
			)
			(5
				(Printf 950 2 objDesc) ; "The %s has nothing to say."
			)
		)
	)
)
