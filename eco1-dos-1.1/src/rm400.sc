	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(4 ; rag
						(Narrator posn: 160 20 init: 3 0 0 95) ; "That doesn't need to be cleaned!"
					)
					(3 ; beaker
						(Narrator posn: 160 20 init: 3 0 0 96) ; "There's no oil there! Adam decides to save the fertilizer solution for something that really needs it."
					)
					(else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(4 ; rag
						(Narrator posn: 160 20 init: 3 0 0 74) ; "Dad's ears are already clean, thank you very much!"
					)
					(3 ; beaker
						(Narrator posn: 160 20 init: 3 0 0 96) ; "There's no oil there! Adam decides to save the fertilizer solution for something that really needs it."
					)
...
(instance jaw of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(10 ; Recycle
				(Narrator posn: 160 20 init: 3 0 0 66) ; "Adam uses this bag for collecting trash when he dives, not for dealing with trash at home."
			)
			(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(if (IsFlag 127)
			...
			(switch theVerb
				...
				(3 ; Do
					(Narrator posn: 160 20 init: 3 0 0 31) ; "The door locks electronically. Something in the next room must require security."
				)
				(else
					(super doVerb: theVerb invItem &rest)
				)
...
(instance envelope of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(if (IsFlag 64)
					(Narrator posn: 160 20 init: 3 0 0 54) ; "The envelope is torn and useless. It's been forwarded too many times."
				else
					(Narrator posn: 160 20 init: 3 0 0 27) ; "The mail has arrived! The letter is addressed to Adam, with many forwarding addresses."
				)
			)
			(10 ; Recycle
				(Narrator posn: 160 20 init: 3 0 0 66) ; "Adam uses this bag for collecting trash when he dives, not for dealing with trash at home."
...
(instance book of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 70) ; "Adam doesn't have time to look at the hint book. (Rats!)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
			...
			(switch theVerb
				...
				(3 ; Do
					(if (gCast contains: father)
						(Narrator posn: 160 20 init: 3 0 0 53) ; "Bacteria from the fertilizer solution breaks down the oil into harmless chemicals."
					else
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 60) ; "After all the trouble his Dad had putting it together, Adam thinks better of touching it."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 55) ; "The chair is fine where it is."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance cabinet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 61) ; "Adam doesn't need any of this old junk."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance labTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 57) ; "The water in the aquarium would slosh over the top if Adam moved it."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance coatRack of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 58) ; "The coat rack is fine where it is."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance labWindow of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Narrator posn: 160 20 init: 3 0 0 59) ; "The window is painted shut."
			)
			(else
				(super doVerb: theVerb &rest)
			)
			...
			(5 ; Talk
				...
				(cond
					((InRect 203 148 242 163 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 76) ; "Zdravietey!"
					)
					((InRect 185 148 202 156 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 77) ; "Bonjour!"
					)
					((InRect 141 150 169 161 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 81) ; "You talkin' to ME?"
					)
					((InRect 135 162 172 176 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 78) ; "Buenos dias!"
					)
					((InRect 243 153 257 161 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 80) ; "Konichiwa!"
					)
					((InRect 126 151 140 159 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 83) ; "Go surfin', dude."
					)
					((InRect 237 161 261 173 gMouseX gMouseY)
						(Narrator posn: 160 20 init: 3 0 0 84) ; "G'day"
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance startUpStuff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Father init: 1 0 0 2 1) ; "Why not take a break?"
...
(instance bounceFromDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Narrator posn: 160 20 init: 3 0 0 25 self) ; "Adam doesn't know the kids who are playing outside. He'd rather stay here."
			)
			(1
...
(instance getBag of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(SetScore 10 207)
				(Narrator posn: 160 20 init: 3 0 0 69 self) ; "Adam picks up the garbage bag. He plans to use it to pick up trash when he goes diving later."
			)
			(2
