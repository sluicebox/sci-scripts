...
(instance rm341 of EcoRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Narrator init: 1 0 0 4) ; "Adam is swimming underwater between Eluria and the fisherman's boat."
			)
			(3 ; Do
				(Narrator init: 1 0 0 5) ; "There's nothing to do with the water except what Adam is already doing - swimming in it."
			)
			(4 ; Inventory
				(Narrator init: 1 0 0 5) ; "There's nothing to do with the water except what Adam is already doing - swimming in it."
			)
...
(instance swimThru of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Narrator init: 1 0 0 1) ; "A load of garbage appears from somewhere above."
			)
			(3
...
(instance cantGoThatWay of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 1 0 0 3 self) ; "The ocean is a huge place. Adam should stick to the path Gregarious showed him between Eluria and the boat."
			)
			(2
				(if (== (gEgo edgeHit:) 4)
					(gEgo
