...
(instance rm335 of EcoRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 14) ; "Adam is swimming on the ocean's surface above Eluria. A small fishing boat is anchored nearby."
			)
			(10 ; Recycle
				(if (> ((User curEvent:) y:) 67)
					(Narrator init: 4 0 0 12) ; "Hey! Not trashing the ocean is what this game's all about!"
				else
...
(instance fromBelow of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Narrator init: 4 0 0 26 self) ; "Adam resurfaces to find Gregarious and the fisherman still eyeing each other suspiciously."
			)
			(4
...
(instance talkToFisherman of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				((ScriptID 2 1) init: 2 0 0 4 1 self) ; Adam, "We just swam up through some of your trash. Lots of creatures have to live in the ocean. You shouldn't dump stuff in here, you know."
			)
			(9
...
(instance useSteelCage of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Narrator init: 4 0 0 29 self) ; "Adam and Gregarious swim under the boat to attach the cage to the propellers."
			)
			(4
				(= cycles 5)
			)
			...
			(11
				(Narrator posn: -1 15 init: 4 0 0 1 self) ; "With Gregarious' help, Adam fits the steel cage over the propellers and fastens it on with the four screws."
			)
			(12
				(SetScore 5 299)
				(INSET dispose:)
...
(instance cantGoThatWay of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator init: 4 0 0 2 self) ; "The ocean is a huge place. Adam should stick close to the boat or he might get lost."
			)
			(2
				(cond
					((== (gEgo edgeHit:) 4)
...
(instance cageOnFisherman of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (IsFlag 34)
					((ScriptID 2 1) init: 2 0 0 10 1 self) ; Adam, "You know, this cage is like the ones my Dad uses to cover the propellers on our lab boats. I bet it would work just as well on your propeller blades."
				else
...
(instance propeller of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 21) ; "The boat has a large motor attached to its stern. Judging from Gregarious' scars, the underwater blades are uncovered and are dangerously sharp! The motor is currently off."
			)
			(3 ; Do
				(Narrator init: 4 0 0 22) ; "Adam can't stop the propellers from hurting surface mammals by covering them up with his hands!"
			)
			(10 ; Recycle
				(Narrator init: 4 0 0 9) ; "Adam can't dispose of Gregarious' problem as easily as that!"
			)
			(4 ; Inventory
				(switch invItem
					(10 ; bikeCage
						(if (IsFlag 42)
							(if (IsFlag 34)
								...
								(Narrator init: 4 0 0 31) ; "Perhaps Adam should talk to the fisherman before making changes to his boat."
							)
						else
							(Narrator init: 4 0 0 4) ; "Good idea, but Adam needs some way to fasten the cage to the boat."
						)
					)
					(18 ; screws
						(Narrator init: 4 0 0 30) ; "If Adam wants to attach something to the propellers with the screws, he'll need to attach the screws to the object first."
					)
					(12 ; trident
						(Narrator init: 4 0 0 23) ; "Adam needs to make the propellers safe, not break them!"
					)
					(16 ; waterPump
						(Narrator init: 4 0 0 23) ; "Adam needs to make the propellers safe, not break them!"
					)
					(else
						(Narrator init: 4 0 0 24) ; "That won't make the propellers safe for surface-floating mammals."
					)
				)
			)
			(else
...
(instance rope of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 18) ; "The fishing boat is anchored."
			)
			(3 ; Do
				(Narrator init: 4 0 0 19) ; "Adam doesn't need to take the boat's anchor."
			)
			(4 ; Inventory
				(Narrator init: 4 0 0 20) ; "The boat's anchor is of no interest to Adam."
			)
			(else
...
(instance boat of PicView
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 13) ; "The boat that's terrorized Gregarious is only a little fishing vessel, but the danger its exposed propellers can create for surface-floating mammals is no small matter."
			)
			(3 ; Do
				(Narrator init: 4 0 0 7) ; "Adam is on a mission and has no desire to desert Eluria and ride away in the fisherman's boat."
			)
			(10 ; Recycle
				(Narrator init: 4 0 0 9) ; "Adam can't dispose of Gregarious' problem as easily as that!"
			)
			(4 ; Inventory
				(switch invItem
					(10 ; bikeCage
						(Narrator init: 4 0 0 3) ; "The cage will only be useful if put over the propellers."
					)
					(18 ; screws
						(Narrator init: 4 0 0 17) ; "If Adam wants to attach something to the boat with the screws, he'll need to attach the screws to the object first."
					)
					(else
						(Narrator init: 4 0 0 15) ; "That won't make the boat manatee-safe."
					)
				)
			)
			(else
...
(instance fisherman of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 28) ; "The fisherman has a face as tanned and wrinkled as old leather. Though he looks as tough and steady as a rock, he doesn't appear to be particularly unkind or spiteful."
			)
			(3 ; Do
				(Narrator init: 4 0 0 6) ; "The fisherman looks like a tough old guy and Adam's not sure he wants to tangle with him. Besides, there's got to be a better solution than that."
			)
			(5 ; Talk
				(if (not (IsFlag 34))
			...
			(10 ; Recycle
				(Narrator init: 4 0 0 11) ; "While the fisherman is ecologically careless, he's hardly trash."
			)
			(4 ; Inventory
				(switch invItem
					(10 ; bikeCage
					...
					(17 ; spine
						(Narrator init: 4 0 0 27) ; "Don't poison the fisherman! He may litter the oceans, but that's no reason to get nasty!"
					)
					(else
						(switch (Random 0 1)
							(0
...
(instance manatee of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 4 0 0 25) ; "Gregarious eyes the boat warily - but at least he's no longer blue."
			)
			(3 ; Do
				(Narrator init: 4 0 0 8) ; "The manatee's skin is sensitive with all of the cuts. You'd better not touch him."
			)
			(5 ; Talk
			...
			(10 ; Recycle
				(Narrator init: 4 0 0 10) ; "Adam is here to help Gregarious, not insult him!"
			)
			(4 ; Inventory
				(switch invItem
					(10 ; bikeCage
					...
					(else
						(Gregarious init: 1 0 0 4) ; "That's real purty, Adam, but I'm not sure what ta do with it."
					)
				)
			)
			(else
