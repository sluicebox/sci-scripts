		...
		(switch (= state newState)
			...
			(3
				(if (IsFlag 126)
					(if (>= register 0)
						...
						(Narrator posn: -1 140 init: 3 0 0 18 self) ; "Adam's dad found a seagull coated with oil."
					)
				else
			...
			(4
				(if (or (IsFlag 126) (>= register 0))
					...
					((ScriptID 400 9) init: 1 0 0 8 0 self) ; Father, "I found another bird this morning. These oil spills make me so mad I can't see straight."
					(SetFlag 126)
				)
			)
			(5
...
(instance cleanBird of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				((ScriptID 400 9) init: 1 0 0 27 0 self) ; Father, "He might. Depends how much of a shock he's had. We're doing all we can."
			)
			(12
			...
			(19
				((ScriptID 2 1) init: 2 0 0 6 1 self) ; Adam, "I wish there was something more to do."
			)
			(20
			...
			(37
				((ScriptID 400 9) init: 1 0 0 25 0 self) ; Father, "Yeah, go ahead. Just take a look at my instructions on the blackboard in the treatment room first, though. We have to make sure we're all doing the same thing with him."
			)
			(38
...
(instance getRag of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator posn: -1 140 init: 3 0 0 72 self 400) ; "Adam takes the clean rag."
			)
			(2
				((ScriptID 400 4) dispose: delete:) ; rag
				(SetScore 2 200)
...
(instance getDetergent of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Narrator posn: -1 140 init: 3 0 0 73 self 400)
			)
			(2
				((ScriptID 400 5) dispose: delete:) ; dishSoap
				(SetScore 2 201)
...
(instance bird of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if (IsFlag 10)
					(Narrator posn: -1 140 init: 3 0 0 51)
				else
					(Narrator posn: -1 140 init: 3 0 0 4) ; "Touching the bird would upset it more."
				)
			)
			(2 ; Look
				(if (IsFlag 10)
					(Narrator posn: -1 140 init: 3 0 0 19) ; "Adam will take care of the bird until its natural oils coat its feathers. Then the bird will be able to float again."
				else
					(Narrator posn: -1 140 init: 3 0 0 1) ; "The seagull is covered with oil and shivering miserably."
				)
			)
			(5 ; Talk
			...
			(4 ; Inventory
				(switch invItem
					...
					(5 ; dishSoap
						(Narrator posn: -1 140 init: 3 0 0 88)
					)
					(else
...
(instance tRag of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator posn: -1 140 init: 3 0 0 lookStr self)
			)
			(3 ; Do
