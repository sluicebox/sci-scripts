...
(procedure (DoPrint)
	(Print &rest #title {Polygon Editor 1.11} #font 999 #width 240)
)
...
(class _EditablePolygon of List
	...
	(method (check &tmp node nextNode point nextPoint angle delta firstNode totAngle firstTime lastAngle backNode backPoint savX savY lastNode cmpNode cmpPoint)
		...
		(cond
			...
			((!= totAngle 360)
				(Printf 943 1 name totAngle) ; "Error in (%s check:) totAngle [%d] != +-360"
			)
		)
	)
...
(class PolyEdit of List
			...
			(evKEYBOARD
				...
				(switch (event message:)
					...
					(KEY_h
						(switch state
							(0
								(DoPrint 943 3 27 1) ; "CREATING POLYGON  Click to create each corner of the polygon, then choose Done from the menu to finish. You can also press Esc or Ctrl-click to finish.  To UNDO a corner, choose Undo.  To change MAP displayed (visual or control), choose Map.  To EXIT the Polygon Editor, choose eXit or press Ctrl-S."
							)
							(1
								(DoPrint 943 4) ; "EDITING POLYGON  To MOVE a corner, click on it and drag it to the new position. To INSERT a new corner, Ctrl-click to create it, then drag it to the correct position. To DELETE a corner, Shift-click on it. To UNDO an action, choose Undo from the menu. To CREATE a new polygon, choose Create. To change a polygon's TYPE (Total, Near or Barred), choose Type. To change MAP displayed (visual or control), choose Map. To EXIT the Polygon Editor, choose eXit or press Ctrl-S.  In addition to using the mouse, you can use Space and BackSpace to select corners and Tab and BackTab to select polygons."
							)
						)
					)
					(KEY_u
					...
					(KEY_a
						(DoPrint 943 5 27 1) ; "by  Mark Wilden  Original program by Chad Bye"
					)
					(KEY_r
						(if (== state 1)
							(self draw:)
	...
	(method (readObstacles)
		(if (gCurRoom obstacles:)
			((gCurRoom obstacles:) eachElementDo: #perform readObstacle)
		)
	)
	...
	(method (exit &tmp [str 100] rc file fileMode)
		...
		(if (not fileName)
			(Format @fileName 943 7 (gCurRoom curPic:)) ; "polygons.%d"
		)
		(if
			(not
				(= rc
		...
		(if
			(and
				...
				(not
					(= rc
						(PrintD
							(Format @str 943 8 @fileName) ; "The file '%s' already exists"
							109
							78
							{Replace}
							1
		...
		(if (not ((= file (File new:)) name: @fileName open: fileMode))
			(Printf 943 9 (file name:)) ; "Error opening '%s'"
			(file dispose:)
			(return 0)
		)
		(file writeString: (Format @str 943 10 {Polygon Editor 1.11})) ; "; %s"
		(file
			writeString:
