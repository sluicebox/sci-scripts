...
(procedure (localproc_0 &tmp temp0)
	...
	(cond
		((not (IsFlag 324))
			(PalVary pvINIT 543 0)
		)
		(local81
			(cond
				((< global244 1)
					(PalVary pvINIT 542 0)
				)
				((< global244 2 13)
					(PalVary pvINIT 541 0)
				)
			)
			(= local81 0)
		)
		...
		(cond
			...
			((and (not local77) (>= global244 14))
				(if (== north 561)
					(= local77 1)
					(= local79 1)
					((ScriptID 2 0) init: 1 0 0 9 1) ; Delphineus, "No! It's too dangerous to go in there. I can sense things you can't and I have a very bad feeling about this! You need more protection to swim into water like that."
				else
			...
			((not local79)
				(= local79 1)
				(Narrator init: 3 0 0 20) ; "The opening still isn't big enough to swim through."
			)
		)
		(super doit: &rest)
			...
			(2 ; Look
				(if (IsFlag 324)
					(if (< ((User curEvent:) x:) 180)
						(Narrator init: 3 0 0 2) ; "The eerie glow from the flashlight fish reveals a small cave. There seems to be an opening to the northeast."
					else
						(Narrator init: 3 0 0 3) ; "Some sort of soft algae has grown all over the walls of the cave. It has an unpleasant smell."
					)
				else
					(Narrator init: 3 0 0 17) ; "Adam can't see anything clearly."
				)
			)
			(3 ; Do
				(if (IsFlag 324)
					...
					(Narrator init: 3 0 0 25) ; "Adam doesn't want to put his hand there unless he can see what he's touching first."
				)
			)
			(4 ; Inventory
				(cond
					...
					(else
						(Narrator init: 3 0 0 33) ; "That's not very illuminating."
					)
				)
			)
			(5 ; Talk
				(cond
					((IsFlag 324)
						(Narrator init: 3 0 0 39) ; "The flashlight fish doesn't answer."
					)
					((IsFlag 73)
						(super doVerb: 5)
					)
...
(instance releaseFish of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Narrator init: 3 0 0 27 self) ; "Adam holds out the glowing jar. The flashlight fish senses the friendly dark of the cave."
			)
			(3
				(SetScore 10 324)
			...
			(4 0)
			(5
				(Narrator init: 3 0 0 28 self) ; "Gathering courage, it swims out of the jar and settles down between two rocks."
			)
			(6
				(= seconds 2)
			)
			...
			(8
				(Narrator init: 3 0 0 42 self) ; "Adam puts the empty jar into his recycle bag."
			)
			(9
				(gEgo put: 23) ; jar
				(gFeatures
...
(instance scareDel of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Narrator init: 3 0 0 26 self) ; "Adam's voice echoes weirdly in the dark cave."
			)
			(1
...
(instance foundToxicWaste of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				((ScriptID 2 1) init: 2 0 0 49 1 self) ; Adam, "Somebody's dumped toxic waste in a whole bunch of barrels. It's incredible - everything in there is dead and the barrels are leaking."
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(switch global244
					(1
						(Narrator init: 3 0 0 31 self) ; "Adam reaches up and pulls on one of the brownish rocks. It falls easily to the floor of the cave. A strange, green light seeps through the small hole."
					)
					(2
						(Narrator init: 3 0 0 19 self) ; "Another rock pulls out easily. It must not have been there very long."
					)
					(4
						(self setScript: easyRocks self)
					)
					(7
						(Narrator init: 3 0 0 32 self) ; "The greenish color of the water intensifies as the rocks come out."
					)
					(10
...
(instance showGlint of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (not (IsFlag 329))
					(Narrator init: 3 0 0 5 self) ; "The light strikes something hidden behind the rocks."
				else
					(= ticks 2)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if (IsFlag 147)
					(+= state 2)
					(Narrator init: 3 0 0 43 self) ; "Adam puts on the protective suit."
				else
					(SetFlag 147)
					(Narrator init: 3 0 0 14 self) ; "Adam feels an immediate relief. Whatever was in the water was making it really hard to think."
				)
			)
			(5
				(if
			...
			(6
				(Narrator init: 3 0 0 15 self) ; "Adam signals "okay" to Delphineus."
			)
			(7
				(gEgo
					view: 542
...
(instance chestClosedTalk of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Narrator posn: -1 140 init: 3 0 0 6 self) ; "A metal box has been concealed in the pile of rocks."
			)
			(1
			...
			(switch theVerb
				(3 ; Do
					(cond
						...
						((> y 130)
							(Narrator init: 3 0 0 22) ; "Adam has no use for the rocks."
						)
						(else
				...
				(2 ; Look
					(cond
						((> y 130)
							(Narrator init: 3 0 0 21) ; "The rocks that were clogging the hole have fallen to the floor of the cave."
						)
						(global244
							(Narrator init: 3 0 0 40) ; "There are still some rocks clogging the way out."
						)
						(else
							(Narrator init: 3 0 0 4) ; "The flashlight fish's steady glow reveals an opening clogged with brownish rocks. It looks almost like a "wall of stones!""
						)
					)
				)
				(4 ; Inventory
					(switch invItem
						(12 ; trident
							(Narrator init: 3 0 0 18) ; "Adam's afraid he might bend the trident."
						)
						(else
							(super doVerb: theVerb invItem &rest)
						)
	...
	(method (doVerb theVerb invItem)
		(if (not (IsFlag 324))
			...
			(switch theVerb
				(3 ; Do
					(if (< global244 15)
						(Narrator init: 3 0 0 34) ; "The greenish substance leaks through the opening. Just beyond Adam sees large shapes. However, it's not clear what they are yet."
					)
				)
				(2 ; Look
					(cond
						...
						((>= global244 14)
							(Narrator init: 3 0 0 35) ; "The greenish, glowing light is so intense that Adam can't make out what's beyond."
						)
						(else
							(Narrator init: 3 0 0 34) ; "The greenish substance leaks through the opening. Just beyond Adam sees large shapes. However, it's not clear what they are yet."
						)
...
(instance spiderWeb of Feature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(3 ; Do
				(Narrator init: 3 0 0 37) ; "The dead sea fan is beyond help."
			)
			(else
		...
		(switch theVerb
			...
			(3 ; Do
				(if (IsFlag 73)
					...
					(Narrator init: 3 0 0 23) ; "Adam should try to stay away from the walls. He might get cut."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
	...
	(method (doVerb theVerb invItem)
		(if cel
			(switch theVerb
				(2 ; Look
					(if (IsFlag 148)
						(Narrator init: 3 0 0 44) ; "Between the rocks is a box with a protective suit in it. The suit is designed for handling dangerous chemicals."
					else
						(SetFlag 148)
			...
			(switch theVerb
				...
				(4 ; Inventory
					(switch invItem
						(4 ; rag
							(Narrator posn: -1 154 init: 3 0 0 11) ; "Adam eases the lock with the oily rag."
							(SetScore 5 327)
						)
						(25 ; boxKey
							(if (IsFlag 327)
								...
								(Narrator posn: -1 154 init: 3 0 0 10) ; "The key turns halfway in the lock and then stops. It seems that saltwater has rusted the mechanism."
							)
						)
						(12 ; trident
							(Narrator posn: -1 154 init: 3 0 0 9) ; "There's no telling what's in the box. Better not risk damaging the contents!"
						)
						(30 ; hackSaw
							(Narrator posn: -1 154 init: 3 0 0 9) ; "There's no telling what's in the box. Better not risk damaging the contents!"
						)
						(else
							(if (IsFlag 327)
								...
								(Narrator posn: -1 154 init: 3 0 0 13) ; "The lock needs easing. That won't help."
							)
						)
					)
				)
...
(instance openChest of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(chest cel: 1 forceUpd:)
				(Narrator posn: -1 154 init: 3 0 0 30 self) ; "The key turns smoothly in the oiled lock. The box pops open to reveal a suit of protective clothing."
			)
			(1
				(if (not (IsFlag 97))
					(suitInChest init: stopUpd:)
...
(instance showChestInset of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Narrator posn: -1 154 init: 3 0 0 6 self) ; "A metal box has been concealed in the pile of rocks."
			)
			(3
				(HandsOn)
			)
