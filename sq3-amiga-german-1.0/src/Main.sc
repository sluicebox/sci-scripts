...
(instance ego of Ego
	...
	(method (handleEvent event)
		(if (or (gUser canControl:) (!= (event type:) evMOUSEBUTTON))
...
(instance logFile of File ; UNUSED
	(properties)
)
...
(instance sqMDH of EventHandler
	(properties)
)

(instance sqKDH of EventHandler
	(properties)
)

(instance sqDDH of EventHandler
	(properties)
)
...
(instance SQ3 of Game
	...
	(method (init &tmp temp0)
		...
		((= gMouseDownHandler sqMDH) add: gRegions gLocales gCast)
		((= gDirectionHandler sqDDH) add: gRegions gLocales)
		((= gKeyDownHandler sqKDH) add: gCast gRegions gLocales)
		(= global193 1)
		(longSong owner: self init:)
		(= gLongSong longSong)
		(gUser canControl: 0 x: -1 y: 160)
		(= gEgo ego)
		(gUser alterEgo: gEgo)
		(SL code: statusCode)
		(gGame setSpeed: 5)
		(= gSystemWindow sysWindow)
		(= gTheMenuBar TheMenuBar)
		(gTheMenuBar init:)
		(HandsOn)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(or
							(Said 'use,activate/belt,invisibility,invisibility')
							(Said 'turn<on/belt')
							(Said 'press/button,button/belt')
							(Said 'switch<on/belt')
						)
						(cond
					...
					((or (Said 'inv,inv') (Said 'look,look,get/inv,inv'))
				...
				(if global200
					...
					(cond
						((== (= temp7 (event modifiers:)) $000a) ; emALT | emLEFT_SHIFT
							(event claimed: 1)
							((gUser alterEgo:) setMotion: JumpTo temp5 temp6)
						)
						((& temp7 emSHIFT)
							(event claimed: 1)
							(= temp1
	...
	(method (wordFail param1 &tmp [temp0 150])
		(Print (Format @temp0 0 31 param1 param1)) ; "Try another word. Apparently, "%s" isn't in the Andromedan dictionary."
	)
	...
	(method (pragmaFail &tmp [temp0 150])
...
(instance Buckazoids of InvI
	...
	(method (showSelf &tmp temp0)
		...
		(Print (Format @global402 0 34 global154 global154) #icon view loop cel) ; "You possess %d of these nifty monetary units known as `Buckazoids'."
		(if temp0
			(SaveSubLang)
		)
	)
