				...
				(cond
					((Said 'look,look>')
						(cond
							...
							((Said '/iro,junk,debris,scrap,heap')
					...
					(
						(Said
							'climb,descend,(climb[<up,down,on,on])/conduit,support[<iro]'
						)
						(Print 10 10) ; "You can't climb that."
					)
					(
						(Said
							'jump,jump,leap,leap,(get,get,climb<on,on)[<off]/conveyer,conveyer,belt'
						)
						(if (== global132 2)
					...
					(
						(Said
							'(get,get<up),stand,stand,(stand,stand<up)[/belt,conveyer,conveyer]'
						)
						(if (== global132 2)
							(if (!= (gEgo view:) 0)
								(HandsOn)
					...
					(
						(Said
							'jump,jump,leap,leap,climb[<on,on,to,up,down]/conduit'
						)
						(Print 10 16) ; "That doesn't look like a safe place to jump to."
					)
					((and (== global132 2) (Said 'climb<up,on/banister'))
						(Print 10 17) ; "It's not within your reach."
					)
					(
						(Said
							'jump,jump,leap,leap[<to,on,on,up,down,off][/banister,banister]'
						)
						(if (== global132 2)
							(if (== (gEgo view:) 0)
								(gEgo setScript: railJump)
							else
								(Print 10 18) ; "You can't from this position."
							)
						else
							(event claimed: 0)
							(return)
...
(instance rmScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(= local2 (Print 10 19 #dispose)) ; "Shredded like an Iran-Contra document, your many independent parts flutter to the bottom of the hopper. This is of little importance to you, what with your being dead and all."
				(= seconds 6)
			)
			(5
				(cls)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(= local2
					(if (== global132 2)
						(Print 10 20 #dispose) ; "You stepped off the conveyer. You're dead again. Way to go. Haven't we taught you anything?"
					else
						(Print 10 21 #dispose) ; "You stepped off the rail! You're dead again. Way to go. Haven't we taught you anything?"
					)
				)
				(= seconds 5)
			)
...
(instance grabScript of Script
		...
		(switch (= state newState)
			(0
				...
				(if (< global148 4)
					...
					(gUser prevDir: (if (== gPrevRoomNum 9) 3 else 7))
				)
			)
		)
	)
	...
	(method (handleEvent event &tmp [temp0 100])
		...
		(cond
			((Said 'look,look/grabber,control,control')
				(Print 10 22) ; "From your seat you see a handle, (presently being gripped by you) which controls motion, and a button marked CLAW."
			)
			(
				(or
				...
				(Print 10 23) ; "Due to a series of pipelines directly below you, the claw is programmed not to work here."
			)
			((Said '/chair[<grabber,machine]')
				(Print 10 24) ; "It looks like something that would cradle your posterior nicely."
			)
			((Said 'disembark,disembark[/grabber,machine,!*]')
				(Print 10 25) ; "That would be unwise."
			)
		)
	)
)
