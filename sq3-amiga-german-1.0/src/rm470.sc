...
(instance rm470 of Rm
	...
	(method (init)
		...
		(gAddToPics doit:)
		(head init: stopUpd:)
		(gameEyes init: setScript: EyeBlink)
		...
		(if (or (not (gLongSong handle:)) (== (gLongSong number:) 22))
			...
			(1
				...
				(= local0
					(PrintSplit ; "Howdy, stranger! The name's Blatz, Fester Blatz. Welcome to World o' Wonders. Go ahead, have a look at some of the trendiest items in the known universe. Make the most of your vacation buckazoid!"
						...
						33
						600
						(and (not (gGame subtitleLang:)) 105)
					)
				)
				(= seconds 15)
			)
...
(instance EyeBlink of Script
	...
	(method (changeState newState &tmp [temp0 50])
		(switch (= state newState)
			...
			(1
				(if (!= (= local3 (gameEyes cel:)) 3)
					(gameEyes loop: 8 cel: 255 setCycle: End self)
				else
					(= cycles 2)
				)
			)
			(2
				(gameEyes loop: 1 cel: local3)
				(self init:)
			)
		)
	)
