...
(instance ortega of Rgn
	...
	(method (handleEvent event)
		...
		(cond
				...
				(cond
					...
					((Said '/craft')
						(switch gCurRoomNum
							...
							(else
								(Print 600 18) ; "I don't know about you, but I don't see any ships here."
							)
						)
					)
				)
			...
			((Said 'open/door')
				(Print 600 19) ; "There are no doors on this planet."
			)
			((Said 'jump,jump<off/cliff')
				(Print 600 20) ; "Jumping into boiling lava is not recommended."
			)
			((Said 'get,get/ladder')
				(if (or (== gCurRoomNum 74) (== gCurRoomNum 75))
					(Print 600 21) ; "The ladder is firmly attached to the machine."
				else
					(Print 600 22) ; "There's no ladder around here."
				)
			)
			((Said 'get,get/rock')
				(Print 600 23) ; "Come on, you've got better things to do than lug rocks around."
			)
			((Said 'get,get/dirt')
				(Print 600 24) ; "Don't bother; you'll just get dirty."
			)
			((Said 'use/pole')
				(if (!= gCurRoomNum 66)
			...
			((Said '(turn<on),press,use/detonator')
				(if (gEgo has: 10) ; Thermal_Detonator
					(Print 600 25) ; "It has an impact switch, just drop it to set it off."
				else
			...
			((Said 'wear/panties')
				(if (gEgo has: 5) ; ThermoWeave_Underwear
					(if global182
						(Print 600 26) ; "They're so comfortable, you forget that you're already wearing them."
					else
						(Print 600 27) ; "It would be a little embarassing to disrobe out here on the planets' surface. Why not go back to your ship?"
					)
				else
					(Print 600 28) ; "You don't have any."
				)
			)
			((Said 'remove/panties')
				(if global182
					(Print 600 29) ; "Nah, just leave 'em on."
				else
					(Print 600 30) ; "You're not wearing any shorts."
				)
			)
			((and (Said 'climb') (< gCurRoomNum 73))
				(Print 600 31) ; "The sharp edges of the cooled lava would slice you to ribbons. You decide against it."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= global159 1)
				(= global171 0)
				(Print 600 32) ; "Too late you realize that walking around unprotected on this planet is hazardous to your health. You feel your blood begin to boil."
				(gEgo
					view: 89
					setLoop: 0
					setCel: 0
			...
			(4
				(Print 600 33) ; "You sizzle into oblivion. This planet wouldn't be so bad if you could keep cool somehow."
...
(instance FallDown of Script
	...
	(method (doit)
		...
		(if (or local0 (not (fallSound nodePtr:)))
			(= local0 0)
			(self changeState: 1)
		)
	)
