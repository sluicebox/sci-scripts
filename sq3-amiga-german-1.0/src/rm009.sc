				...
				(cond
					((Said 'look,look>')
						(cond
							(
								(Said
									'/conveyer,conveyer,belt,bucket,hoist,hoist,elevator'
								)
								(if (== global124 0)
							...
							((Said '/iro,scrap,junk,debris,heap,junk,debris')
								(if (== global124 0)
					...
					((Said 'jump,jump/conveyer,conveyer,belt,bucket')
		...
		(switch (= state newState)
			(0
				...
				(if (< global148 4)
					...
					(gUser prevDir: (if (== gPrevRoomNum 10) 7 else 3))
				)
				(if (> global148 3)
					(gEgo
