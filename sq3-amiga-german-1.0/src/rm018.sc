...
(procedure (localproc_1 param1 param2 param3)
	(if param1
		(SaveSubLang)
		(Display param1 dsWIDTH 50 dsCOORD param2 param3 dsFONT 600 dsCOLOR 0)
		(vHair forceUpd:)
		(RestoreSubLang)
	)
)
...
(instance rm018 of Rm
	...
	(method (init &tmp [temp0 50])
		(if (> global598 4)
			(self picture: (LangSwitch 18 311))
		else
			(self picture: (LangSwitch 180 318))
		)
		(self setRegions: 701) ; travrg
		(if (> global598 4)
			(Load rsVIEW (LangSwitch 46 308))
		else
			(Load rsVIEW (LangSwitch 146 328))
		)
		(Load rsVIEW (LangSwitch 45 307))
		(Load rsVIEW 47)
		(Load rsVIEW 48)
		(Load rsVIEW 51)
		(Load rsVIEW 55)
		...
		(backShield init:)
		(controls
			add: FButton BButton HButton fire place_to_leave
			eachElementDo: #init
			draw:
		)
		(localproc_2)
		...
		(switch (= state newState)
			...
			(3
				...
				(if (== (++ global176) 5)
					...
					(= local5 {NEGATIVE})
					(RadarScript changeState: 0)
					(Print 18 1) ; "The remaining enemy ships have given up and are heading back to the planet. It looks like you were just too much for 'em."
					(gCurRoom setScript: 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (not global174)
					(PrintSplit 18 2) ; "With your shield depleted, the final hit shreds the side of your ship. In the sudden vacuum, your body fluids expand beyond the capacity of your tissues. Your desicated body will drift forever, a grim testament to your blundering stupidity."
				else
					(PrintSplit 18 3) ; "Having the correct shield up could have prevented this!  The final shot shreds the side of your ship. In the sudden vacuum, your body fluids expand beyond the capacity of your tissues. Your desicated body will drift forever, a grim testament to your blundering stupidity."
				)
				(EgoDead 0 0 4 5)
			)
		)
...
(instance Depleted of Script
	...
	(method (doit)
		(if (and (not (shWarn handle:)) (!= (self state:) 2))
			(self changeState: 2)
		)
		(super doit:)
	)
...
(instance HButton of DIcon
	...
	(method (init)
		...
		(= key (LangSwitch 104 63))
		(super init:)
	)

	(method (doit)
		(PrintSplit 18 4) ; "When the enemy ship comes from behind, you will automatically begin tracking. Use the direction keys or mouse to move the target cross-hairs onto the target. When you are locked on, use the space bar to fire."
	)
)

(instance place_to_leave of DIcon
	(properties
		state 1
		nsTop 72
		nsLeft 47
