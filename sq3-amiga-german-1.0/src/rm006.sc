...
(instance rm006 of Rm
	...
	(method (init &tmp [temp0 50])
		...
		(gUser prevDir: (if (== gPrevRoomNum 5) 3 else 7))
		(shadow init:)
		(if (IsItemAt 1) ; Wire
			(wire init:)
		)
				...
				(cond
					((Said 'look,look>')
						(cond
							...
							((Said '[<around,at,in][/area,tube,cave,pit,stage]')
					...
					((Said 'get,get,(pull[<on])/wire,cable')
						(if (IsItemAt 1) ; Wire
					...
					((Said 'get,get/conduit')
						(Print 6 18) ; "They're useless."
					)
					(
						(Said
							'get,get,climb,crawl,hide[<in,in,through,below]/grate'
						)
						(Print 6 19) ; "You can't."
					)
					(
						(Said
							'press,hoist,hoist,move,freeze,manipulate,open,get,get,remove/console,(iro[<sheet])'
						)
