...
(instance rm43 of Rm
	...
	(method (init)
		...
		(if (not global152)
			(Load rsVIEW 66)
			(Load rsVIEW (LangSwitch 67 315))
		)
		(Load rsVIEW (LangSwitch 70 316))
		(Load rsVIEW 121)
		(Load rsSOUND 13)
		(super init:)
		(w init:)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((or (Said 'get,get/cap') (Said 'get<up/cap'))
					...
					(
						(Said
							'look,look/box,table,glass,animal,creature,devil,slime,show,display'
						)
					...
					((Said 'read,look,look/sign,sign[<neon,flashing]')
						(if
							(and
								(gCast contains: door)
					...
					(
						(or
							(Said 'look,look/area')
							(Said 'look,look/building,mog,monster')
							(Said 'look,look[<around]')
						)
						(PrintSplit 43 18) ; "Ah Ha! A tourist trap. This giant metal facsimile of a space beast is nothing more than a cheap marketing ploy designed to suck in any moron dumb enough to fall for such trickery... You suddenly feel like a dumb moron."
					)
				)
			)
			(evKEYBOARD
...
(instance kid1Script of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gLongSong number: 13 priority: 127 loop: -1 playBed:)
...
(instance pop of Actor
	...
	(method (handleEvent event)
		...
		(switch (event type:)
			(evSAID
				(if (Said 'look,look/dad,man,alien')
