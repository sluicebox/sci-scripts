	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 401] temp403 temp404)
		(switch (super handleEvent: event)
			(257
				...
				(PrintSplit ; "German Edition: Produced by Pablo Ghenis Translated by: Brigitte Burkertt-Hinojos and Sabine Duvall Program conversion by: Gary Kamigawachi and Sean Mooney Graphics conversion by: Cheri Lloyd and Jeff Crowe Quality Assurance by: Bill Shockley, Robin Bradley, Mike Harian, Kevin Ray, Sabine Duvall and J\81rg Baumann Bilingual System by Pablo Ghenis"
					997
					1
					33
					3
			...
			(258
				(PrintSplit 997 2 33 3) ; "DURING THE GAME: Click at the top of the screen or press ESC to use the menus. Additional shortcuts are shown there.    IN TYPING WINDOWS: Arrows, Home and End move the cursor, or click anywhere with the mouse. Strg-C clears the line.    IN DIALOG WINDOWS: Enter selects the outlined item, or click on items with the mouse. Tab and Shift-Tab move between choices. ESC always cancels."
			)
			(259
			...
			(260
				(= temp403 (gGame printLang:))
				(= temp404 (gGame subtitleLang:))
					...
					(switch
						...
						(1
							(if temp404
								...
								(gGame
									printLang: (if (!= temp403 1) 1 else 49)
								)
							)
						)
						(2
			...
			(771
				(event claimed: 0 type: evKEYBOARD message: (gUser echo:))
			)
			(773
			...
			(1281
				(if
					...
					(DoSound sndMASTER_VOLUME temp1)
				)
			)
			(1282
				(if (GetMenu 1282 113)
