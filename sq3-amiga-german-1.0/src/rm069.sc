		...
		(cond
			((Said 'look,look>')
				(cond
					...
					((Said '[<around,at,in][/area]')
						(if global253
							(PrintSplit 69 2) ; "The planet Ortega is truly a lava-lovers paradise.  Volcanic activity constantly reshapes its surface, so if you have any maps older than last week, throw 'em out. A few pieces of research equipment are laying around."
						else
					...
					(
						(Said
							'/station,machine,device,equipment[<seismic,research]'
						)
			...
			((Said 'get,get>')
				(cond
					(
						(Said
							'/antenna,station,machine,equipment[<seismic,research]'
						)
			...
			((Said 'use/scope')
				(if (gEgo inRect: 56 139 76 159)
					...
					(Print 69 23) ; "You need to get real close and look in the little hole first."
				)
			)
			((Said 'kill/man,flunky')
				(if global253
					(Print 69 24) ; "Quite the swashbuckler, aren't we? Too bad there's nobody here."
				else
					(Print 69 25) ; "With what, your breath?"
				)
			)
			((Said 'talk,talk[/man,flunky]')
				(if global253
					(Print 69 26) ; "Quite the talker, aren't we? Too bad there's nobody here."
				else
					(Print 69 27) ; "Get a little closer."
				)
			)
			((Said 'open/box')
				(Print 69 28) ; "The crate is already open."
			)
		)
	)
)
...
(instance shootScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 69 29) ; "Way to go, ace! You blundered your way to within range of the pirates' jello gun. You suffocate in an impenetrable block of jello."
				(EgoDead 0 0 7 15)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(pirate1 dispose:)
				(pirate2 dispose:)
				(Print 69 30) ; "You hear the roar of the pirates' scout ship taking off.  The ship streaks across the sky to an unknown destination."
				(zoom play:)
				(= global253 1)
				(= seconds 3)
			)
...
(instance pirate2 of Actor
	...
	(method (handleEvent event)
		...
		(if (Said 'look,look/flunky,man,clerk')
			(Print 69 31) ; "Obviously loyal company men, the ScumSoft employees are happily performing their duties. But looking at their weapons, you probably don't want to get too close."
		)
	)
)
