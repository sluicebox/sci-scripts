			...
			((== (event type:) evSAID)
				(if (Said 'look,look>')
					(cond
						...
						((Said '/machine,grabber,console[<control,control]')
						...
						(
							(Said
								'look,look/throttle,control,control,handle,stick'
							)
							(if (or (== global132 4) (== global132 5))
						...
						((Said '/light,lamp<grabber,machine,safety')
							(if (or (== global132 4) (== global132 5))
				...
				(if (Said 'yell,yell,talk,talk/android')
				...
				(if
					(or
						(Said 'climb[<up][/grabber,machine]')
						(Said 'climb/banister,banister<up<to')
						(Said 'climb<up,on/banister,banister')
					)
					(Print 13 24) ; "You aren't able to do that."
				)
				(if (Said 'enter,(go<in,in)/chute')
					(Print 13 25) ; "Go there yourself."
				)
				(if
					(or
						(Said 'enter,board,board[/machine,grabber,chair]')
						(Said 'get,get<on,in,on,down/[machine,grabber,chair]')
						(Said
							'climb[<on,in,on,in,down][/machine,grabber,chair]'
						)
					)
					(cond
				...
				(if
					(or
						(Said
							'pull,press,use,move,freeze,manipulate,go[<forward,backward,back,back,back][/handle,stick,throttle,control,control,knob]'
						)
						(Said
							'start,move,freeze,manipulate,drive[<forward,backward,back,back,back]/grabber,machine'
						)
					)
				...
				(if
					(Said
						'disembark,disembark,stand,stand,(stand,stand<up),(climb,get,get<off,out)[/grabber,machine,claw,claw,!*]'
					)
