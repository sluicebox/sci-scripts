	...
	(method (handleEvent event)
		...
		(cond
				...
				(cond
					((Said '[<around,at,in][/area]')
						(if (IsItemAt 3) ; Reactor
							(PrintSplit 15 0) ; "You seem to be in a debris-enclosed hollow. Poking out of the ceiling is the chute which you originally entered through. Some crusty lamps linked by non-UL Approved wire provide additional illumination."
						else
							(PrintSplit 15 1) ; "You seem to be in a debris-enclosed hollow. Poking out of the ceiling is the chute which you originally entered through. Some crusty lamps linked by non-SQ Approved wire formerly provided additional illumination."
						)
					)
					((Said '/light,lamp')
						(if (IsItemAt 3) ; Reactor
					...
					((and local0 (Said '/mice,animal,creature'))
					...
					((or (Said '/end<wire,cable') (Said '/wire,cable<end'))
									...
									(Format ; "You peer into the small opening and notice %s."
										...
										(cond
											(
												...
												{einen winzigen Reaktor, der den Strom f\81r die Lampen zu liefern scheint, ein vormals leuchtendes Gl\81hjuwel und ein loses Kabel}
											)
											((and (IsItemAt 3) (IsItemAt 0)) {einen winzigen Reaktor, der den Strom f\81r die Lampen zu liefern scheint, und ein vormals leuchtendes Gl\81hjuwel}) ; Reactor, Glowing_Gem
											((and (IsItemAt 3) (IsItemAt 1)) {einen winzigen Reaktor, der den Strom f\81r die Lampen zu liefern scheint, und ein loses Kabel}) ; Reactor, Wire
											((and (IsItemAt 0) (IsItemAt 1)) {ein vormals leuchtendes Gl\81hjuwel und ein loses Kabel.}) ; Glowing_Gem, Wire
											((IsItemAt 3) {einen winzigen Reaktor, der den Strom f\81r die Lampen zu liefern scheint}) ; Reactor
											((IsItemAt 1) {ein loses Kabel}) ; Wire
											((IsItemAt 0) {ein vormals leuchtendes Gl\81hjuwel}) ; Glowing_Gem
											(else {nichts au\e1er verkrusteten Kabelenden})
										)
									)
								)
							)
					...
					((Said '/chute,door[<debris]')
						(PrintSplit 15 16) ; "The chute juts into the room from high above. It is covered by two large doors. While staring at it, you briefly entertain the thought of trying to climb back up it. You realize that it is firmly sealed and forget about it."
					)
					((Said 'look,look/ladder')
				...
				(cond
					...
					((Said '/ladder')
						(if
							...
							(Print 15 25) ; "You grab the ladder and jam it in your pocket."
							(Print 15 26) ; "Ouch!"
						else
							(Print 15 27) ; "There is no ladder near here to get."
						)
					)
					((or (Said '/end<wire,cable') (Said '/wire,cable<end'))
						(if (gEgo inRect: 0 125 38 155)
							(if (IsItemAt 3) ; Reactor
								(Print 15 28) ; "They seem to be attached to a reactor."
							else
								(Print 15 29) ; "They would not be helpful."
							)
						else
							(Print 15 29) ; "They would not be helpful."
						)
					...
					((Said '/artifact,wing,cover')
						(Print 15 30) ; "This stuff has no value to you."
					)
					((Said '/generator')
						(if (IsItemAt 3) ; Reactor
							(if (gEgo inRect: 0 125 38 155)
								(gGame changeScore: 15)
								(Print 15 31) ; "You unhook the reactor from the cheap wires and take it with you."
								(reactorScript init:)
							else
								(Print 15 32) ; "You aren't near it."
							)
						else
							(Print 15 33) ; "There isn't one here to get."
						)
					)
					((Said '/wire,cable')
						(if (IsItemAt 1) ; Wire
			...
			((Said 'climb,crawl[<through,up,on]/partition,cavity,chute')
				(Print 15 34) ; "You'll not be successful that way."
			)
			((Said 'use,affix,(hook<up)/generator')
				(if (or (gEgo has: 3) (IsItemAt 3)) ; Reactor, Reactor
					(Print 15 35) ; "That won't further your progress."
				else
					(event claimed: 0)
				)
			)
			(
				(Said
					'erect,erect,stand,stand,drop,place,place,use/ladder[/cavity[<by,in]]'
				)
				(if (gEgo inRect: 170 66 292 97)
					(if (gEgo has: 2) ; Ladder
						(gEgo setScript: putLadderScript)
					else
						(Print 15 36) ; "You don't have it."
					)
				else
					(Print 15 37) ; "Not from here."
				)
			)
			((Said 'open/chute,door')
				(Print 15 38) ; "The chute is covered by two large doors which you are unable to open."
			)
		)
	)
...
(instance rmScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(doors addToPic:)
				(Print 15 39) ; "Fortunately, you come to a relatively soft landing in a pile of debris."
				(= seconds 2)
			)
			(6
				(HandsOn)
...
(instance mugScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 15 40) ; "You hear strange growls which send chills through your sleek, yet rugged, frame. What could it be?"
				(Print 15 41) ; "Suddenly, two hideous figures rush from the shadows toward you!"
			...
			(4
				(Print 15 42) ; "Oh my! It appears that your being trapped has made you a prime target for rodent ruffians. What a mess!"
				(EgoDead 901 0 2 3)
			)
		)
	)
