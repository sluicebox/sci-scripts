...
(instance rm028 of Rm
	...
	(method (init &tmp [temp0 50])
		...
		(= global244 0)
		(= global247 1)
		(gLongSong number: 16 priority: 127)
	...
	(method (handleEvent event &tmp [temp0 50])
		...
		(cond
			...
			((== (event type:) evSAID)
				(cond
					((Said 'look,look>')
						(cond
							((or (Said '/pane') (Said '<out'))
								(PrintSplit 28 0) ; "Outside, the cold vacuum of space pulls at the fragile steel membrane that surrounds you.  Apprehension grips you as you realize your insignificance in the vastness of the universe.  The whirling cosmic display seems to... you know, the usual science fiction kinda stuff."
							)
							...
							((Said '/alien,being,man,bystander,folk,creature')
							...
							((Said '[<around,at,in][/area,cafe]')
								(PrintSplit 28 12) ; "The decor, like the food, is the same in Monolith Burgers all over the universe. Generic counter clerks are eagerly waiting to help you. Diverse life-forms are crowded around the counter and sitting in booths consuming what can only loosely be termed food."
							)
						)
					)
					((Said 'down,sit[<down]')
					...
					(
						...
						(cond
							...
							((or (== local5 0) (gEgo inRect: 0 145 37 171))
								(Print 28 23) ; "Nah, let Mr. Employee of the Week clean it up."
							)
							(else
								(Print 28 24) ; "There's no trash around here."
							)
						)
					)
					((Said 'open,close/door')
						(Print 28 25) ; "It's automatic, just walk near it. This is the future, remember?"
					)
					(
						(or
							(Said
								'order,order,buy,buy,get,get,ask[/dinner,dinner,burger]'
							)
							(Said 'talk,talk/clerk')
						)
						(Print 28 26) ; "You'll have to wait your turn. Of course, the way this line's moving, you may never get a turn. Obviously, management carefully schedules the least amount of help for the busiest time of day."
					)
					(
						(Said
							'ask,talk,talk/alien,being,man,bystander,folk,creature,customer'
						)
						(Print 28 27) ; "The customers have better things to do than talk to a small-brained biped from a hick planet."
					)
					((Said 'talk,talk')
						(Print 28 28) ; "You mumble to yourself. The other customers eye you suspiciously."
					)
					(
						(Said
							'kiss,kiss/alien,being,man,bystander,folk,creature,customer'
						)
						(Print 28 29) ; "Inter-species relations are frowned upon around here.  You could get yourself killed for trying something like that."
					)
					(
						(Said
							'smell,smell,smell[/man,being,alien,him,bystander,creature,customer]'
						)
						(Print 28 30) ; "The only thing you can smell is the aroma of home-cooked food. Obviously a piped-in smell to mask the odor of the real food."
					)
					(
						(Said
							'hit[/man,being,alien,him,bystander,creature,customer]'
						)
						(Print 28 31) ; "After thinking it over, you repress your baser urges and remain non-violent."
					)
					((Said 'read/menu')
						(if (== local4 0)
							(Print 28 32) ; "You can't quite see it from here. You'll have to stand up."
						else
							(= global245 (gEgo x:))
							(= global246 (gEgo y:))
							(gCurRoom newRoom: 280)
					...
					(
						(or
							(Said
								'open,enter,board,board,climb,(get,get<in)[/door,door,craft]'
							)
							(Said 'disembark,disembark')
						)
						(cond
							((not (gEgo inRect: 179 72 251 95))
								(Print 28 33) ; "Not from here."
							)
							((gEgo has: 17) ; Bag_of_Fast_Food
								(Print 28 34) ; "Bringing food on board your ship isn't a very good idea. Considering how clumsy you are, you're liable to spill crumbs all over the computer or something."
							)
							(global101
			...
			((and (== (event type:) evMOUSEBUTTON) (== local4 0) (not (event claimed:)))
				(gCurRoom setScript: StandUp)
				(event claimed: 1)
			)
		)
...
(instance EgoEating of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(33
				(Print 28 35) ; "OWWW!!! Hey, what's this in my burger? Oh, it must be my Fun Meal prize! Hey, it's a swell decoder ring!"
			...
			(40
				(Print 28 36) ; "Mmmmmm, that was mighty tasty. Well, maybe mildly tasty. Well, maybe not tasty at all. In fact, it reminded you of the slick skin of a Vorlian mucusworm."
				(gEgo setCycle: 0)
				(gUser canInput: 1)
				(= global159 0)
				(= global249 0)
...
(instance dockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print 28 37 #at -1 130 #width 280) ; "With the docking maneuver completed, the engines shut down. Welcome to Monolith Burger!"
				(HandsOn)
				(= cycles 2)
			)
			(5
			...
			(7
				(Print 28 38) ; "You pop the hatch and ramble on in."
				(gEgo
					view: 0
					setLoop: -1
					loop: 1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(chairMan dispose:)
				(RedrawCast)
				(Print 28 39 #at -1 130 #width 280) ; "You slide back into the ship, closing the hatch behind you. The Docking Control Beam begins guiding you safely clear of Monolith Burger."
				(ship setMotion: MoveTo 286 125 self)
			)
			(3
...
(instance VomitScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(PrintSplit 28 40) ; "Man, that Big Belcher Combo was a little too much for your delicate digestive tract. You feel better now, except for being hungry again. Of course, the thought of eating here doesn't appeal to you right at the moment."
				(= global101 0)
				(HandsOn)
				(client setScript: 0)
			)
...
(instance pow of Prop
	...
	(method (init)
		(super init:)
		(self view: (LangSwitch 64 314) setLoop: 0 setCel: 0 ignoreActors: 1)
	)
)
...
(instance balloon of Prop
	...
	(method (init)
		(super init:)
		(self view: (LangSwitch 64 314) setLoop: 1 setCel: 0 ignoreActors: 1)
	)
)
