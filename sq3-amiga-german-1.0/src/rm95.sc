		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(Said
							'press,press,hit,feel,use/control,control,console,button,button'
						)
					...
					((<= (captureScript state:) 3)
						(cond
							((Said 'look,look[/area,left,right]')
								(PrintSplit 95 9) ; "You cautiously enter a darkened chamber. A seemingly bottomless shaft drops off into a black abyss. On a platform in the center of the chamber, the Two Guys from Andromeda wiggle helplessly in lime jello. The platform can only be reached by the four retractable bridges at each entrance."
							)
							((Said 'look,look,talk,talk/man,folk')
								(Print 95 10) ; "'HELPH!' they slurp from their jello-encased captivity."
							)
							((Said 'free,release,release/man[<2]')
					...
					((Said 'kill,combat,escape,escape')
						(Print 95 22) ; "How now?"
					)
					((Said 'look,look[/area,left,right]')
					...
					((>= (captureScript state:) 16)
						(cond
							(
								(Said
									'look,look/pane,man,guard,folk,programmer,accountant'
								)
								(Print 95 27) ; "Your every move is being observed."
							)
							((Said 'look,look/elmo,elmo,boy,(man<little)')
