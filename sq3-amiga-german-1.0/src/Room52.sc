...
(instance Room52 of Rm
	...
	(method (handleEvent event &tmp [temp0 5])
		...
		(switch (event type:)
			(evSAID
				(cond
					((Said 'look,look,explore/android,junk,debris,heap')
						(cond
								...
								(Print
									(Format ; "The terminator is now a pile of junk lying on the sand under the pustules. %s"
										...
										(if
											...
											{Beim genauen Hinsehen f\84llt Dir auf, da\e1 der Tarng\81rtel des Terminators noch relativ intakt ist.}
										else
											{}
										)
									)
					...
					(
						(or
							(Said 'get,get/belt/orat,handle,stick')
							(Said 'use/handle,stick,orat')
							(Said 'get,get<use<(handle,stick,orat)/belt')
						)
						(cond
							...
							((not (IsItemAt 16 52)) ; Invisibility_Belt
								(Print 52 12) ; "You don't see one here."
							)
							((not (gEgo has: 4)) ; Orat_on_a_Stick
								(Print 52 13) ; "You don't have the proper implement."
							)
							((!= global110 3)
								(Print 52 14) ; "The robot might have something to say about that."
							)
							(
								(or
								...
								(Print 52 15) ; "Try approaching from a slightly different angle."
							)
							(else
					...
					((or (Said 'get,get/belt/pole') (Said 'use/pole'))
						(cond
							((gEgo has: 16) ; Invisibility_Belt
								(Print 52 11) ; "You already have it."
							)
							...
							(
								...
								(Print 52 16) ; "Try approaching from a slightly different angle."
							)
							(else
					...
					((Said 'get,get/belt')
						(if (IsItemAt 16 52) ; Invisibility_Belt
							(Print 52 17) ; "Get closer."
						else
							(Print 52 18) ; "You don't see one here."
						)
					)
					((Said '/ladder')
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(gameMouth setLoop: 4 setCycle: Fwd)
				(local5
					view: 78
					setLoop: 5
					ignoreActors:
