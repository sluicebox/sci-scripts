...
(instance rm180 of LLRoom
	...
	(method (init)
		...
		(super init:)
		(imprinter init: stopUpd: approachVerbs: 3) ; Do
		(light init:)
		(bookcase init:)
		(nudePainting init:)
		(desk init:)
		(lamp init:)
		(magnifier init:)
		(drawer init: stopUpd: approachVerbs: 3) ; Do
...
(instance sGetCard of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(imprinter setCel: 1 approachVerbs: 0 stopUpd:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(drawer setMotion: MoveTo 263 (drawer y:) drawer)
				(= drawerOpen 0)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				...
				(TPrint 180 17 #at -1 185) ; "(Shame on you for thinking thoughts like that!)"
			)
			(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				...
				(TPrint 180 21 #at -1 185) ; "(And it, you!)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
