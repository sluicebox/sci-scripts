...
(instance sClosed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(TPrint 710 0) ; "The Tramp Casino Ballroom is presently closed."
				(gEgo
...
(instance person0 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 1) ; "Leave them alone, Larry; they're concentrating on losing."
			)
			(else
...
(instance girlPic1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 710 2) ; "You find the casino's artwork totally tasteless..."
				(TPrint 710 3) ; "...and fascinating!"
			)
			(3 ; Do
				(TPrint 710 4) ; "Don't touch those paintings!"
			)
			(else
...
(instance rouletteArea of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 5) ; "You? Play roulette? HA!"
			)
			(5 ; Talk
				(TPrint 710 6) ; "The roulette players are too busy to talk to you."
			)
			(4 ; Inventory
				(switch invItem
					(17 ; Silver_Dollar
						(TPrint 710 7) ; "Save your money, Larry! You'll only lose anyway!"
					)
					(else
						(super doVerb: theVerb invItem &rest)
					)
...
(instance blackjackArea of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 8) ; "If you want to play blackjack, buy "Leisure Suit Larry 1!""
				(TPrint 710 9 67 -1 185) ; "(Another fine Al Lowe game!)"
			)
			(5 ; Talk
				(TPrint 710 10) ; "The blackjack players are too busy to talk to you."
			)
			(4 ; Inventory
				(switch invItem
					(17 ; Silver_Dollar
...
(instance chandelier of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 11) ; "You try your best, but just can't reach the chandelier from your current (low) altitude."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance statue1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 12) ; "Larry! She's only a statue!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance statue5 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 710 13) ; "You find the faux marble cool and exciting to the touch!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poker1 of Feature
	...
	(method (doVerb theVerb invItem &tmp [str 100])
		(switch theVerb
			(2 ; Look
				(Format @str 710 14 gPokerJackpot) ; "The only machines accessible to you are these progressive video poker machines. You notice the jackpot is up to $%d!"
				(TPrint @str)
			)
			(3 ; Do
				(if (gEgo has: 17) ; Silver_Dollar
					(Format @str 710 15 gSilvDollars) ; "You insert all %d of your silver dollars into the video poker machine."
					(Points 4 133)
					(TPrint @str)
					(gCurRoom newRoom: 720)
				else
					(TPrint 710 16) ; "You have no silver dollars with which to play this machine."
				)
			)
			(4 ; Inventory
				(switch invItem
