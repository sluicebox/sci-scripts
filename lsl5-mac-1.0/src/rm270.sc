...
(instance rm270 of LLRoom
	...
	(method (init)
		...
		(if (!= gPrevRoomNum (gCurRoom east:))
			(blonde setLoop: 3 stopUpd:)
			(redHead setLoop: 4 stopUpd:)
		)
		(door init: setPri: 4 stopUpd:)
		(camera init:)
		(switch gPrevRoomNum
			(south
				(camera setCel: 5)
			...
			(east
				(blonde init: posn: (+ gBlondeX 319) 82 stopUpd:)
				(redHead init: posn: (+ gRedHeadX 319) 82 stopUpd:)
				(HandsOn)
				(camera setCel: 255)
				(= style 11)
			)
	...
	(method (doit)
		...
		(cond
			...
			((gEgo inRect: 167 105 325 124)
				(if (and (not scaredBlonde) (== (gGame detailLevel:) 3))
					(= scaredBlonde 1)
					(blonde setScript: sBlonde 0 375)
				)
				(if (and (not scaredRed) (== (gGame detailLevel:) 3))
					(= scaredRed 1)
...
(instance blonde of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Format @string 270 19 (blonde description:)) ; "It's %s, AeroDork's blonde bombshell. Once she was a highly motivated employee. Once she was a highly trained professional. Once she never forgot to ask, "Didya wan'sum fries wit' dat?""
				(TPrint @string)
			)
			(3 ; Do
				(Format @string 270 20 (blonde description:)) ; "%s was once a ticket agent here. Now her only responsibility is to the few remaining employees: she fires them!"
				(TPrint @string)
			)
			(5 ; Talk
			...
			(4 ; Inventory
				(Format @string 270 23 (blonde description:)) ; "Get that away from me, dork!" shouts %s. "Anything YOU have, I don't want!"
				(TPrint @string)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance redHead of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Format @string 270 24 (redHead description:)) ; "It's %s. Once she was AeroDork's red-headed, red-blooded, red-hot, temptress of the flight path."
				(TPrint @string)
			...
			(4 ; Inventory
				(Format @string 270 23 (redHead description:)) ; "Get that away from me, dork!" shouts %s. "Anything YOU have, I don't want!"
				(TPrint @string)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance camera of Prop
	...
	(method (doit &tmp theLoop theCel oldCel oldLoop [str 20])
		...
		(if (not (gCurRoom script:))
			(= oldCel cel)
			(= oldLoop loop)
			(= theCel (/ (- (GetAngle x y (gEgo x:) (gEgo y:)) 105) 17))
			(self loop: (if (> (gEgo y:) 159) 1 else 5) cel: (- 8 theCel))
			(if
				(and
					(> (gGame detailLevel:) detailLevel)
					(or (!= oldLoop loop) (!= oldCel cel))
				)
				(self startUpd:)
