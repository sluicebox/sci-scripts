...
(instance rm100 of LLRoom
	...
	(method (handleEvent event)
		(if
			...
			(Print 100 0 80 {Fast Forward} 82 990 11 0 106 81 {OK} 1) ; "Throughout this game, you can "Zap" ahead by clicking this Fast Forward button, which is in the icon bar at the top of this window."
			(event claimed: 1)
		else
...
(instance sCartoon of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(16
				(Display {} dsRESTOREPIXELS [underBs (- (-- letter) 22)])
				(backSpace play:)
				(if (> letter 22)
					(= cycles 4)
					(-- state)
