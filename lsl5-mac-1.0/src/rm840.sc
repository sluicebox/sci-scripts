...
(instance getKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Points 10 148)
				(TPrint 840 22 #at -1 28) ; "Bending over at the base of the tree, you dig into the humus and find the metallic object that you saw earlier. It looks like a desk key."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance replaceKeyScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TPrint 840 24 #at -1 28) ; "You carefully return the key to its hiding place in the humus."
				(HandsOn)
				(gEgo setScript: 0)
			)
		)
...
(instance runCopierScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				(TPrint 840 39 #at -1 185) ; "(An experience not wholly unfamiliar to you!)"
				(= ticks 60)
			)
			(14
				(gEgo loop: 4 setCel: 255 setCycle: Beg self)
