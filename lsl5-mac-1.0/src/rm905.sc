...
(instance rTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(if (== invItem 22) ; Doily
					(TPrint 905 12) ; "Something inside tells you that you need the doily more than the table does."
				else
...
(instance sPhone of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(= local11 (gTheMusic vol:))
				(gTheMusic fade: 10 120 10 0)
				(if gModelessDialog
				...
				(switch whoToTalkTo
					(8544
						...
						(cond
							...
							(else
								(TPrint 905 18) ; "You again? Why don't you just go outside and get it?! She's waiting for you!"
							)
						)
					)
					(3627
						(if (not (SetFlag 18)) ; fCalledDoc
							...
							(TPrint 905 22) ; "You already did that."
						)
					)
					(-1
						(TPrint 905 23) ; "You change your mind and hang up the phone."
					)
					(else
						(TPrint 905 24) ; "Hmmm. Wrong number."
						(Say gEgo 905 25) ; ""Now where did I put that telephone number?" you ask yourself."
					)
				)
				(self setScript: sHangUp self)
			)
			(4
				(gTheMusic fade: local11 120 10 0)
				(= onPhone 0)
				(= usePhone 0)
				(HandsOn)
				(gUser canControl: 0)
		...
		(cond
			((OneOf theVerb 3 5) ; Do, Talk
				(cond
					((OneOf (gEgo view:) 906 907)
						(if (== (gEgo loop:) 3)
							(TPrint 905 26) ; "First, hang up the phone."
						else
							(gEgo setScript: sChair 0 self)
						)
					)
					...
					(else
						(TPrint 905 28) ; "Go away," a voice cries through the glass. "You already have an appointment. We'll see you then!"
					)
				)
			)
			((== theVerb 2) ; Look
				(TPrint 905 29) ; "Ghostly shapes seem to float on the other side of a set of frosted sliding glass windows. There must be someone working back there. On the window a small Dymo label reads, "Knock on window for service.""
			)
			((and (== theVerb 4) (== invItem 20)) ; Inventory, Doc_Pulliam_s_Card
				(TPrint 905 30) ; "Doc's office is already full of Doc's business cards."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
	...
	(method (changeState newState &tmp [temp0 2] [temp2 60])
		(switch (= state newState)
			...
			(6
				(switch windowTalkCount
					(1
						(Say gEgo 905 34) ; "Hello," you say in your most pleasant tone, "my name is Larry; Larry Laffer. I wonder if I might get an appointment with one of your dental hygienists?"
					)
					(else
						(Say gEgo 905 35) ; "Hello," you say to the receptionist. "I'd like an appointment to see one of your dental hygienists. I think I'm due for a little oral cavity work (if you know what I mean)."
					)
				)
				(= ticks 123)
			)
			...
			(9
				(gTheMusic2 stop:)
				(Say gEgo 905 38) ; ""Wait!" you cry, but it's too late."
				(= start (= register 0))
				(HandsOn)
				(NormalEgo 3)
				(self dispose:)
			...
			(10
				...
				(TPrint 905 39) ; "And she grabs a large piece of paper, covered with tiny type."
				(= ticks 123)
			)
			(11
			...
			(12
				(girl setCel: 0)
				(Format @temp2 905 41 (++ talkCounter)) ; "Question #%d"
				(switch talkCounter
					(0 0)
					(1
						(= register
							(Print 905 42 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you have dental insurance?"
						)
					)
					(2
						(= register
							(Print 905 43 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you have LOTS of dental insurance?"
						)
					)
					(3
						(= register
							(Print 905 44 #button {Yes} 1 #button {No} 0 #title @temp2) ; "Are your teeth overly sensitive?"
						)
					)
					(4
						(= register
							(Print 905 45 #button {Yes} 1 #button {No} 0 #title @temp2) ; "Are you allergic to pain?"
						)
					)
					(5
						(= register
							(Print 905 46 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you ENJOY pain?"
						)
					)
					(6
						(= register
							(Print 905 47 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you enjoy sharing your pain with others?"
						)
					)
					(7
						(= register
							(Print 905 48 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you enjoy mechanical objects in your mouth?"
						)
					)
					(8
						(= register
							(Print 905 49 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you enjoy other people's hands in your mouth?"
						)
					)
					(9
						(= register
							(Print 905 50 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you enjoy rubber in your mouth?"
						)
					)
					(10
						(= register
							(Print 905 51 #button {Yes} 0 #button {No} 1 #title @temp2) ; "Do you swallow?"
						)
					)
					(11
						(= register
							(Print 905 52 #button {Yes} 1 #button {No} 0 #title @temp2) ; "Have you ever even heard of AIDS?"
						)
					)
					(12
						(= register
							(Print 905 53 #button {Yes} 1 #button {No} 0 #title @temp2) ; "Do you know anyone who has ever hired a lawyer?"
						)
					)
					(13
						(= register
							(Print 905 54 #button {Yes} 1 #button {No} 0 #title @temp2) ; "Do you know the definition of `malpractice?'"
						)
					)
					(14
						(Points 13 164)
			...
			(15
				(gTheMusic2 stop:)
				(TPrint 905 38) ; ""Wait!" you cry, but it's too late."
				(= start (= register 0))
				(HandsOn)
				(NormalEgo 3)
				(self dispose:)
		...
		(switch theVerb
			...
			(3 ; Do
				(cond
					...
					(else
						(TPrint 905 61) ; "There is nothing here."
					)
				)
			)
			(else
