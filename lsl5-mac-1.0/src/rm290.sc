...
(instance chair of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 1) ; "How thoughtful! AeroDork has provided a comfortable metal folding chair for your sitting convenience!"
			)
			(3 ; Do
...
(instance door of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 2) ; "The door to the jetway is firmly locked, and without protrusions of any kind."
			)
			(3 ; Do
				(TPrint 290 3) ; "You can only open this door by using the ABM to the right."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boardingSign of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 32)
					(TPrint 290 4) ; "The Boarding Pass sign is currently flashing. Are you going to miss the plane?"
				else
					(TPrint 290 5) ; "It appears to be a sign."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance ABM of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(7 ; AeroDork_Gold_Card
						(TPrint 290 6) ; "Your AeroDork Gold Card will only work in the ATM outside the airport."
					)
					(8 ; Boarding_Pass
						(if (IsFlag 32)
							...
							(TPrint 290 7) ; "You're a little early. That flight isn't due to board for another few minutes."
						)
					)
					(else
						(super doVerb: theVerb invItem &rest)
			...
			(2 ; Look
				(TPrint 290 8) ; "Near the far door is one of AeroDork Airline's exclusive "ABMs.""
				(TPrint 290 9) ; "ABM is an acronym for "Automatic Boarding Pass Machine." So far, AeroDork is the only airline to have them."
				(TPrint 290 10 67 -1 185) ; "(You expect it to remain that way!)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance pot of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 11) ; "It's just another example of AeroDork's extravagant attitude towards its customers. Freshly brewed coffee just for you!"
			)
			(3 ; Do
				(TPrint 290 12) ; "YUCK! The pot has boiled dry! Nothing remains but the stench of burned coffee."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance cabinet of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 13) ; "The cabinet holds up the coffee maker."
			)
			(3 ; Do
				(TPrint 290 14) ; "There's nothing in the cabinet that you need."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance light of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 290 15) ; "Wouldn't you think the local transportation authorities would worry about exposed wiring?"
			)
			(3 ; Do
				(TPrint 290 16) ; "Right! Of course you can reach the ceiling in here!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance leftPainting of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 290 17) ; "There's nothing hidden behind the painting."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sDoAnnouncement of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 290 18) ; "A voice comes over the public address system, "AeroDork Airlines is proud to announce the departure of its Chartreuse Service to msdffft, wqpaffw, and paovnnq.""
				(TPrint 290 19) ; "Hmm. That speaker is none too clear."
				(= ticks 120)
			)
			(4
				(RestoreIB)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(6
				...
				(Format @str 290 21 @locStr) ; "This airport looks just like the one in %s!"
				(TPrint @str)
				(= ticks 60)
			)
			(7
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(3
				(Points 7 97)
				(if (== gLarryLoc 0)
					(TPrint 290 22) ; "Your boarding pass is sucked..."
					(TPrint 290 23) ; "...into the slot of the ABM!"
				)
