	...
	(if
		...
		(if (not (HaveMouse))
			(if (and (!= gTheCursor 20) (!= gTheCursor 30))
				(= global183 (mouseCursor x:))
				(= global184 (mouseCursor y:))
...
(instance hoyle3 of Game
	...
	(method (init &tmp temp0)
		...
		(localproc_0)
		(proc700_0)
		((ScriptID 12 5) doit:) ; setHelpStrings
		(proc0_3)
		(Characters
	...
	(method (quitGame &tmp [temp0 5] [temp5 5])
		(if (Print 99 5 #button (Format @temp0 99 6) 1 #button (Format @temp5 99 7) 0) ; "Quit HOYLE 3?", "Yes", "No"
			(super quitGame:)
		)
	)
	...
	(method (doit param1 &tmp [temp0 20] [temp20 10] temp30 temp31 temp32 temp33)
		(cond
			...
			(else
				...
				(Format @global150 0 2)
				(Format @global157 0 2)
				(Format @global164 0 2)
				(Format @global171 0 2)
				(Format @global472 0 2)
				(Format @global479 0 2)
				(Format @global486 0 2)
				(Format @global493 0 2)
				(return 0)
			)
		)
	)
