...
(procedure (localproc_0 param1 &tmp temp0 temp1 [temp2 25])
	(cond
		...
		(else
			(Characters
				say: (Format @global550 499 7 (param1 playerName:)) -32768 443 2 ; "%s goes first."
			)
		)
	)
...
(procedure (localproc_1 &tmp temp0 temp1 temp2 temp3)
	...
	(for ((= temp0 0)) (< temp0 temp2) ((++ temp0))
		...
		(if (== (temp1 type:) 1)
			...
			(Format @local2 499 3) ; "No"
			(Format @local4 499 2) ; "Yes"
			(= temp3 (Print 499 4 #button @local4 0 #button @local2 1)) ; "Would you like to continue the game until all have finished?"
			(break)
		)
	)
	(gSong fade:)
...
(instance startScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if global417
					(Print 499 5) ; "Click to start Pachisi."
				)
				(= global417 0)
				(= local0 0)
		...
		(switch (= state newState)
			...
			(4
				(if (== (global178 type:) 2)
					...
					(Format @temp0 499 8 (global178 playerName:)) ; "%s rolls to see who goes first."
					...
					(if temp28
						(if (== (++ global415) 1)
							...
							(cond
								...
								(else
									(Format @temp0 499 6 (global178 playerName:)) ; "Congratulations, %s, you have won."
									(Print @temp0)
								)
							)
						)
