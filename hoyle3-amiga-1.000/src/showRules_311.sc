	...
	(method (doit)
		...
		(while (!= local0 20)
			(switch local0
				...
				(18
					(Format @global550 399 54) ; "The following rules apply for Acey deucey: The Pieces all start on the Bar and may be entered at any time during the game. Pieces that are hit however must be entered before any other pieces are moved as in Backgammon."
				)
				(19
					(Format @global550 399 55) ; "When a player rolls `Acey Deucey' (1 & 2), he first takes the roll of 1 and 2, then as long as both the 1 and the 2 were playable he may turn the dice to any double number desired, plays that double, and then takes an additional turn."
				)
			)
			(cond
				((== local0 0)
