							...
							(if (== global188 2)
								(Print
									(Format ; "Congratulations %s. You win!"
										...
										299
										1
										((ScriptID 200 6) playerName:) ; player1
									)
								)
							else
							...
							(if (== global188 2)
								(Print
									(Format ; "Congratulations %s. You win!"
										...
										299
										1
										((ScriptID 200 7) playerName:) ; player2
									)
								)
							else
				...
				(if global245
					(if (== (global178 type:) 2)
						...
						(cond
							...
							((not gModelessDialog)
								(Print 299 4 #at 10 100 #dispose) ; "Jump Again"
							)
						)
						(= local5 1)
						(if (== global266 200)
	...
	(method (doit param1 &tmp [temp0 20] [temp20 8])
		(cond
			((and argc (== param1 3))
				(if (self open: 2)
					(self
						writeString: (Format @temp0 201 1 global244 global266) ; "%d%3d"
						close:
					)
					(return 1)
				else
