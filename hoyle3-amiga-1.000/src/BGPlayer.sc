...
(class BGPlayer of List
	...
	(method (checkLegal param1 param2)
		...
		(if (and (bar size:) (!= param1 bar))
			(cond
				...
				(aceyHits
					(gSound play: 905)
					(gChar1 say: (Format @global550 399 23 aceyHits) -32768) ; "You have %d stones on the bar which must be removed first."
					(return 0)
				)
			)
		)
	...
	(method (evaluateOverall param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11 temp12 temp13 temp14)
		...
		(if (== skill 2)
			...
			(if
				(and
					(<= bigPrime 3)
					(> ((NodeValue (whichPoints first:)) number:) 13)
					(< local242 3)
				)
				(= temp14 temp4)
				(= temp1 0)
		...
		(cond
			(local246
				(if (HaveMouse)
					...
					(gGame setCursor: (+ 100 local245) 1 305 185)
				)
	...
	(method (winGame &tmp temp0 temp1 [temp2 31])
		...
		(if (== global179 1)
			...
			(Format @global550 399 11 playerName) ; "%s won the game"
		)
		(if (>= temp1 2)
