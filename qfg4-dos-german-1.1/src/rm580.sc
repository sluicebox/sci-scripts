	...
	(method (init)
		...
		(if (and (== gHeroType 1) gNight (not gCombatMonsterNum)) ; Magic User
			...
			(switch local0
				...
				(2
					(aStaff init: cel: 11 approachVerbs: 4) ; Do
				)
				(3
					((ScriptID 50 1) caller: self) ; enterRoomScr
				)
...
(instance sEvent3 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(26
				(= local8 (Str new:))
				(Message msgGET 580 5 6 29 1 (local8 data:)) ; "I am now the Staff of %s."
				(= local9 (Str new:))
