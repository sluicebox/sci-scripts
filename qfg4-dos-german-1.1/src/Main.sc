	...
	(cond
		(
			...
			(cond
				...
				((or (< 2800 gClock 3700) (<= 0 gClock 771))
					(PalVary 9 (+ (gCurRoom picture:) 1)) ; PalVaryMergeSource
...
(class Glory of Game
	...
	(method (detailLevel param1)
		(if argc
			(= _detailLevel param1)
			(gPlanes eachElementDo: #checkDetail param1)
		)
		(return _detailLevel)
	)
	...
	(method (pragmaFail &tmp temp0)
		(if (User canInput:)
			(switch (= temp0 ((User curEvent:) message:))
				...
				(92
					(if (OneOf gCurRoomNum 270 460 670 740 750 730)
						(gCurRoom doVerb: 92)
					else
						(proc0_5)
					)
				...
				(90
					(if (OneOf gCurRoomNum 270 460 750)
						(gCurRoom doVerb: 90)
					else
						(proc0_5)
					)
		...
		(return
			(if (and (== argc 1) (== param1 1))
				...
				(if (>= temp10 0)
					...
					(= temp9 (GetSaveFiles name (temp0 data:) (temp1 data:)))
					(while
						(and (> temp9 0) (not (CheckFreeSpace (temp6 data:))))
