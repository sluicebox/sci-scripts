		...
		(switch (= state newState)
			...
			(3
				(if
					...
					(if (< (gEgo y:) (lChernovy y:))
						(lChernovy setCycle: Fwd setLoop: 6)
					else
						(lChernovy setCycle: Fwd setLoop: 0)
					)
				)
				(= cycles 60)
			)
			(4
				(if
					...
					(if (< (gEgo y:) (lChernovy y:))
						(rChernovy setCel: 0 setCycle: 0 setLoop: 13 1)
					else
						(rChernovy setCel: 0 setCycle: 0 setLoop: 9 1)
					)
					(= cycles 6)
				else
					(= cycles 1)
			...
			(6
				(if
					...
					(if (< (gEgo y:) (lChernovy y:))
						(rChernovy setCycle: Fwd setLoop: 7)
					else
						(rChernovy setCycle: Fwd setLoop: 1)
					)
				)
				(self changeState: 0)
			)
...
(instance lChernovy of TargActor
	...
	(method (getHurt param1 param2)
		...
		(if (== param1 21)
			...
			(-= local0 (/ (* param2 70) 100))
		)
		(if (and (not (== view 842)) (not (& signal $0008)) (<= local0 0))
...
(instance rChernovy of TargActor
	...
	(method (getHurt param1 param2)
		...
		(if (== param1 21)
			...
			(-= local1 (/ (* param2 70) 100))
		)
		(if (and (not (== view 842)) (not (& signal $0008)) (<= local1 0))
