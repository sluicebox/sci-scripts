	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(PrintDC 33 6) ; "Graham thinks about attempting to jump across the crevasse, but wisely chooses not to."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(29
							(PrintDC 33 7) ; "Perhaps with some momentum Graham could use this to cross the crevasse."
							(event claimed: 1)
						)
						(28
							(event claimed: 0)
						...
						(else
							(PrintDC 33 8) ; "It won't help Graham cross the crevasse."
							(event claimed: 1)
						)
					)
				)
...
(instance mountains of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 33 9) ; "In the distance, snowy mountains loom above a lovely river valley."
					(event claimed: 1)
				)
			)
		)
...
(instance sled of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 33 10) ; "Graham's sled lies in several broken pieces on the eastern side of the icy crevasse."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local0)
						(++ local0)
						(PrintDC 33 11) ; "The broken sled is now no good to anybody."
						(event claimed: 1)
					)
				)
				(4 ; Inventory
