...
(instance Emeralds of KQInvItem
	...
	(method (doVerb theVerb &tmp [temp0 50])
		(switch theVerb
			(2
				(StrCpy @temp0 (LangSwitch {The } {La }))
				(StrCat
					@temp0
					(switch global312
						(3
							(LangSwitch {trio of } {terna de })
						)
						(2
							(LangSwitch {pair of } {pareja de })
						)
						(1 {})
					)
				)
				(StrCat
					@temp0
					(LangSwitch {sparkling emerald} {brillantes esmeralda})
				)
				(StrCat @temp0 (if (> global312 1) {s} else {}))
				(StrCat @temp0 (LangSwitch { dazzle} { deslumbra}))
				(StrCat @temp0 (if (> global312 1) {} else {s}))
				(StrCat @temp0 (LangSwitch { the eyes.} { tus ojos.}))
				(PrintDC @temp0)
			)
			(else
				(super doVerb: theVerb)
...
(instance Leg_of_Lamb of KQInvItem
	...
	(method (doVerb theVerb &tmp [temp0 50])
		(switch theVerb
			(2
				(StrCpy @temp0 (LangSwitch {This is a } {Esto es }))
				(StrCat
					@temp0
					(if global316
						(LangSwitch {left-over, half} {media})
					else
						(LangSwitch
							{large, mouthwatering}
							{una grande y muy apetitosa}
						)
					)
				)
				(StrCat @temp0 (LangSwitch { leg of lamb.} { pierna de cordero.}))
				(PrintDC @temp0)
			)
			(else
				(super doVerb: theVerb)
