		...
		(if (and local7 (event type:) (== gCurRoomNum gNewRoomNum))
			...
			(switch
				(Print
					...
					addButton: 2 4 0 0 2 54 0 790 ; "Restart"
					addButton: 0 4 0 0 3 108 0 790 ; "Continue"
					addButton: 3 4 0 0 4 178 0 790 ; "Quit"
					init:
				)
				(0
					(gGameMusic1 pause: 0)
...
(instance sRunCredits of Script
	...
	(method (changeState newState &tmp [temp0 120] temp120)
		(switch (= state newState)
			...
			(4
				(if (and (== local0 36) (== local4 2))
					(= local4 3)
					(= local0 1)
				)
				(if (and (== local0 6) (== local4 3))
					(= local4 5)
					(= local0 1)
				)
				(= temp120 local0)
			...
			(5
				...
				(if (-- local5)
					...
					(crackSound play:)
					(if (not (OneOf local1 15 18 20 22 24))
						(self changeState: 0)
					)
				)
			)
...
(instance sRunOuttake of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(19
				(if (== register 4)
					(gMessager say: register 0 0 19 20 self 791) ; "OW! Worker's comp! Worker's comp!"
				else
					(= cycles 2)
				)
			)
