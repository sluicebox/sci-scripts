...
(instance sPullTeeth of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(barber view: 642 loop: 1 cel: 0 x: 170 y: 139)
				(UnLoad 128 647)
				(UnLoad 128 648)
				(self changeState: 0)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(57 ; Claim_Check
				...
				(= local1 1)
				(gGame points: 5)
				(gMessager say: 2 57 0 0 self) ; "Sal, do you still have that pair of boots I dropped off for a shine? I know it's been about six years now, but I just remembered I left them here!"
			)
			(42 ; Post_Cards
				...
				(= local2 1)
				(gGame points: 5)
				(gMessager say: 2 42 0 0 self) ; "You hand Salvatore the French Postcards."
			)
			(else
				(super doVerb: theVerb &rest)
			)
