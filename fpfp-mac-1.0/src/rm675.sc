...
(instance sRandomHand of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(armSound play:)
				(thirdHand setLoop: 1 setMotion: MoveTo 165 131 self)
			)
			(3
				(thirdHand stopUpd:)
				(self changeState: 0)
		...
		(switch (= state newState)
			(0
				...
				(thirdHand
					setScript: 0
					setLoop: 1
					setMotion: MoveTo 165 131 self
				)
				(proc0_8 0)
			)
			(1
				(gGame points: 10 141)
				(gMessager say: 14) ; "You're cheating! That's a fake left hand! Your real hand is hidden under the table!"
				(= cycles 1)
			)
			(2
				(thirdHand
