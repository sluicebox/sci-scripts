...
(instance rm240 of FPRoom
	...
	(method (init)
		...
		(cond
			((== local3 0) 0)
			((== gCurPuzzle 14) 0)
			((OneOf gPrevRoomNum 230 310)
				(gGameMusic1 number: 652 flags: 1 loop: -1 play: setVol: 80)
			)
			((== gPrevRoomNum 250)
				(gGameMusic1 number: 640 flags: 1 loop: -1 play: setVol: 80)
			)
			((== gPrevRoomNum 26) ; actBreak
				(gGameMusic1 number: 640 flags: 1 loop: -1 play: setVol: 0)
			)
		)
		(super init:)
		(gCurRoom
		...
		(if (IsFlag 1)
			...
			(if (!= gCurPuzzle 22)
				(if (!= gAct 1)
					...
					(shrub1 init: hide: stopUpd:)
					(shrub2 init: hide: stopUpd:)
					(barberpole init: cycleSpeed: 14 setCycle: Fwd)
				)
			)
			(if (>= gCurPuzzle 20)
	...
	(method (doit &tmp temp0 temp1 [temp2 2])
		...
		(cond
			...
			((and local3 (<= (gEgo x:) 219))
				(if (== (gGameMusic1 number:) 640)
					(gGameMusic1 number: 652 play:)
				)
			...
			((and local3 (>= (gEgo x:) 221))
				(if (> (gGameMusic1 vol:) 0)
					(gGameMusic1 setVol: 0)
				)
				(if (== (gGameMusic1 number:) 652)
					(gGameMusic1 number: 640 play:)
				)
				(cond
					(
						(<
