...
(instance sWaterSampleJoke of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if (== gCurPuzzle 13)
					(gMessager say: 28 0 0 0 self) ; "Hey, buddy, no cutting!"
				else
					(= cycles 1)
				)
			)
		...
		(switch (= state newState)
			...
			(4
				(if (and (not (IsFlag 96)) (== gCurPuzzle 13))
					(Message msgGET 320 42 0 18 1 @local12) ; "Plug-O-Meter"
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (not (IsFlag 100))
					(SetFlag 100)
					(gMessager say: 14 0 20 0 self) ; "You remember your father's words of warning about never using the topmost rungs of a ladder, so you stop just short of the ladder top."
				else
					(= cycles 1)
				)
			)
		...
		(switch theVerb
			(23 ; Tower_H20
				(if (or (IsFlag 45) (IsFlag 20) (IsFlag 21))
					...
					(gMessager say: noun 34 7) ; "You've got enough water in one of the bottles."
				)
			)
			(4 ; Do
				(cond
