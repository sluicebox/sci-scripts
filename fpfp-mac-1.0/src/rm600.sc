	...
	(method (init)
		...
		(cond
			((and (>= gCurPuzzle 2) (<= gCurPuzzle 6))
				(gGameMusic1 number: local8 flags: 1 loop: -1 play:)
			)
			((== gCurPuzzle 14) 0)
			((== gPrevRoomNum 250) 0)
			(else
...
(instance sEnterFrom250 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOn:)
				(localSound number: 2606 setLoop: 1 play:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(cond
					((gCast contains: srini)
						(gMessager say: 42 0 32) ; "Mind the store, Srini. I'm off to uphold justice and stuff."
					)
					((== gCurPuzzle 2)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gGameMusic1 number: local8 play:)
				(client
					view: local10
					setSpeed: 4
					setCycle: StopWalk -1
...
(instance sSheriffShutdown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(= gAct 1)
				(gMessager say: 39 0 0) ; "Fire inspection! I'm here to check out chur fire safety, per the new town regulations."
				(= cycles 1)
			)
			(4
				(= gAct 2)
			...
			(2 ; Talk
				(cond
					(
						...
						(gMessager say: 37 2 29) ; "Don't shout at Srini from behind the counter."
					)
					((== gCurPuzzle 13)
					...
					((== gCurPuzzle 20)
						(gMessager say: 37 2 25) ; "Srini, there's trouble down at the Saloon."
					)
					((== gCurPuzzle 22)
...
(instance penelope of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(71 ; Incorrect_Med
				(if local6
					(gMessager say: 45 0 0) ; "You were just informed that your prescription-fillin' work was SHODDY. You're not going to hand them the same bad prescription back, are you?"
				else
...
(instance medallion of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if local9
					(gMessager say: noun theVerb 4) ; "You can't reach the medallion from here. Perhaps you should set up some sort of pincers and mount it on a pivoting armature and get it that way."
				else
					(SetFlag 63)
					(gGame points: 1)
...
(instance sMovieGag of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(ghoul loop: 0 cel: 0 cycleSpeed: 9 setCycle: End self init:)
				(localSound number: 2948 play:)
			)
			(2
				(UnLoad 132 2948)
				(ghoul dispose:)
