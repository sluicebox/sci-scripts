		...
		(switch param1
			...
			(88
				(if (gEgo inRect: 252 148 308 178)
					(gEgo setMotion: PolyPath 241 176 self)
					(return 0)
				else
					(return 1)
				)
		...
		(switch (= state newState)
			...
			(11
				(Print
					...
					posn: 195 65
					width: 100
					addText: 2 45 0 3 ; "(NERVOUSLY. TO HIMSELF) Well, <sigh> here we go."
					init: self
				)
			)
			(12
			...
			(17
				(Print font: gUserFont posn: -1 20 addText: 2 45 0 5 init: self) ; "YES <a-hem> yes, sir."
			)
			(18
				(gGame givePoints: 4)
