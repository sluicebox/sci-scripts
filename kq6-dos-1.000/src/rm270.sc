		...
		(switch (= state newState)
			...
			(2
				(Print
					...
					ticks: 20
					posn: -1 10
					addText: 14 5 20 2 ; "Alexander picks up and leafs idly through a book called "The Changing Role of Court Entertainers Through the Ages.""
					init: self
				)
				(= cycles 1)
			)
			...
			(1 ; Look
				(if (== ((gInventory at: 1) owner:) gCurRoomNum) ; boringBook
					...
					(Print
						font: gUserFont
						posn: -1 10
						addText: noun theVerb 4 1 ; "There's a small table near the door that bears a sign. The sign has undergone a number of changes. It once read "10 pence," but that was crossed out and replaced with "5 pence," then "1 pence," then "free." The sign currently reads "Take one, PLEASE.""
						init:
					)
					(Print
						font: gUserFont
						posn: -1 10
						addText: noun theVerb 3 1 ; "The table is now empty."
						init:
					)
				)
			)
