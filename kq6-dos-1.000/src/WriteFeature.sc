...
(procedure (DoApproach obj &tmp event eX eY [str 10] ad)
	(if
		(Print
			...
			addButton: 0 {Default to x, y} 0 35
			init:
		)
		(while (!= ((= event (Event new:)) type:) evMOUSEBUTTON)
			(event dispose:)
	...
	(= ad
		(Print
			addText: {How far away must ego be before he tries to approach?}
			addEdit: @str 5 0 24
			addButton: 1 {Select with mouse} 0 38
			addButton: 0 {Always approach} 0 52
			init:
		)
	)
	(cond
...
(class WriteCode
	...
	(method (doit &tmp [temp0 15] temp15 temp16)
			...
			(not
				(= local390
					(Print
						...
						addButton: View {View} 71 12
						addButton: Prop {Prop} 110 12
						addButton: Actor {Actor} 150 12
						init:
					)
				)
			)
		...
		(if
			...
			(Print
				addText: {Click mouse on object's projection onto the ground}
				init:
			)
				...
				(not
					(= temp491
						(Print
							...
							addButton: 2 {Append} 73 24
							addButton: 0 {Cancel} 133 24
							init:
						)
					)
				)
