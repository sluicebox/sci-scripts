...
(local
	...
	[local71 51] = [79 9 3 137 9 3 223 9 3 28 38 1 56 58 5 117 43 2 156 47 4 225 28 3 290 39 1 266 54 1 114 93 6 182 101 1 235 136 2 273 138 4 500 500 0 500 500 0 500 500 0]
	[local122 21] = [82 41 1 49 81 1 185 85 1 279 81 1 196 129 1 500 500 0 500 500 0]
	[local143 78] = [0 0 110 14 33 25 105 0 229 14 191 15 230 0 319 14 272 25 0 14 60 75 57 95 184 40 233 65 226 66 63 14 225 75 152 76 223 14 319 75 284 96 0 74 82 97 173 144 80 74 168 97 61 120 166 74 219 95 183 94 244 74 319 107 195 113 0 117 79 154 195 113 172 92 200 154 320 146]
...
(procedure (localproc_4 param1)
	...
	(if (!= local4 19)
		(= global257 (+ 8 (/ [gEgoStats param1] 10)))
...
(instance wizGame of Rm
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4)
		...
		(= local2
			(cond
				((> [gEgoStats 12] 90) 1) ; magic
				((> [gEgoStats 12] 70) 2) ; magic
				((> [gEgoStats 12] 50) 3) ; magic
				(else 4)
			)
		)
		(SolvePuzzle 616 5 1)
	...
	(method (cue &tmp temp0 temp1)
		(cond
			...
			(local14
				(= local14 0)
				(gMessager say: 4 0 6 1 self) ; "The magic just isn't there."
				(= local11 1)
			)
			((IsFlag 337)
				(= local11 1)
			...
			(((wizBug mover:) completed:)
				(= local11 1)
				(gMessager say: 4 0 5 1 self) ; "Guess I still have the knack for the old Maze game."
			)
			((and (not (gEgo knows: 20)) (not local11)) ; dazzleSpell
				(= local11 1)
		...
		(switch (= state newState)
			...
			(1
				(if (gEgo has: 13) ; manaPotion
					(unknown_32_13 setScript: unknown_32_14)
				)
			)
			(2
				(self dispose:)
...
(instance unknown_32_13 of Prop
	(properties
		x 500
		y 500
		view 0
...
(instance unknown_32_14 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 1 0 0 1 self) ; "Click on the Magic Flask to drink a Magic Power Potion while playing the game."
			)
			(2
				(self dispose:)
			)
