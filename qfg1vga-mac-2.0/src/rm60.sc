...
(instance rm60 of Rm
	...
	(method (init)
		...
		(grass init:)
		(upperRocks init: setOnMeCheck: 1 2)
		(lowerRocks init: setOnMeCheck: 1 2)
	...
	(method (doit)
		...
		(if (and (== gNight 0) (!= local31 (GetTime 1))) ; SysTime12
			(= local31 (GetTime 1)) ; SysTime12
			(if
				(and
					(== local42 2)
					(not local32)
...
(instance rock1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(if local29
					(gMessager say: 7 1 21 1 self) ; "All of the Meeps are hiding deep within their holes. Your vicious attack has them terrified."
				else
					(gMessager say: 8 1 0 1 self) ; "The mountains are very steep here."
				)
			)
			(4 ; Do
				(HandsOff)
...
(instance sLiftRock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(client hide:)
				(gEgo setLoop: local28 setCel: 0 setCycle: End self)
			)
			(4
		...
		(switch (= state newState)
			...
			(4
				(if (not (-- register))
					...
					(cond
						((and (not local34) (not local35))
							(self changeState: 40)
						)
						((and (not local39) (not (Random 0 1)))
							(self changeState: 20)
						)
						(else
							(self changeState: 10)
			...
			(11
				(bossMeep setCel: 2)
				(Apple setCel: 1 posn: 165 87 setCycle: Fwd)
				(= cycles 2)
			)
			(12
				(Apple
...
(instance babySqueak of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= cycles (Random 3 8))
				(meepSound loop: 1 number: 2074 play:)
			)
			(1
				(self changeState: 0)
			)
		...
		(switch (= state newState)
			...
			(1
				(cond
					...
					((or local29 (<= (gEgo distanceTo: bossRock) 130))
						(self init:)
					)
					(else
						(meepSound loop: 1 number: (+ (Random 0 2) 2055) play:)
						(bossMeep
							z: 100
							setCel: 0
...
(instance meepTeller of Teller
	...
	(method (showDialog &tmp temp0)
				...
				(= temp0
					(super
						showDialog:
							...
							local33
							-9
							(and (not (IsFlag 228)) (not local34))
							7
							(not local36)
					)
				)
		...
		(if (and (== temp0 7) (not local36) (not local37) (not (IsFlag 227)))
			(SetFlag 227)
...
(instance sBossOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Load rsVIEW 1060)
				(meepSound number: 2075 loop: -1 play:)
				(= seconds 2)
			)
			(3
				(meep2 stopUpd:)
			...
			(12
				(meepSound loop: 1 number: 2058 play:)
