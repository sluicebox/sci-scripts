...
(instance rm320 of Rm
	...
	(method (init)
		...
		(leftFence init: setOnMeCheck: 26505)
		(rightFence init: setOnMeCheck: 26505)
		(roundStones init: setOnMeCheck: 26505)
		(squareStones init: setOnMeCheck: 26505)
		(aWindow init: setOnMeCheck: 26505)
		(rtWindow init: setOnMeCheck: 26505)
		(onGoodsStore init: setOnMeCheck: 1 2)
		(onSheriffHouse init: setOnMeCheck: 1 4)
		(onStable init: setOnMeCheck: 1 16)
		(onBarrels init: setOnMeCheck: 1 8)
		(onStand init: setOnMeCheck: 1 128)
		(storeDoor
			init:
			approachVerbs:
				4 ; Do
	...
	(method (doit)
		...
		(if (== gNight 1)
			(centaur actions: 0)
			(centaurBody actions: 0)
		else
			(centaur actions: hildeTeller)
			(centaurBody actions: hildeTeller)
		)
		(super doit:)
	)
...
(instance comeFromTown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (or gNight (not (centaur script:)))
					(HandsOn)
				)
				(self dispose:)
			)
