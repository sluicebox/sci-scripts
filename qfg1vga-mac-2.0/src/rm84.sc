...
(instance rm84 of Rm
	...
	(method (init)
		...
		(if (not (IsFlag 253))
			...
			(entrance init: setOnMeCheck: 1 2)
		)
		(super init:)
		(gLongSong fade:)
		(NormalEgo)
		(gEgo init: ignoreActors: 1)
		(mountainRocks init: setOnMeCheck: 1 2)
		(bushesLL init: setOnMeCheck: 1 4)
		(tree init: setOnMeCheck: 1 4)
		(bushesUR init: setOnMeCheck: 1 4)
		(bushesU init: setOnMeCheck: 1 4)
		(rocksUR init: setOnMeCheck: 1 8)
		(ground init:)
		(switch gPrevRoomNum
			(85
...
(instance sRandBounce of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (== (antwerp status:) 4)
					(antwerp cycleSpeed: (Random 8 10))
				)
				(self init:)
			)
		)
...
(instance sMagicRock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(SetFlag 252)
				(gEgo cycleSpeed: local9 moveSpeed: local9)
...
(instance openRock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(theRock setPri: 1 y: 43 z: 40 setCycle: End self)
				((gCurRoom obstacles:) delete: rockPoly)
				(rockPoly dispose:)
				(gCurRoom
					addObstacle:
...
(instance sAvoidEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gGame setCursor: gWaitCursor 1)
				(client status: 1 cycleSpeed: local9 setScript: sRandBounce)
				(if
					(and
...
(instance sFightSword of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(antwerp cycleSpeed: 6 setCycle: Beg self)
			...
			(16
				(antwerp setCycle: Fwd cycleSpeed: (Random 7 10))
				(= state 8)
				(= seconds 2)
			)
			(17
			...
			(18
				(HandsOn)
				(NormalEgo)
				(gEgo cycleSpeed: local9 moveSpeed: local8)
				(self dispose:)
			)
		)
	)
...
(instance sFightDagger of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(antwerp cycleSpeed: 8 setCycle: Beg self)
			...
			(15
				(cond
					...
					(else
						(NormalEgo)
						(gEgo loop: 1 cycleSpeed: local9 moveSpeed: local8)
						(gMessager say: 9 0 14 1 self) ; "Holy Mackerel!"
						(SetFlag 204)
						(HandsOn)
						(antwerp status: 0)
			...
			(16
				(antwerp setCycle: Fwd cycleSpeed: (Random 8 12))
				(= state 8)
				(= seconds 3)
			)
		)
