...
(instance rm82 of Rm
	...
	(method (init)
		...
		(LoadMany rsSOUND (if gNight 72 else 71))
		(if (IsFlag 241)
			(Load rsVIEW 531)
		)
		(super init: &rest)
		(ledge init: setOnMeCheck: 26505)
		(onWater init: setOnMeCheck: 1 4)
		(cliff init: setOnMeCheck: 1 16)
		(ground init: setOnMeCheck: 1 32)
		(onFall init: setOnMeCheck: 1 64)
		(theCliff init: setOnMeCheck: 1 2)
		(rocks init: setOnMeCheck: 1 8)
		(gLongSong stop:)
				...
				(cond
					...
					((CastSpell 17)
						(cond
							...
							(else
								(Print
									addText: 9 51 27 1 0 0 82 ; "The only thing you can open here is the door, and you're not skilled enough to do that."
									y: 10
									ticks: -1
									init:
								)
...
(instance smlSplash2 of Prop ; UNUSED
	(properties
		x 261
		y 54
		noun 12
...
(instance smlSplash4 of Prop ; UNUSED
	(properties
		x 296
		y 53
		noun 14
...
(instance knockScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(SetFlag 275)
				(rm82Sound number: 2078 loop: 1 play:)
				(= cycles 5)
			)
			(2
				(rm82Sound play:)
			...
			(4
				...
				(cond
					((gEgo inRect: 100 62 145 84)
						(rm82Sound number: 2104 loop: 1 play:)
						(SetFlag 274)
...
(instance goOnIn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(rm82Sound number: 2084 loop: 1 play:)
				(gCurRoom newRoom: 83)
			)
		)
	)
...
(instance deadlyTP of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(rm82Sound number: 2028 loop: 1 init: play:)
				(funkyEgo
					view: 81
					setLoop: 5
					cel: 2
			...
			(4
				(gEgo view: 537 setLoop: 0 setCel: 0 setCycle: CT 2 1 self)
			)
			(5
				(gEgo
					view: 537
			...
			(6
				(EgoDead 164 165 0 2 537) ; "That hermit knows his "Trigger" spells pretty well. He sure pulled the trigger on you (not to mention the plug) by teleporting you to the top of the falls without a barrel!"
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gEgo setLoop: 2 setMotion: 0 setCycle: 0 setPri: 12)
				(= ticks 10)
			)
			(2
				(gCurRoom
				...
				(rm82Sound number: 2009 loop: 1 play:)
			...
			(6
				...
				(rm82Sound number: 2010 loop: 1 play: self)
				(ShakeScreen 2)
				(= ticks 30)
			)
			(7
