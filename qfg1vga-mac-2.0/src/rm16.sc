...
(procedure (localproc_1)
	(Print addText: 9 0 0 2 0 0 16 y: 10 ticks: -1 init:) ; "There's not much point in that, now that you've already done what you came here to do."
)
...
(instance rm16 of Rm
	...
	(method (init)
		...
		(cliff init:)
		(theRoom init:)
		(grass init: setOnMeCheck: 1 2)
		(trees init: setOnMeCheck: 1 4)
		(yellowRock init: setOnMeCheck: 1 16384)
	)
		...
		(switch theVerb
			...
			(51 ; openSpell
				(cond
					...
					((< [gEgoStats 17] 35) ; openSpell
						(Print addText: 9 51 8 1 0 0 16 y: 10 ticks: -1 init:) ; "Your skill with the Open spell is not great enough to affect the plants."
					)
					((CastSpell 17)
						(SetFlag 192)
			...
			(82 ; fetchSpell
				(cond
					...
					((IsFlag 192)
						(gCurRoom setScript: unknown_16_37)
					)
					((== local3 0)
						(= local21 1)
...
(instance openUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(gMessager say: 9 0 18 1 self) ; "You take the seed."
			)
			(8
				(flyingSeed dispose:)
				(gEgo setCycle: Beg self)
...
(instance unknown_16_37 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 9 82 27 1 self) ; "You might have tried to cast a spell at the seed, but it's too late now."
			)
			(2
				(self dispose:)
			)
