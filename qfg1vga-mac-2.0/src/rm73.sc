...
(instance rm73 of Rm
	...
	(method (init)
		(Load rsSOUND 2031 2029)
		(Load rsSCRIPT 101)
		(super init: &rest)
		(if global427
			(phonyDagger init: stopUpd:)
		)
		(target init: setOnMeCheck: 26505)
		(wall init: setOnMeCheck: 26505)
		(buildings init: setOnMeCheck: 26505)
		(trees init: setOnMeCheck: 1 2)
		(appleTree init: setOnMeCheck: 1 2)
		(= local9 0)
		(cond
	...
	(method (doVerb theVerb &tmp temp0)
		(switch theVerb
			...
			(82 ; fetchSpell
				(if (or (IsFlag 261) (not (gCast contains: brutus)))
					(Print addText: 4 82 0 1 0 0 73 y: 10 ticks: -1 init:) ; "You waste a spell. Fetch is only good for fetching small, visible objects."
				else
					(Print addText: 4 82 2 1 0 0 73 y: 10 ticks: -1 init:) ; "Just wait. The nice brigand will throw you his knife."
				)
			)
			(else
...
(instance brutus of TargActor
	...
	(method (doVerb theVerb &tmp temp0)
		(switch theVerb
			...
			(82 ; fetchSpell
				(if (or (not (gCast contains: brutus)) (IsFlag 261))
					(Print addText: 2 82 1 1 0 0 73 y: 10 ticks: -1 init:) ; "You waste a spell. Fetch is only good for fetching small, visible objects."
				else
					(Print addText: 2 82 2 y: 10 ticks: -1 init:) ; "Just wait. The nice brigand will throw you his knife."
				)
			)
			(else
				(super doVerb: theVerb &rest)
	...
	(method (getHurt)
		(if (not (IsFlag 261))
			...
			(gMessager say: 2 65 18 1) ; "Brutus is already dead."
		)
		(= global414 global280)
	)
)
...
(instance brutusTalker of Talker
	...
	(method (init)
		(= font gUserFont)
		(super init: brutusBust brutusTalkerMouth brutusEyes &rest)
	)
)
