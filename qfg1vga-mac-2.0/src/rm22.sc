	...
	(method (init)
		...
		(if (not (IsFlag 173))
			(if (not (== global274 4))
				...
				(= local2 221)
				(= local3 109)
				(feet posn: 156 152 init: stopUpd:)
			)
			(hut posn: local2 local3 init: stopUpd:)
			(if (>= global273 5)
...
(instance unknown_22_22 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 6 0 27 1 self) ; "You again?? Do you really want to go back in there?"
			)
			(2
				(gGame setCursor: gWaitCursor)
				(gateEyes dispose:)
...
(instance unknown_22_23 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 6 0 21 1 self) ; "So you made it back, did you? She is expecting you."
			)
			(2
				(gGame setCursor: gWaitCursor)
				(gateEyes dispose:)
				...
				(switch global273
					...
					(else
						(if (and (IsFlag 11) (gEgo has: 21)) ; mandrake
							(gate setScript: unknown_22_23)
						else
							(gate setScript: unknown_22_22)
						)
					)
				)
			)
