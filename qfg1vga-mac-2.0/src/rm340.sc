...
(procedure (localproc_8 param1) ; UNUSED
	(cond
		((< param1 0)
			(= param1 0)
		)
...
(instance rm340 of Rm
	...
	(method (init)
		...
		(gTheIconBar disable: 1 5 8)
		(self setScript: dagnabitScript)
		(theBoard init:)
		(angleF init:)
		(bead init: hide:)
		(forceF init: setOnMeCheck: 1 8)
		(gGame setCursor: 942)
		(gTheIconBar curInvIcon: 0)
	)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(5
				...
				(hand setCel: 0)
				(dagMusic stop:)
				(dagMusic number: (if local20 2029 else 2030) play:)
				(cond
					((> local12 (+ local10 10))
						(= temp0 0)
					)
