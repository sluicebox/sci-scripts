...
(procedure (EgoDead param1 param2 param3 param4 param5 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 [temp8 200] [temp208 70] [temp278 4] [temp282 4])
	(PalVary pvUNINIT)
	...
	(if global275
		(gLongSong number: global275 priority: 15 loop: 1 init: play:)
	)
	(= temp6 (+ (CelWide temp4 temp3 temp2) 10))
	(Message msgGET 815 1 0 temp1 1 @temp208)
	(TextSize @temp282 @temp208 123 260)
	(Message msgGET 815 1 0 temp0 1 @temp8)
	(TextSize @temp278 @temp8 gUserFont (- 260 temp6))
	(= temp5 (+ (- [temp282 2] [temp282 0]) 10))
	(= temp7
		(+
			(Max
				(+ (- [temp278 2] [temp278 0]) 10)
...
(procedure (PrintTimeAndDay &tmp temp0 [temp1 50])
	...
	(Print font: gUserFont addTextF: {%s on day %d.} @temp1 temp0 init:)
	(gGame setCursor: local1)
)

(procedure (EgoGait param1 param2 &tmp temp0 temp1 temp2 temp3)
	...
	(= temp1
		(if
			(and
				...
				(= temp2 ((gEgo mover:) finalX:))
			)
			(= temp3 ((gEgo mover:) finalY:))
		)
	)
	(if (!= param1 -1)
		(= gEgoGait param1)
	...
	(switch gEgoGait
		(1 ; running
			(gEgo setMotion: 0 view: 5 setStep: 8 4 setCycle: StopWalk 4)
			((gTheIconBar at: 1) loop: 6 cursor: 937)
		)
		(2 ; sneaking
			(gEgo setMotion: 0 view: 6 setStep: 3 2 setCycle: StopWalk 8)
	...
	(if temp1
		(gEgo setMotion: PolyPath temp2 temp3)
	)
)
...
(procedure (FixTime param1 param2 &tmp temp0 temp1 temp2)
	...
	(cond
		((and (== gNight 0) (> gClock 3000))
			(= gNight 1)
			(PalVary pvINIT (gCurRoom picture:) 1)
		)
		((and (== gNight 1) (> gClock 750) (< gClock 1200))
			(PalVary pvREVERSE 1)
			(= gNight 0)
		)
	)
				...
				(cond
					((not (IsFlag 110))
						...
						(if (not gModelessDialog)
							(Print
								addText: 1 0 0 10 0 0 814 ; "You are so exhausted that everything you do hurts. Better get some rest."
								y: 10
								ticks: -1
								init:
							)
...
(procedure (CastSpell param1 &tmp temp0)
	(if (< [gEgoStats 16] [global176 (+ (- param1 17) 1)]) ; mana
		(Wait 30)
		(Print addText: 1 0 0 11 0 0 814 y: 10 ticks: -1 init:) ; "You don't have enough Magic Points to cast that spell."
		(= temp0 0)
	else
		(TrySkill param1 0)
...
(procedure (SoundFX param1) ; UNUSED
	(return param1)
)
...
(procedure (CastArea param1)
	(switch param1
		...
		(51
			(if global279
				(Print addText: 1 64 2 1 0 0 814 y: 10 ticks: -1 init:) ; "That spell would not be useful now."
			else
				(CastOpen)
			)
		)
		...
		(82
			(Print addText: 1 0 0 17 0 0 814 y: 10 ticks: -1 init:) ; "Everything here must be bolted down somehow."
		)
		(79
