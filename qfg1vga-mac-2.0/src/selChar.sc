...
(instance selChar of Rm
	...
	(method (init)
		...
		(roundRobin start: (if (== gPrevRoomNum 203) 3 else 2)) ; chAlloc
	...
	(method (doit)
		(cond
			...
			((and local0 (== (fightChar cel:) 2))
				(gLongSong2 number: 2993 setVol: 127 loop: 1 play:)
				(= local0 0)
			)
			((and local1 (== (thiefChar cel:) 2))
				(gLongSong2 number: 2109 setVol: 127 loop: 1 play:)
				(= local1 0)
			)
		)
		(super doit: &rest)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(12
				...
				(= seconds 5)
				(gLongSong2 number: 2028 setVol: 127 loop: 1 play:)
			)
			(13
