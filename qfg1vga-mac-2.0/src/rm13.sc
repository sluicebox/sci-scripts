...
(instance rm13 of Rm
	...
	(method (init)
		...
		(gLongSong fade:)
		(super init:)
		(cave init: setOnMeCheck: 1 8)
		(rocks init: setOnMeCheck: 1 2)
		(trees init: setOnMeCheck: 1 4)
		(ClearFlag 160)
		(NormalEgo)
		(gEgo init:)
		(cond
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (IsFlag 332)
					(if (IsFlag 333)
						(gCurRoom setScript: unknown_13_5)
					else
						(gCurRoom setScript: unknown_13_6)
					)
				else
...
(instance unknown_13_5 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 2 1 1 1 self) ; "The dead Ogre's chest lies open and empty."
			)
			(2
				(self dispose:)
			)
...
(instance unknown_13_6 of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 2 1 4 1 self) ; "The dead Ogre's chest lies open, its contents glittering brightly."
			)
			(2
				(self dispose:)
			)
		...
		(switch theVerb
			...
			(80 ; calmSpell
				(cond
					...
					((IsFlag 331)
						(gMessager say: 3 80 5) ; "How much calmer can he get?"
					)
					(else
						(ogre setScript: ogreCalmed)
					)
				...
				(switch local14
					(0
						(if
							...
							(Print addText: 4 0 19 1 0 0 13 ticks: 8 init:) ; "You retrieve your daggers."
						)
						(gEgo get: 6 global251) ; dagger
						(= [global340 6]
							(= global251 (= global250 (= global252 0)))
						)
						(gMessager say: 4 0 13 0) ; "A search of the dead Ogre's body produces no treasure."
						(if (IsFlag 332)
							(= local14 4)
						else
							(= local14 3)
					...
					(1
						(gMessager say: 4 0 21 1) ; "The lock on the Ogre's chest clicks open. The dead Ogre's chest contains 1 gold and 43 silver, which you take and put away."
						(SetFlag 332)
						(SetFlag 333)
						(= local14 4)
					...
					(2
						(gMessager say: 4 0 14 0) ; "You force the Ogre's chest open."
						(SetFlag 332)
						(SetFlag 333)
						(aChest setCel: 2 stopUpd:)
						(gEgo get: 38 1 get: 0 43) ; gold, silver
					...
					(4
						(if (IsFlag 333)
							(gMessager say: 4 0 9 1) ; "The dead Ogre's chest is empty."
						else
							(gMessager say: 4 0 14 2) ; "The dead Ogre's chest contains 1 gold and 43 silver, which you take and put away."
							(gEgo get: 38 1 get: 0 43) ; gold, silver
							(SetFlag 333)
						)
						(self cue:)
