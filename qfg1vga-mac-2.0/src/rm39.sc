...
(instance rm39 of Rm
	...
	(method (init &tmp temp0 temp1)
		...
		(if (or (== (gLongSong prevSignal:) -1) (!= (gLongSong number:) temp1))
			(gLongSong stop:)
			(gLongSong number: temp1 loop: -1 priority: 0 play:)
		)
		(if (not gNight)
			(rightGuard init: setPri: 2 stopUpd:)
			(leftGuard init: setPri: 2 stopUpd:)
		)
		(castle init: setOnMeCheck: 1 2)
		(barrels init: setOnMeCheck: 1 4)
		(waggon init: setOnMeCheck: 1 2048)
		(curbLeft init: setOnMeCheck: 1 32)
		(curbRite init: setOnMeCheck: 1 8192)
		(wall init: setOnMeCheck: 1 64)
		(court init: setOnMeCheck: 1 256)
		(if (and (< 0 gTimeOfDay 4) (not (IsFlag 249)) (!= global246 gDay))
			(cond
				((IsFlag 239)
		...
		(switch (= state newState)
			(0
				(if (< [gEgoStats 11] 10) ; climbing
					(gEgo setMotion: PolyPath (gEgo x:) 220 self)
				else
					(gCurRoom newRoom: 37)
				)
			)
