...
(class LSL6 of Game
	...
	(method (play)
		...
		(= global182 (Memory memALLOC_NONCRIT 150))
		(if (not (IsFlag 97))
			(Message msgGET 0 7 0 0 (Random 1 15) global182)
		else
			(Message msgGET 0 7 0 0 16 global182) ; "(c) 1994 Sierra On-Line, Inc. Thank you for playing Leisure Suit Larry 6: "Shape Up or Slip Out!"  What did you think you were going to see, you pervert? Gotcha!"
		)
	...
	(method (save param1 &tmp [temp0 100] [temp100 50])
		...
		(cond
			((IsFlag 86)
				(Print addText: 0 0 15 1 0 0 0 init: self) ; "There's not enough free low memory to save at this time. Please try again later."
			)
			((== gCurRoomNum 740)
				...
				(if (or gModelessDialog (gCurRoom script:) isHandsOff)
					...
					(switch
						(Print
							...
							addButton: 0 8 0 3 1 (proc79_4 68 38 68 38 38) 30 0 ; "Screw It"
							addButton: ; "Change Interval"
								2
								8
								0
								6
								1
								(proc79_4 121 91 121 91 91)
								30
								0
							init: revertCursor
						)
...
(instance saveTimer of Timer
	...
	(procedure (localproc_0)
		(= client 0)
		(if (or (gCurRoom script:) (== gCurRoomNum 740))
			(= seconds 5)
		else
			(gGame save: 1)
		)
