		...
		(switch (= state newState)
			(0
				...
				(if register
					...
					(gMessager say: 1 4 73 1 self 610) ; "You've reached the Fresno Insult Line."
				)
			)
			(1
				(if (> gHowFast 12)
			...
			(2
				...
				(if (localproc_0 local5)
					...
					(localproc_1 local5)
					(if (and (not local6) (!= local5 43))
						(= state 0)
					)
					(if (or local6 (== local5 9) ((gUser curEvent:) type:))
						(gTheIconBar handleEvent: (gUser curEvent:))
						(= ticks 60)
					else
						(gMessager say: 1 4 local5 1 self 611)
					)
				)
			)
			(3
				(if (and (not local6) (== local5 43))
					(= state 0)
					(gMessager say: 1 4 local5 3 self 611)
				else
					(= ticks 60)
				)
			)
		...
		(switch (= state newState)
			...
			(9
				...
				(switch register
					...
					(-6832
						(self setScript: insultScr self 0)
					)
					(-24594
						(self setScript: insultScr self 0)
					)
