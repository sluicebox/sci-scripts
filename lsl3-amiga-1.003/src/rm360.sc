		...
		(cond
			(
				(or
					...
					(Said 'drop,drop/keycard,card/door,hole')
					(Said 'drop,drop//keycard,card')
			...
			((Said '/blouse')
				(Print 360 6) ; "You'd rather order a "Leisure Suit Larry 3" T-shirt from Sierra. Just call 1 (800) 326-6654 (or 1 (209) 683-4468) to buy yours today!!"
			)
			((Said 'get/soap')
				...
				(cond
					...
					((Said '/man')
						(if (gCast contains: aRobin)
							(Print 360 13) ; "You don't see anyone."
						else
							(Print 360 14) ; "Behind the counter you see a man."
						)
					)
					((Said '/buffet,buffet<behind,back')
						(if gEgoIsPatti
							(Print 360 15) ; "Looking behind the counter you see..."
							(Print 360 16) ; "...nothing."
						else
							(Print 360 17) ; "There's a man behind the counter."
						)
					)
					((Said '/buffet,buffet')
						(if gEgoIsPatti
							...
							(Print 360 19) ; "A bored man stands behind the counter, patiently counting the minutes until his shift ends."
						)
					)
					((Said '/burn')
