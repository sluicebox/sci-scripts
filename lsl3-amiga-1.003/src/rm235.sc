...
(instance RoomScript of Script
		...
		(cond
			...
			((Said 'get,pick/flower')
				(cond
					...
					((not (InRoom 11))
						(Print 235 10) ; "You need no more orchids."
					)
					(
						(and
						...
						(Print 235 11) ; "Move closer to the flowers near the cave opening. They are more beautiful since they catch more light."
					)
					(else
						(self changeState: 1)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (TestFlag 54)
					(Print 235 12) ; "Fortunately for you, there are plenty of orchids available."
				else
					(SetFlag 54) ; scoredOrchids
					(gGame changeScore: 25)
					(Print 235 13) ; "You pluck a few fresh orchids from the walls of the crevice."
				)
				(NormalEgo)
				(SetRgTimer 1 10 0)
			)
