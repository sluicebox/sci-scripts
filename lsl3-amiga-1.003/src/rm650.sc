...
(procedure (Print650 w &tmp t)
	...
	(Print @string #at -1 10 #width w #time (= t (PrintDelay @string)) #dispose)
	(return (+ 2 t))
)
		...
		(switch (= state newState)
			...
			(20
				(if (TestFlag 43)
					...
					(Format @string 650 10) ; "You know, you could find some of your missing points back at "The Comedy Hut.""
					(= seconds (Print650))
				)
			)
			(21
				(if (TestFlag 70)
					...
					(Format @string 650 11) ; "What a shame you didn't "sleuth" more when you had your divorce."
					(= seconds (Print650))
				)
			)
			(22
				(if (TestFlag 11)
					...
					(Format @string 650 12) ; "Isn't it surprising that you spent so much time at a tropical resort and never took time out to improve your tan?"
					(= seconds (Print650))
				)
			)
			(23
				(if (not (TestFlag 56))
					(Format @string 650 13) ; "You wonder now why you ignored Chip 'n' Dale's on your way to the bamboo forest."
					(= seconds (Print650))
				else
					(self cue:)
				)
			...
			(24
				(if (TestFlag 52)
					...
					(Format @string 650 14) ; "Do you remember when you were at Fat City? Someone was waiting for you, but you missed her!"
					(= seconds (Print650))
				)
			)
			(25
				(if (TestFlag 45)
					...
					(Format @string 650 15) ; "You probably don't know it, but a lot of the points that are missing above got lost when you didn't completely explore the island in your showgirl costume."
					(= seconds (Print650))
				)
			)
			(26
