		...
		(if (Said 'caress,sharpen/ginsu>')
			(cond
				...
				((Said '//fountain,barstool,barstool')
					(Print 250 5) ; "The fountain's rough surface would only dull your knife."
				)
				((not (Said '//stair,carpet'))
					(Print 250 6) ; "That would never sharpen a fine ginsu knife."
				)
				((not onSteps)
					(Print 250 7) ; "Move closer to the steps."
				)
				((!= gEgoState 0)
		...
		(if (Said 'look>')
			(cond
				((Said '/palm,bush,carpet')
					(Print 250 8) ; "The landscaping of the casino blends nicely into the jungle beyond."
				)
				((Said '/cannibal,office')
					(if gEgoIsPatti
						(Print 250 9 gEgoName) ; "You have no need to go that way, %s."
					else
						(Print 250 10) ; "Down the pathway to the lower left is the headquarters of "Natives, Inc.""
					)
				)
				((Said '/water,cascade,fountain')
					(Print 250 11) ; "Isn't it beautiful?"
				)
				((Said '/stair,exit,carpet')
					(Print 250 12) ; "The steps leading into the casino are made of smooth concrete... as smooth as a sharpening stone."
				)
				((Said '[/area]')
					(Print 250 13) ; "You are outside the island's casino. Steps lead up into the lobby, while water splashes merrily down the fountain."
					(if gEgoIsPatti
						(Print 250 14) ; "Through the trees, you see a closed sign on the headquarters of "Natives, Inc.""
					)
				)
			)
		)
