	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if (== (aObstacle loop:) 3)
					(-- lastLoop)
					(Print 560 0 #at -1 10) ; "Lookout, Patti! It's your worst nightmare!"
				)
			)
			(3
				(if (== (aObstacle loop:) 3)
					(Print 560 1 #at -1 10) ; "Children!!"
				)
				(gGame changeScore: lastLoop)
				(self changeState: 0)
			)
			(4
				(Print 560 2 #at -1 10) ; "You made it!"
				(gGame changeScore: (- 150 (- gScore startingScore)))
				(if (TestFlag 14)
					(Printf ; "Attention: QA-type! You will now get %d bonus points to bring your score up to this room's maximum score of %d. (You started with %d and are ending with %d.) Does this seem fair to you?"
						560
						3
						(- 150 (- gScore startingScore))
						150
						startingScore
						(+ gScore (- 150 (- gScore startingScore)))
					)
				)
				(gGame changeScore: (- 150 (- gScore startingScore)))
				(= seconds 3)
