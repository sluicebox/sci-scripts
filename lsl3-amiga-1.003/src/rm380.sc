		...
		(cond
			...
			((Said 'lie,(work<out),use')
				(cond
					...
					(else
						(Print 380 5) ; "There are four separate workout stations on Fat City's exercise equipment: the Leg Curl bench, the Bench Press weight-lifting bench, the Pullup station, and the Bar Pull."
			...
			(
				...
				(cond
					...
					((>= gBarPullCount BarPullMax)
						(Print 380 7) ; "You've done enough bar pulls! How about something else?"
					)
					((& (gEgo onControl:) $0020)
						(self changeState: 28)
					)
			...
			((and (Said 'look>') (Said '[/equipment,pos,area]'))
				(Print 380 16) ; "There are four separate workout stations on Fat City's exercise equipment: the "Leg Curl" bench, the "Bench Press" weight-lifting dumbbell bench, the "Pullup" station, and the "Bar Pull.""
			)
		)
	)
