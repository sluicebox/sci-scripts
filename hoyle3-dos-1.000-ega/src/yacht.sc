...
(procedure (proc500_2 &tmp [temp0 300] [temp300 100])
	...
	(= local56 (gSystemWindow color:))
	(= local57 (gSystemWindow back:))
	(gSystemWindow color: 16 back: 21)
	(StrCpy @temp0 (Format @temp300 500 3)) ; "Yacht Scorecard"
	(StrCat @temp0 (Format @temp300 500 4)) ; "Current Total Score"
	(StrCat @temp0 (Format @temp300 500 5 local60)) ; "Players  Score After %d Game"
	(if (!= local60 1)
		(StrCat @temp0 (Format @temp300 500 6)) ; "s"
	)
	(StrCat @temp0 (Format @temp300 500 7)) ; "-------------------------------"
	...
	(StrCat @temp0 (Format @temp300 500 9)) ; "-------------------------------"
	(if local59
		(StrCat @temp0 (Format @temp300 500 10 local58 (local59 playerName:))) ; "High Game: %3d by %-10s"
	)
	(Print @temp0 33 500 70 188 30 0 179 gDoAbsCode 67 50 -1)
	(gSystemWindow color: local56 back: local57)
	(gSong fade:)
)
...
(class Score of TrackingView
		...
		(if (super handleEvent: event)
			...
			(if (< (rollButton loop:) 6)
				(if (== global178 global500)
					(cond
						((or local61 (== (global178 rollsLeft:) 3))
	...
	(method (showScore param1)
		...
		(if (>= ((global500 at: (- loop 1)) finalScore:) 0)
			(Format @global550 500 0 ((global500 at: (- loop 1)) finalScore:)) ; "%3d"
			(self doDisplay:)
		)
	)
...
(class Keepers of Set
	...
	(method (nextSpace param1 &tmp temp0 temp1)
		(= temp1 (if (not size) [local42 0] else 0))
		(for ((= temp0 0)) (< temp0 5) ((++ temp0))
			(if
				(and
					(or (not argc) (!= param1 [local42 temp0]))
					(self spaceOpen: [local42 temp0])
				)
				(= temp1 [local42 temp0])
				(break)
			)
...
(instance yacht of Rm
	...
	(method (init &tmp temp0 temp1 temp2)
		...
		(if (yachtPlayer1 type:)
			(++ temp0)
			(yachtPlayer1
				color: (if global294 [local38 temp0] else [local34 temp0])
				init:
			)
		)
		(if (yachtPlayer2 type:)
				...
				(if (> (playersList size:) 1)
					...
					(cond
						...
						(else
							...
							(Characters
								say:
									(Format @global550 500 1 (temp1 playerName:)) ; "Congratulations %s, You Won!"
							)
						)
					)
					(proc0_1)
...
(instance totals of Score
	...
	(method (showScore param1)
		...
		(Format @global550 500 0 finalScore) ; "%3d"
		(Display
			@global550
			dsFONT
			500
	...
	(method (doit param1 &tmp [temp0 20] [temp20 8])
		(cond
			((and argc (== param1 3))
				(if (self open: 2)
					(self
						writeString: (Format @temp0 500 11 global519 global536) ; "%d%d"
						close:
					)
					(return 1)
				else
