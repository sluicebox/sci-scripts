	...
	(method (changeState newState &tmp temp0 [temp1 10])
		(switch (= state newState)
			...
			(2
				(gTheIconBar disable:)
				(Format @temp1 975 0)
				(Format @global150 975 1) ; "Player 1"
				(Format @global157 975 2) ; "Player 2"
				(Format @global164 975 3) ; "Player 3"
				(Format @global171 975 4) ; "Player 4"
				...
				(if (== global187 100)
					(Print 975 5 41 @global150 10 81 {OK}) ; "Enter your name:"
					...
					(if (not (mod (human1 loop:) 2))
						(Print 975 6 41 @global150 10 81 {OK}) ; "Player 1, Enter your name:"
						(StrCpy @global472 @global150)
						(if (not (StrCmp @global150 @temp1))
							(Format @global150 975 1) ; "Player 1"
						)
					...
					(if (not (mod (human2 loop:) 2))
						(Print 975 7 41 @global157 10 81 {OK}) ; "Player 2, Enter your name:"
					...
					(if (not (mod (human3 loop:) 2))
						(Print 975 8 41 @global164 10 81 {OK}) ; "Player 3, Enter your name:"
					...
					(if (not (mod (human4 loop:) 2))
						(Print 975 9 41 @global171 10 81 {OK}) ; "Player 4, Enter your name:"
...
(instance chooseControls of GameControls
	...
	(method (dispatchEvent event)
		(if (and (== (event type:) evKEYBOARD) (== (event message:) KEY_ESCAPE))
			(Print 975 10) ; "The Icon Bar is not available at this time."
	...
	(method (doit &tmp temp0)
		...
		(cond
			((or (< (= temp0 (+ global188 global189)) local1) (> temp0 local0))
				(if (!= local1 local0)
					(Print (Format @global550 975 11 local1 local0)) ; "You must select between %d and %d Players."
				else
					(Print (Format @global550 975 12 local1)) ; "You must select %d Players."
				)
			)
			((== global188 0)
				(Print 975 13) ; "At least one human player must play."
			)
			(else
				(gGameControls hide:)
...
(instance iconInfo of ControlIcon
	...
	(method (doit &tmp temp0)
		(switch global187
			(400
				(Print 975 14 #at -1 10) ; "Select the number of players that will play and the type of players that they will be, and also the color of their playing pieces. The button colors match the playing pieces."
				(Print 975 15 #at -1 10) ; "Also, you may choose the placement of the players on the Pachisi board. Player 1 is in the upper left corner, and the rest follow in a clockwise manner."
				(Print 975 16 #at -1 10) ; "From 2 to 4 players of any combination may play as long as there is at least 1 human player selected."
			)
			(200
				(Print 975 17 #at -1 10) ; "Select the type of players that will play, and also the color of their playing pieces. The button colors match the playing pieces."
				(Print 975 18 #at -1 10) ; "The player playing with the black pieces (or green pieces if playing with the frogs) always goes first. The computer's pieces are always at the top of the screen."
				(Print 975 19 #at -1 10) ; "2 players of any combination may play as long as there is at least 1 human player selected."
			)
			(300
				(Print 975 17 #at -1 10) ; "Select the type of players that will play, and also the color of their playing pieces. The button colors match the playing pieces."
				(Print 975 20 #at -1 10) ; "Player 1's inner table is always at the bottom right of the screen. Player 2's inner table is always at the top right of the screen."
				(Print 975 19 #at -1 10) ; "2 players of any combination may play as long as there is at least 1 human player selected."
			)
			(500
				(Print 975 21 #at -1 10) ; "Select the type of players that will play. In Yacht, Player 1 always plays first, then player 2, ect."
				(Print 975 22 #at -1 10) ; "1 to 4 players of any combination may play as long as there is at least 1 human player selected."
			)
		)
	)
)
