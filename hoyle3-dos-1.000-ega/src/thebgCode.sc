...
(procedure (localproc_0)
	...
	((= gGameControls backGamControls)
		...
		add:
			...
			(acey theObj: acey selector: #doit yourself:)
...
(instance acey of ControlIcon
	(properties
		...
		nsTop 23
		signal 387
		helpStr {If this button is set to the On position, two human Players may play Acey Deucey. Computer opponents do not play Acey Deucey.}
		highlightColor -1
	)
	...
	(method (doit)
		...
		(if (or (== (global303 type:) 2) (== (global304 type:) 2))
			...
			(self loop: 4)
			(self show:)
			(Print 310 0 #at -1 160) ; "Acey Deucey may not be played with computer opponents playing."
		else
			(self show:)
			(if global315
				(Print 310 1 #at -1 155) ; "Acey Deucey rules will take effect at the beginning of the next game."
			else
				(Print 310 2 #at -1 155) ; "Backgammon rules will take effect at the beginning of the next game."
			)
		)
	)
)
...
(instance altBoard of ControlIcon
	(properties
		...
		nsTop 65
		signal 387
		helpStr {You may select which Backgammon board that you want to play on.}
		highlightColor -1
	)
...
(instance doubl of ControlIcon
	(properties
		...
		nsTop 23
		signal 387
		helpStr {Allows using Doubling cube or not.}
		highlightColor -1
	)
