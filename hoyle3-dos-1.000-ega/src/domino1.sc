...
(procedure (proc100_1 &tmp temp0 temp1 temp2)
	...
	(Print (Format @global550 100 5 (global111 playerName:)) #at -1 17 #dispose) ; "%s's play first."
				...
				(
					...
					(cond
						((and (global111 validPlay:) global130)
							(if (> size limit)
								(Print 100 0) ; "You cannot draw another Domino since you have a valid play."
							else
								(Print 100 1) ; "You cannot pass since you have a valid play."
							)
						)
						((= global110 (self give:))
							(if (> (++ local2) 1)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(1
				...
				(gSystemWindow back: 21)
				(Print
					(Format @global550 100 3 (BoneYard number:)) ; "Select %d Dominoes each"
					#mode
					1
					#at
					-1
				...
				(
					...
					(cond
						...
						((and global132 (not size))
							(if (not local1)
								(Print 100 6) ; "You must play your highest doublet."
								(= local1 1)
							else
								(= local1 0)
							)
