			...
			(evSAID
				...
				(cond
					...
					(
						...
						(if (and local4 (<= 186 (gEgo x:) 203))
							(if (gEgo has: 11) ; Keycard
								(if (User canControl:)
									(if (== (gEgo loop:) 0)
										(scanScript cue:)
									else
										(Print 90 23) ; "You're not facing a locked door."
									)
								else
									(Print 90 24) ; "These things take time..."
								)
							else
								(Print 90 25) ; "Easier said than done without the proper keycard."
							)
						else
							(Print 90 26) ; "You are not near a locked door."
						)
					)
					((Said '*/door')
						(if local4
							(Print 90 27) ; "Do or don't; the fact remains the door is locked."
						else
							(Print 90 28) ; "The door is not an obstacle."
						)
					)
					((Said 'look/crack,crack')
						(Print 90 29) ; "Don't step on a crack!"
					)
					((Said 'explore,look/pocket')
						(if (gEgo has: 12) ; Coveralls
							(Print 90 30) ; "You've already found the vaporizer; the handkerchief is a permanent fixture."
						else
							(Print 90 31) ; "You haven't got any!"
						)
					)
					(
						(or
						...
						(if (gEgo has: 15) ; a_copy_of_Elmo_s_picture
							(if (== (gEgo view:) 105)
								...
								(Print 90 32) ; "You wouldn't be able to see where you were going."
							)
						else
							(Print 90 33) ; "You don't have such."
						)
					)
				)
			)
			...
			(1
				(if (!= (searchSound prevSignal:) -1)
					...
					(if (IsItemAt 12) ; Coveralls
						(Print 90 34) ; "Rummaging about the cramped closet you find a pair of old, grimy janitorial coveralls."
					else
						(Print 90 35) ; "Your thorough search reveals a lot of used junk, but nothing of value to you."
					)
				)
			)
			(else
...
(instance scanScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 90 36) ; "You hear several clicks. "I'm in" you think to yourself."
				(Print 90 37) ; "Then you hear a synthesized voice say, "Keycard verified. Standby for composite facial scan.""
				(User canInput: 1)
				(gEgo setPri: 13)
			...
			(11
				(if local4
					(Print 90 38 #at -1 20 #width 280) ; "Composite Facial Scan complete: Access denied."
				)
				(hallSound play:)
				(HandsOn)
			)
