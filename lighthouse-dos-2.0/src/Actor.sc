...
(class View of Feature
	...
	(method (init param1 &tmp temp0)
		...
		(if (not approachX)
			((= approachX (Hiliter new:)) client: self)
			(if (not global105)
				(= global105 (Set new:))
			)
	...
	(method (setForeign)
		(return
			(cond
				((not argc)
					(PrintDebug {You called setForeign with no parameters.})
				)
				((< argc 2)
					(PrintDebug
						{You called setForeign with only ENGLISH parameter set.}
					)
				)
				((< argc 3)
					(PrintDebug
						{\nYou called setForeign with only ENGLISH and FRENCH parameter set.\n}
					)
				)
				((< argc 4)
					(not scratch)
...
(class Prop of View
	...
	(method (setScaler theScaler &tmp theActor)
		(if scaler
			(scaler dispose:)
			(= scaler 0)
		)
		(cond
			...
			(else
				(= theActor theScaler)
				(if (= theScaler (theActor scaler:))
					(= scaler (theScaler new:))
					(= scaleSignal (theActor scaleSignal:))
					(= maxScale (theActor maxScale:))
	...
	(method (cantBeHere &tmp ret)
		...
		(= ret
			(cond
				...
				((and blocks (not (blocks allTrue: 3 self))) -2)
			)
		)
	)
