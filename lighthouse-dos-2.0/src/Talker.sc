...
(class Narrator of Obj
	...
	(method (handleEvent event &tmp eType)
		(cond
			((event claimed:)
				(PrintDebug {event claimed before narrator could do anything})
			)
			((== ticks -1)
				(PrintDebug {returning FALSE for no apparent reason})
				(return 0)
			)
			(else
				(= eType (event type:))
				...
				(if
					(or
						(and (& eType $4001) (!= (gNarrator modeless:) 2)) ; evVERB | evMOUSEBUTTON
						(and
							(& eType evKEYBOARD)
					...
					(PrintDebug {getting rid of dialog})
					(event claimed: 1)
