	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(nut view: 307 cel: 0 setCycle: End)
			)
			(3
				(soundEffects number: 285 play:)
		...
		(switch (= state newState)
			...
			(2
				(if (== global126 6)
					...
					(= save1 1)
					(Print 28 16 #at 140 150) ; "Suddenly...the perpetrator regains consciousness."
					(nut
						view: 297
						setPri: -1
						setStep: 3 2
		...
		(switch theVerb
			(3 ; Do
				(if (or (gEgo has: 23) (proc882_2 2)) ; license
					(Print 28 27) ; "There's nothing else of interest in the clothes."
				else
					(HandsOff)
			...
			(4 ; Inventory
				(Print 28 28) ; "So,... You're a tailor now!?"
			)
			(else
		...
		(switch theVerb
			(5 ; Talk
				...
				(cond
					...
					(else
						(HandsOn)
						(Print 28 29) ; "The man isn't listening to you."
					)
				)
			)
			(3 ; Do
				(switch global126
					(1
						(Print 28 30) ; "You can't subdue him from here."
					)
					(2
						(Print 28 31) ; "What are you going to do, kiss him?"
					)
					(4
						(Print 28 32) ; "He's already down. Police brutality wasn't part of your training."
					)
					(else
			...
			(4 ; Inventory
				(switch invItem
					...
					(1 ; gun
						...
						(cond
							((== global126 3)
								(Print 28 33) ; "Too late now..."
							)
							((== global126 4)
								(Print 28 34) ; "You aren't going to shoot an unconscious man in the back are you?"
							)
					...
					(2 ; handcuff
						(switch global126
							...
							(2
								(Print 28 35) ; "You need to subdue him first."
							)
							(3
								(Print 28 33) ; "Too late now..."
							)
							(else
								(Print 28 36) ; "You are not close enough to handcuff him."
							)
						)
					)
					(16 ; nightStick
						(switch global126
							...
							(4
								(Print 28 37) ; "He's already down. Police brutality wasn't part of your training."
							)
							(else
								(Print 28 38) ; "Using a nightstick on someone who hasn't threatened you is against the law."
							)
						)
					)
					(19 ; wallet
						(Print 28 39) ; "That guy wouldn't know or care if you were President of the United States."
					)
					(else
						(Print 28 40) ; "The lunatic is not impressed."
					)
				)
			)
			(else
...
(instance river of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Print 28 41) ; "The water babbles back."
			)
			(4 ; Inventory
				(switch invItem
					(3 ; theKeys
						(if (proc882_2 2)
							(Print 28 42) ; "Calling on your old softball arm, you haul back and throw the keys as far as you can into the river."
							(proc882_1 2)
...
(instance trees of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 28 43) ; "You don't have time to climb trees."
			)
			(else
				(super doVerb: theVerb)
			)
