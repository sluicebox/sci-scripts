	...
	(method (doit)
		(cond
			...
			((& (gEgo onControl: 0) $0002)
				(self x: 48 y: 142 setScript: (ScriptID 896 2) 0 12) ; sWO
			)
		)
		(super doit:)
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 14 43) ; "You could try it, but it would only get you suspended REAL fast."
					)
					(19 ; wallet
						(HandsOff)
			...
			(3 ; Do
				(Print 14 44) ; "If you want to give Officer Gibbs something, use the evidence deposit slot."
			)
			(5 ; Talk
				(HandsOff)
...
(instance inBasket of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 14 45) ; "Just put something in it."
			)
			(4 ; Inventory
				(if
					(or
					...
					(Print 14 46) ; "Evidence doesn't want that."
				)
			)
			(else
				(super doVerb: theVerb)
