		...
		(switch theVerb
			(4 ; Inventory
				(if (trunk showing:)
					...
					(switch invItem
						(39 ; scraper
							(Print 47 19) ; "Rubber's just rubber. If you want some there's easier ways to get it than scraping it off the street."
						)
					)
				)
			)
		...
		(switch theVerb
			(3 ; Do
				(if (== local0 1)
					...
					(Print 47 20) ; "There is no apparent need for anything in the trunk right now."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance carDoor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 21) ; "Convinced that the serial killer will strike here next, you decide to stick around."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance nugget of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 22) ; "Just use the front door."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance mug of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 47 23) ; "If you want a drink, you'd have better luck inside."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
