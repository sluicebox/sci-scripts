	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(cond
					((> gDay 1)
						(Print 20 30) ; "You are in homicide division now. Use the computer there."
					)
			...
			(4 ; Inventory
				(switch invItem
					(21 ; compCard
						(if (> gDay 1)
							(Print 20 32) ; "You are in homicide division now. Use the your computer there."
						else
							(HandsOff)
							(gCurRoom setScript: compScript)
						)
		...
		(switch theVerb
			(3 ; Do
				(if (== gDay 1)
					...
					(Print 20 33) ; "You are in homicide division now. Use your desk there."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance thePhone of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(if (> gDay 1)
					(Print 20 34) ; "You are in homicide division now. Use the phone there."
				else
					(HandsOff)
					(cInset dispose:)
...
(instance ltStuff of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 36) ; "Don't mess with the Lieutenant's stuff."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance garbageCan of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 37) ; "Now why would you want to take a garbage can?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bulletinBoard of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 38) ; "There's nothing even remotely interesting on that bulletin board."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cabinets of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 39) ; "There's nothing you need in the file drawers."
			)
			(else
				(super doVerb: theVerb)
			)
		...
		(switch theVerb
			(3 ; Do
				(if (== gDay 1)
					...
					(Print 20 40) ; "You are in homicide division now. Your new desk and chair are there."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance visitorChair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 41) ; "Why would you want to sit on that hard visitor's chair when you can sit on your own comfy one?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance ltChair of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 42) ; "You have your own chair. Leave the Lieutenant's alone."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 43) ; "You prefer to catch the criminals - let the lawyers read all that crap."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cInset of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 20 44) ; "You need a computer access card first."
			)
			(2 ; Look
				(Print 20 45) ; "It's a computer."
			)
			(else
				(super doVerb: theVerb)
			)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(21 ; compCard
						(Print 20 46) ; "You insert your computer access card."
						(computerCard play:)
