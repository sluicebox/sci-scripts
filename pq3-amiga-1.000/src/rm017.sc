		...
		(switch theVerb
			(3 ; Do
				(if (IsFlag 34)
					...
					(Print 17 1) ; "You can't force open the locker. You'll need the combination."
				)
			)
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 17 2) ; "That would only attract attention, don't you think."
					)
					(else
						(Print 17 3) ; "That won't help you open the locker."
					)
				)
			)
			(else
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					(28 ; noteBook
						(SetScore 174 10)
						(Print 17 4) ; "You take notes on what you've found in Morales' locker."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance cocaine of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 17 5) ; "You'd better leave the cocaine here. If it were missing, Morales would get suspicious."
			)
			(4 ; Inventory
				(switch invItem
					(28 ; noteBook
						(SetScore 174 10)
						(Print 17 6) ; "You take notes on what you have found."
					)
					(else
						(super doVerb: theVerb invItem)
					)
...
(instance bench of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 17 7) ; "You need to get out of here quickly. You don't have time to lounge!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance stalls of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 17 8) ; "It's bad enough that you're even in here!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance wrongLockers of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 17 9) ; "That's not Morales' locker!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
