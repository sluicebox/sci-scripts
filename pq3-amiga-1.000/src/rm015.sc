			...
			((& temp0 $0008)
				(cond
					...
					((not (gCast contains: janitor))
						(gCurRoom x: 237 y: 115 setScript: (ScriptID 896 2) 0 17) ; sWO
					)
					(else
...
(instance storageDoor of Prop
	(properties
		...
		lookStr {That's the door to the storage closet.}
		view 145
		priority 6
		signal 16385
	)
		...
		(switch theVerb
			(3 ; Do
				(if (not cel)
					...
					(Print 15 10) ; "The door's already open."
				)
			)
			(else
...
(instance storageInset of Prop
	(properties
		...
		lookStr {The closet is cluttered with boxes.}
		view 151
		priority 14
		signal 16401
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 11) ; "There's nothing you need in that box."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance flares of Prop
	(properties
		...
		y 159
		description {flares}
		lookStr {There's a box of road flares in the closet.}
		view 151
		loop 2
		priority 15
		signal 16401
		...
		(switch theVerb
			(3 ; Do
				(switch cel
					(0
						(gEgo get: 15) ; flares
						(Print 15 12) ; "You take some flares."
						(self cel: 1 forceUpd:)
						(SetScore 223 3)
					)
					(else
						(Print 15 13) ; "You already have plenty of flares."
					)
				)
			)
			(else
...
(instance flashBat of Prop
	(properties
		...
		lookStr {There's an open box of batteries in the closet.}
		view 151
		loop 1
		priority 15
		signal 16401
		...
		(switch theVerb
			(3 ; Do
				(switch cel
					(0
						(gEgo get: 14) ; battery
						(Print 15 14) ; "You take some batteries from the box."
						(self cel: 1 forceUpd:)
						(SetScore 101 5)
					)
					(else
						(Print 15 15) ; "You already have plenty of batteries."
					)
				)
			)
			(else
...
(instance janitor of Actor
	(properties
		...
		y 140
		description {janitor}
		lookStr {Mr. Stumps, the janitor, is good-natured and hard-working.}
		view 147
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 16) ; "Mr. Stumps is friendly, but he's not THAT friendly."
			)
			(5 ; Talk
				(if (not (IsFlag 212))
					(Print 15 17) ; "The janitor looks friendly enough, so you strike up a conversation."
					(SetFlag 212)
				)
				(HandsOff)
...
(instance cart of View
	(properties
		x 239
		y 143
		lookStr {That's Mr. Stumps' janitorial cart.}
		view 147
		loop 1
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 18) ; "Looking for a new career?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance fireHydrant of Feature
	(properties
		...
		onMeCheck 4096
		lookStr {It's a fire extinguisher.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 19) ; "But... there's no fire."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance smokeDetectors of Feature
	(properties
		...
		onMeCheck 64
		lookStr {The smoke detectors help protect the building from fire.}
	)
)
...
(instance mensEntrance of Feature
	(properties
		...
		onMeCheck 2048
		lookStr {That's the entrance to the men's locker room.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 20) ; "Just walk in."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance womensEntrance of Feature
	(properties
		...
		description {the entrance}
		onMeCheck 1024
		lookStr {That's the entrance to the women's locker room.}
	)
...
(instance nightWin of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 21) ; "You don't need to open the window."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance carpet of Feature
	(properties
		...
		onMeCheck 16422
		lookStr {The floor's color is policeman blue.}
	)
)
...
(instance lightSwitches of Feature
	(properties
		...
		onMeCheck 16
		lookStr {That's a light switch.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 15 22) ; "The lights are left on during the day."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance hallwayRoom of Feature
	(properties
		lookStr {You're standing in the first floor hallway of the Lytton County Police Department.}
	)
)
