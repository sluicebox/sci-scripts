		...
		(switch (event type:)
			(evKEYBOARD
				(switch (event message:)
					...
					(else
						(if (and 1 (not (OneOf gCurRoomNum 21 24 25 51 55)))
							((ScriptID 899) handleEvent: event) ; dbg
						else
							(event claimed: 0)
						)
					)
		...
		(switch theVerb
			(2 ; Look
				(switch state
					...
					(3
						(Print 0 18) ; "This is the drunk's ID card."
					)
					(4
						(Print 0 19) ; "This is Brian Forbes' license - #789612."
					)
					(5
...
(instance goldPaint of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (IsFlag 2)
					(Print 0 23) ; "Sample envelope with a gold paint sample in it."
				else
					(Print 0 24) ; "Sample envelope with a light red paint sample in it."
				)
			)
			(else
				(super doVerb: theVerb invItem)
...
(instance icon6 of IconI
	...
	(method (show &tmp [temp0 7] [temp7 4] temp11)
		(super show: &rest)
		(= temp11 30)
		(Format @temp0 0 25 gScore gPossibleScore) ; "%d of %d"
