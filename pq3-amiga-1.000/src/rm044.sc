...
(instance egoAfterWarrant of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(= save1 1)
				(Say iEgo 44 5) ; "No, not the warrant - just gaining entry. The place is fortified. There's only one way to get into that house. I'm requesting a special order to use the department's ram unit."
				(= seconds 10)
			)
			(7
				(if gModelessDialog
			...
			(8
				(Print 44 6) ; "The judge studies you closely, then says..."
				(ijudge init:)
				(= cycles 1)
			)
			(9
				(= save1 1)
				(Say ijudge 44 7) ; "I see. Alright, Officer. I trust your judgement."
				(= seconds 5)
			)
			(10
				(if gModelessDialog
			...
			(13
				(= save1 1)
				(Say ijudge 44 8) ; "Just make sure you have plenty of back up."
				(= seconds 5)
			)
			(14
				(if gModelessDialog
...
(instance giveLocket of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(SetFlag 230)
				(Print 44 9 #at 40 115) ; "You explain to the judge about recovering Marie's locket. You tell her you believe the cult is responsible for Marie's attempted murder."
				(= cycles 1)
			)
			(1
				(switch local1
					...
					(else
						(Print 44 10) ; "OOPS THINGS GIVEN DOESN'T WORK"
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(Print 44 11 #at 40 115) ; "You tell the judge how the photograph of Michael and Jessie Bains helped you locate the suspected crack house."
				(= cycles 1)
			)
			(1
				(switch local1
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(Print 44 12 #at 40 115) ; "You hand the news clipping to the judge."
				(SetScore 186 3)
				(= cycles 1)
			)
			(1
...
(instance givenOne of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 44 13 #at 40 115) ; "You try to convince the judge, but she insists on more evidence."
				(= cycles 1)
			)
			(1
				(ijudge init:)
			...
			(2
				(= save1 1)
				(Say ijudge 44 14) ; "I can see you've done some good detective work, Officer Bonds, but I need more evidence to justify a search warrant."
				(= seconds 12)
			)
			(3
				(ijudge dispose:)
...
(instance givenTwo of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say iEgo 44 15) ; "Your Honor, I'm convinced that some sort of cult operating a crack house at this location."
				(= seconds 9)
			)
			(2
				(if gModelessDialog
			...
			(5
				(= save1 1)
				(Say ijudge 44 16) ; "I wish I were as convinced, Officer Bonds. Is there anything else?"
				(= seconds 10)
			)
			(6
				(if gModelessDialog
...
(instance givenThree of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say ijudge 44 17) ; "Well! This looks like a very nasty business indeed! You've sold me. I'll sign the warrant."
				(= seconds 10)
			)
			(2
				(if gModelessDialog
			...
			(4
				(Print 44 18 #at 40 115) ; "Judge Simpson lays the warrant on the desk and says..."
				(= cycles 1)
			)
			(5
				(ijudge init:)
			...
			(6
				(= save1 1)
				(Say ijudge 44 19) ; "Good luck, Officer. Contact me if you need any further assistance."
				(= seconds 10)
			)
			(7
				(if gModelessDialog
...
(instance wheresTheHouse of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say ijudge 44 20) ; "Sounds like you have a good case Bonds. But I need a location and at least an attempt to gain normal entry."
				(= seconds 9)
			)
			(2
				(= local0 1)
...
(instance judgeTalkToldNo of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say ijudge 44 21) ; "I wish I could help, but you must go to the location and see if anyone is home first."
				(= seconds 7)
			)
			(2
				(if gModelessDialog
...
(instance judgeTalk of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say iEgo 44 22) ; "Your Honor,... I need to obtain a search warrant."
				(= seconds 6)
			)
			(2
				(if gModelessDialog
			...
			(5
				...
				(cond
					((not (IsFlag 177))
						(Say ijudge 44 23) ; "Well then... How about an address...?"
						(= seconds 6)
					)
					((<= local1 1)
						(Say ijudge 44 24) ; "Well then... How about some evidence...?"
						(= seconds 6)
					)
					(else
						(= ticks 1)
...
(instance dontForgetSomething of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= save1 1)
				(Say ijudge 44 25) ; "Are you forgetting something?"
				(= seconds 4)
			)
			(2
				(if gModelessDialog
...
(instance judge of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 44 26) ; "The judge is not the kind of person you want to mess with."
			)
			(5 ; Talk
				(HandsOff)
				(cond
					...
					((gEgo has: 8) ; warrant
						(HandsOn)
						(Print 44 27) ; "The judge doesn't want to talk now."
					)
					((gEgo has: 9) ; judicialOrder
						(HandsOn)
						(Print 44 28) ; "You got what you wanted. What are you waiting for?"
					)
					(else
						(HandsOn)
						(Print 44 29) ; "The judge just wants to see some evidence."
					)
				)
			)
			(4 ; Inventory
				(switch invItem
					(37 ; locket
						(cond
							((IsFlag 230)
								(Print 44 30 #at 40 115) ; "You've already shown Marie's locket to Judge Simpson."
							)
							((not (gEgo has: 8)) ; warrant
								(HandsOff)
								(++ local1)
							...
							(else
								(Print 44 31) ; "The judge doesn't care about Marie's locket now."
							)
						)
					)
					(6 ; photo
						(if (== ((gInventory at: 6) state:) 1) ; photo
							(Print 44 32 #at 40 115) ; "You've already shown the photograph to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(35 ; article
						(if (== ((gInventory at: 35) state:) 1) ; article
							(Print 44 33 #at 40 115) ; "You've already shown the news clipping to Judge Simpson."
						else
							(HandsOff)
							(++ local1)
					...
					(1 ; gun
						(Print 44 34) ; "She'd rip you apart, Bonds."
					)
					(2 ; handcuff
						(Print 44 34) ; "She'd rip you apart, Bonds."
					)
					...
					(19 ; wallet
						(Print 44 35) ; "I remember you perfectly well, Officer Bonds!"
					)
					(else
						(Print 44 36) ; "The judge isn't interested in that."
					)
				)
			)
			(else
...
(instance warrant of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (gEgo has: 8) ; warrant
					(Print 44 37) ; "The judicial order lies on the desk."
				else
					(Print 44 38) ; "The search warrant lies on the desk."
				)
			)
			(3 ; Do
				(if (gEgo has: 8) ; warrant
					(SetScore 189 5)
					(Print 44 39) ; "You pick up the judicial order."
					(gEgo put: 8 0) ; warrant
					(gEgo get: 9) ; judicialOrder
					(= local3 0)
				else
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 44 40) ; "The judge would not appreciate you handling her things."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance windows of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 44 41) ; "If the judge wants her windows open, she'll open them herself."
			)
			(else
				(super doVerb: theVerb)
			)
