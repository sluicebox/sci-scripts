		...
		(switch theVerb
			...
			(82 ; fetchSpell
				(if (and (not (IsFlag 150)) (gCast contains: kobKey))
					...
					(gMessager say: 9 82 0) ; "You don't need to cast Fetch -- everything here is in easy reach."
				)
				(return 1)
			)
			(78 ; dazzleSpell
...
(instance chest of Prop
	...
	(method (doit)
		(if (and (< (gEgo distanceTo: chest) 10) (not (IsFlag 216)))
...
(instance chestBlows of KScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(client loop: 2 cel: 0 cycleSpeed: 12 setCycle: End self)
				((= local14 (Sound new:)) number: 62 priority: 15 init: play:)
				(if register
					(= register 0)
					(gEgo setLoop: 2)
				else
...
(instance kobWakeUp of KScript
	...
	(method (changeState newState &tmp [temp0 40])
		(switch (= state newState)
			...
			(2
				(Message msgGET 15 9 0 26 1 @temp0) ; "You go! You no stay here!"
				(Print addText: @temp0 init:)
				(client setScript: kobAwake)
			)
		)
...
(instance kobEnter of KScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not (IsFlag 5))
					(gMessager say: 9 0 20) ; "This section of cave has an eerie quality. You see a kobold resting on a ledge across from you."
				)
				(HandsOn)
				(= local55 1)
				(self dispose:)
