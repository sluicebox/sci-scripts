...
(instance rm54 of Rm
	...
	(method (init)
		...
		(theSign init:)
		(hut init: approachVerbs: 4) ; Do
		(gardenL init: approachVerbs: 4) ; Do
		(gardenR init: approachVerbs: 4) ; Do
		(castle init:)
		...
		(switch theVerb
			...
			(82 ; fetchSpell
				(cond
					...
					(else
						(gMessager say: 9 82 12) ; "That spell is no longer useful here (but you did get some practice)."
					)
				)
			)
			(else
...
(instance door of Prop
	...
	(method (cue)
		...
		(switch (++ local5)
			...
			(3
				(closingDoor stop:)
				(gEgo illegalBits: 0 setMotion: MoveTo 257 105 self)
			)
			(4
				(gCurRoom newRoom: 55)
			)
