		...
		(switch theVerb
			(4 ; Do
				(cond
					...
					((and local1 (!= (brig1 script:) brigandsSouth))
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Do
				(cond
					(local2
						(gMessager say: 3 4 1) ; "The candelabra has already fallen."
					)
					(local7
						(HandsOff)
					)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Do
				(cond
					(local14
						(gMessager say: 6 4) ; "No time for that again (although it WAS fun). Just get the heck outta here."
					)
					(local7
						(gEgo setScript: egoSwings)
					)
					(else
						(gMessager say: 6 4 1) ; "The chandelier is out of your reach."
					)
				)
			)
			(else
		...
		(switch theVerb
			(4 ; Do
				(cond
					...
					(else
						(gMessager say: 11 4) ; "Leave the rope alone for now. It's serving a very important function by holding up the chandelier."
					)
				)
			)
			(else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Do
				(cond
					(local0
						(gMessager say: 5 4) ; "The door is already blocked."
					)
...
(instance openBDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo setPri: 2 illegalBits: 0 setMotion: MoveTo 60 86 self)
			)
			(2
				(NormalEgo)
				(gCurRoom newRoom: 96)
...
(instance barFDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gMessager say: 10 0 2) ; "You close and bar the door."
				(fDoor addToPic:)
				(NormalEgo)
				(= local1 1)
				(HandsOn)
...
(instance pushChair of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 10 0 5) ; "You move the chair in front of the door."
				(rDoor addToPic:)
...
(instance pushCan of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo setPri: 6 setMotion: PolyPath 247 95 self)
			)
			(2
		...
		(switch (= state newState)
			...
			(3
				(if local8
					...
					(gMessager say: 10 0 3) ; "Oh, a wise guy, eh?"
...
(instance handOnDrunk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 10 0 4) ; "There's no need to knock this guy out; the Troll's Sweat he's been drinking has already done the job for you."
				(self cue:)
			)
			(2
				(= ticks 30)
