		...
		(switch theVerb
			...
			(50 ; detectMagicSpell
				(if (CastSpell temp0) ; UNINIT
					(gMessager say: 6 50 0) ; "The Giant (who does not seem pleased at your casting a spell in his presence) radiates a magical aura. The strongest magic centers on the glowing gem at his belt."
				)
			)
		...
		(switch (= state newState)
			...
			(7
				(if (or (IsFlag 213) (not (TakeDamage 20)))
					(EgoDead 86 87 3 3 59) ; "Next time, pick on someone your own size."
				else
					(SetFlag 213)
					(self cue:)
				)
...
(instance egoRuns of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(cond
					((and (> (gEgo y:) 180) (> (gEgo x:) 128))
						(gEgo posn: (- (gEgo x:) 12) (- (gEgo y:) 10))
					)
					((> (gEgo y:) 148)
