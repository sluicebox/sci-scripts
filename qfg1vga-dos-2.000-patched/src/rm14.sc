...
(procedure (localproc_3)
	...
	(= local3 (if (< (gEgo x:) (bear x:)) 40 else 12))
)
		...
		(switch (= state newState)
			(0
				...
				(if (== gPrevRoomNum 15)
					...
					(= local2 110)
					(= local3 40)
					(gEgo init: posn: -15 225 setMotion: MoveTo 30 180 self)
				)
			)
			(1
				(bear cel: 1)
			...
			(2
				(if register
					(gMessager say: 3 0 11) ; "As your eyes adjust from sunlight to darkness, you examine the interior of this eerie cavern. You sense something moving off to your right."
					(SetFlag 149)
				)
				(HandsOn)
				(self dispose:)
