		...
		(cond
			((Said 'drain,(get<off)/dress')
				(cond
					...
					((gEgo has: 16) ; Bra
						(Print 484 4) ; "You don't really need to."
					)
			...
			((Said 'wear,drain,(on<drop),(alter<in),(drop<on),get>')
				(cond
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
						(event claimed: 1)
			...
			((Said 'drop,decrease,drain,(get<off)/sheet')
			...
			((Said 'look>')
				(cond
					((Said '/buffet')
						(if (InRoom 13)
							(Print 484 19) ; "The now-empty bottle of wine that Larry brought you rests on the table at the foot of your bed."
						else
							(Print 484 20) ; "The table at the foot of your bed contains nothing but an empty room-service tray."
						)
						(if (or (InRoom 15) (InRoom 14) (InRoom 16))
							(Print 484 21) ; "Some of your clothing lies on the table near your dressing screen."
						else
					...
					((Said '/lotion,(buffet<dressing),cloth')
						(cond
							((InRoom 15)
								(Print 484 23) ; "A pair of pantyhose lies on your dressing table."
							)
							((InRoom 14)
								(Print 484 24) ; "A pair of black lace panties rests on the dressing table beside the screen."
							)
							((InRoom 16)
								(Print 484 25) ; "A black lace brassiere is all that is left on the dressing table."
							)
							(else
								(Print 484 22) ; "Your dressing table is empty."
							)
						...
						(if (InRoom 17)
							(Print 484 26) ; "Your white gown with the long slit hangs on a hanger on your dressing screen."
						)
					)
					((Said '/tray')
						(if (InRoom 13)
					...
					((Said '/bed')
						(Print 484 28) ; "That's where it happened, all right!"
					)
					((Said '/keyboard')
						(Print 484 29) ; "Wasn't it nice of the casino management to rent a piano for your suite."
					)
					((Said '/binocular')
						(Print 484 30) ; "There's no use looking for Larry now, you know where he went. Now go find him!"
					)
					((Said '/burn')
						(Print 484 31) ; "The lamps are just hanging around here."
					)
					((Said '/bush')
						(Print 484 32) ; "You're not interested in plants, Patti!"
					)
					((Said '/balcony,look,cup')
						(Print 484 33) ; "The view is spectacular, but you want to see your Larry!"
					)
					((Said '[/area]')
						(Print 484 34) ; "You'll never be able to look at this suite again without thinking of your Larry!"
					)
				)
			)
			((Said '/cloth')
				(Print 484 35) ; "You must refer specifically to each article of clothing."
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 484 36) ; "You slide your dress over the screen and drop the sheet on the floor."
			...
			(2
				...
				(gEgo view: 482)
				(gGame changeScore: -10)
				(Print 484 37) ; "You grab the sheet again, and hang your dress back on the screen."
