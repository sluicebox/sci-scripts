	...
	(method (changeState newState &tmp destX destY)
		...
		(switch (= state newState)
			...
			(5
				...
				(gEgo setMotion: MoveTo destX destY self)
			)
			(6
				(PutInRoom 13 484)
			...
			(11
				(Format @string 480 22) ; "Rest assured, Honey; I promise you: something will happen!"
				(= seconds (PrintPatti))
			)
			(12
				(Format @string 480 23) ; "Would you care for a glass of wine?"
				(= seconds (PrintLarry))
			)
			(13
				(Format @string 480 24) ; "I thought you'd never ask."
				(= seconds (PrintPatti))
			)
			(14
			...
			(16
				(Format @string 480 25) ; "Say when."
				(= seconds (PrintLarry))
				(gEgo loop: 2 cel: 0 setCycle: Fwd)
			)
			(17
				(Format @string 480 26) ; "Right after this drink, Larry!"
				(= seconds (PrintPatti))
			...
			(23
				(Format @string 480 27) ; "So, Patti, how did you get started in the music business?"
				(= seconds (- (PrintLarry) 1))
			)
			(24
				(Format @string 480 28) ; "Shut up, Larry!"
				(= seconds (PrintPatti))
				(gEgo hide:)
			...
			(26
				(Format @string 480 29) ; "Oh, Patti."
				(= seconds (PrintLarry))
			)
			(27
				(Format @string 480 30) ; "Oh, Larry!"
				(= seconds (PrintPatti))
			)
			(28
				(aPatti setCycle: End)
			...
			(30
				(Format @string 480 31) ; "Wherever did you learn to kiss like that?"
				(= seconds (PrintPatti))
			)
			(31
				(Format @string 480 32) ; "Well, when I was younger I DID play trumpet in a mariachi band..."
				(= seconds (PrintLarry))
			)
			(32
			...
			(34
				(Format @string 480 33) ; "OH, LARRY!"
				(= seconds (PrintPatti))
			)
			(35
				(Format @string 480 34) ; "Oh! (mmmmmhhh)! Ah."
				(= seconds (PrintPatti))
			)
			(36
				(Format @string 480 35) ; "Ohhhhh! (aaaaahhh)!"
				(= seconds (- (PrintPatti) 1))
			)
			(37
				(Format @string 480 36) ; "Larry, (oooh) you're the first man (aaaahhh) who's ever made me feel (mmmmmhhh) THAT!"
				(= seconds (- (PrintPatti) 1))
			)
			(38
				(Format @string 480 37) ; "Where did (ooohh) you learn (oooohhhhhh) to do (uuuhhhh) that?!"
				(= seconds (- (PrintPatti) 1))
			)
			(39
				(Format @string 480 38) ; "And that!"
				(= seconds (- (PrintPatti) 2))
			)
			(40
				(Format @string 480 39) ; "And, oh yes, THAT! Yes, THAT!! Please, more of that!!"
				(= seconds (- (PrintPatti) 2))
			)
			(41
				(Format @string 480 40) ; "Well, well, Larry. Perhaps you learned more from Kalalau than you realized."
				(Print @string #time (PrintDelay @string))
				(Format @string 480 41) ; "No longer is our Larry Laffer the Laugher Lover!"
				(Print @string #time (PrintDelay @string))
			...
			(44
				(Print 480 42 #at 10 5 #width 290) ; "Oh, Larry, I'm so glad you came. As you can see, I slipped into something more comfortable -- my bed! But, I'm not naked: I'm wearing your lei!"
				(Print 480 43 #at -1 144) ; "(All right, Larry! You're on, big guy!)"
				(= seconds 3)
			)
			(45
				(Print 480 44 #at 10 5 #width 290) ; "Did you bring the wine?" Patti asks softly. "I'd love a drink right now."
				(gMusic number: 9 loop: 1 play: self)
				(Print 480 45) ; "Oh, no!" you reply, "I knew there was something I was supposed to do! I'll be right back!! Don't move!"
			)
			(46
