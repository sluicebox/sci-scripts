...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			...
			((Said 'pull/binocular')
				(Print 206 2) ; "You attempt to turn the binoculars so as not to see into that woman's room, but you just can not!"
				(Print 206 3 #at -1 144) ; "(Hee, hee!)"
			)
			((Said 'look<in/binocular')
				(Print 206 4) ; "This is as far in as you can look!"
			)
			((Said 'look/area')
				(Print 206 5) ; "You are voyeuring into a woman's room in the hotel."
				(Print 206 6 #at -1 144) ; "(No wonder this is called a scenic viewpoint!)"
			)
			((Said 'look[/woman]')
				(switch stripperState
					(1
						(Printf 206 7 gEgoName) ; "%s! You're looking into a woman's hotel room!"
					)
					(2
						(Print 206 8) ; "She's turned off the lights!"
					)
					(3
						(Print 206 9) ; "She's pulling down the shades."
					)
					(4
						(Print 206 10) ; "Oh, my gawd! She's taking off her clothes!"
					)
					(5
						(Print 206 11) ; "WOW!"
					)
					(6
						(Print 206 12) ; "Did you see that?!"
					)
					(else
						(Print 206 13) ; "Where?"
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(26
				(SetFlag 16)
				(gGame changeScore: 2 setSpeed: gOldGameSpeed)
				(Print 206 14) ; "You wipe the sweat from your hands as you remove your eyes from the binoculars."
				(= gEgoState gOldEgoState)
				(gCurRoom newRoom: 200)
			)
		)
...
(instance aGull of Act
	...
	(method (init)
		(super init:)
		(self setPri: 14 setCycle: Fwd ignoreActors: setMotion: MoveTo 319 44)
	)
)
