	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(14
				(HandsOff)
				(= gEgoState 1003)
				(Print (Format @string 550 25 gExpletive) #at -1 10 #dispose) ; "%s"
				(= drowningLoop 0)
				(gEgo
			...
			(16
				...
				((ScriptID 40) ; DyingScript
					caller: 816
					register: (Format @string 550 26) ; "Patti! Be more careful! There are treacherous undercurrents in this river."
					next: (Format @string2 550 27) ; "Head over heels but not in love!"
				)
			)
		)
	)
