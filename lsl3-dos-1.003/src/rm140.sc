...
(procedure (LoadSetup &tmp questionFile soundState [str 30])
	(if (!= (= questionFile (FOpen {LARRY3.DRV} 1)) -1)
...
(procedure (SaveSetup &tmp [str 40] questionFile)
	(if (!= (= questionFile (FOpen {LARRY3.DRV} 2)) -1)
		(Format @str 140 12 [filesSeen 0] [filesSeen 1] [filesSeen 2] 140 13) ; "%6d %6d %6d %5s"
...
(instance RoomScript of Script
	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 33])
	...
	(method (changeState newState &tmp age i filthFile [tmpString 200])
		(switch (= state newState)
			(0
				...
				(switch age
					(-1
						(Print 140 4) ; "Come back with your legal guardian!"
						(= gQuit 1)
					)
					(0
						(Print 140 5) ; "Welcome to the "Clean" version!"
						(self changeState: 5)
					)
					(else
						...
						(Format @string 140 6 age) ; "To prove you are at least %d years old, just answer these five simple questions."
			...
			(2
				(if (== hisAnswer theAnswer)
					...
					(Print 140 8 #at 190 8 #time 3 #dispose) ; "Correct"
					(++ correct)
					(++ suitCel)
				else
					(HighLight hisAnswer 4)
					(gMusic number: 141 loop: 1 play:)
					(Print 140 9 #at 190 8 #time 3 #dispose) ; "Wrong"
					(if suitCel
						(-- suitCel)
					)
				)
			...
			(5
				...
				(if (!= (= filthFile (FOpen {RESOURCE.LL3} 2)) -1)
					(Format @tmpString 140 11 gFilthLevel) ; "%7d"
