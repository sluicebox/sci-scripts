...
(instance RoomScript of Script
		...
		(cond
			...
			(
				(or
					...
					(Said 'attach/hemp/me,self,entertainer')
					(Said 'use/dress')
			...
			((and potState (Said 'test,look/hemp,knot'))
				(Print 530 23) ; "It might be okay."
			)
			((and (== potState 3) (Said 'use,climb,throw/hemp'))
			...
			((or (Said 'make,pull/blade') (Said 'burn,smoke/bush,bush,blade'))
				(cond
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
			...
			((or (Said 'drop/blade,bush/lip') (Said 'eat,eat/bush,bush,blade'))
				(cond
					((!= gEgoState 0)
						(NotNow) ; "Good idea. You might try that again later."
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(34
				(= seconds (= cycles 0))
				(Print (Format @string 530 81 gExpletive) #dispose #at -1 10) ; "%s"
				(gMusic number: 4 loop: 1 play:)
			...
			(55
				(Print 530 91 #at 10 5 #width 290) ; "Although your previous experience with knots has been limited to fashion accessories (plus a few weekends spent on yachts in ports around the world serving as deck hand au naturel), you tie the rope to the palm tree as securely you can."
				(NormalEgo 1)
			...
			(65
				(HandsOff)
				(Print (Format @string 530 81 gExpletive) #at -1 10 #dispose) ; "%s"
				(gMusic number: 4 loop: 1 play:)
				(gEgo
					view: 813
