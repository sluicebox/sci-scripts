...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'cable,attach,(on<pull)/cable,equipment,handle')
				(if (== gEgoState 0)
					(ItIs) ; "It is."
				else
			...
			(
				(Said
					'disconnect,drain,(off<pull),stop,pull/cable,equipment,handle'
				)
				(cond
					((!= gEgoState 630)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(2
				(Printf 630 10 gExpletive) ; "%s" you shout. "Now what?"
				(gEgo
					observeControl: 8192
					baseSetter: SQ3Base
			...
			(7
				...
				(if (== gBgMusicLoops -1)
					(gMusic number: 631 loop: 1 play: self)
				else
					(= cycles 22)
				)
			)
