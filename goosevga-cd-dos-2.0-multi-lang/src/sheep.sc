...
(procedure (localproc_1 param1 param2)
	(param1
		...
		setCycle: Walk
		setMotion: NewFollow gEgo (+ (* param2 6) 20)
	)
)
...
(procedure (localproc_3 param1 &tmp temp0 temp1)
	(if (!= global166 1)
		...
		(*= temp0 (Random 1 10))
		(*= temp1 (Random 1 10))
		(if (and (> gCurRoomNum 23) (< gCurRoomNum 27))
			(= temp1 0)
		)
		(param1
			x: (+ (gEgo x:) temp0)
...
(procedure (localproc_5 &tmp temp0 temp1)
	(for ((= temp0 1)) (< temp0 4) ((++ temp0))
		(= temp1 (* (- temp0 1) 2))
		([local0 temp0]
			setAvoider: ((Avoid new:) offScreenOK: 0)
		...
		(cond
			...
			((== global164 1)
				...
				(gEgo
					...
					setMotion: MoveTo temp2 temp3 global133
				)
				(sheepCage init:)
				(localproc_4)
				(++ global164)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(if (or (gCurRoom script:) (not (User controls:)))
					(= seconds 1)
				else
					(Timer set60ths: self (DoAudio audPLAY (Random 31 33)))
				)
			)
			(2)
			(3
		...
		(switch (= state newState)
			(0
				...
				(switch gCurRoomNum
					(39
						(for ((= temp0 1)) (< temp0 4) ((++ temp0))
							([local0 temp0] setMotion: MoveTo 141 190 self)
						)
					)
					(40
						(for ((= temp0 1)) (< temp0 4) ((++ temp0))
							([local0 temp0] setMotion: MoveTo 234 138 self)
						)
					)
					(else
						(for ((= temp0 1)) (< temp0 4) ((++ temp0))
							([local0 temp0] setMotion: MoveTo 200 144 self)
						)
					)
				)
				(Timer setCycle: self 15)
			)
			(1
				(sheep dispose:)
				(self dispose:)
