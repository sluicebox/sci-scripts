...
(procedure (localproc_0) ; UNUSED
	(Format @global100 211 0) ; "Welcome to Z-Mart. Home of low cost, low quality, and cheap help."
	(Format @global100 211 1) ; "Welcome to Z-Mart, where low prices don't always mean low quality!"
	(Format @global100 211 2) ; "Welcome to Z-Mart. Our overhead is so low, the clerks are stoop- shouldered!"
	(Format @global100 211 3) ; "Welcome to Z-Mart. Our overhead is so low, we hire only midgets!"
	(Format @global100 211 4) ; "Welcome to Z-Mart. If we see a line with more than 50 people waiting, we'll open another!"
	(Format @global100 211 5) ; "Welcome to Z-Mart, where our everyday values are other stores' remainders!"
	(Format @global100 211 6) ; "Welcome to Z-Mart, where shoddy merchandise comes home to roost!"
	(Format @global100 211 7) ; "Welcome to Z-Mart, where yesterday's trash is today's bargain!"
	(Format @global100 211 8) ; "Welcome to Z-Mart. If you can't find it here, it must be worth having!"
	(Format @global100 211 9) ; "Welcome to Z-Mart, where Quality is something we often talk about!"
	(Format @global100 211 10) ; "Welcome to Z-Mart, where we've closed over 600 stores from coast- to-coast!"
	(Format @global100 211 11) ; "Thank you for shopping at Z-Mart."
	(Format @global100 211 12) ; "Have a very good day!"
	(Format @global100 211 13) ; "Have you checked out our red light specials?"
	(Format @global100 211 14) ; "Please visit us again soon."
	(Format @global100 211 15) ; "Can I have a price check on register two please!"
	(Format @global100 211 16) ; "Come back again. We have new specials each week."
	(Format @global100 211 17) ; "If you want cheap, we got it."
	(Format @global100 211 18) ; "Save some for our next...customer."
	(Format @global100 211 19) ; "Well, you get what you pay for!"
	(Format @global100 211 20) ; "Check out our new shipment of Taiwanese microwave cozies!"
	(Format @global100 211 21) ; "Don't forget to look in Aisle 14 for slightly irregular automotive parts!"
	(Format @global100 211 22) ; "Be sure to look for our service department...and if you find it, let us know!"
	(Format @global100 211 23) ; "Have you noticed our 83-piece steak knife set for only $4.95?"
	(Format @global100 211 24) ; "We're having a special on dented or scratched floppy disks...only $3.50 a box!"
	(Format @global100 211 25) ; "Take a look in Aisle 22 for half-price batteries. Sorry, demo models only!"
	(Format @global100 211 26) ; "If you don't see what you're looking for, look underneath something else!"
)
		...
		(if (not global518)
			...
			(for ((= temp0 0)) (> size 10) ((++ temp0))
				(if [global385 temp0]
					...
					(self delete: (self at: (= temp2 (Random 4 (- size 1)))))
					(= [global385 temp0] temp2)
				)
			)
			(if (== (global302 worksAt:) 11)
				(= temp3 (self indexOf: exitButton))
				(self addAfter: (self at: (- temp3 1)) workButton)
			)
			(for ((= temp0 (= temp4 0))) (< temp0 size) ((++ temp0))
				(if ((self at: temp0) isKindOf: DiscountDItem)
					((self at: temp0) nsTop: (+ (* (+ temp4 1) 13) 20))
					(if (>= temp4 2)
						((self at: temp0) nsLeft: 78)
					)
					(= [local0 temp4] ((self at: temp0) celNum:))
					(++ temp4)
				)
			)
			(items cel: [local0 0])
...
(instance refrigerator of DiscountDItem
	...
	(method (doFormat param1)
		(if (< price 1000)
			(Format param1 211 28 text price) ; "%s.||$%3d"
		else
			(Format param1 211 29 text price) ; "%s$%4d"
		)
	)
