...
(class Gauge of Dialog
	...
	(method (init value &tmp dx dy)
		(= window SysWindow)
		(self update: value)
		((= downI (DButton new:)) text: lower moveTo: 5 5 setSize:)
		(self add: downI setSize:)
		((= gaugeI (DText new:))
			text: @str
			moveTo: (+ (downI nsRight:) 5) 5
			font: 0
			setSize:
		)
		...
		((= upI (DButton new:))
			text: higher
			moveTo: (+ (gaugeI nsRight:) 5) 5
			setSize:
		)
		(self add: upI setSize:)
		(+= nsBottom 10)
		((= okI (DButton new:)) text: {OK} setSize: moveTo: 5 nsBottom)
		((= normalI (DButton new:))
			text: {Normal}
			setSize:
			moveTo: (+ (okI nsRight:) 5) nsBottom
		)
		((= noI (DButton new:))
			text: {Cancel}
			setSize:
			moveTo: (+ (normalI nsRight:) 5) nsBottom
		)
		(self add: okI normalI noI setSize:)
		(= dx (- (- nsRight (noI nsRight:)) 5))
		((= textI (DText new:))
			text: description
			font: gSmallFont
			setSize: (- nsRight 10)
