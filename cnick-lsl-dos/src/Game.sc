...
(class Game of Obj
	...
	(method (save &tmp [comment 20] num oldCur buf1 [buf2 100] [str 5])
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Unknown_Class_62 doit: @comment)) -1)
				(= parseLang buf1)
				(= oldCur (self setCursor: gWaitCursor 1))
				(if (not (SaveGame name num @comment gVersion))
					(Message msgGET 994 1 0 0 1 @buf2)
					(Message msgGET 994 2 0 0 1 @str)
