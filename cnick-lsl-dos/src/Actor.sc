...
(class View of Feature
	...
	(method (setScale theY &tmp temp0 temp1 temp2)
		(switch theY
			(0
				(&= vm_signal $fffd)
				(= maxScale 0)
			...
			(else
				(= temp0 (- theY (gCurRoom vanishingY:)))
				(= temp2 (+ (/ (* (= temp1 (- 190 theY)) 100) temp0) 100))
				(|= vm_signal $0003)
...
(class Actor of Prop
	...
	(method (doit &tmp temp0 temp1 temp2 temp3 temp4 [temp5 2])
		(if (& signal $8000)
			(return)
		)
		(if script
		...
		(cond
			...
			(mover
				(mover doit:)
				(if (& vm_signal $0001)
					(= temp3 (/ (* origXStep scaleX) 128))
					(= temp4 (/ (* origYStep scaleY) 128))
	...
	(method (setStep xs ys)
		...
		(if (and (IsObject mover) (mover isMemberOf: MoveTo))
			(mover init:)
		)
	)
