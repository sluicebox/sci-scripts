...
(instance rm240 of Room
	...
	(method (init)
		...
		(if (>= gDay 6)
			(= local0 gDoVerbCode)
			(= gDoVerbCode (ScriptID 241 31)) ; myDVC
		)
		(gFeatures
			add:
		...
		(switch (= state newState)
			...
			(8
				(mosely
					...
					view: (if (IsFlag 99) 2451 else 245)
					setCycle: StopWalk -1
					setMotion: PolyPath 227 128 self
				)
			)
			(9
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(36
				...
				(= local4 0)
				(= local5 1)
				(gMessager say: 27 0 65 19 self) ; "I tell you, at times like this, I'd kill for truth serum and damn the civil rights!"
			)
			(37
				(= gNarrator GKNarrator)
				(gMessager say: 27 0 65 20 self) ; "Can I quote you on that?"
			)
			(38
				(= local14 gNarrator)
				(= gNarrator officeNarrator)
				(gMessager sayRange: 27 0 65 21 22 self) ; "(ARCC)Huh? Hell, no! Damn! We can only keep him for twenty-four hours. Tomorrow morning I'm gonna have to let him go."
			)
			(39
				(= gNarrator GKNarrator)
				(gMessager say: 27 0 65 23 self) ; "(DRY)It certainly is. I'll see what I can do."
			)
			(40
				(= local14 gNarrator)
				(= gNarrator officeNarrator)
...
(instance getSomeCoffee of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(mosely setCycle: Walk setMotion: MoveTo 100 92 self)
				(if (IsFlag 16)
...
(instance openTheDrawer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (and (<= 220 (gEgo x:) 222) (<= 125 (gEgo y:) 126))
					(gEgo posn: 222 125)
					(= cycles 1)
				else
