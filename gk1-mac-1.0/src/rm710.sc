...
(class rm710 of Room
	...
	(method (init)
		...
		(if (== (gGame detailLevel:) 3)
			...
			(gGkSound1 setLoop: -1 number: 712 flags: 5 play:)
		)
	)
...
(instance keyWorks of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(4
				(gGkSound2 number: 714 setLoop: 1 play: self)
			)
			(3
				(ShakeScreen 5)
				(= cycles 10)
			...
			(7
				...
				(local9 dispose:)
				(gGkSound2 stop: number: 726 setLoop: 1 play:)
				(guard1
					setCel: 0
					setLoop: 0
					view: 709
			...
			(8
				(if (and (!= (gCurRoom script:) (ScriptID 711 0)) (not local2)) ; theDeathScript
					(gCurRoom setScript: (ScriptID 711 0)) ; theDeathScript
				)
			)
		)
	)
	...
	(method (doit)
		(if
			(and
				...
				(not local15)
				(!= (gCurRoom script:) (ScriptID 711 0)) ; theDeathScript
				(not local2)
			)
			(gCurRoom setScript: (ScriptID 711 0)) ; theDeathScript
		)
		...
		(switch (= state newState)
			(0
				...
				(switch local0
					...
					(else
						(guard1 setCel: 0 setLoop: 3 view: 709 posn: 150 127)
						(gGame handsOff:)
					)
				)
				(guard1
			...
			(1
				(cond
					(
						...
						(if (!= (gCurRoom script:) (ScriptID 711 0)) ; theDeathScript
							(gCurRoom setScript: (ScriptID 711 0)) ; theDeathScript
						)
					)
					((<= local0 2)
		...
		(switch (= state newState)
			...
			(1
				(cond
					...
					((not local4)
						(= local4 1)
						(gMessager say: 12 12 9 1 self) ; "(PICKING UP STONE TILE. THOUGHTFUL)This stone looks interesting."
					)
					(else
						(self cue:)
					)
...
(instance lookAtTile of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gTheIconBar enable: 0 1 2 4 6 7 8 9)
				(if (gTheIconBar curInvIcon:)
					(gTheIconBar enable: 12)
				)
				(gEgo normalize: 0 901)
				(gEgo ignoreActors:)
				(self dispose:)
...
(instance putTileOnWall of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(gEgo setCel: 7 setLoop: 1 view: 7121 setCycle: CT 6 -1 self)
			)
			(2
