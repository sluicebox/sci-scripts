...
(instance drumInset of Inset
	...
	(method (init)
		...
		(= local5
			(IDArray
				with:
					(Str with: {SUMMON:  i a a a e i})
					(Str with: {WARN:  i e g b b i})
					(Str with: {ENGAGE:  i b d d b i})
					(Str with: {REQUEST:  i a a g b i})
					(Str with: {INQUIRE:  i b b d b i})
					(Str with: {ANNOUNCE:  i d a b e i})
					(Str with: {CALL CONCLAVE:  i h a a a i})
					(Str with: {CALL TRIBAL MEETING:  i h b b e i})
					(Str with: {NO MESSAGE:  i d d d i})
					(Str with: {TODAY:  g b b g})
					(Str with: {TONIGHT:  g a a g})
					(Str with: {TOMORROW DAY:  g e e g})
					(Str with: {TOMORROW NIGHT:  g d d g})
					(Str with: {TODAY +n DAYS:  g b b n- e g})
					(Str with: {NEXT MOON:  g c c g})
					(Str with: {THIS MOON +n MOONS:  g c c n-f g})
					(Str with: {NEXT YEAR:  g f f g})
					(Str with: {THIS YEAR +n YEARS:  g f f n-i g})
					(Str with: {FUTURE:  g h i g})
					(Str with: {VILLAGE:  h b b h})
					(Str with: {HOUNFOUR:  h f f h})
					(Str with: {FOREST:  h d h})
					(Str with: {MOUNTAINS:  h c c h})
					(Str with: {PLAINS:  h a a h})
					(Str with: {LAKE:  h g a a h})
					(Str with: {RIVER:  h g b b h})
					(Str with: {SWAMP:  h g d d h})
					(Str with: {SEA:  h g f f h })
			)
		)
		(= local6
			(IDArray
				with:
					(Str with: {SISTER ALLIGATOR:  e f b b e})
					(Str with: {BROTHER EAGLE:  e d g d e})
					(Str with: {BROTHER TIGER:  e c c b e})
					(Str with: {SISTER OWL:  e d f c e})
					(Str with: {SISTER GAZELLE:  e b b c e})
					(Str with: {BROTHER RHINO:  e i b b e})
					(Str with: {BROTHER LION:  e g b c e})
			)
		)
