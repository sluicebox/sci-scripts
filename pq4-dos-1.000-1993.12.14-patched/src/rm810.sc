		...
		(switch theVerb
			(65 ; caseGlassJar
				(if (gEgo has: 20) ; shoe
					...
					(gMessager say: noun theVerb 8) ; "If you suspect hidden evidence, you'll need to look"
				)
			)
			(52 ; notebook
				(if (gEgo has: 20) ; shoe
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				3
				(if local1
					(gMessager say: 7 4 9) ; "You receive no answer to your knocking."
					(register dispose:)
					(gEgo normalize: 0)
					(gGame handsOn:)
					(self dispose:)
			...
			(8
				8
				(gMessager say: 2 2 5) ; "Yeah, what do ya want?"
				(= seconds 11)
			)
			(9
				9
				(if (or register seconds)
					(gGame handsOff:)
					(if (gPqFlags test: 0)
					...
					(dannyD cel: (dannyD lastCel:) init: setCycle: Beg self)
					(gEgo normalize: 0 setScaler: Scaler 90 65 161 103 init:)
					(proc0_4 0)
					(FrameOut)
				)
			)
