...
(procedure (localproc_0 param1 &tmp temp0)
	...
	(if local2
		...
		(if param1
			(SetCursor 1 (light y:) 2 (+ (light y:) 1))
		else
			(SetCursor 317 (light y:) 318 (+ (light y:) 1))
		)
	)
...
(instance wearVest of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(gMessager say: 5 15 0 1 self) ; "You put on your Kevlar vest. A good safety precaution."
			)
			(1
				(gGame handsOn:)
				(gEgo put: 5 0) ; vest
...
(instance light of Prop
	...
	(method (doVerb theVerb)
		(cond
			...
			(local2
				(gMessager say: 5 6 0 0) ; "What are you attempting to do exactly?"
			)
...
(instance showMsg of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if register
					(gMessager say: 0 41 0 1 self) ; "The draft blows the flame out."
				else
