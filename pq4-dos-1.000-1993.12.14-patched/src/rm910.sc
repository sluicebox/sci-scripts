...
(instance rm910 of Room
	...
	(method (init)
		...
		(leftPic init: sightAngle: (if (== gDay 3) 180 else 40))
		(rightPic init: sightAngle: (if (== gDay 3) 180 else 40))
		(poolTable init: sightAngle: (if (== gDay 3) 180 else 40))
		(poolBalls init: sightAngle: (if (== gDay 3) 180 else 40))
		(pretzels0 init: sightAngle: (if (== gDay 3) 180 else 40))
		(pretzels1 init: sightAngle: (if (== gDay 3) 180 else 40))
		(pretzels2 init: sightAngle: (if (== gDay 3) 180 else 40))
		(pretzels3 init: sightAngle: (if (== gDay 3) 180 else 40))
		(videoGame init: sightAngle: (if (== gDay 3) 180 else 40))
		(videoGame2 init: sightAngle: (if (== gDay 3) 180 else 40))
		(tv init: sightAngle: (if (== gDay 3) 180 else 40))
		(stools init: sightAngle: (if (== gDay 3) 180 else 40))
		(bartender
			init:
			signal: (| (bartender signal:) $4000)
			setScript: bartScr
			sightAngle: (if (== gDay 3) 180 else 40)
		)
		(barGlass
			init:
		...
		(if (== gDay 3)
			...
			(newsGuy init: signal: (| (newsGuy signal:) $4000) sightAngle: 180)
			(newsGuyHead
				init:
				signal: (| (newsGuyHead signal:) $4000)
				sightAngle: 180
			...
			(if (> (Random 0 100) 50)
				(newsGuy init: signal: (| (newsGuy signal:) $4000))
				(newsGuyHead init: signal: (| (newsGuyHead signal:) $4000))
			)
			(toMap init:)
			(gGame handsOn:)
		)
	...
	(method (dispose)
		(gEgo put: 42) ; beer
		(gTheIconBar curInvIcon: 0 curIcon: (gTheIconBar at: 0) disable: 4)
...
(instance passOutScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 3 62 3 0 self) ; "Sam, we better take him home."
			)
			(1
				(gPqFlags set: 178)
				(= gDay 4)
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 2) ; Talk
				(switch (++ local6)
					(1
						(gMessager sayRange: noun theVerb 0 1 2) ; "Sam, how many autopsies do you think you've performed?"
					)
					(2
						(gMessager sayRange: noun theVerb 0 3 4) ; "Sam, what do you like to do in your free time?"
					)
					(3
						(gMessager sayRange: noun theVerb 0 5 8) ; "Sam, how long have you and Chester known each other?"
					)
					(else
						(gMessager sayRange: noun theVerb 0 9 10) ; "Sam, have you ever dated Sherry Moore?"
					)
				)
			)
			(else
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(gMessager say: 7 0 0 1 self) ; "Carey! The Mayor has requested your presence at a City Council meeting first thing tomorrow. I want you outta here tonight before you have too much too drink. I need you alert and articulate tomorrow!"
			)
			(4
				(ltBlock setLoop: 1 1 cel: 0 setCycle: End)
				(gMessager say: 7 0 0 2 self) ; "And you Chester, what are you doing here consuming alcoholic beverages in your uniform?! I should slap you with a 181 personnel infraction! I want both you and Carey to pick yourselves up and go home. Remember, Carey, tomorrow, City Hall, 9:00 AM."
			)
			(5
...
(instance egoStandScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo normalize: 9120 setScaler: Scaler 90 66 145 108)
...
(instance videoGame of Feature
	...
	(method (init)
		(self createPoly: 319 102 286 97 286 72 293 64 291 35 319 33)
		(super init: &rest)
	)
)
...
(instance videoGame2 of Feature
	...
	(method (init)
		(self
			createPoly: 147 103 175 103 176 67 174 61 175 30 180 30 180 24 148 23
		)
		(super init: &rest)
	)
)
...
(instance poolTable of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 4) ; Do
			(if (== gDay 3)
				(gMessager say: noun theVerb 10 0) ; "You can't play pool sitting down."
			else
				(super doVerb: theVerb &rest)
			)
		else
...
(instance poolBalls of Feature
	...
	(method (init)
		(self
			createPoly: 274 101 284 103 308 112 300 126 279 116 276 109 249 113 245 110 258 101 266 100
		)
		(super init: &rest)
	)
...
(instance bartender of Prop
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 2) ; Talk
				(if (== gDay 3)
					(gMessager say: noun theVerb 10 0) ; "Refrain from hollering at Neal. People will think"
				else
					(switch (++ local3)
						(1
			...
			((== theVerb 4) ; Do
				(if (== gDay 3)
					(gMessager say: noun theVerb 10 0) ; "If you could reach Neal from here, you'd be Mr. Elastic."
				else
					(super doVerb: theVerb &rest)
				)
			)
