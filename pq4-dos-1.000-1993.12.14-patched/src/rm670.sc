		...
		(if (== gPrevRoomNum 671)
			...
			(gCurRoom
				addObstacle:
					((Polygon new:)
						type: PContainedAccess
						init: 12 97 129 93 319 136 319 117 128 88 12 91
						yourself:
					)
			)
		)
			...
			((== gPrevRoomNum 610)
				(cond
					((and (gPqFlags test: 128) (not (gPqFlags test: 101)))
						...
						(self setScript: startDreamScript)
					)
					((and (== gDay 6) (not (gPqFlags test: 135)))
						(msMitch scaleSignal: 1 scaleX: 72 scaleY: 72 init:)
						(gCurRoom setScript: msMitchScript)
					)
					(else
						(gEgo
							setScaler: Scaler 66 30 131 93
...
(instance exitScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo setMotion: PolyPath 306 130 self)
			)
			(2
...
(instance msMitch of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(22 ; handgun
				(gMessager say: 6 22 3 0) ; "There's no guarantee you'll mark your target given the proximity"
			)
			(27 ; badge
		...
		(switch theVerb
			...
			(1 ; Look
				(cond
					...
					((== gDay 6)
						(gMessager say: noun theVerb 6 0) ; "The door leads to the backstage area."
					)
					(else
						(gMessager say: 4 1 5 0) ; "There is a door leading to who knows where."
					)
				)
			)
			(else
