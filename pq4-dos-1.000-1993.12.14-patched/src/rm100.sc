...
(instance rm100 of Room
	...
	(method (init)
		...
		(gEgo init:)
		(if (and (== gDay 2) (OneOf gPrevRoomNum 17 6)) ; MapRoom, speedRoom
			(gGlobalSound0 number: 110 loop: -1 play:)
		)
		(switch gDay
			(1
				(dumpster
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(if (== gDay 1)
					(gMessager say: 0 1 16 0) ; "Back alley to the Lucky Mini Mart... Doesn't look so lucky now."
				else
			...
			(switch theVerb
				(4 ; Do
					(if (not (dumpsterLid cel:))
						...
						(gMessager say: 27 4 37 0) ; "Disturbing evidence now can result in a mistrial later."
					)
				)
				(1 ; Look
					(if (not (dumpsterLid cel:))
				...
				(38 ; caseChalk
					(if (gPqFlags test: 8)
						(gMessager say: 26 38 38 0) ; "Chalking the dumpster is unnecessary."
					else
						(gMessager say: 26 38 28 0) ; "What is the purpose of that?"
					)
				)
				(73 ; crimeSceneReport
					(if (dumpsterLid cel:)
				...
				(65 ; caseGlassJar
					(if (dumpsterLid cel:)
						(gMessager say: 26 65 38 0) ; "Any fluids to be taken from the boy will be"
					else
						(super doVerb: theVerb)
					)
				)
				...
				(47 ; handcuffs
					(if (dumpsterLid cel:)
						(gMessager say: 26 47 38 0) ; "That has no meaning."
					else
						(super doVerb: theVerb)
					)
				)
				(63 ; casePryBar
					(if (dumpsterLid cel:)
						(gMessager say: noun theVerb 38 0) ; "The dumpster is open. It doesn't need prying."
					else
						(gMessager say: noun theVerb 28 0) ; "The dumpster is not locked. Try opening it the conventional way."
					)
				)
				(2 ; Talk
					(if (dumpsterLid cel:)
...
(instance toObjectScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if (and (== register 2) (not local3))
					(= local3 1)
					(gMessager say: 26 4 22 0 self) ; "The dumpster lid is quite heavy."
				else
					(= cycles 2)
				)
			)
...
(instance car of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gMessager say: 30 4 15 0) ; "It's locked."
			)
			(46 ; keys
			...
			(4
				(switch register
					(0
						(gGame points: 3 1)
						(gPqPointFlags set: 1)
						(gMessager say: 25 52 0 0 self) ; "Noting physical evidence of the deceased is necessary"
					)
					(1
						(gMessager say: 14 52 0 0 self) ; "Graffiti is often left behind by gangs."
					)
					(2
						(gGame points: 2 7)
						(gPqPointFlags set: 7)
						(gMessager say: 27 52 0 0 self) ; "Recording information at the crime scene"
					)
				)
			)
			(5
