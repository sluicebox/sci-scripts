...
(instance rm615 of SQRoom
	...
	(method (init)
		...
		(band setCycle: Fwd init:)
		(if (not (gEgo has: 13)) ; matches
			(theMatches init: stopUpd:)
		)
		...
		(switch theVerb
			(2 ; Look
				(if (!= (++ local3) 6)
					...
					(Print 615 7) ; "Quality Assurance by:    Mike Harian    Dan Scott   Tate Deloach   Keri Cooper   Bill Shockley    Neil Matz   Gordon Owens   Dave Fleming   Danny Woolard  Chris Carr   John Ratcliffe  Robin Bradley   Joe Perry     Mike Pickhinke   Sharon Smith   Sharon Simmons   Kevin Ray     Roger Clendenning   Gary Cox     Mike Brosius   Judy Crites    Dave Clingman   Bill Hilton"
				)
			)
			(else
