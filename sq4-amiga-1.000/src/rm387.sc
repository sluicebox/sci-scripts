...
(procedure (localproc_1 param1 param2 param3 param4 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 [temp11 5])
	(= temp2 (proc0_18 global158 global155 global157))
	(= temp8 14)
	(= temp4 global130)
	(= temp7 (proc0_18 global161 global130 global130))
	(= temp6 (proc0_18 global157 global156 global156))
	(= temp5 (proc0_18 global156 global156 global155))
	(= temp1 3)
	(= temp9 2)
	(= temp3 3)
	(= temp0 (GetPort))
...
(instance instructScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(restoreCode doit:)
				(Print 387 1) ; "This is so easy, a human could probably do it. Burger comes out of the oven. Drop on yer lettuce, yer pickle, squeeze on yer mayo, squirt on yer mustard, on goes yer ketchup, top it off with yer sesame-seed bun. You make 'em MY way, and if you mess up enough times, you're outta here. Got it?"
				(Print 387 2) ; "If ya need to know how to run the assembly line, read the sign. Just click it with the mouse or press R."
				(= cycles 1)
			)
			(2
				...
				(gGame setCursor: (= local1 ((gTheIconBar curIcon:) cursor:)))
			)
		)
	)
)
...
(instance readSignCode of Code
	...
	(method (doit)
		(Print 387 3) ; "Instructions for the MasterBurger 2000 This notice must be posted wherever the MasterBurger 2000 is in use. The hand cursor must be active in order to pick up and install condiments."
		(Print 387 4) ; "Mouse: Click on condiment to pick it up. Click it again on top of burger to install it. If you're carrying a condiment and you cycle to another cursor, the condiment drops back into its bay."
		(Print 387 5) ; "Joystick: Moving up & down switches between condiments and burger. When on condiment level, moving left & right moves cursor between the condiments. When on burger level, cursor follows burger."
	)
)
...
(instance lettuceBay of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 6) ; "Uhmm! Lettuce. Real old lettuce."
			)
			(11 ; Smell
				(Print 387 7) ; "Smells like lettuce."
			)
			(else
...
(instance pickleBay of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 8) ; "Tastes like pickelene slices."
			)
			(11 ; Smell
				(Print 387 9) ; "Smells like pickles."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance bunBay of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 10) ; "They almost taste fresh."
			)
			(11 ; Smell
				(Print 387 11) ; "Must be PermaBuns. They have no smell."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance mayo of Condiment
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 12) ; "It tastes white."
			)
			(11 ; Smell
				(Print 387 13) ; "It has an oily smell."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance must of Condiment
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 14) ; "Tastes a little spicey."
			)
			(11 ; Smell
				(Print 387 15) ; "Smells a little spicey."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance cats of Condiment
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 16) ; "It tastes wet. Otherwise, it's tasteless."
			)
			(11 ; Smell
				(Print 387 17) ; "It has no smell."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance pickle of Condiment
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 18) ; "Tastes like pickle slices."
			)
			(11 ; Smell
				(Print 387 9) ; "Smells like pickles."
			)
...
(instance burger of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(10 ; Taste
				(Print 387 19) ; "Hey! This belongs to someone else!"
			)
			(11 ; Smell
				(Print 387 20) ; "There's that Monolith Burgers Secret Smell(tm)."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance grabber of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 21) ; "That's a good way to put your tongue out, buddy."
			)
			(11 ; Smell
				(Print 387 22) ; "Smells greasy."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance rejectSign of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 23) ; "It tastes greasy."
			)
			(11 ; Smell
				(Print 387 24) ; "It smells greasy."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance belt of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 25) ; "Essence of Monolith Burger now coats your tongue - forever!"
			)
			(11 ; Smell
				(Print 387 26) ; "It smells like the essence of Monolith Burger."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance topCounter of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 27) ; "You must be bored. Maybe we should turn up the speed."
			)
			(11 ; Smell
				(Print 387 27) ; "You must be bored. Maybe we should turn up the speed."
			)
...
(instance upperPipes of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 28) ; "Don't mess with it."
			)
			(11 ; Smell
				(Print 387 29) ; "Smells like a pipe!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance hood of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 387 30) ; "It definitely hasn't been cleaned in awhile. Maybe later when you get time you could give it a buff."
			)
			(11 ; Smell
				(Print 387 31) ; "It smells likes escaping air."
			)
			(else
				(super doVerb: theVerb invItem)
			)
