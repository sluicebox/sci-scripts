...
(class mall of Rgn
	...
	(method (init &tmp temp0)
		...
		(if (and (!= gCurRoomNum 387) (> (gGame detailLevel:) 0))
			(shopper2
...
(instance shopper2 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print
					{These guys are scary looking. It wouldn't be a good idea to mess with them.}
				)
			)
			(5 ; Talk
				(Print 700 0) ; ""Help me fertilize my mother's cat", the being believes it hears you say! Embarrassed by your presence, it acts as though it doesn't see you."
			)
			(11 ; Smell
				(Print 700 1) ; "Taking in a whiff of air, you instantly notice its three feet. You thought you smelled a Dr. Scholl's product!"
			)
			(10 ; Taste
				(Print
					{As much as we enjoy seeing you get in trouble, we must warn you that this type of behavior is universally considered either rude or amorous, which could get you either killed or married (with the former being preferable in many cases).}
				)
			)
			(else
...
(instance shopper3 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(11 ; Smell
				(Print 700 2) ; "Uhmm! Sulfur! This shopper smells as though it comes from a planet where the only showers are volcanic."
			)
			(10 ; Taste
				(Print
					{As much as we enjoy seeing you get in trouble, we must warn you that this type of behavior is universally considered either rude or amorous, which could get you either killed or married (with the former being preferable in many cases).}
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					...
					(4 ; battery
						(Print 700 3) ; "No way! That would be assualt with intent to emit battery!"
					)
					(6 ; paper_with_gum
						(Print
...
(instance belts of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 700 4) ; "Your index digit scrapes the interesting coating that now resides on the surface of the belt. Just think, there's probably toe jam from all corners of the universe wedged up under your fingernail!"
			)
			(11 ; Smell
				(Print 700 5) ; "You attempt to smell the moving walkway and end up filing down your nose slightly."
			)
			(10 ; Taste
				(Print 700 6) ; "Very well, you give the moving walkway a warm kiss. Now you both feel special."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance restOfMall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 700 7) ; "The mall has that high-rent, low-occupancy smell...like any office building in downtown Los Angeles."
			)
			(10 ; Taste
				(Print 700 8) ; "It's against the Third Law of Mall Security to be caught licking mall components."
			)
			(else
