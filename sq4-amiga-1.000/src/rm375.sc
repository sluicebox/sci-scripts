	...
	(method (init &tmp temp0 temp1)
		...
		(cond
			((and (!= gPrevRoomNum 376) (not (IsFlag 9)))
				(gLongSong2 number: 19 loop: -1 flags: 1 playBed:)
			)
			((IsFlag 9)
		...
		(light1 setCycle: Fwd init:)
		(light2 setCycle: Fwd init:)
		(super init:)
		(self
			addObstacle:
				((Polygon new:)
...
(instance door of PicView ; UNUSED
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 375 0) ; "It's locked down tight. You can't open it."
			)
			(11 ; Smell
				(Print 375 1) ; "It smells like the kind of place where unwashed teen-agers stand around smoking, sweating and performing other unmentionable teen-age acts."
			)
			(10 ; Taste
				(Print 375 2) ; "It tastes like it smells."
			)
			(else
...
(instance light1 of Prop
	(properties
		...
		y 110
		description {sign}
		lookStr {These lights are hypnotic. Maybe they're some of those "subliminal optic stimulators" you've read about. As you gaze into their glare, you become very hungry for sushi.}
		view 375
		signal 16384
		detailLevel 3
	)
...
(instance bush1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 375 3) ; "There's a fresh woodsy scent coming from the plants."
			)
			(10 ; Taste
				(Print 375 4) ; "Urp...somebody's dumped their potpourri on these plants."
			)
			(3 ; Do
				(Print 375 5) ; "Get some taste! These are phoney mall plants!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
