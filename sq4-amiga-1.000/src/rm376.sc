...
(instance rm376 of SQRoom
	...
	(method (init)
		...
		(screen4 setCycle: Fwd detailLevel: 2 init:)
...
(instance zapMsChickenS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 376 0) ; "Whoa! This game's circuits have melted into a mass of molten silicon. But the fireworks sure were pretty!"
				(blast view: 499)
				(self dispose:)
			)
		)
...
(instance pod of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(11 ; Smell
				(Print 376 1) ; "It smells scorched."
			)
			(10 ; Taste
				(Print 376 2) ; "It tastes scorched. Oxidized materials now coat your tongue."
			)
			(else
...
(instance sp1 of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(11 ; Smell
				(Print
					{Seeing as how these guys are trying to kill you, your best line of defense would be a straight one out of here.}
				)
			)
			(10 ; Taste
			...
			(4 ; Inventory
				(Print
					(switch invItem
						(0 ; buckazoid
							(Print
								{The Sequel Police are closely monitored by Vohaul. Bribing them would be out of the question.}
							)
						)
						(1 ; rope
							(Print
								{Even if you were to lasso a Sequel Policeman, what would you do with him?}
							)
						)
						(3 ; rabbit
							(Print
								{"Ooooo! Don't throw the bunny at Sequel Policeman! He may have a hare trigger. (Sorry!)}
							)
						)
						(4 3) ; battery
						(6 ; paper_with_gum
						...
						(9 ; pen
							(Print
								{He's gonna SHOOT you, not write you a ticket!}
							)
						)
						(10 ; atmCard
							(Print
						...
						(else
							(Print
								{You'd better not. You might need that later.}
							)
						)
					)
				)
...
(instance screen1 of PicView
	...
	(method (doVerb theVerb invItem)
		(if (or (== theVerb 3) (and (== theVerb 4) (== invItem 0))) ; Do, Inventory, buckazoid
			(cond
				((>= global191 5)
					(Print 376 4) ; "The Ms. Astro Chicken machine looks as though it's taken its last buckazoid and given its last "BACOCK!"."
				)
				(gBuckazoidCount
				...
				(else
					(Print 376 5) ; "You seem to be fresh out of buckazoids. Darn the luck!"
				)
			)
		else
			(switch theVerb
				(11 ; Smell
					(Print
						{Gooey, greasy little fingers, and various other types of appendages have left a coating of something that smells like a cross between cotton candy and dead gerbils.}
					)
				)
				(10 ; Taste
					(Print
						{Auugghh! You had to taste it! This is going to linger for awhile. That coating of something that smells like a cross between cotton candy and dead gerbils tastes more like the latter!}
					)
				)
				(else
...
(instance screen6 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 376 6) ; "That's too high for you to reach."
			)
			(11 ; Smell
				(Print 376 7) ; "That's too high for you to smell."
			)
			(10 ; Taste
				(Print 376 8) ; "That's too high for you to taste."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance basket of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 376 9) ; "Resist those inbred custodial tendencies to collect garbage. You have much more important garbage to tend to!"
			)
			(5 ; Talk
				(Print 376 10) ; "You're quite startled to hear the wastebasket respond..."
				(Print 376 11) ; "Max, is that you? Agent 99? I always get these lousy disguises. Would you put a word in with the Chief for me? I wouldn't mind, but all these kids eat is ice cream and candy bars. Do you know how many Big Chewies I've eaten in the last hour? Not to mention the wad of Big Bang BubbleGum I've got stuck to my shoe phone... (sigh!)"
			)
			(11 ; Smell
				(Print 376 12) ; "Give your nose a break! It's a garbage can!"
			)
			(10 ; Taste
				(Print 376 13) ; "A garbage can?!"
			)
			(4 ; Inventory
				(Print 376 14) ; "You'd better not throw that away. It'll probably come in handy later. I know it doesn't seem likely, but trust me on this one."
			)
			(else
...
(instance kid of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 376 15) ; "Leave the kid alone. He's totally engrossed in his game."
			)
			(5 ; Talk
				(Print 376 16) ; "Talking to a kid in the middle of an arcade game is almost as interesting as talking to the wall. Give it up."
			)
			(11 ; Smell
				(Print 376 17) ; "Perhaps you haven't been around a young, active being lately. I can tell you that walking up to one and giving it a sniff could be considered unstable and even dangerous behavior."
			)
			(10 ; Taste
				(Print 376 18) ; "Perhaps you haven't read the nose message."
			)
			(else
				(super doVerb: theVerb)
			)
		...
		(switch theVerb
			(3 ; Do
				(cond
					((not (gEgo inRect: 116 102 206 124))
						(Print 376 19) ; "You're not close enough to the change machine to touch it."
					)
					((OneOf (gEgo view:) 373 374)
						(Print 376 20) ; "A real lady wouldn't try to break a change machine."
					)
					((and local6 (not (gCurRoom script:)))
						(gCurRoom setScript: changeMachineDeathS)
					)
					(else
						(Print 376 21) ; "Banging on the change machine will only activate its self-defense mechanisms."
						(= local6 1)
					)
				)
			)
			(4 ; Inventory
				(if (== invItem 0) ; buckazoid
					(Print 376 22) ; "In all the known universe, the lowly buckazoid is the lowest denomination of all. You can't get change for it."
				else
					(super doVerb: theVerb invItem)
				)
			)
...
(instance sushiBar of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 376 23) ; "Rocking the sushi bar machine is impossible. It's bolted to the floor."
			)
			(4 ; Inventory
				(if (== invItem 0) ; buckazoid
					(Print 376 24) ; "In all the known universe, the lowly buckazoid is the lowest denomination of all. You can't even get a dead fish for it."
				else
					(super doVerb: theVerb invItem)
				)
			)
			(11 ; Smell
				(Print 376 25) ; "It smells like old dead fish."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance arcadeRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 376 26) ; "Touching anything in here might be dangerous to your health."
			)
			(11 ; Smell
				(Print 376 27) ; "The aroma of the sushi bar dominates the air in here. However, you can also detect the smell of smoke, sweat and ozone, reminding you of your favorite cologne: "Night in Programming.""
			)
			(else
				(super doVerb: theVerb)
			)
