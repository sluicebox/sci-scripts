		...
		(switch gPrevRoomNum
			...
			(else
				...
				(if (IsFlag 1)
					...
					(pod init:)
					(if (not (IsFlag 26))
						(sp1 loop: 0 init: stopUpd:)
						(sp2 view: 7 loop: 0 x: 185 y: 91 init: stopUpd:)
					)
					(gEgo
						x: 282
						y:
					...
					(sp1 init: setScript: policeArrive)
...
(instance policeArrive of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(sp1 setHeading: 180 stopUpd:)
				(hatch init: cel: 0 setPri: (+ (hatch priority:) 2) stopUpd:)
				(pod setCycle: End)
				(= cycles 12)
			)
			(4
