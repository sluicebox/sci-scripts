...
(class Game of Obj
	...
	(method (save &tmp temp0 [temp1 4])
		(= gStr (Str new:))
		(Save 0 name temp0 (gStr data:) (KString 9 gVersion)) ; SaveGame, UNINIT, StrGetData
		(gStr dispose:)
	)
	...
	(method (restore &tmp num oldCur buf1 buf2 str aPlane aList obj bit node1 node2 node3)
		...
		(if (== gCurRoomNum 100)
			(gWaitCursor view: 150 loop: 3 cel: 1)
		)
		(self setCursor: gWaitCursor 1)
		(Save 1 name num gVersion) ; RestoreGame, UNINIT
		(for
			((= node1 (KList 3 (gPlanes elements:)))) ; FirstNode
			node1
			((= node1 (gPlanes nextNode:)))
