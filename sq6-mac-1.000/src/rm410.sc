	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(section2 setCycle: End self)
			)
			(2
				(section3 setCycle: End)
				(section4 setCycle: End self)
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(sfxSound number: 411 setLoop: 0 play:)
				(digger
					view: 414
					setLoop: 0
					setCel: 0
			...
			(4
				(sfxSound number: 412 setLoop: 0 play:)
			...
			(6
				(grave dispose:)
				(sfxSound number: 413 setLoop: 0 play:)
				(digger
					view: 4141
					cycleSpeed: (+ local0 8)
					setCel: 0
...
(instance sRogerTalks of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(sfxSound number: 414 setLoop: 0 play:)
			...
			(6
				(gMessager say: 19 0 4 local1 self)
			)
			(7
				(if (< local1 2)
					(++ local1)
					(kielbasa setScript: 0)
					(self changeState: (- state 1))
				else
					(if (& gMsgType $0001)
						(= local3 (Str format: {Roger\n}))
						(gTextScroller addString: (local3 data:) 50 84 0)
						(local3 dispose:)
						(Message msgGET 410 19 0 4 3 (local4 data:)) ; "Of my friend I can say only say this: Of all the soles I have encountered in my cleaning, (EMOTIONAL PAUSE) hers were the most scu... (VOICE BREAKING) SCUFF-RESISTANT!"
						(= local3 (Str format: {%s\n\n} local4))
						(= local6
							(gTextScroller
								addString: (local3 data:) gUserFont 82 0 0
							)
						)
						(local3 dispose:)
						(local4 dispose:)
