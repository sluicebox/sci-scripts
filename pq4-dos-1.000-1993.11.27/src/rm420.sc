...
(instance insetMurderScene of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 13) ; caseBaggies
			(if (gPqFlags test: 42)
				(gMessager say: noun theVerb 3) ; "There is no evidence to collect at this time."
			else
				(gMessager say: noun theVerb 4) ; "There is no evidence to place in the bag."
			)
		else
		...
		(cond
			((== theVerb 4) ; Do
				(cond
					...
					((gPqFlags test: 41)
						(gMessager say: noun theVerb 4) ; "It looks human, handle it carefully."
					)
					(else
			...
			((== theVerb 13) ; caseBaggies
				(cond
					((gPqFlags test: 42)
						(gMessager say: noun theVerb 5) ; "There is no evidence to collect at this time."
					)
					((gPqFlags test: 41)
						(gGame points: 3)
						(gMessager say: noun theVerb 4) ; "Placing the bone in the bag is essential to maintaining the"
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 22) ; handgun
				(cond
					((not local0)
						(gMessager say: 1 22 1) ; "The dog is not rabid. Holster your gun!"
						(++ local0)
					)
...
(instance touchDogScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 1 4 0 1) ; "Hey, boy... how ya doin'..."
				(gEgo setHeading: 45 self)
			)
			(2
				(gEgo
			...
			(4
				(gEgo normalize: 0 setScaler: Scaler 90 16 145 78)
				(gMessager say: 1 4 0 2) ; "Careful, this dog is unfriendly."
				(gGame handsOn:)
