...
(instance rm115 of Room
	...
	(method (init)
		...
		(theApple init: approachVerbs: 4 1) ; Do, Look
		(cashRegister init:)
		(counter init:)
		(paper0 init:)
		(paper1 init:)
...
(instance firstTimeScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(tvScreen setLoop: 2 cel: 0 setCycle: End self)
			)
			(11
				(gMessager say: 1 2 1 5 self) ; "What are you doing? Hey!"
			)
			(12
				(careyTalker x: 5 y: 15)
				(tvStinger fade: 0 2 3 1)
				(tvScreen setLoop: 3 cel: 0 setCycle: End self)
			)
			(13
				(tvScreen setLoop: 5 setCycle: 0)
				(= cycles 2)
			)
			(14
				(gMessager say: 1 0 1 0 self) ; "Not too smart, big policeman. You push girl on TV."
			)
			(15
				(= seconds 4)
			)
			(16
				(tvScreen setLoop: 6 setCycle: Fwd)
		...
		(cond
			((== theVerb 4) ; Do
				(if (or (gEgo has: 9) (gPqFlags test: 49)) ; apple
					(gMessager say: noun theVerb 2 0) ; "That apple for my friend, not for sale!"
				else
					(gEgo get: 9) ; apple
					(gGame points: 2 111)
					(gMessager say: noun theVerb 1 0) ; "The apples look good, you take one."
				)
			)
			((== theVerb 19) ; apple
				(if (not (gPqFlags test: 49))
					(gMessager say: noun theVerb 16 0) ; "You handle fruit rough! Watch out, you bruise my fruit!"
				else
					(gMessager say: noun theVerb 17 0) ; "You don't like, you put back? You buy - you keep, no refund!"
				)
			)
			(else
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(2 ; Talk
				(cond
					((== (kim script:) stealScr)
						(gMessager say: 1 0 7 2) ; "No steal!"
					)
					((== gDay 2)
						(switch local4
							(0
			...
			(57 ; photo
				(if (not local2)
					(++ local2)
					(gMessager say: noun theVerb 1 0) ; "Do you recognize the man in this picture?"
				else
					(super doVerb: theVerb &rest)
				)
			)
			...
			(19 ; apple
				(if (gPqFlags test: 49)
					(gMessager say: noun theVerb 17) ; "No exchange!"
				else
					(gMessager say: noun theVerb 16) ; "You want? You pay."
				)
			)
			(17 ; glue
				(if (gPqFlags test: 48)
					(gMessager say: noun theVerb 17) ; "No return, no cash back!"
				else
					(gMessager say: noun theVerb 16) ; "You want? You pay."
				)
			)
			(52 ; notebook
				(if (not local3)
					(++ local3)
					(gGame points: 2 109)
					(gMessager say: noun theVerb 1 0) ; "Can you tell me your name, and what your position is here?"
				else
					(gMessager say: noun theVerb 2 0) ; "You've already recorded the information."
				)
			)
			(29 ; coins
				(cond
					(
						...
						(gMessager say: noun theVerb 3) ; "What you buy?"
					)
					((and (gEgo has: 7) (not (gPqFlags test: 48))) ; glue
						(gMessager say: noun theVerb 4) ; "You try cheat me? You pay more."
						(if (== script stealScr)
							(script cue:)
						)
					)
			...
			(20 ; wallet
				(cond
					(
						...
						(gMessager say: noun theVerb 3) ; "No make change! What you buy?"
					)
					((and (gEgo has: 7) (not (gPqFlags test: 48))) ; glue
						(gGame handsOff:)
						(gEgo get: 7) ; glue
					...
					((and (gEgo has: 9) (not (gPqFlags test: 49))) ; apple
						(gMessager say: noun theVerb 5) ; "That too much, apple old, costs you less."
						(if (== script stealScr)
							(script cue:)
						)
					)
...
(instance stealScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(stealStinger play:)
				(kim view: 1154 setLoop: 0 cel: 0 setCycle: End self)
			)
			(1
				(if (> local0 1)
					(= cycles 2)
...
(instance getGlueScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gMessager say: 3 4 0 1) ; "You take a container of glue."
				(gGame points: 2 110)
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			(if (not local5)
				...
				(gMessager say: noun theVerb 2 0) ; "Lottery tickets... fools' gold."
			)
		else
			(super doVerb: theVerb &rest)
		)
