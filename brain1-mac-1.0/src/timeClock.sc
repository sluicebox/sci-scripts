...
(procedure (localproc_5 &tmp temp0)
	(if local20
		...
		(Graph grUPDATE_BOX 20 (- 48 local22) 131 (+ local22 71) 1)
		(localproc_6)
	else
...
(procedure (localproc_6 &tmp temp0)
	(= temp0 (GetPort))
	(SetPort 0)
	(= local19 (Graph grSAVE_BOX 0 0 180 (proc0_1 47 46) 1))
...
(instance timeClock of PuzzleBar
	...
	(method (buyClue)
		(cond
			((not (gEgo has: 7))
				(proc5_9 190 2) ; "You need to find a time card before you can punch in."
			)
			((super buyClue: &rest)
				(= local48 1)
				(if local20
					...
					(proc5_9 190 3) ; "Of course, Dr. Brain values cleverness even more than punctuality. Try using the key you got from the Math Marvel to open the Time Clock and reset the time."
	...
	(method (select)
		(if (and (== gTheCursor 111) (super select: &rest))
			...
			(if (localproc_0)
				...
				(proc5_11 190 5) ; "It's not the right time to clock in yet."
			)
	...
	(method (highlight param1 &tmp temp0 temp1 [temp2 50])
		(if param1
			...
			(if (not local48)
				...
				(Format @temp2 190 6 temp1 temp0) ; "The %s card has previously been used at: %s"
