...
(procedure (localproc_5)
	(if (localproc_0)
		...
		(Wait 60)
		(rewardSound play:)
		(Print 320 11 #at 59 12) ; "Congratulations! You've formed the secret password "ENTER" with your tangram pieces. The door to the Puzzle Room opens."
			...
			(cond
				((!= value 7)
					(if (== gTheCursor 27)
						...
						(temp0 posn: (+ (tan eventX:) 32) (+ (tan eventY:) 9))
					)
				)
				(local84
					(gGame setCursor: 8)
...
(instance leftFeat of Feature
	(properties
		description {left-hand door}
		onMeCheck 2
		lookStr {A sign on the door reads "Word Games".}
	)
...
(instance backFeat of Feature
	(properties
		...
		onMeCheck 8
		lookStr {The sign over this door reads "Puzzle Room". The door has a curious square-shaped lock mechanism.}
	)

	(method (doVerb theVerb invItem)
		(if (or (== theVerb 3) (and (== theVerb 4) (== invItem 20)))
			(gGame setCursor: 8)
			(cond
				((IsFlag 9)
					(doorSound play:)
				...
				(else
					(proc5_9 320 8) ; "You will need to find some sort of "key" to open the door's strange-looking lock."
				)
			)
		else
...
(instance tree of Feature
	(properties
		...
		onMeCheck 4
		lookStr {It's a rubber tree. These can be very useful should you fall off a high cliff. They are much less useful inside a castle.}
	)

	(method (doVerb theVerb)
		(if (== theVerb 3)
			(gCMusic2 number: 968 flags: 1 setLoop: 1 setVol: 127 play:)
			(Print 320 9 #at 67 12) ; "The rubber tree feels kind of rubbery. What a surprise!"
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance drBrainBust of Feature
	(properties
		...
		description {bust}
		onMeCheck 256
		lookStr {This is a bust of Dr. Brain on a classic Ionic column, as used by the ancient Greeks. (You had no idea he was so old! Or so vain.)}
	)

	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 320 10) ; "The marble bust is cool to the touch. (After all, it hardly knows you!)"
		else
			(super doVerb: theVerb &rest)
		)
	)
