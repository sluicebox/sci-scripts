	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(Print 700 3 #at 15 -1 #width 280 #font gGiantFont) ; "VIRGIN!!!"
				(= seconds 3)
			)
			(4
				(gEgo
...
(instance sDropoff of Script
	...
	(method (doit)
		...
		(if (or (== state 1) (== state 5))
			(taxi x: (- 150 register))
			(gTheMusic3 send: 9 224 (+ (* (- 150 (taxi x:)) 10) 3000))
			(if (< (taxi x:) -800)
				(= cycles 1)
			)
			(*= register 2)
...
(instance sTaxiScript of Script
	...
	(method (doit)
		(super doit: &rest)
		(if (== state 4)
			(gTheMusic3 send: 9 224 (* (- (taxi x:) 150) 50))
		)
		(if (== state 5)
			(taxi x: (+ 150 register))
			(gTheMusic3 send: 9 224 (* (- (taxi x:) 150) 30))
			(if (== register 0)
				(gTheMusic3 send: 9 224 0)
				(= cycles 1)
			)
			(if (== register 16)
				(soundFX number: 202 setLoop: 1 flags: 1 play:)
				...
				(if (not (gCast contains: taxi))
					...
					(cond
						((== (taxi x:) 150)
							...
							(Print 700 8) ; "Stop whistling and get in the cab!"
						)
						((< (taxi x:) 150)
							(if gModelessDialog
								(gModelessDialog dispose:)
							)
							(Print 700 9) ; "Too late now, Ace-(in-the)-hole!" shouts the rude cabbie, "I'm outta here!"
						)
						(else
							(if gModelessDialog
								(gModelessDialog dispose:)
							)
							(Print 700 10) ; "Hold your horses, he's on his way!"
						)
					)
					(HandsOn)
					(self dispose:)
...
(instance sTaxiWait of Script
	...
	(method (doit)
		...
		(if (== state 7)
			...
			(gTheMusic3 send: 9 224 (* (- 150 (taxi x:)) 10))
			(if (< (taxi x:) -120)
				(= cycles 1)
			)
			(*= register 2)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Print 700 11 #dispose #time 4) ; ""I ain' gonna wait all day, bub!" yells the cabbie."
				(= cycles 9)
			)
			(3
				(if
...
(instance sFlattenLarry of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Death 806 2)
				(Format @gYourPart 700 12) ; "That's you all over!"
				(GameOver 700 13)
			)
		)
	)
...
(instance sEnterTaxi of Script
	...
	(method (doit &tmp maxVal)
		...
		(if (== state 3)
			...
			(gTheMusic3 send: 9 224 (if (< maxVal 2000) maxVal else 2000))
			(if (< (taxi x:) -80)
				(= cycles 1)
			)
			(*= register 2)
...
(instance taxi of Actor
	...
	(method (cue)
		...
		(Print 700 14) ; "It's not very clean."
		(Print 700 15) ; "It's not very safe."
		(Print 700 16 #at -1 140) ; "(And those are its good points!)"
	)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(11 ; Taste/Smell
				...
				(Print 700 17) ; "It smells like an old cab."
			)
			(3 ; Do
				(if (< (taxi x:) 150)
					(if gModelessDialog
					...
					(if (not (IsFlag 34))
						...
						(Print 700 19) ; "Vowing to hold your breath forever, you enter this miserable excuse for public transportation."
					)
					(HandsOff)
					(taxi setScript: 0)
					(soundFX stop:)
					(gEgo setScript: sEnterTaxi)
				)
			)
			(else
	...
	(method (doVerb theVerb invItem)
		...
		(switch theVerb
			(2 ; Look
				(cond
					((== (dog view:) 821)
						(Print 700 20) ; "Obviously, he thinks you're a fire plug."
					)
					((== (dog script:) sRunOff)
						(Print 700 21) ; "You hope he's soon a "Hmong" friends."
					)
					(else
						(Print 700 22) ; "Dogs like that make you want to keep moving."
					)
				)
			)
			(3 ; Do
				(Print 700 23) ; "As you bend over to show that dog who's boss, his bare teeth convince you there are worse things than damp trousers!"
			)
			(11 ; Taste/Smell
				(Print 700 24) ; "The dog smells quite bad, although still better than you."
			)
			(10 ; Zipper
				(Print 700 25) ; "It would seem turn-about would be fair play, but you just don't have it in you!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance taxiSignProp of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 700 26) ; "You whistle loudly for a taxi."
				(HandsOff)
