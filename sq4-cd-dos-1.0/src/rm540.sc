...
(instance police of Sq4Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gNarrator say: 1) ; "You look briefly, careful not to attract any attention."
			)
			(2 ; Talk
				(= local70 1)
			)
...
(instance drTop of Sq4Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (IsFlag 7)
					(gNarrator say: 2) ; "Thanks to your janitorial ingenuity, the doorlock is now destroyed and you may enter and leave at will."
				else
					(gNarrator say: 3) ; "Your inate sense of curiosity makes you wish you knew the way to open this thing."
				)
			)
			(4 ; Do
				(HandsOff)
			...
			(13 ; jar
				(cond
					...
					((== ((Inv at: 5) cel:) 2) ; jar
						(gNarrator say: 4) ; "There is no reason to do that again."
					)
					(else
		...
		(switch (= state newState)
			...
			(2
				(if (IsFlag 7)
					...
					(gNarrator say: 6) ; "None of the codes you punch in seem to have any effect on the door."
					(HandsOn)
					(self dispose:)
				)
			)
...
(instance acidScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gNarrator say: 7) ; "Well, there's not much left of the lock now."
				(SetScore 7 10)
				((Inv at: 5) cel: 1 cursor: 952) ; jar
		...
		(switch (= state newState)
			...
			(6
				(if (& gMessageMode $0001)
					...
					(tROGJR2 modNum: 557 say: 18 self) ; "Dad?"
				)
			)
			(7
				(Face gEgo 115 130 self)
			)
			(8
				(gEgo moveHead: 0)
				(= seconds 1)
			...
			(9
				(if (& gMessageMode $0001)
					(tROGJR ; "I'm glad I got to see you, even if for only a few minutes. Xenon owes you a lot. Goodbye, Dad."
						modNum: 557
						say:
							19
							self
					...
					(tROGJR2 modNum: 557 say: 19 self) ; "I'm glad I got to see you, even if for only a few minutes. Xenon owes you a lot. Goodbye, Dad."
				)
				(= seconds 10)
			)
			(10
				(gEgo moveHead: 1)
				(Face gEgo 101 115 self)
			)
			(11
				(gEgo setLoop: 3)
				(= cycles 1)
			...
			(12
				(gEgo view: 558 setCel: 0 cycleSpeed: 12 setCycle: End self)
			)
			(13
				(gLongSong stop:)
				(= seconds 1)
...
(instance tunnel of Sq4Feature
	(properties
		...
		sightAngle 180
		onMeCheck 16
		lookStr 8 ; "This tunnel extends out over pipes and beams and toward the center of the structure."
	)
)
