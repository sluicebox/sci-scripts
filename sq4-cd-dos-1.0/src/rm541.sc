...
(instance rm541 of SQRoom
	...
	(method (newRoom newRoomNumber)
		(if (and (== newRoomNumber (gCurRoom south:)) (brain formatting:))
			(EgoDead 0 19) ; "As you leave the laser tunnel, you hear a voice announce that the formatting program is beginning, erasing all data in the super computer and dashing all hopes of regaining your son!"
		else
			(gLongSong fade:)
...
(instance hand of Sq4View
	...
	(method (doit &tmp temp0 temp1 temp2)
		(= temp0 (User curEvent:))
		(if (and (not (& signal $8000)) (not (& (temp0 type:) $0047))) ; direction | evMOUSEKEYBOARD | evMOUSERELEASE
			(GlobalToLocal temp0)
			(= temp1 (temp0 x:))
			(= temp2 (+ (temp0 y:) 10))
			(if
...
(instance keyPad of Sq4View
	...
	(method (doit &tmp temp0)
		...
		(repeat
			(= gLastEvent (= temp0 ((gUser curEvent:) new:)))
			(temp0 localize:)
			...
			(MapKeyToDir temp0)
			(cond
				((OneOf (temp0 type:) 1 256)
					(gMouseDownHandler handleEvent: temp0)
				)
				((& (temp0 type:) $0004)
...
(instance smokeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 515 setLoop: 2 cycleSpeed: 12 setCycle: End self)
			)
			(2
			...
			(7
				(gNarrator say: 3) ; "You cast the old stogie to the ground."
...
(instance roomFeature of Sq4Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (>= global185 4096)
					(gNarrator say: 4) ; "Thin cigar smoke fills the long tunnel, making the laser beams visable."
				else
					(gNarrator say: 5) ; "This long tunnel contains three circular steel portals, each containing nozzle-like projections."
				)
			)
			(else
				(super doVerb: theVerb &rest)
