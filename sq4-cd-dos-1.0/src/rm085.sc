...
(instance door of Sq4Prop
	(properties
		...
		view 85
		signal 26624
		lookStr 1 ; "This hatch, locked from the other side, was your portal to this area."
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(4 ; Do
				(gNarrator say: 2) ; "This hatch opens from the other side only."
			)
			(2 ; Talk
				(gNarrator say: 3) ; "Nobody can hear you through the thick door."
			)
			(else
...
(instance enterDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gNarrator say: 4 self) ; "Oh, no! The door has shut and there's no way back... What are you going to do now?"
			)
			(5
				(HandsOn)
				(self dispose:)
