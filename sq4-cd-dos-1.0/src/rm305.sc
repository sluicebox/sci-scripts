...
(instance pod of Sq4Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gNarrator say: 1) ; "Your high-tech but butt-ugly pod stands ready to ride the wild planes of time."
			)
			(4 ; Do
				(HandsOff)
			...
			(6 ; Smell
				(gNarrator say: 2) ; "The smells of stressed carbon fiber, cooling engines and ozone fill the air."
			)
			(7 ; Taste
				(gNarrator say: 3) ; "Tasting the cooling time pod would not be a good idea."
			)
			(else
...
(instance theStairs of Sq4Feature
	(properties
		...
		nsRight 320
		onMeCheck 16384
		lookStr 4 ; "The shale-like shelves provide a stairway to the unknown."
	)
)
...
(instance theRoom of Sq4Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(gNarrator say: 5) ; "You've seen a lot of plateaus and sloping hills before, but this one's a butte. The entire horizon undulates in enticing peaks, curves and crevices for which Estros is famous."
			)
			(else
				(super doVerb:)
			)
