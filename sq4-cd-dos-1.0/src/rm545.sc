...
(instance rm545 of SQRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gNarrator modNum: 545 say: 1) ; "The PocketPal's screen displays a wealth of almost useful information."
			)
			(else
	...
	(method (doit &tmp [temp0 40] temp40 temp41 temp42 temp43 temp44 temp45)
		...
		(if (and local1 (not (-- local1)))
			(EgoDead 8 23) ; "While you were busy staring blankly at your PocketPal screen, a droid snuck up behind you, tapped you on the shoulder. "Zap. You're it." (DO METALLIZE VOICE, ECHO, ETC.)"
		)
...
(instance powerButton of Sq4Feature
	(properties
		...
		nsBottom 140
		nsRight 47
		lookStr 2 ; "It's the PocketPal's power button."
	)
...
(instance keyboard of Sq4Feature
	(properties
		...
		nsRight 303
		sightAngle 90
		lookStr 3 ; "Hmm. The keys are just painted on."
	)

	(method (doVerb theVerb)
		(if (== theVerb 4) ; Do
			(gNarrator modNum: 545 say: 4) ; "They work as well as they look."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance dandyName of Sq4Feature
	(properties
		...
		nsBottom 167
		nsRight 95
		lookStr 5 ; "Yes, it's the PocketPal, from the people who brought you the CouchPotato universal remote control."
	)
)
...
(instance blip1 of Sq4Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Talk
				(gNarrator modNum: 545 say: 7) ; "Maybe if you wait a while, the nice droid will come around to where you are and talk to you. (EVIL SNICKER)"
			)
			(1 ; Look
				(if ((ScriptID 704 1) mover:) ; droid1
					(gNarrator modNum: 545 say: 7) ; "Maybe if you wait a while, the nice droid will come around to where you are and talk to you. (EVIL SNICKER)"
				else
					(gNarrator modNum: 545 say: 8) ; "It seems to have stopped, as if it were... waiting for something."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance vohaulScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (== gMessageMode 2)
					(tVOHAUL ; "Take a good look, Roger:"
			...
			(3
				(if (== gMessageMode 2)
					(tVOHAUL say: 2 self) ; "Remember this poor wretched soul..."
				else
					(localproc_0
						0
