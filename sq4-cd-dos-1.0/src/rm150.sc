...
(instance rm150 of SQRoom
	(properties
		picture 150
		south 505
		lookStr 1 ; "This must be Vohaul's chamber, where his dirtiest of deeds are set into motion. You can sense that this place is alive with raw energy."
	)
...
(instance rescueScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(tVOHAULJR modNum: 558 say: 1 self) ; "Hee, hee, hee! Hello again, Roger. It's me - your old friend Sludge Vohaul. I've taken the liberty of borrowing your son's body. I had to remove him first to make room for my mental self. His is on this disk. Say, it is most enjoyable to be in a young, healthy body - even if it is from YOUR blood line."
			)
			(9
				(tVOHAULJR modNum: 558 say: 2 self) ; "In fact, I like it so much I think I'll keep it. I guess we'll have no use for this anymore."
			)
			(10
			...
			(13)
			(14
				(tVOHAULJR modNum: 558 say: 3 self) ; "Now it's time to settle things once and for all. You'd better be careful, though. Keep this in mind - if I die while in this body, that disk will be useless. Your son will NEVER draw another breath and, if you don't defend yourself you will never live to buff another helmet!"
			)
			(15
				(tVOHAULJR modNum: 558 say: 4 self) ; "Come on - show me what you've got, mop jockey!"
			)
			(16
		...
		(switch (= state newState)
			(0
				...
				(if (OneOf ((Inv at: 14) owner:) gEgo gCurRoomNum diskDrive) ; diskette
					(gNarrator say: 2) ; "You don't really want to go down there AGAIN..."
					(= register 1)
...
(instance jrFreeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(tROGERJR modNum: 557 say: 1 self) ; "Follow me. There are many things we need to talk about."
			)
			(6
...
(instance diskDrive of Sq4Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(22 ; diskette
				(if (gCast contains: (ScriptID 152 0)) ; driveCloseUp
...
(instance ladder of Sq4Feature
	(properties
		...
		nsRight 100
		sightAngle 90
		lookStr 7 ; "The ladder leads down to the floor far below."
	)
...
(instance lOrb of Sq4Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if cycler
					(gNarrator say: 8) ; "The orbs crackle and sizzle with raw energy."
				else
					(gNarrator say: 9) ; "The orbs just seem to hover in mid air."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance overhang of Sq4Feature
	(properties
		lookStr 10 ; "This is the top half of an odd I/O unit. It also provides lighting for the area."
	)
...
(instance pedestal of Sq4Feature
	(properties
		lookStr 11 ; "The large pedestal standing in the center of this chamber seems to be some sort of huge I/O unit for the Super Computer."
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(7 ; Taste
				(gNarrator say: 12) ; "Even standing on your tiptoes, you'd never be able to reach that with your tongue."
			)
			(6 ; Smell
				(gNarrator say: 13) ; "It smells like a desperate, sweaty maintenance engineer has been walking around here."
			)
			(else
				(super doVerb: theVerb &rest)
			)
