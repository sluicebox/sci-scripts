...
(instance fightScript of Script
	...
	(method (dispose)
		...
		(super dispose:)
		(DisposeScript (ScriptID 151)) ; fightScript
	)
	...
	(method (changeState newState &tmp temp0 temp1 temp2)
		(switch (= state newState)
			(0
				(= local0 0)
				(gCurRoom notify: 0)
				(gGame setCursor: (ScriptID 0 19) 1) ; waitC
				...
				(if local1
					(if register
						(if local2
							...
							(= local2 1)
							(gNarrator modNum: 151 say: 1 self) ; "You were nearly thrown off the edge of the platform! Be more careful!"
						)
					else
						(= cycles 1)
					)
			...
			(8
				(if register
					...
					((ScriptID 150 1) setCycle: End self) ; RogerJr
				)
			)
			(9
				(if register
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(brain
					makePolygon: 127 101 121 103 112 96 111 91 111 87 113 80 115 2 169 2 162 81 160 88 158 95 149 100 142 102
				)
				(= cycles 6)
			)
			(2
			...
			(3
				(tVOHAULJR say: 1 self) ; "Is that the best effort you can muster? All this space hero nonsense must be getting to you. You're getting OLD, Roger! I, on the other hand, am enjoying the physical joys of youth."
			)
			(4
				(gEgo
					setCycle: Walk
