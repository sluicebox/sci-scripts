...
(instance rm400 of SQRoom
	...
	(method (init)
		...
		(if (or (== gPrevRoomNum east) (== gPrevRoomNum west))
	...
	(method (doit &tmp temp0 temp1 temp2)
		(cond
			...
			((& temp1 local295)
				(gCurRoom
					setScript:
						(if (& temp1 $0020) downLatorScript else joyRiding)
				)
			)
		)
	)
...
(instance skater0 of Sq4Prop
	(properties
		...
		priority 2
		signal 24592
		lookStr 1 ; "BlowBy foot thrusters boost the skaters along nicely."
	)
)
...
(instance lator1 of Sq4Prop
	(properties
		...
		priority 4
		signal 16400
		lookStr 2 ; "This escalator carries weary, destitute shoppers back to their vehicles in the parking garage below."
	)
)
...
(instance lator2 of Sq4Prop
	(properties
		...
		priority 4
		signal 16400
		lookStr 3 ; "This escalator carries excited, wealthy shoppers up from the parking garage below."
	)
)
...
(instance cigar of Sq4View
	(properties
		...
		loop 5
		signal 16400
		lookStr 4 ; "It's a cigar, but not much more than a butt."
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(6 ; Smell
				(gNarrator say: 5) ; "Geeze! You wonder what pachyderm-scented grease loaf contributed this used cigar to the decor."
			)
			(7 ; Taste
				(gNarrator say: 6) ; "You don't know this cigar's history. But, you better not try it."
			)
			(else
...
(instance card of Sq4Prop
	(properties
		...
		priority 1
		signal 18448
		lookStr 7 ; "It's an AutoBucks Teller Machine card."
	)
...
(instance getSomethingScript of Script
	...
	(method (doit)
		(super doit:)
		(if (& (gEgo signal:) $0400)
			(gNarrator say: 8) ; "This geek seems to have gotten in your way."
			(HandsOn)
			(self dispose:)
		)
	)
	...
	(method (changeState newState &tmp temp0 temp1 temp2)
		(switch (= state newState)
			...
			(1
				(switch register
					(card
						(gNarrator say: 9) ; "You pick up the ATM Card."
						(gEgo get: 10) ; atmCard
						(SetScore 80 2)
					)
					(cigar
						(gNarrator say: 10) ; "You retrieve the cigar butt."
						(gEgo get: 12) ; cigar
						(SetScore 81 5)
					)
				)
...
(instance joyRiding of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gNarrator say: 11) ; "Wasn't much fun, was it?"
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance fromCarScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(tBabes say: 1 self 2 64 190 100 27 1) ; "SALE!!"
			)
			(8
				(babe1
					setLoop: 3
...
(instance plugEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 8 25) ; "Boy, these guys pop up in the most inconvenient places! They seem to have this area secured. Your extra crispy parts surely provide no threat."
			)
		)
	)
)
...
(instance uStayIGo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(tSP2 ; "I shall pursue the Wilco unit.   Stay and guard the area."
					say:
						1
						self
						2
...
(instance downLatorScript of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(6
				(EgoDead 8 26) ; "Boy, isn't it just like a Sequel Police cyborg to guard the main entrances and exits?"
			)
		)
	)
)
...
(instance skateorama of Sq4Feature
	(properties
		...
		y 64
		z 24
		lookStr 12 ; "It's a Zero-G Skate-O-Rama. The skaters look to be having fun."
	)
...
(instance island1 of Sq4Feature
	(properties
		...
		nsBottom 130
		nsRight 82
		lookStr 14 ; "They're just islands between the beltways."
	)
)
...
(instance belts of Sq4Feature
	(properties
		sightAngle 180
		lookStr 15 ; "These convenient beltways move shoppers around the mall, allowing them to save energy to spend more money."
	)
...
(instance bush1 of Sq4Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(6 ; Smell
				(gNarrator modNum: 370 say: 13) ; "The fabricated flora has an equally artificial smell."
			)
			(7 ; Taste
				(gNarrator modNum: 370 say: 14) ; "You know that eating plastic plants places pressure on your plumbing. You learned that reading JANITORIAL DIGEST."
			)
			(else
				(super doVerb: theVerb &rest)
			)
