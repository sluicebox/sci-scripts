...
(class Feature of Obj
	...
	(method (approachVerbs args &tmp i theBits)
		(= _approachVerbs 0)
		(if (and argc global66 [args 0])
			(for ((= i 0)) (< i argc) ((++ i))
				(= theBits (global66 doit: [args i]))
			...
			(
				...
				(if
					(and
						...
						global66
						(& _approachVerbs (global66 doit: (event message:)))
					)
					(gEgo
						setMotion:
							PolyPath
	...
	(method (doVerb theVerb &tmp defaultCode who)
		...
		(if (and gMessageMode (Message msgGET modNum noun theVerb 0 1))
