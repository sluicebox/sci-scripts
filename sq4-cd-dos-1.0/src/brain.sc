...
(procedure (proc704_8 &tmp temp0)
	(switch (= temp0 (brain formatting:))
		...
		(2000
			(StrCpy @local212 {GOOD})
		)
		(1500
			(StrCpy @local212 {FAIR})
		)
		(1000
	...
	(if (and (< 0 temp0 3000) (not (mod temp0 250)))
		(proc704_6 (Format @local112 704 0 temp0 @local212)) ; "T minus %d - Safety margin: %s"
	...
	(if (and temp0 (!= gCurRoomNum 544) (!= (gCurRoom curPic:) 699))
		...
		(Format @local12 704 1 temp0) ; "%d"
		(if local11
			(proc0_12 local11)
		)
		(if (== gNewRoomNum gCurRoomNum)
			(= local11 (proc0_12 @local12 64 90 180 67 25 25 local10))
		)
	)
)
...
(procedure (localproc_3 param1 &tmp [temp0 40])
	(switch (localproc_2 param1)
		(0
			(gNarrator modNum: 704 say: 1) ; "You hear an electronic hum approaching from ahead of you."
		)
		(1
			(gNarrator modNum: 704 say: 2) ; "You hear an electronic hum approaching from behind you."
		)
		(3
			(gNarrator modNum: 704 say: 3) ; "You hear an electronic hum approaching from your left."
		)
		(2
			(gNarrator modNum: 704 say: 4) ; "You hear an electronic hum approaching from your right."
		)
	)
...
(class brain of Rgn
	...
	(method (init &tmp temp0 temp1 temp2)
		...
		(cond
			...
			((!= (gLongSong number:) temp2)
				(gLongSong number: temp2 vol: 127 loop: -1 playBed:)
			)
		)
		(if (= temp1 (localproc_1 droid1))
			(localproc_3 temp1)
		)
		(if (= temp1 (localproc_1 droid2))
			(localproc_3 temp1)
		)
	)
	...
	(method (doit &tmp temp0)
		(super doit:)
		(if formatting
			(if (< (Abs (- gGameTime local523)) gGameSpeed)
				0
			else
				(= local523 gGameTime)
				(if (and local12 (== (gCurRoom curPic:) 699))
					(= local12 0)
				)
				(proc704_8)
				(if (not (-- formatting))
...
(class SecurityDroid of Sq4Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gNarrator modNum: 704 say: 5) ; "It's a security droid!"
			)
			(else
...
(instance droidPath1 of RegionPath
	...
	(method (next)
		(if (and (OneOf value 71 205) (!= gCurRoomNum 515))
			(+= value 2)
			(client posn: [local252 (++ value)] [local252 (++ value)])
		)
		(super next:)
	)
...
(instance terminal1 of Sq4Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= (brain level:) 1)
			(gNarrator modNum: 704 say: 6) ; "You can't reach it from this level."
		else
			(switch theVerb
				(1 ; Look
...
(instance pocketPalPlug of Sq4Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gNarrator modNum: 704 say: 7) ; "Slot A (Insert terminal plug here)"
			)
			(else
				(super doVerb: theVerb &rest)
			)
				...
				(if register
					(cond
						((not (== ((gInventory at: 11) owner:) 1)) ; plug
							(gNarrator modNum: 704 say: 8) ; "The PocketPal doesn't plug in directly. Maybe you need some kind of adapter."
						)
						((not (== ((gInventory at: 4) owner:) 1)) ; battery
							(gNarrator modNum: 704 say: 9) ; "You turn the power on, but nothing happens."
						)
						((!= ((gInventory at: 11) state:) global170) ; plug
							(gNarrator modNum: 704 say: 10) ; "The adapter plug doesn't seem to fit into Slot A of the interface."
						)
						(else
