...
(instance rm611 of SQRoom
	...
	(method (init)
		...
		(building2 init:)
		(graffiti init:)
		(if (and (== (ulence status:) 1) (> (Random 0 100) 20))
...
(instance graffiti of Sq4Feature
	(properties
		...
		nsRight 60
		sightAngle 45
		lookStr 2 ; "You don't know what it says but you're sure it's not flattering."
	)
)
...
(instance building1 of Sq4Feature
	(properties
		...
		sightAngle 45
		onMeCheck 2
		lookStr 3 ; "This must be the back of the bar. Some fading graffiti adorns the wall."
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(6 ; Smell
				(gNarrator say: 4) ; "Back here, the bar smells like everything they didn't want stinking up the front."
			)
			(7 ; Taste
				(gNarrator say: 5) ; "An interesting mix of flavors! Stale Keronian ale, regurgitated Keronian ale, and Keronian ale that's otherwise been used and disposed of."
			)
			(else
...
(instance building2 of Sq4Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(1 ; Look
				(gNarrator say: 6) ; "You see the Droids-B-Us store in the distance."
			)
			(6 ; Smell
				(gNarrator say: 7) ; "From here, you can't really smell the Droids-B-Us building."
			)
			(7 ; Taste
				(gNarrator say: 8) ; "You'll have to get closer if you want to lick the Droids-B-Us building."
			)
			(else
				(super doVerb: theVerb &rest)
			)
