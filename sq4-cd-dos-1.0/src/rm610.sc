	...
	(method (doit)
		...
		(cond
			...
			((gEgo inRect: 192 122 221 137)
				(if (not (== (gCurRoom script:) enterBar))
...
(instance runOverScript3 of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(3
				((ScriptID 706 1) setPri: -1 setMotion: MoveTo 33 91 self) ; theBiker
			)
			(4
				((ScriptID 706 1) hide:) ; theBiker
				(gLongSong2 fade: 0 10 10 0)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			(0
				(HandsOff)
				(ulence status: 99)
				(LoadMany rsVIEW 615 616 632 633 634 50)
				(SetScore 75 5)
...
(instance theSign of Sq4Prop
	(properties
		...
		priority 12
		signal 24592
		lookStr 1 ; "The sign suggests that this place might be a bar."
	)
)
...
(instance bike1 of Sq4Prop
	(properties
		...
		priority 12
		signal 16401
		lookStr 2 ; "As you check out the sand bikes, you wonder what kind of cheeseball would own one."
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(1 ; Look
				(gNarrator say: 2) ; "As you check out the sand bikes, you wonder what kind of cheeseball would own one."
			)
			(4 ; Do
				(cond
					((not (IsFlag 15))
						(gNarrator say: 3) ; "Why would you want to wreck the nice bike?"
					)
					((not (IsFlag 75))
...
(instance ship1 of Sq4Feature
	(properties
		...
		sightAngle 45
		onMeCheck 16
		lookStr 4 ; "This long-legged beauty stands silently waiting for its jet-faced owner to return."
	)
)
...
(instance ship3 of Sq4Feature
	(properties
		...
		sightAngle 45
		onMeCheck 8
		lookStr 5 ; "It looks like it came from the Planet of the Warts! This ship must be the Plantars Deluxe model with authentic Mosaic upholstery."
	)
)
...
(instance door of Sq4Feature
	(properties
		...
		sightAngle 45
		onMeCheck 4
		lookStr 6 ; "This must be the entrance to this building."
	)

	(method (doVerb theVerb invItem)
		(switch theVerb
			(1 ; Look
				(gNarrator say: 6) ; "This must be the entrance to this building."
			)
			(4 ; Do
				(if (not (== (gCurRoom script:) enterBar))
					(gCurRoom setScript: enterBar)
