					...
					(cond
						...
						((TrySkill 13 60) ; communication
							...
							(Say ; "Whew! Smooth talking, Dude. Ya got the gift for gab, Tab."
								...
								250
								27
								(Format @local28 250 28 theBreak) ; "Tell ya what I'm going ta do. Seein' as you beein' a Heero and all, I'll reduce the price by %d whole simoleons."
								250
								29
							)
							(-= local129 theBreak)
...
(instance keapon of Talker
	...
	(method (showText what)
		(cond
			((== what 72)
				(if gClumpsLeft
					(Say self msgFile 72 250 47) ; "It's used to make dark stains on light clothes. If you need it to ground old Blowhard, it's yours for a song. Dust in the Wind. All you need is dust in the wind."
				else
...
(instance booksLeft of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(HighPrint (Format @local28 250 49 [local251 local262])) ; "There seem to be books in many languages, few of which are comprehensible to you, but you do recognize some old favorites, such as %s"
			(if (> (++ local262) 10)
				(= local262 0)
			)
		else
