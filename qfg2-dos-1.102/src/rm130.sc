...
(instance rm130 of Stage
	...
	(method (init)
		...
		(gCSound number: 130 priority: 0 loop: -1 playBed:)
	)
				...
				(cond
					((and firstTime (<= 4 gDay 6))
						(if (< gElementalState 2)
							...
							(Say harikHead self 130 52) ; "I have heard that it was you that vanquished the Fire Elemental. How woundrously brave you must be."
						)
					)
					((and firstTime (OneOf gDay 12 13) (== gElementalState 5))
						(Say harikHead self 130 53) ; "I have heard that it was you that vanquished the Elemental of Air as well as of Flame. Perhaps you now need to know something about the Element of Earth."
					)
					((and firstTime (OneOf gDay 15 16))
						(Say harikHead self 130 54) ; "I have heard a rumor that you are taking a caravan to Raseir. It would be best if you purchased some extra pills to take with you."
					)
					(else
						(switch gHarikVisits
							(1
								(Say harikHead self 130 55) ; "This is the Apothecary, where one can buy many wondrous pills and oils. I am Harik. If you need something, just let me know."
							)
							(else
								(Say harikHead self 130 56) ; "Welcome back. If you see something which interests you, let me know."
							)
						)
					)
				)
...
(instance showFlame of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say harikHead self 130 57) ; "Watch this."
			)
			(2
				(leftBottle show:)
				(rightBottle show:)
			...
			(4
				(harik loop: 5 cel: 0)
				(gSpareSound number: 561 priority: 10 loop: 1 play:)
			...
			(12
				...
				(switch askedAbout
					(1
						(Say harikHead self 130 58) ; "See how the flame reacts to incense? Fire is attracted to the things it burns because it is very vain."
					)
					(2
						(Say harikHead self 130 59) ; "See how the earth is consumed by the flames! Pretty amazing, isn't it? I'm sure there's a lesson to be learned from this, although I don't quite know what that might be."
					)
				)
			)
			(13
			...
			(14
				(switch askedAbout
					(1
						(cond
							((== gElementalState 1)
								(Say harikHead self 130 60) ; "It would be very possible to lure the Fire Elemental away from the plazas using this incense, and where it is weak from hunger and water, capture it."
							)
							((< gElementalState 1)
								(Say harikHead self 130 61) ; "You know, incense would make quite a lure for a Fire Elemental."
							)
							(else
								(Say harikHead self 130 62) ; "As you have discovered, incense is quite a lure for Fire Elementals."
							)
						)
					)
					(2
			...
			(16
				(Say harikHead self 130 64) ; "Now I must get back to my work. If you see something which interests you, let me know."
			)
			(17
	...
	(method (changeState newState &tmp howMany)
		(switch (= state newState)
			...
			(5
				(switch doing
					(0
						(Format @str 130 65 (/ pillCost 100)) ; "The pills you request are a mere %d dinars."
						(Say harikHead payS @str)
						(= doses 3)
						(client setScript: payS)
					)
					(1
						(Say harikHead payS 130 66) ; "Because you are such a fine person, for you I will make a special deal. I will sell you three doses of pills at regular price, and throw in a fourth dose free of charge."
						(= doses 4)
						(client setScript: payS)
					)
					(2
						(Say harikHead payS 130 67) ; "The incense will cost you but the price of my ingredients, a mere 70 centimes."
						(client setScript: payS)
					)
					(3
						(if (<= 3 gDay 6)
							(Say harikHead payS 130 68) ; "Very well. If it will save the city, you may take the incense."
						else
							(Say harikHead payS 130 69) ; "Very well. You may take the incense."
						)
						(= incenseCost 0)
						(client setScript: payS)
					)
					(4
						(Say harikHead payS 130 70) ; "The oil will cost you 20 centimes."
						(client setScript: payS)
					)
					(5
						(Say harikHead payS 130 71) ; "You speak as one born to bargain. It shall be 15 centimes."
						(= oilCost 15)
						(client setScript: payS)
					)
					(6
					...
					(7
						(Say harikHead 130 72) ; "I still need the "Fruit of Compassion." You need to add the victim's hair at the last moment."
					)
					(8
						(Say harikHead 130 73) ; "I still need the feather of the Griffin. You need to add the victim's hair at the last moment."
					)
					(9
						(= howMany [gInvNum 29]) ; ScorpionTail
						(cond
							((>= gNumScorpTails 5)
								(Say harikHead 130 74) ; "I am in awe of your bravery that you have defeated so many of theses fearful monsters, but I am presently overstocked with scorpion venom, and I can purchase no more."
							)
							((== gNumScorpTails 4)
								(Say harikHead putMoney 130 75) ; "My, you are the brave and fortunate one! I am now almost completely stocked with scorpion venom. I will purchase one more scorpion tail."
							...
							(else
								(Say
									harikHead
									putMoney
									(Format @str 130 76 (* howMany 20)) ; "Ah, wonderful. I was almost out of Poison Cure Pills. Now I can make up some more. Here is your reward of %d dinars."
								)
					...
					(10
						...
						(cond
							((>= gNumGhoulClaws 10)
								(Say harikHead 130 77) ; "My spine shivers at the thought of so much battle with the fiendish Ghouls, but despite your bravery, I regret that my larder of Ghoul claws is full, and I can purchase no more."
							)
							((>= gNumGhoulClaws 7)
								(Say harikHead putMoney 130 78) ; "Your valor in the face of the dreaded Ghouls is impressive. My supply of claws is almost complete, but I will yet purchase a few more."
								(if (> howMany 2)
									(= howMany 3)
								)
							...
							(else
								(Say
									harikHead
									putMoney
									(Format @str 130 79 (* howMany 15)) ; "Now I can continue with my experiments. A thousand thanks for these, and may this reward of %d dinars gladden your heart."
								)
								(gEgo put: 30 howMany) ; GhoulClaw
								(gEgo get: 2 (* 15 howMany)) ; Dinar
								(+= gNumGhoulClaws howMany)
					...
					(11
						(HighPrint 130 80) ; "You take an oil-filled lamp."
						(SolvePuzzle 624 7)
		...
		(switch (= state newState)
			(1
				...
				(switch (Buy item cost qty moreThanOne howMany quiet)
					(1
						(Say harikHead self 130 81) ; "I'm sorry, but I can only accept the coins of Shapeir. You will need to first see the Money Changer."
					)
					(0
						(Say harikHead self 130 82) ; "I'm afraid you do not have enough money for the purchase."
					)
					(else
						(cond
							((or (== doing 0) (== doing 1))
								...
								(Say harikHead 130 83) ; "There is a special money-back guarantee with every pill I sell. If you die because of a pill, I will gladly refund your money."
							)
							((or (== doing 2) (== doing 3))
								(SolvePuzzle 607 7)
							)
		...
		(switch (= state newState)
			(0
				(cond
					...
					(else
						(HighPrint 130 90) ; "You give the Fruit of Compassion and the Griffin Feather to Harik."
						(gEgo put: 28) ; Feather
						(gEgo put: 36) ; CompassionFruit
...
(instance makeDispelS of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say harikHead self 130 91) ; "This will only take me a second."
			)
			(1
				(= seconds 3)
			)
			...
			(3
				(harik setScript: 0 loop: 2 cel: 5)
				(Say harikHead self 130 92 130 93 130 94 130 95) ; "There. I do for you what I would not for another. This is a gift for all you have done for the city."
			)
			(4
				(HighPrint 130 96) ; "You take the three potion bottles and put them away."
				(SolvePuzzle 608 7)
				(gEgo get: 21 3) ; DispelPotion
				(Say harikHead self 130 97) ; "Now, please excuse me. I really must get back to my work."
			)
			(5
				(harik setScript: mixingS)
				(self dispose:)
			...
			(switch what
				...
				(-5
					(cond
						...
						(else
							(Say harikHead 130 101) ; "If you can but get me the Griffin's feather and the "Fruit of Compassion", I will give you the potion."
						)
					)
				)
				(54
				...
				(52
					(if (IsFlag 606)
						...
						(Say self 130 102) ; "I'm not sure what you wish to capture."
					)
				)
				(28
					(if (IsFlag 606)
...
(instance scale of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1
				(HighPrint 130 103) ; "It's a scale, without which Harik would be unbalanced."
			)
			(7
				(HighPrint 130 104) ; "You can't get a weigh with that. You'd get pounded."
			)
			(else
...
(instance bookshelf of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(HighPrint (Format @str 130 105 [bookMsg msgCounter])) ; "There seem to be books in many languages, few of which are comprehensible to you, but you do recognize some old favorites, such as %s"
			(if (> (++ msgCounter) 10)
				(= msgCounter 0)
			)
		else
...
(instance shelf of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(HighPrint (Format @str 130 106 [shelfMsg msgCounter])) ; "The shelves are filled with unusual objects. You see something that looks like %s."
			(if (> (++ msgCounter) 10)
				(= msgCounter 0)
			)
		else
