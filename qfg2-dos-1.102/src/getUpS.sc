...
(instance doorR of TargActor
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'grease/hinge,door,armoire') (Said 'use/grease'))
				(cond
					((< (gEgo x:) 70)
						(HighPrint 261 5) ; "You need to get closer to the wardrobe first."
					)
					((not [gInvNum 42]) ; Oil
						(HighPrint 261 6) ; "You don't have any oil."
					)
					(else
						(doorR
							lookStr:
			...
			((Said 'look,look/hinge')
				...
				(cond
					((not (and (< 131 (gEgo x:) 196) (< 124 (gEgo x:))))
						(HighPrint 261 7) ; "Get closer to the wardrobe."
					)
					((IsFlag 60) ; fHingesOiled
						(gEgo loop: 3)
						(RedrawCast)
						(HighPrint 261 8) ; "The hinges look well oiled."
					)
					(else
						(gEgo loop: 3)
						(RedrawCast)
						(HighPrint 261 9) ; "The hinges look very rusty. Someone should oil them."
					)
				)
			)
			((Said 'search/armoire')
				(cond
					(
						...
						(if (IsFlag 58) ; fStoleBowl
							(HighPrint 261 10) ; "You find nothing of value."
						else
							(HighPrint 261 11) ; "You find an emerald-studded silver bowl and stash it away."
							(SolvePuzzle 703 3 2)
					...
					((and (< 146 (gEgo x:) 180) (< (gEgo y:) 124))
						(HighPrint 261 12) ; "The wardrobe's door isn't open."
					)
					(else
						(NotClose)
					)
			...
			(
				...
				(cond
					((== cel 0)
						(HighPrint 261 13) ; "The wardrobe's doors are already closed."
					)
					((gEgo script:)
						(HighPrint 261 14) ; "You're too busy right now."
					)
					(else
	...
	(method (hurtMe)
		(EgoDead 1 261 15 #title {No Stealing Aloud!}) ; "As it strikes the wardrobe, the sound alerts the old man. He calls out for his sons. They are less than gentle with you."
	)
