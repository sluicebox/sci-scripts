...
(local
	...
	local15
	local16
	[local17 4] = [1680 0 1 0]
	[local21 5] = [1680 24 1 2 0]
	[local26 4] = [1680 27 1 0]
	[local30 4] = [1680 28 1 0]
	[local34 4] = [1680 29 1 0]
	[local38 4] = [1680 31 1 0]
	[local42 4] = [1680 32 1 0]
	[local46 5] = [1680 34 1 2 0]
	[local51 4] = [1680 38 1 0]
	[local55 4] = [1680 39 1 0]
	[local59 4] = [1680 41 1 0]
	[local63 4] = [1680 42 1 0]
	[local67 4] = [1680 43 1 0]
	[local71 6] = [1680 44 2 1 2 0]
	[local77 4] = [1680 47 2 0]
	[local81 4] = [1680 49 2 0]
	[local85 4] = [1680 30 1 0]
	local89
)
...
(instance rm680 of Rm
	...
	(method (init)
		...
		(gCurRoom
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: 12 31 319 55 318 147 276 146 228 153 166 127 121 135 84 123 73 133 68 139 47 152 14 155 106 189 0 179
					yourself:
				)
		)
	...
	(method (doit)
		(++ local9)
		(if (and (> (gGame detailLevel:) 2) (== (mod local9 5) 0))
		...
		(cond
			...
			((and (not local89) (or (Monastery waterDie:) (== global115 2)))
				(= local89 1)
				local16
				(theBoat setMotion: 0)
				(gEgo setMotion: 0)
			...
			((and (& (gEgo onControl: 1) $0002) (not local12))
	...
	(method (handleEvent event)
		(if
			...
			(cond
				((and (ineptus onMe: event) (== local10 2))
					(event claimed: 1)
					(= local10 3)
					(gEgo setScript: raiseTheGrate)
				)
				((and (ieiunus onMe: event) (== local10 1))
					(event claimed: 1)
					(= local10 2)
					(gCurRoom setScript: (ScriptID 681 1)) ; youGotIeiunus
				)
				((and (cogito onMe: event) (== local10 0))
					(event claimed: 1)
					(= local10 1)
...
(instance fulk of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Converse @local26 (Monastery tFULK:) 0 self) ; "Even though this boat terrifies me, I'll not leave it until we reach the shore of these fens."
			)
			(5 ; Talk
				(Converse @local30 (Monastery tFULK:) 0 self) ; "Please, let's leave at once! It's bad enough I must get in this flimsy boat."
			)
			(4 ; Inventory
				(switch invItem
					(0 ; bucks
					...
					(2 ; halfHeart
						(Converse @local85 (Monastery tFULK:) 0 self) ; "You are too generous. I could not take such a gift."
					)
					(14 ; amethyst
						(Converse @local38 (Monastery tFULK:) 0 self) ; "I am right fond of amethyst. Thank you."
						(gEgo put: 14) ; amethyst
					)
					(17 ; handScroll
						(Converse @local42 (Monastery tFULK:) 0 self) ; "You risked too much for that scroll to give it to me."
					)
				)
			)
		)
...
(instance stepInBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse @local51 (Monastery tFULK:) 0 self) ; "Please let's go at once. As soon as they discover I'm gone, they'll come looking for us."
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(if (== local13 1)
					(= local13 0)
					(Converse @local55 8 0 self) ; "For a famous outlaw, you're rather indecisive."
				)
...
(instance moveAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Converse @local59 (Monastery tFULK:) 0) ; "There's only a wall here. The water goes no further. Please, return to the gate and let's leave at once!"
				(gEgo
					cel: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: 0)
				(Converse @local63 (Monastery tFULK:) 0 self) ; "There's no safety anywhere else! We must escape through this grate or they'll kill us."
			)
			(1
				(gEgo
					cel: 0
...
(instance monkKillDock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse @local67 11 self) ; "I thought I might find you here!"
			)
			(3
...
(instance monkKillBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local67 11 8 self) ; "I thought I might find you here!"
			)
			(2
				(killMonk view: 585 setLoop: 3 setCel: 0 setCycle: End self)
			)
...
(instance priorKillBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local71 8 (Monastery tPRIOR:) self) ; "As I suspected, you've trapped yourself. Will you surrender or--"
			)
			(2
				(thePrior setHeading: 310 self)
			)
			(3
				(Converse @local77 8 (Monastery tPRIOR:) self) ; "Vocalis, speak!"
			)
			(4
				(SetMessageColor 32)
			...
			(5
				(Converse @local81 8 (Monastery tPRIOR:) self) ; "Call death down upon the head of yonder sinner."
			)
			(6
...
(instance askTheDwarf of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local46 8 (Monastery tFULK:) self) ; "Fulk, how did they raise the grate?"
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance lookAtFulk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local21 8 (Monastery tFULK:) self) ; "What troubles you so, my friend? You were so brave before."
			)
			(2
				(HandsOn)
				(self dispose:)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(SetScore 25 96)
				(Converse @local34 (Monastery tFULK:) 0 self) ; "You are too kind. Thank you, but please, let us leave here!"
			)
			(1
				(HandsOn)
				(self dispose:)
