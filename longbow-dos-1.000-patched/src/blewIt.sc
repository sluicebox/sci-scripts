...
(local
	local0
	local1
	[local2 4] = [1601 94 1 0]
	[local6 5] = [1601 75 1 1 0]
	[local11 5] = [1601 97 1 0 0]
	[local16 5] = [1601 86 1 2 0]
)
...
(instance knockItOff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if register
					(Converse 1 @local11 11 0 self) ; "I can't abide clodpates."
				else
					(Converse 1 @local2 11 0 self) ; "Some people never learn."
				)
			)
			(6
				(EgoDead)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			(0
				...
				(if (== global136 2)
					(Converse 2 @local16 8 0 11 3 self) ; "Was that an insult?"
				else
					(self cue:)
				)
			)
			...
			(2
				(if (== global115 2)
					(Converse 1 @local6 11 0 self) ; "That's a fine ruby you wear upon your hand, Brother."
				else
					(self cue:)
				)
			)
