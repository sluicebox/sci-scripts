...
(instance RoomScript of Script
		...
		(cond
			((Said 'boo')
				(if comicOnStage
					...
					(Printf 340 2 gExpletive) ; "Oh, yeah," retorts the comic, "%s"
			...
			((and gDebugging (Said 'test/joke'))
				(= testCount (GetNumber {First joke (from 1 to LAST JOKE):}))
				(Printf 340 6 testCount) ; "Ok. Next joke will be #%d."
			)
			((Said 'talk/comedian')
				(if comicOnStage
			...
			((Said 'talk/man,bill,woman,al')
				(if
					...
					(Printf 340 12 gEgoName) ; "These people are here to hear the comedian, not you. Find a seat, %s, and enjoy yourself."
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(Printf 340 40 @string) ; "Hey! I've got an idea. How about if you and I sit in a comedy club and we make Larry walk up to us and say something like, `%s'"
			...
			(10
				(Printf 340 41 @string) ; "`%s', eh?" says Bill, "Are you crazy? No way. That's so lame! Larry would never say that!"
				(= seconds 3)
			)
			(11
				(aBillTop hide:)
			...
			(13
				(Printf 340 42 @string) ; "Yeah, I suppose you're right," says Al. "This is not a good idea. Let's get out of here! `%s' Really!!"
				(aAl dispose:)
				(aBill dispose:)
...
(instance ComicScript of Script
	...
	(method (changeState newState &tmp [jokeString 200] [jokeArgs 4] count line i j k)
		(switch (= state newState)
			...
			(13
				(gMusic fade:)
				(Print 340 52) ; "And now," says the comic, "because you've been so patient, I'd like to do my famous impersonation of a duck!"
				(aComic setMotion: MoveTo 158 75 self)
			)
			(14
				(Print 340 53) ; "And it goes something like this..."
				(if (not (TestFlag 43))
			...
			(21
				(Print 340 54) ; "Ladies and gentlemen, Paul Paul!"
			...
			(23
				(Print 340 55) ; "Look! He's getting a sitting ovation!"
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(switch (Random 1 3)
					(1
						(= currentDrinker aLadyLR_Top)
					)
					(2
						(= currentDrinker aLadyUL_Top)
					)
					(3
						(= currentDrinker aManUL_Top)
					)
				)
				(currentDrinker setCycle: End self)
			)
			(2
				(if (== currentDrinker aLadyUL_Top)
					(= state -1)
				)
				(= cycles (Random 5 22))
			)
