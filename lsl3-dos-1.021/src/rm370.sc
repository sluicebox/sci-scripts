		...
		(cond
			...
			(
				...
				(cond
					...
					((TestFlag 51)
						(Print 370 26) ; "Your locker is empty! You didn't leave it open when you left before, did you?"
					)
					(else
						(Format @string 370 27) ; "You return to your leisure suit, once again ready to face the world of women."
						(= gEgoState 7)
						(self changeState: 1)
					)
				)
			...
			((Said 'wear,get,(alter<in),(drop<on)/towel')
				(cond
					((== gEgoState 8)
						(Print 370 28) ; "You're already wearing the towel."
					)
					((not (gEgo has: 8)) ; Beach_Towel
						(DontHave) ; "You don't have it."
					)
					((not (& (gEgo onControl:) $0040))
						(Print 370 29) ; "You would, but you really don't want to!"
					)
					((not (< (aLocker y:) 999))
						(Print 370 21) ; "Your locker is still closed."
					)
					...
					(else
						(Format @string 370 30) ; "You carefully tie the beach towel around your waist, leaving your leisure suit in the locker with the sweatsuit."
						(= gEgoState 8)
						(self changeState: 1)
					)
				)
			...
			(
				...
				(cond
					((== gEgoState 9)
						(Print 370 31) ; "You're already wearing the sweatsuit."
					)
					((TestFlag 50)
						(Print 370 32) ; "You can't change into the sweatsuit with water all over your body!"
					)
					((not (& (gEgo onControl:) $0040))
						(Print 370 25) ; "You can't reach your clothes from here!"
					)
					...
					(else
						(Format @string 370 33) ; "Gosh! Don't you look sweet in a pair of (newly-larger) women's sweats!"
						(= gEgoState 9)
						(if (not (TestFlag 41))
							(SetFlag 41)
							(gGame changeScore: 4)
			...
			(
				...
				(cond
					(
						...
						(Print 370 34) ; "You're freezing; not even close."
					)
					((> howFar 80)
						(Print 370 35) ; "You're cold."
					)
					((& (gEgo onControl:) $0200)
						(Print 370 36) ; "You're warm."
					)
					((& (gEgo onControl:) $0100)
						(Print 370 37) ; "You're getting warmer."
					)
					((& (gEgo onControl:) $0080)
						(Print 370 38) ; "You're hot; very hot! You're burning up!!"
					)
					((& (gEgo onControl:) $0040)
						(Print 370 39) ; "HEY! This is it! You've found good ol' number 69!"
					)
					(else
						(Print 370 40) ; "It must be in here somewhere."
					)
				)
			)
			((Said 'pick/bolt,locker,69')
				(Print 370 41) ; "You feel fortunate when you can successfully pick your nose!"
			)
			((Said 'caress/locker,top,bay')
				(Print 370 42) ; "It feels rusty."
			)
			((Said '(look<in),explore,(look<in)/locker,(door<locker)')
				(cond
					((not (& (gEgo onControl:) $0040))
						(Print 370 43) ; "It's difficult to see through those narrow slots."
					)
					((not (< (aLocker y:) 999))
						(Print 370 44) ; "It's difficult to see through the narrow slots of your closed locker door."
					)
					((TestFlag 51)
						(Print 370 45) ; "It's empty!"
						(Print 370 46) ; "Where's your stuff?"
						(Print 370 47) ; "You didn't leave the area without locking your locker, did you?"
					)
					((== gEgoState 7)
						(Print 370 48) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, and a set of woman's sweats."
					)
					((== gEgoState 8)
						(Print 370 49) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, your leisure suit, and a set of woman's sweats."
					)
					((== gEgoState 9)
						(Print 370 50) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, and your leisure suit."
					)
					((or (== gEgoState 5) (== gEgoState 6))
						(Print 370 49) ; "There's a photo of a scantily-clad Tom Selleck, some spray deodorant, your leisure suit, and a set of woman's sweats."
					)
			...
			((Said 'get,spray,wear,use/can,spray,deodorant')
				(cond
					((not (& (gEgo onControl:) $0040))
						(Print 370 51) ; "You can't reach the deodorant from here."
					)
					((not (< (aLocker y:) 999))
						(Print 370 21) ; "Your locker is still closed."
					)
					((or (== gEgoState 5) (== gEgoState 6) (== gEgoState 8))
						(Print 370 52) ; "Pssssssst."
						(Print 370 52) ; "Pssssssst."
						(ClearFlag 62)
						(if (not (TestFlag 60))
							(SetFlag 60)
						...
						(Print 370 53) ; "You spray on some deodorant, then return the can to the locker."
					)
					(else
						(Print 370 54 #at -1 144) ; "(Does it really help to spray that stuff on your clothes?)"
						(gGame changeScore: -1)
					)
				)
			)
			(
				...
				(cond
					((not (gEgo has: 8)) ; Beach_Towel
						(Print 370 55) ; "You have no towel."
					)
					((and (!= gEgoState 8) (!= gEgoState 6) (!= gEgoState 5))
						(Print 370 56) ; "Now?"
					)
					((not (TestFlag 50))
						(Print 370 57) ; "You're not wet."
					)
					((and (!= gEgoState 8) (not (< (aLocker y:) 999)))
						(Print 370 58) ; "Didn't you leave your towel inside the locker?"
					)
					(else
						(ClearFlag 50)
						(if (not (TestFlag 39))
						...
						(Print 370 59 #icon 8 0 0) ; "Good idea. You carefully dry every tiny pore to prevent chafing!"
					)
				)
			)
			((Said 'talk')
				(Print 370 60) ; "You are all alone in the locker room, but you can hear severe grunting coming from the weight room."
			)
			(
				(or
					(Said '/combination')
				...
				(Print 370 61) ; "Yes. That's the puzzle, isn't it?!"
			)
			((Said 'look>')
				(cond
					((Said '/man')
						(Print 370 62) ; "Looking around, you see some naked men."
					)
					((Said '/door,door')
						(cond
							((& (gEgo onControl:) $0002)
								(Print 370 63) ; "You peer through the doorway and see lots of naked men!"
							)
							((& (gEgo onControl:) $0008)
								(Print 370 64) ; "You look through the doorway and see lots of men and women exercising on weight machines."
								(Print 370 65) ; "They are all wearing sweatsuits..."
								(Print 370 66) ; "...sweaty sweatsuits!"
							)
							(else
								(Print 370 67) ; "The left door on the far wall leads to the showers."
								(Print 370 68) ; "The right door leads to the weight room."
							)
						)
					)
					((and (< (aLocker y:) 999) (Said '/art'))
						(Print 370 69) ; "In your opinion, he looks a lot like you."
					)
					((Said '/number')
						(if
							(or
							...
							(Print 370 70) ; "Hey! There's locker #69 over there!!"
						else
							(Printf 370 71 (Random 1 999)) ; "The locker nearest you might be #%d."
							(Print 370 72 #at -1 144) ; "(Then again, it might not.)"
						)
					)
					((Said '/69,(locker<69)')
						(Print 370 73) ; "It's over there in that skinny little corner of the lockers. You can just walk to it from here."
					)
					((Said '/locker<my')
						(Print 370 61) ; "Yes. That's the puzzle, isn't it?!"
					)
					((Said '/locker,bay')
						(Print 370 74) ; "The lockers are made of steel, painted in a lovely cyan hue."
					)
					((Said '/sweatpants')
						(if (!= gEgoState 9)
							(DontHave) ; "You don't have it."
						else
							(Print 370 75) ; "These sweats are so bulky, you can't tell what shape your body's in!"
						)
					)
					((Said '/deodorant,can,spray')
						(Print 370 76) ; "Perhaps you should use it!"
					)
					((Said '[/area]')
						(Print 370 77) ; "You are in the "Fat City" spa's locker room. On the far wall are two doorways. There is another door that leads to the lobby. Identical lockers are everywhere. Which one should you use?"
					)
				)
			)
			((or (Said '/69') (Said '//69'))
				(Print 370 78) ; "There you go again!"
			)
		)
	)
