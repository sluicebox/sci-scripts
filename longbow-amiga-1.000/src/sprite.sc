...
(local
	...
	local9
	local10
	[local11 5] = [1185 4 1 2 0]
	[local16 6] = [1185 6 1 2 2 0]
	[local22 5] = [1185 9 1 2 0]
	[local27 5] = [1185 15 1 1 0]
	[local32 9] = [186 0 2 4 22 24 19 16 8]
	[local41 8] = [-1 0 0 0 0 0 0 0]
)
...
(instance funnySprite of PicView ; UNUSED
	(properties)
...
(instance spriteCaught of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				3
				(Converse @local11 3 @local32 self) ; "At last! Pixie, grant my wish. I wish--."
			)
			(4
				(Converse @local16 2 @local32 self) ; "What do you mean? You can't grant wishes?!"
			)
			(5
				5
				(Converse @local22 3 @local32 self) ; "Very well. Go!."
			)
			(6
				6
			...
			(9
				...
				(sprite
					...
					setPri: -1
					setCycle: Walk
					setMotion: PolyPath local6 local7 self
				)
			)
			(10
				10
	...
	(method (changeState newState param2)
		(switch (= state newState)
			...
			(5
				(sprite dispose:)
				(gSFX number: 213 loop: 1 play:)
				(if (== (gCurRoom picture:) 240)
					(bag posn: 145 120)
				else
					(bag posn: 145 150)
				)
				(bag view: 245 loop: 1 setCel: 0 init: setCycle: End self)
			)
			(6
				(bag view: 771 setLoop: 3 setCel: 7)
				(= ticks 20)
			)
			(7
				(NormalEgo)
...
(instance easterEgg of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Converse 1 @local27 @local41 0 self) ; "'Od's Blood! The forest has grown gigantic!"
			)
			(5
				(HandsOn)
				(gTheIconBar disable: 5 4 6 7)
...
(instance dumpSprite of Code
	...
	(method (doit)
		(if (gCast contains: sprite)
			(sprite setCycle: 0 setScript: 0 dispose: delete:)
		)
		(gSFX stop:)
