...
(local
	...
	local21
	[local22 26]
	[local48 26] = [133 175 137 153 157 132 157 170 168 134 133 147 135 162 159 133 134 174 172 170 174 160 178 176 138 158]
	[local74 26] = [89 79 66 53 42 32 70 28 104 108 18 97 63 75 39 73 45 110 94 74 55 30 9 28 21 15]
	[local100 26] = [3 0 1 3 3 3 0 0 0 1 1 1 2 2 2 3 3 0 0 0 0 3 2 2 1 3]
	[local126 12]
	[local138 13] = [252 0 249 126 71 295 259 195 232 26 291 170 7]
	[local151 13] = [169 184 74 175 158 62 189 165 148 168 137 154 155]
	[local164 4] = [1700 42 1 0]
)
...
(procedure (localproc_2 &tmp temp0)
	(for ((= temp0 0)) (< temp0 26) ((++ temp0))
		(= [local22 temp0] (clIvy new:))
		([local22 temp0]
			x: [local48 temp0]
			y: [local74 temp0]
			setLoop: [local100 temp0]
			setCel: 221
			ignoreActors: 1
			init:
			stopUpd:
...
(instance rm700 of Rm
	...
	(method (notify param1)
		(switch param1
			...
			(2
				(if (and (== global115 1) (not local13))
					(= local13 1)
					(= local12 3)
	...
	(method (init)
		...
		(super init:)
		(if (and (== global115 1) (not (gCast contains: local2)))
			(localproc_0)
		)
	...
	(method (doit &tmp temp0)
		...
		(if (and (not global200) (== global202 256))
			(++ local15)
			(if (and (> (gGame detailLevel:) 2) (== (mod local15 5) 0))
		...
		(cond
			((and (== global115 1) (!= gTheCursor 5) (not local0))
				(= local0 1)
				(localproc_0)
			)
			((and (!= global115 1) local0)
				(gRgnMusic2 stop:)
				(wispHolder dispose:)
				(= local0 0)
				(for ((= temp0 0)) (< temp0 local1) ((++ temp0))
					(if (IsObject [local2 temp0])
			...
			((and (not (<= 130 (gEgo x:) 170)) (not local16) local17 local18)
				(= local16 1)
		...
		(if local0
			(gRgnMusic2 setVol: (- 127 (/ (gEgo distanceTo: local2) 3)))
		else
			(gRgnMusic2 stop:)
		)
	)
	...
	(method (dispose &tmp temp0)
		(for ((= temp0 0)) (< temp0 26) ((++ temp0))
			(if
				(and
					(gCast contains: [local22 temp0])
					(IsObject [local22 temp0])
				)
				([local22 temp0] dispose:)
			)
		)
...
(instance theIvy of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(cond
					((and (== global124 21) local17 (not local18))
...
(instance ivyDoubled of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(for ((= temp0 0)) (< temp0 26) ((++ temp0))
					(if (IsObject [local22 temp0])
						([local22 temp0] stopUpd:)
					)
				)
		...
		(switch (= state newState)
			...
			(1
				(if (AyeOrNay)
					...
					(Converse @local164 1 0 self) ; "Is there nothing else you can do to help me here?"
				)
			)
			(2
				(= cycles 2)
...
(instance outtaHere of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gCurRoom newRoom: (if local14 600 else 620))
			)
		)
	)
)
...
(instance naiadPrinter of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (and local12 (<= local11 local12))
					(-= state 2)
				)
				(self cue:)
			)
