...
(instance rm530 of StdRoom
	...
	(method (init &tmp temp0)
		...
		(keeper
			...
			approachVerbs: 5 4 ; Talk, Inventory
			stopUpd:
		)
		...
		(if (and (not (IsFlag 125)) (!= gDay 10))
			...
			(if (< temp0 4)
				(man1 init: stopUpd: setScript: drinking)
				(gCurRoom
					addObstacle:
						((Polygon new:)
							type: PBarredAccess
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(giles setCycle: Walk init: stopUpd:)
				(jack setCycle: Walk init: stopUpd:)
...
(instance pour of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(keeper view: 536 loop: 1 cel: 0 setCycle: CT 2 1 self)
			)
			(8
				(keeper stopUpd:)
				(= ticks 50)
			...
			(14
				(keeper actions: local264 stopUpd:)
...
(instance notBehindBar of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(keeper setHeading: local262 self)
			)
			(3
				(keeper stopUpd:)
...
(instance fDrunk of Feature
	...
	(method (onMe param1 param2)
		(return (& onMeCheck (OnControl CONTROL param1 param2)))
	)
