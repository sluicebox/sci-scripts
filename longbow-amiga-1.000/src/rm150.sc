		...
		(1
			...
			(cond
				((not (IsFlag 71))
					...
					((ScriptID 21) actions: alansActions detailLevel: 1) ; Alan
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(NormalEgo)
				(if (gCast contains: (ScriptID 20)) ; Will
			...
			(2
				(if local12
					(gEgo stopUpd: setScript: local12)
				else
					(HandsOn)
				)
				(NormalEgo)
...
(instance fireSound of Sound ; UNUSED
	(properties
		flags 1
		number 537
		loop -1
...
(instance streamSound of Sound ; UNUSED
	(properties
		flags 1
		number 581
		loop -1
