...
(local
	...
	local19
	local20
	[local21 4] = [1680 0 1 0]
	[local25 5] = [1680 24 1 2 0]
	[local30 4] = [1680 27 1 0]
	[local34 4] = [1680 28 1 0]
	[local38 4] = [1680 29 1 0]
	[local42 4] = [1680 31 1 0]
	[local46 4] = [1680 32 1 0]
	[local50 5] = [1680 34 1 2 0]
	[local55 4] = [1680 38 1 0]
	[local59 4] = [1680 39 1 0]
	[local63 4] = [1680 41 1 0]
	[local67 4] = [1680 42 1 0]
	[local71 4] = [1680 43 1 0]
	[local75 6] = [1680 44 2 1 2 0]
	[local81 4] = [1680 47 2 0]
	[local85 4] = [1680 49 2 0]
	[local89 4] = [1680 30 1 0]
	local93
)
...
(instance rm680 of Rm
	...
	(method (init)
		...
		(cogito init:)
		(ineptus init: stopUpd:)
		(vocalis init: stopUpd:)
		(hilaris init: stopUpd:)
		(waterGate init: stopUpd:)
		(AddToFeatures
			malitia
			ieiunus
			deformis
	...
	(method (doit &tmp temp0)
		(if (== global202 256)
			(++ local13)
			(if (and (> (gGame detailLevel:) 2) (== (mod local13 5) 0))
		...
		(cond
			(
				(and
					(not local93)
					(or
						(and (Monastery waterDie:) (not local16))
						(and (== global115 2) (not local16))
					)
				)
				(= local93 1)
				local20
			...
			((and (& (gEgo onControl: 1) $0002) (not local16))
	...
	(method (handleEvent event)
		(if
			...
			(cond
				((and (ineptus onMe: event) (== local14 2))
					(event claimed: 1)
					(= local14 3)
					(gEgo setScript: raiseTheGrate)
				)
				((and (ieiunus onMe: event) (== local14 1))
					(event claimed: 1)
					(= local14 2)
					(gCurRoom setScript: (ScriptID 681 1)) ; youGotIeiunus
				)
				((and (cogito onMe: event) (== local14 0))
					(event claimed: 1)
					(= local14 1)
...
(instance dockLeft of Feature
	...
	(method (init)
		...
		((= onMeCheck (Polygon new:))
			type: PTotalAccess
			init: 0 189 0 145 29 146 96 166 121 168 182 188
			yourself:
		)
		(super init:)
	)
...
(instance dockRight of Feature
	...
	(method (init)
		...
		((= onMeCheck (Polygon new:))
			type: PTotalAccess
			init: 319 189 202 189 280 168 319 176
			yourself:
		)
		(super init:)
	)
...
(instance theBoat of Actor
	...
	(method (doit)
		(if (and local16 (not (gEgo script:)))
			(if (gEgo mover:)
				(= x (+ (gEgo x:) 68))
				(= y (- (gEgo y:) 10))
				(fulk posn: (+ (gEgo x:) 45) (+ (gEgo y:) 6))
			)
		else
			(super doit:)
		)
...
(instance fulk of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(Converse @local30 (Monastery tFULK:) 0 self) ; "Even though this boat terrifies me, I'll not leave it until we reach the shore of these fens."
			)
			(5 ; Talk
				(Converse @local34 (Monastery tFULK:) 0 self) ; "Please, let's leave at once! It's bad enough I must get in this flimsy boat."
			)
			(4 ; Inventory
				(switch invItem
					(0 ; bucks
					...
					(2 ; halfHeart
						(Converse @local89 (Monastery tFULK:) 0 self) ; "You are too generous. I could not take such a gift."
					)
					(14 ; amethyst
						(Converse @local42 (Monastery tFULK:) 0 self) ; "I am right fond of amethyst. Thank you."
						(gEgo put: 14) ; amethyst
					)
					(17 ; handScroll
						(Converse @local46 (Monastery tFULK:) 0 self) ; "You risked too much for that scroll to give it to me."
					)
				)
			)
		)
...
(instance viciouslyPullTongue of Script
	...
	(method (init)
		(gEgo setPri: (+ (theBoat priority:) 4) signal: 2064 stopUpd:)
		(fulk setPri: (+ (theBoat priority:) 4) signal: 16 stopUpd:)
	)

	(method (doit)
		(gEgo x: (- (theBoat x:) 68) y: (+ (theBoat y:) 10) cel: 0)
		(fulk x: (- (theBoat x:) 23) y: (+ (theBoat y:) 16))
		(theBoat loop: 1 cel: 1)
		(super doit:)
	)
...
(instance backOntoDock of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo setCycle: (if (< (gEgo x:) 115) Beg else End))
				(if local12
					(gEgo setMotion: PolyPath 216 182 self)
				else
					(gEgo setMotion: PolyPath 115 167 self)
				)
			)
			(1
				(= local10 1)
			...
			(3
				(NormalEgo 2)
				(if local12
					(gEgo setMotion: MoveTo 226 190)
				else
					(gEgo setMotion: MoveTo 110 190)
				)
			)
		)
	)
...
(instance stepInBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(fulk stopUpd:)
				(Converse @local55 (Monastery tFULK:) 0 self) ; "Please let's go at once. As soon as they discover I'm gone, they'll come looking for us."
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(if (== local17 1)
					(= local17 0)
					(Converse @local59 8 0 self) ; "For a famous outlaw, you're rather indecisive."
				)
...
(instance split of Script
	...
	(method (doit)
		...
		(theBoat priority: 1 signal: (| (theBoat signal:) $4010) illegalBits: 0)
		(super doit:)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo view: 20 setLoop: 4 setCel: 1 setCycle: CT 3 1 self)
			)
			(1
				(secretSound play:)
				(= cycles 2)
			...
			(4
				(waterGate stopUpd:)
				(ineptus stopUpd:)
				(gEgo view: 585 setLoop: 2 setCel: 0 setScript: split)
				(gateSound stop:)
				(self dispose:)
			)
		)
...
(instance moveAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Converse @local63 (Monastery tFULK:) 0) ; "There's only a wall here. The water goes no further. Please, return to the gate and let's leave at once!"
				(if (< (gEgo y:) 155)
					(gEgo
						cel: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(fulk illegalBits: 0)
				(Converse @local67 (Monastery tFULK:) 0 self) ; "There's no safety anywhere else! We must escape through this grate or they'll kill us."
			)
			(1
				(gEgo
					cel: 0
...
(instance monkKillDock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse @local71 11 self) ; "I thought I might find you here!"
			)
			(3
...
(instance monkKillBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local71 11 8 self) ; "I thought I might find you here!"
			)
			(2
				(killMonk view: 585 setLoop: 3 setCel: 0 setCycle: End self)
			)
...
(instance priorKillBoat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local75 8 (Monastery tPRIOR:) self) ; "As I suspected, you've trapped yourself. Will you surrender or--"
			)
			(2
				(thePrior setHeading: 310 self)
			)
			(3
				(Converse @local81 8 (Monastery tPRIOR:) self) ; "Vocalis, speak!"
			)
			(4
				(SetMessageColor 32)
			...
			(5
				(Converse @local85 8 (Monastery tPRIOR:) self) ; "Call death down upon the head of yonder sinner."
			)
			(6
				(thePrior stopUpd:)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(ineptus ignoreActors:)
				(hilaris ignoreActors:)
				(cogito ignoreActors:)
...
(instance askTheDwarf of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local50 8 (Monastery tFULK:) self) ; "Fulk, how did they raise the grate?"
			)
			(2
				(HandsOn)
				(gTheIconBar disable: 5 4)
...
(instance lookAtFulk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse @local25 8 (Monastery tFULK:) self) ; "What troubles you so, my friend? You were so brave before."
			)
			(2
				(HandsOn)
				(gTheIconBar disable: 5 4)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(SetScore 25 96)
				(Converse @local38 (Monastery tFULK:) 0 self) ; "You are too kind. Thank you, but please, let us leave here!"
			)
			(1
				(HandsOn)
				(gTheIconBar disable: 5 4)
