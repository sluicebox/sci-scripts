...
(local
	...
	local40
	local41
	[local42 6] = [1010 0 1 1 1 0]
	[local48 14] = [1010 7 2 1 1 2 1 2 1 3 4 1 3 0]
	[local62 6] = [1010 18 2 1 2 0]
	[local68 6] = [1010 21 2 1 2 0]
	[local74 6] = [1010 24 2 1 1 0]
	[local80 4] = [1010 27 1 0]
	[local84 5] = [1010 28 1 2 0]
	[local89 4] = [1010 30 1 0]
	[local93 5] = [1010 31 1 1 0]
	[local98 6] = [1010 44 1 2 1 0]
	[local104 5] = [1010 47 2 1 0]
	[local109 6] = [1010 49 2 1 2 0]
	[local115 9] = [1010 52 1 2 1 2 1 2 0]
	[local124 8] = [1010 58 1 0 1010 59 1 0]
	[local132 4] = [1010 60 1 0]
	[local136 4] = [1010 61 1 0]
	[local140 4] = [1010 62 1 0]
	[local144 4] = [1010 64 1 0]
	[local148 5] = [1010 65 1 1 0]
	[local153 9] = [1010 67 1 2 1 2 2 1 0]
	[local162 6] = [1010 73 1 2 1 0]
	[local168 5] = [1010 76 1 1 0]
	[local173 4] = [1010 41 1 0]
	[local177 4] = [1010 42 1 0]
	[local181 4] = [1010 79 1 0]
	[local185 4] = [1010 80 1 0]
	[local189 13] = [1010 82 2 1 1 2 1 2 1 2 1 1 0]
	[local202 4] = [1010 92 1 0]
	[local206 4] = [1010 93 1 0]
	[local210 4] = [1010 94 1 0]
	[local214 6] = [1010 95 2 1 2 0]
	[local220 6] = [1010 98 2 1 2 0]
	[local226 6] = [1010 101 2 1 2 0]
	[local232 6] = [1010 104 2 1 2 0]
	[local238 4] = [1010 107 1 0]
)
			...
			(else
				(switch theVerb
					...
					(4 ; Inventory
						(switch invItem
							(1 ; horn
								(Converse 1 @local206 @local0 3) ; "I don't want your thieves' horn."
							)
							(0 ; bucks
								(if (= local31 (gMoney doit:))
									(Converse 1 @local210 @local0 3) ; "I can use the money, outlaw. I gambled all mine away last night."
								)
							)
							(2 ; halfHeart
								(Converse @local214 1 @local0) ; "Trying to bribe me, outlaw? For what, a pardon from the Queen?"
								(gEgo put: 2 fKnight) ; halfHeart
								(|= local32 $0001)
							)
							(14 ; amethyst
								(Converse @local220 1 @local0) ; "I hear an amethyst can keep a man from getting drunk."
								(gEgo put: 14 fKnight) ; amethyst
								(|= local32 $0002)
							)
							(10 ; fireRing
								(Converse @local226 1 @local0) ; "Nice bit of stone."
							...
							(18 ; waterRing
								(Converse @local232 1 @local0) ; "How'd you come by this?"
							...
							(else
								(Converse 1 @local238 @local0 3) ; "What do I want with that? If you have business with me, get on with it."
							)
						)
					)
					(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(if local30
					(if local40
						(Converse 1 @local173 13 3) ; "I'll see to the horse, not to worry."
					else
						(Say 1010 40) ; "Little John can deal with the horse. He has a way with the beasts."
					)
				else
					(Converse 1 @local177 @local0 3) ; "Keep your hands off my horse, knave."
				)
			)
			(5 ; Talk
				(if local39
...
(instance nayTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Converse 1 @local132 1 2 self) ; "Horse, do you speak?"
			)
			(1
				(horseSound play:)
				(Converse 1 @local136 @local9 0 self) ; "Nay."
			)
			(2
				(Converse 1 @local140 1 2 self) ; "I thought not."
			)
			(3
				(HandsOn)
				(self dispose:)
...
(instance fKnightEncounter of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse 4 @local48 1 2 @local0 3 13 2 14 3 self) ; "So you're the leader of this rabble. What's the meaning of this?"
			)
			(3
				(= ticks 60)
			)
			...
			(8
				...
				(bigJohn dispose:)
				(bigTuck dispose:)
				(Converse 2 @local62 1 2 @local0 3 self) ; "Well, explain yourself or step aside. I've no time to waste on a common outlaw."
			)
			(9
...
(instance willSez of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Converse 1 @local42 14 0 self) ; "Robin, I've been looking for you."
			)
			(2
				(= ticks 60)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not local33)
					(= local33 1)
					(Converse 2 @local68 1 2 @local0 3 self) ; "What were you doing with your hand?"
				else
					(= register 1)
					(Converse 2 @local74 2 2 @local0 3 self) ; "What's the meaning of that? Answer me!"
				)
			)
			(2
				(if register
...
(instance liftSword of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Converse 1 @local80 @local0 3 self) ; "Here's your answer!"
			)
			(3
				(= ticks 120)
			)
...
(instance doWarning of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(Converse 2 @local84 1 2 @local0 3 self) ; "Come down from your horse and--"
			)
			(3
...
(instance robinDeath of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(Converse 1 @local89 1 2 self) ; "Come down from your horse or I'll--."
			)
			(3
				(fKnight
					cycleSpeed: 3
			...
			(6
				(swordSound dispose:)
				(if local35
					(Converse 1 @local93 @local0 3 self) ; "So much for empty threats."
				else
					(Converse 1 @local144 @local0 3 self) ; "Prince John will reward me well for this day's work!"
				)
			)
			(7
				(= gDeathNum 3)
...
(instance yakItUp1 of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(HandsOff)
				(Converse 2 @local98 1 2 @local0 3 self) ; "Do you serve the Queen?"
			)
			(1
			...
			(3
				(Converse 2 @local104 1 2 @local0 3 self) ; "You dare to threaten me?"
			)
			(4
			...
			(6
				(Converse 2 @local109 1 2 @local0 3 self) ; "You should have said so at once. Turn everything over to me."
			)
			(7
				(HandsOn)
				(gTheIconBar disable: 5 0)
...
(instance yakItUp2 of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(HandsOff)
				(Converse 2 @local115 1 2 @local0 3 self) ; "Have you no token, no message to prove you're the Queen's Knight?"
			)
			(1
				(Converse 1 @local124 2 2 self) ; "You false Knight! What have you done with the true messenger?!"
			)
			(2
				(= local39 1)
				(if (not (HaveMouse))
...
(instance hereComesJohn of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(Converse 1 @local148 13 3 self) ; "I stayed within range of your horn, Master. What's happened here?"
			)
			(3
				(if (gEgo has: 9) ; message
					(Converse @local162 1 13 self) ; "He wasn't the Queen's man. He carried a written message to the Sheriff from the Prior, proof of their conspiracy with Prince John."
				else
					(Converse @local153 1 13 self) ; "I'm sure the man was false."
				)
			)
			(4
				(HandsOn)
...
(instance showLetter of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(Converse @local189 1 13 self) ; "I cannot read, as you know. What's it say?"
			)
			(2
				(SetScore 50)
				(HandsOn)
...
(instance johnActions of SpecialDoVerb
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				1
				(Converse 1 @local181 13 3) ; "I'm here. What more do you want?"
			)
			(5 ; Talk
				1
				(Converse 1 @local185 13 3) ; "Leave this mess with me. I'll get him out of the road and see to the horse."
			)
			(10 ; Longbow
				1
			...
			(4 ; Inventory
				(if (== invItem 9) ; message
					1
					(if local38
						(Converse 1 @local202 13 3) ; "It'll be safer in your keeping, Master."
					else
						(= local38 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					(local40
						(Converse 1 @local168 13 3 self) ; "The other men have spread out and may not hear your horn, Robin."
					)
					(local30
						(self setScript: (ScriptID 5 1) self) ; blowinIt
					)
