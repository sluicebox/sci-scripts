...
(instance rm600 of Rm
	...
	(method (doit)
		(if (and (== global202 256) (not global200))
			(++ local0)
			(cond
				((== local1 1)
					(if (== (mod local0 5) 0)
...
(instance cloud of PicView
	(properties
		z 15
		lookStr 23 ; "The fog grows ever thicker. It would be easy to become lost within."
		view 590
		signal 24576
	)
	...
	(method (doVerb theVerb)
		(if (== theVerb 3) ; Do
			(Say 1600 24 1) ; "How can one grasp insubstantial tendrils of mist?"
		else
...
(instance aPlant of PicView
	(properties
		lookStr 25 ; "Only stunted bushes and thick reeds grow near these fens."
		signal 16384
	)
)
...
(instance theWater of Feature
	(properties
		lookStr 27 ; "Faith, that is dark and unwholesome water, just deep enough to drown a man."
	)

	(method (doVerb theVerb)
		(if (== theVerb 3) ; Do
			(Say 1600 28 1) ; "I care not to touch this water."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance theSky of Feature
	(properties
		nsBottom 80
		nsRight 319
		lookStr 31 ; "A perpetual gloom hangs over these fens."
	)
)
...
(instance theGround of Feature
	(properties
		...
		nsBottom 189
		nsRight 319
		lookStr 30 ; "The ground here is boggy and unpleasant."
	)
)
...
(instance bumpBog of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Say 1600 26 self) ; "There's a thicket and marshy ground up ahead. I'll have to turn back."
			)
			(1
				(gEgo
...
(instance walkDown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say 1600 22 self) ; "Working my way through the dense fog, I can but return to the fen shore."
			)
			(2
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(gEgo setCel: 5 stopUpd:)
				((ScriptID 601 1) setPri: -1 stopUpd:) ; rgnBoat
			)
			(2
				(fulk
					setLoop: 5
			...
			(21
				(if (<= gHowFast 1)
					(gEgo stopUpd:)
					((ScriptID 601 1) ; rgnBoat
						setStep: temp0 temp0 ; UNINIT, UNINIT
						setMotion:
							MoveTo
							(+ ((ScriptID 601 1) x:) 220) ; rgnBoat
							(- ((ScriptID 601 1) y:) 50) ; rgnBoat
