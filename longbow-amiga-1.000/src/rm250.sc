...
(instance rm250 of Rm
	...
	(method (doit)
		(++ local81)
		(if (and (> (gGame detailLevel:) 2) (not global200))
			(Palette palANIMATE 240 250 1)
			(cond
				((and local84 (not (mod local81 65)))
					(widow setCycle: End widow)
				)
				((and local83 (not (mod local81 90)))
					(lamb setCycle: End lamb)
				)
				((and local85 (not (mod local81 147)))
					(hal setCel: (if (hal cel:) 0 else 1))
				)
				((and local85 (not (mod local81 153)))
					(hob setCel: (if (hob cel:) 0 else 1))
				)
				((and local85 (not (mod local81 178)))
