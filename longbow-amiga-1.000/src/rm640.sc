	...
	(method (init &tmp temp0)
		...
		(gCurRoom
			addObstacle:
				...
				((Polygon new:)
					type: PBarredAccess
					init: 0 0 319 -1 319 101 250 101 235 87 227 87 247 102 273 137 168 136 164 123 164 90 131 91 82 91 70 103 0 103
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
					init: 40 138 78 100 155 100 138 140
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
...
(instance meetThePrior of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gEgo stopUpd:)
				((ScriptID 625 2) stopUpd:) ; prior
				(= ticks (+ 20 (* gHowFast 10)))
			)
			(4
...
(instance takeAndRun of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (== local15 monkey1)
					(monkey1 setCycle: Walk setMotion: MoveTo 239 95 self)
				else
					(monkey2 setCycle: Walk setMotion: MoveTo 239 95 self)
				)
			)
			(4
				(if (== local15 monkey1)
