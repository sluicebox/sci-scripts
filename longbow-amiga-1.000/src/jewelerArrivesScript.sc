...
(local
	...
	[local136 7] = [1403 59 3 2 2 1 0]
	[local143 7] = [1403 63 1 3 3 2 0]
	[local150 5] = [1403 67 1 2 0]
	[local155 5] = [1403 69 1 2 0]
	[local160 5] = [1403 71 1 3 0]
	[local165 6] = [1403 73 1 3 3 0]
	[local171 8] = [1403 76 2 2 1 2 2 0]
	[local179 4] = [1403 81 2 0]
	[local183 5] = [1403 82 3 3 0]
	[local188 4] = [1403 84 3 0]
	[local192 10] = [1403 85 2 2 1 2 1 2 2 0]
	[local202 4] = [1403 92 2 0]
	[local206 5] = [1403 93 3 3 0]
	[local211 4] = [1403 95 3 0]
	[local215 13] = [1403 96 1 3 3 2 3 2 1 2 1 3 0]
	[local228 5] = [1403 106 2 1 0]
	[local233 9] = [1403 108 2 1 1 2 1 2 0]
	[local242 34] = [1403 114 2 1 2 1 1 3 1 1 3 1 1 1 3 1 2 2 1 2 1 2 1 1 2 1 2 1 3 3 2 2 2 0]
	[local276 4] = [1403 145 3 0]
	[local280 5] = [1403 146 1 3 0]
	[local285 6] = [1403 148 2 1 3 0]
	[local291 4] = [1403 151 1 0]
	[local295 4] = [1403 152 3 0]
)
...
(instance jewelerArrivesScript of TScript
	...
	(method (doit &tmp temp0 temp1 temp2 temp3 temp4)
		...
		(cond
			...
			((= temp3 (temp0 isMemberOf: PolyPath))
				(= temp1 (temp0 finalX:))
				(= temp2 (temp0 finalY:))
			)
			(else
		...
		(if (and (== (gEgo x:) temp1) (== (gEgo y:) temp2))
			(return)
		)
		(if (and 1 (< temp1 (gEgo x:)))
			(gEgo setMotion: 0)
			(if local7
		...
		(switch theVerb
			...
			(4 ; Inventory
				(switch invItem
					...
					(7 ; jewels
						(localproc_0 (if local7 @local280 else @local276))
					)
					(8 ; rouge
...
(instance EmToMatildaJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(localproc_0 @local143 self) ; "But if you demand more proof than this, then I shall give up and leave at once."
			)
			(3
...
(instance AmToMatilda of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local160 self) ; "If your Ladyship would care to examine this amethyst---"
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance AmToMatildaJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local165 self) ; "I'll gladly give you this amethyst if you'll help the Sheriff change his mind."
			)
			(1
				(self dispose:)
			)
...
(instance FireToMatilda of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local183 self) ; "It is a glorious ruby, but a man's ring."
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance FireToMatildaJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local188 self) ; "What a shame that glorious ruby is set in a man's ring."
			)
			(1
				(self dispose:)
			)
...
(instance WaterToMatilda of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local206 self) ; "That sapphire is perfect, but set in a man's ring."
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance WaterToMatildaJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local211 self) ; "How sad that perfect sapphire is set in a man's ring."
			)
			(1
				(self dispose:)
			)
...
(instance RougeToMatilda of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(localproc_0 @local291 self) ; "Allow me to gift you with some rouge, for I've heard you have a fine collection of jewels."
			)
			(1
				(gEgo
					setMotion:
			...
			(3
				(localproc_0 @local295 self) ; "Good, I was running low and have many fine pieces of jewelry in need of a polish."
			)
			(4
...
(instance AmToSheriff of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local150 self) ; "I have this amethyst crystal in my wares."
			)
			(2
				(proc400_1 gEgo 10 self)
			)
...
(instance AmToSheriffJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local155 self) ; "Will this change your mind?"
			)
			(1
				(self dispose:)
			)
...
(instance FireToSheriff of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local171 self) ; "This is all you have to show?"
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance FireToSheriffJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local179 self) ; "It's a nice enough ring, but it's not proof enough."
			)
			(1
				(self dispose:)
			)
...
(instance WaterToSheriff of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local192 self) ; "This is all you have to show?"
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance WaterToSheriffJewels of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(localproc_0 @local202 self) ; "You'll not wring 5,000 marks from me with only this for proof."
			)
			(1
				(self dispose:)
			)
...
(instance JewelsToSheriff of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local215 self) ; "Feast your eyes upon this exquisite workmanship and the finest jewels of all creation!"
			)
			(2
				(local0 loop: 1 cel: 0 setCycle: End self)
			)
			(3
				(localproc_0 @local228 self) ; "Matilda! Still your tongue, woman."
			)
			(4
			...
			(5
				...
				(localproc_0 @local233 self) ; "Then you'll take the 50 marks?!"
			)
			(6
				(local2 setMotion: MoveTo -35 (local2 y:) self)
			)
			(7
				(localproc_0 @local242 self) ; "Come, my friend, tell me all."
			)
			(8
...
(instance RougeToSheriff of TScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 @local285 self) ; "Why the devil are you giving me this?"
			)
			(2
				(gEgo put: 8 gCurRoomNum) ; rouge
				(HandsOn)
