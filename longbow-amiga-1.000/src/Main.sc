...
(procedure (IsFlag param1)
	(return (& [global204 (/ param1 16)] (>> $8000 (mod param1 16))))
)

(procedure (SetFlag param1 &tmp temp0)
	(= temp0 (IsFlag param1))
	(= [global204 (/ param1 16)]
		(| [global204 (/ param1 16)] (>> $8000 (mod param1 16)))
	)
	(return temp0)
)
		...
		(GameControls
			...
			add:
				...
				(iconSave
					theObj: self
					selector: #save
					lowlightColor: (if (== global202 64) 18 else global179)
					yourself:
				)
				(iconRestore
					theObj: self
					selector: #restore
					lowlightColor: (if (== global202 64) 58 else 60)
					yourself:
				)
				(iconRestart
					theObj: self
					selector: #restart
					lowlightColor: (if (== global202 64) 25 else global181)
					yourself:
				)
				(iconQuit
					theObj: self
					selector: #quitGame
					lowlightColor: (if (== global202 64) 22 else global174)
					yourself:
				)
				(iconAbout
					theObj: (ScriptID 809 0) ; aboutCode
					selector: #doit
					lowlightColor: (if (== global202 64) 31 else global178)
					yourself:
				)
				(iconOk lowlightColor: (if (== global202 64) 48 else global180))
				(iconHelp
					lowlightColor: (if (== global202 64) 16 else global177)
				)
			eachElementDo: #highlightColor global184
