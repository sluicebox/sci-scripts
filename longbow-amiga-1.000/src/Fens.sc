...
(local
	...
	local31
	[local32 208]
	[local240 12] = [0 0 0 0 0 0 0 0 1601 5 1 0]
	[local252 4] = [1601 98 1 0]
	[local256 4] = [1601 60 1 0]
	[local260 4] = [1601 65 1 0]
	[local264 4] = [1601 66 1 0]
	[local268 4] = [1601 67 1 0]
	[local272 4] = [1601 68 1 0]
	[local276 4] = [1601 69 1 0]
	[local280 4] = [1601 70 1 0]
	[local284 4] = [1601 71 1 0]
	[local288 4] = [1601 72 1 0]
	[local292 4] = [1601 73 1 0]
	[local296 4] = [1601 74 1 0]
	[local300 4] = [1601 7 1 0]
	[local304 4] = [1601 8 1 0]
	[local308 4] = [1601 77 1 0]
	[local312 6] = [1601 80 2 1 2 0]
	[local318 4] = [1601 83 1 0]
	[local322 5] = [1601 84 1 2 0]
	[local327 5] = [1601 95 1 0 0]
	[local332 5] = [1601 96 1 0 0]
	[local337 4] = [1601 79 1 0]
)
...
(procedure (localproc_0 &tmp temp0)
	...
	(if (and (== gCurRoomNum 610) (== gDay 13))
		(= local31 (ScriptID 610 1)) ; hiddenWisp
		(for ((= temp0 0)) (< temp0 local14) ((++ temp0))
			(= [local32 temp0] (Wisp new:))
			([local32 temp0]
				setLoop: (Random 0 2)
				setStep: 10 10
				x: (+ (local31 x:) (Random 0 60))
				y: (+ (local31 y:) (Random 0 60))
				ignoreActors: 1
				init:
				setScript: (= [local240 temp0] (frenzy new:))
			)
		)
	else
		(wispCage init:)
...
(instance Fens of Rgn
	...
	(method (doit &tmp temp0)
		(cond
			...
			((and (!= global115 1) local7 (not local3))
				(if (== gCurRoomNum 610)
					(= local3 1)
					(for ((= temp0 0)) (< temp0 local14) ((++ temp0))
						([local32 temp0] hide:)
			...
			(
				...
				(= local28 1)
				(Converse 1 @local256 11 0 0) ; "Where are you going, Brother?"
			)
			(
				(and
					local4
		...
		(if local7
			(if (> gHowFast 1)
				(gRgnMusic2 setVol: (- 127 (/ (gEgo distanceTo: local32) 3)))
			)
		else
			(gRgnMusic2 stop:)
		)
		...
		(switch theVerb
			...
			(5 ; Talk
				(cond
					...
					((!= (gEgo view:) 587)
						(if (or (gCast contains: rgnBoat) local25)
...
(instance rgnBoat of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(if (and local25 (!= gCurRoomNum 610))
					(if (or (== gDisguiseNum 6) local7) ; fens monk
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(3 ; Do
				(cond
					((== gCurRoomNum 610)
						(Converse 1 @local260 11 0 0) ; "Remain where you are. I'm the one poling this boat."
					)
					((== (gEgo view:) 19)
						(Converse 1 @local260 11 0 0) ; "Remain where you are. I'm the one poling this boat."
					)
					(local25
						(Converse 1 @local264 11 0 0) ; "I will pole the boat, Brother. Get in and sit down."
					)
					(else
						(super doVerb: theVerb &rest)
					)
			...
			(5 ; Talk
				(cond
					((== gCurRoomNum 610)
						(switch local6
							(0
								(Converse 1 @local268 11 0 0) ; "Shhhh. I must concentrate."
							)
							(1
								(Converse 1 @local272 11 0 0) ; "Silence! I must find my way by sound and feel. Disturb me again and you'll sleep at the bottom of the fens this day."
							)
							(2
					...
					(local25
						(Converse 1 @local276 11 0 0) ; "Please get in and sit down, Brother."
					)
					(else
						(super doVerb: theVerb &rest)
					)
			...
			(4 ; Inventory
				(switch invItem
					(0 ; bucks
						(Converse 1 @local280 11 0 0) ; "I cannot take it. You must turn over all money to the Prior."
					)
					(2 ; halfHeart
						(Converse 1 @local284 11 0 0) ; "Something that precious must be given to the Prior as soon as you are admitted. IF you are admitted."
					)
					(10 ; fireRing
						(Converse 1 @local288 11 0 0) ; "Our vows require us to give up all earthly possessions. You must give this ring to the Prior."
					)
					(5 ; pipe
						(Converse 1 @local292 11 0 0) ; "Why should you give this to me?"
					)
					(6 ; gems
						(Converse 1 @local296 11 0 0) ; "How do you expect to enter without the gemstones?"
					)
					(else
						(super doVerb: theVerb &rest)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if (AyeOrNay)
					(Converse 1 @local300 999 0 self) ; "Aye, how can you help me?"
				else
					(+= state 3)
					(self cue:)
				)
			...
			(6
				(Converse 1 @local252 999 0 self) ; "Nay, I've no wish to risk returning to the monastery without good cause."
			)
			(7
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				...
				(gCurRoom notify: 1)
				(for ((= temp0 0)) (< temp0 local14) ((++ temp0))
					([local32 temp0] ignoreBlocks: wispCage setStep: 10 10)
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 temp4)
		(switch (= state newState)
			(0
				...
				(= temp2 (+ (local31 x:) (Random 0 temp0)))
				(= temp3 (+ (local31 y:) (Random 0 temp1)))
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				...
				(Converse 1 @local304 999 0 self) ; "I seek a missing knight and suspect these monks of treachery. Can you help me?"
			)
			(1
				(= seconds 2)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond
					((gCast contains: monk)
						(Converse 1 @local337 11 0 self) ; "There is no point giving the signal again, Brother. I am here."
					)
					(local4
			...
			(1
				(cond
					...
					(local4
						(Converse 1 @local308 11 0 self) ; "Step in, then, and be seated. I'll take you across."
					)
					(else
						(self cue:)
					)
			...
			(7
				...
				(switch global136
					(0
						(Converse 2 @local312 999 0 11 3 self) ; "I heard your signal, Brother, but I don't know your face. Have you newly joined our order?"
					)
					(1
						(Converse 1 @local318 11 0 self) ; "I grow mighty weary of your indecision, Brother. I may not answer a third time."
					)
					(2
						(Converse 2 @local322 999 0 11 3 self) ; "Sorry, Brother. I'm ready this time."
					)
				)
			)
			(8
...
(instance flutePlayer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= local10 (rgnBoat xStep:))
				(= local11 (rgnBoat yStep:))
				(rgnBoat setStep: 0 0)
				(monk setCycle: 0)
				(self setScript: (ScriptID 3 0) self) ; pipeIt
			)
			(1
				(if local5
					(Converse 1 @local327 11 0 self) ; "That does it."
				else
					(Converse 1 @local332 11 0 self) ; "Stop that you fool! I won't warn you again!"
				)
			)
			(2
				(if local5
					...
					(rgnBoat setStep: local10 local11)
					(monk setCycle: Fwd)
					(= local5 1)
					(self dispose:)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Say
					1601
					(if local9 local9 else register)
					self
					67
					(Random 15 200)
					(Random 5 100)
