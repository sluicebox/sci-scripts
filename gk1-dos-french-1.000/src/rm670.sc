...
(instance rm670 of Room
	...
	(method (init)
		...
		(SetFlag 220)
		(gGkMusic1 number: 350 setLoop: -1 play: setVol: 0 fade: 127 5 10 0)
		(fire init: setCycle: Fwd)
		(super init: &rest)
		(if (IsFlag 407)
			(mosely init:)
		)
		(self setScript: (if (IsFlag 407) sayDead else sayAlive))
	)
...
(instance endNar of Narrator
	...
	(method (display param1 &tmp temp0)
		...
		(switch global180
			(1
				(gabeHead init: setCycle: RandCycle (* (param1 size:) 4) 0 1)
			)
			(8
				(moselyHead init: setCycle: RandCycle (* (param1 size:) 4) 0 1)
			)
			(2
				(graceHead init: setCycle: RandCycle (* (param1 size:) 4) 0 1)
			)
		)
		(if (& gMsgType $0001)
