...
(instance bookstore of Room
	...
	(method (init)
		...
		(cond
			...
			((== gPrevRoomNum 220)
				(if (and (== gDay 7) (IsFlag 269) (IsFlag 400))
					(= local4 1)
					(SetFlag 6)
					(graceProp init:)
					(gEgo setScript: comeFromStudio)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gGame handsOn:)
				(if (and (IsFlag 269) (IsFlag 262))
					(gCurRoom setScript: (ScriptID 213 1)) ; tellAboutGermany
				)
				(if local4
					(= local4 0)
					(gCurRoom setScript: talkToGrace)
				)
				(self dispose:)
			)
		)
...
(instance getReferenceBooks of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 521 setLoop: 3 setCel: 0 setCycle: End self)
			)
			(2
				(if (== global177 8)
					(= global177 2)
			...
			(4
				(gEgo view: 521 setLoop: 3 setCel: 7 setCycle: Beg self)
			)
			(5
				(gEgo normalize: 1 901 ignoreActors: 1)
				(++ global177)
