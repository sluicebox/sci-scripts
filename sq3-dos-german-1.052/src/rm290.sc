...
(procedure (localproc_1)
	(if (gEgo has: 8) ; Buckazoids
		...
		(SaveSubLang)
		(Display ; "Insert Buckazoid!"
			290
			13
			dsCOORD
			(LangSwitch 52 44 52 44)
			178
			dsWIDTH
			(LangSwitch 88 98 88 98)
			dsCOLOR
			0
			dsBACKGROUND
			0
		...
		(Display 290 12 dsCOORD 148 178 dsFONT 600 dsWIDTH 135 dsCOLOR 0) ; "PRESS F6 TO EXIT"
		(RestoreSubLang)
		(guysLeft init:)
		(localproc_0)
		(= local3 22)
		...
		(Print 290 19) ; "You don't even have a buck."
	)
)
...
(instance rm290 of Rm
	...
	(method (init)
		(Load rsVIEW 290)
		(Load rsVIEW (LangSwitch 291 331))
		(Load rsPIC 290)
		(Load rsPIC (LangSwitch 291 319))
		(Load rsPIC 292)
		(Load rsSOUND 26)
		(Load rsSOUND 33)
		(super init:)
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					(
						(or
							(Said 'disembark,disembark,quit[/game,machine]')
							(Said '/bye')
						)
						(= global193 0)
						(= global159 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= global159 0)
				(guysLeft dispose:)
				(client drawPic: (LangSwitch 291 319))
				(SaveSubLang)
				(Display 290 12 dsCOORD 148 178 dsFONT 600 dsWIDTH 135 dsCOLOR 0) ; "PRESS F6 TO EXIT"
				(Display ; "Insert Buckazoid!"
					290
			...
			(1
				...
				(RestoreSubLang)
				(Display ; "ASTRO CHICKEN MUST LAND ON THE ASTRO CHICKEN LANDING PAD. HE'S DEPENDING ON YOU TO BRING HIM TO SAFETY!"
					290
					14
					dsCOORD
					60
					(LangSwitch 70 70 45 45)
					dsWIDTH
					200
					dsCOLOR
					15
			...
			(2
				...
				(Display ; "Controls: Left arrow: move left Down arrow: stop left/right movement Right arrow: move right Up arrow: toggle flapping Feed: don't use up all your feed!"
					290
					15
					dsCOORD
					60
					(LangSwitch 65 65 35 35)
					dsWIDTH
					200
					dsCOLOR
					15
			...
			(3
				...
				(Display ; "Controls: Hit the landing pad too fast and you'll bounce back up. Landing outside the landing pad is fatal. If you fly too high you'll bounce off the atmosphere and plummet back to the surface."
					290
					16
					dsCOORD
					60
					(LangSwitch 60 60 25 25)
					dsWIDTH
					200
					dsCOLOR
					15
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(SaveSubLang)
				(= local14
					(Display ; "HELP US! WE ARE BEING HELD CAPTIVE BY SCUMSOFT ON THE SMALL MOON OF PESTULON. AN INPENETRABLE FORCE FIELD SURROUNDS THE MOON. IT MUST FIRST BE DEACTIVATED. IT'S ORIGIN IS UNKNOWN TO US. SCUMSOFT SECURITY IS ARMED WITH JELLO PISTOLS. WE'RE COUNTING ON YOU WHOEVER YOU ARE. TWO GUYS IN TROUBLE"
						290
						17
						dsCOORD
						(LangSwitch 60 57 60 57)
						(LangSwitch 30 28 30 28)
						dsWIDTH
						(LangSwitch 200 206 200 206)
						dsCOLOR
						15
						dsBACKGROUND
						0
...
(instance guysLeft of Prop
	...
	(method (init)
		(super init:)
		(self view: 290 setLoop: 7 setCel: 0 setPri: 15 posn: 57 39 stopUpd:)
	)
...
(instance gameOver of Prop
	...
	(method (init)
		(super init:)
		(self view: (LangSwitch 291 331) setLoop: 3 setPri: 14 posn: 155 91)
	)
)
...
(instance explosion of Prop
	...
	(method (init)
		(super init:)
		(self view: (LangSwitch 291 331) setPri: 14 setLoop: 0 setCel: 0)
	)
)
