				...
				((Said 'read/sign,sign')
					(if (== (gEgo onControl: 1) 8192)
						(if (or (== (gEgo loop:) 1) (== (gEgo loop:) 3))
							(Print 44 0) ; "Just Ahead! Visit the Mog Memorial and the galaxy famous World O' Wonders"
						else
							(Print 44 1) ; "Try facing in the right direction."
						)
					else
						(Print 44 2) ; "You're not close enough."
					)
				)
				((Said 'look,look/sign,sign')
					(Print 44 3) ; "Yes, that's a sign."
				)
				((and (Said 'look,look>') (Said '[<around,at,in][/area]'))
					(Print 44 4) ; "You see sand, mountains in the distance, and a sign."
				)
			)
		)
	)
