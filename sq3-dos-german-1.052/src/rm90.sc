			...
			(evSAID
				(if (and (< (gEgo x:) 84) (== local3 90))
					(cond
						(
							(Said
								'look,look[/area,partition,left,right,up,down,ceiling,deck]'
							)
						...
						((Said 'look,look,explore/area,closet')
							(gEgo setScript: searchScript)
						)
						(
							(Said
								'get,get,wear,drop,change,change,switch,explore/attire'
							)
							(if (gEgo has: 12) ; Coveralls
								(Print 90 2) ; "That you did."
							else
								(PrintSplit 90 3) ; "What a great idea, Roger! No one would be suspicious of a janitor walking around. You grab the coveralls and pull them on (seizing the opportune moment to dump all of the old items you've been pocketing along the way). What a great disguise! Wait... what's this?"
								(PrintSplit 90 4) ; "You reach down into the pocket of the grimy coveralls and find... well, what do you know, Mr. Garbage! A trash vaporizer. You've seen these babies in all the janitorial supply catalogs but your superiors were always too cheap to outfit you with one."
						...
						(
							(Said
								'disembark,disembark,disembark,disembark[/area,closet]'
							)
				...
				(cond
					...
					(
						(Said
							'look,look[/area,corridor,deck,dirt,ceiling,partition,up,down,left,right]'
						)
						(cond
							((<= 114 (gEgo x:) 203)
								(if
									...
									(Printf 90 6 local0 local0) ; "You are walking down a circular hallway... going %swhere.%"
								else
					...
					((Said 'remove/attire[<janitor]')
						(if (gEgo has: 12) ; Coveralls
							(Print 90 14) ; "Second guessing yourself, you leave the `disguise' on."
						else
							(event claimed: 0)
						)
					)
					...
					((Said 'find,get,get/card')
						(Print 90 17) ; "Do that."
					)
					(
						(Said
							'look,look/device,keylock,latch,latch,card,system,machine'
						)
					...
					((Said '/device,keylock,latch,latch,system,machine')
						(if (and (== local1 eHallDoor) (== local3 95))
					...
					((Said 'look,look/crack,crack,crack')
						(Print 90 30) ; "Don't step on a crack!"
					)
					((Said 'explore,look,look/pocket,pocket')
						(if (gEgo has: 12) ; Coveralls
					...
					(
						(or
							(Said 'copy,copy<use')
							(Said
								'drop,halt,hold,show,use,position,position,place,place/original,copy,copy'
							)
						)
