...
(instance Room421 of Rm
	...
	(method (handleEvent event)
		...
		(if (== (event type:) evSAID)
			(cond
				((Said 'look,look,talk,talk/blatz')
				...
				((Said 'look,look>')
					(cond
									...
									(if
										...
										(Print
											(Format ; "The terminator is now a pile of junk on the first level platform. %s%"
												...
												(if (IsItemAt 16 421) ; Invisibility_Belt
													{Beim genauen Hinsehen f\84llt Dir auf, da\e1 der Tarng\81rtel des Terminators noch einigerma\e1en intakt ist.}
												else
													{}
												)
											)
						...
						((Said '/engine,machine,equipment')
							(Print 421 14) ; "The large electric motor providing power for Mog's automation hums noisily away on the second level platform."
						)
						((Said '/cog')
							(PrintSplit 421 15) ; "It appears that the gears provide the power linkage for the machinery moving Mog's arms and jaw. It seems that they haven't heard of OSHA here on Phleebhut. There isn't even a guard rail around them. I sure wouldn't want to fall in there!"
						)
						((Said '/rope,rope,pulley,block,hook')
							(PrintSplit 421 16) ; "There are two rope pulleys near the motor and gears. Apparently they are used for maintenance of the equipment. Looking up, you notice that they are mounted on tracks running the length of the second floor platform."
						)
						((Said '/banister')
							(Print 421 17) ; "The two rope pulleys are mounted on the tracks."
						)
						((Said '/bolt')
							(Print 421 18) ; "Nuts to you!"
						)
						((Said '/pit')
							(Print 421 19) ; "The unguarded elevator shaft runs down to the entrance below. I wonder how many tourists they lose falling down there every year. I hope Fester has good liability insurance."
						)
						((Said '/girder')
							(Print 421 20) ; "Steel girders provide the structural support for the statue and equipment."
						)
						((Said '/elevator')
							(if (or (== local1 2) (== local1 1))
								(Print 421 21) ; "Yes, that's an elevator."
							else
								(Print 421 22) ; "You don't see one here."
							)
						)
						((Said '/button,button,control,control')
							(if
								(and
									(gEgo inRect: 0 37 73 133)
									(== global104 2)
								)
								(Print 421 23) ; "Gee, button one says "Up" and one says "Down"."
							else
								(Print 421 24) ; "You're not close enough."
							)
						)
					)
				)
				((and (Said 'explore/android,junk,debris,body') (== local0 5))
					(if
						(and
							(== global104 2)
							(<= (gEgo distanceTo: global111) 30)
						...
						(Print 421 25) ; "The terminator is now a pile of junk on the first level platform."
					)
				)
				((or (Said 'press<up') (Said 'press/button,button<up'))
					(if (gEgo inRect: 39 123 62 133)
						(Print 421 26) ; "Nothing happens."
					else
				...
				((Said 'talk,talk/android')
					(if (gCast contains: global111)
						(Print 421 28) ; "I don't think he's interested in talking."
					else
						(Print 421 29) ; "You don't see anyone here to talk to."
					)
				)
				((Said 'board,board,pull,halt,hold/pulley,rope,rope,hook')
					(Print 421 30) ; "That wouldn't do much good."
				)
				((Said 'swing,get,press,use/pulley,block,rope,rope,hook')
					(switch local3
						(0
							(gEgo setScript: doPulley)
						)
						(2
							(Print 421 31) ; "There's no need for that now."
						)
						(else
							(Print 421 32) ; "Not now."
						)
					)
				)
				((Said 'get,get/pulley,banister,rope,rope')
					(Print 421 33) ; "There's no way you could do that."
				)
				((Said 'get,get/belt')
					(cond
						((gEgo has: 16) ; Invisibility_Belt
							(Print 421 34) ; "You already took it."
						)
						((not (IsItemAt 16 421)) ; Invisibility_Belt
							(Print 421 35) ; "You don't seen one here."
						)
				...
				((Said 'get,get/android')
					(if (== local0 5)
						(Print 421 37) ; "There is no way you could carry all that."
					else
						(Print 421 38) ; "No way! % Unm\94glich!"
					)
				)
				((Said 'get,get<in/elevator')
					(Print 421 39) ; "Just go there."
				)
				((Said 'kill/android')
					(Print 421 40) ; "That's nice in theory, but you'd need to find some way to put it into practice."
				)
				((Said 'use/orat,handle,stick')
					(Print 421 41) ; "That wouldn't help any."
				)
				((Said 'jump,jump')
					(Print 421 42) ; "You would have a better chance of surviving if you took the elevator."
				)
				((Said 'turn<off/engine')
					(Print 421 43) ; "You have no way of doing that."
				)
				((Said 'climb')
					(Print 421 44) ; "You really are not all that fond of heights."
				)
				((Said '/pulley,hook,rope,rope,cog,machine,engine')
					(Print 421 45) ; "That wouldn't accomplish much."
				)
			)
		)
		(if
...
(instance doPulley of Script
	...
	(method (doit)
		...
		(if
			...
			(= state 4)
			(RedrawCast)
			(Print 421 47) ; "Unfortunately, the pulley has come to the end of its track."
			(= local3 0)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(cond
					(
						...
						(Print 421 48) ; "It's too late now."
						(gEgo setScript: 0)
					)
					(
						(and
			...
			(1
				(Print 421 49 #at -1 (- (gEgo y:) 20)) ; "Reaching up, you give the rope pulley a mighty shove."
				(switch (gEgo loop:)
					(0
						(local2
							startUpd:
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(== local8 -200)
				(RedrawCast)
				(Print 421 50 #at 66 53 #dispose #title {Fester}) ; "Hey! What's going on here! Didn't you read the sign saying we're closed for repairs?"
				(= seconds 8)
			)
			(4
				(cls)
				(RedrawCast)
				(Print 421 51 #at 66 29 #dispose #title {Fester}) ; "Oh! I see you've gotten rid of that grease swilling android. Never did like that terminator series. Good riddance to bad circuits!"
				(= seconds 12)
			)
			(5
				(cls)
				(Print 421 52 #at 66 65 #dispose #title {Fester}) ; "Well, you might as well ride down with me."
				(gEgo ignoreControl: 4096)
				(= local1 2)
				(= seconds 4)
			)
			...
			(7
				(cls)
				(Print ; "Come on, come on! I don't have all day."
					421
					53
					#at
					66
