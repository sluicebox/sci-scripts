				...
				((Said 'look,look>')
					(cond
						((Said '[<around,at,in][/area]')
							(Print 42 0) ; "The desert to the north grows darker as storm clouds loom overhead. You find yourself at the base of a gigantic metal model of a great beast."
						)
						((Said '/feet,toe,leg')
							(Print 42 1) ; "Whoever built this thing did a thorough job. Right down to the polyfoam toejam."
						)
						((Said '/entrance,cavity')
							(Print 42 2) ; "You see an entrance to the inside of the foot near its north end."
						)
						((Said '/door,door')
							(Print 42 3) ; "The doorway leads to the interior of Mog."
						)
						((Said '/mog,beast')
							(Print 42 4) ; "Boy! It's a relief to know this baby is extinct."
						)
						((Said '/sign,sign')
							(Print 42 5) ; "There are two signs here."
						)
					)
				)
				((Said 'get,get,pull,hit/sign,sign,mog')
					(Print 42 6) ; "Leave it alone. %F La\e1 das!"
				)
				((Said 'read/sign,sign')
					(cond
						((& (gEgo onControl: 0) $2000)
							(if
								...
								(Print 42 7 #mode 1 #at -1 100) ; "Mog: one of many large beasts that once roamed this vast desert. But they all eventually died of boredom and are now extinct."
							else
								(Print 42 8) ; "Try facing in the right direction."
							)
						)
						((& (gEgo onControl: 0) $1000)
							(if (!= (gEgo loop:) 3)
								...
								(Print 42 9 #at -1 100) ; "See Phleebhut from Mog's Head Entrance Free. sorry.. temporarily closed for repair."
							)
						)
						(else
							(Print 42 10) ; "You're not close enough to read it."
						)
					)
				)
			)
