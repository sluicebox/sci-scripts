				...
				(cond
					((Said 'look,look>')
						(cond
							...
							((Said '/acme')
								(Print 3 2) ; "This one is another fine, but worthless, Acme product."
							)
							((Said '/jup,2')
							...
							((Said '[<around,at,in][/area]')
								(Print 3 12) ; "Various types of abandoned spacecraft litter the floor of this intergalatic junkyard. All this place needs is a junk yard dog. You shiver at the thought."
							)
						)
					)
					...
					(
						(Said
							'hoist,hoist,move,freeze,manipulate,press,pull/craft,craft,leech,bow,bow,tie,bore,acme,jup,2,(craft<flying)'
						)
					...
					(
						(Said
							'open,board,board,enter,(climb[<in,in])/craft,craft,bow,bow,tie,bore,acme,jup,2,(craft<flying),door'
						)
						(Print 3 16) ; "There is no visible means of entry."
					)
					((Said 'walk,climb[<on,up]/craft,craft')
						(Print 3 17) ; "You are not skilled in ship climbing."
					)
					((Said 'clean')
						(Print 3 18) ; "It's against your nature to do that."
					)
					((Said 'get,get,move,freeze,manipulate/box')
						(Print 3 19) ; "Don't waste your time with that stupid box."
					)
					(
						(Said
							'erect,erect,stand,stand,place,place,drop,lower,lower,use/ladder[/cavity[<in,in,in]]'
						)
					...
					((Said 'use,get,get,climb/junk,debris,scrap,iro')
...
(instance motivator of Act
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'look,look>')
				(cond
					...
					((Said '[<down,below,at][/dirt,deck]')
						(Print 3 40) ; "The floor is composed of a mosaic of overlapping welded steel plates. The menage of used metal plates appear to have been cut from a variety of sources. There is a small round object lying nearby."
					)
				)
			)
			(
			...
			((Said 'press,pull,remove,get,get/plug[<modular,8,eightprong]')
...
(instance jupiter of Feature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/craft,craft')
				(Print 3 44) ; "This ship says Jupiter 2. This baby must have been floating around out there for a long time. It doesn't hold your interest for very long."
			)
		)
	)
)
...
(instance bow of Feature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/craft,craft,bow,tie,bow')
				(Print 3 45) ; "This bulbous craft looks like it has seen a lot of action in its day. You believe it to be a bowtie fighter dating back to the cologne wars - a true relic."
			)
		)
	)
)
...
(instance bore of Feature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/craft,craft,bore,implement')
				(Print 3 46) ; "This looks like some type of tunnel boring implement."
			)
		)
	)
)
...
(instance acme of Feature
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/craft,craft')
				(Print 3 47) ; "This ship is another fine, but worthless, Acme product"
			)
		)
	)
)
