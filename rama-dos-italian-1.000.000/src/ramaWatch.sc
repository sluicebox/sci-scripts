...
(instance ramaWatch of RamaRoom
	...
	(method (init)
		...
		((= plane (Plane new:))
			init: 0 0 640 480
			priority: 255
			addCast: ramaWtchCast
		)
		(super init: &rest)
		(gCast add: ramaWtchCast)
		(myNormalCursor init:)
		(gTheCursor setNormalCursor: myNormalCursor)
		(night1Box init: ramaWtchCast)
		(night2Box init: ramaWtchCast)
		(night3Box init: ramaWtchCast)
		(theExit init: ramaWtchCast)
		(= local15 0)
		(= local16 0)
		(= local17 -1)
	)
	...
	(method (dispose)
		...
		(gCast delete: ramaWtchCast)
		(super dispose: &rest)
	)
)

(instance ramaWtchCast of Cast
	(properties)
)
