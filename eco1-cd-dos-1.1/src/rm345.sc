...
(instance rm345 of EcoRoom
	...
	(method (doVerb theVerb invItem)
		...
		(switch theVerb
			(1 ; Look
				(if (== local7 1)
					(EcoNarrator init: 3 0 0 41) ; "The driftnet that once spread death is now gone - and so is DELPHINEUS!"
				else
					(EcoNarrator init: 3 0 0 46) ; "Adam and Delphineus are trapped in a driftnet while Flesh-Eater circles them, gleefully toying with his dinner!"
				)
			)
			(else
...
(instance manta of EcoActor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 3 0 0 43) ; "The horrid monster, Flesh-Eater, only circles them as though gloating over his treasures."
			)
			(2 ; Talk
				(EcoNarrator init: 3 0 0 43) ; "The horrid monster, Flesh-Eater, only circles them as though gloating over his treasures."
			)
			(4 ; Do
				(EcoNarrator init: 3 0 0 44) ; "Adam can't do anything to the manta while he's tangled in the net."
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 44) ; "Adam can't do anything to the manta while he's tangled in the net."
			)
			(6 ; Recycle
				(EcoNarrator init: 3 0 0 51) ; "The Flesh-Eater problem won't be that easy to deal with!"
			)
			(else
...
(instance driftNet of EcoActor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(if local8
					(EcoNarrator init: 3 0 0 25) ; "Adam struggles against the rough nylon rope but he's too entangled to pull himself free. He'll have to cut the net."
				else
					(EcoNarrator init: 3 0 0 24) ; "Adam can't do anything to help Delphineus while he himself is caught in the net."
				)
				(^= local8 $0001)
			)
			(1 ; Look
				(if (== local3 0)
					(if local9
						(EcoNarrator init: 3 0 0 28) ; "The coarse nylon rubs roughly against Adam's skin. The nylon is a terrible prison."
					else
						(EcoNarrator init: 3 0 0 27) ; "Delphineus struggles madly in the net. He's clearly terrified and unable to free himself."
					)
					(^= local9 $0001)
				)
			)
			(44 ; Inventory
				(switch temp0
					...
					(37
						(EcoNarrator init: 3 0 0 26) ; "Adam can't wield that hacksaw while he's entangled - it's too big. Besides, the hacksaw is too dangerous to use so close to Adam's flesh."
					)
					(else
						(EcoNarrator init: 3 0 0 45) ; "That won't cut the nylon mesh!"
					)
				)
			)
			(2 ; Talk
...
(instance chaseSequence of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EcoNarrator init: 3 0 0 49) ; "Adam and Delphineus flee in terror from the huge, flesh-eating monster!"
			...
			(12
				(EcoNarrator init: 3 0 0 2) ; "With a feeling of utter helplessness, Adam prepares to feel the sharp bite of Flesh-Eater's jaws. The monster is so close that his hot wake ruffles the back of Adam's neck!"
			)
			(13
				(manta hide:)
				(localproc_1 5)
...
(instance crashIntoNet of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EcoNarrator posn: -1 130 init: 3 0 0 30 self) ; "In their panicked terror of the danger behind them, Adam and Delphineus fail to notice the danger in front of them. They both plow headfirst into the driftnet."
			)
			(5
			...
			(6
				(EcoNarrator posn: -1 130 init: 3 0 0 31 self) ; "The nylon mesh wraps its arms around them. The two are trapped!"
			)
			(7
				(gEgo setScript: trappedTalk self)
			)
			(8
				(EcoNarrator posn: -1 130 init: 3 0 0 32 self) ; "Adam waits for the manta to finish them off - two helpless victims trapped in the net. To his surprise, Flesh-Eater only circles them."
			)
			(9
...
(instance mantaCircles of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if (== local5 0)
					(EcoNarrator posn: -1 130 init: 3 0 0 33 self) ; "Flesh-Eater's toying with them!"
					(= local5 1)
				else
					(self cue:)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(HandsOff)
				(EcoNarrator posn: -1 130 init: 3 0 0 42 self) ; "Adam maneuvers the sharp shell around and begins to rub it against the nylon net."
			)
			(1
				(gEgo
					view: 342
			...
			(2
				(EcoNarrator posn: -1 130 init: 3 0 0 36 self) ; "The shell saws through the nylon, loosening the net's grasp on Adam. He's free!"
			)
			(3
				(if (manta script:)
					(if (driftNet script:)
			...
			(6
				...
				(EcoNarrator posn: -1 130 init: 3 0 0 37 self) ; "Adam turns back frantically to cut loose his friend..."
			)
			(7
...
(instance mantaCarriesOffDel of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(EcoNarrator posn: -1 130 init: 3 0 0 38 self) ; "...But before he can free Delphineus, Flesh-Eater swoops in! Enraged to see one of his victims escape and determined not to lose the other, he seizes the net with poor Delphineus still in it..."
			)
			(2
			...
			(3
				(EcoNarrator posn: -1 130 init: 3 0 0 47 self) ; "...And knocks Adam aside with one flick of his huge wing!"
			)
			(4
			...
			(14
				(EcoNarrator posn: -1 130 init: 3 0 0 39 self) ; "Adam finds himself suddenly alone. The driftnet with its precious cargo and the monster are gone. What on earth, Adam thinks, can he do now?"
			)
			(15
				(= local7 1)
				(EcoNarrator posn: -1 130 init: 3 0 0 40 self) ; "And will he ever see Delphineus again?"
			)
			(16
...
(instance emptyRoom of HandsOffScript
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(EcoNarrator init: 3 0 0 41 self) ; "The driftnet that once spread death is now gone - and so is DELPHINEUS!"
			)
			(2
