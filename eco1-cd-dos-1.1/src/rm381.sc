...
(instance rm381 of EcoRoom
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			(EcoNarrator posn: 160 20 init: 5 0 0 1 0 380) ; "The Lab has a specially designed room for treating stranded or injured animals."
		else
...
(instance labDoor of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		(if (not (IsFlag 70))
			...
			(EcoNarrator posn: 160 20 init: 5 0 0 69 0 380) ; "Adam must go inside to do that."
		)
	)
)
...
(instance outsideGateLever of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(44 ; Inventory
				(EcoNarrator posn: 160 20 init: 6 0 0 32 0 380) ; "The lever for the hatch is operated by hand. It's not necessary to use anything on the lever."
			)
			(1 ; Look
				(EcoNarrator init: 5 0 0 lookStr 0 380)
			)
...
(instance bucket of EcoFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				((ScriptID 2 1) init: 2 0 0 lookStr 1 0 380) ; Adam
			)
			(4 ; Do
				((ScriptID 2 1) init: 2 0 0 lookStr 1 0 380) ; Adam
			)
...
(instance boats of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator init: 5 0 0 2 0 380) ; "The Lab uses the speedboats as little as possible. The gasoline really hurts the reef."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance dock of EcoFeature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator init: 5 0 0 27 0 380) ; "The docks are in good repair."
			)
			(2 ; Talk
				(dolphin doVerb: 2)
			)
...
(instance dolphin of EcoActor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(2 ; Talk
				((ScriptID 2 0) init: 1 0 0 33 1 0 380) ; Delphineus, "Adam, hurry up, I'm really worried and need to get going!"
			)
			(44 ; Inventory
...
(instance tFrisbee of EcoView ; UNUSED
	(properties
		z 100
		view 380
		loop 5
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator posn: 160 20 init: 6 0 0 16 0 380) ; "Adam doesn't need that anymore. The dolphin has lost interest in frisbee."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance startUp of Script
	...
	(method (changeState newState &tmp [temp0 250])
		(switch (= state newState)
			...
			(1
				(Message msgGET 380 5 0 2 11 @temp0) ; "Days pass..."
				(= register (Display @temp0 dsCOORD 135 85 dsCOLOR global207 dsFONT 6 dsSAVEPIXELS))
				(= seconds 7)
			)
			(2
			...
			(14
				((ScriptID 2 1) init: 2 0 0 11 0 self 380) ; Adam, "Delphineus! I'm so glad to see you! I've been wondering what happened to you all week."
			)
			(15
				((ScriptID 2 1) init: 2 0 0 12 1 self 380) ; Adam, "Did you find your missing king?"
			)
			(16
				((ScriptID 2 0) init: 1 0 0 17 1 self 380) ; Delphineus, "No, things are worse than ever...Gotta catch my breath."
			)
			(17
				((ScriptID 2 1) init: 2 0 0 13 1 self 380) ; Adam, "Slow down. Take your time. What do you mean?"
			)
			(18
				((ScriptID 2 0) init: 1 0 0 18 1 self 380) ; Delphineus, "Strange things are happening in the kingdom, oh, it's too hard to explain! If only you could see it!"
			)
			(19
				((ScriptID 2 1) init: 2 0 0 14 1 self 380) ; Adam, "Well, I could see it. I'll just go back there with you!"
			)
			(20
				((ScriptID 2 0) init: 1 0 0 19 1 self 380) ; Delphineus, "You! A human? You must be joking! Human lungs could never last where we'd be going!"
			)
			(21
				(if (gEgo has: 19) ; airTanks
					((ScriptID 2 1) init: 2 0 0 15 1 self 380) ; Adam, "I could so! I'm checked out on all the new diving equipment. Say it's O.K., c'mon!"
				else
					((ScriptID 2 0) init: 1 0 0 21 1 self 380) ; Delphineus, "You'll have to get ready, Adam, before I could even consider it!"
				)
			)
			(22
				(if (gEgo has: 19) ; airTanks
					((ScriptID 2 0) init: 1 0 0 20 1 self 380) ; Delphineus, "Hmmm, well, they did say 'Bring help!' Okay, I'll take the responsibility. Come on!"
				else
					(= ticks 3)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(insideGate init: stopUpd:)
				(gCurRoom drawPic: 380)
				(gEgo init: get: 17 posn: 334 141) ; scubaGear
				(dolphin
					view: 382
					loop: 6
					init:
...
(instance leaveRoom of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(EcoNarrator init: 6 0 0 18 self 380) ; "Adam empties his pockets."
			)
			(2
...
(instance getGearOn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(EcoNarrator init: 6 0 0 30 self 380) ; "He puts on his fins, gloves, and mask and jumps into the water."
			)
			(10
				(self dispose:)
			)
