...
(instance rm223 of ApartmentRoom
	...
	(method (init)
		...
		(NormalEgo)
		(gEgo setPri: 8)
		(if (== ((Inv at: 34) owner:) 223) ; sixPackRing
			(rings init: cel: 14 setPri: 0 stopUpd:)
		)
		(if (gEgo has: 6) ; tweezers
			(tools init:)
		)
		(if (not (IsFlag 50))
			(swordFish init:)
...
(instance firstTalk of Script
	...
	(method (changeState newState &tmp [temp0 500])
		(switch (= state newState)
			...
			(4
				(Hippocrates init: 1 0 0 2 1 self) ; "I wath digging for food, like I alwayths do. Thees abominathion wath under the thand."
				(= cycles 10)
			)
			(5
				(bubble dispose:)
...
(instance useShell of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				((Inv at: 34) owner: 223) ; sixPackRing
			...
			(6
				(Hippocrates init: 1 0 0 5 0 self) ; "Ah! What blessed relief! I was afraid I was a bit doomed, good fellow, but your speedy and accurate surgical techniques were my salvation!"
			)
			(7
			...
			(10
				(Hippocrates init: 1 0 0 11 1 self) ; "Do you need a check-up, lad? I must confess I know distressingly little about human anatomy, but you're simple creatures - it can't be that difficult."
			)
			(11
			...
			(20
				(SetScore 10 288)
				(gEgo get: 6) ; tweezers
				(EcoNarrator init: 3 0 0 63 self) ; "Adam catches the fish-bone tweezers."
			)
			(21
				((ScriptID 2 1) init: 2 0 0 19 1 self) ; Adam, "Thank you, Hippocrates!"
			)
			(22
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(if (== register 4)
					(gEgo get: 34) ; sixPackRing
				else
					((Inv at: 34) owner: 0) ; sixPackRing
				)
			)
			(5
				(if (== register 6)
					(EcoNarrator init: 3 0 0 3 self) ; "Adam picks up the plastic and puts it in his garbage bag. For a moment, he has the suspicion that there's something he's forgotten to do with the rings. Oh well, too late now."
				else
					(EcoNarrator init: 3 0 0 68 self) ; "Adam picks up the six-pack rings."
				)
			)
			(6
				(NormalEgo 0)
...
(instance swordFish of EcoActor
	...
	(method (init)
		...
		(self
			...
			ignoreActors:
			setLoop: Grooper
			approachVerbs: 4 9 1 2 ; Do, ???, Look, Talk
			setScript: stuckShut
		)
	)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (not (gEgo has: 6)) ; tweezers
					(EcoNarrator init: 3 0 0 54) ; "The swordfish has gotten his snout trapped in the plastic rings from a six-pack of soda. The plastic is tight and he can't open his mouth enough to eat."
				)
			)
			(4 ; Do
				(EcoNarrator init: 3 0 0 55) ; "The plastic is wrapped tightly around the swordfish's snout. Adam can't pull it off with his hands."
			)
			(44 ; Inventory
				(switch temp0
					(17
					...
					(12
						(EcoNarrator init: 3 0 0 65) ; "The healing potion won't help get those rings off the swordfish's snout."
					)
					(16
						(EcoNarrator init: 3 0 0 66) ; "The sea urchins can't do anything for the poor swordfish."
					)
					(else
						(EcoNarrator init: 3 0 0 56) ; "That won't help Adam get the rings off of the swordfish's snout."
					)
				)
			)
			(2 ; Talk
	...
	(method (cue)
		(if (and (SetFlag 38) (not ((Inv at: 34) owner:))) ; sixPackRing
			(Hippocrates init: 1 0 0 3 1) ; "I'm thorry, but ith too difficult to thalk right now."
		else
...
(instance rings of EcoProp
	...
	(method (init)
		(super init: &rest)
		(self approachVerbs: 4 6) ; Do, Recycle
		(&= signal $efff)
	)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 3 0 0 67) ; "The six-pack rings that were holding Hippocrates' snout closed are now lying on the floor."
			)
			(4 ; Do
			...
			(44 ; Inventory
				(switch temp0
					(17
						(EcoNarrator init: 3 0 0 69) ; "Good idea! Pick the rings up first."
					)
					(else
						(EcoNarrator init: 3 0 0 70) ; "Adam can't do that to the six-pack rings!"
					)
				)
			)
			(else
...
(instance golfClubs of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 3 0 0 57) ; "Golf clubs? The swordfish is, apparently, just like the doctors back home."
			)
			(4 ; Do
				(EcoNarrator init: 3 0 0 16) ; "Adam can't imagine how a person could play golf under the water, and, anyway, he'd rather be diving... and he is!"
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
			)
			(6 ; Recycle
				(EcoNarrator init: 3 0 0 72) ; "The swordfish doctor might not have much, but what he has he wants to keep, thank you very much."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance bookCase of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 3 0 0 58) ; "The bookcase contains `Fishbein's Medical Encyclopedia.'"
			)
			(4 ; Do
				(EcoNarrator init: 3 0 0 15) ; "Yuck! Fishy medical books! No thanks!"
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
			)
...
(instance wallplaque of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 3 0 0 71) ; "The shield is a bit pretentious for Adam's tastes."
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
			)
...
(instance cert of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 3 0 0 17) ; "If Adam wants a diploma in medicine, he'll have to earn one himself!"
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
			)
...
(instance cabinet of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 3 0 0 60) ; "The table holds the swordfish Doctor's medical supplies."
			)
			(4 ; Do
				(if (IsFlag 50)
					(EcoNarrator init: 3 0 0 13) ; "Adam doesn't want to take anything from the swordfish's apartment that wasn't given to him."
				else
					(EcoNarrator init: 3 0 0 64) ; "Unfortunately, there doesn't seem to be anything on the table that would help the swordfish out of his predicament."
				)
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
...
(instance sculpture of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 3 0 0 18) ; "Nah. Adam has a REAL dolphin to play with."
			)
			(44 ; Inventory
				(EcoNarrator init: 3 0 0 73) ; "Adam doesn't need to mess with the doctor's personal belongings."
			)
...
(instance Hippocrates of EcoTalker
	...
	(method (init)
		(if (= cel (!= ((Inv at: 34) owner:) 223)) ; sixPackRing
