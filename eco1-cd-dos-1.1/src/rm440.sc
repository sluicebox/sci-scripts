		...
		(switch theVerb
			(44 ; Inventory
				(if
					...
					(EcoNarrator init: 3 0 0 30) ; "Even if he has swallowed the key, the red fish doesn't deserve THAT."
				else
					(super doVerb: theVerb temp0 &rest)
				)
			)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 91)
					...
					(EcoNarrator init: 3 0 0 10) ; "The key has fallen to the sand here."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
...
(instance clownFish of EcoActor
	...
	(method (init)
		...
		(self approachVerbs: 4 1 9 2) ; Do, Look, ???, Talk
	)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(44 ; Inventory
				(if (OneOf temp0 20 8 28)
					(EcoNarrator init: 3 0 0 45) ; "Adam has no reason to do that."
				else
					(super doVerb: theVerb temp0 &rest)
				)
			)
			(6 ; Recycle
				(EcoNarrator init: 3 0 0 46) ; "The clown fish lives among the anemone's tendrils. It shouldn't be moved."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance bigAnemoneTop of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		(bigAnemoneBottom doVerb: theVerb temp0 &rest) ; UNINIT
	)
)
		...
		(switch theVerb
			(4 ; Do
				(cond
					...
					((IsFlag 21)
						(EcoNarrator init: 3 0 0 19) ; "Some anemones have a venom that stings humans. Even with gloves, Adam shouldn't touch it."
					)
					((IsFlag 91)
			...
			(1 ; Look
				(cond
					...
					(else
						(EcoNarrator init: 3 0 0 18) ; "The anemone heaves with discomfort. Clearly, he's bitten off more than he can chew."
					)
				)
			)
			(44 ; Inventory
				(switch temp0
					(36
						(if (IsFlag 21)
							...
							(EcoNarrator init: 3 0 0 33) ; "The lure attracts the anemone, but it is happily digesting something else right now."
						)
					)
					(29
						(EcoNarrator init: 3 0 0 12) ; "The trident would not only puncture his vanity, it would also put some serious holes in his pride."
					)
					(37
						(if (IsFlag 21)
							(EcoNarrator init: 3 0 0 40) ; "Cutting the anemone open would get the key - but it would also get the anemone."
						else
							(EcoNarrator init: 3 0 0 41) ; "Adam is curious about the anemones, but cutting them open isn't the answer."
						)
					)
					(17
						(if (IsFlag 21)
							(EcoNarrator init: 3 0 0 43) ; "Cutting open the anemone isn't a good idea."
						else
							(EcoNarrator init: 3 0 0 42) ; "The sharp shell would injure the anemones."
						)
					)
					(12
						(if (IsFlag 21)
							(EcoNarrator init: 3 0 0 52) ; "That won't cure the anemone's indigestion."
						else
							(super doVerb: theVerb temp0 &rest)
						)
					)
					(else
						(cond
							((IsFlag 21)
								(EcoNarrator init: 3 0 0 35) ; "That just seems to make the anemone feel worse."
							)
							((IsFlag 91)
								(EcoNarrator init: 3 0 0 23) ; "The anemone should be left in peace now."
							)
							(else
								(EcoNarrator init: 3 0 0 34) ; "That won't help the anemones."
							)
						)
					)
				)
			...
			(2 ; Talk
				(cond
					...
					((IsFlag 91)
						(EcoNarrator init: 3 0 0 32) ; "The anemone has said all it's going to say."
					)
					(else
			...
			(6 ; Recycle
				(cond
					...
					((IsFlag 21)
						(EcoNarrator init: 3 0 0 20) ; "The anemone's stomach is like a garbage bag, but that doesn't mean he wants to be in one."
					)
					(else
						(EcoNarrator init: 3 0 0 38) ; "Anemones are animals. As such, they don't want to be bagged."
					)
				)
			)
			(else
...
(instance blackSeaweed of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 3 0 0 48) ; "The weeds are fine where they are."
			)
			(6 ; Recycle
				(EcoNarrator init: 3 0 0 49) ; "The weeds are living. They don't need to be thrown out."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance fishSkeleton of EcoProp ; UNUSED
	(properties
		x 135
		y 160
		approachX 120
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 9) ; "A small opening with just enough room for one person to squeeze through...before lunch that is."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
		...
		(switch theVerb
			(4 ; Do
				(cond
					...
					((IsFlag 91)
						(EcoNarrator init: 3 0 0 29) ; "Adam remembers that anemones can sting their prey and draws back."
					)
					((not (IsFlag 21))
						(if local1
							((ScriptID 2 0) init: 2 0 0 9 1) ; Delphineus, "Aaah! Don't touch! Some anemones have a toxin that stings humans. They catch their prey by paralyzing them with the toxin first."
					...
					(else
						(EcoNarrator init: 3 0 0 27) ; "They can't do anything about this."
					)
				)
			)
			(1 ; Look
				(cond
					...
					((IsFlag 91)
						(EcoNarrator init: 3 0 0 28) ; "The anemones wave their tendrils."
					)
					((not (IsFlag 21))
						(EcoNarrator init: 3 0 0 16) ; "A cluster of anemones waves its tendrils. Unwary fish who swim through will be swallowed!"
					)
					(else
						(EcoNarrator init: 3 0 0 26) ; "No doubt the other anemones are jealous... On second, thought, maybe not."
					)
				)
			)
			(2 ; Talk
				(cond
					...
					((or (IsFlag 21) (IsFlag 91))
						(EcoNarrator init: 3 0 0 31) ; "The anemones are speechless."
					)
					(else
						((ScriptID 2 0) init: 2 0 0 18 1) ; Delphineus, "Anemones use their mouths only for eating, not for talking."
					)
...
(instance Anemones2 of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		(Anemones1 doVerb: theVerb temp0 &rest) ; UNINIT
	)
)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 5) ; "The crown-o'-thorns starfish chews up the coral. If their population gets too big, they can do in a whole reef."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
		...
		(switch theVerb
			(4 ; Do
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 50) ; "Careful! Adam can get a nasty infection if one of those sea urchin spines punctures his skin."
				)
			)
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 2) ; "Sea urchins eat algae - and that encourages the coral to grow more. The spines have fallen off this skeleton."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
		...
		(switch theVerb
			(4 ; Do
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 51) ; "Despite its neat-o name, Adam decides not to gather the sea plume for a pirate hat."
				)
			)
			(1 ; Look
				(if (IsFlag 73)
		...
		(switch theVerb
			(4 ; Do
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 24) ; "Sea stars can grow new arms. Still, it doesn't do to pry one up."
				)
			)
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 8) ; "Sea stars can open a whole clam with their suction cups."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 3) ; "Redbeard sponge looks soft and, well, spongy."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 4) ; "The tree coral looks a lot like regular you-know-whats."
				)
			)
			(2 ; Talk
				(if (IsFlag 73)
		...
		(switch theVerb
			(4 ; Do
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 11) ; "Adam doesn't have any use for a handful of sand."
				)
			)
			(1 ; Look
				(if (IsFlag 73)
					...
					(EcoNarrator init: 3 0 0 1) ; "The ground-up coral makes beautiful, white sand."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
...
(instance chaseFish of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EcoNarrator posn: -1 130 init: 3 0 0 17 self) ; "Out of nowhere a lurking fish darts forward."
			)
			(3
...
(instance dangleHeavePuke of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EcoNarrator posn: -1 25 init: 3 0 0 21 self) ; "Adam moves the bright fishing lure in front of the anemone."
			)
			(3
				(= ticks 30)
			)
			(4
				(gEgo put: 27 0) ; fishLure
				(SetFlag 91)
				(bigAnemoneTop
					view: 442
					loop: 1
...
(instance egoGetsKey of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EcoNarrator init: 3 0 0 14 self) ; "Adam picks up the key and tries not to think about where it's been."
			)
			(2
				(gEgo get: 24) ; boxKey
