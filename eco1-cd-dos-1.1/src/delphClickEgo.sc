	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (== gPrevRoomNum 400)
					((ScriptID 2 1) init: 2 0 0 44 1 self) ; Adam, "You've had a tough time, but we're not trying to keep you or anything. We're just trying to help you get better."
				else
			...
			(2
				(if (== gPrevRoomNum 400)
					(EcoNarrator posn: 160 20 init: 5 0 0 33 self) ; "Adam's soothing voice got through to him. It looks like he's decided to make friends."
				else
					(EcoNarrator posn: 160 20 init: 6 0 0 20 self) ; "The dolphin suddenly focuses on Adam. Adam's voice must have gotten through to him!"
				)
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(43
				(SetScore 5 240)
				(HandsOff)
				((ScriptID 2 1) init: 3 0 0 20 1 self) ; Adam, "Whoa, I accept!"
			)
			(44
				(EcoNarrator posn: 160 20 init: 6 0 0 7 self) ; "Adam cups his hand and the dolphin swims up under it and pulls Adam into a whirling, swooping ride around the pool."
			)
			(45
