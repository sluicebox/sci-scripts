					...
					(switch (= local0 (proc140_19 event))
						...
						(0
							(if
								...
								(EcoNarrator init: 4 0 0 52 0 140) ; "Adam is swimming in the Temple of Poseidon."
							else
								(EcoNarrator init: 4 0 0 9 0 140) ; "The mosaic tile picture on the wall is now unscrambled. Strange symbols appear in the tile's design."
							)
						)
					)
					(= state 2)
		...
		(switch (= state newState)
			...
			(3
				...
				(if (& gMsgType $0002)
					(= temp500 gMsgType)
					(= gMsgType 2)
					((ScriptID 140 9) init: 2 0 0 (* global118 4) 0 0 140) ; Oracle
					(= gMsgType temp500)
				)
			)
			(4
			...
			(5
				...
				((ScriptID 140 9) init: 2 0 0 5 1 self 140) ; Oracle, "The riddle you have answered true. A vision will I grant to you."
			)
			(6
				(switch global118
					(1
					...
					(2
						((ScriptID 140 9) init: 2 0 0 9 0 self 140) ; Oracle, "And so an innocent must fall, And become the stalker of us all. Mutated by a glowing breeze, Man makes a murderer of the seas."
					)
					(3
