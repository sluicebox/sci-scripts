...
(procedure (localproc_3 &tmp temp0)
	...
	(cond
		...
		((not (IsFlag 4))
			(EcoNarrator init: 1 0 0 3) ; "Adam has no reason to talk to the empty room."
		)
		((and (not (IsFlag 2)) (IsFlag 3))
			(EcoNarrator init: 1 0 0 15) ; "Adam calls out to the disembodied voice, but gets no reply. HE must still be pondering."
		)
		(else
			(= temp0 0)
		)
	...
	(method (moveColumn &tmp temp0)
		(if (and (not (IsFlag 2)) (not locked))
			...
			(if (and (self lock:) (not (self checkAlignment: 1)))
				(if (not (SetFlag 119))
					(EcoNarrator init: 1 0 0 40 caller) ; "The column piece locks into place. It now matches the corresponding column piece on the right side of the Council Chambers."
				else
					(EcoNarrator init: 1 0 0 30 caller) ; "The column piece locks into place."
				)
			)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(cond
					((IsFlag 2)
						(EcoNarrator init: 1 0 0 32) ; "The columns on the left of the Council Chambers now match the columns on the right."
					)
					(locked
						(EcoNarrator init: 1 0 0 36) ; "That piece has locked into place and now matches it's corresponding piece in the columns on the opposite wall."
					)
					((IsFlag 3)
						(gCurRoom setScript: doRiddle)
					)
					(else
						(EcoNarrator init: 1 0 0 4) ; "Six beautiful columns line the Council Chambers."
					)
				)
			)
			(4 ; Do
				(cond
					((not (IsFlag 3))
						(EcoNarrator init: 1 0 0 2) ; "The columns interest Adam, but he's not sure what to do with them."
					)
					((IsFlag 2)
						(EcoNarrator init: 1 0 0 37) ; "The columns are now perfectly aligned. Adam doesn't want to mess with perfection."
					)
					(locked
						(EcoNarrator init: 1 0 0 5) ; "That piece is already locked in its correct position."
					)
			...
			(44 ; Inventory
				(cond
					((not (IsFlag 3))
						(EcoNarrator init: 1 0 0 52) ; "Adam can't do anything with the columns, at least not now."
					)
					((IsFlag 2)
						(EcoNarrator init: 1 0 0 52) ; "Adam can't do anything with the columns, at least not now."
					)
					(else
						(EcoNarrator init: 1 0 0 53) ; "That won't help Adam solve the challenge of the columns."
					)
				)
			)
			(6 ; Recycle
				(EcoNarrator init: 1 0 0 51) ; "The Council Chamber columns have great symbolic meaning and are hardly garbage bag material."
			)
			(else
...
(instance rm160 of EcoRoom
	...
	(method (init)
		...
		(= picture (if (== gPrevRoomNum 200) 901 else 160))
		...
		(if (!= gPrevRoomNum 200)
			(cond
				...
				((and (not (gEgo has: 2)) (!= ((Inv at: 2) owner:) 140)) ; goldMask, goldMask
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(6 ; Recycle
				(EcoNarrator init: 1 0 0 48) ; "There is no garbage in the Council Chambers."
			)
			(else
...
(instance thankYou of Script
	...
	(method (changeState newState &tmp [temp0 40])
		(switch (= state newState)
			...
			(1
				(Message msgGET 160 1 0 1 25 @temp0) ; "Sometime later..."
				(Display @temp0 dsCOORD 105 65 dsCOLOR global207 dsFONT 6)
				(= seconds 5)
			)
			(2
			...
			(18
				(SetScore 2 268)
				(NormalEgo)
				(gEgo get: 2 ignoreHorizon: 1) ; goldMask
				(= cycles 1)
			)
			(19
...
(instance maskFall of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(EcoNarrator init: 1 0 0 31 self) ; "The last column piece shifts into place. Adam feels the water tremble..."
			)
			(1
			...
			(2
				(EcoNarrator init: 1 0 0 34 self) ; "The room shakes as the building settles down on the columns! The gold mask is knocked from the wall!"
			)
			(3
				(Face gEgo goldMask)
			...
			(7
				(EcoNarrator init: 1 0 0 35 self) ; "A dazed hermit crab stumbles from the fallen mask."
			)
			(8
				(HandsOn)
				(crab setScript: crabCry)
...
(instance whoDares of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 2 0 0 1 self gCurRoomNum) ; "HALT! Who DARES enter the Chambers of the GREAT SENATOR?"
			)
			(1
				(EcoNarrator init: 1 0 0 29) ; "The booming voice seems to come from nowhere."
				(= seconds 10)
			)
			(2
				(EcoNarrator init: 2 0 0 2 self gCurRoomNum) ; "I SAID, who DARES enter the Chambers of the GREAT SENATOR?"
			)
			(3
				(= seconds 20)
			)
...
(instance senatorSpeech of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EcoNarrator init: 2 0 0 3 self) ; "I AM ME. Which is to say <Ahem>, the GREAT SENATOR. Tremble before me."
			)
			(5
				((ScriptID 2 1) init: 3 0 0 2 1 self) ; Adam, "Oh. Well, okay. I'm Adam. Delphineus brought me here to help."
			)
			(6
				(EcoNarrator init: 2 0 0 4 self) ; "Adam? I know of no Elurians named Adam... <EEK!> You're HUMAN! A SON OF MAN? HERE? Where is that blasted blubber farm of a whale?!"
			)
			(7
				((ScriptID 2 1) init: 3 0 0 3 1 self) ; Adam, "What whale? You mean Cetus?"
			)
			(8
				(EcoNarrator init: 2 0 0 5 self) ; "Of course, Cetus! But I didn't say `blasted blubber farm of a whale'. Not me! I never said that <gulp>. `Benevolent, impressively large whale' is what I actually said."
			)
			(9
				(EcoNarrator init: 2 0 0 31 self) ; "Great Cetus maketh the food plentiful and the city pure. He banisheth the sharks and conger aliketh, he maketh the water clear and filtereth the yuckey stuff. Oh, and he maketh us cleanseth behind our gills."
			)
			(10
				((ScriptID 2 1) init: 3 0 0 4 1 self) ; Adam, "But Cetus is missing."
			)
			(11
				(EcoNarrator init: 2 0 0 6 self) ; "I KNOW THAT!!! Alas, poor Eluria! Cetus has vanished. For lo many days, Cetus is gone. Only Poseidon knows where he has hithered to."
			)
			(12
				(EcoNarrator init: 2 0 0 32 self) ; "But fear not, for the GREAT SENATOR will personally see to it that Eluria, great kingdom, survives."
			)
			(13
			...
			(14
				(EcoNarrator init: 2 0 0 7 self) ; "Help? You? The GREAT SENATOR needs no help. The GREAT SENATOR has not acted because HE is still pondering the situation. If YOU'RE so smart, try solving the challenge of the columns!"
			)
			(15
				((ScriptID 2 1) init: 3 0 0 44 1 self) ; Adam, "What challenge of the columns?"
			)
			(16
				(EcoNarrator init: 2 0 0 36 self) ; "Find the one that turns no other, Make it match its right-hand brother. Repeat until all nine are done, And the Council's favor you'll have won."
			)
			(17
				(EcoNarrator init: 2 0 0 33 self) ; "And now I will answer no more questions, human boy. The GREAT SENATOR will now stay silent and silently ponder."
			)
			(18
				(SetFlag 3)
				(HandsOn)
...
(instance getInCleanShell of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(26
				(SetScore 5 267)
				(gEgo put: 22 160 get: 1) ; hermetShell, starFish
				(Superfluous init: 2 0 0 30 1 self) ; "Here. Here's the badge. Now off with ya, son, and don't let me down!"
			)
			(27
				(EcoNarrator posn: -1 15 init: 1 0 0 1 self) ; "Adam takes the deputy badge from Superfluous. Why... it's a starfish!"
			)
			(28
				(gSoundEffects number: 163 loop: 1 play:)
				(crab
...
(instance muralLook of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 28 self) ; "A mural of Greek Senators highlights the back wall of the Council Chambers."
			)
			(1
				(if (not (IsFlag 2))
					(EcoNarrator init: 1 0 0 8 self) ; "The council leader's face is covered by an elaborate gold mask. It looks out of place on the painted mural."
				else
					(= cycles 1)
				)
			)
...
(instance handOnMask of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (and (IsFlag 2) (not (gEgo has: 2))) ; goldMask
					(cond
						((IsFlag 13)
							(EcoNarrator init: 1 0 0 27 self) ; "Adam doesn't want to disturb Superfluous' mask. He might ask Superfluous to come out, though."
						)
						(
							(and
								(not (IsFlag 13))
							...
							(EcoNarrator init: 1 0 0 38 self) ; "That mask belongs to the hermit crab. If Adam wants it, he'll have to earn it."
						)
					)
				else
					(EcoNarrator init: 1 0 0 14 self) ; "While out of place on the mural, the mask is firmly held down. It's not going anywhere."
				)
			)
			(4
				(HandsOn)
...
(instance noExit of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(EcoNarrator init: 1 0 0 43 self) ; "Adam can leave the Council Chambers the way he came in."
			)
			(1
				(gEgo
...
(instance lookInRoom of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 12 self) ; "This is a Greek Tholos, a round building in which the Greek Senators met to hold council. Alas, despite the sad state of the city, there's nobody meeting here now."
			)
			(1
				(if (not (IsFlag 2))
					(EcoNarrator init: 1 0 0 13 self) ; "The room seems empty."
				else
					(= cycles 1)
				)
			)
...
(instance doRiddle of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(EcoNarrator init: 1 0 0 57 self) ; "Adam thinks about the Great Senator's column challenge:"
			)
			(1
				(EcoNarrator init: 2 0 0 36 self) ; "Find the one that turns no other, Make it match its right-hand brother. Repeat until all nine are done, And the Council's favor you'll have won."
				(= cycles 1)
		...
		(switch theVerb
			(1 ; Look
				(if (not (IsFlag 3))
					(switch cel
						(0
							(EcoNarrator store: 19)
						)
						(1
							(EcoNarrator store: 18)
						)
						(2
							(EcoNarrator store: 17)
						)
					)
					(EcoNarrator init: 1 0 0 4) ; "Six beautiful columns line the Council Chambers."
				else
				...
				(if (IsFlag 3)
					(if (< ((User curEvent:) x:) 160)
						(if (IsFlag 2)
							...
							(EcoNarrator init: 1 0 0 62) ; "The column's top and base do not move."
						)
					else
						(EcoNarrator init: 1 0 0 46) ; "The columns on the right side of the Council Chambers are already aligned and Adam doesn't need to move them."
					)
				else
					(EcoNarrator init: 1 0 0 2) ; "The columns interest Adam, but he's not sure what to do with them."
				)
...
(instance senatorMural of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator init: 1 0 0 44) ; "The mural of the Senators is interesting to look at, but Adam can't do anything with it."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 44) ; "The mural of the Senators is interesting to look at, but Adam can't do anything with it."
			)
...
(instance senatorLook of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 9 self) ; "The seated Senator must have been the head of the council."
			)
			(1
				(if (not (IsFlag 2))
					(EcoNarrator init: 1 0 0 10 self) ; "A gold mask looks out of place on his face."
				else
					(= cycles 1)
				)
			)
...
(instance opening of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 42) ; "The ceiling of the Council Chambers is open to the sea as it once was open to the night sky."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 43) ; "Adam can leave the Council Chambers the way he came in."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance floor of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 41) ; "The Council Chambers floor is decorated with a large inlaid star."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 45) ; "There's nothing to do with the floor. Adam can't break dance under the ocean - not enough gravity."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 54) ; "Adam doesn't need to mess with the floor of the Council Chambers."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
		...
		(if
			...
			(cond
				(
					...
					(EcoNarrator init: 1 0 0 58) ; "If Adam wants to talk to someone, he can talk to the hermit crab."
					(event claimed: 1)
				)
				(
					(and
					...
					(EcoNarrator init: 1 0 0 60) ; "Adam might want to ask the hermit crab what he needs before he starts giving things to him."
					(event claimed: 1)
				)
				...
				(
					...
					(EcoNarrator init: 1 0 0 39) ; "There's no one there to give the shell to."
					(event claimed: 1)
				)
				(
					(and
					...
					(EcoNarrator init: 1 0 0 59) ; "Adam doesn't need to mess with the hermit crab's mask right now."
					(event claimed: 1)
				)
				(else
					(super handleEvent: event)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 2)
					(if (IsFlag 12)
						(EcoNarrator init: 1 0 0 26) ; "On the floor is the gold mask that once covered the Great Senator's face. The mask is made of beaten gold and is fit for a king."
					else
						(EcoNarrator init: 1 0 0 20) ; "Wow! The gold mask fell right off of the mural! It must have happened when the columns set into place and the room shifted!"
						(SetFlag 12)
					)
				else
					(EcoNarrator init: 1 0 0 11) ; "An elaborate gold mask has been placed over the seated Senator's face."
				)
			)
			(4 ; Do
			...
			(2 ; Talk
				(cond
					...
					((IsFlag 2)
						(EcoNarrator init: 1 0 0 49) ; "The gold mask can't talk!"
					)
				)
			)
			(6 ; Recycle
				(EcoNarrator init: 1 0 0 47) ; "The gold mask looks much too important and valuable to throw away!"
			)
			(44 ; Inventory
				(switch temp0
					(31
						(if (IsFlag 2)
							...
							(EcoNarrator init: 1 0 0 50) ; "Adam doesn't see anyone there to give the shell to."
						)
					)
					(else
						(if (IsFlag 2)
							(if (IsFlag 13)
								(EcoNarrator init: 1 0 0 27) ; "Adam doesn't want to disturb Superfluous' mask. He might ask Superfluous to come out, though."
							else
								(EcoNarrator init: 1 0 0 38) ; "That mask belongs to the hermit crab. If Adam wants it, he'll have to earn it."
							)
						else
							(EcoNarrator init: 1 0 0 16) ; "That won't get the gold mask off of the wall."
						)
					)
				)
			)
...
(instance crab of EcoActor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 11)
					(EcoNarrator init: 1 0 0 24) ; "Superfluous is a hermit crab. Hermit crabs move from shell to shell as they outgrow their old ones (like getting a new pair of shoes)!"
				else
					(EcoNarrator init: 1 0 0 21) ; "Hmmm. It's a little hermit crab. He must have been inside the mask when it fell. He appears quite upset."
				)
			)
			(4 ; Do
				(if (IsFlag 11)
					(EcoNarrator init: 1 0 0 56) ; "The Mayor is too dignified to be toyed with."
				else
					(EcoNarrator init: 1 0 0 22) ; "Maybe Adam should talk to him instead - the hermit crab might not like being picked up."
				)
			)
			(2 ; Talk
				(cond
				...
				(switch temp0
					...
					(else
						(if (not (IsFlag 11))
							...
							(EcoNarrator init: 1 0 0 23) ; "That won't help the hermit crab."
						)
					)
				)
			)
