...
(instance rm460 of EcoRoom
	...
	(method (init)
		...
		(if ((Inv at: 24) ownedBy: 460) ; boxKey
			(SetFlag 18)
			(= local0 0)
			(ppike init:)
			(= local2 1)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 2) ; "Adam rises into a small air pocket underneath the reef. Somebody found it before he did. In fact, the guy is still here."
			)
			(else
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(1 ; Look
				...
				(EcoNarrator init: 1 0 0 11) ; "It's a Conquistador's pike. Someone made sure the pirate on duty would get the point."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(1 ; Look
				...
				(if (IsFlag 41)
					(EcoNarrator init: 1 0 0 6) ; "The chest was empty except for the crab. Someone else has been here recently - unless, of course, you believe in hundred-year-old crabs."
				else
			...
			(44 ; Inventory
				(switch temp0
					...
					(37
						(EcoNarrator init: 1 0 0 27) ; "No need to do major surgery. The lock is already broken."
					)
					(else
						(EcoNarrator init: 1 0 0 28) ; "The lock has been snapped. The chest should open easily."
					)
				)
			)
			(2 ; Talk
				(EcoNarrator init: 1 0 0 31) ; "The chest gave up its secrets already."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (and (== local0 2) local2)
					...
					(EcoNarrator init: 1 0 0 15) ; "The water laps against the beach."
				)
			)
			(else
				(super doVerb: theVerb temp0 &rest)
...
(instance KeyInset of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(1 ; Look
				(EcoNarrator init: 1 0 0 24) ; "No doubt this is a skeleton key."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance Skeleton of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 1 0 0 18) ; "Adam feels creepy about touching a skeleton."
			)
			(2 ; Talk
				(EcoNarrator init: 1 0 0 29) ; ""Are you by any chance related to a guy named LeChuck?"... No answer from the skeleton."
			)
			(1 ; Look
				(if (== local0 2)
					(EcoNarrator init: 1 0 0 4) ; "The pike is gone. The pirate doesn't feel any better however."
				else
					(EcoNarrator init: 1 0 0 3) ; "The pirate ghost has the job of guarding the treasure. The pirate chief made sure he wouldn't quit."
				)
			)
			(44 ; Inventory
				(switch temp0
					(29
						(EcoNarrator init: 1 0 0 22) ; "The pirate can't be threatened anymore."
					)
					(37
						(EcoNarrator init: 1 0 0 32) ; "What a grisly idea."
					)
					(12
						(EcoNarrator init: 1 0 0 34) ; "The time for that is long past."
					)
					(30
						(EcoNarrator init: 1 0 0 35) ; "He's already got a spine. Haha."
					)
					(else
						(EcoNarrator init: 1 0 0 33) ; "The skeleton is beyond help."
					)
				)
			)
			(else
...
(instance Sky of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(4 ; Do
				(EcoNarrator init: 1 0 0 17) ; "Adam can't get to the surface from here."
			)
			(1 ; Look
				(EcoNarrator init: 1 0 0 20) ; "The reef formation is close to the coastline."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance Cliff of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 16) ; "Adam has found a small air pocket in the cliff walls that line the coast."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 14) ; "Adam's too wet to climb the slippery rock."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance lookChestTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 5 self) ; "A pirate crew left its treasure in this secret spot. They installed a security system before they left - a pirate ghost."
			)
			(1
				(EcoNarrator init: 1 0 0 12 self) ; "The pirate loot was stored in this chest. Maybe the pirates lost their claim ticket or something."
			)
			(2
				(self dispose:)
			)
...
(instance useKeyOnChestTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 1 self) ; "The key looks more modern than the chest. It won't open that old lock."
			)
			(1
				(EcoNarrator init: 1 0 0 19 self) ; "Anyway the lock has been sawed through."
			)
			(2
				(self dispose:)
			)
...
(instance lookBeachTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 8 self) ; "A small beach is just big enough for a couple of people to stand on."
			)
			(1
				(EcoNarrator init: 1 0 0 9 self) ; "Part of it is occupied by a permanent guest."
			)
			(2
				(self dispose:)
			)
...
(instance doPikeSecond of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(EcoNarrator init: 1 0 0 21 self) ; "The pike snaps into pieces and the key goes flying into the water."
			)
			(6
...
(instance openChest of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EcoNarrator init: 1 0 0 25 self) ; "Dreaming of pirate treasure, Adam opens the chest."
			)
			(3
				(chest hide:)
				(gEgo
			...
			(6
				...
				(EcoNarrator init: 1 0 0 26 self) ; "A tiny blue crab has been napping in the chest. He scuttles away to find new shelter."
			)
			(7
				(chest setCel: 1 show:)
