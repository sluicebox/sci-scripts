...
(instance rm580 of EcoRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(EcoNarrator store: 75 init: 1 0 0 74) ; "Adam is swimming in front of a huge, strange cavern. In the distance, an undersea mountain rises to a peak. A black cave mouth scars the mountain."
			)
			(else
...
(instance exitToLair of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 84 self) ; "Now that Flesh-Eater is distracted by the mighty whale, Adam is free to enter the beast's lair."
			)
			(1
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond
					((IsFlag 88)
						(EcoNarrator init: 1 0 0 59 self) ; "Adam can't desert Cetus now!"
						(= state 2)
					)
					((& (gEgo onControl: 1) $0800)
						(= cycles 1)
	...
	(method (changeState newState &tmp [temp0 500] temp500)
		(switch (= state newState)
			(0
				...
				(EcoNarrator init: 1 0 0 86 self) ; "Adam, I don't like the looks of it here."
			)
			(1
			...
			(4
				(EcoNarrator init: 1 0 0 90 self) ; "A-a-dam..."
			)
			(5
				(EcoNarrator init: 1 0 0 92 self) ; "Be-behind you!..."
			)
			(6
			...
			(8
				(EcoNarrator init: 1 0 0 93 self) ; "Flesh-Eater!!!! RUN!"
			)
			(9
...
(instance tryToPryOpenDoor of HandsOffScript
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(6
				((ScriptID 2 1) init: 4 0 0 5) ; Adam, "It's stuck! The cable is really wedged in there."
				(self dispose:)
			)
		)
	)
...
(instance pryOpenDoor of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(3
				(EcoNarrator init: 1 0 0 24 self) ; "Adam wedges the trident under the crack of the bloated ship's door and pushes on the handle as hard as he can."
			)
			(4
			...
			(5
				(EcoNarrator posn: -1 20 init: 1 0 0 25 self) ; "The waterlogged wood suddenly gives way and the door pops free of its frame!"
			)
			(6
				(cable1 dispose:)
				(cable2 dispose:)
			...
			(9
				(EcoNarrator posn: -1 20 init: 1 0 0 26 self) ; "The cable that had been jammed under the door suddenly jerks tight as though pulled by a mighty weight."
			)
			(10
				(= cycles 2)
			)
			(11
				(EcoNarrator posn: -1 20 init: 1 0 0 27) ; "...And through the open ship's door, Adam hears a sound..."
				(NormalEgo)
			...
			(13
				(EcoNarrator init: 1 0 0 28 self) ; "...a sound unmistakably sad, unmistakably deep, unmistakably haunting..."
			)
			(14
				(EcoNarrator init: 1 0 0 29 self) ; "...unmistakably a whale's call!"
			)
			(15
...
(instance endOfFight of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(EcoNarrator init: 1 0 0 56 self) ; "There's a moment of terror in which Adam is sure the poison will not be enough to even slow down the huge beast!"
			)
			(1
				(manta setMotion: DPath 249 25 226 51 206 45 self)
			)
			...
			(3
				(EcoNarrator init: 1 0 0 57 self) ; "But it's enough to make him hesitate in his attack, if only for a moment..."
			)
			(4
			...
			(5
				...
				(EcoNarrator init: 1 0 0 58 self) ; "And that moment is enough!"
			)
			(6
...
(instance stickManta of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(HandsOff)
				(EcoNarrator init: 1 0 0 51 self) ; "Adam arms himself with the tiny lionfish spine, and with all the courage he can muster, and heads towards the black monster."
			)
			(1
				(gEgo setCycle: 0)
			)
			...
			(5
				(EcoNarrator init: 1 0 0 54 self) ; "While the manta is distracted by Cetus, Adam manages to get close to one huge, black wing!"
			)
			(6
				(EcoNarrator init: 1 0 0 55 self) ; "And pierce the tough hide with the lionfish spine!"
			)
			(7
...
(instance tryToStabManta of Script
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(6
				(= register 0)
				(EcoNarrator init: 1 0 0 52 self) ; "But before he can wield his weapon, he is spotted by the manta and flicked disdainfully away."
			)
			(7
				(gEgo
					view: 587
			...
			(9
				(EcoNarrator init: 1 0 0 53 self) ; "He'll have to get more careful in his approach if he's to get close enough to use the spine."
			)
			(10
				(NormalEgo)
				(gEgo setHeading: 0 self)
...
(instance smackedByManta of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(EcoNarrator init: 1 0 0 30 self) ; "Adam gathers up his courage and approaches Flesh-Eater's lair, determined to save his friend Delphineus."
			)
			(1
			...
			(6
				(EcoNarrator init: 1 0 0 31 self) ; "But the horrid manta is not in the mood for guests and sends Adam flying as easily as a human might swat a gnat."
			)
			(7
				(manta dispose:)
				(= cycles 1)
			...
			(10
				(EcoNarrator init: 1 0 0 32) ; "Adam is no match for the great brute. If Delphineus is to be saved, Adam must find Cetus!"
				(self dispose:)
			)
		)
	)
...
(instance cantGoThatWay of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EcoNarrator init: 1 0 0 11 self) ; "Adam would be cut to ribbons on the cliff wall if he swam that way."
			)
			(2
...
(instance mantaLair of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 73)
					(EcoNarrator init: 1 0 0 15) ; "Flesh-Eater's lair looms in the distance. Adam eyes it nervously."
				else
					(EcoNarrator init: 1 0 0 14) ; "Far in the distance is a cave mouth. It looks like a gaping wound."
				)
			)
			(4 ; Do
				(if (IsFlag 71)
					(EcoNarrator init: 1 0 0 85) ; "Delphineus has been released. Adam has no reason to go back to the manta's lair."
				else
					(localproc_0)
				)
			)
			(2 ; Talk
				(EcoNarrator init: 1 0 0 63) ; "The cave is too far away for anyone to hear Adam's call."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 64) ; "Adam can't do anything to the horrid cave."
			)
			(else
...
(instance coral of EcoFeature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 10) ; "The wall of coral rises, sharp and jagged, as high as Adam can see. The only way to the other side is through the ship."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cactusPlant of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 17) ; "Tube coral here has taken on a strange and very unnatural color."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 65) ; "Adam doesn't want to touch these strange plants!"
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 66) ; "Adam doesn't want anything to do with these strange plants."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance harpoonGun of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (not (IsFlag 341))
					(EcoNarrator store: 94)
				)
				(EcoNarrator init: 1 0 0 4) ; "The harpoon gun is a reminder of the dreadful practice of whale hunting. This one has been triggered."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 13) ; "The gun is jammed from its last firing."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 70) ; "The harpoon gun has already caused all the damage it will ever cause and is no longer of concern to Adam."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance brokenHull of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 19) ; "The boards of the hull are battered and broken."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 20) ; "The hull is too broken to repair."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 67) ; "Adam can't do anything with the broken hull."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance rockyLedge of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator init: 1 0 0 12) ; "The wall is full of jagged coral."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 73) ; "Adam can't alter the coral cave."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance harpoonNozzle of EcoFeature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 6) ; "The harpoon gun appears to have been fired recently."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance boat of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator init: 1 0 0 3) ; "A modern harpooning vessel has rammed into the reef shelf. Only the prow is visible."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 72) ; "Adam can't do anything with the ship's cabin."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 71) ; "There's nothing Adam can do with the ship's cabin."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance doorWay of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 88)
					(EcoNarrator init: 1 0 0 34) ; "The ship's door is open."
				else
					(EcoNarrator init: 1 0 0 33) ; "The ship's door is now open and the sound of a whale's call drifts from somewhere on the other side of the ship."
				)
			)
			(4 ; Do
				(gCurRoom setScript: intoShipScript)
			...
			(2 ; Talk
				(EcoNarrator init: 1 0 0 76) ; "There is no response from beyond the ship's door."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance shipExit of EcoFeature
	...
	(method (doVerb theVerb &tmp temp0)
		(doorWay doVerb: theVerb temp0 &rest) ; UNINIT
	)
)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(cond
					((not (IsFlag 98))
						(EcoNarrator store: 61 init: 1 0 0 1) ; "The ship's cabin door is jammed shut."
					)
					((IsFlag 88)
						(EcoNarrator init: 1 0 0 34) ; "The ship's door is open."
					)
				...
				(cond
					((== temp0 29)
						(if (not (IsFlag 98))
							...
							(EcoNarrator init: 1 0 0 77) ; "The door is already open."
						)
					)
					((not (IsFlag 98))
						(EcoNarrator init: 1 0 0 81) ; "That won't open the ship's door."
					)
					(else
						(EcoNarrator init: 1 0 0 77) ; "The door is already open."
					)
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (not (IsFlag 100))
					(if (IsFlag 71)
						(EcoNarrator init: 1 0 0 36) ; "Cetus is losing ground in the battle. Flesh-Eater shrewdly concentrates his attacks on the whale's harpoon wound and Cetus, in return, seems unable to land a solid blow against his quick-moving opponent. Despite his bravery, Cetus clearly could use some help."
					else
						(EcoNarrator store: 36 init: 1 0 0 35) ; "Cetus is trying hard to keep up with Flesh-Eater, but he's weakened from his recent ordeal and lack of food."
					)
				)
			)
			(4 ; Do
				(if (not (IsFlag 100))
					(EcoNarrator init: 1 0 0 38) ; "Adam's done all he can to strengthen Cetus, there's nothing he can do for him now."
				)
			)
			(2 ; Talk
				(if (not (IsFlag 100))
					(if (IsFlag 71)
						(EcoNarrator init: 1 0 0 37) ; "The whale is struggling to maintain his ground in the battle and isn't paying attention to Adam."
					else
						(localproc_5 1)
					)
				)
			...
			(44 ; Inventory
				(switch temp0
					(30
						(EcoNarrator init: 1 0 0 78) ; "Be careful with that spine! Adam doesn't want to stick Cetus."
					)
					(12
						(EcoNarrator init: 1 0 0 79)
					)
					(else
						(EcoNarrator init: 1 0 0 80) ; "That won't help Cetus."
					)
				)
			)
			(else
...
(instance manta of EcoActor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator store: 42 init: 1 0 0 41) ; "Flesh-Eater is crazed and vicious, enraged that Cetus has escaped his trap. He's exploiting Cetus' weakness by attacking the fresh harpoon wound."
			)
			(4 ; Do
				(if (not (IsFlag 71))
					(localproc_5 2)
				else
					(EcoNarrator init: 1 0 0 43) ; "Adam can't fight the manta with his bare hands."
				)
			)
			(2 ; Talk
				(EcoNarrator init: 1 0 0 39) ; "Though once an Elurian, the toxic waste has mutated Flesh-Eater beyond all sense of reasoning. He does not speak."
			)
			(6 ; Recycle
				(EcoNarrator init: 1 0 0 44) ; "Ah, if only it were that simple!"
			)
			(44 ; Inventory
				(if (IsFlag 71)
					(switch temp0
						...
						(29
							(EcoNarrator init: 1 0 0 45) ; "The trident's points would be but a pin prick to the enraged manta."
						)
						(37
							(EcoNarrator init: 1 0 0 46) ; "Flesh-Eater would never stand still for the length of time it would take to do any real damage with that saw. Besides, that would be gross!"
						)
						(17
							(EcoNarrator init: 1 0 0 48) ; "That would barely scratch the surface of the problem."
						)
						(12
							(EcoNarrator init: 1 0 0 50) ; "Far in the distance is a cave mouth. It looks like a gaping wound."
						)
						(else
							(EcoNarrator init: 1 0 0 49) ; "That won't stop the rampaging manta!"
						)
					)
				else
					(localproc_5 2)
...
(instance cable1 of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(EcoNarrator store: 60 init: 1 0 0 5) ; "The thick, steel cable from the gun is wedged under the ship's door, then continues up into the coral cave."
			)
			(4 ; Do
				(EcoNarrator init: 1 0 0 11) ; "Adam would be cut to ribbons on the cliff wall if he swam that way."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 69) ; "Adam can't do anything to the cable from this side of the coral cave."
			)
			(else
				(super doVerb: theVerb temp0 &rest)
			)
...
(instance cable2 of EcoView
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(4 ; Do
				(EcoNarrator init: 1 0 0 21) ; "The cable is stuck fast in the door."
			)
			(44 ; Inventory
				(EcoNarrator init: 1 0 0 69) ; "Adam can't do anything to the cable from this side of the coral cave."
			)
...
(instance cable3 of EcoProp
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 98)
					(EcoNarrator init: 1 0 0 83) ; "There's something big on the other end of that cable!"
				else
					(EcoNarrator store: 60 init: 1 0 0 5) ; "The thick, steel cable from the gun is wedged under the ship's door, then continues up into the coral cave."
				)
			)
			(4 ; Do
				(if (IsFlag 98)
					(EcoNarrator init: 1 0 0 62) ; "The cable is pulled taut by something powerful on the other end. There's no way Adam can pull it in."
				else
					(EcoNarrator init: 1 0 0 21) ; "The cable is stuck fast in the door."
				)
			)
