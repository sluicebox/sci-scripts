	...
	(method (setCur node displayCursor)
		(if (= curNode node)
			...
			(if (or (< argc 2) displayCursor)
				(gGlory setCursor: 999 1 (curPt x:) (curPt y:))
			)
		)
	)
				...
				(switch (event message:)
					(KEY_TAB
						(if (and (== state 1) curPolygon)
							(self advanceRetreat: 76 160)
						)
					)
					(KEY_SHIFTTAB
						(if (and (== state 1) curPolygon)
							(self advanceRetreat: 164 163)
						)
					)
					(KEY_SPACE
						(if (and (== state 1) curPolygon)
