	...
	(method (init)
		...
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 0 177 43 177 24 189 0 189
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
					init: 0 140 0 0 319 0 319 164 302 166 287 154 297 135 292 132 274 133 268 125 254 125 238 120 133 120 133 124 235 124 235 140 126 140 95 151 53 151 88 96 28 63 25 67 82 98 42 152 0 152
					yourself:
				)
		)
		...
		(pRightDoor
			setCel: (if (== gPrevRoomNum 627) 5 else 0)
			init:
			approachVerbs: 4 32 ; Do, theOil
		)
		(super init: &rest)
		...
		(vClosentDoor init: approachVerbs: 4 32) ; Do, theOil
		(fSouth init:)
		(fUp init:)
		(fDown init:)
	)
			...
			(1
				(switch local0
					(1
						...
						(self setScript: sListenToKatrina)
					)
					(2
		...
		(switch (= state newState)
			...
			(2
				...
				(cond
					...
					((IsFlag 113)
						(self setScript: sHallDeath)
					)
					(else
						(self setScript: sSqueakDeath)
					)
				)
			)
			(3
			...
			(5
				(pRightDoor setCycle: CT 0 -1 self)
			)
			(6
				(= seconds 2)
			)
...
(instance sSqueakDeath of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(aZap setCycle: End self)
				(spellSound stop:)
				(pRightDoor setCycle: CT 4 1 self)
			)
			(4
				(aZap hide: dispose:)
