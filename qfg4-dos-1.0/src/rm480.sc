...
(instance rm480 of GloryRm
	...
	(method (init)
		(gGlory handsOff:)
		(if (and (== global348 1) (IsFlag 336) (gEgo has: 26)) ; theHat
			(= global348 2)
		)
...
(instance heroTeller of Teller
	...
	(method (sayMessage)
		(if (== iconValue 83) ; Tell About Gnome
			(SetFlag 336)
		)
...
(instance placePie of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(theHut signal: (| (theHut signal:) $0001) setCycle: End)
				(gEgo
					normalize: 0
					cycleSpeed: register
		...
		(switch theVerb
			(44 ; theHat
				(if (IsFlag 336)
					...
					(gMessager say: 5 44 114) ; "You don't have any good reason to do that... yet."
				)
			)
			(46 ; thePiepan
				(if
