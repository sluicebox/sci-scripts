...
(instance rm500 of GloryRm
	...
	(method (init)
		...
		(rusalkaGrave init:)
		(loverGrave init:)
		(cryptDoor init: approachVerbs: 4 29 42) ; Do, theKeyRing, theToolkit
...
(instance sLigeiaOPEN of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gCurRoom setScript: sLigeia 0 1)
			)
		)
	)
)
...
(instance sLigeia of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(ligeia signal: (& (ligeia signal:) $fffe))
				(= ticks 60)
			)
			(4
				(gEgo setCycle: End self)
			...
			(6
				...
				(if gNight
					(if (IsFlag 246)
						(gMessager say: 21 4 9) ; "Now that you've sent Ligeia's ghost to another realm, nothing of interest remains in this tomb."
					else
						(SetFlag 246)
						(gMessager say: 21 4 10) ; "Ligeia's Ghost comes forth... and she looks MAD! This could turn into a really bad day."
					)
				else
					(gMessager say: 21 4 8) ; "Sheesh! That rotting corpse sure gave you a fright. Fortunately, it was just a dead body, not some sort of horrible undead creature. (Those only come out at night.)"
				)
				(= register 0)
				(gGlory handsOn:)
				(self dispose:)
...
(instance rusalkaGrave of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(58 ; theBroom
				(if (and ((gInventory at: 39) state:) (not (IsFlag 228))) ; theBroom
					(SetFlag 228)
			...
			(33 ; theGrapnel
				(cond
					...
					((not [gEgoStats 11]) ; climbing
						(gMessager say: 30 4 42) ; MISSING MESSAGE
					)
					(else
						(= local5 1)
						(gCurRoom setScript: sTryTree)
