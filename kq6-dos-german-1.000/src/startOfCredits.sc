...
(instance findSize of Code
	...
	(method (doit param1)
		(TextSize @local663 param1 2207 315)
		(return (- (/ (- 180 (- [local663 2] [local663 0])) 2) 10))
	)
)
...
(instance openingCreditsToo of Script ; UNUSED
	(properties)
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(1
				(++ local661)
				(Message msgGET 94 0 0 -1 [local655 local622] @local0)
				(= temp1 (findSize doit: @local0))
				(++ local622)
				(= local620
					(Display
				...
				(Message msgGET 94 0 0 -1 [local655 local622] @local310)
				(++ local622)
				(= local621
					(Display
						@local310
