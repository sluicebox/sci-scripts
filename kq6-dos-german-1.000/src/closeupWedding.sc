...
(instance closeupWedding of Script
	...
	(method (handleEvent event)
		(if
			...
			(event claimed: 1)
			(= next (ScriptID 742 2)) ; saladinKillEgo
			(self cue:)
		)
		(event claimed:)
	)
			...
			(12
				...
				(if (not next)
					(if (not register)
						...
						((ScriptID 742 2) register: 29) ; saladinKillEgo
						(= next (ScriptID 742 2)) ; saladinKillEgo
					)
				)
				(self dispose:)
...
(instance showMirror of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(client seconds: 0 next: (ScriptID 742 4)) ; genieCastSpell
				((ScriptID 740 5) view: 7424 loop: 0 cel: 0 setCycle: 0) ; saladin
				(gTheIconBar disable:)
				(client dispose:)
			)
...
(instance showReplicaLamp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(closeupWedding next: (ScriptID 742 2) changeState: 12) ; saladinKillEgo
			)
		)
	)
)
	...
	(method (doVerb theVerb)
		(cond
			...
			((OneOf theVerb 56 2) ; fakeLamp5, Talk
				(gGame handsOff:)
				(closeupWedding seconds: 0 next: (ScriptID 742 2)) ; saladinKillEgo
