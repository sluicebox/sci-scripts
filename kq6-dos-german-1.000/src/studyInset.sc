		...
		(switch (= state newState)
			...
			(12
				...
				(if (not ((ScriptID 80 0) tstFlag: #rFlag2 256)) ; rgCastle
					(Print addText: 4 1 10 1) ; "And now to see to the final preparations. It's almost time for the wedding. Hee, hee!"
				else
					(Print addText: 4 1 9 8) ; "I think it's about time to see if Shamir has taken care of the wench as I asked. It's almost time for the wedding."
				)
				(Print posn: -1 10 modeless: 1 ticks: 20 init: self)
			)
			(13
				(self setScript: convScript self)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print
					font: gUserFont
					addText: 4 1 (+ 8 (not ((ScriptID 80 0) tstFlag: #rFlag3 16))) 1 ; rgCastle
					init: self
				)
			)
			(2
