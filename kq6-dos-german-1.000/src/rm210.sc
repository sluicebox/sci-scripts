...
(instance rm210 of KQ6Room
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4 temp5)
		...
		((ScriptID 10 4) onMeCheck: 2 init:) ; rocks
...
(instance windUpBirdScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print font: gUserFont posn: 10 10 addText: 4 37 10 2 init: self) ; "It plays its pleasant, tinny tune."
			)
			(3
				(Print font: gUserFont posn: 10 10 addText: 4 37 10 3 init: self) ; "The living nightingale in the tree listens to the tune curiously."
			)
			(4
				(= ticks 30)
			)
...
(instance befriendSSScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Print font: gUserFont posn: 10 10 addText: 4 37 11 2 init: self) ; "The mechanical nightingale sings a sweet, tinny tune. The real nightingale in the tree cocks her head and listens intently."
			)
			(5
			...
			(7
				(Print font: gUserFont posn: 10 10 addText: 4 37 11 3 init: self) ; "The nightingale flies to a lower branch and looks at Alexander curiously as if she were deciding that this human might not be so bad."
			)
			(8
				(= ticks 48)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				...
				(gMessager say: 5 5 0 2 self oneOnly: 0) ; "Dearest Alexander:   I cannot believe you are here, my friend! Please, please be careful! Abdul isn't about to let anyone interfere with his plans. Watch out for Abdul's genie, Alexander, and do not do anything rash. I am not without resources, and I will prevail if I can only find some small means of defense. Do nothing to try to get to me. You must not be endangered again for my sake.  Greatly in your family's debt,  Cassima"
			)
			(12
				(gNarrator talkWidth: local36)
...
(instance rocks of Feature ; UNUSED
	(properties
		noun 2
		onMeCheck 2
	)
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb 1 5 2) ; Look, Do, Talk
			...
			(gMessager say: noun) ; MISSING MESSAGE
		)
	)
)
