...
(class KQ6Room of Rm
	...
	(method (setScript param1 &tmp temp0)
		(cond
			...
			((!= (gEgo view:) 900)
				(Print font: gUserFont posn: -1 10 addText: 7 0 16 1 0 0 0 init:) ; "Alexander is a little busy for that right now. Perhaps he should try it again later."
			)
			(
				(or
						...
						(cond
							(
								...
								(Print
									font: gUserFont
									posn: -1 10
									addText: 0 0 88 1 0 0 899 ; "The Realm of the Dead is a very dangerous place! There's no time for that here!"
									init:
								)
							)
							((== gCurRoomNum 461) ; blackWidowInset
								(Print
									font: gUserFont
									posn: -1 10
									addText: 0 0 5 1 0 0 899 ; "Alexander is a bit busy with the black widow at the moment! Try that again later!"
									init:
								)
							)
							((= temp0 (localproc_0))
								(Print
									font: gUserFont
									posn: -1 10
									addText: 0 0 gCurRoomNum 1 0 0 899
									init:
								)
							)
						)
