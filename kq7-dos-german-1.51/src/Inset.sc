...
(class Inset of Obj
	...
	(method (init whoCares theRoomOrInset reg delayDraw &tmp delay i)
		...
		(= oldTimers gTimers)
		(= oldHotCast global123)
		((= global123 (Cast new:)) name: {inHotCast} add:)
		((= gTimers (Set new:)) name: {inTimers} add:)
		(= local0 gTheDoits)
		(= gTheDoits (EventHandler new:))
	...
	(method (dispose refreshOrNot &tmp whoCares i)
		...
		(global123 release: dispose:)
		(= global123 oldHotCast)
