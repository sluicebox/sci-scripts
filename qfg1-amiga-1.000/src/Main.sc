...
(procedure (FindTime &tmp whatDay [str 30])
	...
	(HighPrint (Format @str 0 50 297 (+ 40 gTimeOfDay) whatDay)) ; "%s on day %d."
)
...
(procedure (EgoGait newGait gaitMsg &tmp theView)
	(if gaitMsg
		(cond
			((not (User canControl:))
				(HighPrint 0 51) ; "You can't do that now."
				(return)
			)
			((== gEgoGait newGait)
				(HighPrint 0 52) ; "Go ahead. Just do it."
				(return)
			)
		)
	)
...
(procedure (NotClose)
	(HighPrint 0 53) ; "You're not close enough."
)

(procedure (AlreadyDone)
	(HighPrint 0 54) ; "You've already done that."
)
...
(procedure (DontHave)
	(HighPrint 0 55) ; "You don't have it."
)
...
(procedure (FixTime newClock newMinutes &tmp oldTime)
	...
	(if (and (== gTimeOfDay 6) (!= oldTime 6))
		(if (== (++ global268) 1)
			(Print 0 56) ; "You are getting tired."
		else
			(Print 0 57) ; "You are exhausted from lack of sleep."
		)
	)
)
...
(procedure (SkillUsed skillNum learnValue &tmp learnSign)
	...
	(= learnValue (Abs learnValue))
	(= learnSign 0)
	(+= [gEgoStats 13] (/ learnValue 4)) ; experience
	(+= [global164 skillNum] learnValue)
	(while (>= [global164 skillNum] [gEgoStats skillNum])
...
(procedure (UseStamina pointsUsed &tmp foo)
	(SkillUsed 3 (/ pointsUsed 4)) ; vitality
	(cond
		((< (= foo (-= [gEgoStats 15] pointsUsed)) 0) ; stamina
			...
			(if (not (IsFlag 110))
				(SetFlag 110)
				(HighPrint 0 58) ; "You are so exhausted that everything you do hurts. Better get some rest."
			)
			(if (<= [gEgoStats 14] 0) ; health
				(EgoDead 0 59 80 {Death from Overwork} 82 800 1 4) ; "That was a little too much for you. You collapse from exhaustion and die."
			)
		)
		((> foo 4)
			(ClearFlag 110)
...
(procedure (TakeDamage damage &tmp temp0)
	(= temp0 (+ (Abs damage) 1))
	(SkillUsed 3 (/ temp0 2)) ; vitality
...
(procedure (CastSpell spellNum &tmp temp0)
	(cond
		((not [gEgoStats 12]) ; magic
			(HighPrint 0 60) ; "You don't know how to cast spells."
		)
		((not (gEgo knows: spellNum))
			(HighPrint 0 61) ; "You don't know that spell."
		)
		((< [gEgoStats 16] [global190 (- spellNum 17)]) ; mana
			(HighPrint 0 62) ; "You don't have enough Magic Points to cast that spell."
		)
		(else
			(TrySkill spellNum 0)
	...
	(cond
		...
		([gInvNum 3] ; food
			(if (not (-- [gInvNum 3])) ; food
				(LowPrint 0 63) ; "You just ate your last ration; you'd better get some more food soon."
			)
		)
		((IsFlag 124)
			(SetFlag 125)
			(LowPrint 0 64) ; "You're starving. Better find some food *soon*!"
		)
		(else
			(SetFlag 124)
			(LowPrint 0 65) ; "You're really getting hungry."
		)
	)
)
...
(instance HQ of Game
	...
	(method (handleEvent event &tmp temp0 [temp1 4] temp5)
		(switch (event type:)
			(evKEYBOARD
				...
				(cond
					...
					((Said 'cast>')
						(switch (= temp0 (SaidSpell event))
							...
							(18
								(if (CastSpell temp0)
									(HighPrint 0 16) ; "You sense no magic in this area."
								)
							)
							(21
								(if (CastSpell temp0)
									(= global250 (+ 1 (/ [gEgoStats 21] 10))) ; zapSpell
									(if (or (gEgo has: 6) (gEgo has: 7)) ; blade, dagger
										(HighPrint 0 17) ; "Your weapon is now magically charged."
									else
										(HighPrint 0 18) ; "Gee! You lost your weapon!"
									)
								)
							)
							(17
								(if (CastSpell temp0)
									(HighPrint 0 19) ; "You cannot open anything here."
								)
							)
							(else
								(HighPrint 0 20) ; "That spell is not useful here."
							)
						)
					)
					((Said 'fight')
						(HighPrint 0 21) ; "Aggressive, aren't you?"
					)
					((Said 'talk')
						(HighPrint 0 22) ; "No one responds."
					)
					((or (Said 'put<down>') (Said 'drop>'))
						(cond
							((not (= temp0 (SaidInv event)))
								(HighPrint 0 23) ; "I don't know what you're trying to drop."
							)
							([gInvNum temp0]
								(if (== temp0 9)
								...
								(HighPrint 0 24) ; "Ok, you drop it."
								(if (not (gEgo has: 30)) ; magic mushroom
									(ClearFlag 174)
								)
							)
							(else
								(HighPrint 0 25) ; "You can't drop something you don't have."
							)
						)
					)
					((Said 'walk')
					...
					((or (Said '[use]/stealth') (Said 'sneak'))
						(if (!= gEgoGait 2) ; sneaking
							(if (TrySkill 8 5 0) ; stealth
								...
								(HighPrint 0 26) ; "You're about as stealthy as the average Goon."
							)
						)
					)
					((Said 'unlock,lockpick/door,hasp')
						(HighPrint 0 27) ; "There's no point in trying that here."
					)
					((Said 'lockpick/nose')
						(if (CanPickLocks)
							(HighPrint 0 28) ; "You delicately insert the lockpick into your left nostril."
							(if (not (TrySkill 9 40)) ; pick locks
								...
								(HighPrint 0 30) ; "Success! You now have an open nose."
							)
						else
							(HighPrint 0 31) ; "You don't have an appropriate tool."
						)
					)
					((Said 'ask')
						(HighPrint 0 32) ; "You get no response."
					)
					((Said 'buy')
						(HighPrint 0 33) ; "It's not for sale."
					)
					((Said 'eat>')
						(Eat event)
					)
					...
					((Said 'thank')
						(HighPrint 0 34) ; "You're welcome!"
					)
					((Said '/sorry')
						(HighPrint 0 35) ; "That's OK."
					)
					((Said 'yes,no')
						(HighPrint 0 22) ; "No one responds."
					)
					...
					((Said 'open/door')
						(HighPrint 0 38) ; "You're not close enough to a door."
					)
					((Said 'open/box,musicbox')
						(if (gEgo has: 16) ; music box
							(HighPrint 0 39) ; "You open the music box, which plays Beethoven's "Fur Elise." After listening to it for a bit, you close the box."
						else
							(HighPrint 0 40) ; "You don't have a box to open."
						)
					)
					((Said 'knock')
						(HighPrint 0 41) ; "No one hears you knocking."
					)
					((Said 'sat')
						(HighPrint 0 42) ; "You don't need to."
					)
					((Said 'give')
						(HighPrint 0 43) ; "You have no reason to do that."
					)
					((Said 'search>')
						(if (Said '[/!*]')
							(HighPrint 0 44) ; "Search what?"
						else
							(event claimed: 1)
							(HighPrint 0 45) ; "You don't find anything interesting."
						)
					)
					((Said 'look,read>')
						(= temp5
						...
						(cond
							...
							(else
								(HighPrint 0 46) ; "You don't see any here."
							)
						)
					)
					((Said 'rest[/!*]')
	...
	(method (replay)
		...
		(SetMenu 774 110 (if (DoSound sndSET_SOUND) {Turn Off} else {Turn On}))
		(super replay:)
	)

	(method (wordFail param1)
		(Printf 0 47 param1) ; "You will not need to use the word "%s" in this game."
	)

	(method (syntaxFail)
		(HighPrint 0 48) ; "Please try a different way of saying that."
	)

	(method (pragmaFail)
		(HighPrint 0 49) ; "I'm not sure what you're trying to do."
	)
)
