...
(instance rm21 of Rm
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said '/spider') (MouseClaimed spider event 3))
				(HighPrint 21 3) ; "Is that a SMILE on that spider's face?"
			)
			((or (Said '/web') (MouseClaimed web event 3))
			...
			((or (Said '/window') (MouseClaimed hutWindow event 3))
				(HighPrint 21 5) ; "The glass seems to distort the view of the outside world."
			)
			((or (Said '/caldron,pan') (MouseClaimed cauldron event 3))
			...
			((or (Said '/fire') (MouseClaimed fire event 3))
				(HighPrint 21 8) ; "The fire burns hot under the kettle."
			)
			((or (Said '/chimney') (MouseClaimed chimney event 3))
				(HighPrint 21 9) ; "The stone hearth and chimney radiates too much heat."
			)
			((or (Said '/cabinet') (MouseClaimed cupboard event 3))
				(HighPrint 21 10) ; "It's a plain wooden kitchen cupboard."
			)
			((or (Said '/dagger,implement') (MouseClaimed knives event 3))
				(HighPrint 21 11) ; "The kitchen knives look old but sharp."
			)
			((or (Said '/hay,bed,blanket') (MouseClaimed blanket event 3))
			...
			((== (event type:) evSAID)
				(cond
					...
					((Said 'talk/baba')
						(HighPrint 21 14) ; "You cannot see the witch anywhere."
					)
					((Said 'talk[/*]')
						(HighPrint 21 15) ; "You might as well be talking to yourself."
					)
					((Said 'cast')
						(HighPrint 21 16) ; "Your magic is useless here."
					)
					((Said 'look>')
						(cond
							((Said '[<at,around][/hut,house,room]')
								(if (IsFlag 11)
									(HighPrint 21 17) ; "A quick look around shows that this is a creepy place, despite the pot (cauldron?) simmering briskly on the fire. Look at the size of that spider!"
								else
									(HighPrint 21 18) ; "Two witches had two watches. Which witch would watch which watch? Which watch would which witch watch? (Is that too witchy watchy for you???)"
								)
							)
							((Said '/baba,ogress')
								(HighPrint 21 14) ; "You cannot see the witch anywhere."
