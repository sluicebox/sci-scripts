	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'look/box') (MouseClaimed door3 event 3))
				(HighPrint 96 63) ; "It's a box of some sort...or so the sign says."
			)
					...
					((Said 'look>')
						(cond
							((or (Said '<up') (Said '/ceiling,ladder'))
								(HighPrint 96 69) ; "There appear to be stairs leading up (?) to nowhere. There is also what looks like a bowl full of fireflies."
							)
							(
								(or
									(Said '<down')
								...
								(HighPrint 96 70) ; "There is a deep pit with a walkway winding above it."
							)
							((Said '/wall')
								(HighPrint 96 71) ; "Wow!"
							)
							((Said '/chain,rope,handle')
								(HighPrint 96 72) ; "In the "upstairs" doorway, there is a chain with a handle attached."
							)
							((Said '/sign')
								(HighPrint 96 73) ; "DOOR" "SECRET ENTRANCE" "EXIT" "ME" "BOX"
							)
							((Said '/arrow')
