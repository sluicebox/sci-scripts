...
(procedure (ThrowKnife atWhat &tmp pushX pushY)
...
(instance knifeScript of Script
	...
	(method (doit)
		...
		(if
			(and
				(== state 1)
				(or
					(and (< -15 thisX 15) (< -15 thisY 15))
					(not (< 0 (projectile x:) 319))
					(not (< 0 (projectile y:) 189))
				)
			)
			(self cue:)
		else
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(client hide:)
				(if (and register (< -15 thisX 15) (< -15 thisY 15))
					(-- global271)
					(++ global272)
