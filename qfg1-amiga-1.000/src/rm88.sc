	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(else
				...
				(if (not (IsFlag 78))
					(HighPrint 88 0) ; "There is a strong odor of decay and other more unpleasant smells filling the air. A pile of something unpleasant lies near the center of this cave."
				)
			)
		)
	)
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'search/troll,monster,creature')
						(cond
							((!= local0 4)
					...
					(
						...
						(if (gEgo inRect: 39 150 106 157)
							(= local2 1)
							(self setScript: egoSearch)
						else
							(HighPrint 88 17) ; "Get closer to the pile."
						)
					)
					((Said 'get,get>')
						(cond
					...
					((Said 'feed/troll,monster,creature')
						(if (== local0 2)
							(HighPrint 88 22) ; "This Troll looks as though he'd rather eat YOU!"
						else
							(HighPrint 88 14) ; "You can't do that."
						)
					)
					((Said 'listen/')
						(if (or (== local0 0) (== local0 4))
							(HighPrint 88 23) ; "You hear the constant "drip... drip..." of water."
						else
							(HighPrint 88 24) ; "You hear the constant "drip... drip..." of water, and the unmistakeable sound of something breathing nearby."
						)
					)
					((Said 'cast>')
						...
						(if (CastSpell temp0)
							(switch temp0
								(18
									(HighPrint 88 25) ; "There is no magic in this cave."
								)
								(20
									(HighPrint 88 26) ; "There is nothing here to dazzle."
								)
								(23
									(HighPrint 88 27) ; "There is nothing here to use that spell on."
								)
								(22
									(HighPrint 88 28) ; "There is nothing here to calm."
								)
								(17
									(HighPrint 88 29) ; "There is nothing here to open."
								)
								(else
									(HighPrint 88 30) ; "That spell is useless here."
								)
							)
						)
					)
					((Said 'fight')
						(if (IsFlag 250)
							(HighPrint 88 31) ; "The cave troll is dead."
						else
...
(instance trollOut of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 88 32) ; "Before you can notice him, a Troll sneaks up behind you, and the encounter begins."
				(gCurRoom newRoom: 450) ; trollArena
			)
		)
	)
...
(instance trollKills of Script ; UNUSED
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 88 33 80 {WHATA TROLL!} 82 800 0 0) ; "The Troll's brute force was too much for your skull."
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(1
				(switch local2
					(0
						(if (IsFlag 340)
							(HighPrint 88 34) ; "There is nothing of value on the Troll."
						else
							(HighPrint 88 35) ; "You find nothing on the Troll but a little hair."
						)
					)
					(1
						(HighPrint 88 36) ; "You dig around in the pile until the odor almost makes you sick."
						(if (IsFlag 335)
							(HighPrint 88 37) ; "You find nothing more of value."
						else
							(SetFlag 335)
							(HighPrint 88 38) ; "You find 30 silvers and 5 gold."
							(gEgo get: 2 5) ; gold
					...
					(2
						(if (IsFlag 340)
							(HighPrint 88 39) ; "There is no beard hair left on the Troll."
						else
							(HighPrint 88 40) ; "You cut some beard hair and put it away."
							(SetFlag 340)
