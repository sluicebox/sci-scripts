...
(instance bun of Condiment
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 29) ; "The top half of a hamburger bun."
			)
			(10 ; Taste
				(Print 387 15) ; "They almost taste fresh."
			)
...
(instance burger of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 387 30) ; "Beef at its finest(?)."
			)
			(3 ; Do
				(cond
					((== (lettuce followWho:) 1)
			...
			(10 ; Taste
				(Print 387 31) ; "Hey! This belongs to someone else!"
			)
			(11 ; Smell
				(Print 387 32) ; "There's that Monolith Burgers Secret Smell(tm)."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance grabber of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 33) ; "BurgerMation Grabitron unit."
			)
			(10 ; Taste
				(Print 387 34) ; "That's a good way to put your tongue out, buddy."
			)
			(11 ; Smell
				(Print 387 35) ; "Smells greasy."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance rejectSign of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 36) ; "If this is flashing, you've just blown it."
			)
			(10 ; Taste
				(Print 387 37) ; "It tastes greasy."
			)
			(11 ; Smell
				(Print 387 38) ; "It smells greasy."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance belt of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 39) ; "This conveys the burgers from the nucleo-carbonic cooking chamber to you for construction and then on to the box grabber for transport out to the customer, or rejection if you've blown it."
			)
			(10 ; Taste
				(Print 387 40) ; "Essence of Monolith Burger now coats your tongue - forever!"
			)
			(11 ; Smell
				(Print 387 41) ; "It smells like the essence of Monolith Burger."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance topCounter of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 42) ; "It's more unimportant counter space."
			)
			(10 ; Taste
				(Print 387 43) ; "You must be bored. Maybe we should turn up the speed."
			)
			(11 ; Smell
				(Print 387 43) ; "You must be bored. Maybe we should turn up the speed."
			)
...
(instance upperPipes of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 44) ; "More pipes"
			)
			(10 ; Taste
				(Print 387 45) ; "Don't mess with it."
			)
			(11 ; Smell
				(Print 387 46) ; "Smells like a pipe!"
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance redTube of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 47) ; "It's red tubing. You have no idea what it does."
			)
			(10 ; Taste
				(Print 387 45) ; "Don't mess with it."
			)
...
(instance hood of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 387 49) ; "This sucks most of the noxious gases from the room."
			)
			(10 ; Taste
				(Print 387 50) ; "It definitely hasn't been cleaned in awhile. Maybe later when you get time you could give it a buff."
			)
			(11 ; Smell
				(Print 387 51) ; "It smells likes escaping air."
			)
			(else
				(super doVerb: theVerb invItem)
			)
