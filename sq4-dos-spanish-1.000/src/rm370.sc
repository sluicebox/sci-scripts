	...
	(method (init &tmp temp0 temp1)
		...
		(cond
			((and (!= gPrevRoomNum 371) (not (IsFlag 8)))
				(gLongSong2 number: 371 loop: -1 flags: 1 playBed: 0)
			)
			((IsFlag 8)
...
(instance tossScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 370 0) ; "You should know by now that you can't just take things without paying for them, Roger."
...
(instance door of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 1) ; "This store is closed. A security door blocks the entrance."
			)
			(else
...
(instance light2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 2) ; "It's a flashing light."
			)
			(11 ; Smell
				(Print 370 3) ; "You give the window a sniff. Nothing is smelled."
			)
			(10 ; Taste
				(Print 370 4) ; "Dragging your tongue along the window brings you to the realization that it hasn't been washed in a long, long time. Countless people, beings, and creatures have exhaled on or near this, their little ones applying a variety of snacking compounds along the lower half."
			)
			(5 ; Talk
				(Print 370 5) ; "You speak in the direction of the glass but your syllables merely come careening back."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance dummy1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 6) ; "In the window, a robotic garment rack struts its stuff."
			)
			(11 ; Smell
				(Print 370 3) ; "You give the window a sniff. Nothing is smelled."
			)
...
(instance dummy2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 7) ; "In the window a robotic garment rack struts its stuff."
			)
			(11 ; Smell
				(Print 370 3) ; "You give the window a sniff. Nothing is smelled."
			)
...
(instance store of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 8) ; "This is Sack's, a high-class, high-priced dress shoppe."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance steps of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 9) ; "These steps lead down to the Skate-O-Rama."
			)
			(11 ; Smell
				(Print 370 10) ; "It smells like something someone would walk on."
			)
			(10 ; Taste
				(Print 370 11) ; "It tastes like something someone would walk on!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bush1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 12) ; "This is a fine example of the wonders of fabricated flora."
			)
			(11 ; Smell
				(Print 370 13) ; "The fabricated flora has an equally realistic and artificial smell."
			)
			(10 ; Taste
				(Print 370 14) ; "You know that eating plastic plants places pressure on your plumbing. You learned that reading JANITORIAL DIGEST."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bush2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 370 15) ; "This is a fine example of the wonders of fabricated flora."
			)
			(11 ; Smell
				(Print 370 16) ; "They have a minty-fresh smell that keeps you refreshed."
			)
			(10 ; Taste
				(Print 370 17) ; "You know that eating plastic plants places pressure on your plumbing. You learned that reading JANITORIAL DIGEST."
			)
			(else
				(super doVerb: theVerb &rest)
			)
