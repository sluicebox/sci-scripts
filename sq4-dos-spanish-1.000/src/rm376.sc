...
(instance screen10 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 376 31) ; "This game; "DWEEB HUNTER" reminds you too much of your present predicament. And guess who the dweeb is!"
			)
			(3 ; Do
				(Print 376 22) ; "Back off. It's not your turn."
			)
...
(instance basket of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 376 32) ; "Gazing at the wastebasket, your impulse - nay, your very instinct - nay, the very fibres of your being - scream out for you to pick up the refuse and empty the basket of its waste. But you're much too busy for that right now."
			)
			(3 ; Do
				(Print 376 33) ; "Resist those inbred custodial tendencies to collect garbage. You have much more important garbage to tend to!"
			)
			(5 ; Talk
				(Print 376 34) ; "You're quite startled to hear the wastebasket respond..."
				(Print 376 35) ; "Max, is that you? Agent 99? I always get these lousy disguises. Would you put a word in with the Chief for me? I wouldn't mind, but all these kids eat is ice cream and candy bars. Do you know how many Big Chewies I've eaten in the last hour? Not to mention the wad of Big Bang BubbleGum I've got stuck to my shoe phone... (sigh!)"
			)
			(11 ; Smell
				(Print 376 36) ; "Give your nose a break! It's a garbage can!"
			)
			(10 ; Taste
				(Print 376 37) ; "A garbage can?!"
			)
			(4 ; Inventory
				(Print 376 38) ; "You'd better not throw that away. It'll probably come in handy later. I know it doesn't seem likely, but trust me on this one."
			)
			(else
...
(instance kid of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 376 39) ; "Watching this fine youngster brings back fond memories of your youth. Little did you know that all of those hours playing "BLAZIN' PADDLES" were in preparation for being a space-guy-hero-type."
			)
			(3 ; Do
				(Print 376 40) ; "Leave the kid alone. He's totally engrossed in his game."
			)
			(5 ; Talk
				(Print 376 41) ; "Talking to a kid in the middle of an arcade game is almost as interesting as talking to the wall. Give it up."
			)
			(11 ; Smell
				(Print 376 42) ; "Perhaps you haven't been around a young, active being lately. I can tell you that walking up to one and giving it a sniff could be considered unstable and even dangerous behaviour."
			)
			(10 ; Taste
				(Print 376 43) ; "Perhaps you haven't read the nose message."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance changeMachine of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 376 44) ; "The change machine looks like a time machine more than a time machine looks like a time machine... for a change."
			)
			(3 ; Do
				(cond
					((not (gEgo inRect: 116 102 206 124))
						(Print 376 45) ; "You're not close enough to the change machine to touch it."
					)
					((OneOf (gEgo view:) 373 374)
						(Print 376 46) ; "A real lady wouldn't try to break a change machine."
					)
					((and local6 (not (gCurRoom script:)))
						(gCurRoom setScript: changeMachineDeathS)
					)
					(else
						(Print 376 47) ; "Banging on the change machine will only activate its self-defense mechanisms."
						(= local6 1)
					)
				)
			)
			(4 ; Inventory
				(if (== invItem 0) ; buckazoid
					(Print 376 48) ; "In all the known universe, the lowly buckazoid is the lowest denomination of all. You can't get change for it."
				else
					(super doVerb: theVerb invItem)
				)
			)
...
(instance sushiBar of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 376 49) ; "The dead fish in the sushi bar machine appear to have been dead far longer than they were fish. The sushi bar wasn't one of Buckazoid Bill's greatest ideas."
			)
			(3 ; Do
				(Print 376 50) ; "Rocking the sushi bar machine is impossible. It's bolted to the floor."
			)
			(4 ; Inventory
				(if (== invItem 0) ; buckazoid
					(Print 376 51) ; "In all the known universe, the lowly buckazoid is the lowest denomination of all. You can't even get a dead fish for it."
				else
					(super doVerb: theVerb invItem)
				)
			)
			(11 ; Smell
				(Print 376 52) ; "It smells like old dead fish."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance arcadeRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 376 53) ; "As you look around the arcade, you're delighted to see that some of your old favorites are still in operation: MONACO PRINCESS STUNT DRIVIN'; CHOKE 'N CROAK; GHETTO BLASTER; and DWEEB HUNTER. The air is filled with smoke, sweat and ozone, reminding you of your favorite cologne: "Night in Programming.""
			)
			(3 ; Do
				(Print 376 54) ; "Touching anything in here might be dangerous to your health."
			)
			(11 ; Smell
				(Print 376 55) ; "The aroma of the sushi bar dominates the air in here. However, you can also detect the smell of smoke, sweat and ozone, reminding you of your favorite cologne: "Night in Programming.""
			)
			(else
				(super doVerb: theVerb)
			)
