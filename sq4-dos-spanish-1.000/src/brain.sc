...
(procedure (localproc_1 &tmp temp0)
	(switch (= temp0 (brain formatting:))
		(5000
			(StrCpy @local112 (LangSwitch {EXCELLENT} {EXCELENTE}))
			(= local11 global137)
		)
		(3000
			(StrCpy @local112 (LangSwitch {VERY GOOD} {MUY BIEN}))
		)
		(2000
			(StrCpy @local112 (LangSwitch {GOOD} {BIEN}))
		)
		(1500
			(StrCpy @local112 (LangSwitch {FAIR} {REGULAR}))
		)
		(1000
			(StrCpy @local112 (LangSwitch {DANGER} {PELIGRO}))
			(= local11 global135)
		)
		(500
			(StrCpy @local112 (LangSwitch {CRITICAL} {CRITICO}))
			(= local11 global143)
		)
	)
	...
	(if (and temp0 (!= gCurRoomNum 544) (!= (gCurRoom curPic:) 699))
		...
		(proc0_12 local10)
		(= local10
			(proc0_12 (Format @local12 704 0 temp0) 67 105 180 70 25 28 local11) ; "%d"
		)
	)
)
...
(procedure (localproc_4 param1 &tmp [temp0 90])
	(Print
...
(instance droid1 of SecurityDroid
	...
	(method (doit)
		...
		(switch room
			(525
				(cond
					((and (self inRect: 0 0 64 40) (== level 3))
						(self setPri: 4)
					)
					((and (self inRect: 0 72 114 104) (== level 3))
						(self setPri: 5)
					)
				)
			)
...
(instance droidPath1 of RegionPath
	...
	(method (at param1)
		...
		(return [local202 param1])
	)

	(method (next)
		(if (and (OneOf value 71 205) (!= gCurRoomNum 515))
			(+= value 2)
			(client posn: [local202 (++ value)] [local202 (++ value)])
		)
		(super next:)
	)
...
(instance terminal1 of Feature
	...
	(method (doVerb theVerb invItem)
		(if (!= (brain level:) 1)
			(Print 704 2) ; "You can't reach it from this level."
		else
			(switch theVerb
				(2 ; Look
				...
				(if register
					(cond
						((not (== ((gInventory at: 11) owner:) 1)) ; plug
							(Print 704 3) ; "The PocketPal(tm) doesn't plug in directly. Maybe you need some kind of adapter."
						)
						((not (== ((gInventory at: 4) owner:) 1)) ; battery
							(Print 704 4) ; "You turn the power on, but nothing happens."
						)
						((!= ((gInventory at: 11) state:) global170) ; plug
							(Print 704 5) ; "The adapter plug doesn't seem to fit into Slot A of the interface."
						)
						(else
