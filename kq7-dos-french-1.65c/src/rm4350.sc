	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(13
				...
				(diggerRegister setCycle: Fwd setMotion: MoveTo 85 103 self)
			...
			(15
				...
				(diggerRegister loop: 2 cel: 0 x: 171 y: 109 setCycle: End self)
			)
			(16
				(diggerRegister loop: 3 cel: 0 setCycle: Fwd)
				(deadFall
					view: 4358
					loop: 1
					cel: 0
			...
			(19)
			(20
				(gEgo put: 49 normalize: 7) ; Grave_Digger_s_Horn
				(gKqMusic1 fade:)
				(sfx stop:)
				(sfx2 stop:)
				(hole init:)
				(diggerRegister hide: z: 1000)
				(SetFlag 172)
				(gGame handsOn:)
				(self dispose:)
			)
...
(instance diggerComes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(diggerRegister init: hide:)
				(digTalk
					clientCel: -2
					loop: 21
					view: 43550
					client: diggerRegister
				)
				(if (> (gEgo x:) 165)
					(gEgo setMotion: PolyPath 150 120 self)
				else
			...
			(9
				...
				(diggerRegister
					setLoop: 1 1
					setCycle: Rev
					setMotion: MoveTo 319 (diggerRegister y:) self
				)
			)
			(10
				(sfx fade: 0 10 2 1)
...
(instance diggerRegister of Actor
	(properties
		view 43550
	)
)
