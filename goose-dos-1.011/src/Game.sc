...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] [str 40])
	...
	(if (and (not global140) (DeviceInfo diPATHS_EQUAL @saveDevice @curDevice) (DeviceInfo diIS_FLOPPY @curDevice))
...
(class Game of Obj
	...
	(method (save &tmp [comment 20] num [oldCur 75] [oldPause 51])
		(if (PromptForDiskChange 1)
			(if gModelessDialog
				(gModelessDialog dispose:)
			)
			(= global240 0)
			(Format @oldPause 994 0 global131) ; "%d"
			(StrCat @global240 @oldPause)
			(Format @oldPause 994 0 global114) ; "%d"
			(StrCat @global240 @oldPause)
