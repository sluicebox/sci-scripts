...
(instance MG of Game
	...
	(method (init)
		...
		(User blocks: 0 canControl: 0 x: -1 y: 150)
		(= gEgo ego)
		(User alterEgo: gEgo)
		(= gPossibleScore 18)
		(TheMenuBar init:)
	...
	(method (handleEvent event &tmp [temp0 80] temp80 temp81 temp82 temp83 temp84 temp85 temp86 temp87 temp88 temp89 temp90 [temp91 50])
		(if (event claimed:)
			(return)
		)
				...
				(if global100
					(cond
						((== temp89 $000a) ; emALT | emLEFT_SHIFT
							(event claimed: 1)
							((User alterEgo:)
								setMotion: Unknown_Class_35 temp87 temp88
							)
						)
						((& temp89 emSHIFT)
							(event claimed: 1)
			...
			(evKEYBOARD
				(switch (event message:)
					...
					(KEY_F2
						...
						(= temp80 (Sound pause: 1))
						(switch global208
							(0
								(Print 0 7 #time 5) ; "Music and Sound Effects on."
							...
							(1
								(Print 0 8 #time 5) ; "Music Off. Sound Effects on."
							...
							(2
								(Print 0 9 #time 5) ; "Music and Sound Effects off."
								(SetMenu 1026 110 {Turn On})
							)
						)
						(DoSound sndPAUSE temp80)
					)
				)
			)
		)
	...
	(method (wordFail param1 &tmp [temp0 100])
		(Print (Format @temp0 0 10 param1)) ; "Try another word. Apparently, "%s" is not in the Mother Goose dictionary."
	)

	(method (syntaxFail)
		(Print 0 11) ; "Mother Goose does not understand. Please try again."
	)

	(method (pragmaFail &tmp [temp0 100])
		(Print 0 12) ; "Pardon me. Mother Goose does not understand your sentence. Please try something else."
	)
)
