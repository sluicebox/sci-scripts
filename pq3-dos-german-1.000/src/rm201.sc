...
(procedure (localproc_2 &tmp temp0 [temp1 200])
	...
	(cond
		...
		((and (> (- (temp0 y:) [local32 (+ (* 2 local10) 1)]) 2) 1)
			(= global186 (Memory memALLOC_CRIT (StrLen (temp0 description:))))
			(= global192 (Memory memALLOC_CRIT (StrLen (temp0 description:))))
			(StrSplitInTwo global186 global192 (temp0 description:))
...
(instance rm201 of Rm
	...
	(method (init &tmp temp0)
		...
		(lArrow init:)
		(rArrow init:)
		(trashBut view: (LangSwitch 2004 2005) x: (LangSwitch 23 28) init:)
		(quitBut view: (LangSwitch 2004 2005) x: (LangSwitch 296 299) init:)
		(helpBut view: (LangSwitch 2004 2005) x: (LangSwitch 255 250) init:)
		(searchBut view: (LangSwitch 2004 2005) x: (LangSwitch 72 90) init:)
		(lFace init:)
		(cFace init:)
		(rFace init:)
		(dummy init:)
...
(instance searchScript of rmnScript
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(localproc_1 201 9 (LangSwitch 20 15) 170 (LangSwitch 70 85)) ; "searching..."
				(= seconds 1)
			)
			(1
				(Graph grFILL_BOX 170 (LangSwitch 20 15) 180 (LangSwitch 70 85) 1 0 -1 -1)
				(Graph grUPDATE_BOX 170 (LangSwitch 20 15) 180 (LangSwitch 70 85) 1)
				(if (< local5 3)
					(= state -1)
					(++ local5)
				)
	...
	(method (handleEvent event)
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (LangSwitch 99 108))
						(== (event message:) (LangSwitch 67 76))
				...
				(if
					(or
						(and
							(== (gCurRoom script:) runDraw)
							(or
								(== (event message:) (LangSwitch 115 118))
								(== (event message:) (LangSwitch 83 86))
							)
						)
						(and
							(== (event message:) KEY_RETURN)
