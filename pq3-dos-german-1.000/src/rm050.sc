...
(instance envelope of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(2 ; Look
				(Print 50 25) ; "There's a large manilla envelope on the counter."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance cabinets of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 26) ; "Knowing what a strange individual Leon is, you decide to stay out of his cabinets."
			)
			(2 ; Look
				(Print 50 27) ; "Leon certainly has a lot of storage space in here."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance chart of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 28) ; "Even if Leon GAVE you that chart, you wouldn't take it."
			)
			(2 ; Look
				(Print 50 29) ; "You'd rather not look at that chart on the wall. You see enough of that stuff at accident scenes."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bottles of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 30) ; "You don't have much call for embalming fluid, Sergeant."
			)
			(2 ; Look
				(Print 50 31) ; "The bottles contain embalming fluid."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance table of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 32) ; "There'll be time enough for you to be on the table when you're dead."
			)
			(2 ; Look
				(Print 50 33) ; "The sight of the cold, stainless steel autopsy table sends a chill through the spine as you recall the many times you've come close to ending up on one."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance instruments of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 34) ; "This isn't "Life & Death". There's a REASON you didn't go into forensics. Just looking at those instruments makes you queasy."
			)
			(2 ; Look
				(Print 50 35) ; "A number of cruel-looking stainless steel instruments lie on a towel on the counter."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance skeleton of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 36) ; "You'd better leave "Demi" alone. You know how Leon is about his bones."
			)
			(2 ; Look
				(Print 50 37) ; "That's Leon's 'girlfriend', Demi Bones."
			)
			(else
				(super doVerb: theVerb)
			)
					...
					(switch (local5 corpseType:)
						(0
							(if 1
								...
								(Printf 50 38 global186 global192) ; "The toe tag reads "%s"."
						...
						(1
							...
							(Printf 50 39) ; "Good old Leon. He has so much fun naming the John Does."
						)
						(2
							(if 1
								(= global186
								...
								(StrSplitInTwo global186 global192 (local5 toeStr:))
								(Printf 50 40 global186 global192) ; "The toe tag reads "%s" - the poor stiff you found in the alley."
								(Memory memFREE global186)
								(Memory memFREE global192)
							)
						)
						(3
							...
							(Printf 50 41) ; "Underneath the name is a note: "Bonds - Caught you peeking! Stepped out for a bite. Be back soon - Leon.""
							(= local6 1)
						)
					)
				else
...
(instance sink of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 42) ; "This place does have a way of making you want to wash your hands, doesn't it?"
			)
			(2 ; Look
				(Print 50 43) ; "The Coroner's office is equipped with a sink. You hate to think about what washes down those drains!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance embalmMachine of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(Print 50 44) ; "I don't think Leon would appreciate your playing with his toys."
			)
			(2 ; Look
				(Print 50 45) ; "The embalming machine is a kind of mechanical vampire. Out with the old, in with the new..."
			)
			(else
				(super doVerb: theVerb)
			)
