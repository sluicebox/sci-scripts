...
(instance locker of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 10) ; "That's your locker."
			)
			(3 ; Do
				(HandsOff)
			...
			(4 ; Inventory
				(switch invItem
					(1 ; gun
						(Print 16 11) ; "This is the men's locker room - not target practice."
					)
					(else
						(Print 16 12) ; "Try your combination."
					)
				)
			)
			(else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(cond
					((== gDay 1)
						(Print 16 13) ; "Oops. The t.p. dispenser is empty. Good thing you noticed it while you're still standing."
					)
					((or (== gDay 2) (== gDay 3))
						(Print 16 14) ; "Someone's used up all the toilet paper. You'll have to wait for the janitor to fill the dispenser."
					)
					((== gDay 4)
						(Print 16 15) ; "The dispenser is empty."
					)
					((== gDay 5)
						(Print 16 16) ; "The janitor hasn't filled the dispenser with paper yet."
					)
					(
						(and
							(== gDay 6)
						...
						(Print 16 17) ; "The "Roll King"dispenser is filled with a fresh new roll of standard issue, two-ply "Timbersoft" toilet paper."
					)
					(
						(or
							(== gDay 7)
						...
						(Print 16 18) ; "The roll is empty. You emptied it, remember?"
					)
					(else
						(Print 16 19) ; "It seems the last person used all the toilet paper."
					)
				)
			)
			(3 ; Do
				(cond
					...
					((== gDay 1)
						(Print 16 20) ; "Oops. The t.p. dispenser is empty. Good thing you notice it while you're still standing."
					)
					((== gDay 2)
						(Print 16 21) ; "The toilet paper is all gone. You begin to wonder if the janitor is doing his job."
					)
					((== gDay 3)
						(Print 16 22) ; "They can put men on the moon... but they can't make a dispenser that won't run out of paper!"
					)
					((== gDay 4)
						(Print 16 23) ; "Damn! No paper!"
					)
					((== gDay 5)
						(Print 16 24) ; "No time to sit now. Besides, the toilet paper is all gone. You leave the janitor a note to fill the toilet paper dispenser."
					)
					(
						(and
							(== gDay 6)
					...
					(else
						(Print 16 25) ; "No paper! You wish the chief would stop using this stall."
					)
				)
			)
			(4 ; Inventory
				(Print 16 26) ; "I would have thought you would know the use for toilet paper."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance toilet of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 27) ; "Just your regular ol' run-of-the-mill john."
			)
			(3 ; Do
				(if (& (gEgo onControl: 0) $1000)
					(self setScript: flushScript)
				else
					(Print 16 28) ; "You can't do that from there..."
				)
			)
			(4 ; Inventory
				(switch invItem
					...
					(else
						(Print 16 29) ; "They have a name for people who are obsessed with toilets, you know."
					)
				)
			)
			(else
...
(instance stallDoor of Prop
	...
	(method (doVerb theVerb invItem &tmp temp0)
		(switch theVerb
			(2 ; Look
				(Print 16 30) ; "The toilet stalls lend a pleasant ambiance to the locker room."
			)
			(3 ; Do
...
(instance lockerInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 31) ; "You store some of your equipment in this locker."
			)
			(3 ; Do
				(self hide:)
				(stickInset hide:)
...
(instance stickInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 32) ; "This is your PR\24 (nightstick)."
			)
			(3 ; Do
				(Print 16 33) ; "You remove your nightstick."
...
(instance noteInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 34) ; "This is your notebook."
			)
			(3 ; Do
				(Print 16 35) ; "You remove your notebook."
...
(instance flashInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 36) ; "This is your flashlight."
			)
			(3 ; Do
				(Print 16 37) ; "You remove your flashlight."
...
(instance janitor of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 38) ; "Looks as if the janitor is having trouble with your handy work. That should keep him busy for quite a while."
			)
			(5 ; Talk
				(HandsOff)
...
(instance otherLocker of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 39) ; "This is the men's locker room. Hence, the lockers."
			)
			(3 ; Do
				(Print 16 40) ; "That locker belongs to somebody else, Bonds!"
			)
			(4 ; Inventory
				(Print 16 40) ; "That locker belongs to somebody else, Bonds!"
			)
...
(instance bench of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 41) ; "The benches provide a place to sit while you're getting dressed."
			)
			(3 ; Do
				(Print 16 42) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance theStalls of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 43) ; "The toilet stalls lend a pleasant ambiance to the locker room."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance theRoom of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 16 44) ; "You're standing in the men's locker room."
			)
			(else
				(super doVerb: theVerb invItem)
			)
