	...
	(method (doit)
		(cond
			...
			((& (gEgo onControl: 1) $0008)
				(HandsOff)
				(self posn: 215 108 setScript: (ScriptID 896 2) 0 19) ; sWO
			)
		)
		(super doit:)
	)
...
(instance homicideDoor of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 3) ; "That's the door to the Homicide office."
			)
			(3 ; Do
				(HandsOff)
...
(instance speakers of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 4) ; "The station's public address system."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance nightWin of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 5) ; "The window overlooks scenic downtown Lytton."
			)
			(3 ; Do
				(Print 18 6) ; "You don't need to open the window."
			)
			(4 ; Inventory
				(Print 18 7) ; "That won't get you anywhere."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance homWindow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 8) ; "That's Homicide's hall window."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance bulletinBoard of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 9) ; "The bulletin board is where people post articles of interest, personal ads, and chinese restaurant business cards."
			)
			(3 ; Do
				(if (== (Random 0 100) 99)
					(Print 18 10) ; "Thanks to the gang of Doug Oldfield, Chris Hoyt, Kim Bowdish and Mike Larson. The programmers who really deserve the main credit for this game."
				else
					(Print 18 11) ; "You look over the bulletin board, but see nothing of interest."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance carpet of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 12) ; "Hmm, 'policeman blue'."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance theRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 13) ; "You're in the hallway of the second level of the Lytton Police Station. Several office doors line the hallway."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance elFeat of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 18 14) ; "You are standing in the elevator."
			)
			(else
				(super doVerb: theVerb)
			)
