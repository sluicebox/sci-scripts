		...
		(switch theVerb
			...
			(5 ; Talk
				(switch (++ local8)
					...
					(else
						(Print 48 33) ; "The guy's busy drowning his sorrows and doesn't pay any attention to you."
					)
				)
			)
			(3 ; Do
				(Print 48 34) ; "He hasn't done anything wrong. Leave him alone."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sPatron2Talks of rmnScript
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= save1 1)
				(Say gEgo 48 35) ; "Excuse me sir, is that your car parked right out front?"
				(= seconds 4)
			)
			(1
				(= save1 1)
				(Say patron2 48 36) ; "Not me. I drive a Harley, man!"
				(= seconds 4)
			)
			(2
				(= save1 1)
				(Say gEgo 48 37) ; "Sorry to bother you."
				(= seconds 4)
			)
			(3
				(HandsOn)
...
(instance poolTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 38) ; "Touching the pool table while someone is playing isn't a good idea."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance boose of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 39) ; "If you want a drink just order one."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tables of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 40) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance poolCues of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 41) ; "Ahh... if you only had time!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tvFeature of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 42) ; "You don't have time to watch television."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance barStools of Feature ; UNUSED
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Print 48 43) ; "You don't have time to sit down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
