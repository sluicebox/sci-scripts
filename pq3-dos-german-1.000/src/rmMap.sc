...
(instance rmMap of Rm
	...
	(method (init)
		...
		(pointBut view: (LangSwitch 197 1971) x: (LangSwitch 50 54) init:)
		(drawBut view: (LangSwitch 197 1971) x: (LangSwitch 156 167) init:)
		(cleanBut view: (LangSwitch 197 1971) x: (LangSwitch 237 238) init:)
		(gTheIconBar disable:)
	)
...
(instance addPoint of Script
	...
	(method (doit &tmp [temp0 15] temp15)
		(super doit:)
		(if (& (OnControl CONTROL gMouseX gMouseY) $007e)
			...
			(if 1
				(= global186 (Memory memALLOC_CRIT (StrLen temp15)))
				(= global192 (Memory memALLOC_CRIT (StrLen temp15)))
				(StrSplitInTwo global186 global192 temp15)
				(Display
					(Format ; "%3d %s"
						@temp0
						202
			...
			(Display
				(Format @temp0 202 4 (* 100 (Abs (/ (- 91 gMouseY) 16))) temp15) ; "%3d %s"
				dsFONT
				1
				dsCOORD
				20
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (LangSwitch 112 102))
						(== (event message:) (LangSwitch 80 70))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
					(self loop: 1 forceUpd:)
					(Animate (gCast elements:) 0)
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (LangSwitch 99 112))
						(== (event message:) (LangSwitch 67 80))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
	...
	(method (handleEvent event &tmp [temp0 2])
		(switch (event type:)
			(evKEYBOARD
				(if
					(or
						(== (event message:) (LangSwitch 101 108))
						(== (event message:) (LangSwitch 69 76))
						(and (== (event message:) KEY_RETURN) (MousedOn self event))
					)
					(self loop: 1 forceUpd:)
					(Animate (gCast elements:) 0)
		...
		(switch (event type:)
			...
			(evKEYBOARD
				(cond
					(
						(or
							(== (= temp1 (event message:)) (LangSwitch 101 108))
							(== temp1 (LangSwitch 69 76))
						)
						(self clean:)
					)
					(
						(or
							(== temp1 (LangSwitch 113 101))
							(== temp1 (LangSwitch 81 69))
						)
						(= state 0)
						(self exit:)
						(return)
					...
					(
						(or
							(== temp1 (LangSwitch 112 102))
							(== temp1 (LangSwitch 80 70))
						)
						(= local7 1)
						(self exit:)
						(return)
