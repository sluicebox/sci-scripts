...
(procedure (localproc_2 param1)
	(= local10 2)
	(PrintDebug {actor's scaler now 0})
	(localproc_1)
				...
				(switch (event message:)
					(KEY_DELETE
						(if (not (act9 onMeCheck:))
							...
							(= local10 2)
							(PrintDebug {function replaced by avirobot})
						)
					)
					(KEY_DIVIDE
					...
					(else
						(cond
							((OneOf (event message:) JOY_DOWNRIGHT JOY_DOWNRIGHT)
								(= temp0 (local29 view:))
								(= local10 2)
								(PrintDebug {actors view unloaded from memory})
								(local29 view: 2)
								(localproc_0)
								(UnLoad 128 temp0)
								(Purge 16000)
							...
							((OneOf (event message:) $000f $000f)
								(if (gCurRoom obstacles:)
									(= local10 2)
									(PrintDebug {removing room's obstacles})
							...
							((OneOf (event message:) KEY_T KEY_t)
								(cond
									((or local3 local7)
										...
										(= local10 2)
										(= local3 0)
										(PrintDebug {turned off robot looping})
										(DoRobot)
									)
									((act9 onMeCheck:)
										(= local7 0)
