			...
			(
				(and
					...
					(or
						...
						(and (== local4 964) (or (== local5 61) (== local5 6)))
		...
		(switch param1
			...
			(940
				...
				(switch local10
					...
					(930
						(= local5 (determineFrame doit: 130 160 21 61))
						(= local10 947)
...
(instance rotateLever of TrackingProp
	...
	(method (cue)
		...
		(if (and (== global229 2) local3 (IsFlag 273))
			...
			(switch theVerb
				(5 ; Do
					(if
						...
						(PrintDebug {returning on first condition.})
						(return 0)
					)
					(if
						(and
						...
						(if (not (getRobot doit:))
							(return
								(PrintDebug
									{returning because getRobot said to.}
								)
							)
						)
					)
						...
						(switch (throttle cel:)
							(7
								(if local8
									(return
										(PrintDebug
											{returning because cel is 7 and still have botSpeed.}
										)
									)
								)
								(= local29 0)
						...
						(switch (smallThrottle cel:)
							(7
								(if local8
									(return
										(PrintDebug
											{returning because smallThrottle cel is 7 and still have botSpeed.}
										)
									)
								)
								(= local29 0)
						...
						(switch (smallThrottle cel:)
							(7
								(if local8
									(return
										(PrintDebug
											{ForwardFeat is returning because botSpeed still has a value.}
										)
									)
								)
								(= local29 0)
...
(instance trainBot of RobotPlayer
	...
	(method (doit)
		...
		(if (<= local8 1)
			(PrintDebug {Slowed down enough to stop})
			(= local8 0)
			(self stop:)
		)
		(switch local4
				...
				((== local27 0)
					(if
						...
						(if (or (== local5 (self maxFrame:)) (== local5 0))
							...
							(personChanger cue:)
							(PrintDebug {just cued the personChanger\n})
							(= global229 0)
							(self start: local9 local8)
						else
							(= local22 0)
...
(instance rotateTrain of Code
	...
	(method (doit &tmp temp0 temp1 temp2)
		(PrintDebug {quarterTurns = %d} local3)
		(switch global237
			(2
				(= temp2 986)
			)
