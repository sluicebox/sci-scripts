...
(instance sSendPackage of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gCurRoom scratch: 0)
				(PlayScene 1158 self 130) ; srLibRoom
			)
		)
	)
)
	...
	(method (doVerb theVerb)
		...
		(return
			(switch theVerb
				...
				(80 ; invLudwigBio
					(cond
						((not (IsFlag 281)) ; "Ludwig's Diary" in rm8412Topic
	...
	(method (doVerb theVerb)
		(cond
			...
			(
				(and
					(== theVerb 62) ; Do
					(IsFlag 179) ; "Ludwig II" in gastTopicRm
					(IsFlag 150) ; "Church File" in rm711c2d2
					(IsFlag 465)
			...
			((and (== gChapter 4) (IsFlag 302)) ; "Special Permit" in srHallCh4
				(if (IsFlag 634)
