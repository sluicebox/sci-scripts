	...
	(if (temp0 size:)
		...
		(switch local1
			(-1
				...
				(localproc_12)
				(if (== gChapter 3)
					(= local2 (CD 0 2 1)) ; Check
				else
					(= local2 (CD 0 gChapter 1)) ; Check
				)
				(InitChapter)
				(return)
			)
		...
		(==
			(= gChapter
				(Print
					y: (+ (local0 at: 3) 14)
					font: 50
					addTitle: {Choose Chapter}
					addButton: 1 {Chapter 1} -1 0
					addButton: 2 {Chapter 2} -1 20
					addButton: 3 {Chapter 3} -1 40
					addButton: 4 {Chapter 4} -1 60
					addButton: 5 {Chapter 5} -1 80
					addButton: 6 {Chapter 6} -1 100
