...
(instance rm047 of KQ5Room
	...
	(method (doit)
		(super doit:)
		(if (and (gCast contains: cedric) (not (gCurRoom script:)))
			(cond
				((== (sailBoat loop:) 0)
					(cedric
						posn: (+ (sailBoat x:) 20) (+ (sailBoat y:) 4)
...
(instance sink of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(sailBoat cycleSpeed: 5 setCycle: End self)
				(if (and (not (IsFlag 54)) (not (IsFlag 55)))
					(cedric view: 259 loop: 1 cycleSpeed: 5 setCycle: End)
				)
			)
			(3
				(= global330
...
(instance islets2 of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 47 7) ; "Way in the distance, Graham can make out small, rocky islands."
					(event claimed: 1)
				)
			)
		)
...
(instance cedric of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(5 ; Talk
					(PrintDC 47 8) ; "Cedric isn't in the mood to talk."
					(event claimed: 1)
				)
			)
		)
...
(instance rock1 of View ; UNUSED
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 47 9) ; "Many jagged rocks stand in the shallow waters."
					(event claimed: 1)
				)
			)
		)
...
(instance rock2 of View ; UNUSED
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 47 10) ; "Many jagged rocks stand in the shallow waters."
					(event claimed: 1)
				)
			)
		)
...
(instance seaMonsterHead of Actor
	...
	(method (doit)
		(super doit:)
		(self x: (seaMonster x:) y: (seaMonster y:))
	)
)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (gCast contains: cedric)
						(PrintDC 47 11) ; "Graham and Cedric spy an interesting island to the north."
					else
						(PrintDC 47 12) ; "Graham spies an interesting island to the north."
					)
					(event claimed: 1)
				)
			)
...
(instance gull1 of Actor ; UNUSED
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 47 13) ; "Graham notices beautiful seagulls flying in the distance."
					(event claimed: 1)
				)
			)
		)
