		...
		(switch theVerb
			(2
				(if (theObj lookStr:)
					...
					(Printf 950 0 objDesc objDesc) ; "The %s looks like any other %s.#SEl %s se parece a cualquier %s."
				)
			)
			(4
				(if (= iItem (gInventory at: invNo))
					(Printf ; "You clicked inv item %s on %s.#STu presionaste el art\a1culo inv %s en %s."
						950
						1
						(iItem description:)
						objDesc
			...
			(5
				(Printf 950 2 objDesc objDesc) ; "The %s has nothing to say.#SEl/ella %s no tiene nada que decir."
			)
		)
	)
)
