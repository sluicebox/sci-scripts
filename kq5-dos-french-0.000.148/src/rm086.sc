...
(instance useHammer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 86 1 #at 10 120 #time 8) ; "Using the cobbler's hammer, Graham pounds on the rusty lock until it breaks apart."
				((gEgo head:) hide:)
				(gEgo
					normal: 0
					view: 822
...
(instance rescue of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 86 3 #at 20 20 #dispose) ; "Struggle as he might, Graham cannot escape his bonds."
				(= seconds 7)
			)
			(2
				(cls)
			...
			(10
				(PrintDC 86 4 #at 10 140 #width 280 #dispose) ; "I told you I'd repay your kindness when you saved me from that horrible cat. Good luck, friend."
				(= seconds 11)
			)
			(11
				(cls)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 5) ; "A small set of rickety, wooden stairs lead up to the cellar door."
					(event claimed: 1)
				)
			)
		)
...
(instance ratHole of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 6) ; "In the corner of the cellar, Graham notices a large rathole."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 86 7) ; "There is nothing that Graham would be interested in inside the rathole."
					(event claimed: 1)
				)
			)
		)
...
(instance room of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 8) ; "Graham has unceremoniously found himself in the country inn's dusty, dirty cellar."
					(event claimed: 1)
				)
			)
		)
...
(instance door of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 9) ; "A small set of rickety, wooden stairs lead up to the cellar door."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local36)
						(PrintDC 86 10) ; "A rusty lock secures the door preventing Graham from leaving the cellar."
					else
						(HandsOff)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(1
							(PrintDC 86 11) ; "The key won't fit this lock."
							(event claimed: 1)
						)
						(22
							(if (not local36)
								...
								(PrintDC 86 12) ; "The lock has already been broken."
							)
							(event claimed: 1)
						)
						(28
						...
						(else
							(PrintDC 86 13) ; "That won't help Graham unlock the door."
							(event claimed: 1)
						)
					)
				)
...
(instance rope of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 14) ; "Lying in a heap on the cold, stone floor, Graham sees the rope that once held him captive."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
