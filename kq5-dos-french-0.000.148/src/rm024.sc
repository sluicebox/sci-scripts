					...
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(14
							(if (!= ((gInventory at: 6) owner:) 200) ; Brass_Bottle
								(PrintDC 24 21) ; "Graham decides to wait until he sees a good reason to toss away these fine emeralds."
								(event claimed: 1)
							else
								(if (not (gCurRoom script:))
									(SetScore 2)
						...
						(else
							(PrintDC 24 22) ; "The small creatures, whoever they are, don't seem to be interested in it."
							(event claimed: 1)
						)
					)
				)
...
(instance puddle of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 24 23) ; "A sticky mess of honey lies on the ground."
					(event claimed: 1)
				)
			)
		)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (< (self x:) 100)
						(PrintDC 24 24) ; "A large rock-like creature blocks further passage to the west."
					else
						(PrintDC 24 25) ; "Rocky has moved aside to allow Graham passage through the forest."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (> (self x:) 100)
						(PrintDC 24 26) ; "The creature is not inclined to move further."
						(event claimed: 1)
					)
				)
				(5 ; Talk
					(if (> (self x:) 100)
						(PrintDC 24 27) ; "Rocky isn't talking."
						(event claimed: 1)
					)
				)
			)
