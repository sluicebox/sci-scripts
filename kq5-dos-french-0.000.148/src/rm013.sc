		...
		(cond
			...
			((= local0 (self edgeToRoom: (gEgo edgeHit:)))
				(if (== local0 (gCurRoom west:))
					(gEgo loop: 1)
					(RedrawCast)
					(proc0_28 75 13 0 67 10 10 25 10) ; "There's nothing but a hot, dry desert further west. Most people avoid it, because there are bandits out there! If you insist on going, I'll wait for you HERE!"
...
(instance warnScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(proc0_28 75 13 1 67 20 100 25 7) ; "Keep your eye on the gypsies, Graham. I don't trust 'em."
				(client setScript: 0)
			)
		)
	)
...
(instance relievedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(proc0_28 75 13 2 67 20 100 25 7) ; "Well, there you are! I was starting to get concerned!"
				(proc0_28 160 13 3 67 20 20 25 8) ; "Don't worry about me, Cedric. I'm used to this kind of thing."
				(= cycles 1)
			)
			(3
				(HandsOn)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (not local2)
					(++ local2)
					(proc0_28 103 13 4 25 8) ; "Eet veel cost you one gold coin to see Madame Mushka."
				else
					(proc0_28 103 13 5 25 5) ; "Zee cost eez one gold coin."
				)
				(self cue:)
			)
			(4
...
(instance giveCoinSit of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(if local55
					(proc0_28 103 13 6 67 10 10 25 10) ; "Vell, eet eez not a gold coin, but I guess eet veel be fine. Go on een and see Madame Mushka."
				else
					(proc0_28 103 13 7 67 10 10 25 5) ; "You may see Madame Mushka now."
				)
				(gEgo
					normal: 1
					view: 0
...
(instance giveCoinStand of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if local55
					(proc0_28 103 13 8 67 10 10 25 10) ; "Vell, eet eez not a gold coin, but I guess eet veel be fine. Go on een and see Madame Mushka."
				else
					(proc0_28 103 13 9 67 10 10 25 5) ; "You may see Madame Mushka now."
				)
				(gypsy
					setCycle: Walk
					setLoop: 1
...
(instance getTambourine of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(PrintDC 13 10) ; "Not seeing the tambourine's owner, Graham bends down and rescues it from the ground."
				(gEgo
					normal: 1
					view: 0
					setCycle: Walk
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 11) ; "A gypsy wagon is encamped here at the edge of the woods. On the side of the wagon, the words "Fortune Teller" have been painted."
					(event claimed: 1)
				)
			)
		)
...
(instance door of Prop ; UNUSED
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 12) ; "The wagon door looks inviting to those who wish to have their fortunes told."
					(event claimed: 1)
				)
				(3 ; Do
...
(instance ox of Prop
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2
					(PrintDC 13 13) ; "A large ox, tied to the gypsies' wagon, peacefully munches on grass."
				)
				(3
					(PrintDC 13 14) ; "Graham finds the huge beast completely uninteresting."
				)
				(5
					(PrintDC 13 15) ; "The ox's mouth is too full of grass to even attempt a conversation with Graham!"
				)
				(4
					(switch invItem
						(28 1)
						(else
							(PrintDC 13 16) ; "The ox is not interested in any of Graham's possessions."
						)
					)
				)
			)
...
(instance body of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 17) ; "Within the gypsy encampment, Graham notices a dark, burly man who keeps a suspicious eye on him and Cedric."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 18) ; "The burly man keeps a watchful eye on Graham and Cedric."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(11
							(if (gEgo has: 27) ; Amulet
								(proc0_28 103 13 19 67 10 10 25 10) ; "Madame Mushka eez tired. No more for today."
							else
						...
						(4
							(proc0_28 103 13 20 67 10 10 25 10) ; "Eet costs one GOLD coin to see Madame Mushka, not one SILVER coin!"
							(event claimed: 1)
						)
						(28
							(event claimed: 0)
				...
				(5 ; Talk
					(PrintDC 13 21) ; "The sullen man doesn't look like the conversational sort."
					(event claimed: 1)
				)
			)
		)
				...
				(4 ; Inventory
					(if (not (gCast contains: gypsy))
						(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
							...
							(else
								(proc0_28 103 13 22) ; "Zat eez NOT a gold coin!"
								(event claimed: 1)
							)
						)
					)
...
(instance gypsy of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 23) ; "The dark gypsy man bars Graham's entry into the fortune teller's wagon without proper payment."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 18) ; "The burly man keeps a watchful eye on Graham and Cedric."
...
(instance coals of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 24) ; "A steaming pot of stew bubbles over a small campfire near the gypsy wagon."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 13 25) ; "Obviously, the man wouldn't tolerate Graham taking his supper."
					(event claimed: 1)
				)
			)
		)
...
(instance grassRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 26) ; "Graham notices a grassy clearing at the edge of the woods."
					(event claimed: 1)
				)
			)
		)
...
(instance tambourine of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 13 27) ; "A tambourine lies on the ground near the abandoned gypsy encampment."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
