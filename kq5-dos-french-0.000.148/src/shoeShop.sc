	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(5 ; Talk
					(PrintDC 205 25) ; "The old shoemaker wearily hammers tiny nails in a shoe sole. He doesn't even seem to notice Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 205 26) ; "The shoemaker isn't even paying any attention to Graham."
							(event claimed: 1)
						)
					)
				)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (gCast contains: woman)
						(PrintDC 205 27) ; "The old shoemaker, eyes squinted and fingers calloused from years of making shoes, drives tiny nails into a shoe sole with a small cobbler's hammer."
					else
						(PrintDC 205 28) ; "The shoeshop looks even more empty now that the shoemaker and his wife have left."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local1)
						(++ local1)
						(PrintDC 205 29) ; "The old shoemaker wearily hammers tiny nails in a shoe sole. He doesn't even seem to notice Graham."
						(event claimed: 1)
					)
				)
				(5 ; Talk
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(proc0_28 177 205 30 67 10 10 25 7) ; "I TOL' you we ain't got no shoes for sale right now! Come back another day."
							(event claimed: 1)
						)
					)
				)
...
(instance woman of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 205 31) ; "The shoemaker's wife, looking haggard and worn, tiredly stitches away at a large piece of shoe leather."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local2)
						(++ local2)
						(PrintDC 205 32) ; "The poor woman has enough troubles of her own without worrying about Graham's."
						(event claimed: 1)
					)
				)
				(5 ; Talk
					(switch local5
						...
						(1
							(PrintDC 205 33) ; "So as not to upset the poor woman further, Graham politely decides not to question her again."
							(event claimed: 1)
						)
					)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 205 34) ; "The old woman isn't interested in that."
							(event claimed: 1)
						)
					)
				)
...
(instance dog of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 205 35) ; "A skinny old dog lies down on the shoeshop's hard floor."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 205 36) ; "This old dog doesn't look very appealing."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 205 37) ; "The old dog pays no attention to Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 205 38) ; "The dog doesn't show any interest in it."
							(event claimed: 1)
						)
					)
				)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (gCast contains: woman)
						(PrintDC 205 39) ; "Business doesn't seem to be so good for the shoemaker and his wife. There isn't one pair of shoes for sale, and the old couple look worn out."
					else
						(PrintDC 205 40) ; "The old shop is empty now that the shoemaker and his wife have retired."
					)
					(event claimed: 1)
				)
			)
