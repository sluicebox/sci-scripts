	...
	(method (doit &tmp temp0 temp1)
		...
		(cond
			...
			((and (not (IsFlag 89)) (== global361 45) (gEgo has: 18)) ; Beeswax
				(SetFlag 89)
				(proc0_28 75 45 0 67 10 10 25 8) ; "Look Graham, there is a boat here! Maybe we can use it!"
			)
...
(instance rocks of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 45 5) ; "Jagged rocks surround the waterfall at the base of the cliff and then continue on into the ocean."
					(event claimed: 1)
				)
			)
		)
