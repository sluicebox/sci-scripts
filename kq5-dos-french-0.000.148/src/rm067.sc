...
(local
	...
	local18
	local19
	[local20 16] = [92 149 75 159 31 155 32 120 32 0 319 0 319 168 222 149]
...
(instance rm067 of KQ5Room
	...
	(method (doit &tmp temp0 temp1)
		...
		(if (and (& (gEgo onControl: 1) $0004) (gEgo mover:))
			...
			(splash cycleSpeed: (gEgo cycleSpeed:) posn: temp0 temp1 show:)
		else
			(splash hide:)
		)
		(cond
...
(instance moveStone of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(proc0_28 101 67 2 67 10 10 25 3) ; "Hello!"
				(proc0_28 160 67 3 67 100 10 25 5) ; "What? Princess Cassima! Where did you come from?!"
				(proc0_28 101 67 4 67 10 10 25 10) ; "From the labyrinth. I spend a lot of time down here, you know...with my friends."
				(proc0_28 160 67 5 67 100 10 25 3) ; "Friends?"
				(proc0_28 101 67 6 67 10 10 25 12) ; "Yes. Like Dink...and Sam. I don't know if you ever saw Sam or not. Anyway, I found this loose stone once that led here...to this cell. Now come on! You'd better get out of here!"
				(= cycles 1)
			)
			(8
				...
				(3 ; Do
					(if (not (gEgo has: 32)) ; Moldy_Cheese
						(switch local17
							...
							(1
								(PrintDC 67 20 #at 40 40) ; "Graham can't reach into the mousehole far enough to retrieve the piece of cheese."
								(event claimed: 1)
							)
						)
					else
						(PrintDC 67 21) ; "There is nothing more of interest inside the mousehole."
						(event claimed: 1)
					)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 67 22) ; "That implement won't quite work here."
							(event claimed: 1)
						)
					)
				)
...
(instance hole of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 67 23) ; "A fallen stone has revealed a large hole in the cell wall."
					(event claimed: 1)
				)
				(3 ; Do
					(proc550_17)
...
(instance dieScumScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(PrintDC 67 24) ; "After observing no discernible escape route from the small cell, Graham sinks to the floor in despair, knowing he will never see the light of day again."
				(= cycles 1)
			)
			(3
				((gEgo head:) hide:)
...
(instance grate of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 67 25) ; "Above him, Graham can see a damp, rusty grate leading to...who knows where?"
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 67 26) ; "The grate is too high up.  Graham can do nothing with it."
					(event claimed: 1)
				)
			)
		)
