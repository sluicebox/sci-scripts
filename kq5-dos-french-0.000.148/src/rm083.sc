...
(instance enterRoom of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo setCycle: SyncWalk setMotion: MoveTo 271 51 self)
			)
			(2
			...
			(5
				(if (!= ((gInventory at: 33) owner:) 83) ; Elf_Shoes
					...
					(PrintDC 83 2 #at 10 150 #dispose) ; "Over here! I've got something for you."
					(= seconds 5)
...
(instance dying of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(= global330
					{Gotta watch those drop-offs, Graham!}
				)
				(EgoDead 248)
			)
		)
