...
(local
	...
	[local103 16] = [162 152 162 166 152 175 106 175 75 169 43 163 50 159 80 152]
...
(instance useWell of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(PrintDC 15 3) ; "Ah! Life-giving water! Nectar of the gods! Graham can now feel renewal flowing through him."
...
(instance getBoot of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(PrintDC 15 4) ; "Uneasily, Graham reaches down and removes the old shoe from the desert sand."
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if local20
						(PrintDC 15 5) ; "All around him, Graham sees nothing but more desert. How much further can he go on? Thankfully, though, a nearby pool of water can relieve Graham's overwhelming thirst."
					else
						(PrintDC 15 6) ; "The scorching sun burns down on the dry desert as Graham struggles through the hot sand. He looks around, but all he can see is...more desert!"
					)
					(event claimed: 1)
				)
			)
...
(instance skeleton of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 15 7) ; "Oh, oh! A picked-clean and sun-bleached skeleton of a man lies in the sand of the hot, dry desert. What happened? Who can say...but it makes Graham uneasy nevertheless."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 15 8) ; "Graham can't do much to help the poor man now...and vice versa!"
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 15 9) ; "It's too late for conversation now!"
					(event claimed: 1)
				)
			)
		)
...
(instance bootInSand of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 15 10) ; "An old shoe lies, forgotten, near the poor man's skeleton."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
...
(instance rope of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 15 11) ; "A rope and water basket sit near a small well."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local18)
						...
						(PrintDC 15 12) ; "Graham has drank all the water he needs for now."
					)
					(event claimed: 1)
				)
			)
			...
			(switch (event message:)
				(2 ; Look
					(if (== (gCurRoom curPic:) 210)
						(PrintDC 15 13) ; "A small well! Graham's hot, thirsty body is irresistibly drawn to it."
					else
						(PrintDC 15 14) ; "A small oasis! Its tantalizing water, so sparkling in the desert sun! Graham's hot, thirsty body is irresistibly drawn to it."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local18)
						...
						(PrintDC 15 15) ; "Graham has drank all the water he needs for now."
					)
					(event claimed: 1)
				)
			)
