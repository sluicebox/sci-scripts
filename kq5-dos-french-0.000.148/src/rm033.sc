	...
	(method (doit &tmp temp0)
		(cond
			...
			((& (gEgo onControl: 0) $1000)
				(gEgo setPri: (- (gEgo priority:) 1) setLoop: 1)
			...
			((& (gEgo onControl: 0) $0004)
				...
				(gEgo setPri: (- (gEgo priority:) 2) setLoop: 0)
				(self setScript: falling)
			)
			(
				(or
...
(instance bridge of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 33 6) ; "A deep crevasse splits the snowy mountain ridge in two. A thin strip of ice crosses it. It looks too wide to jump across."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 33 7) ; "Graham thinks about attempting to jump across the crevasse, but wisely chooses not to."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(29
							(PrintDC 33 8) ; "Perhaps with some momentum Graham could use this to cross the crevasse."
							(event claimed: 1)
						)
						(28
							(event claimed: 0)
						...
						(else
							(PrintDC 33 9) ; "It won't help Graham cross the crevasse."
							(event claimed: 1)
						)
					)
				)
...
(instance mountains of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 33 10) ; "In the distance, snowy mountains loom above a lovely river valley."
					(event claimed: 1)
				)
			)
		)
...
(instance sled of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 33 11) ; "Graham's sled lies in several broken pieces on the eastern side of the icy crevasse."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local0)
						(++ local0)
						(PrintDC 33 12) ; "The broken sled is now no good to anybody."
						(event claimed: 1)
					)
				)
				(4 ; Inventory
