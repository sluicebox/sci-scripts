			...
			(switch (event message:)
				(2 ; Look
					(if (not (IsFlag 21))
						...
						(PrintDC 64 8) ; "Numerous books of various scholarly subjects lie scattered atop a nearby table."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 64 9) ; "None of these particular books interest Graham."
					(event claimed: 1)
				)
			)
		)
