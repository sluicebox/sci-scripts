					...
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(3
							(if (not (gEgo has: 29)) ; Sled
								...
								(PrintDC 204 57) ; "Graham sees nothing else of interest in the toyshop."
							)
							(event claimed: 1)
						)
						(9
						...
						(4
							(if (not (gEgo has: 29)) ; Sled
								...
								(RedrawCast)
								(proc0_28 160 204 58 67 10 10 25 6) ; "Can I buy the sled with a silver coin?"
								(proc0_28 192 204 59 67 10 10 25 8) ; "No, I'm sorry. I'd love to sell it to you, but I'm afraid I need a bit more than that."
							else
								(PrintDC 204 60) ; "Graham sees nothing else of interest in the toyshop."
							)
							(event claimed: 1)
						)
						(11
						...
						(else
							(PrintDC 204 61 #at 10 10) ; "The toymaker wouldn't find that suitable for his business."
							(event claimed: 1)
						)
					)
				)
...
(instance sled of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 62 #at 10 10) ; "Hanging on a side wall, Graham notices a strong little sled."
					(event claimed: 1)
				)
				(3 ; Do
					(if (not local2)
						...
						(PrintDC 204 63 #at 10 10) ; "The little sled has captured Graham's interest."
					)
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(4
							(proc0_28 160 204 64 67 10 10 25 6) ; "Can I buy the sled with a silver coin?"
							(proc0_28 192 204 65 67 10 10 25 8) ; "No, I'm sorry. I'd love to sell it to you, but I'm afraid I need a bit more than that."
							(event claimed: 1)
						)
						(else
							(PrintDC 204 61 #at 10 10) ; "The toymaker wouldn't find that suitable for his business."
...
(instance door of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 66 #at 10 10) ; "Apparently the toymaker's son, working in the back room, is learning the toy business from his father."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 204 67 #at 10 10) ; "The back door is for employees only."
					(event claimed: 1)
				)
			)
		)
...
(instance fishBowl of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 68 #at 10 10) ; "A pet goldfish swims endlessly around its fishbowl."
					(event claimed: 1)
				)
			)
		)
...
(instance girl of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 69 #at 10 10) ; "A pretty little girl, who must be the toymaker's granddaughter, plays with a doll while Grandfather looks on."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 204 70 #at 10 10) ; "The little girl seems happy enough playing with the doll. She needs nothing more."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 204 71 #at 10 10) ; "The little girl is too interested in her doll. She pays no attention to Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 204 72) ; "The little girl only seems to care about the doll she's playing with."
							(event claimed: 1)
						)
					)
				)
...
(instance shopRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 204 73 #at 10 10) ; "A child's wonderland of toys fills this cute little toyshop."
					(event claimed: 1)
				)
			)
		)
