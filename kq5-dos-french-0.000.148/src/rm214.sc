...
(instance door of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 214 8) ; "Looming before him, the huge temple door beckons tantalizingly to Graham."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 214 9) ; "It has no effect on the massive door."
							(event claimed: 1)
						)
					)
				)
...
(instance room of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 214 10) ; "Built into the rocky cliffs, an ancient temple towers above Graham as he surveys its ornately carved columns and friezes."
					(event claimed: 1)
				)
			)
		)
...
(instance brokenStaff of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 214 11) ; "The staff lies in several broken pieces on the temple steps."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 214 12) ; "The staff is broken and is of no use anymore."
					(event claimed: 1)
				)
			)
		)
