...
(instance bees of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 19) ; "A swarm of bees buzz around a hole in a rotted old tree."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 11 20) ; "Bees don't have much to say."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 11 21) ; "It would be wise if Graham didn't bother the bees."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 11 22) ; "The bees don't care about it."
							(event claimed: 1)
						)
					)
				)
...
(instance stick of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 23) ; "Graham notices a large stick lying on the ground near the old tree."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
