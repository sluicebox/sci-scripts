...
(instance KQInv of InvTRANSLATE
	(properties
					...
					(global312
						(StrCpy
							...
							(LangSwitch
								...
								{Cette escarcelle de cuir contient }
							)
						)
						(StrCat
							@temp0
...
(instance Emeralds of KQInvItem
	...
	(method (doVerb theVerb &tmp [temp0 50])
		(switch theVerb
			(2
				(StrCpy @temp0 (LangSwitch {The } {Les }))
				(StrCat
					@temp0
					(switch global312
						(3
							(LangSwitch {trio of } {trio de})
						)
						(2
							(LangSwitch {pair of } {paire de})
						)
						(1 {})
					)
				)
				(StrCat
					@temp0
					(LangSwitch {sparkling emerald} {\82meraude \82tincellante})
				)
				(StrCat @temp0 (if (> global312 1) {s} else {}))
				(StrCat @temp0 (LangSwitch { dazzle} { \82blouissement}))
				(StrCat @temp0 (if (> global312 1) {} else {s}))
				(StrCat @temp0 (LangSwitch { the eyes.} { les yeux.}))
				(PrintDC @temp0)
			)
			(else
				(super doVerb: theVerb)
...
(instance Leg_of_Lamb of KQInvItem
	...
	(method (doVerb theVerb &tmp [temp0 50])
		(switch theVerb
			(2
				(StrCpy @temp0 (LangSwitch {This is a } {C'est un }))
				(StrCat
					@temp0
					(if global316
						(LangSwitch {left-over, half} {reste,demi})
					else
						(LangSwitch {large, mouthwatering} {gros, succulent})
					)
				)
				(StrCat @temp0 (LangSwitch { leg of lamb.} { jambe d'agneau}))
				(PrintDC @temp0)
			)
			(else
				(super doVerb: theVerb)
...
(instance Elf_Shoes of KQInvItem
	(properties
		name {Elf Shoes}
		view 892
		loop 2
