...
(instance startTrn of Script
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11)
		(switch (= state newState)
			...
			(2
				(if
					...
					(for
						...
						(if (!= (((ScriptID 1 2) at: temp4) playing:) 3) ; players
							(if (not temp11)
								(= temp11 ((ScriptID 1 2) at: temp4)) ; players
							)
							(++ temp5)
						)
					)
					(if (> temp5 1)
						(switch
							(Print ; "What should be done with the remaining players?"
								111
								0
								#button
								{Continue}
				...
				(if (not global448)
					...
					(global302 xcred: 0)
					(= local12 ((global302 durables:) objectAtIndexQuan: 33))
					(= local13 ((global302 durables:) objectAtIndexQuan: 32))
					(= local14 ((global302 durables:) objectAtIndexQuan: 31))
					(= local15 ((global302 durables:) objectAtIndexQuan: 29))
					(if (and local12 local13 local14)
						(global302 xcred: (+ (global302 xcred:) 1))
					)
					(if local15
						(global302 xcred: (+ (global302 xcred:) 1))
					)
					...
					(if (= local3 ((global302 consumables:) objectAtIndex: 9))
						(if (> (local3 quantity:) (= global553 (Random 0 500)))
							(= local4
								(cond
									((<= global553 (/ (local3 quantity:) 20)) 5000)
									((<= global553 (/ (local3 quantity:) 5)) 500)
									(else 200)
								)
							)
							(proc0_13 (if (== local4 5000) 10 else 5))
							(proc0_10 local4)
							(self
								setScript:
			...
			(4
				(if
					...
					(proc0_13 3)
					(proc0_10 (= local10 (Random 20 100)))
					(self setScript: moveNotice 0 10 local10)
					(return)
				)
				(self cue:)
			)
			...
			(10
				(if
					(and
						(= local6 ((global302 consumables:) objectAtIndex: 1))
						(local6 quantity:)
					)
					(cond
			...
			(14
				...
				(if (and (> (proc0_11) 0) (not global553))
					...
					(proc0_10 (- 0 local7))
					(self setScript: moveNotice 0 3 local7)
					(return)
				)
				(self cue:)
			)
				...
				(for
					...
					(if
						(and
							...
							(>=
								(((global302 durables:) at: local0) quantity:)
								1
							)
							(or
								(not
						...
						(if (not (Random 0 local8))
							...
							(proc0_13 -1)
							(proc0_10 (- 0 local9))
							(self setScript: moveNotice 0 1 local9)
							(return)
						)
					)
				)
			...
			(26
				(if
					(and
						(not (((ScriptID 1 2) at: 0) finStat:)) ; players
						(<= 1 global373 2)
					)
			...
			(28
				(if
					(and
						...
						(not (((ScriptID 1 2) at: 1) finStat:)) ; players
						(<= 1 global373 2)
					)
			...
			(30
				(if
					(and
						...
						(not (((ScriptID 1 2) at: 2) finStat:)) ; players
						(<= 1 global373 2)
					)
			...
			(32
				(if
					(and
						...
						(not (((ScriptID 1 2) at: 3) finStat:)) ; players
						(<= 1 global373 2)
					)
			...
			(34
				...
				(if
					(and
						...
						(not (global302 netWorthHi:))
						(< (global302 netWorth:) 300)
					...
					(if (> (global302 nakedCount:) 1)
						...
						(proc0_10 local75)
						(self setScript: moveNotice 0 12 local75)
						(return)
					)
				)
				(self cue:)
...
(instance moveNotice of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= local74 (if global480 global480 else global302))
			...
			(3
				(if register
					...
					(cond
						((or (== register 7) (== register 11))
							(= local16
								(Format @local21 111 1 (local74 actualName:)) ; "%s"
							)
							(= local73 1)
						)
						((== register 1)
							(= local16
								(Format ; "%s"
									...
									1
									700
									(((local74 durables:) at: local0) indexNum:)
								)
							)
							(= local73 1)
						)
					...
					(if local73
						(TextSize @[local17 0] local16 4 0)
						(Display
							local16
							dsCOORD
							(- 160 (/ [local17 3] 2))
				...
				(if register2
					(cond
						((== register 11)
							(= local16 (Format @global100 111 2 register2)) ; "$%d an hour"
						)
						((== register 10)
							(= local16 (Format @global100 111 3 register2)) ; "$%d"
						)
						((== register 12)
							(= local16 (Format @global100 111 3 register2)) ; "$%d"
						)
...
(instance ambulance of Act
	...
	(method (handleEvent event)
		(if (and (not (event claimed:)) (not (& (event type:) evMOUSERELEASE)))
			(event claimed: 1)
