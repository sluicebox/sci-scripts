...
(procedure (localproc_0 &tmp temp0 temp1 temp2 temp3 temp4)
	...
	(for ((= temp1 0)) (< temp1 750) ((++ temp1))
		(= [local0 temp1] 0)
	)
	(= temp2 (global302 durables:))
	(= temp3 0)
	(if (temp2 size:)
		(for ((= temp1 0)) (< temp1 (temp2 size:)) ((++ temp1))
			(+=
				temp3
				(* ((temp2 at: temp1) pricePaid:) ((temp2 at: temp1) quantity:))
			)
		)
	)
	...
	(Format (StrEnd @local0) 231 10 temp3) ; "Total Goods: $%-11d"
	...
	(if
		(and
			(= temp4 ((global302 consumables:) objectAtIndex: 36))
			(temp4 quantity:)
		)
		(Format (StrEnd @local0) 231 0 231 14 temp4) ; "%=25s"
	)
	(if
		(and
			(= temp4 ((global302 consumables:) objectAtIndex: 35))
			(temp4 quantity:)
		)
		(Format (StrEnd @local0) 231 0 231 15 temp4) ; "%=25s"
	)
	(if
		(and
			(= temp4 ((global302 consumables:) objectAtIndex: 34))
			(temp4 quantity:)
		)
		(Format (StrEnd @local0) 231 0 231 16 temp4) ; "%=25s"
	)
	(if
		(and
			(= temp4 ((global302 consumables:) objectAtIndex: 1))
			(temp4 quantity:)
		)
		(Format (StrEnd @local0) 231 17 (temp4 quantity:)) ; "%3d Weeks of Food"
	)
	(= temp2 (global302 durables:))
	(if (temp2 size:)
		(for ((= temp1 0)) (< temp1 (temp2 size:)) ((++ temp1))
			(if ((temp2 at: temp1) quantity:)
				(Format ; "%3d %-21s"
					(StrEnd @local0)
					231
					18
					((temp2 at: temp1) quantity:)
					700
					((temp2 at: temp1) indexNum:)
				)
			)
		)
	)
	...
	(if (global302 numDegrees:)
		...
		(if (temp2 size:)
			(for ((= temp1 0)) (< temp1 (temp2 size:)) ((++ temp1))
				(if (global302 hasDegree: ((temp2 at: temp1) indexNum:))
					(Format ; "%-25s"
						(StrEnd @local0)
						231
						4
	...
	(= temp2 (global302 investments:))
	(for ((= temp1 0)) (< temp1 (temp2 size:)) ((++ temp1))
		(if ((temp2 at: temp1) shares:)
			(Format ; "%-25s"
				(StrEnd @local0)
				231
				4
...
(instance inventories of Dialog
	...
	(method (init param1 &tmp temp0 [temp1 2] temp3 temp4)
		...
		(= global443 1)
		(= client param1)
		(global38 color: 0 back: (if global535 142 else 15))
		(= prevTalker global413)
		(= global413 0)
		(self window: global38 add: background invSelector doneButton)
		(localproc_0)
