...
(procedure (StillDown param1 &tmp event ret)
	...
	(= ret (not (& (event type:) evMOUSERELEASE)))
	(event dispose:)
	(return ret)
)
...
(procedure (Print args &tmp temp0 temp1 temp2 temp3 [temp4 6] temp10 temp11 temp12 temp13 temp14 temp15 temp16 temp17 temp18 temp19 temp20 temp21 temp22 [temp23 2] temp25 temp26 temp27 temp28 temp29 temp30 temp31 temp32 [temp33 1000])
	...
	(temp10 text: @temp33 moveTo: 4 4 font: gUserFont setSize:)
	(temp1 add: temp10)
	...
	(if temp11
		(temp11 moveTo: 4 4)
		(temp10 moveTo: (+ 4 (temp11 nsRight:)) (temp10 nsTop:))
		(temp1 add: temp11)
	)
	(temp1 setSize:)
	(if temp12
		(temp12 moveTo: (temp10 nsLeft:) (+ 4 (temp10 nsBottom:)))
...
(class Item of Obj
	...
	(method (track event &tmp temp0 temp1 temp2)
		(if (& $0001 (event type:))
			(self setPort:)
			(= temp1 0)
			(repeat
				...
				(if (!= (= temp0 (self check: event)) temp1)
					(HiliteControl self)
					(= temp1 temp0)
				)
				(event dispose:)
			...
			(if (and (& (event type:) $0004) (== (event message:) key))
				(self hilite: 1)
			)
			(return self)
		)
...
(class Dialog of List
	...
	(method (doit def &tmp temp0 temp1 temp2 temp3)
		(= temp0 0)
		(= busy 1)
		(if (<= argc 1)
	...
	(method (handleEvent event &tmp ret eType eMsg)
		...
		(if
			(or
				...
				(== (event type:) evNULL)
				(and
					(not (& evMOUSEBUTTON (event type:)))
					(not (& evKEYBOARD (event type:)))
					(not (& $0040 (event type:))) ; direction
					(not (& evJOYDOWN (event type:)))
				)
			)
			(if theItem
		...
		(if (= ret (self firstTrue: #handleEvent event))
			...
			(if (or (not (ret checkState: 2)) (ret checkState: 128))
				(if theItem
