...
(instance giant of Act
	...
	(method (handleEvent event &tmp temp0 temp1)
		(cond
			...
			((IsFlag 208) 1)
			((or (MouseClaimed self event 3) (Said 'look/giant,brauggi,man'))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(HandsOn)
				(if (not (IsFlag 48))
					(localproc_0 58 20) ; "Far from the frost field, fares forth this fighter, Hunger has hurled me hither from home. My name it is known in the Northlands as Brauggi, Barter with blade's clash, or bargain with me."
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(localproc_0 58 21) ; "Fruit you have found, to fill all my food stores, Thus filled the bargain, my gem you have bought. Brauggi has bartered, and all has been answered; The mead it may mellow, and now I head home."
				(SolvePuzzle 679 8)
				(= seconds 2)
			)
			(1
...
(instance Challenge of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 58 22) ; "Fiercer than frost's bite, my fury is fearless, My axe it is edged as is the ice. This warrior is willing to wield now the war drum, The battleaxe blazes and bellows for blood."
				(client setScript: giantBlocks)
			)
		)
	)
...
(instance giantFights of Script
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7)
		(switch (= state newState)
			...
			(9
				(localproc_0 58 24) ; "Courage you carry, but skill it is scanty; Face not this foe before fiercer you fight."
				(HandsOn)
				(client setScript: giantBlocks)
			)
		)
			...
			(1
				(if (gEgo has: 11) ; apple
					(cond
						...
						((>= global329 40)
							(LowPrint 58 25) ; "Your fruit definitely makes a generous pile in the Giant's cupped hands, but they're not full yet."
						)
						((== global329 0)
							(HighPrint 58 26) ; "Oops!" you say. "I guess I don't have any fruit for you. Sorry!"
						)
						(else
							(LowPrint 58 27) ; "Your meagre selection of fruit makes a paltry pile at the bottom of the Giant's cupped hands. You'll need to get quite a few more apples to fill THOSE hands!"
						)
					)
				else
					(HighPrint 58 26) ; "Oops!" you say. "I guess I don't have any fruit for you. Sorry!"
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if local1
					(TimePrint 4 58 28) ; "As you start to throw..."
				else
					(TimePrint 4 58 29) ; "As you begin to cast your spell..."
				)
				(= seconds 2)
			)
			(1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(HandsOn)
				(NormalEgo)
				(HighPrint 58 31) ; "Running on the ice is apparently not a very good idea."
				(self dispose:)
			)
		)
	)
