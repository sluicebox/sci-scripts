...
(procedure (proc160_0 param1 &tmp temp0)
	(cond
		...
		((Said 'cast>')
			(cond
				((!= (= temp0 (SaidSpell param1)) 17)
					(param1 claimed: 0)
				)
				((gEgo inRect: 250 162 310 190)
					(if ((ScriptID 95 0) notify: 3) ; rm95
						(HighPrint 160 11) ; "You don't dare; the brigands are coming."
					else
						(HighPrint 160 12) ; "The door is open."
					)
				)
				(
					(or
					...
					(HighPrint 160 13) ; "The door is barred on the other side."
				)
				((not (gEgo inRect: 34 75 73 95))
					(HighPrint 160 14) ; "You aren't near a door."
				)
				((CastSpell 17)
		...
		((Said 'get/boulder')
			(HighPrint 160 15) ; "There are no rocks in here."
		)
		((Said 'open[/gate,door]')
			(cond
				...
				(
					...
					(HighPrint 160 16) ; "The door is locked from the other side."
				)
				(else
					(HighPrint 160 17) ; "You're not near a door."
				)
			)
		)
