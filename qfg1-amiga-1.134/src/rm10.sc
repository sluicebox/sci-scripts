			...
			(evSAID
				(cond
					...
					((Said 'lockpick,get>')
						(cond
							((Said '/blossom')
								(HighPrint 10 30) ; "The lovely blossoms should stay on the tree."
							)
							((Said '/apple')
								(cond
									((not (gEgo inRect: 109 88 223 106))
									...
									(else
										(HighPrint 10 31) ; "The fruit is very soft and juicy.  It would be impossible to keep in your pack."
									)
								)
							)
							((Said '/grass')
								(HighPrint 10 32) ; "The grasses in the meadow are covered over with flowers."
							)
							((Said '/boulder,brick')
								(localproc_5)
							)
							...
							((Said '/flower')
								(if (IsFlag 140)
									(HighPrint 10 33) ; "You take another handful of the lovely, fragrant flowers."
								else
									(HighPrint 10 34) ; "As you pick a variety of the sweet-smelling flowers, they seem to glow in your hands. You put them safely away."
								)
								(SetFlag 140)
