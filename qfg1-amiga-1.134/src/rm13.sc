			...
			(evSAID
				(cond
					...
					(
						(or
							(Said
								'unlock,lockpick/trunk,box,chest,hasp,keyhole'
							)
					...
					((Said 'get,grab/club,weapon')
						(cond
							...
							((IsFlag 331)
								(HighPrint 13 37) ; "The dead Ogre's huge club is much too heavy for you to lift."
							)
							(else
								(HighPrint 13 0) ; "You're kidding, right?"
							)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HandsOn)
				(if (not (IsFlag 331))
					(HighPrint 13 38) ; "Hostile intent is evident. You prepare for battle."
					(gCurRoom newRoom: 455) ; ogreArena
				)
			)
		)
		...
		(switch (= state newState)
			(0
				(if (gEgo inRect: 151 19 222 79)
					(HighPrint 13 39) ; "You need to get closer to the dead Ogre."
					(self dispose:)
				else
					(HandsOff)
					(gEgo
				...
				(switch local0
					(0
						(if
							...
							(HighPrint 13 40) ; "You retrieve your daggers."
						)
						(= [gInvDropped 7] ; dagger
							(= global272 (= global271 (= global273 0)))
						)
						(HighPrint 13 41) ; "A search of the dead Ogre's body produces no treasure."
						(HighPrint 13 42) ; "However, the treasure chest he was carrying lies beside his body."
					)
					(1
						(HighPrint 13 43) ; "The lock on the Ogre's chest clicks open."
						(SetFlag 332)
						(HighPrint 13 44) ; "The dead Ogre's chest contains 1 gold and 43 silver, which you take and put away."
						(SetFlag 333)
						(gEgo get: 2 1) ; gold
						(gEgo get: 1 43) ; silver
					)
					(2
						(HighPrint 13 45) ; "You force the Ogre's chest open."
						(SetFlag 332)
						(HighPrint 13 44) ; "The dead Ogre's chest contains 1 gold and 43 silver, which you take and put away."
						(SetFlag 333)
						(gEgo get: 2 1) ; gold
					...
					(3
						(HighPrint 13 46) ; "The chest is locked."
					)
					(4
						(if (IsFlag 333)
							(HighPrint 13 47) ; "The dead Ogre's chest is empty."
						else
							(HighPrint 13 44) ; "The dead Ogre's chest contains 1 gold and 43 silver, which you take and put away."
							(SetFlag 333)
							(gEgo get: 2 1) ; gold
...
(instance ogreCalmed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 13 48) ; "The Ogre appears to have recovered from your spell."
