...
(instance rm30 of Rm
	...
	(method (init)
		...
		(gAddToPics
			add: frame bigChair smChair hang1 hang2 lunk plane armor mummy shelf
			eachElementDo: #init
			doit:
		)
	...
	(method (handleEvent event &tmp temp0)
		(cond
			...
			((or (Said 'look/dragon,head,trophy') (MouseClaimed dragon event 3))
				(HighPrint 30 8) ; "It was a moderately large and fearful dragon at one time."
			)
			((or (Said 'look/chandelier,ball') (MouseClaimed theLight event 3))
				(HighPrint 30 9) ; "The Ball of Light floats magically over the center of the room."
			)
			((or (Said 'look/peacock,bird') (MouseClaimed peacock event 3))
				(if (> (peacock cel:) 0)
			...
			((or (Said 'look/desk') (MouseClaimed onDesk event 3))
				(HighPrint 30 14) ; "It is a standard roll-top desk. There is nothing on it."
			)
			((or (Said 'look/armor,chainmail') (MouseClaimed armor event 3))
			...
			((or (Said 'look/airplane') (MouseClaimed plane event 3))
				(HighPrint 30 16) ; "You have no idea what it is, but it has the words "Lytton P. D." inscribed upon it."
			)
			((or (Said 'look/mummy,coffin') (MouseClaimed mummy event 3))
			...
			((or (Said 'look/chest') (MouseClaimed onChest event 3))
				(HighPrint 30 24) ; "This item appears to be an ebony chest. It could contain anything."
			)
			((or (Said 'look/table') (MouseClaimed onTable event 3))
			...
			((or (Said 'look/shield') (MouseClaimed onShield event 3))
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(Print 30 41 #at 215 43 #width 75 #dispose #time 4) ; "I wouldn't do that if I were you!"
				(= seconds 3)
			)
			(1
				(= local1 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(cond
					(local4
						(Print 30 42 #at 215 43 #width 90 #dispose #time 3) ; "Those rooms are forbidden!"
					)
					(local5
						(Print 30 43 #at 215 43 #width 90 #dispose #time 3) ; "Since you're leaving anyway..."
					)
					(else
						(Print 30 44 #at 215 43 #width 90 #dispose #time 3) ; "That'll learn ya!"
					)
				)
				(gEgo hide:)
				(Magic setCycle: End)
