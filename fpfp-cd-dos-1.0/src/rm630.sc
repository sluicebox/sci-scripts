		...
		(switch theVerb
			...
			(1 ; Look
				(cond
					...
					(else
						(gMessager say: noun 1 0) ; "Your Dad unloaded his old vaudeville trunk on you. Not on purpose, though; he didn't know you were standing underneath when he unloaded it off the top of the stagecoach. Ever since then, you've had brief blackouts. But what the hey? Free trunk, right?"
					)
				)
			)
			(else
		...
		(switch theVerb
			...
			(1 ; Look
				(cond
					...
					(else
						(gMessager say: noun 1 0) ; "While window-shopping in Chowchilla, your eye fell on this plain but serviceable nightstand. They got your eye back in, but the owner didn't want to touch the nightstand anymore. He let you keep it."
					)
				)
			)
			(else
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 42)
					...
					(gMessager say: noun 1 0) ; "You picked up this old armoire at the farmer's market. It gave you a hernia. So instead of paying the doctor's bills, the owner let you keep it."
				)
			)
			(else
				(super doVerb: theVerb &rest)
		...
		(switch theVerb
			...
			(1 ; Look
				(if (IsFlag 41)
					...
					(gMessager say: noun 1 0) ; "The drawer at the base of your armoire is empty, except for a few stray black widows."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance dresserDrawer of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(cond
					((and (IsFlag 43) (not (IsFlag 112)))
						(gEgo stopUpd:)
						(gCurRoom setInset: inClaimCheck)
					)
					((IsFlag 43)
						(gMessager say: noun 1 1) ; "The drawer is empty, save for a few cobwebs, dust balls, and dead gnats. If you had some socks and underwear in there, they'd need washing pretty badly."
					)
					(else
						(gMessager say: noun 1 0) ; "You stumbled onto this big ol' dresser at a moving sale. And broke your toe. So rather than pay to have your toe fixed, the owner let you keep the dresser. In fact, you get ALL your furniture by accident!"
					)
				)
			)
			(else
...
(instance bed of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (IsFlag 1)
					(gMessager say: noun theVerb 4) ; "You'll never get back to sleep now. There's trouble afoot, and that gnaws at your gut. Or maybe it's that whole pouch of "Gummy Buffalo" you had just before you went to sleep!"
				else
