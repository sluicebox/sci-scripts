...
(instance crucible1 of LabItem
	...
	(method (doVerb theVerb)
		(cond
			...
			(else
				(gMessager say: noun 0 25) ; "The crucible is only for melting pure, or nearly pure, metals."
			)
		)
	)
)
							...
							(if (== amount 6)
								...
								(cond
									...
									((== gCurPuzzle 8)
										(ClearFlag 104)
										(= global143 1)
										(gMessager say: 34 0 33 0 self) ; "You attempt to make something that will cure the horses' flatulence. But until you find a way to "breathe easier," you won't be able to get near enough to those horses to use it! So you toss it away and decide to make a fresh batch after you've got some sort of gas mask."
									)
									(else
										(gEgo get: -1 26) ; Incorrect_Med2
										(= global143 1)
		...
		(cond
			...
			((and (== (global142 objName:) 48) (== (global142 contents:) 25))
				(if (== form 30)
					...
					(gMessager say: noun 82 23) ; "First, light the alcohol lamp, otherwise you'll wait here a long time for that silver to melt!"
				)
			)
			((== (global142 objName:) 48)
			...
			((== (global142 objType:) 3)
				...
				(if (== form 30)
					(cond
						((== (global142 amount:) 0)
							(gGameMusic2 number: 2926 loop: 1 flags: 1 play:)
							(gMessager say: noun 76 12) ; "There's no need to heat up the mixing device if there's nothing inside it. In other words, no preheating necessary!"
						)
		...
		(switch theVerb
			...
			(68 ; Wax_Filled_Mold
				(if (== form 30)
					...
					(gMessager say: noun theVerb 23) ; "Nothing much happens since the lamp isn't lit."
				)
			)
			(else
		...
		(cond
			...
			((== (global142 objType:) 3)
				(cond
					...
					(else
						(gGameMusic2 number: 2926 loop: 1 flags: 1 play:)
						(gMessager say: noun 76 1) ; "You must have at least one solid ingredient in a mixture to make pills. It'll be years before "gelcaps" are invented."
					)
				)
			)
			((== (global142 objType:) 2)
