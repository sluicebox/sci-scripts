...
(instance sTalkToSrini of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 1 2 register 1 5 self)
			)
			(1
				(gMessager say: 1 2 register 6 10 self)
			)
			(2
...
(instance sPourBeer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gMessager say: 2 33 0 0 self) ; "Hey, boys! It's Miller time!!"
			)
			(4
				(localSound stop:)
				(gGame points: 16)
...
(instance srini of Actor
	...
	(method (doVerb theVerb &tmp temp0)
		...
		(switch theVerb
			...
			(2 ; Talk
				(if (IsFlag 2)
					(gMessager say: 1 2 1) ; "Thank you again, Mr. F. I am so much grateful!"
				else
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			(if (gCast contains: srini)
				(gMessager say: noun theVerb 10) ; "Nasty fire ants are swarming around the poor potential Indian Sidekick seated at the top of their anthill. Why, a hungry gaggle of ants like that could strip the flesh off an Indian Sidekick in minutes, leaving nothing but white Sidekick Bones. Better find some way to get him offa there!"
			else
				(gMessager say: noun theVerb 11) ; "A dense swarm of dense fire ants diligently attempts to collect plenty of sand for the coming winter. Just a few more trips and they'll have it all!"
			)
		else
