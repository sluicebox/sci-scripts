...
(instance sTimeToConfront of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 22 0 0 0 self) ; "Freddy dragged his bleedin' self over to the Schoolhouse... the anger and hurt wuz jus' rippin' at his gut like a swarm of botflies on roadkill, only worse 'cuz Freddy, unlike roadkill, wuz still alive. The schoolhouse door wuz unlocked fer once, and Freddy walked right on in."
			)
			(2
		...
		(switch theVerb
			(4 ; Do
				(if (OneOf gCurPuzzle 9 8 7)
					(gMessager say: noun theVerb 3) ; "You're going to play on the seesaw, when the whole town is about to be gassed? I think not."
				else
...
(instance schoolDoor of Door
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gMessager say: noun theVerb) ; "Penelope keeps this door locked all the time, to keep the kids in and the rabble out. Thank goodness there's no such thing as a fire code yet!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance penelope of Actor
	...
	(method (doVerb theVerb &tmp temp0)
		(switch theVerb
			(83 ; Incorrect_Med2
				(gMessager say: noun 71) ; "This looks like somebody else's prescription, Frederick! You'd better get back to work and give it to him right away. It might be important!"
			)
			(13 ; Med_2
				(gMessager say: noun 71) ; "This looks like somebody else's prescription, Frederick! You'd better get back to work and give it to him right away. It might be important!"
			)
		...
		(switch theVerb
			(4 ; Do
				(if (OneOf (slider loop:) 3 4)
					(gMessager say: noun theVerb 0 0 self) ; "The ladder, held in place by a couple of old loose screws, easily comes away from the slide. You somehow cram it into your pocket along with the rest of the junk you're carrying. Way to go, Freddy! Wreck the little kids' playground equipment! (If only they knew how good and true your heart is.)"
					(gEgo get: -1 28) ; Ladder
				else
					(gMessager say: noun theVerb 1) ; "Someone's using the ladder at this very moment."
				)
			)
			(else
				(super doVerb: theVerb &rest)
