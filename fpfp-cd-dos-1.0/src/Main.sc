...
(class PointsSound of FPSound
	...
	(method (check)
		...
		(if (or signal (!= theAudCount (DoAudio 13)))
			(= prevSignal signal)
			(= signal 0)
			(if (IsObject client)
				(client cue: self)
...
(class FP of Game
	...
	(method (points param1 param2 param3)
		(if (and (> argc 1) (!= param2 0) (SetFlag param2))
			(= param1 0)
		)
		(if param1
			...
			(PointsSound play: (and (> argc 2) param3))
			(Wait 2)
...
(instance gasMaskTimer of Timer
	...
	(method (cue)
		(if (IsFlag 70)
			...
			(gMessager say: 22 0 0 0 0 0) ; "You're beginning to feel a little light-headed. Better use your gas mask!"
			(SetFlag 70)
