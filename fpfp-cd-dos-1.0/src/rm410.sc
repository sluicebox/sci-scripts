		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 1)
					...
					(gMessager say: noun theVerb 2) ; "The room is a riot of plush red velvet. The girls are upstairs, lying down. (This time, on their stomachs!)"
				)
			)
			(else
...
(instance postcards of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gCurRoom setScript: sGetPostcards)
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sGetPostcards of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo get: 45 postcards) ; Post_Cards
				(SetFlag 107)
				(postcards dispose:)
				(self dispose:)
			)
