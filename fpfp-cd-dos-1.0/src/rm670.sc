...
(instance sRowdyWarning of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(gMessager say: 45 0 0 0 self) ; "Without warning, you hear gunshots out in front of the Saloon, plus lots of yelling and shouting. What's going on out there?"
			)
			(7
...
(instance sDocReadsRX of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gMessager say: 7 0 6 3 self) ; "Teshtosterate... hmmm... no, I spoze that would be uneffi... <|c2|hic|c|> Ineffi... <|c2|hic|c|> ...a poor choice. Leshee, this was for Sadie Ovaree, right? I mushta meant shomethin' else. Lemme fix it..."
				(= gCurPuzzle 5)
			)
			(6
			...
			(10
				(gMessager say: 7 0 6 4 6 self) ; "Here ya go, take thish back to your Pharmashy and have fun."
				(= gCurPuzzle 5)
			)
			(11
				(gGame points: 7)
			...
			(2 ; Talk
				(cond
					...
					((or (== gCurPuzzle 8) (== gCurPuzzle 9))
						(gMessager say: 11 2 12) ; "Sam, Sam! Don't let anyone go out on the street 'til I give you the say-so. The town's being smothered in horse gas. I don't know how it's happened, but I'm gonna do something it 'afore we all choke to death."
					)
					((== gCurPuzzle 10)
						(gMessager say: 11 2 13) ; "Sam! There's a stampede headed for town! Don't go outside!"
					)
					((== gAct 3)
						(switch global139
							(0
...
(instance doc of Prop
	...
	(method (doVerb theVerb)
		(if (>= gCurPuzzle 20)
			(gMessager say: noun 0 4) ; "Don't bother him. He's sleeping it off. Then, when he wakes up, he'll have a hair of the dog that bit him, and the cycle will begin anew... like the miracle of life itself!"
		else
			(switch theVerb
				(2 ; Talk
					(cond
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(cond
					((< gCurPuzzle 20)
						(gMessager say: noun 4 1) ; "You're not the type to gamble. In other words, you're POOR!"
					)
					((== gCurPuzzle 20)
						(gMessager say: noun 4 2) ; "You don't want to start playing Poker with this guy. He's known far and wide as a cheat. In fact, if you could catch him red-handed, you might be able to stop him before he wins away every last piece of property in town!"
					)
				)
			)
			(else
		...
		(switch theVerb
			(4 ; Do
				(if local6
					...
					(gEgo get: -1 50) ; Shot_Glass
					(gMessager say: noun theVerb 0 0 self) ; "You pick up the whisky glass."
				else
