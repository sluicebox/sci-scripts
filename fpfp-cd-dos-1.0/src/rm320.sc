...
(instance rm320 of FPRoom
	...
	(method (handleEvent event &tmp [temp0 20])
		(if (& (event type:) evMOVE)
			(if (IsFlag 45)
				(gMessager say: 26 0 0) ; "You can't walk that way. It's too treacherous! (Of course, you can walk to lots of other too-treacherous places in the game! You just can't walk here.)"
				(event claimed: 1)
			else
				(return 0)
			)
...
(instance sWaterDripping of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(client show: setCycle: End self cycleSpeed: 1)
				(if (== local10 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(gMessager say: 18 0 1 0 self) ; "Pardon me, this stall is occu-pewed."
			)
			(6
				(client setCycle: Beg self)
			)
...
(instance sPourSolution of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gMessager say: 5 39 5 0 self) ; "You carefully pour the purification solution into the town's water supply. Excellent job!"
			)
			(6
		...
		(switch (= state newState)
			...
			(10
				(if (== gCurPuzzle 13)
					...
					(gMessager say: 43 0 0 0 self) ; "The water, obviously contaminated, has really "opened up your sluices." You spend a few minutes in extreme discomfort."
				)
			)
			(11
				(localSound number: 2146 loop: 1 play:)
		...
		(switch (= state newState)
			...
			(5
				(if (== gCurPuzzle 13)
					...
					(gMessager say: 40 0 17) ; "Hot dog, you roped the top of the water tower! What a shame there's no audience to appreciate your deed."
				)
				(= cycles 1)
			)
			(6
				(gGame points: 5 138 self)
			)
			(7
				(windMillBlades startUpd:)
				(gGame handsOn:)
		...
		(switch (= state newState)
			...
			(3
				...
				(cond
					((and (not (IsFlag 96)) (== gCurPuzzle 13))
						(gMessager say: 39 0 0) ; "As you reach the top of the water tower, the crowd cheers!"
					)
					((not (IsFlag 96))
						(gMessager say: 39 0 17) ; "You reached the top. Too bad nobody was here to applaud your achievement!"
					)
				)
				(= cycles 1)
			)
			(4
				(if (and (not (IsFlag 96)) (== gCurPuzzle 13))
					(Message msgGET 320 42 0 18 1 @local12) ; "Plug-O-Meter\04"
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(gGame handsOn:)
				(if (not (IsFlag 100))
					(gMessager say: 14 0 20) ; "Remembering your father's advice about never using the topmost rungs of a ladder, you stop just short of the ladder top."
					(SetFlag 100)
				)
				(self dispose:)
			)
			...
			(4 ; Do
				(cond
					...
					((== gCurPuzzle 13)
						(switch (Random 0 1)
							(0
								(gMessager say: 4 0 3 1) ; "It may be awhile. It looks like everyone's exercising those lower abdominals!"
							)
							(1
								(gMessager say: 4 0 3 2) ; "Wait your turn. It's just like at the bank. (Other people want to make their deposits first.)"
							)
						)
					)
					((== cel 0)
		...
		(switch theVerb
			(4 ; Do
				(if (IsFlag 21)
					...
					(gMessager say: noun 4 2) ; "You can't reach the water tank hatch from down here."
				)
			)
			(39 ; Pure_Solution
				(if (IsFlag 21)
					(if (== (shingle cel:) 0)
						(gMessager say: noun 39 6) ; "Open the hatch first instead of pouring the purification solution on the roof of the water tank!"
					else
						(gCurRoom setScript: sPourSolution)
					)
				else
					(gMessager say: noun 39 2) ; "You can't pour the purification solution into the water tank from down here."
				)
			)
			(38 ; Lasso
		...
		(switch (= state newState)
			(0
				(if (IsFlag 135)
					...
					(gGame points: 1 135 self)
				)
			)
			(1
				(gEgo put: 18) ; Empty_Bottles
				(gEgo get: -1 57 1) ; Tower_H20
				(gMessager say: 37 34) ; "You carefully fill one of your empty beer bottles with water from the tower."
				(self dispose:)
			)
		)
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(if (not (IsFlag 21))
					(if (IsFlag 45)
						(gMessager say: noun theVerb 9) ; "You can't seem to pick up the ladder right now. It suddenly seems so much heavier, for some strange reason!"
					else
		...
		(switch theVerb
			(4 ; Do
				(if (or (IsFlag 45) (IsFlag 21))
					...
					(gMessager say: noun 4 2) ; "You can't reach the roof of the water tank from here."
				)
			)
			(38 ; Lasso
				(gEgo setScript: sRopeMiss)
		...
		(switch theVerb
			(23 ; Tower_H20
				(if (or (IsFlag 45) (IsFlag 20) (IsFlag 21))
					...
					(gMessager say: 37 34 7) ; "One bottle full of water is enough."
				)
			)
			(4 ; Do
				(cond
