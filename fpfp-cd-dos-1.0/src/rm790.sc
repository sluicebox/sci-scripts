...
(instance rm790 of FPRoom
	...
	(method (init)
		...
		(gNarrator x: 20 y: 40 name: {Director} showTitle: 1)
		(super init:)
	...
	(method (handleEvent event)
		(if (and local7 (event type:) (== gCurRoomNum gNewRoomNum))
			(event claimed: 1)
			(event claimed: 1)
			(gGame setCursor: 999 1 128 175)
			(switch
				(Print
					posn: 100 160
					addButton: 0 4 0 0 1 0 0 790 ; "Continue"
					addButton: 1 4 0 0 2 60 0 790 ; "Quit"
					init:
				)
				(0
					(gGameMusic1 pause: 0)
	...
	(method (changeState newState &tmp [temp0 260])
		(switch (= state newState)
			...
			(1
				...
				(Message msgGET 791 5 0 0 2 @temp0) ; "A total of 34 animals were injured or maimed in the making of this game. After all, we could accept nothing less than total and complete realism."
		...
		(switch (= state newState)
			...
			(16
				(cond
					...
					((== register 4)
						(gMessager say: register 0 0 13 14 self 791) ; "This is ridiculous. I swear, I'll sue if something goes wrong. This is what happened to Margaret Hamilton, y'know."
					)
					(else
						(= cycles 2)
					)
			...
			(19
				(if (== register 4)
					(gMessager say: register 0 0 15 16 self 791) ; "|c0|OW!|c| Worker's comp! Worker's comp!"
				else
					(= cycles 2)
				)
			)
