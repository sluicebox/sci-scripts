...
(instance rm265 of FPRoom
	...
	(method (init)
		(LoadMany rsSOUND 2269 2264 637)
		(LoadMany rsVIEW 261)
...
(instance sEgoJumps of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(gGame points: 12 0 self)
			)
			(10
				(gGameMusic2 number: 260 loop: -1 play:)
				(gMessager say: 23 0 0 0 self) ; "What pluck! What prowess! What a ridiculous solution! (Still, you thought of it!)"
			)
			(11
				(gGame handsOn:)
				(self dispose:)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(cond
					((IsFlag 82)
						(gMessager say: noun 4 19) ; "You're not quite skillful enough to make a perfect three-point landing from the swing to the seesaw without breaking every bone in your body. (In other words, try it and you'll be the man with the flopping trapezium!)"
					)
					(local0
						(cond
							((IsFlag 7)
		...
		(switch theVerb
			(41 ; Baking_Soda
				(if (and (not local0) (not (IsFlag 82)))
					...
					(gMessager say: noun theVerb 19) ; "You're too uncoordinated to heave the baking soda AND hold onto the swing at the same time. Maybe you should practice on smaller accomplishments first... like walking and chewing tobacco at the same time!"
				)
			)
			(else
				(super doVerb: theVerb &rest)
