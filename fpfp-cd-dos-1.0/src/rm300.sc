		...
		(switch (= state newState)
			...
			(1
				...
				(if (IsFlag 131)
					...
					(gGame points: 2 131 self)
				)
			)
			(2
				(gMessager say: 11 63 0 0 self) ; "You start to dig up the freshly-laid grave. Muscles that haven't been used in years begin to groan and whine. But with the gritty determination of a professional grave-robber, you toil on... and on... and on..."
			)
			(3
				(Narrator modeless: 0)
				(proc0_15 1 4 self)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gMessager say: 14 63 0 0 self) ; "You begin the arduous task of replacing the heaping mound of dirt, one shovelful at a time... over... and over... and over..."
			)
			(2
				(Narrator modeless: 0)
				(proc0_15 1 4 self)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(gMessager say: 14 4 5 0 self) ; "You carefully search through the many pockets of Graves's three-dollar suit, until you discover... the safe deposit box key you entrusted to Phil oh so many years ago!"
			)
			(4
		...
		(switch (= state newState)
			(0
				(if (IsFlag 129)
					...
					(gGame points: 1 129 self)
				)
			)
			(1
				(gEgo get: 51 shovel 2 shovel) ; Shovel
				(self dispose:)
			)
		)
	)
...
(instance stone1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gMessager say: noun theVerb) ; "Here Lies Eric DeUnreddie      1830-1875 He Had a Fatal Error Reading Drive B."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance stone2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gMessager say: noun theVerb) ; "Here lies Les Moanly. He died with his shrinkwrap still on.      1861-1883"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sStone3 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 24 1 0 1 self) ; "Pardon our Dust, but Here We Grow Again! A New Grave is Being Erected on this Site in Honor of Philip D. Graves."
			)
			(1
				(gMessager say: 24 1 0 2 3) ; "Looks like your dear friend Philip has passed on. Gee, it seems like only a day or two ago you filled a prescription for him."
				(self dispose:)
			)
		)
	)
...
(instance sStone4 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 25 1 0 1 self) ; "Here Lieth James Ernest Haupher.     1824-1880"
			)
			(1
				(gMessager say: 25 1 0 2) ; "Ah-ha! You'd always wondered where Jim E. Haupher was buried!"
				(self dispose:)
			)
		)
	)
...
(instance stone5 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gMessager say: noun theVerb) ; "Here Lies Butch Casualty. He Also Cheats, Steals, Murders,   And Writes Bad Checks.     1849-1882"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance stone6 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(gMessager say: noun theVerb) ; "Under this rock lies Ernie Fleabeak.   Mere text cannot describe    how he'll be missed.      1858-1886"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance stone7 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gMessager say: noun theVerb) ; "You'd give yourself  A hernia, And hear from Ed's  attorneya.  Burma-Shave"
			)
			(1 ; Look
				(gCurRoom setScript: sStone7)
			)
			(2 ; Talk
				(gCurRoom setScript: sStone7Talk)
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sStone7 of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 28 1 0 1 self) ; "Oh, boy! It's one of those rare "Burma Shave" headstones:"
			)
			(1
				(gMessager say: 28 1 0 2) ; "Beneath This Plot Lies Edward Fiskers. His Time Grew Short, But Not His Whiskers.   Burma Shave  1845-1885"
				(self dispose:)
			)
		)
	)
...
(instance sStone7Talk of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 28 2 0 1 2 self) ; "Hey! I like your gravestone!"
			)
			(1
				(gMessager say: 28 2 0 3) ; "I'm in a box That's six feet deep,  So shut your mouth  And let me sleep!"
				(self dispose:)
			)
		)
	)
...
(instance stone8 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gMessager say: noun theVerb) ; "Poor Prudence Brown Would moan and groan  If you attempt  To steal her stone."
			)
			(1 ; Look
				(gCurRoom setScript: sStone8)
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance sStone8 of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 29 1 0 1 self) ; "The epitaph reads:"
			)
			(1
				(gMessager say: 29 1 0 2) ; "Poor Prudence Brown  Was Merely Dozing.   Never Woke,  Now Decomposing.    1839-1880"
				(self dispose:)
			)
		)
	)
