...
(instance rm310 of FPRoom
	...
	(method (init)
		...
		(churchDoor init: approachVerbs: 1 4) ; Look, Do
...
(instance candles of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (IsFlag 66)
					(gMessager say: noun 4 5) ; "You've got as much wax as you'll need."
				else
					(gGame points: 1 0 self)
				)
			)
	...
	(method (cue)
		(gMessager say: 20 4) ; "You would never dream of stealing a candle from a church. But... they wouldn't miss one of these puddles of candle wax."
...
(instance rightWindow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(leftWindow doVerb: theVerb &rest)
			)
			(else
...
(instance rightDoor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (gEgo has: 6) ; Church_Key
					(gMessager say: noun 4 3) ; "What? You want to steal something else from the church? Haven't you committed enough sacrilege for one game?"
				else
