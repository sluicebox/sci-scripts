...
(instance sMiss of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gMessager say: 7 47 0 0 self) ; "You take careful aim with your pistol and slowly squeeze off a shot."
			)
			(4
				(gEgo normalize:)
				(gGame handsOn:)
...
(instance sShotTheCannister of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gMessager say: 5 47 0 1 self) ; "You take careful aim with your pistol and slowly squeeze off a shot."
			)
			(2
				(gunSound play: self)
			)
...
(instance bigCannister of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(47 ; Pistols
				(gMessager say: 5 47 11) ; "Oh, no you don't! You can't shoot the canister the EASY way, from up close! Try it from far away, just like you'd HAVE to in real life!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(47 ; Pistols
				(cond
					((IsFlag 83)
						(gMessager say: 7 theVerb 4) ; "There's no longer any need to shoot the bunting."
					)
					((proc0_1 gEgo 2)
		...
		(switch theVerb
			(1 ; Look
				(cond
					...
					((< (gEgo x:) 145)
						(gMessager say: noun theVerb 2) ; "You can't see the hotel through the brothel. You left your X-Ray glasses at the Pharmacy!"
					)
					(else
			...
			(47 ; Pistols
				(cond
					((not (gCast contains: cannister))
						(gMessager say: noun theVerb 5) ; "Potshots at the hotel? From here? Save your ammo!"
					)
					((IsFlag 83)
						(gMessager say: noun theVerb 4) ; "There's no longer any need to shoot the bunting."
					)
					((proc0_1 gEgo 2)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 33)
					...
					(gMessager say: noun theVerb 5) ; "The bunting is clearly advertising Open Mike Night for comedians at Mom's Cafe. Now that the Hotel is out of business, Mom gets free advertising space!"
				)
			)
			(8 ; icon10
				(self dispose:)
