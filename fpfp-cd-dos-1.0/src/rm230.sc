...
(instance sSepiaStart of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gMessager say: 28 0 0 0 self) ; "Freddy Pharkas? You wuz askin' about Freddy Pharkas? The man whut saved Coarsegold, only nobody knowed it wuz him? Why, jes' hop up here on ol' Whittlin' Willy's lap and I'll tell ya all about it."
			)
			(3
...
(instance sCloseHotel of Script
	...
	(method (doit)
		(super doit:)
		(cond
			((and (not local3) (!= (client cel:) 0))
				(= local3 1)
			)
			((and local3 (== (client loop:) 0) (== (client cel:) 0))
...
(instance manBoarding of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(2 ; Talk
				(gMessager say: noun theVerb 3) ; "Billy, the town handyman and jack-of-all-trades, is boarding up yet another Coarsegold business, the "Dirty Sheets Hotel," the finest hostelry in all of Eastern Madera County. (Okay, so maybe it was only the best after that other hotel shut down, but in its day, The Dirty Sheet was really something!)"
			)
			(else
				(super doVerb: theVerb)
			)
		...
		(switch theVerb
			(4 ; Do
				(if (< (gEgo y:) 75)
					...
					(gMessager say: noun 4 8) ; "Just fly there. (NOT!!)"
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance momsDoor of Door
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(if (> gCurPuzzle 17)
					(gMessager say: 6 4 6) ; "It's locked! Mom's is closed! Sheriff Shift strikes again! (And stomachs all over town are heaving... sighs of relief!)"
				else
					(gCurRoom north: 660)
					(super doVerb: theVerb)
				)
		...
		(switch theVerb
			(1 ; Look
				(if (> gCurPuzzle 17)
					...
					(gMessager say: 3 1) ; "Peering through Mom's greasy glass, you can see she's open for business. You've always been fond of Mom's cooking... at least, the first time it goes down!"
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance hotelDoor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (< (gEgo y:) 75)
					(gMessager say: noun theVerb 10) ; "That would be quite a trick... opening the Hotel door from the balcony. You're not quite up to it. (Or down to it, as the case may be!)"
				else
					(super doVerb: theVerb)
				)
			)
		...
		(switch theVerb
			(1 ; Look
				(if (IsFlag 1)
					...
					(gMessager say: 26 1) ; "Apparently Mom's jumping on the comedy club bandwagon. The sign reads, "Coming soon -- Open Megaphone Night. Take your best shot! You'll die laughing!""
				)
			)
			(40 ; Nitrous_Oxide
				(if (< (gEgo y:) 75)
					(if (== gCurPuzzle 22)
						...
						(gMessager say: 26 40 9) ; "You've got no reason to put the canister on the railing. (At least, not YET.)"
					)
				else
