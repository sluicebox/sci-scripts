...
(instance rm250 of FPRoom
	...
	(method (init)
		...
		(trough init: setOnMeCheck: 26505 approachVerbs: 4 29) ; Do, Deflatulizer
		(if (and (>= gCurPuzzle 8) (<= gCurPuzzle 9))
			(horse1
				init:
				cycleSpeed: 10
		...
		(if (and (== gCurPuzzle 19) (not (IsFlag 110)))
			(SetFlag 110)
			(thinMan init:)
		)
	)
	...
	(method (cue)
		(gMessager say: 1 9 1) ; "You unlock the door. Ta-daa! You're halfway through the game."
	)
)
...
(instance sReveireScreams of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gMessager say: 26 0 0 0 self) ; "Board up yer winders 'n' doors! Lock up the wimmen and children! Run fer the border, Louise! Thar's a |c2|stampede|c| a-comin'! A |c2|stam-peeeeede!|c| Hurry up, Freddy! Ya only got a week and a half a'fore they git here."
			)
			(5
...
(instance sEnterThinMan of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(thinMan setSpeed: 10 setCycle: End self)
			)
			(2
				(thinMan
					setLoop: 1
