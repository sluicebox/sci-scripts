		...
		(switch theVerb
			...
			(58 ; Boots
				(if (IsFlag 58)
					...
					(gMessager say: noun theVerb 2 0 0 19) ; "Of course you'll want to wear the boots later, but wait until you have a complete outfit."
				)
			)
			(49 ; Coffee
				(gEgo put: 11) ; Coffee
			...
			(71 ; Incorrect_Med
				(gMessager say: noun theVerb 0 0 self 19) ; "You take a sip of the medication. Unfortunately, the first rule of the Old West Medicine Chest is: never trust nobody else's prescription!"
			)
			(11 ; Med_1
				(gMessager say: noun theVerb 0 0 self 19) ; "You take a sip of the medication. Unfortunately, the first rule of the Old West Medicine Chest is: never trust nobody else's prescription!"
			)
			...
			(59 ; Neckerchief
				(if (IsFlag 58)
					...
					(gMessager say: noun theVerb 2 0 0 19) ; "You'll want to wear your neckerchief later, after you have completed your outfit."
				)
			)
			(40 ; Nitrous_Oxide
				(giggleSound play:)
				(gMessager say: noun theVerb 0 0 self 19) ; "You open the valve on the tank of nitrous oxide, and suddenly the game seems much funnier! Unfortunately, you soon die laughing!"
			)
			(47 ; Pistols
				(if (IsFlag 54)
					...
					(gMessager say: noun theVerb 5 0 0 19) ; "What are you doing? Trying to rob yourself? Who are you trying to kid? Those pistols aren't loaded!"
				)
			)
			(39 ; Pure_Solution
				(gMessager say: noun theVerb 0 0 self 19) ; "You take a swig of the concentrated water purification solution. That'll clean your clock!"
			)
			(60 ; Sharp_Ear
				(if (IsFlag 81)
					(gCurRoom notify: 1)
			...
			(30 ; Snails
				(gMessager say: noun theVerb 0 0 self 19) ; "What? Uncooked? And without garlic butter? AUGGGH!"
			)
			(61 ; Sword
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(gMessager say: 1 global133 0 0 self 19)
			)
			(5
				(gEgo setSpeed: local122)
				(gGame handsOn:)
...
(instance sWaterSample of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gGame points: 1 134)
				(gMessager say: 1 23 0 0 self 19) ; "You take a swig of the water from the water tower. Uh-oh. Your stomach starts to make strange sounds. Your small intestine begins to whine. Your lower intestine starts complaining loudly."
			)
			(2
				(if (!= gCurRoomNum 320)
					(SetFlag 74)
