...
(instance rm660 of FPRoom
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "If you get your fingerprints on anything in this spotlessly clean, cozy little cafe, Mom will check out the fingerprints, trace them to you, and make you one sorry hombre."
				else
					(gMessager say: noun theVerb 4) ; "Get your greasy fingerprints on anything and I'll staple your fingers over them."
				)
			)
			(1 ; Look
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "It's Mom's Cafe, owned by Helen Back, "The Surliest Mom In The West.""
				else
					(gMessager say: noun theVerb 4) ; "It's Mom's Cafe, owned by Helen Back, "The Surliest Mom In The West.""
				)
			)
			(66 ; Horse_Plop
...
(instance sGetFlies of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gMessager say: 35 0 0 (= local3 (Random 1 5)) self)
			)
			(3
				(flyTimer seconds: 1)
				(if (== local3 5)
					(gMessager say: 35 0 0 6 self) ; "Dummy Msg."
				else
					(= cycles 1)
				)
			)
...
(instance sGetCan of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 29 4 4 0 self) ; "Hey, Mom! Would you mind if I took this empty can?"
			)
			(1
				(tinCan dispose:)
...
(instance sDunkPlop of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6 0)
			(7
				(gMessager say: 36 0 0 0 self) ; "|c2|LOOK WHAT YOU'VE DONE!!|c| Freddy Pharkas, I'll see you run out of town for this!! Hop Singh, hop to it! Get out here and clean up this mess! Damn these flies!"
			)
			(8
				(hopSingh
					view: 664
			...
			(9
				(gMessager say: 37 0 0 0 self) ; "Oh, oh, oh! You in big water now!"
			)
			(10
				(gGameMusic1 number: 501 loop: -1 play:)
				(hopSingh
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Talk
				(cond
					((gEgo has: 54) ; Snails
						(gMessager say: noun 2 2) ; "Helen, have you ever thought of putting escargot on the menu?"
						(SetFlag 32)
					)
					...
					((and (== gCurPuzzle 16) (IsFlag 103))
						(gMessager say: noun theVerb 16) ; "Mom, I apologize for leaving that little souvenir with you."
					)
					((== gCurPuzzle 16)
						(gMessager say: noun theVerb 15) ; "Mom, there was a fire last night that almost burned the whole town down."
					)
					((== gCurPuzzle 17)
			...
			(30 ; Snails
				(if (IsFlag 32)
					(gCurRoom setScript: sGiveSnails)
				else
					(gMessager say: noun 2 2 0 self) ; "Helen, have you ever thought of putting escargot on the menu?"
					(SetFlag 32)
				)
			)
			(else
...
(instance sGiveSnails of Script
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo put: 54) ; Snails
				(gMessager say: 11 0 3 1 self) ; "Hmmph. Dunno who around here will eat these, since they don't give you gas, but I'll put them in a bag of cornstarch and think about it. And I suppose a thank-you is in order, so thank you."
			)
			(1
				(gGame points: 1)
				(self dispose:)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(cond
					((gEgo has: 11) ; Coffee
						(gMessager say: noun 4 1) ; "Mr. Pharkas, I have no objection to your coming in here and taking a cup of coffee. But please, limit yourself to one cup at a time. Or I will have no choice but to break your skinny bones like matchsticks. Are we clear on this now? Good."
					)
					((IsFlag 157)
						(gEgo get: -1 11 2) ; Coffee
						(gMessager say: noun 4 17) ; "You take another cup of coffee."
					)
					(else
						(gCurRoom setScript: sGetCoffee)
					)
				)
			)
			(else
...
(instance sGetCoffee of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame points: 1 157 self)
			)
			(1
				(gEgo get: 11 coffee 2) ; Coffee
				(self dispose:)
			)
		)
	)
			...
			(2 ; Talk
				(cond
					((and (== gAct 1) (not (IsFlag 90)))
						(gMessager say: noun theVerb 6) ; "Howdy, Hop! How's the restaurant business?"
						(SetFlag 90)
					)
					((and (== gAct 1) (not (IsFlag 91)))
						(gMessager say: noun theVerb 7) ; "Can I ask you a question, Hop Singh?"
						(SetFlag 91)
					)
					((== gAct 1)
						(gMessager say: noun theVerb 8) ; "Hey, Hop!"
					)
					((and (== gCurPuzzle 16) (IsFlag 103))
						(gMessager say: noun theVerb 16) ; "Hop Singh, you're not mad at me, are you?"
					)
					((== gCurPuzzle 16)
						(gMessager say: noun theVerb 15) ; "Say, Hop! Hear about the big fire last night?"
					)
					((== gCurPuzzle 17)
						(gMessager say: noun theVerb 13) ; "Hop Singh, I'll be leaving town for a while."
					)
					((== gCurPuzzle 13)
						(gMessager say: noun theVerb 12) ; "Hey, Hop Singh! Don't serve any water 'til I've had a chance to check it out. I think something's up with the water supply."
					)
					((== gCurPuzzle 8)
						(gMessager say: noun theVerb 9) ; "Hop Singh, I just want to warn you NOT to go outside and NOT to let your customers go outside 'til we've cleared the air."
					)
					((== gCurPuzzle 9)
						(gMessager say: noun theVerb 9) ; "Hop Singh, I just want to warn you NOT to go outside and NOT to let your customers go outside 'til we've cleared the air."
					)
					((== gCurPuzzle 12)
						(gMessager say: noun theVerb 11) ; "Hey, Hop Singh, you know anything about getting people off of anthills?"
					)
					((== gCurPuzzle 10)
						(gMessager say: noun theVerb 10) ; "Have you heard, Hop Singh? Thar's a stampede a-comin'!"
					)
					(else
						(gMessager say: noun 2) ; MISSING MESSAGE
					)
...
(instance orderWheel of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Better not let Mom catch you fooling around with her order wheel!"
				else
					(gMessager say: noun theVerb 4) ; "Do you have an order to put on the order wheel?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance swingDoors of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: 21 theVerb 5) ; "Hopalong Singh (Hop, for short) doesn't take kindly to people invading his inner sanctum."
				else
					(gMessager say: 21 theVerb 4) ; "Nobody goes into my kitchen without my permission. |c0|Nobody,|c| do you hear?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance tinCan of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Better not disturb any of Mom's shelves unless you ask her permission first."
				else
...
(instance rightTable of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "You consider doing your Famous Tablecloth Trick, but there's nobody around to be impressed!"
				else
					(gMessager say: noun theVerb 4) ; "Anybody wanna see my Spectacular Tablecloth Trick?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance shelf of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Better wait 'til Mom is here before you try to take anything from her shelves."
				else
					(gMessager say: noun theVerb 4) ; "I'm so glad you like the decor in here, Mr. Pharkas. Of course, if you were to rearrange it, or move it, or touch it in any way, I would have to kill you. You do understand, don't you?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance kitchenDoor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Hopalong Singh (Hop, for short) doesn't take kindly to people invading his inner sanctum."
				else
					(gMessager say: noun theVerb 4) ; "Nobody goes into my kitchen without my permission. |c0|Nobody,|c| do you hear?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
