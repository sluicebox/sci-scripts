...
(instance doSleep of Code
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4 [temp5 60])
		...
		(if (OneOf gCurRoomNum 150 160 170 180)
			...
			(temp4 nsTop: 25 nsLeft: 25 cursor: 1)
			(local1 add: temp4)
			(= temp4 (sleepIcon new: 1 0 0 2))
			(temp4 nsTop: 25 nsLeft: 105 cursor: 2)
			(local1 add: temp4)
			...
			(temp4 nsLeft: 40 nsTop: 28 cursor: 1)
			(local1 add: temp4)
			(= temp4 (sleepIcon new: 2 0 0 2))
			(temp4 nsLeft: 40 nsTop: 45 cursor: 2)
			(local1 add: temp4)
			(= temp4 (sleepIcon new: 2 0 0 3))
			(temp4 nsLeft: 40 nsTop: 62 cursor: 3)
			(local1 add: temp4)
			(if gNight
				(= temp4 (sleepIcon new: 2 0 0 4))
				(temp4 nsLeft: 40 nsTop: 79 cursor: 4)
				(local1 add: temp4)
			else
				((local1 window:) bottom: 138)
			)
			(= temp4 (sleepIcon new: 2 0 0 5))
			(temp4 nsLeft: 40 nsTop: (if gNight 96 else 79) cursor: 5)
			(local1 add: temp4)
			(local1 init: show: dispose:)
			(= local1 0)
			(switch local2
...
(instance showTime of Code
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4 [temp5 30] [temp35 7])
		(= temp0 gDay)
		(Message msgGET 7 0 0 10 1 @temp5) ; "It's day"
		(Format @temp35 {%d} (+ gDay 1))
		...
		(Display @temp5 dsCOLOR 17 dsCOORD 0 66 dsWIDTH 156 dsFONT 999)
		(Display @temp35 dsCOLOR 17 dsCOORD 40 66 dsWIDTH 156 dsFONT 999)
		(if (not gNight)
			(DrawCel 938 1 0 0 3)
