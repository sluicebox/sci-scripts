...
(procedure (localproc_8 &tmp temp0)
	...
	(if (and (== local1 0) [gEgoStats 23] [gEgoStats 29]) ; zapSpell, reversalSpell
		(= local1 1)
	)
...
(instance importHero of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(6
				(if (and (IsFlag 144) [gEgoStats 23]) ; zapSpell
					(if (localproc_10)
						(= gHeroType 3) ; Paladin
						(= gBaseHeroType 0) ; Fighter
					else
						(= gHeroType 1) ; Magic User
						(= gBaseHeroType 1) ; Magic User
					)
				)
