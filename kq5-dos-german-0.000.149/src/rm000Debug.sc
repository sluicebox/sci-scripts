...
(instance rm000Debug of Code
	...
	(method (doit param1 &tmp temp0 temp1 temp2 temp3 temp4 [temp5 255])
		(switch (param1 message:)
			...
			(17152
				(= temp1 (Sound pause: 1))
				(if (Print 889 4 #font 0 #button {  Restart  } 1 #button { Continue } 0) ; "Do you wish to start over?"
					(gGame restart:)
				)
				(Sound pause: temp1)
			)
			...
			(11264
				(= gQuit
					(Print 889 5 #font 0 #button {    Quit    } 1 #button { Continue } 0) ; "Do you really want to quit?"
				)
			)
			(else
				(param1 claimed: 0)
