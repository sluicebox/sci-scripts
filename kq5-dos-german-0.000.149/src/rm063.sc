	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(proc0_28 73 63 9 67 30 10 25 6) ; "Good-bye, KING Graham of Daventry...heh, heh, heh!"
				(switch global351
					(135
						(= temp0 8)
					)
...
(instance windows of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 10) ; "Graham looks out the window and views the distorted rocks and twisted forms of Mordack's strange island."
					(event claimed: 1)
				)
			)
		)
...
(instance robin of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 11) ; "Special thanks to Dan, Bill, Keri and the rest of the QA staff."
					(event claimed: 1)
				)
			)
		)
