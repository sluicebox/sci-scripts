		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(Said
							'play,capture,kill,hit,get,attack,fight/antwerp,baby'
						)
						(if (> (gCast size:) 2)
...
(instance stag of TargActor
	...
	(method (handleEvent event)
		(if (or (MouseClaimed self event 3) (Said 'look/buck,buck'))
			(event claimed: 1)
			(switch local6
				(0
...
(instance antwerped of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 78 17 80 {Trounced by a bounce!} 82 800 0 0) ; "You're obviously in no shape to continue the game."
			)
		)
	)
)
