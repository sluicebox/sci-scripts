...
(procedure (localproc_1 param1)
	(Print &rest #at -1 150 #width 0 #mode 1 #dispose #time param1)
)
...
(instance rm15 of Rm
	...
	(method (handleEvent event &tmp temp0 temp1 temp2)
		...
		(cond
			...
			((!= temp0 evSAID))
			((and (not (IsFlag 150)) (Said 'talk,ask,awaken'))
			...
			((Said 'search')
				(if (IsFlag 153)
					...
					(gEgo setScript: (ScriptID 112 0)) ; searchCave
				)
			)
			...
			((and (IsFlag 216) (Said 'get,grab,get,move/chest,box,trunk'))
			...
			(
				...
				(cond
					...
					(else
						(gEgo setScript: (ScriptID 116 0) 0 0) ; pickChest
					)
				)
			)
			((Said 'force,break,pry/chest,box,trunk,hasp,lid')
				(cond
					...
					(else
						(gEgo setScript: (ScriptID 116 0) 0 1) ; pickChest
					)
				)
			)
			(
			...
			((Said 'run,escape')
				...
				(if global290
					(self setScript: (ScriptID 117 1)) ; stopFight
				)
			)
			((Said 'fight,attack,kill,hit[/kobold,creature,man,wizard]')
				(cond
					(global290
					...
					(else
						(self setScript: (ScriptID 117 0)) ; startFight
					)
				)
			)
			((Said 'throw>')
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(localproc_2 3 15 56) ; "You go! You no stay here!"
				(= seconds 3)
			)
			(2
				(client setScript: kobAwake)
...
(instance kobAsleep of KScript
	...
	(method (doit)
		(if
			...
			(localproc_1 5 15 57) ; "Your foot slips on the wet cavern floor and makes a scuffing noise."
			(EgoGait 0 0) ; walking
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (not (IsFlag 174))
					(SetFlag 174)
					(localproc_1 5 15 58) ; "The Kobold casts a spell with which you are not familiar."
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if global290
					(gCurRoom setScript: (ScriptID 117 1)) ; stopFight
				)
			)
			(1
				...
				(if global289
					(NormalEgo)
					(gEgo illegalBits: 0)
					((ScriptID 117 0) changeState: 1) ; startFight
				)
				(if global348
					(client posn: 52 84)
				else
		...
		(switch (= state newState)
			...
			(1
				(cond
					...
					(else
						(EgoDead 15 61 80 {You've been deep-fried} 82 532 0 1) ; "You are overcome by the power of the Kobold Wizard's magic. You need to either practice your skills more or plan a better attack strategy."
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (not (IsFlag 5))
					(localproc_1 5 15 62) ; "This section of cave has an eerie quality."
				)
