	...
	(method (handleEvent event)
		(if
			(or
				...
				(Said 'look/zara,woman,girl,keeper,shopkeeper')
			)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onParphernalia event 3)
				(HighPrint 314 79) ; "Paraphernalia and apparatus."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onAccoutrements event 3)
				(HighPrint 314 80) ; "Accoutrements and equipment."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onBat event 3)
				(HighPrint 314 81) ; "Darned if I know!"
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(switch (Random 1 5)
					(1
						(HighPrint 314 82) ; "Odds and ends."
					)
					(2
						(HighPrint 314 83) ; "Ends and odds."
					)
					(3
						(HighPrint 314 84) ; "Things and stuff."
					)
					(4
						(HighPrint 314 85) ; "Jeff's jimcracks."
					)
					(5
						(HighPrint 314 86) ; "Magical junk and mystical garbage."
					)
				)
			)
		)
