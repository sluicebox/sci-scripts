...
(instance rm310 of Rm
	...
	(method (doit &tmp temp0)
		...
		(cond
			...
			((and (not (gCurRoom script:)) (== temp0 2048))
				(gCurRoom setScript: climbUpStairs)
			)
			((and (not (gCurRoom script:)) (== temp0 4096))
				(gCurRoom setScript: climbDownStairs)
			)
		)
	)
		...
		(switch theVerb
			...
			(4 ; Do
				(if gNight
					...
					(gMessager say: 1 4 1) ; "You enter the Adventurers' Guild Hall."
...
(instance climbUpStairs of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: PolyPath 201 137 self)
			)
			(1
				(gEgo setMotion: PolyPath 191 117 self)
			)
			(2
				(HandsOn)
				(self dispose:)
...
(instance climbDownStairs of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo setMotion: MoveTo 206 146 self)
			)
			(2
				(HandsOn)
				(self dispose:)
