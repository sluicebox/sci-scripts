	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				((ScriptID 39 2) ; theMaster
					setLoop: (if (== ((ScriptID 39 2) loop:) 5) 1 else 0) ; theMaster
					cycleSpeed: 0
					moveSpeed: 0
					setCycle: Walk
					setMotion:
...
(instance tooTired of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(8
				(if ((ScriptID 218 0) endFight:) ; fighter
					(((ScriptID 218 0) opponent:) setScript: (ScriptID 224 0)) ; fighter, leaveRoom
				)
				(ClearFlag 304)
				(= ticks 14)
			)
