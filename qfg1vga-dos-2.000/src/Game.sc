...
(class Game of Obj
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Save doit: @comment)) -1)
				...
				(if (not (SaveGame name num @comment gVersion))
					(Message msgGET 994 1 0 0 1 @buf2) ; "Your save game disk is full. You must either use another disk or save over an existing saved game."
					(Message msgGET 994 2 0 0 1 @str) ; "OK"
					(Print font: 0 addText: @buf2 addButton: 1 @str init:)
				)
	...
	(method (restore &tmp [comment 20] num oldCur buf1 [buf2 100] [str 5])
		(= buf1 parseLang)
		(= parseLang 1)
		(Load rsFONT gSmallFont)
		(ScriptID 990)
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Restore doit: &rest)) -1)
				...
				(if (CheckSaveGame name num gVersion)
					...
					(Message msgGET 994 3 0 0 1 @buf2) ; "That game was saved under a different interpreter. It cannot be restored."
					(Message msgGET 994 2 0 0 1 @str) ; "OK"
					(Print font: 0 addText: @buf2 addButton: 1 @str init:)
					(self setCursor: oldCur (HaveMouse))
					(= parseLang buf1)
