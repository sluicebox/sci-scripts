...
(instance rm10 of Rm
	...
	(method (doit)
		...
		(if (not (-- local21))
			(= local21 10)
			(if (and (> global195 16) (not (IsFlag 360)))
	...
	(method (doVerb theVerb &tmp [temp0 20])
		(switch theVerb
			(50 ; detectMagicSpell
				(gMessager say: 3 50) ; "This entire area is magical. In particular, the fruit on the tree radiates magic, and you sense that a spell has been placed on the large stone. All of the magic has a warm, nurturing feel to it."
			)
			(52 ; Sleep
				(if (not (CanSleep))
			...
			(51 ; openSpell
				(if local19
					(gMessager say: 6 51) ; "There is no further purpose in casting the Open spell."
				else
			...
			(80 ; calmSpell
				(gMessager say: 3 80 9) ; "The very air in this meadow radiates feelings of calmness and peace. You wouldn't be able to tell if the spell worked here."
			)
			(else
...
(instance goToSleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(gMessager say: 6 0 0 1 self) ; "You sleep comfortably among the fragrant flowers."
			)
			(8
				(= seconds 2)
			)
