...
(instance rm331 of Rm
	...
	(method (doit)
		...
		(cond
			((and (gEgo inRect: 190 140 310 166) (not local10))
				(= local10 1)
				(gEgo setScript: cardScript)
			)
			((and (not (gEgo inRect: 190 140 310 166)) local10)
				(= local10 0)
				(gEgo setScript: 0)
			)
		)
...
(instance talkToHeroScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gTheIconBar disable: 1 4 3 7 2 8 5)
				(= seconds 1)
			)
			(1
				(if gModelessDialog
...
(instance describeTavern of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gMessager say: 20 1 86 0 self) ; "The grimy window lets little light into this tavern. It smells like stale ale and other more unpleasant things. The floor is covered with dirt, and the bar with sticky beer."
			)
			(1
				(if (not local2)
