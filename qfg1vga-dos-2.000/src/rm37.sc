	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(= local89 0)
				(if local96
					(gMessager say: 7 0 36 1 self) ; "Once more the guard raises the portcullis."
				else
					(gMessager say: 7 0 38 1 self) ; "The guard, sensing something is awry, raises the portcullis."
				)
			)
			(4
...
(instance egoLeavesCastle of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo init: posn: 161 159 setMotion: MoveTo 161 164 self)
