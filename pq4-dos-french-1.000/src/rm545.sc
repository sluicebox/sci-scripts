...
(instance rm545 of Room
	...
	(method (init)
		(if (== gPrevRoomNum 500)
			(gGlobalSound0 number: 983 setLoop: -1 setVol: 62 play:)
		)
		(gCurRoom
			addObstacle:
				((Polygon new:)
		...
		(switch (= state newState)
			...
			(1
				(gEgo
					heading: 270
					setPri: -1
					setScaler: Scaler 93 65 133 105
					normalize: 9120
				)
				(gGame handsOn:)
				(self dispose:)
...
(instance blockOnPhone of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(gMessager sayRandom: 1 0 9 self)
			)
			(3
