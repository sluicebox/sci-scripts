			...
			(4 ; Do
				(cond
					...
					(((gInventory at: 44) state:) ; actionReports
						(gMessager say: noun theVerb 8) ; "Give the form you have to Hal so he can put"
					)
					(else
						(gMessager say: noun theVerb 3) ; "Your report can fit on the form you already have."
					)
				)
			)
			(else
...
(instance leftDrawer of Prop
	...
	(method (doVerb theVerb)
		(cond
			...
			((not cel)
				(gMessager say: noun 0 4) ; "Try opening the drawer first."
			)
			((== theVerb 57) ; photo
				(photo init:)
				(gEgo put: 41) ; photo
			...
			((== theVerb 67) ; actionReports
				(gMessager say: noun theVerb 1) ; "This form does not belong in this drawer."
			)
			(else
				(super doVerb: theVerb)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				...
				(gGlobalSound1 number: 964 setLoop: 1 play:)
				(gCurRoom setInset: (ScriptID 33)) ; pad
			)
			(else
				(super doVerb: theVerb)
			)
