...
(instance rm008 of Rm
	...
	(method (init &tmp [temp0 50])
		...
		(= global56 120)
		(if (!= gPrevRoomNum 11)
			(self setLocales: 700 setFeatures: thePod theMallard)
		)
		(Load rsSCRIPT 991)
		(= local7 991)
		(Load rsVIEW 16)
	...
	(method (handleEvent event param2 &tmp [temp0 50])
		...
		(cond
			((Said 'look>')
				(cond
					((Said '/scrap,iron,debris,garbage')
					...
					((Said '/aluminum,craft[<big]')
						(Print 8 5) ; "It's a sleek-looking number if you can disregard the junk it's rooted in. It must be a recent addition to the collection as everything seems to be intact. Etched on each side is the name `ALUMINUM MALLARD'. On top is a small hatch."
					)
					((Said '/nozzle')
					...
					((Said '/stair')
						(Print 8 24) ; "There are none of those here."
					)
					((Said '/tube,hose,conduit')
						(Print 8 25) ; "There is nothing interesting about them."
					)
					((Said '[<around,at,in][/area,!*]')
						(Print 8 26) ; "You find yourself at the bottom of another trash pit. An interesting array of alien artifacts is strewn from one end to the other. A large ship is in the middle and a small one is off to one side."
					)
				)
			)
			((Said '(play[<with]),use[/leggo,domino,artifact]')
				(Print 8 27) ; "Get serious. We've got a game to play."
			)
			(
				(Said
					'hoist,manipulate,press,pull/craft,leech,aluminum[<escape,big,little]'
				)
				(Print 8 28) ; "Despite technological advances in strong, lightweight ship materials, you can't possibly budge it."
			)
			((Said 'get>')
				(cond
					((Said '/plug[<modular]')
						(if local0
							(Print 8 29) ; "You can't. It's part of the ship."
						else
							(event claimed: 0)
						)
					)
					((Said '/leggo,domino,artifact')
						(Print 8 30) ; "Forget it. They're buried too deeply. Besides, you're too old to be playing with toys - at least you should be!"
					)
					((Said '/ladder')
						(if (IsItemAt 2) ; Ladder
							(cond
								(
									...
									(ladder dispose:)
									(gGame changeScore: -5)
									(Print 8 31) ; "You grab the ladder and jam it in your pocket."
									(Print 8 32) ; "Ouch! (again)"
									(= global194 0)
								)
								...
								(local0
									(Print 8 33) ; "You don't need it up here."
								)
								(else
									(NotClose) ; "It's not quite within reach."
								)
							...
							(Print 8 34) ; "That's not here to be gotten."
						)
					)
				)
			)
			((Said 'enter/!*')
				(Print 8 35) ; "Be more specific."
			)
			((Said 'yell/hal')
				(Print 8 36) ; "I told you not to do that."
			)
			(
				(and
				...
				(Print 8 37) ; "It's too small for you."
			)
			(
				(Said
					'use,erect,drop,stand,place[<up,on]/ladder/leech,(craft<little)'
				)
				(Print 8 38) ; "That won't help."
			)
			(
				(or
			...
			(
				(or
					(Said 'open,enter,(get<in,in)/door<leech,(craft<little)')
					(Said 'climb/leech,(craft<little,escape)')
				)
				(Print 8 39) ; "That can't be done."
			)
			((Said 'cable,generator')
				(Print 8 40) ; "Nothing is done with that here."
			)
			((or (Said 'plug,afix/motivator,device,artifact') (Said 'use/plug'))
				(Print 8 41) ; "That isn't necessary."
			)
			((Said 'close/door')
				(Print 8 42) ; "Check again."
			)
			((Said 'use,erect,drop,stand,place[<up,on,down]/ladder[/craft]')
				(if (gEgo has: 2) ; Ladder
					(cond
						...
						((gEgo inRect: 2 64 91 109)
							(Print 8 43) ; "You're wasting your time messing with this relic."
						)
						(else
							(Print 8 44) ; "It won't work here."
						)
					)
				else
					(Print 8 45) ; "You don't have that."
				)
			)
			((Said 'use,get,climb/debris,scrap,iron,artifact,domino,pole,relic')
				(if (gEgo inRect: 17 122 61 155)
					(gEgo setScript: bleedScript)
				else
					(Print 8 46) ; "That would be fruitless. Don't bother."
				)
			)
			((Said 'climb,crawl[<through,in,in]/pane,port,(cavity<port)')
				(Print 8 47) ; "You can't. It's closed permanently."
			)
			(
				(or
					(Said
					...
					(Said
						'reach,appendage<in,in,in/cavity,leech,craft[<little]'
					)
					(Said
						'reach,appendage<in,in,in/appendage/cavity,leech,craft[<little]'
					)
				)
				(if (gEgo inRect: 2 64 91 109)
					(Print 8 48) ; "I don't think there is much of you that could fit into the seriously snug aperture."
				else
					(NotClose) ; "It's not quite within reach."
				)
			)
			((Said 'descend,climb[<down,up]/ladder')
				(cond
					((IsItemAt 2) ; Ladder
						(cond
							((== (ladder cel:) 1)
								(climbLadderScript init:)
							)
							((== (ladder cel:) 2)
								(Print 8 49) ; "Not with it down there."
							)
							(else
								(Print 8 50) ; "There is no ladder close enough to climb."
							)
						)
					)
					((gEgo has: 2) ; Ladder
						(Print 8 51) ; "There doesn't seem to be a ladder ready for scaling."
					)
					(else
						(Print 8 52) ; "That would be great. If only someone had been smart enough to bring a ladder."
					)
				)
			)
			((Said 'climb[<!*][/!*]')
				(cond
					...
					(
						...
						(if (IsItemAt 2) ; Ladder
							...
							(Print 8 53) ; "There is no good way to scale the slick ship."
						)
					)
					(else
						(Print 8 54) ; "You're not in a good location for climbing that."
					)
				)
			)
			((Said 'climb[<in]/leech')
				(Print 8 55) ; "That's not possible."
			)
			(
				(or
		...
		(if
			...
			(if (gEgo inRect: 215 114 226 121)
				(if (IsItemAt 2) ; Ladder
					...
					(Print 8 56) ; "The ship is too slick. You seem to remember an ectomorphic programmer friend telling you about ships with non-stick coatings for greater debris collision tolerance."
				)
			else
				(Print 8 54) ; "You're not in a good location for climbing that."
			)
		...
		(if (Said 'break/pane,glass')
			(Print 8 57) ; "You can't do that, nor would it be helpful."
		)
	)
	...
	(method (changeState newState &tmp [temp0 50])
		(switch (= state newState)
			...
			(8
				...
				(= local0 0)
				(RedrawCast)
				(Print 8 59) ; "You got lucky this time."
				(HandsOn)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= local6 (Print 8 60 #dispose)) ; "YEOW!"
			)
			(1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(HandsOff)
				(Print 8 62) ; "You move into position and, grabbing the dull finish of the hatch's handle, commence to open and enter the ship."
				(= cycles 2)
			)
			(2
				...
				(cond
					((== global135 3)
						(if (== global147 1)
							...
							(RedrawCast)
							(Print 8 64) ; "Sensing an adequate surface, the claw releases its cargo and begins the ascent to the grabber unit. The object thuds into place within the cavity of the ship."
							(= global135 4)
							(= global149 global147)
						else
							(gLongSong number: 76 loop: 1 play:)
							(gGame changeScore: -15)
							(RedrawCast)
							(Print 8 65) ; "Sensing the floor, the claw releases its cargo and begins the ascent to the grabber unit."
							(motivator init:)
					...
					(
						...
						(gGame changeScore: 15)
						(RedrawCast)
						(Print 8 66) ; "The claw senses contact with the warp motivator, grasps it firmly, and begins the ascent back to the grabber."
					)
					(else
						(Print 8 67) ; "Finding nothing here, the claw begins to ascend to the grabber unit."
						(= global148 4)
					)
				)
				(= cycles 2)
			...
			((Said 'look>')
				(cond
					((Said '/motivator,artifact,device')
						(if global186
							(Print 8 68) ; "It looks like the warp motivator you saw on the Aluminum Mallard's Status Computer. Sturdily constructed, its only protrusion is a modular plug near its base."
						else
							(Print 8 69) ; "A closer look at the object on the floor reveals that it is a warp motivator. It looks a bit more high-tech than the other junk strewn about this locale. Sturdily constructed, its only protrusion is a modular plug near its base."
						)
					)
					((Said '/plug[<modular,8,eightprong]')
						(Print 8 70) ; "The eight-pronged plug protrudes only slightly from the motivator's exterior."
					)
					((Said '[<down,below,at][/dirt,deck]')
						(Print 8 71) ; "The floor is composed of a mosaic of overlapping welded steel plates. The menage of used metal plates appear to have been cut from a variety of sources. There is a small round object lying nearby."
					)
				)
			)
			(
				(Said
					'remove,turn,get,manipulate,press,roll,pull/motivator,artifact,device'
				)
				(Print 8 72) ; "While relatively small, it seems to be quite dense. I think we're looking at a potential hernia if any attempts are made to manually relocate it."
			)
			((Said '(turn<on),start/motivator,artifact,device[<little,round]')
				(Print 8 73) ; "You aren't capable of doing that."
			)
			((Said 'remove,pull,press,get/plug[<modular,8,eightprong]')
				(Print 8 74) ; "You can't. It's permanently attached."
			)
		)
	)
)
...
(instance thePod of Feature
	...
	(method (handleEvent event param2)
		(cond
			...
			((Said 'look/craft,leech[<little]')
				(switch (= param2 (Random 1 2))
					(1
						(Print 8 75) ; "It's a cute little thing. You've never seen anything like it in these parts, but then where are these parts? Some writing on its exterior reads `For a good time, don't call HAL!'."
					)
					(2
						(Print 8 76) ; "It's a cute little thing. You've never seen anything like it in these parts, but then where are these parts? Some writing on its exterior reads `Bowman was here'."
					)
				)
			)
		)
