...
(class GK of Game
	...
	(method (init &tmp [temp0 9] temp9)
		...
		(gSysLogPath format: {})
		(gkDummy init:)
		(gkMusic1 type: 0 owner: self musicVolume: 85 flags: 1 init:)
		(gkMusic2 type: 0 owner: self musicVolume: 85 flags: 1 init:)
	...
	(method (handleEvent event)
		...
		(if (and (& (event type:) evKEYBOARD) (== (event message:) KEY_ALT_n))
			(event claimed: 1)
			(if (not (Print dialog:))
				((ScriptID 64952) doit: (gSysLogPath data:) 0) ; sysLogger
			)
			(return)
		)
