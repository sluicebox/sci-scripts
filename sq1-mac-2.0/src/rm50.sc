...
(instance floatAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(EgoDead 948 0 0 50 0) ; "You drift endlessly through space wishing you had worn a jetpack to maneuver with. As you drift away never to be seen again the only thing you can think of to say is "Mommy!""
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(4
				(if (gEgo has: 11) ; Jetpack
					...
					(Print 50 2) ; "As you undo your seatbelts and exit the escape pod. You can feel yourself just start to uncontrollably drift away."
				)
				(= cycles 24)
			)
			(5
