		...
		(switch (= state newState)
			...
			(6
				...
				(if (or (IsFlag 51) (IsFlag 52) (IsFlag 54))
					(EgoDead 949 0 0 71 1) ; "We hate to tell you this, but you're dead again. At this rate you'll never see the end of the game."
				else
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gCurRoom drawPic: 17)
				(gLongSong
					number: (if (> global106 4) 702 else 701)
					loop: -1
					flags: 1
					play:
				)
