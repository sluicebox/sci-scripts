	...
	(method (changeState newState &tmp [str 268])
		(switch (= state newState)
			...
			(4
				(switch gSilvDollars
					(0
						(Say Doorman_FPortier 700 5 108 139 self) ; "Yeah, that's what Mr. Lowe says, too."
					)
					(1
						(Say Doorman_FPortier 700 6 108 139 self) ; "Right."
					)
					(else
						(Say Doorman_FPortier 700 7 108 139 self) ; "WOW! Come back again, Sir, and soon!"
					)
				)
...
(instance doorman of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(5 ; Talk
				(Say Doorman_FPortier 700 16 108 139 self) ; "May I summon a limousine for you, Sir?"
			)
			(10 ; Zipper
				(Say Doorman_FPortier 700 17 108) ; "Well, you're a little ugly, but I suppose.... Oh, okay. I get off at midnight!"
			)
			(4 ; Inventory
				(Say Doorman_FPortier 700 18 108) ; "Thanks for the offer, but we employees of Tramp's NEVER accept gratuities!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
	...
	(method (cue)
		(if
			...
			(Points 2 132)
			(Say Doorman_FPortier 700 21 108) ; "No problem," says Brewster. "Just watch this!"
...
(instance Doorman_FPortier of Talker
	(properties
		name {Doorman}
		nsTop 15
		nsLeft 50
