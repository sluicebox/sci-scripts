	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(switch (Random 0 5)
					(0
						(Say Lana_FLana 730 2 108) ; "C'mon, Larry! Let's see what you've got!"
					)
					(1
						(Say Lana_FLana 730 3 108) ; "What'sa matter, Larry? Are you a wimp or a mouse?"
					)
					(2
						(Say Lana_FLana 730 4 108) ; "What a wussy! Get up here!"
					)
					(3
						(Say Lana_FLana 730 5 108) ; "A REAL man would be up here already!"
					)
					(4
						(Say Lana_FLana 730 6 108) ; "What's wrong, Big Boy? Afraid I might win?"
					)
					(5
						(Say Lana_FLana 730 7 108) ; "Geez, Larry! What a chicken!"
					)
				)
	...
	(method (doVerb theVerb invItem &tmp [str 200])
		(switch theVerb
			...
			(5 ; Talk
				(switch mainEvent
					(0
						(Say The_Bouncer_FLe_Videur 730 18 108) ; "Da ballroom's closed, Mac! Beat it, before I lose my delly-cate sense of humor."
					)
					(2
						(cond
							((< (gEgo y:) y)
								(Say The_Bouncer_FLe_Videur 730 19 108) ; "There's one good seat down front, right at ringside!"
							)
							((Random 0 1)
								(Say The_Bouncer_FLe_Videur 730 20 108) ; "$25.00, that's the cover charge and ya ain' gettin' in fer less!"
							)
							(else
								(Say The_Bouncer_FLe_Videur 730 21 108) ; "Plenty o' good seats lef, Bud! Butcha ain' gettin' past me 'lessin' ya pays ma price!"
							)
						)
					)
					(1
						(if (< (gEgo y:) y)
							(Say The_Bouncer_FLe_Videur 730 22 108) ; "You wanna chance to play wit' da goils? It'll cost ja $500.00, please! Up front. No refunds when ya get beat!"
						else
							(Say The_Bouncer_FLe_Videur 730 23 108) ; "If ya wanna watch, I kin letcha in for a measly $25.00. If ya wanna participate, it'll cost ya 5 big ones! (Need I mention, it's worth it, too!)"
						)
					)
				)
			)
				...
				(switch invItem
					(17 ; Silver_Dollar
						(cond
							...
							(paid
								(if justWatching
									(Say The_Bouncer_FLe_Videur 730 24 108) ; "Ya paid yer money. Now sit down!"
								else
									(Say The_Bouncer_FLe_Videur 730 25 108) ; "Ya paid yer money. Now get in there!"
								)
							)
							((gEgo sitting:)
								(gEgo setScript: sStandUp)
							...
							((> payoff gSilvDollars)
								...
								(Say The_Bouncer_FLe_Videur 730 28 108) ; "Yeah, and not just in that wimpy little fake eelskin wallet, Donkey-doo! Get out of here!"
							)
							(else
								(if (== payoff 25)
									(Say gEgo 730 29) ; "Hello, Mister," you say to the bouncer. "I'd like to purchase one of your better seats. Here's my $25.00."
									(Say The_Bouncer_FLe_Videur 730 30 108) ; "Thanks for nuttin'. Ya gets da best seat in da house--which is also da only seat in da house!"
									(= justWatching 1)
								else
									(Points 12 134)
...
(instance The_Bouncer_FLe_Videur of Talker
	(properties
		name {The Bouncer}
		nsTop 15
		nsLeft 20
