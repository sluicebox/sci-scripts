...
(instance rm180 of LLRoom
	...
	(method (init)
		...
		(super init:)
		(imprinter init: approachVerbs: 3 stopUpd:) ; Do
		(light init:)
		(bookcase init:)
		(nudePainting init:)
		(desk init:)
		(lamp init:)
		(magnifier init:)
		(drawer init: approachVerbs: 3 stopUpd:) ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 180 0) ; "PPC's file room is filled with file cabinets which are filled with files."
			)
			(else
...
(instance imprinter of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (gEgo has: 7) ; AeroDork_Gold_Card
					(TPrint 180 1) ; "The empty credit card imprinter holds no fascination for you."
				else
					(TPrint 180 2) ; "Say! An exclusive "AeroDork Airlines Gold Card" rests in the credit card imprinter. Probably just someone racking up a few charges on the ol' company dole!"
				)
			)
			(3 ; Do
				(if (gEgo has: 7) ; AeroDork_Gold_Card
					(TPrint 180 3) ; "The imprinter is of no value to you. The Gold Card was all you needed."
				else
			...
			(2 ; Look
				(cond
					((gEgo has: 5) ; Michelle_Milken_s_Resume
						(TPrint 180 4) ; "The "Winners" drawer is now empty."
					)
					(drawerOpen
						(TPrint 180 5) ; "There are three resume files here."
					)
					(else
						(TPrint 180 6) ; "Why, look! Here it is!! You've found it! This drawer is labeled "Winners!""
					)
				)
			)
			(3 ; Do
				(cond
					((gEgo has: 5) ; Michelle_Milken_s_Resume
						(TPrint 180 7) ; "There was nothing else of value in that drawer."
					)
					(drawerOpen
...
(instance sGetCard of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(TPrint 180 8) ; "Assuming that your recent promotion now authorizes such extravagance, you remove the luxurious "AeroDork Gold Card" from the credit card imprinter."
				(HandsOn)
				(self dispose:)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(drawer setMotion: MoveTo 263 (drawer y:) self)
				(= drawerOpen 0)
				(drawer approachVerbs: 0)
			)
			(6
				(if (== register 1)
					(TPrint 180 9) ; "You take the three resumes from the drawer."
				)
				(gEgo normalize:)
				(HandsOn)
				(drawer stopUpd:)
...
(instance bookcase of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 10) ; "The bookcase contains nothing that is of use to you. Perhaps you should look elsewhere."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance nudePainting of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 11) ; "You'd love to fold this up for your wallet, but you refuse to create a bulge in your leisure suit."
			)
			(5 ; Talk
				(TPrint 180 12) ; ""Were you ever featured in `National Geographic?'" you wonder."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance desk of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 13) ; "What? You work?! No way!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance lamp of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 14) ; "You have no use for the lamp."
			)
			(5 ; Talk
				(Say gEgo 180 15) ; ""Testing 1, 2, 3!" you say forcefully, thinking that desk lamp is a hidden microphone."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance loserDrawer1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 20) ; "As you start to open the drawer, you read the label, and decide this is not the drawer for you."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 180 23) ; "Remember: the Sexiest Woman in America. THE Sexiest Woman in America! You're not going to find her sitting around in here!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fTwins of Feature
	...
	(method (doVerb theVerb &tmp [str 225])
		(if (== theVerb 2) ; Look
			(Format ; "This poster is your personal favorite. You've always had a soft spot for %s-haired twins."
				@str
				180
				24
				(if (IsFlag 0) {red} else {blonde}) ; isVga
				(if (IsFlag 0) {frousses} else {blondes}) ; isVga
			)
			(TPrint @str)
		else
