...
(instance Letter_Opener of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 111 #title name) ; "This is merely a razor-sharp letter opener; don't try anything funny with it like picking locks!"
			)
			(3 ; Do
...
(instance Desk_Key of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 113 #title name) ; "Did you know that one key will fit the lock of every piece of pressed-cardboard furniture in the world!?"
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Folder_Of_Evidence of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 4 146)
				(TPrint 19 114 #title name) ; "Examining the papers inside the folder carefully, you find one with some references to gangster activities. It is signed only with a scrawled name you make out to be "Julius.""
				(TPrint 19 115 #title name) ; "These papers provide proof positive of Mob payola at K-RAP Radio! But how will you get them out of here without Krapper finding out?"
			)
			(3 ; Do
...
(instance Photocopied_Evidence of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 118 #title name) ; "These copies provide proof positive of Mob payola at K-RAP Radio!"
			)
			(3 ; Do
...
(instance Reel_To_Reel_Tape of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if state
					(TPrint 19 120 #title name) ; ""D D D" This tape is filled with digitally-recorded incriminating evidence!"
				else
					(TPrint 19 121 #title name) ; "If you could look closely enough, you could see billions of magnetic particles pointing in random directions."
					(TPrint 19 122 #at -1 185) ; "(In other words, this tape is still blank!)"
				)
			)
			(3 ; Do
...
(instance Hooter_Shooter of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 2 76)
				(TPrint 19 124 #title name) ; "Commander Twit's boys may be into explosives, but they know nothing about comfort or adequate support."
			)
			(3 ; Do
				(self doVerb: 2)
			)
