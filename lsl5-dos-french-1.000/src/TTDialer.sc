			...
			(1
				(Format ; "You have reached Directory Assistance for the Greater %s Area. We're sorry, but we're unable to come to the phone right now. We're presently busy (yeah, busy %s!). Let us assure you that your call IS important; it's just not important to us!"
					...
					(if (== gCurRoom 200)
						...
						(switch gLarryLoc
							(1 {La r\82gion de New York City})
							(2 {La partie d'Atlantic City non-infest\82e par Tramp})
							(3 {Dade County})
							(else {`Indiquez la ville ici'})
						)
					)
					(if (== gCurRoom 200)
						{descendre le Potomac}
					else
						(switch gLarryLoc
							(1 {se faire aggresser})
							(2 {mettre des pi\8aces dans les machines\85 sous})
							(3 {bronzer sur la plage})
							(else {utiliser les chiottes})
						)
					)
				)
				(TPrint @string)
