...
(instance rm200 of LLRoom
	...
	(method (cue)
		(if gEgoIsLarry
			...
			(HandsOff)
			(Say gEgo 201 1) ; ""Home, Bobby!" you signal to your driver, and he nods in ready agreement."
			(= gDestination 425)
	...
	(method (notify what)
		...
		(switch what
			...
			(-1
				(TPrint 201 2) ; "You change your mind and hang up the telephone."
			)
			(else
				(TPrint 201 3) ; "Hmmm. Wrong number."
			)
		)
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 201 4) ; "You've always wondered what it would be like to travel by limousine."
			)
			(1 ; Walk
...
(instance fish of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 5) ; "There's a carsick goldfish in the limousine's aquarium."
			)
			(3 ; Do
				(TPrint 201 6) ; "As much as you would love to pet the little fishie, he would prefer to be left alone."
			)
			(5 ; Talk
				(TPrint 201 7) ; "Do you think talking to fish will make them grow?"
			)
			(10 ; Zipper
				(TPrint 201 8) ; "Geez, you are the little sick-o, aren't you!"
			)
			(else
...
(instance glass of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 9) ; "A piece of soundproof glass blocks conversation between you and your driver."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance dayTrotter of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 10) ; "Look! Some poor unfortunate traveler forgot his appointment calendar."
				(= lookedAtDT 1)
			)
			(3 ; Do
				(HandsOff)
...
(instance legs of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 11) ; "You've always felt your legs were one of your better assets."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
			...
			(3 ; Do
				(if gEgoIsLarry
					...
					(if tvOn
						(= tvOn 0)
						(tv hide:)
						(TPrint 201 15) ; "You turn the television set off."
					else
						(= tvOn 1)
						(tv show:)
						(TPrint 201 16) ; "You turn the television set on."
					)
				else
					(TPrint 201 17) ; "It's broken."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance phone of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 18) ; "A cellular phone rests conveniently beside you."
			)
			(3 ; Do
	...
	(method (cue)
		...
		(switch (++ armCue)
			...
			(3
				(= armCue 0)
				(if (not lookedAtDT)
					(TPrint 201 19) ; "Look! Some poor unfortunate traveler forgot his appointment calendar."
				)
				(TPrint 201 20) ; "Yeah, too bad for the other guy. He'll never know. Besides, he would probably want you to have it!"
				(if (not missedDayTrotter)
					(HandsOn)
					(User canControl: 0)
					(if
...
(instance bottle of View
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 21) ; "A chilled bottle of champagne awaits your pleasure."
			)
			(3 ; Do
				(HandsOff)
...
(instance vcr of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 22) ; "A professional quality videotape player rests above the television set."
			)
			(3 ; Do
				(TPrint 201 23) ; "Too bad it's broken."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance tapes of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 24) ; "There are many videotapes in that rack that you would love to see. Unfortunately, the VCR is broken!"
			)
			(3 ; Do
				(TPrint 201 25) ; "You have no way to play those tapes since the limo's videotape player is broken."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fishTank of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 26) ; "How bizarre! A limousine with an aquarium!"
			)
			(3 ; Do
				(if (not (Random 0 10))
					(TPrint 201 27) ; "You would love to grab the little fishy and pet him and make him your friend and take him home and the two of you grow old together and spend his final years resting in a small bowl of briny water on your nightstand."
				else
					(TPrint 201 28) ; "As much as you would love to pet the little fishie, he would prefer to be left alone."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance bottles_Glasses of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 29) ; "There are enough bottles of liquor and glasses to throw a party. All you need now is some friends!"
			)
			(3 ; Do
				(if (gCast contains: bottle)
					(bottle doVerb: 3)
				else
					(TPrint 201 30) ; "Leave the bottles and glasses alone."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance stereo of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 31) ; "The limousine's stereo has both a cassette tape player and a CD."
			)
			(3 ; Do
				(TPrint 201 32) ; "Both the CD player and the cassette deck are broken."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance rightDoor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 201 33) ; "A small, locked liquor compartment rests beneath the right front passenger's seat. You wonder when it will be your turn to live high on the hog."
			)
			(3 ; Do
				(TPrint 201 34) ; "The cabinet is locked!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance windowL of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 201 35) ; "If you roll down the windows, you'll spoil the air-conditioned comfort in which you ride."
			)
			(2 ; Look
				(if driving
					(TPrint 201 36) ; "It looks like you are moving."
				else
					(TPrint 201 37) ; "Bored silly, you find yourself staring out the windows!"
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (and whichFax (not phonedDesmond))
					...
					(TPrint 201 40) ; "The fax machine has no way to dial out. If you want to make a telephone call, your cellular phone rests right beside your right hand."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance driversWindow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if windowOpen
					(TPrint 201 41) ; "Your limo driver."
				else
					(TPrint 201 42) ; "You are sealed off from your driver by that big soundproof glass window."
				)
			)
			(else
		...
		(switch (= state newState)
			...
			(31
				...
				(if (and (== gDestination 500) (not (gEgo has: 11))) ; DayTrotter
					(TPrint 201 45) ; "As you are ready to exit the limousine, you notice a small leather binder on the seat beside you. You feel certain the limo company will make no attempt to locate its rightful owner, so you take it, vowing to search relentlessly until you find its owner and return his missing DayTrotter\05 to him!"
					(= missedDayTrotter 1)
			...
			(32
				...
				(CenterDisplay 2 gColWhite 200 0)
				(= seconds 3)
			)
			(33
				(switch gDestination
		...
		(switch theVerb
			(2 ; Look
				(if gEgoIsLarry
					(switch gLarryLoc
						(0
							(TPrint 201 53) ; "Bobbi Bang, your limo driver, is as cute as a button."
						)
						(1
							(TPrint 201 54) ; "Bobbi Bangue, your present limo driver, is as cute as a pin."
						)
						(2
							(TPrint 201 55) ; "Bobbi Bahng, your latest limo driver, is as cute as a baby's bottom."
						)
						(3
							(TPrint 201 56) ; "Bobbi Biang, your current limo driver, is as cute as any limo driver you've ever had."
						)
					)
				else
					(TPrint 201 57) ; "Bobby Bang, your limo driver for the entire tour, is about as cute as guys come."
				)
			)
			(3 ; Do
				(cond
					(driving
						(TPrint 201 58) ; "Please leave your driver alone!"
					)
					(gEgoIsLarry
						(TPrint 201 59) ; "You tap on the glass, but there is no answer."
					)
					(else
						(HandsOff)
			...
			(5 ; Talk
				(cond
					...
					(gEgoIsLarry
						(switch (Random 1 2)
							(1
								(TPrint 201 60) ; "You try to get the driver's attention, but she ignores you."
							)
							(2
								(TPrint 201 61) ; "The driver points to her ear and shrugs, indicating she is unable to hear you through the soundproof glass window."
							)
						)
					...
					(switch invItem
						...
						(0 ; Camcorder
							(switch ((Inv at: 0) state:) ; Camcorder
								(0
									(TPrint 201 89) ; "Your driver stares blankly at the totally blank DataMan."
								)
								(1
									(cond
										((== gPattiLoc 1)
		...
		(switch (= state newState)
			...
			(3
				(cond
					...
					(phonedDesmond
						(TPrint 201 108) ; "Hang up! You have an incoming fax! Don't tie up the line."
						(++ state)
						(= cycles 2)
					)
					((not whichFax)
					...
					(else
						(TPrint 201 109) ; "You don't need to use the phone right now. But perhaps later."
						(++ state)
						(= cycles 2)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(TPrint 201 122) ; "The fax machine awakens..."
				(= ticks 90)
			)
			(2
				(faxPaper
		...
		(switch (++ faxPaperCue)
			...
			(3
				(switch whichFax
					(1
						(Points 13 103)
						(TPrint 201 123) ; "You grab the fax from the machine. The header says "Reverse Biaz.""
						(gEgo get: 3) ; A_Blank_Videotape_b
					)
					(2
						(Points 13 104)
						(TPrint 201 124) ; "You grab the fax from the machine. The header says "P. C. Hammer.""
