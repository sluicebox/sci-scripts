...
(procedure (TalkFromBooth)
	...
	(Say Reverse_Biaz 640 58 #dispose) ; "Hello, out there! You must be Passionate Patti! Everything's all ready for your cut-in session. Just come on into the studio, I'm waiting for you."
)
		...
		(switch theVerb
			...
			(3 ; Do
				(cond
					...
					(askedInToStudio
						(SetFlag 21) ; fWarned
						(Say Reverse_Biaz 640 25 #dispose) ; "Where are you going, Patti? There are a lot of people in this town who would love a chance to blow synth on a session here. I was happy to give you the gig. But, if you leave me now, I promise you I'll be on the phone to the Union before you're downstairs!"
					)
					(else
						(TalkFromBooth)
					)
...
(instance plant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 52) ; "Upon close examination, you determine this is a plant."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance shelves of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 53) ; "The office's shelves are filled with records, tapes, and books."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance recordJacketsA of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 54) ; "Many famous record album jackets adorn the walls of the office of des Rever Records."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fGoldRecords of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 55) ; "These gold records are firmly attached to the wall."
			)
			(3 ; Do
				(TPrint 640 56) ; "You are unable to remove this gold record from its plaque."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
