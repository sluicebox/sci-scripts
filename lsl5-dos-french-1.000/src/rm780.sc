...
(instance rm780 of LLRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 780 0) ; "Now you've done it! You're swimming along under the beautiful Tramp Boardwalk, having a wonderful time. The question is: how far from shore are you?"
			)
			(else
...
(instance post1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 780 1) ; "You are unable to climb the slimy, slippery surface of the pilings."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance swimActions of Actions
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(TPrint 780 2) ; "You are swimming beneath the boardwalk!"
			)
			(3 ; Do
				(TPrint 780 3) ; "There's no need to dive, Larry."
			)
			(else
				(return 0)
			)
