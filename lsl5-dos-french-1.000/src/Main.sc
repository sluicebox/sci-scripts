...
(procedure (TPrint arg1 arg2 &tmp [str 410] theTime saveCursor)
	(if gModelessDialog
...
(instance LSL5 of Game
						...
						(if gEgoIsLarry
							(switch theItem
								(0
									...
									(TPrint 0 9 67 -1 185) ; "(But is your pocket camcorder working?)"
								)
								(6
								...
								(else
									(if 1
										...
										(Format @str 0 16 global200 global203) ; "Try finding some place else to place %s."
							...
							(switch theItem
								(3
									...
									(TPrint 0 18 67 -1 185) ; "(But not much!)"
								)
								(4
								...
								(7
									(TPrint 0 21) ; "You wave the fax in the air. You have fun."
									(TPrint 0 18 67 -1 185) ; "(But not much!)"
								)
								(13
									(TPrint 0 22) ; "You briefly contemplate firing your bra at that, but decide to hold your "fire.""
								)
								(else
									(if 1
										(= global200
	...
	(method (quitGame &tmp [str1 10] [str2 8] theFile)
		...
		(if
			...
			(if (theFile open: 2)
				(Format @str2 0 24 gPokerJackpot) ; "%d"
				(theFile
					writeString: @str1
					writeString: {\n}
					writeString: @str2
	...
	(method (startRoom roomNum)
		...
		(if (and gDebugging (not (OneOf roomNum 200 460)))
			(= gDebugging 0)
		)
		(gTheIconBar enable:)
			...
			(if
				...
				(if (== gFfRoom 1000)
					(if (IsObject gFfCueObj)
						...
						(Print 0 26) ; "ERROR: Object passed to SetFFRoom -1 ain't."
					)
				else
					(if (== gCurRoomNum 535)
		...
		(switch theVerb
			...
			(2
				(if (theObj lookStr:)
					...
					(if 1
						...
						(Format @str 0 27 global200 global203) ; "Why, look! It's %s."
						(Memory memFREE global200)
						(Memory memFREE global203)
					)
					(TPrint @str)
			...
			(5
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 28 global200 global203) ; "You engage in a short, but personally meaningful, conversation with %s."
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
			...
			(3
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 29 global200 global203) ; "That feels exactly like %s."
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
			...
			(4
				...
				(if 1
					...
					(Format @str 0 30 global200 global201 global203 global204) ; "It seems %s just doesn't work with %s."
					(Memory memFREE global200)
			...
			(10
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen objDesc)))
					(StrSplitInTwo global200 global203 objDesc)
					(Format @str 0 31 global200 global203) ; "Don't do that to %s!"
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @str)
...
(instance gcWin of SysWindow
	...
	(method (open &tmp [temp0 2] temp2 temp3 temp4 temp5 temp6 temp7 [temp8 4] temp12 temp13 [temp14 20] [temp34 4])
		(= temp12 -1)
		(self
		...
		(Format @temp14 0 32 gScore gPossibleScore gScore gPossibleScore) ; "Score: %d of %d"
		(TextSize @temp34 @temp14 999 0)
		(Display
			@temp14
			dsFONT
			999
