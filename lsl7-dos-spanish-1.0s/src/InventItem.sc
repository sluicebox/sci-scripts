...
(class InvSlot of View
	...
	(method (getName)
		(if ioMine
			...
			(PrintDebug {trying to get name of empty slot. INV.SC DJM})
			(return)
		)
	)
	...
	(method (doVerb theVerb &tmp temp0)
		(if ioMine
			...
			(PrintDebug {empty slot getting doVerbs. INV.SC DJM})
			(return)
		)
	)
)
...
(class InvHandler of Obj
	...
	(method (findFirstOpen &tmp temp0)
		...
		(Prints {error -- can't find open inventory slot. djm, dminv.sc})
		(return -1)
	)
