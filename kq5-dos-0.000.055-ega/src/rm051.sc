...
(local
	...
	[local1 2]
	local3
	[local4 22] = [0 189 0 130 130 130 123 154 81 157 104 158 118 162 114 168 88 168 92 175 71 189]
	...
	[local58 18] = [231 189 240 178 258 177 260 162 188 161 170 156 174 121 319 121 319 189]
	[local76 8] = [106 180 130 172 154 177 138 184]
	[local84 8] = [173 164 205 164 206 179 175 183]
	[local92 16] = [319 118 172 118 169 83 150 74 240 49 0 14 0 0 319 0]
)
...
(instance rm051 of Rm
	...
	(method (init)
		...
		(poly5 points: @local58 size: 9)
		(poly6 points: @local76 size: 4)
		(poly7 points: @local84 size: 4)
		(poly8 points: @local92 size: 8)
		(sailBoat init:)
		(rubble init:)
		(mast init: setPri: 11)
		...
		(switch (= state newState)
			(0
				(if (gCast contains: global322)
					(Say 75 51 0 67 10 10 25 5) ; "I think I'll stay here until you come back."
				)
				(= cycles 1)
			)
			(1
...
(instance marooned of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (not (IsFlag 56))
					(Say 75 51 1 67 100 20 25 4) ; "Help me, I'm caught! Oh, help!"
					(Cedric
						cycleSpeed: 3
						ignoreActors: 1
						setPri: -1
			...
			(6
				(Say 160 51 2 67 20 20 25 4) ; "Are you all right, Cedric?"
				(gEgo
					normal: 1
					view: 0
					posn: 162 173
			...
			(7
				(Say 75 51 3 67 100 20 25 3) ; "Well, let me see..."
				(Cedric loop: 11 cel: 0 setPri: 14 setCycle: End self)
			)
			(8
				(Say 75 51 4 67 100 20 25 7) ; "I'm fine, Graham. Just a bit ruffled, is all."
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				...
				(gEgo normal: 0)
				(if (gCast contains: global322)
					(Say 75 51 5 67 20 20 25 3) ; "No, don't...!"
				)
				(= cycles 1)
			)
			(1
...
(instance getFish of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: PolyPath 115 160 self)
			)
			(1
				((gEgo head:) hide:)
				(gEgo
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(4 ; Talk
					(if (not local3)
						(Say 75 51 12 67 10 10 25 8) ; "I hate to say this, Graham, but...I don't like this place at all!"
						(Say 160 51 13 67 10 10 25 5) ; "I know what you mean, Cedric."
						(= local3 1)
						(event claimed: 1)
					else
						(event claimed: 0)
