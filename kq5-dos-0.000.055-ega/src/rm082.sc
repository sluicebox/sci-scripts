	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 82 0) ; "A narrow tunnel has been dug down into the ground. Weak sunlight brightens one end while a dim, eerie light beckons from the other end."
					(event claimed: 1)
				)
			)
		)
