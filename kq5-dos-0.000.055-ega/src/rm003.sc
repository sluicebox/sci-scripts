...
(instance rm003 of Rm
	...
	(method (init)
		(super init:)
		(gGlobalSound number: 29 loop: -1 vol: 127 play:)
		(gGlobalSound2 number: 886 loop: -1 vol: 80 play:)
		(Load rsVIEW 48)
		(cedric init:)
		(if (== global81 3)
	...
	(method (doit)
		(cond
			...
			((& (gEgo onControl: 0) $4000)
				(if (not (IsFlag 30))
					(SetFlag 30)
					(Say 75 3 0) ; "If you go on into town I'll wait for you here. I had a nasty run-in with a dog once and I'd feel safer out here."
				)
				(HandsOff)
...
(instance drownHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say 75 3 1 67 20 20) ; "Wait, Graham! Be careful...!"
				(gEgo
					normal: 0
					view: 48
					setPri: 15
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(Say 75 3 9) ; "Come on, Graham! Don't waste your time with that cow!"
					(event claimed: 1)
				)
			)
		)
		...
		(if
			...
			(switch (event message:)
				...
				(4 ; Talk
					(if (not local1)
						(++ local1)
						(Say 75 3 11) ; "If you follow this road along the river you'll come to the town bakehouse. You might find something delicious...I mean, useful there."
					else
						(switch (Random 0 3)
							(0
