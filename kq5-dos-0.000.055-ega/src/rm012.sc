...
(local
	...
	[local43 18] = [319 127 194 132 172 138 123 138 108 123 166 99 251 97 251 40 319 40]
...
(instance rm012 of Rm
	...
	(method (init)
		...
		(if (!= ((gInventory at: 6) owner:) 200) ; Brass_Bottle
			(gAddToPics add: wagon doit:)
		)
		(proc0_24 128 338 325)
		(gEgo ignoreHorizon: 1 offset: 3 init:)
		(if (and (or (gEgo has: 8) (gEgo has: 16)) (not (IsFlag 107))) ; Shoe, Stick
			(proc0_24 128 332)
			(Load rsSCRIPT 991)
		...
		(cond
			...
			((= local0 (self edgeToRoom: (gEgo edgeHit:)))
				(if (== local0 (gCurRoom west:))
					...
					(gEgo loop: 1)
					(RedrawCast)
					(Say 75 12 0 67 10 10 25 10) ; "There's nothing but a hot, dry desert further west. Most people avoid it, because there are bandits out there! If you insist on going, I'll wait for you HERE!"
					(gGlobalSound fade:)
					(HandsOn)
...
(instance warnScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 12 1 #at 20 10 #time 7) ; "A bully of a dog terrorizes the poor ants as he digs up their large anthill."
				(HandsOn)
				(client setScript: 0)
			)
		)
...
(instance relievedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Say 75 12 2 67 20 100 25 5) ; "Well, there you are! I was starting to get concerned!"
				(Say 160 12 3 67 20 20 25 5) ; "Don't worry about me, Cedric. I'm used to this kind of thing."
				(HandsOn)
				(client setScript: 0)
			)
		)
...
(instance antDance of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo cycleSpeed: 2 cel: 0 setLoop: 1 setCycle: End self)
			)
			(2
			...
			(6
				(Say 75 12 4 67 10 10 25 4) ; "Is that a new dance, Graham? The BUG-a-loo?"
				(HandsOn)
				(client setScript: 0)
			)
		)
...
(instance throwStick of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 12 5 #at 100 10 #dispose) ; "Here, boy!"
				((gEgo head:) hide:)
				(gEgo
					normal: 0
					view: 296
			...
			(2)
			(3
				(gEgo normal: 1 view: 0 cycleSpeed: 0 setCycle: Walk)
				((gEgo head:) show:)
				(proc0_7 gEgo stick)
			...
			(10
				...
				(PrintDC 12 6 #at 20 20 #width 140 #dispose) ; "Allow me to introduce myself. I'm King Antony the Great. May I ask who you are?"
				(= seconds 7)
			)
			(11
				(cls)
				...
				(mouth setCycle: 0)
				(RedrawCast)
				(Say 160 12 7 67 15 90 25 20 70 280) ; "Why certainly. I'm King Graham of Daventry and this is my friend, Cedric. We're seeking a way to cross the Great Mountains to the ocean on the other side."
				(RedrawCast)
				(= cycles 1)
			)
			(12
				(PrintDC 12 8 #at 10 10 #width 140 #dispose) ; "That is a very perilous undertaking. I wish you would reconsider. But if you shall not, in return for rescuing our home from that flea-bitten cur I wish to offer you our help if perchance you may ever need it."
			...
			(13
				...
				(mouth setCycle: 0)
				(RedrawCast)
				(Say 160 12 9 67 15 100 70 280 25 15) ; "Thank you very much, King Antony. Cedric and I appreciate your kind offer. We look forward to meeting you again."
		...
		(switch (= state newState)
			(0
				(if (not register)
					(++ register)
					(cls)
					(Say 75 12 10 67 20 20 25 7) ; "Oh, Graham! That dog looks mean! I don't like dogs!"
				)
				(dog loop: 0)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 12 11) ; "A colony of large ants parade up and down a huge anthill built amid some scrubby bushes."
					(event claimed: 1)
				)
				(4 ; Talk
					(PrintDC 12 12) ; "The ants are too busy to engage in conversation with Graham."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 12 13) ; "The ants don't seem to take kindly to uninvited trespassing."
					(event claimed: 1)
				)
				(5 ; Inventory
					(switch global69
						(16 ; Stick
							(PrintDC 12 14) ; "The ants aren't interested in it."
							(event claimed: 1)
						)
						(28 ; Wand
							(event claimed: 0)
...
(instance dog of Act
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 12 15) ; "A mangy old dog digs playfully at the huge anthill while the poor ants scurry frantically around."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 12 16) ; "The dog doesn't seem to be a friendly sort at all!"
					(event claimed: 1)
				)
				(4 ; Talk
					(PrintDC 12 17) ; "The dog doesn't pay any attention to Graham."
					(event claimed: 1)
				)
				(5 ; Inventory
					(switch global69
						...
						(else
							(PrintDC 12 18) ; "That wouldn't divert the dog's attention from the anthill."
							(event claimed: 1)
						)
					)
				)
...
(instance wagon of RPicView
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 12 19) ; "A gypsy wagon can be seen in the distance to the north."
					(event claimed: 1)
				)
			)
		)
