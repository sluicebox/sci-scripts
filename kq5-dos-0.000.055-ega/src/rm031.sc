...
(local
	...
	local8
	local9
	[local10 12] = [0 142 18 139 20 132 78 122 78 0 0 0]
	[local22 10] = [281 117 298 128 282 138 248 139 227 117]
	[local32 10] = [204 112 277 112 319 122 319 0 204 0]
	[local42 12] = [205 158 206 189 264 189 262 159 242 152 223 152]
	[local54 8] = [42 150 107 136 75 189 42 189]
	[local62 8] = [54 55 56 57 55 60 58 59]
)
	...
	(method (init)
		...
		(switch gPrevRoomNum
			(30
				...
				(gGlobalSound2 number: 54 loop: 1 vol: 100 play:)
			)
			(else
				(gEgo view: 10 offset: 3 posn: 312 156 setPri: 11 init:)
				(= local0 8)
		...
		(poly2 points: @local22 size: 5)
		(poly3 points: @local32 size: 5)
		(poly4 points: @local42 size: 6)
		(poly5 points: @local54 size: 4)
	...
	(method (doit &tmp temp0)
		(cond
			...
			((& (gEgo onControl: 1) $2000)
				(HandsOff)
				(if (not (IsFlag 35))
					(Say 75 31 0 67 10 10 25 5) ; "No! Stay away from the edge...!"
				)
...
(instance jumping of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (and (not (IsFlag 35)) (not local5))
					(Say 75 31 2 67 10 10 25 5) ; "Do be careful, Graham!"
					(++ local5)
				)
				(cond
					((== local1 7)
			...
			(3
				...
				(if (or local9 (== local1 100))
					...
					(= local0 local1)
					(gGlobalSound3 number: [local62 local1] loop: -1 vol: 100 play:)
					(gEgo
						setLoop: (+ (gEgo loop:) 6)
						cel: 0
						setCycle: End self
			...
			(6
				(PrintDC 31 1 #at 20 20 #dispose) ; "Aaaaiiiieeeeeeee!!"
				(gGlobalSound3 number: 790 vol: 80 loop: 1 play: self)
				(if (!= local1 100)
					(local7
						view: 497
						setLoop: local8
