...
(local
	...
	[local32 27]
	[local59 20]
	[local79 3] = [281 282 283]
	local82
	[local83 10]
	[local93 10]
	[local103 150]
	[local253 3] = [33 85 137]
	[local256 3] = [76 128 180]
)
...
(procedure (localproc_1 &tmp temp0 temp1 temp2 [temp3 30])
	(= local82 (= temp1 0))
	(= temp0 [local79 gDifficulty])
	(while (Message msgGET temp0 (+ local82 2) 7 0 1 @temp3)
		(= [local93 local82] temp1)
		(++ local82)
		(for
			((= temp2 1))
			(Message msgGET temp0 (+ local82 1) 7 0 temp2 @temp3)
			((++ temp2))
			
			(= [local103 temp1] (ReadNumber @temp3))
			(++ temp1)
		)
		(= [local83 (- local82 1)] (- temp2 1))
	)
)
...
(instance bookCase of PuzzleBar
	...
	(method (init &tmp temp0)
		...
		(= local29
			(= local31
				(= [local23 0] (= [local23 1] (= [local23 2] (= local2 0))))
			)
		)
		(switch gDifficulty
			(0 ; Novice
	...
	(method (showHelp)
		(super showHelp: &rest)
		(Say self (+ local3 2) 1 0 1 0 [local79 gDifficulty])
	)
	...
	(method (buyClue &tmp temp0 temp1 temp2 [temp3 20] [temp23 30])
		(if (super buyClue: &rest)
			...
			(Message msgGET [local79 gDifficulty] 1 1 (temp2 message:) 1 @local59)
			(Message msgGET msgModule 13 4 7 1 @temp23)
			(Printf @temp23 @temp3 @local59)
		)
	)
...
(instance book of CodeIcon
	...
	(method (init param1 &tmp temp0 temp1)
		...
		(= modifiers (localproc_0 [local23 value] 1 [local26 value]))
		(|= [local23 value] (<< $0001 modifiers))
		(= temp0 1)
		(while temp0
			(if
				(not
					(&
						[local32 (/ (= temp1 (mod (Random 0 480) 24)) 8)]
						(<< $0001 (mod temp1 8))
					)
				)
		...
		(cond
			...
			(param1
				...
				(Print
					...
					x: (+ nsLeft 80)
					y: (+ nsTop 14)
					addText: 1 4 message modifiers 0 0 [local79 gDifficulty]
					init:
				)
			)
...
(instance shelf of CodeIcon
	...
	(method (init param1 &tmp temp0 temp1)
		(= temp0 (localproc_0 local29 0 (- [local83 local3] 1)))
		(|= local29 (<< $0001 temp0))
		(= message [local103 (+ [local93 local3] temp0)])
		(self
			value: param1
			nsTop: [local253 param1]
			nsBottom: (+ [local253 param1] 41)
		)
		(for ((= temp1 (= modifiers 0))) (< temp1 local0) ((++ temp1))
			(= temp0 (localproc_0 modifiers 0 8))
			(|= modifiers (<< $0001 temp0))
		)
		(= [local32 param1] modifiers)
		(= [local26 param1] (proc15_6 [local79 gDifficulty] 1 4 message))
		(= [local20 param1] self)
	)
			...
			(cond
				((& modifiers (<< $0001 temp0))
					(DrawCel
						286
						0
						(= temp13 (localproc_0 local30 4 12))
						temp1
						temp2
						-1
					)
				...
				((not (& [local32 value] (<< $0001 temp0)))
					(DrawCel 286 0 (Random 1 3) temp1 nsTop -1)
				)
			)
		)
		(Message msgGET [local79 gDifficulty] 1 1 message 1 @temp3)
		(Display @temp3 dsCOORD 17 [local256 value] dsWIDTH 118 dsALIGN alCENTER dsFONT 999)
	)
