	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gGkSound2 setLoop: -1 number: 801 play:)
	...
	(method (dispose)
		...
		(cond
			...
			(else
				...
				(if (== ((ScriptID 800 0) drumCount:) 3) ; Hounfour
					(sCeremonialRoom setScript: sGabeDies)
				else
					(gGame handsOn:)
				)
			)
...
(instance sGabeDies of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gMessager say: 13 0 10 3 0 900) ; "You lie! You'll die for that!"
			...
			(14
				((ScriptID 800 1) show: posn: 0 0) ; xDJ
				(gCurRoom newRoom: 770) ; deathRm
			)
		)
	)
