...
(instance rm230 of Room
	...
	(method (notify)
		(return
			(cond
				(local6 2)
				((or (IsFlag 100) local2 (not local1)) 1)
				(local17 3)
				(else 0)
			)
		)
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(else
				(if (not (or (IsFlag 100) local2 (not local1)))
					(if (IsFlag 11)
...
(instance sEnterMosDoorFromLobby of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (or (IsFlag 100) local2 (not local1))
...
(instance sInGateWithoutPermission of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gMessager say: 6 6 (if (IsFlag 16) 21 else 9) 1)
				(gEgo
					view: 233
					loop: 0
					cel: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(25
				...
				(if
					(and
						(not (or (IsFlag 100) local2 (not local1)))
						(IsFlag 11)
					)
					((ScriptID 231 0) init: 1) ; sal
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(extDoor show: setCel: 5 setCycle: Beg self)
			)
			(3
				(gGkSound2 number: 374 loop: 1 play:)
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb 6 9 8) ; Open, Move, Operate
			...
			(cond
				...
				((or (IsFlag 100) local2 (not local1))
...
(instance inBox of Feature
	...
	(method (init)
		...
		(if (and (not (gEgo has: 16)) (IsFlag 13) (not (IsFlag 3))) ; policeVeve
			((ScriptID 231 22) init:) ; folder
		)
		(super init: &rest)
	)
