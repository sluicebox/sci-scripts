...
(instance bayouRitual of Room
	...
	(method (init)
		(gGkMusic1 setLoop: -1 number: 480 play:)
		(Load rsPIC 480)
		(Load rsSCRIPT 64941)
...
(instance roomScript of Script
	...
	(method (handleEvent event)
		...
		(if (and local4 (== (event type:) evKEYBOARD) (== (event message:) KEY_SPACE))
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(14
				(gGkSound1 setLoop: -1 number: 482 play:)
			...
			(21
				(local2 add: (closeChicken init: yourself:))
				(gMessager say: 5 0 16 1 self) ; "Damballah Oueddo, Ou couleuvre moins!"
			)
			(22
				(local2 add: (bowl init: yourself:))
				(= cycles 10)
			)
			(23
				(local2
			...
			(24 1)
			(25
				(local2 add: (deadChickens init: yourself:))
			...
			(37
				(UnLoad 129 610)
				(local2 add: (ohMyHead init: setCycle: End self yourself:))
				(gMessager say: 5 0 16 4 self) ; "(A DRUG IS BEING SPRAYED INTO HIS FACE)Uggh <gasp>!"
			)
			(38 1)
			(39
				(local2 add: (drums init: yourself:))
				(gMessager say: 5 0 16 5 self) ; "Tetelo! Ou couleuvre moins!"
			)
			(40
				(local2 add: (theEyes init: yourself:))
				(local2 add: (eyeBlink init: setCycle: End self yourself:))
			)
			(41
				(local2 add: (voodooShit init: yourself:))
				(= seconds 2)
			)
			(42
				(local2 add: (maliaWithSnake init: yourself:))
			...
			(51
				...
				(PalCycle 4 192) ; Off
				(PalCycle 4 208) ; Off
				(local2 add: (knife init: yourself:))
				(for ((= temp1 0)) (< temp1 100) ((+= temp1 4))
					(Palette 2 0 256 temp1) ; PalIntensity
					(FrameOut)
				)
			...
			(53
				(local2 add: (gabeNoMask init: yourself:))
				(= seconds 2)
			)
			(54
				(local2 add: (gabeFace init: yourself:))
				(local2 add: (eyebrow init: setCycle: End self yourself:))
			)
			(55
				(for ((= temp1 100)) (> temp1 0) ((-= temp1 4))
					(Palette 2 0 256 temp1) ; PalIntensity
			...
			(60
				(local2 add: (grabMask init: yourself:))
				(gMessager say: 5 0 16 9 self) ; "!!!Blank msg"
			)
			(61
				(local2 add: (uncovered init: yourself:))
				(= seconds 1)
			)
			(62
				(local2 add: (maliaFace init: yourself:))
				(local2 add: (maliaGasp init: setCycle: End self yourself:))
			...
			(69
				(UnLoad 129 614)
				(local2 add: (possessionFace init: yourself:))
				(= seconds 2)
			)
			(70
				(local2 add: (gabeStaggers init: yourself:))
				(gMessager say: 5 0 16 12 self) ; "(FEARFUL--QUESTIONING--IS THAT YOU?)Malia?"
			)
			(71
				(local2 add: (madMalia init: yourself:))
			...
			(74
				(local2 add: (crazedMalia init: yourself:))
				(local2 add: (aaagh init: yourself:))
				(= cycles 3)
			)
			(75
			...
			(76 1)
			(77
				(local2 add: (graceRescues init: yourself:))
				(= seconds 2)
			)
			(78
				(gGkSound2 setLoop: 1 number: 481 play:)
				(local2 add: (graceHand init: setCycle: End self yourself:))
			)
			(79
				(= seconds 2)
			)
...
(instance questions of GKTeller
	...
	(method (showCases)
		(super showCases: 5 (not local0) 7 (not local0) 6 local0 8 local0) ; Maitresse Erzulie., Papa Gede?, Buddha, perhaps?, Ogoun Badagris.
	)
