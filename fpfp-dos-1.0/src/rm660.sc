		...
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					...
					(gMessager say: noun theVerb 4) ; "Get any fingerprints on anything and I'll be happy to staple your fingers over them."
				)
			)
			(1 ; Look
				(if (== gCurPuzzle 3)
...
(instance sGetFlies of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(flyTimer seconds: 1)
				(gMessager say: 35 0 0 (Random 1 5) self)
			)
			(3
				(= next sHelenWorks)
				(self dispose:)
...
(instance sDunkPlop of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6 0)
			(7
				(gMessager say: 36 0 0 0 self) ; "|c2|LOOK WHAT YOU'VE DONE!!|c| Freddy Pharkas, I'll see you run out of town for this!!"
			)
			(8
				(hopSingh
					view: 664
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(cond
					((gEgo has: 11) ; Coffee
						(gMessager say: noun 4 1) ; "Mr. Pharkas, I have no objection to your coming in here and taking a cup of coffee. But please, limit yourself to one cup at a time."
					)
					((IsFlag 157)
		...
		(switch theVerb
			(2 ; Talk
				(cond
					...
					((== gCurPuzzle 13)
						(gMessager say: noun theVerb 12) ; "Hey, Hop Singh! Don't serve any water 'till I've had a chance to check it out. I think something's up with the water supply."
					)
					((== gCurPuzzle 8)
						(gMessager say: noun theVerb 9) ; "Hop Singh, I just want to warn you NOT to go outside and NOT to let your customers go outside 'till we've cleared the air."
					)
					((== gCurPuzzle 9)
						(gMessager say: noun theVerb 9) ; "Hop Singh, I just want to warn you NOT to go outside and NOT to let your customers go outside 'till we've cleared the air."
					)
...
(instance tinCan of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Better not disturb any of Mom's shelves unless you're here to get her permission."
				else
...
(instance shelf of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (== gCurPuzzle 3)
					(gMessager say: noun theVerb 5) ; "Better wait 'till Mom is here before you try to take anything from her shelves."
				else
					(gMessager say: noun theVerb 4) ; "I'm so glad you like the decor in here, Mr. Pharkas."
				)
			)
			(else
				(super doVerb: theVerb &rest)
