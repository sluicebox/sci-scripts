...
(instance rm800 of LLRoom
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 800 0) ; "You are outside the studios of `K-Rap Radio.' It appears to be an amazingly small building for a station of such importance."
			)
			(else
...
(instance sIntoBuilding of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TPrint 800 1 #at -1 185 #width 280 #dispose self) ; "Although you can't see it, the K-RAP building is quite impressive when viewed from the opposite side."
			)
			(2
				(= ticks 180)
			)
			(3
				(TPrint 800 2 #at -1 185 #width 280 #dispose self) ; "From the tiny top floor you just entered, it stretches hundreds of feet down the hillside, in a series of cascading terraces, gardens, and patios."
			)
			(4
				(= ticks 180)
			)
			(5
				(TPrint 800 3 #at -1 185 #width 280 #dispose self) ; "Obviously, there's more to this place than meets the eye!"
			)
			(6
				(= ticks 180)
			)
