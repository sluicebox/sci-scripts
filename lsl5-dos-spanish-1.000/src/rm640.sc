...
(procedure (TalkFromBooth)
	(= askedInToStudio 1)
	(TPrint 640 56) ; "A speaker crackles in the background..."
	(Say Reverse_Biaz_SReverse_Biaz 640 57 108) ; "Hello, out there! You must be Passionate Patti! Everything's all ready for your cut-in session. Just come on into the studio, I'm waiting for you."
)
...
(instance sEnterElevator of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(Say Reverse_Biaz_SReverse_Biaz 640 20 108 139 self) ; "Patti, I promise you: you'll never work in this town again!"
			)
			(5
				(elevatorDing number: 622 play:)
				(elevator setCycle: Beg self)
...
(instance elevator of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 24) ; "This is the elevator in which you arrived."
			)
			(3 ; Do
				(cond
					((IsFlag 21)
					...
					(askedInToStudio
						(SetFlag 21)
						(Say Reverse_Biaz_SReverse_Biaz 640 25 108) ; "Where are you going, Patti? There are a lot of people in this town who would love a chance to blow synth on a session here. I was happy to give you the gig. But, if you leave me now, I promise you I'll be on the phone to the Union before you're downstairs!"
					)
					(else
						(TalkFromBooth)
					)
...
(instance reverseBiaz of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 26) ; "This is your man: Reverse Biaz, chief engineer of `des Rever Records' and the man you've been sent to investigate."
				(if (not askedInToStudio)
					(TalkFromBooth)
				)
			)
			(5 ; Talk
				(TPrint 640 27) ; "The double-paned, soundproof glass prevents him from hearing you."
				(if (not askedInToStudio)
					(TalkFromBooth)
				)
			)
			(3 ; Do
				(TPrint 640 28) ; "It's difficult to do anything through this glass."
			)
			(4 ; Inventory
				(switch invItem
					(4 ; A_Blank_Videotape_c
						(TPrint 640 29) ; "You don't pour THROUGH glass, you pour INTO glass!"
					)
					(else
						(TPrint 640 30) ; "He's probably not going to break that double-paned, soundproof glass to accept it, Patti!"
					)
				)
			)
			(10 ; Zipper
				(TPrint 640 31) ; "Gee, Patti, he IS cute--but aren't you here for business?"
			)
			(else
				(TPrint 640 32) ; "There's nothing you can do with Reverse Biaz from out here."
			)
		)
	)
)
...
(instance turntable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 33) ; "What an unusual turntable; it still has a 78 RPM setting."
			)
			(3 ; Do
				(TPrint 640 34) ; "The turntable doesn't require a push start."
			)
			(4 ; Inventory
				(switch invItem
					(5 ; Michelle_Milken_s_Resume
...
(instance turnTableWRecord of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (self cycler:)
					(TPrint 640 35) ; "Round and round and round it goes."
				else
					(TPrint 640 36) ; "Why don't you take it for a spin?"
				)
			)
			(3 ; Do
				(if (self cycler:)
					(TPrint 640 37) ; "Don't remove the gold record while it's spinning!"
				else
					(ClearFlag 22)
...
(instance needle of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if (self cel:)
					(TPrint 640 38) ; "That stylus is really "getting in the groove.""
				else
					(TPrint 640 39) ; "The tone arm rests gently in its holder, awaiting your next action."
				)
			)
			(3 ; Do
				(cond
					...
					((IsFlag 22)
						(if
							...
							(TPrint 640 40) ; "Bring the turntable up to speed before applying the tonearm and cartridge."
						)
					)
					(else
						(TPrint 640 41) ; "Don't do that! You'll wreck the stylus."
					)
				)
			)
			(1 ; Walk
...
(instance speakerOne of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 42) ; "A small plaque on the rear of the speaker reads, "Digital Ready.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance door of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 43) ; "A small plaque on the door reads, "Studio A.""
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance reverseButton of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 640 44) ; "This button is marked "Reverse.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance stopButton of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 640 45) ; "This button is marked "Stop.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance forwardButton of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 640 46) ; "This button is marked "Forward.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance button33 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 640 47) ; "This button is marked "33.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance button78 of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 640 48) ; "This button is marked "78.""
			)
			(1 ; Walk
				(gCurRoom setScript: sReturnsFromStereo)
			)
...
(instance statue of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 49) ; "This sculpture represents the patron saint of recording companies-- St. Needle."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance speakerA of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 50) ; "This transducer converts electrical energy into rapidly pulsating sound waves."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance plant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 51) ; "Upon close examination, you determine this is a plant."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance shelves of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 52) ; "The office's shelves are filled with records, tapes, and books."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance recordJacketsA of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 53) ; "Many famous record album jackets adorn the walls of the office of des Rever Records."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance Reverse_Biaz_SReverse_Biaz of Talker
	(properties
		name {Reverse Biaz}
		nsTop 15
		nsLeft 35
...
(instance fGoldRecords of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 640 54) ; "These gold records are firmly attached to the wall."
			)
			(3 ; Do
				(TPrint 640 55) ; "You are unable to remove this gold record from its plaque."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
