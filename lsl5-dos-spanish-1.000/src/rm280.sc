		...
		(switch theVerb
			(2 ; Look
				(if 1
					...
					(StrSplitInTwo global200 global203 (blonde description:))
					(Format @temp0 280 19 global200 global203) ; "It's %s, AeroDork's blonde bombshell. Once she was a highly motivated employee. Once she was a highly trained professional. Once she never forgot to ask, "Didya wan'sum fries wit' dat?""
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @temp0)
			...
			(3 ; Do
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen (blonde description:))))
					(StrSplitInTwo global200 global203 (blonde description:))
					(Format @temp0 280 20 global200 global203) ; "%s was once a ticket agent here. Now her only responsibility is to the few remaining employees: she fires them!"
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @temp0)
			...
			(4 ; Inventory
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen (blonde description:))))
					(StrSplitInTwo global200 global203 (blonde description:))
					(Format @temp0 280 23 global200 global203) ; "Get that away from me, dork!" shouts %s. "Anything YOU have, I don't want!"
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @temp0)
		...
		(switch theVerb
			(2 ; Look
				(if 1
					...
					(Format @temp0 280 24 global200 global203) ; "It's %s. Once she was AeroDork's red-headed, red-blooded, red-hot, temptress of the flight path."
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @temp0)
			...
			(4 ; Inventory
				(if 1
					...
					(= global203 (Memory memALLOC_CRIT (StrLen (redHead description:))))
					(StrSplitInTwo global200 global203 (redHead description:))
					(Format @temp0 280 29 global200 global203) ; "Get that away from me, dork!" shouts %s. "Anything YOU have, I don't want!"
					(Memory memFREE global200)
					(Memory memFREE global203)
				)
				(TPrint @temp0)
	...
	(method (changeState newState &tmp [temp0 2] [temp2 150])
		(switch (= state newState)
			...
			(30
				(Points 3 1)
				(SetFlag 38) ; fLimoParked
				(TPrint 280 70 #at -1 28) ; "Good day. Tramp Limousine at your service."
				(= ticks 30)
			)
			(31
				(TPrint 280 71) ; "Good day, Mr. Tramp," you presume, mistakenly. "I'm at the airport, and require transportation to the city."
				(= ticks 30)
			)
			(32
				(TPrint 280 72 #at -1 28) ; "Yes sir!" he replies, "I'll send one of my best girls right over!"
				(= ticks 30)
			)
			(33
				(TPrint 280 63 #at -1 185) ; "(And he hangs up the phone.)"
			...
			(40
				(Points 7 12)
				(TPrint 280 73) ; "`JesGreeenCars.' Si habla Espanol y Eeengleesh. Haf ju `lost jour carrrd?'"
				(= ticks 30)
			)
			(41
				(TPrint 280 74) ; "Hello, I'm looking for a limousine to take me from the airport into town, I guess."
				(= ticks 30)
			)
			(42
				(TPrint 280 75) ; "Ho-kay. Aeropuerto? Ho-kay. Wee deeleefer jur carrrd! On dee trash can. Outside de aeropuerto. Ju leeef a-ten-dousand pesos outside de aeropuerto. On dee trash can. No problemo!"
				(= ticks 30)
			)
			(43
				(TPrint 280 63 #at -1 185) ; "(And he hangs up the phone.)"
			...
			(50
				(TPrint 280 76) ; "Please deposit seventy-five cents for the next three minutes."
				(= ticks 30)
			)
			(51
