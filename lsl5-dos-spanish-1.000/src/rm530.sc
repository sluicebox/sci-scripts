	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(switch register
					(0
						(Say Michelle_Milken 530 1 #dispose #caller self) ; ""Nearly as many times as I'm here," she responds."
					)
					(1
						(Say Michelle_Milken 530 2 #dispose #caller self) ; "That may be, but I look even better out of it!"
					)
					(2
						(Say Michelle_Milken 530 3 #dispose #caller self) ; "Well, you may sit down, but it may be for dessert!"
					)
				)
			)
			(1
				(switch register
					(0
						(TPrint 530 4 #at -1 185) ; "(Now what did THAT mean?)"
						(self dispose:)
					)
					(1
						(TPrint 530 5 #at -1 185) ; "(Say, this may turn out all right yet!)"
						(self dispose:)
					)
					(2
						(TPrint 530 6 #at -1 185) ; "(A better offer you've never had, Larry! Go for it!!)"
...
(instance sOpenCurtain of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(Say Michelle_Milken 530 7 #dispose) ; ""Allow me to make our booth just a little more private," says Michelle, standing up."
...
(instance drape2 of Prop
	...
	(method (cue)
		...
		(switch (++ cueCounter)
			(1
				(TPrint 530 13 #dispose self) ; "DEAR!" interrupts his wife, "that's not necessary! This is a family restaurant!"
			)
			(2
				(self setCycle: End self)
				(table2 dispose:)
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				(TPrint 530 14) ; "Every booth has curtains to assure your total privacy while dining."
			)
			(5 ; Talk
				(if talkedToTable2
					(drape1 doVerb: 5)
...
(instance drape3 of Prop
	...
	(method (cue)
		(switch (++ cueCounter2)
			(1
				(super cue:)
				(TPrint 530 15) ; ""Why doesn't that come as a surprise?" the woman replies snootily."
				(self setCycle: End self)
				(table3 dispose:)
				(extra3 dispose:)
				(= talkedToTable3 1)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 16) ; "You can't stand up; you have nowhere else to go."
			)
			(1 ; Walk
				(TPrint 530 17) ; "Why not talk to that beautiful woman instead. Go on, Larry; strike up a conversation."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance table1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 19) ; "You can't sit with a strange couple!"
			)
			(5 ; Talk
				(if (not talkedToTable1)
					(HandsOff)
					(Say gEgo 530 20 #at -1 185) ; ""Excuse me, may I join you?" you ask the couple at the left table."
					(TPrint 530 21 #dispose drape1) ; "No. I'm sorry, but we're eating." replies the wit at the table. "And I don't want to lose my appetite."
				else
		...
		(switch theVerb
			...
			(5 ; Talk
				(if (not talkedToTable2)
					(HandsOff)
					(Say gEgo 530 22 #at -1 185) ; "Excuse me," you shout to that far table, "but can you explain why I don't talk to that lovely woman sitting all alone at that table in the corner?"
					(TPrint 530 23 #dispose drape2) ; "Could your head be up your..."
				else
					(super doVerb: theVerb invItem)
				)
			)
		...
		(switch theVerb
			...
			(5 ; Talk
				(if (not talkedToTable3)
					(HandsOff)
					(Say gEgo 530 24 #at -1 185 #dispose drape3) ; ""Hi, folks; I'm from out of town!" you yell to the diners at the table across the room."
				else
					(super doVerb: theVerb invItem)
				)
			)
...
(instance theTable of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 25) ; "You know what you want to do, Larry. Your problem is figuring out how to do it!"
			)
			(4 ; Inventory
				(TPrint 530 26) ; "Perhaps you should sit down with her first."
			)
			(10 ; Zipper
				(TPrint 530 27) ; "A little premature, aren't you, Larry? Maybe first a little dinner, then a show, perhaps a little dancing...."
			)
			(5 ; Talk
				(switch (++ michelleCount)
					(1
						(Say gEgo 530 28 #at -1 185) ; "Hello, Lovely Lonely Lady," you say to the gorgeous babe sitting all alone in the booth near you. "Allow me to introduce myself. My name is Larry; Larry Laffer."
						(TPrint 530 29) ; "You hope she doesn't just ignore you."
						(Say Michelle_Milken 530 30 #dispose) ; "Hi, Larry," she says with a friendly smile, "new in town?"
					)
					(2
						(Say gEgo 530 31 #at -1 185) ; "Remembering your boss' warning not to reveal your identity, you decide to begin with a casual opener, "Do you come here often?" you ask."
						(gCurRoom setScript: sTalk 0 0)
					)
					(3
						(Say gEgo 530 32 #at -1 185) ; "Perhaps I could mention her "power clothes," you think to yourself. A compliment on her apparel might be appropriate..."
						(Say gEgo 530 33 #at -1 185) ; "What a lovely dress you're almost wearing," you tell her. "And you certainly look good in it!"
						(gCurRoom setScript: sTalk 0 1)
					)
					(4
						(Say gEgo 530 34 #at -1 185) ; ""I notice you're dining alone," you try again."
						(Say Michelle_Milken 530 35 #dispose) ; ""Yes, and dining isn't the only thing I don't enjoy doing alone!" she responds with a smile."
					)
					(else
						(Say gEgo 530 36 #at -1 185) ; ""I was wondering: may I join you for dinner?" you ask."
...
(instance cart of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(TPrint 530 37) ; "You can't move the cart. Then you'd have no table!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
