...
(instance Alberta_SAlberta of Talker
	(properties
		name {Alberta}
		nsTop 33
		nsLeft 22
				...
				(switch whoToTalkTo
					...
					(3627
						(if (not (SetFlag 18))
							(SetFlag 18)
							(if (IsFlag 4)
								(Say Alberta_SAlberta 905 29 108) ; "Good afternoon. You've reached Dr. Pulliam's exchange. I'm sorry, but everyone in their office has gone home for the evening. But please feel free to call back tomorrow. This has been a recording."
							else
		...
		(switch (= state newState)
			...
			(5
				(switch (++ windowTalkCount)
					(1
						(Say Alberta_SAlberta 905 43 108 139 self) ; "Oh," she growls, "and just what do we think we're doing here, eh, little man?"
					)
					(else
						(Say Alberta_SAlberta 905 44 108 139 self) ; "Yeah, what is it?"
					)
				)
			...
			(7
				(switch windowTalkCount
					(1
						(Say Alberta_SAlberta 905 47 108 139 self) ; ""No!" she says, closing the window."
					)
					(else
						(= state 9)
						(= talkCounter 0)
						(Say Alberta_SAlberta 905 48 108 139 self) ; "Yeah, maybe," she replies, "but first, let's do your paperwork."
					)
				)
			)
			(8
			...
			(11
				(Say Alberta_SAlberta 905 51 108 139 self) ; "Okay, sweetie, let's fill out this lil' ol' admission form together, now shall we?"
			)
			(12
				(girl setCel: 0)
				...
				(switch talkCounter
					...
					(14
						...
						(= state 17)
						(= talkedToGirl 1)
						(Say Alberta_SAlberta 905 66 108) ; "Since this is not an emergency, I'll go ahead and set you up with an appointment for, uh, let's see... how about nine months from today? Ten o'clock is good for me!"
					)
				)
				(cond
					(register
			...
			(13
				(Say Alberta_SAlberta 905 67 108 139 self) ; "Oops! Well, gosh, I'm sorry, but I just realized we'll be unable to accept you as a client. I just remembered... we have no more openings this century!"
			)
			(14
			...
			(15
				(gTheMusic2 stop:)
				(TPrint 905 68) ; ""Wait!" you cry, but it's too late."
				(= start (= register 0))
				(HandsOn)
				(NormalEgo 3)
				(self dispose:)
			...
			(16
				...
				(cond
					((== (gEgo view:) 908)
						...
						(Points 17 162)
						(SetFlag 33)
						(Say Alberta_SAlberta 905 70 108 139 self) ; "Oh, you poor dear! You look like you're in terrible pain! Why, just go right on in. I'll have one of our technicians attend to you right away!"
					)
					((IsFlag 18)
						(Say gEgo 905 71) ; "Excuse me, ma'am," you say, hoping she doesn't recognize your voice. "My name is Larry; Larry Laffer."
						(Points 13 163)
						(Say Alberta_SAlberta 905 72 108 139 self) ; "Well, Larry, that was a quick trip! Please, just walk through the door to the right! I'll have one of our technicians attend to you right away!"
					)
				)
			)
			(17
...
(instance toothTable2 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 905 74) ; "You can tell that Doc's wife decorated this place. She's suffering from terminal cuteness!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sGetDoily of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(TPrint 905 75 108 self) ; "You take the lace doily from the waiting room table, but you really don't know why."
			)
			(5
				(HandsOn)
				(self dispose:)
