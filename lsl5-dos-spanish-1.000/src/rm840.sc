...
(instance johnDoor of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 17) ; "A sign on the door says "Krapper's Crapper." A smaller sign below it says "John's John"."
			)
			(3 ; Do
				(cond
					((gEgo has: 10) ; Some_Change
						(TPrint 840 18) ; "Mr. Krapper will surely miss his file folder, Patti! You'd better return it."
					)
					((gEgo has: 9) ; AeroDork_s_In-Flight_Magazine
						(TPrint 840 19) ; "Mr. Krapper will surely miss his desk key, Patti! You'd better return it."
					)
					((gEgo has: 8) ; Boarding_Pass
						(TPrint 840 20) ; "Mr. Krapper will surely miss his letter opener, Patti! You'd better return it."
					)
					(else
...
(instance lobbyDoor of Door
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 21) ; "This is the door through which you entered. If you walked out it, you would return to the lobby."
			)
			(3 ; Do
...
(instance table of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 22) ; "Krapper has a beautiful glass coffee table."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 23) ; "This is the kind of furniture you would buy if you ever settled down."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance sofa of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 24) ; "What an unusual sofa! You so love modern furniture."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance telephone of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 25) ; "John Krapper's telephone has very few buttons. He must be REALLY important!"
			)
			(3 ; Do
				(TPrint 840 26) ; "You'd better not use this telephone, Patti. You might get caught by the switchboard operator."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance computer of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 27) ; "Look at the dust on that keyboard! He must never use his computer!"
			)
			(3 ; Do
				(TPrint 840 28) ; "You briefly consider booting up John Krapper's computer in an attempt to contact Inspector Desmond, but realize there's no way his computer would be connected to FBI-Net!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance myWindow of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 29) ; "Krapper's office has a giant window looking out over beautiful downtown Philadelphia."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance stuff of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 840 30) ; "Krapper's bookcase is filled with items, all of them useless to your mission."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
