...
(instance sWakeUpAthens of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(TPrint 320 19 #at -1 17 #width 280) ; "You awaken with a start to hear the stewardess say..."
...
(instance sWakeUpVenice of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(TPrint 320 23 #at -1 17 #width 280 #dispose self) ; "Awakening from your dream, you realize you just filled your lap with complimentary beverage!"
			)
			(12
				(= ticks 30)
			)
...
(instance sDreamTaj of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(TPrint 320 24 #dispose self #at -1 17 #width 280) ; "In your dream, you are watching Passionate Patti perform a rock concert before the reflecting pool of the Taj Mahal! What a great musician! What a romantic setting! What a gorgeous babe!!"
			)
			(2
				(= ticks 90)
			)
...
(instance sWakeUpTaj of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(5
				(TPrint 320 26 #at -1 17 #width 280 #dispose) ; "You soon realize the snake is not the only thing Patti has charmed!"
			...
			(10
				(TPrint 320 21 #at -1 185 #width 280) ; "A voice crackles through the intercom speaker above your head, "Please tighten your seatbelts and return your tray tables to their fully upright and locked position. We'll be landing momentarily.""
				(TPrint 320 27 #at -1 185 #width 280) ; "You hope none of the other passengers notices that your tray table is already up!"
				(gTheMusic fade: 0 15 10 1)
				(gTheMusic2 fade: 0 15 10 1)
				(= seconds 3)
			)
...
(instance sDreamVenice of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(TPrint 320 28 #at -1 185 #width 280) ; "Ah, this is the life! In your dream, you are the handsome gondolier, poling through the canals of Venice with your lone, lovely female customer: Passionate Patti! Everything is going well until..."
				(larry
					cycleSpeed: 8
					moveSpeed: 12
					setCycle: Fwd
			...
			(5
				(TPrint 320 29 #at -1 28) ; "...you stick it in too deeply."
				(TPrint 320 30 #at -1 185) ; "(An experience wholly unfamiliar to you!)"
			...
			(9
				(TPrint 320 31 #at -1 17 #width 280) ; "Deciding to seize the moment, you wriggle down beside your Patti, relying on the subtle canal currents to propel you through your night of love..."
				(= ticks 30)
			)
			(10
				(= dreaming 0)
...
(instance sDreamCasa of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(Say Leisure_Suit_Bogie 320 32 #dispose #caller self) ; "It's not hard to see, Patti, the problems of two little people don't amount to a hill of beans in this crazy mixed-up world..."
			)
			(2
				(= seconds 3)
			)
			(3
				(Say Ingrid_Patti 320 33 #dispose #caller self) ; "Larry, in all the scenes, in all the games, in all the world... you had to walk into mine!"
			)
			(4
				(= seconds 3)
			)
			(5
				(Say Ingrid_Patti 320 34 #dispose #caller self) ; "Remember Larry: we'll always have Nontoonyt!"
			)
			(6
				(= seconds 3)
			)
			(7
				(Say Leisure_Suit_Bogie 320 35 #dispose #caller self) ; "Here's looking at you, kid!"
			)
			(8
				(gTheMusic fade: 0 15 10 1)
				(= dreaming 0)
...
(instance chair of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 320 36) ; "You've hated airplane travel ever since "Leisure Suit Larry 2: Looking for Love (in Several Wrong Places).""
				(TPrint 320 37 #at -1 185) ; "(Another fine Al Lowe game.)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance floor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 320 36) ; "You've hated airplane travel ever since "Leisure Suit Larry 2: Looking for Love (in Several Wrong Places).""
				(TPrint 320 38 #at -1 185) ; "(Another fine Al Lowe game.)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
