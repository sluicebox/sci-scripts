...
(instance sWakeUpAthens of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(TPrint 320 19 67 -1 17 70 280) ; "You awaken with a start to hear the stewardess say..."
...
(instance sWakeUpVenice of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(TPrint 320 23 67 -1 17 70 280 108 self) ; "Awakening from your dream, you realize you just filled your lap with complimentary beverage!"
			)
			(12
				(= ticks 30)
			)
...
(instance sDreamTaj of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(TPrint 320 24 108 self 67 -1 17 70 280) ; "In your dream, you are watching Passionate Patti perform a rock concert before the reflecting pool of the Taj Mahal! What a great musician! What a romantic setting! What a gorgeous babe!!"
			)
			(2
				(= ticks 90)
			)
...
(instance sWakeUpTaj of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(5
				(TPrint 320 26 67 -1 17 70 280 108) ; "You soon realize the snake is not the only thing Patti has charmed!"
...
(instance sDreamVenice of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(9
				(TPrint 320 31 67 -1 17 70 280) ; "Deciding to seize the moment, you wriggle down beside your Patti, relying on the subtle canal currents to propel you through your night of love..."
				(= ticks 30)
			)
			(10
				(= dreaming 0)
...
(instance sDreamCasa of Script
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				(Say Leisure_Suit_Bogie_S_Larry_Bogart 320 32 108 139 self) ; "It's not hard to see, Patti, the problems of two little people don't amount to a hill of beans in this crazy mixed-up world..."
			)
			(2
				(= seconds 3)
			)
			(3
				(Say Ingrid_Patti_SIngrid_Patti 320 33 108 139 self) ; "Larry, in all the scenes, in all the games, in all the world... you had to walk into mine!"
			)
			(4
				(= seconds 3)
			)
			(5
				(Say Ingrid_Patti_SIngrid_Patti 320 34 108 139 self) ; "Remember Larry: we'll always have Nontoonyt!"
			)
			(6
				(= seconds 3)
			)
			(7
				(Say Leisure_Suit_Bogie_S_Larry_Bogart 320 35 108 139 self) ; "Here's looking at you, kid!"
			)
			(8
				(gTheMusic fade: 0 15 10 1)
				(= dreaming 0)
...
(instance Leisure_Suit_Bogie_S_Larry_Bogart of Talker
	(properties
...
(instance Ingrid_Patti_SIngrid_Patti of Talker
	(properties
		name {Ingrid Patti}
		x 140
		y 180
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				...
				(TPrint 320 37 67 -1 185) ; "(Another fine Al Lowe game.)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance floor of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 320 36) ; "You've hated airplane travel ever since "Leisure Suit Larry 2: Looking for Love (in Several Wrong Places).""
				(TPrint 320 38 67 -1 185) ; "(Another fine Al Lowe game.)"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
