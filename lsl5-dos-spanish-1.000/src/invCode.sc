...
(instance Camcorder of InvI
	...
	(method (doVerb theVerb invItem &tmp [temp0 60] temp60 temp61)
		...
		(switch theVerb
			(2 ; Look
				(TPrint 19 13 #width 250 #title name) ; "Auto-focus, auto-level, auto-color balance, auto-zoom, auto-pan, .5-lux, Complete-Video- Studio-in-a-Pocket-Protector. 'The Rob Lowe Model'"
				(Format ; "Power: %s Charge: %d min %d sec"
					...
					(if (>= state 100) {Encendido} else {Apagado})
					(/ (gChargeTimer seconds:) 60)
					(mod (gChargeTimer seconds:) 60)
				)
...
(instance Battery_Charger of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 24 #title name) ; "Insert into any 110-volt, 20-amp GFI-protected circuit. Recharges most batteries in one minute!"
			)
			(3 ; Do
		...
		(switch theVerb
			(2 ; Look
				(switch state
					...
					(1
						(TPrint 19 28 #title name) ; "The videotape is already degaussed."
					)
					(2
...
(instance Michelle_Milken_s_Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 35 #font gMonoFont #icon 937 2 1 #title name) ; "NAME    Michelle Milken AGE     28 HAIR    Black EYES    Brown HEIGHT   6' 1" WEIGHT   131"
				(TPrint 19 36 #font gMonoFont #width 222 #title name) ; "OCCUPATION junk bond sales HANGOUTS  Wall Street,       computer stores TURNONS   anything covered       with whipped cream TURNOFFS  strong moral fiber"
...
(instance Hard_Disk_Cafe_Napkin of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 39 #title name #width 222 #mode 1) ; "If computers are your life, grab a quick byte in our cafe! In the Heart of the Fabulous Times Square Wine Country 920 W. 42nd Street New York City"
			)
			(3 ; Do
...
(instance AeroDork_Gold_Card of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 41 #title name) ; "Whether in the U.S.A. or the South Pacific, it's Nontoonyt for you on AeroDork!"
			)
			(3 ; Do
...
(instance AeroDork_s_In_Flight_Magazine of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 45 #title name) ; "Good idea; save this in case the restroom runs out of paper!"
			)
			(3 ; Do
...
(instance Some_Change of InvI
	...
	(method (doVerb theVerb invItem &tmp [temp0 55])
		(switch theVerb
			(2 ; Look
				(Format @temp0 19 48 (* gQuarters 2) (* gQuarters 2)) ; "You always were a %d-bit game player!"
				(TPrint @temp0 #title name)
			)
			(3 ; Do
...
(instance DayTrotter_ of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if state
					(TPrint 19 50 #title name) ; "You always wanted to have one of these, but you never had anything to schedule!"
				else
					(Points 11 74)
					(TPrint 19 51 #title name) ; "Inside you find: some folding money with more than two digits in the corners, many credit cards, some mutual fund account access codes, and a Swiss bank account number."
					(gTheIconBar curInvIcon: 0)
					(Inv curIcon: 0 hide:)
					(= state 1)
			...
			(3 ; Do
				(if state
					(TPrint 19 52 #title name) ; "You thumb through the pages looking for anything that might help you, but find nothing except some alphabetical page dividers, which you rip out of the binder in disgust and throw away!"
				else
					(self doVerb: 2)
				)
			)
...
(instance Money of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 53 #title name) ; "This is even better than having your own savings and loan (as you think you may have had in "LSL4: The Missing Floppies!")"
			)
			(3 ; Do
...
(instance Credit_Cards of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 55 #title name) ; "Whoever this Pat Patterson guy was, he was sure loaded!"
			)
			(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(switch state
					(1
						(TPrint 19 57 #title name) ; "What an interesting way to make a club membership card! No one uses this antique, 9-track, paper tape any more!"
					)
					(2
						(TPrint 19 58 #title name) ; "Whoever said you're not a man of credit?"
					)
					(3
						(TPrint 19 59 #title name) ; "How clever you are! A few extra holes and you're a new man!"
					)
				)
			)
			(3 ; Do
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(switch state
					(0
						(TPrint 19 61 #title name) ; "You are amazed the Japanese didn't come up with this product first!"
					)
					(1
						(TPrint 19 62 #title name #mode 1) ; "Reverse Biaz des Rever Records Suite 900 Shill Building 1000 Upson Downs Lane Baltimore, MD"
						(SetFlag 17) ; fSeenRBOffice
					)
					(2
						(Points 1 78)
						(TPrint 19 63 #title name #mode 1) ; "P. C. Hammer K-RAP Radio 1709 Hillview Vista Philadelphia, PA Access code: 45954 often in Control Room B Access code: n/a"
					)
				)
			)
			(3 ; Do
...
(instance Reverse_Biaz_DataPak of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 70 #title name #mode 1) ; "Reverse Biaz Insert one end first"
			)
			(3 ; Do
...
(instance P__C__Hammer_DataPak of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 72 #title name #mode 1) ; "P. C. Hammer Insert one end first"
			)
			(3 ; Do
				(TPrint 19 71) ; "You check the DataPak carefully for any signs of damage. You find none."
			)
...
(instance Lana_Luscious__Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 73 #font gMonoFont #icon 937 2 2 #title name) ; "NAME    Lana Luscious AGE     21 HAIR    Blond EYES    Blue HEIGHT   5' 10" WEIGHT   123"
				(TPrint 19 74 #font gMonoFont #width 277 #title name) ; "OCCUPATION professional mud wrestler,       former "Miss Wet T-Shirt       of Atlantic City" HANGOUTS  Tramp's Boardwalk TURNONS   polyurethane, mud TURNOFFS  high IQ's"
...
(instance Tramp_Casino_Matchbook of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 77 #title name #mode 1) ; "Visit beautiful Tramp Casino on the beautiful Tramp Boardwalk outside the luxurious Tramp Hotel near the palatial Tramp Overscale Miniature Golf Course right off filthy Tramp Beach! 777 High Roller Drive Atlantic City"
			)
			(3 ; Do
				...
				(TPrint 19 79 #at -1 185) ; "(Thus, you amuse yourself greatly!)"
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Roller_skates of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 83 #title name) ; "With these on your feet you feel as though you could fly... right off the boardwalk."
			)
			(3 ; Do
				(if (== gCurRoomNum 760)
					(Inv hide:)
...
(instance Champagne of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 86 #title name) ; "You do so love a nice wet Brute!"
			)
			(3 ; Do
...
(instance Gold_Record of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if state
					(TPrint 19 88 #title name) ; "Excellent detective work, Patti!"
				else
					(TPrint 19 89 #title name) ; "For years you've wondered what happened to "The Four Shabangs!""
				)
			)
			(3 ; Do
...
(instance Cassette_Tape of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 91 #title name) ; "A smoking gun of the subtle degradation of our nation's youth by those filth-mongering blasphemers, wouldn't you say, Patti?"
			)
			(3 ; Do
...
(instance Chi_Chi_Lambada_s_Resume of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 93 #font gMonoFont #icon 937 2 0 #title name) ; "NAME    Chi Chi Lambada AGE     24 HAIR    Raven EYES    Chestnut HEIGHT   5' 8" WEIGHT   101"
				(TPrint 19 94 #font gMonoFont #width 240 #title name) ; "OCCUPATION dental hygienist and       former gymnast HANGOUTS  the gym; the office TURNONS   locker room odor,       dental irrigators TURNOFFS  burning rubber"
...
(instance Doc_Pulliam_s_Card of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 1 77)
				(TPrint 19 97 #title name #width 222 #mode 1) ; "Can Doc Fixem? Nope, but can 'Doc Pulliam!!'" Doc Pulliam's Dental Hygiene Heaven, 169 Lower Wacker Drive Miami, FL Dial 554-3627 Our Motto: "Beats a doorknob and string!"
			)
			(3 ; Do
...
(instance Green_Card of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 100 #title name) ; "Official U. S. Government Alien Resident Permit"
			)
			(3 ; Do
...
(instance Doily of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 102 #title name) ; "Isn't this sweet! It would look great back home in your apartment."
				(TPrint 19 103 #at -1 185 #title name) ; "(If you HAD an apartment!)"
				(TPrint 19 104 #at -1 185 #title name) ; "(If you had a HOME!)"
			)
			(3 ; Do
				(cond
					((!= gCurRoomNum 905)
...
(instance P__C__Hammer_Fax of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 109 #title name #width 200 #mode 1) ; "P. C. Hammer K-RAP Radio 1709 Hillview Vista Philadelphia, PA Access code: 45954 often in Control Room B Access code: n/a"
			)
			(3 ; Do
				(TPrint 19 85) ; "This fax paper sure is flimsy!"
			)
...
(instance Letter_Opener of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 110 #title name) ; "This is merely a razor-sharp letter opener; don't try anything funny with it like picking locks!"
			)
			(3 ; Do
				(TPrint 19 111) ; "You nearly cut your hand on the needle-like letter opener."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Desk_Key of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 112 #title name) ; "Did you know that one key will fit the lock of every piece of pressed-cardboard furniture in the world!?"
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Folder_Of_Evidence of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 4 146)
				(TPrint 19 113 #title name) ; "Examining the papers inside the folder carefully, you find one with some references to gangster activities. It is signed only with a scrawled name you make out to be "Julius.""
				(TPrint 19 114 #title name) ; "These papers provide proof positive of Mob payola at K-RAP Radio! But how will you get them out of here without Krapper finding out?"
			)
			(3 ; Do
				(if (gEgo has: 11) ; DayTrotter
					(TPrint 19 115) ; "There is nothing more to find in this folder. Besides, you already have photocopies of everything inside."
				else
					(TPrint 19 116) ; "Checking through these papers makes you realize you need some way to photocopy them."
				)
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
...
(instance Photocopied_Evidence of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 19 117 #title name) ; "These copies provide proof positive of Mob payola at K-RAP Radio!"
			)
			(3 ; Do
				(TPrint 19 118) ; "Good job sneaking these photocopies out of K-RAP, Patti!"
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Reel_To_Reel_Tape of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(if state
					(TPrint 19 119 #title name) ; ""D D D" This tape is filled with digitally-recorded incriminating evidence!"
				else
					(TPrint 19 120 #title name) ; "If you could look closely enough, you could see billions of magnetic particles pointing in random directions."
					(TPrint 19 121 #at -1 185) ; "(In other words, this tape is still blank!)"
				)
			)
			(3 ; Do
				(TPrint 19 122) ; "You really need a tape deck to use this, Patti."
			)
			(4 ; Inventory
				(localproc_0 (Inv indexOf: self) invItem)
			)
...
(instance Hooter_Shooter of InvI
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Points 2 76)
				(TPrint 19 123 #title name) ; "Commander Twit's boys may be into explosives, but they know nothing about comfort or adequate support."
			)
			(3 ; Do
				(self doVerb: 2)
			)
