		...
		(return
			(switch theVerb
				...
				(3 ; Do
					(SetFlag 17) ; fSeenRBOffice
					(Points 3 120)
					(TPrint 620 40 #at -1 28) ; "That's it, Patti! "des Rever Records" is located in Room 900."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessFive of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 41 #at -1 28) ; "Ship Heads, Inc. provides the finest commodes available for ocean-going vessels."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSix of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 42 #at -1 28) ; "AeroDork is a very high-class airline. So high-class their offices are forbidden to the public."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSeven of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 43 #at -1 28) ; "Big Buttons, Inc. supplies buttons of all sizes for every occasion: political campaigns, birthday parties, fund raisers, software company identification, etc."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessEight of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 44 #at -1 28) ; "Hard Disk Services can handle all of your computer hardware needs--from correcting corrupted database files to undeleting sensitive information, to recovering formatted hard drives."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
