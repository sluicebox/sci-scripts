...
(instance sGuardWakes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= guardStatus 1)
				(Say The_Guard_SEl_Guarda 620 2 108) ; "Yes?" mumbles the sleepy guard, "You need somethin'?"
				(HandsOn)
				(Delay 6 2 self)
			)
			(2
...
(instance sGuardSleeps of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say The_Guard_SEl_Guarda 620 5 108) ; "Honey, if you don't know where you're going, how am I supposed to know?"
				(guard setLoop: 0 setCel: 0 setCycle: End self)
			)
			(2
				(guard setScript: sSleeping)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(cond
					((IsFlag 45)
						(Say The_Guard_SEl_Guarda 620 11 108) ; "Lady, I already told you: Reverse Biaz says, `you blew the gig when you split'--whatever that means!"
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					)
					(datamanSolution
						(Say The_Guard_SEl_Guarda 620 12 108) ; "Hey, pretty cool! I was thinkin' about gettin' one of these for the little missus! Now, let's see... `Biaz?' `Suite 900?' Sure, no problem. I'll summon the elevator for you right now!"
						(= datamanSolution 0)
						(Points 6 118)
					)
					(wrongDataman
						(Say The_Guard_SEl_Guarda 620 13 108) ; "Hey, pretty cool! I was thinkin' about gettin' one of these for the little missus! Now, let's see... `P. C. Hammer?' I'm sorry, but there's nobody by that name in this building."
						(= wrongDataman 0)
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					...
					(faxSolution
						(Say The_Guard_SEl_Guarda 620 14 108) ; "Kinda flimsy paper, ain't it? Well, let's see here... You know, it's hard to read this stuff; the text is all fuzzy-looking. Oh, here it is... `Biaz?' Why, he's in Suite 900. Sure, no problem. I'll summon the elevator for you right now!"
						(= faxSolution 0)
						(Points 6 119)
					)
					(wrongFax
						(Say The_Guard_SEl_Guarda 620 15 108) ; "Kinda flimsy paper, ain't it? Well, let's see here... You know, it's hard to read this stuff; the text is all fuzzy-looking. Hmm... `P. C. Hammer?' Nah, they only let us use screwdrivers and pliers on the computers here!"
						(= wrongFax 0)
						(HandsOn)
						(guard setScript: sBackToSleep)
						(self dispose:)
					...
					(else
						(Say The_Guard_SEl_Guarda 620 16 108) ; ""Huh? Oh, right. Give me just a second while I make sure he's in," sighs the overworked guard."
						(SetFlag 48)
					)
				)
				(self cue:)
			...
			(8
				(if (IsFlag 21)
					(Say The_Guard_SEl_Guarda 620 17 108) ; "I'm sorry, but he says he's found someone else. Take a hike!"
					(SetFlag 45)
					(HandsOn)
					(guard setScript: sBackToSleep)
					(self dispose:)
				else
					(Say The_Guard_SEl_Guarda 620 18 108) ; "Okay, honey. He's waiting for you upstairs. I'm calling for the elevator for you right now."
					(guard
						cycleSpeed: 18
						setLoop: 0
						setCel: 0
...
(instance guard of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
					...
					(switch invItem
						(0 ; Camcorder
							(cond
								...
								(else
									(Say The_Guard_SEl_Guarda 620 27 108) ; "I'm sorry, lady, but that don't mean nothing to me."
									(guard setScript: sBackToSleep)
								)
							)
						)
			...
			(3 ; Do
				(if (== guardStatus 1)
					(Say The_Guard_SEl_Guarda 620 31 108) ; "Oh, Ma'am," sighs the guard, "that feels good!"
				else
		...
		(return
			(switch theVerb
				...
				(3 ; Do
					(SetFlag 17)
					(Points 3 120)
					(TPrint 620 40 67 -1 28) ; "That's it, Patti! "des Rever Records" is located in Room 900."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessFive of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 41 67 -1 28) ; "Ship Heads, Inc. provides the finest commodes available for ocean-going vessels."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSix of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 42 67 -1 28) ; "AeroDork is a very high-class airline. So high-class their offices are forbidden to the public."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessSeven of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 43 67 -1 28) ; "Big Buttons, Inc. supplies buttons of all sizes for every occasion: political campaigns, birthday parties, fund raisers, software company identification, etc."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance businessEight of Feature
	...
	(method (doVerb theVerb invItem)
		(return
			(switch theVerb
				(2 ; Look
					(TPrint 620 44 67 -1 28) ; "Hard Disk Services can handle all of your computer hardware needs--from correcting corrupted database files to undeleting sensitive information, to recovering formatted hard drives."
				)
				(1 ; Walk
					(gCurRoom setScript: sReturnsFromBoard)
					1
...
(instance plant of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 45) ; "This plant is obviously well-maintained, but superfluous to your mission."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance mural of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 46) ; "It is quite an interesting mural."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fStairs of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 47) ; "A staircase winds its way upwards, but a velvet rope across the bottom indicates it's not to be used under penalty of law. It looks like the elevator is your only hope, Patti."
			)
			(3 ; Do
				(TPrint 620 48) ; "Since the staircase is presently being renovated, you are unable to use it. Besides, you know how much you hate climbing stairs in heels!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fLamp1 of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 49) ; "A lovely Art Deco lamp rises above the staircase."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance fAshtray of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 50) ; "Carefully examining the ashtray for a disco pass, for a brief moment you think you're playing "Leisure Suit Larry 1: In the Land of the Lounge Lizards." Of course, you're not."
			)
			(3 ; Do
				(TPrint 620 50) ; "Carefully examining the ashtray for a disco pass, for a brief moment you think you're playing "Leisure Suit Larry 1: In the Land of the Lounge Lizards." Of course, you're not."
			)
...
(instance sconceA of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 51) ; "My, the Shill Building has tasteful sconces."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance clock of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 52) ; "A lovely Deco time clock is built into the end of the Security Guard's counter for those employees still paid by the hour."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance breasts of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(TPrint 620 53) ; "What? Patti! When did you develop an interest in other women?"
			)
			(3 ; Do
				(TPrint 620 54) ; "You would, but you have no ladder!"
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance The_Guard_SEl_Guarda of Talker
	(properties
		name {The Guard}
		nsTop 15
		nsLeft 35
