...
(procedure (GetDirectory where &tmp result [newDir 33] [str 100] [saveParseLang 50])
	(repeat
		(if
			(not
				(= result
					(Print
						font: 0
						addText: 1 0 0 1 0 0 990 ; MISSING MESSAGE
						addEdit: (StrCpy @newDir where) 29 0 20 where
						addButton: 1 27 0 0 1 0 34 990 ; MISSING MESSAGE
						addButton: 0 38 0 0 1 50 34 990 ; MISSING MESSAGE
						init:
					)
				)
			)
		...
		(if (ValidPath @newDir)
			...
			(Message msgGET 990 29 0 0 1 @saveParseLang)
...
(procedure (NeedDescription)
	(Print font: 0 addText: 3 0 0 1 0 0 990 init:) ; MISSING MESSAGE
)
				...
				(cond
					((== local1 deleteI)
						...
						(if
							(not
								(Print
									addText: 12 0 0 1 0 0 990 ; MISSING MESSAGE
									addButton: 0 31 0 0 1 0 35 990 ; MISSING MESSAGE
									addButton: 1 32 0 0 1 50 35 990 ; MISSING MESSAGE
									init:
								)
							)
							(self init: theComment @temp3 @temp364)
