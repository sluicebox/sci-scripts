...
(procedure (localproc_3 &tmp temp0)
	(if (== (= temp0 (GetPort)) gModelessPort)
		(SetPort (timeClock port:))
	)
	(if (> (= local29 (/ local16 60)) 12)
		(-= local29 12)
		(-= local16 720)
	)
	(= local32 (/ local29 10))
	(= local33 (mod local29 10))
	(= local30 (/ (= local17 (mod local16 60)) 10))
	(= local31 (mod local17 10))
	(Graph grFILL_BOX 38 30 52 90 1 [local0 0] -1 -1)
	(Graph grUPDATE_BOX 38 30 52 90 1)
	(if (> local29 9)
		(Format @local34 {%1d} local32)
		(proc5_8 @local34 [local0 1] 35 40 -1 201)
	)
	(Format @local34 {%1d} local33)
	(proc5_8 @local34 [local0 1] 43 40 -1 201)
	(proc5_8 {:} [local0 1] 56 40 -1 201)
	(Format @local34 {%1d} local30)
	(proc5_8 @local34 [local0 1] 65 40 -1 201)
	(Format @local34 {%1d} local31)
	(proc5_8 @local34 [local0 1] 77 40 -1 201)
	(SetPort temp0)
)
...
(instance clockCardIcon of CodeIcon
	...
	(method (highlight param1 &tmp temp0 temp1 [temp2 50])
		(if param1
			(DrawCel view loop 3 nsLeft nsTop 15)
			(if (not local47)
				(cond
				...
				(Format @temp2 190 7 temp1 temp0) ; "The %s card has previously been used at: %s"
				(Print @temp2 #at 166 232 #time 3 #dispose)
			)
		else
			(if (IsObject gModelessDialog)
				(gModelessDialog dispose:)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				...
				([local42 0] signal: (& ([local42 0] signal:) $fffb))
				([local42 1] signal: (& ([local42 1] signal:) $fffb))
				([local42 2] signal: (& ([local42 2] signal:) $fffb))
				(localproc_3)
				(if (gEgo has: 7)
					(DrawCel 125 5 0 64 111 15)
				)
...
(instance timeButtonScript of HandsOffScript
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				(gGame setCursor: 80 (HaveMouse))
				(DrawCel 125 2 2 [local36 register] [local39 register] 15)
				(switch register
					(0
						(= temp0 (/ local16 60))
						(+= local16 60)
