...
(local
	[local0 30] = [325 0 325 1 325 2 325 3 325 4 325 5 325 6 325 7 325 8 325 9 325 10 325 11 325 12 325 13 325 14]
	[local30 225]
	local255
	local256
	local257
	[local258 100] = [9 3 0 12 12 14 5 14 3 3 12 12 5 0 13 0 6 10 6 2 3 12 9 12 9 9 14 4 13 14 13 7 7 5 7 9 9 6 9 13 2 12 2 6 1 13 1 6 1 2 1 5 4 6 4 1 13 13 8 13 14 12 14 3 2 2 2 6 10 7 5 12 4 13 4 9 7 3 7 0 9 4 14 4 8 1 3 6 8 5 13 0 13 1 8 1 0 3 0 11]
	[local358 44] = [0 14 4 14 6 5 6 2 10 3 13 3 11 2 8 2 12 6 12 11 4 0 0 0 5 11 5 8 11 8 9 10 10 5 13 5 8 3 8 5 1 7 4 7]
	local402
	[local403 16]
	[local419 16] = [-1 1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 16384]
	local435
	local436 = -1
	local437
	local438
...
(procedure (localproc_3 &tmp temp0 temp1)
	(for ((= temp0 0)) (< temp0 15) ((++ temp0))
		(for ((= temp1 0)) (< temp1 15) ((++ temp1))
			(if (not (& [local403 temp0] [local419 (+ temp1 1)]))
				(localproc_0 (+ (* temp0 15) temp1) temp1 temp0 43 10)
			)
		)
	)
)

(procedure (localproc_4 &tmp [temp0 200])
	(cond
		((== local402 1)
		...
		(else
			(Format @temp0 325 20 local402 (- local435 local402)) ; "You have located %d words; you have %d left to go."
			(Print @temp0 #at 65 6)
		)
	)
)
...
(procedure (localproc_7 param1 param2 param3 param4 param5 &tmp temp0 temp1 temp2 temp3 [temp4 4] temp8 temp9 temp10 [temp11 100])
	...
	(repeat
		(cond
			((== param5 2)
				(|= [local403 temp3] [local419 (+ temp2 1)])
				(localproc_0 (+ (* temp3 15) temp2) temp2 temp3 temp8 temp9)
			)
			((== param5 0)
				(if (& [local403 temp3] [local419 (+ temp2 1)])
					(localproc_0 (+ (* temp3 15) temp2) temp2 temp3 21 52)
				else
					(localproc_0 (+ (* temp3 15) temp2) temp2 temp3 temp8 temp9)
				)
			)
...
(procedure (localproc_8 param1 param2 param3 param4 &tmp temp0 temp1)
	(for ((= temp0 0)) (< temp0 25) ((++ temp0))
		...
		(cond
			...
			(
				(and
					(== param2 [local258 temp1])
					(== param1 [local258 (+ temp1 1)])
					(== param4 [local258 (+ temp1 2)])
					(== param3 [local258 (+ temp1 3)])
				)
				(= [local258 temp1] (- 0 [local258 temp1]))
				(= [local258 (+ temp1 1)] (- 0 [local258 (+ temp1 1)]))
				(= [local258 (+ temp1 2)] (- 0 [local258 (+ temp1 2)]))
				(= [local258 (+ temp1 3)] (- 0 [local258 (+ temp1 3)]))
				(return 1)
			)
			(
				(and
					(== (- 0 param2) [local258 temp1])
					(== (- 0 param1) [local258 (+ temp1 1)])
					(== (- 0 param4) [local258 (+ temp1 2)])
					(== (- 0 param3) [local258 (+ temp1 3)])
				)
				(return 2)
			)
			(
				(and
					(== param2 [local358 temp1])
					(== param1 [local358 (+ temp1 1)])
					(== param4 [local358 (+ temp1 2)])
					(== param3 [local358 (+ temp1 3)])
				)
				(return 3)
			)
		)
		...
		(cond
			((and (<= 0 temp6 14) (<= 0 temp7 14))
				...
				(switch temp0
					(evNULL
						(if (!= temp8 local255)
							(localproc_0 temp8 temp6 temp7 40 30)
							(if
								(&
									[local403 local257]
									[local419 (+ local256 1)]
								)
								(localproc_0 local255 local256 local257 21 52)
							else
								(localproc_0 local255 local256 local257 43 10)
							)
						)
						(= local255 temp8)
						(= local256 temp6)
			...
			((or (!= local256 local440) (!= local257 local441))
				(localproc_7 local256 local257 local256 local257 0 7)
				(= local440 local256)
				(= local441 local257)
			)
		)
...
(instance wordWindow of SysWindow
	...
	(method (open &tmp temp0 temp1 [temp2 15] temp17 temp18 temp19 temp20)
		...
		(for ((= temp0 0)) (< temp0 30) ((+= temp0 2))
			(Format @temp2 [local0 temp0] [local0 (+ temp0 1)] &rest)
			(for ((= temp1 0)) (< temp1 15) ((++ temp1))
				(= temp17
					(= [local30 (+ (* 15 (/ temp0 2)) temp1)]
						(StrAt @temp2 temp1)
					)
				)
				(Display @temp17 dsCOORD temp18 temp19 dsFONT 999 dsCOLOR 10 dsBACKGROUND 43)
				(if (& [local403 (/ temp0 2)] [local419 (+ temp1 1)])
					(localproc_0
						(+ (* (/ temp0 2) 15) temp1)
						temp1
						(/ temp0 2)
				...
				(if (and (== temp20 0) (!= temp1 14))
					(Graph grDRAW_LINE 22 (- temp18 2) 156 (- temp18 2) 49 -1 -1)
					(Graph grUPDATE_BOX 23 (- temp18 3) 156 (- temp18 1) 1)
				)
			)
			(= temp20 1)
			(= temp18 5)
			(+= temp19 9)
			(if (!= temp0 28)
				(Graph grDRAW_LINE (- temp19 2) 4 (- temp19 2) 168 49 -1 -1)
				(Graph grUPDATE_BOX (- temp19 3) 5 (- temp19 1) 168 1)
			)
		)
		(if (or (not local438) (!= local439 global114))
			(localproc_5)
			(= local438 1)
		)
		(localproc_4)
...
(instance sPlaySounds of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(proc5_14 325 18 local402) ; "Congratulations! You've found %d sports and games. You hear a "swish" and a "click" as the words slide into another puzzle."
				(= cycles 1)
			)
			(2
				(gCMusic2
