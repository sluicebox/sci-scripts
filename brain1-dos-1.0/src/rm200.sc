	...
	(if local153
		...
		(cond
			((and temp0 (& [local3 local137] local142))
				(DrawCel local412 3 2 218 75 15)
			)
			((and temp0 (& [local3 local137] local146))
				(DrawCel local412 6 2 218 75 15)
			)
		)
		(cond
			((and temp0 (& [local3 local139] local142))
				(DrawCel local412 3 2 169 75 15)
			)
			((and temp0 (& [local3 local139] local146))
				(DrawCel local412 6 2 169 75 15)
			)
		)
	)
	...
	(if local154
		...
		(if temp1
			...
			(if (& [local3 local134] $0010)
				(DrawCel local412 12 1 228 91 15)
			)
			(cond
	...
	(cond
		...
		(temp1
			...
			(if (& [local3 local132] $0010)
				(DrawCel local412 12 0 242 101 15 (|= [local67 local132] $0010))
			)
			(cond
				((& [local3 local132] local142)
					(DrawCel local412 4 0 243 57 15)
					(|= [local67 local132] local142)
				)
				((& [local3 local132] local146)
					(DrawCel local412 4 3 243 57 15)
				)
			)
		)
	)
		...
		(temp2
			...
			(cond
				((& [local3 local133] local142)
					(DrawCel local412 5 0 156 57 15)
					(|= [local67 local133] local142)
				)
				((& [local3 local133] local146)
					(DrawCel local412 5 3 156 57 15)
				)
			)
		)
	)
...
(instance controlPanel of Feature
	(properties
		...
		sightAngle 180
		onMeCheck 16384
		lookStr {The control panel has six colored buttons for moving and turning the elevator car.}
	)
)
...
(instance leftArrow of Prop
	(properties
		...
		lookStr {Pushing this button turns the elevator car to the left.}
		view 200
		cel 1
		signal 4
	)
...
(instance rightArrow of Prop
	(properties
		...
		lookStr {Pushing this button turns the elevator car to the right.}
		view 200
		loop 1
		cel 1
		signal 4
...
(instance forwardArrow of Prop
	(properties
		...
		lookStr {Pushing this button moves the elevator car forward.}
		view 200
		loop 2
		cel 1
		signal 4
	...
	(method (doVerb theVerb &tmp temp0)
		(cond
			...
			((& [local3 local0] local146)
				(cond
					((== local146 64)
						...
						(Wait 0)
						(Wait 45)
						(Print 200 1) ; "You leave the maze through the north exit."
					...
					((== local146 128)
						...
						(Wait 0)
						(Wait 45)
						(Print 200 2) ; "You leave the maze through the south exit."
			...
			((& [local3 local0] local142)
				...
				(Print 200 3 #time 5 #dispose #at -1 12) ; "Try as you might, you just can't crash through the wall."
			)
			(else
				(gCMusic2 number: 922 setLoop: 1 play:)
...
(instance backwardArrow of Prop
	(properties
		...
		lookStr {Pushing this button moves the elevator car back.}
		view 200
		loop 3
		cel 1
		signal 4
...
(instance downMover of Prop
	(properties
		...
		lookStr {Pushing this button moves the elevator car down.}
		view 200
		loop 4
		cel 1
		signal 4
	...
	(method (doVerb theVerb)
		(cond
			...
			(else
				...
				(Print 200 4 #time 5 #dispose #at -1 12) ; "Try as you might, you just can't bump through the floor."
			)
		)
	)
)
...
(instance upMover of Prop
	(properties
		...
		lookStr {Pushing this button moves the elevator car up.}
		view 200
		loop 5
		cel 1
		signal 4
	...
	(method (doVerb theVerb)
		(cond
			...
			(else
				...
				(gCMusic2 number: 920 setLoop: 1 play:)
				(ShakeScreen 2 ssUPDOWN)
				(Print 200 5 #time 5 #dispose #at -1 12) ; "Try as you might, you just can't bump through the ceiling."
			)
		)
	)
)
...
(instance changeButton of Actor
	(properties
		...
		lookStr {Pushing this button changes the map display between a three-dimensional grid and a flat map of the current level.}
		view 200
		loop 7
	)
		...
		(cond
			...
			((== theVerb 3)
				...
				(if (not global114)
					...
					(proc5_9 200 6) ; "The button doesn't seem to work."
				)
			)
			((and (== theVerb 2) global114)
				(proc5_9 200 7) ; "Out of Order."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance flasher of Actor
	(properties
		...
		lookStr {The lights on the control panel show that the elevator is working -- always a comfort.}
		view 200
		loop 6
		cycleSpeed 18
	)
...
(instance display of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 2)
			(cond
				((== global114 2)
					(= lookStr {The screen is presently blank})
				)
				(1
					(= lookStr
...
(instance railing1 of Feature
	(properties
		...
		sightAngle 180
		onMeCheck 4
		lookStr {You can hold on to these if you tend to become seasick.}
	)
)
...
(instance mach of Feature
	(properties
		...
		sightAngle 180
		onMeCheck 16
		lookStr {The machinery allows the elevator to turn in any direction and to move through the maze.}
	)
)
...
(instance bulb of Feature
	(properties
		...
		sightAngle 180
		onMeCheck 32
		lookStr {This powerful bulb provides enough light to let you see some of the maze outside the elevator car. It's just one more of Dr. Brain's brilliant ideas.}
	)
)
