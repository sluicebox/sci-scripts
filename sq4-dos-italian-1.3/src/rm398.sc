		...
		(switch (= state newState)
			...
			(1
				(switch local5
					...
					(4
						...
						(= local7
							(Display
								{how to get the dog into the hanging basket? Want to find out how to attach the melon? Buy this hintbook and it all becomes obvious. You'll hit your head and say, "Boy, how stupid could I have been? A moron could've figured this out. I must be a real dimwit, a pathetic nimnal, a wretched, idiotic excuse for a human being not to have figured out these simple puzzles in the first place." unless you have this hintbook. Try it and see!}
								dsWIDTH
								170
								dsCOORD
								150
			...
			(2
				(switch
					...
					(0
						(Printf 398 6 (local10 myPrice:) (local10 myPrice:)) ; "This one is %d buckazoids."
						(-= state 1)
						(= cycles 1)
					)
					(1
						(if (!= local5 4)
							(Print 398 7) ; "You don't have that much money!"
							(-= state 1)
							(= cycles 1)
						else
							(gEgo get: 8) ; hintbook
			...
			(3
				(gTheIconBar disable:)
				(Display 398 8 dsRESTOREPIXELS local6)
				(= cycles 1)
			)
			(4
				(gTheIconBar disable:)
				(Display 398 8 dsRESTOREPIXELS local7)
				(= cycles 1)
			)
			(5
				(gTheIconBar disable:)
				(Display 398 8 dsRESTOREPIXELS local8)
				(= cycles 1)
			)
			(6
