...
(instance theRack2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(3 ; Do
				(Print 371 67) ; "While enjoying this banquet o' fashion you wonder what you might look like in one of these cute little frocks.."
				(Print 371 66) ; "Hey! What kind of thing is that for a studly guy like you to be thinking! Get ahold of yourself, fella!"
			)
			(11 ; Smell
				(Print 371 64) ; "AAAhh! The smell of brand new simulated fabric!"
...
(instance theArea of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 68) ; "All the best-dressed bipedal female fashion slaves shop here at Sack's. The dazzling decor is just eye-numbing enough to make the price tags hard to read. "Wow", you think to yourself, "this place is fancier than Frederick's of Uranus!""
			)
			(11 ; Smell
				(Print 371 69) ; "The air is thick with the smell of synthetics."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance model1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 70) ; "It's a nice outfit, but you're not sure you have the midriff for it."
			)
			(3 ; Do
				(Print 371 71) ; "Oooooh! Real plastic!"
			)
			(11 ; Smell
				(Print 371 72) ; "It doesn't smell great, but it at least smells better than you."
			)
			(10 ; Taste
				(Print 371 73) ; "You tongue the mannequin and find that it doesn't please your palate."
			)
			(5 ; Talk
				(Print 371 74) ; "Although just a mannequin, it's not intelligent enough to converse with even you."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance model2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 75) ; "The skin color and cape make a statement - the kind taken in police stations!"
			)
			(3 ; Do
				(Print 371 76) ; "Wow! Genuine Zynthion leather."
			)
			(11 ; Smell
				(Print 371 77) ; "Whoaa, baby! Where have you been? Eau de Water Buffalo went out of style epochs ago."
			)
			(10 ; Taste
				(Print 371 78) ; "Yechh! Seriously gross!"
			)
			(5 ; Talk
				(Print 371 79) ; "Sorry. No brain."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance rogerDance of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 371 80) ; "Let's boogie, girls!"
				(roger init:)
				(gEgo y: 1000)
				(arm setCycle: Osc)
				(= cycles 1)
			...
			(5
				(Print 371 81) ; "I sure know how to bust a move!"
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance wigs of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 82) ; "These wigs are the latest fashion if you have a conehead or like the purple haze look."
			)
			(3 ; Do
				(Print 371 83) ; "Yep. It's synthetic hair."
			)
			(10 ; Taste
				(Print 371 84) ; "Dragging your tongue across the fine strands of of synthetic hair is almost as enjoyable as running it through a french fry slicer."
			)
			(11 ; Smell
				(Print 371 85) ; "No smell."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance changeRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 87) ; "It's the dressing room door."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance wigs2 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 371 88) ; "These are some darn attractive wigs. If you want a head like an ax, or you've wanted to capture that Ed Grimly look, they're just the thing."
			)
			(3 ; Do
				(Print 371 89) ; "Yep. It's synthetic hair all right."
			)
			(10 ; Taste
				(Print 371 84) ; "Dragging your tongue across the fine strands of of synthetic hair is almost as enjoyable as running it through a french fry slicer."
			)
