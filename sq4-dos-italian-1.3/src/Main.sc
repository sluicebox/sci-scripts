...
(procedure (proc0_14 param1 &tmp temp0 [temp1 100] [temp101 200] [temp301 4] temp305)
	(if (!= param1 -1)
		(= global188 param1)
	)
	...
	(switch global188
		(1
			(StrCat
				@temp1
				(LangSwitch
					{\1b - The Sarien Encounter}
					{\1b - L'Incontro Sarieno}
				)
			)
		)
		(3
			(StrCat
				@temp1
				(LangSwitch
					{\1c - The Pirates of Pestulon}
					{\1c - I Pirati di Pestulonia}
				)
			)
		)
		(4
			(StrCat
				@temp1
				(LangSwitch
					{\1a - Roger Wilco and The Time Rippers}
					{\1a - Roger Wilco e i Viaggiatori del Tempo}
				)
			)
		)
		(10
			(StrCat
				@temp1
				(LangSwitch
					{\1e - Latex Babes of Estros}
					{\1e - Le Pupe di Gomma di Estros}
				)
			)
		)
		(12
			(StrCat
				@temp1
				(LangSwitch
					{\1d - Vohaul's Revenge \1f}
					{\1d - La Vendetta di Vohaul \1f}
				)
			)
			(= temp0 global148)
		)
	)
	(TextSize @temp301 @temp1 0 -1)
	(StrCpy @temp101 {\06})
	(for
		((= temp305 (/ (- 326 (- [temp301 3] [temp301 1])) 2)))
		(> temp305 0)
		((-- temp305))
		
		(StrCat @temp101 {\06})
	)
	(StrCat @temp101 @temp1)
	(DrawStatus @temp101 0 (proc0_18 global158 global155))
)
...
(procedure (proc0_21)
	(= global195 (gGame parseLang:))
	(gGame parseLang: 1)
)

(procedure (proc0_22)
	(gGame parseLang: global195)
)
...
(instance ego of SQEgo
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 0 0) ; "It's you. Roger Wilco, space guy."
			)
			(5 ; Talk
				(Print 0 1) ; "You talk to yourself but are stumped for a reply."
			)
			(3 ; Do
				(Print 0 2) ; "Hey! Keep your hands off yourself!  This is a family game."
			)
			(10 ; Taste
				(Print 0 3) ; "I'll bet you wish you could!"
			)
			(11 ; Smell
				(Print 0 4) ; "Ahhh! The aroma of several adventure games emanates from your person."
			)
			(4 ; Inventory
				(switch invItem
					(12 ; cigar
						(Print 0 5) ; "You slide the pre-smoked tobacco tube in your mouth and, after noticing a hint of grease and cruel breath, decide you'll save it for a special occasion."
					)
					(else
						(proc0_16)
					)
...
(instance egoHead of Head
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 0 6) ; "Roger Wilco, space guy."
			)
			(else
...
(instance sq4 of Game
	...
	(method (init &tmp temp0 [temp1 15])
		(Format @temp1 0 7 800) ; "%d.scr"
	...
	(method (newRoom)
		(if (== ((gInventory at: 12) state:) 1) ; cigar
			(Print 0 10) ; "The cigar goes, as cigars often do, out."
	...
	(method (handleEvent event &tmp [temp0 15])
		...
		(switch (event type:)
			(evKEYBOARD
				(switch (event message:)
					...
					(KEY_ALT_n
						(Format @temp0 0 7 952) ; "%d.scr"
						(if (and gDebugOn (FileIO fiEXISTS @temp0))
							(if (HaveMem 1536)
								(proc0_21)
								((ScriptID 952) doit: @gSysLogPath 0) ; MISSING SCRIPT
							else
								(Print 0 11) ; "Not Enough Memory!!"
							)
							(event claimed: 1)
						)
					)
					(else
						(Format @temp0 0 7 800) ; "%d.scr"
						(if (and gDebugOn (FileIO fiEXISTS @temp0))
							(proc0_21)
...
(instance ok of IconI
	...
	(method (init)
		(self highlightColor: 0 lowlightColor: (proc0_18 global158 global155))
		(super init:)
	)
)
...
(instance buckazoid of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Printf ; "You have %d recession-riddled Buckazoid%s"
					0
					12
					gBuckazoidCount
					(if (== gBuckazoidCount 1) {.} else {s.})
					gBuckazoidCount
				)
			)
		)
...
(instance jar of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== cel 2)
					(Print 0 13) ; "You have a jar of green acid"
				else
					(Print 0 14) ; "You have an empty jar"
				)
			)
		)
	)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(3 ; Do
				(cond
					((gCurRoom script:)
						(Print 0 15) ; "You're far too busy for that now."
					)
					((and (OneOf gCurRoomNum 397 398) (not (IsFlag 31)))
						(Print 0 16) ; "Hey! This isn't a free hint service. You need to pay for it before you read it!"
					)
					((not (HaveMem 6144))
						(Print 0 17) ; "Sorry, you don't have enough memory to view the hint book."
					)
					(else
						(Inv curIcon: (gInventory at: 9) hide:) ; pen
						(gTheIconBar disable: 7 8)
		...
		(switch theVerb
			(4 ; Inventory
				(cond
					...
					(state
						(Print 0 18) ; "The horrid-smelling smoke eminating from its tip would indicate that the cigar is already lit."
					)
					(else
						(Print 0 19) ; "You slide the conveniently pre-moistened stogie between your lips, apply fire via a match, and proceed to nearly hack up a lung."
						(= state 1)
					)
				)
			)
...
(instance diskette of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 0 20) ; "The label on the diskette reads:    Roger Jr.   Brain Tools   Stunt Flyer@"
			)
		)
	)
)
		...
		(switch theVerb
			...
			(3 ; Do
				(if (== view 700)
					...
					(SetScore 67 3)
					(Print 0 22) ; "You extract the portable power pellet from the rear confines of the hare's polyplastoid torso."
					(Inv hide:)
					(gEgo get: 4) ; battery
					(self view: 700 loop: 0 cel: 11)
					(Inv show: gEgo)
			...
			(4 ; Inventory
				(switch invItem
					(4 ; battery
						...
						(Print 0 23) ; "You clumsily insert the battery into the furry burrow inhabitant's hollow trunk."
						(Inv hide:)
						(Inv curIcon: 0 show: gEgo)
						(gGame setCursor: 999 1)
						(gTheIconBar curIcon: icon0)
					)
				)
			)
			(else
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((== ((gInventory at: 4) owner:) 1) ; battery
						(Print 0 24) ; "Your PocketPal now has power. Now all you need is a place to use it."
					)
					(else
						(Inv hide:)
			...
			(3 ; Do
				(if (== ((gInventory at: 4) owner:) 1) ; battery
					(Print 0 25) ; "You liberate the battery from the PocketPal."
					(Inv hide:)
					(gEgo get: 4) ; battery
					(Inv show: gEgo)
				else
					(Print 0 26) ; "This is just a dumb terminal. It won't do anything by itself."
				)
			)
			(4 ; Inventory
				(switch invItem
					(4 ; battery
						...
						(SetScore 72 3)
						(Print 0 27) ; "You plug the mobile power cylinder into the laptop."
						(Inv hide:)
						(self view: 700 loop: 0 cel: 15)
						(Inv curIcon: 0 show: gEgo)
						(gGame setCursor: 999 1)
					...
					(11 ; plug
						(gEgo put: 11 1) ; plug
						(SetScore 71 3)
						(Print 0 28) ; "You attach the plug to the PocketPal."
						(Inv hide:)
						(Inv curIcon: 0 show: gEgo)
						(gGame setCursor: 999 1)
						(gTheIconBar curIcon: icon0)
...
(instance paper_with_gum of InvI
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if cel
					(Print 0 29) ; "These symbols look vaguely familiar. Now, where have you seen them?"
				else
					(Print 0 30) ; "Ummm! This lumpy paper-wrapped wad looks mighty interesting."
				)
			)
			(3 ; Do
				(if (not cel)
...
(instance gcWin of BorderWindow
	...
	(method (open &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10 temp11 temp12 temp13 [temp14 40] [temp54 4])
		(self
