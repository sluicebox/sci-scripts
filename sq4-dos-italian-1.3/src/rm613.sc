...
(instance rm613 of SQRoom
	...
	(method (doit)
		(if
			(and
				(== (ulence status:) 4)
				(< ((ScriptID 706 1) distanceTo: gEgo) 25) ; theBiker
			)
			(HandsOff)
			(ulence status: 6)
			(gEgo setScript: 0)
		...
		(switch (= state newState)
			...
			(4
				(if (!= (ulence status:) 6)
					...
					(gEgo setScript: 0)
					(Print 613 0) ; "Boy, that was close!"
				)
				(= cycles 2)
			)
			(5
...
(instance ship of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(10 ; Taste
				(Print 613 1) ; "Licking a timepod that's just emerged from the chronostream is like sticking your tongue in a curling iron."
			)
			(2 ; Look
				(Print 613 2) ; "You give the Time Pod a long glance. It's not the most attractive vehicle you've ever seen. It resembles an overgrown titanium tennis shoe. A tinted glass shield seals the top."
			)
			(3 ; Do
				(if (== (ulence egoBusy:) 1)
					(Print 613 3) ; "Not now!"
			...
			(11 ; Smell
				(Print 613 4) ; "It smells like a time pod broiling in the Ulence sun."
			)
			(else
...
(instance building of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(11 ; Smell
				(Print 613 5) ; "It smells like an old lawsuit coming back to haunt the Two Guys From Andromeda."
			)
			(10 ; Taste
				(Print 613 6) ; "Taste-tempting stucco! And what a nice sanding job it does on your tongue!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
