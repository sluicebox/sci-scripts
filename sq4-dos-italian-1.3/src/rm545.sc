...
(instance rm545 of SQRoom
	...
	(method (doit &tmp temp0 temp1 temp2 temp3 temp4 temp5)
		...
		(cond
			...
			((temp4 isKindOf: RegionPath)
				(= temp0 (- (temp4 value:) 1))
				(= temp1 (temp4 value:))
				(blip1 setMotion: MoveTo [local7 temp0] [local7 temp1])
			)
		)
		(cond
			(script)
			...
			((temp5 isKindOf: RegionPath)
				(= temp2 (- (((ScriptID 704 2) mover:) value:) 1)) ; droid2
				(= temp3 (((ScriptID 704 2) mover:) value:)) ; droid2
				(blip2 setMotion: MoveTo [local7 temp2] [local7 temp3])
			)
		)
	)
...
(instance blip3 of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 5) ; Talk
			(Print 545 3) ; "You talk to yourself, but you don't hear you."
		else
			(super doVerb: theVerb &rest)
		)
	)
