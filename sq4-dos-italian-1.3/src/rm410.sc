			...
			((StepOn gEgo 2)
				...
				(cond
					((OneOf (gEgo view:) 373 374)
						...
						(Print 410 0) ; "A real lady NEVER goes Zero-G swimming in a dress!IUna VERA signora non si mette a nuotare a Gravit\85 Zero vestita cos\8d!"
						(HandsOn)
					)
...
(instance skateorama of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 2) ; "It's a Zero-G Skate-O-Rama. In here, the mall's gravity generators have no effect, providing an ideal and unique entertainment opportunity for the entire family."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance light of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 3) ; "The Skate-O-Rama is illuminated by a nearby sun whose glow is reflected through a mirror set into the floor. With this latest innovation, you can now enjoy Zero-G while tuning up your tan."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance steps of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 4) ; "These steps lead up to the rest of the Galaxy Galleria Mall."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance wall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 5) ; "These walls protect shoppers from errant skaters."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance restOfMall of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 410 6) ; "The rest of the Galaxy Galleria awaits the unwary and uninformed shopper."
			)
			(else
				(super doVerb: theVerb &rest)
			)
