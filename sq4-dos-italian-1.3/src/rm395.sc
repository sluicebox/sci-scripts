...
(instance letters of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 395 16) ; "QA people were here."
			)
		)
	)
)
...
(instance atm of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 395 17) ; "Wow, an AutoBucks Teller Machine!"
			)
			(3 ; Do
				(if (not (& (gEgo onControl: 1) $4000))
					(Print 395 18) ; "Your arms won't reach that far."
				else
					(Print 395 19) ; "You play with the buttons, but the machine does not respond."
				)
			)
			(4 ; Inventory
				(switch invItem
			...
			(11 ; Smell
				(Print 395 20) ; "It smells more like fresh Buckazoids than the other leading brand of ATMs."
			)
			(10 ; Taste
				(Print 395 21) ; "It tastes more like fresh Buckazoids than the other leading brand of ATMs."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance crowd of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 395 22) ; "For some reason, there is a gathering here."
			)
			(5 ; Talk
				(if (not (gCurRoom script:))
					(if (gEgo inRect: 198 64 239 86)
						...
						(Print 395 23) ; "No one can hear you from over here."
					)
				)
			)
			(11 ; Smell
				(Print 395 24) ; "You never noticed how much adventure game players tend to smell like potato chips, beer and money."
			)
			(10 ; Taste
				(Print 395 25) ; "With the way this crowd smells, you'd want to lick them?"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance store of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 395 26) ; "It's the Software Excess store - "If it's soft, we're aware.""
			)
			(11 ; Smell
				(Print 395 27) ; "The software store smells of turnovers...high employee turnovers and low product turnovers."
			)
			(10 ; Taste
				(Print 395 28) ; "If you think the store leaves a bad taste in your mouth NOW, wait 'till you see the prices."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bush1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 395 29) ; "This is a fine example of the wonders of fabricated flora."
			)
			(11 ; Smell
				(Print 395 30) ; "This plant was provided courtesy of Shapeir Florists."
			)
			(10 ; Taste
				(Print 395 31) ; "This plant tastes like another cheap plug for other Sierra products."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance zapper of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 395 32) ; "Probably some kind of customer detection system."
			)
			(else
				(super doVerb: theVerb &rest)
			)
