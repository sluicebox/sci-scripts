...
(procedure (localproc_0 param1 &tmp temp0 temp1 temp2 temp3 temp4 [temp5 200])
	(if (== param1 522)
		(Message msgGET 520 60 0 10 1 @temp5) ; "I PAY HOMAGE TO THEE YE LORDS OF ETERNITY RA STRONG IS THY SAIL IN THE WIND AS THE LAKE OF FIRE IN THE UNDERWORLD BEHOLD SHU THE MOTHER CREATING THE GODS"
	else
		(Message msgGET 520 60 0 11 1 @temp5) ; "IN SILENCE FROM HER WOMB QUICKER THAN GREYHOUNDS AND FLEETER THAN LIGHT LET ME NOT BE BURNT LET ME NOT BE CONSUMED LET ME NOT PERISH AS MY MUMMY LIES PROSTRATE IN MY TOMB"
	)
	(= local0 0)
	(= local1 0)
	(= temp2 (StrLen @temp5))
	(= temp4 21)
	(for ((= temp3 0)) (< temp3 (+ (/ temp2 2) (mod temp2 2))) ((++ temp3))
		(= temp0 (& [temp5 temp3] $00ff))
		(= temp1 (>> [temp5 temp3] $0008))
		(localproc_1 temp0 temp4)
		(if (>= temp1 32)
			(localproc_1 temp1 temp4)
		)
	)
)

(procedure (localproc_1 param1 param2 &tmp temp0 temp1 [temp2 54])
	(= [temp1 0] param1)
	(TextSize @[temp2 0] @temp1 1900 0 0)
	(= temp0 (- [temp2 3] [temp2 1]))
	(if (> (+ local0 temp0) 320)
		(= local0 0)
		(+= local1 param2)
	)
	(Display @temp1 dsCOORD local0 local1 dsFONT 1900 dsCOLOR 7)
	(+= local0 temp0)
)
...
(instance rm520 of LBRoom
	...
	(method (init)
		...
		(ratFore init: approachVerbs: 4 1 8 setOnMeCheck: 1 4) ; Do, Look, magnifier
		(rosetta init: approachVerbs: 4 1 8 setOnMeCheck: 1 256) ; Do, Look, magnifier
		(hieroglyphics init: approachVerbs: 4 1 8 setOnMeCheck: 1 2048) ; Do, Look, magnifier
		(certificate init:)
		(skeletonLegs init:)
		(cobraCage init:)
		(bookShelf init:)
...
(instance cobraLoose of Actor
	...
	(method (doit)
		(if (and (not local7) (not (cobraDoor script:)) (not local8))
			(cond
				(
					(and
						local11
				...
				((and (< (gEgo x:) (self x:)) local9)
					(= local9 0)
					(self setScript: sCobraTurn)
				)
				((and (> (gEgo x:) (self x:)) (not local9))
					(= local9 1)
					(self setScript: sCobraTurn)
				)
			)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(25 ; snakeOil
				...
				(cond
					((or (> local6 13) (and (> local6 3) (< local6 9)))
						(= global150 0)
						(-- local6)
