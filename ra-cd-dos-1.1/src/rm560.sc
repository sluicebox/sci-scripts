...
(instance rm560 of LBRoom
	...
	(method (init)
		...
		(desk init: approachVerbs: 1 setOnMeCheck: 1 2048) ; Look
		...
		(fireplaceIn init: approachVerbs: 4 1 8 setOnMeCheck: 1 256) ; Do, Look, magnifier
		(bookcase init: setOnMeCheck: 1 16)
		(genericBookshelf init:)
		(book init: approachVerbs: 4 1 8 setOnMeCheck: 1 64) ; Do, Look, magnifier
		(bigPainting init:)
	...
	(method (doit)
		...
		(if
			(and
				local0
				(not ((ScriptID 561 0) cel:)) ; safePicture
...
(instance sGetBook of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(sFX number: 566 flags: 1 loop: 1 play:) ; sPulBookFromShelf
				(gEgo setCycle: Beg self)
			)
			(5
				(= ticks 60)
...
(instance sDumpSafe of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if local1
					(sFX number: 561 flags: 1 loop: 1 play:) ; sCloseSafe
					(= local1 0)
					(= ticks 60)
				else
					(= cycles 1)
			...
			(2
				(if (!= ((ScriptID 561 0) cel:) 0) ; safePicture
					...
					(sFX number: 45 flags: 1 loop: 1 play:) ; sSqueakyDoorClose
				else
					(= cycles 1)
				)
			)
