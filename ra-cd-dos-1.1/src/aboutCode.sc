...
(instance aboutCode of Code
	...
	(method (doit &tmp [temp0 300] [temp300 300])
		...
		(Message msgGET 13 1 0 0 3 @temp0) ; "To place a domestic order, contact Sierra Sales at 800-326-6654. To place an international order, contact them at 0734 303171. To interrogate someone in Technical Support, call 209-683-8989.            [ version %s ]"
		(Format @temp300 @temp0 gVersion)
		(if (not (Print addText: @temp300 init:))
			(self dispose:)
			(return)
		)
		(if (not (Print addText: 1 0 0 4 0 0 13 init:)) ; "Our lonely Customer Servants will happily respond to your calls from 8:15 a.m. to 4:45 p.m. Pacific Time, Monday through Friday."
			(self dispose:)
			(return)
		)
