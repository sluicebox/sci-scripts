		...
		(if (not (IsFlag 4))
			...
			(cond
				((not (TimeCheck $b102)) ; has 11:15 pm occurred?
					...
					(blender init:)
					(snakeLasso ignoreActors: 1 approachVerbs: 4 1 8 init:) ; Do, Look, magnifier
				)
...
(instance sGetWireCutters of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(gEgo get: 10 normalize: 831 setScale: 155 loop: 1) ; wireCutters
...
(instance sGetLasso of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gEgo get: 19 normalize: 831 setScale: 155 loop: 1) ; snakeLasso
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(sFX number: 558 loop: 1 flags: 1 play:) ; sSwitch
			)
			(3
				(sFX number: 721 loop: -1 flags: 1 play:) ; sHeavyStoneMoved
				(SetFlag 119)
				(= ticks 180)
			)
			(4
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(self hide:)
				(ClearFlag 19)
				(sFX number: 641 loop: 1 flags: 1 play:) ; sToolBoxClose
			)
			(else
				(super doVerb: theVerb)
			)
		...
		(switch theVerb
			...
			(4 ; Do
				(if (IsFlag 19)
					...
					(SetFlag 19)
					(sFX number: 640 loop: 1 flags: 1 play:) ; sToolBoxOpen
				)
			)
			(else
				(super doVerb: theVerb)
