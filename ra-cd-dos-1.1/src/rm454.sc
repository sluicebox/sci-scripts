...
(instance rm454 of LBRoom
	...
	(method (init)
		...
		(mummy1 init: approachVerbs: 1 4 2 6 setOnMeCheck: 1 2) ; Look, Do, Talk, Ask
		(secretDoor1 init: approachVerbs: 1 4 2 6) ; Look, Do, Talk, Ask
		(secretDoor2 init: approachVerbs: 1 4 2 6) ; Look, Do, Talk, Ask
		(mummy3 init: approachVerbs: 1 4 2 6 setOnMeCheck: 1 16) ; Look, Do, Talk, Ask
		(rosetta init: approachVerbs: 1 4 6 2 8 setOnMeCheck: 1 4) ; Look, Do, Ask, Talk, magnifier
		...
		(switch (= state newState)
			...
			(7
				(if (IsFlag 39)
					...
					(gGameMusic2 number: 444 flags: 1 loop: 1 play:) ; sBreakDownDoor
				else
					(= cycles 1)
				)
				(gEgo show:)
	...
	(method (doVerb theVerb)
		(return
			(switch theVerb
				...
				(2 ; Talk
					(gMessager say: 0 2 0 1 0 0 0) ; "You get no response."
					1
				)
				(6 ; Ask
					(gMessager say: 0 6 0 2 0 0 0) ; ""It is better to know some of the questions than all of the answers." -- James Thurber"
					1
				)
				(else
					(super doVerb: theVerb &rest)
