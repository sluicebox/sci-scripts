...
(instance rm250 of LBRoom
	...
	(method (init &tmp [temp0 50])
		...
		(noise number: 41 flags: 1 play:) ; sCarDoorClose
		(super init:)
		(proc0_8 1)
		...
		(switch (= state newState)
				...
				(switch (gCurRoom setInset: (ScriptID 20 0)) ; inNotebook
					...
					(519
						...
						(if (not (gCast contains: trash1))
							...
							(gMessager say: 17 14 7 0) ; "Lady, youse is already IN New York. WHERE do ya want to go? I ain't got all day."
						)
					)
					(-1
						(= local0 250)
					...
					(else
						...
						(if (not (gCast contains: trash1))
							...
							(gMessager say: 12 14 7 0) ; "You some kinda of tourist, lady? I doesn't know what youse is talkin' about. Tell me WHERE ya wanna go!"
						)
					)
				)
				(= cycles 1)
			...
			(3
				(cond
					...
					((not (gCast contains: trash1))
						(gCurRoom setScript: sDoTakeOffFlight)
					)
					(else
						(gCurRoom setScript: sMoveBuildings)
					)
				)
			)
			(4
	...
	(method (changeState newState &tmp [temp0 50])
		(switch (= state newState)
			...
			(12
				(gGameMusic2 send: 2 224 0)
				(if (& gMsgType $0002)
					((ScriptID 1902 13) dispose:) ; Rocco
				)
				(gGame handsOn:)
				(gTheIconBar enable: 5)
				(if (!= local0 gCurRoomNum)
		...
		(switch theVerb
			...
			(6 ; Ask
				(if local2
					...
					(if (<= 512 temp0 665)
						(gMessager say: 11 6 7 0) ; "I knows where it is, but dat's all. I, uh, never leave my taxi, so I never been in dere."
					else
						(gMessager say: 12 6 7 0) ; "Look, lady, I'm paid to DRIVE youse around town, but I DON'T like TALKING to da passengers."
					)
				)
			)
			(2 ; Talk
	...
	(method (handleEvent event)
		(cond
			(
				...
				(if (!= local1 self)
					(= local1 self)
					(noise number: 54 loop: 1 flags: 1 play:) ; sDiggingInGarbage
				else
					(= local1 0)
				)
				(event claimed: 1)
