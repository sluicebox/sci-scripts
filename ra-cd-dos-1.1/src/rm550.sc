...
(instance eastDoor of LbDoor
	...
	(method (cue)
		(super cue:)
		(if (and (== doorState 0) (== local1 embrace))
			(gGame handsOff:)
...
(instance deskLamp of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(8 ; magnifier
				(if (== cel 2)
					(gMessager say: noun theVerb) ; "Since the floor is an odd location for a desk lamp, and since other desk items have been disturbed, you conclude that the lamp was knocked down during some sort of physical activity that took place on the desk."
				else
					(gCurRoom setInset: inDeskLamp)
				)
			)
			...
			(39 ; carbonPaper
				(if (and (not (IsFlag 40)) (not (IsFlag 64)))
					...
					(gMessager say: 45 theVerb 6) ; "You can't read it this way."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance sOlympiaEnters of Script
	...
	(method (changeState newState &tmp temp0 temp1 temp2 temp3)
		(switch (= state newState)
			...
			(14
				(olympiaConv add: 1550 1 0 1 6 init: self) ; "Yvette is very fond of EVERYONE, my dear. Now, if you'll excuse me, I have some business to attend to."
			)
			(15
				(olympia
					setCycle: Walk
