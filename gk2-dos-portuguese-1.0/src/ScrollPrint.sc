...
(class ScrollPrint of Print
	...
	(method (dispose)
		...
		(if (and BAD_SELECTOR (cUpInvBut scratch:))
			(gInventory firstTrue: #showSelf (gUser BAD_SELECTOR:))
		)
		(super dispose: &rest)
	)
		...
		(if (== 1 (param1 type:))
			...
			(repeat
				...
				(if (and (= temp0 (self onMe: param1)) (not (-- temp2)))
					(= temp2 BAD_SELECTOR)
					(client BAD_SELECTOR: BAD_SELECTOR)
				)
				(if (!= temp0 temp1)
					(if temp0
						(client BAD_SELECTOR: 1)
						(= cel 2)
						(++ textTop)
						(++ textBottom)
					else
	...
	(method (handleEvent event)
		...
		(cond
			...
			(
				(and
					(== (gUser BAD_SELECTOR:) 62)
					(!= (gCurInvView BAD_SELECTOR:) -1)
				)
				(gUser BAD_SELECTOR: (gCurInvView verb:))
				((gUser BAD_SELECTOR:) update:)
			)
		)
	)
