...
(instance Room48 of Rm
	...
	(method (init)
		...
		(switch gPrevRoomNum
			(49
				(NormalEgo 0)
				(gEgo posn: 130 123 view: 2 loop: 0 setStep: 3 2 init:)
			)
			(else
				(NormalEgo 0)
				(gEgo posn: 135 134 view: 2 loop: 0 setStep: 3 2 init:)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(gEgo hide:)
				(gEgo hide:)
				(client view: 48 cycleSpeed: 2 cel: 255 setCycle: End self)
			)
			(5
				(Print 48 28 #at -1 10) ; "Poor Rosella! Looks like you'll join the deer in the stew pot tonight."
				(= seconds 4)
			)
			(6
				(= gDeathFlag 1)
