...
(instance Room15 of Rm
	...
	(method (init)
		...
		(super init:)
		(if gNight
			(gCurRoom overlay: 115)
		)
		(self setRegions: 507) ; Woods_Region
		(gEgo view: 2 setStep: 3 2 init:)
		(if (== global110 0)
			(Load rsVIEW 518)
			(Load rsVIEW 375)
			(Load rsVIEW 372)
			...
			(cond
					...
					(cond
						...
						((== global110 3)
							(if (< (gEgo distanceTo: local3) 13)
								...
								(Print 15 30) ; "You're not near enough."
							)
						)
					)
				)
				((Said 'get/lilies,pad')
					(Print 15 31) ; "You have no need for a lily pad."
				)
				((Said 'talk[/frog]')
					(if (or (== global110 0) (== global110 3) (== global110 4))
						(Print 15 32) ; "Ribbit! Ribbit!"
						(Print 15 33) ; "Well, what else did you expect from a frog?"
					else
						(Print 15 34) ; "There's no one nearby."
					)
				)
				((Said 'throw,drop,place,give/ball')
					(if (gEgo has: 5) ; Gold_Ball
							...
							((== global110 0)
								(if ((Inv at: 10) ownedBy: 200) ; Small_Crown
									(if
										...
										(HandsOff)
										(LookAt gEgo local3)
										(Print 15 35 #draw) ; "You "accidentally" drop the golden ball into the pond. From atop a lily pad, a large green frog leaps into the water, and disappears from view. A moment later, the frog emerges with...YES!...your lost ball in its mouth! It seems as if the frog is kindly returning your gold ball to you."
										(= global110 2)
										(frogActions changeState: 3)
									else
										(Print 15 36) ; "Try another spot."
									)
								else
									(Print 15 37) ; "That's not necessary any more."
								)
							)
							((!= (gEgo view:) 2)
								(Print 15 38) ; "Not here."
							)
							(else
								(Print 15 39) ; "Not now."
							)
						...
						(Print 15 40) ; "You don't have it."
					)
				)
				((or (Said 'kiss/frog') (Said 'kiss[/!*]'))
					(if (== global110 4)
						(Print 15 41) ; "You look at the frog's green lips. Mmmmm. Good. You feel silly doing this, as you slowly put your mouth against the frog's."
						(frogActions changeState: 50)
						(= global110 5)
					else
						(Print 15 42) ; "First, you need to get the frog."
					)
				)
				((Said 'get/ball')
					(if (== (gEgo view:) 2)
						(cond
							...
							(else
								(Print 15 43) ; "How can you do that?"
							)
						)
					else
						(CantDo) ; "You can't do that now."
					...
					(if (= temp0 (gInventory saidMe:))
						(if (gEgo has: (gInventory indexOf: temp0))
							(if (gCast contains: local3)
								(Print 15 44) ; "He is not interested."
							else
								(Print 15 45) ; "There's no one here to give anything to."
							)
						else
...
(instance frogActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(= global110 1)
				(Print 15 47 #time 3) ; "Oops, you scared the frog away!"
				(local3
					view: 373
					cycleSpeed: 0
					loop: 0
			...
			(13
				...
				(RedrawCast)
				(Print 15 48) ; "Squinching up your nose in disgust, you catch the large frog and hold it in your hand. It stares at you with its big bulgy eyes, and wobbles its throat."
				(= global110 4)
				(HandsOn)
			)
			(50
			...
			(51
				(gEgo loop: 3 cel: 255 setCycle: End self)
			)
			(52
				(poofSound play:)
				((= local4 (Prop new:))
			...
			(54
				(local4 dispose:)
				(Print 15 49 #at -1 10) ; "Suddenly, the little green frog changes into a handsome prince!"
				(Print 15 50 #at -1 10) ; ""Who are you?" the prince demands. "I thought you were supposed to be a princess?! Why, you're nothing more than a PEASANT girl!" You blanch a bit at that remark, but say nothing."
			...
			(60
				...
				(local3
					...
					setMotion: MoveTo (- (local6 x:) 3) (local6 y:) self
					init:
				)
			)
			(61
...
(instance drinking of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= seconds 5)
				(= global120 (Print 15 52 #at -1 10 #dispose)) ; "You bend down and drink a bit of the pond water. Well, it's not wonderful, but it satisfies your thirst."
			)
			(2
				(gEgo setCycle: Beg self)
			)
