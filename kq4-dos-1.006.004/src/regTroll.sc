...
(instance regTroll of Rgn
	...
	(method (doit)
		...
		(if (and (not (proc0_20)) (not (& (gEgo onControl: 0) $0002)))
			(gEgo dispose:)
			(RedrawCast)
		...
		(switch (= state newState)
			...
			(1
				(if
					...
					(Print 605 14 #at 200 40 #dispose) ; "GRRRRRRR!!"
					(= gTrollChasing 1)
					((= local0 (Act new:))
						view: 190
						setAvoider: Avoid 1
			...
			(3
				(Print 605 15 #time 4 #at 200 10 #dispose) ; "Watch out!"
				(self cue:)
			)
			(4
				(if (gCast contains: local0)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(caughtMusic play:)
				(cls)
				(Print 605 16 #at -1 10) ; "Oh, no! You have been caught by the vicious troll!"
				(= local3 1)
				(regTroll notify: 0)
				(Print 605 17 #at -1 10) ; "Fate be what it may, you are dragged off to meet it."
				(= seconds 4)
			)
			(2
				(= gDeathFlag 1)
