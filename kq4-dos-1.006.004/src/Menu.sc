...
(class TheMenuBar of MenuBar
	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 4] temp6 [temp7 288])
		(switch (= temp0 (super handleEvent: event))
			...
			(513
				(if (not (HaveMem 1028))
					(Print 997 2) ; "The game can not be saved at this time."
				else
					(gGame save:)
				)
			)
			(514
				(if (not (HaveMem 1028))
					(Print 997 3) ; "The game can not be restored at this time."
				else
					(gGame restore:)
				)
			)
			...
			(769
				(= temp6 (Sound pause: 1))
				(Print 997 6 #font 0 #mode 1 #button { Continue } 0) ; "King's Quest IV "The Perils of Rosella" is paused."
				(Sound pause: temp6)
			)
			(770
				(if (not (HaveMem 2348))
					(Print 997 7) ; "Inventory can not be displayed at this time."
				else
					(= temp6 (Sound pause: 1))
					(Inv showSelf: gEgo)
					(Sound pause: temp6)
			...
			(1025
				(if (not (HaveMem 1850))
					(Print 997 8) ; "Change speed using plus (+) and minus (-) keys."
				else
					(= temp1
						((Gauge new:)
							description:
			...
			(1281
				(if (not (HaveMem 1850))
					(Print 997 9) ; "Volume can not be changed at this time."
				else
