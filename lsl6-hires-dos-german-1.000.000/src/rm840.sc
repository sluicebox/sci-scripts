...
(instance rm840 of LarryRoom
	...
	(method (init)
		...
		(table2 init:)
		(razorWire init:)
		(dancer1 init: setPri: 30 cycleSpeed: 10 setScript: boogie)
		(guyChugger init: setPri: 30 ignoreActors: 1)
		(girlChugger init: setPri: 30 ignoreActors: 1 setScript: guzzleBeer)
		(tub init:)
		(gWalkHandler addToFront: tent)
		(gGlobalSound1 number: 840 loop: -1 play: setVol: 50 fade: 127 10 10 0)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo setMotion: PolyPath 138 82 self)
			)
			(1
				(gEgo
					view: 840
			...
			(2
				(gEgo view: 843 setMotion: MoveTo 153 83 self)
			)
			(3
				(if (== local4 3)
					(= local4 (= register 0))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(gEgo view: 843 setCycle: Walk get: 3) ; beer
				(= register 0)
