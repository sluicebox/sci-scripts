...
(instance Room23 of Rm
	...
	(method (doit)
		...
		(if
			(and
				(!= (= local0 (gEgo onControl: 0)) local1)
				(== (gEgo script:) 0)
			)
			(= local1 local0)
			(if (& local0 $0010)
				(gEgo setScript: fallRed)
			)
			(if (& local0 $0004)
				(gEgo setScript: fallSouth)
			)
			(if (& local0 $1000)
				(gEgo setScript: shortFall)
			)
		)
	)
...
(instance shortFall of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 18 setStep: 3 2 loop: 0 setCycle: Fwd)
				(gSounds eachElementDo: #stop 0)
				(dizzySound play:)
				(= seconds 4)
			)
...
(instance birdActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(local2 cel: 255 loop: 3 setCycle: End self)
			)
			(2
				(= local5 (Prop new:))
				(local5
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				(HandsOff)
				(local4 dispose:)
				(gEgo viewer: 0 view: 21 setCycle: End self)
			)
			(11
				(local5 dispose:)
				(= local5 0)
