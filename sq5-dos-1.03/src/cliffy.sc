	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gMessager say: 4 2 7 0 self) ; "Captain, ya |f8|gotta|f| do something with that weak-bladdered beastie of yours!"
			)
			(1
				(if (== global126 3)
				...
				(cond
					((IsFlag 61)
						(if (IsFlag 116)
							...
							(SetFlag 116)
							(gMessager say: 4 2 9 0 self) ; "Cap'n, that torpedo knocked out our warp drive. We're a sitting duck for whoever that is out there. By the way, why did they shoot at us?"
						)
					)
					((IsFlag 115)
						(ClearFlag 115)
					...
					(else
						(gMessager say: 4 2 0 0 self) ; "Cliffy is too engrossed in what he's doing to talk to you right now."
					)
				)
			)
			(1
