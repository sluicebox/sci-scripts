...
(instance closeupWedding of Script
	...
	(method (handleEvent event)
		(if
			...
			(event claimed: 1)
			(= next (ScriptID 744 1)) ; MISSING SCRIPT
			(self cue:)
		)
		(event claimed:)
	)
			...
			(12
				...
				(if (not next)
					(if (not register)
						...
						((ScriptID 744 1) register: 29) ; MISSING SCRIPT
						(= next (ScriptID 744 1)) ; MISSING SCRIPT
					)
				)
				(self dispose:)
...
(instance showMirror of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(client seconds: 0 next: (ScriptID 744 0)) ; MISSING SCRIPT
				((ScriptID 740 5) view: 7424 loop: 0 cel: 0 setCycle: 0) ; saladin
				(gTheIconBar disable:)
				(client dispose:)
			)
...
(instance showReplicaLamp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(closeupWedding next: (ScriptID 744 1) changeState: 12) ; MISSING SCRIPT
			)
		)
	)
)
	...
	(method (doVerb theVerb)
		(cond
			...
			((OneOf theVerb 56 2) ; fakeLamp5, Talk
				(gGame handsOff:)
				(closeupWedding seconds: 0 next: (ScriptID 744 1)) ; MISSING SCRIPT
