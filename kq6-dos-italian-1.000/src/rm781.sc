	...
	(method (init)
		(if (= register (not ((ScriptID 80 0) tstFlag: #rFlag1 128))) ; rgCastle
			(tempGuard1
				...
				posn: 192 126
				setSpeed: (if (< gHowFast 2) 3 else 5)
				init:
			)
			(tempGuard2
				posn: 200 128
		...
		(return
			(and
				...
				(or
					...
					(self approachX: 55 approachY: 161 approachVerbs: 5) ; Do
					(= noun 12)
				)
			)
		)
...
(instance paper_BoxInset of InsetView
	...
	(method (doVerb theVerb)
		(if (== theVerb 1) ; Look
			...
			(KQ6Print posn: -1 10 say: 1 noun theVerb 0 1 init:) ; "Inside the ebony box is a piece of paper with the word "ZEBU" printed on it."
		else
			(super doVerb: theVerb &rest)
		)
	)
