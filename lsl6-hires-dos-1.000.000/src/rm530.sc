...
(instance getOrangeScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo get: 27 normalize: 900 8 cel: 3 x: (+ (gEgo x:) 6)) ; orange
				(= cycles 2)
			)
			(3
				(gNarrator y: 10)
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(cond
					((== (local0 owner:) 530)
						(gMessager say: noun theVerb 2) ; "La Costa Lotta's salad bar is presently nothing but off-color, half-melted ice. Come back at dinner time and it will be different."
					)
					((== (local0 owner:) 5301)
						(gMessager say: noun theVerb 1) ; "A fresh orange, in surprisingly good shape, now rests on top of the salad bar's ice."
					)
					(else
						(gMessager sayRange: noun theVerb 3 1 2) ; "La Costa Lotta's salad bar is presently nothing but off-color, half-melted ice. Come back at dinner time and it will be different."
					)
				)
			)
			(36 ; washCloth [ wet ]
...
(instance cook1 of Actor
	...
	(method (init)
		(super init: &rest)
		(self setLoop: 3 1 hide: ignoreActors: 1 setScript: cook1Scr)
	)
)
...
(instance cook3 of Actor
	...
	(method (init)
		(super init: &rest)
		(self setLoop: 2 1 hide: ignoreActors: 1 setScript: cook2Scr)
	)
)
...
(instance cook2 of Actor
	...
	(method (init)
		(super init: &rest)
		(self setLoop: 5 1 hide: ignoreActors: setScript: cook3Scr)
	)
)
...
(instance cook2Scr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(client setLoop: (Random 2 3) 1 cel: 0 setCycle: End self)
			)
			(3
				(if (-- register)
					(-= state 2)
...
(instance toKitchenScr of Script
	...
	(method (doit)
		(if (and (not (doorFX handle:)) (== (rightDoor cel:) 5))
...
(instance fromKitchenScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gGame handsOff:)
				(gEgo setScaler: Scaler 100 76 78 61 posn: 267 67 cel: 5)
				(= cycles 2)
			)
			(1
				(= local1 1)
...
(instance rightDoor of Prop
	...
	(method (init)
		(super init: &rest)
		(self approachVerbs: 4 ignoreActors: 1) ; Do
	)
