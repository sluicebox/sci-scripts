...
(class Game of Obj
	...
	(method (save &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
		(= temp0 (Str new:))
			...
			(-2
				...
				(if
					(Print
						...
						back: (Palette 3 127 127 127) ; PalMatch
						addText: temp3
						addButtonBM: 64990 0 0 0 temp4 0 40
				...
				(if
					(not
						(SaveGame
							...
							temp1
							(temp0 data:)
							(KString 9 (gVersion data:)) ; StrGetData
						)
					)
					(= temp3 (Str new:))
					(= temp4 (Str new:))
					...
					(Print
						...
						back: (Palette 3 127 127 127) ; PalMatch
						addText: temp3
						addButtonBM: -1 0 0 1 temp4 0 40
						init:
					)
					(temp3 dispose:)
					(temp4 dispose:)
					...
					(= global176 temp1)
					(global177 copy: temp0)
					(global190 copy: gCurSaveDir)
				)
				(gTextScroller
					kWindow: (ScrollWindow 0 gTextScroller (gTextScroller maxItems:)) ; ScrollCreate
				)
				(ScrollWindow 19 (gTextScroller kWindow:) global258) ; ScrollReconstruct
	...
	(method (restore &tmp num oldCur buf1 buf2 str aPlane aList obj bit node1 node2 node3)
		...
		(if (!= (= num (Restore doit: &rest)) -1)
			...
			(if (CheckSaveGame name num (KString 9 (gVersion data:))) ; StrGetData
				(ScrollWindow 17 (gTextScroller kWindow:)) ; ScrollDestroy
				(RestoreGame name num (gVersion data:))
			else
				(for
					((= node1 (FirstNode (gPlanes elements:))))
					node1
