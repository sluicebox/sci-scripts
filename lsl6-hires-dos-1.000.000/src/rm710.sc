...
(instance rm710 of LarryRoom
	...
	(method (init)
		...
		(if (== gPrevRoomNum 720)
			(= local2 1)
			(gEgo posn: (shamra approachX:) (shamra approachY:) cel: 6)
			(gGlobalSound1 number: 700 loop: -1 play: setVol: 50 fade: 127 10 10 0)
			(gGame handsOn:)
		else
		...
		(shamra
			...
			init:
			setPri: 25
			approachVerbs: 4 1 2 5 6 13 ; Do, Look, Talk, Take, Zipper, orchid
			cycleSpeed: 12
		)
		(if (>= (gGame detailLevel:) 3)
			(fish1 init: setCycle: Fwd setScript: (fishScr new:) setPri: 25)
			(fish2 init: setCycle: Fwd setScript: (fishScr new:) 0 1 setPri: 25)
			(fish3 init: setCycle: Fwd setScript: (fishScr new:) setPri: 25)
			(fish4 init: setCycle: Fwd setScript: (fishScr new:) 0 1 setPri: 25)
			(fish5 init: setCycle: Fwd setScript: fishScr 0 1 setPri: 25)
		)
	...
	(method (doit)
		...
		(cond
			(script 0)
			((balconyPoly onMe: (gEgo x:) (gEgo y:))
				(if (not local2)
