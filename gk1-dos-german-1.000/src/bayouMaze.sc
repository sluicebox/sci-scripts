...
(instance bayouMaze of Room
	...
	(method (init)
		(gGkMusic1 loop: -1 number: 490 play:)
		(Load rsPIC 490)
		(= global178 0)
	...
	(method (doVerb theVerb)
		(if (and (== theVerb 45) local7) ; tracker
		...
		(switch (= state newState)
			(0
				...
				(if local7
					...
					(= temp3 (not (= temp4 (localproc_0 local8 local9))))
					(= temp5 (== local8 local9))
					(= temp4
						(if temp4
							(not temp5)
					...
					(blip posn: (+ local10 temp6) (+ local11 temp7))
				)
				(= temp0
					(cond
						(global178 global178)
				...
				(if local6
					(cond
						...
						((and (not (== temp0 15)) (not (Random 0 1)))
							(gMessager
								say: 7 0 (if local7 9 else 12) (Random 1 5)
							)
						)
					)
				)
			...
			(1
				...
				(gCurRoom
					addObstacle:
						((Polygon new:)
							type: PContainedAccess
							init: 0 145 319 145 319 94 301 106 194 103 194 84 160 83 146 91 80 96 0 74
							yourself:
						)
				)
