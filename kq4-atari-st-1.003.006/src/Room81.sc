			...
			((== (event type:) evSAID)
				(cond
					((not (& (gEgo onControl: 0) $0800))
						(cond
							...
							((Said 'unlatch/door')
								(cond
									((== gPrevRoomNum 85)
										(Print 81 19) ; "You just got out of there! Why go back in?"
									)
									((not local8)
										(Print 81 20) ; "It already is."
									)
									...
									((gEgo has: 32) ; Gold_Key
										(= local8 0)
										(Print 81 21) ; "Using the little gold key you quickly unlock the door."
										(gGame changeScore: 2)
									)
									(else
										(Print 81 22) ; "How can you do that without a key?"
									)
								)
							)
							((or (Said '[use]/key<gold') (Said 'gold'))
								(cond
									...
									((not local8)
										(Print 81 23) ; "The door is already unlocked."
									)
							...
							((Said 'open/door')
								(cond
									...
									(else
										(Print 81 26) ; "It is already is open."
									)
								)
							)
							((Said 'close/door')
							...
							((Said 'get,untie,detach/key')
								(if
									...
									(Print 81 28) ; "You remove the little gold key from the red rose."
									(= global182 1)
					...
					((or (Said 'look[<around][/!*]') (Said 'look/room'))
						(Print 81 29) ; "You see Edgar's bedroom door, and a winding stairway leading down the tower."
					)
					((Said 'look/stair')
						(Print 81 30) ; "The stone stairway leads down the tower."
					)
					((Said 'look/door')
						(Print 81 12) ; "It's just a plain wooden door."
					)
					((Said 'look/wall')
						(Print 81 31) ; "There is nothing of importance on the walls."
					)
					((Said 'close/door')
						(Print 81 27) ; "You don't need to."
					)
					((Said 'open/door')
						(cond
							...
							(else
								(Print 81 32) ; "It already is open."
							)
						)
					)
					((Said 'unlatch/*')
						(Print 81 33) ; "It already is unlocked."
					)
				)
			)
		)
...
(instance lockUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 81 34 #at -1 40 #time 5) ; "You think to yourself, NOW WHAT!!"
			...
			(7
				(= local0 (Print 81 35 #at -1 35 #dispose)) ; "You hear something at the door!"
				(local4 cycleSpeed: 7 setLoop: 0 cel: 0 setCycle: CT 10 1)
			...
			(12
				(Print 81 36 #at -1 20 #width 260) ; "It looks like you're destined to be...Mrs. Edgar!"
				(= seconds 12)
			)
			(13
				(HandsOn)
...
(instance lockUpLast of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(Print 81 37 #at -1 20 #width 260 #time 10) ; "The henchmen have taken your rose and you no longer have the gold key! It looks like you're destined to be...Mrs. Edgar!"
