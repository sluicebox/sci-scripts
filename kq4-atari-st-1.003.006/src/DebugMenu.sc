...
(instance DebugMenu of Script
	...
	(method (doit param1 &tmp temp0 [temp1 6] [temp7 33])
		(switch param1
			...
			(1544
				(= global300 0)
				(GetInput @global300 8 {Inv. Object})
				(= global111 (ReadNumber @global300))
				(= global300 0)
				(GetInput @global300 12 {Owner})
				(if (not (StrCmp {ego} @global300))
					((gInventory at: global111) moveTo: gEgo)
				else
					((gInventory at: global111) moveTo: (ReadNumber @global300))
				)
				(= global300 0)
			)
			(1545
				(= global300 0)
				(GetInput @global300 8 {Variable No.})
				(= global111 (ReadNumber @global300))
				(= global300 0)
				(GetInput @global300 8 {Value})
				(= [gNight (- global111 100)] (ReadNumber @global300))
				(= global300 0)
			)
			(1546
