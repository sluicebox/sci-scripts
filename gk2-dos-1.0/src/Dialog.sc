...
(class Dialog of Set
	...
	(method (doit def &tmp event ret done checked)
		...
		(while (not ret)
			...
			(event dispose:)
			(if (or (= checked (self check:)) (== modeless 2))
				(if (and checked (== modeless 2))
					(self dispose:)
				)
				(break)
			)
	...
	(method (advance &tmp obj node)
		(if theItem
			...
			(repeat
				...
				(= theItem (KList 8 node)) ; NodeValue
	...
	(method (move h v)
		...
		(if onScreen
			...
			(if (> nsRight gLastScreenX)
				(+= nsLeft (- gLastScreenX nsRight))
				(= nsRight gLastScreenX)
			)
			(if (< nsTop 0)
				(+= nsBottom (- 0 nsTop))
				(= nsTop 0)
			)
			(if (> nsBottom gLastScreenY)
				(+= nsTop (- gLastScreenY nsBottom))
				(= nsBottom gLastScreenY)
			)
		)
		(if plane
