...
(procedure (localproc_0)
	(switch gChapter
		...
		(3
			...
			(if (and (IsFlag 518) (IsFlag 515) (gEgo has: 34)) ; invGrosPhoneNumber
				(= local4 1)
			)
		)
		(5
...
(class MapButton of DButton
	...
	(method (doit)
		(if (== (GKHotCursor curHotObject:) self)
			(self setFlash: 1)
			(= doneOnce 0)
			(gTopMessage newText: modNum noun 0)
		else
			(if (not doneOnce)
				(self setHintFlash:)
				(= doneOnce 1)
			)
			(if (not (gCurRoom pHintMode:))
				(self setFlash: 0)
				(if pHilite
					(self hilite: 1)
				)
...
(class MapRoom of GK2Room
	...
	(method (hintMode)
		...
		(if pHintMode
			...
			(myList eachElementDo: #setHintFlash)
		else
	...
	(method (init &tmp [temp0 2])
		...
		(proc11_13)
		(myList init:)
		((gInterfacePlane doitList:) delete: (ScriptID 0 4)) ; inventoryButton
		((gInterfacePlane eventList:) delete: (ScriptID 0 4)) ; inventoryButton
	...
	(method (doit)
		(if (not (GKHotCursor curHotObject:))
	...
	(method (dispose)
		((gInterfacePlane doitList:) delete: (ScriptID 0 6)) ; mapHintButton
		((gInterfacePlane eventList:) delete: (ScriptID 0 6)) ; mapHintButton
		(if (or (< gChapter 5) (and (== gChapter 5) (gEgo has: 6))) ; invTapeRecorder
...
(instance buttonList of List
	...
	(method (init)
		...
		(if (IsFlag 26) ; "Sample Analysis" in rm4220
