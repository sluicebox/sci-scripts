...
(instance swimAndShoot of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(if local2
					(EgoDead 411 0) ; "** Hint: hitting a moving target is more difficult than hitting a LUMP!"
				else
					(client dispose:)
				)
			)
...
(instance egoFollowed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead 411 1) ; "Were you looking for trouble? Maybe you thought you could beat them senseless with your pink bunny rabbit?"
			)
		)
	)
)
