...
(instance ropeScript of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(3
				(gEgo setMotion: 0 view: 23 setLoop: (+ register 2) cel: 0)
				(theRope
					init:
					setLoop: register
					cel: 0
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(gEgo get: 3) ; rabbit
				(gEgo put: 1 0) ; rope
				(Print 705 3) ; "** You discard the worn rope as it has frayed to uselessness."
				(ropeScript start: 10)
				(gCurRoom setScript: ropeScript)
			)
		)
		...
		(switch theVerb
			(4 ; Do
				(if
					...
					(Print 705 4) ; "The hare, anticipating your clumsy attempt to catch it barehanded, won't come anywhere near you."
				)
			)
			(5 ; Talk
				...
				(switch invItem
					(1 ; rope
						(cond
							...
							((IsFlag 0)
								(Print 705 6) ; "*** You don't have time for that now!"
							)
							((& (gEgo onControl: 1) $0020)
								(Print 705 7) ; "There's not enough room for that here!"
							)
							((not (gCurRoom script:))
								(HandsOff)
								(gCurRoom setScript: ropeScript)
