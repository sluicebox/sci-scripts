...
(local
	...
	local5
	local6
	[local7 8] = [161 189 186 154 120 91 87 118]
...
(instance fightScript of Script
	...
	(method (init)
		...
		(gGameControls addToFront: self addToFront: (ScriptID 150 1)) ; RogerJr
		(= local1 1)
		(= local5 0)
		(= local6 4)
	)
	...
	(method (dispose)
		...
		(gGameControls delete: self delete: (ScriptID 150 1)) ; RogerJr
		(super dispose:)
	)
	...
	(method (handleEvent event &tmp temp0 temp1)
		(if local0
			(switch (event type:)
				...
				($0040 ; direction
					(if (and (== (gEgo view:) 526) (not (gEgo mover:)))
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(brain
					makePolygon: 127 101 121 103 112 96 111 91 111 87 116 85 121 78 131 75 147 75 156 80 160 88 158 95 149 100 142 102
				)
				(= cycles 6)
			)
			(2
			...
			(3
				(Print 151 2) ; "C'mon, ya yellow-bellied cheese head!"
				(= cycles 1)
			)
			(4
				(gEgo
