...
(local
	...
	local4
	local5
	[local6 25] = [5 0 124 82 5 1 116 90 5 2 110 93 5 3 112 95 5 4 112 95 5 5 113 95 -32768]
	[local31 37] = [6 0 295 110 6 1 277 113 6 2 265 118 6 3 258 132 6 4 259 132 6 5 259 132 6 6 259 132 6 7 259 132 6 8 259 132 -32768]
	[local68 17] = [1 0 131 152 1 1 131 150 1 2 131 148 1 0 131 146 -32768]
	[local85 17] = [3 1 135 152 3 2 135 150 3 3 135 148 3 4 135 146 -32768]
)
...
(procedure (localproc_0 param1 param2 param3 param4 &tmp temp0 temp1) ; UNUSED
	...
	(= local4 temp1)
	(= local5 param4)
	(= local0 (Graph grSAVE_BOX temp0 (- param2 1) temp1 (+ param4 1) 1))
	(Graph grDRAW_LINE param1 param2 param3 param4 global134 -1 -1)
	(Graph grREDRAW_BOX temp0 (- param2 1) temp1 (+ param4 1))
)

(procedure (localproc_1) ; UNUSED
	(Graph grRESTORE_BOX local0)
	(Graph grREDRAW_BOX local2 (- local3 1) local4 (+ local5 1))
)

(procedure (localproc_2)
	(Display 320 12 dsRESTOREPIXELS local1)
)
...
(instance rm320 of SQRoom
	...
	(method (init)
		...
		(LoadMany rsSOUND 143 51 132 133)
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 319 189 0 189 0 0 165 0 242 123 208 148 174 145 171 133 142 144 191 158 193 170 297 146 315 132 310 116 294 115 295 110 304 106 298 94 310 90 299 59 303 52 298 49 303 47 308 38 319 29
					yourself:
				)
				((Polygon new:)
					type: PNearestAccess
		...
		(if (or (and (== gPrevRoomNum north) (butte egoSwims:)) (== gPrevRoomNum 297))
			...
			(self style: 42 setScript: enterScript)
		)
		(gEgo setPri: 8 init:)
		(super init:)
		(if
		...
		(switch (= state newState)
			...
			(2
				(gEgo
					...
					setCycle: End self
					xStep: 2
					setMotion: MoveTo 185 (gEgo y:)
				)
			)
			(3
				(NormalEgo 0 0)
				(gEgo illegalBits: $8010 setPri: 8)
				(gEgo
					xStep: 3
					yStep: 2
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(babe2 init: setCycle: MCyc @local6)
				(babe3 init: setCycle: MCyc @local31 self)
			)
			(1
				(= cycles 10)
			)
			...
			(3
				(sub setCycle: MCyc @local68)
				(subTop init: setCycle: MCyc @local85 self)
			)
			(4
				(sub setCycle: Fwd)
				(= cycles 10)
			...
			(12
				(eyes setCel: 1)
				(mouth loop: 2 setCycle: RandCycle 30)
				(self
					save1:
						(proc0_12
			...
			(35
				...
				(DrawPic 320 30)
				(babeLeader loop: 0 cel: 0 posn: 117 144)
				(mouth dispose:)
				(arm dispose:)
				(eyes dispose:)
				(= cycles 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(EgoDead 320 0) ; "OW! Now this is a pain that'll linger! All the Ben Gay in Florida won't relieve this stiffness."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(babeLeader loop: 4 cel: 0 posn: 130 144 setCycle: End self)
			)
			(4
				(babeLeader dispose:)
			...
			(6
				(gLongSong number: 51 loop: -1 play:)
				(subTop setCycle: Beg)
				(subTop dispose:)
...
(instance babeLeader of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(Print 320 1) ; "Hey! Would you like your chest cavity to host her spear?"
			)
			(5 ; Talk
				(Print 320 2) ; "The time for talk on you're part appears to have passed. Speak now and forever hold your pieces."
			)
			(2 ; Smell
				(Print 320 3) ; "You can bet she smells better than you. Better just speculate on this one or you'll be snorting spear."
			)
			(3 ; Taste
				(Print 320 4) ; "Best just keep your buds packed, Buckwheat. Besides, you should be worried about dying now - not thinking about stuff like that."
			)
			(else
...
(instance babe2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(Print 320 5) ; "Hey! You must want to know how you look with convenient handles protruding from each side of your less-than-studly self."
			)
			(5 ; Talk
				(Print 320 2) ; "The time for talk on you're part appears to have passed. Speak now and forever hold your pieces."
			)
			(2 ; Smell
				(Print 320 6) ; "This would be an excellent time to keep your nostrils to yourself."
			)
			(3 ; Taste
				(Print 320 7) ; "Gee! Maybe you should walk right on up to her and give it a try."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance babe3 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(Print 320 8) ; "Do it and die."
			)
			(5 ; Talk
				(Print 320 9) ; "She look completely uninterested in conversing with the likes of you."
			)
			(2 ; Smell
				(Print 320 6) ; "This would be an excellent time to keep your nostrils to yourself."
			)
			(3 ; Taste
				(Print 320 10) ; "Yeah! Right!"
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance sub of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(5) ; Talk
			(2 ; Smell
				(Print 320 11) ; "This isn't a good time for your interesting vessel-sniffing hobby."
			)
			(3 ; Taste
				(Print 320 7) ; "Gee! Maybe you should walk right on up to her and give it a try."
			)
