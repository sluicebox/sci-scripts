...
(local
	...
	[local277 24] = [3500 100 73 2505 140 71 3510 182 72 3525 187 102 2525 189 114 3515 93 114 2515 73 125 1515 92 119]
)
...
(instance rm545 of SQRoom
	...
	(method (init &tmp temp0 temp1 temp2 temp3)
		...
		(if (== (++ global162) 3)
			...
			(blip3 view: 506 setLoop: 5 x: temp2 y: temp3 init: setCycle: Osc)
			(level1 init:)
			(level2 init:)
			(level3 init:)
			(powerButton init:)
	...
	(method (doit &tmp [temp0 40] temp40 temp41 temp42 temp43 temp44 temp45)
		...
		(if (and local1 (not (-- local1)))
			(Print 545 0) ; "While you were busy staring blankly at your PocketPal screen, a droid clumsily snuck up to you and tapped you on the shoulder. "Zap," it said in its droning metallic voice, "you're it.""
			(= gQuit 1)
		)
...
(instance keyboard of Feature
	...
	(method (doVerb theVerb)
		(if (OneOf theVerb 6 4) ; Inventory, Do
			(Print 545 1) ; "They work as well as they look."
		else
...
(instance blip1 of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(5 ; Talk
				(Print 545 2) ; "Maybe if you wait a while, the nice droid will come around to where you are and then maybe it'll talk to you."
			)
			(1 ; Look
				(if ((ScriptID 704 1) mover:) ; droid1
					(Print lookStr)
				else
					(Print 545 3) ; "It seems to have stopped, as if it were... waiting for something."
				)
			)
			(else
				(super doVerb: theVerb &rest)
...
(instance blip3 of Prop
	...
	(method (doVerb theVerb)
		(if (== theVerb 5) ; Talk
			(Print 545 4) ; "You talk to yourself, but you don't hear you."
		else
			(super doVerb: theVerb &rest)
		)
	)
...
(instance vohaulScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 0 {Take a good look, Roger:})
				(mouth setCycle: ForwardCounter 7)
				(eyes setCycle: ForwardCounter 4)
				(= seconds 3)
			)
			(2
				(proc0_12 local0)
			...
			(3
				(localproc_0 0 {Remember this poor wretched soul...})
				(= seconds 5)
			)
			(4
				(proc0_12 local0)
				(gCurRoom overlay: 546)
				(localproc_0 0 {...for he is your SON!})
				(mouth show: setCycle: ForwardCounter 7)
				(eyes show: setCycle: ForwardCounter 4)
				(= seconds 3)
			)
			(5
				(gCurRoom newRoom: (brain lastRoom:))
