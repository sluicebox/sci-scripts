...
(instance droidPath1 of RegionPath
	...
	(method (nextRoom &tmp temp0)
		...
		(super nextRoom: &rest)
		(if (not (= temp0 (localproc_2 client)))
		else
			(localproc_4 temp0)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if register
					(cond
						((not (== ((gInventory at: 12) owner:) 1)) ; plug
						...
						((!= ((gInventory at: 12) state:) global170) ; plug
