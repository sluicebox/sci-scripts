;;; Sierra Script 1.0 - (do not remove this comment)
;;; Decompiled by sluicebox
(script# 530)
(include sci.sh)
(use Main)
(use SQRoom)
(use PolyPath)
(use Polygon)
(use Feature)
(use LoadMany)
(use StopWalk)
(use Sound)
(use Motion)
(use Actor)
(use System)

(public
	rm530 0
)

(local
	local0
	[local1 48] = [131 40 143 42 157 41 168 36 175 30 180 23 184 16 255 15 260 24 262 50 248 63 240 64 224 79 219 79 203 80 187 80 187 79 172 79 171 80 146 80 146 78 123 78 129 52 124 37]
)

(procedure (localproc_0 param1 param2 param3 param4 &tmp temp0 temp1 temp2)
	(aSound number: 105 loop: 1 vol: 127 play:)
	(if (< param1 param3)
		(= temp0 (- param1 1))
		(= temp1 (+ param3 1))
	else
		(= temp0 (- param3 1))
		(= temp1 (+ param1 1))
	)
	(= temp2 (Graph grSAVE_BOX temp0 (- param2 1) temp1 (+ param4 1) 1))
	(Graph grDRAW_LINE param1 param2 param3 param4 global143 -1 -1)
	(Graph grREDRAW_BOX temp0 (- param2 1) temp1 (+ param4 1))
	(Wait 1)
	(Wait 4)
	(Graph grRESTORE_BOX temp2)
	(Graph grREDRAW_BOX temp0 (- param2 1) temp1 (+ param4 1))
)

(instance rm530 of SQRoom
	(properties
		picture 530
		east 535
		vanishingX 319
		vanishingY -60
	)

	(method (init)
		(HandsOff)
		(LoadMany rsVIEW 0 378 530 300)
		(LoadMany rsSOUND 124 125 131)
		(gCurRoom
			addObstacle:
				((Polygon new:)
					type: PBarredAccess
					init: 257 172 270 152 319 152 319 172
					yourself:
				)
				((Polygon new:)
					type: PBarredAccess
					init: 31 170 0 170 0 0 319 0 319 52 267 52 254 65 246 67 230 83 133 83 118 81 71 114 77 117 111 112 144 119 158 126 181 114 205 115 228 134 216 140 177 140 143 151 128 150 119 138 85 149 69 150 60 145
					yourself:
				)
				((Polygon new:)
					type: PTotalAccess
					init: 167 165 167 189 127 189 127 166
					yourself:
				)
		)
		(timePod0 init:)
		(terminal init:)
		(switch gPrevRoomNum
			(531
				(self
					setScript:
						(if (IsFlag 2)
							egoArrives
						else
							(if (not (IsFlag 28))
								(LoadMany rsVIEW 7 13)
								(Load rsSOUND 105)
								(sp1 loop: 2 init:)
								(sp2 view: 7 loop: 2 x: 185 y: 91 init:)
							)
							(gEgo
								view: 378
								loop: 0
								cel: 0
								posn: 188 149
								setPri: 11
								ignoreActors:
								illegalBits: 0
								cycleSpeed: 1
								normal: 0
								moveHead: 0
								init:
							)
							egoExits
						)
				)
			)
			(else
				(LoadMany rsVIEW 7 13)
				(Load rsSOUND 105)
				(self style: 40)
				(if (IsFlag 1)
					(HandsOff)
					(hatch init:)
					(pod init:)
					(if (not (IsFlag 28))
						(sp1 loop: 0 init:)
						(sp2 view: 7 loop: 0 x: 185 y: 91 init:)
					)
					(self setScript: enterScript)
				else
					(sp1 init:)
					(gEgo x: 284 y: 130 init:)
					(self setScript: policeArrive)
				)
			)
		)
		(SetFlag 1)
		(theRedBall init:)
		(theKnobs init:)
		(theCityView init:)
		(theRailView1 init:)
		(theRailView2 init:)
		(building init:)
		(super init:)
		(buildingPoly points: @local1 size: 24)
	)

	(method (doit)
		(super doit: &rest)
		(cond
			((StepOn gEgo 4)
				(HandsOff)
				(gCurRoom setScript: fallScript)
			)
			((> (gEgo x:) 300)
				(HandsOff)
				(gCurRoom setScript: exitScript)
			)
		)
	)
)

(instance enterScript of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(cond
					((not (IsFlag 28))
						(gEgo y: 130)
					)
					((> (gEgo y:) 152)
						(gEgo y: 152)
					)
				)
				(= cycles 1)
			)
			(1
				(gEgo
					x: 282
					init:
					setCycle: StopWalk 4
					setMotion: MoveTo 255 (gEgo y:) self
				)
				(AnimateEgoHead)
			)
			(2
				(if (IsFlag 28)
					(HandsOn)
					(client setScript: 0)
				else
					(policeArrive start: 16)
					(client setScript: policeArrive)
				)
			)
		)
	)
)

(instance exitScript of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(gCurRoom newRoom: 535)
			)
		)
	)
)

(instance fallScript of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(aSound number: 131 vol: 127 loop: 1 play:)
				(gEgo
					view: 300
					setLoop:
					setCycle: 0
					setStep: 3 20
					setPri:
					illegalBits: 0
					setMotion: MoveTo (gEgo x:) 250 self
				)
			)
			(1
				(= seconds 2)
			)
			(2
				(aSound fade: 0 1 2 1)
				(= seconds 1)
			)
			(3
				(EgoDead 530 0)
			)
		)
	)
)

(instance egoArrives of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(= cycles 8)
			)
			(1
				(sparks init: setCycle: Fwd)
				(pod cel: 0 init: setCycle: CT 6 1 self)
			)
			(2
				(= cycles 12)
			)
			(3
				(sparks dispose:)
				(= cycles 1)
			)
			(4
				(gCurRoom newRoom: 531)
			)
		)
	)
)

(instance policeArrive of Script
	(properties)

	(method (init)
		(super init: &rest)
		(= register 200)
	)

	(method (doit)
		(super doit: &rest)
		(if (> state 11)
			(-- register)
		)
		(if (or (not register) (and (< (gEgo y:) 128) (< state 16)))
			(= register -1)
			(= state 15)
			(self cue:)
		)
	)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(= cycles 8)
			)
			(1
				(sp1 setHeading: 90)
				(sparks init: setCycle: Fwd)
				(pod cel: 0 init: setCycle: CT 6 1 self)
			)
			(2
				(sp1 setHeading: 180)
				(hatch init: cel: 0)
				(pod setCycle: End)
				(= cycles 12)
			)
			(3
				(sp1 stopUpd:)
				(sparks dispose:)
				(pod stopUpd:)
				(= cycles 1)
			)
			(4
				(gLongSong2 number: 124 loop: 1 vol: 127 play:)
				(hatch setCycle: End self)
				(sp2 view: 378 loop: 2 cel: 0 x: 198 y: 148 init:)
			)
			(5
				(hatch stopUpd:)
				(sp2 cycleSpeed: 1 setCycle: End self)
			)
			(6
				(= cycles 2)
			)
			(7
				(sp2
					view: 7
					loop: 0
					x: (+ (sp2 x:) 13)
					y: (+ (sp2 y:) 5)
					cycleSpeed: 0
					setCycle: Walk
					setMotion: PolyPath 185 91 self
				)
			)
			(8
				(HandsOn)
				(sp1 setHeading: 90 self)
				(gEgo setMotion: MoveTo 262 130)
			)
			(9
				(sp1 stopUpd:)
				(sp2 setHeading: 270 self)
			)
			(10
				(sp2 stopUpd:)
				(= local0
					(proc0_12
						{"I have just completed\na scan of the Labion\nsector of Space Quest II."}
						67
						178
						5
						28
						global135
						29
						global129
					)
				)
				(= cycles 50)
			)
			(11
				(proc0_12 local0)
				(= local0
					(proc0_12
						{"No sign of presence at\nthis time."}
						67
						178
						15
						28
						global135
						29
						global129
					)
				)
				(= cycles 50)
			)
			(12
				(HandsOn)
				(proc0_12 local0)
				(= local0
					(proc0_12
						{"Ghraa... slooh... uuhrp."}
						67
						25
						25
						28
						global132
						29
						global129
					)
				)
				(= cycles 50)
			)
			(13
				(proc0_12 local0)
				(= local0
					(proc0_12
						{"Mrghl... pifhh... zaaft."}
						67
						178
						25
						28
						global135
						29
						global129
					)
				)
				(= cycles 50)
			)
			(14
				(proc0_12 local0)
				(= local0
					(proc0_12
						{"Jobhr... fhelz... klogd."}
						67
						25
						25
						28
						global132
						29
						global129
					)
				)
				(= cycles 50)
			)
			(15
				(proc0_12 local0)
			)
			(16
				(HandsOff)
				(Face sp1 gEgo)
				(= cycles 3)
			)
			(17
				(Face sp2 gEgo)
				(proc0_12 local0)
				(sp1 view: 13 cel: 0 setCycle: End self)
			)
			(18
				(if (< (sp1 x:) (gEgo x:))
					(localproc_0
						(- (sp1 y:) 27)
						(sp1 x:)
						(- (gEgo y:) 32)
						(gEgo x:)
					)
				else
					(localproc_0
						(- (gEgo y:) 32)
						(gEgo x:)
						(- (sp1 y:) 27)
						(sp1 x:)
					)
				)
				(gEgo
					view: 26
					loop:
						(switch (gEgo loop:)
							(4 0)
							(5 1)
							(6 0)
							(7 1)
							(else
								(gEgo loop:)
							)
						)
					cel: 0
					cycleSpeed: 0
					setCycle: CT 2 1 self
				)
			)
			(19
				(gEgo setCycle: CT 1 -1 self)
			)
			(20
				(gEgo setCycle: End self)
			)
			(21
				(= seconds 2)
			)
			(22
				(= local0
					(proc0_12 {"HALT."} 67 85 25 28 global132 29 global129)
				)
				(= cycles 30)
			)
			(23
				(proc0_12 local0)
				(= local0
					(proc0_12
						{"Some people just\nwon't follow instructions."}
						67
						25
						25
						28
						global132
						29
						global129
					)
				)
				(= cycles 50)
			)
			(24
				(proc0_12 local0)
				(EgoDead)
			)
		)
	)
)

(instance egoExits of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(pod setCel: 7 init: stopUpd:)
				(gLongSong2 number: 124 loop: 1 vol: 127 play:)
				(hatch init: setCycle: End self)
			)
			(1
				(= cycles 4)
			)
			(2
				(gEgo setCycle: End self)
			)
			(3
				(gEgo loop: 1 normal: 1 moveHead: 1 view: 0)
				(NormalEgo)
				(= cycles 1)
			)
			(4
				(if (IsFlag 28)
					(HandsOn)
					(client setScript: 0)
				else
					(policeArrive start: 16)
					(client setScript: policeArrive)
				)
			)
		)
	)
)

(instance egoEntersPod of Script
	(properties)

	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(gEgo setMotion: PolyPath 188 150 self)
			)
			(1
				(= global57 0)
				(gEgo
					view: 378
					loop: 1
					cel: 0
					setPri: 11
					posn: 191 126
					ignoreActors:
					illegalBits: 0
					cycleSpeed: 2
					normal: 0
					moveHead: 0
					setCycle: End self
				)
			)
			(2
				(= cycles 4)
			)
			(3
				(hatch init: cel: 5 setCycle: Beg self)
			)
			(4
				(= global57 1)
				(gCurRoom newRoom: 531)
			)
		)
	)
)

(instance pod of Prop
	(properties
		x 127
		y 129
		description {time pod}
		lookStr {You give the Time Pod a long glance. It's not the most attractive vehicle you've ever seen. It resembles an overgrown titanium tennis shoe.}
		view 530
		loop 1
		cel 7
		priority 10
		signal 16400
		cycleSpeed 2
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gCurRoom setScript: egoEntersPod)
			)
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance hatch of Prop
	(properties
		x 162
		y 113
		view 530
		loop 2
		cel 9
		priority 12
		signal 16400
	)
)

(instance sparks of Prop
	(properties
		x 145
		y 108
		view 530
		priority 13
		signal 16400
	)
)

(instance sp1 of Actor
	(properties
		x 156
		y 91
		description {secret police}
		lookStr {** you look, but by staring may draw some unwanted attention.}
		view 7
		loop 3
		signal 16384
	)
)

(instance sp2 of Actor
	(properties
		description {secret police}
		lookStr {** you look, but by staring may draw some unwanted attention.}
		signal 16384
	)
)

(instance terminal of Feature
	(properties
		x 159
		y 61
		heading 180
		nsTop 43
		nsLeft 148
		nsBottom 80
		nsRight 170
		description {terminal}
		sightAngle 150
		lookStr {This is the Sequel Police dispatch monitor. At the moment nothing is being displayed.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance theRedBall of Feature
	(properties
		x 147
		y 141
		z 120
		nsBottom 200
		nsRight 320
		description {red sphere}
		onMeCheck 16
		lookStr {Much like myself, you must be curious as to the identity of and the purpose for the large, red sphere.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance theKnobs of Feature
	(properties
		nsBottom 200
		nsRight 320
		description {knobs}
		sightAngle 359
		onMeCheck 128
		lookStr {You can't actually see those from where you're standing.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance theRailView1 of Feature
	(properties
		x 59
		y 68
		nsBottom 200
		nsRight 320
		description {rail view}
		onMeCheck 32
		lookStr {The view beyond the railing reveals the mess of pipes and beams which this structure is composed of.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance theRailView2 of Feature
	(properties
		x 270
		y 33
		nsBottom 200
		nsRight 320
		description {rail view}
		sightAngle 180
		onMeCheck 64
		lookStr {The view beyond the railing reveals the mess of pipes and beams which this structure is composed of.}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance theCityView of Feature
	(properties
		x 153
		y 89
		heading 179
		nsTop 166
		nsBottom 189
		nsRight 319
		description {city view}
		sightAngle 90
		lookStr {From this vantage point high above Xenon you can truly appreciate the scope of the destruction. What a mess!}
	)

	(method (doVerb theVerb)
		(switch theVerb
			(4) ; Do
			(5) ; Talk
			(else
				(super doVerb: theVerb)
			)
		)
	)
)

(instance timePod0 of Feature
	(properties
		description {Time Pod}
		lookStr {You give the Time Pod a long glance. It's not the most attractive vehicle you've ever seen. It resembles an overgrown titanium tennis shoe. A gold-tinted glass shield seals the top.}
	)

	(method (onMe param1)
		(return (& (OnControl CONTROL (param1 x:) (param1 y:)) $0002))
	)
)

(instance building of Feature
	(properties
		description {dispatch building}
		sightAngle 90
		lookStr {** dispatch building lookStr}
	)

	(method (init)
		(super init: &rest)
		(self onMeCheck: buildingPoly)
	)
)

(instance buildingPoly of Polygon
	(properties)
)

(instance aSound of Sound
	(properties)
)

