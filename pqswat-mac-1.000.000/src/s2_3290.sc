	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gMessager say: 0 0 2 0) ; "Pacmeyer to Twenty David - Two down! Suspects in control!"
			)
			(2
				(SetFlag 93)
				(SetFlag 98)
...
(instance sFlashBangPlot2 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
				...
				(if (not local0)
					...
					(cond
						((== (global112 curColIdx:) 15)
							(= global128 32909)
							(WalkieTalkie showFrame: 32909 0 240 70)
						)
						((>= (global112 curColIdx:) 16)
							(= global128 32910)
							(WalkieTalkie showFrame: 32910 0 136 43)
						)
					)
				)
				(= cycles 1)
			...
			(4
				(if local0
					(gMessager say: 0 16 11 0 self) ; "Officer coming out! You're outta here."
				else
					(Palette 2 42 254 150) ; PalIntensity
					(FrameOut)
					(= cycles 1)
