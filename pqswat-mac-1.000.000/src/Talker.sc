...
(class Narrator of Obj
	...
	(method (init)
		(if (& gMsgType $0002)
			...
			(gGame masterVolume: (- curVolume 13))
		)
		(= initialized 1)
		(super init:)
			...
			(else
				...
				(if
					(and
						...
						(or (not global432) (global432 doit: event))
					)
					(event claimed: 1)
	...
	(method (say param1 param2)
		...
		(+= ticks (+ (if gLashInterface 10 else 60) gGameTime))
		(return 1)
	)
	...
	(method (startAudio theKeys)
		...
		(= ticks
			(DoAudio audPLAY audModNum audNoun audVerb audCase audSequence 0 127)
		)
	)
