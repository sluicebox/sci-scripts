...
(class Plane of Obj
	...
	(method (dispose)
		...
		(= bitmapCast (= theFtrs (= casts (= bitmap 0))))
		(gPlanes delete: self)
		(DeletePlane self)
		(super dispose:)
	)
	...
	(method (setBitmap v l c scale &tmp temp0 temp1 temp2)
		(= temp2 0)
		(if bitmap
			(bitmap dispose:)
			(= bitmapCast (self findCast: {planeBM}))
		else
			((= bitmapCast (Cast new:)) name: {planeBM})
			(self addCast: bitmapCast)
		)
		(if (< argc 3)
			((= bitmap (v new:)) posn: 0 0 init: bitmapCast)
			(if (> argc 1)
				(= temp2 l)
			)
		else
		...
		(= temp0 (CelWide (bitmap view:) (bitmap loop:) (bitmap cel:)))
		(= temp1 (CelHigh (bitmap view:) (bitmap loop:) (bitmap cel:)))
		(if temp2
			(self scaleBitmap:)
		else
