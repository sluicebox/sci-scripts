...
(class View of Feature
	...
	(method (setScaleDirect scaleParm)
		(self setScale:)
		(if argc
			(= scaleY (= scaleX scaleParm))
		)
	)

	(method (setScalePercent scaleParm)
		(self setScale:)
		(if argc
			(= scaleX (MulDiv scaleParm 128 100))
			(= scaleY scaleX)
		)
	)
...
(class Actor of Prop
	...
	(method (setMotion mType &tmp nHeading)
		...
		(if mType
			...
			(if
				(and
					looper
					(looper isMemberOf: Grooper)
					cycler
					(cycler isMemberOf: Grycler)
				)
	...
	(method (isStopped)
		(return (or (not mover) (& signal $0400)))
	)
	...
	(method (approach oFtr oCuee)
		...
		(cond
			(
				(>
					(GetDistance x y (oFtr approachX:) (oFtr approachY:))
					(oFtr approachDist:)
				)
				(self
					setMotion:
	...
	(method (approachThenFace oFtr oCuee)
		(if (< argc 2)
			(= oCuee 0)
		)
		(if (== self oFtr)
		...
		(= facer (FaceCue new:))
		(facer whoToTurn: self whatToFace: oFtr whoToCue: oCuee)
		(self approach: oFtr facer)
	)

	(method (face oFtr oCuee)
		(if (< argc 2)
			(= oCuee 0)
		)
		(if (== self oFtr)
		...
		(self setHeading: (GetAngle x y (oFtr x:) (oFtr y:)) oCuee)
	)
