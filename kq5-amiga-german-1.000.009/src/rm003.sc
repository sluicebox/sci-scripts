...
(instance drownHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(proc0_28 75 3 0 67 20 20) ; "Wait, Graham! Be careful...!"
				(gEgo
					normal: 0
					view: 48
					setPri: 15
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (not (IsFlag 29))
					(SetFlag 29)
					(proc0_28 75 3 1) ; "If you go on into town I'll wait for you here. I had a nasty run-in with a dog once and I'd feel safer out here."
					(= cycles 1)
				else
					(= cycles 1)
				)
