	...
	(method (handleEvent event &tmp temp0 temp1 temp2 [temp3 3])
		(if
			(and
				...
				(not
					(and (& (event type:) evMOUSEBUTTON) (== (gCurRoom inset:) Walkman))
				)
			)
			(repeat
		...
		(cond
			...
			(
				(and
					(BAD_SELECTOR sel_732:)
					(or
						(< (event x:) (BAD_SELECTOR sel_726:))
						(> (event x:) (BAD_SELECTOR sel_727:))
						(< (event y:) (BAD_SELECTOR sel_724:))
						(> (event y:) (BAD_SELECTOR sel_725:))
					)
					(or
						(< (event x:) (BAD_SELECTOR sel_731:))
						(> (event x:) (BAD_SELECTOR sel_730:))
						(< (event y:) (BAD_SELECTOR sel_728:))
						(> (event y:) (BAD_SELECTOR sel_729:))
					)
				)
				(event claimed: 1)
				(return)
