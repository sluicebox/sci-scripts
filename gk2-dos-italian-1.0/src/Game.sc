...
(class Game of Obj
			...
			(-2
				...
				(if
					(Print
						...
						back: (Palette 3 127 127 127) ; PalMatch
						addText: buf1
						addButtonBM: 64990 0 0 0 buf2 0 96
						addButtonBM: 64990 2 0 1 {Change Dir} 110 96
						init:
					)
					(GetDirectory gCurSaveDir)
				)
		...
		(if (!= (= num (Restore doit: &rest)) -1)
			...
			(if (Save 3 name num (KString 9 gVersion)) ; CheckSaveGame, StrGetData
				...
				(Print
					...
					back: (Palette 3 127 127 127) ; PalMatch
					addText: (buf1 data:)
					addButtonBM: -1 0 0 1 (buf2 data:) 0 96
					init:
				)
				(self setCursor: oldCur (HaveMouse))
			)
	...
	(method (getDisc n &tmp temp0 temp1 temp2)
		(= gNumCD n)
		(while (and (!= -1 (= temp0 (CD 0 n))) (!= temp0 n)) ; Check
			(switch (gGame printLang:)
				(351
					(= temp1
						(Str
							format:
								{Por favor, insira o CD %d e pressione <ENTER>}
								n
						)
					)
				)
				(39
					(= temp1
						(Str
							format: {Inserisci il CD %d e premi <ENTER>} n
						)
					)
				)
				(else
					(= temp1
						(Str
							format:
								{Please insert CD #%d then press <ENTER>}
								n
						)
					)
				)
