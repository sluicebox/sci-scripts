...
(class TopMessage of PanelText
	...
	(method (dispose)
		(if (and BAD_SELECTOR (!= BAD_SELECTOR bitmap))
			(Bitmap 1 BAD_SELECTOR) ; Dispose
		)
		(= BAD_SELECTOR 0)
		(super dispose: &rest)
	)
	...
	(method (save param1)
		...
		(= BAD_SELECTOR (if (or (not argc) param1) bitmap else 0))
	)

	(method (setBitmap)
		(if (== BAD_SELECTOR bitmap)
			(= bitmap 0)
		)
			...
			(351
				(= gQuit
					(Print
						...
						back: 4
						fore: 14
						addTitle: {Obrigado por jogar!}
						addText: {Tem certeza que deseja sair?} 0 3
						addButton: 1 { Si } 55 20
			...
			(39
				(= gQuit
					(Print
						...
						back: 4
						fore: 14
						addTitle: {Grazie per aver giocato!}
						addText: {Sei sicuro di voler uscire?} 0 3
						addButton: 1 { Si } 55 20
						addButton: 0 { No } 95 20
						init:
					)
