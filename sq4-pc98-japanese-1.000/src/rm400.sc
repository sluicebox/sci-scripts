...
(instance lator1 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 1) ; "This escalator carries weary, destitute shoppers back to their vehicles stowed in the parking garage below."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance lator2 of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 2) ; "This escalator carries excited, wealthy shoppers up from the parking garage below."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cigar of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 3) ; "It's a cigar, but not much more than a butt."
			)
			(3 ; Do
				(HandsOff)
				(proc700_5 1)
			...
			(11 ; Smell
				(Print 400 4) ; "Geeze! You wonder what pachyderm-scented grease loaf was kind enough to contribute this used cigar to the cuff-level decor."
			)
			(10 ; Taste
				(Print 400 5) ; "You don't know this cigar's history. But, you better not try it."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance card of Prop
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 6) ; "It's an AutoBucks Teller Machine card."
			)
			(3 ; Do
				(HandsOff)
				(proc700_5 1)
...
(instance getSomethingScript of Script
	...
	(method (doit)
		(super doit:)
		(if (& (gEgo signal:) $0400)
			(Print 400 7) ; "This geek seems to have gotten in your way."
			(HandsOn)
			(self dispose:)
		)
	)
	...
	(method (changeState newState &tmp temp0 temp1 temp2)
		(switch (= state newState)
			...
			(1
				(switch register
					(card
						(Print 400 8) ; "You pick up the ATM Card."
						(gEgo get: 10) ; atmCard
						(SetScore 80 2)
					)
					(cigar
						(Print 400 9) ; "You retrieve the cigar butt."
						(gEgo get: 12) ; cigar
						(SetScore 81 5)
					)
				)
...
(instance joyRiding of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 400 10) ; "Wasn't much fun, was it?"
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance skateorama of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 11) ; "It's a Zero-G Skate-O-Rama. The skaters look to be having fun."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance island1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 12) ; "They're just islands between the beltways."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance belts of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 13) ; "These convenient beltways move shoppers around the mall, allowing them to save energy to spend more money."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bush1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 400 14) ; "This is a fine example of the wonders of fabricated flora."
			)
			(11 ; Smell
				(Print 400 15) ; "The fabricated flora has an equally realistic and artificial smell."
			)
			(10 ; Taste
				(Print 400 16) ; "You know that eating plastic plants places pressure on your plumbing. You learned that reading JANITORIAL DIGEST."
			)
			(else
				(super doVerb: theVerb &rest)
			)
