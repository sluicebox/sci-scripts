...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] str)
	(= str (Memory memALLOC_CRIT (if 81 200 else 80)))
	(= ret 1)
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= num (Save doit: @comment)) -1)
				...
				(if (not (SaveGame name num @comment gVersion))
					(Print 994 0 #font 0 #button {OK} 1) ; "Your save game disk is full. You must either use another disk or save over an existing saved game. saved game."
				)
...
(class SL of Obj
	...
	(method (doit &tmp theLine)
		(if code
			(= theLine (Memory memALLOC_CRIT (if 81 240 else 82)))
			(code doit: theLine)
