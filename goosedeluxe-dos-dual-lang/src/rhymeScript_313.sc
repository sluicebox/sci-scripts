		...
		(switch (= state newState)
			...
			(2
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {Y a todas partes que Mar(a iba})
						)
					)
					(else
			...
			(3
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {El corderito iba por seguro.})
						)
					)
					(else
			...
			(4
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {La persigui& un d(a a la escuela;})
						)
					)
					(else
						(proc0_16
			...
			(5
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {Eso era en contra de la regla;})
						)
					)
					(else
			...
			(6
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {Hizo los ni+os reir y jugar}))
					)
					(else
						(proc0_16
							(temp0
			...
			(7
				(if (< (rhymeSync prevSignal:) 80)
					...
					(if gLanguage
						(proc0_16
							(temp0
								format: {Para ver a un corderito en la escuela.}
							)
						)
					else
						(proc0_16 (temp0 format: {To see a lamb at school.}))
					)
					(self dispose:)
				)
			)
