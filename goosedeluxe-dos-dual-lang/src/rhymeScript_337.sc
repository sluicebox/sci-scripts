	...
	(method (changeState newState &tmp temp0)
		...
		(switch (= state newState)
			...
			(5
				(if gLanguage
					(proc0_16
						(temp0 format: {El viejo King Cole era un buen alma,})
					)
				else
					(proc0_16
						(temp0 format: {Old King Cole was a merry \nold soul,})
					)
				)
				(= cycles 1)
			)
			...
			(7
				(if gLanguage
					(proc0_16 (temp0 format: {Y un buen alma era $l,}))
				else
					(proc0_16 (temp0 format: {And a merry old soul \nwas he;}))
				)
				(= cycles 1)
			)
			(8
			...
			(9
				(if gLanguage
					(proc0_16 (temp0 format: {El llam& por su pipa,}))
				else
			...
			(11
				(if gLanguage
					(proc0_16 (temp0 format: {Y llam& por su copa,}))
				else
			...
			(13
				(if gLanguage
					(proc0_16
						(temp0 format: {Y llam& por sus tres violinistas.})
					)
				else
					(proc0_16
						(temp0
							format: {And he called for his \nfiddlers three.}
						)
					)
				)
