		...
		(switch (= state newState)
			...
			(9
				(if gLanguage
					...
					(proc0_16 (temp0 format: {Jack Sprat could eat no fat.}))
				)
			...
			(10
				(if (< (rhymeSync prevSignal:) 20)
					...
					(if gLanguage
						(proc0_16
							(temp0
								format: {Su esposa no pod(a comer carne magra,}
							)
						)
					else
						(proc0_16 (temp0 format: {His wife could eat no lean;}))
					)
			...
			(11
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {Y as( entre ambos,}))
					)
					(else
			...
			(12
				(if (< (rhymeSync prevSignal:) 40)
					...
					(if gLanguage
						(proc0_16 (temp0 format: {Lamieron el plato limpio.}))
					else
						(proc0_16
