		...
		(switch (= state newState)
			...
			(3
				(if (< (rhymeSync prevSignal:) 10)
					...
					(if gLanguage
						(proc0_16 (temp0 format: {Peter, Peter Pumpkin Eater;}))
					else
						(proc0_16
				...
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0
								format:
									{Ten(a una esposa y no la pudo mantener;}
							)
						)
					)
					(else
			...
			(5
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {La puso en una concha de calabaza,})
						)
					)
					(else
						(proc0_16
			...
			(6
				(if (>= (rhymeSync prevSignal:) 40)
					(if gLanguage
						(proc0_16 (temp0 format: {Y all( la mantuvo muy bien.}))
					else
						(proc0_16
							(temp0 format: {And there he kept her very well.})
						)
					)
					(= ticks 180)
				else
