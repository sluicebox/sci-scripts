		...
		(switch (= state newState)
			...
			(4
				(if (< (rhymeSync prevSignal:) 10)
					...
					(if gLanguage
						...
						(proc0_16
							(temp0 format: {Jack and Jill went up the hill,})
						)
					)
					(global109
						setLoop: 9 1
			...
			(5
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {Para buscar un balde de agua;})
						)
					)
					(else
			...
			(6
				(if (< (rhymeSync prevSignal:) 30)
					...
					(if gLanguage
						(proc0_16 (temp0 format: {Jack se call&,}))
					else
			...
			(8
				(if (< (rhymeSync prevSignal:) 50)
					...
					(if gLanguage
						(proc0_16 (temp0 format: {Y Jill se vino rodando.}))
					else
						(proc0_16
