		...
		(switch (= state newState)
				...
				(if (< (rhymeSync prevSignal:) 20)
					...
					(if gLanguage
						(proc0_16
							(temp0
								format:
									{El rat&n corri& hacia arriba del reloj,}
							)
						)
					else
						(proc0_16 (temp0 format: {The mouse ran up the clock;}))
					)
			...
			(11
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {El Reloj son& uno,}))
					)
					(else
			...
			(16
				(if gLanguage
					(proc0_16 (temp0 format: {El rat&n corri& hacia abajo,}))
				else
