		...
		(switch (= state newState)
			...
			(3
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {Mediaria, Mediaria,}))
					)
					(else
			...
			(4
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {/C&mo crece t* jard(n?}))
					)
					(else
			...
			(6
				(if (< (rhymeSync prevSignal:) 40)
					...
					(if gLanguage
						(proc0_16 (temp0 format: {Y conchas de berberecho,}))
					else
				...
				(if (< (rhymeSync prevSignal:) 50)
					...
					(if gLanguage
						(proc0_16
							(temp0
								format:
									{Y todas las lindas sirvientas en una hilera.}
							)
						)
					else
						(proc0_16
