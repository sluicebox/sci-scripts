		...
		(switch (= state newState)
			...
			(4
				(cond
					...
					(else
						(proc0_16 (temp0 format: {Sat in the corner}))
					)
				)
				(= cycles 1)
			)
			(5
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0 format: {Comiendo un pastel de Navidad;})
						)
					)
					(else
			...
			(6
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {El puso en su pulgar,}))
					)
					(else
			...
			(7
				(cond
					...
					(gLanguage
						(proc0_16 (temp0 format: {Y sac& una ciruela,}))
					)
					(else
			...
			(8
				(cond
					...
					(gLanguage
						(proc0_16
							(temp0
								format: {Y dijo, "=Qu$ buen muchacho soy Yo!"}
							)
						)
					)
					(else
