...
(instance rm398 of SQRoom
	...
	(method (init)
		...
		(= local10 (gGame setSpeed:))
		(gGame setSpeed: 0)
		(gNarrator disable: 0 3 4 5 6)
	)

	(method (dispose)
		(gNarrator enable:)
		(= gUseSortedFeatures local9)
		(gGame setSpeed: local10)
		(super dispose:)
	)
)
		...
		(switch (= state newState)
			...
			(2
				(switch
					...
					(0
						(Printf 398 1 (local8 myPrice:)) ; "This one is %d buckazoids."
						(-= state 1)
						(= cycles 1)
					)
					(1
...
(class boxProp of Prop
	...
	(method (doit &tmp temp0 [temp1 2])
		(super doit:)
		(if (and (== local2 self) (== ((gNarrator curIcon:) message:) 2))
			(= temp0 (User curEvent:))
			(= local0 (temp0 x:))
			(= local1 (+ (temp0 y:) 10))
			(if (> local0 235)
