...
(instance rm310 of SQRoom
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(south
				(HandsOff)
				(self setScript: enterScript style: 43)
		...
		(switch (= state newState)
			(0
				(if (not (or (== gPrevRoomNum 300) (== gPrevRoomNum 320)))
					(butte onLedgeFlag: 1)
					(gEgo setMotion: MoveTo 133 -2 self)
				else
...
(instance shadowMoves of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				3
				(Print 310 0) ; "You get the feeling you're not alone here."
				(shadow dispose:)
				(self dispose:)
			)
		)
...
(instance theRoom of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(1 ; Look
				(Print 310 1) ; "As you look around at the towering rock structures; immense and ageless formations carved over millions of years by the magnificent and placid Sea of Estros, you can't help but be overwhelmed with a sense of your own total insignificance."
			)
			(else
				(super doVerb:)
			)
