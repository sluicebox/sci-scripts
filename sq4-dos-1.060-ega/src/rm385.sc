	...
	(method (init &tmp temp0 temp1)
		...
		(cond
			((and (!= gPrevRoomNum 386) (not (IsFlag 11)))
				...
				(tossedSFX init:)
				(egoLandsSFX init:)
				(gLongSong2 number: 4 loop: -1 vol: 10 flags: 1 playBed:)
			)
			((IsFlag 11)
		...
		(switch (= state newState)
			...
			(4
				...
				(if (== register 1)
					...
					(= local140
						(proc0_12
							{"You're FIRED! My JANITOR could do a better job than you could!"}
							70
							310
							67
							10
...
(instance door of PicView
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Do
				(Print 385 0) ; "It's locked down tight. You can't open it."
			)
			(11 ; Smell
				(Print 385 1) ; "You smell that famous but unidentifiable Monolith Burger Secret Smell(tm)."
			)
			(10 ; Taste
				(Print 385 2) ; "It tastes like it smells."
			)
			(else
...
(instance boss of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 385 3) ; "He doesn't smell spring fresh, that's for sure."
			)
			(10 ; Taste
				(Print 385 4) ; "Not with your taste buds!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance cigar of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 385 5) ; "If you had it in your possession that might be possible."
			)
			(10 ; Taste
				(Print 385 5) ; "If you had it in your possession that might be possible."
			)
...
(instance store of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 385 6) ; "Wow, it smells just like a giant, juicy, delicious Monolith Slugburger Combo!"
			)
			(10 ; Taste
				(Print 385 7) ; "Wow, it tastes just like a giant, delicious Monolith Slugburger Combo, coated with a heavy duoplasticizer and shellacked to look extra juicy!"
			)
			(2 ; Do
				(Print 385 8) ; "It feels like a real burger place!"
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance bush1 of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(11 ; Smell
				(Print 385 9) ; "It smells a little like Julanar, the enchanted plant woman from QUEST FOR GLORY II."
			)
			(10 ; Taste
				(Print 385 10) ; "It tastes like another cheap plug for another Sierra product."
			)
			(2 ; Do
				(Print 385 11) ; "Hey! They had to be bolted to the super-structure because of people like you."
			)
			(else
				(super doVerb: theVerb &rest)
			)
