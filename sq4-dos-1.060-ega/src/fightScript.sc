...
(instance fightScript of Script
	...
	(method (init)
		...
		(gWalkHandler addToFront: self addToFront: (ScriptID 150 1)) ; RogerJr
		(= local1 1)
		(= local5 0)
		(= local6 4)
	...
	(method (dispose)
		...
		(gWalkHandler delete: self delete: (ScriptID 150 1)) ; RogerJr
		(super dispose:)
	)
		...
		(switch (= state newState)
			...
			(7
				(if (and local1 register)
					(if local2
						(gTextSpeed delete: client)
						(gWalkHandler delete: client delete: (ScriptID 150 1)) ; RogerJr
