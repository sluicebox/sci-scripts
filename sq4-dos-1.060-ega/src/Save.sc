...
(procedure (GetDirectory where &tmp result [newDir 33] [str 40])
	(repeat
		...
		(if (ValidPath @newDir)
			...
			(Print (Format @str 990 4 @newDir) #font 0) ; "%s is not a valid directory"
		)
	)
)
...
(procedure (localproc_2)
	(Print 990 5 #font 0) ; "You must type a description for the game."
)
...
(class SRDialog of Dialog
	...
	(method (init theComment names nums)
		...
		(okI
			...
			state:
				(if (or (and (== status 0) (not numGames)) (== status 3))
					0
				else
					3
				)
	...
	(method (doit theComment &tmp temp0 temp1 temp2 [temp3 361] [temp364 21] [temp385 80])
		...
		(repeat
			...
			(if (== i changeDirI)
				(if (GetDirectory gCurSaveDir)
					...
					(self dispose: init: theComment @temp3 @temp364)
				)
			else
