...
(local
	[local0 20] = [206 62 268 76 237 82 138 91 75 98 161 133 145 151 0 148 0 0 203 0]
	[local20 14] = [219 0 219 62 290 78 94 101 162 124 319 126 319 0]
	[local34 8] = [319 156 211 169 152 189 319 189]
	[local42 8] = [0 167 105 168 74 189 0 189]
)
...
(instance rm007 of Rm
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(north
				(gEgo view: 2 setStep: 2 1 posn: 265 77 offset: 2)
				(self setScript: (ScriptID 202 1)) ; stdWalkIn
			)
			(south
				(gEgo posn: 120 186)
		...
		(gEgo setCycle: Walk init:)
		(bird setCycle: Fwd init:)
		(poly1 points: @local0 size: 10)
		(poly2 points: @local20 size: 7)
		(poly3 points: @local34 size: 4)
		(poly4 points: @local42 size: 4)
		(self addObstacle: poly1 poly2 poly3 poly4)
	)

	(method (doit &tmp temp0)
		(if (& (gEgo onControl: 1) $4000)
			(gEgo view: 2 setStep: 2 1 offset: 2)
		)
		(if (& (gEgo onControl: 1) $2000)
			(gEgo view: 0 setStep: 3 2 offset: 3)
		)
		(cond
			(script
				(script doit:)
...
(instance princeLeave of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Say 160 7 0 67 10 90 25 4) ; "Excuse me, young man."
			...
			(4
				(Say 160 7 1 67 10 85 25 8) ; "Sorry to bother you, but I couldn't help but notice you sitting there on that log. I was wondering if there was anything wrong."
				(RedrawCast)
				(Say 163 7 2 67 10 10 25 10) ; "Why, yes...as a matter of fact, there is. I've been searching everywhere for my fiancee. She's a beautiful princess with long golden tresses, fetching blue eyes, and smooth, creamy skin. Have you seen her anywhere about?"
				(RedrawCast)
				(Say 160 7 3 67 10 90 25 4) ; "No, sorry. I haven't seen anyone like that."
				(RedrawCast)
				(Say 163 7 4 67 10 10 25 8) ; "That's what I figured. No one has seen her. I think that old witch who lives in the Dark Forest had something to do with her disappearance."
				(RedrawCast)
				(Say 160 7 5 67 10 90 25 8) ; "I'll keep an eye out for her. If I see her I'll let her know you're looking for her."
				(RedrawCast)
				(Say 163 7 6 67 10 10 25 12) ; "I would appreciate that. Well, I guess I'd better get back to looking for her. I'm not ever going to find her just sitting around here. Thanks for your concern."
				(RedrawCast)
				(prince
					cycleSpeed: 0
					setLoop: 0
...
(instance prince of Act
	...
	(method (handleEvent event)
		(cond
			((and (proc0_18 global322 event) (== (event message:) 4)) ; Talk
				(Say 75 7 10) ; "Let's say 'hello' to the young prince, Graham. He looks like he could use a cheering up."
				(event claimed: 1)
			)
			(
				(or
...
(instance bird of Act
	...
	(method (doit)
		(super doit:)
		(if (and (not (self script:)) (< (gEgo distanceTo: self) 30))
