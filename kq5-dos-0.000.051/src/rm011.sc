...
(local
	...
	[local22 16] = [0 149 46 149 109 140 145 140 175 126 108 124 86 118 0 122]
...
(instance rm011 of Rm
	...
	(method (init)
		...
		(self setFeatures: river2 tree anthill setRegions: 202) ; owl
		(if (== global81 3)
			(water init:)
		)
		(if (gEgo has: 5) ; Fish
		...
		(gEgo view: 0 ignoreHorizon: 1 setStep: 3 2 offset: 3 init:)
		...
		(cond
			...
			((not local9)
				...
				(cond
					((gCast contains: bear)
						(if (not (IsFlag 86))
							(SetFlag 86)
							(Say 75 11 0 67 20 20 25 6) ; "Watch out for the bear, Graham!"
					...
					((not (IsFlag 87))
						(SetFlag 87)
						(Say 75 11 1 67 20 20 25 6) ; "Oh, oh! Stay away from those bees!"
					)
				)
			)
			((= local0 (self edgeToRoom: (gEgo edgeHit:)))
				(if (== local0 (gCurRoom west:))
					...
					(RedrawCast)
					(cls)
					(Say 75 11 2 67 10 10 25 10) ; "There's nothing but a hot, dry desert further west. Most people avoid it, because there are bandits out there! If you insist on going, I'll wait for you HERE!"
...
(instance relievedScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Say 75 11 3 67 20 100 25 7) ; "Well, there you are! I was starting to get concerned!"
				(Say 160 11 4 67 20 20 25 8) ; "Don't worry about me, Cedric. I'm used to this kind of thing."
				(HandsOn)
				(client setScript: 0)
			)
		)
...
(instance drownHim of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Say 75 11 5 67 20 20 25 3) ; "No, don't...!"
				((gEgo head:) hide:)
				(gEgo
					normal: 0
					view: 69
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(13
				...
				(PrintDC 11 6 #width 150 #at 150 10 #dispose) ; "I am Queen Beetrice, kind sir. I wish to thank you ever so much for saving our hive from the claws of that horrible bear."
				(= seconds 10)
			)
			(14
				(cls)
				(PrintDC 11 7 #width 150 #at 150 10 #dispose) ; "In return, I offer you a luscious honeycomb from our hive. Please feel free to retrieve one. I promise my bees won't harm you. It may come in handy on your travels."
				(= seconds 10)
			)
			(15
				(cls)
...
(instance getWax of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(PrintDC 11 8) ; "Graham reaches a hand into the beehive and retrieves a very sticky chunk of honeycomb. Wrapping it in a protective piece of cloth, he then pockets it."
...
(instance getStick of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(PrintDC 11 9) ; "Graham bends down and picks up the large stick from the ground."
				((gEgo head:) hide:)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 10) ; "The swift river turns sharply to the south here."
					(event claimed: 1)
				)
				(3 ; Do
					(gEgo
