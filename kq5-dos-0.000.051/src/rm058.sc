...
(instance rm058 of Rm
	...
	(method (init)
		(super init:)
		(gEgo view: 34 init: setPri: 10 offset: 3)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 58 0) ; "A hideous, yet fascinating pipe organ commands attention as Graham walks through a downstairs hallway."
					(event claimed: 1)
				)
				(3 ; Do
					(cond
...
(instance angel of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 58 1) ; "Two distorted figures of bird-like women flank both sides of the monstrous pipe organ."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 58 2) ; "The weird figures don't appear to have any use at all."
					(event claimed: 1)
				)
			)
		)
...
(instance doorWay1 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 58 3) ; "This doorway leads back into the kitchen."
					(event claimed: 1)
				)
			)
		)
...
(instance doorWay2 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 58 4) ; "Graham can see into a large dining hall through the doorway."
					(event claimed: 1)
				)
			)
		)
	...
	(method (changeState newState &tmp [temp0 2])
		(switch (= state newState)
			...
			(1
				...
				(RedrawCast)
				(PrintDC 58 6) ; "Graham watches in horrified fascination as the grotesque organ begins to magically play a weird tune all by itself."
				(gGlobalSound number: 128 loop: 1 play: self)
				(leftSnake
					setCycle: (if (== global81 3) Fwd else 0)
					cycleSpeed: 5
				)
				(rightSnake
					setCycle: (if (== global81 3) Fwd else 0)
				...
				(keys1
					...
					setCycle: (if (> global81 1) Fwd else 0)
					setCycle: Fwd
				)
				(keys2
					init:
