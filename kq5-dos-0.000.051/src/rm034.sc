...
(local
	...
	local2
	[local3 5] = [2 0 3 1 2]
	[local8 8] = [31 187 31 189 175 189 175 173]
	[local16 8] = [159 151 210 151 209 156 158 156]
	[local24 12] = [0 171 29 166 36 159 74 159 119 145 0 0]
	local36
)
...
(instance rm034 of Rm
	...
	(method (init)
		(super init:)
		(gEgo view: 12 offset: 2 posn: 7 173)
		(= local2 1)
...
(instance talkEagleScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say 165 34 0 67 10 10 25 4) ; "What's wrong, Mr....uh, Eagle?"
				(Say 161 34 1 67 110 10 25 9) ; "I'm so weak from hunger. I haven't been able to catch any food for days. I can barely fly anymore."
				(Say 165 34 2 67 10 10 25 7) ; "I'd love to help you...let me see what I can do."
				(Say 161 34 3 67 110 10 25 5) ; "Thank you. I need food...(cough, cough)."
				(HandsOn)
				(client setScript: 0)
			)
		)
...
(instance feedEagle of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say 165 34 4 67 10 10 25 7) ; "Here, take this. Perhaps it will help you."
				((gEgo head:) hide:)
				(gEgo
					normal: 0
					view: 530
			...
			(6
				(Say 161 34 5 67 110 10 25 10) ; "You are a kind man to share your meager food with a poor bird...especially up here, in the snowy mountains."
				(Say 165 34 6 67 10 10 25 10) ; "Well, I couldn't just stand there and let you starve to death! What kind of person would I be?"
				(Say 161 34 7 67 110 10 25 10) ; "You have shown yourself to be a kind, compassionate man and I will not forget what you did for me. Good-bye, dear friend."
				(eagleHead dispose:)
				(eagleWing dispose:)
				(eagle setScript: 0)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 34 8) ; "A snowy path leads from the mountain ridge to a mysterious, ice-covered castle."
					(event claimed: 1)
				)
			)
		)
...
(instance aCastle of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 34 9) ; "A castle, buried in snow and ice, perches precariously atop a steep pinnacle."
					(event claimed: 1)
				)
				(3 ; Do
...
(instance eagle of Act
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 34 10) ; "A shivering eagle perches weakly upon a small rock. Despite his own problems, Graham's heart goes out to the poor thing."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 34 11) ; "The poor eagle gives Graham a pitiful look. Graham would like to help him...but how?"
					(event claimed: 1)
				)
				(5 ; Inventory
					(switch global69
						...
						(else
							(PrintDC 34 12) ; "That won't help the poor eagle."
							(event claimed: 1)
						)
					)
				)
				(4 ; Talk
					(if local0
						(PrintDC 34 13) ; "The eagle appears to be too weak to indulge in any further conversation."
						(event claimed: 1)
					else
						(= local0 1)
