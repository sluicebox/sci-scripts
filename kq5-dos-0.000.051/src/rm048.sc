	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (not (IsFlag 55))
					(+= gScore 3)
					(Say 75 48 0 25 6) ; "Look, Graham! An island! Perhaps we should explore it."
				)
				(= cycles 1)
			)
			(1
				(if (not (IsFlag 55))
					(Say 160 48 1 25 6) ; "Yes, I think we should, Cedric."
				)
				(= cycles 1)
			)
			(2
