	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(if (< gDay 3)
					(if (not local0)
						(gMessager say: 5 4 3 0) ; "The sounds of friendly chatter emanate from inside."
					)
					(gCurRoom setScript: knockLeftDoorScript)
				else
					(gMessager say: 5 4 9 0) ; "The door is locked."
				)
			)
			(1 ; Look
			...
			(27 ; badge
				(gMessager say: noun theVerb 3 0) ; "Identifying yourself to the door seems pointless."
			)
			(29 ; coins
				(gMessager say: noun theVerb 3 0) ; "Buying your way in is pathetic."
			)
			(else
...
(instance rightSky of Feature
	...
	(method (init)
		(self
			createPoly: 244 10 319 10 319 40 286 50 285 64 282 68 263 67 263 52 252 52 251 33 245 33
		)
		(super init:)
	)
)
