...
(instance vending of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(29 ; coins
				(if local1
					(gMessager say: 1 29 3) ; "You already deposited sixty-five cents."
				else
					(= local1 1)
					(sFx number: 1005 setLoop: 1 play:)
					(gMessager say: 1 29 4) ; "You drop sixty-five cents into the slot."
				)
			)
			(4 ; Do
				(if local1
					(if (not (gEgo has: 48)) ; candyBar
						(sFx number: 1004 setLoop: 1 play:)
			...
			(4 ; Do
				(cond
					((and (>= gDay 4) (not (gEgo has: 31))) ; matches
						(gEgo get: 31) ; matches
						(gGame points: 3)
						(gMessager say: 2 4 5 0) ; "Someone has left a pack of matches. Never knowing when"
					)
					((gEgo has: 31) ; matches
		...
		(switch theVerb
			(4 ; Do
				(if cel
					...
					(gMessager say: noun theVerb 0 0) ; "Maybe if you had the strength of Arnold you could"
				)
			)
			(22 ; handgun
				(if cel
			...
			(2 ; Talk
				(if cel
					(gMessager say: 11 theVerb 0 0) ; "There's no one in the elevator to talk to."
				else
					(super doVerb: theVerb)
				)
			)
...
(instance elevatorArrive of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gGlobalSound1 stop: number: 942 setLoop: 1 play: self)
			)
			(3
				(= ticks 150)
			)
...
(instance getInElevator of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo setMotion: PolyPath 284 140 self)
			)
			(1
...
(instance elevatorLeave of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(doors priority: (if local0 145 else 0) setCycle: Beg self)
				(if (not local0)
