...
(instance dogFallAsleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(gMessager say: 5 39 0 2 self) ; "The dog swallows the sedatives."
			)
			(6
...
(instance dog of Prop
	...
	(method (doVerb theVerb)
		...
		(if (gPqFlags test: 142)
			(if (== theVerb 2) ; Talk
				(gMessager say: noun theVerb 1 0) ; "Shh... Don't wake him."
			else
				(gMessager say: noun 0 1 0) ; "Leave the dog alone, you don't want to wake him."
			)
		else
			(switch theVerb
				(39 ; pills
