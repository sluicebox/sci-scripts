	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo
					setSpeed: 6
					setMotion: PolyPath (+ (sam x:) 57) 139 self
				)
			)
			(1
				(sam setHeading: 90)
...
(instance sawSound of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gGlobalSound1 number: 982 loop: 1 play: self)
			)
			(2
				(if (== register 3)
					(self dispose:)
