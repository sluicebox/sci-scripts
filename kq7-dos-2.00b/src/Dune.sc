...
(class DuneHandler of EventHandler
		...
		(for ((= temp0 0)) (< temp0 argc) ((++ temp0))
			(if ([param1 temp0] isKindOf: Dune)
				(if (not (self isDuplicate: [param1 temp0]))
					(KList 11 elements (KList 2 [param1 temp0] [param1 temp0])) ; AddToEnd, NewNode
					(++ size)
	...
	(method (delete param1 &tmp temp0)
		(for ((= temp0 0)) (< temp0 argc) ((++ temp0))
			(if ([param1 temp0] isKindOf: Dune)
				(if (and nextNode (== [param1 temp0] (KList 8 nextNode))) ; NodeValue
					(= nextNode (KList 6 (KList 15 elements [param1 temp0]))) ; NextNode, FindKey
				)
				(if (KList 16 elements [param1 temp0]) ; DeleteKey
					(-- size)
				)
			else
