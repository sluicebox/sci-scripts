...
(instance rm300 of Rm
	...
	(method (init)
		(LoadMany rsVIEW (LangSwitch 300 305) 515 606)
		(rm300
			style:
				(switch gPrevRoomNum
					(301 3)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/deputy,otto,goon,assistant')
				(HighPrint 300 3) ; "There's no hint of the Ogre."
			)
			((Said 'look,look/meisterson,man')
				(HighPrint 300 4) ; "You see no sign of the Sheriff."
			)
			(
				(or
					(Said 'look,look/jail')
					(Said 'look,look/office,building<meisterson')
			...
			((or (Said 'look,look/scissors') (MouseClaimed onScissors event 3))
				(if (> (gEgo x:) 125)
			...
			((or (Said 'look,look/pole') (MouseClaimed onBarberPole event 3))
				(if (> (gEgo x:) 125)
			...
			((or (Said 'look,look/gate') (MouseClaimed local2 event 3))
			...
			(
				(or
					(Said 'look,look,read/note,sign,sign,door,shop,shop')
				...
				(switch (event type:)
					(evSAID
						(cond
							...
							((Said 'close,close,hasp,hasp,lock,bar,bar/gate')
								(if (local2 cel:)
			...
			(4
				...
				(if (== (StrLen @global401) 0)
					(if 1
						...
						(Format @global594 300 36) ; "\d1\d2\b2 \c9 \cb\b0\db\b0"
					)
				else
					(StrCpy @global594 @global401)
				)
				(HighPrint (Format @temp0 300 37 @global401 @global594)) ; "Good luck in your quest, %s.%"
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(HighPrint 300 38) ; "There's more than a hint of Ogre about this strange and bulky character. He seems cheerful, though."
			)
			((or (Said 'look,look/meisterson,man') (MouseClaimed onSheriff event 3))
				(HighPrint 300 39) ; "Schultz appears to be an affable old coot, content to smoke his pipe. There's a hardness behind his eyes, however, that gives you reason to believe that he's no pushover."
			)
			((or (Said 'look,look/yo,yo') (MouseClaimed onYoyo event 3))
				(HighPrint 300 40) ; "It goes up and down...up and down...up and down...up and down....."
			)
			((or (Said 'look,look/pipe') (MouseClaimed onPipe event 3))
				(HighPrint 300 41) ; "The Sheriff's pipe is a truly impressive hand- carved ivory pipe."
			)
			(else
				(switch (event type:)
					(evSAID
						(cond
							((Said 'talk,talk/meisterson,man')
								(if (gEgo inRect: 100 100 242 171)
									(HighPrint 300 42) ; "The Sheriff informs you that he's much too busy for idle chit-chat."
								else
									(NotClose)
								)
							)
							((Said 'talk,talk,ask/goon,otto,deputy,assistant')
								(if (gEgo inRect: 100 100 242 171)
									(HighPrint 300 43) ; "Otto only expresses himself physically."
								else
									(NotClose)
								)
							)
							(
								...
								(cond
									((Said '//hamlet')
										(= local10 0)
										(HighPrint 300 44) ; "Just walk around and find out for yourself."
									)
									((Said '//hero,adventure,adventuring')
										(HighPrint 300 45) ; "You'll need to talk to the Baron about that. Or go ask Wolfgang down at the Guild Hall, which is just down the street."
										(HighPrint 300 46) ; "So maybe you're going to be a hero? Well, we could surely use one around here, what with all the brigands."
									)
									((Said '//bandit,robbery')
										(HighPrint 300 47) ; "There's a whole band of brigands hiding out somewhere in the mountains around us. They robbed a merchant just last week. Got a fair amount of treasure, I hear."
									)
									((Said '//name')
										(HighPrint 300 48) ; "I am Sheriff Schultz Meistersson, and this is Otto Von Goon, my assistant."
									)
									((Said '//meisterson')
										(HighPrint 300 49) ; "The Sheriff replies, "Why, that's me!""
									)
									((Said '//abdulla')
										(HighPrint 300 50) ; "The one who was robbed? He's staying at the Inn, I believe."
									)
									((Said '//hotel')
										(HighPrint 300 51) ; "Go talk to the Innkeeper. He will help you there."
									)
									((Said '//hill')
									...
									((Said '//danger')
										(HighPrint 300 53) ; "My friend," the Sheriff says, "this world's filled with dangers. Sometimes I think it might be better to choose your danger, rather than sitting around, waiting for it, as I seem to do."
										(HighPrint 300 54) ; "But enough of this philosophical rambling. I am content enough to be Sheriff of Spielburg."
									)
									((Said '//prisoner')
										(HighPrint 300 55) ; "It's simple. When someone breaks the law, and we catch them, we hold them prisoner."
									)
									((Said '//loot')
										(HighPrint 300 56) ; "Obtaining treasure involves putting yourself in danger. If you're brave, foolish, or lucky, you might get some treasure."
									)
									((Said '//monster')
										(HighPrint 300 57) ; "The monsters come down from the mountains every winter, but they're usually gone by now. The Baron just doesn't have enough men to handle them."
										(HighPrint 300 58) ; "Wolfgang is the one to talk to about monsters. He's had a lot of experience with them."
									)
									((Said '//baron,castle')
										(HighPrint 300 59) ; "Baron Stefan keeps mostly to himself at his castle north of here. We haven't seen him here in town for years."
									)
									((Said '//abenteuer,master')
										(HighPrint 300 60) ; "Wolfgang, down at the Guild hall, is the one to talk to about monsters. He's had a lot of experience with them."
									)
									(
										(Said
											'//goon,otto,deputy,assistant,help,help'
										)
										(HighPrint 300 61) ; "Otto is a big help to me," the Sheriff tells you. "He's very smart for a Goon. His prisoners only suffer a few broken bones when he grabs them."
										(HighPrint 300 62) ; "It took a bit of doing to teach him to bring prisoners back alive, but he hardly ever forgets, now."
									)
									((Said '//labor,labor,(hall[<club,about])')
										(HighPrint 300 63) ; "The Guild Hall is at the Southwest corner of town. If you're looking for work, that's a good place to go."
									)
									(
										(Said
											'//magic,zara,spell,potion,scroll,wand,(shop<magic)'
										)
										(HighPrint 300 64) ; "Just next to the Inn is a small Magic Shop. The owner, Zara, will let you in if you have some abilities in that area. She's a strange one, all right!"
									)
									((Said '//*')
										(= local10 0)
										(HighPrint 300 65) ; "The Sheriff says, "You think I'm going to tell you everything I know about everything? Go look around and find things out for yourself.""
									)
								)
								(if local10
									(SolvePuzzle 718 1)
...
(instance egoWakes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(NormalEgo)
				(HighPrint 300 66) ; "However, this was not a good place to sleep. You're stiff and sore all over and your wallet feels very light."
				(= [gInvNum 1] 0) ; silver
				(= [gInvNum 2] 0) ; gold
				(HandsOn)
				(= cycles 2)
			...
			(2
				(HighPrint 300 67) ; "As safe as our streets are, you still should not sleep on them. It is a good way to get robbed."
			)
		)
	)
)
