...
(instance rm16 of Rm
	...
	(method (handleEvent event &tmp temp0)
		(switch (event type:)
			...
			(evSAID
				(cond
					((Said 'look,look<for/boulder,brick')
						(localproc_2)
					)
					((Said 'look,look>')
						(cond
					...
					(
						(or
							(Said 'cut,hit,attack,kill[/plant,flower,pod]')
							(Said 'use,draw/blade')
						)
						(switch global326
							(1
					...
					(
						(or
							(Said 'climb,climb,get,get<down')
							(Said 'japaneseclimbdown')
						)
						(cond
							((and (> global326 1) (== global327 0))
								(HighPrint 16 19) ; "Wait a second!"
							)
							((>= global326 1)
								(gEgo setScript: climbDown)
							)
							(else
								(HighPrint 16 20) ; "You're already on the ground."
							)
						)
					)
					(
						(or
							(Said 'climb,climb[<up][/cliff,boulder]')
							(Said 'japaneseclimbup')
						)
						(cond
							((>= global326 1)
								(HighPrint 16 21) ; "You don't think you can climb any further up the cliff."
							)
							((IsFlag 136)
								(localproc_1)
							)
					...
					((Said '(pick,lockpick<up),find,search>')
						(cond
							((Said '/boulder,brick')
								(localproc_2)
							)
	...
	(method (handleEvent event)
		(if
			(or
				...
				(Said 'look,look/flower,plant,pod')
			)
			(event claimed: 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (not (IsFlag 190))
					(HighPrint 16 46) ; "It looks difficult. You look around for a good place to climb."
				)
			...
			(1
				(if (not (IsFlag 190))
					(SetFlag 190)
					(HighPrint 16 47) ; "This looks like a good place to you."
				)
				(gEgo
					view: 517
					setMotion: 0
			...
			(3
				(Print 16 48 #at -1 140 #width 300) ; "You valiantly scrabble against the cliff wall, but it looks like your climbing skill could stand some improvement."
				(= seconds 1)
			)
			(4
				(gEgo
			...
			(6
				(HandsOn)
				(NormalEgo)
				(HighPrint 16 49) ; "Perhaps there is some other way to get to your objective."
				(client setScript: 0)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (not (IsFlag 190))
					(HighPrint 16 50) ; "You look for an appropriate place to climb."
					(SetFlag 190)
				)
			)
			(1
			...
			(2
				(if (not (TakeDamage 5))
					(EgoDead ; "Had you been healthier, you probably could have survived that fall. In your weakened condition, however, you succumbed to your injuries."
						...
						5
						80
						{Your figure remains still and silent.%j\b7\d0\c9\b6\d7\c0\de\dc \bc\bd\de\b6\c6 \c1\b2\dd\d3\b8\a6 \c2\c2\de\b9\d9}
					)
				else
					(TimePrint 3 16 52) ; "Ohhhhhhh!"
					(= seconds 3)
				)
			)
			(3
				(Print 16 53) ; "You remain unconscious on the ground for an indeterminate amount of time. As you finally revive, you realize that you've caused yourself a fair amount of damage."
				(NormalEgo)
...
(instance getRock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HighPrint 16 54) ; "You pick up a few small rocks."
				(gEgo setCycle: Beg self)
			)
			(3
...
(instance youMissed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(rock setStep: 30 20 setMotion: MoveTo 0 (- (rock y:) 100) self)
			)
			(1
				(rock setMotion: JumpTo (Random 130 160) (Random 145 160) self)
			)
			(2
				(HandsOn)
				(rock hide: setScript: 0)
			)
		)
	)
)
...
(instance heCaughtIt of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo setCel: 4)
				(flyingSeed posn: 142 56 show:)
				(= seconds 4)
			)
			(3
				(TimePrint 4 16 55) ; "You put the seed into your pack and climb down."
				(= global327 2)
				(= global328 2)
				(= seconds 4)
			)
