...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 100] [curDevice 100] [str 100])
	(= ret 1)
	...
	(if (and (DeviceInfo diPATHS_EQUAL @saveDevice @curDevice) (!= 81 81) (DeviceInfo diIS_FLOPPY @curDevice))
		(Format ; "Please insert your %s disk in drive %s."
			...
			@curDevice
			(if saveDisk
				{\85\bc\85\ae\85\ca\85\dc\85\c1\85\dc\85\a6\85\bb\85\b6}
			else
				{\85\b7\85\dc\85\ae\85\cf}
			)
			@curDevice
		)
...
(class Game of Obj
	...
	(method (save &tmp [comment 50] num oldCur oldPause)
	...
	(method (restore &tmp [comment 50] num oldCur oldPause)
		(Load rsFONT gSmallFont)
		(Load rsCURSOR gWaitCursor)
		(= oldCur (self setCursor: gNormalCursor))
		(= oldPause (Sound pause: 1))
