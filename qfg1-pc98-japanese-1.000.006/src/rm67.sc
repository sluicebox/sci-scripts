				...
				(cond
					...
					((Said 'look,look>')
						(cond
							...
							(
								(or
									(Said '/trap,hasp,hasp,lock,chain')
									(and
										(Said '<down')
					...
					((Said 'get,get>')
						(if (Said '/trap,hasp,hasp,lock,chain')
							(cond
								(local12
					...
					((Said 'cast>')
						(switch (= temp0 (SaidSpell event))
							(18
								(if (CastSpell temp0)
									(if local12
										(HighPrint 67 11) ; "There is an aura of magic around the fox."
									else
										(HighPrint 67 12) ; "There is no magic present."
									)
								)
							)
							(20
								(if local12
									(HighPrint 67 13) ; "The fox is in such pain that it's already dazzled."
								else
									(HighPrint 67 14) ; "There is no point to that."
								)
							)
							(23
								(if (CastSpell temp0)
									(if local12
										(if local17
											...
											(HighPrint 67 15) ; "You don't have to put me out of my misery, you just have to get me out of this trap."
											(= local17 1)
										)
									else
										(HighPrint 67 14) ; "There is no point to that."
							...
							(22
								(if local12
									(HighPrint 67 16) ; "You realize, of course, I really find it hard to relax with this thing on my leg."
								else
									(HighPrint 67 14) ; "There is no point to that."
								)
							)
...
(instance foxCallForHelp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(clr)
				(TimePrint 6 67 17) ; "Help me, Brave and Kind Hero."
			...
			(4
				(fox loop: 1 cel: 0 setCycle: Fwd)
				(clr)
				(TimePrint 6 67 18) ; "Please help me, Brave and Kind Hero."
				(= seconds 2)
			)
			(5
				(fox setCycle: End self)
...
(instance foxWillNotFight of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(clr)
				(TimePrint 7 67 19) ; "Please have mercy on a creature who is in great pain."
				(fox loop: 1 cel: 0 setCycle: Fwd)
				(= seconds 5)
			)
			(1
			...
			(2
				(HighPrint 67 20) ; "The fox does not seem to want to fight."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(clr)
				(TimePrint 9 67 21) ; "My foot is caught in this cruel trap and I am in great pain. Surely you could take a minute and set me free."
				(fox loop: 1 cel: 0 setCycle: Fwd)
				(= seconds 3)
			)
			(1
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(= global272 0)
				(HandsOn)
				(HighPrint 67 22) ; "That was strange, no sooner did you kill the fox than it vanishes."
				(client dispose:)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(cond
					(local14
						(HighPrint 67 23) ; "Hmmm. Perhaps it needed a stronger spell."
					)
					(local13
						(HighPrint 67 24) ; "Your powerful Open spell springs the trap."
					)
					(else
						(HighPrint 67 25) ; "You spring the trap."
					)
				)
				(self cue:)
			)
			...
			(8
				(LowPrint 67 26) ; "In exchange for your kindness, I will give you some advice and a bit of information. First of all, it sometimes pays off to be polite, even to rude people."
				(self cue:)
			)
			(9
				(LowPrint 67 27) ; "As for the amusing tidbit - Baba Yaga put an enchantment on the Baron's Daughter some years back. To break the spell you need to talk to the Dryad. Au Revoir, Ta-ta, see you sometime."
				(self cue:)
			)
			(10
				(fox
...
(instance getTrap of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HighPrint 67 28) ; "The trap seems to be caught on something. You can't pull it loose."
				(self cue:)
			)
			(4
				(= cycles 8)
	...
	(method (handleEvent event &tmp temp0)
		(switch (event type:)
			(evSAID
				(cond
					((Said 'talk,talk')
						(HighPrint 67 29) ; "I am in such pain and agony I cannot think.  Help me, good adventurer, please."
					)
					((Said 'pat/fox')
						(HighPrint 67 30) ; "That's not worth any game points, and will not make the fox feel any better."
					)
					...
					((or (Said 'look,look/feet,feet') (Said 'ask//feet,feet'))
						(HighPrint 67 31) ; "My foot is caught in this trap. If I had hands like you, instead of paws, I could free myself."
					)
					((Said 'ask>')
						(if (Said '//help,help,trap')
							(if (not local16)
								(HighPrint 67 21) ; "My foot is caught in this cruel trap and I am in great pain. Surely you could take a minute and set me free."
								(= local16 1)
							else
								(HighPrint 67 29) ; "I am in such pain and agony I cannot think.  Help me, good adventurer, please."
							)
					...
					(
						(or
							(Said 'free,free,free,release/fox,animal')
							(Said 'open,open,open,get/trap')
						)
						(if local12
							(if
								(and
								...
								(HighPrint 67 32) ; "Walk around to the other side of the fox, where the trap is."
							else
								(SolvePuzzle 686 10)
								(gEgo setScript: foxFreed)
							)
					...
					((Said 'attack/fox,animal')
						(if local12
							(fox setScript: foxWillNotFight)
						else
							(HighPrint 67 33) ; "There's no one here to fight."
						)
					)
					((Said 'kill,attack,attack,attack/fox,animal')
						(if local12
							(cond
								((not (gEgo has: 6)) ; blade
									(HighPrint 67 34) ; "You would need a sword."
								)
								((gEgo inRect: 55 111 120 120)
