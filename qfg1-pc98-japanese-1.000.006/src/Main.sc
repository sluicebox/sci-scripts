...
(procedure (FindTime &tmp temp0 [temp1 60] [temp61 60] [temp121 60])
	(= temp0 gDay)
	(if (or (!= gTimeOfDay 6) (> gClock 500))
		(++ temp0)
	)
	(Format @temp1 0 50 297 (+ 40 gTimeOfDay)) ; "%s"
	(StrSplitInTwo @temp61 @temp121 @temp1)
	(clr)
	(Print
		(Format @temp1 0 51 @temp61 temp0 temp0 @temp121) ; "%s on day %d.%"
		#at
		-1
		12
		#width
...
(procedure (EgoGait newGait gaitMsg &tmp theView)
	(if gaitMsg
		(cond
			((not (User canControl:))
				(HighPrint 0 52) ; "You can't do that now."
				(return)
			)
			((== gEgoGait newGait)
				(HighPrint 0 53) ; "Go ahead. Just do it."
				(return)
			)
		)
	)
...
(procedure (NotClose)
	(HighPrint 0 54) ; "You're not close enough."
)

(procedure (AlreadyDone)
	(HighPrint 0 55) ; "You've already done that."
)
...
(procedure (DontHave)
	(HighPrint 0 56) ; "You don't have it."
)
...
(procedure (FixTime newClock newMinutes &tmp oldTime)
	...
	(if (and (== gTimeOfDay 6) (!= oldTime 6))
		(if (== (++ global268) 1)
			(Print 0 57) ; "You are getting tired."
		else
			(Print 0 58) ; "You are exhausted from lack of sleep."
		)
	)
)
...
(procedure (UseStamina pointsUsed &tmp foo)
	...
	(cond
		((< (= foo (-= [gEgoStats 15] pointsUsed)) 0) ; stamina
			...
			(if (not (IsFlag 110))
				(SetFlag 110)
				(HighPrint 0 59) ; "You are so exhausted that everything you do hurts. Better get some rest."
			)
			(if (<= [gEgoStats 14] 0) ; health
				(EgoDead ; "That was a little too much for you. You collapse from exhaustion and die."
					0
					60
					80
					{Death from Overwork}
...
(procedure (CastSpell spellNum &tmp temp0)
	(cond
		((not [gEgoStats 12]) ; magic
			(HighPrint 0 61) ; "You don't know how to cast spells."
		)
		((not (gEgo knows: spellNum))
			(HighPrint 0 62) ; "You don't know that spell."
		)
		((< [gEgoStats 16] [global190 (- spellNum 17)]) ; mana
			(HighPrint 0 63) ; "You don't have enough Magic Points to cast that spell."
		)
		(else
			(TrySkill spellNum 0)
	...
	(cond
		...
		([gInvNum 3] ; food
			(if (not (-- [gInvNum 3])) ; food
				(LowPrint 0 64) ; "You just ate your last ration; you'd better get some more food soon."
			)
		)
		((IsFlag 124)
			(SetFlag 125)
			(LowPrint 0 65) ; "You're starving. Better find some food *soon*!"
			(TakeDamage 1)
		)
		(else
			(SetFlag 124)
			(LowPrint 0 66) ; "You're really getting hungry."
		)
	)
)
		...
		(if (u> (MemoryInfo 1) (+ 20 (MemoryInfo 0))) ; FreeHeap, LargestPtr
			(cond
				...
				((!= roomNum 601) ; CharSave
					(EgoDead ; "Suddenly, the deadly poison Fragmentation Bug leaps out of a crack in the system, and injects you with its poison. Alas, there is no cure, save to . . ."
						0
						7
						80
		...
		(switch (event type:)
			...
			(evMOUSEBUTTON
				(if
					...
					(clr)
					(Print 297 (Random 26 34) #at -1 12 #width 300 #mode 1)
				)
			)
			(evMOUSERELEASE
			...
			(evSAID
				(cond
					...
					((or (Said 'quit') (Said 'done,done,done/game'))
						(PromptQuit)
					)
					(
						(or
							(Said 'kiss,kiss,fuck,blow,shit,shit,leak')
							(Said 'get/shit,shit,leak')
							(Said 'eat,eat/shit,shit')
						)
					...
					((Said 'yes,no,please')
						(HighPrint 0 35) ; "No one responds."
					)
					((Said '/hello')
						(HighPrint 0 36) ; "Hi!"
					)
					...
					((Said 'open,open,open/box,musicbox')
					...
					((Said 'search[/!*,room,area,area]')
						(HighPrint 0 44) ; "You don't find anything interesting."
					)
					((Said 'look,look,read>')
						(= temp5
							(+
						...
						(cond
							...
							(else
								(HighPrint 0 45) ; "You don't see any here."
							)
						)
					)
					(
