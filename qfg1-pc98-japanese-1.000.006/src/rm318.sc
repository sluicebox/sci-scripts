...
(instance rm318 of Rm
	...
	(method (init)
		(Load rsVIEW (LangSwitch 318 319))
		(Load rsPIC (LangSwitch 318 317))
		(= picture (LangSwitch 318 317))
		(super init: &rest)
	...
	(method (handleEvent event &tmp temp0 temp1)
		(switch (event type:)
			...
			(evSAID
				(cond
					((Said 'stop,done,done,done,done,done')
						(event claimed: 1)
						(gCurRoom newRoom: 311)
					)
					((Said 'look,look')
