...
(instance rm310 of Rm
	...
	(method (init)
		...
		(Load rsVIEW (LangSwitch 310 316))
		(Load rsVIEW 515)
		(super init: &rest)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'knock,open,open,open/door')
				(cond
					(
						(and
							(== (gEgo onControl: 1) 8192)
			...
			((Said 'look,look[<at,around][/!*,hamlet,street,building]')
			...
			((or (MouseClaimed local1 event 3) (Said 'look,look/shop,shop[<magic]'))
				(HighPrint 310 12) ; "Not only does this building look weird, but it feels weird to be near it."
			)
			(
				(or
					(MouseClaimed local0 event 3)
					(Said 'look,look/house[<lady,lady,woman]')
				)
				(HighPrint 310 13) ; "The house looks closed up. There is no sign of the Little Old Lady."
			)
			((or (MouseClaimed local3 event 3) (Said 'look,look/hall[<club]'))
				(if gNight
			...
			((MouseClaimed onGuildSign event 3)
				(HighPrint 310 19) ; "Judging from what the sign says, the building at the end of the street is the Guild Hall. \95W\8e\a6\82\a9\82\e7\94\bb\92f\82\b7\82\e9\82\c6\81A\92\ca\82\e8\82\cc\92[\82\c9\82\a0\82\e9\8c\9a\95\a8\82\cd\83M\83\8b\83h\81E\83z\81[\83\8b\82\be\82\cb\81B"
			)
			((Said 'look,look/sign,sign')
				(if (> (gEgo x:) 200)
					(HighPrint 310 18) ; "It would seem that the strangely decorated building is involved in the magic trade."
				else
					(HighPrint 310 20) ; "Judging from what the sign says, the building at the end of the street is the Guild Hall."
				)
			)
			(
				(or
					...
					(Said 'look,look/flame')
				)
				(if (> (flameScript state:) 0)
					(HighPrint 310 21) ; "Isn't it hard to tear your eyes away? "PYROMANIA.""
				else
					(HighPrint 310 22) ; "Flames? What flames? You don't see any flames."
				)
			)
		)
	)
...
(instance egoWakes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(NormalEgo)
				(HighPrint 310 23) ; "However, this was not a good place to sleep. You're stiff and sore all over and your wallet feels very light."
				(= [gInvNum 1] 0) ; silver
				(= [gInvNum 2] 0) ; gold
				(HandsOn)
			)
...
(instance LOL of Prop
	...
	(method (handleEvent event)
		(cond
			((or (MouseClaimed LOL event 3) (Said 'look,look/lady,lady,woman'))
				(HighPrint 310 24) ; "She's deeply asleep in her rocker."
			)
			((Said 'look,look/rocker,chair')
				(HighPrint 310 25) ; "In it sits the Little Old Lady, asleep."
			)
			((Said 'awaken/lady,lady,woman')
				(HighPrint 310 26) ; "She's really sleeping soundly and doesn't notice your presence."
			)
			((Said 'ask,talk,talk')
				(HighPrint 310 27) ; ""ZZZZZZZZZZZZZZZZZZZZ"!"
				(HighPrint 310 28) ; ""Grmpf..snrt..hmphspft"!"
				(HighPrint 310 29) ; "ZZZZZZZZZZZZZZZZZZZZ"
			)
			(
				(or
					(MouseClaimed local0 event 3)
				...
				(HighPrint 310 30) ; "This looks like a nice, neat little house. There is a grandmotherly little old lady rocking on the front lawn."
			)
		)
	)
)
