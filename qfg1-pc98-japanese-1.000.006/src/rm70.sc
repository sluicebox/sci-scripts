...
(procedure (localproc_1 param1 &tmp temp0 temp1)
	...
	(= temp1 5)
	(clr)
	(Print &rest #at temp0 temp1 #width 150 #mode 1 #dispose #time param1 #window local2)
)
		...
		(if (== (event type:) evSAID)
			(if (> local4 1)
				...
				(cond
					...
					((or (Said 'attack,kill,hit') (Said 'use/blade,dagger'))
						(if local1
							(localproc_2)
							(= local107 1)
					...
					((Said 'ask>')
						(if (and local1 (== local4 0))
							(= local109 1)
							(cond
								((Said '//mushroom,toadstool,ring,ring')
									(localproc_2)
									(= local107 1)
					...
					((Said 'pick,lockpick,get,get,gather/mushroom')
					...
					((Said 'look,look>')
						(cond
							...
							((Said '/ring,ring,mushroom,toadstool')
								(if local1
							...
							((Said '/faerie,creature,chandelier,chandelier')
								(if local1
...
(instance deathDance of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(EgoDead ; "For such wimpy-looking creatures, those fairies sure can play rough! You never danced so hard in your entire life, which is now over."
					70
					26
					80
					{Land of 1000 Dances}
...
(instance pickEm of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if local0
					(HighPrint 70 27) ; "You eat a few of the lovely mushrooms."
				else
