		...
		(cond
			...
			(
				(or
					...
					(Said 'look,look/stuff,stuff,shield,spear')
				)
...
(instance brutus of TargActor
	...
	(method (handleEvent event)
		(cond
			...
			(
				(or
					(Said 'look,look/brutus,man,thief,bandit,body')
					(MouseClaimed self event 3)
				)
				(if (IsFlag 261)
...
(instance bruno of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((or (Said 'look,look/bruno') (MouseClaimed self event 3))
	...
	(method (handleEvent event)
		(cond
			...
			(
				(or
					(MouseClaimed self event 3)
					(Said 'look,look/target,board,sign,sign')
				)
				(if local7
		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'search,look,look/body,bandit,bruno,brutus')
						(if (IsFlag 261)
					...
					((Said 'look,look/east,south,west,forest,forest')
						(HighPrint 73 30) ; "The forest is very overgrown near here."
					)
					(
						(Said
							'look,look[<at,around][/!*,range,clearing,area,hamlet,wall,north,building]'
						)
...
(instance brigsMeet of Script
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look,look/man,man,thief,bandit')
...
(instance egoLoses of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(EgoDead ; "Done in by poisoned daggers and desperate desperadoes! Try to avoid annoying that deadly duo next time."
					73
					62
					82
					516
...
(instance brutusThrows of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(EgoDead ; "Done in by daggers and a desperate desperado! Try to avoid annoying that deadly duo next time."
					73
					64
					82
					516
		...
		(cond
			...
			((Said 'throw/dagger,dagger')
				(cond
					...
					((< (gEgo y:) 135)
						(HighPrint 73 66) ; "You're too close to the target for it to be a challenge."
					)
					((gEgo has: 7) ; dagger
			...
			((and (Said 'get,get/dagger') (localproc_0 1))
				(self changeState: 1)
			)
		)
	)
