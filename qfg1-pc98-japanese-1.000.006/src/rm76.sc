		...
		(switch (event type:)
			(evSAID
				...
				(cond
					...
					((Said 'pick,lockpick<up/acorn,nut')
						(localproc_0)
					)
					((Said 'burn,torch,torch/tree,oak')
						(if (== local0 2)
					...
					((Said 'look,look>')
						(cond
							((Said '[<at,around][/!*,forest,greenery,clearing]')
								(HighPrint 76 34) ; "You are in a strange and beautiful part of the forest. There is something special about this place."
								(HighPrint 76 35) ; "The large, gnarled oak seems to draw your attention."
							)
							((Said '/dryad,girl,woman')
								(if (== local0 2)
									(HighPrint 76 36) ; "The Dryad appears to be a woman made of wood and leaves."
								else
									(HighPrint 76 37) ; "You cannot look at what you cannot see."
								)
							)
							((Said '/tree,oak')
								(HighPrint 76 35) ; "The large, gnarled oak seems to draw your attention."
							...
							((Said '/boulder')
								(HighPrint 76 38) ; "There is a rock wall northwest of the large oak tree."
							)
							((Said '/buck')
								(if (IsFlag 202)
									(HighPrint 76 39) ; "The majestic white stag is an outstanding example of the beauty of the animal kingdom."
								else
									(HighPrint 76 40) ; "You see no such animal."
								)
							)
							((Said '/acorn,nut')
								(if local1
									(HighPrint 76 41) ; "There is a large, slightly glowing acorn on the ground."
								else
									(HighPrint 76 42) ; "There are acorns here and there on the ground."
								)
							)
							((Said '/west')
								(HighPrint 76 43) ; "The forest becomes so lush and thick that it is difficult to see far."
							)
							((Said '/south')
								(HighPrint 76 44) ; "The brush and undergrowth has formed an impassable thicket."
							)
							((Said '/north')
								(HighPrint 76 45) ; "You see thick brush, rocks, and heavy woods to the north. A large oak tree dominates the view."
							)
							((Said '/east')
								(HighPrint 76 46) ; "You see a clearing back the way you came."
							)
							((Said '/bush')
								(HighPrint 76 47) ; "Most of the bushes around here seem to be bramble and are very thorny."
							)
						)
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(gEgo posn: 300 130 init: setMotion: MoveTo 290 130)
				(= local0 1)
			)
			(6
				(= cycles 1)
			...
			(7
				(if (not (IsFlag 66))
					(HighPrint 76 48) ; "You follow the stag into this forest corner. You feel as though the eyes of the forest are watching you."
				else
					(HighPrint 76 49) ; "Again, you feel a sense of closeness with nature."
				)
				(stag
					setLoop: 9
					cel: 0
			...
			(9
				(if (not (IsFlag 66))
					(HighPrint 76 50) ; "You watch the stag, fascinated with his grace and beauty. There is something special about this place."
				else
					(HighPrint 76 51) ; "What a beautiful animal!"
				)
		...
		(switch (= state newState)
			...
			(3
				(cond
					...
					((IsFlag 198)
						(HighPrint 76 52) ; "Have you brought the seed I requested?"
						(= global114 150)
						(User canInput: 1)
					)
					...
					((IsFlag 197)
						(HighPrint 76 53) ; "Well, Hero-to-be! Are you yet a friend of the woods?"
						(= global114 150)
						(User canInput: 1)
					)
					(else
						(HighPrint 76 54) ; "I am the Dryad, keeper of the woods. Are you one with the woods?"
						(= global114 150)
						(User canInput: 1)
					)
				)
		...
		(switch (= state newState)
			...
			(3
				(if (IsFlag 201)
					...
					(= local1 1)
					(HighPrint 76 55) ; "You see an acorn fall slowly to the ground."
				)
				(HandsOn)
				(= local0 3)
				(dryad setScript: 0)
...
(instance hasSeed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo view: 72 setLoop: 0 cel: 1 forceUpd:)
				(= cycles 15)
			)
			(3
			...
			(4
				(HighPrint 76 56) ; "You drop the seed into the Dryad's limbs."
				(SolvePuzzle 694 7)
				(gEgo use: 20 setCycle: End self) ; seed
			)
			(5
				(if (or (IsFlag 137) (IsFlag 138) (IsFlag 139))
					(HighPrint 76 57) ; "You tell the Dryad: "No amount of persuasion could convince the flowers to relinquish their seed easily, so I was forced to hack my way to the seed with my weapon.""
			...
			(6
				(HighPrint 76 58) ; "Now this will boldly grow where none has grown before!"
				(self cue:)
			)
			(7
				(HighPrint 76 59) ; "Heed now my words, friend of the forest, and heed them well."
				(HighPrint 76 60) ; "Friend, you must know that there is an evil in this valley which perverts the ways of nature. The prophecy says that a hero will bring a young human from out this darkness."
				(HighPrint 76 61) ; "If you are the one to accomplish this task, there is a potion to break enchantments which you must have the Healer make."
				(HighPrint 76 62) ; "You must gather these ingredients:"
				(HighPrint 76 63) ; "Flowers from Erana's Peace,"
				(HighPrint 76 64) ; "Green Fur,"
				(HighPrint 76 65) ; "Fairy Dust,"
				(HighPrint 76 66) ; "a Magic Acorn,"
				(HighPrint 76 67) ; "and Flying Water."
				(HighPrint 76 68) ; "Farewell, friend. I must return to my concentration. May the forest forever surround you."
				(SetFlag 201)
				(ClearFlag 198)
				(dryad setScript: intoTree)
			)
		...
		(switch (= state newState)
			...
			(2
				(cond
					...
					(local2
						(HighPrint 76 69) ; "You pick it up and eat it."
					)
					(else
						(HighPrint 76 70) ; "The acorn appears too ordinary. You drop it back on the ground."
					)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(stag setScript: stagBolts)
				(HighPrint 76 72) ; "You have destroyed the plant that was to be preserved!"
				(HighPrint 76 73) ; "As you have destroyed, so shall you now become!"
				(self cue:)
			)
			(1
			...
			(11
				(EgoDead ; "Well, it looks like the Dryad really made you eat dirt. You're the spitting image of a Spirea plant, and you'll be hanging around for a long, long time."
					76
					74
					80
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond
					((IsFlag 203)
						(HighPrint 76 75) ; "You have attempted to harm a free creature of the forest!"
						(HighPrint 76 76) ; "You shall now become a part of the forest!"
					)
					(local6
						(HighPrint 76 77) ; "You have misused your skills, oh Magician! You have used the Flame Dart on my forest."
						(HighPrint 76 78) ; "You must be made less dangerous."
					)
					(else
						(HighPrint 76 79) ; "You dare to harm the Dryad of this forest! You must be made less dangerous."
					)
				)
				(self cue:)
			)
...
(instance goToSleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(TimePrint 5 76 81) ; "You sleep comfortably and well in this quiet and protected corner of the woods."
				(= seconds 5)
			)
			(3
				(HandsOn)
