...
(local
	...
	[local217 4] = [48 122 198 274]
	[local221 9] = [0 0 10 10 10 0 -10 -10 -10]
	[local230 9] = [6 -6 -6 0 6 6 6 0 -6]
)
...
(procedure (localproc_9 param1 &tmp temp0 temp1 temp2)
	(if (= temp1 [local11 param1])
		...
		(temp1 posn: [local11 (+ 32 temp0)] [local11 (+ 33 temp0)])
	)
)

(procedure (localproc_10 param1 param2 param3 &tmp temp0 temp1 temp2 temp3 temp4)
	(switch param2
		(3
			(= temp0 (= temp2 0))
			(= temp1 8)
...
(class SpecialBug of MazeBug
	...
	(method (nearRock param1)
		(localproc_10 self 3 param1)
	)

	(method (nearBridge param1)
		(localproc_10 self 2 param1)
	)

	(method (nearLadder param1)
		(localproc_10 self 1 param1)
	)
...
(instance wizGame of Rm
	...
	(method (dispose)
		...
		(Joystick 12 0)
		(LoadMany 0 238 205 810)
		(super dispose:)
	)

	(method (init &tmp temp0 temp1 temp2 temp3 temp4)
		(LoadMany rsSCRIPT 238 205 810)
		(LoadMany rsVIEW (LangSwitch 32 35) 33)
		(LoadMany rsVIEW (LangSwitch global302 806))
		(super init: &rest)
		(gGame setCursor: gNormalCursor 1)
		(++ global593)
		(SolvePuzzle 616 5 1)
	...
	(method (cue &tmp temp0 temp1)
		(cond
			...
			((not (gEgo knows: 20))
				...
				(Printf 32 2 global593 temp1 temp0 global593 temp0) ; ""Well, you seem to have beginner's luck. It only took you %d game%s to win.  Hmph. It usually takes at least %d tries for anyone less than a full Wizard to defeat me."%"
...
(instance chooseSpell of Script
	...
	(method (handleEvent event &tmp [temp0 3] temp3 temp4)
		...
		(cond
			...
			((== (event type:) evMOUSEBUTTON)
				...
				(event claimed: 1)
				(cond
					((localproc_1 temp3 temp4 17 18 79 29)
						(= global202 0)
						(= global203 2)
						(self dispose:)
					)
					((localproc_1 temp3 temp4 91 18 153 29)
						(= global202 1)
						(= global203 3)
						(self dispose:)
					)
					((localproc_1 temp3 temp4 167 18 229 29)
						(= global202 2)
						(= global203 4)
						(self dispose:)
					)
					((localproc_1 temp3 temp4 243 18 305 29)
						(= global202 3)
						(= global203 5)
						(self dispose:)
					)
			...
			((or (Said 'cast/fetch,fetch') (Said 'fetch,fetch,get,get'))
				(= global202 0)
				(= global203 2)
				(self dispose:)
			)
			((or (Said 'cast/open,open,open') (Said 'open,open,open'))
				(= global202 1)
				(= global203 3)
				(self dispose:)
			)
			...
			(
				(or
					...
					(Said '/flame,fire,torch,torch,fireball')
				)
				(= global202 3)
				(= global203 5)
				(self dispose:)
			...
			((Said 'throw')
				(Print 32 5) ; "Your bug doesn't have any hands. This makes throwing anything out of the question."
			)
			((or (Said 'sneak') (Said 'run[/!*]'))
				(Print 32 6) ; "Looks like all you can do is wriggle."
			)
			((Said 'drink,drink')
				(event claimed: 0)
			)
			(else
				(Print 32 7) ; "Better concentrate on the game."
				(event claimed: 1)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(SetCursor gTheCursor 1 [local217 global202] 23)
				(objList empty:)
				(localproc_0)
			)
		)
...
(instance doFlame of Script
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3)
		(cond
			((super handleEvent: event))
			((== (event type:) $0040) ; direction
				(= temp1 [local221 (= temp3 (event message:))])
				(= temp2 [local230 temp3])
				(cond
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(flameSign stopUpd:)
				(HandsOn)
				(flame posn: 274 23 setCycle: Fwd)
			)
		)
	)
)
