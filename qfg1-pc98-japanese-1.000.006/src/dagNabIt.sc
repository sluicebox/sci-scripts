...
(instance dagNabIt of Rm
	...
	(method (init)
		(super init: &rest)
		(Load rsVIEW 340)
		(Load rsVIEW (LangSwitch 341 342))
...
(instance dagnabitScript of Script
	...
	(method (handleEvent event &tmp temp0 temp1 temp2)
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					((Said 'stop,end,end,done,done,quit,done')
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(= local35 0)
				(if 81
					(localproc_1 local14 14 47 174)
				else
					(localproc_1 local14 14 47 173)
				)
				(if 81
					(localproc_1 local15 11 73 174)
				else
