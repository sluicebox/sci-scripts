				...
				(cond
					((Said 'look,look>')
						(cond
							...
							((Said '/blossom')
								(HighPrint 10 17) ; "The tree's blossoms are soft, green and fragrant."
							)
							((Said '/apple')
								(if gNight
									(HighPrint 10 18) ; "The fruit glows and shimmers in the moonlight. It produces a fragrance like a sun-ripened strawberry."
								else
									(HighPrint 10 19) ; "The fruit sparkles in the sunlight, yet it appears soft and fuzzy."
								)
							)
							((Said '/hill,north,dragon,smoke,peak')
								(HighPrint 10 20) ; "To the north, the high, snowy peaks of the Dragon Smoke mountains are clearly visible."
							)
							((Said '/cliff,cliff,east,west')
								(HighPrint 10 21) ; "The meadow is an oasis at the edge of the snowline. The rocky hillside looks very steep and slippery."
							)
							((Said '/ice')
								(HighPrint 10 22) ; "You are at the snowline. All around you, except for the forest, the rocks and mountains glisten with slippery snow."
							)
							((Said '/south,forest')
								(HighPrint 10 23) ; "To the south, you see the deep forest through which you came to this meadow."
							)
							(
								(or
									(Said '/boulder,brick')
									(MouseClaimed magicStone event 3)
								)
								(if local1
									(HighPrint 10 24) ; "The stone has been moved."
								else
									(HighPrint 10 25) ; "The large stone appears to be ancient and deliberately placed. Marks carved into the stone almost look like writing."
								)
							)
							...
							((Said '/chasm')
								(cond
									((not local1)
										(HighPrint 10 26) ; "You see no holes."
									)
									((not (IsFlag 141))
										(localproc_0)
									)
									((gEgo inRect: 80 115 160 160)
										(HighPrint 10 27) ; "The hole that was beneath the stone is empty."
									)
									(else
										(NotClose)
									)
					...
					((Said 'open,open,open,force,move,move/brick,boulder')
						(localproc_5)
					)
					((Said 'eat,eat/apple')
						(cond
							...
							((not (gEgo inRect: 109 88 223 106))
								(HighPrint 10 28) ; "Go over to the tree and pick some."
							)
							(else
								(HighPrint 10 29) ; "The sweet, juicy fruit of the tree is amazingly satisfying and refreshing."
								(= gFreeMeals 4)
								(ClearFlag 124)
								(ClearFlag 125)
								(SolvePuzzle 645 2)
						...
						(cond
							((Said '/apple')
								(cond
									...
									(else
										(HighPrint 10 30) ; "The fruit is very soft and juicy.  It would be impossible to keep in your pack."
									)
								)
							)
							((Said '/grass')
								(HighPrint 10 31) ; "The grasses in the meadow are covered over with flowers."
							)
							((Said '/boulder,brick')
								(localproc_5)
							)
							...
							((Said '/flower')
								(if (IsFlag 140)
									(HighPrint 10 32) ; "You take another handful of the lovely, fragrant flowers."
								else
									(HighPrint 10 33) ; "As you pick a variety of the sweet-smelling flowers, they seem to glow in your hands. You put them safely away."
								)
								(SetFlag 140)
								(gEgo get: 22 5) ; flower
							)
							((Said '/blossom')
								(HighPrint 10 34) ; "The lovely blossoms should stay on the tree."
							)
						)
					)
					((Said 'smell,smell,odor/flower,blossom,grass,clearing')
						(HighPrint 10 35) ; "The smell reminds you of laughter."
					)
					((Said 'cast>')
						(switch (= temp0 (SaidSpell event))
							(17
								(cond
									(local1
										(HighPrint 10 36) ; "There is no further purpose in casting the Open spell."
									)
									((CastSpell temp0)
							...
							(18
								(if (CastSpell temp0)
									(HighPrint 10 37) ; "There is an aura of magic throughout this meadow. It seems to be benevolent and restorative."
								)
							)
							(else
								(event claimed: 0)
					...
					((or (Said 'nap,nap') (Said 'go[<to]/nap,nap'))
						(if (not (CanSleep))
							(HighPrint 10 38) ; "You just can't sleep during the daytime."
							(DisposeScript 7)
						else
							(SolvePuzzle 615 5 1)
					...
					((Said 'japaneseclimbup,climb,climb')
						(HighPrint 10 39) ; "The rock faces are slippery with melted snow, and there is no need to climb the little tree."
					)
					((Said 'throw')
						(HighPrint 10 40) ; "The atmosphere here is peaceful and calm.  There is no need to throw anything."
					)
				)
			)
		)
...
(instance goToSleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TimePrint 3 10 41) ; "You sleep comfortably among the fragrant flowers."
				(= seconds 3)
			)
			(2
				(EgoSleeps 6 0)
		...
		(cond
			...
			(
				(or
					...
					(MouseClaimed onTrunk event 3)
					(Said 'look,look/tree,bush,trunk')
				)
				(HighPrint 10 42) ; "The small tree is most amazing. It bears blossoms and fruit at the same time, and the fruit on its boughs seems to shyly appear and disappear, shimmering."
			)
		)
	)
)
