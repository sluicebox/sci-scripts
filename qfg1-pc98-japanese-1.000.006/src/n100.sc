...
(procedure (CastDart param1 &tmp temp0 temp1 temp2)
	...
	(if param1
		...
		(if
			(and
				...
				(not (TrySkill 12 0 (- 50 (/ (gEgo distanceTo: param1) 5)))) ; magic
			)
			(if (< local4 0)
	...
	(method (doit &tmp temp0)
		...
		(if
			(and
				(== state 1)
				(or
					(= temp0 (and (< -25 local1 25) (< -25 local2 25)))
					(not (client mover:))
					(>
						(GetDistance (client x:) (client y:) local3 local4)
						(+
							25
							(GetDistance
								(gEgo x:)
