		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'open,open,open,lift,lift/gate,portcullis,door')
						(HighPrint 37 1) ; "There is nobody to open the portcullis."
					)
					(
						(Said
							'japaneseclimbup,climb,climb,climb,climb[/gate,wall,gatehouse]'
						)
						(if gNight
							(cond
...
(instance closeGate of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(if local7
					(EgoDead ; "You're not a quick learner. Now you known why the grass grows so well around the portcullis."
						37
						18
						80
		...
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					(
						(or
							(Said 'ask//gate,portcullis,door<open,open,open')
							(Said
								'open,open,open,lift,lift/gate,portcullis,door'
							)
						)
						(if (localproc_0)
							(cond
					...
					(
						(Said
							'close,close,close,close,lower,lower/gate,portcullis,door'
						)
						(if (localproc_0)
							(if local0
						...
						(cond
							...
							(
								(or
									(Said '//in<go,come')
									(Said
										'//portcullis,gate,door<open,open,open,lift,lift'
									)
								)
								(if (localproc_0)
									(= local11 0)
							...
							((Said '//job,labor,labor,labor,labor')
								(if (localproc_0)
							...
							(
								(Said
									'//(master[<weapon,about]),attack,practice,practice'
								)
								(if (localproc_0)
		...
		(cond
			...
			(
				...
				(cond
					...
					(else
						(HighPrint 37 87) ; "The gatehouse is a massive structure with a portcullis closing off access to the castle."
					)
				)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(HighPrint 37 88) ; "The mountains in the distance ring the valley of Spielburg, creating a formidable barrier to travelers."
			)
		)
	)
)
