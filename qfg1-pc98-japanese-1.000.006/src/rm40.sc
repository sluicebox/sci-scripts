		...
		(switch (event type:)
			(evSAID
				(cond
						...
						(cond
							...
							(
								(or
									(Said '<down')
									(Said
										'/ground,floor,hay,shit,shit,shit,shit'
									)
								)
								(if (IsFlag 217)
					...
					((Said 'talk,talk/man,stablekeeper')
						(if local23
					...
					((Said 'ask>')
						(cond
							((not local23)
								(HighPrint 40 22) ; "The stableman can't hear you."
								(event claimed: 1)
							)
							((Said '//nap,nap')
								(HighPrint 40 23) ; "If you put in a day's work, and you wanna take a nap, it's OK with me."
							)
							((Said '//horse,animal')
								(HighPrint 40 24) ; "These horses, they act just like animals or somethin'."
							)
							((Said '//baron,barnard,castle,hamlet,valley')
								(HighPrint 40 25) ; "Just get out and wander around a little. You'll find out."
							)
							((Said '//magic,baba,zara,erasmus,spell,potion')
								(HighPrint 40 26) ; "Don't ask me about anything to do with magic. I don't wanna know."
							)
							((Said '//monster,troll,cheetaur,saurus')
								(HighPrint 40 27) ; "Those things scare me silly!"
							)
							((Said '//antwerp')
								(HighPrint 40 28) ; "Ha-ha-ha-ha-ha! Hee-hee-hee-ho-ho!"
							)
							((Said '//labor,labor,hoe,shit,shit')
								(if (IsFlag 217)
									(localproc_3)
								else
									(HighPrint 40 29) ; "Sure! C'mon in and grab a rake."
									(gEgo setScript: goToWork)
								)
							)
							((Said '//*')
								(HighPrint 40 30) ; "Don't tire me out with a bunch of questions. I don't need your comments, either. Can't you see what a hard life I have?"
							)
						)
					)
					((Said 'buy,get,get/horse')
						(HighPrint 40 31) ; "Sorry, the Baron's horses aren't for sale."
					)
					((Said '/horse')
						(HighPrint 40 32) ; ""Neigh!" says the horse."
					)
				)
			)
		)
...
(instance getPaid of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(TimePrint 3 40 33) ; "OK. C'mon over here."
			...
			(4
				(TimePrint 5 40 34) ; "The stableman hands you some coins and says, "Now you're five silvers richer.""
				(gEgo get: 1 5) ; silver
				(= seconds 5)
			)
			(5
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(EgoGait 0 0) ; walking
				(gEgo hide:)
				(HighPrint 40 35) ; "You curl up on some clean hay and get some rest."
				(self cue:)
			)
			(1
				(if local24
			...
			(3
				(TimePrint 4 40 36) ; "Z-Z-Z-Z-Z-Z-Z-Z-Z-Z..."
				(= seconds 4)
			)
			(4
				(if gNight
...
(instance sleepyIntro of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HighPrint 40 37) ; "It IS a rude awakening, however."
				(= cycles 2)
			)
			(1
				(HighPrint 40 38) ; "WHAT DO YOU THINK THIS IS, A REST HOME??? GET TO WORK! NOW!!!"
				(= cycles 10)
			)
			(2
...
(instance intro of Script
	...
	(method (doit)
		(cond
			...
			((== global114 1)
				(= global114 0)
				(TimePrint 5 40 39) ; "Well, if you don't want to answer..."
				(self changeState: 5)
			)
		)
		(super doit:)
	...
	(method (handleEvent event)
		(if (and (== (event type:) evSAID) (> global114 1))
			(cond
				...
				((Said 'yes,please')
					(= global114 0)
					(HighPrint 40 40) ; "Good! Come in here and take a rake."
					(gEgo setScript: goToWork)
					(self changeState: 5)
				)
				((Said 'no')
					(= global114 0)
					(HighPrint 40 41) ; "Too bad! I could use some help.""
					(self changeState: 5)
				)
				((Said 'ask//labor,labor,hoe,shit,shit')
					(HighPrint 40 42) ; "That's what I'm asking YOU about, nimrod."
				)
				(else
					(event claimed: 1)
					(self changeState: 3)
		...
		(switch (= state newState)
			...
			(1
				(cond
					...
					((IsFlag 229)
						(TimePrint 3 40 43) ; "I see you're back. Need some work?"
						(= global114 120)
						(HandsOff)
						(User canInput: 1)
					)
					(else
						...
						(User canInput: 1)
						(if (== (gEgo onControl: 1) 16384)
							(HighPrint 40 44) ; "Someone approaches you."
							(TimePrint 3 40 45) ; "You lookin' for some work?"
						else
							(TimePrint 3 40 45) ; "You lookin' for some work?"
						)
						(= global114 120)
			...
			(3
				(TimePrint 3 40 46) ; "Quit fidgeting and answer me...yes or no!"
