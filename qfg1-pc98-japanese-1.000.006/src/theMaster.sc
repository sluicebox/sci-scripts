		...
		(switch (event type:)
			(evSAID
				(cond
					...
					((Said 'talk,talk/man,swordsman,master')
						(if (not local0)
					...
					((Said 'talk,talk,ask>')
						...
						(cond
							(
								(Said
									'//attack,art,skill,strength,strength,agility'
								)
							...
							((Said '//teach,learn,lesson,practice,practice')
					...
					(
						(or
							(Said 'practice,practice,attack,learn')
							(Said 'buy,get,get/lesson')
						)
						(localproc_1)
					)
				)
