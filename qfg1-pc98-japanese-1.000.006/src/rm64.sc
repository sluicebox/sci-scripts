		...
		(switch (event type:)
			(evSAID
				(cond
					...
					(
						(Said
							'look,look,read/brick,marker,gravestone,epitaph[<grave]'
						)
						(cond
						...
						(cond
							...
							((Said '/root,mandrake,plant')
								(cond
									...
									((not (gCast contains: mandrake))
										(HighPrint 64 28) ; "You don't see any particularly interesting roots."
									)
									((gEgo inRect: 130 113 168 140)
										(HighPrint 64 7) ; "Growing up out of the grave is an evil-looking plant with a slimy, poisonous sheen."
									)
									(else
										(HighPrint 64 29) ; "The red root growing out of one of the graves has a strange and evil appearance."
									)
								)
							)
							((Said '[<at,around][/!*,cemetery,cemetery]')
								(if gNight
									(HighPrint 64 30) ; "It appears that the spirits frolic here during the dark hours."
								else
									(HighPrint 64 31) ; "It's certainly creepy here even during the daylight hours."
								)
							)
						)
					)
					((Said 'kill')
						(HighPrint 64 32) ; "Everything here is already dead."
					)
					((Said 'get,get>')
						(cond
							((Said '/ghost')
								(HighPrint 64 33) ; "You don't get ghosts. Ghosts get YOU!"
							)
		...
		(if
			(and
				(or
					...
					(< (gEgo distanceTo: twister) 25)
					(< (gEgo distanceTo: tumbler) 25)
				)
				(not local2)
			)
			(= local2 1)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(twister setMotion: Follow gEgo 5)
				(tumbler setMotion: Follow gEgo 5)
				(= cycles 15)
			)
			(3
...
(instance getHimSouth of Script ; UNUSED
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not (IsFlag 114))
					(HighPrint 64 35) ; "It appears that you have company in the graveyard tonight."
				)
			)
			(2
				(HandsOff)
...
(instance safeIntro of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 64 36) ; "This must be the town graveyard. What a run-down place!"
				(if gNight
					(HighPrint 64 30) ; "It appears that the spirits frolic here during the dark hours."
				else
					(HighPrint 64 31) ; "It's certainly creepy here even during the daylight hours."
...
(instance fallInHole of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead ; "You stepped into a hole whose depth is unfathomable. As you fall ever downward, you feel icy fingers clutching at you, and you think: "How could I be so careless?""
					64
					37
					82
					800
...
(instance getRoot of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (== gTimeOfDay 6)
					(HighPrint 64 38) ; "You wrench the mandrake plant free from the tombstone on which it has grown. You hear a scream like that of a dying child as you yank the root from the ground."
					(SolvePuzzle 683 6)
					(gEgo get: 4) ; mandrake
				else
					(HighPrint 64 39) ; "You wrench the mandrake plant free from the tombstone on which it has grown. Somehow you thought this would be a lot harder. You wonder if you did this right."
					(HighPrint 64 40) ; "The root shrivels and twists in your hands until nothing is left. Astonished, you look at your empty hands."
					(= global277 gDay)
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= local6 local4)
				(= local7 local5)
				(= local4 (Random 20 300))
				(= local5 (Random 10 130))
				(twister setCycle: (if (> local4 local6) Fwd else Rev))
				(if (IsFlag 164)
