...
(instance rm84 of Rm
	...
	(method (handleEvent event &tmp [temp0 90] temp90 temp91)
		(switch (event type:)
			(evMOUSEBUTTON
				(cond
					((super handleEvent: event))
					((MouseClaimed gEgo event 3)
						(HighPrint (Format @temp0 84 0 @global401 @global594)) ; "By golly, It's %s!%"
					)
				)
			)
			(evSAID
				(cond
					...
					((Said 'look,look,search>')
						(cond
							...
							((Said '/door,keyhole,hasp,hasp,lock')
					...
					(
						(Said
							'attack,kill[/bouncer,antwerp,animal,beast,monster,creature]'
						)
						(cond
							((IsFlag 206)
								(HighPrint 84 4) ; "The only Antwerp known in these parts has split... into parts."
					...
					(
						(or
							(Said
								'unlock,pick,lockpick/door,boulder,hasp,hasp,lock,keyhole'
							)
							(Said 'use/key,lockpick')
							(Said 'open,open,open/hasp,hasp,lock,keyhole')
						...
						(cond
							...
							((IsFlag 252)
								(HighPrint 84 26) ; "It's already unlocked."
							)
							((and (gEgo has: 5) (IsFlag 245)) ; key
								(HighPrint 84 27) ; "The lock in the rock clicks open."
								(SetFlag 252)
							)
							((not (CanPickLocks))
								(HighPrint 84 28) ; "You'd have a much easier time of this if you had the key."
							)
							((TrySkill 9 85 global189) ; pick locks
								(HighPrint 84 29) ; "Ah, got it! The lock in the rock clicks open."
								(SetFlag 252)
							)
							(else
								(HighPrint 84 30) ; "The lock is beyond your present skill. It might help if you had the key."
								(if (not (gEgo has: 24)) ; thief kit
									(HighPrint 84 31) ; "... Or at least a better set of tools."
								)
							)
						)
					)
					(
						(Said
							'shove,move,move,force,get,open,open,open/boulder,door'
						)
						(if (not (IsFlag 253))
							(if (IsFlag 252)
								(if (TrySkill 0 40 0) ; strength
									...
									(HighPrint 84 32) ; "You are not strong enough yet to open the rock door."
								)
							else
								(HighPrint 84 33) ; "Despite your mightiest efforts, the rock does not move."
							)
						else
							(HighPrint 84 34) ; "The rock door has already been opened."
						)
					)
					((or (Said 'say,yell<goseke/goseke') (Said 'goseke/goseke'))
						(if
							(and
								(IsFlag 235)
							...
							(SetFlag 247)
							(SolvePuzzle 702 5)
							(HighPrint 84 35) ; "You hear the sound of someone... or something...moving deeper into the cave to let you pass."
						else
							(HighPrint 84 36) ; "Ok, you say Hiden Goseke."
						)
					)
					((Said 'enter/cave,entrance')
						(HighPrint 84 37) ; "Go ahead. If you dare. \82\a2\82\a2\82\e6\81B\82\e0\82\b5\81A\82\c7\82\a4\82\b5\82\c4\82\e0\82\bb\82\a4\82\b5\82\bd\82\a2\82\f1\82\c8\82\e7\82\cb\81B"
					)
				)
			)
		)
...
(instance antwerp of TargActor
	...
	(method (getHurt)
		(HighPrint 84 38) ; "That's funny. It bounced right off."
	)
		...
		(switch (event type:)
			(evMOUSEBUTTON
				(if (MouseClaimed antwerp event 3)
					(HighPrint 84 39) ; "By golly, it's an Antwerp!"
				)
			)
			(evSAID
				(cond
					...
					(
						...
						(HighPrint 84 40) ; "The Antwerp plays rough."
					)
				)
			)
		)
		...
		(switch (= state newState)
			...
			(6
				(EgoDead ; "The old ticker just couldn't keep going. Maybe you shouldn't have missed the annual visit with your local Healer."
					...
					5
					80
					{Your figure remains still and silent.%j\b7\d0\c9 \bd\b6\de\c0\ca \c4\cf\d8 \c1\dd\d3\b8\a6 \c2\c2\de\b9\d9}
				)
			)
		)
	)
...
(instance fightAntwerp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(TimePrint 5 84 42) ; "Cockily, you loosen up to fight."
			...
			(9
				(TimePrint 4 84 43) ; "Confident and loose, you approach the Antwerp."
				(NormalEgo)
				(gEgo
					setLoop: 1
					moveSpeed: 1
			...
			(23
				...
				(SetFlag 204)
				(HandsOn)
				(TimePrint 3 84 44) ; "Holy Mackerel!"
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(1
				...
				(if (< [gEgoStats 17] 50) ; openSpell
					(HighPrint 84 45) ; "Your spell has unlocked the lock on the rock, but it is not yet powerful enough to open the rock door."
					(HandsOn)
					(NormalEgo)
					(gEgo illegalBits: -32704)
					(client setScript: 0)
...
(instance antwerpAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HighPrint 84 46) ; "You seem to have scared the Antwerp with your behavior."
			)
		)
	)
)
