...
(instance ring of Prop
	...
	(method (handleEvent event)
		(cond
			((Said 'look,look/glimmer,reflection,chandelier,chandelier')
				(if (not (IsFlag 210))
			...
			((Said 'japaneseclimbup,climb,climb/branch')
				(cond
					((IsFlag 210)
						(AlreadyDone)
					)
...
(instance bird of Act
	...
	(method (handleEvent event)
		(cond
			(
				(or
					(Said 'look,look/bird,creature,lizard')
					(MouseClaimed bird event 3)
				)
				(HighPrint 54 9) ; "The creature looks more like a small flying lizard than a bird."
			)
			((Said 'eat,eat,get,get,kill,attack,cut,chop,hit/bird')
...
(instance nest of Act
	...
	(method (handleEvent event)
		(cond
			((or (Said 'look,look/nest') (MouseClaimed nest event 3))
				(cond
					((not (IsFlag 212))
			...
			(evSAID
				(cond
					...
					(
						(and
							(IsFlag 278)
							(or (Said 'throw,walk,sneak') (Said 'run[/!*]'))
						)
					...
					(
						(or
							(Said 'wear/ring,ring')
							(Said 'put[<on]/ring,ring,finger')
							(Said 'put/ring,ring/finger')
						)
								...
								(switch temp0
									...
									(24
										(if
											...
											(HighPrint 54 27) ; "That spell is no longer useful here (but you did get some practice)."
										)
									)
									(else
										(event claimed: 0)
					...
					((Said 'climb,climb,japaneseclimbup,japaneseclimbdown>')
						(cond
							(
								(or
									(Said
										'[climb,climb,japaneseclimbup]/tree,oak'
									)
									(Said '<up')
								...
								(cond
									...
									((gEgo inRect: 0 167 112 189)
										(if (TrySkill 11 30 0) ; climbing
											(gEgo setScript: climbTree)
										else
											(gEgo setScript: cantClimbTree)
										)
									)
									((IsFlag 278)
							...
							((or (Said '<down') (Said 'japaneseclimbdown'))
								(if (IsFlag 278)
					...
					((Said 'grab,get,get,pick,lockpick/herb,flower,plant')
					...
					((Said 'look,look>')
						(cond
							...
							((Said '/north,castle')
								(HighPrint 54 62) ; "The road winds northward toward a walled castle. \93\b9\82\cd\81A\96k\95\fb\82\cc\82\a8\8f\e9\82\cc\95\bb\82\d6\82\c6\81A\8b\c8\82\aa\82\e8\82\ad\82\cb\82\c1\82\c4\91\b1\82\a2\82\c4\82\a2\82\e9\81B"
							)
			...
			(1
				(if (not (TakeDamage 8))
					...
					(switch (= local6 (Random 1 5))
						...
						(else
							(HighPrint 54 76) ; "Remember what happened to Humpty Dumpty. \83n\83\93\83s\83e\83B\81[\81E\83_\83\93\83s\83e\83B\81[\82\aa\82\c7\82\f1\82\c8\89H\96\da\82\c9\82\c8\82\c1\82\bd\82\a9\89\af\82\a6\82\c4\82\e9\82\be\82\eb\82\a4\81B"
						)
					)
					(++ local6)
					(self cue:)
...
(instance intro54 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 54 78) ; "The bright smell of fresh herbs mingles with the aroma of wood smoke as you near a hut by the side of the road."
				(client setScript: 0)
			)
		)
	)
...
(instance cantClimbTree of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HighPrint 54 79) ; "It takes a lot of skill and practice to climb this tree."
				(HandsOn)
				(NormalEgo)
...
(instance outOnALimb of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(HighPrint 54 80) ; "You carefully bend down and reach into the nest."
				(gEgo posn: 59 72 setLoop: 9 setCycle: End self)
			)
			(7
				(HighPrint 54 81) ; "In the nest, you find a gold ring."
				(HighPrint 54 82) ; "You take the ring and move carefully back along the limb."
				(SetFlag 210)
			...
			(14
				(HighPrint 54 83) ; "Made it! Now back down the tree."
				(gEgo setScript: climbDown)
			)
		)
	)
...
(instance getRock of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(= local11 (gEgo signal:))
				(= local12 (gEgo priority:))
				(= local13 (gEgo illegalBits:))
				(HandsOff)
				(gEgo
					view: 510
			...
			(1
				(TimePrint 4 54 84) ; "You pick up a few small rocks."
				(gEgo get: 21 10 setCycle: Beg self) ; boulder
			)
			(2
				(NormalEgo)
				(HandsOn)
				(gEgo priority: local12 illegalBits: local13 signal: local11)
				(client setScript: 0)
			)
		)
	)
