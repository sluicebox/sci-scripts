...
(instance delFromCave of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(if (not (IsFlag 324))
					((ScriptID 2 1) init: 1 1 self) ; Adam, "Cave diving has always made me nervous. My dad completely forbids it."
				else
			...
			(4
				((ScriptID 2 0) init: 6 1 self) ; Delphineus, "I agree with him! No way we should go in there without a light of some kind."
			)
			(5
				(gDelph setScript: delFromWest)
				(self dispose:)
...
(instance redFishGoBack of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Narrator init: 17 self) ; "Psych! The red fish takes off again!"
			)
			(3
		...
		(switch theVerb
			(3 ; Do
				(if (IsFlag 73)
					...
					(Narrator init: 12) ; "Adam can't reach that from here."
				)
			)
			(else
		...
		(switch theVerb
			(2 ; Look
				(if (IsFlag 73)
					...
					((ScriptID 2 0) init: 1 1) ; Delphineus, "It looks scary in there, Adam! Feel all that cold water?"
				)
			)
			(else
				(super doVerb: theVerb &rest)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((not (IsFlag 73))
						((ScriptID 2 0) init: 5 1) ; Delphineus, "Nasty. Leave them alone."
					)
					(else
						(Narrator init: 15) ; "Adam picks up as much as he can, but there's just too much to do."
					)
				)
			)
			(10 ; Recycle
...
(instance deadPlants of Feature
	(properties
		onMeCheck 2048
		lookStr 14
	)
		...
		(switch theVerb
			(3 ; Do
				(if (IsFlag 73)
					...
					(Narrator init: 13) ; "The coral is lifeless. It's useless to touch it."
				)
			)
			(10 ; Recycle
				(Narrator init: 15) ; "Adam picks up as much as he can, but there's just too much to do."
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((< y 50)
						(Narrator init: 4) ; "The reef ends here in a huge cliff. A dark opening and colder water promise depths beyond."
					)
					(else
						(Narrator init: 5) ; "A cliff rises at the reef's end."
					)
				)
			)
			(else
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((not (IsFlag 73))
						((ScriptID 2 0) init: 2 1) ; Delphineus, "Yech - that junk sticks to everything."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance greenPlants of Feature
	(properties
		onMeCheck 128
		lookStr 8
	)
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((not (IsFlag 73))
						((ScriptID 2 0) init: 3 1) ; Delphineus, "Adam! Stay away from that stuff! I've never seen anything like it down here."
					)
					(else
						(super doVerb: theVerb &rest)
					)
...
(instance distantStuff of Feature
	(properties
		onMeCheck 64
		lookStr 6
	)
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((not (IsFlag 73))
						((ScriptID 2 0) init: 4) ; Delphineus, "Dead staghorn coral. Remember the stuff we saw before? It's hard to believe this was once the same."
					)
					(else
						(super doVerb: theVerb &rest)
					)
