	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			...
			(2 ; Look
				...
				(Narrator init: 11) ; "It's a Conquistador's pike. Someone made sure the pirate on duty would get the point."
			)
			(else
...
(instance chest of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(if (IsFlag 41)
					((ScriptID 2 1) init: 14 1) ; Adam, "The lid's stuck. Oh, well, it's empty anyhow."
				else
			...
			(2 ; Look
				...
				(if (IsFlag 41)
					(Narrator init: 6) ; "The chest was empty except for the crab. Someone else has been here recently - unless, of course, you believe in hundred-year-old crabs."
				else
			...
			(4 ; Inventory
				(switch invItem
					...
					(30 ; hackSaw
						(Narrator init: 27) ; "No need to do major surgery. The lock is already broken."
					)
					(else
						(Narrator init: 28) ; "The lock has been snapped. The chest should open easily."
					)
				)
			)
			(5 ; Talk
				(Narrator init: 31) ; "The chest gave up its secrets already."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
		...
		(switch theVerb
			...
			(2 ; Look
				(if (and (== local0 2) local2)
					((ScriptID 2 1) init: 16 1) ; Adam, "The key is sinking fast!"
				else
					(Narrator init: 15) ; "The water laps against the beach."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
...
(instance KeyInset of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				((ScriptID 2 1) init: 8 1) ; Adam, "The key's stuck under the pike tip!"
			)
			(2 ; Look
				(Narrator init: 24) ; "No doubt this is a skeleton key."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance Skeleton of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Narrator init: 18) ; "Adam feels creepy about touching a skeleton."
			)
			(5 ; Talk
				(Narrator init: 29) ; ""Are you by any chance related to a guy named LeChuck?"... No answer from the skeleton."
			)
			(2 ; Look
				(if (== local0 2)
					(Narrator init: 4) ; "The pike is gone. The pirate doesn't feel any better however."
				else
					(Narrator init: 3) ; "The pirate ghost has the job of guarding the treasure. The pirate chief made sure he wouldn't quit."
				)
			)
			(4 ; Inventory
				(switch invItem
					(12 ; trident
						(Narrator init: 22) ; "The pirate can't be threatened anymore."
					)
					(30 ; hackSaw
						(Narrator init: 32) ; "What a grisly idea."
					)
					(21 ; healingPotion
						(Narrator init: 34) ; "The time for that is long past."
					)
					(17 ; spine
						(Narrator init: 35) ; "He's already got a spine. Haha."
					)
					(else
						(Narrator init: 33) ; "The skeleton is beyond help."
					)
				)
			)
			(else
...
(instance Sky of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Narrator init: 17) ; "Adam can't get to the surface from here."
			)
			(2 ; Look
				(Narrator init: 20) ; "The reef formation is close to the coastline."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance Cliff of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(Narrator init: 14) ; "Adam's too wet to climb the slippery rock."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance Grass of Feature
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Narrator init: 16) ; "Adam has found a small air pocket in the cliff walls that line the coast."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
...
(instance lookChestTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Narrator init: 5 self) ; "A pirate crew left its treasure in this secret spot. They installed a security system before they left - a pirate ghost."
			)
			(1
				(Narrator init: 12 self) ; "The pirate loot was stored in this chest. Maybe the pirates lost their claim ticket or something."
			)
			(2
				(self dispose:)
			)
...
(instance useKeyOnChestTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Narrator init: 1 self) ; "The key looks more modern than the chest. It won't open that old lock."
			)
			(1
				(Narrator init: 19 self) ; "Anyway the lock has been sawed through."
			)
			(2
				(self dispose:)
			)
...
(instance lookBeachTalk of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Narrator init: 8 self) ; "A small beach is just big enough for a couple of people to stand on."
			)
			(1
				(Narrator init: 9 self) ; "Part of it is occupied by a permanent guest."
			)
			(2
				(self dispose:)
			)
...
(instance doPikeFirst of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				((ScriptID 2 1) init: 15 1 self) ; Adam, "I think I felt it give a little!"
			)
			(11
				(self dispose:)
			)
...
(instance doPikeSecond of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Narrator init: 21 self) ; "The pike snaps into pieces and the key goes flying into the water."
			)
			(6
...
(instance openChest of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Narrator init: 25 self) ; "Dreaming of pirate treasure, Adam opens the chest."
			)
			(3
				(chest hide:)
				(gEgo
			...
			(6
				...
				(Narrator init: 26 self) ; "A tiny blue crab has been napping in the chest. He scuttles away to find new shelter."
			)
			(7
				(chest setCel: 1 show:)
			...
			(9
				((ScriptID 2 1) init: 1 1 self) ; Adam, "I guess he was napping. Not a good place for it."
			)
			(10
				(SetFlag 41)
				(HandsOn)
