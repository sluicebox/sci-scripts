		...
		(switch (= state newState)
			...
			(10
				(if (and (gEgo has: 8) (== gCurRoomNum 381)) ; scubaGear
					((ScriptID 2 1) init: 22 1 self 380) ; Adam, "Okay, now I'm ready! I've got everything!"
				else
					(= ticks 3)
				)
			)
			(11
				(if (and (gEgo has: 8) (== gCurRoomNum 381)) ; scubaGear
					((ScriptID 2 0) init: 23 1 self 380) ; Delphineus, "Hurry up, then! I have the worst feeling we don't have much time."
				else
					(= ticks 3)
				)
			)
...
(instance getItem of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(Narrator init: 89 self 380) ; "Adam picks up the diving equipment and the transmitter."
			)
			(7
