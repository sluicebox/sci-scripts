...
(instance cameFromNorthScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 232 setCycle: Walk heading: 0 setLoop: Grooper)
				(HandsOn)
				(self dispose:)
			)
		)
...
(instance cameFromSouthScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(gEgo view: 232 setPri: -1 setMotion: MoveTo 124 102 self)
			)
			(2
				(HandsOn)
...
(instance climbStairsScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if 1
					(Print 41 0) ; "You don't need to use the emergency hatch at this time."
					(self dispose:)
					(return)
				else
					(HandsOff)
			...
			(1
				(gEgo view: 42 heading: 0 setMotion: MoveTo 124 39 self)
			)
			(2
...
(instance decendStairsScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo view: 42 heading: 0 setPri: 5)
				(if (gEgo looper:)
