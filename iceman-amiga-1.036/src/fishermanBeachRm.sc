...
(instance fishermanBeachRm of Rm
	...
	(method (init)
		...
		(gEgo init: ignoreActors: 0 observeControl: 64 cycleSpeed: 0)
		(switch gPrevRoomNum
			(north
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look/rock')
				(Print 70 2) ; "This is a very rocky area."
			)
		)
	)
)
			...
			((Said '[/man,iceman,(man<ice),disguise,fisherman]>')
				(cond
					((CantBeSeen client gEgo 270)
						(cond
							((Said 'look[<at]/man,fisherman')
								(Print 70 4) ; "Facing in that direction you see no one."
							)
							((Said 'talk')
								(Print 70 5) ; "Please face the fisherman before you speak!"
							)
							((Said '/iceman')
								(Print 70 6) ; "From here the fisherman cannot hear you."
							)
						)
					)
					((Said 'look[<at]')
						(Print 70 7) ; "You see a solitary fisherman pulling in his net."
					)
					((> (gEgo distanceTo: fisherMan) 35)
						(Print 70 8) ; "Move in closer so you don't have to shout."
						(event claimed: 1)
					)
					((Said 'get/disguise')
						(if (proc316_1 gCurRoomNum 3)
							(Print 70 9) ; "The fisherman continues his work as if no one is near."
						else
							(= local2 1)
						)
					)
					((or (gEgo has: 3) (gEgo has: 4) (gEgo has: 5)) ; Tahiti: Black_Book | Sub: Explosive | Tunisia: Fish, Tahiti: Change | Sub: Rum_Bottle | Tunisia: Capsule, Tahiti: Key | Sub: Storage_Compartment_Key | Tunisia: Map
						(Print 70 10) ; "The fisherman ignores you."
						(event claimed: 1)
					)
					((Said '/iceman,(man<ice)')
						(if (proc316_1 gCurRoomNum 3)
							...
							(Print 70 11) ; "The fisherman ignores your additional references to the code name."
							(event claimed: 1)
						)
					)
					((Said 'talk[/man,fisherman]')
						(= local3 1)
					)
				)
			)
...
(instance acknowledgeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(proc310_2)
				(Print 70 12 #at 25 10) ; "Using the Arabic language, you mention code name "Iceman.""
				(Print 70 13 #at 25 10) ; "Without speaking, the fisherman acknowledges by shaking his head yes."
			...
			(8
				(Print 70 14) ; "He then hands you a fish."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(proc310_2)
				(= cycles 1)
				(Print 70 15 #at 25 10) ; "You ask the fisherman the whereabouts of your disguise, and..."
				(Print 70 16 #at 25 10) ; "Persisting in his silence, he turns and points toward the old abandoned dwellings."
			)
			(2
				(fisherMan
					view: 68
...
(instance wontTalkScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(proc310_2)
				(Print 70 17 #at 25 10) ; "Speaking in good Arabic, you try striking up a conversation with the fishermen, but he ignores you."
				(HandsOn)
				(self dispose:)
			)
		)
		...
		(cond
			...
			((Said '[/wave]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 70 18) ; "You see the waves coming to the shore."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/building]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 70 19) ; "Abandoned buildings."
					)
				)
			)
		)
