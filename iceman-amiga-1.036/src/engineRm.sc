...
(procedure (localproc_2)
	(= local2 -1)
	(if (== (= local2 (localproc_0 {Enter distance to waypoint #1:})) -1)
		(localproc_1)
		(return)
	else
	...
	(= local2 -1)
	(if (== (= local2 (localproc_0 {Enter heading to waypoint #1:})) -1)
		(localproc_1)
		(return)
	else
	...
	(= local2 -1)
	(if (== (= local2 (localproc_0 {Enter distance to waypoint #2:})) -1)
		(localproc_1)
		(return)
	else
	...
	(= local2 -1)
	(if (== (= local2 (localproc_0 {Enter heading to waypoint #2:})) -1)
		(localproc_1)
		(return)
	else
	...
	(method (handleEvent event)
		(cond
			...
			((Said '/prop>')
				(cond
					((Said 'look[<at]')
						(Print 37 40) ; "A standard propeller."
					)
					((Said 'examine,check')
						(Print 37 41) ; "It's fine."
					)
					((Said 'measure')
			...
			((or (Said 'install,(adjust<on)/washer') (Said 'washer<use'))
				(cond
					((& (subMarine roomFlags:) $0020)
						(Print 37 42) ; "You already did that."
					)
					((not (gEgo has: 10)) ; Sub: Washer | Tunisia: Flour_Canister
						(Print 37 43) ; "You don't have a washer."
					)
					((== (subMarine invStatus4:) 1)
						(Print 37 44) ; "The washer is too small."
					)
					((> (subMarine invStatus4:) 2)
						(Print 37 45) ; "The washer is too big."
					)
					((& (subMarine roomFlags:) $0040)
						(Print 37 46) ; "The nut is in the way."
					)
					(else
						(Print 37 47) ; "You slide the washer onto the shaft."
			...
			((Said 'tighten/nut')
				(cond
					...
					((and (gEgo has: 16) (== (subMarine invStatus2:) 2)) ; Open_End_Wrench
						...
						(SetScore subMarine 407 4096 3)
					)
					((and (gEgo has: 16) (!= (subMarine invStatus2:) 2)) ; Open_End_Wrench
						(Print 37 56) ; "Your wrench is the wrong size for the 1/2" nut."
					)
					(else
						(Print 37 57) ; "You've already tigthened it as much as you can by hand and you don't have a wrench."
					)
				)
			)
			((Said 'get,detach,(get<off)/nut')
				(cond
					((gEgo has: 11) ; Sub: Nut | Tunisia: Coffee_Canister
						(Print 37 58) ; "You already have one."
					)
					((!= ((gInventory at: 11) owner:) gCurRoom) ; Sub: Nut | Tunisia: Coffee_Canister
						(event claimed: 0)
					)
					((not (& (subMarine roomFlags:) $0080))
						(Print 37 59) ; "You remove the nut from the shaft."
					...
					((& (subMarine roomFlags:) $0080)
						(cond
							((not (gEgo has: 16)) ; Open_End_Wrench
								(Print 37 60) ; "It's too tight to do by hand."
							)
...
(instance drawer of Prop
				...
				(cond
					...
					((Said 'get[<!*]/*')
						(cond
							...
							((gEgo has: 15) ; Hammer
								(Print 37 64) ; "You already have the hammer."
							)
							(else
								(gEgo get: 15) ; Hammer
					...
					((Said 'adjust')
						(cond
							((not (gEgo has: 15)) ; Hammer
								(Print 37 65) ; "You don't have a hammer."
							)
							((not cel)
								(Print 37 62) ; "Open the drawer first."
							)
				...
				(cond
					...
					((Said 'look[<at]')
						(if (not cel)
							...
							(Print 37 66) ; "You see various open-end wrenches."
						)
					)
					((GoToIfSaid self event self 20 'get[<!*]/*,adjust' 37 19))
					((Said 'get[<!*]/*')
						(cond
							...
							(else
								(Print 37 70) ; "You don't pick up a wrench."
							)
						)
					)
					((Said 'adjust')
						(cond
							((not (gEgo has: 16)) ; Open_End_Wrench
								(Print 37 71) ; "You don't have a wrench."
							)
							((not cel)
								(Print 37 62) ; "Open the drawer first."
							)
	...
	(method (cue)
		...
		(self stopUpd:)
		(if cel
			(Print 37 73) ; "You open the drawer and find a small variety of hand tools."
		)
	)
)
		...
		(cond
			...
			((Said '[/ladder]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 74) ; "This is the ladder to the emergency escape hatch."
					)
					...
					((Said 'climb<down')
						(Print 37 75) ; "The ladder only goes up."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/ladder]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 76) ; "This is the ladder to the reactor control room."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/transducer]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 77) ; "The transducers are attached to the steel casing surrounding the shaft. They pick up generated sound waves which are fed into a computer. Here, the computer creates the precise opposite sound wave which eliminates the noise. This makes for much improved silent running."
					)
					((Said 'examine')
						(Print 37 78) ; "All systems and equipment are functioning properly."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/turbine]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 79) ; "The large turbine is powered by steam which is generated by the nuclear reactor. The turbine is the power source for the electric generator."
					)
					((Said 'examine')
						(Print 37 80) ; "The reactor and turbine are functioning properly."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/generator]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 81) ; "The generator supplies power to the main electrical engine which turns the drive shaft of the submarine."
					)
					((Said 'examine')
						(Print 37 82) ; "The generator is functioning properly."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/engine]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 83) ; "This powerful electric engine produces 65000 shaft horse power which can propel the USS Blackhawk in excess of 50 knots submerged."
					)
					((Said 'examine')
						(Print 37 78) ; "All systems and equipment are functioning properly."
					)
		...
		(cond
			...
			((Said '[/pump]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 84) ; "The pumps move water and hydraulic fluid through pipes to various systems."
					)
					((Said 'examine')
						(Print 37 78) ; "All systems and equipment are functioning properly."
					)
		...
		(cond
			...
			((Said '[/tank]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 85) ; "This is the compressed air tank."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/tank]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 37 86) ; "This is the fresh water storage tank."
					)
				)
			)
		)
