	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'raise,up/periscope') (Said 'periscope<use'))
				(cond
					((scopeProp2 cel:)
						(Print 25 6) ; "You can't look through two scopes at once."
					)
					((& (Submarine flags:) $0010)
						(Print 25 7) ; "The observation scope has been rendered inoperable."
					)
					((>= (Submarine _depth:) 70)
						(Print 25 8) ; "The sub is too deep."
					)
					((<= (Submarine hSpeed:) 12)
						(if cel
							(Print 25 9) ; "It's up."
					...
					((& (subMarine roomFlags:) $0200)
						(Print 25 10) ; "You were warned about the speed limit for the observation scope. Now it is inoperable."
						(Submarine flags: (| (Submarine flags:) $0010))
					)
					(else
						(Print 25 11) ; "Severe damage will occur when raising the observation scope at speeds in excess of 12 knots."
			...
			((Said 'look/periscope>')
				(if (or cel (Said 'look<through'))
					...
					(Print 25 13) ; "This is the observation scope."
				)
				(event claimed: 1)
			)
		)
			...
			((or (Said 'raise,up/periscope') (Said 'periscope<use'))
				(cond
					...
					((& (Submarine flags:) $0020)
						(Print 25 14) ; "The attack scope has been rendered inoperable."
					)
					((>= (Submarine _depth:) 70)
						(Print 25 8) ; "The sub is too deep."
					)
					...
					((& (subMarine roomFlags:) $0100)
						(Print 25 15) ; "You were warned about the speed limit for the attack scope. It is now inoperable."
						(Submarine flags: (| (Submarine flags:) $0020))
					)
					(else
						(Print 25 16) ; "Severe damage will occur when raising the attack scope at speeds in excess of 5 knots."
...
(instance rightArms of Prop
	...
	(method (addToPic)
		(leftArms setScript: 0 stopUpd: signal: (| (leftArms signal:) $0100))
		(super addToPic: &rest)
	)
)
