		...
		(cond
			...
			((Said 'look>')
				(cond
					...
					((Said '/plant')
						(Print 305 15) ; "You see undersea plant life waving in the current."
					)
					(else
						(Print 305 16) ; "You look but find nothing that will help you."
						(event claimed: 1)
					)
				)
			)
			((Said 'get')
				(Print 305 17) ; "You don't need it."
			)
			((Said 'move>')
				(if (Said '/rock')
					(Print 305 18) ; "Moving the rock would be useless."
				else
					(Print 305 19) ; "You're wasting your time trying to move it."
				)
			)
		)
	)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/bubbles]')
				(Print 305 20) ; "You see the air bubbles from your regulator."
			)
		)
	)
)
...
(instance fish1 of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/fish]')
				(Print 305 21) ; "This is a small fish."
			)
			((Said 'catch/fish')
				(Print 305 22) ; "You don't need the fish."
			)
			((Said 'kill/fish')
				(Print 305 23) ; "You have no time for idle pleasures."
			)
		)
	)
...
(instance schoolOfFish of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/fish]')
				(Print 305 24) ; "You see a school of small fish."
			)
			((Said 'catch/fish')
				(Print 305 25) ; "There are too many in this school."
			)
			((Said 'kill/fish')
				(Print 305 23) ; "You have no time for idle pleasures."
			)
...
(instance torpedoRay of Act
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'look[<at][/ray]')
				(Print 305 26) ; "You see what appears to be a ray of some kind."
			)
			((Said 'catch/ray')
...
(instance electricuteScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 951 2 0 305 27) ; "What a shocking experience!"
			)
		)
	)
)
