		...
		(switch temp2
			...
			(24
				(if (IsFlag 68)
					...
					(local1 dispose:)
					((= local1 (DText new:))
						text: (KString 8 {Bowl with Green Water}) ; StrDup
						fore: 1
						back: 0
						font: 0
						setSize:
		...
		(switch (event type:)
			...
			(evMOUSEBUTTON
				(cond
					...
					((rotRBox onMe: event)
						(rotRBox handleEvent: event)
					)
					((rotLBox onMe: event)
						(rotLBox handleEvent: event)
					)
					...
					(else
						(= temp2 (gGame setCursor: theSkullCursor 1))
						(for ((= temp1 0)) (< temp1 3) ((++ temp1))
							(for ((= temp0 0)) (< temp0 3000) ((++ temp0))
							)
						)
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3)
		(switch (event type:)
			(evMOUSEBUTTON
				...
				(while (not temp2)
					(= temp3 ((User curEvent:) new:))
					(if (& (= temp0 (temp3 type:)) $0002)
						(self setCel: 0)
						(= temp2 1)
					else
						(itemProp rotate: 3)
...
(instance rotLBoxHotSpot of HotSpot
	(properties)
...
(instance rotRBoxHotSpot of HotSpot
	(properties)
	...
	(method (doVerb &tmp temp0 temp1 temp2)
		(switch view
			(204
				...
				(local1 dispose:)
				((= local1 (DText new:))
					text: (KString 8 (temp0 name:)) ; StrDup
					fore: 1
					back: 0
					font: 0
					setSize:
