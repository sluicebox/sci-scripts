	...
	(cond
		(
			(and
				...
				(NoRoom 8 (gEgo illegalBits:))
			)
			(HighPrint 65 4) ; "Give yourself a little room, Hero."
			(return 0)
		)
		((== prType 28)
			(cond
		...
		((not (gEgo has: [thingInvs (- prType 1)]))
			(HighPrint (Format @str 65 5 [thingNames (- prType 1)])) ; "That's hard to do without %s."
			(return 0)
		)
		((== prType 1)
			(if gClumpsLeft
				...
				(HighPrint 65 6) ; "Uh oh! You don't have any dirt."
				(return 0)
			)
		)
		((== prType 2)
			(if global318
				...
				(HighPrint 65 7) ; "You don't have any sand."
				(return 0)
			)
		)
		((== prType 3)
			(if (not (gEgo has: 39)) ; BurningPowder
				(HighPrint 65 8) ; "You have no more powder."
				(return 0)
			else
