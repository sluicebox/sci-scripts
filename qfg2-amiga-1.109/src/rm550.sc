...
(instance adAvis of Talker
	...
	(method (messages)
		(super messages: &rest)
		(adAvisHead setCycle: Fwd cycleSpeed: 2)
	)

	(method (endTalk)
		(adAvisHead setCycle: 0 cel: 0)
		(super endTalk: &rest)
	)
	...
	(method (handleEvent event)
		(cond
			((Said 'ask,tell//mirror')
				(Say adAvis askAboutMirror 550 5) ; "Don't bother me with silly quest... Mirror? Do you have a mirror? USE it, you fool!"
				(gCurRoom setScript: askAboutMirror)
			)
			((Said 'ask,tell//door,star,constellation')
				(if noticedDoor
					(Say adAvis 550 6) ; "Silence, fool! I am pondering the significance of that at this very instant."
				else
					(= responseTimer 0)
					(Say adAvis 550 7) ; "What about it? The door is clearly there, emblazoned with stars. Wait, they are constellations! There is the scorpion, below is the dragon. Perhaps you are not totally worthless after all. We need but wait for the moon to shine upon the door."
					(++ noticedDoor)
					(= responseTimer 40)
				)
			)
			((Said 'ask,tell//*')
				(switch (++ annoyed)
					(1
						(Say adAvis 550 8) ; "Questions! Do you have nothing better to do than waste my time? Let me think, and I'd advise you to do the same!"
					)
					(2
						(Say adAvis 550 9) ; "Enough. You do not want to make me angry!"
					)
					(3
						(Say adAvis 550 10) ; "I grow impatient with your nonsense. Either find a way to make the moon obey you, or you will face my wrath!"
					)
					(4
						(Say adAvis 550 11) ; "You would try the patience of a dervish! Do something!"
					)
					(5
...
(instance cartoon1 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say adAvis 550 16) ; "You there, try walking through the door now!"
			...
			(3
				(Say adAvis self 550 17) ; "No, no, fool. Pass through it! Oh, get out of the way."
			)
			(4
			...
			(5
				(TurnHead 6)
				(Say adAvis self 550 19) ; "I cannot be mistaken. My translation of the prophecy must be correct. It must be you. Of all the stupid Heroes in the world, I had to be cursed with you!"
			)
			(6
			...
			(8
				(TurnHead 6)
				(Say adAvis self 550 20) ; "What is wrong? How does it go?"
			)
			(9
				(Say adAvis self 550 21 550 22 550 23 550 24) ; ""For one thousand years and a year, Iblis will be bound..." I know that it has been one thousand and one years to this very day, so that MUST be correct."
			)
			(10
				(TurnHead 4)
				(Say adAvis self 550 25) ; "When the moonlight shines between the Dragon's jaws... And is caught and held there by the Scorpion's claws."
			)
			(11
				(TurnHead 6)
				(= cycles 1)
			...
			(12
				(TurnHead 5)
				(Say adAvis self 550 26) ; "The Moon! It is not due to rise between the two constellations until tomorrow! How can I be wrong!?"
			)
			(13
				(TurnHead 4)
				(Say adAvis 550 27) ; "I WILL NOT BE DENIED!!!"
				(= cycles 1)
			)
			(14
				(adAvis setScript: castOpen self 2)
			...
			(15
				(Say adAvis self 550 28 550 29) ; "I cannot be wrong. It did not say a thousand and one years and a day. No!"
			)
			(16
				(DontTalk 0)
				(= responseTimer 25)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(Say adAvis self 550 30) ; "So be it! "By the Powers of all Darkness. By the Power of My Name Shall your soul be mirrored outward, and your body be the same!""
			)
			(1
				(TurnHead 4)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				...
				(TurnHead 6)
				(Say adAvis self 550 33) ; "At last! Now how does the rest of the prophecy go? "And at last the Hero 'He who Waits Behind' shall see, While Iblis rises by the Dark One and the light shall flee. By the name of Suleiman, so shall this be!""
			)
			(11
				(adAvis setLoop: 2 setCel: 2)
				(TurnHead 4)
				(Say adAvis self 550 34) ; "Now you know all you need to know. Go in there and bring me the statue of Iblis!"
			)
			(12
				(= tombDoorOpen 1)
				(NormalEgo)
...
(instance noNoticeDoor of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(DontTalk 1)
				(Say adAvis self 550 35) ; "Bah! What use are you? Be silent!"
			)
			(1
				(= seconds 3)
			)
			...
			(4
				(adAvis view: 495 setLoop: 3 setCycle: Fwd)
				(Say adAvis self 550 37) ; "Passing through the door... what is it about that door? Door with stars, constellations! The scorpion and the dragon are on the door! It must be when the moon shines upon the door that you may pass though it."
			)
			(5
				(Say adAvis self 550 38) ; "Let me see, where is the moon now? The moon is to the southeast. But the door is facing north! The moonlight will not pass over us enough to cast light upon the door. Why is this happening to me? After all these years of planning, how can so many things go wrong?"
			)
			(6
				(adAvis
					view: 555
					...
					cel: 1
					setCycle: 0
					setScript: turnToEgo self
				)
			)
			(7
				(Say adAvis self 550 39) ; "Well, Hero from the Northland, are you so powerful a magician that you can move the moon from its orbit and force it to shine on the door? Or are you so strong that you can bend light? What can you say or do that will let you pass the door?"
			)
			(8
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(DontTalk 1)
				(if lastChance
					(Say adAvis self 550 41) ; "Bah! You have no way of getting through that door. And if your usefulness has ended...."
				else
					(Say adAvis self 550 42) ; "What is it about you that will let you pass the door? There must be something. The prophecy cannot be wrong."
				)
			)
			(1
				(DontTalk 0)
