...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'pull,break,get,break/blade')
				(cond
					((!= gEgoState 0)
			...
			((Said 'cut/blade>')
				(cond
					...
					((or (not (Said '//ginsu')) (not (gEgo has: 2))) ; Ginsu_Knife
	...
	(method (changeState newState &tmp theLine)
		...
		(switch (= state newState)
			...
			(18
				...
				(Print 230 51 #at -1 10 #title {the Doorman says} #mode 1 #icon BJicon)
				(= seconds 3)
			)
			(19
...
(instance CreditsScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(aCredit3 setCycle: End)
				(= cycles 20)
			)
			(6
				(aCredit1 setCycle: Beg)
				(aCredit3 setCycle: Beg self)
			)
			(7
				(aCredit3 dispose:)
