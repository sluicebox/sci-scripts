...
(instance putOnSpacesuit of Script
	...
	(method (doit)
		(super doit:)
		(if (and (== (gEgo loop:) 1) (== (gEgo cel:) 1) local2)
			(= local2 0)
			(spaceSuit _approachVerbs: 26505 dispose:)
		)
		(if (and (== (gEgo loop:) 2) (== (gEgo cel:) 3) local3)
			(= local3 0)
				...
				(= local0
					(cond
						(
							(and
								(gEgo cycler:)
								((gEgo cycler:) respondsTo: #vWalking)
							)
							((gEgo cycler:) vWalking:)
						)
						(local2 0)
						(else 1)
					)
				...
				(= local1
					(cond
						(
							(and
								(gEgo cycler:)
								((gEgo cycler:) respondsTo: #vStopped)
							)
							((gEgo cycler:) vStopped:)
						)
						(local2 60)
						(else 61)
					)
