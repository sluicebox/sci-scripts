...
(instance radarPost of Feature
	...
	(method (doVerb theVerb)
		...
		(switch theVerb
			(3 ; Do
				(Print 702 8) ; "You'd better not risk a shocking experience."
			)
			(4 ; Inventory
				(Print 702 9) ; "You'd better not. It might be damaged by the force field generator."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance robot of Actor
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(5 ; Talk
				(Print 702 10) ; "The droid emits a cheerful chirp, but has nothing further to say right now. Fortunately, you bought it as a pilot, not a conversationalist."
			)
			(3 ; Do
				(Print 702 11) ; "Although small, the NAV-201 is surprisingly heavy. You don't think you'll be able to lift it."
			)
			(4 ; Inventory
				(if (OneOf invItem 4 15) ; Knife, Widget
					(Print 702 12) ; "You wouldn't want to damage your droid. You wasted - I mean spent - much hardly-earned cash on it."
				else
					(super doVerb: theVerb invItem)
				)
			)
			(12 ; Smell
				(Print 702 13) ; "There's nothing like the smell of a new used droid."
			)
			(11 ; Taste
				(Print 702 14) ; "Like many things in your experience, it looks better than it tastes."
			)
			(else
				(super doVerb: theVerb invItem)
			)
...
(instance zapEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (>= global162 4)
					(EgoDead 29 2 0 702 15) ; "Yikes! It looks like you hit that force field one too many times. Not only did it burn every follicle of hair from your sleek frame, but your aorta, if you could see it, now resembles the end of a red celery stalk."
				else
					(gEgo
						loop: (+ (gEgo loop:) 2)
						cel: 0
			...
			(4
				(Print 702 16) ; "Ouch! That hurt!"
				(Print 702 17) ; "You are standing next to the protective barrier encircling Ulence Flats. Due to the fact that you are land-based, you are not able to pass."
				(SetFlag 38)
				(HandsOn)
				(self dispose:)
			)
