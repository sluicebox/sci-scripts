...
(procedure (EgoDead param1 param2 param3 &tmp temp0 temp1 temp2 [temp3 300])
	...
	(if argc
		...
		(= temp1 0)
		(= temp2 0)
		(Format @temp3 0 29) ; "Thank you for playing Space Quest I. Too bad you've failed miserably and doomed all of your people to a horrible death at the hands of the Sariens. You quickly glance about the room to see if anyone saw your silly mistake. Better luck next time."
	)
...
(procedure (proc0_12 param1 &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8)
	(if (== argc 1)
		(Display 0 30 dsRESTOREPIXELS [param1 0])
		(if (not (HaveMouse))
...
(procedure (localproc_0 param1 param2 param3 &tmp [temp0 30] [temp30 30] temp60 [temp61 100] [temp161 19] [temp180 30] [temp210 20])
	...
	(Format @temp61 0 31 gCurRoomNum @temp180 ((gTheIconBar curIcon:) helpStr:)) ; "New response: rm%03d %s  %s"
	(if (== param1 4)
...
(instance sq1 of Game
	...
	(method (init &tmp temp0)
		...
		(ScriptID 982)
		RandCycle
		(LoadMany rsCURSOR 6 997 999 19 30 20 7 31)
		(super init: &rest)
		(StrCpy @gSysLogPath {})
		...
		(invWin
			...
			rgtBordColor: (proc0_17 global155 global156 global155)
			botBordColor: (proc0_17 global155 global156 global155)
			insideColor: (proc0_17 global155 global156 global129)
			topBordColor2: (proc0_17 global129 global129 global155)
			lftBordColor2: (proc0_17 global129 global129 global155)
			botBordColor2: (proc0_17 global158 global130 global130)
			rgtBordColor2: (proc0_17 global159 global129 global130)
		)
		((= gTheIconBar IconBar)
			...
			eachElementDo:
				#lowlightColor
				(proc0_17 global158 global155 global156)
			curIcon: icon0
			useIconItem: icon4
			helpIconItem: icon9
			disable:
		...
		(Inv
			...
			eachElementDo:
				#highlightColor
				(proc0_17 global129 global129 global156)
			eachElementDo:
				#lowlightColor
				(proc0_17 global155 global156 global129)
			eachElementDo: #init
			window: invWin
...
(instance invHand of IconI
	...
	(method (init)
		(self
			highlightColor: global129
			lowlightColor: (proc0_17 global158 global155 global156)
		)
		(super init:)
	)
)
		...
		(switch theVerb
			(4 ; Inventory
				(if (== (Inv at: invItem) self)
					(Print 0 19) ; "What did you expect them to do, reproduce?"
				else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(cond
					((gCurRoom script:)
						(Print 0 20) ; "You're far too busy for that now."
					)
					((IsFlag 48)
						(ClearFlag 48)
						(invSound number: 902 loop: 1 flags: 1 play:)
						(Print 0 21) ; "You switch the gadget off to conserve its energy."
					)
					(else
						(SetFlag 48)
						(invSound number: 902 loop: 1 flags: 1 play:)
						(Print 0 22) ; "You switch the gadget on."
					)
				)
			)
			(else
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(3 ; Do
				(cond
					((gCurRoom script:)
						(Print 0 23) ; "You're far to busy for that now."
					)
					((== cel 1)
						(Print 0 24) ; "You scrape that sticky goo from the bottom of the rock."
						(ClearFlag 70)
						(gEgo get: 16) ; Plant
						(self cel: 0)
					...
					(else
						(Print 0 25) ; "There is nothing you can do to the rock."
					)
				)
			)
			(else
		...
		(switch theVerb
			(3 ; Do
				(cond
					...
					((IsFlag 61)
						(Print 0 26) ; "You've already opened the survival kit and removed its contents."
					)
					(else
						(SetFlag 61)
						(Print 0 27) ; "Upon opening the survival kit you discover a Xenon army knife and a canister of dehydrated water."
...
(instance buckazoid of RInvItem
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Printf 0 28 gBuckazoidCount (if (== gBuckazoidCount 1) {.} else {s.})) ; "You have %d Buckazoid%s"
			)
			(4 ; Inventory
				(= view 560)
				(super doVerb: theVerb invItem)
