...
(instance Room84 of Rm
	...
	(method (init)
		...
		(if (== ((gInventory at: 33) owner:) 84) ; Magic_Hen
			...
			((= local2 (Sound new:)) number: 79 loop: 10000 play:)
		)
		(if (or (== gPrevRoomNum 87) (== gPrevRoomNum 0))
			(gEgo posn: 157 163 view: 4 xStep: 4 yStep: 1 init:)
			(if global175
				(= global175 0)
			)
		)
						...
						(cond
							...
							((Said '/chicken')
								(if (== ((gInventory at: 33) owner:) 84) ; Magic_Hen
									...
									(Print 84 14 #at -1 20) ; "There's no hen here."
								)
							)
						)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(gEgo view: 40 cel: 0 setMotion: 0 setCycle: End self)
			)
			(1
