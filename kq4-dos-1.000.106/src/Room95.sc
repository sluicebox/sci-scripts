...
(instance Room95 of Rm
	...
	(method (init)
		...
		(if (not gNight)
			...
			(local6 setScript: gullActions)
		)
		(if (== global116 1)
			(= local4 (Act new:))
			(= local5 (Sound new:))
			(local5 number: 14 play:)
			(local4
				view: 233
				posn: 177 146
				setPri: 13
...
(instance manActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(local4 view: 234 loop: 0 cel: 0 setCycle: End self)
			)
			(2
...
(instance gullActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(local6 view: 327 cel: 0 loop: 0 setCycle: End self)
			)
			(2
				(local6 loop: 1 cel: 0 setCycle: Fwd)
				(local6 setMotion: MoveTo -10 25 self)
			)
			(3
				(local6 dispose:)
			)
...
(instance fishing of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(Print 95 40 #at -1 25) ; "Eagerly, from the end of the pier, you fling the baited line of the fishing pole into the ocean."
				(gEgo
					view: 13
					loop: 2
					cel: 3
			...
			(13
				(if (< (Random 1 100) 50)
					(gEgo loop: 1)
					(Print 95 41 #at -1 25) ; "Almost immediately, you feel a sharp tug on the line! Something pulls and fights your line as you slowly reel it in. There it is! You have caught yourself...a fine fat fish!"
					(self changeState: 14)
			...
			(14
				(gEgo view: 14 loop: 1 cel: 0 cycleSpeed: 1 setCycle: End)
				(= seconds 4)
			)
			(15
				(= global182 1)
			...
			(21
				(Print 95 42 #at -1 25) ; "Eagerly, from the end of the pier, you fling the line of the fishing pole into the ocean."
				(gEgo
					view: 13
					loop: 1
					cel: 3
			...
			(23
				(if (and (< (Random 1 100) 50) (== local13 0))
					(gEgo loop: 1)
					(self changeState: 24)
				else
					(Print 95 43 #at -1 25) ; "But wait! You haven't baited your hook! Feeling rather sheepish, you reel in your empty fishing line."
					(self changeState: 30)
				)
			)
			(24
				(gEgo view: 14 loop: 0 cel: 0 cycleSpeed: 1 setCycle: End)
				(= seconds 4)
			)
			(25
				(Print 95 44 #at -1 25) ; "Almost immediately, you feel a sharp tug on the line! Something pulls and fights your line as you slowly reel it in. There it is! You have caught yourself...an old waterlogged boot. But, since you have no need of a boot, you toss it back into the water."
				(= local13 1)
				(gEgo
					view: 2
					setLoop: -1
			...
			(30
				(gEgo view: 13 cel: 0 loop: 1 cycleSpeed: 2 setCycle: End)
				(= seconds 3)
			)
			(31
				(gEgo
			...
			(50
				(gEgo view: 13 cel: 0 loop: 2 cycleSpeed: 2 setCycle: End)
				((ScriptID 0 4) setReal: self 4) ; timer1
			)
			(51
				(Print 95 45 #at -1 25) ; "The fish don't seem to be biting."
				(gEgo
					view: 2
					loop: 1
					cycleSpeed: 0
