		...
		(switch (= state newState)
			(0
				...
				(gEgo
					...
					view: 17
					setCycle: Fwd
					setMotion: MoveTo (gEgo x:) local6 self
				)
			)
			(1
			...
			(3
				(gEgo setCycle: Walk view: 2 illegalBits: -32768)
				(HandsOn)
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(HandsOff)
				(= local5 (Sound new:))
				(local5 number: 29 loop: 1 play: stopMusic)
				(gEgo
					view: 80
					setLoop: 4
					ignoreHorizon:
