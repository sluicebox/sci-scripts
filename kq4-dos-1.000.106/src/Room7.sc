...
(instance door of Prop
	...
	(method (cue)
		(self stopUpd:)
		(HandsOn)
		(gCurRoom newRoom: 42) ; FishermanShack
	)
)
...
(instance Room7 of Rm
	...
	(method (init)
		...
		(gEgo xStep: 2 yStep: 1 init: viewer: water)
		(gEgo y: (& (gEgo y:) $fffe))
		(fisherCage init:)
	)
...
(instance waveActions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(for ((= local12 0)) (< local12 (waves size:)) ((++ local12))
					((View new:)
						view: ((waves at: local12) view:)
						loop: ((waves at: local12) loop:)
						cel: 0
						setPri: 0
						ignoreActors:
						x: ((waves at: local12) x:)
						y: ((waves at: local12) y:)
						init:
						addToPic:
						yourself:
					)
			...
			(1
				((waves at: local12) cel: 0 show: setCycle: End self)
			)
			(2
				((waves at: local12) hide:)
				(if (< local12 (- (waves size:) 1))
					(++ local12)
				else
					(= local12 0)
				)
