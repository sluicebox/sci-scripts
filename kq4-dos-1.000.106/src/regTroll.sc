...
(instance regTroll of Rgn
	...
	(method (newRoom newRoomNumber)
		...
		(cls)
		(cond
			((or (== newRoomNumber 77) (== newRoomNumber 70)) ; Room_70
			...
			(1
				(if
					(and
						(not gTrollChasing)
						(or
							(== gCurRoomNum 71) ; TrollEnter
							(== gCurRoomNum 72) ; TrollCave72
							(== gCurRoomNum 74) ; TrollCave74
							(== gCurRoomNum 75) ; TrollCave75
						)
					)
					...
					(trollCaveMusic keep: 0 dispose:)
					(trollMusic dispose: loop: 1 play:)
				else
					(if (or (!= gTrollChasing 1) (== gCurRoomNum 73)) ; TrollCave73
						(return)
					)
					((= local0 (Act new:))
						view: 190
