...
(instance Room31 of Rm
		...
		(switch gPrevRoomNum
			(43
				(if (== (gEgo view:) 312)
					...
					(User canControl: 0)
					(= global205 1)
					(gEgo posn: 5 100 setMotion: MoveTo 400 114)
				else
					(= local2 100)
					(= local1 100)
				)
			...
			(44 ; Room_44
				...
				(= local1 100)
				(= local4 (View new:))
				(local4 view: 526 loop: 0 cel: 0 posn: 134 55 init:)
				(local0 hide:)
				(if gNight
					(gCurRoom drawPic: 131)
				else
				...
				(gEgo setScript: Whale_Actions)
				(Whale_Actions changeState: 10)
			)
			(1
				(= local2 4)
				(= local1 0)
	...
	(method (doit)
		...
		(if (and (not gNight) (== (gCurRoom script:) 0) (== (Random 1 100) 9))
			(gCurRoom setScript: _3_Fish_Actions)
		)
		(if
			(and
				(== (gCurRoom script:) 0)
			...
			(gCurRoom setScript: _1_Fish_Actions)
		)
		(if (gEgo edgeHit:)
			(++ local3)
			(= global136 local1)
			...
			(if (!= (gEgo view:) 312)
				...
				(if
					...
					(gEgo setScript: Ego_drowns)
					(Ego_drowns changeState: 1)
					1
				)
			)
			(cond
			...
			(cond
				...
				((== local1 0)
					(switch local2
						...
						(2
							(gCurRoom newRoom: 13) ; R13
						)
						(1
							(gCurRoom newRoom: 19)
						)
						(0
							(gCurRoom newRoom: 25) ; Room_25
						)
					)
				)
				(
				...
				((== (gEgo script:) Shark_Actions)
					(if (gCast contains: local9)
						(cond
							...
							((< (gEgo y:) (+ horizon 10))
								(local9 posn: (gEgo x:) (- horizon 10))
							)
							((> (gEgo x:) 300)
								(local9 posn: 339 (gEgo y:))
							)
							((> (gEgo y:) 174)
								(local9 posn: (gEgo x:) 204)
							)
						)
					)
				)
				(
					(and
						(or
							...
							(>= (Random 1 100) 75)
							(and (!= gNight 0) (<= (Random 100) 50))
						)
						(!= (gEgo view:) 312)
						(!= (gEgo script:) Shark_Actions)
						(!= (gEgo script:) Whale_Actions)
					)
					(gEgo setScript: Shark_Actions)
				)
			)
			(if (== gNewRoomNum 31)
...
(instance _3_Fish_Actions of Script
	(properties
		name {3 Fish Actions}
	)
...
(instance _1_Fish_Actions of Script
	(properties
		name {1 Fish Actions}
	)
...
(instance Whale_Actions of Script
	(properties
		name {Whale Actions}
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				((ScriptID 0 6) setReal: self (Random 2 10)) ; timer3
			)
			(1
				(= local8 (Prop new:))
				(= local14 (Sound new:))
				(local14 number: 36 play:)
				(local8
					view: 315
					cel: 0
					loop: 0
...
(instance Shark_Actions of Script
	(properties
		name {Shark Actions}
	)
