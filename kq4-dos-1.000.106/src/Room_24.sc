...
(instance Room_24 of Rm
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(east
				(gEgo view: 8 loop: 1 posn: 207 160)
			)
			(0
				(gEgo x: 200 y: 188)
			)
			...
			(cond
				...
				((Said 'wear,place/crown')
					(if (gEgo has: 10) ; Small_Crown
						(if (== (gEgo view:) 2)
							(gEgo setScript: Ego_As_Frog)
							(Ego_As_Frog changeState: 1)
							(if (== global200 0)
								(gGame changeScore: 5)
								(= global200 1)
							)
...
(instance Ego_As_Frog of Script
	(properties
		name {Ego As Frog}
	)
...
(instance Ego_Views_Frog of Script
	(properties
		name {Ego Views Frog}
	)
...
(instance Ego_swept_back of Script
	(properties
		name {Ego swept back}
	)
