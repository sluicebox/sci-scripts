...
(instance Room_44 of Rm
	...
	(method (init)
		...
		(Load rsVIEW 73)
		(Load rsSOUND 51)
		(gEgo view: 8 posn: 39 161 viewer: In_Whale_Viewer init:)
		(= local0 (Prop new:))
		(local0 view: 0 posn: 161 76 init: hide:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				...
				(gEgo illegalBits: 0 cel: 0 setCycle: End self)
			)
			(11
				(self changeState: 1)
			)
...
(instance In_Whale_Viewer of Script
	(properties
		name {In Whale Viewer}
	)
	...
	(method (doit)
		...
		(if (== (gEgo script:) 0)
			(cond
				...
				((and (!= (gEgo view:) 2) (& (gEgo onControl: 1) $0010))
					(self changeState: 1)
				)
			)
		)
