...
(instance Room_37 of Rm
	...
	(method (init)
		...
		(= local2 (Sound new:))
		(local2 number: 33 priority: -1 play:)
	)
			...
			(cond
				...
				((Said 'open/door')
					(if (gEgo inRect: 140 93 180 103)
						...
						(Print 37 17) ; "You're not close enough."
					)
				)
				((or (Said 'kiss[/!*]') (Said 'kiss/fairies'))
					(Print 37 18) ; "You can't get close enough."
				)
				((or (Said 'talk/fairies') (Said 'talk[/!*]'))
					(Print 37 19) ; "You speak to the little fairies, but they do not respond. Perhaps, they cannot talk."
				)
				((Said 'bang')
					(if (gEgo inRect: 141 94 175 103)
						(Print 37 20) ; "The tiny fairies watch you closely."
					else
						(Print 37 17) ; "You're not close enough."
					)
				)
