...
(procedure (GetInput str maxLen prompt &tmp [temp0 2] temp2 temp3)
	(= temp3 (Sound pause: 1))
	(= temp2 (Print (if (>= argc 3) prompt else {}) #edit str maxLen &rest))
	(Sound pause: temp3)
	(return temp2)
)
...
(procedure (Print args &tmp theDialog textI iconI editI ret i atX atY fixWidth keepIt default curPort [buttons 6] buttonWide buttonsUsed butAtX [buffer 1000])
	...
	(= [buttons 0]
		(= [buttons 1]
			(= [buttons 2] (= [buttons 3] (= [buttons 4] (= [buttons 5] 0))))
		)
	)
	...
	(theDialog name: {PrintD} add: textI)
	(for ((= i i)) (< i argc) ((++ i))
		(switch [args i]
			(60
				(++ i)
...
(procedure (Printf &tmp [str 500]) ; UNUSED
	(Format @str &rest)
	(Print @str)
)
		...
		(if
			(and
				...
				(or
					...
					(and (== temp1 evKEYBOARD) (== (event message:) key))
...
(class DSelector of DItem
	...
	(method (at what &tmp temp0 temp1)
		(= temp0 (+ (= temp0 text) (* x what)))
	)
