...
(instance Room68 of Rm
	...
	(method (init)
		...
		((View new:) view: 536 loop: 7 cel: 0 posn: 77 110 setPri: 8 addToPic:)
		(if gNight
			((Prop new:)
				view: 536
				loop: 4
		...
		(switch gPrevRoomNum
			(67
				(gEgo posn: 56 166 loop: 0 init: baseSetter: (ScriptID 0 1)) ; smallBase
			)
			(64
				(gEgo posn: 262 168 loop: 1 init: baseSetter: (ScriptID 0 1)) ; smallBase
			)
			(62 ; HauntedMasterBedroom
				(gEgo posn: 97 80 loop: 2 init: baseSetter: 0)
			)
			(60
				(gEgo posn: 198 82 loop: 2 init: baseSetter: 0)
			)
			(17
				(gEgo posn: 153 183 loop: 3 init: baseSetter: (ScriptID 0 1)) ; smallBase
			)
		)
		((= local1 (Act new:))
			illegalBits: 0
