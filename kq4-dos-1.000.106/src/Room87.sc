...
(instance Room87 of Rm
	...
	(method (init)
		...
		(super init:)
		(Notify 604 0)
		((= local8 (Sound new:)) number: 41 loop: 0 init:)
		((= local6 (Prop new:)) view: 608 posn: 111 118 init: hide:)
		((= local3 (Prop new:))
			posn: 24 65
			view: 633
			cel: 2
		...
		(if (or (== gPrevRoomNum 88) (== gPrevRoomNum 0))
			(gEgo posn: 261 134 view: 4 xStep: 4 yStep: 1 init:)
			(local6 cel: 0 init: stopUpd:)
		)
		(if (== gPrevRoomNum 86)
			(gEgo posn: 25 142 view: 4 xStep: 4 yStep: 1 init:)
			(local6 cel: 0 init: stopUpd:)
		)
		(if (== gPrevRoomNum 84)
			(gEgo posn: 131 124 view: 4 xStep: 4 yStep: 1 init:)
			(local6 cel: 0 init: stopUpd:)
		)
	)
			...
			((== (event type:) evSAID)
				(cond
					...
					((Said 'look>')
						(cond
							((Said '/goon')
								(Print 87 1 #at -1 20) ; "Lolotte's goons are not appealing at all! Besides having an unnatural pallor to their skin, they are creepy little creatures who fly upon wings that resemble those of bats."
							)
							((Said '/door')
					...
					((Said 'get/candelabra,candle')
						(Print 87 7 #at -1 20) ; "It's not for you."
					)
					((Said 'bang/door')
						(if (< (gEgo distanceTo: local6) 15)
							(Print 87 8 #at -1 20) ; "There's nobody there..."
						else
					...
					((Said 'open/door')
						(cond
							((!= (local6 cel:) 0)
								(Print 87 9 #at -1 20) ; "It is already open."
							)
							((> (gEgo distanceTo: local6) 15)
								(Print 800 1) ; "You're not close enough."
							)
							(else
					...
					((Said 'close/door')
						(Print 87 10) ; "It already is."
					)
					((Said 'unlatch/door')
						(if (< (gEgo distanceTo: local6) 15)
							(Print 87 11) ; "Why don't you just open it?"
						else
							(Print 800 1) ; "You're not close enough."
						)
					)
					((Said '/goon,man>')
						(cond
							((Said 'talk')
								(Print 87 12 #at -1 20) ; "Lolotte's henchmen must not speak, as you only hear them utter guttural sounds."
							)
							((Said 'get,capture')
								(Print 87 13 #at -1 20) ; "Why would you want one?!"
							)
							((Said 'kiss')
								(Print 87 14 #at -1 20) ; "That's an AWFUL thought!"
							)
							((Said 'give')
								(Print 87 15 #at -1 20) ; "The henchmen are not interested in your offering."
							)
						)
					)
				)
