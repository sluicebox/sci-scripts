...
(instance Room54 of Rm
	...
	(method (init)
		...
		(super init:)
		((View new:) view: 533 loop: 1 cel: 0 posn: 295 107 setPri: 9 addToPic:)
		((= local12 (Act new:))
			view: 290
			loop: 3
			illegalBits: 0
		...
		(= gIndoors 1)
		(NormalEgo)
		(gEgo view: 4 observeControl: 4 setStep: 4 1)
		(cond
			((== gPrevRoomNum 53)
				(if (== gDwarfEscortOut 1)
					(local12 posn: 253 87 setMotion: Follow gEgo show:)
					(gEgo posn: 205 105 loop: 1 init:)
					(bounceBitchScript changeState: 6)
				else
					(local12 posn: 147 1195 show:)
					(gEgo posn: 222 88 init:)
					(if global115
						(Load rsVIEW 61)
						(Load rsVIEW 62)
			...
			((== gPrevRoomNum 654)
				(gEgo posn: 270 136 loop: 2 init:)
			)
			(else
				(local12 posn: 147 1195 show:)
				(gEgo posn: 147 158 loop: 3 init:)
				(= global115 0)
			)
		)
		(if (not global177)
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(HandsOff)
				(= local25 1)
				(gEgo setMotion: MoveTo 232 88 self)
			)
			(2
				(gEgo setMotion: MoveTo 243 90 self)
			)
