...
(instance Room93 of Rm
	...
	(method (init)
		...
		(if (or (== gPrevRoomNum 92) (== gPrevRoomNum 0))
			...
			(if global175
				(if (> (Random 0 100) 90)
					(Print 93 0 #at -1 20 #font gSmallFont) ; "I don't know how you did it, but you managed to escape Lolotte's henchman. You might not be so lucky next time. Watch Out!!"
					(= global175 0)
				else
					((= local3 (Act new:))
						view: 141
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(10
				...
				((= local2 (Sound new:)) number: 51 loop: 1 play:)
			)
			(11
				(gEgo view: 41 cel: 0 setPri: -1 setCycle: End self)
			)
			(12
				(gEgo
					view: 4
...
(instance henchChase93 of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				((= local1 (Sound new:)) number: 41 loop: 1 play:)
				(local3 setMotion: Chase gEgo 15 self)
			)
			(1
				(User canControl: 0 canInput: 0)
				(gEgo moveSpeed: 0 setMotion: 0)
				(local1 number: 42 loop: 1 play:)
				(= seconds 4)
			)
			(2
				(= global205 1)
