...
(instance Room81 of Rm
	...
	(method (init)
		...
		((View new:) view: 633 loop: 2 cel: 0 posn: 122 71 setPri: 3 addToPic:)
		((View new:) view: 633 loop: 2 cel: 0 posn: 196 72 setPri: 3 addToPic:)
		(if gNight
			((= local5 (Prop new:))
				view: 646
				loop: 1
		...
		(= local7 0)
		(= local8 0)
		((= local11 (Sound new:)) number: 43 loop: 0 init:)
		((= local3 (Prop new:))
			view: 604
			posn: 83 154
			cel: 0
	...
	(method (doit)
		...
		(if
			(and
				(< (gEgo distanceTo: local3) 25)
				((gInventory at: 34) ownedBy: 81) ; Rose
			)
			(Print 81 0 #font gSmallFont #icon 409 0 0 #at -1 25) ; "What's this?! Why, a beautiful red rose has been pushed under your door! You pick it up and carry it with you."
			(= global182 1)
			(gEgo get: 34) ; Rose
		)
	)
			...
			((== (event type:) evSAID)
				(cond
					((not (& (gEgo onControl: 0) $0800))
						(cond
							...
							(
								...
								(Print 81 14) ; "You don't have time for that!"
							)
							(
								(or
									(Said 'open/chest')
							...
							((Said 'open/window')
								(Print 81 15) ; "The windows do not open."
							)
							((Said 'break/window')
								(Print 81 16) ; "That would not accomplish anything."
							)
							((Said '[use]/key<skeleton')
								(cond
									...
									((not local8)
										(Print 81 17) ; "The door is unlocked."
									)
									(else
										(Print 81 18) ; "The skeleton key does not fit the lock."
									)
								)
							)
							((Said 'unlatch/door')
								(cond
									...
									((gEgo has: 32) ; Gold_Key
										(= local8 0)
										(Print 81 20) ; "Using the little gold key you quickly unlock the door."
										(gGame changeScore: 2)
									)
									(else
										(Print 81 21) ; "How can you do that without a key?"
									)
								)
							)
							((or (Said '[use]/key<gold') (Said 'gold'))
								(cond
									...
									((not local8)
										(Print 81 22) ; "The door is already unlocked."
									)
							...
							((Said 'open/door')
								(cond
									...
									(else
										(Print 81 25) ; "It is already is open."
									)
								)
							)
							((Said 'close/door')
							...
							((Said 'get,untie,detach/key')
								(if
									...
									(Print 81 27) ; "You remove the little gold key from the red rose."
									(= global182 1)
					...
					((or (Said 'look[<around][/!*]') (Said 'look/room'))
						(Print 81 28) ; "You see Edgar's bedroom door, and a winding stairway leading down the tower."
					)
					((Said 'look/stair')
						(Print 81 29) ; "The stone stairway leads down the tower."
					)
					((Said 'look/door')
						(Print 81 12) ; "It's just a plain wooden door."
					)
					((Said 'look/wall')
						(Print 81 30) ; "There is nothing of importance on the walls."
					)
					((Said 'close/door')
						(Print 81 26) ; "You don't need to."
					)
					((Said 'open/door')
						(cond
							...
							(else
								(Print 81 31) ; "It already is open."
							)
						)
					)
					((Said 'unlatch/*')
						(Print 81 32) ; "It already is unlocked."
					)
				)
			)
		)
...
(instance lockUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 81 33 #at -1 40 #time 5) ; "You think to yourself, NOW WHAT!!"
			...
			(7
				(= local0 (Print 81 34 #at -1 35 #dispose)) ; "You hear something at the door!"
				(local4 cycleSpeed: 7 setLoop: 0 setCycle: CT 12 1)
				((ScriptID 0 4) setReal: self 3) ; timer1
			)
			(8
				(local4 cel: 12 setCycle: End self)
			)
			(9
				(cls)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(gEgo loop: 0)
				(if gNight
					(local5 cycleSpeed: 7 cel: 0 setCycle: End self)
					(local6 cycleSpeed: 7 cel: 0 setCycle: End)
				)
			)
			(7
				(Print 81 35 #at -1 20 #width 260 #time 10) ; "The henchmen have taken your rose and you no longer have the gold key! It looks like you're destined to be...Mrs. Edgar!"
