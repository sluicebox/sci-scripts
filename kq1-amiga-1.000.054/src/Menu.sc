	...
	(method (handleEvent event &tmp temp0 temp1 temp2 [temp3 300])
		(switch (super handleEvent: event (User blocks:))
			(257
				...
				(Print
					(Format @temp3 997 3) ; "King's Quest I SCI Amiga Conversion by: STEVEN COALLIER, RANDY MACNEILL and JOHN RETTIG."
					#mode
					1
					#width
					180
			...
			(258
				(Print 997 4 #font gSmallFont) ; "DURING THE GAME: ESC opens and closes the menus,  which show additional shortcuts. IN DIALOG WINDOWS: Your current choice is outlined. Tab and Shift-Tab move between  choices. ESC always cancels. IN TYPING WINDOWS: Arrows, Home, and End move the  cursor. Ctrl-C clears the line."
			)
			(513
				(gGame save:)
			)
			...
			(769
				(cond
					(global130
						(Print 997 7) ; "You are unable to duck here."
					)
					((GetMenu 769 113)
						((gCurRoom script:) cue:)
					)
					...
					((IsFlag 1)
						(Print 997 8) ; "You have no reason to duck - nobody can see you while you're invisible."
					)
					(else
			...
			(770
				(cond
					...
					(global130
						(Print 997 9) ; "You can't jump here."
					)
					((gCurRoom script:)
						(PrintTooBusy)
					)
			...
			(773
				(if (u< (MemoryInfo 0) 1550) ; LargestPtr
					(Print 997 11) ; "This is not a good time to rummage through your belongings. Try again a little later."
				else
		...
		(cond
			...
			((== (event type:) evSAID)
				(cond
					...
					(else
						(Print 997 13) ; "You'll have to try that when you're on your feet."
						(event claimed: 1)
					)
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if (IsFlag 17)
					(if (IsFlag 1)
						(Print 997 14) ; "As you jump, the ring slips off your finger, and you are visible again."
					else
						(Print 997 15) ; "As you jump, the ring slips off your finger."
					)
					(ClearFlag 1)
					(ClearFlag 17)
					(proc0_8 5)
