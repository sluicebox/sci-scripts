...
(procedure (LogText param1 param2 &tmp [temp0 80]) ; UNUSED
	(= temp0 0)
	(logFile
		name: @global63
		writeString: (Format @temp0 0 129 gCurRoomNum param2 param1) ; "rm%03d: %s: %s"
		close:
	)
)
...
(procedure (proc0_4)
	(Print 0 130) ; "It's not quite within reach, Sir Graham."
)

(procedure (proc0_6)
	(Print 0 131) ; "You don't have anything that fits that description in your inventory."
)
...
(procedure (PrintTooBusy)
	(if (Random 0 1)
		(Print 0 132) ; "You're too busy."
	else
		(Print 0 133) ; "You're too busy right now."
	)
)
...
(procedure (PlayBackSound param1)
	(if (!= param1 (backSound number:))
		...
		(backSound number: param1 vol: 127 changeState: playBed:)
	)
)
...
(procedure (AmigaPauseBackSound param1)
	(backSound pause: param1)
)
...
(instance kq1 of Game
	...
	(method (syntaxFail)
		(Print 0 116) ; "Please try to rephrase that thought, Sir Graham."
	)

	(method (pragmaFail)
		(Print 0 117) ; "Please try something else, Sir Graham."
	)
)
...
(instance statusCode of Code
	...
	(method (doit param1)
		(Format param1 0 118 gScore gPossibleScore 0 119 {King's Quest I} 0 119) ; "Score: %d of %d%13s%s%1s"
	)
)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1
				(cond
					(global119
						(Print 0 120) ; "You see a useless dead goat lying lifeless on the ground."
					)
					((== view 166)
						(Print 0 121) ; "The goat is grazing nearby."
					)
					((IsFlag 2)
						(Print 0 122) ; "The old goat, trotting briskly, seems happy to be with you."
					)
					((not (IsFlag 36))
						(if (& (gGoat onControl:) $3000)
							(Print 0 123) ; "The goat is wandering this way and that, exploring his little pen."
						else
							(Print 0 124) ; "The goat has wandered off to the other side of his pen."
						)
					)
					(else
						(Print 0 9) ; "The goat doesn't seem to be interested in you."
	...
	(method (doVerb theVerb)
		(cond
			...
			((OneOf view 120 121 123 124 126)
				(= description {troll})
				(if (IsFlag 13)
					(Print 0 125) ; "This is the ugliest, meanest, nastiest troll you have ever seen. And he is not about to let anybody cross his bridge."
				else
					(Print 0 126) ; "There is no troll here."
				)
			)
...
(instance _Pebbles_ of InvI
	...
	(method (showSelf &tmp [temp0 80])
		(if (== global140 1)
			(Print (Format @temp0 0 127 global140) #icon 510 0 0) ; "You have %d smooth, rounded pebble."
		else
			(Print (Format @temp0 0 128 global140) #icon 510 (- global140 1) 0) ; "You have %d smooth, rounded pebbles."
		)
	)
)
