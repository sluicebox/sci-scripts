	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Print 30 10) ; "John boy" he continues, "Let's shove off and get back to work!"
		...
		(cond
			...
			((Said 'open/envelope[<manila]')
				(switch local7
					...
					(1
						(Print 30 27) ; "Opening the envelope, you look in and see that the contents are a chart and set of orders."
						(= local7 2)
					)
					(2
			...
			((Said 'look,read[<at]/chart,map')
				(cond
					...
					(local22
						(Print 30 30 #icon 430 5 1) ; "Again you look the chart over and see that your destination is the Mediterranean."
					)
					(else
						(Print 30 31 #icon 430 5 1) ; "You carefully look the chart over and see that your destination is the Mediterranean."
						(= local22 1)
					)
				)
				(if local20
			...
			((Said 'get/order')
				(if (== local7 2)
					(Print 30 32) ; "You already have them. Just read them."
				else
					(proc0_38) ; "You don't have that."
				)
			)
			((Said 'look,read[<at]/order')
				(cond
					(local20
						(Print 30 33) ; "You already read them."
					)
					((== local7 2)
						(Print 30 34) ; "You read over the orders which contain the following:"
						(Print 30 35) ; "Commit the following orders to memory before destroying them:"
						(Print 30 36) ; "The expediency of this mission is of utmost importance to the survival of the Ambassador."
						(Print 30 37) ; "Use extreme caution navigating the Bering Sea. The USSR is threatening blockade of the Straits due to claimed violations of United States military operating inside her territorial waters."
						(Print 30 38) ; "Washington has denied these accusations and has issued a counter communique warning the Soviets that appropriate action would be taken if any blockade is encountered."
						(Print 30 39) ; "Arrive Mediterranean at 12 degrees longitude and 36 degrees latitude no later than 0700 January 29th."
						(Print 30 40) ; "Rendezvous with female CIA agent at nearby oasis at 1000 hours January 29th."
						(Print 30 41) ; "Come to periscope depth at 0930 hours this date for further coded messages."
						(Print 30 42) ; "Your destination is the Mediterranean Sea and the east coast of Tunisia. The route you must follow will include the following areas:"
						(Print 30 43) ; "Bering Sea, Bering Straits, Arctic Ocean, Greenland Sea, Denmark Strait between Greenland and Iceland."
						(Print 30 44) ; "Proceed south through the North Atlantic, east through the Straits of Gibraltar into the Mediterranean."
						(Print 30 45) ; "From time to time you will receive additional orders by radio."
						(Print 30 46) ; "Good luck and Godspeed!"
						(if local22
							(self cue:)
						else
							(= seconds 6)
...
(instance waitForEgoScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 30 47) ; "Well, son!" the Captain says, "If you're confused over what I just said, just follow me and I'll lead you back to the control room!"
			...
			(7
				(Print 30 48) ; "Well, if you're not willing to do the job you were assigned to," says the Captain with disgust, "I guess we'll have to find someone else!"
				(captain setMotion: MoveTo (+ (entryDoor x:) 10) 122 self)
			)
			(8
				(entryDoor setCycle: Beg self)
			...
			(9
				(EgoDead 7 0 0 30 49) ; "What a wimp, next time do what needs to be done."
			)
		)
	)
)
...
(instance openSafeScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(12
				(Print 30 50) ; "The machine has rejected your ID card."
				(HandsOn)
				(self init:)
			)
		)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'open/briefcase')
				(if (not (case cel:))
					(Print 30 51 #edit @local8 12) ; "Input combination (ESC to abort) (xxxxxx)"
					(cond
						...
						(else
							(Print 30 52) ; "Nothing happens."
						)
					)
				else
					(Print 30 53) ; "You already did that."
				)
			)
			((Said 'close/briefcase')
				(if (case cel:)
					(if (not (gEgo has: 2)) ; ID_Card
						(Print 30 54) ; "You remove the ID card from the slot."
						(slot dispose:)
					)
			...
			((Said 'examine,look[<at,in]/briefcase')
				(if (case cel:)
					(Print 30 55) ; "Having removed the Manila envelope from the briefcase, you take a closer look."
					(Print 30 17) ; "You think to yourself, "Something looks odd about the top section of the briefcase.""
					(Print 30 18) ; "Looking closer, you can definitely tell there is a second compartment in the top section of the briefcase."
					(Print 30 19) ; ""If I can just figure out how to get into it," you think."
				else
					(Print 30 56) ; "It's a brown leather briefcase."
				)
			)
			((Said 'look[<at]/top')
				(if (case cel:)
					(Print 30 57) ; "You find a small slot located in the top of the case."
				else
					(Print 30 25) ; "The case is currently closed."
				)
			)
			((Said 'open/top,compartment')
				(if (case cel:)
					(Print 30 58) ; "The top compartment of the briefcase requires a card with specially coded security clearance to open it."
				else
					(Print 30 25) ; "The case is currently closed."
				)
			)
			((Said 'adjust,insert,use/id,card[<id][/slot,hole]')
				(cond
					...
					((gEgo has: 2) ; ID_Card
						(HandsOff)
						(Print 30 59) ; "Taking the ID card from your pocket, you insert it into the slot."
						(gEgo put: 2) ; ID_Card
						(self cue:)
					)
					(else
						(Print 30 60) ; "You don't have your ID card."
					)
				)
			)
			((Said 'get,get/id,card[<id]')
				(if (gEgo has: 2) ; ID_Card
					(Print 30 61) ; "You already have it."
				else
					(slot dispose:)
					(Print 30 62) ; "You remove the card from the slot."
					(gEgo get: 2) ; ID_Card
					(self init:)
				)
			)
			((Said 'look,read[<at]/order')
				(Print 30 63) ; "They've been destroyed."
			)
		)
	)
)
...
(instance ficheScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HandsOn)
				(Print 30 64) ; "In the top compartment of the briefcase is a viewer used to magnify microfilm so that it is readable."
			)
			(3
				(HandsOff)
				(codeView dispose:)
			...
			(5
				(slot dispose:)
				(Print 30 65) ; "You pocket your ID card after it emerges from the viewer."
				(gEgo get: 2) ; ID_Card
				(case setScript: closeCaseScript)
			)
		)
		...
		(cond
			...
			((Said 'use/viewer')
				(if (gEgo has: 1) ; Microfilm
					(Print 30 66) ; "You need to "insert microfilm" into the viewer."
				else
					(Print 30 67) ; "You need microfilm in order to utilize the viewer."
				)
			)
			((Said 'insert/film')
				(if (gEgo has: 1) ; Microfilm
					(Print 30 68) ; "You insert the microfilm into the viewer."
					(gEgo put: 1) ; Microfilm
					(codeView setPri: 15 init:)
					(Animate (gCast elements:) 0)
					(Print 30 69) ; "CIA decoding offsets to Washington codes   by branch:   A = -2   N = 3  AF = -1   M = 5"
				else
					(Print 30 70) ; "You don't have any microfilm."
				)
			)
		...
		(cond
			...
			((Said '[/captain]>')
				(cond
					...
					((Said 'look[<at]')
						(Print 30 71) ; "He doesn't look well."
					)
					((Said 'talk')
						(Print 30 72) ; "The paramedic speaks up and says, "I'm sorry Commander, but the Captain is under medication and sleeping.""
						(Print 30 73) ; ""The amount of pain he was experiencing from the head injury made it necessary to sedate him," he explains."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/doc]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 74) ; "Looks the medical type."
					)
					((Said 'talk')
						(if (not local5)
							(= local5 1)
							(Print 30 75) ; "The "Doc" looks at you and says, "I'm sure the Captain will require this medication for a few days to come.""
							(Print 30 76) ; "Well," he continues, "It looks like the Blackhawk and crew is in your hands now."
						else
							(Print 30 77) ; "Please excuse me," he says, "I must attend to the Captain."
						)
					)
				)
			)
		...
		(cond
			...
			((Said '[/chair]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 78) ; "A typical Navy chair."
					)
					((Said 'sit')
						(Print 30 79) ; "This is no time for a break."
					)
					((Said 'stand')
						(Print 30 80) ; "You are already standing."
					)
				)
			)
		)
		...
		(cond
			...
			((Said '[/safe]>')
				(cond
					...
					((Said 'look<in')
						(if (not cel)
							(Print 30 81) ; "You can't see into it."
						else
							(Print 30 82) ; "You see the inside of the safe."
						)
					)
					((Said 'look[<at]')
						(Print 30 83) ; "A safe."
					)
					((GoToIfSaid self event 222 115 'open' 30 84))
					((Said 'open')
						(if (safeDoor cel:)
							(Print 30 53) ; "You already did that."
						else
			...
			((Said '[/code,keypad]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 85) ; "You see a keypad next to the safe."
					)
					((GoToIfSaid self event 222 115 'enter/code' 30 84))
					((Said 'enter')
						(if (safeDoor cel:)
							(Print 30 53) ; "You already did that."
						else
			...
			((Said '[/door]>')
				(cond
					...
					(
						(and
							(> (gEgo x:) x)
							(GoToIfSaid self event stupidAvoider 3 'open' 30 84)
						))
	...
	(method (handleEvent event)
		(cond
			((Said 'look[<at,around][/room]')
				(Print 30 87) ; "You look around the small but adequate bathroom facility."
				(Print 30 88) ; "It is equipped with a small sink, mirror, shower and marine toilet. A small light is above the toilet."
			)
			((Said 'look/sink')
				(Print 30 89) ; "It's small and made of stainless steel."
			)
			((or (Said 'use/sink') (Said 'wash/hand') (Said 'turn<on/water'))
				(Print 30 90) ; "In an effort to conserve the water supply, you quickly wash your hands then turn off the water."
			)
			((Said 'look/mirror')
				(Print 30 91) ; "You look into the mirror and think, "Yep, I still look the same.""
			)
			((Said 'comb/hair')
				(Print 30 92) ; "Since cleanliness and grooming are part of Navy life, you look into the mirror and comb your hair."
			)
			((Said 'look/shower')
				(Print 30 93) ; "The only thing special about this shower is that it's small."
			)
			((Said '(turn<on),use,get/shower')
				(Print 30 94) ; "In record time you take a hot shower."
			)
			((Said 'look/light')
				(Print 30 95) ; "This is a typical marine toilet that dumps waste into the ship's holding tanks."
				(Print 30 96) ; "When the holding tanks are full, they are pressurized to force the waste into the open ocean."
				(Print 30 97) ; "If you flush the toilet when this light is on, the pressure will forcibly expelled the contents of the holding tanks through your toilet."
				(if local12
					(Print 30 98) ; "The light is currently on."
				else
					(Print 30 99) ; "The light is currently not on."
				)
			)
			((Said 'look[/toilet]')
				(Print 30 95) ; "This is a typical marine toilet that dumps waste into the ship's holding tanks."
				(Print 30 96) ; "When the holding tanks are full, they are pressurized to force the waste into the open ocean."
				(Print 30 100) ; "It is highly recommended that the toilet not be flushed when the tanks are pressurized."
			)
			((Said 'flush[/toilet]')
				(cond
					(local11
						(= local11 0)
						(flushNoise play:)
						(Print 30 101) ; "Your failure to follow explicit instruction has left you a stinking mess."
						(Print 30 102) ; "As you hurriedly clean the enormous amount of human waste off your body, we are busy deducting 3 points from your score."
						(gGame changeScore: -3)
					)
					(local12
						(Print 30 103) ; "The holding tanks are pressurized. Flushing is not recommended."
						(= local12 (Random 0 1))
						(= local11 1)
					)
					(else
						(Print 30 104) ; "You flush the toilet."
						(= local12 (Random 0 1))
					)
				)
			)
			((Said 'sit,use/toilet,bathroom')
				(Print 30 105) ; "You use the toilet, flush it, then wash your hands."
			)
			((or (Said 'leak') (Said 'get//leak'))
				(Print 30 106) ; "Step closer, Johnny, it's shorter than you think."
			)
			((or (Said 'shit') (Said 'get//shit'))
				(Print 30 107) ; "The human process of waste elimination has made you two pounds lighter."
			)
			((Said 'get/*')
				(Print 30 108) ; "You don't need it."
			)
			((Said 'look/*')
				(Print 30 109) ; "That's not important to you now."
			)
			((Said 'open[/door]>')
				(bathDoor handleEvent: event)
			)
			((Said '*')
				(Print 30 110) ; "Not in here."
			)
		)
	)
)
		...
		(cond
			...
			((Said '[/door]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 111) ; "The door to this cabin."
					)
					((Said 'close')
						(proc0_31) ; "It is."
					)
					((< (gEgo x:) (bathDoor x:)))
					((GoToIfSaid self event 267 125 'open' 30 84)
						(= local21 1)
					)
					((Said 'open')
						(= local21 1)
		...
		(cond
			...
			(
				...
				(if (StrCmp @local0 {23448803})
					(Print 30 112) ; "That is not the correct combination."
					(HandsOn)
					(keyPad dispose:)
					(= local0 0)
				else
		...
		(cond
			...
			((Said '[/closet,cabinet]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 113) ; "A storage closet."
					)
					((Said 'open')
						(Print 30 114) ; "There's nothing of interest in there."
					)
					((Said 'close')
						(proc0_31) ; "It is."
					)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/bunk,bed]')
				(Print 30 115) ; "It's the latest in submarine slumber."
			)
			((Said 'sit')
				(Print 30 79) ; "This is no time for a break."
			)
		...
		(cond
			...
			((Said '[/couch]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 116) ; "A plush couch like this is only found in officer's quarters."
					)
					((Said 'sit')
						(Print 30 79) ; "This is no time for a break."
					)
		...
		(cond
			...
			((Said '[/bookcase,shelf]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 118) ; "The captain's bookshelf."
					)
					((Said 'open')
						(Print 30 119) ; "No, it's private."
					)
					((Said 'close')
						(proc0_31) ; "It is."
					)
		...
		(cond
			...
			((Said '[/desk]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 120) ; "The captain's desk."
					)
					((Said 'open')
						(Print 30 119) ; "No, it's private."
					)
		...
		(cond
			...
			((Said '[/dresser]>')
				(cond
					((TurnIfSaid self event 'look[<at]/*'))
					((Said 'look[<at]')
						(Print 30 122) ; "The captain's nightstand."
					)
					((Said 'open')
						(Print 30 119) ; "No, it's private."
					)
