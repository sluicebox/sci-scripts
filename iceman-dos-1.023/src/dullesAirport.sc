...
(instance dullesAirport of Rm
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'open/door')
				(Print 17 2) ; "The doors open automatically when you walk near them."
			)
		)
	)
)
...
(instance driverScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Print 17 7) ; "The driver asks you, "Are you Commander Westland?""
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((< state 2))
			((GoToIfSaid client event client 15 'show,give/id,card' 17 8)
				(= register 0)
			)
			((Said 'show,give/id,card')
				(if (gEgo has: 2) ; ID_Card
					...
					(Print 17 9) ; "You forgot your id card on the island."
...
(instance endOfGameScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0 0)
			(1
				(EgoDead 918 0 0 17 10) ; "Since you have lost your ride, you have no alternative other than starting the game over, or restoring a previous save game."
			)
		)
	)
)
...
(instance notWestlandScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0 0)
			(1
				(Print 17 11) ; "For the purpose of this game you are, in fact, Commander Westland."
...
(instance getEgoIdScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(Print 17 12) ; ""It is my responsibility to look at your identification, sir," he says."
			)
			(1
				(Print 17 13) ; "You show the driver your military identification card.  He examines it closely, thanks you as he returns it, and says..."
				(User canInput: 1)
				(Print 17 14) ; "Please have a seat, Commander."
				(gGame changeScore: 1)
				(= seconds 1)
			)
			(2
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'talk')
				(Print 17 15) ; "The driver says to you, "Sorry, Sir, I have instructions not to divulge any information."
			)
		)
	)
)
...
(instance waitForResponseScript of Script
	...
	(method (handleEvent)
		(cond
			...
			((Said 'talk/man')
				(Print 17 16) ; "The driver says to you, "Sorry sir, I have instructions not to divulge any information."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'call/cab[/cab]')
				(cond
					((& signal $0008)
						(Print 17 17) ; "Your ride will be here shortly."
					)
					((OneOf (script state:) 3 4)
						(Print 17 18) ; "You missed your ride."
					)
					(else
						(Print 17 19) ; "Use the limo."
					)
				)
			)
			((TurnIfSaid self event 'look[<at]/*'))
			((Said 'look[<at][/car,limo]')
				(if (& (limo signal:) $0008)
					(Print 17 20) ; "You don't see any cars at the moment."
				else
					(Print 17 21) ; "A large shiny black limo with dark windows."
				)
			)
		)
	)
...
(instance driver of Act
	...
	(method (handleEvent event)
		(cond
			...
			((TurnIfSaid self event 'look[<at]/*'))
			((Said 'look[<at][/man]')
				(Print 17 22) ; "This is your driver."
			)
		)
	)
)
...
(instance wheel1 of Act
	...
	(method (handleEvent event)
		(cond
			...
			((TurnIfSaid self event 'look[<at]/*'))
			((not y))
			((Said 'look[<at][/tire,wheel]')
				(Print 17 23) ; "Although a little worn, these tires should be all right for a while, unless this is the limo for the Sierra company Christmas party."
			)
		)
	)
)
...
(instance slidingDoor1 of Act
	...
	(method (doit)
		...
		(cond
			(
				(and
					(not (InRect 45 120 170 160 gEgo))
					(not (InRect 45 120 170 160 driver))
					(== x 64)
				)
			...
			(
				(and
					(or
						(InRect 48 124 165 154 gEgo)
						(InRect 48 128 165 154 driver)
					)
					(== x 93)
				)
...
(instance slidingDoor2 of Act
	...
	(method (doit)
		...
		(cond
			(
				(and
					(not (InRect 120 120 290 160 gEgo))
					(not (InRect 120 120 290 160 driver))
					(== x 146)
				)
			...
			(
				(and
					(or
						(InRect 130 124 284 154 gEgo)
						(InRect 130 128 284 154 driver)
					)
					(== x 175)
				)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look[<at][/airplane,airplane,jet]')
				(Print 17 24) ; "Looks like it's about to land."
			)
		)
	)
)
