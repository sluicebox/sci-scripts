...
(instance outOfAir of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if (gEgo has: 6) ; Tahiti: Earring | Sub: Diver | Tunisia: Key
					(EgoDead 157 0 1 51 10) ; "Forced to the surface because your air ran out, you were captured."
				else
					(EgoDead 157 0 1 51 11) ; "You were forced to the surface because your air ran out. The dive vehicle would have extended your range."
				)
			)
		)
	)
...
(instance headingGauge of Code
	...
	(method (doit &tmp [temp0 3])
		...
		(Display 51 12 dsCOORD 219 17 dsCOLOR 8 dsALIGN alRIGHT dsWIDTH 25 dsFONT 30) ; "888"
		(Display
			(Format @temp0 51 13 local22) ; "%d"
			dsCOORD
			219
			17
			dsCOLOR
		...
		(Display 51 12 dsCOORD 267 17 dsCOLOR 8 dsALIGN alRIGHT dsWIDTH 25 dsFONT 30) ; "888"
		(Display
			(Format @temp0 51 13 local23) ; "%d"
			dsCOORD
			267
			17
			dsCOLOR
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond
					((localproc_6)
						(Print 51 14) ; "You look for a good spot to place the explosives."
						(gEgo
							illegalBits: 0
					...
					(else
						(= local26 0)
						(Print 51 15) ; "You're not near enough to the oil rig."
						(HandsOn)
						(self dispose:)
					)
				)
			...
			(6
				(Print 51 16) ; "You start the timer."
...
(instance blowUp1 of Script
	...
	(method (changeState newState &tmp temp0 temp1)
		(switch (= state newState)
			...
			(2
				(soundExplosion dispose:)
				(EgoDead 926 1 0 51 17) ; "Your next assignment is as a crash test dummy for GM."
			)
		)
	)
)
			...
			(3
				...
				(cond
					((not (& (subMarine roomFlags:) $0020))
						(Print 51 18) ; "You should have checked the DV-3X. A missing washer has caused the diver to malfunction."
					)
					((not (& (subMarine roomFlags:) $0040))
						(Print 51 19) ; "Although you suppiled a missing washer, you forgot to put a nut on the washer."
					)
					(else
						(Print 51 20) ; "You did not tighten the nut properly and now you are without the diver."
					)
				)
				(self dispose:)
			)
