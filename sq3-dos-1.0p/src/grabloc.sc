		...
		(cond
			...
			((Said 'look>')
				(cond
					...
					((Said '/partition,console[<w,e,north,w]')
						(Print 703 25) ; "The walls are made of the same composite material as in other locations around this junk-strewn hell-hole."
					)
					((Said '/banister,banister,beam[<iron]')
						(if (< global132 4)
							(Print 703 26) ; "The rail is supported by beams hanging from the ceiling. It's probably used by some sort of scrap transport equipment."
						else
							(Print 703 27) ; "The rail is supported by beams hanging from the ceiling. The grabber hangs beneath it."
						)
					)
					((Said '/lamp')
						(Print 703 28) ; "All available light is being ported into the room via a series of vents in the ceiling."
					)
					((Said '/grate')
						(Print 703 29) ; "Light is pouring in from small vents around the ship."
					)
				)
			)
			((Said 'drop,hold[<in]/crystal/mouth')
				(Print 703 30) ; "That's only helpful in Space Quest II."
			)
			((Said 'eat,break/crystal[<glowing]')
				(Print 703 31) ; "That won't help you."
			)
			(
				(Said
					'sit,crawl,(get<off)[<down,on]/banister,banister,beam,conduit,bar'
				)
				(Print 703 32) ; "That wouldn't be helpful."
			)
			((Said 'use,get/motivator<motivator')
				(if (== global135 3)
					(Print 703 33) ; "Leave it alone. It has no human interface capability."
				else
					(Print 703 34) ; "That isn't here."
				)
			)
			((Said 'nap,(lie<down)[/deck]')
				(Print 703 35) ; "Are you crazy? You've got adventuring to do."
			)
			((or (== global132 4) (== global132 5))
				(cond
					((Said 'look/chair[<grabber,device]')
						(Print 703 36) ; "You seem to be covering all available seat surface area making viewing impossible."
					)
					((Said '(stand[<up]),(get<up,out,off)')
						(Print 703 37) ; "Please remain seated while riding the grabber."
					)
					((Said 'look/button[<claw]')
						(Print 703 38) ; "A grimy button on the panel reads "CLAW.""
					)
					((Said 'get[<up]/motivator')
						(if (== global135 3)
							(Print 703 39) ; "Leave that to the grabber."
						else
							(Print 703 40) ; "You can't do that yourself."
						)
					)
					((Said 'look/control,console[<device,grabber]')
						(Print 703 41) ; "Your view of the grabber from here reveals a Forward/Reverse control stick (currently grasped), and a button marked 'CLAW'"
					)
					(
						(or
							(Said
						...
						(Print 703 42) ; "Just move as you've been moving."
					)
					((Said 'release[/throttle,stick,control]')
						(Print 703 43) ; "That's not necessary."
					)
					((Said 'look/claw[<grabber,device]')
						(Print 703 22) ; "Your viewing angle doesn't allow that."
					)
					((Said 'climb')
						(Print 703 44) ; "You are unable to scale anything here."
					)
					(
						(or
						...
						(Print 703 45) ; "That won't help."
					)
					((Said 'get,jump,leap,climb[<onto,on,to]/conduit,bar')
						(Print 703 46) ; "That wouldn't be safe."
					)
					(
						(Said
							'stand,climb,get[<up,onto,off,down,on,out]/grabber,device,claw,banister,beam'
						)
						(Print 703 47) ; "You aren't able to do that presently."
					)
					((Said 'open/grate')
						(Print 703 48) ; "You can't."
					)
					((Said 'jump/craft')
						(Print 703 49) ; "Dream on."
					)
					((Said 'jump[<down,up]')
						(Print 703 50) ; "That isn't currently one of your options."
					)
				)
			)
		)
