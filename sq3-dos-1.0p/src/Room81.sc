			...
			(cond
				((Said 'look>')
					(cond
						(
							...
							(Print 81 1) ; "You are hiding behind some of the strange local vegetation looking out on a clearing and an entrance to a large underground complex. From the logo on the entrance, you deduce that this must be the infamous ScumSoft headquarters."
						)
				...
				(
					...
					(cond
						...
						(else
							...
							(RedrawCast)
							(Print 81 9) ; ""WOW! This thing really works." You then quickly realize that you only have a few moments before the belts power pack is depleted."
							(= global121 350)
						)
					)
				)
				((or (Said 'turn<off/belt') (Said 'deactivate/belt'))
					(cond
						...
						((not global126)
							(Print 81 10) ; "You're not wearing it."
						)
						((!= global105 1)
							(Print 81 11) ; "It's not on."
						)
						(else
							(Print 81 12) ; "OK"
							(= global105 0)
				...
				((Said 'disembark,disembark/')
					(switch
						(= local0 (Print 81 13 #button {Ship} 1 #button {ScumSoft} 2)) ; "Where to Buddy?"
						(1
							(gCurRoom newRoom: 80)
						)
						(2
...
(instance Actions of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 81 14) ; "You make your way through the forest of strange trees to this clearing where you discover the entrance to some large underground complex. This must be ScumSoft. Suddenly the door to the complex begins to open."
				(= global129 1)
				(door
					illegalBits: 0
					ignoreActors:
			...
			(7
				(guard4 setLoop: 2)
				(RedrawCast)
				(Print 81 15) ; "Several guards file out of the entrance and disperse into the woods. They must have been alerted to your presence when you landed. Two guards remain behind to watch the entrance."
				(= seconds 12)
			)
			(8
				(switch
