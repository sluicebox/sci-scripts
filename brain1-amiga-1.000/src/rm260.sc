...
(instance trap of Prop
	...
	(method (doVerb theVerb)
		(cond
			...
			((IsObject cycler)
				(proc5_9 260 4) ; "This transporter is active. It would probably be a good idea to keep the robot away from here."
			)
			(else
				(proc5_9 260 5) ; "This transporter is off right now."
			)
		)
	)
)
	...
	(method (doVerb theVerb &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6)
		...
		(cond
			((== theVerb 2)
				(if temp1
					(if [local40 temp0]
						(proc5_9 260 6) ; "This is a right turn signal. When the robot hits this signal, it will turn right. (If this makes it hit a wall, it will bounce off and go the opposite direction.)"
					else
						(proc5_9 260 7) ; "Click the hand here to cause the robot to turn right when it hits the signal. Right now, the signal is inactive."
					)
				)
			)
			((!= theVerb 3)
...
(instance hintCoin of Feature
	...
	(method (doVerb theVerb invItem &tmp [temp0 100])
		(cond
			...
			((not global115)
				(proc5_9 260 8) ; "You are out of hint coins."
			)
			((== global166 31)
				(proc5_9 260 9) ; "You can't buy any more hints here."
			)
			(else
				(if gModelessDialog
					(gModelessDialog dispose:)
				...
				(if (not (-- global115))
					(gEgo put: 0)
					(proc5_9 260 10) ; "You insert your last hint coin in the slot."
				else
...
(instance helpSquare of Feature
	...
	(method (doVerb)
		(proc5_9 260 12) ; "Help the robot travel through the maze by clicking on intersections. At an "active" intersection, the robot will turn right. At "inactive" intersections, the robot goes straight. Get the robot to pass over as many "A" squares as possible -- they will write Robot Riddle answers to your data card."
		(proc5_9 260 13) ; "Watch out for the red and white spirals -- they are traps which will transport the robot back to the beginning of the maze. When the robot runs over a switch, one or more of the traps will turn on -- or off."
	)
)
