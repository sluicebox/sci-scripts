...
(class IconBar of Set
	...
	(method (noClickHelp &tmp event lastIcon thisIcon [tmpStr 150] oldPort eO)
		(= lastIcon (= thisIcon 0))
		(= oldPort (GetPort))
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 temp4 temp5 temp6 temp7 temp8)
		...
		(cond
			...
			(
				...
				(self doit:)
				(= temp7
					(if (and (== temp6 curIcon) (== temp8 curInvIcon))
						temp5
					else
						(curIcon cursor:)
					)
				...
				(if temp0
					(gGame setCursor: temp7 1 oldMouseX oldMouseY)
				else
	...
	(method (dispatchEvent event &tmp evtY evtX evtType evtMsg thisIcon evtClaimed thePort)
		...
		(if (& evtType $0040) ; direction
			...
			(switch evtType
				(evNULL
					(cond
						((not (and (<= -10 evtY height) (<= 0 evtX 320)))
							(if
								(and
									(& state $0400)
									(or
