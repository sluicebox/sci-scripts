...
(class TopMessage of PanelText
	...
	(method (toggleText &tmp temp0)
		(if (vmdText text:)
			...
			(vmdText newText: {Cliquez pour interrompre cette vid\82o})
		)
	)
...
(class GK2 of Game
	...
	(method (getDisc param1 &tmp temp0)
		...
		(while (and (!= -1 (= temp0 (CD 0 param1))) (!= temp0 param1)) ; Check
			(Printf {Ins\82rez CD #%d et appuyez sur ENTREE} param1)
		)
		(return temp0)
	)
	...
	(method (changeScore param1 &tmp [temp0 2])
		...
		(return (scoreText newText: (Str format: {Score: %d sur 678} gScore)))
	)
	...
	(method (quitGame)
		...
		(= gQuit
			(Print
				...
				back: 4
				fore: 14
				addTitle: {Merci d'avoir jou\8a \85 Gabriel Knight!}
				addText: {Etes-vous certain de vouloir quitter?} 0 3
				addButton: 1 { Oui } 55 20
				addButton: 0 { Non } 95 20
				mode: 1
				init:
			)
		)
