...
(instance RoomScript of Script
	...
	(method (handleEvent event)
		...
		(cond
			((Said 'fuck,fuck,board,board,(climb,lie,get,get<on)/log')
				(cond
					((!= gEgoState 0)
			...
			(
				(Said
					'rise,exit,(get,get<off),(climb,get,get<off,up),(get,get<up),(rise,nightstand<up)'
				)
				(if (!= gEgoState 550)
			...
			((Said 'pull,pull,pull,pull,use/log')
				(cond
					((not (< (gEgo distanceTo: aLog) 25))
						(NotClose) ; "You're not close enough."
					)
			...
			((or (Said 'board,exit,exit,(get,get<in)/water') (Said 'swim,wade'))
				...
				(cond
					((Said '/log')
						(cond
							...
							(else
								(Print 550 17) ; "Look! A small log rests in the weeds near the riverbank."
							)
						)
					)
					((Said '/plant,bush')
						(Print 550 18) ; "Dutch rush grows in the shallow water at the edge of the stream."
					)
					((Said '/palm')
						(Print 550 19) ; "The only trees you can see from here are all the way across the river, and completely out of your reach."
					)
					((Said '[/area,water,creek]')
						(Print 550 20) ; "A roaring river rushes by. It looks too deep to wade across and too rapid to swim. How will you proceed from here?"
					)
				)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(Print 550 21) ; "You mount the gnarly log."
				(if (>= gFilthLevel 3)
					(Print 550 22 #at -1 144) ; "(...an experience not wholly unfamiliar to you!)"
				)
			)
			(7
				(Print 550 23) ; "Boy, is this fun! Sitting on a log, mired in the mud of the river bottom."
				(gUser canInput: 1)
			)
			(8
				(Ok) ; "O.K."
			...
			(11
				(Print 550 24) ; "You did it!"
				(Print 550 25) ; "The current has caught your log and is taking you downstream... toward your Larry?"
			)
			(12
				(gEgo setLoop: 3 setMotion: MoveTo 348 73 self)
			)
			(13
				(Print 550 26) ; "Press F8 to bypass the next scene."
				(gCurRoom newRoom: 560)
			)
			(14
				(HandsOff)
				(= gEgoState 1003)
				(Print (Format @string 550 27 global289) #at -1 10 #dispose) ; "%s"
				(= drowningLoop 0)
				(gEgo
			...
			(16
				...
				((ScriptID 40) ; DyingScript
					caller: 816
					register: (Format @string 550 28) ; "Patti! Be more careful! There are treacherous undercurrents in this river."
					next: (Format @string2 550 29) ; "Head over heels but not in love!"
				)
			)
		)
	)
