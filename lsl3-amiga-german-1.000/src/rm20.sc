...
(instance rm20 of Locale
	...
	(method (handleEvent event &tmp i theObj xyWindow evt [string 90] [string1 90] [string2 90])
		...
		(switch (event type:)
			...
			(evKEYBOARD
				(switch (event message:)
					...
					(KEY_ALT_f
						(^= gDebugOn $0001)
						(Print
							(Format ; "Debug will%spop up on next newRoom."
								@string
								20
								1
								(if gDebugOn { } else { NOT })
								(if gDebugOn { } else { NICHT })
							)
						)
					)
					(KEY_ALT_g
					...
					(KEY_ALT_r
						(Print (Format @string 20 2 gCurRoomNum gCurRoomNum)) ; "You are currently enjoying room %d."
					)
					(KEY_ALT_s
						(if gEgoIsPatti
							(= gEgoIsPatti 0)
			...
			(evSAID
				...
				(if (and (Said 'get,get>') (= theObj (gInventory saidMe:)))
					(event claimed: 1)
