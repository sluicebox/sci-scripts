...
(procedure (StartExercising onWhat)
	...
	(gUser mapKeyToDir: 0 canInput: 1)
	...
	(method (changeState newState)
		...
		(switch (= state newState)
			...
			(4
				...
				(if touchedBottom
					(if (== (++ gLegCurlsCount) LegCurlsMax)
						(Printf 380 17 gLegCurlsCount gLegCurlsCount) ; "Congratulations! You've done %d leg curls."
					)
					(UpdateCounters)
				)
			)
			...
			(6)
			(7
				(gUser mapKeyToDir: 1 canInput: 0 canControl: 0)
			...
			(14
				...
				(if touchedBottom
					(if (== (++ gBenchPressCount) BenchPressMax)
						(Printf 380 18 (* 100 gBenchPressCount) (* 100 gBenchPressCount)) ; "Congratulations! You've pumped over %d pounds."
					)
					(UpdateCounters)
				)
			)
			...
			(24
				...
				(if touchedBottom
					(if (== (++ gPullupsCount) PullupsMax)
						(Printf 380 19 gPullupsCount gPullupsCount) ; "Congratulations! You've done %d pull ups."
					)
					(UpdateCounters)
				)
			)
			...
			(32
				...
				(if touchedBottom
					(if (== (++ gBarPullCount) BarPullMax)
						(Printf 380 20 gBarPullCount gBarPullCount) ; "Congratulations! You've done %d bar pulls."
					)
					(UpdateCounters)
				)
			)
