...
(procedure (HaveMem howMuch)
	(if (u> (MemoryInfo 1) howMuch) ; FreeHeap
		(return 1)
	else
		(Print 0 182) ; "You can't do that here; at least, not now."
		(return 0)
	)
)
...
(instance lslMDH of EventHandler
	(properties)
)

(instance lslKDH of EventHandler
	(properties)
)

(instance lslDDH of EventHandler
	(properties)
)
...
(instance LSL3 of Game
	...
	(method (init &tmp temp0 temp1)
		((= gSystemWindow theWindow) color: (= gTextColor 1) back: (= gBackgroundColor 15))
		(= gUser PUser)
		(super init:)
		(= global290
		...
		((= gMouseDownHandler lslMDH) add: gRegions gLocales gCast)
		((= gDirectionHandler lslDDH) add: gRegions gLocales)
		((= gKeyDownHandler lslKDH) add: gCast gRegions gLocales)
		(= global591 8)
		(DoSound sndMASTER_VOLUME 8)
		(SL code: statusCode)
	...
	(method (startRoom roomNum)
		(LoadMany 0 993 991 988 973 971 969 967 21 50 51 42 43 44 45 46 40 421)
		(DisposeScript 958)
		(if gDebugOn
			(= gDebugOn 0)
			(SetDebug)
		...
		(cond
			...
			(
				(and
					...
					(or
						(Said 'look<back,back/keycard,card')
						(Said 'look/back,back/keycard,card')
						(Said 'look/(keycard,card)/back,back')
						(Said 'look/back,back[<keycard,card]')
...
(instance statusCode of Code
	...
	(method (doit str)
		(if global303
			(Format ; "Score: %d of 4000 %7s Room Number: %d"
				str
				0
				172
				gScoreDisplayed
