...
(local
	[local0 180]
	[local180 66]
	[local246 20] = [-16564 5177 19666 27846 18175 19476 14668 -11668 -14778 -2049 -12039 -6221 -28275 -28200 -29441 -24077 -12441 8987 9137 6655]
	local266 = 500
	local267
	local268
	local269
...
(procedure (RedrawMaze &tmp i)
	(gCurRoom drawPic: local266)
	(if (BitSet (+ 0 local267))
		(gCurRoom overlay: (+ local266 4))
	)
	(if (BitSet (+ 80 local267))
		(gCurRoom overlay: (+ local266 2))
	)
	(if (BitSet (+ 240 local267))
		(gCurRoom overlay: (+ local266 3))
	)
	(if (BitSet (+ 160 local267))
		(gCurRoom overlay: (+ local266 1))
	)
)

(procedure (BitSet index)
	(return
		(if (& [local246 (/ index 16)] (>> $8000 (mod index 16))) 1 else 0)
	)
)
...
(instance RoomScript of Script
	...
	(method (handleEvent event &tmp i)
		...
		(cond
			...
			((Said 'rise,(get,get,rise,nightstand<up)')
				(Print 500 5) ; "You simply can not."
			)
			(
				(or
					(Said 'sip,sip/water')
					(Said 'get/drink,drink<1')
					(Said 'use,drink,drink,drain,drain/water,beer,bottle')
				)
				(cond
					((!= gEgoState 0)
			...
			((Said 'look,look>')
				(cond
					...
					((Said '/bamboo')
						...
						(Print
							(Format @local0 500 12 gBambooCount gBambooCount) ; "Which is exactly %d."
							#at
							-1
							144
						)
		...
		(switch (= state newState)
			...
			(6
				...
				((ScriptID 40) ; DyingScript
					...
					next: (Format @local180 500 24) ; "Bamboo-zled"
				)
			)
		)
	)
