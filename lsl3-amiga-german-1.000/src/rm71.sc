...
(instance RegionScript of Script
	...
	(method (handleEvent event &tmp temp0 [temp1 30] [temp31 30])
		(if (or (!= (event type:) evSAID) (event claimed:))
			(return)
		)
		(cond
			((Said 'embrace,embrace,embrace')
				(Print 71 1) ; "How you'd love to sneak a little peck onto that gorgeous cheek!"
			)
			((Said 'eat')
				(Printf 71 2 gEgoName gEgoName) ; "%s! Just think where that tongue of yours has been!"
			)
			((Said 'drain/cloth,skirt,dress,dress')
				(Print 71 3) ; "That may be possible if you'd play the game better!"
			)
			(
				(or
					(Said 'eat,fuck,fuck/me')
					(Said
						'clit,shit,shit,leak,leak,fuck,fuck,fart,fart,boob,ass,bullshit'
					)
				)
				(Printf 71 4 gEgoName gEgoName) ; "My, %s! You certainly have a charming way with the opposite sex."
			)
			((Said '/casino')
				(PrintL 71 5) ; "Do you enjoy gambling at the casino?"
				(PrintP 71 6) ; "Are you crazy? Nobody ever wins there!"
			)
			((Said '/entertainer,maller,bambi,attorney,dale,cheri')
				(Printf 71 7 (StrSplitInTwo @temp1 @temp31 @nameStr) @temp31) ; "Where? You're talking to %s, remember?"
			)
			((Said 'talk,talk')
				(Printf 71 8 gEgoName gEgoName) ; "Just say what you wish to say, %s."
			)
			((Said '/name')
				(PrintL 71 9) ; "What's your name, baby?"
				(PrintP 71 10) ; "You said your name, then I said my name. Don't you remember?"
			)
		)
	)
