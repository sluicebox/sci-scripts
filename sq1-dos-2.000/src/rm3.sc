...
(instance rm3 of SQRoom
	...
	(method (doit)
		(super doit:)
		(if (and (not local0) (& (gEgo onControl: 1) $0002))
			(gSoundEffects number: 311 loop: 1 play:)
			(= local0 1)
			(leftDoor setCycle: End leftDoor)
		)
		(if (and (not local0) (& (gEgo onControl: 1) $0008))
			(gSoundEffects number: 311 loop: 1 play:)
			(= local0 1)
			(rightDoor setCycle: End rightDoor)
		)
		(if (and local0 (not (& (gEgo onControl: 1) $000a)))
			(gSoundEffects number: 311 loop: 1 play:)
			(= local0 0)
			(if (leftDoor cel:)
	...
	(method (newRoom newRoomNumber)
		...
		(if (!= (gLongSong number:) 355)
			...
			(if (and (== newRoomNumber 103) (not (proc700_1 553 16)))
...
(instance mainComputer of Feature
	(properties
		...
		sightAngle 45
		onMeCheck 64
		lookStr {This is where the sophisticated electronics for the complete Data Archive System are housed.}
	)
