...
(class DeltaurRegion of Rgn
	...
	(method (init)
		...
		(if (< gPrevRoomNum 50)
			(Print 703 42) ; "DEBUG *REMOVE* Teleport initialization."
			(if (== gCurRoomNum 54)
				(= egoStatus 0)
				(gEgo get: 4) ; Knife
			else
			...
			(4 ; Inventory
				(if (== level global166)
					(switch invItem
						...
						(0 ; Cartridge
							(Print 703 44) ; "You don't really want to show an Arcada data cartridge to the guys who blew the ship to smithereens."
						)
						(19 ; Sarien_ID_Card
							(Print 703 45) ; "He seems satisfied that you're a Sarien, all right."
						)
						(else
							(super doVerb: theVerb invItem &rest)
						)
				...
				(cond
					((and (== gCurRoomNum 66) (not regionPathID))
						(if (== (DeltaurRegion egoStatus:) 1)
							...
							(Print 703 46) ; "Are you crazy! You haven't got time to talk to this guy. He's too busy doing his talking with his gun."
						)
					)
					(
						(and
					...
					(else
						(Print 703 47) ; "You're not close enough to talk to the guard."
					)
				)
			)
			(2 ; Look
				(if (== global166 level)
					(Print 703 48) ; "It's just another one of those ugly Sarien guards."
				else
					(Print 703 49) ; "You can't do that from here."
				)
			)
			(else
				(super doVerb: theVerb invItem &rest)
		...
		(switch (= state newState)
			(0
				...
				(if (== (DeltaurRegion timeToBlow:) 20)
					(Print 703 50) ; "You'd better hurry up, Roger! You've got 20 seconds before you're turned into such tiny particles that they couldn't find you with an ion microscope."
				)
