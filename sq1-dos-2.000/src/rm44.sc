...
(instance rm44 of Rm
	...
	(method (init &tmp [temp0 30])
		...
		(if global175
			(info cel: 1 stopUpd:)
			(localproc_0 (Format @temp0 44 0 global175 gBuckazoidCount)) ; "You Bet $%u You Have $%u"
		else
			(info setCycle: Fwd)
		)
	)
			...
			(3
				(if (!= local0 1000)
					...
					(if local0
						...
						(localproc_0
							(Format @temp0 44 9 (* local0 global175) gBuckazoidCount) ; "You Win $%u You Have $%u"
						)
					...
					(10 ; buckazoid
						(cond
							...
							((< global175 3)
								...
								(localproc_0
									(Format @temp0 44 0 global175 gBuckazoidCount) ; "You Bet $%u You Have $%u"
								)
							)
							(else
