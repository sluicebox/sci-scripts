			...
			(switch theVerb
				...
				(2 ; Look
					(cond
						...
						(else
							(Print 64 11) ; "The guard lies in a red heap on the floor, overcome by the toxic gas from the grenade. Some kind of electronic gadget is hanging from his belt."
						)
					)
				)
				(5 ; Talk
...
(instance killEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 64 19) ; "Oh no Roger! The guard has seen you and you're not wearing the helmet! You're in for it now Wilco!"
			...
			(12
				(EgoDead 940 2 0 64 20) ; "Boy, you're no Marshall Dillon, that's for sure. And you were so close. You'll need to react a little faster be little sharper than that. Well, throw a little bondo into that hole and get back in there."
				(self dispose:)
			)
		)
	)
		...
		(switch (= state newState)
			...
			(5
				(if (not (< (guard nsLeft:) local0 (guard nsRight:)))
					(grenade dispose:)
					(if (not (IsFlag 47))
						(Print 64 21) ; "The Sarien warrior is guarding the stolen Star Generator so intensly, he didn't notice the gas grenade exploding."
						(Print 64 22) ; "Actually, his walkman is cranked up too high."
					else
						(Print 64 23) ; "The dead Sarien guard might have noticed that, if he were alive."
					)
				)
				(gEgo
					setLoop: (+ (gEgo loop:) 2)
...
(instance getRemote of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 64 24) ; "You remove the device from the guard's belt. It looks like you have found the remote control that turns off the force field around the Star Generator."
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(leftShieldEmitter dispose:)
				(Print 64 25) ; "You press the stud on the remote and the force field around the Star Generator begins to deplete."
...
(instance toastEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDead 945 0 0 64 26) ; "Ouch! That stings just a tad. You'll obviously need to disarm that baby before trying that again."
				(self dispose:)
			)
		)
	)
...
(instance showCard of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(Printf 64 27 global184) ; "Sir, the destruct code is %d."
				(gEgo setCycle: Beg self)
			)
			(4
				(proc703_1)
...
(instance bumpOffShield of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Print 64 28) ; "It appears the shield is preventing you from getting to the Star Generator's control panel and boy is it hot. You'd better not try to get close to the Star Generator again."
...
(instance setToBlow of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(Print 64 29) ; "The Star Generator is already set to blow Roger! You had better find someway to get a million miles from the Deltaur and quick!"
				(gEgo setMotion: PolyPath 163 172 self)
			)
			(1
				(HandsOn)
...
(instance shootTheEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 64 30) ; "Oh No! Your rotten aim has attracted the attention of one of the patrol guards. You're in for it now!"
				(if (> (gEgo x:) 120)
					(= cycles 10)
				else
					(gEgo
			...
			(5
				(EgoDead 940 2 0 64 31) ; "Boy, you're no Marshall Dillon, that's for sure. And you were so close. You'll need to react a little faster be little sharper than that. Well, throw a little bondo into that whole and get back in there."
				(self dispose:)
			)
		)
	)
