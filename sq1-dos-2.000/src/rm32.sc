		...
		(if (== global166 2)
			(self
				addObstacle:
					...
					((Polygon new:)
						type: PBarredAccess
						init: 319 173 282 177 253 153 227 153 195 154 163 151 146 140 121 133 74 126 48 125 43 132 30 136 12 134 6 123 0 121 0 0 319 0
						yourself:
					)
					((Polygon new:)
						type: PBarredAccess
...
(instance upperledge of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(if (== global166 1)
					(Print 32 27) ; "This is an upper pathway in a slightly smaller chamber of the underground complex. Near the middle section of the path, the acid drops have formed a pattern of little holes. No, it's not a dot-to-dot puzzle."
				else
					(Print 32 28) ; "This is an upper pathway which is inacessible from here."
				)
			)
			(else
				(super doVerb: theVerb &rest)
			...
			(2 ; Look
				(cond
					((& (gEgo onControl: 0) $4000)
						(if (== (door cel:) 0)
							(Print 32 29) ; "Your view is restricted by the closed door."
						else
							(Print 32 30) ; "You'll need to move further into the room to receive a better view."
						)
					)
					((== global166 2)
						(Print 32 31) ; "This is a slightly smaller chamber in the underground complex. There is a path above which is not accessible from here. There is a pool on the right side."
					)
					(else
						(Print 32 32) ; "This is the lower ledge for this room. You can't reach it from here -- try going back down."
					)
				)
			)
			(else
...
(instance door of Prop
	...
	(method (doVerb theVerb invItem)
		(switch theVerb
			(2 ; Look
				(Print 32 33) ; "It's an odd, closed door with no apparent mechanism for opening it."
			)
			(3 ; Do
				(Print 32 34) ; "You are unable to open the door by hand."
			)
			(5 ; Talk
				(Print 32 35) ; "No amount of screaming will open this door."
			)
			(12 ; Smell
				(Print 32 36) ; "Smells pretty much like a strange door which has stood next to a sulfur-spewing geyser for a long period of time."
			)
			(11 ; Taste
				(Print 32 37) ; "It tastes like rotten Krebble eggs!."
			)
			(4 ; Inventory
				(Print 32 38) ; "That doesn't seem to have any effect on the door."
			)
			(else
				(super doVerb: theVerb invItem &rest)
			)
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((& (gEgo onControl:) $2000)
						(Print 32 39) ; "You gaze intently at the greenish pool of liquid, the first real sign of moisture on the planet. The pool seems to have no bottom. The gentle dripping has a soothing effect on your frazzled nerves. A small plume of mist rises as each drop hits the pool's surface."
					)
					((== global166 1)
						(Print 32 40) ; "You can't see anything from up here."
					)
					(else
						(proc0_15)
					)
			...
			(5 ; Talk
				(cond
					...
					((& (gEgo onControl:) $2000)
						(Print 32 41) ; "Drip, drip, drip. It must be talking to you."
					)
					((== global166 1)
						(Print {You can't do that from here.})
					)
			...
			(4 ; Inventory
				(cond
					...
					((& (gEgo onControl:) $2000)
						(Print 32 42) ; "Nah, not here. Try using it somewhere else."
					)
					((== global166 1)
						(Print {You can't do that from here.})
					)
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((& (gEgo onControl:) $2000)
						(Print 32 43) ; "From tiny cracks in the ceiling several small drops of liquid succumb to the influence of gravity."
					)
					((== global166 1)
						(Print {You can't do that from here.})
					)
		...
		(switch theVerb
			(2 ; Look
				(cond
					...
					((& (gEgo onControl:) $2000)
						(Print 32 44) ; "A light-green mist floats gently upward after each drop of liquid hits the surface of the pool."
					)
					((== global166 1)
						(Print {You can't do that from here.})
					)
...
(class AcidDrop of Actor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(2 ; Look
				(Print 32 45) ; "From tiny cracks in the ceiling several small green drops fall in a gently rhythmic pattern that stimulates your alpha levels. They make a heckuva great rust remover, too."
			)
			(3 ; Do
				(Print 32 46) ; "There's nothing you can do to these from here."
			)
			(12 ; Smell
				(Print 32 47) ; "They're hard to catch, but you do get a slight burning sensation."
			)
			(11 ; Taste
				(Print 32 48) ; "They're hard to catch. You might need to get right under them."
			)
			(5 ; Talk
				(Print 32 49) ; "They're too busy falling right now."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance egoHitByAcidDrop of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 32 50 #at 50 100) ; "You are unpleasantly surprised by a drop of searing acid which bores its way to your feet."
				(= cycles 3)
			)
			(3
				(EgoDead 938 0 0 32 51) ; "Now, that's some seriously deep pain."
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(switch register
					(1
						(Print 32 52 #at 50 10) ; "You lean over to drink from the tempting pool of liquid. As your lips touch the fluid, you feel a pain which could be likened to kissing a lit rocket nozzle. Now you know what they mean when they say 'Don't drink the water.'"
					)
					(2
						(Print 32 53 #at 50 10) ; "You lean over the pool to get a good solid whiff, and WHOOA! Talk about clean sinuses!"
					)
				)
				(= cycles 9)
			)
			...
			(7
				(EgoDead 938 0 0 32 54) ; "That's right. You have no head. That darn pool must be filled with acid. You obviously can't go on living that way."
			)
		)
	)
)
