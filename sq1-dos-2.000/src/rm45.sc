...
(instance tiny of Actor
	...
	(method (cue)
		(if (IsFlag 24)
			(Printf 45 5 local3) ; "Of course, with your 70 buckazoid rebate, that's just %u to you."
		)
		(super cue: &rest)
	)
					...
					(local3
						(tinyTalk
							init:
								...
								tinyEyes
								tinyLips
								(Format @temp0 45 6 [local0 local2]) ; "I'll tell ya, ol' buddy, %u buckazoids is as low as I can go on that baby."
								0
								0
								1
						)
					...
					(10 ; buckazoid
						(if local3
							(cond
								(
									(or
										(and (== local2 1) (IsFlag 26))
										(and (== local2 2) (IsFlag 27))
									)
									(tinyTalk
										init:
											tinyHead
