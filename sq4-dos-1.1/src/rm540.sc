...
(instance rm540 of SQRoom
	...
	(method (init)
		...
		(switch gPrevRoomNum
			...
			(west
				(self style: 41 setScript: enterFromWest 0 (IsFlag 32))
			)
			(else
				(gEgo x: 160 y: 160)
			)
		...
		(switch (= state newState)
			...
			(2
				(if (IsFlag 7)
					...
					(Print 540 4) ; "Try as you might, none of the codes you punch in seem to have any effect on the door."
					(HandsOn)
					(self dispose:)
				)
			)
...
(instance acidScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(Print 540 5) ; "Well, I don't think there's much left of the lock now."
				(SetScore 7 10)
