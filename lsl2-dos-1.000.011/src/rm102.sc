...
(instance rm102 of Rm
	...
	(method (init)
		...
		((= aDoorWest (Prop new:)) view: 202 posn: 83 116 setPri: 4 init:)
		((= aDoorEast (Prop new:))
			view: 202
			setLoop: 1
			posn: 250 117
		...
		(if (== gPrevRoomNum 103)
			...
			(gEgo posn: 67 123 observeControl: 8192 ignoreActors:)
		...
		(if (Said 'look>')
			...
			(if (Said '/door')
				(if doorIsOpen
					...
					(Print 102 4 #at -1 152) ; "(Why aren't you walking through it?)"
				else
					(Print 102 5) ; "There are doors to the east, west and south, which are all locked. Tight. And, there's no key."
					(Print 102 6 #at -1 152) ; "(Looks like you're trapped in here until someone comes to your rescue, Larry!)"
				)
			)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(26
				...
				(aProducer dispose:)
				(Print 102 18 #at -1 152) ; "(Oh no, Larry. Here we go again!)"
			)
		)
	)
)
