...
(instance rm40 of Rm
	...
	(method (init)
		...
		(NormalEgo 0)
		(gEgo ignoreActors: illegalBits: 0 posn: 44 189 init:)
		(User canControl: 0)
		(= gCurrentStatus 5)
...
(instance rm40Script of Script
	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			(if (Said '/carpet')
				(Print 40 0) ; "The grounds of the resort are covered with elaborate, hand-carved and inlaid patterns, none of which show up on the cheap graphics board and monitor that you're using!"
			)
		...
		(if (Said 'get,cut,get/flower')
			(cond
				...
				(else
					...
					(Print 40 12 #at -1 20 #draw) ; "You reach over and break off the beautiful flower at its base. "Aw," you rationalize, "they'll never miss just one flower!""
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(gEgo posn: 16 171 setMotion: MoveTo 91 171 self)
			)
			(1
				(if (== gTimesInRm40 257)
					(= state 62)
			...
			(7
				(gEgo posn: 249 160 setMotion: MoveTo 157 160 self)
			)
			(8
				(= cycles 15)
			)
			...
			(21
				(gEgo posn: 119 143 setMotion: MoveTo 2 143 self)
			)
			(22
				(= cycles 30)
				(if (or (!= lclEgoView 145) (> gTimesInRm40 7))
					(= state 62)
				)
			)
			(23
			...
			(63
				...
				(if (!= lclEgoView 145)
					(Print 40 13) ; "Say, you're getting faster at this!"
				else
					(Print 40 14) ; "At last you find an exit."
					(Print 40 15 #at -1 152) ; "(But where does it lead?)"
					(switch gRmAfter40
						(42
							(Print 40 16) ; "Why, here you are back at the beach!"
						)
						(43
							(Print 40 17) ; "Why, you've found a lovely restaurant!"
						)
						(44
							(Print 40 18) ; "Why, you've found an empty guest room!"
						)
						(45
							(Print 40 19) ; "Why, you've found the resort's barber shop!"
						)
					)
				)
