...
(instance rm13Script of Script
	...
	(method (handleEvent event)
		...
		(if (Said '(look<in),open,increase/lid,barrel,barrel')
			(if (not (gEgo inRect: 230 115 284 137))
				...
				(Print 13 1 #at -1 152) ; "(What did you expect? A passport?)"
			)
		)
		...
		(if (Said 'look/fence')
			...
			(Print 13 4 #at -1 152) ; "(Unless you find that knothole interesting.)"
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(aHead posn: 198 74 setCycle: End self)
			)
			(3
				(aHead setCel: 0 posn: 198 1074)
				(= state 0)
...
(instance pissScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(aPuddle posn: 243 118 setCycle: End self)
			)
			(1
				(NormalEgo)
				(aPuddle dispose:)
