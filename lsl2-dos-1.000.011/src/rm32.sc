...
(instance rm32Script of Script
	...
	(method (handleEvent event)
			...
			(cond
				...
				((not (gEgo has: 5)) ; Swimsuit
					(if (gEgo has: 15) ; Bikini_Top
						(Print 32 18) ; "Nah. Who'd wear half a swimsuit?"
					else
				...
				((not gTimesInRm33)
					(Print 32 19) ; "Don't you want to see what's behind that door to the right?"
				)
				(else
					(= gCurrentEgoView 132)
					(Print 32 20) ; "Ok. This IS a good place to change clothes."
					(gEgo view: 132)
				)
			)
		)
		(if (Said 'open/bureau,new,new')
			(Print 32 21) ; "You open the nightstand's sole drawer and inspect it thoroughly."
			(Print 32 22) ; "Nothing."
			(Print 32 23) ; "Zip."
			(Print 32 24) ; "Nada."
			(Print 32 25) ; "You close the drawer."
		)
		(if
			(and
			...
			(Print 32 26) ; "Ok. You feel refreshed and fruitless."
		...
		(if (Said 'lie,board,bath[/bed,nap,barstool]')
			(cond
				...
				(else
					(= gCurrentStatus 1005)
					(Print 32 27) ; "Ok. Thinking a little shut-eye might do you some good, you collapse on the bed, and immediately begin snoring."
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Print 32 29) ; "Night has fallen on the good ship U.S.S. Love Tub, so you head for your bed and a few hours of relaxing sleep."
			...
			(6
				(if (!= (aDoor doorState:) 0)
					...
					(Print 32 30) ; "Unbeknownst to our "little boy blue," the door to the adjoining cabin swings open."
					(aDoor locked: 0 notify: self force: 1 open:)
				)
			)
			(7
				(Print 32 31) ; "Suddenly a deep rumbling shakes your cabin, not the steady roar of the ship's engines (to which you've now grown accustomed), this sounds more like an onrushing steam locomotive encountering a steep gradient while the fireman pours on the coal!"
				(= seconds 2)
			)
			(8
			...
			(9
				(aMama setLoop: 1 setCel: 0 posn: 177 111 setPri: 13)
				(= cycles 2)
			)
			(10
				(aMama posn: 171 109 setCel: 1)
				(= cycles 2)
			)
			(11
				(aMama posn: 165 107 setCel: 2)
				(= cycles 2)
			)
			(12
				(aMama posn: 162 105 setCel: 3)
				(= cycles 2)
			)
			(13
				(Print 32 32 #at -1 20) ; "IT'S MAMA!!!!"
				(aMama setLoop: 2 posn: 162 105 setCel: 0)
				(= cycles 2)
			)
			(14
				(aMama posn: 162 105 setCel: 1)
				(= cycles 2)
			)
			(15
				(aMama posn: 162 105 setCel: 2)
				(= cycles 2)
			)
			(16
				(aMama posn: 160 103 setCel: 3)
				(= cycles 2)
			)
			(17
				(aMama posn: 158 99 setCel: 4)
				(= cycles 2)
			)
			(18
				(aMama posn: 154 93 setCel: 5)
				(= cycles 2)
			)
			(19
				(aMama posn: 158 90 setCel: 5)
				(= cycles 2)
			)
			(20
				(theSound play:)
				(aMama setLoop: 3 setMotion: 0 setCycle: Fwd posn: 129 107)
				(gEgo setLoop: (if (== gCurrentEgoView 100) 1 else 3) posn: 125 94)
				(= seconds 4)
			)
			(21
				(aMama setLoop: 0 setCel: 0 posn: 155 104)
				(Print 32 33 #at -1 15 #width 280 #draw) ; "Oops," says the big Mother, "I didn't mean ta hurt 'cha, lil' fella!"
				(Print 32 34 #at -1 15 #width 280) ; ""Shooooot, an' he was such a cute 'n, too!" she moans."
				(= seconds 1)
			)
			(22
				(= gCurrentStatus 1001)
				(if (== causeOfDeath 2)
					(Print 32 35) ; "Now it becomes clear to you. You must escape from this vessel before darkness falls or you'll continue to be the main course at a Mama party!"
				else
					(Print 32 36 #at -1 15 #width 280) ; "Too bad, Larry, but it seems your bed just isn't the place to be, if you want to be alone!"
				)
			)
		)
	)
