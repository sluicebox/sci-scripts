	...
	(method (handleEvent event &tmp msg i [string2 100])
		(switch (= msg (super handleEvent: event))
			(257
				...
				(Print
					(Format @gString 997 1 gVersion) ; "Sierra's Creative Interpreter developed by Jeff Stephenson, Bob Heitman, Pablo Ghenis, and Stuart Goldstein Graphics Systems by Bob Heitman Music composed and performed by Al Lowe Version %s, October 27, 1988"
					#font
					gSmallFont
					#mode
					1
			...
			(770
				(if (not (HaveMem 1024))
					(Print 997 7) ; "Inventory cannot be displayed at this time."
				else
			...
			(1025
				(if (not (HaveMem 2048))
					(Print 997 11) ; "Speed cannot be changed at this time."
				else
					(= i
						((Gauge new:)
							description:
			...
			(1281
				(if (not (HaveMem 2048))
					(Print 997 12) ; "Volume cannot be changed at this time."
				else
					(= i
						((Gauge new:)
							description:
			...
			(1282
				...
				(if i
					...
					(Print 997 13) ; "Sound is off."
				else
					(SetMenu 1282 110 {Turn Off})
					(Print 997 14) ; "Sound is on."
				)
			)
			(else
				(if gDebugMenu
