...
(instance rm33 of Rm
	...
	(method (init)
		...
		(aBigMama view: 313 posn: 273 1059 setPri: 15 setCycle: Fwd init:)
		((= aPorthole (Prop new:))
			view: 305
			loop: 1
			setPri: 4
	...
	(method (handleEvent event)
		...
		(if (Said 'look>')
			...
			(if (Said '/bed')
				...
				(if (> gFilthLevel 10)
					(Print 33 13 #at -1 152) ; "(A condition not unknown to you!)"
				)
			)
			(if (Said '/woman')
				(cond
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(Print 33 42 #at -1 152) ; "(She fails.)"
				(HandsOn)
			)
			(6
				(= seconds 0)
			...
			(13
				...
				(Print 33 51 #at -1 152) ; "(You can see why this broad's a widow!)"
				(= gCurrentStatus 1001)
			)
			(14
				(= mamaInRoom 2)
