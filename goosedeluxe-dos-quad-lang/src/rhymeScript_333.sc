		...
		(switch (= state newState)
			...
			(2
				(cond
					...
					((== gLanguage 1)
						(proc0_16 (temp0 format: {Jack s$ #gil,}))
					)
					((== gLanguage 2)
						(proc0_16
							(temp0
								format:
									{Au bord de la rivi1re\nMalon tralala Malon tralal1re}
							)
						)
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {Im Sommer gehn' wir baden,}))
					)
					(else
			...
			(3
				(cond
					...
					((== gLanguage 1)
						(proc0_16 (temp0 format: {Jack s$ r#pido,}))
					)
					((== gLanguage 2)
						(proc0_16
							(temp0
								format:
									{au bord de la rivi1re\npetit Jean va sauter}
							)
						)
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {juchheissa, das is fein!}))
					)
					(else
			...
			(4
				(if (< (rhymeSync prevSignal:) 30)
					...
					(cond
						...
						((== gLanguage 2)
							(proc0_16 (temp0 format: {petit Jean va sauter}))
						)
						((== gLanguage 3)
							(proc0_16
								(temp0
									format: {Die Sonne macht das Wasser warm,}
								)
							)
						)
						(else
							(proc0_16
								(temp0
									format: {Jack jumped over the candlestick.}
								)
							)
						)
					)
			...
			(5
				(cond
					...
					((== gLanguage 2)
						(proc0_16
							(temp0
								format: {saute, saute\npar-dessus la chandelle}
							)
						)
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {da springen wir hinein.}))
					)
					(else
						(proc0_16 (temp0 format: {Jack be nimble,}))
					)
			...
			(7
				(cond
					...
					((== gLanguage 2)
						(proc0_16 (temp0 format: {sans te br]ler les ailes.}))
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {da springen wir hinein.}))
					)
