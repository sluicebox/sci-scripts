		...
		(switch (= state newState)
					...
					(cond
						((== gLanguage 1)
							(proc0_16
								(temp0
									format:
										{Monta un bravo caballo por Banbury Cross,}
								)
							)
						)
						((== gLanguage 2)
							(proc0_16
								(temp0 format: {Nous n'irons plus au bois})
							)
						)
						((== gLanguage 3)
							(proc0_16
								(temp0 format: {Wenn alle Br9nnlein flie~en,})
							)
						)
						(else
							(proc0_16
								(temp0
									format: {Ride a cockhorse to Banbury Cross,}
								)
							)
						)
					)
				...
				(cond
					...
					((== gLanguage 1)
						(proc0_16
							(temp0
								format:
									{Veo una bella dama encima de un caballo blanco,}
							)
						)
					)
					((== gLanguage 2)
						(proc0_16 (temp0 format: {les lauriers sont coup$s,}))
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {so mu~ man trinken.}))
					)
					(else
						(proc0_16
							(temp0
				...
				(cond
					...
					((== gLanguage 1)
						(proc0_16
							(temp0
								format:
									{Lleva anillos en los dedos de las manos,}
							)
						)
					)
					((== gLanguage 2)
						(proc0_16
							(temp0
								format: {la belle que voil0, ira les ramasser.}
							)
						)
					)
					((== gLanguage 3)
						(proc0_16
							(temp0
								format: {Wenn ich mein Schatz nicht rufen darf,}
							)
						)
					)
					(else
			...
			(8
				(if (< (rhymeSync prevSignal:) 40)
					...
					(cond
						...
						((== gLanguage 3)
							(proc0_16 (temp0 format: {ju ja, rufen darf,}))
						)
						(else
			...
			(9
				(cond
					...
					((== gLanguage 3)
						(proc0_16 (temp0 format: {tu' ich ihm winken.}))
					)
					(else
						(proc0_16
							(temp0
