		...
		(switch (= state newState)
				...
				(cond
					...
					((== gLanguage 1)
						(proc0_16
							(temp0
								format:
									{La peque+a Miss Muffet se sent& en un banquito,}
							)
						)
					)
					((== gLanguage 2)
						(proc0_16 (temp0 format: {C'$tait Mam'selle Furet}))
					)
					((== gLanguage 3)
						(proc0_16 (temp0 format: {Das Fr5ulein Muffet}))
					)
					(else
						(proc0_16
							(temp0
			...
			(6
				(cond
					...
					((== gLanguage 1)
						(proc0_16
							(temp0 format: {comiendo su reques&n y su leche.})
						)
					)
					((== gLanguage 2)
						(proc0_16 (temp0 format: {sous un noyer,}))
					)
					((== gLanguage 3)
						(proc0_16
							(temp0
								format:
									{sa~ in dem Sessel,\nSie verlor die Sinne,}
							)
						)
					)
					(else
			...
			(8
				(if (< (rhymeSync prevSignal:) 30)
					...
					(cond
						...
						((== gLanguage 2)
							(proc0_16
								(temp0 format: {qui prenait son petit-d$jeuner})
							)
						)
						((== gLanguage 3)
							(proc0_16 (temp0 format: {als sie sah die Spinne,}))
						)
						(else
			...
			(9
				(if (< (rhymeSync prevSignal:) 40)
					...
					(cond
						((== gLanguage 1)
							(proc0_16 (temp0 format: {que se sent& a su lado}))
						)
						((== gLanguage 2)
							(proc0_16
								(temp0
									format:
										{quand une grosse araign$e vint 0 passer}
								)
							)
						)
						((== gLanguage 3)
							(proc0_16
								(temp0
									format:
										{dann wollt' sie nicht mehr essen,\nund hat sich hingesessen.}
								)
							)
						)
						(else
							(proc0_16
								(temp0 format: {And sat down beside her,})
							)
						)
					)
			...
			(10
				(if (< (rhymeSync prevSignal:) 50)
					...
					(cond
						((== gLanguage 1)
							(proc0_16 (temp0 format: {y asust& a Miss Muffet.}))
						)
						((== gLanguage 2)
							(proc0_16
								(temp0
									format: {et lui fit peur 0 l'en chasser.}
								)
							)
						)
						((== gLanguage 3)
						...
						(else
							(proc0_16
								(temp0
									format: {And frightened Miss Muffet away.}
								)
							)
						)
					)
