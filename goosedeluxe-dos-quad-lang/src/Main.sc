...
(instance icon0 of MGIconItem
	...
	(method (init)
		(if (or (== gLanguage 2) (== gLanguage 1))
			(self view: (+ 801 (* 1000 gLanguage)))
			(= mainView view)
		)
		(self setPri: 255)
		(super init: &rest)
...
(instance MG of Game
	...
	(method (startRoom roomNum &tmp [temp0 5])
		...
		(if
			...
			(if
				(and
					(== [global200 gCurRoomNum] 58)
					(or (== gLanguage 2) (== gLanguage 3))
				)
				(global150 view: (+ 58 (* 100 gLanguage)))
			)
		else
			(= global150 0)
		)
		...
		(= temp0
			(switch gLanguage
				(0
					...
					(= temp2 (Str format: {%sE%d.DTA} gCurSaveDir gMacSaveNumber))
				)
				(2
					(Save
						0 ; SaveGame
					...
					(= temp2 (Str format: {%sF%d.DTA} gCurSaveDir gMacSaveNumber))
				)
				(3
					(Save
						0 ; SaveGame
					...
					(= temp2 (Str format: {%sG%d.DTA} gCurSaveDir gMacSaveNumber))
				)
				(1
					(Save
						0 ; SaveGame
					...
					(= temp2 (Str format: {%sS%d.DTA} gCurSaveDir gMacSaveNumber))
				)
			)
		)
		(= temp4 (Str format: {\0d\n%d} (gEgo view:)))
	...
	(method (restore param1)
		...
		(switch gLanguage
			(1
				(if (Save 3 {MGS} param1 gVersion) ; CheckSaveGame
					(Save 1 {MGS} param1 gVersion) ; RestoreGame
				)
			)
			(2
				(if (Save 3 {MGF} param1 gVersion) ; CheckSaveGame
					(Save 1 {MGF} param1 gVersion) ; RestoreGame
				)
			)
			(3
				(if (Save 3 {MGG} param1 gVersion) ; CheckSaveGame
					(Save 1 {MGG} param1 gVersion) ; RestoreGame
				)
			)
			(0
				(if (Save 3 {MGE} param1 gVersion) ; CheckSaveGame
					(Save 1 {MGE} param1 gVersion) ; RestoreGame
				)
			)
		)
	)
