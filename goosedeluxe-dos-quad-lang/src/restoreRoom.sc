...
(instance restoreRoom of Room
	...
	(method (init &tmp temp0)
		...
		(= local0
			(switch gLanguage
				(1
					(Save 5 {MGS} (temp0 data:) (local1 data:)) ; GetSaveFiles
				)
				(2
					(Save 5 {MGF} (temp0 data:) (local1 data:)) ; GetSaveFiles
				)
				(3
					(Save 5 {MGG} (temp0 data:) (local1 data:)) ; GetSaveFiles
				)
				(0
					(Save 5 {MGE} (temp0 data:) (local1 data:)) ; GetSaveFiles
				)
			)
		)
		(temp0 dispose:)
...
(class OldKid of Feature
	...
	(method (init &tmp temp0 temp1 temp2 temp3 temp4)
		(= temp0
			(switch gLanguage
				(1
					(Str format: {%sS%d.DTA} gCurSaveDir fileNum)
				)
				(2
					(Str format: {%sF%d.DTA} gCurSaveDir fileNum)
				)
				(3
					(Str format: {%sG%d.DTA} gCurSaveDir fileNum)
				)
				(0
					(Str format: {%sE%d.DTA} gCurSaveDir fileNum)
				)
			)
		)
		(= temp2 (File new:))
