...
(class Game of Obj
	...
	(method (save &tmp [temp0 20] temp20 temp21 [temp22 100] [temp122 5] [temp127 100] [temp227 100])
		(if (not (ValidPath gCurSaveDir))
			(Message msgGET 994 9 0 0 1 @temp22) ; "%s is not a valid directory."
			(StrCpy @temp227 gCurSaveDir)
			(Format @temp127 @temp22 @temp227)
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= temp20 (Save doit: @temp0)) -1)
				...
				(if (not (SaveGame name temp20 @temp0 gVersion))
					(Message msgGET 994 1 0 0 1 @temp22) ; "Your save game disk is full. You must either use another disk or save over an existing saved game."
					(Message msgGET 994 2 0 0 1 @temp122) ; "OK"
					(Print
						font: 0
						addText: @temp22
	...
	(method (restore &tmp [temp0 20] temp20 temp21 [temp22 100] [temp122 5] [temp127 100] [temp227 100])
		(if (not (ValidPath gCurSaveDir))
			(Message msgGET 994 9 0 0 1 @temp22) ; "%s is not a valid directory."
			(StrCpy @temp227 gCurSaveDir)
			(Format @temp127 @temp22 @temp227)
		...
		(if (PromptForDiskChange 1)
			...
			(if (!= (= temp20 (Restore doit: &rest)) -1)
				...
				(if (CheckSaveGame name temp20 gVersion)
					...
					(Message msgGET 994 3 0 0 1 @temp22) ; "That game was saved under a different interpreter. It cannot be restored."
					(Message msgGET 994 2 0 0 1 @temp122) ; "OK"
					(Print
						font: 0
						addText: @temp22
