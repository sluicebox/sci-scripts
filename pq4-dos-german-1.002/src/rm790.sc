...
(instance rm790 of Room
	...
	(method (init)
		...
		(LoadMany rsVIEW 7950 9057)
		(gGlobalSound0 number: 790 loop: -1 play:)
		(if (== gDay 2)
		...
		(gEgo
			...
			posn: 87 122 0
			setPri: 5
			setScaler: Scaler 113 91 130 116
			init:
		...
		(switch (= state newState)
			...
			(1
				(if register
					...
					(gMessager say: 0 0 26 1) ; "Leaving now would be rude."
				)
				(gGame handsOn:)
				(self dispose:)
			)
	...
	(method (doVerb theVerb)
		(if (== theVerb 14) ; hickmanStuff
			(if (or (not (gCast contains: kath)) (== (kath script:) waitScr))
				(gMessager say: noun theVerb 19 0) ; "It's not a good idea to leave this without first"
			else
...
(instance sitDownScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(gEgo view: 9057 setLoop: 1 1 cel: 0 setCycle: End self)
			)
			(3
...
(instance clothes4 of Feature
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 39) ; pills
				(gMessager say: noun theVerb 9 0) ; "Use your detective skills, keep these."
			)
			(else
				(super doVerb: theVerb &rest)
			)
...
(instance giveEnvScr of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if register
					(gEgo heading: 235 normalize: 0 5)
				)
				(= cycles 2)
			)
			(2
			...
			(4
				(gEgo view: 9031 setLoop: 1 cel: 0 setCycle: CT 5 1 self)
			)
			(5
			...
			(7
				(gEgo normalize: 0 put: 4 790) ; hickmanStuff
				(gGame handsOn:)
				(self dispose:)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(gEgo put: 4 790 get: 5 setCycle: End self) ; hickmanStuff, vest
			)
			(7
				(gEgo normalize: 0)
				(= cycles 2)
			...
			(9
				(gMessager say: 3 0 16) ; "John, this was Bob's Kevlar vest. I'd like you to have it... maybe it will keep you safe."
				(= cycles 2)
			)
			(10
				(kath
