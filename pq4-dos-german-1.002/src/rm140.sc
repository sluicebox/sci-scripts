...
(instance rm140 of Room
	...
	(method (init)
		...
		(if (and (> gDay 2) (!= gPrevRoomNum 130) (!= gPrevRoomNum 150))
			(gGlobalSound1 number: 917 setLoop: 1 play:)
		)
		(if (< gDay 3)
			(gCurRoom
				addObstacle:
...
(instance exitDoorScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(gMessager say: 16 4 0 0 self) ; "You pull the heavy door open."
			)
			(4
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(52 ; notebook
				(cond
					(local4
						(gMessager say: 2 52 4) ; "Recording information twice is unnecessary."
					)
					((not (gPqPointFlags test: 17))
						(gMessager say: noun theVerb 6) ; "What is it you wish to record?"
					)
					(else
		...
		(switch theVerb
			...
			(52 ; notebook
				(cond
					...
					((not local0)
						(gMessager say: noun theVerb 6) ; "What would you like to notate?"
					)
					(else
...
(instance car of View
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4 ; Do
				(gMessager say: 12 4 0 0) ; "Leaving?"
...
(instance trunk of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(gMessager say: 17 4 0 0) ; "The trunk to your car is locked."
			)
			(else
				(super doVerb: theVerb)
			)
...
(instance rainbow of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (== gDay 2)
					(gMessager say: 4 1 1 0) ; "The colorful building adds a bit of charm to this older neighborhood."
				else
					(gMessager say: 4 1 3 0) ; "The cafe is closed, the sting operation foiled."
				)
			)
			(29 ; coins
				(if (== gDay 2)
					(gMessager say: 4 29 1 0) ; "Buying your way in is tacky."
				else
					(gMessager say: 4 29 3 0) ; "Don't throw money away! You work too hard for it."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance cafeDoor of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1 ; Look
				(if (== gDay 2)
					(gMessager say: 5 1 1 0) ; "Doorway to enchilada heaven."
				else
					(gMessager say: 5 1 3 0) ; "The door has been locked and sealed by CRASH."
				)
			)
			(4 ; Do
				(if (== gDay 2)
					...
					(gMessager say: 5 4 3 0) ; "The door is locked, the operation closed down."
				)
			)
			(22 ; handgun
				(if (== gDay 2)
					(gMessager say: 5 22 1 0) ; "Shooting your way in is no answer. Holster your gun!"
				else
					(gMessager say: 5 22 3 0) ; "There's no cause or reason to shoot up the door! Holster your weapon."
				)
			)
			(46 ; keys
				(if (== gDay 2)
					(gMessager say: 5 46 1 0) ; "It's unlocked and open for business, unfortunately,"
				else
					(gMessager say: 5 46 3 0) ; "It's locked up tight, your keys do no good."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance field of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			...
			(4 ; Do
				(if (== gDay 2)
					(gMessager say: 6 4 1 0) ; "You have better things to do than go on a scavenger hunt."
				else
					(gMessager say: 6 4 3 0) ; "SID has swept the area clean, all that remains are memories."
				)
			)
			(else
				(super doVerb: theVerb)
...
(instance burnedBldg of Feature
	...
	(method (doVerb theVerb)
		(switch theVerb
			(13 ; caseBaggies
				(if (== gDay 2)
					(gMessager say: 2 13 1 0) ; "Nothing to collect here."
				else
					(gMessager say: 2 13 3 0) ; "SID has swept the area clean, nothing to collect here."
				)
			)
			(4 ; Do
				(if (== gDay 2)
			...
			(52 ; notebook
				(cond
					((== gDay 2)
						(gMessager say: 2 52 1 0) ; "There is nothing noteworthy about this building at this time."
					)
					((not (gPqPointFlags test: 26))
						(gPqPointFlags set: 26)
						(gMessager say: 2 52 3 0) ; "Notating details of the crime scene is effective"
					)
					(else
						(gMessager say: 2 52 4 0) ; "Recording information twice is unnecessary."
					)
				)
			)
			(else
