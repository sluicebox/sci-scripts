...
(instance tellJoke of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(= local2 (gMessager sayRandom: 7 2 2 self))
			)
			(2
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(Load rsVIEW 274)
				(= local2 0)
				(assistant view: 274 loop: 0 cel: 0 setCycle: End self)
			)
			(3
				(assistant cycleSpeed: 2 loop: 1 cel: 0 setCycle: Osc 1 self)
			)
			(4
	...
	(method (doVerb theVerb)
		(cond
			...
			((== theVerb 22) ; handgun
				(if (not local5)
					(++ local5)
					(gMessager say: 7 22 0 1) ; "Use of force is unnecessary at this time, Carey!"
				else
					(gMessager say: 7 22 0 2) ; "The consequences are deadly, Carey. Put the gun away!"
				)
			)
			(else
