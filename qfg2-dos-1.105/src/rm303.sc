	...
	(method (doit)
		...
		(cond
			...
			((and (< (gEgo y:) 115) (not outOfStuff) (not (gEgo script:)))
				(HighPrint 303 0) ; "You have lured the Fire Elemental away from the Plaza. Now you must try to weaken and capture it."
				(NormalEgo)
	...
	(method (handleEvent event &tmp spellNum)
		(cond
			...
			(
				...
				(cond
					((OneOf (gEgo view:) 18 21)
						(HighPrint 303 1) ; "You're already laying it out as fast as you can!"
					)
					((gEgo has: 13) ; Incense
						(HighPrint 303 2) ; "There's no need to drop more incense -- the Fire Elemental is well inside the alley now."
					)
					(else
						(HighPrint 303 3) ; "Unless you feel like scraping some off the alley floor, you'd better get used to the fact that you are incenseless."
					)
				)
			)
			((not outOfStuff)
				(event claimed: 1)
				(HighPrint 303 4) ; "You should lead the Elemental farther into the alley."
			)
			((Said '/lamp>')
				(cond
					...
					((Said 'put<down')
						(HighPrint 303 5) ; "Ok, "You worthless, second-hand, unpolishable, thriftstore-hatched piece of unwanted brass! Your kind makes me sick! (and various grumblings)." How's that?"
						(gEgo setScript: doLamp 0 1)
					)
					((Said 'drop')
						(HighPrint 303 6) ; "You drop the lamp. It is very hurt and angry.  "I have a new flame now," you say to the lamp, "and she's the light of my life. You couldn't hold a candle to her." But inside your heart burns with the loss of your bright young friend, the lamp."
						(gEgo setScript: doLamp 0 1)
					)
					((Said 'use,set')
			...
			(
				...
				(cond
					((<= gDrinksLeft 0)
						(HighPrint 303 7) ; "You have no water."
					)
					((> (fireElemental y:) 180)
						(HighPrint 303 8) ; "You should let the elemental get farther into the alley first."
					)
					((> (gEgo distanceTo: fireElemental) 50)
						(HighPrint 303 9) ; "The Elemental is too far away to douse."
					)
					(else
						(= gDrinksLeft 0)
	...
	(method (changeState newState &tmp theX theY)
		(switch (= state newState)
			...
			(1
				...
				(if (< (gEgo y:) theY)
					(HighPrint 303 10) ; "You barely manage to escape the Fire Elemental's searing heat."
					(gEgo hide:)
					(gCurRoom newRoom: 704) ; alleyRoom
				else
					(gEgo
			...
			(2
				(HighPrint (Format @string 303 11 [owTable (Random 1 8)])) ; "%s"
			...
			(4
				(if (== register 1)
					...
					(if (== gElementalState 2)
						(HighPrint 303 13) ; "As you pick up the lamp and rub it, a small flame appears. When you rub it again, the flame goes away. You now have a magic lamp. You put the lamp safely away in your backpack."
					else
						(HighPrint 303 14) ; "You put the lamp away."
					)
					(self cue:)
				)
			)
...
(instance lamp of Prop
		...
		(cond
			...
			((Said '(lockpick<up)/lamp')
				(if lampDown
					...
					(HighPrint 303 15) ; "You don't see a lamp here."
				)
			)
		)
	)
	...
	(method (doVerb theVerb)
		(cond
			((== theVerb 1)
				(if (!= gElementalState 1)
					(HighPrint 303 16) ; "You can see the Fire Elemental burning brightly inside the brass lamp."
				else
					(HighPrint 303 17) ; "The lamp is made of brass."
				)
			)
			((== theVerb 7)
				(if lampDown
					...
					(HighPrint 303 18) ; "You don't see one here."
				)
			)
		)
	)
...
(instance fleeTimer of Timer
	...
	(method (cue)
		(if lampDown
			...
			(HighPrint 303 19) ; "Without any means of containment, the Fire Elemental makes a hasty retreat back into the plaza."
			(fireElemental
				cycleSpeed: 0
				yStep: 5
