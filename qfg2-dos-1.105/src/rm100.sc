...
(procedure (MornMsg whoCares &tmp mealNum)
	(cond
		((OneOf gDay 2 4 5 8 11)
			(Say shema whoCares (shema msgFile:) (+ 51 gDay))
		)
		...
		((and (== gElementalState 8) (not (gEgo has: 27))) ; SapphPin
			(Say shema whoCares (shema msgFile:) 66)
		)
		(else
			(= mealNum (ChooseFromCycle gDay 1 5))
			(Say shema whoCares (shema msgFile:) (+ 46 (- mealNum 1)))
		)
	)
)
			...
			(3
				(cond
					...
					(
						...
						(= whatOrdered (if (< gInnLastAte gNow) 0 else 1))
...
(instance shema of Talker
		...
		(if (< (shema x:) 240)
			(cond
				...
				((or (Said 'give/bouquet') (Said 'give//bouquet'))
					(GiveFlowers self 100 52)
				)
				((or (Said 'hello,greet') (Said '/hello,greet'))
					(Say self 100 53) ; "It is always a pleasure to see you, Great Hero."
				)
			)
		)
	)
	...
	(method (showText what)
		(if (== what 0)
			(HighPrint 100 54) ; "Shema looks shyly down and shakes her head."
		else
