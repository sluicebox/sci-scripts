...
(instance rm390 of RasPlaza
	...
	(method (init)
		...
		(cond
			...
			((and (== gDay 28) (> gTimeODay 4) (!= gPrevRoomNum 450))
				(rGuard6 init:)
			)
		)
	)
		...
		(cond
			...
			((Said 'free,use/(elemental[<water]),(water<magic)')
				(if (gEgo has: 43) ; MagicWater
					(HighPrint 390 11) ; "Something in the back of your mind tells you to hold on to the Water Elemental."
				else
					(DontHave)
				)
			)
	...
	(method (cue)
		(cond
			((< (self distanceTo: gEgo) 30)
				(Print 390 12 #title {Guard:} #dispose #time 3) ; "This is none of your business. Go away."
			)
			((> (self distanceTo: gEgo) 30)
				(NormalEgo)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'look,look/guard,guard,man')
				(Print 390 13) ; "The guards of Raseir look like they are itching for a fight."
			)
			((Said 'attack,kill,hit')
				(Print 390 14) ; "That would only bring more guards."
			)
		)
	)
)
	...
	(method (doit)
		(cond
			...
			((and (not warned) (< (gEgo x:) 20))
				...
				(NormalEgo)
				(= warned 1)
				(Print 390 15 #title {Guard:}) ; "You were told that you may not pass. Turn now or die!"
			)
			((and (< (self distanceTo: gEgo) 25) (not isNear3))
				(= isNear3 1)
				(gEgo setCycle: 0 setMotion: 0)
				(NormalEgo)
				(Print 390 16 #title {Guard:}) ; "No one may pass until Khaveen gives the order."
			)
...
(instance theChase of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 390 17 #title {Guard:} #time 4) ; "You there, Stop!"
				(if (== (gEgo view:) 6)
					(gEgo setScript: 0)
					(HandsOff)
				)
			...
			(3
				(Face client gEgo)
				(Print 390 18 #title {Guard:}) ; "It is forbidden to be out at night. You are in violation of the laws of Raseir."
				(= cycles 1)
			)
			(4
				(HandsOff)
...
(instance runningMan of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 390 19 #title {Guard:}) ; "Guards!! There is an intruder in Fountain One. To me!!"
				(= cycles 2)
			)
			(2
				(if (< (gEgo y:) 150)
