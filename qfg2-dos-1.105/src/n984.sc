...
(procedure (SortedAdd &tmp frontList2 outList2 backList2)
	...
	(gSortedFeatures add: frontList2)
	(Sort outList outList2 frontSortCode)
	(gSortedFeatures add: outList2)
	(gSortedFeatures add: gRegions)
	(gSortedFeatures add: gLocales)
	(Sort backList backList2 backSortCode)
	(gSortedFeatures add: backList2)
...
(instance frontSortCode of Code
	...
	(method (doit obj &tmp dist ang)
		...
		(if (< (= dist (Abs (CosDiv (/ ang 2) dist))) 0)
			(= dist 32767)
		)
		(return dist)
	)
