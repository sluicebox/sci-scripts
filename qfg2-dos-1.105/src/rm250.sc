...
(local
	[local0 16] = [61 0 142 12 152 22 153 38 188 57 250 74 250 8 303 0]
	[local16 8] = [173 130 127 146 140 174 208 155]
	local24
	local25
	local26
	local27
	...
	local129
	local130
	[local131 8] = [0 10 15 20 25 30 125 15]
	[local139 38] = [83 84 85 86 87 88 89 90 91 92 93 95 96 97 98 82 99 82 100 82 101 102 103 104 105 106 107 108 109 110 111 112 113 82 82 82 82 82]
	[local177 74] = [56 '//detect,(spell,magic<detect),(spell<magic<detect)' 57 '//trigger,(spell<trigger)' 55 '//force,bolt,(spell<force,bolt)' 58 '//levitate,fly,(spell<levitate,fly)' 59 '//dazzle,(spell<dazzle)' 70 '//invisibility' 115 '//flame,fire,dart,fireball,(spell<flame,fire,dart,fireball)' 116 '//fetch,(spell<fetch)' 117 '//open,open,(spell<open,open)' 118 '//zap,(spell<zap)' 119 '//reversal,reflection,mirror,(spell<reversal,reflection,mirror)' 73 '//rope' 77 '//glasses,disguise,disguise' 49 '//caws' 50 '//shop' 51 '//wonder' 52 '//owner' 53 '//yorick' 76 '//cost,cost<rope' 54 '//spell,cost,cost,merchandise' 60 '//bedouin' 61 '//hero' 63 '//hoodoo' 72 '//dirt,earth<fooler' 64 '//dirt,earth' 65 '//whirl,beard' 66 '//gnome' 67 '//erasmus' 68 '//scroll' 69 '//beast,saurii' 71 '//wizard' 78 '//carp,carp,dolphin,aquarium,oasis,sea' 81 '//air,wind' 114 '//iblis' 115 '//flame,fire,dart,fireball,(spell<flame,fire,dart,fireball)' 120 '//contain,capture,bellows' 0 0]
	[local251 11] = [{"Ghost Writers in the Sky."} {"To Air is Human."} {"Fear of Flying."} {"The Wind in the Bellows."} {"Gnome with the Wind."} {"The Pizza Elemental: Fact or Fiction?.'} {"Air Conditioning and Grooming.} {"H-I-J-K-Elemental-P."} {"We're Here to Pump You Up."} {"The Air Apparent."} {"Three's a Cloud."}]
	local262
)
...
(instance rm250 of Stage
	...
	(method (init)
		...
		(InitFeatures250)
		(cloudPoly points: @local0 size: 8)
		(sunPoly points: @local16 size: 4)
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'sell,give/whirl,beard')
				(if (gEgo use: 14 1) ; Beard
					(Say keapon 250 2) ; "You uncurled me some whirl from Dervish? Oh, frabjous day, hoorah, hooray! Here's the Dinars that I must pay."
					(gEgo get: 2 15) ; Dinar
				else
					(HighPrint 250 3) ; "You don't have any to sell."
				)
			)
			((Said 'grab')
				(HighPrint 250 4) ; "You don't dare while the shopkeepers watching. He might throw a pie at you."
			)
			((Said 'tell')
				(Say keapon 250 5) ; "You don't say? And I sure won't."
			)
			((Said 'buy>')
				(cond
					(
					...
					((Said '/glasses')
						(if (gEgo has: 54) ; Glasses
							(= local128 0)
							(Say keapon 250 6) ; "You have my last pair there, Claire, and I haven't got a spare."
						else
							(= local128 7)
						)
					)
					...
					(else
						(= local128 0)
						(event claimed: 1)
						(Say keapon 250 7 250 8) ; "What do I look like, a department store?"
					)
				)
				(if local128
			...
			((Said 'bargain>')
				(cond
					...
					((or (Said '/glasses') (Said '//glasses'))
						(if (gEgo has: 54) ; Glasses
							(= local128 0)
							(Say keapon 250 9) ; "Well, we can bargain all you like, Mike, but that won't change the fact that you already bought the only pair I had."
						else
							(= local128 7)
						)
					)
			...
			((or (Said 'look,look<up') (Said 'look,look/cloud,ceiling,sky'))
				(if (shadow onMe: gEgo)
					(HighPrint 250 10) ; "By golly, it is! There's a silver lining on the underside of Keapon's cloud."
				else
					(HighPrint 250 11) ; "The fact that the clouds on the ceiling don't move leads you to the conclusion that they were not painted on with an air brush."
				)
			)
			((or (Said 'look,look<down') (Said 'look,look/floor,sun'))
				(HighPrint 250 12) ; "Talk about topsy-turvy. The sun shines up from the floor onto the clouds above."
			)
			((Said 'thank')
				(SayThanks keapon 250 13)
			)
		)
	)
)
		...
		(switch (= state newState)
			...
			(2
				(if (and [gEgoStats 12] (== gMagicTrips 1)) ; magic use
					(Say keapon self 250 17 250 18 250 19 250 20) ; "Hello sailor, new in town? So how ya doin, Bruin?"
				else
					(self cue:)
				)
			)
...
(instance bumpNose of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(if (not local24)
					(HighPrint 250 21) ; "It sure LOOKED real!"
					(= local24 1)
				)
				(self dispose:)
			)
...
(instance sideDoor of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say keapon self 250 22 250 23) ; "You looks awfully familiar and familiarly awful."
			)
			(2
				(= cycles 1)
			)
...
(instance buyItemS of HandsOffScript
	...
	(method (changeState newState &tmp theCel theBreak)
		(switch (= state newState)
			(0
				(cond
					((and (< local128 6) (not [gEgoStats 12])) ; magic use
						(Say keapon 250 24) ; "You still don't have the skill, Phil, so I'll spare you the bill."
						(self dispose:)
					)
					((and (== local128 6) (gEgo has: 16)) ; Rope
						(Say keapon 250 25) ; "Say Jay, what the hey? The rope you've got is one and the same."
						(self dispose:)
					)
					(else
						(self cue:)
			...
			(1
				(= local129 [local131 local128])
				(if register
					(cond
						((== local128 7)
							(Say keapon self 250 26 250 27) ; "Well, the tag says 15 dinars, but I guess I can take 14 for 'em..."
						)
						((TrySkill 13 60) ; communication
							(= theBreak (if (== local128 1) 5 else 10))
							(Say ; "Whew! Smooth talking, Dude. Ya got the gift for gab, Tab."
								keapon
								self
								250
								28
								(Format @local28 250 29 theBreak) ; "Tell ya what I'm going ta do. Seein' as you beein' a Heero and all, I'll reduce the price by %d whole simoleons."
								250
								30
							)
							(-= local129 theBreak)
						...
						(else
							(Say keapon 250 31 250 32 250 33) ; "If you want to do some dealing, then pick a better deck."
							(self changeState: 9)
						)
					)
				else
			...
			(2
				...
				(if (or (not (IsFlag 120)) (== (Random 1 10) 1)) ; fKeaponZoomed
					...
					(switch (ChooseFromCycle (+ gMagicTrips 2) 0 2)
						(0
							(Say keapon self 250 34 250 35) ; "Ah, I have that here, Dear."
						)
						(1
							(Say keapon self 250 36 250 37) ; "Nothing up my sleeve..."
						)
						(2
							(Say keapon self 250 38) ; "Eins, Zwei, Drei, Vier, Make it snappy und bring it here!"
						)
					)
				)
			)
			...
			(8
				(switch (CheckFunds (* local129 100))
					(0
						(Say keapon self 250 39 250 40) ; "Need more money, Honey. Your moolah's too low, Moe."
					)
					(1
						(Say keapon self 250 41 250 42) ; "So solly, Chollie. This money's funny."
					)
					(2
						(Say keapon self 250 43) ; "Here you go, Joe. Hope you're happy, Pappy."
						(GiveMoney (* local129 100))
						(if (== local128 6)
							(SolvePuzzle 700 5 2)
							(self changeState: 10)
						else
			...
			(15
				(if (and local130 (not (gEgo learn: local130 10)))
					(HighPrint 250 44) ; "The spell looks and feels very familiar. In fact, you realize that you already knew it! What a waste of money!"
				)
				(= cycles 4)
			)
			(16
...
(instance zoomS of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				(Say keapon self 250 46 250 47) ; "Silly me. I had it here the whole time."
			)
			(12
				(= cycles 2)
			)
...
(instance keapon of Talker
	...
	(method (atSpec which)
		(return [local177 which])
	)

	(method (atGen which)
		(return [local139 which])
	)
	...
	(method (endTalk)
		...
		(floatTimer2 dispose:)
		(if local27
			(HighPrint 250 49) ; "You put the pot of dirt away."
			(= local27 0)
		)
	)
)
...
(instance floatTimer2 of Timer
	...
	(method (cue)
		(keapon x: (- (keapon x:) local26))
		(= local26 (- (Random 0 2) 1))
		(keapon x: (+ (keapon x:) local26))
...
(instance booksLeft of Feature
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(HighPrint (Format @local28 250 50 [local251 local262])) ; "There seem to be books in many languages, few of which are comprehensible to you, but you do recognize some old favorites, such as %s"
			(if (> (++ local262) 10)
				(= local262 0)
			)
		else
