...
(instance rm229 of Stage
	...
	(method (handleEvent event &tmp aSpell)
		(cond
			...
			((Said 'look,look,feel[/alley,passage,street,road,wall,ground]')
				(cond
					(seeDoor
					...
					(else
						(HighPrint 229 2) ; "You are walking through one of the many twisty streets of the city."
					)
				)
			)
			((Said 'look,look/door,gate')
				(if seeDoor
					...
					(HighPrint 229 3) ; "You don't detect any doors here at the moment."
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(if register
					(RedrawCast)
					(Print 229 5) ; "That didn't have any effect on the door."
				else
					(door cel: 0)
					(RedrawCast)
					(Print 229 6) ; "The wall seemed to absorb your spell's energies."
				)
				(self dispose:)
			)
		)
		...
		(switch (= state newState)
			...
			(6
				(if (and (== gPrevRoomNum 220) (not (IsFlag 65)))
					(HighPrint 229 7) ; "You hear the voices."
					(if (IsFlag 162)
						(VoicesPrint 229 8) ; "Our time is much too valuable to be wasted upon one with so little magic. Do not bother returning until you have acquired more power."
					else
						(VoicesPrint 229 9) ; "You will certainly have to be more clever and more skilled if you hope to be initiated. Perhaps you should practice your spells more."
					)
				)
				(HandsOn)
				(self dispose:)
