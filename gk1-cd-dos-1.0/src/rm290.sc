			...
			(6
				...
				(if (>= (gGame _detailLevel:) 3)
					...
					(gGkSound1
						number: 15
						setLoop: -1
						flags: (| $0004 (gGkSound1 flags:))
						play:
					)
				)
			)
			(7
				(gGkSound1 stop: flags: (^ $0004 (gGkSound1 flags:)))
				(UnLoad 140 15)
				(= local11 0)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(4
				...
				(gGkSound2 number: 291 setLoop: 1 play:)
			)
			(5
				(= ticks 10)
			)
			(6
				(gGkSound2 number: 292 setLoop: 1 play:)
				(snake dispose:)
...
(instance sStopDance of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(gGkSound2 number: 292 setLoop: 1 play: self)
				(snake init:)
			)
			(8
				(magentia
			...
			(if (not local3)
				(cond
					...
					(
						...
						(gMessager say: noun 0 0 0) ; "Does this mean anything to you?"
					)
					(else
						(gCurRoom setScript: sHalfShow 0 theVerb)
					)
