...
(class SRDialog of Dialog
	...
	(method (update theComment names nums &tmp temp0 temp1 temp2)
		...
		(= theStatus (GetStatus))
		(if (== gCurRoomNum 770) ; deathRm
			(cancelI state: (& (cancelI state:) $fffe))
		)
		(= temp1 (Str new:))
		(switch theStatus
			(0
			...
			(else
				(Message msgGET 64990 30 0 0 1 (temp1 data:)) ; "This directory/disk is full. Replace a saved game, or use Change Directory to save on another directory/disk."
			)
		)
		(textI text: (KString 8 (temp1 data:)) x: 0 y: 0) ; StrDup
		(if (OneOf theStatus 0 1)
			(textI setSize: 240 moveTo: 9 23)
		else
			(textI setSize: 260 moveTo: 5 20)
		)
		(temp1 dispose:)
		...
		(if (== theStatus 1)
			...
			(if (self contains: editI)
				...
				(if (editI bitmap:)
					(DisposeTextBitmap (editI bitmap:))
					(editI bitmap: 0)
				)
			)
		)
