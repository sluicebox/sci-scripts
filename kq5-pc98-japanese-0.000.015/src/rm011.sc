...
(instance water of Waters
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 11) ; "The swift river turns sharply to the south here."
					(event claimed: 1)
				)
				(3 ; Do
					(gEgo
...
(instance tree of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 12) ; "A swarm of bees buzz around a hole in a rotted old tree."
					(event claimed: 1)
				)
			)
		)
...
(instance hive of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 13) ; "It appears the bees have a hive inside the hole of the old dead tree."
					(event claimed: 1)
				)
				(3 ; Do
					(if (!= ((gInventory at: 17) owner:) 11) ; Honeycomb
						(PrintDC 11 14) ; "Graham has already taken a honeycomb. It wouldn't be wise to antagonize the now-friendly bees."
					else
						(HandsOff)
...
(instance anthill of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 15) ; "Graham notices a huge anthill in the distance to the north."
					(event claimed: 1)
				)
			)
		)
...
(instance bear of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 16) ; "A large bear seems to be mighty interested in the honey inside the old tree."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 11 17) ; "Graham should be very careful around a big old bear!"
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 11 18) ; "The bear is too intent on the honey to listen to Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 11 19) ; "The bear is more interested in the honey."
							(event claimed: 1)
						)
					)
				)
...
(instance bees of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 20) ; "A swarm of bees buzz around a hole in a rotted old tree."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 11 21) ; "Bees don't have much to say."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 11 22) ; "It would be wise if Graham didn't bother the bees."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 11 23) ; "The bees don't care about it."
							(event claimed: 1)
						)
					)
				)
...
(instance stick of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 11 24) ; "Graham notices a large stick lying on the ground near the old tree."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
