...
(local
	...
	local5
	local6
	[local7 42] = [0 189 0 0 319 0 319 142 255 136 255 131 227 126 137 126 138 127 205 127 214 135 117 142 101 142 101 147 74 149 55 152 37 159 29 165 36 171 54 173 136 189]
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 0) ; "A hot fire burns fiercely in the hideous fireplace near the bed."
					(event claimed: 1)
				)
			)
		)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if local1
						(PrintDC 63 1) ; "Mordack's wand lies temptingly upon the small table near his bed."
						(event claimed: 1)
					)
				)
				(3 ; Do
...
(instance statue of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 2) ; "A strange statue of a winged woman sits close to Mordack's bed."
					(event claimed: 1)
				)
			)
		)
			...
			(switch (event message:)
				(2 ; Look
					(if (gCast contains: (ScriptID 550 7)) ; theWizard
						(PrintDC 63 3) ; "Mordack lies asleep on his huge, horrendous bed."
					else
						(PrintDC 63 4) ; "A large, grotesque bed occupies one corner of Mordack's bedroom."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (gCast contains: (ScriptID 550 7)) ; theWizard
						(PrintDC 63 5) ; "Now would not be a good time to bother Mordack!"
					else
						(PrintDC 63 6) ; "The thought of lying on that horrible bed is totally revolting to Graham."
					)
					(event claimed: 1)
				)
			)
...
(instance door of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 7) ; "The doorway leads into an upstairs hallway."
					(event claimed: 1)
				)
			)
		)
...
(instance interior of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 8) ; "Graham surveys the bizarre furnishings and horrible figures in Mordack's bedroom."
					(event claimed: 1)
				)
			)
		)
...
(instance wand of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 9) ; "Mordack's wand lies temptingly upon the small table near his bed."
					(event claimed: 1)
				)
				(3 ; Do
					(proc550_17)
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				...
				(proc0_28 73 63 10 67 30 10 25 6) ; "Good-bye, KING Graham of Daventry...heh, heh, heh!"
				(switch global351
					(135
						(= temp0 8)
					)
...
(instance windows of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 11) ; "Graham looks out the window and views the distorted rocks and twisted forms of Mordack's strange island."
					(event claimed: 1)
				)
			)
		)
...
(instance robin of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 63 12) ; "Special thanks to Robin Bradley and the rest of the QA staff."
					(event claimed: 1)
				)
			)
		)
