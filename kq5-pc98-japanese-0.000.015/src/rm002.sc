...
(instance rm002 of KQ5Room
	...
	(method (init)
		...
		(= global321 48)
		(= global325
			{"There's nothing interesting around here. Come on!"}
		)
					...
					(4 ; Inventory
						(if
							...
							(if (gCast contains: snake)
								...
								(PrintDC 2 1) ; "Graham hits and shakes the tambourine with much ruckus, but finds it doesn't accomplish anything."
							)
							(event claimed: 1)
						)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(proc0_28 75 2 2 67 10 20 25 5) ; "Watch out! A poisonous snake!"
				(client setScript: 0)
			)
		)
	)
...
(instance shakeTambourine of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(snake dispose:)
				(PrintDC 2 3 #at 10 10 #time 8) ; "Frightened by the noisy tambourine, the snake reluctantly slithers away."
				(gEgo
					normal: 1
					view: local0
					loop: 7
...
(instance mountPath of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 2 4) ; "A worn dirt path wanders through a thick wood alive with the sound of many creatures. Between the trees, to the east, Graham can see the outline of a great mountain range."
					(event claimed: 1)
				)
			)
		)
...
(instance snake of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 2 5) ; "A large, venomous snake blocks Graham's passage to the east."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 2 6) ; "This snake has a menacing look which Graham should heed."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 2 7 #at 100 10) ; "The snake looks at Graham with cold eyes and doesn't answer."
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(else
							(PrintDC 2 8) ; "That won't scare the snake away."
							(event claimed: 1)
						)
					)
				)
...
(instance room of RFeature
	...
	(method (handleEvent event &tmp temp0)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 2 9) ; "A worn dirt path wanders through a thick wood alive with the sound of many creatures. Between the trees, to the east, Graham can see the outline range."
					(event claimed: 1)
				)
			)
		)
