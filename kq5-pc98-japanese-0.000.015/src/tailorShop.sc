	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				...
				(if
					(and
						(not (== (gCurRoom script:) greet))
						(not (== (gCurRoom script:) soldCloak))
						(not (== (gCurRoom script:) helpWithCloak))
						(not (== (gCurRoom script:) helpCounter))
						(not (== (gCurRoom script:) tailorTalk))
					)
					(HandsOff)
					(switch local5
						(0
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (not local3)
						(++ local3)
						(HandsOff)
						(proc0_28 105 203 45 67 20 20 25 6) ; "Here. Let me help you with that."
						(gCurRoom setScript: helpWithCloak)
					else
						(proc0_28 105 203 46 67 20 20 25 6) ; "It's a fine cloak. It could be very useful."
					)
					(event claimed: 1)
				)
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						...
						(4
							...
							(RedrawCast)
							(proc0_28 160 203 39 67 20 20 25 8) ; "I would like to buy that fine cloak hanging in the corner. Will you take a silver coin for it?"
							(proc0_28 105 203 47 67 20 20 25 8) ; "A silver coin! You MUST be joking! That beautiful cloak is worth MUCH more than that!"
							(event claimed: 1)
						)
						(11
							(= local6 1)
						...
						(26
							(proc0_28 105 203 48 67 20 20 25 7) ; "I hope you enjoy your fine cloak."
							(event claimed: 1)
						)
						(else
							(proc0_28 105 203 49 67 20 20 25 10) ; "No, that doesn't even begin to cover the cost of the material."
							(event claimed: 1)
						)
					)
				)
...
(instance curtain of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 203 50 #at 10 10) ; "At the back of the shop, Graham notices a dressing room in which a finicky customer tries on, and rudely rejects, many items of fine clothing."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 203 51 #at 10 10) ; "The dressing room appears to be occupied at the moment."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 203 52 #at 10 10) ; "The rude customer doesn't care to talk to Graham."
					(event claimed: 1)
				)
			)
		)
...
(instance employee of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 203 53 #at 20 20) ; "An employee of the tailor, a friendly, good-looking young man, caters hopelessly to a disagreeable customer in the back dressing room."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 203 54 #at 10 10) ; "The young man is too busy with another customer to be bothered by Graham."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 203 54 #at 10 10) ; "The young man is too busy with another customer to be bothered by Graham."
...
(instance shopRoom of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 203 55 #at 20 20) ; "The busy tailorshop bustles with activity as Graham looks around with interest at all the tailor's wares."
					(event claimed: 1)
				)
			)
		)
