...
(instance brother of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 47) ; "In the kitchen of the bakehouse, Graham can see another big, burly man doing the day's baking."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 206 48) ; "The kitchen is off-limits to customers. Besides, Graham would only be in the way."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 206 49) ; "The baker's brother is too busy to chat with Graham."
					(event claimed: 1)
				)
				(4 ; Inventory
					(if (== (gInventory indexOf: (gTheIconBar curInvIcon:)) 28)
						...
						(PrintDC 206 50) ; "The man in the kitchen is doing the baking and NOT tending to customers."
						(event claimed: 1)
					)
				)
			)
...
(instance cat of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 51) ; "The Baker brothers seem to have a pet; a large, mangy cat."
					(event claimed: 1)
				)
				(5 ; Talk
					(PrintDC 206 52) ; "The cat doesn't appear to like Graham, much less answer him."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 206 53) ; "The cat doesn't look too friendly."
					(event claimed: 1)
				)
				(4 ; Inventory
					(if (== (gInventory indexOf: (gTheIconBar curInvIcon:)) 28)
						...
						(PrintDC 206 54) ; "The cat is too lazy to care about it."
						(event claimed: 1)
					)
				)
			)
...
(instance pie of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 55) ; "Delicious, mouthwatering custard pies lining the countertop attract Graham's eyes... and nose!"
					(event claimed: 1)
				)
				(3 ; Do
					(if (not (gCurRoom script:))
						(cond
							...
							((gEgo has: 2) ; Pie
								(PrintDC 206 56) ; "Graham doesn't need another pie."
								(event claimed: 1)
							)
							(else
								(PrintDC 206 57) ; "Graham would love to buy a pie if he could."
								(event claimed: 1)
							)
						)
					)
...
(instance kitchenDoor of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 58) ; "Graham can see the bakehouse kitchen through the partially-open Dutch door."
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 206 59) ; "Only employees are allowed in the kitchen."
					(event claimed: 1)
				)
			)
		)
...
(instance shop of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 60) ; "Mmmmmmm. The wonderful smells of the bakehouse set Graham's stomach to rumbling and his mouth to watering."
					(event claimed: 1)
				)
			)
		)
...
(instance counterTop of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 61) ; "Delicious, mouthwatering custard pies lining the countertop attract Graham's eyes...and and nose!"
					(event claimed: 1)
				)
				(3 ; Do
					(if (not (gCurRoom script:))
...
(instance boy of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 62) ; "The little boy looks all dressed up for a special occasion."
					(event claimed: 1)
				)
				(5 ; Talk
					(event claimed: 1)
					(if (or local5 (> (boy y:) (+ (gEgo y:) 25)))
						(PrintDC 206 63) ; "He appears in too much of a hurry to talk to Graham."
					else
						(++ local5)
						(User canInput: 0)
...
(instance woman of Actor
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 206 64) ; "This lady looks like she's in a real hurry."
					(event claimed: 1)
				)
				(5 ; Talk
					(event claimed: 1)
					(if (or local4 (> (woman y:) (+ (gEgo y:) 25)))
						(PrintDC 206 65) ; "She appears in too much of a hurry to talk to Graham."
					else
						(++ local4)
						(User canInput: 0)
