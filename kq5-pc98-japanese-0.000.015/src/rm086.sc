	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (not local36)
						(PrintDC 86 9) ; "A rusty lock secures the door preventing Graham from leaving the cellar."
					else
						(HandsOff)
				...
				(4 ; Inventory
					(switch (gInventory indexOf: (gTheIconBar curInvIcon:))
						(1
							(PrintDC 86 10) ; "The key won't fit this lock."
							(event claimed: 1)
						)
						(22
							(if (not local36)
								...
								(PrintDC 86 11) ; "The lock has already been broken."
							)
							(event claimed: 1)
						)
						(28
						...
						(else
							(PrintDC 86 12) ; "That won't help Graham unlock the door."
							(event claimed: 1)
						)
					)
				)
...
(instance rope of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 86 13) ; "Lying in a heap on the cold, stone floor, Graham sees the rope that once held him captive."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
