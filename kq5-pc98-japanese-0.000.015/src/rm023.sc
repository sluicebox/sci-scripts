...
(instance wheel of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 23 12 #at 120 140) ; "An intricate spinning wheel is tucked away in the trunk."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (== ((gInventory at: 1) owner:) 23) ; Key
						(PrintDC 23 13) ; "An interesting incense burner hangs from a protruding tree branch. Occasionally, from within it, a tiny glint winks."
					else
						(PrintDC 23 14) ; "An interesting incense burner hangs from a protruding tree branch."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (== ((gInventory at: 1) owner:) 23) ; Key
						...
						(PrintDC 23 15) ; "The incense burner is now empty."
						(event claimed: 1)
					)
				)
			)
			...
			(switch (event message:)
				...
				(3 ; Do
					(cond
						...
						((not local5)
							(PrintDC 23 16) ; "There is nothing else inside the drawer."
							(++ local5)
							(event claimed: 1)
						)
					)
...
(instance pouch of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 23 17 #at 20 140) ; "A small pouch is tucked away in the drawer."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
