...
(instance duneLooper of Script
	...
	(method (doit)
		...
		(gEgo setLoop: [local4 (/ (gEgo heading:) 45)])
	)
)
...
(instance cupboard of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 56 6) ; "A cupboard in the corner of the pantry catches Graham's interest."
					(event claimed: 1)
				)
				(3 ; Do
					(cond
						((!= ((gInventory at: 24) owner:) 56) ; Bag_of_Peas
							(PrintDC 56 7) ; "There is nothing more of interest in the cupboard."
						)
						((not local2)
							(++ local2)
							(HandsOff)
						...
						((== ((gInventory at: 24) owner:) 56) ; Bag_of_Peas
							(PrintDC 56 8) ; "The cupboard is already open."
						)
					)
					(event claimed: 1)
				)
...
(instance bag of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 56 9) ; "A bag labeled Peas sits in the cupboard."
					(event claimed: 1)
				)
				(3 ; Do
					(HandsOff)
...
(instance bCel1 of Prop
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 56 10) ; "Graham's eyes fall on a bag of dried peas in the open cupboard."
					(event claimed: 1)
				)
			)
		)
			...
			(switch (event message:)
				...
				(3 ; Do
					(if (== ((gInventory at: 24) owner:) 56) ; Bag_of_Peas
						...
						(PrintDC 56 11) ; "There is nothing else of interest in cupboard."
					)
					(event claimed: 1)
				)
			)
