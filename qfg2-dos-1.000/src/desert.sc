...
(procedure (calcRoom oldRoom oldDir &tmp index nr)
	(if (< (= nr oldRoom) 0)
		...
		(if (< nr 0)
			(Printf 665 18 nr oldDir) ; "Bad desert room: room %d, direction %d"
			(= nr 1300)
			(= oldDir 3)
			(gEgo posn: 160 70)
		)
...
(procedure (GolfPrint &tmp [string 20] [printRect 4] theWidth theX)
	(if (== (++ strokeCount) 900)
		...
		(Format @string 665 19 [golfPhrases (Random 0 9)] strokeCount) ; "%s! %d..."
	)
...
(procedure (ReadArray theArray idx)
	...
	(return (| (StrAt theArray (* idx 2)) (<< (StrAt theArray 1) $0008)))
)

(procedure (WriteArray theArray idx theValue)
	(StrAt theArray (+ 0 (* idx 2)) (& theValue $00ff))
	(StrAt theArray (+ 1 (* idx 2)) (& (>> theValue $0008) $00ff))
)
...
(instance desert of Rm
	...
	(method (init &tmp nr)
		...
		(gCSound number: 660 priority: 0 loop: -1 playBed:)
		(if gNight
			(nightSound play:)
		)
		...
		(cond
			...
			((Said 'get,get,cut,search,get/tail,stinger')
				(cond
					...
					((== (deadBody view:) 671)
						(HighPrint 665 8) ; "You get the scorpion's tail, carefully avoiding it's venom."
			...
			((Said 'get,get>')
				(cond
					((Said '/boulder')
						(HighPrint 665 15) ; "You pick up a rock from the sand."
						(gEgo get: 9) ; Rock
					)
					((Said '/sand')
						(if (gEgo has: 38) ; ClothBag
							(HighPrint 665 16) ; "You scoop some sand into the cloth bag."
							(= gClumpsLeft (+ (* (/ gClumpsLeft 10) 10) 9))
						else
							(HighPrint 665 17) ; "You pick up a handful of sand and watch mournfully as it trickles through your fingers and disappears."
						)
					)
				)
			)
...
(instance msgTimer of Timer
	...
	(method (cue &tmp [string 40])
		(Print (Format @string 665 20 [heatMsg (Random 0 5)]) #at -1 20 #dispose) ; "%s"
...
(instance drinkTimer of Timer
	...
	(method (cue)
		(if (and (not (DrinkWater 0)) (not (TakeDamage (* gDrinksLeft -10))))
			...
			(EgoDead
				1
				(Format @string2 665 21 string1) ; "You collapse under the %s, and perish from dehydration. The desert is no place to be without water."
				#title
...
(instance desertChecker of Code
	...
	(method (doit spellNum)
		(if (or (== spellNum 6) (== spellNum 28))
			(HighPrint 665 22) ; "There is no need for that here."
			(return 0)
		else
			(return 1)
		)
			...
			(3
				...
				(if gBucks
					...
					(if (= theC (mod gBucks 100))
						(Format @string1 665 23 theC) ; "and %d Centimes."
					else
						(Format @string1 665 24) ; "."
					)
					(HighPrint (Format @string2 665 25 theD @string1)) ; "You search the carnage and find %d Dinars%s"
					(gEgo get: 2 theD) ; Dinar
					(gEgo get: 1 theC) ; Centime
					(= gBucks 0)
				else
					(HighPrint 665 26) ; "You find nothing of value."
				)
				(= cycles 1)
			)
			(4
				(if gHitDaggers
					(HighPrint
						(Format @string2 665 27 gHitDaggers) ; "You remove %d of your daggers from the body."
