		...
		(switch (= state newState)
			...
			(3
				(cond
					...
					(register
						(gCSound number: 20 loop: 1 priority: 12 play: self)
					)
					(else
						(= seconds 3)
					)
...
(instance chest of TargActor
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'unlock,lockpick/lock,chest,box,strongbox,safe')
				(cond
					((== (chest cel:) 2)
						(HighPrint 140 58) ; "Not much point in that! The chest is already open."
					)
					((not (self passedChecks: 2)))
					((not (CanPickLocks))
						(HighPrint 140 59) ; "You don't have the skill or the tools."
					)
					((TrySkill 9 75 gLockPickBonus) ; pick locks
						(self setScript: openChest 0 1)
					)
					(else
						(HighPrint 140 60) ; "The lock is too tricky for you. You'll have to keep practicing."
					)
				)
			)
		)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1
				(HighPrint 140 61) ; "Issur's strongbox is in this hole in the floor."
			)
			(7
				(if (not niteTime)
		...
		(cond
			...
			((Said 'replace,(put<back)/anvil')
				...
				(if (IsFlag 114) ; fAnvilMoved
					...
					(HighPrint 140 78) ; "The anvil is in its proper place."
				)
			)
			((Said 'force,slide/anvil')
				(cond
					...
					((== (trapDoor cel:) 1)
						(HighPrint 140 79) ; "You can't move the anvil -- the trap door is in your way."
					)
					(else
						(MoveAnvil)
					)
