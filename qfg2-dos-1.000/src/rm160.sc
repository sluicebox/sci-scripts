...
(local
	...
	[uhuraSpecList 60] = [2 '//labor' 3 '//note' 4 '//book,log,logbook' 5 '//plaque,emblem,logo,crest[<eof]' 6 '//spear,bow,arrow,quiver' 7 '//simba,baby,baby' 8 '//home' 9 '//scorpion' 10 '//bandit' 11 '//ghoul' 12 '//jackal,jackal,(man<jackal)' 13 '//dinosaur,(saurii<terror)' 14 '//griffin' 15 '//dad' 16 '//errand,bulletin,sign,board[<errand]' 17 '//rakeesh,lion' 18 '//paladin' 19 '//leg,bandage,injury' 20 '//eof,(order[<eternal,fighter])' 21 '//wizard,(user<magic),mage' 22 '//afreet' 23 '//attar,apothecary' 24 '//keapon,(clerk<magic),keapon' 25 '//issur,armorer,armorer' 26 '//prize,service' 27 '//blade,soulforge,(forge<soul)' 28 '//hero,adventurer' 29 '//moose' 30 '//train,practice,attack,battle' 0 0]
		...
		(cond
			...
			((Said 'get,get,get,borrow/blade,(forge<soul),soulforge')
				(if (and (== gDay 12) (== gElementalState 5) (== gOrigHeroType 0))
					...
					(Say rakeesh 160 7) ; "I will keep my sword, thank you."
				)
			)
			(
				(Said
			...
			((Said 'read/log,logbook,(book[<log])')
				(cond
					...
					(else
						(HighPrint 160 10) ; "The log is for the most part written in a language you can't read."
					)
				)
			)
			(
			...
			((Said 'kiss/guildmaster,woman,(master<club)')
				(HighPrint 160 14) ; "You would probably anger her if you tried, and you don't want to see her mad."
			)
			...
			((Said 'join/eof,order')
				(cond
					((gCast contains: rakeesh)
						(Say rakeesh 160 15) ; "One does not just join the Eternal Order of Fighters, nor is this the place if you could."
					)
					((gCast contains: uhura)
						(Say uhura 160 15) ; "One does not just join the Eternal Order of Fighters, nor is this the place if you could."
					)
					(else
						(HighPrint 160 15) ; "One does not just join the Eternal Order of Fighters, nor is this the place if you could."
					)
				)
			)
		)
...
(instance egoTakesSword of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Face gEgo rakeesh)
				(Say rakeesh self 160 16) ; "You will need the magic of a flaming sword to defeat the Earth Elemental."
			)
			(2
				(rakeesh x: 198 z: 36)
				(rakeeshHead hide:)
			...
			(3
				(Say rakeesh self 160 17) ; "Permit me to loan you my sword, "Soulforge.""
			)
			(4
			...
			(7
				(rakeesh x: 200)
				(Say rakeesh self 160 18 160 19 160 20) ; "When you are in battle against the Elemental, the flame of the sword will aid you."
			)
			(8
...
(instance egoGivesSword of HandsOffScript
	...
	(method (changeState newState &tmp theX theY)
		(switch (= state newState)
			...
			(1
				(if (== uhuraResponse 0)
					(Say uhura self 160 21) ; "Good sword, good fight. There are many be proud of you."
				else
					(Say uhura self 160 22) ; "That was better."
				)
			)
			(2
...
(instance babyTurns of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if babyMessage
					(HighPrint 160 23 80 {Hero}) ; "Goo goo ga ga goo."
				)
			...
			(2
				(if babyMessage
					(= babyMessage 0)
					(Print 160 24 #title {Baby Simba} #at -1 25 #window babyWindow) ; "Poltergeists make up the principle types of spontaneous material manifestations."
					(= babyTimer 5)
				else
...
(instance babyToss of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(simba loop: 11 setCycle: Fwd)
				(HighPrint 160 26 80 {Baby Simba} 35 babyWindow 103) ; "Watch it buddy! I'm not getting hazardous duty pay on this gig, so don't mess with me. They don't pay me enough to put up with a lowlife like you. Cheeeeeezzzz!!"
				(= seconds 6)
			)
			(2
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(if (<= 3 (uhuraBody loop:) 4)
				(HighPrint 160 27) ; "The warrior woman cradles baby Simba in her arms."
			else
				(HighPrint 160 28) ; "The warrior woman stands tall and proud."
			)
		else
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'tell/guildmaster,woman/(spielburg,bandit)[<about]')
				(Say self 160 29) ; "Maybe you find work for Hero here, too."
				(TrySkill 13) ; communication
			)
			((or (Said 'give/bouquet') (Said 'give//bouquet'))
				(GiveFlowers self 160 30)
			)
			((Said 'thank')
				(SayThanks self 160 31)
			)
			((or (Said 'hello,greet') (Said '/hello,greet'))
				(Say self 160 32) ; "Maybe today you and I get in some more practice at fighting.  It will be good to feel the spear in my hand again."
			)
			(
				(and
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'hello,greet[/rakeesh]') (Said '/hello,greet'))
				(Say self 160 34) ; "It is good to see you, young Hero. A nice day to be adventuring."
			)
			((Said 'give,replace,refuse/purse,alm,prize')
				(cond
					...
					((or (not (IsFlag 610)) (IsFlag 600))
						(Say self 160 36) ; "My thanks, but I have no need for gifts."
					)
					(else
						(gEgo use: 2 50) ; Dinar
						(SkillUsed 14 50) ; honor
						(Say self 160 37) ; "You show great honor. May your deeds continue to shine!"
					)
				)
			)
		)
...
(instance rakeeshTail of Extra
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(if (gCast contains: simba)
				(HighPrint 160 38) ; "Imagine! Such a warrior's tail used as a playtoy for a baby!"
			else
				(HighPrint 160 39) ; "Rakeesh relaxes but his tail stays ever alert."
			)
		else
			(super doVerb: theVerb &rest)
		)
		...
		(switch theVerb
			(1
				(if (and (< (rakeeshBody loop:) 5) (gCast contains: rakeesh))
					(HighPrint 160 40) ; "The baby fearlessly plays with the Liontaur's tail."
				else
					(HighPrint 160 41) ; "The kid's hangin' loose."
				)
			)
			(9
				(if (not (IsFlag 71)) ; fBabyMessage
					...
					(HighPrint 160 42) ; "Don't bother. He only tries to show off."
				)
			)
			(7
				(HighPrint 160 43) ; "Rakeesh wouldn't appreciate it."
			)
			(else
				(super doVerb: theVerb &rest)
			)
		...
		(cond
			...
			((Said 'hit/baby,simba')
				(cond
					...
					(oneKick
						(HighPrint 160 44) ; "Hero types don't do that sort of thing more than once."
					)
					(else
						(HighPrint 160 45) ; "Hero types don't do that sort of thing."
					)
				)
			)
			((Said 'get,get,get/baby')
				(HighPrint 160 46) ; "Uhura would have your head off before you got 10 feet. Besides, there is no baby market in Shapeir."
			)
		)
	)
)
...
(instance responseTimer of Timer
	...
	(method (cue)
		(switch (++ uhuraResponse)
			(1
				(Face gEgo uhuraBody)
				(Say uhura 160 47) ; "Maybe you do not hear so good. I be thinking you give me back Rakeesh's sword, or maybe I be getting plenty mad."
				(StartTimer self 15)
			)
			(2
				(gEgo use: 31) ; Soulforge
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(if (gEgo inRect: 64 110 137 130)
				...
				(HighPrint 160 48) ; "The standard Guild Hall Quest Board has an assortment of notices posted, but you must move closer in order to read them."
			)
		else
			(super doVerb: theVerb &rest)
		)
...
(instance EOFBoard of PicView
	(properties
		x 192
		y 87
		noun '/plaque,emblem,logo,crest[<eof]'
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'read/plaque,emblem,logo,crest[<eof]')
				(self doVerb: 1)
			)
		)
	)
	...
	(method (doVerb theVerb)
		(if (== theVerb 1)
			(if (gEgo inRect: 155 110 231 130)
				(HighPrint 160 49) ; "If you got real guts and don't know the meaning of fear; If you got real muscles and don't take guff from no one; If you got a real weapon and know how to use it; then maybe you are good enough to join the Eternal Order of Fighters."
				(HighPrint 160 50) ; "We pick only the best to join and we'll be watching you, so don't just be good, be great! Be a member of EOF! Eternal Order of Fighters The Guts, The Glory, The Greatest!"
			else
				(HighPrint 160 51) ; "There is some writing on the EOF emblem on the wall, but you are not close enough to read it."
			)
		else
			(super doVerb: theVerb &rest)
		)
