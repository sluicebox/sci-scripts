	...
	(method (handleEvent event)
		(cond
			...
			((Said 'throw/dirt,earth,pan')
				(gCurRoom setScript: dirtScript 0 1)
			)
			((Said 'throw/sand,bag')
				(gCurRoom setScript: dirtScript 0 0)
			)
		)
	)
...
(instance blowEgo of HandsOffScript
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'drop,throw/sand')
				(DontTalk 1)
				(if (mod gClumpsLeft 10)
					(HighPrint 241 6) ; "You force some sand into the center of the Air Elemental's funnel."
					(-- gClumpsLeft)
					(GroundAir)
				else
					(DontHave)
			...
			(else
				(event claimed: 1)
				(HighPrint 241 7) ; "It's too windy to do that."
			)
		)
	)
				...
				(cond
					((TrySkill 0 125) ; strength
						(if (not heardStrongest)
							(HighPrint 241 8) ; "You manage to withstand the force of the wind."
							(= heardStrongest 1)
						)
						(DontTalk 0)
						(airElemental
					...
					((> [gEgoStats 0] 100) ; strength
						(if (not heardStrong)
							(HighPrint 241 9) ; "You try to force yourself into the wind, but it is stronger than you, and you can't quite get to the center of the funnel."
							(= heardStrong 1)
						)
						(self cue:)
					)
					(else
						(if (not heardWeak)
							(HighPrint 241 10) ; "You are just not strong enough to withstand the wind."
							(= heardWeak 1)
						)
						(self cue:)
					)
...
(instance levRopeSaids of Script
	...
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
			((Said 'drop/dirt,earth')
				(if
					(and
						(< (gEgo distanceTo: airElemental) 20)
						(> gClumpsLeft 9)
					)
					(HighPrint 241 11) ; "You scoop some dirt into the funnel."
					(-= gClumpsLeft 10)
					(GroundAir)
				else
					(HighPrint 241 12) ; "You don't seem to be close enough for that."
				)
			)
			((Said 'drop/sand')
				(if
					...
					(HighPrint 241 13) ; "You pitch some sand into the funnel."
					(-- gClumpsLeft)
					(GroundAir)
				else
					(HighPrint 241 12) ; "You don't seem to be close enough for that."
			...
			((Said 'throw/dirt,earth,pan,sand,bag')
				(HighPrint 241 14) ; "You will need to be on the ground in order to do that."
			)
			(else
				(event claimed: 0)
			)
		...
		(switch (= state newState)
			...
			(8
				(if airGrounded
					...
					(HighPrint 241 15) ; "You capture the Air Elemental in the bellows and proudly put them away."
				else
					(HighPrint 241 16) ; "The swift Air Elemental easily avoids the bellows."
				)
				(= cycles 1)
			)
			(9
