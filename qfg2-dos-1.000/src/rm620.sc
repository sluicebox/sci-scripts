	...
	(method (handleEvent event &tmp saurusCheck)
		(cond
			...
			((Said 'nap')
				...
				(if gNight
					(EgoSleeps 5 0)
					(HighPrint 620 8) ; "You awaken at dawn, much refreshed."
				else
					(EgoSleeps 20 0)
			...
			((Said 'attack[/dervish]')
				(cond
					...
					(else
						(HighPrint 620 13) ; "It appears that not only will he not fight with you, your attacks do him no harm."
					)
				)
				(++ thatsOne)
			)
...
(instance dervBod of TargActor
	...
	(method (hurtMe)
		(HighPrint 620 14) ; "Your attacks seem unable to hurt the dervish."
		(return 0)
	)
)
...
(instance dervish of Talker
	...
	(method (doVerb theVerb)
		(if (== theVerb 9)
			(Say dervish 620 15) ; "If you want right answer, ask right person."
		else
		...
		(switch (= state newState)
			...
			(2
				...
				(if waterGotten
					...
					(HighPrint 620 18) ; "You take a deep drink of the warm, but refreshing water."
					(= waterGotten 1)
				)
				(= cycles 12)
			)
				...
				(cond
					(
						...
						(if (> [gInvNum 37] 1) ; Waterskin
							(HighPrint 620 19) ; "You check and realize that your waterskins are full."
						else
							(HighPrint 620 20) ; "You check and realize that your waterskin is full."
						)
					)
					((> [gInvNum 37] 1) ; Waterskin
						(HighPrint 620 21) ; "You then check and refill all your waterskins."
						(= gDrinksLeft (* [gInvNum 37] 10)) ; Waterskin
					)
					((gEgo has: 37) ; Waterskin
						(HighPrint 620 22) ; "You then check and refill your waterskin."
						(= gDrinksLeft (* [gInvNum 37] 10)) ; Waterskin
					)
					(else
						(HighPrint 620 23) ; "It's a shame you don't have a waterskin to fill."
					)
				)
				(= cycles 4)
			)
...
(instance thrownForLoop of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Say dervish self 620 24) ; "Perhaps you should not attempt such a thing while riding a saurus."
				(= cycles 3)
			)
			(2
				(saurus
			...
			(7
				(cond
					((not (<= (gEgo distanceTo: dervish) 75))
						(LowPrint 620 25) ; "Move over to the dervish first."
					)
					((gEgo has: 14) ; Beard
				...
				(if (IsFlag 168) ; fDervishDailyMsg
					...
					(if (< gDay 13)
						(Say
							dervish
							self
							(Format @str 620 29 [enterSaying gDay]) ; "%s."
						)
					else
						(Say dervish self 620 30) ; "Man who waits too late gets no response from man who waits even later."
					)
				)
			)
			(1
