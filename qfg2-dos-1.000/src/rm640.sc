...
(instance rm640 of Stage
	...
	(method (handleEvent event)
		(cond
			...
			(
				(or
					(Said 'give,pour,offer/potion[<disenchant,disenchant]')
					(Said 'give,pour,offer//potion[<disenchant,disenchant]')
				)
				(cond
					...
					((IsFlag 44) ; fHeardPlant
						(HighPrint 640 27) ; "You remember Aziza's story, and you know that the gifts you have given to Julanar are what needed to be done for her."
					)
					(else
						(HighPrint 640 28) ; "You sense that your kind words and deeds have done more than the potion could."
					)
				)
			)
			(
