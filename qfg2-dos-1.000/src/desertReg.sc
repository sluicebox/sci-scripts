...
(procedure (CheckSaurus event &tmp retVal theX theY)
	...
	(cond
			...
			(cond
				((Said 'mount,ride,(climb,climb,get,get<on,up)[/saurii,roget]')
					(if (or (gCast contains: saurus) (== (gEgo view:) 12))
						...
						(HighPrint 660 18) ; "Your saurus is not here."
					)
				)
		...
		((Said 'mount,ride,(climb,climb,get,get<on,up)[/saurii,roget]')
			(HighPrint 660 22) ; "Whee! Nothing like a Saurus ride in the desert!"
		)
		...
		((Said 'go/home')
			...
			(HighPrint 660 23) ; "Your saurus' natural homing instinct quickly sets you on the right path."
		...
		(else
			(event claimed: 1)
			(HighPrint 660 24) ; "You can't do that while riding a Saurus. You'd fall off."
		)
	)
	(return retVal)
)
...
(instance dismountSaurus of HandsOffScript
	...
	(method (changeState newState &tmp egoPri)
		(switch (= state newState)
			(0
				(if (or (NoRoom 15) (and (== gCurRoomNum 665) (SaurusToFeature))) ; desert
					(HighPrint 660 10) ; "Your saurus needs more room before you can dismount."
					(self dispose:)
				else
					(self cue:)
				)
...
(instance saurus of TargActor
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1
				(if (== gEgoGait 3) ; riding
					(HighPrint 660 11) ; "Don't these things ever sweat?"
				else
					(HighPrint 660 12) ; "In Spielburg, these things weren't so friendly."
				)
			)
			(7
				(if (== gEgoGait 3) ; riding
					(HighPrint 660 13) ; "You got it."
				else
					(HighPrint 660 14) ; "Just walk on over and get it."
				)
			)
			(4
				(HighPrint 660 15) ; "Where's that clothespin!"
			)
			(6
				(HighPrint 660 16) ; "If it tastes anything like it smells...!"
			)
			(else
	...
	(method (hurtMe)
		(HighPrint 660 17) ; "That's not a good idea."
	)
)
