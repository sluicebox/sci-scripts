...
(instance thark of TargActor
	...
	(method (cue)
		(super cue:)
		(HighPrint 500 23) ; "Good idea! He does look top-heavy with all those arms, doesn't he? Unfortunately, his mass is far too great for that spell to have any effect."
	)
...
(instance dococ of Monster
	...
	(method (fetchMe)
		(HighPrint 500 24) ; "You've got to be kidding! That statue must weigh a thousand pounds!"
		(return 0)
	)
...
(instance candle of TargActor
	...
	(method (fetchMe)
		(cond
			...
			(hitCandle
				(djinniPrint 500 25) ; "You've already broken the spell, Master. Stop that human!"
			...
			(else
				(djinniPrint 500 26) ; "You must hit an unlit candle to break the spell, Master.  Please stop that human!"
				(return 0)
			)
		)
	)
	...
	(method (hurtMe)
		(cond
			((== lastLit 5)
				(djinniPrint 500 27) ; "You're too late, Master. We've failed"
				(return 0)
			)
			((> xStep lastLit)
		...
		(switch theVerb
			(7
				(cond
					...
					((gCurRoom script:)
						(HighPrint 500 29) ; "You're too busy right now."
					)
					(else
		...
		(cond
			...
			((Said 'hit,move,knock/candle')
				(cond
					...
					(else
						(HighPrint 500 30) ; "Just run up to one and do it."
					)
				)
			)
		)
...
(instance brazier of TargActor
	...
	(method (hurtMe amount whatHurt)
		(cond
			...
			((== (whatHurt type:) 25)
				(HighPrint 500 31) ; "The brazier is already lit. Your spell has no effect."
				(return 1)
			)
			((> hitY 106)
	...
	(method (fetchMe)
		(HighPrint 500 33) ; "The brazier is too heavy to fetch."
		(return 0)
	)
...
(instance theSpellChecker of Code
	...
	(method (doit theSpell)
		(if (OneOf theSpell 28 6)
			(djinniPrint 500 34) ; "No, Master! You need to keep your feet on the ground."
			(return 0)
		else
			(return 1)
		)
