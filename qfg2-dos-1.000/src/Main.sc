...
(instance statLn of Code
	...
	(method (doit str)
		(Format str 0 13 gScore gPossibleScore) ; "Quest for Glory 2: Trial By Fire [%d of %d]"
	)
)
...
(instance Trial of Game
	...
	(method (startRoom nr &tmp newRegion)
		...
		(if
			(and
				...
				(Print 0 19 #button {test} 1) ; "Memory fragmented."
			)
			(gGame showMem:)
			(SetDebug)
		)
	...
	(method (handleEvent event &tmp num index gotOne theRoom dropQty strNum [str 50] [str2 20])
		(if
			...
			(if (u> (MemoryInfo 0) 1536) ; LargestPtr
				((ScriptID 952) doit: @global63 addLog) ; sysLogger
			else
				(Print 0 20) ; "Not Enough Memory!!"
			)
			(event claimed: 1)
		)
		(switch (event type:)
			...
			(evSAID
				(cond
					...
					(
						...
						(HighPrint 0 24) ; "It's not as effctive as Raseirian Express."
					)
					...
					((or (Said 'give/bouquet') (Said 'give//bouquet'))
						(HighPrint 0 75) ; "I don't know who you want to give the flowers to."
					)
					((Said 'give,kiss')
