	...
	(method (doit)
		...
		(cond
			...
			(
				...
				(= caughtOnce 1)
				(localproc_0 400 7) ; "You there! Halt!!"
...
(instance rightDr of TargActor
	...
	(procedure (localproc_7) ; UNUSED
		(Print 400 37) ; "There is a strong sense of Magic beyond the Palace walls."
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(djinniPrint 400 38) ; "You can't stop now, Iblis is starting to awaken!"
			)
			(1
				(= backToRoom 0)
				(self dispose:)
		...
		(switch (= state newState)
			(0
				(if (and g1Down g2Down)
					...
					(HighPrint 400 39 103 25 3) ; "You move deeper into the shadows to avoid the guards."
					(cond
						((> (gEgo x:) 219)
...
(instance guardAttack of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if (gCast contains: eunuch)
					(localproc_0 400 40) ; "Now! Guards, get that intruder!!"
				)
				(gEgo setMotion: 0)
...
(instance guardsRushIn of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HighPrint 400 41) ; "A swarm of five guards come storming out of the palace after you. You fight valiantly and desperately against overwhelming odds."
				(= cycles 2)
			)
			(4
				(Print 400 42) ; "Suddenly, the Djinni cries "No! Iblis is loose!" You and the guards turn as you hear a rumbling and crashing from inside the Palace."
				(= cycles 2)
			)
			(5
				(DontMove 0)
...
(instance badLevitate of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HighPrint 400 43 80 {Guard:}) ; "Hey, this guy knows Magic."
				(= cycles 2)
			)
			(1
				(if (> (eunuch x:) 316)
			...
			(2
				...
				(localproc_0 400 44 25 5) ; "Guards, To me!"
				(= seconds 1)
			)
			(3
				(gEgo setScript: guardsRushIn)
...
(instance holler of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(localproc_0 400 45) ; "What is going on? Guards! The gate!!"
				(= cycles 2)
			)
			(2
				(self changeState: 4)
		...
		(switch (= state newState)
			(0
				(if (not (gCast contains: rGuard2))
					...
					(HighPrint 400 46 80 {Other Guard:} 103 25 4) ; "Go get him, Elija-One! I'll watch you make short work of this hero."
					(= gRasGuardLeft 1)
				)
				(if (gCast contains: rGuard2)
...
(instance letsFight of HandsOffScript
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 400 47 80 {Guard:}) ; "Come out and fight me like a man!"
				(= cycles 6)
			)
			(2
				(HighPrint 400 48) ; "You have no choice but to fight."
				(gEgo setMotion: MoveTo 145 159 self)
			)
			(3
				(Face gEgo client 1)
