...
(procedure (PromptForDiskChange saveDisk &tmp ret [saveDevice 40] [curDevice 40] str) ; UNUSED
	...
	(if
		...
		(Format str 994 1 (if saveDisk {SAVE GAME} else {GAME}) @saveDevice) ; "Please insert your %s disk in drive %s."
...
(instance fastCast of EventHandler
	(properties)
)
...
(class Game of Obj
	...
	(method (doit &tmp temp0)
		(if (gFastCast isEmpty:)
			...
			(while (not (gFastCast isEmpty:))
				(gFastCast eachElementDo: #doit)
				(if ((= temp0 (Event new:)) type:)
	...
	(method (save &tmp [temp0 20] temp20 [temp21 2])
		(SaveGame name temp20 @temp0 gVersion) ; UNINIT
	)

	(method (restore param1 &tmp [temp0 23])
