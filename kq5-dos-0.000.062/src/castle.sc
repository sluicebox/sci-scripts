			...
			(switch (event message:)
				...
				(5 ; Inventory
					(switch global69
						...
						(else
							(PrintDC 550 6) ; "THAT won't help Graham against this huge beast!"
							(event claimed: 1)
						)
					)
				)
				(4 ; Talk
					(PrintDC 550 7) ; "This is no time for conversation!"
					(event claimed: 1)
				)
			)
		)
...
(instance theWizard of Act
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(3 ; Do
					(PrintDC 550 8) ; "Now would not be a good time to bother Mordack!"
					(event claimed: 1)
				)
				(4 ; Talk
					(switch global331
						(5
							(PrintDC 550 9) ; "Graham wisely decides to keep his mouth shut."
						)
					)
					(event claimed: 1)
				)
				(2 ; Look
					(switch global331
						(5
							(if (== gCurRoomNum 64)
								(PrintDC 550 10) ; "Through the open doorway, Graham can see the wizard lying in his bed."
							else
								(PrintDC 550 11) ; "Mordack lies asleep on his huge, horrendous bed."
							)
						)
					)
					(event claimed: 1)
		...
		(if
			...
			(switch (event message:)
				(2 ; Look
					(switch global332
						(2
							(PrintDC 550 12) ; "The cat is interested in your fish."
						)
						(3
							(PrintDC 550 13) ; "The cat is greedily eating the fish."
						)
						(4
							(PrintDC 550 14) ; "The cat seems content after eating the fish."
						)
						(6
							(PrintDC 550 15) ; "He seems in a hurry to leave."
						)
						(7
							(PrintDC 550 16) ; "The cat struggles in vain inside the bag."
						)
						(else
							(PrintDC 550 17) ; "The black cat eyes Graham suspiciously."
						)
					)
					(event claimed: 1)
				)
				(3 ; Do
					(if (== global332 7)
						(PrintDC 550 18) ; "It wouldn't be wise to carry around the cat."
					else
						(PrintDC 550 19) ; "Those claws look awfully sharp."
					)
					(event claimed: 1)
				)
				(5 ; Inventory
					(switch global69
						...
						(24 ; Bag_of_Peas
							(cond
								((not (IsFlag 64))
									(PrintDC 550 20) ; "It won't sway the cat. Graham's in trouble!"
								)
								((not (IsFlag 63))
						...
						(else
							(PrintDC 550 21) ; "Don't bother the cat with THAT!"
						)
					)
					(event claimed: 1)
				)
		...
		(switch (= state newState)
			(0
				...
				(if (== global332 4)
					...
					(= global332 5)
					(Say 549 550 23 67 10 10 25 6) ; "You! How did you get here?"
					(Say 160 550 24 67 30 10 25 6) ; "I have journeyed far; over land and sea..."
					(Say 549 550 25 67 10 10 25 6) ; "Never mind! Your journey is now over!"
				)
				(= cycles 1)
			)
			(1
...
(instance theWizardScript of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(2
				(Say 73 550 26 67 30 10 25 6) ; "Good-bye, KING Graham of Daventry...heh, heh, heh!"
				(= cycles 1)
			)
			(3
