...
(local
	local0
	[local1 14] = [172 189 102 170 148 150 141 144 150 134 0 29 0 189]
...
(instance rm052 of Rm
	...
	(method (doit &tmp temp0 temp1)
		...
		(cond
			...
			((and (not (IsFlag 98)) (not (IsFlag 56)))
				(Say 75 52 0 67 10 10 25 5) ; "I don't like this place. It's creepy."
				(SetFlag 98)
			)
	...
	(method (handleEvent event)
		(cond
			...
			(
				...
				(switch global69
					(21 ; Crystal
						(if (and (gEgo has: 21) (not (IsFlag 36))) ; Crystal
							(event claimed: 1)
					...
					(else
						(PrintDC 52 1) ; "It won't do much good here."
						(event claimed: 1)
					)
				)
			)
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(if (IsFlag 36)
						(PrintDC 52 2) ; "Two monstrous statues with burned out eyes face each other across the narrow trail leading to Mordack's castle."
					else
						(PrintDC 52 3) ; "Two monstrous statues of grotesque, distorted serpents face each other across the narrow trail leading to Mordack's castle."
					)
					(event claimed: 1)
				)
				(3 ; Do
					(PrintDC 52 4) ; "The giant figures can't be climbed without risking life and limb, and since they appear to be made of stone, can't be harmed."
					(event claimed: 1)
				)
				(4 ; Talk
					(PrintDC 52 5) ; "The stone statues don't respond."
					(event claimed: 1)
				)
				(5 ; Inventory
					(switch global69
						...
						(else
							(if (not (IsFlag 36))
								(PrintDC 52 6) ; "That won't help with the stone serpents."
								(event claimed: 1)
							)
						)
					)
...
(instance path52 of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 52 7) ; "The narrow, rocky trail leads straight to the front gate of Mordack's castle."
					(event claimed: 1)
				)
			)
		)
...
(instance theCastle of RFeature
	...
	(method (handleEvent event)
		(if
			...
			(switch (event message:)
				(2 ; Look
					(PrintDC 52 8) ; "A bizarre castle, seeming to rise up out of the very rocks of the strange island like some sort of grotesque growth, beckons, yet repels, at the same time."
					(event claimed: 1)
				)
				(3 ; Do
