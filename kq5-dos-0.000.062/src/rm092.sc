...
(instance rm092 of Rm
	...
	(method (init)
		...
		(gEgo normal: 0 view: 856 loop: 13 cel: 0 init:)
		((gEgo head:) hide:)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				...
				(iceQueen loop: 7 cel: 1 posn: 248 80)
				(smallHead loop: 10 x: (iceQueen x:) y: (- (iceQueen y:) 35))
				(qArm cycleSpeed: 8 setCycle: RandCycle init:)
				(wolf1 hide:)
				(wolf2 hide:)
				(PrintDC 92 2 #at 120 100 #dispose) ; "...have so thoughtlessly invaded my territory without my permission or knowledge, I have decided you shall both be put to death! Take him away, my pets!"
				(= seconds 13)
			)
			(7
				(= local8 0)
			...
			(19
				...
				(gEgo hide:)
				(iceQueen hide:)
				(mouth cycleSpeed: 4 setCycle: RandCycle init:)
				(eyes setScript: moveScript init:)
	...
	(method (handleEvent event)
		(if
			(or
				...
				(not (proc0_18 gEgo event))
			)
			(return)
		else
			(switch (event message:)
				(5 ; Inventory
					(if (proc0_18 gEgo event)
						(switch global69
							(10 ; Harp
								(SetScore 4)
