...
(local
	...
	[local56 12] = [2 1 3 0 1 2 2 0 2 0 0 0]
...
(instance rm180 of Rm
	...
	(method (init &tmp temp0)
		...
		(cond
			...
			((IsFlag 1)
				(gCurRoom setScript: elevatorOpenCloseScript)
			)
		)
	)
...
(instance books of Feature
	(properties
		...
		description {books}
		onMeCheck 1024
		lookStr {Here are some of Dr. Brain's most timely reference works. There's a copy of Stephen Hawking's "A Brief History of Time", Robert Heinlein's "Time Enough For Love", Martin Heidegger's "Being and Time", and Madeleine L'Engle's "A Wrinkle in Time".}
	)
...
(instance alarmDevice of Feature
	(properties
		...
		onMeCheck 4096
		lookStr {It's an Acme Intruder Alert Alarm.}
	)
...
(instance cuckooClock of Feature
	(properties
		...
		onMeCheck 4
		lookStr {You've never seen a water-powered cuckoo clock before. This must be another of Dr. Brain's incredible inventions!}
	)
...
(instance clockRadio of Feature
	(properties
		...
		onMeCheck 16384
		lookStr {This specially-designed clock radio only plays rap music. Dr. Brain hates rap music, but he got a really good deal on the radio. Besides, an Elf will dance to almost anything.}
	)
...
(instance sundial of Feature
	(properties
		...
		sightAngle 180
		onMeCheck 512
		lookStr {Dr. Brain wanted to make sure he could still tell time if the power went out, so he installed this sundial. Of course, he forgot about the minor problem of having two floors of the castle between this room and the sun. That triangular thing in the center is called a "gnomon".}
	)
...
(instance hourGlasses of Feature
	(properties
		...
		onMeCheck 8192
		lookStr {This desk features a state-of-the-art Acme 40-second time lock. Of course, Acme is in a very small state (it makes Rhode Island look like a continent). A pair of hourglasses has been provided to help open the lock.}
	)
...
(instance canElf of Prop
	(properties
		...
		lookStr {It's one of the famous Dancing Elves. Turn on the radio, and the Elf will dance for you.}
		view 180
		loop 3
		cycleSpeed 18
	)
...
(instance cuckoo of Prop
	(properties
		...
		lookStr {It's a cuckoo, Dr. Brain's favorite bird.}
		view 185
		loop 5
		cycleSpeed 1
	)
...
(instance drawer of Prop
	...
	(method (doVerb theVerb)
		(cond
			...
			((not (gEgo has: 7))
				(proc5_9 180 21) ; "The time cards may come in useful"
			)
			(else
				(proc5_9 180 22) ; "There is nothing more for you to do with the drawer."
			)
		)
	)
)
...
(instance timeCards of Prop
	(properties
		...
		lookStr {There is a set of three time cards in the drawer.}
		view 196
		loop 1
		priority 15
		signal 16400
...
(instance clock of Prop
	(properties
		...
		onMeCheck 2
		lookStr {Visitors to the castle need to punch in on this time clock. Dr. Brain is very big on punctuality, and expects you to punch in at exactly the right time.}
		view 180
		loop 7
	)
		...
		(if
			...
			(cond
				...
				((and (not (gEgo has: 7)) (not (IsFlag 1)))
					(proc5_9 180 23) ; "You'll have to get a time card before you can punch in. There are a few in the desk drawer."
				)
				((IsFlag 1)
					(proc5_9 180 24) ; "You've already punched in. Dr. Brain might get suspicious if you punch in again during the same visit."
				)
				((not (IsFlag 58))
					(proc5_9 180 25) ; "The time clock won't open until things quiet down in here."
				)
			)
		else
...
(instance wallClock of Prop
	(properties
		...
		lookStr {Another of Dr. Brain's famous garage sale artifacts, this is a clock that runs backwards... sometimes. It doesn't keep very good time even when it's running (but might make a pretty good fan if you attached some fan blades).}
		view 180
		loop 8
		cycleSpeed 0
	)
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(proc5_9 180 26) ; "No thanks, the clock already has a perfectly good pair of hands!"
		else
			(super doVerb: theVerb &rest)
		)
	)
			...
			(cond
				...
				((== value 3)
					...
					(if (not local118)
						...
						(proc5_9 180 27) ; "WARNING! SECURITY ALERT! ATTEMPT TO OPEN LOCK AT WRONG TIME!"
					else
...
(instance hourGlass of PuzzleBar
	...
	(method (show &tmp temp0 temp1)
		...
		(if local118
			(self solvePuzzle:)
			(proc5_9 180 28) ; "As the last sands fall into place, and you press the "Open" button, the drawer opens to reveal three time cards."
		)
		(return local118)
	)
	...
	(method (buyClue)
		(if (super buyClue:)
			(if global114
				(proc5_9 180 29) ; "Both hourglasses start running when you press "Start". The trick is to measure 40 seconds with a 35- and a 15-second hourglass. Figure out how to end up with just five seconds of sand in the 15-second glass when the 35-second glass runs out."
				(proc5_9 180 30) ; "(Remember, you can flip EITHER glass once one runs out, so the five seconds worth of sand can be in either the top or the bottom of the 15-second glass when the 35-second glass expires.)"
			else
				(proc5_9 180 31) ; "Both hourglasses start running when you press "Start". The trick is to measure 40 seconds with a 25- and a 15-second hourglass. Since 25+15 equals 40, wait until the 25-second hourglass runs out, then turn over the 15-second hourglass. When it runs out, press the "Open" button."
			)
			(Animate (gCast elements:) 1)
		)
	)
	...
	(method (showHelp)
		(proc5_9 180 32) ; "This is one of Acme's famous "time locks". To open it, you need to press the "Open" button exactly 40 seconds after pressing "Start". You can flip either hourglass immediately after either runs out."
		(proc5_14 180 33 (if global114 35 else 25)) ; "Of course, the lock is really controlled by the hourglasses, not by time, so you have a few seconds to make each flip. But you will have to flip the hourglasses in the right order to measure out 40 seconds to open the drawer. The large hourglass will run for %d seconds when flipped, and the small one for 15 seconds. Spend a Hint Coin to get some tips."
		(Animate (gCast elements:) 1)
	)
...
(instance elevatorOpenCloseScript of HandsOffScript
	(properties)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				(Wait 0)
				(Wait 30)
				(elevatorDoor setCycle: End)
				(= cycles 30)
			)
			(3
				(gCMusic2 number: 938 setLoop: 1 play:)
...
(instance elevatorDoor of Prop
	(properties
		...
		lookStr {This looks like an elevator door.}
		view 180
		loop 5
	)
	...
	(method (doVerb theVerb)
		(if (== theVerb 3)
			(if (IsFlag 1)
				...
				(proc5_9 180 34) ; "A special security feature prevents the elevator doors from being opened until you've punched in."
			)
		else
			(super doVerb: theVerb &rest)
		)
