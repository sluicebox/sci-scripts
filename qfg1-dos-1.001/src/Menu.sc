...
(procedure (proc997_1 param1)
	(if (not (IsFlag 345))
		(if param1
			(SetMenu 776 113 1 110 {Normal Speed Hero})
			(if (not global201)
				(gEgo setStep: (* (gEgo xStep:) 2) (* (gEgo yStep:) 2))
				(= global201 1)
			)
		else
			(SetMenu 776 113 0 110 {High Speed Hero})
			(if global201
				(gEgo
					setStep:
						(/ (+ (gEgo xStep:) 1) 2)
...
(class MenuBar of MenuBar
	...
	(method (init)
		...
		(AddMenu
			{ Game }
			{Faster Animation`+:Normal Animation`=:Slower Animation`-:--!:Sound Volume...`^v:Turn Off Sound`#2=1:--!:High Speed Hero`#4=0}
		)
		(AddMenu
			{ Action }
	...
	(method (handleEvent event &tmp [temp0 3] temp3 [temp4 300] temp304)
		(switch (super handleEvent: event)
			(257
				(aWin back: (if (< global211 16) 15 else 11))
				(Print
