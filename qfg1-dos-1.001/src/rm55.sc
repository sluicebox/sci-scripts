				...
				(cond
					...
					((Said 'talk,ask>')
						...
						(cond
							...
							((or (Said '/potion<healing') (Said '/healing'))
						...
						(cond
								...
								(if (gEgo has: 30) ; magic mushroom
									(cond
										...
										(else
											(= local5 10)
											(healer setScript: healerPleased)
											(TimePrint 8 55 69) ; ""These are very nice. I'll dry them and grind them into a powder. Let me think...oh yes! I said these are worth a gold. Here you are." You put it away."
											(Purchase -10)
											(++ global262)
							...
							((Said '/claw,cheetaur')
								(if (= local1 (gEgo has: 31)) ; cheetaur claw
									(= local5 10)
									(healer setScript: healerPleased)
									(TimePrint 8 55 70) ; "Do you mean to tell me that you actually managed to kill a Cheetaur? You're quite a hero, aren't you? Here is your money. You might think about purchasing a Healing Potion in case you have to fight such monsters again."
							...
							((Said '/beard,troll')
								(if (gEgo has: 32) ; troll beard
									(= local5 10)
									(healer setScript: healerPleased)
									(TimePrint 8 55 71) ; ""Don't tell me you actually killed a Troll? Why, you are really amazing. I haven't had any Troll's Beard for a long time. Here are two Healing Potions for the beard." You put them away."
							...
							((Said '/dust[<faerie]')
								(if (gEgo has: 28) ; faerie dust
									...
									(if
										...
										(HighPrint 55 72) ; "So, you say this is one of the ingredients for the Dispel Potion. Well, I had better get started making it."
									else
										(HighPrint 55 73) ; "Thank you. I'm sure that I'll find a good use for Fairy Dust."
									)
							...
							((Said '/water[<floating]')
								(cond
									...
									(global263
										(HighPrint 55 74) ; "I don't need any more Flying Water. Thank you anyway."
									)
									((not (IsFlag 334))
										(HighPrint 55 75) ; "Thank you for the offer, but I really don't need any water."
									)
									((not (IsFlag 329))
										(HighPrint 55 76) ; "The Healer makes some sort of arcane gesture over the flask of water, then looks surprised."
										(HighPrint 55 77) ; "I'm sorry, but this water does not seem to have any magical potential at all. This can't be the `Flying Water' you described."
									)
									(else
										(if (> [gInvNum 29] 1) ; water
											(HighPrint 55 78) ; "The Healer makes some sort of arcane gesture over each flask of water, then smiles."
										else
											(HighPrint 55 79) ; "The Healer makes some sort of arcane gesture over the flask of water, then smiles."
										)
										(= local5 10)
										(healer setScript: healerPleased)
							...
							((Said '/fur')
								(if (gEgo has: 27) ; green fur
									...
									(healer setScript: healerPleased)
									(if (IsFlag 334)
										(HighPrint 55 81) ; "Those Meeps sound so interesting. I'd like to meet them sometime. I'll get to work on that potion of yours."
									else
										(HighPrint 55 82) ; "Thank you. I'll save this for a later use."
									)
							...
							((Said '/acorn')
								(if (gEgo has: 41) ; acorn
									(= local5 10)
									(healer setScript: healerPleased)
									(HighPrint 55 83) ; "So you helped the Dryad, that's nice. She does keep the forest around here healthy. So that's how to make a Dispel Potion, is it? Thanks for letting me know."
									(gEgo use: 41) ; acorn
									(SetFlag 221)
									(SolvePuzzle 675 5)
									(localproc_1)
							...
							((Said '/bottle')
								(if (gEgo has: 26) ; empty bottle
									(= local5 6)
									(healer setScript: healerPleased)
									(TimePrint 8 55 84) ; "Thank you, I always need flasks."
							...
							((Said '/root,plant[<mandrake]')
								(HighPrint 55 85) ; "I don't have any use for that."
							)
						)
					)
					((Said 'buy,buy,get,get>')
						(cond
							...
							((Said '/bottle')
								(HighPrint 55 87) ; "I need all my flasks for potions. You'll have to get one somewhere else."
							)
							((Said '/root,plant[<mandrake]')
								(HighPrint 55 88) ; "I don't have any."
							)
						)
					)
					((Said 'grab/potion,herb')
						(cond
							((IsFlag 226)
								(HighPrint 55 89) ; "You're getting greedy. She'll notice if you take any more."
							)
							(local2
								(HighPrint 55 90) ; "You carefully grab a couple of Healing Potions and conceal them under your cape."
								(gEgo get: 34 2) ; healing
								(SetFlag 226)
							)
							(else
								(HighPrint 55 91) ; "If you're going to steal, at least wait until the Healer is not looking."
							)
						)
					)
					((or (Said 'kill,fight') (Said 'cast/flame,dart,spell'))
						(HighPrint 55 92) ; "You don't really think that is the correct way to actually win this game, do you?"
					)
					((Said 'throw')
						(HighPrint 55 93) ; "Please, show some respect."
					)
					((Said 'cast>')
						(= temp0 (SaidSpell event))
						(if (CastSpell temp0)
							(if (== temp0 18)
								(HighPrint 55 94) ; "There is an aura of magic throughout this small cabin."
							else
								(event claimed: 0)
							)
						)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onBird event 3)
				(HighPrint 55 95) ; "Small on a prehistoric scale, but still a pterosaur."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onLadder event 3)
				(HighPrint 55 97) ; "Ladder up to the loft where the Healer has her personal quarters. No, you can't go up the ladder."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onTable event 3)
				(HighPrint 55 98) ; "The makings of various potions."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onLitterBox event 3)
				(HighPrint 55 99) ; "The pterosaur's litter box."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onLamp event 3)
				(HighPrint 55 100) ; "Iron holder fit for a lit torch."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onWindow event 3)
				(HighPrint 55 101) ; "The view from the window has been obscured by soot from the fire."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onThings event 3)
				(HighPrint 55 102) ; "This and that."
			)
		)
	)
)
	...
	(method (handleEvent event)
		(cond
			...
			((MouseClaimed onBins event 3)
				(HighPrint 55 104) ; "Petrified icicles and greens."
			)
		)
	)
)
...
(instance egoEnters of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if (not (IsFlag 225))
					(HighPrint 55 105) ; "The fragrance of the herbs mingle with some other rather pungent odors as you step into the Healer's house."
				)
			...
			(2
				(if
					...
					(HighPrint 55 106) ; "Well, what can I do for you this time?"
				)
				(if (not (IsFlag 225))
					(HighPrint 55 107) ; "Lovely day, isn't it? My, you look very healthy for an adventurer. You must be new."
					(HighPrint 55 108) ; "What can I do for you? I can sell you potions if you like. I also buy spell components if you are interested in gathering some for me."
					(HighPrint 55 109) ; "Don't mind me, I always have so much to do around here."
				)
				(if (and (IsFlag 201) (not (IsFlag 334)))
					(HighPrint 55 110) ; "You say to the Healer "I have been to visit the Dryad of the Woods. She gave me a Magic Acorn and told me the formula for a potion to dispel enchantments!""
					(SetFlag 334)
				)
				(if (IsFlag 223)
					(HighPrint 55 111) ; "Here is the Potion of Dispel! Use it by splashing it on the victim of a magic spell. Mind you, this won't work if the spell is caused by a magic item rather than a cast spell."
...
(instance healerKisses of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(HighPrint 55 112) ; "Thank you so much. Oh, I'm so happy to have my ring back!"
			...
			(10
				(HighPrint 55 113) ; "You put the reward away and tell her she is welcome as you leave to avoid being kissed again."
				(= cycles 2)
			)
			(11
				(gEgo
...
(instance waitForHealer of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(HighPrint 55 114) ; "The Dispel Potion will take a while to make. Why don't you come back later?"
				(self dispose:)
			)
		)
	)
