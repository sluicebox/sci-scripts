...
(procedure (localproc_0 param1 param2 &tmp [temp0 400])
	(Format @temp0 &rest)
	(Print @temp0 #at -1 12 #mode 1 #width param2 #dispose #time param1)
)
	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 10])
		(if
			...
			(if (User getInput: event)
				(cond
					((proc802_1 (User inputLineAddr:) {look})
						(event type: evSAID)
						(event claimed: 0)
					...
					(
						(or
							(proc802_1 (User inputLineAddr:) {ask})
							(proc802_1 (User inputLineAddr:) {where})
						)
						(= local5 2)
						(= local7 1)
						(SetFlag 143)
					...
					(else
						(switch local9
							(1
								(if (proc802_1 (User inputLineAddr:) @global401)
		...
		(switch (= state newState)
			...
			(6
				...
				(cond
					...
					(local1
						(localproc_0 4 210 29 21)
					)
					((IsFlag 19)
						(localproc_0 4 140 29 22)
					)
					(else
						(localproc_0 4 110 29 23)
					)
				)
				(= seconds 4)
			)
