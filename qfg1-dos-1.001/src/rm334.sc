...
(instance rm334 of Rm
	...
	(method (handleEvent event)
		...
		(cond
			...
			((and (== local0 1) (Said 'look/thief,thief,man,man'))
			...
			(
				(and
					(== local0 1)
					(or (MouseClaimed onDagger event 3) (Said 'look/dagger'))
				)
				(HighPrint 334 6) ; "The dagger looks sharp and very deadly."
			)
			((or (MouseClaimed coin event 3) (Said 'look/alm,item'))
				(if (<= (gEgo y:) 150)
			...
			((Said 'get,move,lockpick<up,grab>')
				(if (Said '/barrel')
					(HighPrint 334 14) ; "The barrels are too heavy to move."
				)
			)
			((and (not (and (not local0) (<= (gEgo y:) 132))) (Said 'climb'))
...
(instance ambushScript of Script
	...
	(method (handleEvent event &tmp [temp0 30])
		(switch (event type:)
			(evSAID
				(cond
					((and (not local1) (Said 'climb'))
						(TimePrint 3 334 18) ; "That's a pretty tall wall."
						(HandsOff)
						(self changeState: 7)
					)
					((Said 'show,make,give/sign[<thief]')
						(cond
							(local1
							...
							(else
								...
								(if (not (gEgo has: 25)) ; thief certificate
									...
									(HighPrint
										(Format @temp0 334 25 257 global270) ; "The password is '%s'."
									)
								)
								(SetFlag 123)
								(HighPrint 334 21) ; "Now go before I forget you made the sign and I make you pay anyway."
					...
					(
						...
						(cond
							(local1
								(HighPrint 334 26) ; "That's all right; we already have all your money. But it was certainly nice of you to offer."
							)
							((not (Purchase 5))
								(HighPrint 334 27) ; "Hey! You don't have enough money to be worth our while. How do you expect us poor independent businessmen to make a living?"
								(HandsOff)
								(self changeState: 7)
							)
							(else
					...
					((Said 'talk,ask')
						(HighPrint 334 29) ; "Slink: "My finger is starting to get a bit twitchy, so forget about questions, just pay the man and get out alive.""
					)
					(
						(or
							(Said 'fight,kill,attack,capture,cut,hit')
							(Said 'throw/dagger,boulder')
						)
						(HighPrint 334 30) ; "As you start to draw your weapon, the thieves jump you."
						(HandsOff)
						(self changeState: 7)
					)
					((Said 'cast')
						(HighPrint 334 31) ; "Before you can begin to use any magic, the thieves decide that you're up to no good."
						(HandsOff)
						(self changeState: 7)
					)
				)
