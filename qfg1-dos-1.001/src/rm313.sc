...
(instance rm313 of Rm
	...
	(method (handleEvent event)
		(cond
			...
			((or (Said 'lift,get/cover') (Said 'uncover/birdcage,birdcage'))
				(localproc_5)
			)
			...
			((or (MouseClaimed onRug event 3) (Said 'look/carpet'))
			...
			((or (MouseClaimed onDoily event 3) (Said 'look/doily'))
			...
			((or (MouseClaimed cat event 3) (Said 'look/cat'))
				(HighPrint 313 28) ; "The little house cat seems harmless, but strangely restless."
			)
			((or (MouseClaimed onDesk event 3) (Said 'look/desk'))
			...
			((or (MouseClaimed onBag event 3) (Said 'look/bag,basket'))
...
(instance birdieSings of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 313 37 #at -1 15 #title {Little Birdie}) ; "CHIRP! CHIRP!"
				(Print 313 38 #at -1 15 #title {Little Old Lady}) ; "Kitty, you'd better not be bothering the little birdie again!"
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cat loop: 0 cycleSpeed: 1 setMotion: MoveTo 260 142 self)
			)
			(1
		...
		(switch (= state newState)
			...
			(11
				(if local14
					...
					(EgoDead 313 54 80 { What a cute little kitty! } 82 637 1 0) ; "When the Little Old Lady awakes to see what's going on, you have to concede to her (through lips that are as raw as hamburger) that you've been licked! She summons the Sheriff and his goon, Otto."
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 313 59 #at -1 20 #mode 1 #title {Little Old Lady}) ; "Help! Burglars! Sheriff! Help! Kitty! Kitty!"
				(= local14 1)
				(gEgo setScript: climbDown)
			)
		)
...
(instance climbDown of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(if local14
					(Print 313 60 #at -1 15 #title {L.O.L.}) ; "KITTY!!!"
