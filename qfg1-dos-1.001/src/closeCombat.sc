...
(instance closeCombat of Script
	...
	(method (init)
		(LoadMany rsSCRIPT 146 147 148 149 150 151 152 153 154 155)
		(Load rsVIEW 540)
		(if (not (gEgo has: 9)) ; shield
			(Load rsVIEW 535)
		)
		(= global266 1)
		(super init: &rest)
		...
		(switch (event type:)
			(evSAID
				...
				(cond
					...
					((and local3 (Said 'cast'))
						(HighPrint 215 2) ; "You cannot make the arcane gestures to cast spells while carrying a shield."
					)
					((Said 'cast>')
						...
						(if (CastSpell local0)
							...
							(if
								...
								(HighPrint 215 3) ; "This might not be the best time for practicing non-combat spells."
								(event claimed: 1)
							)
						)
					)
					(
						...
						(HighPrint 215 4) ; "Go ahead!"
					)
					((Said 'throw')
						(HighPrint 215 5) ; "You're too close."
					)
					(else
						(HighPrint 215 6) ; "You don't have time for that."
						(event claimed: 1)
					)
				)
			)
